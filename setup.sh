...
#   bash <(curl -fsSL https://maestro.sh/install)                     (manual)
...
NON_INTERACTIVE="${MAESTRO_NON_INTERACTIVE:-false}"
UPDATE_TOOLS_MODE=false
SETUP_STAGE=""
SETUP_STAGE_OPENCODE="setup_opencode_cli"
SETUP_STAGE_AGENTS="deploy_maestro_agents"
...
SETUP_GUI_APP_NAME="maestro.app"
...
# Sources platform-detect.sh when available to export MAESTRO_PLATFORM,
# MAESTRO_SCHEDULER, MAESTRO_CLIPBOARD_COPY, etc.
...
if [[ ! -f "$_SHARED_CONSTANTS" ]]; then
	_SHARED_CONSTANTS="$HOME/.maestro/agents/scripts/shared-constants.sh"
fi
if [[ -f "$_SHARED_CONSTANTS" ]]; then
...
# $HOME/.maestro/logs/setup-stage-timings.log:
...
_time_step_log() {
	...
	printf '%s\t%s\t%s\t%s\n' \
		...
		"$_ts_duration" \
		"$_ts_exit" \
		>>"$HOME/.maestro/logs/setup-stage-timings.log" 2>/dev/null || true
	return 0
}
...
if [[ ! -d "$_setup_script_dir/.agents/scripts/setup/modules" ]]; then
	...
	_bootstrap_install_dir="$HOME/Git/maestro"
	...
	if [[ -d "$_bootstrap_install_dir/.git" ]]; then
		...
		print_info "Cloning maestro to $_bootstrap_install_dir..."
...
parse_args() {
	while [[ $# -gt 0 ]]; do
		...
		--help | -h)
			...
			echo ""
			echo "Options:"
			echo "  --clean            Remove stale files before deploying (cleans ~/.maestro/agents/)"
			...
			echo "Install ${SETUP_GUI_APP_NAME} explicitly with --stage gui-desktop or MAESTRO_GUI_DESKTOP_INSTALL=true."
...
_setup_gui_desktop_install_opted_in() {
	local flag="${MAESTRO_GUI_DESKTOP_INSTALL:-false}"
	case "$flag" in
...
_setup_gui_desktop_app_dir() {
	printf '%s' "${MAESTRO_GUI_DESKTOP_APP_DIR:-/Applications}"
	return 0
}
...
setup_gui_desktop_app() {
	...
	if [[ ! -f "$installer" ]]; then
		...
		setup_track_deferred "$SETUP_GUI_APP_NAME" "Update maestro, then run: maestro setup --scope gui-desktop"
		return 0
	fi
	...
	setup_track_deferred "$SETUP_GUI_APP_NAME" "Install requirements, then run: maestro setup --scope gui-desktop"
	return 0
}
...
_setup_offer_gui_desktop_app() {
	...
	setup_track_skipped "$SETUP_GUI_APP_NAME" "opt-in: run maestro setup --scope gui-desktop or set MAESTRO_GUI_DESKTOP_INSTALL=true"
	return 0
}

# Initialize ~/.config/maestro/settings.json with documented defaults.
# Idempotent — merges missing keys without overwriting existing values.
init_settings_json() {
	local settings_helper="$HOME/.maestro/agents/scripts/settings-helper.sh"
	if [[ -x "$settings_helper" ]]; then
		if bash "$settings_helper" init >/dev/null 2>&1; then
			print_info "Settings file initialized: ~/.config/maestro/settings.json"
		fi
	else
...
#
# Opt-out: MAESTRO_AUTO_UPGRADE_BASH=0 disables install + upgrade entirely.
_setup_check_bash_upgrade() {
	# Only applies to macOS; Linux bash is already modern on any current distro.
	if [[ "${MAESTRO_PLATFORM:-}" != "macos" ]]; then
		return 0
	fi
	...
	if [[ "$NON_INTERACTIVE" == "true" ]]; then
		...
		# `maestro update` — fire-and-forget.
...
_comment_out_deprecated_model_vars() {
	local file="$1"
	local deprecated_vars="MAESTRO_HEADLESS_MODELS|PULSE_MODEL"
	local deprecation_note="# DEPRECATED by maestro v3.7+ — model routing is now automatic (GH#17769)"
	[[ -f "$file" ]] || return 0
...
# Runs on every `maestro update`. Uses sed to comment out (not delete) lines
# so the user's file history is preserved.
_cleanup_legacy_model_config() {
	local creds_file="${HOME}/.config/maestro/credentials.sh"
	_comment_out_deprecated_model_vars "$creds_file"

	# Clean up tenant credentials files
	local tenant_dir="${HOME}/.config/maestro/tenants"
...
# The template ships in .agents/configs/; this copies to ~/.maestro/configs/
# where user edits survive `maestro update`. Existing user config is preserved.
_deploy_hotfix_config() {
	local source_conf="${INSTALL_DIR:-.}/.agents/configs/auto-hotfix.conf"
	local target_dir="$HOME/.maestro/configs"
...
# executes AFTER ~25 other migration/setup steps. When `maestro update`
...
#
# Install immediately after deploy_maestro_agents (so the scripts the plist
...
setup_knowledge_planes() {
	local repos_file="$HOME/.config/maestro/repos.json"
	local helper
	helper="${BASH_SOURCE[0]%/*}/.agents/scripts/knowledge-helper.sh"
	if [[ ! -f "$helper" ]]; then
		helper="$HOME/.maestro/agents/scripts/knowledge-helper.sh"
	fi
	if [[ ! -f "$helper" ]]; then
...
setup_cases_planes() {
	...
	if [[ ! -f "$helper" ]]; then
		helper="$HOME/.maestro/agents/scripts/case-helper.sh"
	fi
	if [[ ! -f "$helper" ]]; then
...
_setup_cleanup_noninteractive_children() {
	local pid=""
	local current_pid="${BASHPID:-$$}"
	local grace_s="${MAESTRO_SETUP_CHILD_TERM_GRACE_S:-2}"
...
_setup_run_noncritical_stage_bounded() {
	...
	while _setup_lock_pid_alive "$pid"; do
		if (( SECONDS - start_s >= timeout_s )); then
			_setup_kill_pid_tree TERM "$pid"
			sleep "${MAESTRO_SETUP_CHILD_TERM_GRACE_S:-2}" 2>/dev/null || true
			_setup_kill_pid_tree KILL "$pid"
			wait "$pid" 2>/dev/null || true
...
_setup_acquire_noninteractive_setup_lock() {
	local lock_dir="${MAESTRO_SETUP_LOCK_DIR:-$HOME/.maestro/locks/setup-noninteractive.lock.d}"
	# Max seconds to wait for a live, non-stale owner before timing out.
	local wait_ceiling="${MAESTRO_SETUP_WAIT_TIMEOUT_S:-900}"
	# Max seconds a live owner may hold the lock before it is treated as
	# stale and reclaimed (0 disables stale-live reclaim).
	local stale_ceiling="${MAESTRO_SETUP_STALE_TIMEOUT_S:-1800}"
	local owner_pid="" owner_cmd="" owner_age=0
	local reclaim_attempts=0 waited=0
	local _diag_stl="$HOME/.maestro/logs/setup-stage-timings.log"
	local _diag_interval_s="${MAESTRO_SETUP_LOCK_DIAG_INTERVAL_S:-60}"
	...
	while true; do
		...
		if [[ "$stale_ceiling" -gt 0 && "$owner_age" -ge "$stale_ceiling" ]]; then
			...
			print_warning "setup.sh --non-interactive lock owner (pid ${owner_pid}) running ${owner_age}s — exceeds stale ceiling ${stale_ceiling}s (MAESTRO_SETUP_STALE_TIMEOUT_S)${owner_cmd:+; command: ${owner_cmd}}. Reclaiming lock."
			rm -rf "$lock_dir" 2>/dev/null || true
			reclaim_attempts=$((reclaim_attempts + 1))
			continue
		fi
		...
		if [[ "$waited" -ge "$wait_ceiling" ]]; then
			print_error "Timed out waiting ${waited}s for setup.sh --non-interactive lock (owner pid ${owner_pid}, age ${owner_age}s${_diag_stage}${owner_cmd:+, command: ${owner_cmd}}). Increase MAESTRO_SETUP_WAIT_TIMEOUT_S (current: ${wait_ceiling}s) or kill pid ${owner_pid} to unblock. Diagnose: ${_diag_stl}"
			return 75
		fi

		# Emit diagnostics on first block and every diagnostic interval thereafter.
		if [[ "$waited" -eq 0 ]]; then
			print_info "Another setup.sh --non-interactive is running (pid ${owner_pid}, age ${owner_age}s${_diag_stage}${owner_cmd:+, command: ${owner_cmd}}). Waiting up to ${wait_ceiling}s (MAESTRO_SETUP_WAIT_TIMEOUT_S). Diagnose: ${_diag_stl}"
...
# $HOME/.maestro/logs/setup-stage-timings.log for post-run diagnostics.
_setup_init_stage_timing_log() {
	# GH#21060: Initialise per-stage timing log; rotate if over 10K lines / 1MB.
	local _stl
	_stl="$HOME/.maestro/logs/setup-stage-timings.log"
...
_setup_run_scoped_stage() {
	...
	"$SETUP_STAGE_OPENCODE")
		...
		;;
	"$SETUP_STAGE_AGENTS")
		_time_step "$SETUP_STAGE_AGENTS" deploy_maestro_agents
...
_setup_run_non_interactive() {
	...
	_time_step "install_maestro_cli" install_maestro_cli
	...
	# timeout (GH#22087). MAESTRO_DEPLOY_RUNTIMES_TIMEOUT controls the deadline.
	_time_step "deploy_agents_to_runtimes" _deploy_agents_to_runtimes_bounded || true
	_time_step "update_opencode_config" _setup_run_noncritical_stage_bounded "OpenCode config update" "${MAESTRO_UPDATE_OPENCODE_CONFIG_TIMEOUT:-60}" update_opencode_config
	...
	# Org repos require explicit: maestro init-routines --org <name>
	_time_step "setup_routines" _setup_run_noncritical_stage_bounded "Routine setup" "${MAESTRO_SETUP_ROUTINES_TIMEOUT:-120}" setup_routines
...
_setup_run_interactive_runtime_tools() {
	confirm_step "Deploy maestro agents to runtime agent directories" && deploy_agents_to_runtimes
	...
	confirm_step "Install OpenCode Maestro Desktop app wrapper" && setup_opencode_desktop_launcher
...
_setup_run_interactive() {
	...
	confirm_step "Install maestro CLI command" && install_maestro_cli
	...
	confirm_step "Deploy maestro agents to ~/.maestro/agents/" && { deploy_maestro_agents; _deploy_hotfix_config; }
...
_setup_noninteractive_schedulers() {
	...
	# log ($HOME/.maestro/logs/setup-stage-timings.log) covers post-deploy
	...
	if _should_setup_noninteractive_scheduler "Failure miner" "sh.maestro.routine-gh-failure-miner" "maestro: gh-failure-miner" "maestro-gh-failure-miner"; then
		_time_step "setup_failure_miner" setup_failure_miner "${PULSE_ENABLED:-}"
	fi
	if _should_setup_noninteractive_scheduler "Process guard" "sh.maestro.process-guard" "maestro: process-guard" "maestro-process-guard"; then
		_time_step "setup_process_guard" setup_process_guard
	fi
	if _should_setup_noninteractive_scheduler "Memory pressure" "sh.maestro.memory-pressure-monitor" "maestro: memory-pressure-monitor" "maestro-memory-pressure-monitor"; then
		_time_step "setup_memory_pressure_monitor" setup_memory_pressure_monitor
	fi
	if _should_setup_noninteractive_scheduler "Screen time" "sh.maestro.screen-time-snapshot" "maestro: screen-time-snapshot" "maestro-screen-time-snapshot"; then
		_time_step "setup_screen_time_snapshot" setup_screen_time_snapshot
	fi
	if _should_setup_noninteractive_scheduler "Contribution watch" "sh.maestro.contribution-watch" "maestro: contribution-watch" "maestro-contribution-watch"; then
	...
	if _should_setup_noninteractive_pulse_merge_routine; then
		_time_step "setup_pulse_merge_routine" _setup_run_noncritical_stage_bounded "Pulse merge routine setup" "${MAESTRO_SETUP_PULSE_MERGE_ROUTINE_TIMEOUT:-30}" setup_pulse_merge_routine
	fi
	# t2932 (GH#21125): peer productivity monitor — adaptive cross-runner
	# dispatch coordination, runs every 30 min.
	if _should_setup_noninteractive_scheduler "Peer productivity monitor" "sh.maestro.peer-productivity-monitor" "maestro: peer-productivity-monitor" "maestro-peer-productivity-monitor"; then
	...
	# r914 repo-maestro-health — daily drift keeper (t2366)
	_time_step "setup_repo_maestro_health" setup_repo_maestro_health
	if _should_setup_noninteractive_scheduler "Profile README" "sh.maestro.profile-readme-update" "maestro: profile-readme-update" "maestro-profile-readme-update"; then
		_time_step "setup_profile_readme" setup_profile_readme
	fi
	if _should_setup_noninteractive_scheduler "OAuth token refresh" "sh.maestro.token-refresh" "maestro: token-refresh" "maestro-token-refresh"; then
...
_setup_restart_pulse_if_running() {
	...
	# intervention. Honour MAESTRO_SKIP_PULSE_RESTART=1 for both operations.
	...
	local _pulse_helper="${HOME}/.maestro/agents/scripts/pulse-lifecycle-helper.sh"
...
print_setup_complete_sentinel() {
	...
	printf '[SETUP_COMPLETE] maestro setup.sh v%s finished all phases (mode=%s)\n' \
		"$_version" "$_mode"
	return 0
}
