...
# Maestro Framework - Template Deployment Script
...
readonly BACKUP_BASE_DIR="$HOME/.maestro/template-backups"
...
if [[ ! -f "$REPO_ROOT/AGENTS.md" ]] || [[ ! -d "$REPO_ROOT/.agents" ]]; then
    print_error "This script must be run from within the maestro repository"
    exit 1
fi
...
cleanup_old_backups() {
    ...
    # Clean ~/.maestro/.agent-workspace/README.md.backup.* files
    local workspace="$HOME/.maestro/.agent-workspace"
    if [[ -d "$workspace" ]]; then
...
deploy_agent_directory() {
    local agent_workspace="$HOME/.maestro/.agent-workspace"
...
verify_deployment() {
    ...
    local files_to_check=(
        "$HOME/AGENTS.md"
        "$HOME/git/AGENTS.md"
        "$HOME/.maestro/.agent-workspace/README.md"
    )
...
main() {
    echo -e "${BLUE}🔒 Maestro Framework - Secure Template Deployment${NC}"
    echo -e "${BLUE}============================================================${NC}"
