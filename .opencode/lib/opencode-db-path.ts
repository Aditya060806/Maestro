...
type DbPathEnv = {
  OPENCODE_DB?: string
  MAESTRO_VAULT_MANAGED_SESSION_HISTORY?: string
  MAESTRO_VAULT_MANAGED_HISTORY_ROOT?: string
  MAESTRO_VAULT_STATUS?: string
  MAESTRO_VAULT_STATUS_OVERRIDE?: string
...
export function getDbPath(env: DbPathEnv = process.env): string {
  const managedHistory = env.MAESTRO_VAULT_MANAGED_SESSION_HISTORY
  if (managedHistory === "1" || managedHistory === "true") {
    const status = env.MAESTRO_VAULT_STATUS_OVERRIDE || env.MAESTRO_VAULT_STATUS
    if (status !== "unlocked") {
      throw new Error("VAULT_LOCKED: OpenCode managed session/history requires an unlocked Vault")
    }
    const root = env.MAESTRO_VAULT_MANAGED_HISTORY_ROOT || join(homedir(), ".maestro", ".agent-workspace", "vault", "managed-session-history")
