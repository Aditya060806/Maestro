...
import { withMaestroTitleSuffix } from "../lib/session-title-suffix"
...
function renameSession(sessionID: string, title: string): { success: boolean; message: string } {
  ...
  const displayTitle = withMaestroTitleSuffix(title)
...
function syncSessionWithBranch(
  ...
  try {
    ...
    try {
      ...
      const displayTitle = withMaestroTitleSuffix(branch)
      const nowMs = Date.now()
      const result = db.run(
        "UPDATE session SET title = ?, time_updated = ? WHERE id = ?",
        [displayTitle, nowMs, sessionID],
