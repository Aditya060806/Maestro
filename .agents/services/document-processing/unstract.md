...
- **Credentials**: `UNSTRACT_API_KEY` + `API_BASE_URL` in `~/.config/maestro/credentials.sh` (chmod 600)
...
```bash
~/.maestro/agents/scripts/unstract-helper.sh install
# Or: ~/.maestro/agents/scripts/setup-mcp-integrations.sh unstract
```

Clones to `~/.maestro/unstract/`, disables analytics, starts Docker Compose. Visit http://frontend.unstract.localhost (login: unstract/unstract).
...
- `.agents/maestro/mcp-integrations.md` — all MCP integrations
