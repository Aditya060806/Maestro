...
- **Credentials**: Vaultwarden for card details; `maestro secret` for API keys
...
5. Store API key: `maestro secret set STRIPE_ISSUING_SECRET_KEY`

**Revolut Business:**

1. Open account at https://business.revolut.com
2. Enable API access, generate certificate and token
3. Store credentials: `maestro secret set REVOLUT_API_TOKEN`
...
```text
Vaultwarden Organization: "maestro-missions"
...
- **API keys**: Via `maestro secret set` (gopass or credentials.sh)
...
```json
{
  ...
  "vaultwarden_org": "maestro-missions"
}
```
