...
- **Credentials**: `~/.config/maestro/credentials.sh` → `SOCKET_YOURNAME`
...
```bash
echo 'export SOCKET_YOURNAME="sktsec_..."' >> ~/.config/maestro/credentials.sh
...
- Verify the token is set: `source ~/.config/maestro/credentials.sh && echo $SOCKET_YOURNAME`
