...
- **Credentials**: `~/.config/maestro/credentials.sh` → `SENTRY_YOURNAME`
...
```bash
echo 'export SENTRY_YOURNAME="sntryu_..."' >> ~/.config/maestro/credentials.sh
...
```bash
source ~/.config/maestro/credentials.sh
...
- **`Not authenticated`**: verify (`source ~/.config/maestro/credentials.sh && printenv | grep '^SENTRY_YOURNAME$'`), test API (`curl -H "Authorization: Bearer $SENTRY_YOURNAME" https://sentry.io/api/0/`), restart runtime.
