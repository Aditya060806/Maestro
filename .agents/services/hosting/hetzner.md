...
- **Auth**: Bearer token per project (stored in `~/.config/maestro/credentials.sh`)
...
Each Hetzner Cloud project gets its own API token. Store in `~/.config/maestro/credentials.sh`:
...
"hetzner-myproject": {
  "type": "local",
  "command": ["/bin/bash", "-c",
    "source ~/.config/maestro/credentials.sh && HCLOUD_TOKEN=$HCLOUD_TOKEN_MYPROJECT /Users/you/.local/bin/mcp-hetzner"],
  "enabled": true
}
```
