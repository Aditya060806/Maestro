...
```bash
~/.maestro/agents/scripts/email-agent-helper.sh send --mission "$MISSION_ID" --template "$TEMPLATE"
~/.maestro/agents/scripts/email-health-check-helper.sh precheck "$DOMAIN" "$HTML_FILE"
~/.maestro/agents/scripts/email-delivery-test-helper.sh report "$DOMAIN"
```
