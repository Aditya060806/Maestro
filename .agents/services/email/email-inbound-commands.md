...
- **Purpose**: Allowlisted senders can create maestro tasks or request bounded status/help replies without using the terminal.
...
- **Config**: `~/.config/maestro/email-inbound-commands.conf`
- **Dedup state**: `~/.maestro/.agent-workspace/email-inbound-commands/processed-message-ids.txt`
...
Create `~/.config/maestro/email-inbound-commands.conf`:
...
| Error | Fix |
|-------|-----|
| `Config not found` | Create `~/.config/maestro/email-inbound-commands.conf` with allowlist entries. |
