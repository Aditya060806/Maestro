...
- **Credentials**: `maestro secret set GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE` (gopass) or `credentials.sh`
...
gws auth export > credentials.json.enc && maestro secret set GWS_EXPORT_PASSWORD
# On headless machine:
export GWS_EXPORT_PASSWORD="$(maestro secret get GWS_EXPORT_PASSWORD)"
...
maestro secret get GWS_CREDENTIALS_JSON > "$GWS_CREDS_FILE" && chmod 600 "$GWS_CREDS_FILE"
...
gws gmail users labels create --params '{"userId":"me"}' \
  --json '{"name":"maestro/processed","labelListVisibility":"labelShow","messageListVisibility":"show"}'
...
gws people connections list \
  ...
  --page-all > ~/.maestro/.agent-workspace/work/contacts/google-contacts.ndjson
```
...
- **Credentials**: never commit `~/.config/gws/credentials.json`; store via `maestro secret set GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE`; use encrypted export (AES-256-GCM) for headless/CI
