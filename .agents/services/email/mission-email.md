...
- **Database**: `~/.maestro/.agent-workspace/mail/mission-email.db` (SQLite)
...
Location: `~/.maestro/agents/templates/email/*.txt`. Placeholders: `{{KEY}}`, replaced by `--var KEY=value`.
...
```bash
code=$(sqlite3 ~/.maestro/.agent-workspace/mail/mission-email.db \
...
maestro secret set VENDOR_API_KEY  # user enters the value

sqlite3 ~/.maestro/.agent-workspace/mail/mission-email.db \
