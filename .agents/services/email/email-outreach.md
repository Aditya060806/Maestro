...
4. **Run platform helper**:
    ...
    ```bash
    ~/.maestro/agents/scripts/<platform>-helper.sh create-campaign "$CAMPAIGN_NAME"
    ~/.maestro/agents/scripts/<platform>-helper.sh import-leads "$LEADS_FILE"
    ~/.maestro/agents/scripts/<platform>-helper.sh set-limits "$CAMPAIGN_ID" "$DAILY_LIMIT"
    ```
