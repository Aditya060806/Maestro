...
- **Token cache**: `~/.maestro/.agent-workspace/microsoft-graph/token-cache.json` (0600)
...
1. **New registration** — Name: `maestro-graph-adapter`, Account type: `Single tenant`
...
- **Credentials**: stored via `maestro secret set` (gopass) or `credentials.sh` (0600). Never in config files or conversation.
