...
- **Database**: `~/.maestro/.agent-workspace/email-agent/actions.db` (SQLite)
...
```bash
task_id=$(claim-task-id.sh --repo-path ~/Git/maestro --title "Email: <subject>")
...
```bash
claim-task-id.sh --repo-path ~/Git/maestro --title "Legal: <subject>" --priority high --tag legal
```
...
email-triage-helper.sh extract-training --message-id <id> --type domain-knowledge \
  --output ~/.maestro/.agent-workspace/training/newsletters/
...
```json
{
  ...
  "training_output_dir": "~/.maestro/.agent-workspace/training/newsletters/",
