...
- **CLI**: `fips`, `fipsctl`, `fipstop`, `fips-gateway`; maestro wrapper: `.agents/scripts/nostr-vpn-helper.sh`.
...
- **Secrets**: Use `maestro secret set FIPS_NSEC` only for import/recovery; never paste Nostr private keys into chat or commit key files.
...
Do not present FIPS as the default maestro networking layer until upstream protocol stability and security audit status improve.
...
- Run maestro helpers on one node while using compute or services on another.
...
For stronger privacy, combine FIPS with other maestro guidance and tools:
...
4. Generate a persistent identity on each device, or import one from `maestro secret set FIPS_NSEC` during recovery only.
...
Use maestro secret storage for private Nostr key recovery material and OpenCode access tokens:

```bash
maestro secret set FIPS_NSEC
maestro secret set OPENCODE_SERVER_TOKEN
```
...
4. Store service auth tokens with `maestro secret set OPENCODE_SERVER_TOKEN`.
5. Verify with `nostr-vpn-helper.sh diagnostics` and an authenticated application-level request.

Other maestro-adjacent candidates after SSH is proven: private Git remotes, MCP servers, local dashboards, homelab storage, GPU workers, staging apps, and CI/debug workers. Bind each service to loopback or the FIPS interface; do not publish public listeners as a shortcut.
...
If `fipsctl` output format changes, inspect upstream docs before updating maestro parsing; do not guess around private key or ACL semantics.
