...
- **Config**: `~/.config/maestro/msteams-bot.json` (600 permissions)
...
**Security**: No E2E encryption — messages accessible to Microsoft and tenant admins via eDiscovery. For sensitive comms, use SimpleX or Matrix. Store credentials in gopass (`maestro secret set MSTEAMS_*`). Never commit secrets. Bot output captured by M365 compliance systems.
...
Teams Client --> Bot Framework Service (Azure) --> Your Bot Server (Node.js/Express)
                        |                          |
                        v                          v
              Microsoft Graph API          Runner Dispatch (maestro)
```
...
```bash
az ad app create --display-name "maestro-teams-bot" --sign-in-audience "AzureADMyOrg"
az ad app credential reset --id <appId> --years 2
maestro secret set MSTEAMS_APP_ID && maestro secret set MSTEAMS_CLIENT_SECRET && maestro secret set MSTEAMS_TENANT_ID
az bot create --resource-group mygroup --name maestro-teams-bot \
  --app-type SingleTenant --appid <appId> --tenant-id <tenantId>
az bot update --resource-group mygroup --name maestro-teams-bot \
  --endpoint "https://your-server.example.com/api/messages"
az bot msteams create --resource-group mygroup --name maestro-teams-bot
```
...
Sideload: `zip -j maestro-teams-bot.zip manifest.json outline-32x32.png color-192x192.png` → Teams > Apps > Manage your apps > Upload a custom app.
...
class MaestroBot extends TeamsActivityHandler {
...
`~/.config/maestro/msteams-bot.json` (600 permissions). `appId`/`clientSecret` in gopass. Empty arrays = all allowed. `channelMappings`: channel ID → runner name.
...
```bash
az webapp create --resource-group mygroup --plan myplan --name maestro-teams-bot --runtime "NODE:18-lts"
az webapp config appsettings set --resource-group mygroup --name maestro-teams-bot --settings \
  MSTEAMS_APP_ID="@Microsoft.KeyVault(VaultName=...)" MSTEAMS_TENANT_ID="<tenantId>"
az webapp deployment source config-zip --resource-group mygroup --name maestro-teams-bot --src bot.zip
```
...
docker run -d --name msteams-bot --restart unless-stopped -p 3978:3978 \
  --env-file /etc/msteams-bot/env maestro-teams-bot:latest
