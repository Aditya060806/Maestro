---
description: WhatsApp bot via Baileys (TypeScript, unofficial WhatsApp Web API) — QR linking, multi-device, messaging, access control, maestro runner dispatch, Matterbridge bridging
mode: subagent
tools:
  read: true
  write: false
...
async function startBot(): Promise<void> {
  ...
  const sock: WASocket = makeWASocket({ auth: state, logger: pino({ level: "warn" }), printQRInTerminal: true, browser: ["maestro Bot", "Chrome", "1.0.0"] })
