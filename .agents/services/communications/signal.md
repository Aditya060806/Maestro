---
description: Signal bot integration via signal-cli — registration, JSON-RPC daemon, messaging, access control, maestro dispatch, Matterbridge bridging
mode: subagent
tools:
  read: true
  write: false
...
```bash
signal-cli link -n "maestro-bot" | tee >(xargs -L 1 qrencode -t utf8)
...
- `services/communications/matrix-bot.md` — Matrix bot for maestro runner dispatch
