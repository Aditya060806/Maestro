...
Config: `~/.config/maestro/urbit-bot.json`

```json
{ "ship_name": "sampel-palnet", "ship_url": "http://localhost:8080",
  "ship_code_gopass_path": "maestro/urbit-bot/ship-code", "allowed_ships": ["~zod"] }
```

Dispatch: SSE subscription → validate sender → parse command → `urbit-dispatch-helper.sh` → poke response.

```bash
gopass insert maestro/urbit-bot/ship-code     # +code for Eyre API
gopass insert maestro/urbit-bot/master-ticket # Master ticket (CRITICAL — controls Azimuth NFT)
```
