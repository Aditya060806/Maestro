...
- **Purpose**: Bridge Matrix rooms to maestro runners via OpenCode with entity-aware context
- **Script**: `matrix-dispatch-helper.sh [setup|start|stop|status|map|unmap|mappings|sessions|test|logs|cleanup-invites]`
- **Config**: `~/.config/maestro/matrix-bot.json` (600 permissions)
- **Data**: `~/.maestro/.agent-workspace/matrix-bot/`
- **Session DB**: `~/.maestro/.agent-workspace/memory/memory.db` (shared entity tables, SQLite WAL)
