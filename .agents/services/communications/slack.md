---
description: Slack Bot integration — Bolt SDK setup, Socket Mode, slash commands, interactive components, Agents API, security (no E2E, AI training), Matterbridge, maestro dispatch
mode: subagent
tools:
  read: true
  write: false
...
- **Config**: `~/.config/maestro/slack-bot.json` (600 perms)
...
```bash
gopass insert maestro/slack/bot-token && gopass insert maestro/slack/app-token && gopass insert maestro/slack/signing-secret
...
**Runner dispatch**: `runner-helper.sh` handles headless sessions, memory isolation, entity-aware context, run logging. Entity resolution: Slack user ID → `entity_channels` → entity profile; new users via `entity-helper.sh create`. **Config** `~/.config/maestro/slack-bot.json` (600 perms) — store tokens in gopass. `botPrefix` empty = `@mention`/slash commands only; set (e.g., `!ai`) for prefix triggering.
