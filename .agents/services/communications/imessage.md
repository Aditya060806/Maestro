---
description: iMessage/BlueBubbles bot integration — REST API, imsg CLI, macOS setup, access control, privacy, maestro dispatch
mode: subagent
tools:
  read: true
  write: false
...
`iPhone/iPad/Mac → iMessage (E2E) → Apple → Messages.app + BlueBubbles → REST API/Webhooks → Bot → maestro`
...
- **Keepalive** (`sh.maestro.imessage-keepalive`): Restart Messages/BlueBubbles if missing.
...
- **Secret**: `maestro secret set BLUEBUBBLES_PASSWORD`.
...
`iMessage User → Bot (webhook) → maestro Runner → AI session → response`
