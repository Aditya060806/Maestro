---
description: Telegram Bot Integration — Bot API, grammY SDK (TypeScript/Bun), BotFather setup, long polling vs webhooks, group/DM access control, inline keyboards, forum topics, security model, Matterbridge native support, and maestro dispatch integration
mode: subagent
tools:
  read: true
  write: false
...
```bash
gopass insert maestro/telegram/bot-token
# Or: echo 'export TELEGRAM_BOT_TOKEN="<token>"' >> ~/.config/maestro/credentials.sh
...
| Config | `~/.config/maestro/telegram-bot.json` |
...
```json
{
  "token_source": "gopass:maestro/telegram/bot-token",
  "mode": "polling",
  "admin_ids": [123456789],
