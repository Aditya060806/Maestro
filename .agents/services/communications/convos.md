...
Full template: [`convos-bridge-template.sh`](convos-bridge-template.sh) — replace `your-ai-dispatch` with your backend. For maestro: `claude run --session-id "$SESSION_ID" --message "..."`. If stateless, prepend `SYSTEM_MSG` to every call.
