---
description: Nextcloud Talk — self-hosted team communication with strongest corporate privacy, Talk Bot API (webhook-based, OCC CLI), server-side encryption, Matterbridge bridging, and maestro dispatch
mode: subagent
tools:
  read: true
  write: false
...
| Config | `~/.config/maestro/nextcloud-talk-bot.json` (600 permissions) |
| Data | `~/.maestro/.agent-workspace/nextcloud-talk-bot/` |
...
sudo -u www-data php /var/www/nextcloud/occ talk:bot:install \
  "maestro" "http://localhost:8780/webhook" "AI-powered DevOps assistant" "YOUR_SHARED_SECRET_HERE"
...
# Credentials
openssl rand -hex 32
gopass insert maestro/nextcloud-talk/webhook-secret
# App password: Nextcloud UI → Settings > Security > Devices & sessions > "maestro-talk-bot"
gopass insert maestro/nextcloud-talk/app-password
...
```json
{
  ...
  "object": { "type": "Message", "id": "42", "name": "Hello @maestro, review the latest PR?", "mediaType": "text/markdown" },
...
const BOT_USER = process.env.BOT_USER || "maestro-bot";
...
`~/.config/maestro/nextcloud-talk-bot.json` (600 permissions):
