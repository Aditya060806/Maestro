---
description: Discord bot integration — discord.js (TypeScript), bot setup (Developer Portal, intents, OAuth2), DM/guild/thread messaging, slash commands, interactive components v2, role-based routing, access control, privacy/security assessment, maestro runner dispatch, Matterbridge bridging
mode: subagent
...
2. **Bot** tab → "Add Bot" → copy token: `maestro secret set DISCORD_BOT_TOKEN`
...
Config at `~/.config/maestro/discord-bot.json` (600 permissions). Store `botToken` in gopass, not in this file.
...
## Integration with maestro Runners
