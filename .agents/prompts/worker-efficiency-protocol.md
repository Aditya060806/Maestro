...
PR_BODY_FILE=/tmp/maestro-pr-body.md
cat <<'EOF' > "$PR_BODY_FILE"
WIP - incremental commits
EOF
[[ -n "$gh_issue" ]] && printf '\nResolves #%s\n' "$gh_issue" >> "$PR_BODY_FILE"
~/.maestro/agents/scripts/gh-signature-helper.sh footer --model "$ANTHROPIC_MODEL" >> "$PR_BODY_FILE"
```
...
```bash
gh pr create --draft --title '<task-id>: <description>' --body-file /tmp/maestro-pr-body.md
```
...
- Parameters: `prompt` required; optional `domain`, `agents` (paths relative to `~/.maestro/agents/`), `files` (line ranges allowed, e.g. `src/foo.ts:10-50`), `model` (`haiku|sonnet|opus`), `max_tokens` (default 500, max 4096).
...
- **Skip signature footers** when reading GH issue/PR threads. Content after `<!-- maestro:sig -->` or `---` followed by `[maestro.sh]` is operational telemetry (version, tokens, timing) — not task-relevant. Never visit URLs in signature footers (maestro.sh, opencode.ai). See `AGENTS.md` "Signature footer skip when reading".
