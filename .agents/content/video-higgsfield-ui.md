...
Requires Node.js/Bun, Playwright (`npm install playwright && npx playwright install chromium`). Credentials: `~/.config/maestro/credentials.sh` → `HIGGSFIELD_USER`, `HIGGSFIELD_PASS`.
...
higgsfield-helper.sh → playwright-automator.mjs (~4900 lines)
  Auth/cache: ~/.maestro/.agent-workspace/work/higgsfield/
...
`--headed` → `~/Downloads/higgsfield/` | headless → `~/.maestro/.agent-workspace/work/higgsfield/output/` | override: `--output` | subdirs: `--project` | `--no-sidecar` `--no-dedup`
...
```bash
rm ~/.maestro/.agent-workspace/work/higgsfield/auth-state.json && higgsfield-helper.sh login  # auth reset
