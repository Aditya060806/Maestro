...
`xurl` stores isolated app/account profiles, so maestro can operate multiple brands, client accounts, developer apps, and X API subscription tiers from one machine without sharing secrets in chat.
...
.agents/scripts/xurl-helper.sh search "maestro" --limit 10
...
OpenCode's xAI/Grok provider connection can be used for reasoning by selecting that model in the runtime, but it does not replace X API OAuth. Keep model-provider auth and X API `xurl` auth separate so maestro can run with any capable model while `xurl` owns X account permissions.
