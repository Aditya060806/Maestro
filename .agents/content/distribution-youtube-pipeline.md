...
4. For each opportunity:
   ...
   d. Save to workspace: ~/.maestro/.agent-workspace/work/youtube/scripts/
...
```text
1. Read draft scripts from: ~/.maestro/.agent-workspace/work/youtube/scripts/
2. For each script:
...
ls ~/.maestro/.agent-workspace/work/youtube/scripts/
```
...
| Issue | Solution |
|-------|----------|
| Worker fails to start | Check `maestro pulse status` for error messages |
...
```text
~/.maestro/.agent-workspace/work/youtube/
├── scripts/
