...
// maestro-specific attributes (intent, task_id, session_origin, runtime)
...
// `maestro.*` to avoid collision with standard OTel conventions.
//
...
/**
 * Enrich the currently-active OTEL span with maestro attributes.
 *
...
/**
 * Detect the current maestro task ID from the environment.
 *
 * Resolution order:
 *   1. MAESTRO_TASK_ID env var (set by full-loop-helper when dispatched).
...
export function detectTaskId(cwd) {
  const envId = process.env.MAESTRO_TASK_ID;
