 ...
 * Reads Cursor tokens from ~/.maestro/oauth-pool.json, starts the gRPC proxy
...
async function setCursorAuth(client) {
  try {
    ...
    console.error(`[maestro] Cursor proxy: failed to set auth entry: ${err.message}`);
  }
}
...
async function discoverCursorModels() {
  ...
  try {
    ...
    console.error(`[maestro] Cursor proxy: discovered ${models.length} models`);
    return models;
  } catch (err) {
    console.error(`[maestro] Cursor proxy: model discovery failed (${err.message}), using fallback list`);
    return [];
  }
}
...
export async function startCursorProxy(client) {
  ...
  if (cachedModels.length > 0) {
    try {
      ...
      console.error(`[maestro] Cursor proxy: failed to persist provider to opencode.json: ${err.message}`);
    }
  }
...
export async function ensureCursorProxyServer() {
  return cursorLifecycle.ensureStarted({
    ...
    launch: async () => {
      ...
      console.error(`[maestro] Cursor proxy: gRPC proxy started on port ${port}`);
      return { port };
    },
  });
}
...
export async function stopCursorGrpcProxy() {
  if (cursorLifecycle.getPort() !== null) {
    ...
    console.error("[maestro] Cursor proxy: stopped");
  }
}
...
function persistCursorProvider(port, models) {
  ...
  try {
    ...
    console.error("[maestro] Cursor proxy: cannot read opencode.json, skipping persist");
    return;
  }
  ...
  try {
    ...
    console.error(`[maestro] Cursor proxy: persisted ${models.length} models to opencode.json (port ${port})`);
  } catch (err) {
    console.error(`[maestro] Cursor proxy: failed to write opencode.json: ${err.message}`);
  }
}
