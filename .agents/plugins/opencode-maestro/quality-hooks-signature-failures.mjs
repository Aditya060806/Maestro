...
const STANDARD_FIXES =
  ...
  `       source ~/.maestro/agents/scripts/shared-gh-wrappers.sh\n` +
...
export function formatGateThrowMessage(failure, cmdSnippet) {
  ...
  return (
    `maestro: gh write command blocked at signature gate (t2685, t2893).\n\n` +
    `${causeBlock}${sameCallHint}\n\n` +
    `${STANDARD_FIXES}\n\n` +
    `Cmd snippet: ${cmdSnippet}\n\n` +
    `Emergency bypass (last resort, breaks audit trail): set\n` +
    `MAESTRO_GH_SHIM_DISABLE=1 — but this plugin hook still blocks.`
  );
}
