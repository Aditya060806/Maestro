...
const SQLITE_SENTINEL = "__MAESTRO_QUERY_DONE__";
const SQLITE_BUSY_RE = /database is locked|SQLITE_BUSY|database table is locked/i;
const SQLITE_QUEUE_MAX = Number.parseInt(process.env.MAESTRO_SQLITE_QUEUE_MAX || "1000", 10);
const SQLITE_TIMEOUT_MS_RAW = Number.parseInt(process.env.MAESTRO_SQLITE_TIMEOUT_MS || "15000", 10);
...
function _spawnSqlite() {
  ...
  try {
    ...
    console.error(`[maestro] Failed to spawn sqlite3: ${err.message}`);
    return;
  }
  ...
  _sqliteProc.stderr.on("data", (chunk) => {
    ...
    if (msg && !isSqliteBusyError(msg)) console.error(`[maestro] SQLite: ${msg}`);
  });
  ...
  _sqliteProc.on("error", (err) => {
    console.error(`[maestro] SQLite process error: ${err.message}`);
    _sqliteProc = null;
    if (_currentCallback) {
      const cb = _currentCallback;
      _currentCallback = null;
...
export function sqliteExec(sql) {
  ...
  if (_queryQueue.length >= queueMax) {
    if (process.env.MAESTRO_SQLITE_QUEUE_WARN !== "0") {
      console.error(`[maestro] SQLite async queue full (${queueMax}); dropping observability write`);
    }
    return;
  }
  _queryQueue.push({
    sql,
    callback: (err) => {
      if (err && !isSqliteBusyError(err.message)) console.error(`[maestro] SQLite async exec failed: ${err.message}`);
    },
  });
  _drainQueue();
}
...
export function sqliteExecSync(sql, timeout = 5000) {
  try {
    ...
    console.error(`[maestro] SQLite sync exec failed: ${e.stderr || e.message}`);
    return null;
  }
}
