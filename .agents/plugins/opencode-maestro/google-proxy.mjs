...
async function rotateOnRateLimit(currentEmail) {
  ...
  for (const candidate of available) {
    ...
    if (token) {
      ...
      console.error(`[maestro] Google proxy: rotated to ${candidate.email} after 429`);
      return { token, email: candidate.email };
    }
  }

  console.error("[maestro] Google proxy: no alternate account available after 429");
  return null;
}
...
async function retryWithRotatedGoogleAccount(accountEmail, forwardHeaders, targetUrl, method, body) {
  console.error("[maestro] Google proxy: 429 from Google API, attempting rotation");
...
async function handleGoogleProxyFetch(req) {
  ...
  try {
    ...
    console.error(`[maestro] Google proxy: request error: ${err.message}`);
    return jsonResponse(
...
function handleGoogleProxyServerError(err) {
  console.error(`[maestro] Google proxy: server error: ${err.message}`);
...
async function discoverModelsForGoogleProxy() {
  ...
  try {
    ...
    console.error(`[maestro] Google proxy: failed to get token for discovery: ${err.message}`);
    return [];
  }
  ...
  try {
    ...
    console.error(`[maestro] Google proxy: model discovery failed (${err.message}), using empty list`);
    return [];
  }
}
...
export async function startGoogleProxy(_client) {
  ...
  if (cachedModels.length > 0) {
    try {
      ...
      console.error(`[maestro] Google proxy: failed to persist provider to opencode.json: ${err.message}`);
    }
  }
...
export async function ensureGoogleProxyServer() {
  return googleLifecycle.ensureStarted({
    ...
    launch: async () => {
      ...
      proxyServer = server;
      console.error(`[maestro] Google proxy: started on port ${server.port}`);
...
export function stopGoogleProxy() {
  if (proxyServer) {
    ...
    proxyServer = null;
    console.error("[maestro] Google proxy: stopped");
  }
}
