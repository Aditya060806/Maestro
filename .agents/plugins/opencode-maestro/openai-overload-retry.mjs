...
export function overloadRetryDelaysMs() {
  const raw = process.env.MAESTRO_OPENAI_OVERLOAD_RETRY_DELAYS_MS || "";
  const parsed = raw
    .split(",")
...
async function retryOpenAIOverloadStream(ctx) {
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    console.error(`[maestro] OpenAI provider: overloaded stream error — retrying request (${attempt + 1}/${retryDelays.length})`);
    await onRetry?.({
      attempt: attempt + 1,
