...
export const BUILTIN_TTSR_RULES = [
  ...
  },
  {
    ...
    correction: "SECURITY: Never expose credentials in output. Use `maestro secret set NAME` for secure storage.",
    severity: "error",
...
export function loadTtsrRules(state) {
  ...
  if (userContent) {
    try {
      ...
      console.error("[maestro] Failed to parse TTSR rules file — using built-in rules only");
    }
  }
