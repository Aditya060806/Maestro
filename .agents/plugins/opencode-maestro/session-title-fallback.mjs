...
import { readMaestroVersion, withMaestroTitleSuffix } from "./session-title-suffix.mjs";
...
async function applyFallbackTitle(deps, sessionID, prompt) {
  ...
  const version = readMaestroVersion(deps.agentsDir);
  const title = withMaestroTitleSuffix(deriveFallbackTitleFromPrompt(prompt), version);
