...
//
// Tests for model-limits.mjs (t2435 — MAESTRO_OPUS_47_CONTEXT env override).
//
//   node --test .agents/plugins/opencode-maestro/tests/test-model-limits.mjs
//
// Scope:
...
describe("resolveOpus47Context", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.MAESTRO_OPUS_47_CONTEXT;
    delete process.env.MAESTRO_OPUS_47_CONTEXT;
  });

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env.MAESTRO_OPUS_47_CONTEXT = originalEnv;
    } else {
      delete process.env.MAESTRO_OPUS_47_CONTEXT;
    }
  });
  ...
  test("garbage with leading digits → parsed as the leading int", () => {
    ...
    process.env.MAESTRO_OPUS_47_CONTEXT = "300000abc";
    assert.equal(resolveOpus47Context(), 300000);
  });

  test("above MAX → clamped to OPUS_47_CONTEXT_MAX", () => {
    process.env.MAESTRO_OPUS_47_CONTEXT = "5000000";
...
describe("describeOpus47Override", () => {
  ...
  test("explicit default → null (no-op, no warn)", () => {
    process.env.MAESTRO_OPUS_47_CONTEXT = "250000";
    assert.equal(describeOpus47Override(), null);
  });
  ...
  test("negative → kind=invalid", () => {
    process.env.MAESTRO_OPUS_47_CONTEXT = "-5";
    const desc = describeOpus47Override();
