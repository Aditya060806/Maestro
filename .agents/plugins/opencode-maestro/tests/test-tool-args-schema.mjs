...
// maestro tool must expose an `args` object even when arguments are optional.
...
describe("maestro plugin tool schemas", () => {
  test("all registered tools expose args schemas", () => {
    const tools = {
      ...createTools("/tmp/maestro-test-scripts", () => ""),
  ...
  test("schemas use OpenCode-compatible Zod fields", () => {
    const tools = createTools("/tmp/maestro-test-scripts", () => "");
    const pool = createPoolTool({});

    assert.ok(tools.maestro.args.command?._zod, "maestro.command must be Zod");
    assert.ok(tools.maestro_pre_edit_check.args.task?._zod, "pre-edit task must be Zod");
