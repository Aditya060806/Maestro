...
//
//   node --test .agents/plugins/opencode-maestro/tests/
//
// or to run just this file:
//
//   node --test .agents/plugins/opencode-maestro/tests/test-otel-enrichment.mjs
//
...
describe("detectTaskId — worktree path shapes", () => {
  const originalTaskIdEnv = process.env.MAESTRO_TASK_ID;

  before(() => {
    delete process.env.MAESTRO_TASK_ID;
  });

  after(() => {
    if (originalTaskIdEnv !== undefined) {
      process.env.MAESTRO_TASK_ID = originalTaskIdEnv;
    } else {
      delete process.env.MAESTRO_TASK_ID;
    }
  });
  ...
  test("wt default: ~/Git/<repo>.feature-tNNNN-name (dot separator)", () => {
    assert.equal(
      detectTaskId("/Users/x/Git/maestro.feature-t2177-otel-introspect"),
      "t2177",
    );
  });
  ...
  test("GH# form with dot separator: <repo>.bugfix-gh-NNNNN-name", () => {
    assert.equal(
      detectTaskId("/Users/x/Git/maestro.bugfix-gh-19634-zombie"),
      "GH#19634",
    );
  });
  ...
  test("terminal task ID: path ends with tNNNN (no trailing separator)", () => {
    assert.equal(
      detectTaskId("/Users/x/Git/maestro.feature-t2181"),
      "t2181",
    );
  });
  ...
  test("returns '' when cwd contains no task-id-like token", () => {
    assert.equal(detectTaskId("/Users/x/Git/maestro"), "");
  });
});
...
describe("detectTaskId — MAESTRO_TASK_ID env var", () => {
  test("env var overrides cwd parsing when valid", () => {
    const original = process.env.MAESTRO_TASK_ID;
    try {
      process.env.MAESTRO_TASK_ID = "t9999";
      assert.equal(
        detectTaskId("/Users/x/Git/maestro.feature-t2177-other"),
        "t9999",
      );
    } finally {
      if (original !== undefined) process.env.MAESTRO_TASK_ID = original;
      else delete process.env.MAESTRO_TASK_ID;
    }
  });
  ...
  test("env var accepts GH# form", () => {
    const original = process.env.MAESTRO_TASK_ID;
    try {
      process.env.MAESTRO_TASK_ID = "GH#12345";
  ...
  test("malformed env var falls back to cwd parsing", () => {
    const original = process.env.MAESTRO_TASK_ID;
    try {
      process.env.MAESTRO_TASK_ID = "garbage";
      assert.equal(
        detectTaskId("/Users/x/Git/maestro.feature-t2177-other"),
        "t2177",
      );
...
describe("detectSessionOrigin", () => {
  ...
  test("returns 'worker' when MAESTRO_HEADLESS is any non-empty value", () => {
    process.env.MAESTRO_HEADLESS = "yes";
    try {
      assert.equal(detectSessionOrigin(), "worker");
    } finally {
      delete process.env.MAESTRO_HEADLESS;
    }
  });
});
