...
test("installed fetch guard rotates on response failures and pre-request cooldowns", async () => {
  const home = mkdtempSync(join(tmpdir(), "maestro-openai-rotation-"));
  const script = String.raw`
    ...
    const maestroDir = join(home, ".maestro");
    mkdirSync(maestroDir, { recursive: true });
    const poolPath = join(maestroDir, "oauth-pool.json");
  ...
  execFileSync(process.execPath, ["--input-type=module", "--eval", script], {
    ...
    env: { ...process.env, HOME: home, MAESTRO_OPENAI_OVERLOAD_RETRY_DELAYS_MS: "0,0" },
    stdio: "pipe",
  });
});

test("OpenAI startup injection honors current auth availability", async () => {
  const home = mkdtempSync(join(tmpdir(), "maestro-openai-startup-"));
  const script = String.raw`
    import assert from "node:assert/strict";
    import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
    import { join } from "node:path";
