...
test("old gh prerequisite warning becomes OpenCode warning toast", () => {
  const warningLine =
    "[WARN] GitHub CLI (gh) 2.50.0 is too old; gh api --paginate --slurp requires gh >= 2.51.0. Run maestro setup to upgrade gh.";
  const buckets = classifyLines(`maestro v3.15.32 running in OpenCode v1.14.48\n${warningLine}`);
  ...
  assert.match(toast.message, /maestro v3\.15\.32 running in OpenCode/);
});
