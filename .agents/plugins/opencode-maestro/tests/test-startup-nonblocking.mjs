...
test("index.mjs does not await optional proxy preparation during plugin startup", () => {
  ...
  assert.match(
    ...
    "MaestroPlugin should use a fire-and-forget helper for optional proxy preparation.",
  );
  assert.match(
    src,
...
test("initPoolAuth does not seed unsupported pending auth entries", async () => {
  const tempHome = mkdtempSync(resolve(tmpdir(), "maestro-oauth-pool-"));
  ...
  assert.deepEqual(
    ...
    [],
    "Startup must not pass maestro pending pool state to OpenCode auth.set().",
  );
  assert.deepEqual(
