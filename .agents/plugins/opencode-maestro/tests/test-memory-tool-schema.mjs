...
//
// Regression coverage for the OpenCode maestro_memory tool args schema.
...
const SCRIPTS_DIR = "/tmp/maestro-test-scripts";

function withMemoryHelper(fn) {
  const dir = mkdtempSync(join(tmpdir(), "maestro-memory-tool-"));
...
describe("maestro_memory execution", () => {
  test("empty payload returns actionable validation without invoking helper", async () => {
    ...
    const result = await withMemoryHelper(async (scriptsDir) => {
      ...
      return tools.maestro_memory.execute({});
    });
  ...
  test("recall action without a query returns a validation error", async () => {
    ...
    const result = await withMemoryHelper(async (scriptsDir) => {
      ...
      return tools.maestro_memory.execute({ action: "recall", limit: 5 });
    });
  ...
  test("empty store content returns a validation error", async () => {
    const result = await withMemoryHelper(async (scriptsDir) => {
      ...
      return tools.maestro_memory.execute({ action: "store", content: "   " });
    });
