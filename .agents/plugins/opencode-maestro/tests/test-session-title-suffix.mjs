...
async function withTempAgentsDir(fn) {
  const root = mkdtempSync(join(tmpdir(), "maestro-title-suffix-"));
...
async function withoutEnvVersion(fn) {
  const saved = process.env.MAESTRO_VERSION;
  try {
    delete process.env.MAESTRO_VERSION;
    return await fn();
  } finally {
    if (saved === undefined) delete process.env.MAESTRO_VERSION;
    else process.env.MAESTRO_VERSION = saved;
  }
}
...
test("title suffix appends and replaces idempotently", () => {
  assert.equal(
    withMaestroTitleSuffix("Investigate title path", "3.20.102"),
    "Investigate title path · Maestro 3.20.102",
  );
  assert.equal(
    withMaestroTitleSuffix("Investigate title path · Maestro 3.20.101", "3.20.102"),
    "Investigate title path · Maestro 3.20.102",
  );
});

test("default session title detection ignores Maestro suffix", () => {
  assert.equal(isDefaultSessionTitle("New session - 2026-06-20T21:27:42.505Z"), true);
  assert.equal(isDefaultSessionTitle("New session - 2026-06-20T21:27:42.505Z · Maestro 3.21.2"), true);
  assert.equal(isDefaultSessionTitle("Study newsjack repository capabilities · Maestro 3.21.2"), false);
});
...
test("version reader prefers deployed agents VERSION", async () => {
  await withoutEnvVersion(() =>
    withTempAgentsDir((agentsDir) => {
      ...
      assert.equal(readMaestroVersion(agentsDir), "3.20.102");
    }),
  );
});
  ...
  await withoutEnvVersion(() =>
    withTempAgentsDir(async (agentsDir) => {
      ...
      assert.deepEqual(calls, [
        {
          path: { id: "ses_test" },
          body: { title: "Work on live title fix · Maestro 3.20.103" },
        },
      ]);
    }),
  );
  ...
  await withTempAgentsDir(async (agentsDir) => {
    ...
    await handler({
      event: {
        ...
        properties: {
          sessionID: "ses_test",
          info: { id: "ses_test", title: "Work · Maestro 3.20.103" },
        },
      },
    });
  ...
  await withTempAgentsDir(async (agentsDir) => {
    ...
    await handler({
      event: {
        ...
        properties: {
          sessionID: "ses_test",
          info: { id: "ses_test", title: "New session - 2026-06-20T22:54:09.982Z · Maestro 3.21.4" },
        },
      },
    });
  ...
  await withoutEnvVersion(() =>
    withTempAgentsDir(async (agentsDir) => {
      ...
      assert.deepEqual(calls[1], {
        ...
        body: { title: "Fallback path · Maestro 3.20.103" },
      });
    }),
  );
});
  ...
  await withoutEnvVersion(() =>
    withTempAgentsDir(async (agentsDir) => {
      ...
      assert.deepEqual(calls.at(-1), {
        path: { id: "ses_test" },
        body: { title: "Study the newsjack repository capabilities · Maestro 3.21.2" },
      });
    }),
  );
});
...
test("message part fallback preserves meaningful session title", async () => {
  await withTempAgentsDir(async (agentsDir) => {
    ...
    await handler({
      event: {
        type: "session.updated",
        properties: { sessionID: "ses_test", info: { id: "ses_test", title: "Existing title · Maestro 3.21.2" } },
      },
    });
    await handler({
      event: {
