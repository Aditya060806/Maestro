...
export async function tryRecoverAccount(client, account, waitStart, logPrefix) {
  ...
  console.error(`[maestro] provider-auth: ${logPrefix} recovered via ${account.email} after ${elapsed}s wait`);
...
export function logExhaustionOnce(accounts, currentEmail, logPrefix, now) {
  ...
  console.error(`[maestro] provider-auth: ${logPrefix} for ${currentEmail} — all accounts on cooldown, waiting... ${accountSummary}`);
}
...
export function forceClearAllCooldowns(logPrefix) {
  ...
  console.error(`[maestro] provider-auth: ${logPrefix} — force-cleared all cooldowns after ${MAX_EXHAUSTION_WAIT_MS / 1000}s. Returning response for opencode retry.`);
}
...
export async function refreshSessionOwnAccount(client, accounts, sessionAccountEmail) {
  ...
  console.error(`[maestro] provider-auth: refreshed session account ${myAccount.email}`);
