...
function trackBashOperation(ctx, title, outputText) {
  if (title.includes("git commit") || title.includes("git push")) {
    console.error(`[maestro] Git operation detected: ${title}`);
...
function handleToolBefore(ctx, log, input, output) {
  ...
  // OTEL span enrichment (t2177) — attaches maestro attributes to opencode's
  ...
  // failures.
  enrichActiveSpan({
    "maestro.intent": intent,
    "maestro.tool_name": input.tool || "",
    "maestro.task_id": detectTaskId(),
    "maestro.session_origin": detectSessionOrigin(),
