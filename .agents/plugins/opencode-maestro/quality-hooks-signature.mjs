...
// command posts a body ending with the canonical maestro signature footer.
...
//   2. cmd contains the canonical HTML marker `<!-- maestro:sig -->` →
...
function _signatureHelperEnv() {
  const helperEnv = {
    ...process.env,
    MAESTRO_SIG_CLI: process.env.MAESTRO_SIG_CLI || "OpenCode",
    MAESTRO_SIG_TOKENS: process.env.MAESTRO_SIG_TOKENS || "0",
  };
  const model = process.env.MAESTRO_SIG_MODEL || process.env.OPENCODE_MODEL || "";
  if (model) {
    helperEnv.MAESTRO_SIG_MODEL = model;
  } else {
    delete helperEnv.MAESTRO_SIG_MODEL;
  }
  return helperEnv;
}
