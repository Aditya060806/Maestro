...
function refreshCCHCache() {
  const script = join(homedir(), ".maestro", "agents", "scripts", "cch-extract.sh");
  try {
...
function refreshCacheIfVersionChanged(raw, cacheFile) {
  ...
  console.error(`[maestro] CCH: CLI updated ${raw.version} → ${liveVersion}. Re-extracting constants...`);
  if (!refreshCCHCache()) return raw;
  const updated = tryReadCacheFile(cacheFile);
  if (updated?.version) {
    console.error(`[maestro] CCH: constants refreshed for v${updated.version}`);
    return updated;
  }
  return raw;
}
...
function ensureCacheExists(raw, cacheFile) {
  if (raw) return raw;
  console.error("[maestro] CCH: no cache found. Extracting constants...");
...
export function loadCCHConstants() {
  ...
  const cacheFile = join(homedir(), ".maestro", "cch-constants.json");
