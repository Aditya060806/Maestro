 ...
 * colons, hash signs (#), and at-signs (@) — sufficient for all maestro subcommands and file path arguments.
...
function getMemoryArgsError(args) {
  ...
  if (!args.action && !query && !content) {
    error = 'Error: maestro_memory requires a complete payload. Use {action:"recall", query:"<keywords>", limit:"5"} or {action:"store", content:"<lesson>", confidence:"medium"}; do not use empty calls as placeholders.';
...
/**
 * Create the maestro CLI tool.
 * @param {function} run - Shell command runner
 * @returns {object} Tool definition
 */
function createMaestroTool(run) {
  return tool({
    description:
      'Run maestro CLI commands (status, repos, features, secret, etc.). Pass command as string e.g. "status", "repos", "features"',
    args: {
      command: z.string().describe('maestro command and arguments, e.g. "status" or "repos"'),
    },
    async execute(args) {
 ...
 *
 * Consolidates the former maestro_memory_recall and maestro_memory_store tools.
...
function createMemoryTool(scriptsDir, run) {
  return tool({
    description:
      'Recall or store memories in the maestro cross-session memory system. ' +
 ...
 *
 * Tools (4 total):
 *   - maestro              — maestro CLI runner
 *   - maestro_memory       — unified recall/store (merged from former recall + store pair)
 *   - maestro_pre_edit_check — git safety check before file edits
 *   - model-accounts-pool   — OAuth account pool management (added in index.mjs)
 *
 * NOTE: maestro_quality_check was removed. Quality checks run automatically
 ...
 *
 * NOTE: maestro_install_hooks was removed. Hook installation is a one-time
 * setup operation best done via Bash: `bash ~/.maestro/agents/scripts/install-hooks-helper.sh install`
 * or `maestro security posture`. A dedicated plugin tool adds ~90 lines of
...
export function createTools(scriptsDir, run) {
  return {
    maestro: createMaestroTool(run),
    maestro_memory: createMemoryTool(scriptsDir, run),
    maestro_pre_edit_check: createPreEditCheckTool(scriptsDir),
  };
}
