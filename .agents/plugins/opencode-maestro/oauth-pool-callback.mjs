...
export function startOAuthCallbackServer(expectedState) {
  ...
  server = createServer((req, res) => {
    ...
    if (expectedState) {
      ...
      if (returnedState !== expectedState) {
        console.error("[maestro] OAuth pool: state mismatch in callback — possible CSRF");
  ...
  server.on("error", (err) => {
    ...
    if (err.code === "EADDRINUSE") {
      console.error(`[maestro] OAuth pool: port ${OAUTH_CALLBACK_PORT} in use`);
      resolveReady(false);
      return;
    }
    console.error(`[maestro] OAuth pool: callback server error: ${err.message}`);
    resolveReady(false);
    rejectCode(err);
  });

  server.listen(OAUTH_CALLBACK_PORT, "127.0.0.1", () => {
    console.error(`[maestro] OAuth pool: callback server listening on port ${OAUTH_CALLBACK_PORT}`);
    resolveReady(true);
  });
...
export async function saveAccountAndInject(opts) {
  ...
  console.error(`[maestro] OAuth pool: added ${email} (${total} account${total === 1 ? "" : "s"} total)`);
  const INSTR = {
  ...
  console.error(`[maestro] OAuth pool: Account added successfully. ${INSTR[provider] || ""}`);
...
export async function initCallbackServerSafe(expectedState) {
  try {
    ...
    if (!ready) {
      console.error("[maestro] OAuth pool: callback server failed -- manual code paste required");
...
export async function acquireAuthCode(manualCode, cs) {
  ...
  if (cs.code) {
    console.error("[maestro] OAuth pool: using auto-captured code");
    if (cs.server) cs.server.close();
    return cs.code;
  }
  if (cs.ready && cs.server) {
    try {
      ...
      console.error("[maestro] OAuth pool: received code from callback server");
      cs.server.close();
      return code;
    } catch {
      console.error("[maestro] OAuth pool: no code received");
      cs.server.close();
      return null;
    }
  }
