...
function attachAbortSignalToChild(abortSignal, child, label) {
  ...
  const onAbort = () => {
    if (child.exitCode === null && !child.killed) {
      try {
        child.kill("SIGTERM");
        console.error(`[maestro] Claude proxy: ${label} aborted by client, killed child pid=${child.pid}`);
      } catch {
        // best effort
      }
    }
...
async function runClaudeJsonWithAccount(body, directory, account, abortSignal) {
  ...
  const timeout = setTimeout(() => {
    console.error(`[maestro] Claude proxy: json child timeout (${CHILD_TIMEOUT_MS}ms), killing`);
    child.kill("SIGKILL");
  }, CHILD_TIMEOUT_MS);
...
export async function runClaudeJson(body, directory, abortSignal) {
  ...
  for (const account of accountsWithFallback) {
    ...
    console.error(`[maestro] Claude proxy: trying account ${account.email} (json mode)`);
