...
export async function discoverGoogleModels(accessToken) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.error(`[maestro] Google proxy: model discovery failed: HTTP ${resp.status}`);
      return models;
    }
    ...
    console.error(`[maestro] Google proxy: discovered ${models.length} models`);
  } catch (err) {
    console.error(`[maestro] Google proxy: model discovery error: ${err.message}`);
  }
  return models;
}
...
export function persistGoogleProvider(port, models) {
  ...
  try {
    ...
    if (err.code !== "ENOENT") {
      console.error(`[maestro] Google proxy: cannot read opencode.json: ${err.message}`);
      return;
    }
  }
  ...
  try {
    ...
    console.error(`[maestro] Google proxy: persisted ${models.length} models to opencode.json (port ${port})`);
  } catch (err) {
    console.error(`[maestro] Google proxy: failed to write opencode.json: ${err.message}`);
  }
}
