...
function checkCooldownGate(cooldownUntil, context, providerLabel) {
  ...
  console.error(`[maestro] OAuth pool: ${context} skipped — ${providerLabel} rate limited, ${Math.ceil((cooldownUntil - Date.now()) / 60000)}m remaining.`);
...
function curlTokenEndpoint(url, options, context) {
  ...
  try {
    ...
    console.error(`[maestro] OAuth pool: ${context} curl failed (${reason})`);
...
async function fetchProviderTokenEndpoint(opts, context) {
  ...
  if (response.status === 429) {
    ...
    console.error(`[maestro] OAuth pool: ${context} rate limited by ${opts.providerLabel}. Cooldown ${Math.ceil(cdMs / 60000)}m.`);
  } else if (!response.ok) {
    console.error(`[maestro] OAuth pool: ${context} failed: HTTP ${response.status}`);
  }
