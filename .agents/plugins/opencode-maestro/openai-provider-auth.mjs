...
async function handleOpenAIUsageLimit(ctx) {
  ...
  console.error(`[maestro] OpenAI provider: usage/rate limit for ${currentEmail} — rotating pool account`);
...
async function maybeRotateBeforeOpenAIFetch(client, input, init) {
  ...
  const currentEmail = current?.email || "unknown";
  console.error(`[maestro] OpenAI provider: current account ${currentEmail} unavailable before request — rotating pool account`);
  const rotated = await rotateOpenAIPoolToken(client, current?.email);
