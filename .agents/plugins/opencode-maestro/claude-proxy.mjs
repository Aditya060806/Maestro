...
function readOpencodeConfig() {
  try {
    ...
    console.error(`[maestro] Claude proxy: cannot read opencode.json: ${err.message}`);
    return null;
  }
}
...
function persistClaudeProvider(port, models) {
  ...
  try {
    ...
    console.error(`[maestro] Claude proxy: persisted ${models.length} models to opencode.json (port ${port})`);
  } catch (err) {
    console.error(`[maestro] Claude proxy: failed to write opencode.json: ${err.message}`);
  }
}
...
async function handleChatCompletions(req, directory) {
  ...
  console.error(
    `[maestro] Claude proxy: request model=${body.model} agent=${body.agentName} stream=${body.stream} systemChars=${body.systemPrompt.length} promptChars=${body.prompt.length}`,
  );
...
async function launchProxyServer(client, directory) {
  ...
  console.error(`[maestro] Claude proxy: started on port ${proxyServer.port}`);
