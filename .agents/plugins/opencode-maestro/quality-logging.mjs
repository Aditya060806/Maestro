...
function rotateLogIfNeeded(logPath, maxBytes) {
  try {
    ...
    console.error(`[maestro] Log rotation failed: ${e.message}`);
  }
}
...
function qualityDetailLog(ctx, label, filePath, report) {
  try {
    ...
    console.error(`[maestro] Quality detail logging failed: ${e.message}`);
  }
}
...
export function logQualityGateResult(ctx, result) {
  ...
  if (totalViolations > 0) {
    ...
    if (errorLevel === "ERROR") {
      ...
      console.error(`[maestro] ${label}: ${totalViolations} issue${plural} in ${filePath}:\n${consoleReport}`);
    }
  } else {
