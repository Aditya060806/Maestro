...
export async function tryGetAndInjectToken(client, account, forceRefresh) {
  ...
  try {
    ...
    console.error(`[maestro] provider-auth: ensureValidToken failed for ${account.email}: ${err.message}`);
    return null;
  }
  ...
  try {
    ...
    console.error(`[maestro] provider-auth: failed to inject token for ${account.email}: ${err.message}`);
    return null;
  }
  return token;
}
...
export async function tryActivatePoolAccount(client, account) {
  ...
  console.error(`[maestro] provider-auth: refreshed via pool account ${account.email}`);
...
export async function rotatePoolAccounts(client, accounts) {
  for (const account of sortAccountsByPriority(accounts)) {
    if (isOnCooldown(account)) {
      console.error(`[maestro] provider-auth: skipping ${account.email} — cooldown active`);
      continue;
    }
...
export function getMostRecentlyUsedAccount(accounts) {
  ...
  console.error(`[maestro] provider-auth: no token match — assuming ${mru.email} (most recently used)`);
  return mru;
}
