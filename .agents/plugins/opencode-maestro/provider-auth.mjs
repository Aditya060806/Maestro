...
async function forceRefreshCurrentAccount(client, currentAccount, currentEmail) {
  ...
  console.error(`[maestro] provider-auth: token refreshed for ${currentEmail} — retrying request`);
  return freshToken;
}
...
async function markAndRotateAccount(client, accounts, currentAccount, currentEmail) {
  console.error(`[maestro] provider-auth: refresh failed for ${currentEmail} — rotating to next account`);
  if (currentAccount) {
...
async function getAuthWithPoolRecovery(getAuth, client, sessionAccountEmail) {
  try {
    ...
    console.error(
      `[maestro] provider-auth: built-in token refresh failed before request (${err.message}) — attempting pool recovery`,
    );
...
async function handle401Recovery(client, response, accessToken, sessionAccountEmail, ctx) {
  ...
  console.error(
    `[maestro] provider-auth: ${response.status} (invalid/revoked token) for ${currentEmail} — attempting refresh...`,
  );
  ...
  if (rotated) {
    console.error(`[maestro] provider-auth: rotated to ${rotated.email} — retrying request once`);
    return applyTokenAndRetry(rotated.token, rotated.email, ctx);
  }
  console.error(
    `[maestro] provider-auth: ${response.status} for ${currentEmail} — all accounts exhausted. ` +
...
async function handle429Recovery(client, response, ctx429) {
  ...
  const { accounts, currentAccount, currentEmail } = resolvePoolState(sessionAccountEmail, accessToken);
  console.error(
    `[maestro] provider-auth: 429 rate limit hit for ${currentEmail} mid-session (cooldown ${Math.ceil(cooldownMs / 1000)}s) — attempting pool rotation`,
  );
  ...
  if (alternates.length === 0) {
    console.error(`[maestro] provider-auth: all ${tried.size} accounts tried — giving up`);
    return { response, sessionAccountEmail };
  }
  const rotated = await rotateToAlternateAccount(client, alternates, false);
  if (rotated) {
    tried.add(rotated.email);
    console.error(`[maestro] provider-auth: rotated to ${rotated.email} — retrying`);
  ...
  console.error(`[maestro] provider-auth: all accounts rate-limited — giving up`);
  return { response, sessionAccountEmail };
}
...
async function handle400ThirdPartyRecovery(client, response, accessToken, sessionAccountEmail, ctx) {
  const { accounts, currentAccount, currentEmail } = resolvePoolState(sessionAccountEmail, accessToken);
  console.error(
    `[maestro] provider-auth: 400 third-party billing error for ${currentEmail} — rotating account`,
  );
  if (currentAccount) {
  ...
  console.error(
    `[maestro] provider-auth: all accounts hit third-party billing error. ` +
