 ...
 *
 * Every maestro local proxy (Claude CLI, Cursor gRPC, Google auth-translating)
 ...
 *   "Google"). Used as `[maestro] ${name} proxy: …`.
...
async function tryAdoptExisting(lifecycle) {
  ...
  console.error(
    `[maestro] ${lifecycle.name} proxy: adopted existing server on port ${lifecycle.port}`,
  );
...
async function attemptEaddrinuseAdoption(lifecycle) {
  ...
  if (adoptedPort) {
    ...
    console.error(
      `[maestro] ${lifecycle.name} proxy: adopted sibling server on port ${lifecycle.port} after EADDRINUSE`,
    );
    return { port: lifecycle.port, adopted: true };
  }
  console.error(
    `[maestro] ${lifecycle.name} proxy: port ${lifecycle.targetPort} in use but no responsive proxy found after ${lifecycle.probeRetryAttempts} probes — port may be poisoned by another process`,
  );
  return null;
}
...
async function tryLaunch(lifecycle, launch) {
  ...
  try {
    ...
    console.error(`[maestro] ${lifecycle.name} proxy: failed to start: ${message}`);
    return null;
  } finally {
...
async function tryLaunchIfPossible(lifecycle, opts) {
  if (typeof globalThis.Bun === "undefined") {
    console.error(
      `[maestro] ${lifecycle.name} proxy: skipped (not running under Bun and no existing proxy found)`,
    );
    return null;
  }
