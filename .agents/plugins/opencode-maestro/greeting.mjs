...
//
// Routes the output of `maestro-update-check.sh --interactive` through
...
//   ~/.maestro/cache/session-greeting.txt
...
//   With MAESTRO_PLUGIN_DEBUG=1, the trace log shows handler-completed
//   BEFORE emitToast-* — the reversed ordering confirms async delivery.
//
// Diagnostics: set MAESTRO_PLUGIN_DEBUG=1 to trace every handler invocation
...
const CACHE_DIR = join(homedir(), ".maestro", "cache");
...
function runGreetingAsync(scriptsDir, client) {
  const script = join(scriptsDir, "maestro-update-check.sh");
  execAsync(`bash ${JSON.stringify(script)} --interactive`, {
    ...
    .then(async ({ stdout }) => {
      ...
      if (!output) {
        if (process.env.MAESTRO_PLUGIN_DEBUG) {
          console.error("[maestro] greeting: update-check returned empty, skipping toasts");
        }
        return;
      }
      ...
      if (process.env.MAESTRO_PLUGIN_DEBUG) {
        ...
        if (toast) {
          console.error(`[maestro] greeting: built 1 toast (variant=${toast.variant}, ${bucketCounts})`);
        } else {
          console.error(`[maestro] greeting: no lines to show (${bucketCounts})`);
        }
      }
    ...
    .catch((err) => {
      if (process.env.MAESTRO_PLUGIN_DEBUG) {
        console.error(`[maestro] greeting: update-check failed: ${err.message}`);
      }
    });
...
async function getOpenCodeMaintenanceNotice(scriptsDir) {
  ...
  try {
    ...
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      console.error(`[maestro] greeting: opencode maintenance notice failed: ${err.message}`);
    }
    return "";
  }
}
...
/**
 * Write raw update-check output to ~/.maestro/cache/session-greeting.txt
...
function cacheGreeting(output) {
  try {
    ...
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      console.error(`[maestro] greeting: cache write failed: ${err.message}`);
    }
  }
}
...
async function emitToast(client, body) {
  try {
    ...
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      console.error(`[maestro] greeting: toast emitted (variant=${body.variant}, ${body.message.length} chars)`);
    }
  } catch (err) {
    // Log on DEBUG; otherwise swallow (failures here are non-fatal —
    // the user still has the message-context greeting in Phase 1).
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      console.error(`[maestro] greeting: toast failed: ${err.message}`);
    }
  }
}
...
export function createGreetingHandler({ scriptsDir, client }) {
  ...
  return async ({ event }) => {
    ...
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      ...
      console.error(`[maestro] greeting: triggered (mode=${mode}, type=${event.type})`);
    }
    ...
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      console.error("[maestro] greeting: handler-completed");
    }
  };
}
