 ...
 *
 * CRUD operations for the pool credential file (~/.maestro/oauth-pool.json).
...
export function savePool(data) {
  try {
    ...
    console.error(`[maestro] OAuth pool: failed to save pool file: ${err.message}`);
  }
}
...
export function withPoolLock(fn) {
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) throw new Error("[maestro] OAuth pool: timed out waiting for pool lock");
  ...
  return withPoolLock(() => {
    ...
    if (account.email === "unknown") {
      ...
      if (namedAccounts.length > 0) {
        ...
        console.error(
          [
            "[maestro] OAuth pool: REFUSED to save account with unknown email.",
...
export function savePendingToken(provider, tokenData) {
  withPoolLock(() => {
    ...
    console.error(
      [
        `[maestro] OAuth pool: token saved to pending for ${provider}.`,
...
export function assignPendingToken(provider, email) {
  return withPoolLock(() => {
    ...
    savePool(pool);
    console.error(`[maestro] OAuth pool: assigned pending token to ${email}`);
    return true;
  });
}
