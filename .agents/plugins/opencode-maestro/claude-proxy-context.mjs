 ...
 *   - per-agent system prompt cache (~/.maestro/agents/<agent>.md)
...
/**
 * Load and cache the Maestro framework prompt (build.txt + AGENTS.md).
 ...
 * Files are read from ~/.maestro/agents/ (the deployed copy).
 */
export function getFrameworkPrompt() {
  if (_frameworkPromptCache !== null) return _frameworkPromptCache;

  const agentsDir = join(homedir(), ".maestro", "agents");
  const files = [
  ...
  if (_frameworkPromptCache) {
    console.error(
      `[maestro] Claude proxy: loaded framework prompt (${_frameworkPromptCache.length} chars from ${parts.length} files)`,
    );
  }
...
const AGENTS_DIR = join(homedir(), ".maestro", "agents");

/**
 * Map of known agent identifiers to their file names in ~/.maestro/agents/.
...
const MCP_DEFINITIONS = new Map([
  ...
  ["gsc", {
    command: "/bin/bash",
    args: ["-c", "GOOGLE_APPLICATION_CREDENTIALS=${GOOGLE_APPLICATION_CREDENTIALS:-~/.config/maestro/gsc-credentials.json} npx -y mcp-server-gsc"],
    type: "stdio",
  }],
  ["dataforseo", {
    command: "/bin/bash",
    args: ["-c", "source ~/.config/maestro/credentials.sh && DATAFORSEO_USERNAME=$DATAFORSEO_USERNAME DATAFORSEO_PASSWORD=$DATAFORSEO_PASSWORD npx -y dataforseo-mcp-server"],
    type: "stdio",
  }],
...
const MCP_CONFIG_DIR = join(homedir(), ".maestro", ".agent-workspace", "tmp");
...
export function getMcpConfigForAgent(agentName) {
  ...
  console.error(`[maestro] Claude proxy: generated MCP config for agent=${name} at ${configPath}`);
  return configPath;
}
