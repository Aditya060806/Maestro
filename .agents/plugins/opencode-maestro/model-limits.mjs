...
//
// model-limits.mjs — single source of truth for maestro-managed model
...
//   the cold-retrieval failure mode. The MAESTRO_OPUS_47_CONTEXT env var
//   is the opt-in.
//
 ...
 * Reads `process.env.MAESTRO_OPUS_47_CONTEXT` each call (so tests can mutate
...
export function resolveOpus47Context() {
  const raw = process.env.MAESTRO_OPUS_47_CONTEXT;
...
export const CLAUDE_MODEL_LIMITS = {
  ...
  // MAESTRO_OPUS_47_CONTEXT=<integer> if you understand the MRCR tradeoff.
...
if (_override) {
  if (_override.kind === "applied") {
    ...
    console.warn(
      `[maestro] MAESTRO_OPUS_47_CONTEXT=${_override.raw} — opus-4-7 context overridden to ${_override.resolved} ` +
    ...
    // eslint-disable-next-line no-console
    console.warn(
      `[maestro] MAESTRO_OPUS_47_CONTEXT=${_override.raw} exceeds the 1M API ceiling. ` +
      `Clamped to ${OPUS_47_CONTEXT_MAX}.`
    );
  } else {
    // eslint-disable-next-line no-console
    console.warn(
      `[maestro] MAESTRO_OPUS_47_CONTEXT=${_override.raw} is not a positive integer. ` +
