...
export const TOOL_PREFIX = "mcp__maestro__";
...
function sanitizeSystemPrompt(system) {
  return system.map((item) => {
    ...
    // ~/.maestro/.agent-workspace/work/maestro/third-party-name-test/.
    //
...
function redistributeSystemToMessages(parsed) {
  ...
  console.error(`[maestro] provider-auth: redistributed ${overflow.length} system blocks (${overflowText.length} chars) to user message to stay under third-party detection threshold`);
}
 ...
 * why plugins/opencode-maestro/intent-tracing.mjs sees empty args for every
...
// that don't match any known name get mcp__maestro__ prefixed as fallback.
// ---------------------------------------------------------------------------
...
function stripMcpPrefix(text) {
  ...
  // and strip mcp__maestro__ prefix from custom tools.
  text = text.replace(/"name"\s*:\s*"mcp__maestro__([^"]+)"/g, '"name":"$1"');
...
// so any "mcp__maestro__XYZ" token is always on one line. Buffering up to
