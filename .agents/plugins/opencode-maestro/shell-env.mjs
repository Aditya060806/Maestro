...
export function createShellEnvHook(deps) {
  ...
  /**
   * Inject maestro environment variables into shell sessions.
  ...
  return async function shellEnvHook(input, output) {
    // Ensure maestro scripts are on PATH
    ...
    // Set maestro workspace directory
    output.env.MAESTRO_AGENTS_DIR = agentsDir;
    output.env.MAESTRO_WORKSPACE_DIR = workspaceDir;
    output.env.MAESTRO_SESSION_ORIGIN = shellSessionOrigin(output.env);

    // Set maestro version if available. Prefer the deployed framework version
    // source; ~/.maestro/version is a legacy/stale compatibility fallback.
    const version = precomputedVersion ||
      (hasAgentsDir(agentsDir)
        ...
        : "");
    if (version) {
      output.env.MAESTRO_VERSION = version;
    }
    ...
    if (modelId && !output.env.MAESTRO_SIG_MODEL) {
      output.env.MAESTRO_SIG_MODEL = modelId;
    }
