...
async function refreshProviderToken(account, fetchFn, label) {
  try {
    ...
    console.error(`[maestro] OAuth pool: ${label} token refresh error for ${account.email}: ${err.message}`);
    return null;
  }
}
...
async function refreshCursorAccessToken(account) {
  try {
    ...
    console.error(`[maestro] OAuth pool: Cursor token refresh failed for ${account.email}`);
    return null;
  } catch (err) {
    console.error(`[maestro] OAuth pool: Cursor token refresh error for ${account.email}: ${err.message}`);
    return null;
  }
}
...
export function markAuthRefreshFailure(provider, account) {
  ...
  console.error(
    `[maestro] OAuth pool: ${provider} token refresh failed for ${account.email}; ` +
...
export function normalizeExpiredCooldowns(provider, accounts) {
  ...
  if (normalized > 0) {
    ...
    console.error(`[maestro] OAuth pool: auto-cleared ${normalized} expired cooldown(s) for ${provider}`);
  }
  return normalized;
}
