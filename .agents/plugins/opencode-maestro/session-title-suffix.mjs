...
export function readMaestroVersion(agentsDir) {
  const candidates = [
...
export function withMaestroTitleSuffix(title, version) {
  const baseTitle = String(title || "").replace(MAESTRO_TITLE_SUFFIX_RE, "");
  if (!version) return baseTitle;
  return `${baseTitle} · Maestro ${version}`;
}

function isDefaultSessionTitle(title) {
  const baseTitle = String(title || "").replace(MAESTRO_TITLE_SUFFIX_RE, "").trim();
...
export function createSessionTitleSuffixHandler({ agentsDir, client }) {
  ...
  return async function sessionTitleSuffixHandler(input) {
    ...
    const version = readMaestroVersion(agentsDir);
    const suffixedTitle = withMaestroTitleSuffix(update.title, version);
