...
function logStreamComplete(session, exitCode) {
  console.error(
    `[maestro] Claude proxy: stream complete model=${session.model} account=${session.account.email} exitCode=${exitCode} textChunks=${session.textChunkCount} textChars=${session.textCharCount} stderr=${JSON.stringify(session.stderrText.trim().slice(0, 300))}`,
  );
}
...
function onStreamClose(session, exitCode) {
  ...
  if (session.rateLimitBailed) {
    console.error(
      `[maestro] Claude proxy: killed rate-limited child account=${session.account.email} exitCode=${exitCode}`,
    );
    return;
  }
...
function tryStreamWithAccount(streamCtx, account) {
  return new Promise((resolve) => {
    ...
    session.timeout = setTimeout(() => {
      console.error(`[maestro] Claude proxy: stream child timeout (${CHILD_TIMEOUT_MS}ms), killing`);
      child.kill("SIGKILL");
    }, CHILD_TIMEOUT_MS);
...
async function iterateAccounts(streamCtx, accounts) {
  for (const account of accounts) {
    ...
    console.error(`[maestro] Claude proxy: trying account ${account.email} (stream mode)`);
    const result = await tryStreamWithAccount(streamCtx, account);
    if (result === "rate_limited") {
      console.error(`[maestro] Claude proxy: account ${account.email} rate-limited, trying next...`);
      continue;
    }
...
function handleStreamCancel(abortRef, reason) {
  ...
  try {
    child.kill("SIGTERM");
    console.error(`[maestro] Claude proxy: stream cancelled by client (${reason || "no-reason"}), killed child pid=${child.pid}`);
  } catch {
    // best effort
  }
}
