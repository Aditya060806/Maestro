// ---------------------------------------------------------------------------
// maestro OpenCode Plugin — Entry Point (t1914 decomposition)
//
...
const HOME = homedir();
const AGENTS_DIR = join(HOME, ".maestro", "agents");
const SCRIPTS_DIR = join(AGENTS_DIR, "scripts");
const PLUGIN_DIR = join(AGENTS_DIR, "plugins", "opencode-maestro");
const WORKSPACE_DIR = join(HOME, ".maestro", ".agent-workspace");
const LOGS_DIR = join(HOME, ".maestro", "logs");
...
// ---------------------------------------------------------------------------
// Plugin logging — informational messages gated behind MAESTRO_PLUGIN_DEBUG
// to avoid stderr text overlapping OpenCode's TUI (GH#TBD).
// Actual errors always use console.error directly.
// ---------------------------------------------------------------------------

/**
 * Plugin stderr suppression — prevents [maestro] informational messages
 * from rendering over OpenCode's TUI input area. Only actual errors pass
 * through. Set MAESTRO_PLUGIN_DEBUG=1 to see all messages.
 *
 * This wraps console.error at the process level so ALL plugin modules
 * benefit without individual imports.
 */
const PLUGIN_DEBUG = !!process.env.MAESTRO_PLUGIN_DEBUG;
if (!PLUGIN_DEBUG) {
  const _origConsoleError = console.error;
  console.error = (...args) => {
    // Let actual errors through (stack traces, "failed", "error" in message)
    const msg = typeof args[0] === "string" ? args[0] : "";
    if (msg.startsWith("[maestro]") && !(/fail|error|warn|disabled/i.test(msg))) {
      return; // suppress informational [maestro] messages
    }
 ...
 */
export async function MaestroPlugin({ directory, client }) {
  ...
  const prepareOptionalProxy = (label, prepare) => {
    prepare()
      .catch((err) => {
        console.error(`[maestro] ${label} proxy failed to register: ${err.message}`);
      });
  };
  ...
  if (cursorAccounts.length > 0) {
    prepareOptionalProxy("Cursor gRPC", async () => {
      ...
      if (cursorProxyResult) {
        console.error(`[maestro] Cursor gRPC proxy registered on port ${cursorProxyResult.port} with ${cursorProxyResult.models.length} models (listener lazy)`);
      }
    });
  }
  ...
  if (googleAccounts.length > 0) {
    ...
    prepareOptionalProxy("Google", async () => {
      ...
      if (googleProxyResult) {
        console.error(`[maestro] Google proxy registered on port ${googleProxyResult.port} with ${googleProxyResult.models.length} models (listener lazy)`);
      }
    });
  }
    ...
    if (providerID && !isHeadless()) {
      ...
      if (starter) {
        try {
          ...
          console.error(`[maestro] ${providerID} proxy lazy-start failed: ${err.message}`);
        }
      }
    }
  ...
  const debugEventError = (label, err) => {
    if (process.env.MAESTRO_PLUGIN_DEBUG) {
      console.error(`[maestro] ${label} error:`, err);
    }
  };
