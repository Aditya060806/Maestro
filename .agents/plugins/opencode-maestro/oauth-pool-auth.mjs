...
async function resolveAnthropicEmail(accessToken) {
  for (const endpoint of [
    ...
    if (email) {
      console.error(`[maestro] OAuth pool: resolved email ${email} from ${endpoint}`);
      return email;
    }
  }
  console.error("[maestro] OAuth pool: could not resolve email from profile API");
  return null;
}
...
async function handleAnthropicCallback(code, pkce, expectedState, email, client) {
  ...
  if (returnedState !== undefined && returnedState !== expectedState) {
    console.error("[maestro] OAuth pool: Anthropic state mismatch — possible CSRF");
    return { type: "failed" };
  }
...
async function handleOpenAICallback(code, pkce, email, callbackState, client) {
  ...
  if (json.access_token) {
    ...
    if (resolvedEmail === "unknown") {
      ...
      if (resolvedEmail !== "unknown") console.error(`[maestro] OAuth pool: resolved OpenAI email ${resolvedEmail}`);
    }
  }
  return saveAccountAndInject({
...
async function handleCursorAuthorize(email, client) {
  ...
  if (creds) console.error("[maestro] OAuth pool: found Cursor credentials in auth.json");
  if (!creds) {
    creds = readCursorStateDbCredentials(email);
    if (creds) console.error("[maestro] OAuth pool: found Cursor credentials in state DB");
  }
  if (!creds) {
    if (!isCursorAgentAvailable()) {
      console.error("[maestro] OAuth pool: cursor-agent not found");
      return { type: "failed" };
    }
    console.error("[maestro] OAuth pool: running cursor-agent login...");
    try {
      ...
      console.error(`[maestro] OAuth pool: cursor-agent login failed: ${err.message}`);
      return { type: "failed" };
    }
  }
  if (!creds) {
    console.error("[maestro] OAuth pool: no Cursor access token obtained");
    return { type: "failed" };
  }
...
async function handleGoogleCallback(code, pkce, expectedState, email, client) {
  ...
  if (json.id_token && resolvedEmail === "unknown") {
    ...
    if (resolvedEmail !== "unknown") console.error(`[maestro] OAuth pool: resolved Google email ${resolvedEmail} from ID token`);
  }
  if (resolvedEmail === "unknown" && json.access_token) {
    resolvedEmail = (await resolveEmailFromEndpoint(json.access_token, "https://www.googleapis.com/oauth2/v3/userinfo")) || "unknown";
    if (resolvedEmail !== "unknown") console.error(`[maestro] OAuth pool: resolved Google email ${resolvedEmail}`);
  }
  return saveAccountAndInject({
