...
/**
 * Shared model definition template for Claude models managed by maestro.
 ...
 * (MAESTRO_OPUS_47_CONTEXT) and the MRCR rationale.
...
/** Models registered under the built-in anthropic provider (via maestro OAuth pool). */
const ANTHROPIC_MODELS = buildClaudeModelMap({
  "claude-haiku-4-5":  "Claude Haiku 4.5 (via maestro)",
  "claude-sonnet-4-5": "Claude Sonnet 4.5 (via maestro)",
  "claude-sonnet-4-6": "Claude Sonnet 4.6 (via maestro)",
  "claude-opus-4-5":   "Claude Opus 4.5 (via maestro)",
  "claude-opus-4-6":   "Claude Opus 4.6 (via maestro)",
  "claude-opus-4-7":   "Claude Opus 4.7 (via maestro)",
});
...
/**
 * Upsert maestro-managed models into the anthropic and claudecli providers.
...
function registerAnthropicModels(config) {
  ...
  // anthropic provider — via maestro OAuth pool
  ...
  if (!config.provider.claudecli) {
    ...
    config.provider.claudecli.name === "Claude CLI (via maestro proxy)"
  ) {
...
async function discoverAndRegisterModels(opts) {
  ...
  try {
    ...
    console.error(`[maestro] Config hook: ${provider} model registration failed: ${err.message}`);
  }
  return 0;
}
...
function logConfigSummary(counts) {
  ...
  if (parts.length > 0) {
    console.error(`[maestro] Config hook: ${parts.join(", ")}`);
  }
}
...
function logVersionDriftAsync(pluginDir) {
  checkOpenCodeVersionDriftAsync(pluginDir, (versionDrift) => {
    console.error(`[maestro] Version drift: ${versionDrift}`);
  });
}
...
export function createConfigHook(deps) {
  ...
  /**
   * Modify OpenCode config to register maestro subagents, MCP servers,
