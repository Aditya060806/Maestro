...
const HOME = homedir();
const DEFAULT_OBS_DIR = join(HOME, ".maestro", ".agent-workspace", "observability");
// MAESTRO_OBS_DB_OVERRIDE lets tests redirect to a temp DB without touching
...
const DB_PATH = process.env.MAESTRO_OBS_DB_OVERRIDE || join(DEFAULT_OBS_DIR, "llm-requests.db");
...
function loadPricingFromJSON() {
  // Resolve relative to this file's location (works in both dev repo and deployed ~/.maestro/)
  const thisDir = dirname(fileURLToPath(import.meta.url));
  const candidates = [
    join(thisDir, "..", "..", "configs", "model-pricing.json"),          // repo: .agents/plugins/../../configs/
    join(HOME, ".maestro", "agents", "configs", "model-pricing.json"), // deployed
  ];
  ...
  console.error("[maestro] Observability: model-pricing.json not found, using hardcoded fallback");
...
function initDatabase() {
  try {
    ...
    console.error("[maestro] Failed to create observability directory");
    return false;
  }
  ...
  try {
    ...
    console.error("[maestro] sqlite3 not found — observability disabled");
    return false;
  }
...
`;
  ...
  if (result === null) {
    console.error("[maestro] Observability: schema creation failed");
    return false;
  }
  return true;
}
 ...
 *
 * Lock is per-DB-path so the test suite (which sets `MAESTRO_OBS_DB_OVERRIDE`)
...
function _withInitLock(fn) {
  ...
  if (!_acquireInitLock(LOCK_DIR, OWNER_FILE, STALE_MS, deadline)) {
    console.error("[maestro] Observability: init lock timeout — proceeding without lock");
    return fn();
  }
  ...
  const timer = setTimeout(() => {
    try {
      ...
      if (process.env.MAESTRO_PLUGIN_DEBUG) {
        console.error(`[maestro] Observability: delayed cost backfill failed: ${err.message}`);
      }
    }
  }, 2500);
  timer.unref?.();
...
`;
  ...
  if (parseInt(count, 10) > 0) {
    console.error(`[maestro] Observability: backfilled cost for ${count} rows`);
...
GROUP BY session_id;
    `, 30000);
    console.error("[maestro] Observability: rebuilt session_summaries with corrected costs");
  }
...
export function initObservability() {
  ...
  if (dbReady) {
    console.error("[maestro] Observability: SQLite DB ready at " + DB_PATH);
