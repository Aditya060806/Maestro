...
function buildCorrectionMessage(violations, sessionID) {
  ...
  const correctionText = [
    "[maestro TTSR] Rule violations detected in recent output:",
    ...corrections, "",
...
function buildSessionStartGreetingInstruction(agentsDir, readIfExists) {
  ...
  const cacheMatch = cacheLine.match(/^maestro v(\S+) running in (.+?) v(\S+)(?:\s|$)/);
  ...
  const versionLine = runtimeVersion
    ? `We're running maestro v${version} in ${runtime} v${runtimeVersion}.`
    : `We're running maestro v${version}.`;

  return [
    "## Session-start greeting order",
    "On the first assistant turn of an interactive session, the first visible text in the assistant response MUST be this exact maestro greeting:",
    "",
    "Hi!",
    "",
    versionLine,
...
async function ttsrSystemTransform(input, output, state, intentField, isHeadless, agentsDir, readIfExists) {
  ...
  output.system.push([
    "## maestro Quality Rules (enforced)",
