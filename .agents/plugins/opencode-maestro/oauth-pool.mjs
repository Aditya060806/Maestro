 ...
 * (~/.maestro/oauth-pool.json) to avoid conflicts with OpenCode's auth.json.
 *
 * Architecture:
...
async function selectPoolAccount(provider, skipEmail) {
  ...
  for (const c of sorted) {
    ...
    console.error(`[maestro] OAuth pool: skipping invalid ${provider} token for ${c.email}`);
  }
...
export async function injectPoolToken(client, skipEmail) {
  ...
  console.error(`[maestro] OAuth pool: injected token for ${account.email} into built-in anthropic provider`);
  return true;
}
...
export async function applyOpenAIPoolAccount(client, account) {
  ...
  console.error(`[maestro] OAuth pool: injected token for ${account.email} into built-in openai provider`);
  return account;
}
...
export async function ensureCursorProxy(client) {
  ...
  try {
    ...
    if (result && result.port) {
      ...
      console.error(`[maestro] OAuth pool: cursor gRPC proxy running on port ${result.port}`);
      return cursorProxy.baseURL;
    }
    throw new Error("gRPC proxy returned no port");
  } catch (err) {
    console.error(`[maestro] OAuth pool: cursor gRPC proxy failed: ${err.message}`);
    throw err;
  }
}
...
export function stopCursorProxy() {
  if (cursorProxy.port) {
    ...
    console.error("[maestro] OAuth pool: cursor proxy stopped");
  }
}
...
export async function injectCursorPoolToken(client, skipEmail) {
  ...
  try {
    ...
    console.error(`[maestro] OAuth pool: cursor proxy failed to start: ${err.message}`);
  }
  ...
  try {
    ...
    console.error(`[maestro] OAuth pool: injected Cursor token for ${account.email}`);
    return true;
  } catch (err) {
    console.error(`[maestro] OAuth pool: failed to inject Cursor token: ${err.message}`);
    return false;
  }
}
...
export async function injectGooglePoolToken(client, skipEmail) {
  ...
  console.error(`[maestro] OAuth pool: injected Google token for ${account.email} as GOOGLE_OAUTH_ACCESS_TOKEN`);
  return true;
}
...
export async function initPoolAuth(client) {
  ...
  try { await injectGooglePoolToken(client); } catch (err) {
    console.error(`[maestro] OAuth pool: Google token injection failed (isolated): ${err.message}`);
  }
}
