...
```json
{"ts":"2026-04-26T00:00:00Z","kind":"open","actor":"Aditya Pandey and Harvest","content":"Case opened: case-2026-0001-acme-dispute","ref":""}
{"ts":"2026-04-26T10:00:00Z","kind":"attach","actor":"Aditya Pandey and Harvest","content":"Attached source: src-001 (role: evidence)","ref":"src-001"}
{"ts":"2026-04-26T11:00:00Z","kind":"status_change","actor":"Aditya Pandey and Harvest","content":"Status changed: open → hold. Reason: awaiting client","ref":""}
{"ts":"2026-04-26T12:00:00Z","kind":"note","actor":"Aditya Pandey and Harvest","content":"Reviewed contract terms","ref":"notes/notes.md"}
{"ts":"2026-04-26T13:00:00Z","kind":"comm","actor":"Aditya Pandey and Harvest","content":"Comm logged: in via email — Received settlement offer","ref":"comms/comms.log"}
{"ts":"2026-04-26T14:00:00Z","kind":"deadline","actor":"Aditya Pandey and Harvest","content":"Deadline added: filing deadline on 2026-08-31","ref":""}
{"ts":"2026-04-26T15:00:00Z","kind":"party","actor":"Aditya Pandey and Harvest","content":"Party added: Opposing Counsel (opponent)","ref":""}
{"ts":"2026-04-27T09:00:00Z","kind":"status_change","actor":"Aditya Pandey and Harvest","content":"Case closed. Outcome: settled. Agreed in mediation","ref":""}
{"ts":"2026-04-27T09:01:00Z","kind":"archive","actor":"Aditya Pandey and Harvest","content":"Case archived","ref":""}
```
...
```json
[
  {"id": "src-001", "attached_at": "2026-04-26T10:00:00Z", "attached_by": "Aditya Pandey and Harvest", "role": "evidence"},
  {"id": "src-002", "attached_at": "2026-04-26T10:01:00Z", "attached_by": "Aditya Pandey and Harvest", "role": "background"}
]
```
...
Provided by `case-helper.sh`. Route via `maestro case <subcommand>`.

### Provisioning

```bash
maestro case init [<repo-path>]
```
...
```bash
maestro case open <slug> [--kind <type>] [--party <name>] [--party-role <role>] \
...
```bash
maestro case list [--status open|hold|closed|archived|all] [--kind <type>] [--party <name>] [--json]
maestro case show <case-id> [--json]
```

### Lifecycle operations

```bash
maestro case status <case-id> <open|hold> [--reason "..."] [--json]
maestro case close <case-id> --outcome <outcome> [--summary "..."] [--json]
maestro case archive <case-id> [--json]
```
...
```bash
maestro case attach <case-id> <source-id> [--role evidence|reference|background] [--json]
```
...
```bash
maestro case note <case-id> --message "Internal context here" [--json]
```
...
```bash
maestro case deadline add    <case-id> --date 2026-08-31 --label "filing deadline" [--json]
maestro case deadline remove <case-id> --label "filing deadline" [--json]
```

### Parties

```bash
maestro case party add    <case-id> --name "Opposing Counsel" --role "opponent" [--json]
maestro case party remove <case-id> --name "Opposing Counsel" [--json]
```

### Communications log

```bash
maestro case comm log <case-id> --direction in|out --channel email --summary "Received offer" [--json]
```
...
- **ntfy** — POST to the configured ntfy topic via `curl`. Server configurable via `MAESTRO_NTFY_SERVER` env var (default: `https://ntfy.sh`).
...
maestro case alarm-test <case-id> [<repo-path>]
...
```json
{
  ...
  "ntfy_topic":  "maestro-case-alarms",
  "per_case_overrides": {
...
Set `chasers_enabled: true` in `dossier.toon` before using `maestro case chase`. This is a deliberate opt-in: cases in active dispute or sensitive negotiation should not be auto-chased.
...
maestro case chase <case-id> --template payment-reminder

# Dry-run: show substituted email without sending
maestro case chase <case-id> --template payment-reminder --dry-run

# Template management
maestro case chase-template list
maestro case chase-template test --case <case-id> --template payment-reminder
maestro case chase-template add my-custom-template
```
...
- **First failure:** logged with `status: error`, `retry_allowed: true`. Manual retry via `maestro case chase retry <case-id> <message-id>`.
- **Second consecutive failure:** case status set to `hold` + alarm fired via `case-alarm-helper.sh fire` (if available).

### SMTP credentials

SMTP host/port resolved from `_config/mailboxes.json` (per-repo) or `~/.config/maestro/mailboxes.json` (global). Provider SMTP settings auto-detected from `email-providers.json.txt` using the mailbox `provider` field.

Credentials fetched at send-time from `gopass` (via `password_ref: gopass:maestro/email/<id>/password`). Never stored in logs or output.

## Dependencies

- **Provisioned by:** `maestro case init`
...
- **Filter→case-attach:** uses `maestro case attach` (P5c)
