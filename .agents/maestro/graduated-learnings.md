...
- Mailbox uses SQLite (`mailbox.db`) not TOON files. Prune shows storage report by default; `--force` to delete. Migration from TOON runs automatically on `maestro update` *(medium, 8x)*
...
- OpenCode: `prompt` field in `opencode.json` replaces (not appends) `anthropic_default`. All active agents must have `build.txt` set or fall back to upstream `anthropic.txt`, losing maestro overrides *(high, 1x)*
...
- [task:feature] t136.5: Scaffold maestro-pro/anon repos | PR #792 | [model:opus] [duration:1206s] *(medium, 51x)*
...
- CRITICAL: Cron pulse on macOS needs (1) `/usr/sbin` in PATH, (2) `GH_TOKEN` cached to file (keyring inaccessible), (3) `get_maestro_identity` validates `gh api` output. Fixed PR #780 *(medium, 52x)*
- SYSTEMIC: Deployed scripts at `~/.maestro/agents/scripts/` NOT auto-updated after merging. Run `maestro update` after script changes. *(medium, 37x)*
