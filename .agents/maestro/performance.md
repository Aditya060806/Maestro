...
maestro-managed work: campaign results, case outcomes, project delivery metrics,
...
`~/.maestro/.agent-workspace/performance/reach-capture.jsonl` (or
`MAESTRO_REACH_PERFORMANCE_LOG` in tests). Each line is one JSON object so
...
- `maestro performance ...` CLI commands.
