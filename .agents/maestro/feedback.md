...
| Need | Read | Contains |
|------|------|----------|
| Capturing or classifying raw feedback | `.agents/maestro/feedback/capture-retention.md` | Required capture metadata, examples, sensitivity tiers, retention outcomes, consent/provenance constraints, placeholder examples. |
| Mining feedback or promoting themes | `.agents/maestro/feedback/mining-promotion.md` | Cold-start mining loop, evidence thresholds, review gates, destination-specific promotion paths, no-promote/retire outcomes, provenance/privacy rules. |
| Designing commands or recurring operations | `.agents/maestro/feedback/cli-routines.md` | `maestro feedback` command contract, capture/list/mine/promote/retire IO, routine examples, implementation boundary. |
...
`.agents/maestro/feedback/mining-promotion.md` under "Reach Feedback
Thresholds".

## Implementation Boundary

This is design-only. Do not wire `maestro feedback` into
`.agents/scripts/maestro.sh` until the phase contracts in the chapters are
stable. The first implementation task should create `.agents/scripts/feedback-helper.sh`
and have `maestro.sh` delegate to it, mirroring other plane helpers rather than
