...
```bash
maestro secret set email/personal-icloud/password
# or directly: gopass insert maestro/email/personal-icloud/password
```

**2. Register the mailbox interactively:**

```bash
maestro email mailbox add
```
...
```bash
maestro email mailbox list       # shows all registered mailboxes + state
maestro email mailbox test <id>  # dry-run: connect + fetch 1 message, no writes
```
...
~/.maestro/agents/scripts/email-poll-helper.sh tick

# Back-fill historical messages from 2026-01-01 (rate-limited to 100 msg/min)
~/.maestro/agents/scripts/email-poll-helper.sh backfill <mailbox-id>     --since 2026-01-01     --rate-limit 100

# Show all mailboxes and their last-polled timestamps
~/.maestro/agents/scripts/email-poll-helper.sh list
```
  ...
  "mailboxes": [
    {
      ...
      "port": 993,
      "user": "you@icloud.com",
      "password_ref": "gopass:maestro/email/personal-icloud/password",
...
```bash
maestro email build   [knowledge-root] [--force]        # Rebuild thread index
maestro email thread  <message-id> [knowledge-root]     # Look up thread by message-id
```
...
```bash
maestro email filter tick   [knowledge-root]             # Run filter pass (called by r045)
maestro email filter list   [knowledge-root]             # List rules with match summaries
maestro email filter add    [knowledge-root]             # Interactive rule builder
maestro email filter test   <rule-name> [knowledge-root] # Dry-run against last 50 sources
```
