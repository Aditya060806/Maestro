...
| `local` | No-op — operations logged to `~/.maestro/logs/platform-local-ops.log` |

### Usage

```bash
# Source and use directly
source ~/.maestro/agents/scripts/platform-helper.sh
...
Per-day per-provider costs are accumulated at `~/.maestro/.agent-workspace/llm-costs.json`:
...
`~/.maestro/configs/ollama-bundle.json`). Three tiers:
