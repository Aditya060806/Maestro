...
- Sync target: `~/.maestro/agents/custom/<source-name>/`
- Survives framework updates and is never overwritten by `setup.sh`
- Sync runs on `maestro update`, `./setup.sh` after `deploy_maestro_agents`, or `maestro sources sync`
- `mode: primary` docs symlink into `~/.maestro/agents/`
...
2. Mark the repo in `~/.config/maestro/repos.json` with `"agent_source": true`.
3. Run `maestro init` in the repo to seed root `AGENTS.md` and the core-style `.agents/` skeleton.
4. Register the repo with `maestro sources add <path>`.
5. Run `maestro update` or `maestro sources sync`.
6. Synced files land in `~/.maestro/agents/custom/<source-name>/`.
...
```bash
maestro sources add ~/Git/my-private-agents                  # Add local repo
maestro sources add-remote git@github.com:user/agents.git    # Clone and add
maestro sources list                                         # List sources
maestro sources status                                       # Path, agent count, git state
maestro sources sync                                         # Pull, rsync, symlink
maestro sources remove my-private-agents                     # Remove source, keep files on disk
```
...
|-------------|----------------|---------------|
| `mode: primary` | Primary agent | Sync and symlink to `~/.maestro/agents/` root |
...
Both layouts may coexist in one source repo. `maestro sources sync` preserves
...
2. Add the repo to `~/.config/maestro/repos.json` with `"agent_source": true` or compatibility form `"role": "agent-source"`.
3. Run `maestro init`; missing root `AGENTS.md`, `.agents/AGENTS.md`, and core-style skeleton directories are seeded from `.agents/templates/agent-source-repo/`.
...
7. Use `mode: primary` when the agent should auto-discover in `~/.maestro/agents/`.
...
- **Detection:** `maestro init` and `maestro update` treat a repo as an agent source when its project `.maestro.json` or managed `repos.json` entry has `"agent_source": true`; `"role": "agent-source"` is accepted for older configs.
- **Initial seed:** `maestro init` creates root `AGENTS.md`, `.agents/AGENTS.md`, and the standard `.agents/` directory skeleton when they are missing.
- **Update propagation:** `maestro update` traverses registered repos from `repos.json` and refreshes framework-owned template blocks in every agent-source repo.
- **Non-destructive rule:** only blocks marked with `<!-- maestro:agent-source-template:start -->` and `<!-- maestro:agent-source-template:end -->` are refreshed. User-authored files or files without those markers are left untouched.
...
single manifest and writes `~/.maestro/agents/agent-source-capabilities.toon`.
...
| `upstream_candidate`, `core_candidate` | Whether the pack may graduate into shared maestro |
...
```json
{
  ...
  "artifact_path": "~/.maestro/.agent-workspace/work/<pack-name>/notes/",
  "sensitivity": "private-local",
  "allowed_destinations": ["local-workspace"]
}
```

Use `~/.maestro/.agent-workspace/work/<pack-name>/` for private artifacts that
must survive the session. Use `~/.maestro/.agent-workspace/tmp/session-*` for
...
|---------|---------------|---------|
| Config | `agent-sources.json` | `.maestro.json` per project |
...
| Sync trigger | `maestro update` / `sources sync` | `maestro plugin update` |

## Configuration

Source metadata lives in `~/.maestro/agents/configs/agent-sources.json` with: `name`, `local_path`, `remote_url`, `added_at`, `last_synced`, and `agent_count`.

Capability metadata lives in `~/.maestro/agents/agent-source-capabilities.toon`
