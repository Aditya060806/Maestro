...
- **Script**: `~/.maestro/agents/scripts/onboarding-helper.sh`
- **Settings**: `~/.config/maestro/settings.json` — `settings-helper.sh list|set|reset`
- **Credentials**: `~/.config/maestro/credentials.sh` (600 perms) | `configs/*-config.json` (600, gitignored) | `~/.config/coderabbit/api_key` (600)
...
Discovery prompts: `/skills recommend "TASK"`, `/skills browse CATEGORY`, or ask "what maestro capability should handle this?".
...
- Auth tokens stored in `~/.config/maestro/credentials.sh` or `gopass` (one set of values, used everywhere)
...
~/.maestro/agents/scripts/list-keys-helper.sh
```
...
```bash
# Key not loading
grep "credentials.sh" ~/.zshrc ~/.bashrc && source ~/.config/maestro/credentials.sh
# MCP not connecting
opencode mcp list && ~/.maestro/agents/scripts/mcp-diagnose.sh <name>
# Permission denied
chmod 600 ~/.config/maestro/credentials.sh && chmod 700 ~/.config/maestro
```

## Agents & Commands

- **Layers**: Main agents (Tab) → subagents (`@name`) → commands (`/name`)
- **Main**: `Build+`, `Automate`, `Maestro`, `Business`, `Content`, `Health`, `Legal`, `Marketing-Sales`, `Product`, `Research`, `SEO` | **Init**: `cd ~/your-project && maestro init`
...
mkdir -p ~/.config/maestro && { [ -s ~/.config/maestro/repos.json ] || echo '{}' > ~/.config/maestro/repos.json; } && \
  tmp_file="$(mktemp "${TMPDIR:-/tmp}/repos.XXXXXX")" && \
  jq --argjson dirs '["~/Git", "~/Projects"]' '. + {git_parent_dirs: $dirs}' \
  ~/.config/maestro/repos.json > "$tmp_file" && mv "$tmp_file" ~/.config/maestro/repos.json
maestro repo-sync enable
maestro config set orchestration.supervisor_pulse true && ./setup.sh  # Enable autonomous orchestration
...
1. **Playground**: `mkdir ~/Git/maestro-playground && cd $_ && git init && maestro init`
