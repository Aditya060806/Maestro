...
maestro ships a consistent badge block for every managed repo: a native
...
Use `maestro metrics generate` for the current repo, or `maestro badges
...
~/.maestro/agents/scripts/repo-metrics-helper.sh generate \
   --legacy-badge-dir .github/badges

# 2. Drop in the repo metrics refresh workflow caller
cp ~/.maestro/agents/templates/workflows/loc-badge-caller.yml \
...
~/.maestro/agents/scripts/readme-badges-helper.sh inject README.md owner/repo
git add README.md
...
```markdown
<!-- maestro:badges:start -->
<!-- managed by maestro badges; edit the template, not this block -->
[![GitHub Actions](...)](...)
[![License](...)](...)
...
.agents/scripts/repo-metrics-helper.sh generate \
  --output-dir /tmp/maestro-metrics \
  --badge-dir /tmp/maestro-metrics/badges
ls /tmp/maestro-metrics/      # repo-metrics.json + repo-metrics.md + badges/
open /tmp/maestro-metrics/badges/languages.svg
...
```bash
.agents/scripts/readme-badges-helper.sh render Aditya060806/Maestro
.agents/scripts/readme-badges-helper.sh check README.md Aditya060806/Maestro
```
...
- Phase 2 issue (filed at end of this PR) — `maestro badges` CLI + `init` hook
