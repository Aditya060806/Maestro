...
~/.maestro/agents/scripts/mcp-diagnose.sh check-all
...
| "Permission denied" | Missing credentials | Check `~/.config/maestro/credentials.sh` |
...
```bash
~/.maestro/agents/scripts/mcp-diagnose.sh check-all   # scan all servers (t1682)
~/.maestro/agents/scripts/mcp-diagnose.sh <mcp-name>  # diagnose specific MCP
~/.maestro/agents/scripts/tool-version-check.sh        # check versions
~/.maestro/agents/scripts/tool-version-check.sh --update  # update outdated tools
...
- [add-new-mcp-to-maestro.md](add-new-mcp-to-maestro.md) — MCP setup workflow
