---
description: Advanced MCP integrations for Maestro
mode: subagent
tools:
  read: true
  write: true
...
# Advanced MCP Integrations for Maestro
...
| Development | Claude Code MCP | Forked server (`github:Aditya060806/claude-code-mcp`) |
...
```bash
claude mcp add claude-code-mcp "npx -y github:Aditya060806/claude-code-mcp"
```
...
```bash
# Store in ~/.config/maestro/credentials.sh:
...
```json
{
  "fluentcrm": {
    "type": "local",
    "command": ["/bin/bash", "-c", "source ~/.config/maestro/credentials.sh && node ~/.local/share/mcp-servers/fluentcrm-mcp-server/dist/fluentcrm-mcp-server.js"],
    "enabled": false
  }
}
```
...
```json
{
  "unstract": {
    "type": "local",
    "command": ["/bin/bash", "-c", "source ~/.config/maestro/credentials.sh && docker run -i --rm -v /tmp:/tmp -e UNSTRACT_API_KEY -e API_BASE_URL -e DISABLE_TELEMETRY=true unstract/mcp-server:${UNSTRACT_IMAGE_TAG:-latest} unstract"],
    "enabled": false
  }
}
```
...
- [Troubleshooting](.agents/maestro/mcp-troubleshooting.md)
