...
Detailed contract chapter for `.agents/maestro/feedback.md`.

## CLI Design

The public surface is `maestro feedback <command>`. Phase 5 defines the
...
maestro feedback capture --source <uri-or-label> --channel <channel> --text <text>
maestro feedback capture --file path/to/note.md --source <uri-or-label> --channel <channel>

# List retained captures and mined themes
maestro feedback list
maestro feedback list --state captured|mined|promoted|retired --sensitivity public|internal|client|privileged --since 2026-05-01

# Mine captures into candidate themes with evidence thresholds
maestro feedback mine
maestro feedback mine --since 30d --segment <segment> --min-evidence 3 --dry-run

# Promote a reviewed capture or theme into another plane
maestro feedback promote <feedback-id|theme-id> --to knowledge|campaign|project|case|performance|task --reason <why>

# Retire sensitive, obsolete, duplicate, or consent-expired feedback
maestro feedback retire <feedback-id|theme-id> --reason <why> [--delete-raw]
```
...
This document is design-only. Do not wire `maestro feedback` into
`.agents/scripts/maestro.sh` until the earlier phase contracts are stable:
...
`.agents/scripts/feedback-helper.sh` and have `maestro.sh` delegate to it,
