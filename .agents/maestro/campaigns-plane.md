...
**Provision:** `maestro campaign init`
**Repair:** `maestro campaign provision` is idempotent — safe to re-run.
...
Provisioning commands: `campaigns-provision-helper.sh`. Route via `maestro campaign <subcommand>`.

```bash
# Provision _campaigns/ in current repo
maestro campaign init [<repo-path>]

# Re-provision / repair (idempotent)
maestro campaign provision [<repo-path>]

# Show provisioning state and campaign counts
maestro campaign status [<repo-path>]

# Show one campaign's detailed lifecycle dossier
maestro campaign status <campaign-id> [--repo <repo-path>]

# List campaigns (active + launched)
maestro campaign ls [--active|--launched|--all] [<repo-path>]
```
...
maestro campaign asset add <file> [--target lib-brand|lib-swipe|campaign]
                                   [--campaign <id>] [--sensitivity <tier>]
                                   [--no-preview] [--repo <path>]

# Generate a 640px-wide PNG preview thumbnail for AI review
maestro campaign asset preview <file> [--size <px>] [--output <path>]

# List all ingested assets from the manifest
maestro campaign asset list [--type image|video|audio|pdf|all] [--campaign <id>]

# Show full JSON manifest entry for an asset
maestro campaign asset manifest <asset-id>
```
...
Large files (>=30MB) are stored in `~/.maestro/.agent-workspace/knowledge-blobs/<repo>/campaigns/<asset-id>/`
