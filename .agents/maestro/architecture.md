---
description: Maestro framework architecture context
mode: subagent
tools:
  read: true
  write: false
...
- **Extension**: See `.agents/maestro/extension.md`
...
Key integrations: agents via `generate-opencode-agents.sh`, 41 slash commands, compaction plugin at `.agents/plugins/opencode-maestro/`, system prompt at `.agents/prompts/build.txt`, native tools at `.opencode/tool/*.ts`.
...
maestro replaced a 37,000-line deterministic bash supervisor with a simple pattern: an AI agent reads guidance docs, fetches live state from GitHub, reasons, and acts. When the agent errs, fix the guidance — not a new script.
...
- **Specialist subagents**: `@maestro` for framework ops, `@plan-plus` for planning-only

## Agent Design Patterns

Implements proven patterns from Lance Martin (LangChain), validated across Claude Code, Manus, and Cursor.

| Pattern | maestro Implementation |
|---------|------------------------|
| **Give Agents a Computer** | `~/.maestro/.agent-workspace/`, helper scripts, bash tools |
...
**Adding a new MCP requires two files** (both in `.agents/plugins/opencode-maestro/`):
...
Full guide: `.agents/maestro/extension.md`. Naming conventions: `tools/build-agent/build-agent.md`.
...
- **Public entrypoints:** user-facing commands and repo metadata such as `setup.sh`, `maestro.sh`, `README.md`, `LICENSE`, `VERSION`, and governance docs.
