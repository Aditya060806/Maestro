...
- **Server**: `~/.maestro/dev-browser/server.sh` (port 9222)
- **Profile**: `~/.maestro/dev-browser/skills/dev-browser/profiles/browser-data/`
- **Install**: `bash ~/.maestro/agents/scripts/dev-browser-helper.sh setup`
...
```bash
bash ~/.maestro/agents/scripts/dev-browser-helper.sh setup          # Install (Bun + deps)
bash ~/.maestro/agents/scripts/dev-browser-helper.sh start          # Start (reuses profile)
bash ~/.maestro/agents/scripts/dev-browser-helper.sh start-headless # Start headless
bash ~/.maestro/agents/scripts/dev-browser-helper.sh start-clean    # Start with fresh profile
bash ~/.maestro/agents/scripts/dev-browser-helper.sh status         # Check status
bash ~/.maestro/agents/scripts/dev-browser-helper.sh profile        # View profile details
bash ~/.maestro/agents/scripts/dev-browser-helper.sh reset-profile  # Delete all browser data
bash ~/.maestro/agents/scripts/dev-browser-helper.sh stop           # Stop server
```

## Usage Pattern

Start the server, then execute scripts via `bun x tsx` heredoc. All scripts run from `~/.maestro/dev-browser/skills/dev-browser`.

**Script template** (every script follows this structure):

```bash
cd ~/.maestro/dev-browser/skills/dev-browser && bun x tsx <<'EOF'
...
BROWSER_EXECUTABLE="/Applications/Brave Browser.app/Contents/MacOS/Brave Browser" \
  bash ~/.maestro/agents/scripts/dev-browser-helper.sh start
```
...
|---------|-------------|--------------------|------------|----------------|-----------|
| **State** | Persistent maestro profile | Existing live browser profile | Per-tab | Fresh each call | Fresh |
...
Use `chromium-debug-use` when you need to understand a live session that is already open. Switch to dev-browser when that investigation turns into a repeatable workflow that should keep its own persistent state under maestro control.
