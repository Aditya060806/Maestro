...
- **Promoted private agents**: `~/.maestro/agents/custom/auto-browse/` after user review.
...
Use maestro' existing browser, crawler, profile, and data-extraction tools to find the cheapest reliable path for a web task, then graduate the result into a reusable artifact.
...
3. **Private by default for custom workflows** — account-specific agents, traces, downloads, cookies, profiles, and client notes stay under private maestro user data.
