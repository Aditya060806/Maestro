...
| Tool | Scope | Setup |
|------|-------|-------|
| Playwright | Full | `npm init -y && npm i playwright` in `~/.maestro/playwright-bench/` |
...
| Crawl4AI | Navigate + extract only | `python3 -m venv ~/.maestro/crawl4ai-venv && pip install crawl4ai` |
| Stagehand | Full (AI-dependent latency) | `npm init -y && npm i @browserbasehq/stagehand` in `~/.maestro/stagehand-bench/` — needs `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` |
| Playwriter | Full | `npm i -g playwriter` — requires manual extension activation (localhost:19988); may skip in automated runs |

## Running Benchmarks

```bash
cd ~/.maestro/.agent-workspace/work/browser-bench/
node bench-playwright.mjs | tee results-playwright.json
bash bench-agent-browser.sh | tee results-agent-browser.txt
source ~/.maestro/crawl4ai-venv/bin/activate && python bench-crawl4ai.py | tee results-crawl4ai.json
OPENAI_API_KEY=... node bench-stagehand.mjs | tee results-stagehand.json
# dev-browser: bun x tsx ~/.maestro/dev-browser/skills/dev-browser/bench.ts | tee ~/results-dev-browser.json
```
