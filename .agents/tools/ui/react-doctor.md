...
- There is no repo-global maestro wrapper for React Doctor. Use the project-pinned command when available, for example `npm run react-doctor -- --staged`, or the equivalent package-manager script documented by the target project.
