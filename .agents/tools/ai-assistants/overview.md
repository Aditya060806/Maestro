---
description: Overview of supported AI coding tools
mode: subagent
tools:
  read: true
  grep: true
  webfetch: true
---

<!-- SPDX-License-Identifier: MIT -->
<!-- SPDX-FileCopyrightText: 2025-2026 Aditya Pandey and Harvest -->

# AI Coding Tools - Supported by maestro

<!-- AI-CONTEXT-START -->
**Primary**: OpenCode (TUI, Desktop, Extension) | **Companion**: claude-code CLI | **Setup**: `maestro update`
<!-- AI-CONTEXT-END -->

## Supported Tools

### OpenCode (Primary)

Variants: TUI (terminal), Desktop, Extension (Zed/VSCode/forks).

- **Install**: `curl -fsSL https://opencode.ai/install | bash`
- **Config**: `~/.config/opencode/opencode.json` or project `.opencode/`
- **Agents**: Generated by `setup.sh` into `~/.config/opencode/agent/`
- **MCPs**: Configured per-agent via YAML frontmatter, managed by setup.sh
- **Docs**: `tools/opencode/opencode.md` | server API: `opencode-server.md`

### claude-code CLI (Companion)

Spawned from OpenCode for sub-tasks and headless dispatch.

- **Usage**: `claude -p "subtask"`
- **Config**: `~/.claude/`, `~/.config/Claude/`
- **Docs**: `claude-code.md`

### OpenClaw (Multi-Channel Assistant)

WhatsApp, Telegram, Slack, Discord, Signal, iMessage, Teams.

- **Install**: `npm install -g openclaw@latest && openclaw onboard --install-daemon`
- **Docs**: `openclaw.md`

## Collaborator Compatibility

`maestro init` generates pointer files (`.cursorrules`, `.windsurfrules`, etc.) referencing `AGENTS.md`. maestro does not install into or manage those tools.

## Related

- `tools/opencode/opencode.md` — Full OpenCode reference
- `tools/opencode/opencode-anthropic-auth.md` — Anthropic auth setup
- `claude-code.md` — Claude Code CLI reference
- `openclaw.md` — OpenClaw multi-channel assistant
- `headless-dispatch.md` — Parallel agent dispatch
- `opencode-server.md` — OpenCode server API
