...
| Scenario | Use | Why |
|----------|-----|-----|
| Writing code, debugging, PRs | **maestro** | Full IDE integration, file editing, git workflow |
...
| Complex multi-file refactor | **maestro** | Edit/Write tools, worktrees, preflight |
| Voice interaction while driving | **OpenClaw** | Talk Mode, Voice Wake |
| SEO research and analysis | **maestro** | DataForSEO integration, structured output |
| Client communication bot | **OpenClaw** | Multi-channel, pairing, session isolation |
| CI/CD and deployment | **maestro** | GitHub Actions, Coolify, release workflow |

**Cross-integration:** maestro agents manage the server OpenClaw runs on (`@hetzner`, `@cloudflare`, `@tailscale`, `@orbstack`). OpenClaw can trigger maestro workflows via messaging — e.g., "deploy the latest release" runs deployment scripts; cron jobs monitor server health via maestro scripts; webhooks trigger CI/CD pipelines.
