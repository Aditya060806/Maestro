...
```bash
D="$HOME/.maestro/.agent-workspace/datasets"
...
**Promote from traces**: Find trace ID via `jq '.request_id' ~/.maestro/.agent-workspace/observability/metrics.jsonl | tail`, then `dataset-helper.sh promote --trace-id <id> --tags "edge-case"`. Edit the promoted entry with expected output.
