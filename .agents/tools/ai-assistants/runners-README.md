...
1. **Draft** — save to `~/.maestro/agents/draft/` with `status: draft` in frontmatter
2. **Custom** — move to `~/.maestro/agents/custom/` for permanent private use
3. **Shared** — refine to framework standards and submit a PR to `.agents/` in the maestro repo
