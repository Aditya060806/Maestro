...
**Deploy-time (all runtimes):** `setup.sh` resolves tier shorthands in `model:` frontmatter to FQIDs using `model-routing-table.json`. Deployed files at `~/.maestro/agents/` contain fully-qualified IDs that any runtime can consume directly.
...
`model-registry-helper.sh` maintains `~/.maestro/.agent-workspace/model-registry.db`. Syncs on `maestro update`.
