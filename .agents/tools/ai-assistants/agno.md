...
# Agno Integration for Maestro Framework
...
- **Start/Stop/Status:** `start-agno-stack.sh` / `stop-agno-stack.sh` / `agno-status.sh` (in `~/.maestro/scripts/`)
- **URLs:** Agent-UI http://localhost:3000 · AgentOS API http://localhost:8000 · Docs /docs
- **Config:** `~/.maestro/agno/.env` — `OPENAI_API_KEY` required
...
# Manual: AgentOS
mkdir -p ~/.maestro/agno && cd ~/.maestro/agno
python3 -m venv venv && source venv/bin/activate
pip install "agno[all]"

# Manual: Agent-UI
mkdir -p ~/.maestro/agent-ui && cd ~/.maestro/agent-ui
...
Update: `cd ~/.maestro/agno && source venv/bin/activate && pip install --upgrade "agno[all]"` · `cd ~/.maestro/agent-ui && npm update`
...
Add to `~/.maestro/agno/agent_os.py`:
...
```bash
cd ~/.maestro/agno && source venv/bin/activate
...
chmod +x ~/.maestro/agno/start_agno.sh
chmod +x ~/.maestro/agent-ui/start_agent_ui.sh
```
