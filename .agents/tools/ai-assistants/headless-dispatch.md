...
- **Runners**: `runner-helper.sh [create|run|status|list|stop|destroy]` → `~/.maestro/.agent-workspace/runners/`
...
**Draft agents**: Share domain instructions across workers via `~/.maestro/agents/draft/`. See `tools/build-agent/build-agent.md`.
...
```bash
HELPER="$(maestro config get paths.agents_dir | sed "s|^~|$HOME|")/scripts/headless-runtime-helper.sh"
