...
Skills from the Context7 registry can be imported into maestro using `/add-skill`. See "Skill Discovery and Import" below.

<!-- AI-CONTEXT-END -->

## Installation & Setup

> **Note**: maestro configures Context7 automatically via `setup.sh`. The sections below document config formats for other tools as MCP reference.
...
### Importing into maestro

Use `/add-skill` to convert Context7 skills to maestro subagent format with frontmatter, update tracking, and placement in `.agents/`.
...
|--------|----------------------|--------------|
| Format | SKILL.md (as-is) | Converted to maestro subagent |
...
Recommended in automated/CI environments. Add the export to `~/.config/maestro/credentials.sh` or your shell profile.
