...
- **Script**: `~/.maestro/agents/scripts/mcp-index-helper.sh`
...
**Not installed by maestro** (use subagent instead):
...
1. **Index**: Tool descriptions extracted into SQLite FTS5 (`~/.maestro/.agent-workspace/mcp-index/mcp-tools.db`)
...
See `tools/mcp-toolkit/mcporter.md` for full docs and `maestro/mcp-integrations.md` for setup.
...
- `maestro/architecture.md` - Agent design patterns
