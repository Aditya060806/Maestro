...
2. **Provider filter** (`MAESTRO_HEADLESS_PROVIDER_ALLOWLIST`) → optional local pinning such as `openai`
...
`MAESTRO_VAULT_PROVIDER_AI_APPROVED=1` dispatch gate. `secret` classification
...
- **Pulse**: Resolves `sonnet` through `model-availability-helper.sh resolve sonnet`, so it follows routing-table order, health checks, local routing-table overrides, and `MAESTRO_HEADLESS_PROVIDER_ALLOWLIST`.
...
- **Local switch**: Set `MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=openai` to force both pulse and workers onto the default OpenAI fallbacks. If you want OpenAI primary but Anthropic fallback, reorder `custom/configs/model-routing-table.json` and omit the allowlist.
...
- **GPT-5.5 standard workers**: For `openai/gpt-5.5` on worker `sonnet`/`pro` tiers, maestro omits env-derived variants such as `MAESTRO_HEADLESS_VARIANT_SONNET=high` so OpenCode sends no explicit thinking override. Explicit CLI `--variant` still wins because it bypasses automatic variant resolution.
- **Tier-aware effort**: Headless dispatch can now apply variants by resolved tier. `MAESTRO_HEADLESS_VARIANT_SONNET=high` and `MAESTRO_HEADLESS_VARIANT_OPUS=xhigh` make standard work run at `high` and reasoning-tier work run at `xhigh` even when both tiers use `openai/gpt-5.5`. The GPT-5.5 standard-worker exception above only applies to env-derived sonnet/pro variants.
...
- **Deprecated**: `PULSE_MODEL` and `MAESTRO_HEADLESS_MODELS` env vars are respected as overrides for one release cycle with deprecation warnings. Remove from `credentials.sh`.

### Per-user override that survives auto-update

Auto-update overwrites `~/.maestro/agents/configs/*.json` and `~/.maestro/agents/scripts/*`, so user-specific routing must live outside those paths.

- Put persistent model-order overrides in `~/.maestro/agents/custom/configs/model-routing-table.json`
- Put the provider pin in `~/.config/maestro/credentials.sh`
...
```bash
export MAESTRO_HEADLESS_PROVIDER_ALLOWLIST="openai"
```

Example reasoning effort override:

```bash
export MAESTRO_HEADLESS_VARIANT_SONNET="high"
export MAESTRO_HEADLESS_VARIANT_OPUS="xhigh"
```

Role-specific overrides still exist when needed:

```bash
export MAESTRO_HEADLESS_PULSE_VARIANT="high"
export MAESTRO_HEADLESS_WORKER_VARIANT="xhigh"
```
