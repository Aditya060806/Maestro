...
- **Output**: `~/.maestro/.agent-workspace/work/context/{repo}-{mode}-{timestamp}.{format}`
- **Invocation**: `@context-builder compress ~/projects/myapp` or call helper directly

### Helper Script

```bash
~/.maestro/agents/scripts/context-builder-helper.sh <command> [args]
```
...
- **"Permission denied"**: `chmod +x ~/.maestro/agents/scripts/context-builder-helper.sh`
