...
1. Single secret → `maestro secret set NAME`
...
**Commands:** `maestro secret set NAME` / `maestro secret run CMD` (gopass) · `sops-helper.sh encrypt config.enc.yaml` (SOPS) · `gocryptfs-helper.sh create vault-name` (gocryptfs)

**Storage:** gopass → `~/.local/share/gopass/stores/root/maestro/` (fallback: `~/.config/maestro/credentials.sh`, 600 perms) · SOPS → encrypted files in git (age preferred, GPG, AWS/GCP/Azure KMS) · gocryptfs → `~/.maestro/.agent-workspace/vaults/` (AES-256-GCM)
...
maestro secret init
maestro secret set DATABASE_URL
maestro secret set API_KEY

# 2. Initialize SOPS for config files (two options)
maestro init sops                    # Project-level: creates .sops.yaml + age key
sops-helper.sh init                   # Standalone: init without maestro project config
...
maestro secret run docker build .
...
- **gopass** stores individual `secret` values. Agents use `maestro secret` so
...
- `reference/vault.md` -- Maestro Vault threat model and architecture RFC
