...
> **Anthropic uses OAuth only** (Claude Pro/Max). `opencode auth login` prompts for an API key — do not use it for OAuth. Use `maestro model-accounts-pool add anthropic` (opens browser) or OpenCode TUI: `Ctrl+A` → Anthropic → Login with Claude.ai.

## Start here: diagnose

```bash
maestro update                                   # ensure latest version first
maestro model-accounts-pool diagnose             # full pipeline check — covers everything below
```
...
```bash
maestro model-accounts-pool status               # 1. pool health at a glance
maestro model-accounts-pool check                # 2. live token validity per account
maestro model-accounts-pool rotate anthropic     # 3. switch account if rate-limited
...
All commands prefixed with `maestro model-accounts-pool`.
...
**Health check:** `maestro model-accounts-pool check google` validates against `generativelanguage.googleapis.com/v1beta/models`.

## Key diagnostic facts

- "Key Missing" = plugin didn't load or pool is empty — check `maestro model-accounts-pool status`.
...
- Pool file: `~/.maestro/oauth-pool.json` — if corrupt or missing, `add` recreates it.
