...
- **Script**: `credential-helper.sh` (`bash ~/.maestro/agents/scripts/credential-helper.sh <cmd>` if not on PATH)
- **Storage**: `~/.config/maestro/tenants/{tenant}/credentials.sh`
- **Priority**: Project (`.maestro-tenant`) > Global (`active-tenant`) > `default`
...
Manages separate credential sets for multiple clients, environments, or accounts. For encrypted storage, see `tools/credentials/gopass.md` — use `maestro secret` for encrypted secrets alongside `credential-helper.sh` for tenant switching.
...
**Script**: `source <(bash ~/.maestro/agents/scripts/credential-helper.sh export --tenant client-acme)`
...
- `.maestro-tenant` auto-added to `.gitignore`
