...
- **Store**: `maestro secret set NAME` (gopass, preferred) or `setup-local-api-keys.sh set NAME TOKEN` (plaintext fallback)
...
2. **Never accept** secret values in AI conversation — instruct user to run `maestro secret set NAME`
...
| Tier | Location | Use |
|------|----------|-----|
| **Encrypted** (preferred) | gopass (`maestro secret`) | All secrets — see `gopass.md` |
| **Plaintext fallback** | `~/.config/maestro/credentials.sh` (600) | When gopass unavailable — see `api-key-setup.md` |
...
3. **Update** local (`maestro secret set NAME`) + GitHub Secrets
