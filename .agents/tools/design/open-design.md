...
- **Role in maestro**: optional peripheral for sandboxed previews, design artifacts, decks, and media surfaces
- **Source of truth**: maestro remains self-contained; `.agents/` and Google `DESIGN.md` stay canonical
...
Open Design is valuable as an **optional artifact studio** beside maestro, not as a framework dependency. Keep the boundary explicit:

| Layer | Owner | Rule |
|-------|-------|------|
| Agent/source registry | maestro | `.agents/` remains canonical; no Open Design symlink registry as source of truth |
| Design system | maestro | Google `DESIGN.md` with YAML tokens, linted via `@google/design.md` |
| Preview/runtime | Open Design optional | Use for live iframe preview, exports, and artifact workspaces |
| Local HTTPS | maestro | Use `localdev-helper.sh` when `.local` HTTPS is needed |
| Verification | maestro | Run UI/accessibility/design verification after artifact generation |
...
- Generated output should be reviewed with maestro Playwright/accessibility gates before shipping.

Do not use for normal code changes, backend work, or any task where maestro agents can directly implement and verify faster.
...
3. **Compress** to maestro shape: `{name}-skill.md` plus flat `{name}-skill/` references, not nested runtime folders.
...
Open Design accepts broad Markdown design systems; maestro uses the Google `DESIGN.md` standard with YAML tokens. Bridge rules:

- Convert Open Design systems into Google `DESIGN.md` before reuse.
- Run `npx @google/design.md lint DESIGN.md` and resolve errors.
- Use maestro library examples as canonical where duplicates exist.
...
- `tools/design/open-design-ingestion.md` — all Open Design skills classified for maestro value
- `tools/design/design-md.md` — Google DESIGN.md workflow
- `tools/design/library/` — maestro design-system library
