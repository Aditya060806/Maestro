...
- **Normative spec**: [google-labs-code/design.md](https://github.com/google-labs-code/design.md) (Apache 2.0, format version `alpha`; maestro tracks upstream through upstream commit `18508f2` after the v0.2.0 review). Full spec: [`docs/spec.md`](https://github.com/google-labs-code/design.md/blob/main/docs/spec.md)
...
6. **Brand guide** — run `maestro design guidelines . --pdf` to create `_reports/brand-guidelines/` Markdown, HTML, and PDF handoff files.
...
- If the preference is uncertain, too broad, or outside the PR scope, create a worker-ready follow-up issue instead; if the learning is generic across repos, route it as an maestro self-improvement issue with anonymised, repo-safe evidence.
...
| # | Section | Aliases | maestro tokens |
|---|---------|---------|-----------------|
...
| 8 | Do's and Don'ts | — | — |
| 9 | Responsive Behaviour | *(maestro extension)* | — |
| 10 | Agent Prompt Guide | *(maestro extension)* | — |

Sections 1-8 are the Google Labs spec. Sections 9-10 are maestro-specific extensions the spec's unknown-content rule explicitly permits (unknown `##` headings are preserved, not errored). Duplicate section headings are an error — the linter rejects the file.
...
**maestro convention**: zero errors mandatory; warnings reviewed before committing. The maestro library examples may carry missing-section or genuinely custom orphaned-token warnings while migration is in progress — see the library migration task.
...
`maestro init` now records `has_interface` and creates root `DESIGN.md` for standard-scope repos plus minimal-scope repos where interface markers are detected. Use `maestro design scaffold . --force` for unusual GUI repos without detectable markers.
...
The spec format is `alpha` — expect changes. maestro mitigations:
...
- `templates/DESIGN.md.template` -- Skeleton for `maestro init`
