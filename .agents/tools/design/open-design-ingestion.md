...
Source: [nexu-io/open-design](https://github.com/nexu-io/open-design) (Apache-2.0). This plan treats Open Design as an optional peripheral and evaluates each upstream skill for maestro value before any import.
...
| **Adapt** | Valuable but must be reshaped around maestro `DESIGN.md`, verification, or domain agents |
| **Combine** | Merge into existing maestro agent/workflow instead of adding a new skill |
...
| Skill | Upstream mode/scenario | maestro action | Why |
|-------|------------------------|-----------------|-----|
...
| `email-marketing` | prototype/marketing | Adapt | Pair with maestro email testing and client-render checks |
...
| `critique` | prototype/design | Combine | Fold into maestro design review and UI verification rubrics |
...
| `html-ppt` | deck/marketing | Adapt | Broad deck studio; convert to smaller style-specific maestro skills |
...
| `html-ppt` | Adapt | Split into concise maestro deck subskills instead of one broad prompt |
...
- Replace Open Design UI-only metadata with maestro routing notes unless needed by `/open-design`.
- Add verification commands to every adopted skill: UI screenshot, accessibility, email, deck export, or render smoke test.
- Prefer existing maestro agents for semantics; imported skills should improve artifact craft, not duplicate domain reasoning.
