...
**Key principle**: maestro markdown files are the source of truth. Beads syncs from them.
...
```bash
~/.maestro/agents/scripts/beads-sync-helper.sh push [/path/to/project]  # TODO.md → Beads
~/.maestro/agents/scripts/beads-sync-helper.sh pull                      # Beads → TODO.md (manual merge required)
~/.maestro/agents/scripts/beads-sync-helper.sh status                    # Check sync status
~/.maestro/agents/scripts/beads-sync-helper.sh sync                      # Two-way sync with conflict detection
~/.maestro/agents/scripts/beads-sync-helper.sh sync --force              # Force sync (TODO.md wins on conflict)
~/.maestro/agents/scripts/todo-ready.sh                                  # Show unblocked tasks
~/.maestro/agents/scripts/todo-ready.sh --json                           # JSON output
~/.maestro/agents/scripts/todo-ready.sh --count                          # Count only
```
...
```bash
./setup.sh                                      # Via maestro setup (installs bd automatically)
brew install steveyegge/beads/bd                # Manual
go install github.com/steveyegge/beads/cmd/bd@latest
maestro init beads                             # Init project: creates .beads/, syncs TODO.md/PLANS.md, adds .beads to .gitignore
```
