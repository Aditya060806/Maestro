...
| Item | Value |
|------|-------|
| Script | `~/.maestro/agents/scripts/remote-dispatch-helper.sh` |
| Config | `~/.config/maestro/remote-hosts.json` (fields: `address`, `transport`, `container`, `user`, `added`) |
| Logs | `~/.maestro/.agent-workspace/supervisor/logs/remote/` |
| Task | t1165.3 |
...
| `REMOTE_DISPATCH_HOSTS_FILE` | `~/.config/maestro/remote-hosts.json` | Hosts config |
...
| Logs not collected | `remote-dispatch-helper.sh logs t123 gpu-server` · `ssh user@host "ls -la /tmp/maestro-worker/t123/worker.log"` |
