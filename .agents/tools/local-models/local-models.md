...
cp build/bin/llama-server ~/.maestro/local-models/bin/
```
...
Directory layout: `~/.maestro/local-models/{bin/llama-server,models/,config.json}` · DB: `~/.maestro/.agent-workspace/memory/local-models.db`
...
Server defaults (`~/.maestro/local-models/config.json`): port 8080, host 127.0.0.1, ctx_size 8192, threads auto (perf cores), gpu_layers 99, flash_attn true.
