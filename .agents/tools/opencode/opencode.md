...
- **Primary Agent**: `maestro` — full framework access
- **Subagents**: hostinger, hetzner, wordpress, seo, code-quality, browser-automation, etc.
- **Setup**: `cd ~/Git/maestro && .agents/scripts/generate-opencode-agents.sh`
...
| maestro agents | `~/.maestro/agents/` (after setup.sh) |
| Credentials | `~/.config/maestro/credentials.sh` |
...
|-------|-------------|--------------|
| `maestro` | Full framework (primary) | context7 |
...
# Helper shortcuts
~/.maestro/agents/scripts/opencode-test-helper.sh test-mcp dataforseo SEO
~/.maestro/agents/scripts/opencode-test-helper.sh list-tools Build+
~/.maestro/agents/scripts/opencode-test-helper.sh serve 4096
```
...
Credentials in `~/.config/maestro/credentials.sh`:

```bash
export HOSTINGER_API_TOKEN="your-token"
export HCLOUD_TOKEN_PROJECT="your-token"   # Hetzner per-account
# GSC: service account JSON at ~/.config/maestro/gsc-credentials.json
```
...
~/.maestro/agents/scripts/worktree-helper.sh add feature/parallel-task
```
