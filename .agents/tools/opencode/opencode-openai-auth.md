...
- **Pool file**: `~/.maestro/oauth-pool.json` (key `openai`) — 0600 permissions, never commit
...
MAESTRO_OPENAI_ADD_MODE=callback maestro model-accounts-pool add openai
```
...
jq -r '.openai[].email' ~/.maestro/oauth-pool.json
```
...
| Callback fallback | N/A | `localhost:1455/auth/callback` (`MAESTRO_OPENAI_ADD_MODE=callback`) |
