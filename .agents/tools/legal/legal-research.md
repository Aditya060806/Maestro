...
2. **Index**: `zvec.create_and_open(path=f"~/.maestro/.agent-workspace/work/{matter_id}/{collection}")` per collection (framework-standard persistent path); attach metadata on every chunk.
