...
Store keys via `maestro secret set NAME` (never paste in conversation). Keys load automatically from `~/.config/maestro/credentials.sh`.

```bash
# Required for keyed providers:
maestro secret set ABUSEIPDB_API_KEY
maestro secret set VIRUSTOTAL_API_KEY
maestro secret set IPQUALITYSCORE_API_KEY
maestro secret set SCAMALYTICS_API_KEY
maestro secret set SHODAN_API_KEY
maestro secret set IPHUB_API_KEY
# Optional (increases rate limits):
maestro secret set PROXYCHECK_API_KEY
maestro secret set GREYNOISE_API_KEY
```
