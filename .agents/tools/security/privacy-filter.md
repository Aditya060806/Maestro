...
- **Script**: `~/.maestro/agents/scripts/privacy-filter-helper.sh`
...
- **Global config**: `~/.maestro/config/privacy-patterns.txt`
- **Project config**: `.maestro/privacy-patterns.txt`
...
if [[ -n "$staged_files" ]]; then
    echo "$staged_files" | xargs ~/.maestro/agents/scripts/privacy-filter-helper.sh scan
    if [[ $? -ne 0 ]]; then
...
**False positives**: Add exception to `.maestro/privacy-patterns.txt` (prefix with `!` for exclusions, e.g., `!test/fixtures/`), or modify content to avoid the pattern.
...
- `maestro/architecture.md` — Self-improving agent system
