...
- **Script**: `audit-log-helper.sh` (`~/.maestro/agents/scripts/audit-log-helper.sh`)
- **Log file**: `~/.maestro/.agent-workspace/observability/audit.jsonl`
...
- **Sequential writes.** Concurrent writers may race. In practice, maestro serializes operations (one pulse at a time).
