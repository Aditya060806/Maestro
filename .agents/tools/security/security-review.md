...
|--------|--------|-------------|
| `allow` | Add domain to Tier 3 (known tools, allowed + logged) | `~/.config/maestro/network-tiers-custom.conf` |
...
| `trust` | Add MCP server to trusted list | `~/.config/maestro/mcp-trusted-servers.txt` |
| `dismiss` | Mark as false positive, no config change | `reviewed.jsonl` |

## Usage

```bash
# View queue (~/.maestro/.agent-workspace/security/quarantine/{pending,reviewed}.jsonl)
