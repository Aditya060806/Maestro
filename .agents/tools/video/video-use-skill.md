...
- `ELEVENLABS_API_KEY` resolves — preferably via `gopass` or `maestro secret set ELEVENLABS_API_KEY`, with environment or `.env` at the video-use repo root as fallbacks. Never ask the user to paste secrets directly into the chat.
