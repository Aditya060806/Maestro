---
description: Architecture design for maestro-opencode plugin
mode: subagent
tools:
  read: true
  write: true
...
# maestro-opencode Plugin Architecture

- **Status**: Implemented (`t008.1` PR #1138, `t008.2` PR #1149, `t008.3` PR #1150)
- **Purpose**: Native OpenCode plugin wrapper for maestro
- **Implementation**: Single-file ESM plugin at `.agents/plugins/opencode-maestro/index.mjs`
...
| Custom tools | `tool` registration | Exposes maestro-specific tools |
| Quality gates | `tool.execute.before` / `tool.execute.after` | Runs checks, tracking, and audit logging |
| Shell environment | `shell.env` hook | Exports maestro paths and version |
...
- Loads subagents from `~/.maestro/agents/`, parses YAML frontmatter, injects into `config.agent`, skips shell-generated entries (`t008.1`).
...
| Hook | Coverage |
|---|---|
| `tool` registration | `maestro`, `maestro_memory`, `maestro_pre_edit_check`, `model-accounts-pool` |
...
| `tool.execute.after` (`t008.3`) | Git operation detection, pattern tracking via cross-session memory, audit logging to `~/.maestro/logs/quality-hooks.log` |
| `shell.env` | Prepends `~/.maestro/agents/scripts/` to `PATH`; exports `MAESTRO_AGENTS_DIR`, `MAESTRO_WORKSPACE_DIR`, `MAESTRO_VERSION` |
...
- Implementation: `.agents/plugins/opencode-maestro/index.mjs`
- Plan: `todo/PLANS.md` section `maestro-opencode Plugin` (`p001`)
