...
- **Setup**: `bash .agents/scripts/langflow-helper.sh setup` → edit `~/.maestro/langflow/.env` → `~/.maestro/scripts/start-langflow.sh`
- **Lifecycle**: `start-langflow.sh` / `stop-langflow.sh` / `langflow-status.sh` (all in `~/.maestro/scripts/`)
- **Endpoints**: localhost:7860 (UI) | `/docs` (API) | `/health` (health check)
- **Config**: `~/.maestro/langflow/.env` | **venv**: `~/.maestro/langflow/venv/`
...
Manual: `mkdir -p ~/.maestro/langflow && cd ~/.maestro/langflow && python3 -m venv venv && source venv/bin/activate && pip install langflow && langflow run`
...
`~/.maestro/langflow/.env` — set API keys via `maestro secret` or edit directly:
...
Custom components: place `.py` files in `~/.maestro/langflow/components/`, load with `langflow run --components-path ~/.maestro/langflow/components/`. Subclass `langflow.custom.CustomComponent`, implement `build()` → `langflow.schema.Data`.
...
| Database errors | `rm ~/.maestro/langflow/langflow.db && langflow run` |
| Component not loading | `python -c "from components.my_component import MyCustomComponent"` |
| Debug logs | `LANGFLOW_LOG_LEVEL=DEBUG langflow run` or `tail -f ~/.maestro/langflow/langflow.log` |
