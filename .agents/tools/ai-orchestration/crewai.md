...
- **Start/Stop/Status**: `~/.maestro/scripts/start-crewai-studio.sh` / `stop-crewai-studio.sh` / `crewai-status.sh`
- **URL**: http://localhost:8501 (CrewAI Studio)
- **Config**: `~/.maestro/crewai/.env`
- **Install**: `pip install crewai 'crewai[tools]'` in venv at `~/.maestro/crewai/venv/`
...
nano ~/.maestro/crewai/.env
~/.maestro/scripts/start-crewai-studio.sh

# Manual
mkdir -p ~/.maestro/crewai && cd ~/.maestro/crewai
