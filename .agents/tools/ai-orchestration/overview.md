...
- **Common Pattern**: `~/.maestro/{tool}/` with venv, .env, start scripts
...
**Draft agents**: Reusable patterns discovered during orchestration tasks → `~/.maestro/agents/draft/`. Promote to `custom/` (private) or `.agents/` (shared) after review. See `tools/build-agent/build-agent.md` "Agent Lifecycle Tiers".
...
| Agno | - | localhost:3000 | `pip install "agno[all]"` | `~/.maestro/scripts/start-agno-stack.sh` |
...
- **Management scripts**: `~/.maestro/scripts/` — `start-{tool}-stack.sh`, `stop-{tool}-stack.sh`, `{tool}-status.sh`
...
```bash
~/.maestro/scripts/localhost-helper.sh check-port 7860  # check if port is in use
~/.maestro/scripts/localhost-helper.sh find-port 7860   # find next available port
~/.maestro/scripts/localhost-helper.sh list-ports       # list all allocated ports
~/.maestro/scripts/localhost-helper.sh kill-port 7860   # release a port
# Manual fallback: lsof -i :7860 && kill -9 $(lsof -t -i:7860)
```

**Venv issues**: `rm -rf ~/.maestro/{tool}/venv && bash .agents/scripts/{tool}-helper.sh setup`

**API key errors**: `cat ~/.maestro/{tool}/.env` or `env | grep -E "(OPENAI|ANTHROPIC|OLLAMA)"`
