...
- **API keys**: Soniox, Cartesia, Anthropic/OpenAI (store via `maestro secret set`)
...
maestro secret set SONIOX_API_KEY
...
```bash
mkdir -p ~/.maestro/.agent-workspace/work/pipecat-voice-agent
cd ~/.maestro/.agent-workspace/work/pipecat-voice-agent
...
async def run_agent(webrtc_connection: SmallWebRTCConnection):
    ...
    messages = [{
        "role": "system",
        "content": (
            "You are a Maestro assistant in a voice conversation. "
...
transport = DailyTransport(
    ...
    "Maestro Agent", DailyParams(audio_in_enabled=True, audio_out_enabled=True),
)
```
