...
- **Store secrets**: `~/.config/maestro/credentials.sh` (600 perms) or gopass
...
**Auth:** CLI auth (tokens in system keyring, not repo files). Fallback: `~/.config/maestro/credentials.sh` (600 perms). Rotate every 6-12 months or on exposure. Prefer short-lived CI/CD credentials. See `git/authentication.md`.
