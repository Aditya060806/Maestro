...
- **Memory**: `maestro-memory` — cross-session finding persistence
...
**1.4 Recall cross-session memory:** `maestro-memory recall "autoagent $PROGRAM_NAME" --limit 10` → store as MEMORY_CONTEXT.
...
while true:
    ...
    if is_improvement(metric_result, BEST_METRIC, METRIC_DIR):
        ...
        maestro-memory store "autoagent {PROGRAM_NAME}: {hypothesis[:80]} → keep ({METRIC_NAME}: {metric_result})" --confidence medium
    else:
        git -C WORKTREE_PATH reset --hard HEAD
        ...
        maestro-memory store "autoagent {PROGRAM_NAME}: {hypothesis[:80]} → discard ({METRIC_NAME}: {metric_result})" --confidence medium
```
