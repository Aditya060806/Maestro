...
- **Workspace**: `~/.maestro/.agent-workspace/tmp/security-audit/`
...
```bash
AUDIT_DIR="$HOME/.maestro/.agent-workspace/tmp/security-audit"
mkdir -p "$AUDIT_DIR"
...
For maestro projects, verify `prompt-guard-helper.sh` integration. See `tools/security/prompt-injection-defender.md`.
