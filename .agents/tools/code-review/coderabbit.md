...
- **Helper script**: `~/.maestro/agents/scripts/coderabbit-cli.sh [install|auth|review|status]`
...
If the user wants CodeRabbit findings to keep arriving after an maestro-managed PR is merged, suggest the repo-level `.coderabbit.yaml` setting:
...
Trade-off: this can surface useful post-merge findings for maestro to file as follow-up issues, but may add comments on already-merged PRs. Keep maestro review-bot gate protections unless verified noise/latency data shows they should be relaxed.
...
**Legacy**: Issue #2386 in `Aditya060806/Maestro` was the original single-repo CodeRabbit-only review issue. Superseded by multi-repo persistent quality review issues from `run_daily_quality_sweep()`.
