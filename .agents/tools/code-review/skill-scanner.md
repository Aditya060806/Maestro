...
- **Entry points**: `maestro skill scan`, `security-helper.sh skill-scan`, `add-skill-helper.sh`
...
| VirusTotal | Free tier | ~16s/request | `VIRUSTOTAL_ADITYA060806` or `VIRUSTOTAL_API_KEY` |
| Cisco AI Defense | Enterprise | ~1s | `AI_DEFENSE_API_KEY` |

Keys: `~/.config/maestro/credentials.sh` (600 perms) or `maestro secret set <KEY_NAME>`.

## maestro Integration

- **Import gate**: `add-skill-helper.sh` — `CRITICAL`/`HIGH` blocks unless `--skip-security` (or explicit interactive override)
- **Batch scans**: `maestro skill scan`, `security-helper.sh skill-scan all`
- **Update path**: `setup.sh` scans all skills during `maestro update` (non-blocking); `skill-update-helper.sh update` re-imports with `--force`
