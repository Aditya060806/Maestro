...
```bash
MAINTAINER=$(jq -r '.initialized_repos[] | select(.slug == "<slug>") | .maintainer // empty' ~/.config/maestro/repos.json)
...
SIG_FOOTER=$(~/.maestro/agents/scripts/gh-signature-helper.sh footer 2>/dev/null || echo "")
