...
- **Config**: `~/.config/maestro/cron-jobs.json`
...
Storage:
  ~/.config/maestro/cron-jobs.json    (job definitions)
  ~/.maestro/.agent-workspace/cron/   (execution logs)
  ~/.maestro/.agent-workspace/mail/   (result delivery)
```
...
```bash
0 9 * * * ~/.maestro/agents/scripts/cron-dispatch.sh job-001 >> ~/.maestro/.agent-workspace/cron/job-001.log 2>&1
```
...
**macOS (launchd)** — label `sh.maestro.opencode-server`, plist `~/Library/LaunchAgents/sh.maestro.opencode-server.plist`:
...
launchctl load ~/Library/LaunchAgents/sh.maestro.opencode-server.plist
```
...
cron-helper.sh add --schedule "0 3 * * 0" --name "weekly-maintenance" --workdir "~/.maestro" \
...
# Permission issues
chmod +x ~/.maestro/agents/scripts/cron-*.sh
ls -la ~/.maestro/.agent-workspace/cron/
```
