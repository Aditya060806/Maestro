...
1. **Start narrow** with `rtk-helper.sh` for supported summary commands:
   ...
   - RTK v0.41.0 also adds upstream Gradle wrapper support, `rtk init --agent hermes`, `rtk init --dry-run`, `transparent_prefixes` for wrapper command passthrough, tee tail hints, Docker Compose log tail forwarding, compact Kubernetes pod/service summaries, and git push/status filtering fixes; prefer those upstream features over custom maestro shims when integrating supported runtimes, build tools, or noisy terminal summaries.
