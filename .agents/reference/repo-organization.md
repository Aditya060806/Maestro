...
Create worktrees under `${MAESTRO_WORKTREE_BASE_DIR:-~/Git/_worktrees}` with flat `<repo>-<branch-slug>` names. `worktree-helper.sh add <branch>` already derives this path; existing sibling worktrees from older versions remain valid until cleanup removes them safely.
