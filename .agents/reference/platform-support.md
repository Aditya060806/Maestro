...
Add brew to your PATH per post-install instructions. Alternatively, use apt — maestro detects and uses it automatically.
...
```bash
source ~/.maestro/agents/scripts/platform-detect.sh
...
|----------|-------|-----------------|-------------------|------|
| `MAESTRO_PLATFORM` | `macos` | `linux` | `linux` | `wsl2` |
| `MAESTRO_SCHEDULER` | `launchd` | `systemd` | `cron` | `systemd` or `cron` |
| `MAESTRO_CLIPBOARD_COPY` | `pbcopy` | `xclip -selection clipboard` | `xclip ...` or empty | `clip.exe` |
| `MAESTRO_CLIPBOARD_PASTE` | `pbpaste` | `xclip -selection clipboard -o` | ... | `powershell.exe -c Get-Clipboard` |
| `MAESTRO_OPEN_CMD` | `open` | `xdg-open` | `xdg-open` | `wslview` |
| `MAESTRO_FILE_SEARCH` | `mdfind` | `fd` | `fd` or `locate` | `fd` |
| `MAESTRO_PKG_INSTALL` | `brew install` | `sudo apt-get install -y` | varies | `sudo apt-get install -y` |
...
- Label prefix: `com.maestro.*`
- Manage: `launchctl list | grep maestro`
- Disable pulse: `maestro config set orchestration.supervisor_pulse false && ./setup.sh`

### Linux — systemd user services (preferred)

When `systemctl --user status` succeeds, maestro installs systemd user timers. Preferred over cron: supports `Restart=on-failure`, journal logging, and dependency ordering.

- Service files: `~/.config/systemd/user/maestro-*.{service,timer}`
- List: `systemctl --user list-timers | grep maestro`
- Disable pulse: `systemctl --user disable --now maestro-supervisor-pulse.timer`

### Linux — cron (fallback)

Used when systemd is unavailable (containers, older distros, WSL2 without systemd).

- List: `crontab -l | grep maestro`
