...
```bash
maestro opencode-db check    # preflight: DB exists, no locks, integrity OK
maestro opencode-db report   # stats (size, pages, free list, top tables)
maestro opencode-db maintain # run once (refuses if processes active)
maestro opencode-db maintenance-window --force-opencode
maestro opencode-db status   # scheduler install state
```
...
active DB. This matters for maestro-managed isolated project shards under
`~/.maestro/.agent-workspace/work/opencode-interactive/`.
...
1. Stops maestro-managed pulse/headless workers with `pulse-lifecycle-helper.sh
   stop`.
...
database. To keep Desktop off the shared CLI hot spot, maestro installs a
macOS app wrapper when Desktop is present:

```bash
maestro opencode-desktop install-shortcut
open ~/Applications/OpenCode\ Maestro.app
```

The generated `~/Applications/OpenCode Maestro.app` calls the deployed
`opencode-launcher-helper.sh`, which exports an isolated Desktop data directory
under `~/.maestro/.agent-workspace/work/opencode-desktop/` before execing
...
```bash
maestro opencode-desktop                 # launch Desktop with isolated DB
maestro opencode-desktop --dir ~/Git/repo # per-project Desktop shard
maestro opencode-desktop status          # show wrapper/source status
```
...
maestro opencode-db sessions --query "issue-or-title-text" --include-archive
maestro opencode-db sessions --directory "/absolute/path/to/repo" --include-archive --limit 50
```

The maestro lookup is read-only. It searches the active OpenCode DB and, with `--include-archive`, `opencode-archive.db`. Use `--id ses_...` for an exact session ID and `--json` for scriptable output.
...
**Symptom:** The TUI `/sessions` picker shows only the current session (or a handful of recent ones), not the full history. Closing and reopening opencode does not help. Prefer the read-only `maestro opencode-db sessions --directory "/absolute/path/to/repo" --include-archive` lookup before writing manual SQL.
...
## What maestro cannot fix from outside opencode
