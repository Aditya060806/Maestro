...
# Maestro Vault Security Architecture RFC
...
**TL;DR:** Maestro Vault is the policy and architecture model for protecting
agent memory, history, workspace, knowledge, mail, metadata, audits, device
state, and sync collections. `maestro vault` now provides the first local
...
- Classify maestro protected data consistently across local machines, third-
...
`MAESTRO_VAULT_MANAGED_SESSION_HISTORY=1` is set, maestro session/history
...
`~/.maestro/.agent-workspace/vault/managed-session-history/<runtime>/...` and
can be relocated with `MAESTRO_VAULT_MANAGED_HISTORY_ROOT`. Runtime launchers
are responsible for pointing supported apps at the managed path only after a
local Vault unlock. Current gates prevent maestro mining/lookup reads while
...
| `internal` | Maestro operational data not intended for publication | Encrypt at rest when practical; avoid public transports in plaintext | Provider-allowed only when task-relevant |
...
`.agents/scripts/vault-crypto-helper.py`, exposed as `maestro vault`:

- `init` creates `vault.json` metadata under `~/.config/maestro/vault/` (or
  `MAESTRO_VAULT_DIR`) with schema version, KDF parameters, salt, wrapped root
...
events through this helper; set `MAESTRO_VAULT_AUDIT_REQUIRE=1` when sensitive
...
When Vault metadata exists, or `MAESTRO_VAULT_REQUIRE=1` is set for tests or
...
`.agents/scripts/vault-migration-helper.sh` migrates current maestro data-plane
