...
- `tail -f ~/.maestro/logs/pulse-wrapper.log` shows workers being killed
...
response cache (`~/.maestro/cache/gh-status-*.json`) so repeated calls
...
2. **Document the correlation on affected issues.** For each issue or PR
   ...
   The block carries an HTML marker (`<!-- maestro:gh-status-correlation -->`)
...
`~/.maestro/.agent-workspace/tmp/dispatch-ledger.lock`. When a worker
...
```bash
ls -la ~/.maestro/.agent-workspace/tmp/dispatch-ledger.lock
# If older than 60 seconds and you're not running a worker, delete it:
rm -rf ~/.maestro/.agent-workspace/tmp/dispatch-ledger.lock
```
...
After a long quiet period or fresh `maestro update`, the first pulse
...
ls -la ~/.maestro/cache/pulse-cache-prime-last-run

# Force a prime
rm -f ~/.maestro/cache/pulse-cache-prime-last-run
...
- [ ] Pulse cycle time back to <5 minutes:
      `tail -50 ~/.maestro/logs/pulse-wrapper.log | grep "cycle complete"`.
- [ ] Ledger health: no entries with `result:failed` in the last
      5 minutes that aren't accompanied by an obvious cause:
      `tail -200 ~/.maestro/.agent-workspace/tmp/dispatch-ledger.jsonl | jq 'select(.result == "failed")'`.
...
- [ ] Correlation comments posted on every issue/PR that timed out
      during the incident window. `gh search issues "<!-- maestro:gh-status-correlation -->"`
...
- `gh-status-helper.sh` cache files at `~/.maestro/cache/gh-status-*.json`.
- Correlation comments on affected issues with the
  `<!-- maestro:gh-status-correlation -->` marker.
- Ledger entries at `~/.maestro/.agent-workspace/tmp/dispatch-ledger.jsonl`
  for every manual dispatch.
- Pulse log at `~/.maestro/logs/pulse-wrapper.log` for cycle-level
  events.
