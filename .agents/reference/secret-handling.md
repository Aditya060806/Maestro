...
- When a user pastes a credential value (API key, token, password, OAuth secret, connection string) into conversation:
  - Immediately warn: "That looks like a credential. Conversation transcripts are stored on disk — treat this value as compromised. Rotate it and store the new value via `maestro secret set NAME` in your terminal."
...
- ALWAYS pass secrets as environment variables, NEVER as command arguments:
  ...
  - SAFE: `maestro secret NAME -- cmd` — injects as env var with automatic output redaction
...
- Deterministic guard locations:
  ...
  - `.agents/hooks/privacy-guard-pre-push.sh` scans public-repo pushes across the configured diff surface; maestro defaults to full-repo scanning.
- Extra private slugs: `~/.maestro/configs/privacy-guard-extra-slugs.txt` (one slug per line).
- Extra local/private path regexes: `~/.maestro/configs/privacy-guard-private-path-patterns.txt` (one POSIX ERE per line). Matches are reported as `[configured-private-path]` so the raw path is not repeated.
...
- Bypasses are explicit and auditable: `MAESTRO_GH_PRIVACY_BYPASS=1` for gh writes; `PRIVACY_GUARD_DISABLE=1` or `git push --no-verify` for pre-push.
