...
Every call to `gh_issue_edit_safe`, `gh_issue_close_safe`, `gh_issue_reopen_safe`, `gh_pr_edit_safe`, `gh_pr_close_safe`, and `gh_pr_merge_safe` writes one NDJSON event to `~/.maestro/logs/gh-audit.log`. A daily routine (`r-gh-audit-scan`) scans the log for anomalies and files a GitHub issue when any are detected.
...
grep '"number":NNN' ~/.maestro/logs/gh-audit.log | jq '.'

# Show all anomalous entries
jq 'select(.suspicious | length > 0)' ~/.maestro/logs/gh-audit.log

# Show recent operations on a specific repo
jq 'select(.repo == "owner/repo")' ~/.maestro/logs/gh-audit.log | tail -20
```
...
ENTRY=$(grep '"number":NNN' ~/.maestro/logs/gh-audit.log | tail -1)
...
- **Log file:** `~/.maestro/logs/gh-audit.log`
...
The scanner:
1. Reads entries since the last scan (tracked in `~/.maestro/logs/gh-audit-scanner.state`)
2. Filters entries with `suspicious[] | length > 0`
3. Files a GitHub issue on `Aditya060806/Maestro` with a summary table when anomalies are found
