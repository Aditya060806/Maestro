...
- **Identity:** Each runner passes `self_login` to `dispatch-dedup-helper.sh` —
  the GitHub login of the machine's authenticated user (e.g., `Aditya060806`,
  `alex-solovyev`).

**What runners do NOT know about each other:** which issues another runner is
currently evaluating (no pre-claim lock); which maestro version another runner
...
(Aditya060806 and johnwaldo) dispatched on the same issue 45 seconds apart.
...
`~/.config/maestro/dispatch-override.conf`. Entries with value `ignore`
...
(Aditya060806 + alex-solovyev runners).
...
grep "STALE_RECOVERED.*#<num>" ~/.maestro/logs/pulse.log | tail -20
```
...
**Why multi-runner makes it worse:** If two runners are on different maestro
...
**Evidence (GH#17503):** Six dispatches from the `Aditya060806` runner + one
from `alex-solovyev`. The `alex-solovyev` runner's pulse predated the GH#18352
fix, so it dispatched on issues the `Aditya060806` runner would have blocked.
...
**Version skew is the root cause.** Keep all runners on the same maestro
version. The version guard (`headless-runtime-helper.sh`) checks
`OPENCODE_PINNED_VERSION` before each dispatch but does not check the maestro
framework version itself. Run `maestro update` on all machines before bringing
up a new session.
...
labels 74 seconds after `Aditya060806` (runner A) restored them.
...
```bash
curl -fsSL https://maestro.sh/install | bash   # Install
...
```bash
maestro repos add Aditya060806/Maestro
maestro repos list
```

### 5.3 Version Parity (Critical)

All active runners **must** run the same maestro version. Version skew causes
race conditions because the dedup chain changes between versions.

```bash
maestro --version
maestro update
bash ~/.maestro/agents/scripts/maestro-update-check.sh   # Verify no pending update
```
...
```bash
# In ~/.config/maestro/pulse.env or shared-constants.sh:
export PULSE_SCOPE_REPOS="Aditya060806/Maestro Aditya060806/some-other-repo"
```
...
```bash
maestro pulse install                                    # Install launchd plist (macOS)
~/.maestro/agents/scripts/pulse-wrapper.sh --dry-run     # Or start manually for testing
tail -f ~/.maestro/logs/pulse.log                        # Verify first cycle
```
...
# 3. Check maestro version on each runner
grep "maestro_version\|MAESTRO_VERSION" ~/.maestro/logs/pulse.log | head -5
```
...
# If origin:interactive is present but pulse still dispatched, check maestro version
...
```bash
SCRIPT=~/.maestro/agents/scripts/dispatch-dedup-helper.sh
...
Config file: `~/.config/maestro/dispatch-override.conf` (sourced by
...
~/.maestro/agents/scripts/dispatch-override-resolve.sh resolve-action \
...
~/.maestro/agents/scripts/dispatch-override-resolve.sh check-legacy
...
| `PEER_QUARANTINE_STATE_FILE` | `~/.maestro/cache/peer-quarantine.json` | State sandbox knob |
| `PEER_QUARANTINE_OVERRIDE_CONF` | `~/.config/maestro/dispatch-override.conf` | Conf file (shared with §8) |
| `PEER_QUARANTINE_ADVISORY_DIR` | `~/.maestro/advisories` | Per-peer advisory output |
| `PEER_QUARANTINE_CACHE_DIR` | `~/.maestro/cache` | Advisory dedup stamp dir |
...
```bash
H=~/.maestro/agents/scripts/pulse-peer-quarantine-helper.sh
...
jq -r '.peers["alex-solovyev"].events[]' \
  ~/.maestro/cache/peer-quarantine.json
...
ls -la ~/.maestro/advisories/peer-quarantine-alex-solovyev.advisory
```
