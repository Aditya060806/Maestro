...
Private slugs are enumerated from `initialized_repos[]` entries with `mirror_upstream` or `local_only: true`, plus optional extras in `~/.maestro/configs/privacy-guard-extra-slugs.txt`.
...
1. **`<repo>/.maestro.json` `.verify` block** (canonical):
...
- `MAESTRO_PREPUSH_AUTOFIX=1` (set explicitly): run `*_fix` on failure, `git add -A && git commit --amend --no-edit --no-verify`, re-run the check.
- `MAESTRO_PREPUSH_AUTOFIX=0`: emit a mentoring failure with the exact suggested fix command, exit 1.
- **Default (when unset):** ON in headless contexts (`FULL_LOOP_HEADLESS` / `MAESTRO_HEADLESS` / `OPENCODE_HEADLESS` / `GITHUB_ACTIONS`), OFF in interactive sessions. The interactive default keeps the user in the loop on automated commit-amends.
- **Typecheck never auto-fixes** — semantic failures need code changes regardless of `AUTOFIX`.

### Skip conditions (exit 0 fast)

- `MAESTRO_PREPUSH_REPO_VERIFY=0` (per-push bypass)
...
- One push: `MAESTRO_PREPUSH_REPO_VERIFY=0 git push ...`
- All hooks: `git push --no-verify`
- Disable autofix only: `MAESTRO_PREPUSH_AUTOFIX=0 git push ...`
- Debug discovery: `MAESTRO_PREPUSH_REPO_VERIFY_DEBUG=1 git push ...`
