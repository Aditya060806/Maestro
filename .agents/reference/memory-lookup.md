...
1. **Cross-session memory**: `memory-helper.sh recall "keywords"` (CLI) or `maestro_memory` MCP tool with `action: "recall"` — curated signal, most likely to have the answer. Solutions, decisions, preferences, patterns.
   ...
   Do not call `maestro_memory` with `{}` or missing required fields. Empty calls are invalid placeholders; if there is no concrete memory to recall/store, continue the task or create a worker-ready follow-up instead.
...
6. **Observability metrics**: `~/.maestro/.agent-workspace/observability/metrics.jsonl` — JSONL log of LLM request metadata, costs, models, tokens per session. Fields per record: provider, model, session_id, request_id, project, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, cost_input, cost_output, cost_total, stop_reason, git_branch, recorded_at. Example: `jq -s '[.[] | select(.project=="maestro")] | group_by(.model) | map({model:.[0].model, total_cost:([.[].cost_total] | add), requests:length})' ~/.maestro/.agent-workspace/observability/metrics.jsonl`
