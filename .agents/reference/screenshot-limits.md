...
The maestro OpenCode plugin intercepts the `experimental.chat.messages.transform`
...
3. Use the returned `$clean` path with the Read tool. The helper copies the file to `~/.maestro/.agent-workspace/tmp/session-PID/` with the U+202F bytes removed from the filename.

### Notes

- The helper is idempotent — calling it on an already-clean path prints the path unchanged without copying.
- The temp copy persists for the session (tied to the shell PID). On session end, it can be cleaned up with `rm -rf ~/.maestro/.agent-workspace/tmp/session-PID/`.
