...
When a critical fix merges (e.g., an OAuth auth regression that causes every dispatch to HTTP 401), remote runners continue running pre-fix code until their next scheduled `maestro update` cycle. The regular update check is rate-limited to ~24 hours, creating a window where remote runners are degraded.
...
Configure in `~/.maestro/configs/auto-hotfix.conf`:
...
The config file is deployed by `setup.sh` to `~/.maestro/configs/auto-hotfix.conf` on first setup. User edits survive `maestro update`.
