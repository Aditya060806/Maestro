...
`AGENTS.md` and the `prompts/build.txt` placeholder are loaded on every conversation turn — they are the full framework prompt cost before the model writes a single word. As maestro grows (new features, new edge cases, new lessons), the natural tendency is to append more inline context. Left unchecked, this reaches a point where the first "hi" costs 48k tokens.
...
import os
files = [
    os.path.expanduser('~/.maestro/agents/AGENTS.md'),
    os.path.expanduser('~/.maestro/agents/prompts/build.txt')
]
for f in files:
