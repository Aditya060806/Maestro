...
3. `maestro-update-check.sh` — calls `bash-upgrade-helper.sh ensure --yes --quiet` in the periodic update cycle. **Actually runs `brew upgrade bash` on drift** (v3.8.28 change — previously advisory-only). Rate-limited internally via `~/.maestro/state/brew-update-last-fetch` so multiple ensure calls in the same day share the fresh Homebrew index.
...
**Why actually upgrade, not just emit an advisory?** The framework's `maestro update` already auto-pulls and auto-deploys new versions of maestro every ~10 minutes — trusting it to `brew upgrade bash` (one binary, minor bumps only) is strictly less risky. Replacing a binary on disk does NOT kill running processes; existing bash processes continue on their in-memory copy. Only new invocations pick up the new version, and the re-exec guard makes that transparent.

**Opt-out**:
- Set `MAESTRO_AUTO_UPGRADE_BASH=0` in your shell profile to disable `ensure` (no auto install, no auto upgrade). You can still run `brew install bash` / `brew upgrade bash` manually at any time.
- Set `MAESTRO_BASH_REEXECED=1` to disable the runtime re-exec guard (scripts will run on whatever bash invoked them — e.g. `/bin/bash` 3.2 on macOS).
...
Regression test: `.agents/scripts/tests/test-bash-reexec-guard.sh` — 15 assertions (as of v3.8.28) covering detection, status output, guard positioning, loop prevention, the live re-exec fire path, `ensure` idempotence, and `MAESTRO_AUTO_UPGRADE_BASH=0` opt-out.
...
- **t2074 (GH#18784)** — #18686 added `getent passwd` to `maestro.sh` to fix Linux-sudo home resolution, omitted the `command -v getent` guard that `approval-helper.sh:33` had. Broke `sudo maestro approve` on macOS with `getent: command not found` for a full release cycle.
...
| `timeout <secs> <cmd>` | coreutils `gtimeout` if installed, else `perl -e 'alarm shift; exec @ARGV' <secs> <cmd>` | `maestro.sh:_timeout_cmd()` |
| `sed -i '...'` (GNU) | `sed -i '' '...'` (BSD requires empty suffix) | `maestro.sh:sed_inplace()` — `if Darwin; sed -i ''; else sed -i; fi` |
...
| `mktemp --suffix=.X` | BSD mktemp uses `-t prefix` differently | `mktemp -t maestro.XXXXXX` works on both; never use `--suffix`. |
...
| `readelf`, `ldd`, `strace`, `lsof` flags | Different or missing | Not typically used in maestro scripts; document if needed. |
...
|---------|----------|---------|
| `sed_inplace` | `maestro.sh:39` | In-place sed edit, BSD/GNU |
| `_timeout_cmd` | `maestro.sh:42-55` | Timeout command with perl fallback |
...
3. Is there a regression test that runs under the MISSING platform's conditions? (e.g., `test-maestro-sh-portability.sh` strips `getent` off PATH to simulate macOS.)
