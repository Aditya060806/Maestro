...
`Aditya060806/Maestro`) left several PRs with `Maintainer Gate` checks
...
1. **Classifies** every open PR that is `APPROVED + MERGEABLE + !draft +
   !hold-for-review` and idle past `MAESTRO_MERGE_STUCK_AGE_MINUTES` (default
...
4. **Detects pattern outages** — when ≥`MAESTRO_MERGE_PATTERN_MIN_PRS` (default
...
5. **Tracks zero-progress** across all repos. If a cycle has eligible-but-
   ...
   increments. Reaching `MAESTRO_MERGE_ZERO_PROGRESS_CYCLES` (default 5
   consecutive cycles ≈ 10 minutes) files a framework-level meta-issue at
   `Aditya060806/Maestro` dedup'd by `<!-- merge-stuck:zero-progress -->`.
...
take precedence (set in `~/.maestro/.env` or shell):

| Variable | Default | Purpose |
| --- | --- | --- |
| `MAESTRO_MERGE_STUCK_AGE_MINUTES` | `240` | Idle threshold (minutes) — only PRs idle this long are classified. |
| `MAESTRO_MERGE_ZERO_PROGRESS_CYCLES` | `5` | Consecutive zero-progress cycles before the framework-level meta-issue fires. |
| `MAESTRO_MERGE_PATTERN_MIN_PRS` | `3` | Minimum stuck PRs sharing one fingerprint before an outage meta-issue fires. |
| `MAESTRO_MERGE_STUCK_ENABLED` | `1` | Master kill-switch (set to `0` to disable the entire module). |
| `MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN` | `50` | Minimum absolute queued workflow runs before saturation is even considered. Set to `0` to disable saturation detection entirely (per-PR escalation behaviour for QUEUED checks reverts to pre-t3211). |
| `MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN` | `10` | Minimum `queued / max(in_progress,1)` ratio that, combined with the absolute minimum above, classifies the repo as saturated. Both conditions must hold — either alone is a false-positive (light-load bursts hit absolute counts; healthy busy periods hit ratio with the runner pool already serving). |
| `MAESTRO_SKIP_ACTIONS_QUEUE_SATURATION` | unset | Emergency bypass for the per-cycle saturation probe. When `1`, the helper short-circuits to `saturated=0` without making any `gh api` call. Use only if the saturation probe itself is degrading the pulse (e.g. severe REST rate-limit pressure). |
...
of every fresh maestro-init'd repo.
...
All four are written to `~/.maestro/logs/pulse-stats.json`:
...
falls back below `MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN` (default 50)
**and** the `queued / in_progress` ratio falls below
`MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN` (default 10). At that point
...
Set `MAESTRO_MERGE_STUCK_ENABLED=0` in your env. The module's entry point
...
- bypass + disable controls (`MAESTRO_SKIP_ACTIONS_QUEUE_SATURATION=1`,
  `MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN=0`)
