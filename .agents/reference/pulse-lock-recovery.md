...
|----------|---------|-------------|
| `MAESTRO_PULSE_LOCK_MAX_AGE_S` | `1800` | Age ceiling in seconds before force-reclaim |

Set via environment variable to adjust without code changes:

```bash
export MAESTRO_PULSE_LOCK_MAX_AGE_S=900  # more aggressive: 15 min
```

## Audit Trail

Every force-reclaim is logged to `$WRAPPER_LOGFILE` (`~/.maestro/logs/pulse-wrapper.log`):
...
records to `~/.maestro/logs/pulse-stage-timings.log`:
...
5. `MAESTRO_PULSE_LOCK_MAX_AGE_S` env override adjusts ceiling
