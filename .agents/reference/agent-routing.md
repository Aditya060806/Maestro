...
|-------|---------------|---------|
| Maestro | maestro, framework, setup, config, troubleshooting, MCP, agent, skill | Framework setup, configuration, troubleshooting, extension, releases |
...
```bash
AGENTS_DIR="$(maestro config get paths.agents_dir)"
AGENTS_DIR="${AGENTS_DIR:-"$HOME/.maestro/agents"}"
