...
All tables live in `~/.maestro/.agent-workspace/memory/memory.db` alongside `learnings`, `learning_access`, and `learning_relations`.
...
self-evolution-helper.sh pulse-scan --auto-todo-threshold 3 --repo-path ~/Git/maestro
```
...
```text
~/.maestro/.agent-workspace/memory/
