...
Env controls:
- `MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=off|detect|enforce` (default `detect` — logs `would-handover` without acting). Flip to `enforce` after 2-3 pulse cycles of clean `detect` telemetry.
...
|------|----------|---------|
| **Draft** | `~/.maestro/agents/draft/` | R&D, experimental, auto-created by orchestration tasks |
| **Custom** | `~/.maestro/agents/custom/` | User's permanent private agents |
