...
maestro is an opinionated framework. The deployed directory (`~/.maestro/agents/`) is a build artifact that is overwritten on every update. This guide explains how to customize without fighting the update cycle.

## Extension Points

| Directory | Purpose | Survives updates | Survives clean mode |
|-----------|---------|:---:|:---:|
| `~/.maestro/agents/custom/` | Your permanent personal agents and scripts | Yes | Yes |
| `~/.maestro/agents/draft/` | R&D, experimental agents (may be promoted) | Yes | Yes |
| `~/.maestro/agents/` (root) | Shared framework agents | No | No |
| `~/.maestro/agents/scripts/` | Deployed framework scripts | No | No |
...
```bash
mkdir -p ~/.maestro/agents/custom/scripts
cp my-script.sh ~/.maestro/agents/custom/scripts/
chmod +x ~/.maestro/agents/custom/scripts/my-script.sh
```

Run it: `~/.maestro/agents/custom/scripts/my-script.sh`
...
```bash
mkdir -p ~/.maestro/agents/custom
```

Create `~/.maestro/agents/custom/my-agent.md` with standard frontmatter:
...
**Do not edit files in `~/.maestro/agents/scripts/`** — these are deployed copies overwritten by every `maestro update` (runs automatically every ~10 minutes). Your edits will be silently lost.

**Do not edit files in `~/.maestro/agents/` (root level)** — same reason. These are framework agents deployed from the canonical source.
...
Deployed scripts differ from canonical source (local edits will be overwritten):
  ~/.maestro/agents/scripts/some-script.sh
    -> canonical: ~/Git/maestro/.agents/scripts/some-script.sh
```
...
2. **Bug fix?** Edit the canonical source at `~/Git/maestro/.agents/scripts/` and run `setup.sh --non-interactive`
...
2. **Use `/log-issue-maestro`** — it includes an architectural alignment check that routes customization needs away from the issue tracker
3. **Do not implement architectural changes** without maintainer approval, even if the change seems obviously beneficial

## Configuration Files

User configuration files (`~/.config/maestro/`) are never overwritten by updates:
