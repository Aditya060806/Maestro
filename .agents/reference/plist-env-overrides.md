...
losing them on every `maestro update` or `setup.sh` run.

## Problem This Solves

`setup.sh` regenerates launchd plists on every run (~every 10 min via
`maestro update`). Any manual edits to
`~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist`
are silently wiped.
...
cp ~/.maestro/agents/configs/plist-env-overrides.json.txt \
   ~/.maestro/agents/configs/plist-env-overrides.json

# 2. Edit the working file — add the env vars you want
#    Keys prefixed with _ are ignored (template examples)
nano ~/.maestro/agents/configs/plist-env-overrides.json

# 3. Run setup.sh to regenerate the plist with your overrides
~/.maestro/agents/../setup.sh --non-interactive
# or: maestro update
```
...
```json
{
  "com.maestro.maestro-supervisor-pulse": {
...
| Label | Plist |
|-------|-------|
| `com.maestro.maestro-supervisor-pulse` | `~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist` |
...
{
  "com.maestro.maestro-supervisor-pulse": {
    "MAESTRO_WORKER_BRIEFED_AUTO_MERGE": "0"
  }
}
```

After `maestro update`, verify the plist was updated:
...
override file for maestro-specific tuning variables only.
...
(deployed to `~/.maestro/agents/configs/plist-env-overrides.json`)
