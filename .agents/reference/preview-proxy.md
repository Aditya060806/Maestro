...
Start dev:  MAESTRO_PREVIEW_PORT=3142 pnpm dev:web
```
...
worktree-helper.sh add
  └─ preview_proxy_auto_allocate()        # best-effort, non-fatal
       └─ preview-proxy-helper.sh allocate <slug> <branch>
            ...
            ├─ Write to ~/.maestro/state/worktree-ports.json
...
| State file | `~/.maestro/state/worktree-ports.json` | Port allocation registry |
| Config file | `~/.config/maestro/preview-proxy.json` | Backend selection, domain template, port pools |
...
### Optional: `~/.config/maestro/preview-proxy.json`
...
### Optional: `.maestro.json` (per-project)
