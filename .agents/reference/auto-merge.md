...
7. Merge throughput is explicitly opted in by one of:
   ...
   - Env: `MAESTRO_INTERACTIVE_PR_AUTO_MERGE=1` (current process)
   - Global config: `orchestration.interactive_pr_auto_merge: true`
   - Per-repo `~/.config/maestro/repos.json`: `"interactive_pr_auto_merge": true`
...
Feature flag: `MAESTRO_WORKER_BRIEFED_AUTO_MERGE` (default `1`=on). Set to `0` to fall back to manual-merge-only for `origin:worker` PRs.
...
**Config file:** `.agents/configs/trusted-issue-authors.conf` — one GitHub login per line; `#` comments and blank lines ignored. Deployed copy: `~/.maestro/agents/configs/trusted-issue-authors.conf`. Override path: `MAESTRO_TRUSTED_AUTHORS_CONF` env var.
...
Cryptographic approval (`sudo maestro approve issue N`) requires the maintainer's root-protected SSH key, which workers cannot access — this is the only reliable human-in-the-loop signal.
...
**Trust chain:** `sudo maestro approve` requires a root-owned SSH private key that workers cannot forge. The signal is stronger than GitHub author-association, which is a proxy for maintainer trust. The four-layer GH#17671 defence-in-depth is preserved — Case P (CONTRIBUTOR + no crypto = refuse) is pinned by `test-pulse-merge-approve-collaborator-guard.sh`.

**Helper:** `_has_maintainer_crypto_approval "$pr_number" "$repo_slug"` in `pulse-merge-gates.sh`. Checks PR-level comments first, then linked-issue comments, for `<!-- maestro-signed-approval -->`. Falls back to marker-presence check when `approval-helper.sh` is unavailable (full crypto verification provided by `_external_pr_linked_issue_crypto_approved` downstream).
...
- **Circuit-breaker trip** (`stale-recovery-tick:escalated`, `cost-circuit-breaker:fired`, `circuit-breaker-escalated` comment markers) → t2007/t2008 safety mechanism fired after retry/cost limit exceeded; auto-approval PRESERVES NMR. Clear with `sudo maestro approve issue <N>` once the underlying problem is fixed.
...
**Problem:** before t3068, `sudo maestro approve issue|pr <N>` posted a verified signature comment but the pulse-merge cycle was up to ~120s away. The maintainer had no feedback that the approval had landed in the merge queue, and ~2 min × N stuck PRs added up across an interactive cleanup session.
...
1. **Marker file** at `~/.maestro/cache/pulse-merge-trigger.txt` — append a tab-separated record `<slug>\t<num>\t<type>\t<iso8601_ts>`. Crash-safe and idempotent: even if the pulse process is dead, the marker survives until the next cycle drains it.
...
- `MAESTRO_SKIP_APPROVE_KICK_PULSE=1` — disable both layers (used in CI / hermetic tests).
- `MAESTRO_SKIP_TRIGGER_DRAIN=1` — disable the drain (test-only knob to verify no-spawn path).
...
jq -r '.initialized_repos[] | select(.local_only != true) | .slug' \
    ~/.config/maestro/repos.json | while read -r slug; do
...
Observed 2026-04-30 in Aditya060806/Maestro:
...
**Threshold override:** `MAESTRO_PULSE_AUTO_MERGE_STUCK_SECONDS` (integer seconds, default 300). Raise to defer longer; lower to fall through faster.
...
1. **Generate the webhook secret** (≥32 random bytes, hex):
   ...
   ```bash
   openssl rand -hex 32 | maestro secret set GITHUB_WEBHOOK_SECRET
   # Or if not using gopass:
   #   echo "export GITHUB_WEBHOOK_SECRET='<hex-secret>'" >> ~/.config/maestro/credentials.sh
...
3. **Expose the receiver via Cloudflare Tunnel** (canonical exposure path; the receiver binds to `127.0.0.1:9301` only and never speaks plaintext HTTP to the public internet):
   ...
   ```bash
   cloudflared tunnel create maestro-webhook
   ...
   cloudflared tunnel route dns maestro-webhook hooks.example.com
   cloudflared tunnel run maestro-webhook
   ```
...
4. **Install the launchd service** (macOS — for Linux, install as a systemd user service following the same ProgramArguments):
   ...
   sed -e "s|{{HOME}}|$HOME|g" \
       ...
       -e "s|{{MAESTRO_AGENTS_SCRIPTS}}|$HOME/.maestro/agents/scripts|g" \
       ~/.maestro/agents/templates/launchd/sh.maestro.merge-webhook-receiver.plist.tmpl \
     > ~/Library/LaunchAgents/sh.maestro.merge-webhook-receiver.plist
   launchctl load ~/Library/LaunchAgents/sh.maestro.merge-webhook-receiver.plist
   ```
