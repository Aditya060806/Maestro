...
| Name | Pattern | Source |
|------|---------|--------|
| `spdx-header` | `SPDX-FileCopyrightText: 2025-2026 Aditya Pandey and Harvest` | All framework files |
| `maestro-sh-domain` | `maestro.sh` | Signature footers, docs |
| `shared-constants-guard` | `_SHARED_CONSTANTS_LOADED` | `shared-constants.sh` |
| `pulse-wrapper-label` | `sh.maestro.pulse` | `pulse-wrapper.sh` |
...
attribution-detection-helper.sh canary add my-comment "# maestro: my-unique-comment" "Custom comment"
```
...
Keep exact search strings private, but monitor for distinctive maestro
...
jq '(.[] | select(.name == "spdx-header") | .attributed_repos) += ["owner/repo"]' \
  ~/.maestro/cache/attribution-canaries.json > /tmp/canaries.json
mv /tmp/canaries.json ~/.maestro/cache/attribution-canaries.json
```
...
All state in `~/.maestro/cache/` — not committed to the public repo.

| File | Purpose |
|------|---------|
| `~/.maestro/cache/attribution-canaries.json` | Registered canary patterns |
| `~/.maestro/cache/attribution-detections.json` | Latest scan results |
