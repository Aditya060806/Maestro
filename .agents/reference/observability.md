...
The opencode-maestro plugin captures every LLM request and tool call to
`~/.maestro/.agent-workspace/observability/llm-requests.db`.
...
The opencode-maestro plugin shell-env hook forwards these variables to
...
The plugin enriches each active tool span with maestro attributes
via `@opentelemetry/api` (dynamic import, no-op when absent):

| Attribute                   | Value                                    |
|-----------------------------|------------------------------------------|
| `maestro.intent`           | The `agent__intent` string for the call  |
| `maestro.tool_name`        | Tool name (Read, Edit, Bash, …)          |
| `maestro.task_id`          | `tNNN` / `GH#NNN` detected from cwd/env  |
| `maestro.session_origin`   | `worker` or `interactive`                |
| `maestro.runtime`          | `opencode`                               |

Namespace `maestro.*` avoids collision with OpenTelemetry's standard
...
- **kit motel** — the dedicated opencode/maestro OTel TUI, still maturing
...
the session. No further maestro configuration is needed.
...
`~/.maestro/.agent-workspace/output-sandbox/` with a SQLite index and WAL mode.
...
**Impact on maestro plugin:** the plugin's `enrichActiveSpan()` in
...
`undefined` and the enrichment silently no-ops. The `maestro.*`
...
under a separate service name (`opencode-maestro`), disconnected
...
4. Check for `maestro.*` attributes. **In TUI/server mode**, these
...
If maestro attributes are missing in a mode where they should work:

- Verify the plugin loaded: `MAESTRO_PLUGIN_DEBUG=1 opencode run ...`
  should emit `[maestro]` startup lines.
