...
The deployed copy at `~/.maestro/agents/scripts/FILE` may differ from source at `~/Git/maestro/.agents/scripts/FILE`. Pulse executes the deployed copy — reading source-as-truth when debugging runtime symptoms wastes hours.
...
- `worker-NNN.log` mtime (`ls -lt ~/.maestro/logs/worker-*.log`) — file touch time, not outcome. A killed worker still leaves a recently-touched log.
...
For provider/model/account questions, use `worker-activity-helper.sh providers --since 1h` before reading raw logs. Do not run broad recursive searches over `~/.maestro/logs` or OpenCode storage for routine diagnostics; use bounded helpers first, then inspect a named log only if the helper identifies a specific timeframe or failure class.
