...
```bash
~/.maestro/agents/scripts/report-token-use-helper.sh report --limit 25
~/.maestro/agents/scripts/report-token-use-helper.sh report --session ses_abc123 --open
~/.maestro/agents/scripts/report-token-use-helper.sh report --daily-days 90
~/.maestro/agents/scripts/report-token-use-helper.sh data --json --since 7d
```

## Output Contract

- Local artifacts live under `~/.maestro/_reports/token-use/<UTC-run-id>/`.
...
| OpenCode tools | `~/.maestro/.agent-workspace/observability/llm-requests.db` | Adds model request counts, tool-call counts, and observed MCP tool names when available. |
| Claude fallback | `~/.maestro/.agent-workspace/observability/metrics.jsonl` | Best-effort token totals; compaction and MCP fields may be empty. |
...
Generated artifacts are local to the user's maestro directory and are not meant
