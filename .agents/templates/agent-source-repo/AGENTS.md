...
This repository is an maestro private agent source. Keep the data-flow contract in
...
- Keep paths under `~/.maestro/.agent-workspace/work/<pack-name>/` unless the
...
<!-- maestro:agent-source-template:start -->
<!-- maestro:agent-source-template-version: 1 -->

# Private Agent Source Repository

This repo stores private maestro agents and shared capabilities. Keep the structure aligned with the core framework so agents can move between private packs and the public framework without re-learning conventions.
...
Register this repo with `maestro sources add <path>` for private-agent sync. Mark it in `~/.config/maestro/repos.json` with `"agent_source": true` or `"role": "agent-source"` so `maestro init` and `maestro update` can seed and refresh the framework-owned organization guidance. Sync preserves the full `.agents/` tree under `~/.maestro/agents/custom/<source-name>/`, symlinks root `mode: primary` agents into `~/.maestro/agents/`, and deploys slash commands from `.agents/scripts/commands/`.

<!-- maestro:agent-source-template:end -->
