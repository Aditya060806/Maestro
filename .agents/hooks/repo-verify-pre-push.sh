...
#   1. <repo_root>/.maestro.json `.verify` block
...
#   - MAESTRO_PREPUSH_AUTOFIX=1: run `*_fix` on failure; if files changed,
#     `git add -A && git commit --amend --no-edit`; re-run check.
#   - MAESTRO_PREPUSH_AUTOFIX=0: emit mentoring failure with exact suggested
#     fix command; exit 1.
#   - Default: 1 in headless contexts (FULL_LOOP_HEADLESS / MAESTRO_HEADLESS /
#     OPENCODE_HEADLESS / GITHUB_ACTIONS), 0 in interactive sessions.
#
# Skip conditions (exit 0 fast):
#   - MAESTRO_PREPUSH_REPO_VERIFY=0
...
#
# Bypass for one push: MAESTRO_PREPUSH_REPO_VERIFY=0 git push ...
...
if [[ "${MAESTRO_PREPUSH_REPO_VERIFY:-1}" == "0" ]]; then
	printf '[%s][INFO] MAESTRO_PREPUSH_REPO_VERIFY=0 — bypassing\n' "$GUARD_NAME" >&2
	exit 0
fi
...
_dbg() {
	local _msg="$1"
	if [[ "${MAESTRO_PREPUSH_REPO_VERIFY_DEBUG:-0}" == "1" ]]; then
		printf '[%s][DBG] %s\n' "$GUARD_NAME" "$_msg" >&2
	fi
}
...
DEFAULTS_DEPLOYED="${HOME}/.maestro/agents/configs/repo-verify-defaults.conf"
...
# Auto-fix default: ON in headless, OFF interactive. MAESTRO_PREPUSH_AUTOFIX
# (when set) wins over the heuristic.
_autofix_default() {
	if [[ -n "${FULL_LOOP_HEADLESS:-}${MAESTRO_HEADLESS:-}${OPENCODE_HEADLESS:-}${GITHUB_ACTIONS:-}" ]]; then
		printf '1\n'
	else
		printf '0\n'
	fi
}
AUTOFIX="${MAESTRO_PREPUSH_AUTOFIX:-$(_autofix_default)}"
...
# Load .verify block from <repo_root>/.maestro.json if present and enabled.
# Returns 0 on hit, 1 on miss/disabled.
_load_verify_from_maestro_json() {
	local cfg="${REPO_ROOT}/.maestro.json"
	[[ -f "$cfg" ]] || return 1
	command -v jq >/dev/null 2>&1 || { _dbg "jq missing — skipping .maestro.json"; return 1; }
	...
	if [[ "$enabled" == "false" ]]; then
		_dbg ".maestro.json .verify.enabled=false — opting out"
		VERIFY_SOURCE='maestro-json-disabled'
		return 0
	fi
	...
	if [[ -n "$VERIFY_FORMAT$VERIFY_LINT$VERIFY_TYPECHECK" ]]; then
		VERIFY_SOURCE='maestro-json'
		_dbg ".maestro.json hit: format=${VERIFY_FORMAT:-<none>} lint=${VERIFY_LINT:-<none>} typecheck=${VERIFY_TYPECHECK:-<none>}"
		return 0
	fi
	return 1
}
...
_run_check() {
	...
	local cmd="$2"
	local log
	log=$(mktemp -t "maestro-prepush-${name}.XXXXXX")
...
_run_autofix() {
	...
	if [[ "$AUTOFIX" != "1" ]]; then
		_log INFO "$name autofix skipped (MAESTRO_PREPUSH_AUTOFIX=0)"
		return 1
	fi
...
_emit_mentor_fail() {
	...
	if [[ -n "$fix_cmd" ]]; then
		...
		printf '    MAESTRO_PREPUSH_AUTOFIX=1 git push\n' >&2
	else
	...
	printf '    MAESTRO_PREPUSH_REPO_VERIFY=0 git push\n' >&2
	printf '\n' >&2
}
...
main() {
	if _load_verify_from_maestro_json; then
		if [[ "$VERIFY_SOURCE" == "maestro-json-disabled" ]]; then
			_log INFO "repo opts out via .maestro.json .verify.enabled=false"
			exit 0
		fi
