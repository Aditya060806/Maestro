...
Installed by: maestro setup (setup.sh) or install-hooks-helper.sh
Location: ~/.maestro/hooks/git_safety_guard.py
...
Adapted for maestro framework (https://maestro.sh)
...
Environment overrides:
  MAESTRO_SKIP_CANONICAL_GUARD=1  — bypass the off-default-branch canonical guard
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": (
                f"BLOCKED by git_safety_guard.py (maestro t1990)\n\n"
                ...
                f"        MAESTRO_SKIP_CANONICAL_GUARD=1 <your-command>"
            ),
        }
    }
...
def _canonical_branch_switch_deny(target: str, default_branch: str) -> dict:
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecision": "deny",
            "permissionDecisionReason": (
                "BLOCKED by git_safety_guard.py (maestro t1990)\n\n"
...
def _canonical_branch_switch_user_request_deny(target: str, default_branch: str) -> dict:
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecision": "deny",
            "permissionDecisionReason": (
                "BLOCKED by git_safety_guard.py (maestro t3573)\n\n"
...
def _check_main_branch_allowlist(file_path: str) -> "dict | None":
    ...
    if _is_linked_worktree(repo_root) or os.environ.get("MAESTRO_SKIP_CANONICAL_GUARD"):
        return None
    ...
    if branch == default_branch:
        ...
        return {
            "hookSpecificOutput": {
                ...
                "permissionDecision": "deny",
                "permissionDecisionReason": (
                    f"BLOCKED by git_safety_guard.py (maestro t1712)\n\n"
        ...
        if re.search(pattern, command):
            output = {
                "hookSpecificOutput": {
                    ...
                    "permissionDecision": "deny",
                    "permissionDecisionReason": (
                        f"BLOCKED by git_safety_guard.py (maestro)\n\n"
                        f"Reason: {reason}\n\n"
