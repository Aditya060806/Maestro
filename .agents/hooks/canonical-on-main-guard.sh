...
#
# Environment:
#   MAESTRO_CANONICAL_GUARD=repair   — default: restore the default branch
#   MAESTRO_CANONICAL_GUARD=strict   — restore the default branch and exit 1
#   MAESTRO_CANONICAL_GUARD=bypass   — suppress the warning entirely
#   MAESTRO_CANONICAL_GUARD=warn     — warn loudly but do not restore
#
# Fail-open cases (exit 0, no warning):
#   - Headless session (FULL_LOOP_HEADLESS / MAESTRO_HEADLESS / OPENCODE_HEADLESS / GITHUB_ACTIONS set)
...
# Explicit bypass
if [[ "${MAESTRO_CANONICAL_GUARD:-repair}" == "bypass" ]]; then
	exit 0
fi
...
restore_default_branch() {
	...
	MAESTRO_CANONICAL_GUARD=bypass git checkout "$default_branch" --quiet 2>/dev/null
	return $?
}
...

{
	...
	printf '  MAESTRO_CANONICAL_GUARD=bypass git checkout <branch>\n'
	printf '\n'
	printf 'To warn without restoring:\n'
	printf '  MAESTRO_CANONICAL_GUARD=warn git checkout <branch>\n'
	printf '\n'
	printf 'To enforce strict mode (restore, then fail the checkout):\n'
	printf '  MAESTRO_CANONICAL_GUARD=strict git checkout <branch>\n'
	printf '\n'
	printf '%s============================================================%s\n' "$RED" "$NC"
	printf '\n'
} >&2

case "${MAESTRO_CANONICAL_GUARD:-repair}" in
warn)
	exit 0
	;;
strict)
