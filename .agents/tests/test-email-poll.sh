...
test_tick_missing_config_skips() {
	...
	if [[ "$exit_rc" -eq 0 ]] && [[ "$output" == *"skipped"* ]] && [[ "$output" == *"maestro email mailbox add"* ]]; then
		pass "$name"
	else
...
test_missing_credentials() {
	...
	make_mailbox_config "${tmpdir}/mailboxes.json" "nocreds-mb" "MAESTRO_NONEXISTENT_ENV_XYZ999"
	unset MAESTRO_NONEXISTENT_ENV_XYZ999 2>/dev/null || true
