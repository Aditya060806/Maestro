...
LOGFILE="${TMPDIR:-/tmp}/maestro-pulse-cache-test.log"
TESTS_PASSED=0
TESTS_FAILED=0
...
_gh_collaborator_permission_lookup() {
	...
	MAESTRO_GH_COLLAB_PERMISSION_HTTP=200
	return 0
}
...
test_author_permission_lookup_uses_cycle_cache() {
	...
	cache_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-author-cache-test.XXXXXX") || return 1
	MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR="$cache_dir"
	_gh_permission_calls=0
	...
	rm -rf -- "$cache_dir"
	unset MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR
...
_GH_REPO_CALLS_FILE="${TMPDIR:-/tmp}/maestro-context-cache-repo-calls.$$"
_GH_PROTECTION_CALLS_FILE="${TMPDIR:-/tmp}/maestro-context-cache-protection-calls.$$"
_GH_RULESET_CALLS_FILE="${TMPDIR:-/tmp}/maestro-context-cache-ruleset-calls.$$"
...
test_required_context_lookup_uses_cycle_cache() {
	...
	cache_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-context-cache-test.XXXXXX") || return 1
	MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR="$cache_dir"
	printf '0\n' >"$_GH_REPO_CALLS_FILE"
	...
	rm -rf -- "$cache_dir"
	unset MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR
