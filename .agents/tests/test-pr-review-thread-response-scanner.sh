...
test_scan_passes_nonempty_owner_and_name_to_graphql() {
	...
	output="$(FAKE_GH_CAPTURE="$capture" PATH="${fake_bin}:$PATH" "$SCANNER" scan "Aditya060806/Maestro")"
	if [[ "$output" == 123$'\t'* ]]; then
	...
	if grep -q -- '-F owner=Aditya060806' "$capture" && grep -q -- '-F name=maestro' "$capture"; then
...
test_reply_recognises_existing_mention_after_whitespace_and_comment() {
	...
	if FAKE_GH_CAPTURE="$capture" FAKE_GH_REPLY_CAPTURE="$reply_capture" PATH="${fake_bin}:$PATH" "$SCANNER" reply "Aditya060806/Maestro" "THREAD_1" "$body_file"; then
...
test_dispatch_prompt_includes_thread_fingerprint() {
	...
	local prompt_file="${state_dir}/Aditya060806-maestro-123-prompt.md"
	_write_fake_gh "$fake_bin"
...
FAKE_HEADLESS
	...
	if FAKE_GH_CAPTURE="$capture" \
		MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR="$state_dir" \
		HEADLESS_RUNTIME_HELPER="$fake_headless" \
		PATH="${fake_bin}:$PATH" \
		"$SCANNER" dispatch-pr "Aditya060806/Maestro" "$TEST_TMPDIR" "123"; then
