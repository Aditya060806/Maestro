...
| 275 | GH#17068 | pre-existing regression from t1880/t1881 attribution protection work — maestro.sh grew by ~35 lines adding signing verification and status checks; awk depth checker counts all if/case/for across the entire file without function-boundary resets, inflating the count for large scripts |
...
| 34 | GH#21181/t2951 (tracking: GH#21186/t2952) | pre-existing drift on main — 32 smells vs threshold 30; top offenders: email_poll.py (4 smells: file-complexity, function-complexity×2, function-parameters×1), knowledge_index_helpers.py (2 smells: file-complexity + function-complexity), opencode-maestro plugins (7 files × 1 smell each: oauth-pool-auth, oauth-pool-callback, oauth-pool-display, observability, provider-auth-request, quality-hooks-signature × 1 qlty:file-complexity each); all pre-existing; t2951 PR adds only YAML + shell test changes that qlty does not flag; 32 actual + 2 buffer = 34; deferred refactors for email_poll.py and knowledge_index_helpers.py tracked as follow-up issues |
