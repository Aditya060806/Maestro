...
Vault agent: protect maestro confidential data across local stores, managed
...
- **Fleet:** `workflows/vault-fleet.md` for device trust, encrypted sync,
  ...
  replication.
- **Command docs:** `scripts/commands/vault.md` before explaining `maestro
  vault`, `maestro fleet`, or dispatch/preflight Vault metadata.
...
5. Before reading protected data, require explicit Vault access evidence such as
   sanitized `maestro vault status`/`setup-state`, deterministic helper success,
