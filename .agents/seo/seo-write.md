...
1. **Research**: Determine search intent for the target keyword
   ...
   ```bash
   python3 ~/.maestro/agents/scripts/seo-content-analyzer.py intent "$ARGUMENTS"
   ```
...
2. **Context**: Check for project context files; read any that exist before writing
   ...
   ls .maestro/context/brand-voice.md .maestro/context/style-guide.md 2>/dev/null
   ```
...
4. **Analyze**: Run content analysis and fix any critical issues
   ...
   ```bash
   python3 ~/.maestro/agents/scripts/seo-content-analyzer.py analyze draft.md \
