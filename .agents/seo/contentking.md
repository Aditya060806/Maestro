...
- **Auth**: Bearer token (`Authorization: token $CONTENTKING_API_TOKEN`), stored in `~/.config/maestro/credentials.sh`
...
```bash
bash ~/.maestro/agents/scripts/setup-local-api-keys.sh set CONTENTKING_API_TOKEN "your_token"
```
