...
- **Auth**: Basic auth via `DATAFORSEO_USERNAME` + `DATAFORSEO_PASSWORD` in `~/.config/maestro/credentials.sh`
...
```bash
bash ~/.maestro/agents/scripts/setup-local-api-keys.sh set DATAFORSEO_USERNAME "your_username"
bash ~/.maestro/agents/scripts/setup-local-api-keys.sh set DATAFORSEO_PASSWORD "your_password"
```
