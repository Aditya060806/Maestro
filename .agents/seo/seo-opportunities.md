...
2. Export all configured platforms: `~/.maestro/agents/scripts/seo-export-helper.sh all $DOMAIN --days $DAYS`
3. Run full analysis: `~/.maestro/agents/scripts/seo-analysis-helper.sh $DOMAIN`
4. Summarize: top 10 quick wins, top 10 striking-distance opportunities, low-CTR pages, content cannibalization issues.

## Outputs

```text
~/.maestro/.agent-workspace/work/seo-data/{domain}/{platform}-{start}-{end}.toon
~/.maestro/.agent-workspace/work/seo-data/{domain}/analysis-{date}.toon
```
