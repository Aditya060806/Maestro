...
- **Script**: `~/.maestro/agents/scripts/gsc-sitemap-helper.sh`
- **Config**: `~/.config/maestro/gsc-config.json`
- **Profile**: `~/.maestro/.agent-workspace/chrome-gsc-profile/`
- **Screenshots**: `~/.maestro/.agent-workspace/gsc-screenshots/` (verification)
...
> All commands use full path `~/.maestro/agents/scripts/gsc-sitemap-helper.sh`, or add `~/.maestro/agents/scripts` to PATH.
...
Store in `~/.config/maestro/gsc-config.json`:

```json
{
  "chrome_profile_dir": "~/.maestro/.agent-workspace/chrome-gsc-profile",
  "default_sitemap_path": "sitemap.xml",
  "screenshot_dir": "~/.maestro/.agent-workspace/gsc-screenshots",
  "timeout_ms": 60000,
  "headless": false
}
```
...
| Problem | Solution |
|---------|----------|
| "Browser isn't secure" warning | Close all Chrome, delete profile (`rm -rf ~/.maestro/.agent-workspace/chrome-gsc-profile`), re-run `gsc-sitemap-helper.sh login` |
| "No access" error | Domain not verified in GSC — check https://search.google.com/search-console |
| Submit button not clicking | Check screenshot in `~/.maestro/.agent-workspace/gsc-screenshots/`; script uses DOM traversal to find correct button (not feedback link) |
