...
# which is invoked by cmd_check after the main maestro update logic.
#
...
_locate_helper_script() {
	...
	local candidate="$HOME/.maestro/agents/scripts/${filename}"
	if [[ -x "$candidate" ]]; then
		echo "$candidate"
		return 0
	fi
...
check_launchd_plist_drift() {
	...
	# opt-out scenarios.
	local drift_check_interval="${MAESTRO_PLIST_DRIFT_CHECK_INTERVAL:-21600}"
...
# Only runs when user has been idle for MAESTRO_TOOL_IDLE_HOURS.
# Delegates to tool-version-check.sh --update --quiet.
# Called from cmd_check after other freshness checks.
# Respects config: maestro config set updates.tool_auto_update false
#######################################
#######################################
...
_locate_upstream_watch() {
	local agents_dir="${MAESTRO_AGENTS_DIR:-$HOME/.maestro/agents}"
...
# Respects config: maestro config set updates.venv_health_check false
#######################################
check_venv_health() {
	# Opt-out via config (env var or config file)
