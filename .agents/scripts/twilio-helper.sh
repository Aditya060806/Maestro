...
# Config can be in repo configs/ or ~/.maestro/configs/
if [[ -f "${SCRIPT_DIR}/../../configs/twilio-config.json" ]]; then
    CONFIG_FILE="${SCRIPT_DIR}/../../configs/twilio-config.json"
elif [[ -f "${HOME}/.maestro/configs/twilio-config.json" ]]; then
    CONFIG_FILE="${HOME}/.maestro/configs/twilio-config.json"
else
    CONFIG_FILE="${SCRIPT_DIR}/../../configs/twilio-config.json"
fi
...
make_call() {
    ...
    if [[ -n "$twiml" ]]; then
        ...
        # Default TwiML
        data="$data -d Twiml=<Response><Say>Hello from Maestro!</Say></Response>"
    fi
