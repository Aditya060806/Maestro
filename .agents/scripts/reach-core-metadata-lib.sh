...
# =============================================================================
# maestro Reach Core Metadata Library
# =============================================================================
...
command_available() {
	...
	if [[ "${MAESTRO_REACH_TEST_FORCE_MISSING:-}" == "1" ]]; then
		return 1
	fi
...
capability_available() {
	...
	case "$capability_key" in
		...
		browser)
			...
			;;
		persistent_profile)
			if helper_available dev-browser-helper.sh || [[ -d "${HOME}/.maestro/.agent-workspace/browser-profiles" ]]; then
				return 0
			fi
			;;
		cookie_session)
