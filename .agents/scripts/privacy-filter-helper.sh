...
[[ -z "${NC+x}" ]] && NC='\033[0m'
readonly MAESTRO_DIR="${HOME}/.maestro"
readonly CONFIG_DIR="${MAESTRO_DIR}/config"
readonly PATTERNS_FILE="${CONFIG_DIR}/privacy-patterns.txt"
readonly PROJECT_PATTERNS=".maestro/privacy-patterns.txt"
...
scan_privacy() {
	...
	if [[ $findings -eq 0 ]]; then
		...
		print_info "  2. Add patterns to .maestro/privacy-patterns.txt to customize"
...
CONFIGURATION:
    Global patterns:  ~/.maestro/config/privacy-patterns.txt
    Project patterns: .maestro/privacy-patterns.txt
