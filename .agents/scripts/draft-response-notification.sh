...
_ensure_draft_repo() {
	...
	gh repo create "$DRAFT_REPO_NAME" --private \
		--description "Private draft responses for external contribution replies (managed by maestro)" \
...
_create_notification_issue() {
	...
	local sig_footer=""
	sig_footer=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$issue_body" 2>/dev/null || true)
