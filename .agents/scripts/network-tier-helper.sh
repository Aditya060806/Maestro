...
#   - User overrides: ~/.config/maestro/network-tiers-custom.conf
#
...
readonly NET_TIER_DIR="${HOME}/.maestro/.agent-workspace/network"
...
init() {
	...
	if [[ ! -r "$NET_TIER_DEFAULT_CONF" ]]; then
		...
		echo "Run 'maestro update' to restore default configuration."
		return 1
	fi
...
Config files:
  ...
  Custom:  ~/.config/maestro/network-tiers-custom.conf

Log files:
  Access:  ~/.maestro/.agent-workspace/network/access.jsonl
  Flagged: ~/.maestro/.agent-workspace/network/flagged.jsonl
  Denied:  ~/.maestro/.agent-workspace/network/denied.jsonl
