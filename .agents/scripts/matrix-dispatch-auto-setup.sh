...
_auto_setup_parse_args() {
	...
	local bot_display="Maestro Bot" runners="" allowed_users=""
...
_auto_setup_dry_run() {
	...
	echo "  5. Store credentials via maestro secret"
...
_auto_setup_register_and_login() {
	...
	if [[ -z "$admin_token" ]]; then
		...
		admin_token=$(gopass show "maestro/${secret_name}" 2>/dev/null || true)
		...
		if [[ -z "$admin_token" ]]; then
			...
			echo "  3. Store it: maestro secret set ${secret_name}"
			echo ""
...
_auto_setup_store_credentials() {
	...
	if command -v gopass &>/dev/null; then
		printf '%s' "$bot_password" | gopass insert -f "maestro/${secret_prefix}_PASSWORD" 2>/dev/null || {
			log_warn "Failed to store bot password in gopass"
		}
		printf '%s' "$bot_access_token" | gopass insert -f "maestro/${secret_prefix}_TOKEN" 2>/dev/null || {
			log_warn "Failed to store bot token in gopass"
		}
		log_success "Credentials stored in gopass (maestro/${secret_prefix}_*)"
	else
		log_warn "gopass not available — credentials stored only in config file"
		log_info "Install gopass for encrypted credential storage: maestro secret set"
	fi
	return 0
}
...
_auto_setup_create_rooms() {
	...
	local runner_helper="$HOME/.maestro/agents/scripts/runner-helper.sh"
...
_auto_setup_summary() {
	...
	if command -v gopass &>/dev/null; then
		...
		echo "  maestro/${secret_prefix}_PASSWORD"
		echo "  maestro/${secret_prefix}_TOKEN"
	fi
	return 0
}
...
_auto_setup_usage() {
	...
	echo "  --bot-display <name>   Bot display name (default: Maestro Bot)"
...
#   --bot-display <name>   Bot display name (default: Maestro Bot)
