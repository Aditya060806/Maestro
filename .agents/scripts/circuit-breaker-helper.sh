...
_cb_state_dir() {
	local dir="${SUPERVISOR_DIR:-${HOME}/.maestro/.agent-workspace/supervisor}"
	mkdir -p "$dir" || true
	echo "$dir"
	return 0
}
...
3. Reset the circuit breaker (one-line copy/paste):
   \`\`\`bash
   maestro circuit-breaker reset
   \`\`\`
   Or use the short alias: \`maestro cb reset\`.
   Direct helper path (if \`maestro\` CLI unavailable): \`circuit-breaker-helper.sh reset\`.
...
_cb_create_new_issue() {
	...
	local sig_footer=""
	sig_footer=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$body" 2>/dev/null || true)
...
- \`~/.maestro/logs/pulse-wrapper.log\` — pulse-side dispatch trace; grep for the failed task ID and surrounding cycle.
- \`~/.maestro/logs/headless-runtime-metrics.jsonl\` — per-worker outcome ledger; filter \`select(.session_key | contains(\"${last_task_id}\"))\` for terminal events.
- \`~/.maestro/.agent-workspace/tmp/pulse/pulse-<repo>-<task>.log\` (or \`$XDG_RUNTIME_DIR/maestro/pulse/\`) — per-worker stdout/stderr if still on disk.
- \`~/.maestro/.agent-workspace/headless-runtime/canary-last-fail*\` — canary failure artifacts when the canary itself is the live blocker.
- \`~/.maestro/logs/dispatch-stages.tsv\` — dispatch-stage timing; recent rows show how far each spawn got before failing.
...
cmd_help() {
	...
	echo "  maestro circuit-breaker [status|reset|check|trip]   (alias: cb)"
	echo ""
	...
	echo "  SUPERVISOR_DIR                             State directory (default: ~/.maestro/.agent-workspace/supervisor)"
	return 0
}
