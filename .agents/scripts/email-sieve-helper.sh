...
# Credentials: maestro secret set SIEVE_PASSWORD
#
# Part of maestro email system (t1503)
...
readonly DEFAULT_OUTPUT_FILE="/tmp/maestro-generated.sieve"
readonly DEFAULT_SCRIPT_NAME="maestro-triage"
...
# Generated by maestro email-sieve-helper.sh
...
deploy_sieve() {
	...
	if [[ -z "${SIEVE_PASSWORD:-}" ]]; then
		...
		print_info "Or: maestro secret set SIEVE_PASSWORD"
		return 1
	fi
	...
	helper_tmp="$(mktemp /tmp/maestro-managesieve-XXXXXX)"
...
GENERATE OPTIONS
  --output <file>       Output file (default: /tmp/maestro-generated.sieve)
...
DEPLOY OPTIONS
  ...
  --script <name>       Script name on server (default: maestro-triage)
  --file <sieve-file>   Sieve file to deploy (default: /tmp/maestro-generated.sieve)

  Password: set SIEVE_PASSWORD environment variable before deploying.
  Never pass password as argument.

VALIDATE OPTIONS
  --file <sieve-file>   File to validate (default: /tmp/maestro-generated.sieve)
...
EXAMPLES
  ...
  email-sieve-helper.sh show-script --server mail.example.com --user me@example.com --script maestro-triage
