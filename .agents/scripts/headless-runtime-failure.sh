...
# =============================================================================
# maestro Headless Runtime Failure — Dispatch Claim & Fast-Fail (GH#19699)
# =============================================================================
...
: "${MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_THRESHOLD:=3}"
: "${MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_LOOKBACK_SECS:=86400}"
: "${MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_COOLDOWN_SECS:=1800}"
...
_hrff_rate_limit_release_circuit_active() {
	...
	if [[ "${MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_OVERRIDE:-0}" == "1" ]]; then
		return 1
	fi
...
_hrff_post_rate_limit_release_circuit_comment() {
	...
	local cooldown_secs="$MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_COOLDOWN_SECS"
...
_hrff_handle_rate_limit_release_circuit() {
	...
	local threshold="$MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_THRESHOLD"
	local lookback="$MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_LOOKBACK_SECS"
	local cooldown="$MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_COOLDOWN_SECS"
...
_hrff_release_repo_state_is_managed() {
	...
	if declare -F maestro_can_manage_repo_issue_state >/dev/null 2>&1; then
		if ! maestro_can_manage_repo_issue_state "$repo_slug"; then
...
_release_dispatch_claim() {
	...
	local machine_readable_part="CLAIM_RELEASED reason=${reason} runner=${runner_name} ts=${release_ts} maestro_version=${maestro_version} opencode_version=${opencode_version}"
...
# Globals:
#   WORKER_GITHUB_LOGIN, MAESTRO_WORKER_GITHUB_LOGIN
# Outputs:
#   GitHub login or OS username fallback.
#######################################
_hrff_resolve_release_runner_login() {
	local runner_login="${WORKER_GITHUB_LOGIN:-${MAESTRO_WORKER_GITHUB_LOGIN:-}}"
...
_worker_archive_dirty_worktree_patch() {
	local work_dir="$1"
	local branch_name="$2"
	local archive_root="${MAESTRO_WORKER_DIRTY_ARCHIVE_DIR:-${HOME}/.maestro/.agent-workspace/work/dirty-worktrees}"
...
#
# Args:
#   $1 - session_key (e.g., "issue-Aditya060806-maestro-17642")
...
_report_failure_to_fast_fail() {
	...
	local state_file="${HOME}/.maestro/.agent-workspace/supervisor/fast-fail-counter.json"
	local state_dir
