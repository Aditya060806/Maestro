...
agents_dir = os.path.expanduser("~/.maestro/agents")
...
def _merge_instructions(config):
    """Merge maestro AGENTS.md into instructions list, preserving user entries."""
    instructions_path = os.path.expanduser("~/.maestro/agents/AGENTS.md")
...
def _ensure_plugin_registered(config):
    """Ensure the maestro plugin is registered in opencode config."""
    maestro_plugin_url = "file://" + os.path.expanduser(
        "~/.maestro/agents/plugins/opencode-maestro/index.mjs"
    )
    plugin_list = config.get('plugin', [])
    if not isinstance(plugin_list, list):
        plugin_list = [plugin_list] if plugin_list else []
    if maestro_plugin_url not in plugin_list:
        plugin_list.append(maestro_plugin_url)
        print(f"  Re-registered maestro plugin (was missing from config)", file=sys.stderr)
...
def _build_hook_entry():
    ...
    return {"type": "command", "command": "$HOME/.maestro/hooks/git_safety_guard.py"}
...
def _build_permission_rules():
    ...
    allow_rules = [
        "Read(~/.maestro/**)", "Bash(~/.maestro/agents/scripts/*)",
    ...
    deny_rules = [
        ...
        "Read(~/.config/maestro/credentials.sh)",
        ...
        "Bash(cat ~/.config/maestro/credentials.sh)",
    ]
    ask_rules = [
