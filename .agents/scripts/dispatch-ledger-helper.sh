...
#
# Storage: JSONL file at ~/.maestro/.agent-workspace/tmp/dispatch-ledger.jsonl
...
LEDGER_DIR="${MAESTRO_DISPATCH_LEDGER_DIR:-${HOME}/.maestro/.agent-workspace/tmp}"
LEDGER_FILE="${LEDGER_DIR}/dispatch-ledger.jsonl"
LEDGER_LOCK="${LEDGER_DIR}/dispatch-ledger.lock"
DEFAULT_TTL="${MAESTRO_DISPATCH_LEDGER_TTL:-3600}" # 60 minutes
...
#   3. Owner alive but lock age > MAESTRO_LEDGER_LOCK_MAX_AGE_S
...
# Canonical incident: Aditya060806/Maestro#21427 — a 24-day-old stale
...
_acquire_lock() {
	...
	local max_age="${MAESTRO_LEDGER_LOCK_MAX_AGE_S:-60}"
	local attempts=0
...
Environment:
  MAESTRO_DISPATCH_LEDGER_DIR   Override ledger directory
  MAESTRO_DISPATCH_LEDGER_TTL   Override default TTL in seconds (default: 3600)
