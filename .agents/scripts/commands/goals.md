...
Use the mission system as maestro' goal engine. This command is a user-friendly
...
1. If `$ARGUMENTS` is empty, show current goals:
   ...
   ```bash
   ~/.maestro/agents/scripts/mission-dashboard-helper.sh summary
   ```
...
2. If `$ARGUMENTS` is `status`, `summary`, `dashboard`, `json`, or `browser`,
   ...
   ```bash
   cmd=(~/.maestro/agents/scripts/mission-dashboard-helper.sh "$ARGUMENTS")
   "${cmd[@]}"
   ```
...
- Store durable work in a mission state file: `todo/missions/*/mission.md` for
  repo-attached goals, or `~/.maestro/missions/*/mission.md` for homeless goals.
