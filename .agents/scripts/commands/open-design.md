...
Open Design is an optional companion studio, not an maestro dependency. Keep maestro `.agents/` and Google `DESIGN.md` canonical.
...
| `skills` | Show maestro ingestion recommendations |
| `route <artifact brief>` | Recommend maestro-native vs Open Design workflow |
...
- Do not symlink Open Design skills into maestro as a source of truth.
...
- Verify generated artifacts with maestro UI/email/video checks before shipping.
