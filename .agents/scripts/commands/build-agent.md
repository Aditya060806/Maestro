---
description: Scaffold a new Maestro agent — service, tool, or workflow subagent with YAML frontmatter, placement decision, and cross-reference discovery
agent: Build+
mode: subagent
---
...
Scaffold a new Maestro agent file with correct placement, frontmatter, and cross-references.
...
- `/add-skill` — imports third-party packages (OpenSkills, ClawdHub, GitHub). Use `/build-agent` for net-new maestro-native content.
...
1. **Regenerate the subagent index:**
   ...
   ```bash
   ~/.maestro/agents/scripts/subagent-index-helper.sh generate
   ```
