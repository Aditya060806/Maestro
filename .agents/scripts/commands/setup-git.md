...
- **Command**: `/setup-git`
- **Aggregator**: `~/.maestro/agents/scripts/setup-debt-helper.sh`
- **Detector**: `~/.maestro/agents/scripts/security-posture-helper.sh` (Phase 7)
...
- You just registered a new repo via `maestro init` or hand-edited `repos.json`
...
- You need API keys for AI services (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.) in `~/.config/maestro/credentials.sh`
...
```bash
~/.maestro/agents/scripts/setup-debt-helper.sh summary --format=human
```

Then list both debt classes:

```bash
~/.maestro/agents/scripts/setup-debt-helper.sh list-sync-pat-missing
~/.maestro/agents/scripts/setup-debt-helper.sh list-cross-account-inherit
```

For each slug, also fetch the platform from `~/.config/maestro/repos.json`
so you can route to the correct PAT URL template:

```bash
jq -r --arg slug "$SLUG" '.initialized_repos[] | select(.slug == $slug) | .platform // "github"' ~/.config/maestro/repos.json
```
...
Fix (one command — run in a separate terminal):
  ...
  maestro sync-workflows --apply --repo <SLUG>
...
After the PR merges, verify:
  ...
  maestro check-workflows --repo <SLUG>
...
Dismiss without re-syncing (e.g., intentional fork with own secrets):
  maestro security dismiss cross-account-inherit-<SLUG_SAFE>
```
...
`maestro security check` treats missing effective PR merge gating as an
...
terminal):
  ...
  1. Ensure review-bot-gate.yml and maintainer-gate.yml are installed/current:
       maestro check-workflows --repo <SLUG>
       maestro sync-workflows --apply --repo <SLUG>   # only if drift/missing
  ...
  3. Verify:
       maestro security check --repo <SLUG>
...
For GitHub-hosted repos in `~/.config/maestro/repos.json` that are not marked
...
Audit:
  ...
  maestro security posture check <PATH_TO_REPO>
...
Step 1 — Create a fine-grained PAT
   Open this URL in your browser. Scopes are pre-filled:
     ...
     https://github.com/settings/personal-access-tokens/new?name=maestro-sync-<SLUG_SAFE>&description=maestro%20issue-sync%20PAT%20for%20<SLUG_URLENC>&expiration=90&target_name=<OWNER>&permissions=contents:write,issues:write,pull_requests:write,metadata:read
...
```bash
# Per-repo
~/.maestro/agents/scripts/setup-debt-helper.sh verify-secret <SLUG> SYNC_PAT
...
```bash
maestro security dismiss sync-pat-<SLUG_SAFE>
```
...
- Pointer to the next pulse cycle re-running `maestro security check` (no manual rerun needed)
...
- `gh auth` scope checks, default-branch drift, workflow-file drift — separate `maestro security check` covers these
...
- `maestro/onboarding.md` — per-account auth (the `/onboarding` command)
...
- `tools/credentials/gopass.md` — preferred secret store for MAESTRO env vars
