...
2. Run `~/.maestro/agents/scripts/report-render-helper.sh validate <input.md|input.json>`.
3. Render HTML with `~/.maestro/agents/scripts/report-render-helper.sh render <input.md|input.json> --output report.html`.
...
```bash
~/.maestro/agents/scripts/report-render-helper.sh render report.md \
  --template axel \
  --theme auto \
  --pdf-profile a4 \
  --output report.html
...
```bash
~/.maestro/agents/scripts/report-render-helper.sh sample instructional-seo-geo > report.md
```
