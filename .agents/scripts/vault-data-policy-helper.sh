...
${prompt_text}"
	...
	if _vault_policy_tokens_have "$data_classification" "provider-allowed" \
		...
		|| [[ "${MAESTRO_VAULT_PROVIDER_AI_APPROVED:-0}" == "1" ]]; then
		provider_approved=0
	fi
	...
	if _vault_policy_tokens_have "$data_classification" "confidential" \
		...
		if [[ "$provider_approved" -ne 0 ]]; then
			print_error "VAULT_POLICY_DENIED: ${data_classification:-confidential} tasks require provider-allowed metadata or MAESTRO_VAULT_PROVIDER_AI_APPROVED=1 before remote provider dispatch"
			return 64
		fi
	fi
