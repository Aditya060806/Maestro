...
#
# Bypass:
#   MAESTRO_FIX_THE_FIXER_DETECTOR_DISABLE=1   — exit 0 immediately
#   MAESTRO_FIX_THE_FIXER_DETECTOR_DRY_RUN=1   — classify, don't write
#
# Tunables:
#   MAESTRO_FIX_THE_FIXER_DETECTOR_MODEL       (default: haiku)
#   MAESTRO_FIX_THE_FIXER_DETECTOR_LIMIT       (default: 10)
...
readonly FIX_THE_FIXER_MARKER="<!-- maestro:fix-the-fixer-detector -->"
...
readonly AUTH_COOLDOWN_FILE="${HOME}/.maestro/cache/fix-the-fixer-detector-auth.cooldown"
...
_auth_item_logs_suppressed() {
	[[ "${MAESTRO_FIX_THE_FIXER_DETECTOR_SUPPRESS_AUTH_ITEM_LOGS:-0}" == "1" ]] || return 1
	return 0
}
...
_auth_cooldown_seconds() {
	local configured="${MAESTRO_FIX_THE_FIXER_DETECTOR_AUTH_COOLDOWN_SECONDS:-$AUTH_COOLDOWN_SECONDS_DEFAULT}"
...
_classify_via_llm() {
	local issue_title="$1"
	local issue_body="$2"
	local model="${MAESTRO_FIX_THE_FIXER_DETECTOR_MODEL:-haiku}"
...
_apply_label_and_comment() {
	...
	if [[ "${MAESTRO_FIX_THE_FIXER_DETECTOR_DRY_RUN:-0}" == "1" ]]; then
...
- \`MAESTRO_VERBOSE_LIFECYCLE=1\` — emits 5+ lifecycle checkpoints visible in pulse.log.
- \`WORKER_STALL_TIMEOUT=180\` — tighter watchdog (vs 300s default) so silent failure is caught faster.
- \`MAESTRO_WORKER_PREFLIGHT_SENTINEL=1\` — worker writes a preflight sentinel before opencode launches; aborts dispatch with clear error if write fails (catches sandbox/FD issues before model invocation).
...
cmd_run() {
	local repo_filter=""
	local limit="${MAESTRO_FIX_THE_FIXER_DETECTOR_LIMIT:-10}"
	local dry_run="${MAESTRO_FIX_THE_FIXER_DETECTOR_DRY_RUN:-0}"
	...
	[[ "$limit" =~ ^[0-9]+$ ]] || limit=10
	export MAESTRO_FIX_THE_FIXER_DETECTOR_DRY_RUN="$dry_run"

	if [[ "${MAESTRO_FIX_THE_FIXER_DETECTOR_DISABLE:-0}" == "1" ]]; then
		_log_info "MAESTRO_FIX_THE_FIXER_DETECTOR_DISABLE=1 — bypassing run"
		return 0
	fi
	...
	export MAESTRO_FIX_THE_FIXER_DETECTOR_SUPPRESS_AUTH_ITEM_LOGS=1
	local slug
...
Bypass:
  MAESTRO_FIX_THE_FIXER_DETECTOR_DISABLE=1   — exit 0 immediately
  MAESTRO_FIX_THE_FIXER_DETECTOR_DRY_RUN=1   — same as --dry-run
