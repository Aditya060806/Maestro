...
readonly MAESTRO_TITLE_SUFFIX_ERE='[[:space:]]+· Maestro [0-9]+\.[0-9]+\.[0-9]+$'
...
_get_maestro_version() {
	...
	version_file="${home_dir}/.maestro/agents/VERSION"
	if [[ -f "$version_file" ]]; then
		tr -d '[:space:]' <"$version_file"
		return 0
	fi
...
# Append the maestro version to titles, replacing any older suffix.
# Arguments:
#   $1 - raw title
# Output: title with idempotent suffix when a version is known
_with_maestro_title_suffix() {
	local raw_title="$1"
	local version
	version="$(_get_maestro_version)"

	local base_title
	base_title="$(printf '%s' "$raw_title" | sed -E "s/${MAESTRO_TITLE_SUFFIX_ERE}//")"
	...
	printf '%s · Maestro %s' "$base_title" "$version"
	return 0
}
...
cmd_rename() {
	...
	local display_title
	display_title="$(_with_maestro_title_suffix "$new_title")"
