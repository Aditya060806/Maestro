...
_sum_issue_token_spend() {
	...
	bodies=$(printf '%s' "$comments_json" | jq -r '
		...
		(map(select(
				((.body // "") | contains("<!-- maestro-signed-approval -->"))
...
<!-- ops:end -->" 2>/dev/null || true
	...
	local _cb_meta_filer="${SCRIPT_DIR:-${HOME}/.maestro/agents/scripts}/circuit-breaker-meta-filer.sh"
...
_check_cost_budget() {
	...
	local _audit_log_helper="${SCRIPT_DIR:-${HOME}/.maestro/agents/scripts}/audit-log-helper.sh"
