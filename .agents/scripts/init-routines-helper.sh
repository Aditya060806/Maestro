...
#
# Options:
#   --org <name>   Create per-org variant: <org>/maestro-routines
#   --local        Create local-only repo (no remote, local_only: true)
#   --dry-run      Print what would be done without making changes
#   --help         Show this help message
#
# Without flags: creates personal <username>/maestro-routines (always private)
...
if [[ -n "${BASH_SOURCE[0]:-}" ]] && [[ "${BASH_SOURCE[0]}" != "$0" || -f "${BASH_SOURCE[0]}" ]]; then
	...
	SCRIPT_DIR="${HOME}/.maestro/agents/scripts"
fi
...
Fields:
  ...
  run:    -- deterministic script relative to ~/.maestro/agents/
  agent:  -- LLM agent dispatched via headless-runtime-helper.sh
  [x] enabled, [ ] disabled/paused

## Core Routines (framework-managed)

<!-- These routines are managed by maestro setup. They reflect the launchd
...
Use the template at `~/.maestro/agents/templates/routine-description-template.md`
...
<!-- Source: Aditya060806/Maestro:.agents/scripts/routines/core-routines.sh -->
<!-- Overwritten by: maestro update / init-routines-helper.sh -->
...
**Detailed logs**: `~/.maestro/.agent-workspace/cron/r000/`
...
# maestro-routines — Worker Instructions
...
consumer of the maestro framework, not the source of truth for routine
code or descriptions.

## Generated files — DO NOT EDIT HERE

Files in `routines/core/` are **generated** by the maestro framework
(`~/.maestro/agents/scripts/routines/core-routines.sh`). They are
overwritten on every `maestro update` or `init-routines-helper.sh` run.
...
2. Fix the source: `describe_rNNN()` function in
   `Aditya060806/Maestro:.agents/scripts/routines/core-routines.sh`
3. The fix will propagate here on the next `maestro update`

**If you find a bug in `routine-log-helper.sh` or other framework scripts:**
1. These scripts live in `Aditya060806/Maestro:.agents/scripts/`
...
- Issues about routine **behaviour or bugs** should be filed on the main
  `Aditya060806/Maestro` repo, not here.
...
2. If it's a **question** (e.g., "why did this fail?"): answer using the
   ...
   `~/.maestro/.agent-workspace/cron/<id>/`, and `routine-log-helper.sh status`
...
4. If it's a **bug report** about the routine itself: create an issue on the
   main `Aditya060806/Maestro` repo (the routine code lives there)
...
# Generated files — changes here will be overwritten by maestro update.
# PRs touching these paths require maintainer review to confirm the fix
# should be in the source repo (Aditya060806/Maestro) instead.
/routines/core/ @Aditya060806
COEOF
	return 0
}
...
Recurring operational routines for the [maestro](https://maestro.sh) framework.
...
routines/
├── core/               # Generated by maestro framework (DO NOT EDIT — see AGENTS.md)
...
|-------|-------|----------------|
| r901-r999 | Framework (maestro) | No — generated, fix at source |
...
_sync_with_remote_before_commit() {
	...
	(
		...
		if [[ -n "$(git status --porcelain)" ]]; then
			if git stash push -u -m "maestro-routines scaffold before remote sync" >/dev/null; then
				stashed=true
			else
				return 1
			fi
...
_commit_and_push() {
	...
	(
		...
		if ! git diff --cached --quiet; then
			if ! git commit -m "chore: scaffold maestro-routines repo"; then
...
_store_routine_description() {
	...
	local state_dir="${HOME}/.maestro/.agent-workspace/cron/${rid}"
...
| View logs | \`cat ~/.maestro/.agent-workspace/cron/${rid}/runs.jsonl | tail -5\` |
...
| Force a run now | \`~/.maestro/agents/${script}\` |
...
_create_core_routine_issues() {
	...
	while IFS='|' read -r rid enabled title schedule estimate script rtype; do
		...
		local state_file="${HOME}/.maestro/.agent-workspace/cron/${rid}/routine-state.json"
...
# ---------------------------------------------------------------------------
# init_personal
# Creates <username>/maestro-routines on GitHub (private), clones, scaffolds.
# ---------------------------------------------------------------------------
init_personal() {
	...
	local slug="${username}/maestro-routines"
	local repo_path="${GIT_PARENT}/maestro-routines"
...
# ---------------------------------------------------------------------------
# init_org <org_name>
# Creates <org>/maestro-routines on GitHub (private), clones, scaffolds.
# ---------------------------------------------------------------------------
init_org() {
	local org_name="$1"
	...
	local slug="${org_name}/maestro-routines"
	local repo_path="${GIT_PARENT}/${org_name}-maestro-routines"
...
init_local() {
	local repo_path="${GIT_PARENT}/maestro-routines"
	local slug="local/maestro-routines"
	...
	(
		...
		git add -A
		if ! git diff --cached --quiet; then
			git commit -m "chore: scaffold maestro-routines repo"
		fi
	)
...
_prompt_org() {
	local org="$1"
	local org_slug="${org}/maestro-routines"
...
detect_and_create_all() {
	...
	if [[ "$non_interactive" == "true" ]]; then
		print_info "Non-interactive mode: skipping org repos (run 'maestro init-routines --org <name>')"
		return 0
	fi
...
Options:
  --org <name>   Create per-org variant: <org>/maestro-routines (always private)
...
The repo is registered in ~/.config/maestro/repos.json with:
