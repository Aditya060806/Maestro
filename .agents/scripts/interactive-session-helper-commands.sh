...
_isc_cmd_claim() {
	...
	if ! user=$(_isc_resolve_manageable_user "claim" "$issue" "$slug" \
		...
		"external repos should receive a PR when possible plus at most one concise explanatory issue comment, not maestro dispatch claims"); then
		return 0
	fi
...
_isc_cmd_lockdown() {
	...
	if ! _isc_can_manage_issue_state "$slug" "$user"; then
		...
		_isc_warn "external repos must not receive maestro lockdown labels, locks, or audit comments"
		return 0
	fi
