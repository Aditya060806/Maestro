...
#
# Override env vars:
#   MAESTRO_DISPATCH_STAGES_LOG          — log path (default
#                                            ~/.maestro/logs/dispatch-stages.tsv)
#   MAESTRO_DISPATCH_STAGES_LOG_MAX_LINES — trim threshold (default 50000)
#   MAESTRO_DISPATCH_STAGES_DISABLE=1    — make all calls no-ops
#
...
# --- Configuration --------------------------------------------------------
DS_LOG="${MAESTRO_DISPATCH_STAGES_LOG:-${HOME}/.maestro/logs/dispatch-stages.tsv}"
DS_LOG_MAX_LINES="${MAESTRO_DISPATCH_STAGES_LOG_MAX_LINES:-50000}"
...
_ds_now_ns() {
	[[ "${MAESTRO_DISPATCH_STAGES_DISABLE:-0}" == "1" ]] && { printf '0'; return 0; }
	local ns
...
# Returns: 0 always.
_ds_record() {
	[[ "${MAESTRO_DISPATCH_STAGES_DISABLE:-0}" == "1" ]] && return 0
...
Subcommands:
  ...
  trim               Rotate log if larger than \$MAESTRO_DISPATCH_STAGES_LOG_MAX_LINES
  clear              Remove log

Log file: ${DS_LOG}

Env vars:
  MAESTRO_DISPATCH_STAGES_LOG            Override log path
  MAESTRO_DISPATCH_STAGES_LOG_MAX_LINES  Trim threshold (default 50000)
  MAESTRO_DISPATCH_STAGES_DISABLE=1      No-op all recording
EOF
		;;
	*)
