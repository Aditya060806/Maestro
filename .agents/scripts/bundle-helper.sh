...
# repos.json "bundle" field or .maestro.json), auto-detection from marker
...
#   2. .maestro.json "bundle" field (per-project config)
...
_resolve_bundles_dir() {
	...
	print_error "Bundle directory not found. Expected at .agents/bundles/ or ~/.maestro/agents/bundles/"
	return 1
}
...
# Read the "bundle" field from a project's .maestro.json.
# Arguments:
...
_read_project_config_bundle() {
	...
	local config_file="${project_path}/.maestro.json"
...
# Read the cached detected bundle from .maestro.json.
# Arguments:
#   $1 - absolute project path
...
# Write detected bundle to .maestro.json cache.
...
# Priority chain: repos.json > .maestro.json > auto-detect > cli-tool fallback.
# Arguments:
#   $1 - project path (defaults to current directory)
...
cmd_resolve() {
	...
	# 2. Check .maestro.json for explicit bundle assignment
