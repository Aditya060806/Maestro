...
readonly DEFAULT_OUTPUT_DIR="${HOME}/.maestro/.agent-workspace/work/voice-pipeline"
...
load_elevenlabs_key() {
	...
	if command -v gopass &>/dev/null; then
		local key
		key=$(gopass show -o "maestro/ELEVENLABS_API_KEY" 2>/dev/null) || true
		if [[ -n "${key:-}" ]]; then
...
require_elevenlabs_key() {
	if ! load_elevenlabs_key; then
		...
		print_info "Set it with: maestro secret set ELEVENLABS_API_KEY"
...
cmd_extract() {
	...
	ffmpeg -y -i "$input_file" \
		...
		-ar "$sample_rate" \
		-ac 1 \
		"$output_file" 2>>"${MAESTRO_LOG_FILE:-/dev/null}"
...
cmd_status() {
	...
	if load_elevenlabs_key; then
		...
		print_info "Set with: maestro secret set ELEVENLABS_API_KEY"
	fi
...
Dependencies:
  ...
  API key:  ELEVENLABS_API_KEY (via maestro secret set)
