...
#
# Emergency bypass:
#   MAESTRO_SKIP_PREDISPATCH_VALIDATOR=1 — exit 0 immediately (with log)
#
...
_load_self_hosting_patterns() {
	local conf_file="${MAESTRO_DISPATCH_PATH_FILES_CONF:-${SCRIPT_DIR}/../configs/self-hosting-files.conf}"
	local patterns=()
	if [[ -f "$conf_file" ]]; then
...
_zero_progress_gauge_from_stats() {
	local stats_file="${PULSE_STATS_FILE:-${HOME}/.maestro/logs/pulse-stats.json}"
...
_close_zero_progress_meta_if_recovered() {
	...
	# #maestro:trust-boundary — recovered-meta handling writes comments and
...
_extract_function_complexity_sweep_cited_file() {
	...
	generator_line=$(printf '%s' "$issue_body" | grep -oE '<!-- maestro:generator=function-complexity-sweep[^>]*-->' | head -1) || generator_line=""
...
_validator_upstream_watch() {
	...
	local state_file="${MAESTRO_UPSTREAM_WATCH_STATE:-${HOME}/.maestro/cache/upstream-watch-state.json}"
...
_sht_check_comment_idempotent() {
	...
	if [[ "$existing" =~ ^[1-9][0-9]*$ ]]; then
		...
		if [[ "${MAESTRO_SELF_HOSTING_DETECTOR_DRY_RUN:-}" != "1" ]]; then
...
#
# Environment:
#   MAESTRO_SKIP_SELF_HOSTING_DETECTOR=1    — skip entirely
#   MAESTRO_SELF_HOSTING_DETECTOR_DRY_RUN=1 — emit what-would-be-applied
# ---------------------------------------------------------------------------
_detect_self_hosting_task() {
	...
	# Bypass guard
	if [[ "${MAESTRO_SKIP_SELF_HOSTING_DETECTOR:-}" == "1" ]]; then
		_log "INFO" "MAESTRO_SKIP_SELF_HOSTING_DETECTOR=1 — skipping self-hosting detector"
		return 0
	fi
...
_rf_is_stopword() {
	...
	case "$word" in
	maestro | automated | auto | body | cited | code | debt | dispatch | file | files | fix | fixed | fixing | from | generated | guidance | issue | label | labels | line | lines | merged | modify | path | paths | please | quality | review | should | source | status | task | that | the | then | there | this | updates | what | when | where | with | worker | workers)
		return 0
		;;
	esac
...
_rf_search_merged_pr_numbers() {
	...
	local limit="${MAESTRO_REVIEW_FEEDBACK_SUPERSESSION_LIMIT:-25}"
...
_detect_review_feedback_supersession() {
	...
	if [[ "${MAESTRO_SKIP_REVIEW_FEEDBACK_SUPERSESSION:-}" == "1" ]]; then
		_log "INFO" "MAESTRO_SKIP_REVIEW_FEEDBACK_SUPERSESSION=1 — skipping review-feedback supersession detector"
		return 0
	fi
...
cmd_validate() {
	...
	if [[ "${MAESTRO_SKIP_PREDISPATCH_VALIDATOR:-}" == "1" ]]; then
		_log "INFO" "MAESTRO_SKIP_PREDISPATCH_VALIDATOR=1 — skipping validator for #${issue_number}"
		return 0
	fi
	...
	#   <!-- maestro:generator=<name> -->
	#   <!-- maestro:generator=<name> cited_file=<path> threshold=<N> -->
	local generator_line
	generator_line=$(printf '%s' "$issue_body" | grep -oE '<!-- maestro:generator=[a-z0-9_-]+[^>]*-->' | head -1) || generator_line=""

	local generator
	generator=$(printf '%s' "$generator_line" | sed 's/<!-- maestro:generator=//;s/ .*//' 2>/dev/null) || generator=""
...
Environment:
  MAESTRO_SKIP_PREDISPATCH_VALIDATOR=1  — bypass all validators (exit 0)
EOF
	return 0
}
