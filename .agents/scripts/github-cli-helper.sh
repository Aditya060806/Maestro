...
merge_pr() {
	...
	if [[ $merge_exit -eq 0 ]]; then
		...
		trigger_maestro_post_merge_sync "$owner/$repo_name"
		return 0
	fi
...
resolve_maestro_sync_repo_dir() {
	if [[ -n "${MAESTRO_SYNC_REPO_PATH:-}" ]] && [[ -d "$MAESTRO_SYNC_REPO_PATH/.agents" ]]; then
		printf '%s\n' "$MAESTRO_SYNC_REPO_PATH"
		return 0
	fi

	local current_repo_root
	current_repo_root=$(git rev-parse --show-toplevel 2>/dev/null || true)
	if [[ -n "$current_repo_root" ]] && [[ "$(basename "$current_repo_root")" == "maestro" ]] && [[ -d "$current_repo_root/.agents" ]]; then
		printf '%s\n' "$current_repo_root"
		return 0
	fi

	local default_repo_path="$HOME/Git/maestro"
...
trigger_maestro_post_merge_sync() {
	local repo_slug="$1"

	if [[ "$repo_slug" != "Aditya060806/Maestro" ]]; then
		return 0
	fi

	local repo_dir
	if ! repo_dir=$(resolve_maestro_sync_repo_dir); then
		print_warning "Merged maestro PR but could not locate local maestro repo for sync"
		return 0
	fi

	local deploy_script="${MAESTRO_SYNC_DEPLOY_SCRIPT:-$repo_dir/.agents/scripts/deploy-agents-on-merge.sh}"
	...
	if [[ "$sync_exit" -eq 0 || "$sync_exit" -eq 2 ]]; then
		print_success "Post-merge maestro agent sync completed"
		return 0
	fi

	print_warning "Post-merge maestro agent sync failed (non-blocking): $sync_output"
	return 0
}
...
EXAMPLES:
  $0 list-repos Aditya060806
  $0 create-repo Aditya060806 my-new-project "My awesome project" public true
  $0 list-issues Aditya060806 my-repo open
  $0 create-issue Aditya060806 my-repo "Bug report" "Describe the issue here"
  $0 create-pr Aditya060806 my-repo "Fix bug" main bugfix
