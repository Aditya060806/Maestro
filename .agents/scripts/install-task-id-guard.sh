...
# ~/.maestro/agents/hooks/task-id-collision-guard.sh (survives maestro updates).
...
HOOK_MARKER="# maestro-task-id-guard"
DEPLOYED_HOOK="$HOME/.maestro/agents/hooks/task-id-collision-guard.sh"

#######################################
# Locate the repo-local copy of the hook (for use before maestro update has
...
cmd_install() {
	...
	if [[ -f "$hook_path" ]]; then
		if grep -q "$HOOK_MARKER" "$hook_path" 2>/dev/null; then
			...
			print_error "existing commit-msg hook at $hook_path is NOT managed by maestro"
...
# Dispatcher that calls the maestro task-id collision guard.
...
cmd_uninstall() {
	...
	print_warning "commit-msg hook at $hook_path is NOT managed by maestro — leaving it alone"
	return 1
}
...
cmd_status() {
	...
	if grep -q "$HOOK_MARKER" "$hook_path" 2>/dev/null; then
		printf 'commit-msg hook: installed (maestro task-id-guard)\n'
		printf '  path: %s\n' "$hook_path"
	else
