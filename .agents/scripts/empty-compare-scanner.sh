...
#
# Exit codes:
#   0 — no violations (or MAESTRO_EMPTY_COMPARE_SKIP=1)
#   1 — violations found
...
#   Global:     MAESTRO_EMPTY_COMPARE_SKIP=1
...
scan_dir() {
	...
	if [ "${MAESTRO_EMPTY_COMPARE_SKIP:-0}" = "1" ]; then
		log "MAESTRO_EMPTY_COMPARE_SKIP=1 — scan bypassed"
		[ -n "$_out" ] && : >"$_out"
		return 0
	fi
