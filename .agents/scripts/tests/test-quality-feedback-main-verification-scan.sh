...
# The exact review body from PR #5637 (Aditya060806, APPROVED, human reviewer).
...
test_skips_pr5637_pulse_supervisor_merge_comment() {
	local result
	result=$(_test_approval_filter "Pulse supervisor: all CI checks green, review-bot-gate PASS, CodeRabbit approved. Merging." "APPROVED" "Aditya060806")
	if [[ "$result" == "skip" ]]; then
...
test_keeps_human_changes_requested_review_gh5668() {
	local result
	result=$(_test_approval_filter "This function has a bug: the return value is not checked. Please fix before merging." "CHANGES_REQUESTED" "Aditya060806")
	if [[ "$result" == "keep" ]]; then
		...
		api)
			while [[ $# -gt 0 ]]; do
				...
				repos/*/pulls/*/reviews)
					...
					echo '[{"id":3996148895,"user":{"login":"Aditya060806"},"state":"APPROVED","body":"Pulse supervisor: all CI checks green, review-bot-gate PASS, CodeRabbit approved. Merging.","submitted_at":"2026-03-24T04:24:00Z","html_url":"https://github.com/Aditya060806/Maestro/pull/5637#pullrequestreview-3996148895"}]'
					return 0
					;;
				repos/*/git/trees/*)
					echo '{"tree":[]}'
