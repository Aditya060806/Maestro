...
#   5. Downstream caller template references Aditya060806/Maestro reusable path
#   6. Event guards in the reusable workflow accept both pull_request and pull_request_target
#   7. Every checkout of the caller repo is paired with a checkout of Aditya060806/Maestro
#      into __maestro/ (so helpers resolve via __maestro/.agents/scripts/*)
...
#  10.  Reusable workflow declares `on: workflow_call:` with maestro_ref input
...
#  13.  Downstream caller template references Aditya060806/Maestro reusable path
#  14.  Reusable workflow uses __maestro/ path (runtime-fetched, no SHA pin)
#
...
# ---------------------------------------------------------------------------
# Test 3: Self-caller (maestro's own issue-sync.yml) uses the local reusable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test 5: Downstream caller references Aditya060806/Maestro reusable path
# ---------------------------------------------------------------------------

if [[ -f "$DOWNSTREAM_TEMPLATE" ]]; then
	if grep -Eq "uses:\s*Aditya060806/Maestro/\.github/workflows/issue-sync-reusable\.yml@" \
		"$DOWNSTREAM_TEMPLATE"; then
		_pass "downstream template references Aditya060806/Maestro reusable path"
	else
		_fail "downstream template references Aditya060806/Maestro reusable path" \
			"expected 'uses: Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml@<ref>' in $DOWNSTREAM_TEMPLATE"
	fi
...
# ---------------------------------------------------------------------------
# Test 7: Framework-script paths use __maestro/ prefix (runtime-fetched)
# ---------------------------------------------------------------------------

if [[ -f "$REUSABLE_WF" ]]; then
	# Count references to .agents/scripts/ that are NOT preceded by __maestro/
	...
	if [[ -z "$stray" ]]; then
		_pass "no bare .agents/scripts/ invocations (runtime-fetched via __maestro/)"
	else
		_fail "no bare .agents/scripts/ invocations" "found:
$stray"
	fi

	# Sanity: at least one __maestro/.agents/scripts/ invocation should exist
	maestro_refs=$(grep -cE "__maestro/\.agents/scripts/" "$REUSABLE_WF" 2>/dev/null || true)
	[[ "$maestro_refs" =~ ^[0-9]+$ ]] || maestro_refs=0
	if (( maestro_refs > 0 )); then
		_pass "reusable workflow invokes framework scripts via __maestro/ ($maestro_refs references)"
	else
		_fail "reusable workflow invokes framework scripts via __maestro/" \
			"no __maestro/.agents/scripts/ references found"
	fi
fi
...
for jname, jdef in jobs.items():
    ...
    # framework checkout (repository: Aditya060806/Maestro, path: __maestro)
    job_yaml = yaml.safe_dump(jdef)
    needs_framework = '__maestro/.agents/scripts/' in job_yaml
    if not needs_framework:
        continue
    has_framework_checkout = any(
        (r == 'Aditya060806/Maestro' and p == '__maestro')
        for (_, r, p) in co_steps
    )
    if not has_framework_checkout:
        bad.append(jname)
if bad:
    print(f"FAIL: jobs use __maestro/ but lack framework checkout: {bad}")
    sys.exit(1)
print("OK")
PYEOF
)"
	if [[ "$pairing_result" == "OK" ]]; then
		_pass "every job using framework scripts checks out Aditya060806/Maestro into __maestro/"
	else
		_fail "every job using framework scripts checks out Aditya060806/Maestro into __maestro/" \
			"$pairing_result"
	fi
else
...
# ---------------------------------------------------------------------------
# Test 10: Reusable review-bot-gate workflow declares workflow_call with maestro_ref input
# ---------------------------------------------------------------------------
...
wc = on['workflow_call'] or {}
inputs = wc.get('inputs', {}) or {}
if 'maestro_ref' not in inputs:
    print(f"FAIL: workflow_call.inputs missing `maestro_ref`. Keys: {list(inputs.keys())}"); sys.exit(1)
print("OK")
PYEOF
)"
		if [[ "$parse_result" == "OK" ]]; then
			_pass "review-bot-gate reusable workflow declares workflow_call with maestro_ref input"
		else
			_fail "review-bot-gate reusable workflow declares workflow_call with maestro_ref input" "$parse_result"
		fi
	fi
fi

# ---------------------------------------------------------------------------
# Test 11: Self-caller (maestro's own review-bot-gate.yml) uses local reusable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test 13: Downstream caller references Aditya060806/Maestro reusable path and uses secrets: inherit
# ---------------------------------------------------------------------------

if [[ -f "$RBG_DOWNSTREAM_TEMPLATE" ]]; then
	if grep -Eq "uses:\s*Aditya060806/Maestro/\.github/workflows/review-bot-gate-reusable\.yml@" \
		"$RBG_DOWNSTREAM_TEMPLATE"; then
		_pass "review-bot-gate downstream template references Aditya060806/Maestro reusable path"
	else
		_fail "review-bot-gate downstream template references Aditya060806/Maestro reusable path" \
			"expected 'uses: Aditya060806/Maestro/.github/workflows/review-bot-gate-reusable.yml@<ref>' in $RBG_DOWNSTREAM_TEMPLATE"
	fi
...
# ---------------------------------------------------------------------------
# Test 14: Reusable review-bot-gate workflow uses __maestro/ (no SHA pin)
# ---------------------------------------------------------------------------

if [[ -f "$RBG_REUSABLE_WF" ]]; then
	# The old pattern hard-pinned a SHA and used path: .maestro-helper
	# GH#20727 fix: runtime-fetched into __maestro/ using inputs.maestro_ref
	if grep -qE "path:\s*__maestro" "$RBG_REUSABLE_WF"; then
		_pass "review-bot-gate reusable workflow fetches helper into __maestro/"
	else
		_fail "review-bot-gate reusable workflow fetches helper into __maestro/" \
			"expected 'path: __maestro' in the checkout step — GH#20727 pattern not applied"
	fi

	# Confirm no SHA pin on the maestro checkout (ref uses inputs.maestro_ref)
	if grep -qE "ref:\s*\\\$\{\{[[:space:]]*inputs\.maestro_ref" "$RBG_REUSABLE_WF"; then
		_pass "review-bot-gate reusable workflow uses inputs.maestro_ref (not a hard-coded SHA)"
	else
		_fail "review-bot-gate reusable workflow uses inputs.maestro_ref" \
			"expected 'ref: \${{ inputs.maestro_ref ... }}' — SHA pin not eliminated"
	fi

	# Helper path must not reference .maestro-helper/ in functional (non-comment) lines.
	# Comments mentioning the old path for historical context are allowed.
	if grep -v "^\s*#" "$RBG_REUSABLE_WF" | grep -q ".maestro-helper"; then
		_fail "review-bot-gate reusable workflow has no functional .maestro-helper/ reference" \
			"found stale .maestro-helper/ path in non-comment line — GH#20727 migration incomplete"
	else
		_pass "review-bot-gate reusable workflow has no functional .maestro-helper/ reference"
	fi
fi
