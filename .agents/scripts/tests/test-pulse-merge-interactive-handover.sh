...
# (t2189).
#
# An origin:interactive PR that has sat idle past MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS
...
test_A_fresh_pr_returns_not_stale() {
	...
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=enforce _interactive_pr_is_stale "100" "owner/repo"
	local rc=$?
	if [[ "$rc" -eq 1 ]]; then
...
test_F_mode_off_returns_not_stale_unconditionally() {
	...
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=off _interactive_pr_is_stale "100" "owner/repo"
	local rc=$?
	if [[ "$rc" -eq 1 ]]; then
...
test_G_mode_detect_logs_and_returns_stale() {
	reset_mock_state
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=detect _interactive_pr_is_stale "100" "owner/repo"
	local rc=$?
	if [[ "$rc" -ne 0 ]]; then
...
test_H_mode_detect_is_noop() {
	...
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=detect _interactive_pr_trigger_handover "100" "owner/repo"
...
test_I_mode_enforce_applies_label_and_posts_comment() {
	reset_mock_state
	: >"${TEST_ROOT}/idempotent-comments.log"
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=enforce _interactive_pr_trigger_handover "100" "owner/repo"
...
test_L_invalid_seconds_returns_not_stale() {
	...
	MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS="24h" \
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=enforce \
	...
	if ! grep -q "invalid MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS" "$LOGFILE"; then
...
test_L2_zero_seconds_returns_not_stale() {
	reset_mock_state
	MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS="0" \
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=enforce \
		_interactive_pr_is_stale "100" "owner/repo"
	local rc=$?
	if [[ "$rc" -ne 1 ]]; then
...
test_L3_empty_seconds_uses_default_14400() {
	...
	MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS="" \
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=enforce \
		_interactive_pr_is_stale "100" "owner/repo"
	local rc=$?
	if [[ "$rc" -ne 0 ]]; then
...
test_L4_negative_seconds_returns_not_stale() {
	reset_mock_state
	MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS="-5" \
	MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=enforce \
		_interactive_pr_is_stale "100" "owner/repo"
	local rc=$?
	if [[ "$rc" -ne 1 ]]; then
