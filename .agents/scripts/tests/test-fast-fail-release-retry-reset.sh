...
write_entry() {
	...
	local retry_after="$3"
	local version="$4"
	printf '{"owner/repo/%s":{"count":%s,"ts":1,"reason":"rate_limit","retry_after":%s,"backoff_secs":600,"crash_type":"","maestro_version":"%s"}}\n' \
...
if _ff_version_gt "v3.14.64" "3.14.63" && ! _ff_version_gt "3.14.64" "3.14.64" && ! _ff_version_gt "3.14.63" "v3.14.64"; then
	pass "pure bash version comparison handles prefixed maestro versions"
else
	fail "pure bash version comparison handles prefixed maestro versions"
fi
...
if [[ "$reset_rc" -eq 1 && "$reset_count" == "0" && "$reset_retry" == "0" && "$reset_version" == "3.14.64" && ! -d "${FAST_FAIL_STATE_FILE}.lockdir" ]]; then
	pass "new maestro version clears old-version backoff once"
else
	fail "new maestro version clears old-version backoff once" \
...
recorded_version=$(jq -r '."owner/repo/125".maestro_version' "$FAST_FAIL_STATE_FILE")
if [[ "$recorded_version" == "3.14.64" ]]; then
	pass "fast_fail_record stores maestro version metadata"
else
	fail "fast_fail_record stores maestro version metadata" "version=${recorded_version}"
fi
