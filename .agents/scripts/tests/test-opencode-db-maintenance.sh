...
if [[ -f "$SANDBOX/fakehome/.maestro/.agent-workspace/work/opencode-maintenance/last-run.json" ]]; then
	_pass "state file written after maintain"
	if grep -q '"outcome":' "$SANDBOX/fakehome/.maestro/.agent-workspace/work/opencode-maintenance/last-run.json"; then
...
rc=$?
set -e
if [[ "$rc" -eq 0 ]] && grep -q "\[OPENCODE MAINTENANCE\]" <<<"$out" && grep -q "maestro opencode-db maintenance-window" <<<"$out"; then
...
rc=$?
set -e
if [[ "$rc" -eq 0 ]] && grep -q "Compact but large" <<<"$out" && ! grep -q "Run maestro opencode-db maintenance-window" <<<"$out"; then
