...
#
# Tests for the MAESTRO_HEADLESS=true export at the top of pulse-wrapper.sh
# main() (GH#18670 / Fix 7).
#
# Behaviors under test:
#   1. Executing pulse-wrapper.sh --self-check exports MAESTRO_HEADLESS=true
...
test_export_line_present_at_top_of_main() {
	...
	snippet=$(awk '
		...
		in_main { print }
	' "$WRAPPER_SCRIPT")
	if printf '%s' "$snippet" | grep -qE '^[[:space:]]*export MAESTRO_HEADLESS=true[[:space:]]*$'; then
		print_result "export MAESTRO_HEADLESS=true present at top of main()" 0
		return 0
	fi
	print_result "export MAESTRO_HEADLESS=true present at top of main()" 1 \
		"Expected 'export MAESTRO_HEADLESS=true' line between 'main() {' and 'local _sc_flag=0'. Got snippet:${snippet}"
	return 0
}

# Test 2: detect_session_origin() reports "worker" when MAESTRO_HEADLESS=true.
...
test_detect_session_origin_returns_worker_when_headless() {
	...
	if [[ "$result" == "worker" ]]; then
		print_result "detect_session_origin returns 'worker' when MAESTRO_HEADLESS=true" 0
		return 0
	fi
	print_result "detect_session_origin returns 'worker' when MAESTRO_HEADLESS=true" 1 \
		"Expected 'worker', got '$result'"
	return 0
}

# Test 3: Sourcing pulse-wrapper.sh without invoking main() must NOT
# set MAESTRO_HEADLESS in the caller shell. Scoping guarantee — protects
...
test_export_is_inside_main_not_top_level() {
	# Find all lines matching `export MAESTRO_HEADLESS=true` anywhere in
	...
	count=$(safe_grep_count -E '^[[:space:]]*export MAESTRO_HEADLESS=true[[:space:]]*$' "$WRAPPER_SCRIPT")
	if [[ "$count" -ne 1 ]]; then
		print_result "export is inside main(), not top-level" 1 \
			"Expected exactly 1 export line, found $count"
		return 0
	fi
	line_num=$(grep -nE '^[[:space:]]*export MAESTRO_HEADLESS=true[[:space:]]*$' "$WRAPPER_SCRIPT" | head -1 | cut -d: -f1)
...
test_export_before_self_check() {
	...
	export_line=$(grep -nE '^[[:space:]]*export MAESTRO_HEADLESS=true[[:space:]]*$' "$WRAPPER_SCRIPT" | head -1 | cut -d: -f1)
