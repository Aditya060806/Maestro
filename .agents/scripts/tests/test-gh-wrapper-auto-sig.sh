...
if [[ "$cmd" == "footer" ]]; then
    ...
    printf '\n<!-- maestro:sig -->\n---\n[maestro.sh](https://maestro.sh) v0.0.0-test stub footer.\n'
fi
exit 0
STUB
...
assert_contains "body now has signature marker" "<!-- maestro:sig -->" "$result"
...
assert_contains "equals-form body has signature" "<!-- maestro:sig -->" "$body_val"
...
assert_not_contains "original file is not mutated" "<!-- maestro:sig -->" "$file_content"
...
if [[ -z "$signed_body_file" ]]; then
	...
	assert_contains "temp body has signature marker" "<!-- maestro:sig -->" "$signed_file_content"
...
result="${_GH_WRAPPER_SIG_MODIFIED_ARGS[*]}"
assert_not_contains "no signature injected without body" "maestro:sig" "$result"
...
assert_contains "gh received body with signature" "<!-- maestro:sig -->" "$captured"
...
marker_count=$(grep -c 'maestro:sig' <<<"$captured" || true)
...
file_content=$(<"$bf")
assert_not_contains "original file did not get signature footer" "<!-- maestro:sig -->" "$file_content"
...
if [[ -z "$captured_body_file" || "$captured_body_file" == "pending" ]]; then
	...
	assert_contains "gh received temp body-file with signature" "<!-- maestro:sig -->" "$captured_file_content"
