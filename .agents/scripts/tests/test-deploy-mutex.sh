...
test_noninteractive_setup_lock_single_owner() {
	...
	(
		HOME="$TEST_ROOT" MAESTRO_SETUP_LOCK_DIR="$lock_dir" \
...
test_failed_deploy_verification_restores_backup() {
	...
	local target="${TEST_ROOT}/.maestro/agents"
	local backup="${TEST_ROOT}/.maestro/agents-backups/20260501_120000/agents"
	...
	local rc=0
	HOME="$TEST_ROOT" MAESTRO_AGENT_DEPLOY_MIN_FILES=100 INSTALL_DIR="${TEST_ROOT}/repo" \
		deploy_maestro_agents || rc=$?
...
test_agents_backup_restore_preserves_live_tree_on_stage_failure() {
	...
	local target="${TEST_ROOT}/restore-preserve/.maestro/agents"
	local backup="${TEST_ROOT}/restore-preserve/.maestro/agents-backups/20260501_130000/agents"
	local rc=0
	mkdir -p "$(dirname "$backup")"
