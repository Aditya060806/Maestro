...
setup() {
	...
	mkdir -p "$TEST_DIR/.maestro/.agent-workspace"
	mkdir -p "$TEST_DIR/.config/maestro"
...
test_budget_tracker_helper() {
	...
	local cost_log="${TEST_DIR}/.maestro/.agent-workspace/cost-log.tsv"
...
test_observability_helper() {
	...
	local metrics="${TEST_DIR}/.maestro/.agent-workspace/observability/metrics.jsonl"
