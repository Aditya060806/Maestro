...
mkdir -p "$HOME/.maestro/logs" "$HOME/.config/maestro" || exit 1
export LOGFILE="$HOME/.maestro/logs/pulse.log"
export STOP_FLAG="$HOME/.maestro/logs/pulse-session.stop"
export REPOS_JSON="$HOME/.config/maestro/repos.json"
export PULSE_MERGE_CHECKPOINT_FILE="$HOME/.maestro/logs/pulse-merge-checkpoint"
