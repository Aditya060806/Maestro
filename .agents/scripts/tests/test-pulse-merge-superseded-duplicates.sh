...
install_stubs() {
	gh() {
		...
		if [[ "$1" == "pr" && "$2" == "view" && "$*" == *"authorAssociation"* ]]; then
			printf '{"labels":[{"name":"origin:worker"}],"author":{"login":"maestro-worker[bot]"},"authorAssociation":"MEMBER"}\n'
			return 0
		fi
...
test_verified_supersession_closes_issue() {
	reset_case
	_close_conflicting_pr "4555" "Aditya060806/Maestro" "t3571: duplicate worker PR"
...
test_unrelated_conflict_routes_worker() {
	reset_case
	TEST_OVERLAP_RC=1
	_close_conflicting_pr "4556" "Aditya060806/Maestro" "t3571: unrelated conflicting PR"
	assert_log_not_contains "$GH_CALL_LOG" "pr close 4556" "unrelated conflict leaves PR open"
	assert_log_contains "$ROUTE_LOG" "4556 Aditya060806/Maestro 23105 conflict" "unrelated conflict routes fix worker"
	return 0
}
...
test_empty_branch_after_upstream_fix_closes_issue() {
	...
	_close_conflicting_pr "4557" "Aditya060806/Maestro" "t3571: empty branch after upstream fix"
...
test_ambiguous_same_file_change_routes_worker() {
	reset_case
	TEST_VERIFY_RC=1
	_close_conflicting_pr "4558" "Aditya060806/Maestro" "t3571: ambiguous same-file change"
	assert_log_not_contains "$GH_CALL_LOG" "pr close 4558" "ambiguous same-file change leaves PR open"
	assert_log_contains "$ROUTE_LOG" "4558 Aditya060806/Maestro 23105 conflict" "ambiguous same-file change routes fix worker"
	return 0
}
...
test_parent_research_comment_avoids_closing_keywords() {
	...
	_close_conflicting_pr "4559" "Aditya060806/Maestro" "t3571: parent duplicate PR"
...
test_label_lookup_failure_skips_issue_closure() {
	...
	_close_conflicting_pr "4560" "Aditya060806/Maestro" "t3571: label lookup failure"
