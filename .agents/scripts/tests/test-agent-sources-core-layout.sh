...
test_sync_preserves_core_layout() {
	local output=""
	output=$(HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/agent-sources-helper.sh" sync 2>&1)
	local status=$?
	if [[ "$status" -ne 0 ]]; then
	...
	local dest="$TEST_HOME/.maestro/agents/custom/private-core"
	...
	local primary_link="$TEST_HOME/.maestro/agents/client-ops.md"
...
test_index_discovers_core_layout() {
	local output=""
	output=$(HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/subagent-index-helper.sh" generate 2>&1)
	local status=$?
	if [[ "$status" -ne 0 ]]; then
