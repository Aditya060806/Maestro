...
# touching the real ~/.maestro files. Tests are offline (--no-pr-check).
...
#                      counted false as a non-empty runtime error marker.
{
	...
	printf '{"ts":%d,"role":"worker","session_key":"issue-3","session_id":"ses_3","issue_number":22349,"repo_slug":"Aditya060806/Maestro","work_dir":"/tmp/wt-3","output_file":"/tmp/excerpt-3.log","result":"watchdog_stall_killed","failure_reason":"watchdog_stall_killed","launch_failure_cause":"stall_hard_killed","kill_reason":"hard_kill_stall","next_action":"redispatch_worker","exit_code":79}\n' "$T_2H_AGO"
...
assert_eq "2h8: failure groups include repo evidence" "Aditya060806/Maestro" \
...
JSONC_DEFAULTS="$FIXTURE_DIR/maestro.defaults.jsonc"
...
JSON=$(env "${RUN_ENV[@]}" "GH_CALL_LOG=$GH_CALL_LOG" "PATH=$GH_STUB_DIR:$PATH" \
	"$HELPER" summary --since 24h --repo Aditya060806/Maestro --pr-check --json 2>&1)
RC=$?
