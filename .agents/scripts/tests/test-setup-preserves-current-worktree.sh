...
make_temp_dir() {
	...
	tmp_dir=$(mktemp -d "$tmp_parent/maestro-setup-worktree-test.XXXXXX")
...
remove_test_dir() {
	...
	case "$resolved_tmp" in
		"$tmp_parent"/maestro-setup-worktree-test.*)
			;;
		*)
...
test_preserves_current_worktree_entry() {
	...
	repos_file="$tmp_dir/home/.config/maestro/repos.json"
	flag_file="$tmp_dir/home/.maestro/logs/.migrated-worktree-repos-json-t2250"
