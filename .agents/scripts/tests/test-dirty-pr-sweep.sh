...
mkdir -p "${HOME}/.maestro/logs" "${HOME}/.maestro/.agent-workspace/supervisor" \
	"${HOME}/.config/maestro"
# Minimal repos.json (empty initialized_repos array)
printf '%s' '{"initialized_repos":[]}' >"${HOME}/.config/maestro/repos.json"

# Point state + interval files at the sandbox so classification tests never
# touch real state.
export DIRTY_PR_SWEEP_LAST_RUN="${HOME}/.maestro/logs/dirty-pr-sweep-last-run"
export DIRTY_PR_SWEEP_STATE_FILE="${HOME}/.maestro/.agent-workspace/supervisor/dirty-pr-sweep-state.json"
export MAESTRO_DIRTY_PR_WORKER_BACKED_CHECK=0
...
decision=$(_dirty_pr_classify "$PR_YOUNG_WORKER" "test/repo" "$REPO_ROOT" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_STALE" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_CODE_CONFLICT" "test/repo" "$REPO_ROOT" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_OPTOUT" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_PARENT" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_ORPHAN" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_RESOLVES" "test/repo" "" "Aditya060806")
case "$decision" in
...
export MAESTRO_DIRTY_PR_WORKER_BACKED_CHECK=1
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_FOR" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_REF" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_8D" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_15D" "test/repo" "" "Aditya060806")
...
decision=$(_dirty_pr_classify "$PR_WORKER_10D" "test/repo" "" "Aditya060806")
case "$decision" in
...
decision=$(_dirty_pr_classify "$PR_INTERACTIVE_PARENT" "test/repo" "" "Aditya060806")
case "$decision" in
...
case "\$1" in
	...
	api)
		case "\$2" in
			user) printf '{"login":"Aditya060806"}' ;;
		esac
		;;
esac
exit 0
