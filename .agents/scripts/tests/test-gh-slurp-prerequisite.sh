...
assert_supported() {
	local version="$1"
	if maestro_version_at_least "$version" "$MAESTRO_GH_MIN_SLURP_VERSION"; then
		print_result "gh ${version} supports --slurp" 0
	else
		print_result "gh ${version} supports --slurp" 1 "expected >= ${MAESTRO_GH_MIN_SLURP_VERSION} to pass"
	fi
	return 0
}
...
assert_parse() {
	...
	local expected="$3"
	local got=""
	got=$(maestro_parse_semver "$input" 2>/dev/null || true)
...
assert_parse_fails() {
	local label="$1"
	local input="$2"
	if maestro_parse_semver "$input" >/dev/null 2>&1; then
...
assert_status_contains() {
	...
	local expected="$2"
	local got=""
	got=$(maestro_gh_slurp_status_message)
	if [[ "$got" == *"$expected"* ]]; then
		print_result "$label" 0
	else
