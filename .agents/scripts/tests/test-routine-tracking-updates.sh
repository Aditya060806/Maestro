...
test_linux_core_scheduler_commands_are_logged() {
	...
	mkdir -p "$fake_home/.maestro/agents/scripts" "$fake_home/.maestro/.agent-workspace/logs"
	local profile_script="$fake_home/.maestro/agents/scripts/profile-readme-helper.sh"
	local screen_script="$fake_home/.maestro/agents/scripts/screen-time-helper.sh"
	...
	HOME="$fake_home"
	_install_profile_readme_scheduler "sh.maestro.profile-readme-update" "maestro-profile-readme-update" "$profile_script" "$fake_home/profile.log"
...
test_pulse_routine_update_uses_flags_and_duration() {
	...
	local agents_dir="$fake_home/.maestro/agents"
...
test_opencode_archive_scheduler_is_daily_and_low_priority() {
	...
	mkdir -p "$fake_home/.maestro/agents/scripts"
	local archive_script="$fake_home/.maestro/agents/scripts/opencode-db-archive-async-helper.sh"
	printf '#!/usr/bin/env bash\nexit 0\n' >"$archive_script"
	...
	if [[ "$captured_service" != "maestro-opencode-db-archive" ]]; then
	...
	if [[ "$captured_command" != "\"${archive_script}\"" || "$captured_log" != "$fake_home/.maestro/logs/opencode-db-archive.log" ]]; then
...
test_opencode_archive_launchd_uses_safe_path_expansion() {
	...
	maestro_launchd_sanitized_path() { local value="${1:-}"; printf '%s\n' "$value"; return $?; }
