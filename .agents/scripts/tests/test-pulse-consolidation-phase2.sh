...
setup_test_env() {
	...
	mkdir -p "${HOME}/.maestro/cache" "${HOME}/.maestro/logs"
...
_sentinel_path() {
	printf '%s/.maestro/cache/pulse-cycle-%s-consolidation-fired\n' \
		"$HOME" "$$"
	return 0
}
...
test_stale_sentinel_glob_cleanup() {
	...
	mkdir -p "${fake_home}/.maestro/cache"

	# Stale sentinel left by a hypothetical previous cycle.
	local stale_sentinel="${fake_home}/.maestro/cache/pulse-cycle-99999-consolidation-fired"
	touch "$stale_sentinel"

	# An unrelated cache file that must NOT be removed.
	local unrelated="${fake_home}/.maestro/cache/some-other-file"
	touch "$unrelated"

	# Exactly the cleanup command from pulse-wrapper.sh.
	# shellcheck disable=SC2086,SC2015
	rm -f "${fake_home}/.maestro/cache/pulse-cycle-"*"-consolidation-fired" 2>/dev/null || true
...
test_dispatch_writes_sentinel() {
	...
	local fake_home="${tmp}/home"
	mkdir -p "${fake_home}/.maestro/cache" "${fake_home}/.maestro/logs"
  ...
  {"user": {"login": "bob",   "type": "User"}, "created_at": "2026-01-01T01:00:00Z", "body": "%s"}
	...
	local sentinel="${fake_home}/.maestro/cache/pulse-cycle-$$-consolidation-fired"
