...
# maestro signature footer as --body without mutating the caller-owned file and
...
export MAESTRO_SIG_CLI="OpenCode"
export MAESTRO_SIG_CLI_VERSION="test"
export MAESTRO_SIG_MODEL="openai/gpt-5.5"
export MAESTRO_SIG_TOKENS="1"
...
count_captured_signatures() {
	grep -c '<!-- maestro:sig -->' "$GH_BODY_CAPTURE_FILE" 2>/dev/null || printf '0\n'
	return 0
}
...
assert_body_file_signed() {
	...
	if [[ "$(count_captured_signatures)" == "1" ]] && ! grep -q '<!-- maestro:sig -->' "$body_file"; then
		print_result "$name" 0
	else
		print_result "$name" 1 "captured signatures=$(count_captured_signatures), original_mutated=$(grep -c '<!-- maestro:sig -->' "$body_file" 2>/dev/null || printf '0')"
	fi
	return 0
}
...
assert_existing_signature_not_duplicated() {
	...
	printf '## Summary\n\nBody\n\n<!-- maestro:sig -->\n---\n[maestro.sh](https://maestro.sh) existing\n' >"$body_file"
	reset_capture
	gh_pr_comment 1 --repo o/r --body-file="$body_file" >/dev/null 2>&1
