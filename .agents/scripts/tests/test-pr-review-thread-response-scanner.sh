...
setup_test_env() {
	...
	export MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR="${TEST_ROOT}/state"
	export HEADLESS_LOG="${TEST_ROOT}/headless.log"
	export HEADLESS_PROMPT_CAPTURE="${TEST_ROOT}/prompt.md"
	mkdir -p "${HOME}" "${TEST_ROOT}/bin" "${TEST_ROOT}/repo" "${MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR}"
	write_fake_gh_stub
...
test_dispatch_launches_worker_and_writes_state() {
	...
	local state_file="${MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR}/owner-repo-1.state"
...
test_dispatch_pr_skips_when_pr_lock_held() {
	setup_test_env
	local lock_dir="${MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR}/owner-repo-1.lock"
	mkdir -p "$lock_dir"
	{
		printf 'pid=%s\n' "$$"
		printf 'created_at=%s\n' "$(date +%s)"
...
5	Five	false	origin:worker	feature/five	worker-bot'
	local state_dir="$MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR"
...
test_reply_and_resolve_use_graphql_mutations() {
	...
	printf '<!-- maestro:review-thread-response:THREAD1 -->\n@reviewer fixed at file.sh:1; verified with test.sh\n' >"$body_file"
	$SCANNER reply owner/repo THREAD1 "$body_file" 'maestro:review-thread-response:THREAD1'
...
test_reply_falls_back_when_thread_author_missing() {
	...
	$SCANNER reply owner/repo THREAD1 "$body_file" 'maestro:review-thread-response:THREAD1'
	captured=$(<"$GRAPHQL_BODY_CAPTURE")
	if [[ "$captured" == '<!-- maestro:review-thread-response:THREAD1 -->'* ]]; then
...
if [[ "$1" == "api" && "${2:-}" == "graphql" ]]; then
	if [[ "$*" == *"comments(first: 100)"* ]]; then
		printf '{"data":{"node":{"comments":{"nodes":[{"body":"<!-- maestro:review-thread-response:THREAD1 --> already"}]}}}}\n'
		exit 0
	fi
	if [[ "$*" == *"addPullRequestReviewThreadReply"* ]]; then
		printf 'reply\n' >>"${GRAPHQL_MUTATIONS_LOG:-/dev/null}"
