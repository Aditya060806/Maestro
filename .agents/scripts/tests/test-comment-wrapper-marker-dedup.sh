...
if [[ "$cmd" == "footer" ]]; then
    ...
    printf '\n<!-- maestro:sig -->\n---\n[maestro.sh](https://maestro.sh) v0.0.0-test footer.\n'
fi
exit 0
STUB
...
assert_contains "sig footer appended" "<!-- maestro:sig -->" "$captured_body"
...
file_content=$(<"$bf")
captured_body=$(<"$GH_STUB_BODY_FILE")
assert_not_contains "original file was not mutated" "<!-- maestro:sig -->" "$file_content"
...
assert_contains "sig footer appended to gh body" "<!-- maestro:sig -->" "$captured_body"
...
captured_body=$(<"$GH_STUB_BODY_FILE")
sig_count=$(grep -c 'maestro:sig' <<<"$captured_body" || true)
if [[ "$sig_count" == "1" ]]; then
	echo "  PASS: marker appears exactly once — no double-sign"
	PASS=$((PASS + 1))
else
	echo "  FAIL: expected exactly 1 maestro:sig marker, got $sig_count"
