...
run_pre_push_with_fake_coderabbit() {
	(
		...
		PATH="${TMP_DIR}/bin:${PATH}" \
			MAESTRO_CODERABBIT_CLI_REVIEW_TIMEOUT="${MAESTRO_CODERABBIT_CLI_REVIEW_TIMEOUT:-1}" \
			MAESTRO_SKIP_CODERABBIT_CLI_REVIEW="${MAESTRO_SKIP_CODERABBIT_CLI_REVIEW:-0}" \
			bash -c '
				set -euo pipefail
...
test_coderabbit_review_times_out_fail_open() {
	...
	start=$(date +%s)
	status=0
	output=$(MAESTRO_CODERABBIT_CLI_REVIEW_TIMEOUT=1 run_pre_push_with_fake_coderabbit 2>&1) || status=$?
	end=$(date +%s)
	duration=$((end - start))
...
test_coderabbit_review_skip_env_bypasses_only_optional_step() {
	local name="MAESTRO_SKIP_CODERABBIT_CLI_REVIEW bypasses optional review"
	setup_tmp
	write_fake_coderabbit 'exit 42'

	local output status
	status=0
	output=$(MAESTRO_SKIP_CODERABBIT_CLI_REVIEW=1 run_pre_push_with_fake_coderabbit 2>&1) || status=$?

	if [[ $status -ne 0 ]]; then
		fail "$name" "expected exit 0, got ${status}; output=${output}"
	elif [[ "$output" != *"MAESTRO_SKIP_CODERABBIT_CLI_REVIEW=1"* ]]; then
