...
test_case_a_owner_issue_passes() {
	...
	printf '{"author_association":"OWNER"}' >"${TEST_ROOT}/issue.json"
	printf '[]' >"${TEST_ROOT}/comments.json"
	export MAESTRO_WORKER_BRIEFED_AUTO_MERGE=1
...
test_case_e_nmr_crypto_cleared_passes() {
	...
	printf '[{"body":"auto-approved-maintainer-issue: cleared NMR"},{"body":"maestro:approval-signature: SHA256:abc123"}]' >"${TEST_ROOT}/comments.json"
...
# =============================================================================
# Case (j): Feature flag MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0 → blocked
...
test_case_j_feature_flag_off_blocked() {
	...
	printf '{"author_association":"OWNER"}' >"${TEST_ROOT}/issue.json"
	printf '[]' >"${TEST_ROOT}/comments.json"
	export MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0
	...
	if [[ "$result" -eq 0 ]]; then
		...
		if grep -q "disabled by MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0" "$LOGFILE" 2>/dev/null; then
...
test_case_k_non_owner_with_crypto_passes() {
	...
	printf '[{"body":"maestro:approval-signature: SHA256:abc123"}]' >"${TEST_ROOT}/comments.json"
	export APPROVAL_VERIFY_RESULT="VERIFIED"
	export MAESTRO_WORKER_BRIEFED_AUTO_MERGE=1
...
test_case_n_trusted_author_allowlist_passes() {
	...
	export MAESTRO_TRUSTED_AUTHORS_CONF="$_conf"
	...
	unset MAESTRO_TRUSTED_AUTHORS_CONF
	teardown_test_env
	return 0
}
...
# Maintainer-operated maestro workers may surface as COLLABORATOR in webhook
...
test_case_v_spoofed_crypto_marker_blocked() {
	...
	printf '{"author_association":"NONE"}' >"${TEST_ROOT}/issue.json"
	printf '[{"body":"maestro:approval-signature: SHA256:spoofed"}]' >"${TEST_ROOT}/comments.json"
	export APPROVAL_VERIFY_RESULT=""
	export MAESTRO_WORKER_BRIEFED_AUTO_MERGE=1
