...
#
# Tests the _check_script_drift function in maestro-update-check.sh:
#
...
#   pulse kept using the old in-memory code for 90+ minutes because maestro
...
#
# Fix (t2156): setup.sh now writes ~/.maestro/.deployed-sha after every
# successful deploy. maestro-update-check.sh (running every ~10 min) detects
# SHA drift and triggers a background silent redeploy if framework code changed.
#
# Implementation: .agents/scripts/maestro-update-check.sh (_check_script_drift)
#                 .agents/scripts/setup/modules/agent-deploy.sh (deploy_maestro_agents stamp write)

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
UPDATE_CHECK_SCRIPT="$(cd "$SCRIPT_DIR/.." && pwd)/maestro-update-check.sh"
...
mkdir -p "$SANDBOX_HOME" "$SANDBOX_MAESTRO"
...
#   $2 — MAESTRO_FRAMEWORK_REPO override (default: $SANDBOX_REPO)
...
call_drift_check() {
	...
	if [[ -n "$stamp" ]]; then
		printf '%s\n' "$stamp" >"${SANDBOX_MAESTRO}/.deployed-sha"
	else
	...
	HOME="$SANDBOX_HOME" MAESTRO_FRAMEWORK_REPO="$repo" bash -c "
		set +e
...
# ---------------------------------------------------------------------------
result=$(call_drift_check "$DEPLOYED_SHA" "/nonexistent/path/maestro")
if [[ -z "$result" ]]; then
...
# ---------------------------------------------------------------------------
NOXEC_REPO="${TEST_ROOT}/maestro-noxec"
