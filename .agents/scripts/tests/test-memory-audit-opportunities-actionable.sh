...
export MAESTRO_MEMORY_AUDIT_LOG_DIR="$TEST_DIR/audit"
mkdir -p "$MAESTRO_MEMORY_DIR" "$MAESTRO_MEMORY_AUDIT_LOG_DIR"
...
seed_repeated_failures() {
	...
	local test_db="$MAESTRO_MEMORY_DIR/memory.db"
	local i
	for i in 1 2 3; do
		sqlite3 "$test_db" \
...
seed_repeated_failures
write_mock_task_cmd
export MAESTRO_MEMORY_AUDIT_TASK_CMD="$MOCK_TASK_CMD"
export MOCK_LOG
...
if ! grep -Fq 'would-file' "$MAESTRO_MEMORY_AUDIT_LOG_DIR/last-opportunity-actions.txt"; then
...
if ! grep -Fq 'skipped-duplicate' "$MAESTRO_MEMORY_AUDIT_LOG_DIR/last-opportunity-actions.txt"; then
	fail "second live audit did not record duplicate suppression: $second_output"
fi

if ! grep -Fq 'Opportunity filing actions:' "$MAESTRO_MEMORY_AUDIT_LOG_DIR"/audit-*.txt; then
