...
# An env override `MAESTRO_WORKTREE_ALLOW_NESTED=1` bypasses the guard
...
#     8. MAESTRO_WORKTREE_ALLOW_NESTED=1 bypasses guard even on nested
...
#     12. `MAESTRO_WORKTREE_ALLOW_NESTED=1 add feature/X some-nested-path`
...
unset MAESTRO_WORKTREE_ALLOW_NESTED
rc=0
...
# Test 8: MAESTRO_WORKTREE_ALLOW_NESTED=1 bypasses guard on nested path
rc=0
if ! MAESTRO_WORKTREE_ALLOW_NESTED=1 _cmd_add_assert_path_outside_repo "main" "feature/x" 2>/dev/null; then
	rc=1
fi
print_result "assert: MAESTRO_WORKTREE_ALLOW_NESTED=1 bypasses guard" "$rc"
unset MAESTRO_WORKTREE_ALLOW_NESTED
...
# =============================================================================
cd "$FAKE_REPO"
export MAESTRO_SKIP_AUTO_CLAIM=1      # skip interactive-session-helper side effects
export MAESTRO_NO_NETWORK=1            # defensive: some helpers honour this
...
rc=0
set +e
stderr_out="$(MAESTRO_WORKTREE_ALLOW_NESTED=1 bash "$HELPER_SRC" add feature/t2701-testbranch3 nested-ok 2>&1 >/dev/null)"
set -e
# The mentoring error contains the word 'MAESTRO_WORKTREE_ALLOW_NESTED' only
# when the guard fires. If the guard was bypassed, that string must NOT appear.
case "$stderr_out" in
	*"MAESTRO_WORKTREE_ALLOW_NESTED=1 — use only"*) rc=1 ;;
esac
print_result "cli: MAESTRO_WORKTREE_ALLOW_NESTED=1 bypasses guard" "$rc"
