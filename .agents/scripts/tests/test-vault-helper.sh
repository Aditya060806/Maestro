...
TEST_ROOT="$(mktemp -d 2>/dev/null || mktemp -d -t maestro-vault-test)"
PASS=0
FAIL=0

cleanup() {
	MAESTRO_VAULT_DIR="$TEST_ROOT/vault" "$VAULT_HELPER" lock >/dev/null 2>&1 || true
	rm -rf "$TEST_ROOT"
	return 0
}
trap cleanup EXIT
...
export MAESTRO_VAULT_DIR="$TEST_ROOT/vault"
export MAESTRO_VAULT_RUNTIME_DIR="$TEST_ROOT/run"
...
if python3 -m json.tool "$MAESTRO_VAULT_DIR/vault.json" >/dev/null; then
	pass "metadata parses as json"
else
	fail "metadata parses as json"
fi
if grep -q "$generated_pass" "$MAESTRO_VAULT_DIR/vault.json"; then
...
broker_pid_file="$MAESTRO_VAULT_RUNTIME_DIR/broker.pid"
...
cp "$MAESTRO_VAULT_DIR/vault.json" "$MAESTRO_VAULT_DIR/vault.json.good"
printf '{not-json' >"$MAESTRO_VAULT_DIR/vault.json"
set +e
...
mv "$MAESTRO_VAULT_DIR/vault.json.good" "$MAESTRO_VAULT_DIR/vault.json"
