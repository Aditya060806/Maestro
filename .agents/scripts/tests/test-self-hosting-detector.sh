...
#   test_bypass_env_var            — MAESTRO_SKIP_SELF_HOSTING_DETECTOR=1
...
teardown_test_env() {
	...
	unset MAESTRO_SKIP_SELF_HOSTING_DETECTOR MAESTRO_SELF_HOSTING_DETECTOR_DRY_RUN 2>/dev/null || true
	return 0
}
...
# test_bypass_env_var — MAESTRO_SKIP_SELF_HOSTING_DETECTOR=1 → skips
test_bypass_env_var() {
	setup_test_env
	local body_file
	body_file=$(create_body_with_dispatch_path)
	...
	export MAESTRO_SKIP_SELF_HOSTING_DETECTOR=1
...
test_dry_run_mode() {
	...
	export MAESTRO_SELF_HOSTING_DETECTOR_DRY_RUN=1

	local rc=0
	local output
	output=$("$HELPER_SCRIPT" validate "105" "Aditya060806/Maestro" 2>&1) || rc=$?
