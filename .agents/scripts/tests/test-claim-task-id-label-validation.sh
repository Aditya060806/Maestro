...
_make_test_repo() {
	...
	local upstream="${tmpdir}/github.com/Aditya060806/Maestro.git"
...
test_case_a_invalid_label_exit3() {
	...
	unset MAESTRO_LABEL_CACHE_FILE 2>/dev/null || true
...
test_case_e_unit_validate_invalid() {
	...
	local rc=0
	_validate_labels_exist "Aditya060806/Maestro" "bug,debt" 2>/dev/null || rc=$?
...
test_case_f_unit_validate_valid() {
	...
	local rc=0
	_validate_labels_exist "Aditya060806/Maestro" "bug,tier:standard,auto-dispatch" 2>/dev/null || rc=$?
