...
PS_FIRING=$(printf '%s\n' \
	"  101 /bin/bash /Users/x/.maestro/agents/scripts/pulse-wrapper.sh" \
	"  102 /bin/bash /Users/x/.maestro/agents/scripts/pulse-wrapper.sh" \
	"  103 /bin/bash /Users/x/.maestro/agents/scripts/pulse-wrapper.sh" \
	"  104 /bin/bash /Users/x/.maestro/agents/scripts/pulse-wrapper.sh" \
	"  105 /bin/bash /Users/x/.maestro/agents/scripts/pulse-wrapper.sh" \
	"  106 /bin/bash /Users/x/.maestro/agents/scripts/pulse-wrapper.sh" \
...
out=$(_run_detector "$RULES_DIR/deployed-vs-source-mtime-drift.sh" \
	"MAESTRO_DEPLOYED_DIR=$DRIFT_DEPLOYED" \
	"MAESTRO_SOURCE_DIR=$DRIFT_SOURCE" \
	"DRIFT_SECONDS=3600" \
...
out=$(_run_detector "$RULES_DIR/deployed-vs-source-mtime-drift.sh" \
	"MAESTRO_DEPLOYED_DIR=$DRIFT_DEPLOYED2" \
	"MAESTRO_SOURCE_DIR=$DRIFT_SOURCE2" \
	"DRIFT_SECONDS=3600" \
	"WATCHED_FILES=pulse-wrapper.sh") && rc=0 || rc=$?
...
# Missing dirs → no-op
out=$(_run_detector "$RULES_DIR/deployed-vs-source-mtime-drift.sh" \
	"MAESTRO_DEPLOYED_DIR=/nonexistent/x" \
	"MAESTRO_SOURCE_DIR=/nonexistent/y") && rc=0 || rc=$?
