...
test_sampler_writes_expected_schema() {
	...
	if jq -e 'select(.pid and .ppid and .role == "worker" and .session_key == "gh-22286" and .repo == "Aditya060806/Maestro" and .issue == "22286" and (.cpu_seconds | type == "number") and (.rss_kb | type == "number") and (.peak_rss_kb | type == "number") and (.elapsed_s | type == "number") and .timestamp)' "$out_file" >/dev/null; then
