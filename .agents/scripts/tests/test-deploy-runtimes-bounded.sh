...
#   2. Kills a slow deployment within MAESTRO_DEPLOY_RUNTIMES_TIMEOUT seconds and
...
_write_bounded_wrapper_script() {
	...
	local timeout_s="$3"    # MAESTRO_DEPLOY_RUNTIMES_TIMEOUT value
...
MAESTRO_DEPLOY_RUNTIMES_TIMEOUT=${timeout_s}

_deploy_agents_to_runtimes_bounded() {
	local timeout_s="\${MAESTRO_DEPLOY_RUNTIMES_TIMEOUT:-120}"
...
test_bounded_wrapper_timeout_env_respected() {
	...
	# Deploy that sleeps 10s; MAESTRO_DEPLOY_RUNTIMES_TIMEOUT=2 should terminate it
	...
	if [[ "$rc" -eq 0 && "$elapsed" -le 7 ]]; then
		print_result "MAESTRO_DEPLOY_RUNTIMES_TIMEOUT env var respected (${elapsed}s)" 0
	else
		print_result "MAESTRO_DEPLOY_RUNTIMES_TIMEOUT env var respected (${elapsed}s)" 1 \
...
test_bounded_wrapper_child_failure_is_noncritical_without_err_trap_noise() {
...

	{
		...
		printf '%s\n' 'MAESTRO_DEPLOY_RUNTIMES_TIMEOUT=5'
		printf '%s\n' "$wrapper_definition"
		printf '%s\n' '_deploy_agents_to_runtimes_bounded'
	} >"$script"
	chmod +x "$script"
...
test_real_bounded_wrapper_timeout_returns_zero() {
...

	{
		...
		printf '%s\n' 'MAESTRO_DEPLOY_RUNTIMES_TIMEOUT=2'
		printf '%s\n' "$wrapper_definition"
		printf '%s\n' '_deploy_agents_to_runtimes_bounded'
	} >"$script"
	chmod +x "$script"
