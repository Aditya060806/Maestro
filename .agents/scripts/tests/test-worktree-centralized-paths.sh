...
mkdir -p "$HOME/.config/maestro" "$TEST_ROOT/Git"

# shellcheck source=../worktree-paths.sh
source "${TEST_SCRIPTS_DIR}/worktree-paths.sh"

REPOS_JSON="$HOME/.config/maestro/repos.json"
export MAESTRO_REPOS_JSON="$REPOS_JSON"

rc=0
maestro_migrate_repos_json_worktree_base_dir "$REPOS_JSON" "~"'/Git/_worktrees' || rc=1
[[ -f "$REPOS_JSON" ]] || rc=1
...
rc=0
base_dir=$(maestro_worktree_base_dir) || rc=1
...
rc=0
path=$(maestro_generate_worktree_path "$REPO" "feature/Great-Thing") || rc=1
...
export MAESTRO_WORKTREE_BASE_DIR="$custom"
path=$(maestro_generate_worktree_path "$REPO" "bugfix/Login") || rc=1
unset MAESTRO_WORKTREE_BASE_DIR
