...
# shellcheck source=../maestro-cli/maestro-repos-lib.sh
source "$REPO_ROOT/.agents/scripts/maestro-cli/maestro-repos-lib.sh"
# shellcheck source=../maestro-cli/maestro-init-lib.sh
source "$REPO_ROOT/.agents/scripts/maestro-cli/maestro-init-lib.sh"
# shellcheck source=../maestro-cli/maestro-update-lib.sh
source "$REPO_ROOT/.agents/scripts/maestro-cli/maestro-update-lib.sh"
...
assert_contains "managed marker present" "$agent_repo/AGENTS.md" "maestro:agent-source-template-version: 1"
...
text = path.read_text()
text = text.replace('maestro:agent-source-template-version: 1', 'maestro:agent-source-template-version: 0')
path.write_text(text + '\nUser-authored note outside managed block.\n')
PY

_update_sync_agent_source_repos "9.9.9"
assert_contains "update refreshes managed block" "$agent_repo/AGENTS.md" "maestro:agent-source-template-version: 1"
...
version=$(jq -r '.agent_source as $a | .version + ":" + ($a|tostring)' "$agent_repo/.maestro.json" 2>/dev/null || true)
