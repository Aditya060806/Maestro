...
install_helper_stubs() {
	gh() {
		printf '%s\n' "$*" >>"$GH_CALL_LOG"
		if [[ "$1" == "api" && "$2" == "repos/Aditya060806/Maestro/pulls/33333" ]]; then
			printf '{"number":33333}\n'
			return 0
		fi
		if [[ "$1" == "api" && "$2" == "repos/Aditya060806/Maestro/pulls/"* ]]; then
			return 1
		fi
...
test_provided_empty_pr_labels_skip_refetch() {
	...
	_handle_post_merge_actions "22585" "Aditya060806/Maestro" "22219" "merged" ""

	assert_log_not_contains "$GH_CALL_LOG" "pr view" \
		"provided empty pr_labels skips fallback fetch"
	assert_log_contains "$SOLVED_LABEL_LOG" "22219 Aditya060806/Maestro interactive" \
		"provided empty pr_labels keeps interactive solved attribution"
	assert_log_contains "$GH_CALL_LOG" "cleanup 22219 Aditya060806/Maestro post-merge-pr-22585" \
...
test_omitted_pr_labels_fetches_fallback() {
	...
	assert_log_contains "$SOLVED_LABEL_LOG" "22219 Aditya060806/Maestro worker" \
...
test_superseded_pr_closes_original_issue() {
	...
	_handle_post_merge_actions "44444" "Aditya060806/Maestro" "33333" "merged" "origin:worker"

	assert_log_contains "$GH_CALL_LOG" "repos/Aditya060806/Maestro/pulls/33333" \
	...
	assert_log_contains "$GH_CALL_LOG" "cleanup 22219 Aditya060806/Maestro post-merge-superseded-pr-44444" \
...
test_closing_comment_uses_pr_base_ref() {
	...
	_handle_post_merge_actions "753" "Aditya060806/Maestro" "25350" "merged" "origin:worker" "develop"
...
test_closing_comment_defaults_to_main_for_legacy_callers() {
	...
	_handle_post_merge_actions "754" "Aditya060806/Maestro" "25350" "" "origin:worker"
