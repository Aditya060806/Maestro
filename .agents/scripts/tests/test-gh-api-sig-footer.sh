...
# The function searches HOME/.maestro/agents/scripts/ and dirname(BASH_SOURCE[0]).
...
export MAESTRO_SESSION_ORIGIN=worker
export MAESTRO_SESSION_USER=testworker
...
if grep -q "<!-- maestro:sig -->" "$_BODY1" 2>/dev/null; then
	pass "_rest_issue_create injects <!-- maestro:sig --> into body file"
else
	fail "_rest_issue_create injects <!-- maestro:sig --> into body file" \
...
if grep -q "<!-- maestro:sig -->" "$_BODY2" 2>/dev/null; then
	pass "_rest_issue_comment injects <!-- maestro:sig --> into body file"
else
	fail "_rest_issue_comment injects <!-- maestro:sig --> into body file" \
...
if grep -q "<!-- maestro:sig -->" "$_BODY3" 2>/dev/null; then
	pass "_rest_pr_create injects <!-- maestro:sig --> into body file"
else
	fail "_rest_pr_create injects <!-- maestro:sig --> into body file" \
...
if _shim_api_is_write_endpoint; then
	...
	if grep -q "<!-- maestro:sig -->" "$_BODY4" 2>/dev/null; then
...
if _shim_api_is_write_endpoint; then
	_shim_api_inject_body_sig
	if grep -q "<!-- maestro:sig -->" "$_BODY5" 2>/dev/null; then
