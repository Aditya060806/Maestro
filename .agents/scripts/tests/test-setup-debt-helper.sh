...
# ~/.maestro/advisories/ tree, then asserts the helper's behaviour.
...
test_single_advisory_human() {
	...
	echo "[ADVISORY] SYNC_PAT not set for exampleorg/examplerepo" > "$HOME/.maestro/advisories/sync-pat-exampleorg-examplerepo.advisory"
	local out
...
test_multi_advisory_count() {
	_setup_sandbox
	echo "[ADVISORY] X" > "$HOME/.maestro/advisories/sync-pat-foo-bar.advisory"
	echo "[ADVISORY] X" > "$HOME/.maestro/advisories/sync-pat-baz-qux.advisory"
	echo "[ADVISORY] X" > "$HOME/.maestro/advisories/sync-pat-quux-quuz.advisory"
	local out
	out="$("$HELPER" summary --format=toast)"
...
test_dismissed_advisory_excluded() {
	...
	echo "[ADVISORY] X" > "$HOME/.maestro/advisories/sync-pat-baz-qux.advisory"
	# Dismiss one
	echo "sync-pat-foo-bar" > "$HOME/.maestro/advisories/dismissed.txt"
	local out
	out="$("$HELPER" summary --format=toast)"
...
test_cross_account_advisory_detected() {
	...
	echo "[ADVISORY] Cross-account secrets:inherit detected for org/repo" \
		>"$HOME/.maestro/advisories/cross-account-inherit-org-repo.advisory"
	local out
...
test_same_account_advisory_not_detected() {
	...
	echo "[ADVISORY] SYNC_PAT not set for sameorg/repo" \
		>"$HOME/.maestro/advisories/sync-pat-sameorg-repo.advisory"
	local out
	out="$("$HELPER" list-cross-account-inherit)"
...
test_missing_workflow_no_false_positive() {
	...
	touch "$HOME/.maestro/advisories/unrelated.txt"
	local out
	out="$("$HELPER" list-cross-account-inherit)"
...
test_summary_aggregates_both_classes() {
	...
	echo "[ADVISORY] SYNC_PAT not set for org/a" \
		>"$HOME/.maestro/advisories/sync-pat-org-a.advisory"
	echo "[ADVISORY] Cross-account detected for org/b" \
		>"$HOME/.maestro/advisories/cross-account-inherit-org-b.advisory"
...
test_cross_account_dismissed_excluded() {
	...
	echo "[ADVISORY] Cross-account detected for org/repo" \
		>"$HOME/.maestro/advisories/cross-account-inherit-org-repo.advisory"
	echo "cross-account-inherit-org-repo" >"$HOME/.maestro/advisories/dismissed.txt"
	local out
	out="$("$HELPER" list-cross-account-inherit)"
