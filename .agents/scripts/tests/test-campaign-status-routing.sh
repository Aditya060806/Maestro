...
# test-campaign-status-routing.sh — regression tests for maestro campaign status routing.
...
test_bare_status_routes_to_provisioning() {
	...
	out="$(cd "${repo_path}" && bash "${MAESTRO_SH}" campaign status 2>&1)" || {
...
test_status_with_campaign_id_routes_to_detail() {
	...
	out="$(cd "${repo_path}" && bash "${MAESTRO_SH}" campaign status c001 2>&1)" || {
