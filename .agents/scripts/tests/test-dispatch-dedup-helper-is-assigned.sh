...
setup_test_env() {
	...
	mkdir -p "${TEST_ROOT}/config/maestro"
	export HOME="$TEST_ROOT"
	export PATH="${TEST_ROOT}/bin:${PATH}"

	# Create a repos.json with a known maintainer
	cat >"${TEST_ROOT}/config/maestro/repos.json" <<'EOF'
{
  "initialized_repos": [
    {
      "path": "/home/user/Git/maestro",
      "slug": "Aditya060806/Maestro",
      "pulse": true,
      "maintainer": "Aditya060806-bot"
    },
    {
...
EOF
	...
	export REPOS_JSON="${TEST_ROOT}/config/maestro/repos.json"
...
test_unassigned_issue() {
	...
	if "$HELPER_SCRIPT" is-assigned 100 Aditya060806/Maestro runner1 >/dev/null 2>&1; then
...
# GH#10521: Aditya060806 is the owner in Aditya060806/Maestro
test_assigned_to_repo_owner() {
	create_gh_stub "Aditya060806"
...
test_assigned_to_maintainer() {
	create_gh_stub "Aditya060806-bot"
...
test_assigned_to_another_runner() {
	...
	local output=""
	if output=$("$HELPER_SCRIPT" is-assigned 100 Aditya060806/Maestro runner1 2>/dev/null); then
		case "$output" in
...
test_mixed_assignees_with_runner() {
	create_gh_stub "Aditya060806,other-runner"
...
test_mixed_assignees_maintainer_and_self() {
	create_gh_stub "Aditya060806,runner1"
...
test_no_self_login_owner_assigned() {
	...
	if "$HELPER_SCRIPT" is-assigned 100 Aditya060806/Maestro >/dev/null 2>&1; then
...
test_owner_assigned_with_active_status_blocks() {
	create_gh_stub "Aditya060806" "status:queued"

	local output=""
	if output=$("$HELPER_SCRIPT" is-assigned 100 Aditya060806/Maestro runner1 2>/dev/null); then
		case "$output" in
		*'ASSIGNED:'*'Aditya060806'*)
...
test_maintainer_assigned_with_active_status_blocks() {
	create_gh_stub "Aditya060806-bot" "status:in-progress"

	local output=""
	if output=$("$HELPER_SCRIPT" is-assigned 100 Aditya060806/Maestro runner1 2>/dev/null); then
		case "$output" in
		*'ASSIGNED:'*'Aditya060806-bot'*)
...
test_owner_assigned_with_status_claimed_blocks() {
	create_gh_stub "Aditya060806" "status:claimed"
...
test_owner_assigned_with_status_in_review_blocks() {
	create_gh_stub "Aditya060806" "status:in-review"
...
test_owner_assigned_with_origin_interactive_blocks() {
	create_gh_stub "Aditya060806" "origin:interactive,bug"
...
test_maintainer_assigned_with_origin_interactive_blocks() {
	create_gh_stub "Aditya060806-bot" "origin:interactive"
...
test_owner_assigned_with_origin_interactive_auto_dispatch_allows() {
	create_gh_stub "Aditya060806" "origin:interactive,auto-dispatch,bug"
...
test_maintainer_assigned_with_origin_interactive_auto_dispatch_allows() {
	create_gh_stub "Aditya060806-bot" "origin:interactive,auto-dispatch,bug"
...
test_owner_assigned_no_status_no_origin_allows_dispatch() {
	create_gh_stub "Aditya060806" "bug"
...
test_override_ignore_preserves_self_assignee() {
	...
	cat >"${TEST_ROOT}/config/maestro/dispatch-override.conf" <<'EOF'
...
test_dispatch_comment_blocks_inside_active_worker_window() {
	...
	if output=$(DISPATCH_COMMENT_MAX_AGE=60 DISPATCH_ACTIVE_WORKER_MAX_AGE=300 "$HELPER_SCRIPT" has-dispatch-comment 100 Aditya060806/Maestro runner1 2>/dev/null); then
		case "$output" in
...
test_dispatch_comment_expires_after_active_worker_window() {
	...
	if DISPATCH_COMMENT_MAX_AGE=60 DISPATCH_ACTIVE_WORKER_MAX_AGE=300 "$HELPER_SCRIPT" has-dispatch-comment 100 Aditya060806/Maestro runner1 >/dev/null 2>&1; then
