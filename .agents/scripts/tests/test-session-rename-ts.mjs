...
const { getMaestroVersion, withMaestroTitleSuffix } = await import(
...
assertEq(
  "MAESTRO_TABBY_ENABLED=false disables title emit",
  false,
  isTerminalTitleEnabled({ MAESTRO_TABBY_ENABLED: "false" }),
);
assertEq("terminal title emit defaults to enabled", true, isTerminalTitleEnabled({}));

// --- Maestro session title suffix helpers ----------------------------------
console.log("\nGroup 4: Maestro session title suffix helpers");
assertEq(
  "suffix is appended",
  "Issue #123: concise title · Maestro 9.8.7",
  withMaestroTitleSuffix("Issue #123: concise title", "9.8.7"),
);
assertEq(
  "existing suffix is idempotently replaced",
  "Issue #123: concise title · Maestro 9.8.7",
  withMaestroTitleSuffix("Issue #123: concise title · Maestro 1.2.3", "9.8.7"),
);
assertEq(
  "issue prefix remains first",
  "Issue #456: preserve prefix · Maestro 9.8.7",
  withMaestroTitleSuffix("Issue #456: preserve prefix", "9.8.7"),
);
assertEq(
  "missing version leaves title without stale suffix",
  "Issue #123: concise title",
  withMaestroTitleSuffix("Issue #123: concise title · Maestro 1.2.3", ""),
);
process.env.MAESTRO_VERSION = "7.6.5";
assertEq("env version override is read", "7.6.5", getMaestroVersion());
...
assertEq(
  ...
  "/tmp/maestro-opencode/opencode/opencode.db",
  getDbPath({ XDG_DATA_HOME: "/tmp/maestro-opencode" }),
);
assertEq(
  "OPENCODE_DB overrides XDG_DATA_HOME",
  "/tmp/custom-opencode.db",
  getDbPath({ OPENCODE_DB: "/tmp/custom-opencode.db", XDG_DATA_HOME: "/tmp/maestro-opencode" }),
);
assertEq(
