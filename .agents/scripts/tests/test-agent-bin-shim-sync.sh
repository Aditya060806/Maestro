...
# ~/.maestro/bin when no longer deployed.
# =============================================================================
...
tmp_home="$(mktemp -d)"
tmp_target="${tmp_home}/.maestro/agents"
trap 'rm -rf "$tmp_home"' EXIT

mkdir -p "${tmp_target}/bin" "${tmp_home}/.maestro/bin" "${tmp_home}/elsewhere"
...
ln -sf "${tmp_target}/bin/retired_shim" "${tmp_home}/.maestro/bin/retired_shim"
ln -sf "${tmp_home}/elsewhere/external" "${tmp_home}/.maestro/bin/external_link"
printf 'user file\n' >"${tmp_home}/.maestro/bin/user_tool"
...
assert_missing "empty target_dir does not link system /bin shims" \
	"${tmp_home}/.maestro/bin/sh"

HOME="$tmp_home" _sync_agent_bin_shims "$tmp_target"

assert_symlink_target "gh_create_pr shim is linked into user PATH bin" \
	"${tmp_home}/.maestro/bin/gh_create_pr" "${tmp_target}/bin/gh_create_pr"
assert_symlink_target "gh_create_issue shim is linked into user PATH bin" \
	"${tmp_home}/.maestro/bin/gh_create_issue" "${tmp_target}/bin/gh_create_issue"
assert_missing "stale maestro-owned shim symlink is removed" \
	"${tmp_home}/.maestro/bin/retired_shim"
assert_exists "external symlink is preserved" \
	"${tmp_home}/.maestro/bin/external_link"
