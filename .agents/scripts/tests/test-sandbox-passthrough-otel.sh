...
#   whitelist covered MAESTRO_*, PULSE_*, GH_*, GITHUB_*, OPENAI_*,
...
#   5. Previously-covered prefix still works (MAESTRO_FOO) — regression guard
...
# -----------------------------------------------------------------------------
# Test 5 — Previously-covered prefix (MAESTRO_*) still works (regression guard)
# -----------------------------------------------------------------------------
csv_out=$(run_with_env MAESTRO_TEST_SENTINEL="1" \
	bash "$HELPER" passthrough-csv 2>/dev/null)

if [[ ",${csv_out}," == *",MAESTRO_TEST_SENTINEL,"* ]]; then
	pass "MAESTRO_* prefix still in passthrough CSV (no regression)"
else
	fail "MAESTRO_* prefix still in passthrough CSV (no regression)" \
		"got CSV: ${csv_out}"
fi
...
if [[ ",${csv_out}," == *",MAESTRO_SESSION_ORIGIN,"* ]] && \
	[[ ",${csv_out}," == *",MAESTRO_HEADLESS,"* ]] && \
	[[ ",${csv_out}," != *",HEADLESS,"* ]] && \
