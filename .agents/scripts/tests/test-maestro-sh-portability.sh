...
# test-maestro-sh-portability.sh — Regression tests for cross-platform command
# portability in maestro.sh (t2074, GH#18784).
#
# Background: PR #18686 added SUDO_USER → getent passwd home resolution to
# maestro.sh but omitted the `command -v getent` guard (which the sibling
# fix in .agents/scripts/approval-helper.sh:33 does include). On macOS, where
# getent is not part of the base system, `set -euo pipefail` killed maestro.sh
# before any subcommand could run, breaking `sudo maestro approve` completely.
#
# This test asserts two things:
#   1. The home-resolution block in maestro.sh guards getent behind a
...
test_getent_structural_guard() {
	_info "Test 1: maestro.sh getent call must be guarded"

	if [[ ! -f "${MAESTRO_SH}" ]]; then
		_fail "maestro.sh not found at ${MAESTRO_SH}"
		return 1
	fi
	...
	getent_line_num=$(grep -n 'getent passwd .*SUDO_USER' "${MAESTRO_SH}" | sed -n '1{s/:.*//;p;}' || true)

	if [[ -z "${getent_line_num}" ]]; then
		_fail "Could not find _MAESTRO_REAL_HOME=... getent passwd line in maestro.sh"
		return 1
	fi
	...
	local guard_block
	guard_block=$(sed -n "${guard_start},${getent_line_num}p" "${MAESTRO_SH}")
...
test_runtime_fallback_no_getent() {
	...
	# execute the exact guard+resolution block copied verbatim from maestro.sh.
	# We inline the block rather than sourcing maestro.sh because sourcing
	...
	out=$(bash -c '
		...
		export HOME="/tmp/maestro-test-fake-home"
		...
		if [[ -n "${SUDO_USER:-}" && "$(id -u)" -eq 0 ]] && command -v getent &>/dev/null; then
			_MAESTRO_REAL_HOME=$(getent passwd "$SUDO_USER" | cut -d: -f6)
		else
	...
	if [[ "${out}" == "/tmp/maestro-test-fake-home" ]]; then
...
test_worktree_command_dispatch() {
	_info "Test 4: maestro.sh exposes worktree helper through stable CLI"

	if grep -qE 'worktree[[:space:]]*\|[[:space:]]*wt\).*worktree-helper\.sh' "${MAESTRO_SH}"; then
		_pass "maestro worktree dispatches to worktree-helper.sh"
	else
		_fail "maestro.sh does not expose the worktree helper via maestro worktree"
		return 1
	fi
	return 0
}
...
# -----------------------------------------------------------------------------
main() {
	_info "maestro.sh cross-platform portability regression tests (t2074, GH#18784)"
	printf '\n'
