...
LOGFILE="${TEST_TMP}/pulse.log" \
	MAESTRO_WORKSPACE_DIR="$TEST_TMP" \
...
resolved_pid=$(PATH="${TEST_TMP}/bin:${PATH}" LOGFILE="${TEST_TMP}/pulse.log" _dlw_systemd_resolve_main_pid "maestro-test" "123") || fail "systemd PID resolver returned failure"
...
printf '{"bundle":"content-site"}\n' >"${bundle_repo}/.maestro.json" || fail "failed to create bundle config fixture"
