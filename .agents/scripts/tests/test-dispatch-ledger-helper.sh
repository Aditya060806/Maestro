...
setup_test_env() {
	TEST_ROOT=$(mktemp -d)
	export MAESTRO_DISPATCH_LEDGER_DIR="${TEST_ROOT}/ledger"
...
test_register_creates_entry() {
	...
	run_helper "$LEDGER_HELPER" register --session-key "issue-42" --issue 42 --repo "owner/repo" --pid $$ --worktree "/tmp/maestro-issue-42"

	local entry_count
	entry_count=$(wc -l <"${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" | tr -d ' ')
	...
	# Verify fields
	local status
	status=$(jq -r '.status' "${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" 2>/dev/null | head -1)
	if [[ "$status" != "in-flight" ]]; then
		result=1
	fi

	local session_key
	session_key=$(jq -r '.session_key' "${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" 2>/dev/null | head -1)
	if [[ "$session_key" != "issue-42" ]]; then
		result=1
	fi

	local worktree_path
	worktree_path=$(jq -r '.worktree_path' "${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" 2>/dev/null | head -1)
	if [[ "$worktree_path" != "/tmp/maestro-issue-42" ]]; then
		result=1
	fi
...
test_terminal_state_immutability() {
	...
	local status1
	status1=$(jq -r 'select(.session_key == "issue-77") | .status' "${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" 2>/dev/null | head -1)
	...
	local status2
	status2=$(jq -r 'select(.session_key == "issue-78") | .status' "${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" 2>/dev/null | head -1)
...
test_prune_old_entries() {
	...
	} >"${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl"

	local pruned_count
	pruned_count=$("$LEDGER_HELPER" prune)

	local remaining
	remaining=$(wc -l <"${MAESTRO_DISPATCH_LEDGER_DIR}/dispatch-ledger.jsonl" | tr -d ' ')
...
test_stale_lock_recovered_dead_pid() {
	...
	local ledger_dir="${MAESTRO_DISPATCH_LEDGER_DIR}"
