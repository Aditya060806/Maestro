...
# token-looking fixture while still reproducing an maestro helper filename with
...
if [[ "$rc" -eq 0 && -z "$out" && "$err" == *'[privacy-scan][ALLOW] maestro script file reference'* ]]; then
	pass "maestro script path reference is allowed with allowlist output"
else
	fail "maestro script path reference" "rc=$rc out=$out err=$err"
fi

precomputed_input="Worker guidance: edit .agents/scripts/$ALLOWED_BASENAME"
redacted=$(_privacy_redact_maestro_script_references "$precomputed_input" "$ALLOWED_BASENAME")
if [[ "$redacted" == *'[maestro-script-reference]'* ]]; then
	pass "precomputed maestro script basename allowlist is used"
else
	fail "precomputed maestro script basename allowlist" "redacted=$redacted"
fi

redacted=$(_privacy_redact_maestro_script_references "$precomputed_input" "")
if [[ "$redacted" == "$precomputed_input" ]]; then
	pass "empty precomputed maestro script basename allowlist skips fallback discovery"
else
	fail "empty precomputed maestro script basename allowlist" "redacted=$redacted"
fi
...
if [[ "$rc" -eq 0 && -z "$out" && "$err" == *'[privacy-scan][ALLOW] maestro script file reference'* ]]; then
	pass "backticked maestro script basename is allowed with allowlist output"
else
	fail "backticked maestro script basename" "rc=$rc out=$out err=$err"
fi
