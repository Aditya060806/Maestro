...
setup_test_env() {
	...
	mkdir -p "${HOME}/.maestro/logs" "${HOME}/.maestro/agents/scripts"
...
load_helpers_under_test() {
	local src="${MAESTRO_SOURCE:-$HOME/Git/maestro-bugfix-t2016-triage-cache-gate/.agents/scripts/pulse-ancillary-dispatch.sh}"
	if [[ ! -f "$src" ]]; then
