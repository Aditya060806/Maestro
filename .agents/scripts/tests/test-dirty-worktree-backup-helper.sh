...
test_backup_preserves_dirty_without_mutation() {
	...
	output=$(MAESTRO_DIRTY_BACKUP_ROOT="$backup_root" "$HELPER" backup --repo "$repo_dir" --reason test --task t1 2>/dev/null) || rc=1
	backup_dir="$output"
...
GH
	...
	PATH="$bin_dir:$PATH" MAESTRO_DIRTY_BACKUP_ROOT="$backup_root" "$HELPER" prune --force >/dev/null 2>&1 || rc=1
	[[ ! -d "$backup_dir" ]] || rc=1
