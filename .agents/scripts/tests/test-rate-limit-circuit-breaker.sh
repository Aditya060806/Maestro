...
#   4. Emergency bypass via MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER=1
...
reset_test_state() {
	: >"$LOGFILE"
	: >"$STATS_COUNTER_FILE"
	rm -f "${HOME}/.maestro/logs/pulse-graphql-circuit-breaker.state"
	rm -f "${HOME}/.maestro/cache/pulse-graphql-rate-limit.json"
	unset MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER 2>/dev/null || true
	unset MAESTRO_PULSE_RATE_LIMIT_CACHE_TTL 2>/dev/null || true
	unset MAESTRO_PULSE_DISPATCH_REST_FALLBACK 2>/dev/null || true
	unset MAESTRO_PULSE_DISPATCH_REST_FALLBACK_ACTIVE 2>/dev/null || true
	unset MAESTRO_GH_FORCE_REST_READS 2>/dev/null || true
	...
	STUB_GH_LIMIT=5000
	export MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD="0.05"
	return 0
}
...
test_emergency_bypass() {
	reset_test_state
	STUB_GH_REMAINING=0
	export MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER=1
	local rc=0
	is_graphql_budget_sufficient || rc=$?
	if [[ "$rc" -eq 0 ]]; then
...
test_disabled_at_zero_threshold() {
	reset_test_state
	STUB_GH_REMAINING=0
	export MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD="0"
	local rc=0
	is_graphql_budget_sufficient || rc=$?
	if [[ "$rc" -eq 0 ]]; then
...
test_custom_threshold_10_percent() {
	reset_test_state
	export MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD="0.10"
...
test_state_file_lifecycle() {
	reset_test_state
	local state_file="${HOME}/.maestro/logs/pulse-graphql-circuit-breaker.state"
...
test_graphql_exhausted_rest_core_healthy_allows_dispatch() {
	...
	if [[ "$rc" -eq 0 && "${MAESTRO_GH_FORCE_REST_READS:-0}" == "1" && "${MAESTRO_PULSE_DISPATCH_REST_FALLBACK_ACTIVE:-0}" == "1" && "$fallback_count" -eq 1 ]]; then
		pass "GraphQL exhausted with healthy REST core enables dispatch_rest_fallback"
	else
		fail "REST fallback should allow dispatch" "rc=$rc force_rest=${MAESTRO_GH_FORCE_REST_READS:-unset} active=${MAESTRO_PULSE_DISPATCH_REST_FALLBACK_ACTIVE:-unset} fallback_count=$fallback_count"
	fi
	return 0
}
...
test_graphql_exhausted_rest_core_low_blocks_dispatch() {
	...
	local rc=0
	is_graphql_budget_sufficient || rc=$?
	if [[ "$rc" -eq 1 && "${MAESTRO_GH_FORCE_REST_READS:-0}" != "1" ]]; then
		pass "GraphQL exhausted with low REST core still blocks dispatch"
	else
		fail "low REST core should not allow fallback" "rc=$rc force_rest=${MAESTRO_GH_FORCE_REST_READS:-unset}"
	fi
	return 0
}
...
test_graphql_exhausted_rest_fallback_disabled_blocks_dispatch() {
	...
	STUB_GH_REMAINING=0
	STUB_GH_CORE_REMAINING=4994
	export MAESTRO_PULSE_DISPATCH_REST_FALLBACK=0
	local rc=0
	is_graphql_budget_sufficient || rc=$?
	if [[ "$rc" -eq 1 && "${MAESTRO_PULSE_DISPATCH_REST_FALLBACK_ACTIVE:-0}" != "1" && "${MAESTRO_GH_FORCE_REST_READS:-0}" != "1" ]]; then
		pass "GraphQL exhausted blocks dispatch when REST fallback is disabled"
	else
		fail "disabled REST fallback should not allow dispatch" "rc=$rc active=${MAESTRO_PULSE_DISPATCH_REST_FALLBACK_ACTIVE:-unset} force_rest=${MAESTRO_GH_FORCE_REST_READS:-unset}"
	fi
	return 0
}
...
if [[ -z "$_NW_FUNC_DEF" ]]; then
	...
	mkdir -p "${NW_HOME}/.maestro/logs"

	NW_STATE_FILE="${NW_HOME}/.maestro/logs/pulse-no-work-breaker.state"
	...
	reset_nw_state() {
		...
		rm -f "$NW_STATE_FILE"
		: >"$NW_STATS_FILE"
		unset MAESTRO_SKIP_NO_WORK_BREAKER 2>/dev/null || true
		export HOME="$NW_HOME"
		export LOGFILE="$NW_LOGFILE"
		export NO_WORK_WINDOW_SECS=600
		export NO_WORK_WINDOW_MAX=10
		unset MAESTRO_NO_WORK_WINDOW_SECS 2>/dev/null || true
		unset MAESTRO_NO_WORK_WINDOW_MAX 2>/dev/null || true
		return 0
	}
	...
	test_nw_emergency_bypass() {
		...
		export MAESTRO_SKIP_NO_WORK_BREAKER=1
		local rc=0
		is_no_work_rate_acceptable || rc=$?
		if [[ "$rc" -eq 0 ]]; then
	...
	test_nw_custom_max_env_var() {
		reset_nw_state
		export MAESTRO_NO_WORK_WINDOW_MAX=5
		write_nw_log_lines 5
		local rc=0
		is_no_work_rate_acceptable || rc=$?
		if [[ "$rc" -eq 1 ]]; then
			pass "no_work breaker: MAESTRO_NO_WORK_WINDOW_MAX=5 trips at 5 events"
		else
			fail "no_work breaker: should trip at MAESTRO_NO_WORK_WINDOW_MAX=5 (rc=$rc)"
		fi
		return 0
	}
	...
	rm -f "${HOME}/.maestro/logs/pulse-graphql-circuit-breaker.state" 2>/dev/null || true
	# Restore GraphQL threshold to test default (0.05).
	export MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD="0.05"
	unset MAESTRO_SKIP_NO_WORK_BREAKER NO_WORK_WINDOW_SECS NO_WORK_WINDOW_MAX 2>/dev/null || true
