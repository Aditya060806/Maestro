...
#
# Regression tests for the first-class `maestro launch-worker` command.
...
test_help_lists_launch_worker() {
	local out rc=0
	out=$(bash "$MAESTRO_SH" help 2>&1) || rc=$?
	local failed=1
...
test_launch_worker_help() {
	local out rc=0
	out=$(bash "$MAESTRO_SH" launch-worker --help 2>&1) || rc=$?
	local failed=1
	[[ "$rc" -eq 0 && "$out" == *"Usage: maestro launch-worker"* && "$out" == *"--batch"* && "$out" == *"defaults to the current git repository"* ]] && failed=0
...
test_launch_worker_invalid_batch_value() {
	local out rc=0
	out=$(bash "$MAESTRO_SH" launch-worker --batch --dry-run Aditya060806/Maestro 2>&1) || rc=$?
	local failed=1
