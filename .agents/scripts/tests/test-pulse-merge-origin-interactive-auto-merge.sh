...
GHEOF
	...
	unset MAESTRO_INTERACTIVE_PR_AUTO_MERGE
...
teardown_test_env() {
	...
	unset MAESTRO_INTERACTIVE_PR_AUTO_MERGE REPOS_JSON
	unset -f config_get 2>/dev/null || true
	return 0
}
...
test_case7_env_override_passes() {
	...
	local labels="origin:interactive"
	local result=0
	MAESTRO_INTERACTIVE_PR_AUTO_MERGE=1 _check_interactive_pr_gates "105" "owner/repo" "$labels" "false" || result=$?
...
test_case10_env_false_blocks_preferences() {
	...
	local labels="origin:interactive"
	local result=0
	MAESTRO_INTERACTIVE_PR_AUTO_MERGE=0 _check_interactive_pr_gates "108" "owner/repo" "$labels" "false" && result=0 || result=$?
...
test_case11_label_overrides_env_false() {
	...
	local labels="origin:interactive,allow-auto-merge"
	local result=0
	MAESTRO_INTERACTIVE_PR_AUTO_MERGE=0 _check_interactive_pr_gates "109" "owner/repo" "$labels" "false" || result=$?
