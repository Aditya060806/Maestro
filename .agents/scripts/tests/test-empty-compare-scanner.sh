...
#   A2 allowlist: MAESTRO_EMPTY_COMPARE_SKIP=1 suppresses all
...
assert_violations() {
	...
	_actual=$(MAESTRO_EMPTY_COMPARE_SKIP="" bash "$SCANNER" scan "$_dir" 2>/dev/null | wc -l | tr -d ' ')
	[ -z "$_actual" ] && _actual=0
	# empty output from a zero-violation scan → wc gives 0
	_actual_violations=$(MAESTRO_EMPTY_COMPARE_SKIP="" bash "$SCANNER" scan "$_dir" 2>/dev/null | grep -c '	' || true)
...
assert_var_flagged() {
	...
	_output=$(MAESTRO_EMPTY_COMPARE_SKIP="" bash "$SCANNER" scan "$_dir" 2>/dev/null || true)
...
# ---------------------------------------------------------------------------
# A2: MAESTRO_EMPTY_COMPARE_SKIP=1 bypasses all detection
# ---------------------------------------------------------------------------
# Use same dir as P1 (which has a violation)
A2_violations=$(MAESTRO_EMPTY_COMPARE_SKIP=1 bash "$SCANNER" scan "$P1_DIR" 2>/dev/null | grep -c '	' || true)
if [ "$A2_violations" -eq 0 ]; then
	printf '[PASS] A2: MAESTRO_EMPTY_COMPARE_SKIP=1 bypasses scan\n'
	PASS=$((PASS + 1))
else
	printf '[FAIL] A2: MAESTRO_EMPTY_COMPARE_SKIP=1 still emitted %d violations\n' "$A2_violations" >&2
	FAIL=$((FAIL + 1))
fi
...
B1_guarded_count=$(MAESTRO_EMPTY_COMPARE_SKIP="" bash "$SCANNER" scan "$B1_DIR" 2>/dev/null | grep -c "b1_sibling_with_guard" || true)
