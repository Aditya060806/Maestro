...
# MAESTRO_SESSION_ORIGIN contract as the GitHub wrappers. This prevents a
...
unset FULL_LOOP_HEADLESS MAESTRO_HEADLESS OPENCODE_HEADLESS GITHUB_ACTIONS OPENCODE OPENCODE_SESSION_ID
export MAESTRO_SESSION_ORIGIN=worker
detected="$(_detect_explicit_session_type)"
if [[ "$detected" == "worker" ]]; then
	print_result "MAESTRO_SESSION_ORIGIN=worker -> worker" 0
else
	print_result "MAESTRO_SESSION_ORIGIN=worker -> worker" 1 "got '${detected}'"
fi
...
if [[ "$detected" == "interactive" ]]; then
	print_result "MAESTRO_SESSION_ORIGIN=interactive overrides headless marker" 0
else
	print_result "MAESTRO_SESSION_ORIGIN=interactive overrides headless marker" 1 "got '${detected}'"
fi

unset MAESTRO_SESSION_ORIGIN MAESTRO_HEADLESS
