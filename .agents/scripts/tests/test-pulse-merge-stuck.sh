...
# Isolate state writes to a temp file so the live ~/.maestro/logs/pulse-stats.json
...
for entry in \
	MAESTRO_MERGE_STUCK_AGE_MINUTES \
	MAESTRO_MERGE_ZERO_PROGRESS_CYCLES \
	MAESTRO_MERGE_PATTERN_MIN_PRS \
	MAESTRO_MERGE_STUCK_ENABLED; do
	TESTS_RUN=$((TESTS_RUN + 1))
...
assert_match "2a: MAESTRO_MERGE_STUCK_AGE_MINUTES is positive int" \
	"^[0-9]+$" "${MAESTRO_MERGE_STUCK_AGE_MINUTES:-x}"
assert_match "2b: MAESTRO_MERGE_ZERO_PROGRESS_CYCLES is positive int" \
	"^[0-9]+$" "${MAESTRO_MERGE_ZERO_PROGRESS_CYCLES:-x}"
assert_match "2c: MAESTRO_MERGE_PATTERN_MIN_PRS is positive int" \
	"^[0-9]+$" "${MAESTRO_MERGE_PATTERN_MIN_PRS:-x}"
assert_match "2d: MAESTRO_MERGE_STUCK_ENABLED is 0|1" \
	"^[01]$" "${MAESTRO_MERGE_STUCK_ENABLED:-x}"

assert_gt "2e: STUCK_AGE_MINUTES > 0" "$MAESTRO_MERGE_STUCK_AGE_MINUTES" "0"
assert_gt "2f: ZERO_PROGRESS_CYCLES > 0" "$MAESTRO_MERGE_ZERO_PROGRESS_CYCLES" "0"
assert_gt "2g: PATTERN_MIN_PRS > 1" "$MAESTRO_MERGE_PATTERN_MIN_PRS" "1"
echo ""
...
repo_allows_pulse_write_actions() {
	local repo_slug="$1"
	[[ "$repo_slug" == "Aditya060806/Maestro" && "${PMS_TEST_ALLOW_WRITE:-1}" == "1" ]]
	return $?
}
...
MAESTRO_MERGE_ZERO_PROGRESS_RECOVERY_CHECK_SECONDS=0
: >"$GH_CALLS"
pulse_stats_set_gauge "pulse_merge_zero_progress_cycles" "0" >/dev/null 2>&1
pulse_merge_zero_progress_record 0 0 >/dev/null 2>&1
if grep -q 'gh issue close 23036 --repo Aditya060806/Maestro --reason completed' "$GH_CALLS"; then
...
if grep -q 'gh issue close 23038 --repo Aditya060806/Maestro --reason completed' "$GH_CALLS"; then
...
if grep -q 'gh issue close 23035 --repo Aditya060806/Maestro --reason completed' "$GH_CALLS"; then
...
pulse_merge_zero_progress_record 1 0 >/dev/null 2>&1
create_count=$(grep -c 'gh_create_issue --repo Aditya060806/Maestro' "$GH_CALLS")
...
: >"$GH_CALLS"
MAESTRO_MERGE_ZERO_PROGRESS_CYCLES="not-a-number"
pulse_stats_set_gauge "pulse_merge_zero_progress_cycles" "4" >/dev/null 2>&1
pulse_stats_set_gauge "pulse_merge_eligible_stuck_pr_count" "0" >/dev/null 2>&1
pulse_merge_zero_progress_record 1 0 >/dev/null 2>&1
got=$(pulse_stats_get_gauge "pulse_merge_zero_progress_cycles")
...
MAESTRO_MERGE_STUCK_AGE_MINUTES=1
MAESTRO_MERGE_PATTERN_MIN_PRS=99
...
MAESTRO_MERGE_PATTERN_MIN_PRS=2
