...
printf '%s\n' \
	'81234 bash /Users/u/.maestro/agents/scripts/headless-runtime-helper.sh run --role worker /full-loop Implement issue #1 --model anthropic/claude-sonnet-4-6' \
	...
	'70003 bash /home/u/.maestro/agents/scripts/worker-watchdog.sh --check' \
...
test_worker_watchdog_script_dir_retry_present() {
	...
	if grep -qF '_resolve_script_dir_with_retry' "$watchdog_script" && \
		grep -qF 'MAESTRO_SCRIPT_DIR_ATTEMPTS' "$watchdog_script" && \
		grep -qF "\${HOME}/.maestro/agents/scripts" "$watchdog_script"; then
