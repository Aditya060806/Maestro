...
#   paths — so the canonical repo at ~/Git/maestro was trashed alongside
...
export MAESTRO_REPOS_JSON="$FAKE_REPOS_JSON"
...
# Test 5 — MAESTRO_CANONICAL_EXTRA_PATHS extension works
EXTRA_PATH="${TMP}/extra-canonical"
mkdir -p "$EXTRA_PATH"
if MAESTRO_CANONICAL_EXTRA_PATHS="$EXTRA_PATH" is_registered_canonical "$EXTRA_PATH"; then
	pass "MAESTRO_CANONICAL_EXTRA_PATHS adds to canonical set"
else
	fail "MAESTRO_CANONICAL_EXTRA_PATHS adds to canonical set" \
