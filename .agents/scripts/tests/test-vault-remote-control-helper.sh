...
set +e
MAESTRO_VAULT_PASSPHRASE="wrong-passphrase" "$REMOTE_HELPER" send-unlock-grant --vault-dir "$controller_dir" --recipient "$target_public" --repo "$transport_repo" --target-device "$target_device" --reason "remote unlock" >/dev/null 2>"$tmp_root/passphrase.err"
passphrase_rc=$?
set -e
