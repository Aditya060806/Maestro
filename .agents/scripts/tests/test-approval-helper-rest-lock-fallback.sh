...
run_case "lock helper treats REST 204 as success" '
	...
	gh() {
		...
		if [[ "$arg1" == "api" && "$arg2" == "-X" && "$arg3" == "PUT" && "$arg4" == "/repos/Aditya060806/Maestro/issues/123/lock" ]]; then return 0; fi
		return 1
	}
	_approval_lock_issue 123 Aditya060806/Maestro
' 0
...
run_case "REST 204 lock fallback succeeds" '
	...
	gh() {
		...
		local arg3="${3:-}"
		local arg4="${4:-}"
		if [[ "$arg1" == "api" && "$arg2" == "user" ]]; then printf "Aditya060806"; return 0; fi
		if [[ "$arg1" == "issue" && "$arg2" == "lock" ]]; then return 1; fi
		if [[ "$arg1" == "api" && "$arg2" == "-X" && "$arg3" == "PUT" && "$arg4" == "/repos/Aditya060806/Maestro/issues/123/lock" ]]; then return 0; fi
		if [[ "$arg1" == "api" && "$arg2" == "/repos/Aditya060806/Maestro/issues/123" ]]; then
			printf "%s" "{\"labels\":[{\"name\":\"auto-dispatch\"}],\"assignees\":[{\"login\":\"Aditya060806\"}],\"locked\":true}"
			return 0
		fi
		return 1
	}
	_approval_apply_issue_lifecycle_updates 123 Aditya060806/Maestro
' 0
...
run_case "PR-backed issue lock falls back to REST without GraphQL fallback" '
	...
	gh() {
		...
		if [[ "$arg1" == "api" && "$arg2" == "user" ]]; then printf "Aditya060806"; return 0; fi
		if [[ "$arg1" == "issue" && "$arg2" == "lock" ]]; then return 1; fi
		if [[ "$arg1" == "api" && "$arg2" == "-X" && "$arg3" == "PUT" && "$arg4" == "/repos/Aditya060806/Maestro/issues/2417/lock" ]]; then return 0; fi
		if [[ "$arg1" == "api" && "$arg2" == "/repos/Aditya060806/Maestro/issues/2417" ]]; then
			printf "%s" "{\"labels\":[{\"name\":\"auto-dispatch\"}],\"assignees\":[{\"login\":\"Aditya060806\"}],\"locked\":true}"
			return 0
		fi
		return 1
	}
	_approval_apply_issue_lifecycle_updates 2417 Aditya060806/Maestro
' 0
...
run_case "PR approval locks conversation with gh pr lock" '
	...
	gh() {
		...
		if [[ "$arg1" == "api" && "$arg2" == "/repos/Aditya060806/Maestro/issues/456" ]]; then
			printf "%s" "{\"locked\":true}"
			return 0
		fi
		return 1
	}
	_post_issue_approval_updates pr 456 Aditya060806/Maestro
' 0
...
run_case "conversation lock verification reuses provided issue JSON" '
	...
	_approval_verify_conversation_locked pr 456 Aditya060806/Maestro "{\"locked\":true}"
' 0
...
run_case "PR approval REST fallback locks conversation" '
	...
	gh() {
		...
		if [[ "$arg1" == "api" && "$arg2" == "-X" && "$arg3" == "PUT" && "$arg4" == "/repos/Aditya060806/Maestro/issues/456/lock" ]]; then return 0; fi
		if [[ "$arg1" == "api" && "$arg2" == "/repos/Aditya060806/Maestro/issues/456" ]]; then
			printf "%s" "{\"locked\":true}"
			return 0
		fi
...
run_case "post-approval protection failure blocks final success" '
	...
	_approve_target issue 123 Aditya060806/Maestro
' 1
