...
#
# The helper supports a mock interface via MAESTRO_GH_STATUS_MOCK_DIR — when
...
# point MAESTRO_GH_STATUS_MOCK_DIR at a specific scenario.
#
...
_isolated_run() {
	...
	MAESTRO_GH_STATUS_MOCK_DIR="$MOCK_BASE/$fixture" \
		MAESTRO_GH_STATUS_CACHE_DIR="$cache_dir" \
		bash "$HELPER" "$@"
	local rc=$?
	rm -rf "$cache_dir"
	return "$rc"
...
test_correlate_emits_marker() {
	...
	if [[ "$out" != *"<!-- maestro:gh-status-correlation -->"* ]]; then
...
test_cache_writes_files() {
	...
	MAESTRO_GH_STATUS_MOCK_DIR="$MOCK_BASE/operational" \
		MAESTRO_GH_STATUS_CACHE_DIR="$cache_dir" \
...
test_network_failure_returns_3() {
	...
	out=$(MAESTRO_GH_STATUS_MOCK_DIR="$empty_dir" \
		MAESTRO_GH_STATUS_CACHE_DIR="$cache_dir" \
