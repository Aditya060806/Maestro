...
test_counter_branch_desync_reconciles_before_claim() {
	...
	if ! printf '%s\n' "$output" | grep -q 'MAESTRO_TASK_COUNTER_STATUS=recovered_contention'; then
...
test_reconciliation_protected_branch_rejection_is_unrecoverable() {
	...
	if ! printf '%s\n' "$output" | grep -q 'MAESTRO_TASK_COUNTER_STATUS=unrecoverable_desync detail=protected_counter_branch'; then
