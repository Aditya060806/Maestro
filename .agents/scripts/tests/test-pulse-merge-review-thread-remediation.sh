...
setup_test_env() {
	...
	export MAESTRO_REPOS_JSON="${TEST_ROOT}/config/repos.json"
	printf '{"initialized_repos":[{"slug":"owner/repo","path":"%s"}]}\n' "${TEST_ROOT}/repo" >"$MAESTRO_REPOS_JSON"
...
SCANNER
	...
	unset MAESTRO_CHANGES_REQUESTED_THREAD_REMEDIATION_FIRST
	return 0
}
...
test_changes_requested_opt_in_dispatches_remediation_without_routing() {
	setup_test_env
	export MAESTRO_CHANGES_REQUESTED_THREAD_REMEDIATION_FIRST=1
	define_helpers_under_test || { teardown_test_env; return 0; }
	local route_log="${TEST_ROOT}/route.log"
	: >"$route_log"
	_route_pr_to_fix_worker() {
