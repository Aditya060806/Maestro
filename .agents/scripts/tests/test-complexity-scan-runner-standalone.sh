...
if grep -qE '^maestro_ensure_pulse_start_epoch\(\) \{' "$SHARED_CONSTANTS_FILE"; then
	pass "3: shared PULSE_START_EPOCH bootstrap helper exists"
else
	fail "3: shared PULSE_START_EPOCH bootstrap helper exists" \
		"missing maestro_ensure_pulse_start_epoch definition"
fi

if grep -qE '^maestro_ensure_pulse_start_epoch$' "$RUNNER_FILE"; then
