...
#   (9) pulse-watchdog-tick.sh: MAESTRO_PULSE_WATCHDOG_DISABLE=1 → no-op exit 0
#
...
_render_watchdog_plist() {
	...
	bash -c '
		...
		print_warning() { :; }
		source "'"$SCHEDULERS_SH"'" 2>/dev/null || true
		_generate_pulse_watchdog_plist_content "sh.maestro.pulse-watchdog" "/path/to/tick.sh" "/opt/homebrew/bin/bash"
	' >"$out_file"
}
...
test_watchdog_plist_label() {
	...
	local rc=0
	if ! grep -q "<string>sh.maestro.pulse-watchdog</string>" "$plist"; then
		rc=1
	fi
	print_result "test_watchdog_plist_label" "$rc" \
		"Watchdog label must be sh.maestro.pulse-watchdog (t2939 layer 2)"
	return 0
}
...
_setup_tick_env() {
	...
	mkdir -p "$stub_home/.maestro/logs" "$stub_home/.config/maestro" \
		"$stub_home/.maestro/agents/scripts"
	echo '{"supervisor":{"pulse_interval_seconds":600}}' >"$stub_home/.config/maestro/settings.json"

	# Stub lifecycle helper. Records every invocation.
	cat >"$stub_home/.maestro/agents/scripts/pulse-lifecycle-helper.sh" <<STUB
#!/usr/bin/env bash
echo "stub:\$*" >>"$stub_home/.maestro/logs/stub-helper-invocations.log"
case "\$1" in
...
STUB
	chmod +x "$stub_home/.maestro/agents/scripts/pulse-lifecycle-helper.sh"
	return 0
}
...
test_tick_alive_fast_exit() {
	...
	local rc=0
	HOME="$stub_home" MAESTRO_AGENTS_DIR="$stub_home/.maestro/agents" \
		bash "$TICK_SH" || rc=$?
	# Must exit 0 with no revival attempt.
	local invocations=""
	if [[ -f "$stub_home/.maestro/logs/stub-helper-invocations.log" ]]; then
		invocations=$(cat "$stub_home/.maestro/logs/stub-helper-invocations.log")
	fi
...
test_tick_dead_within_grace_no_revive() {
	...
	date +%s >"$stub_home/.maestro/logs/pulse-wrapper-last-run.ts"
	local rc=0
	HOME="$stub_home" MAESTRO_AGENTS_DIR="$stub_home/.maestro/agents" \
		bash "$TICK_SH" || rc=$?
	local invocations
	invocations=$(cat "$stub_home/.maestro/logs/stub-helper-invocations.log" 2>/dev/null || echo "")
...
test_tick_dead_past_grace_revives() {
	...
	echo "$(($(date +%s) - 1000))" >"$stub_home/.maestro/logs/pulse-wrapper-last-run.ts"
	local rc=0
	HOME="$stub_home" MAESTRO_AGENTS_DIR="$stub_home/.maestro/agents" \
		bash "$TICK_SH" || rc=$?
	local invocations
...
test_tick_disable_flag() {
	...
	_setup_tick_env "$stub_home" "dead"
	echo "0" >"$stub_home/.maestro/logs/pulse-wrapper-last-run.ts"
	local rc=0
	HOME="$stub_home" MAESTRO_AGENTS_DIR="$stub_home/.maestro/agents" \
		MAESTRO_PULSE_WATCHDOG_DISABLE=1 \
		bash "$TICK_SH" || rc=$?
	local invocations
	invocations=$(cat "$stub_home/.maestro/logs/stub-helper-invocations.log" 2>/dev/null || echo "")
