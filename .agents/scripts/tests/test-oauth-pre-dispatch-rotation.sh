...
make_fixture() {
	...
	#
	# $1 = cooldownUntil_ms for Aditya060806@mac.com in the shared pool
...
{
  "anthropic": [
    {"email": "Aditya060806@mac.com", "access": "PROD-rotated-access-token-abc123", "cooldownUntil": ${cooldown_ms}, "status": "rate_limited", "priority": 10},
...
export MAESTRO_OAUTH_POOL_FILE="$fixture/pool.json"
...
if [[ "$rc" -eq 0 && "$final_email" == "Aditya060806@mac.com" && "$stub_calls" -eq 0 ]]; then
...
export OAUTH_POOL_HELPER="$fixture/bin/oauth-pool-helper.sh"
export MAESTRO_OAUTH_POOL_FILE="/nonexistent/pool.json"
export STUB_LOG="$fixture/bin/invocation.log"
