...
setup() {
	...
	mkdir -p "$HOME/.maestro/logs" "$HOME/.maestro/cache"
	return 0
}
...
test_runtime_metric_accepts_structured_evidence() {
	...
	local metrics_dir="$HOME/.maestro/logs"
	local state_dir="$HOME/.maestro/state"
...
JSONL
	local summary_json
	summary_json=$(WAH_METRICS_FILE="$metrics_file" WAH_PULSE_STATS_FILE="$HOME/.maestro/logs/pulse-stats.json" \
