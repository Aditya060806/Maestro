...
TEST_ROOT="$(mktemp -d 2>/dev/null || mktemp -d -t maestro-vault-setup-test)"
PASS=0
FAIL=0

cleanup() {
	MAESTRO_VAULT_DIR="$TEST_ROOT/vault" MAESTRO_VAULT_RUNTIME_DIR="$TEST_ROOT/run" "$VAULT_HELPER" lock >/dev/null 2>&1 || true
	rm -rf "$TEST_ROOT"
	return 0
}
trap cleanup EXIT
...
archive_count=0
archive_dir=""
for candidate in "$MAESTRO_VAULT_DIR"/archives/lost-passphrase-*; do
	if [[ -d "$candidate" ]]; then
