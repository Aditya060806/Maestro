...
#   3. Custom MAESTRO_CLEANUP_LOG env var is honoured
...
#
# All tests run in isolated temp directories; no real ~/.maestro state is
...
test_log_writes_one_line() {
	...
	export MAESTRO_CLEANUP_LOG="$log_file"
...
# =============================================================================
# Test 3: MAESTRO_CLEANUP_LOG env var is honoured (custom log path)
# =============================================================================
test_custom_log_path() {
	local custom_log="${TEST_DIR}/custom/subdir/audit.log"
	export MAESTRO_CLEANUP_LOG="$custom_log"
	...
	print_result "custom_log_path_honoured" "$rc" "Custom MAESTRO_CLEANUP_LOG path not written"
	return 0
}
