...
reset_stagger_env() {
	...
	unset MAESTRO_PULSE_DISPATCH_RAMP_ENABLED MAESTRO_PULSE_DISPATCH_RAMP_NOW MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH MAESTRO_PULSE_DISPATCH_RAMP_PHASE MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS MAESTRO_PULSE_DISPATCH_RAMP_BOOT_SECS MAESTRO_PULSE_DISPATCH_RAMP_RECOVERY_SECS MAESTRO_GH_READ_RAMP_BOOT_SECS MAESTRO_GH_READ_RAMP_RECOVERY_SECS
	return 0
}
...
test_dispatch_ramp_limits_initial_capacity() {
	reset_stagger_env
	export MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH=1000
	export MAESTRO_PULSE_DISPATCH_RAMP_NOW=1000
	export MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS=120
	local capped
...
test_dispatch_ramp_adds_one_slot_per_pulse_interval() {
	reset_stagger_env
	export MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH=1000
	export MAESTRO_PULSE_DISPATCH_RAMP_NOW=1240
	export MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS=120
	local capped
	capped=$(_dispatch_apply_startup_capacity_ramp 8 0)
	if [[ "$capped" == "3" ]]; then
...
test_dispatch_ramp_never_exceeds_max_capacity() {
	reset_stagger_env
	export MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH=1000
	export MAESTRO_PULSE_DISPATCH_RAMP_NOW=4000
	export MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS=120
	local capped
...
test_dispatch_ramp_can_be_disabled() {
	reset_stagger_env
	export MAESTRO_PULSE_DISPATCH_RAMP_ENABLED=0
	export MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH=1000
	export MAESTRO_PULSE_DISPATCH_RAMP_NOW=1000
	local capped
	capped=$(_dispatch_apply_startup_capacity_ramp 8 0)
...
test_dispatch_ramp_uses_precomputed_boot_timestamp() {
	reset_stagger_env
	export MAESTRO_PULSE_DISPATCH_RAMP_NOW=1100
	export MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS=120
	local capped
...
test_dispatch_ramp_uses_precomputed_cooldown_expiry() {
	reset_stagger_env
	export MAESTRO_PULSE_DISPATCH_RAMP_BOOT_SECS=0
	export MAESTRO_PULSE_DISPATCH_RAMP_NOW=1120
	export MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS=120
	local capped
	capped=$(_dispatch_apply_startup_capacity_ramp 8 0 "" 1000)
	if [[ "$capped" == "2" ]]; then
