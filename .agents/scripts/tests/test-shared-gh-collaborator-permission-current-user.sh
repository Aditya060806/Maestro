...
test_current_user_null_maps_to_lookup_failure() {
	...
	[[ "${MAESTRO_GH_WRITE_PERMISSION_USER-}" == "" ]] || fail "expected empty current user, got '${MAESTRO_GH_WRITE_PERMISSION_USER}'"
	[[ "${MAESTRO_GH_WRITE_PERMISSION_REASON:-}" == "current-user-lookup-failed" ]] || fail "expected current-user-lookup-failed, got '${MAESTRO_GH_WRITE_PERMISSION_REASON:-}'"
