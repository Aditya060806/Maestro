...
#   maestro/architecture.md          — declares bash: false
...
build_fixture_agents_dir() {
	local dir="$1"
	mkdir -p "$dir/maestro" "$dir/mermaid-diagrams-skill" "$dir/a" "$dir/b" "$dir/c"

	cat >"$dir/maestro/architecture.md" <<'EOF'
---
...
# 1. architecture collision: sandboxed maestro/ wins over permissive skill/.
if echo "$winners" | grep -qE "/maestro/architecture\\.md$" &&
