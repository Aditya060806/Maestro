...
JSONEOF
	...
	rm -rf "${TMPDIR:-/tmp}/maestro-pulse-allow-auto-merge-$$" 2>/dev/null || true
...
test_case_5_env_threshold_override() {
	...
	local result=0
	MAESTRO_PULSE_AUTO_MERGE_STUCK_SECONDS=7200 \
	...
	print_result "Case (5): MAESTRO_PULSE_AUTO_MERGE_STUCK_SECONDS override defers when raised" 0
	teardown_test_env
	return 0
}
