...
test_time_step_logs_running_before_command() {
	...
	output=$(
		...
		slow_stage() {
			local marker_file="$1"
			local timing_file="${HOME}/.maestro/logs/setup-stage-timings.log"
		...
		printf 'finished=%s\n' "$(grep -c $'update_claude_config\t.*\t0' "${tmp_home}/.maestro/logs/setup-stage-timings.log" 2>/dev/null || true)"
	) 2>&1
