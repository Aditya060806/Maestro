...
export MAESTRO_REACH_PERFORMANCE_LOG="${temp_dir}/reach-capture.jsonl"
export MAESTRO_SESSION_ID="reach-feedback-test-session-a"
...
if [[ -s "$MAESTRO_REACH_PERFORMANCE_LOG" ]]; then
	PASS=$((PASS + 1))
...
log_text="$(<"$MAESTRO_REACH_PERFORMANCE_LOG")"
...
for session_id in session-a session-b session-c; do
	python3 - "$MAESTRO_REACH_PERFORMANCE_LOG" "$now_iso" "$session_id" <<'PY'
import json
import sys
