...
_setup_fake_home() {
	local _tmpdir="$1"
	mkdir -p "$_tmpdir/.config/maestro"
	mkdir -p "$_tmpdir/.maestro/agents/templates/workflows"
	# Copy the canonical caller template so the helper can resolve it.
	cp "$SCRIPT_DIR/../../templates/workflows/issue-sync-caller.yml" \
		"$_tmpdir/.maestro/agents/templates/workflows/issue-sync-caller.yml"
	return 0
}
...
_write_repos_json() {
	local _tmpdir="$1"
	local _content="$2"
	printf '%s\n' "$_content" >"$_tmpdir/.config/maestro/repos.json"
	return 0
}
...
rm -f "$TMPDIR_6/.config/maestro/repos.json" 2>/dev/null
...
# ─── Test 7: maestro self never appears in sync output ─────────────────────
TMPDIR_7="$(mktemp -d)"
_setup_fake_home "$TMPDIR_7"
mkdir -p "$TMPDIR_7/maestro-repo/.github/workflows"
...
jobs:
  sync:
    ...
    secrets: inherit'
printf '%s\n' "$SELF_CALLER" >"$TMPDIR_7/maestro-repo/.github/workflows/issue-sync.yml"
...
_write_repos_json "$TMPDIR_7" "{\"initialized_repos\":[{\"path\":\"$TMPDIR_7/maestro-repo\",\"slug\":\"Aditya060806/Maestro\"},{\"path\":\"$TMPDIR_7/legacy-repo\",\"slug\":\"owner/legacy-repo\"}]}"
OUT_7=$(HOME="$TMPDIR_7" bash "$HELPER" --json 2>/dev/null)
if ! grep -qF 'Aditya060806/Maestro' <<<"$OUT_7"; then
	printf '%sPASS%s maestro self never targeted by sync\n' "$GREEN" "$NC"
	((_PASS++))
else
	printf '%sFAIL%s maestro self leaked into sync output\n' "$RED" "$NC"
	((_FAIL++))
fi
