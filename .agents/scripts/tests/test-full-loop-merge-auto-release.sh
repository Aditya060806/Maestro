...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8801.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "50" "Aditya060806/Maestro" "8801"

if grep -q "release 8801 Aditya060806/Maestro" "$RELEASE_CALLS" 2>/dev/null; then
	...
	fail "happy path: release called for origin:interactive PR with stamp" \
		"expected 'release 8801 Aditya060806/Maestro' in release calls — got: $(cat "$RELEASE_CALLS" 2>/dev/null || printf '(empty)')"
fi
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8802.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_LABELS_RESPONSE="origin:worker,tier:standard"

release_interactive_claim_on_merge "51" "Aditya060806/Maestro" "8802"
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8803.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_BODY_RESPONSE=""
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "52" "Aditya060806/Maestro" ""
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
MISSING_STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8804.json"
rm -f "$MISSING_STAMP"
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "53" "Aditya060806/Maestro" "8804"
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8805.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "54" "Aditya060806/Maestro" "8805"
release_interactive_claim_on_merge "54" "Aditya060806/Maestro" "8805"
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8806.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

_release_interactive_claim_on_merge "55" "Aditya060806/Maestro" "8806"

if grep -q "release 8806 Aditya060806/Maestro" "$RELEASE_CALLS" 2>/dev/null; then
	pass "backward compat: underscore-prefixed alias fires release"
else
	fail "backward compat: underscore-prefixed alias fires release" \
		"expected 'release 8806 Aditya060806/Maestro' in release calls — got: $(cat "$RELEASE_CALLS" 2>/dev/null || printf '(empty)')"
fi
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8807.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_BODY_RESPONSE="Planning pass for t2809. Ref #8807 — keeps issue open until all phases merge."
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "60" "Aditya060806/Maestro" ""

if grep -q "release 8807 Aditya060806/Maestro" "$RELEASE_CALLS" 2>/dev/null; then
	...
	fail "Ref keyword fallback: release called for empty linked_issue with Ref #NNN in body" \
		"expected 'release 8807 Aditya060806/Maestro' in release calls — got: $(cat "$RELEASE_CALLS" 2>/dev/null || printf '(empty)')"
fi
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8808.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_BODY_RESPONSE="For #8808 — planning-only PR per parent-task keyword rule."
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "61" "Aditya060806/Maestro" ""

if grep -q "release 8808 Aditya060806/Maestro" "$RELEASE_CALLS" 2>/dev/null; then
	...
	fail "For keyword fallback: release called for empty linked_issue with For #NNN in body" \
		"expected 'release 8808 Aditya060806/Maestro' in release calls — got: $(cat "$RELEASE_CALLS" 2>/dev/null || printf '(empty)')"
fi
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8809.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_BODY_RESPONSE="This PR has no issue reference in the body at all."
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "62" "Aditya060806/Maestro" ""
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8810.json"
printf '{"pid":1234}\n' >"$STAMP"
...
release_interactive_claim_on_merge "63" "Aditya060806/Maestro" "" "origin:worker,tier:standard"
...
: >"$RELEASE_CALLS"
: >"$LOGFILE"
STAMP="${CLAIM_STAMP_DIR}/Aditya060806-maestro-8811.json"
printf '{"pid":1234}\n' >"$STAMP"
GH_BODY_RESPONSE="Update the prefix #8811 to include the new format."
GH_LABELS_RESPONSE="origin:interactive,tier:standard"

release_interactive_claim_on_merge "65" "Aditya060806/Maestro" ""
