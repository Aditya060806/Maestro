...
42' && echo 'oimp-nonparent:1' || echo 'oimp-nonparent:0'
		...
		# _should_lia: true for tNNN: title + no maestro labels
...
test_single_pass_wired_in_engine() {
	...
	legacy_calls=$(grep -E 'run_stage_with_timeout.*(close_issues_with_merged_prs|reconcile_stale_done_issues|reconcile_open_issues_with_merged_prs|reconcile_completed_parent_tasks|reconcile_labelless_maestro_issues)' \
