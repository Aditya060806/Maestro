...
_setup_fake_home() {
	local _root="$1"
	# Directory layout mirrors the real maestro install so the helper's
	# `_resolve_canonical_template` picks up the deployed copy.
	mkdir -p "$_root/.config/maestro"
	mkdir -p "$_root/.maestro/agents/templates/workflows"
	cp "$CANONICAL_TEMPLATE" "$_root/.maestro/agents/templates/workflows/issue-sync-caller.yml"
	return 0
}
...
_write_repos_json() {
	local _root="$1"
	local _json="$2"
	printf '%s\n' "$_json" > "$_root/.config/maestro/repos.json"
	return 0
}
...
jobs:
  sync:
    ...
    secrets: inherit
'
_make_repo_with_workflow "$TMPDIR_2/repos/maestro-self" "$SELF_CALLER_YAML"
_write_repos_json "$TMPDIR_2" \
	"$(jq -n --arg path "$TMPDIR_2/repos/maestro-self" '{initialized_repos: [{slug: "x/self", path: $path, local_only: false}]}')"
...
TMPDIR_9="$(mktemp -d)"
mkdir -p "$TMPDIR_9/.config/maestro"
mkdir -p "$TMPDIR_9/.maestro/agents/templates/workflows"
cp "$CANONICAL_TEMPLATE" "$TMPDIR_9/.maestro/agents/templates/workflows/issue-sync-caller.yml"
...
sed \
	-e 's|Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml@main|ORG/.github/.github/workflows/issue-sync-reusable.yml@1234567890abcdef1234567890abcdef12345678|g' \
	-e 's|Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml|ORG/.github/.github/workflows/issue-sync-reusable.yml|g' \
...
sed 's|Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml@main|OTHER/.github/.github/workflows/issue-sync-reusable.yml@main|g' \
