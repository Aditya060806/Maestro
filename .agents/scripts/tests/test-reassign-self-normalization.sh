...
test_available_conflict_feedback_stale_interactive_claim_normalized() {
	setup_test_env
	local json='[
		{"number": 202, "assignees": [{"login": "Aditya060806"}], "labels": [{"name": "status:available"}, {"name": "origin:interactive"}, {"name": "source:conflict-feedback"}]}
	]'
	create_gh_stub "$json"
	export PATH="${TEST_ROOT}/bin:${PATH}"
	source_function_under_test
	...
	if [[ "$calls" != *"--add-label origin:worker"* || "$calls" != *"--remove-label origin:interactive"* || "$calls" != *"--remove-assignee Aditya060806"* ]]; then
		print_result "status:available + conflict-feedback + stale interactive claim → ownership normalized" 1 \
...
test_stale_auto_approved_brief_rewrite_requeues() {
	setup_test_env
	local json='[
		{"number": 4246, "assignees": [{"login": "Aditya060806"}], "labels": [{"name": "auto-dispatch"}, {"name": "origin:worker"}, {"name": "tier:standard"}, {"name": "ai-approved"}, {"name": "needs-brief-rewrite"}]},
		{"number": 4248, "assignees": [{"login": "Aditya060806"}], "labels": [{"name": "auto-dispatch"}, {"name": "origin:worker"}, {"name": "tier:standard"}, {"name": "ai-approved"}, {"name": "status:available"}, {"name": "needs-brief-rewrite"}]}
	]'
	create_gh_stub "$json"
	export PATH="${TEST_ROOT}/bin:${PATH}"
	source_function_under_test
	...
	if [[ "$calls" == *"--add-label status:available"* \
		...
		&& "$calls" == *"--remove-assignee Aditya060806"* \
