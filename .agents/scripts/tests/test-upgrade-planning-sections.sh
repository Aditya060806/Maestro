...
#   - Extract the target functions from maestro.sh via sed
...
MAESTRO_SH="$SCRIPT_DIR/../../../maestro.sh"
...
# Stub the maestro.sh globals the target functions depend on. The test runs
...
# Extract target function bodies from maestro.sh and eval them into this shell.
_extract_function() {
	local name="$1"
	sed -n "/^${name}() {/,/^}/p" "$MAESTRO_SH"
	return 0
}
...
for fn in _extract_todo_section _filter_todo_placeholders _insert_after_toon_marker \
	...
	if [[ -z "$body" ]]; then
		echo "ERROR: could not extract $fn from maestro.sh" >&2
		exit 1
	fi
	eval "$body"
done
...
# matter separators maestro.sh expects (two --- lines).
write_new_template() {
	local path="$1"
	cat >"$path" <<'EOF'
---
