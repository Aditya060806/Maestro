...
_run_classifier_with_log() {
	...
	local log_content="$3"
	local log_file=""
	log_file=$(maestro_pulse_worker_log_path "$repo_slug" "$issue_number")
	# Clean any prior fixture
	rm -f "$log_file" "$(maestro_pulse_worker_log_fallback_path "$issue_number")" 2>/dev/null || true
...
RECLASS_CAPTURE="$(mktemp "${TMPDIR:-/tmp}/maestro-t2820-capture.XXXXXX")"
...
test_reclassify_real_coding_no_op() {
	: >"$RECLASS_CAPTURE"
	local log_file=""
	log_file=$(maestro_pulse_worker_log_path "test/repo-r1" "91001")
...
test_reclassify_no_tool_calls_fires() {
	: >"$RECLASS_CAPTURE"
	local log_file=""
	log_file=$(maestro_pulse_worker_log_path "test/repo-r2" "91002")
	printf 'session init\nwaiting for stream\nconnection refused\n' >"$log_file"
...
test_reclassify_canary_fires() {
	: >"$RECLASS_CAPTURE"
	local log_file=""
	log_file=$(maestro_pulse_worker_log_path "test/repo-r3" "91003")
...
test_reclassify_old_no_tool_calls_skipped() {
	: >"$RECLASS_CAPTURE"
	local log_file=""
	log_file=$(maestro_pulse_worker_log_path "test/repo-r4" "91004")
	printf 'session init\nwaiting\n' >"$log_file"
...
test_reclassify_no_log_falls_through() {
	...
	rm -f "$(maestro_pulse_worker_log_path "test/repo-r5" "91005")" "$(maestro_pulse_worker_log_fallback_path "91005")" 2>/dev/null || true
...
test_reclassify_skips_rate_limit() {
	: >"$RECLASS_CAPTURE"
	local log_file=""
	log_file=$(maestro_pulse_worker_log_path "test/repo-r6" "91006")
