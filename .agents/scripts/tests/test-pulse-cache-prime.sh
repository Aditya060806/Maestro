...
#
# Covers:
#   1. MAESTRO_SKIP_CACHE_PRIME=1 → exit 0, log records skip, no sentinel
...
SH
	...
	mkdir -p "${TEST_ROOT}/.maestro/logs" "${TEST_ROOT}/.maestro/cache"
	return 0
}
...
# Test 1: MAESTRO_SKIP_CACHE_PRIME=1 short-circuits.
test_skip_env_var() {
	local rc=0
	MAESTRO_SKIP_CACHE_PRIME=1 "${TEST_ROOT}/scripts/pulse-cache-prime.sh" >/dev/null 2>&1 || rc=$?
	_assert_eq "skip env var → exit 0" "0" "$rc"

	# Sentinel must NOT exist.
	if [[ ! -f "${TEST_ROOT}/.maestro/cache/pulse-cache-prime-last-run" ]]; then
	...
	if grep -q "MAESTRO_SKIP_CACHE_PRIME=1" "${TEST_ROOT}/.maestro/logs/pulse-cache-prime.log" 2>/dev/null; then
...
SH
	...
	rm -f "${TEST_ROOT}/.maestro/cache/pulse-cache-prime-last-run"
	...
	if grep -q "Cache prime succeeded" "${TEST_ROOT}/.maestro/logs/pulse-cache-prime.log" 2>/dev/null; then
...
test_failure() {
	...
	if grep -q "Cache prime FAILED" "${TEST_ROOT}/.maestro/logs/pulse-cache-prime.log" 2>/dev/null; then
