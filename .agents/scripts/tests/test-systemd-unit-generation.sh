...
generate_scheduler_unit() {
	...
	printf '%s' "[Unit]
Description=maestro test-service
...
generate_autoupdate_unit() {
	...
	local service_file="$2"
	printf '%s' "[Unit]
Description=maestro auto-update-test
After=network.target
...
generate_reposync_unit() {
	...
	local service_file="$2"
	printf '%s' "[Unit]
Description=maestro repo-sync-test
After=network.target
...
generate_supervisor_pulse_unit() {
	...
	local service_file="$2"
	printf '%s' "[Unit]
Description=maestro maestro-supervisor-pulse
After=network.target
...
test_autoupdate_unit_passes_verify() {
	...
	local log_file="/tmp/maestro-update.log"
...
test_reposync_unit_passes_verify() {
	...
	local log_file="/tmp/maestro-repo-sync.log"
...
test_supervisor_pulse_unit_uses_control_group_kill() {
	local tmpdir
	tmpdir=$(mktemp -d)
	local service_file="${tmpdir}/maestro-supervisor-pulse.service"
	local log_file="/tmp/maestro-supervisor-pulse.log"
