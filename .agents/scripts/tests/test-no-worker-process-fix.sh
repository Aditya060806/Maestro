...
test_claim_released_reads_log() {
	...
	if grep -q 'maestro_pulse_worker_log_candidates' "$shared_lifecycle" \
...
test_claim_released_includes_tail_behavioural() {
	local sandbox
	sandbox=$(mktemp -d "${TMPDIR:-/tmp}/maestro-t2814-test.XXXXXX")
...
test_worker_launch_disables_pulse_pr_caches() {
	if grep -q 'MAESTRO_GH_PR_LIST_CACHE_DISABLE=1' "$WORKER_LAUNCH" \
		&& ! grep -q '^[[:space:]]*MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1' "$WORKER_LAUNCH" \
...
test_overload_helper_behavioural() {
	...
	sandbox=$(mktemp -d "${TMPDIR:-/tmp}/maestro-t3210-test.XXXXXX")
	# shellcheck disable=SC2064
	trap "rm -rf '$sandbox' 2>/dev/null || true" RETURN
...
test_prelaunch_reason_parser_behavioural() {
	...
	sandbox=$(mktemp -d "${TMPDIR:-/tmp}/maestro-prelaunch-reason-test.XXXXXX")
	# shellcheck disable=SC2064
	trap "rm -rf '$sandbox' 2>/dev/null || true" RETURN
	helper_extract="${sandbox}/helper.sh"
...
test_prelaunch_reason_parser_preserves_explicit_reason() {
	local sandbox helper_extract log_file reason
	sandbox=$(mktemp -d "${TMPDIR:-/tmp}/maestro-prelaunch-reason-priority-test.XXXXXX")
	# shellcheck disable=SC2064
	trap "rm -rf '$sandbox' 2>/dev/null || true" RETURN
	helper_extract="${sandbox}/helper.sh"
	log_file="${sandbox}/worker.log"
