...
test_matches_stale_maestro_playwright_list() {
	local output
	if output=$(run_matcher 1200 '?' "$HOME/Git/maestro-feature-auto-123" pnpm --filter web exec playwright test --list --grep @flaky --reporter=list 2>&1); then
		if [[ "$output" == MATCH* ]]; then
			print_result "matches stale maestro Playwright list process" 0
		else
			print_result "matches stale maestro Playwright list process" 1 "Unexpected output: $output"
		fi
	else
		print_result "matches stale maestro Playwright list process" 1 "Matcher rejected: $output"
	fi
	return 0
}

test_matches_stale_maestro_playwright_list_with_flexible_spacing() {
	local output
	if output=$(run_matcher 1200 '?' "$HOME/Git/maestro-feature-auto-123" pnpm --filter web exec 'playwright   test   --list' --grep=@flaky --reporter=list 2>&1); then
		if [[ "$output" == MATCH* ]]; then
			print_result "matches stale maestro Playwright list with flexible spacing" 0
		else
			print_result "matches stale maestro Playwright list with flexible spacing" 1 "Unexpected output: $output"
		fi
	else
		print_result "matches stale maestro Playwright list with flexible spacing" 1 "Matcher rejected: $output"
	fi
	return 0
}
...
test_rejects_unrelated_playwright_list() {
	...
	if output=$(run_matcher 1200 '?' "$HOME/projects/app" pnpm --filter web exec playwright test --list --grep @flaky --reporter=list 2>&1); then
		...
		if [[ "$output" == NO_MATCH*"no maestro"* ]]; then
...
test_rejects_interactive_playwright_list() {
	local output
	if output=$(run_matcher 1200 ttys001 "$HOME/Git/maestro-feature-auto-123" playwright test --list --grep @flaky 2>&1); then
...
test_rejects_fresh_maestro_playwright_list() {
	local output
	if output=$(run_matcher 300 '?' "$HOME/Git/maestro-feature-auto-123" playwright test --list --grep @flaky 2>&1); then
		print_result "rejects fresh maestro Playwright list process" 1 "Unexpected match: $output"
	else
		if [[ "$output" == NO_MATCH*"<= 900s"* ]]; then
			print_result "rejects fresh maestro Playwright list process" 0
		else
			print_result "rejects fresh maestro Playwright list process" 1 "Unexpected output: $output"
		fi
	fi
	return 0
}
...
test_rejects_substring_grep_option() {
	local output
	if output=$(run_matcher 1200 '?' "$HOME/Git/maestro-feature-auto-123" playwright test --list --no-grep @flaky 2>&1); then
