...
setup_test_env() {
	...
	mkdir -p "${TEST_ROOT}/home/.maestro/.agent-workspace/tmp"
	mkdir -p "${TEST_ROOT}/home/.maestro/logs"
...
FSETUP
	...
	export LOG_FILE="${TEST_ROOT}/home/.maestro/logs/auto-update.log"
	: >"$LOG_FILE"
...
test_drift_bootstrap_triggers_setup() {
	...
	rm -f "${HOME}/.maestro/.agent-workspace/tmp/schedulers-template-hash.state"
	rm -f "${HOME}/.maestro/.agent-workspace/tmp/plist-drift-check.stamp"
	: >"$TEST_SETUP_LOG"
...
test_drift_match_skips_setup() {
	...
	printf '%s\n' "$current_hash" >"${HOME}/.maestro/.agent-workspace/tmp/schedulers-template-hash.state"
	rm -f "${HOME}/.maestro/.agent-workspace/tmp/plist-drift-check.stamp"
	: >"$TEST_SETUP_LOG"
...
test_drift_mismatch_triggers_setup() {
	...
	printf '%s\n' "deadbeefdeadbeefdeadbeefdeadbeef" \
		>"${HOME}/.maestro/.agent-workspace/tmp/schedulers-template-hash.state"
	rm -f "${HOME}/.maestro/.agent-workspace/tmp/plist-drift-check.stamp"
	: >"$TEST_SETUP_LOG"
...
test_drift_rate_limit_suppresses_check() {
	...
	printf '%s\n' "deadbeefdeadbeefdeadbeefdeadbeef" \
		>"${HOME}/.maestro/.agent-workspace/tmp/schedulers-template-hash.state"
	date +%s >"${HOME}/.maestro/.agent-workspace/tmp/plist-drift-check.stamp"
	: >"$TEST_SETUP_LOG"
...
test_drift_stores_stamp_after_run() {
	...
	local stamp="${HOME}/.maestro/.agent-workspace/tmp/plist-drift-check.stamp"
	if [[ ! -f "$stamp" ]]; then
...
main() {
	...
	rm -rf "${HOME}/.maestro/logs/worker-no-activity" 2>/dev/null || true
	mkdir -p "${HOME}/.maestro/logs/worker-no-activity" 2>/dev/null || true
	test_preserve_moves_to_diag_dir
	test_preserve_size_caps_at_256kb
