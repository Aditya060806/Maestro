...
write_mixed_comments() {
	...
	for item in "$@"; do
		...
		case "$kind" in
		worker)
			body="Worker run.\\n\\n<!-- maestro:sig -->\\n---\\n[maestro.sh](https://maestro.sh) v3.8.78 plugin for [OpenCode](https://opencode.ai) v1.14.18 with claude-sonnet-4-6 spent 4m and ${tokens} tokens on this as a headless worker."
			;;
		interactive)
			body="Maintainer triage comment.\\n\\n<!-- maestro:sig -->\\n---\\n[maestro.sh](https://maestro.sh) v3.8.78 plugin for [OpenCode](https://opencode.ai) v1.14.18 with claude-opus-4-7 spent 1h 20m and ${tokens} tokens on this with the user in an interactive session."
			;;
		nomarker)
