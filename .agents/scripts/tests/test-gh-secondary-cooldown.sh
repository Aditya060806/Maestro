...
export MAESTRO_GH_SECONDARY_COOLDOWN_FILE="${TMP_HOME}/.maestro/cache/gh-secondary-cooldown.json"
export MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE="${TMP_HOME}/.maestro/cache/gh-cooldown-events.jsonl"
export MAESTRO_GH_SECONDARY_COOLDOWN_SECS=600
export MAESTRO_GH_READ_RAMP_STATE_FILE="${TMP_HOME}/.maestro/cache/gh-read-ramp-state.tsv"
...
reset_case() {
	: >"$CALL_LOG"
	: >"$ERR_LOG"
	rm -f "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE"
	rm -f "$MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE"
	rm -f "$MAESTRO_GH_READ_RAMP_STATE_FILE"
	unset GH_SECONDARY_FAIL GH_REST_CORE_403_FAIL GH_CORE_RATE_LIMIT_RESET GH_SEARCH_RATE_LIMIT_RESET GH_HEADER_LIMIT_FAIL GH_GENERIC_403_FAIL GH_ABUSE_403_FAIL GH_PRIMARY_REMAINING_ZERO_FAIL MAESTRO_GH_SECONDARY_COOLDOWN_OVERRIDE MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_LINES MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_BYTES MAESTRO_GH_READ_RAMP_BUDGET MAESTRO_GH_READ_RAMP_BOOT_SECS MAESTRO_GH_READ_RAMP_RECOVERY_SECS MAESTRO_GH_READ_RAMP_OVERRIDE MAESTRO_GH_AUTH_MODE MAESTRO_GH_AUTH_PRINCIPAL MAESTRO_GH_COOLDOWN_OPERATION MAESTRO_GH_COOLDOWN_WRAPPER MAESTRO_GH_COOLDOWN_STAGE MAESTRO_GH_API_POOL MAESTRO_GH_ROUTE_DECISION 2>/dev/null || true
...
test_secondary_response_writes_cooldown() {
	...
	if [[ -f "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" ]] && \
		jq -e '.reason == "github-secondary-rate-limit" and (.expires_at > .first_seen) and .diagnostic.decision_branch == "secondary-text" and .diagnostic.body_classification == "secondary-rate-limit"' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null; then
...
test_header_response_writes_retry_after_cooldown() {
	...
	if [[ -f "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" ]] && \
		jq -e '.reason == "github-api-rate-limit-status-429" and .last_request_id == "REQ-429" and .diagnostic.cooldown_action == "created" and .diagnostic.decision_branch == "status-429" and .diagnostic.method == "GET" and .diagnostic.endpoint == "/repos/<owner>/<repo>/issues" and .diagnostic.query_shape == "state=<redacted>&labels=<redacted>&per_page=<redacted>" and .diagnostic.operation == "gh_api" and .diagnostic.wrapper == "_gh_with_timeout" and .diagnostic.auth_mode == "gh-pat" and .diagnostic.http_status == "429" and .diagnostic.headers.retry_after == "42" and .diagnostic.headers.x_github_request_id == "REQ-429" and ((.expires_at - .first_seen) >= 40) and ((.expires_at - .first_seen) <= 45)' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null && \
		jq -e 'select(.cooldown_action == "created" and .cooldown_reason == "github-api-rate-limit-status-429" and .method == "GET" and .endpoint == "/repos/<owner>/<repo>/issues" and .recent_secondary_count_5m == 0)' "$MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE" >/dev/null; then
...
test_generic_403_diagnostic_distinguishes_forbidden() {
	...
	if [[ ! -f "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" ]] && [[ -f "$MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE" ]] && \
		jq -e 'select(.cooldown_action == "diagnostic-only" and .cooldown_reason == "github-api-forbidden-status-403" and .decision_branch == "status-403-diagnostic-only" and .method == "GET" and .endpoint == "/repos/<owner>/<repo>/issues" and .body_message_class == "resource-not-accessible" and .recent_403_count_1m == 1)' "$MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE" >/dev/null; then
...
test_rest_core_403_uses_rate_limit_reset_and_skips_next_call() {
	...
	if ! jq -e --argjson reset "$GH_CORE_RATE_LIMIT_RESET" '.reason == "github-api-rate-limit-status-403" and .last_request_id == "REQ-CORE" and .expires_at == $reset and .diagnostic.endpoint_family == "rest-core"' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null; then
...
test_rest_search_403_uses_search_rate_limit_reset() {
	...
	if jq -e --argjson reset "$GH_SEARCH_RATE_LIMIT_RESET" '.reason == "github-api-rate-limit-status-403" and .last_request_id == "REQ-CORE" and .expires_at == $reset and .diagnostic.endpoint_family == "rest-search"' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null; then
...
test_abuse_403_diagnostic_distinguishes_abuse_text() {
	...
	if [[ -f "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" ]] && \
		jq -e '.reason == "github-api-rate-limit-status-403" and .last_request_id == "REQ-ABUSE" and .diagnostic.decision_branch == "status-403" and .diagnostic.body_classification == "abuse-detection" and .diagnostic.headers.retry_after == "30"' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null; then
...
test_remaining_zero_diagnostic_classifies_primary_quota() {
	...
	if [[ -f "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" ]] && \
		jq -e '.reason == "github-api-rate-limit-remaining-zero" and .last_request_id == "REQ-PRIMARY" and .diagnostic.decision_branch == "remaining-zero" and .diagnostic.body_classification == "primary-rate-limit" and .diagnostic.headers.x_ratelimit_resource == "graphql"' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null; then
...
test_override_allows_audited_call() {
	reset_case
	_gh_secondary_cooldown_write "test-secondary" "fixture" >/dev/null 2>&1
	export MAESTRO_GH_SECONDARY_COOLDOWN_OVERRIDE=1
...
test_default_path_without_home_is_user_scoped() {
	...
	default_path=$(env -u HOME -u MAESTRO_GH_SECONDARY_COOLDOWN_HOME -u MAESTRO_GH_SECONDARY_COOLDOWN_FILE USER=tester bash -c 'source "$1"; _gh_secondary_cooldown_file' bash "${SCRIPT_DIR}/shared-gh-secondary-cooldown.sh")
	if [[ "$default_path" == "/tmp/.maestro-tester/.maestro/cache/gh-secondary-cooldown.json" ]]; then
...
test_no_jq_fallback_escapes_json_strings() {
	...
	if jq -e '.reason == "quote \" reason" and .last_request_id == "REQ-123" and (.expires_at > .first_seen) and .diagnostic.decision_branch == "quote \" reason" and .diagnostic.request_id == "REQ-123"' "$MAESTRO_GH_SECONDARY_COOLDOWN_FILE" >/dev/null; then
...
test_event_trim_enforces_bytes_below_line_cap() {
	reset_case
	local file="$MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE"
	local dir="${file%/*}"
	local line_count="0"
	local byte_count="0"
	local first_line=""
	...
	export MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_LINES=100
	export MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_BYTES=150
...
test_boot_ramp_defers_after_per_minute_budget() {
	...
	export MAESTRO_GH_READ_RAMP_BOOT_SECS=120
	export MAESTRO_GH_READ_RAMP_BUDGET=1
...
test_cooldown_recovery_ramp_defers_after_budget() {
	...
	local now=""
	now="$(_gh_secondary_cooldown_now)"
	local dir="${MAESTRO_GH_SECONDARY_COOLDOWN_FILE%/*}"
	mkdir -p "$dir"
	printf '{"reason":"test","first_seen":%s,"expires_at":%s,"last_request_id":""}\n' "$((now - 120))" "$((now - 10))" >"$MAESTRO_GH_SECONDARY_COOLDOWN_FILE"
	export MAESTRO_GH_READ_RAMP_RECOVERY_SECS=120
	export MAESTRO_GH_READ_RAMP_BUDGET=1
