...
export MAESTRO_CLEANUP_LOG="${TEST_ROOT}/cleanup_worktrees.log"
...
test_current_cwd_skip() {
	...
	grep -q 'current-worktree.*mode=skipped' "$MAESTRO_CLEANUP_LOG" || fail "current-cwd skip was not audited"
...
test_orphan_removal_unregisters() {
	...
	grep -q 'age-eligible.*mode=permanent' "$MAESTRO_CLEANUP_LOG" || fail "permanent removal was not audited"
...
test_zombie_reaper_requires_ledger_repo() {
	...
	printf '%s\n' '{"initialized_repos":[{"slug":"Aditya060806/Maestro","pulse":true}]}' >"${TEST_ROOT}/repos.json"
