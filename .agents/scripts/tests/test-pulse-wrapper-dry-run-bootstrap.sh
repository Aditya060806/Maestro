...
#   - fix-the-fixer detector (MAESTRO_SKIP_FIX_THE_FIXER_DETECTOR=1)
#   - Cache priming (MAESTRO_SKIP_CACHE_PRIME=1)
#   - Runaway log check (MAESTRO_SKIP_RUNAWAY_LOG_CHECK=1)
...
# Seed a sandbox HOME with the canonical maestro defaults config so
...
test_dry_run_exits_zero_quickly() {
	...
	output=$(
		HOME="${sandbox}/home" \
			...
			MAESTRO_SUPERVISOR_PULSE=true \
			MAESTRO_SKIP_FIX_THE_FIXER_DETECTOR=1 \
			MAESTRO_SKIP_CACHE_PRIME=1 \
			MAESTRO_SKIP_RUNAWAY_LOG_CHECK=1 \
...
test_idle_backoff_available_work_bypass_present() {
	if grep -q '_pulse_available_auto_dispatch_work_exists' "$WRAPPER_SCRIPT" && \
		grep -q 'MAESTRO_PULSE_IDLE_AVAILABLE_WORK' "$WRAPPER_SCRIPT"; then
