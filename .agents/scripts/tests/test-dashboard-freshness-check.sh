...
#      NO second alert (dedup via <!-- maestro:dashboard-freshness:*:* -->
...
HOME_ISO="${TMP}/home"
mkdir -p "${HOME_ISO}/.maestro/logs" \
	"${HOME_ISO}/.maestro/cache/dashboard-freshness" \
	"${HOME_ISO}/.config/maestro" \
	"${HOME_ISO}/.maestro/agents/scripts"

# repos.json: one entry with slug = test/repo so _resolve_slug_from_dashed
# can map "test-repo" → "test/repo".
cat >"${HOME_ISO}/.config/maestro/repos.json" <<'EOF'
{
	"initialized_repos": [
...
HEALTH_CACHE="${HOME_ISO}/.maestro/logs/health-issue-testrunner-test-repo"
printf '%s\n' 424242 >"$HEALTH_CACHE"
...
run_scan_with_stubs() {
	...
	rm -f "${HOME_ISO}/.maestro/cache/dashboard-freshness/last-scan"

	# shellcheck disable=SC2016
	HOME="$HOME_ISO" \
		REPOS_JSON="${HOME_ISO}/.config/maestro/repos.json" \
...
if grep -q 'macOS / launchd:' "$alert_body" \
	&& grep -q 'launchctl list | grep -i maestro-stats-wrapper' "$alert_body" \
	&& grep -q 'Linux / systemd user timers:' "$alert_body" \
	&& grep -q 'systemctl --user status maestro-stats-wrapper.timer --no-pager' "$alert_body" \
	&& grep -q 'systemctl --user status maestro-stats-wrapper.service --no-pager' "$alert_body" \
	&& grep -q "systemctl --user list-timers --all --no-pager 'maestro\*'" "$alert_body" \
	&& grep -q "${home_marker}/.config/systemd/user/maestro-stats-wrapper\.\*" "$alert_body" \
...
now_epoch=$(date -u +%s)
printf '%d' "$now_epoch" \
	>"${HOME_ISO}/.maestro/cache/dashboard-freshness/last-scan"
...
if (( created_count == 1 )) \
	&& grep -q 'Failed to list open issues for dashboard freshness recovery in test/repo' \
		"${HOME_ISO}/.maestro/logs/dashboard-freshness.log" \
	&& ! grep -q 'parse error' "${HOME_ISO}/.maestro/logs/dashboard-freshness.log" \
