...
test_protected_default_creates_planning_pr() {
	...
	output=$(cd "$work_dir" && PATH="${fake_bin}:$PATH" \
		...
		MAESTRO_PLANNING_FORCE_PR_FALLBACK=1 MAESTRO_PLANNING_PR_REPO_SLUG="example/repo" \
...
test_pr_unavailable_fails_before_commit() {
	...
	rc=0
	output=$(cd "$work_dir" && MAESTRO_PLANNING_FORCE_PR_FALLBACK=1 "$PLANNING_HELPER" "plan: add t1001 unavailable pr" 2>&1) || rc=$?
	if [[ $rc -eq 0 ]]; then
