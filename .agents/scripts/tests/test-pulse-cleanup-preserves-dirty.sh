...
test_dirty_worktree_past_grace_skips_removal() {
	...
	if ! grep -qE 'worktree-skipped|dirty-content-protect' "$MAESTRO_CLEANUP_LOG" 2>/dev/null; then
		# Acceptable alternative: no removal event logged at all (worktree was
		# never deemed eligible by _evaluate_worktree_removal).
		if grep -q 'worktree-removed' "$MAESTRO_CLEANUP_LOG" 2>/dev/null; then
			rc=1
		fi
	fi
	print_result "dirty worktree past 30m grace is NOT permanently removed" "$rc" \
		"cleanup_rc=$cleanup_rc dir_exists=$([[ -d "$wt_path" ]] && echo y || echo n) log=$(cat "$MAESTRO_CLEANUP_LOG" 2>/dev/null)"
	return 0
}
...
test_dirty_worktree_past_6h_still_skips() {
	...
	grep -q 'dirty-content-protect' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_reachable_unpushed_commits_protected() {
	...
	if ! grep -qE 'local-commits-no-pr|commits-not-on-remote' "$MAESTRO_CLEANUP_LOG" 2>/dev/null; then
		grep -q 'worktree-removed' "$MAESTRO_CLEANUP_LOG" 2>/dev/null && rc=1
	fi
...
test_sources_under_set_u() {
	(
		...
		export LOGFILE
		MAESTRO_CLEANUP_LOG="${TEST_ROOT}/cleanup-u.log"
		export MAESTRO_CLEANUP_LOG
		unset _PULSE_CLEANUP_LOADED 2>/dev/null || true
		unset _AUDIT_WORKTREE_REMOVAL_HELPER_LOADED 2>/dev/null || true
		# shellcheck source=../pulse-cleanup.sh
