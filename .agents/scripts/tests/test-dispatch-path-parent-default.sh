...
#     test_brief_bypass_env_var      — MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 suppresses notice
#     test_brief_custom_conf         — MAESTRO_DISPATCH_PATH_FILES_CONF overrides pattern set
#
...
#     test_claim_bypass_env_var      — MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 suppresses warning
...
teardown_test_env() {
	...
	unset MAESTRO_SKIP_DISPATCH_PATH_CHECK MAESTRO_DISPATCH_PATH_FILES_CONF 2>/dev/null || true
	return 0
}
...
BRIEF
	...
	MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 \
		_append_dispatch_path_notice "$brief_file" "t9903" 2>/dev/null || true

	if grep -q "Dispatch-Path Classification" "$brief_file"; then
		print_result "brief_bypass_env_var" 1 "MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 should suppress notice"
	else
...
BRIEF
	...
	MAESTRO_DISPATCH_PATH_FILES_CONF="$custom_conf" \
...
test_claim_warning_fired() {
	...
	local stderr_out
	stderr_out=$(MAESTRO_DISPATCH_PATH_FILES_CONF="${CONF_FILE}" \
...
test_claim_bypass_env_var() {
	...
	local stderr_out
	stderr_out=$(MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 \
		MAESTRO_DISPATCH_PATH_FILES_CONF="${CONF_FILE}" \
		_warn_dispatch_path_auto_dispatch 2>&1 1>/dev/null) || true

	if printf '%s' "$stderr_out" | grep -q "t2821"; then
		print_result "claim_bypass_env_var" 1 "MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 should suppress warning"
	else
