...
#
# Asserts the MAESTRO_SKIP_AUTO_CLAIM opt-out and headless-env guards
...
#   5. MAESTRO_HEADLESS=1 → no claim (headless guard)
#   6. Claude_HEADLESS=1 → no claim (headless guard, GH#20102)
#   7. MAESTRO_SKIP_AUTO_CLAIM=1 → no claim (opt-out guard, GH#20102)
...
MOCK_HELPER="${HOME}/.maestro/agents/scripts/interactive-session-helper.sh"
...
unset FULL_LOOP_HEADLESS MAESTRO_HEADLESS Claude_HEADLESS OPENCODE_HEADLESS GITHUB_ACTIONS
unset MAESTRO_SESSION_ORIGIN MAESTRO_SKIP_AUTO_CLAIM
...
# Test 5: MAESTRO_HEADLESS=1 → no claim
export MAESTRO_HEADLESS=1
run_auto_claim "feature/gh20102-auto-claim-worktree" "$FAKE_REPO"
claimed=$(get_claimed_issue)
rc=0
[[ -z "$claimed" ]] || rc=1
print_result "MAESTRO_HEADLESS=1 → no claim" "$rc" "(got: '$claimed', expected: '')"
...
# Test 7: MAESTRO_SKIP_AUTO_CLAIM=1 → no claim (GH#20102 — new opt-out)
export MAESTRO_SKIP_AUTO_CLAIM=1
run_auto_claim "feature/gh20102-auto-claim-worktree" "$FAKE_REPO"
claimed=$(get_claimed_issue)
rc=0
[[ -z "$claimed" ]] || rc=1
print_result "MAESTRO_SKIP_AUTO_CLAIM=1 → no claim (GH#20102)" "$rc" "(got: '$claimed', expected: '')"
unset MAESTRO_SKIP_AUTO_CLAIM
