...
mkdir -p "${HOME}/.maestro/.agent-workspace/headless-runtime"
mkdir -p "${HOME}/.maestro/.agent-workspace/cpu-saturation"
...
write_samples() {
	...
	local now
	now=$(date +%s)
	local state_dir="${MAESTRO_CPU_SATURATION_STATE_DIR}"
	mkdir -p "$state_dir"
...
reset_state() {
	rm -rf "${MAESTRO_CPU_SATURATION_STATE_DIR}/samples.tsv" \
		"${MAESTRO_CPU_SATURATION_STATE_DIR}/.fake-pct-queue" 2>/dev/null || true
	return 0
}
...
test_helper_check_saturated() {
	export MAESTRO_CPU_SATURATION_STATE_DIR="${TEST_ROOT}/sat-saturated"
	reset_state
...
test_helper_check_mixed_idle() {
	export MAESTRO_CPU_SATURATION_STATE_DIR="${TEST_ROOT}/sat-mixed"
	reset_state
...
test_helper_check_short_span() {
	export MAESTRO_CPU_SATURATION_STATE_DIR="${TEST_ROOT}/sat-short"
	reset_state
	# Three samples across 10s only — span < window/2, must fail-open.
	local now
	now=$(date +%s)
	mkdir -p "$MAESTRO_CPU_SATURATION_STATE_DIR"
	for i in 0 5 10; do
		printf '%s\t%s\n' "$((now - 10 + i))" "99" \
			>>"${MAESTRO_CPU_SATURATION_STATE_DIR}/samples.tsv"
	done
	if "$HELPER" check --window 120 --threshold 98 >/dev/null 2>&1; then
...
test_helper_check_no_samples() {
	export MAESTRO_CPU_SATURATION_STATE_DIR="${TEST_ROOT}/sat-empty"
	reset_state
	if "$HELPER" check --window 120 --threshold 98 >/dev/null 2>&1; then
...
test_helper_report_summary_format() {
	export MAESTRO_CPU_SATURATION_STATE_DIR="${TEST_ROOT}/sat-report"
	reset_state
...
run_classifier() {
	...
	export MAESTRO_CPU_SATURATION_STATE_DIR="${TEST_ROOT}/sat-classifier-$RANDOM"
	mkdir -p "$MAESTRO_CPU_SATURATION_STATE_DIR"
