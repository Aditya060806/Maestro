...
# maestro issue backfill pass in pulse-issue-reconcile.sh, extended by
...
#   #500 — maestro-shaped + labelless + MEMBER author          (MUST be blessed internal)
#   #501 — non-maestro shape (random bug title)                (MUST be ignored)
#   #502 — maestro-shaped + already has origin:worker          (MUST be ignored)
#   #503 — maestro-shaped + labelless + CONTRIBUTOR author     (MUST be gated external, t2450)
#   #504 — maestro-shaped + labelless + NONE author, no tags   (MUST be gated external, t2450)
#
# Assertions — internal path (unchanged from t2112):
#   - #500 edit adds origin:worker, tier:standard, and body tags (ai, security)
#   - #500 comment contains the INTERNAL sentinel (<!-- maestro:labelless-backfill -->)
...
[
  ...
  },
  {
    ...
    "body": "Sometimes fails on CI. Not maestro-shaped.",
    "labels": []
  },
  {
    "number": 502,
  ...
  },
  {
    ...
    "body": "External CONTRIBUTOR-authored labelless maestro-shaped issue.\n\n## Tags inline: #ai #security\n",
    "labels": []
  },
  {
    "number": 504,
    "title": "GH#9999: propose additional dedup layer",
    "body": "External NONE-authored labelless maestro-shaped issue with no body tags.",
    "labels": []
  }
]
JSON
...
# Stub the t2393 comment wrappers so reconcile_labelless_maestro_issues's
...
gh() {
	...
	# (t2450: reconcile_labelless_maestro_issues fetches author_association
...
# -----------------------------------------------------------------------------
if ! reconcile_labelless_maestro_issues >"${TEST_TMPDIR}/cmd.out" 2>&1; then
	printf 'FAIL: reconcile_labelless_maestro_issues exited non-zero\n'
...
# #501 must NOT be edited or commented — it's not maestro-shaped
if [[ "$edit_501" -ne 0 ]]; then
	printf 'FAIL: #501 should not be edited (non-maestro shape), got %d edits\n' "$edit_501"
	failed=1
fi
...
if [[ ! -f "$body_file_503" ]]; then
	...
	if ! grep -q 'maestro:labelless-backfill-external' "$body_file_503"; then
...
if [[ ! -f "$body_file_504" ]]; then
	...
	failed=1
elif ! grep -q 'maestro:labelless-backfill-external' "$body_file_504"; then
...
if [[ -f "$body_file_500" ]]; then
	if grep -q 'maestro:labelless-backfill-external' "$body_file_500"; then
		printf 'FAIL: #500 comment body contains EXTERNAL sentinel (should be internal)\n'
		failed=1
	fi
	if ! grep -q 'maestro:labelless-backfill -->' "$body_file_500"; then
