...
#   7. MAESTRO_SCAN_STALE_AUTO_RELEASE=0 env → dead+missing stamp NOT released
#   8. MAESTRO_SCAN_STALE_AUTO_RELEASE=1 env + non-TTY → dead+missing released
#   9. OPENCODE_SESSION_ID set, no TTY, no headless → dead+missing released (t3205)
#   10. MAESTRO_HEADLESS + OPENCODE_SESSION_ID → stamp preserved (headless wins, t3205)
...
# =============================================================================
# Test 7 — MAESTRO_SCAN_STALE_AUTO_RELEASE=0 env → dead+missing stamp NOT released
# =============================================================================
write_stamp "owner-repo-107.json" "99999" "/nonexistent/path/107" "107" "owner/repo"

MAESTRO_SCAN_STALE_AUTO_RELEASE=0 _isc_cmd_scan_stale >/dev/null 2>/dev/null || true

if [[ -f "${STAMP_DIR}/owner-repo-107.json" ]]; then
	pass "MAESTRO_SCAN_STALE_AUTO_RELEASE=0 → stamp preserved"
else
	fail "MAESTRO_SCAN_STALE_AUTO_RELEASE=0 → stamp preserved" \
...
# =============================================================================
# Test 8 — MAESTRO_SCAN_STALE_AUTO_RELEASE=1 env → dead+missing stamp released
# =============================================================================
write_stamp "owner-repo-108.json" "99999" "/nonexistent/path/108" "108" "owner/repo"

MAESTRO_SCAN_STALE_AUTO_RELEASE=1 _isc_cmd_scan_stale >/dev/null 2>/dev/null || true

if [[ ! -f "${STAMP_DIR}/owner-repo-108.json" ]]; then
	pass "MAESTRO_SCAN_STALE_AUTO_RELEASE=1 env → dead+missing stamp released"
else
	fail "MAESTRO_SCAN_STALE_AUTO_RELEASE=1 env → dead+missing stamp released" \
		"stamp still exists despite MAESTRO_SCAN_STALE_AUTO_RELEASE=1"
fi
...
unset MAESTRO_SCAN_STALE_AUTO_RELEASE
...
# =============================================================================
# When BOTH MAESTRO_HEADLESS and OPENCODE_SESSION_ID are set, the headless
...
env -u FULL_LOOP_HEADLESS -u OPENCODE_HEADLESS -u GITHUB_ACTIONS \
	...
	MAESTRO_HEADLESS=1 OPENCODE_SESSION_ID=test-session-id \
	bash -c "
		set -uo pipefail
		unset MAESTRO_SCAN_STALE_AUTO_RELEASE
		# shellcheck source=/dev/null
...
if [[ -f "${STAMP_DIR}/owner-repo-110.json" ]]; then
	pass "MAESTRO_HEADLESS=1 + OPENCODE_SESSION_ID → stamp preserved (headless wins)"
else
	fail "MAESTRO_HEADLESS=1 + OPENCODE_SESSION_ID → stamp preserved (headless wins)" \
