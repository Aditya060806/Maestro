...
#   Every concurrent worker startup hit `[maestro] SQLite: Runtime error
#   near line 171: database is locked (5)` while initialising the shared
#   observability DB at ~/.maestro/.agent-workspace/observability/
...
# observability DB, owned by `.agents/plugins/opencode-maestro/
...
PLUGIN_DIR="${REPO_ROOT}/.agents/plugins/opencode-maestro"
...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/maestro-t2900.XXXXXX")" || exit 1
...
for ((i = 0; i < WORKER_COUNT; i++)); do
	MAESTRO_OBS_DB_OVERRIDE="$DB_PATH" \
