...
test_absolute_cap_hit() {
	...
	[[ -f "${HOME}/.maestro/cache/pulse-wrapper-log-runaway-advisory.txt" ]] && advisory_result=0
...
test_healthy_log_noop() {
	...
	rm -f "${HOME}/.maestro/cache/pulse-wrapper-log-runaway-advisory.txt" 2>/dev/null || true
