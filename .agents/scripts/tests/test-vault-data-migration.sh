...
TEST_ROOT="$(mktemp -d 2>/dev/null || mktemp -d -t maestro-vault-migration-test)"
PASS=0
FAIL=0
...
export MAESTRO_VAULT_REQUIRE=1
export MAESTRO_MEMORY_DIR="$TEST_ROOT/memory"
export MAESTRO_VAULT_MIGRATION_ROOT="$TEST_ROOT/workspace"
export MAESTRO_VAULT_MIGRATION_MANIFEST_DIR="$TEST_ROOT/manifest"
export MOCK_VAULT_STORE_DIR="$TEST_ROOT/mock-store"
export MOCK_VAULT_STATE_FILE="$TEST_ROOT/mock-state"
export MAESTRO_VAULT_HELPER="$TEST_ROOT/bin/mock-vault-helper.sh"
write_mock_vault_helper "$MAESTRO_VAULT_HELPER"
...
mkdir -p "$MAESTRO_VAULT_MIGRATION_ROOT/memory" "$MAESTRO_VAULT_MIGRATION_ROOT/knowledge/sources/a"
printf '%s' 'memory plaintext' >"$MAESTRO_VAULT_MIGRATION_ROOT/memory/memory.db"
printf '%s' 'knowledge plaintext' >"$MAESTRO_VAULT_MIGRATION_ROOT/knowledge/sources/a/source.md"
...
if [[ -f "$MAESTRO_VAULT_MIGRATION_ROOT/memory/memory.db" ]]; then
...
assert_eq "rollback restores memory content" "memory plaintext" "$(cat "$MAESTRO_VAULT_MIGRATION_ROOT/memory/memory.db")"
