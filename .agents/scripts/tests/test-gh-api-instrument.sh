...
export MAESTRO_GH_API_LOG="$TMPDIR/gh-api-calls.log"
export MAESTRO_GH_API_REPORT="$TMPDIR/report.json"
...
assert_file_exists "log file created" "$MAESTRO_GH_API_LOG"

line_count=$(wc -l <"$MAESTRO_GH_API_LOG" | tr -d ' ')
assert_eq "log has 6 lines" "6" "$line_count"

# --- Test 2: aggregate and validate JSON shape -------------------------
gh_aggregate_calls

assert_file_exists "report file created" "$MAESTRO_GH_API_REPORT"

if ! jq -e '.' "$MAESTRO_GH_API_REPORT" >/dev/null 2>&1; then
...
total_calls=$(jq -r '._meta.total_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "report total_calls = 6" "6" "$total_calls"

rest_count=$(jq -r '.by_caller["test-instrument"].rest_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "rest_calls = 3" "3" "$rest_count"

graphql_count=$(jq -r '.by_caller["test-instrument"].graphql_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "graphql_calls = 1" "1" "$graphql_count"

search_graphql_count=$(jq -r '.by_caller["test-instrument"].search_graphql_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "search_graphql_calls = 1" "1" "$search_graphql_count"

search_rest_count=$(jq -r '.by_caller["test-instrument"].search_rest_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "search_rest_calls = 1" "1" "$search_rest_count"

app_auth_count=$(jq -r '.by_auth_mode["github-app"].total' "$MAESTRO_GH_API_REPORT")
assert_eq "github-app auth count = 1" "1" "$app_auth_count"

rest_core_count=$(jq -r '.by_api_pool["rest-core"].total' "$MAESTRO_GH_API_REPORT")
assert_eq "rest-core pool count = 3" "3" "$rest_core_count"

budget_min=$(jq -r '.budget_by_pool["rest-core"].min_remaining' "$MAESTRO_GH_API_REPORT")
...
export MAESTRO_GH_API_LOG_MAX_LINES=4
...
source "${PARENT_DIR}/gh-api-instrument.sh"
gh_trim_log
trimmed_count=$(wc -l <"$MAESTRO_GH_API_LOG" | tr -d ' ')
...
gh_clear_log
if [[ -f "$MAESTRO_GH_API_LOG" ]]; then
	echo "  FAIL: clear left log behind: $MAESTRO_GH_API_LOG"
	FAIL=$((FAIL + 1))
else
	echo "  PASS: clear removed log"
	PASS=$((PASS + 1))
fi
if [[ -f "$MAESTRO_GH_API_REPORT" ]]; then
	echo "  FAIL: clear left report behind: $MAESTRO_GH_API_REPORT"
	FAIL=$((FAIL + 1))
else
...
gh_clear_log
MAESTRO_GH_API_INSTRUMENT_DISABLE=1 gh_record_call rest test-disabled
if [[ -f "$MAESTRO_GH_API_LOG" ]]; then
...
unset MAESTRO_GH_API_INSTRUMENT_DISABLE
gh_clear_log
...
rate_limit_count=$(jq -r '.by_caller["pulse_batch_prefetch_rate_limit"].rest_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "explicit caller: rate-limit REST call counted separately" "1" "$rate_limit_count"

search_issue_count=$(jq -r '.by_caller["pulse_batch_prefetch_search_issues"].search_graphql_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "explicit caller: issue search counted separately" "1" "$search_issue_count"

search_pr_count=$(jq -r '.by_caller["pulse_batch_prefetch_search_prs"].search_graphql_calls' "$MAESTRO_GH_API_REPORT")
assert_eq "explicit caller: PR search counted separately" "1" "$search_pr_count"

tickle_count=$(jq -r '.by_caller["events_tickle_events"].rest_calls' "$MAESTRO_GH_API_REPORT")
...
unset _GH_API_INSTRUMENT_LOADED MAESTRO_GH_API_LOG MAESTRO_GH_API_REPORT
...
PATH="$FAKE_BIN:$PATH" HOME='' TMPDIR="$FALLBACK_TMP" USER="ghapitest" bash -c '
	...
	mode=$(stat -f %Lp "$TMPDIR/maestro-$USER" 2>/dev/null || stat -c %a "$TMPDIR/maestro-$USER")
	[[ "$mode" == "700" ]]
	[[ -f "$TMPDIR/maestro-$USER/.maestro/logs/gh-api-calls.log" ]]
...
ln -s "$SYMLINK_TARGET" "$SYMLINK_TMP/maestro-ghapitest"
set +e
PATH="$FAKE_BIN:$PATH" HOME='' TMPDIR="$SYMLINK_TMP" USER="ghapitest" bash -c '
	...
	[[ "${MAESTRO_GH_API_INSTRUMENT_DISABLE:-0}" == "1" ]]
	[[ ! -e "$TMPDIR/maestro-$USER/.maestro/logs/gh-api-calls.log" ]]
' _ "${PARENT_DIR}/gh-api-instrument.sh"
symlink_status="$?"
set -e
