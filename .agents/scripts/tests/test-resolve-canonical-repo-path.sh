...
#
# Background: auto-discovery via `find ~/Git -name .maestro.json` picks up
# .maestro.json files that exist inside linked git worktrees because worktrees
...
#
# resolve_canonical_repo_path() in maestro.sh uses `git rev-parse --git-dir`
...
#   2. Sources maestro.sh in a stubbed environment (suppresses print_* calls).
...
# function definition. Sourcing maestro.sh whole would run side-effectful
...
eval "$(extract_and_source_function resolve_canonical_repo_path "${MAESTRO_SH}")"

if ! declare -f resolve_canonical_repo_path >/dev/null; then
	_fail "could not source resolve_canonical_repo_path from maestro.sh"
	exit 1
fi
...
_info "Test 4: current canonical maestro repo self-resolves"
if [[ -d "$REPO_ROOT/.git" || -f "$REPO_ROOT/.git" ]]; then
	# The maestro checkout running this test may itself be a linked worktree
	...
	if [[ -d "$actual" ]]; then
		_pass "maestro repo resolves to existing path ($actual)"
	else
		_fail "maestro repo resolved to non-existent path: $actual"
	fi
fi
