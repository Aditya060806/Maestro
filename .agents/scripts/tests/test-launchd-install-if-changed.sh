...
_define_launchd_xpcproxy_recovery() {
	_launchd_recover_xpcproxy_if_stuck() {
		...
		attempts="${MAESTRO_LAUNCHD_XPCPROXY_SETTLE_ATTEMPTS:-5}"
		interval="${MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS:-1}"
...
_load_schedulers_platform_functions() {
	...
	maestro_launchd_sanitized_path() { printf '%s\n' "/usr/bin:/bin"; return 0; }
	_launchd_install_if_changed() { return 0; }
...
test_legacy_cleanup_idempotent() {
	...
	# Create legacy plist
	local legacy_plist="$fake_home/Library/LaunchAgents/com.maestro.supervisor-pulse.plist"
	...
	_install_pulse_launchd \
		"com.maestro.maestro-supervisor-pulse" "/fake/wrapper.sh" "" "false" || rc=$?
	...
	rc=0
	_install_pulse_launchd \
		"com.maestro.maestro-supervisor-pulse" "/fake/wrapper.sh" "" "true" || rc=$?
...
test_xpcproxy_successful_recovery_does_not_warn() {
	...
	local old_interval="${MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS:-}"
	export MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS=0

	local rc=0
	_launchd_install_if_changed "test-label" "$plist_path" "some plist content" 2>"$stderr_file" || rc=$?

	if [[ -n "$old_interval" ]]; then
		export MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS="$old_interval"
	else
		unset MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS
	fi
	unset -f launchctl _launchd_has_agent
...
test_xpcproxy_recovery_waits_for_transient_state() {
	...
	local old_attempts="${MAESTRO_LAUNCHD_XPCPROXY_SETTLE_ATTEMPTS:-}"
	local old_interval="${MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS:-}"
	export MAESTRO_LAUNCHD_XPCPROXY_SETTLE_ATTEMPTS=3
	export MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS=0
	...
	if [[ -n "$old_attempts" ]]; then
		export MAESTRO_LAUNCHD_XPCPROXY_SETTLE_ATTEMPTS="$old_attempts"
	else
		unset MAESTRO_LAUNCHD_XPCPROXY_SETTLE_ATTEMPTS
	fi
	if [[ -n "$old_interval" ]]; then
		export MAESTRO_LAUNCHD_XPCPROXY_SETTLE_SECONDS="$old_interval"
	else
...
test_xpcproxy_state_with_helper_process_not_recovered() {
	...
	_launchd_has_agent() { return 0; }
	_launchd_process_args() { printf '/opt/homebrew/bin/bash /Users/test/.maestro/agents/scripts/profile-readme-helper.sh update\n'; return 0; }
...
test_profile_readme_install_does_not_kickstart() {
	...
	mkdir -p "$fake_home/Library/LaunchAgents"
	local plist_path="$fake_home/Library/LaunchAgents/sh.maestro.profile-readme-update.plist"
...
<dict>
	<key>Label</key>
	<string>sh.maestro.profile-readme-update</string>
	<key>ProgramArguments</key>
	<array>
		<string>/bin/bash</string>
		<string>-lc</string>
		<string>start_epoch=$(date +%s); status=success; &apos;/fake/profile-readme-helper.sh&apos; update; rc=$?; end_epoch=$(date +%s); duration=$(( ${end_epoch:-0} - ${start_epoch:-0} )); if [ &quot;$rc&quot; -ne 0 ]; then status=failure; fi; if [ -x &quot;$HOME/.maestro/agents/scripts/routine-log-helper.sh&quot; ]; then &quot;$HOME/.maestro/agents/scripts/routine-log-helper.sh&quot; update &quot;r908&quot; --status &quot;$status&quot; --duration &quot;$duration&quot; &gt;/dev/null 2&gt;&amp;1 || true; fi; exit &quot;$rc&quot;</string>
	</array>
	<key>StartInterval</key>
	<integer>3600</integer>
	<key>StandardOutPath</key>
	<string>__FAKE_HOME__/.maestro/.agent-workspace/logs/profile-readme-update.log</string>
...
PROFILE_PLIST
	...
	local rc=0
	_install_profile_readme_launchd "sh.maestro.profile-readme-update" "/fake/profile-readme-helper.sh" 2>"$stderr_file" || rc=$?
...
test_interval_jobs_skip_unchanged_loaded_recovery() {
	...
	local complexity_plist="$fake_home/Library/LaunchAgents/sh.maestro.complexity-scan.plist"
	local peer_plist="$fake_home/Library/LaunchAgents/sh.maestro.peer-productivity-monitor.plist"
	...
	complexity_content=$(_install_complexity_scan_launchd_render_fixture "sh.maestro.complexity-scan" "/fake/complexity-scan-runner.sh" "$fake_home/logs")
	peer_content=$(_install_peer_productivity_monitor_launchd_render_fixture "sh.maestro.peer-productivity-monitor" "/fake/peer-productivity-monitor.sh" "$fake_home/logs")
	printf '%s\n' "$complexity_content" >"$complexity_plist"
	printf '%s\n' "$peer_content" >"$peer_plist"

	local rc=0
	_install_complexity_scan_launchd "sh.maestro.complexity-scan" "/fake/complexity-scan-runner.sh" "$fake_home/logs" 2>"$stderr_file" || rc=$?
	_install_peer_productivity_monitor_launchd "sh.maestro.peer-productivity-monitor" "/fake/peer-productivity-monitor.sh" "$fake_home/logs" 2>>"$stderr_file" || rc=$?
