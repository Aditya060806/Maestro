...
# Also tests round-trip preservation of init_scope in .maestro.json and repos.json.
#
...
# Source the maestro.sh to get access to the helper functions
# We need to find the repo root
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
MAESTRO_SH="$REPO_ROOT/maestro.sh"
MAESTRO_INIT_LIB="$REPO_ROOT/.agents/scripts/maestro-cli/maestro-init-lib.sh"
MAESTRO_REPOS_LIB="$REPO_ROOT/.agents/scripts/maestro-cli/maestro-repos-lib.sh"

if [[ ! -f "$MAESTRO_SH" ]]; then
	echo "ERROR: Cannot find maestro.sh at $MAESTRO_SH" >&2
	exit 1
fi

if [[ ! -f "$MAESTRO_INIT_LIB" ]]; then
	echo "ERROR: Cannot find maestro-init-lib.sh at $MAESTRO_INIT_LIB" >&2
	exit 1
fi

if [[ ! -f "$MAESTRO_REPOS_LIB" ]]; then
	echo "ERROR: Cannot find maestro-repos-lib.sh at $MAESTRO_REPOS_LIB" >&2
	exit 1
fi
...
test_scope_includes() {
	...
	# Extract the function from maestro.sh
	local func_body
	func_body=$(sed -n '/_scope_includes() {/,/^}/p' "$MAESTRO_REPOS_LIB")
	eval "$func_body"
...
test_infer_init_scope() {
	...
	# Extract the function from maestro.sh
	local func_body
	func_body=$(sed -n '/_infer_init_scope() {/,/^}/p' "$MAESTRO_REPOS_LIB")
	eval "$func_body"

	# Test 1: .maestro.json with explicit init_scope takes priority
	local test_dir="$TEST_ROOT/test-json-scope"
	mkdir -p "$test_dir"
	git -C "$test_dir" init --quiet 2>/dev/null
	echo '{"init_scope": "public"}' > "$test_dir/.maestro.json"

	local result
	result=$(_infer_init_scope "$test_dir")
	assert_equals "public" "$result" ".maestro.json explicit scope respected"
	...
	# Test 3: Empty .maestro.json (no init_scope) + no remote → minimal
	local test_dir3="$TEST_ROOT/test-empty-json"
	mkdir -p "$test_dir3"
	git -C "$test_dir3" init --quiet 2>/dev/null
	echo '{"version": "1.0"}' > "$test_dir3/.maestro.json"

	result=$(_infer_init_scope "$test_dir3")
	assert_equals "minimal" "$result" "Empty .maestro.json plus no remote → minimal"
...
test_interface_detection() {
	...
	helper_path_func=$(sed -n '/_init_design_helper_path() {/,/^}/p' "$MAESTRO_REPOS_LIB")
	detect_func=$(sed -n '/_init_repo_has_interface() {/,/^}/p' "$MAESTRO_REPOS_LIB")
	...
	printf '%s\n' '{"has_interface": false}' >"$explicit_false_dir/.maestro.json"
...
# Shared setup for scaffold scope tests — extracts needed functions from maestro.sh
_setup_scaffold_test_env() {
	TEST_ROOT=$(mktemp -d)

	local scope_func
	scope_func=$(sed -n '/_scope_includes() {/,/^}/p' "$MAESTRO_REPOS_LIB")
	eval "$scope_func"
	...
	local scaffold_func
	scaffold_func=$(sed -n '/^scaffold_repo_courtesy_files() {/,/^}/p' "$MAESTRO_INIT_LIB")
	local contributing_func
	contributing_func=$(sed -n '/^_scaffold_contributing() {/,/^}/p' "$MAESTRO_INIT_LIB")
	local security_func
	security_func=$(sed -n '/^_scaffold_security() {/,/^}/p' "$MAESTRO_INIT_LIB")
	local coc_func
	coc_func=$(sed -n '/^_scaffold_coc() {/,/^}/p' "$MAESTRO_INIT_LIB")
	local sec_content_func
	sec_content_func=$(sed -n '/^_generate_security_content() {/,/^}/p' "$MAESTRO_INIT_LIB")
...
# ---- Test .maestro.json round-trip ----

test_maestro_json_roundtrip() {
	echo ""
	echo "=== Testing .maestro.json init_scope round-trip ==="
...
EOF
	...
	local scope
	scope=$(jq -r '.init_scope' "$test_dir/.maestro.json")
	assert_equals "minimal" "$scope" ".maestro.json init_scope round-trip"

	# Verify init_scope is present and not null
	local has_scope
	has_scope=$(jq 'has("init_scope")' "$test_dir/.maestro.json")
	assert_equals "true" "$has_scope" ".maestro.json has init_scope field"

	local has_interface
	has_interface=$(jq -r '.has_interface' "$test_dir/.maestro.json")
	assert_equals "true" "$has_interface" ".maestro.json has has_interface field"
...
test_backward_compat() {
	...
	# Repo with a remote but no init_scope in .maestro.json → standard
	...
	echo '{"version": "3.0.0"}' > "$test_dir/.maestro.json"
...
test_label_sync_init_hook() {
	...
	local init_tail
	init_tail=$(sed -n '/local _label_sync_helper=/,/register_repo /p' "$MAESTRO_INIT_LIB")
	...
	repo_slug_block=$(sed -n '/local repo_slug=""/,/local repo_name/p' "$MAESTRO_INIT_LIB")
...
test_repo_metrics_init_hook() {
	...
	local init_tail
	init_tail=$(sed -n '/local _metrics_helper=/,/Run security posture assessment/p' "$MAESTRO_INIT_LIB")
	local init_files
	init_files=$(sed -n '/local init_files=()/,/local committed=false/p' "$MAESTRO_INIT_LIB")
...
test_coderabbit_abort_on_close_init_hook() {
	...
	local init_body
	init_body=$(sed -n '/_init_configure_coderabbit_abort_on_close() {/,/^}/p' "$MAESTRO_INIT_LIB")
	local init_tail
	init_tail=$(sed -n '/_init_scaffold_scope_gated_files/,/Badge initialization/p' "$MAESTRO_INIT_LIB")
	local init_files
	init_files=$(sed -n '/local init_files=()/,/local committed=false/p' "$MAESTRO_INIT_LIB")
	local rc

	if [[ "$init_body" == *"MAESTRO_CODERABBIT_ABORT_ON_CLOSE"* && "$init_body" == *"abort_on_close"* ]]; then rc=0; else rc=1; fi
