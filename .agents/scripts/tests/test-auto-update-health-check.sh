...
_run_in_sandbox() {
	...
	SANDBOX=$(mktemp -d -t maestro-t2898-XXXXXX)
	export HOME="$SANDBOX"
	mkdir -p "$HOME/.maestro/cache" "$HOME/.maestro/logs" "$HOME/.maestro/agents/scripts"
...
#!/usr/bin/env bash
# Stub: emit a launchctl-list-shaped line containing the maestro label.
echo "12345  0   com.maestro.maestro-auto-update"
exit 0
EOF
	else
		cat >"$stub_dir/launchctl" <<'EOF'
#!/usr/bin/env bash
# Stub: empty list (no maestro label).
exit 0
EOF
	fi
	...
	local state_file="$HOME/.maestro/cache/auto-update-state.json"
	case "$state_kind" in
	none) ;; # No state file
	empty)
	...
	PATH="$stub_dir:$PATH" bash "$HELPER" "$@" 2>"/tmp/maestro-t2898-stderr-$$" >/dev/null
	local rc=$?
	# Promote captured stderr to a stable path for assertion convenience.
	mv "/tmp/maestro-t2898-stderr-$$" /tmp/last-stderr 2>/dev/null || true
	rm -rf "$SANDBOX"
	SANDBOX=""
	return "$rc"
}
