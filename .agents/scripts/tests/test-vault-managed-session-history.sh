...
TEST_ROOT="$(mktemp -d 2>/dev/null || mktemp -d -t maestro-vault-managed-history)"
cleanup() {
	rm -rf "$TEST_ROOT"
	return 0
}
...
export MAESTRO_VAULT_MANAGED_SESSION_HISTORY=1
export MAESTRO_VAULT_MANAGED_HISTORY_ROOT="$TEST_ROOT/managed-history"

set +e
locked_output="$(MAESTRO_VAULT_STATUS_OVERRIDE=locked "$HELPER" require-read opencode 2>&1 >/dev/null)"
locked_rc=$?
set -e
...
unlocked_path="$(MAESTRO_VAULT_STATUS_OVERRIDE=unlocked "$HELPER" require-read opencode)"
assert_eq "unlocked OpenCode read resolves broker path" "$TEST_ROOT/managed-history/opencode/opencode.db" "$unlocked_path"

claude_path="$(MAESTRO_VAULT_STATUS_OVERRIDE=unlocked "$HELPER" require-read claude-code)"
assert_eq "unlocked Claude read resolves broker path" "$TEST_ROOT/managed-history/claude-code/projects" "$claude_path"

set +e
unsupported_output="$(MAESTRO_VAULT_STATUS_OVERRIDE=unlocked "$HELPER" require-read codex 2>&1 >/dev/null)"
unsupported_rc=$?
set -e
...
unset MAESTRO_VAULT_MANAGED_SESSION_HISTORY
