	...
	case "\$field" in
		...
		"labels,author,authorAssociation")
			...
			if [[ -f "\$response" ]]; then
				...
				echo '{"labels":[{"name":"origin:worker"}],"author":{"login":"maestro-worker[bot]"},"authorAssociation":"NONE"}'
			fi
			exit 0
			;;
		files)
...
set_responses() {
	...
	if [[ -n "$pr_meta_json" ]]; then
		...
		printf '%s' '{"labels":[{"name":"origin:worker"}],"author":{"login":"maestro-worker[bot]"},"authorAssociation":"NONE"}' \
...
test_wording_with_squash_merge_pr_number() {
	...
	_close_conflicting_pr "18486" "Aditya060806/Maestro" \
...
test_no_close_when_matching_commit_only_touches_planning_files() {
	...
	_close_conflicting_pr "18760" "Aditya060806/Maestro" \
...
test_no_close_for_external_contributor_pr() {
	...
	_close_conflicting_pr "20485" "Aditya060806/Maestro" \
...
test_close_for_worker_origin_pr() {
	...
	_close_conflicting_pr "20486" "Aditya060806/Maestro" \
...
test_close_for_worker_takeover_pr() {
	...
	set_responses \
		...
		'.agents/scripts/pulse-merge-conflict.sh' \
		'.agents/scripts/pulse-merge-conflict.sh' \
		'{"labels":[{"name":"origin:worker-takeover"},{"name":"origin:interactive"}],"author":{"login":"Aditya060806"},"authorAssociation":"OWNER"}'

	load_functions_under_test

	_close_conflicting_pr "20487" "Aditya060806/Maestro" \
	...
	_close_conflicting_pr "20488" "Aditya060806/Maestro" \
		"t9999: worker-takeover PR"
...
test_close_for_bot_author_no_label_pr() {
	...
	_close_conflicting_pr "20489" "Aditya060806/Maestro" \
...
STUB_EOF
	...
	_close_conflicting_pr "99999" "Aditya060806/Maestro" "t9999: metadata-fail test"
