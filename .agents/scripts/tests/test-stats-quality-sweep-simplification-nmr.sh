...
mkdir -p "${TMP}/.config/maestro"
cat >"${TMP}/.config/maestro/repos.json" <<'JSON'
...
_gh_current_user_allows_repo_write() {
	MAESTRO_GH_WRITE_PERMISSION_USER="maintainer"
	MAESTRO_GH_WRITE_PERMISSION_LEVEL="admin"
...
true >"$COUNT_FILE"
qlty_section="caller-owned-section"
unset MAESTRO_GH_WRITE_PERMISSION_USER MAESTRO_GH_WRITE_PERMISSION_LEVEL MAESTRO_GH_WRITE_PERMISSION_REASON
_gh_current_user_allows_repo_write() {
	MAESTRO_GH_WRITE_PERMISSION_REASON="permission-lookup-failed:api-failure"
