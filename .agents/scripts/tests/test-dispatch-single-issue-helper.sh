...
_dsi_repo_slug_for_worktree() {
	...
	/tmp/maestro-existing | /tmp/maestro-recovery) printf '%s\n' "owner/repo" ;;
	*) printf '\n' ;;
	esac
	return 0
}
...
test_maintainer_review_guard_blocks_manual_dispatch() {
	...
	local check=1
	[[ "$rc" -eq 1 && "$out" == *"requires maintainer review"* && "$out" == *"sudo maestro approve issue 24354 owner/repo"* ]] && check=0
...
test_create_worktree_uses_target_repo_path() {
	...
	if grep -Fq "repo_path=\$(_dsi_repo_path_for_slug \"\$repo_slug\")" "$HELPER_PATH" &&
		grep -Fq "cd \"\$repo_path\" && MAESTRO_SKIP_AUTO_CLAIM=1" "$HELPER_PATH" &&
...
test_create_worktree_registers_dispatch_owner() {
	...
	local -x MAESTRO_REPOS_FILE="$repos_file"
...
test_dispatch_base_ref_prefers_repo_configured_pr_base() {
	...
	local -x MAESTRO_REPOS_FILE="$repos_file"
	unset MAESTRO_DISPATCH_BASE_REF MAESTRO_WORKTREE_BASE MAESTRO_DISPATCH_BASE_BRANCH DISPATCH_REPO_PR_BASE_BRANCH MAESTRO_PR_BASE_BRANCH
...
test_dispatch_base_ref_prefers_explicit_env_ref() {
	...
	local repo_path="${test_dir}/repo"
	mkdir -p "$repo_path"
	export MAESTRO_DISPATCH_BASE_REF="upstream/integration"
	...
	unset MAESTRO_DISPATCH_BASE_REF
	rm -rf "$test_dir"
...
test_readiness_accepts_worker_started_marker() {
	...
	local out="" rc=0
	out=$(MAESTRO_DSI_READY_TIMEOUT_SECONDS=0 _dsi_wait_for_worker_readiness 12345 owner/repo "$session_key" "$$" /tmp/manual-ready.log 2>&1) || rc=$?
	rm -f "$runtime_log"
...
test_readiness_rejects_live_child_without_ready_signal() {
	...
	local out="" rc=0
	out=$(MAESTRO_DSI_READY_TIMEOUT_SECONDS=0 _dsi_wait_for_worker_readiness 12345 owner/repo "$session_key" "$$" /tmp/manual-not-ready.log 2>&1) || rc=$?
	rm -f "$runtime_log"
...
test_readiness_rejects_ledger_without_worker_started() {
	MOCK_LEDGER_RECORD=$'ledger\t888\t/tmp/manual.log\t/tmp/maestro-existing\tmanual-cli-12345-ledger'
	...
	local out="" rc=0
	out=$(MAESTRO_DSI_READY_TIMEOUT_SECONDS=0 _dsi_wait_for_worker_readiness 12345 owner/repo "$session_key" "$$" /tmp/manual-ledger-only.log 2>&1) || rc=$?
	rm -f "$runtime_log"
	MOCK_LEDGER_RECORD=""
...
test_live_dispatch_detects_issue_repo() {
	MOCK_LEDGER_RECORD=""
	MOCK_PS_LINES='700 S bash /Users/test/.maestro/agents/scripts/headless-runtime-helper.sh run --role worker --session-key manual-cli-12345-999 --dir /tmp/maestro-existing --title Issue #12345 --prompt-file /tmp/prompt'

	local record="" rc=0
	record=$(_dsi_find_live_dispatch 12345 owner/repo "") || rc=$?

	local found=1
	[[ "$rc" -eq 0 && "$record" == process$'\t'700$'\t'*$'\t'/tmp/maestro-existing$'\t'manual-cli-12345-999 ]] && found=0
...
test_guard_blocks_ledger_duplicate() {
	MOCK_PS_LINES=""
	MOCK_LEDGER_RECORD=$'ledger\t888\t/tmp/manual.log\t/tmp/maestro-existing\tmanual-cli-12345-1'
...
test_guard_blocks_live_worktree_duplicate() {
	MOCK_LEDGER_RECORD=""
	MOCK_PS_LINES='701 S opencode run --dir /tmp/maestro-recovery --title Issue #9999 "/full-loop Implement issue #9999"'

	local out="" rc=0
	out=$(_dsi_guard_no_existing_dispatch 12345 owner/repo /tmp/maestro-recovery 2>&1) || rc=$?

	local blocked=1
	[[ "$rc" -eq 1 && "$out" == *"701"* && "$out" == *"/tmp/maestro-recovery"* ]] && blocked=0
...
test_status_reports_live_process_without_ledger() {
	MOCK_LEDGER_RECORD=""
	MOCK_PS_LINES='702 S bash /Users/test/.maestro/agents/scripts/headless-runtime-helper.sh run --role worker --session-key manual-cli-12345-777 --dir /tmp/maestro-existing --title Issue #12345 --prompt-file /tmp/prompt'
