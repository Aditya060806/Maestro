...
# `os.path.expanduser("~/.maestro/agents")` and similar calls resolve into
# our sandbox rather than the real user's deployment.
run_discovery_script() {
	local script_path="$1"
	shift
	local fake_home="$TEST_DIR/fake-home"
	mkdir -p "$fake_home/.maestro" "$fake_home/.config/opencode" "$fake_home/.claude"

	# Link fixture agents into the fake HOME layout.
	ln -sfn "$TEST_DIR/agents" "$fake_home/.maestro/agents"
