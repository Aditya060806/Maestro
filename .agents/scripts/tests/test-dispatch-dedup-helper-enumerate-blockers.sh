...
test_no_blockers_returns_safe() {
	...
	output=$("$HELPER_SCRIPT" enumerate-blockers 100 Aditya060806/Maestro 2>/dev/null) || exit_code=$?
...
test_is_assigned_still_short_circuits() {
	...
	local output exit_code=0
	output=$("$HELPER_SCRIPT" is-assigned 100 Aditya060806/Maestro runner1 2>/dev/null) || exit_code=$?
