...
unlocked_result="$(MAESTRO_VAULT_MESSAGE_UNLOCKED=1 "$MESSAGE_HELPER" receive --vault-dir "$recipient_dir" --repo "$transport_repo")"
if [[ "$unlocked_result" != *'"locked": false'* || "$unlocked_result" != *'"decrypted": 1'* ]]; then
	printf '%s\n' "unlocked receive did not decrypt exactly one message" >&2
	exit 1
fi

if ! MAESTRO_VAULT_MESSAGE_UNLOCKED=1 "$MESSAGE_HELPER" receive --vault-dir "$recipient_dir" --repo "$transport_repo" >/dev/null; then
