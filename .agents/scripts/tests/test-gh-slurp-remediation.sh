...
UPDATE_CHECK_SCRIPT="${AGENTS_SCRIPTS}/maestro-update-check.sh"
...
setup_offer_output=$(bash -c "
	...
	uname() { printf 'Linux\n'; return 0; }
	maestro_gh_slurp_supported() { return 1; }
	MAESTRO_GH_MIN_SLURP_VERSION=2.51.0
...
setup_yes_output=$(bash -c "
	...
	install_packages() { printf 'INSTALL:%s\n' \"\$*\"; return 0; }
	uname() { printf 'Linux\n'; return 0; }
	maestro_gh_slurp_supported() { return 0; }
	MAESTRO_GH_MIN_SLURP_VERSION=2.51.0
	source '$TOOL_INSTALL_SCRIPT'
	set +eE
	trap - ERR
...
setup_git_clis_output=$(bash -c "
	...
	maestro_gh_slurp_status_message() { printf 'GitHub CLI (gh) is too old\n'; return 0; }
	maestro_gh_slurp_supported() { [[ \"\$slurp_supported\" == 'true' ]]; return \$?; }
	MAESTRO_GH_MIN_SLURP_VERSION=2.51.0
	source '$TOOL_INSTALL_SCRIPT'
	set +eE
	trap - ERR
