...
_rest_issue_create \
	...
	--assignee "Aditya060806,otheruser" >/dev/null 2>&1 || true
...
assignee_main=$(grep -c 'assignees\[\]=Aditya060806' "$GH_CALLS" 2>/dev/null || printf '0')
...
" 2>&1 || true
	...
	zsh_assignee_main=$(grep -c 'assignees\[\]=Aditya060806' "$ZSH_CALLS" 2>/dev/null || printf '0')
