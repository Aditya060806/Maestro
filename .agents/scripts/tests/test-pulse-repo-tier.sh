...
setup_test_env() {
	...
	mkdir -p "${HOME}/.maestro/logs" "${HOME}/.maestro/cache" "${HOME}/.config/maestro"
	export LOGFILE="${HOME}/.maestro/logs/pulse.log"
	export PULSE_TIER_CACHE_FILE="${HOME}/.maestro/cache/pulse-repo-tiers.json"
	export PULSE_TIER_LAST_CHECK_FILE="${HOME}/.maestro/logs/pulse-tier-last-check.json"
...
test_tier_skip_disabled() {
	...
	local state_file="${HOME}/.maestro/logs/pulse-tier-last-check.json"
...
test_tier_skip_hot_no_skip() {
	...
	local cache_file="${HOME}/.maestro/cache/pulse-repo-tiers.json"
	local now
	now=$(date +%s)
	printf '{"owner/repo": {"tier": "hot", "event_count": 50, "ts": %d}}\n' "$now" >"$cache_file"
...
test_update_tier_timestamp_writes_epoch() {
	...
	local state_file="${HOME}/.maestro/logs/pulse-tier-ts-test.json"
...
test_update_tier_timestamp_creates_file() {
	...
	local state_file="${HOME}/.maestro/logs/tier-create-test.json"
	rm -f "$state_file"
...
test_update_tier_timestamp_independent_per_repo() {
	...
	local state_file="${HOME}/.maestro/logs/tier-indep-test.json"
