...
test_orchestration_interval_wins() {
	setup_home
	printf '%s\n' '{"orchestration":{"pulse_interval_seconds":240},"supervisor":{"pulse_interval_seconds":120}}' >"$HOME/.config/maestro/settings.json"
	local output=""
...
test_legacy_supervisor_interval_fallback() {
	setup_home
	printf '%s\n' '{"supervisor":{"pulse_interval_seconds":120}}' >"$HOME/.config/maestro/settings.json"
	local output=""
	output=$(_read_pulse_interval_seconds)
	teardown_home
