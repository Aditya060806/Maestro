...
assert_function_returns() {
	...
	local label="$2"
	local actual_rc=0
	_pr_required_checks_pass "19023" "Aditya060806/Maestro" || actual_rc=$?
...
assert_passing_check_returns() {
	...
	local label="$2"
	local actual_rc=0
	_check_required_checks_passing "Aditya060806/Maestro" "19023" || actual_rc=$?
	if [[ "$actual_rc" -eq "$expected_rc" ]]; then
		print_result "$label" 0
	else
		print_result "$label" 1 "Expected rc=$expected_rc, got rc=$actual_rc"
...
assert_pr_checks_fallback_returns() {
	...
	local actual_rc=0
	local actual_output=""
	actual_output=$(_check_required_pr_checks_passing_fallback "Aditya060806/Maestro" "19023") || actual_rc=$?
...
assert_ruleset_review_gate_returns() {
	...
	local label="$2"
	local actual_rc=0
	_check_ruleset_required_reviews_passing "Aditya060806/Maestro" "19023" "author" || actual_rc=$?
	if [[ "$actual_rc" -eq "$expected_rc" ]]; then
		print_result "$label" 0
	else
		print_result "$label" 1 "Expected rc=$expected_rc, got rc=$actual_rc"
...
assert_ruleset_review_count_output() {
	...
	local actual_output=""
	actual_output=$(_ruleset_required_review_count_for_default_branch "Aditya060806/Maestro" "main" "$rulesets_json") || actual_output="ERR"
...
assert_ruleset_contexts_output() {
	...
	local label="$2"
	local actual_output=""
	actual_output=$(_required_contexts_from_rulesets_for_default_branch "Aditya060806/Maestro" "main") || actual_output="ERR"
	if [[ "$actual_output" == "$expected_output" ]]; then
		print_result "$label" 0
	else
		print_result "$label" 1 "Expected output='$expected_output', got output='$actual_output'"
...
test_ruleset_review_count_accepts_prefetched_rulesets_json() {
	...
	assert_gh_call_absent "api repos/Aditya060806/Maestro/rulesets" \
