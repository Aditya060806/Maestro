...
write_stub_gh '{"state":"OPEN","assignees":[{"login":"Aditya060806"}],"labels":[{"name":"no-auto-dispatch"},{"name":"origin:interactive"},{"name":"status:available"},{"name":"tier:standard"}]}'
...
run_is_assigned 99897 "owner/repo" "Aditya060806"
if [[ "$rc" -eq 0 && "$output" == *"NO_AUTO_DISPATCH_BLOCKED"* ]]; then
