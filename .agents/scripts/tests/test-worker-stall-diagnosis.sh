...
setup_test_env() {
	...
	mkdir -p "${HOME}/.maestro/.agent-workspace/tmp/worker-idle-tracking"
...
conn.execute(
    ...
    ("session-1", "project-1", None, "session-1", "/tmp/maestro", title, "0", now - 600, now),
)
conn.execute(
...
test_recent_message_clears_stall() {
	reset_fixture
	local cmd='opencode run --title "Issue #4136: Diagnose stalls" --dir /tmp/maestro "/full-loop Implement issue #4136"'
	local pid="4101"
	local now
	now=$(date +%s)
...
test_thrashing_guard_detects_zero_commit_high_message() {
	...
	local cmd='opencode run --title "Issue #4187: thrash" --dir /tmp/maestro "/full-loop Implement issue #4187"'
...
test_extract_session_title_handles_unquoted_multiword_titles() {
	reset_fixture
	local cmd='opencode run --dir /tmp/maestro --title Issue #999: Multi Word Session Title --format json /full-loop "Implement issue #999"'
	local extracted
...
test_extract_provider_from_cmd_anthropic() {
	reset_fixture
	local cmd='opencode run --model anthropic/claude-sonnet-4-6 --title "Issue #5650: Fix watchdog" --dir /tmp/maestro "/full-loop Implement issue #5650"'
	local provider
...
test_extract_provider_from_cmd_openai() {
	reset_fixture
	local cmd='opencode run --model openai/gpt-4o --title "Issue #5650: Fix watchdog" --dir /tmp/maestro "/full-loop Implement issue #5650"'
	local provider
	provider=$(extract_provider_from_cmd "$cmd")
...
test_extract_provider_from_cmd_no_model() {
	reset_fixture
	local cmd='opencode run --title "Issue #5650: Fix watchdog" --dir /tmp/maestro "/full-loop Implement issue #5650"'
	local provider
	provider=$(extract_provider_from_cmd "$cmd")
