...
#   5. deploy_maestro_agents postcondition: returns 1 when scripts/ missing
...
# Test 5: deploy_maestro_agents postcondition check — if scripts/ is absent
...
test_postcondition_fails_when_scripts_absent() {
	...
	# Set up the minimum vars deploy_maestro_agents needs.
	...
	local rc=0
	HOME="${TEST_DIR}" INSTALL_DIR="$src" deploy_maestro_agents || rc=$?
...
test_no_change_corrupt_live_scripts_reserved_namespace_recovers() {
	...
	local target="${TEST_DIR}/.maestro/agents"
	local plugins_file="${TEST_DIR}/.config/maestro/plugins.json"
	local sha="abc123456789"

	mkdir -p "$repo/.agents/scripts" "$target" "$(dirname "$plugins_file")" "${TEST_DIR}/.maestro"
	printf 'canonical script\n' >"$repo/.agents/scripts/hello.sh"
	printf '%s\n' "$sha" >"${TEST_DIR}/.maestro/.deployed-sha"
	...
	local rc=0
	HOME="${TEST_DIR}" INSTALL_DIR="$repo" MAESTRO_AGENT_DEPLOY_MIN_FILES=1 deploy_maestro_agents || rc=$?
...
test_rsync_copy_uses_io_timeout() {
	...
	local rc=0
	MAESTRO_RSYNC_TIMEOUT=7 _deploy_agents_copy "$src" "$tgt" || rc=$?
...
test_stale_core_plugin_blocks_deployed_sha_stamp() {
	...
	local target="${TEST_DIR}/.maestro/agents"
	local source_plugin="$repo/.agents/plugins/opencode-maestro/model-limits.mjs"
	local target_plugin="$target/plugins/opencode-maestro/model-limits.mjs"
	local sha="fresh-plugin-sha"

	mkdir -p "$repo/.agents/scripts" "$(dirname "$source_plugin")" "$target/scripts" "$(dirname "$target_plugin")"
	rm -f "${TEST_DIR}/.maestro/.deployed-sha"
	printf 'canonical script\n' >"$repo/.agents/scripts/hello.sh"
	...
	_atomic_stage_and_deploy_agents() {
		local _src="$1"
		local _tgt="$2"
		mkdir -p "$_tgt/scripts" "$_tgt/plugins/opencode-maestro"
		printf 'canonical script\n' >"$_tgt/scripts/hello.sh"
		printf 'export const MODEL_LIMITS = { fresh: false };\n' >"$_tgt/plugins/opencode-maestro/model-limits.mjs"
		return 0
	}
	_deploy_agents_post_copy() { return 0; }
	_warn_deployed_script_drift() { return 0; }
	...
	if [[ "$rc" -ne 0 && ! -f "${TEST_DIR}/.maestro/.deployed-sha" ]]; then
		...
		print_result "stale core plugin: deploy fails before deployed-sha stamp" 1 \
			"rc=$rc, deployed-sha present=$([[ -f "${TEST_DIR}/.maestro/.deployed-sha" ]] && printf yes || printf no)"
	fi
	return 0
}
