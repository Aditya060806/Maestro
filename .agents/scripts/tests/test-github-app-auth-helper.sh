...
export MAESTRO_GITHUB_APP_CONFIG="$TMP/github-app-auth.json"
export MAESTRO_GITHUB_APP_CACHE_DIR="$TMP/cache"
export MAESTRO_GITHUB_APP_RATE_LIMIT_CACHE_TTL=30
...
printf "%s\n%s\n%s\n" "$MAESTRO_GITHUB_APP_HOME" "$MAESTRO_GITHUB_APP_CONFIG" "$MAESTRO_GITHUB_APP_CACHE_DIR"
...
assert_eq "unset HOME uses user-scoped tmp fallback root" "$TMP/fallback-tmp/maestro-fallback-user" "$fallback_home"
assert_eq "unset HOME config stays under user-scoped fallback" "$TMP/fallback-tmp/maestro-fallback-user/.config/maestro/github-app-auth.json" "$fallback_config"
assert_eq "unset HOME cache stays under user-scoped fallback" "$TMP/fallback-tmp/maestro-fallback-user/.maestro/cache/github-app" "$fallback_cache"
mkdir -p "$TMP/fallback-tmp"
ln -s "$TMP" "$TMP/fallback-tmp/maestro-linked-user"
# shellcheck disable=SC2016 # Keep the inner script expanded by the env-isolated bash process.
...
set -euo pipefail
source "$1"
unset MAESTRO_GITHUB_APP_CACHE_DIR MAESTRO_GITHUB_APP_HOME HOME
mkdir() {
	printf "mkdir called\n" >"$MKDIR_LOG"
	return 1
}
...
cat >"$MAESTRO_GITHUB_APP_CONFIG" <<'JSON'
{
  "enabled": true,
  "app_id": "123",
  "installation_id": "456",
