...
# MAESTRO_AGENTS_DIR to point there. The mock simply sleeps so it produces
...
#   13. MAESTRO_SKIP_PULSE_RESTART=1 skips restart-if-running
#   14. MAESTRO_SKIP_PULSE_RESTART=1 skips restart
...
SH
	...
	export MAESTRO_AGENTS_DIR="$TEST_ROOT"
	...
	export MAESTRO_PULSE_PROCESS_PATTERN="${_escaped_root}/scripts/pulse-wrapper\\.sh"

	# Ensure env overrides don't leak between tests.
	unset MAESTRO_SKIP_PULSE_RESTART 2>/dev/null || true
	# Shorten wait windows so tests finish quickly.
	export MAESTRO_PULSE_RESTART_WAIT=0
	export MAESTRO_PULSE_SIGTERM_WAIT=1
}
...
test_skip_env_honoured_in_restart_if_running() {
	...
	local first_pid
	first_pid=$(pgrep -f "${TEST_ROOT}/scripts/pulse-wrapper.sh" | head -1)
	MAESTRO_SKIP_PULSE_RESTART=1 "$HELPER" restart-if-running >/dev/null 2>&1 || true
	sleep 0.5
	local second_pid
	second_pid=$(pgrep -f "${TEST_ROOT}/scripts/pulse-wrapper.sh" | head -1)
	_assert_eq "MAESTRO_SKIP_PULSE_RESTART=1 preserves PID (restart-if-running)" \
...
test_skip_env_honoured_in_restart() {
	...
	local first_pid
	first_pid=$(pgrep -f "${TEST_ROOT}/scripts/pulse-wrapper.sh" | head -1)
	MAESTRO_SKIP_PULSE_RESTART=1 "$HELPER" restart >/dev/null 2>&1 || true
	sleep 0.5
	local second_pid
	second_pid=$(pgrep -f "${TEST_ROOT}/scripts/pulse-wrapper.sh" | head -1)
	_assert_eq "MAESTRO_SKIP_PULSE_RESTART=1 preserves PID (restart)" \
		"$first_pid" "$second_pid"
	_kill_mocks
	return 0
}
...
# MAESTRO_PULSE_EXPECTED_MAX_INSTANCES (default 3).
...
test_status_legacy_strict_threshold_via_env() {
	...
	local rc=0
	local out
	out=$(MAESTRO_PULSE_EXPECTED_MAX_INSTANCES=1 "$HELPER" status 2>&1) || rc=$?
...
test_status_accepts_leading_zero_threshold_env() {
	...
	local rc=0
	local out
	out=$(MAESTRO_PULSE_EXPECTED_MAX_INSTANCES=08 "$HELPER" status 2>&1) || rc=$?
