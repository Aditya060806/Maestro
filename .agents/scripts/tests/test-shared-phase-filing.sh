...
#   2. Feature flag guard: no-op when MAESTRO_SEQUENTIAL_PHASE_AUTOFILE=0
...
export MAESTRO_SEQUENTIAL_PHASE_AUTOFILE=0
...
if printf '%s' "$child_body" | grep -q 'maestro:generator=phase-autofile parent=100 phase=2'; then
...
- **Phase 3 — CLI surface**: design `maestro project new|list|status|link|archive`.
...
export MAESTRO_PHASE_FILING_LOCK_ROOT="${TMP}/phase-locks"
...
export MAESTRO_SEQUENTIAL_PHASE_AUTOFILE=1
