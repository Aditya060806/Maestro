...
#   test_bypass_env_var                            — MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY=1 → exit 0
...
# Test 8: MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY=1 → exit 0 even for CLOSED issue
test_bypass_env_var() {
	setup_test_env
	...
	local rc=0
	MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY=1 \
		"$HELPER_SCRIPT" check 12345 "owner/repo" >/dev/null 2>&1 || rc=$?
