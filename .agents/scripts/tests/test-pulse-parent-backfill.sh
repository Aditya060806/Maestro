...
#   6. MAESTRO_PARENT_BACKFILL_INTERVAL_SECS env override is respected
#
...
# ---------------------------------------------------------------------------
# Test 6 — MAESTRO_PARENT_BACKFILL_INTERVAL_SECS controls gate threshold
...
if grep -q '_pbf_state_file=' "$ORCHESTRATOR" && \
	grep -q 'MAESTRO_PARENT_BACKFILL_INTERVAL_SECS' "$ORCHESTRATOR" && \
