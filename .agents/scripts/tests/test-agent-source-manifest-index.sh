...
setup() {
	...
	local agents_dir="$TEST_HOME/.maestro/agents"
	...
	cat >"$TEST_HOME/private-pack/.agents/agent-pack.json" <<'JSON'
{
  ...
  "outputs": [{"name": "report", "path": "~/.maestro/.agent-workspace/work/private-pack/reports/"}],
...
test_sync_generates_capability_registry() {
	local output=""
	local status=0
	output=$(MAESTRO_AGENTS_DIR="$TEST_HOME/.maestro/agents" HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/agent-sources-helper.sh" sync 2>&1)
	status=$?
	if [[ "$status" -ne 0 ]]; then
	...
	local registry="$TEST_HOME/.maestro/agents/agent-source-capabilities.toon"
...
test_subagent_index_includes_capability_registry() {
	local output=""
	local status=0
	output=$(MAESTRO_AGENTS_DIR="$TEST_HOME/.maestro/agents" HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/subagent-index-helper.sh" generate 2>&1)
	status=$?
	if [[ "$status" -ne 0 ]]; then
	...
	local index_file="$TEST_HOME/.maestro/agents/subagent-index.toon"
...
test_sync_resolves_relative_agents_dir() {
	local output=""
	local status=0
	output=$(cd "$TEST_HOME" && MAESTRO_AGENTS_DIR=".maestro/agents" HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/agent-sources-helper.sh" sync 2>&1)
	status=$?
	if [[ "$status" -ne 0 ]]; then
...
test_check_validates_capability_cardinality() {
	local output=""
	local status=0
	output=$(MAESTRO_AGENTS_DIR="$TEST_HOME/.maestro/agents" HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/subagent-index-helper.sh" check 2>&1)
	status=$?
...
test_sync_without_node_fails_open() {
	...
	local output=""
	local status=0
	output=$(PATH="$path_without_node" MAESTRO_AGENTS_DIR="$TEST_HOME/.maestro/agents" HOME="$TEST_HOME" "$TEST_HOME/.maestro/agents/scripts/agent-sources-helper.sh" sync 2>&1)
	status=$?
