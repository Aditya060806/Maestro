...
#      Uses MAESTRO_PULSE_LOCK_DIR to point at an isolated temp lockdir
...
# MAESTRO_PULSE_LOCK_DIR / MAESTRO_PULSE_PROCESS_PATTERN overrides.
...
SH
	...
	# real user lockdir.
	export MAESTRO_PULSE_LOCK_DIR="${TEST_ROOT}/lockdir"
...
test_status_lock_pid_matches_lockdir() {
	...
	printf '%s\n' "$_mock_pid" >"${MAESTRO_PULSE_LOCK_DIR}/pid"
	...
	_kill_mock
	rm -f "${MAESTRO_PULSE_LOCK_DIR}/pid" 2>/dev/null || true
	return 0
}
...
SH
	...
	local _old_pattern="$MAESTRO_PULSE_PROCESS_PATTERN"
	export MAESTRO_PULSE_PROCESS_PATTERN="${_escaped_t3}/scripts/pulse-wrapper\\.sh"
	...
	rm -rf "$_t3_root"
	export MAESTRO_PULSE_PROCESS_PATTERN="$_old_pattern"
	unset _parent_pid
	return 0
}
