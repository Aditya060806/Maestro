...
setup() {
	...
	local config_dir="$TEST_HOME/.config/maestro"
...
test_plugin_namespace_indexed() {
	local output=""
	local status=0
	output=$(HOME="$TEST_HOME" MAESTRO_AGENTS_DIR="$TEST_HOME/.maestro/agents" "$TEST_HOME/.maestro/agents/scripts/subagent-index-helper.sh" generate 2>&1)
	status=$?
	if [[ "$status" -ne 0 ]]; then
...
test_check_validates_plugin_cardinality() {
	local output=""
	local status=0
	output=$(HOME="$TEST_HOME" MAESTRO_AGENTS_DIR="$TEST_HOME/.maestro/agents" "$TEST_HOME/.maestro/agents/scripts/subagent-index-helper.sh" check 2>&1)
	status=$?
...
test_unset_globals_do_not_read_stdin() {
	local helper="$TEST_HOME/.maestro/agents/scripts/subagent-index-helper.sh"
	local output=""
	local status=0
