...
# ---------------------------------------------------------------------------
# Case 7: MAESTRO_HEADLESS_APPEND_CONTRACT=0 suppresses injection
# ---------------------------------------------------------------------------
test_opt_out_env_var() {
	local prompt="/full-loop Implement GH#1"
	local output
	output=$(MAESTRO_HEADLESS_APPEND_CONTRACT=0 append_worker_headless_contract "$prompt")

	if [[ "$output" == "$prompt" ]]; then
		_pass "opt_out_env_var"
	else
		_fail "opt_out_env_var" \
			"MAESTRO_HEADLESS_APPEND_CONTRACT=0 did not suppress injection"
	fi
	return 0
}
