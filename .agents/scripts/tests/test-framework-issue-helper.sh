...
if [[ "${1:-}" == "issue" && "${2:-}" == "create" ]]; then
	printf '%s\n' "${TEST_CREATED_URL:-https://github.com/Aditya060806/Maestro/issues/9001}"
	exit 0
fi
...
run_auto_dispatch_case() {
	...
	if TEST_DUPLICATE_VALUE="" \
		TEST_CREATED_URL="https://github.com/Aditya060806/Maestro/issues/9100" \
...
run_auto_dispatch_equals_case() {
	...
	if TEST_DUPLICATE_VALUE="" \
		TEST_CREATED_URL="https://github.com/Aditya060806/Maestro/issues/9101" \
		TEST_GH_TRACE="$trace_file" \
		PATH="${stub_dir}:$PATH" \
...
run_flag_value_case() {
	...
	if TEST_DUPLICATE_VALUE="" \
		TEST_CREATED_URL="https://github.com/Aditya060806/Maestro/issues/9103" \
		TEST_GH_TRACE="$trace_file" \
		PATH="${stub_dir}:$PATH" \
...
run_missing_value_case() {
	...
	if TEST_DUPLICATE_VALUE="" \
		TEST_CREATED_URL="https://github.com/Aditya060806/Maestro/issues/9102" \
		PATH="${stub_dir}:$PATH" \
...
run_case "[]" "https://github.com/Aditya060806/Maestro/issues/9001" "${TMP_DIR}" "$empty_output"
...
run_case "not-a-number" "https://github.com/Aditya060806/Maestro/issues/9002" "${TMP_DIR}" "$malformed_output"
...
run_case "12345" "https://github.com/Aditya060806/Maestro/issues/9003" "${TMP_DIR}" "$valid_output"
...
assert_contains "$valid_text" "issue_url=https://github.com/Aditya060806/Maestro/issues/12345" "valid duplicate result emits valid issue URL"

invalid_create_output="${TMP_DIR}/invalid-create.out"
if run_case "" "https://github.com/Aditya060806/Maestro/issues/[]" "${TMP_DIR}" "$invalid_create_output"; then
	...
	assert_not_contains "$invalid_create_text" "issue_url=https://github.com/Aditya060806/Maestro/issues/[]" "invalid created issue URL is not emitted"
fi
