...
#
#   1. maestro.sh::cmd_update — when local_hash == remote_hash (git already
...
MAESTRO_SH="$WORKTREE_ROOT/maestro.sh"
AUTO_UPDATE_SH="$WORKTREE_ROOT/.agents/scripts/auto-update-helper.sh"

# Test 1: maestro.sh cmd_update references .deployed-sha in the VERSION-match branch
if grep -q '\.deployed-sha' "$MAESTRO_SH" &&
	grep -q 't2706' "$MAESTRO_SH"; then
	print_result "maestro.sh cmd_update has stamp-drift check (t2706 marker)" 0
else
	print_result "maestro.sh cmd_update has stamp-drift check (t2706 marker)" 1 \
		"(expected .deployed-sha reference and t2706 marker)"
fi

# Test 2: maestro.sh filters for framework code paths (skips docs-only drift)
if grep -q '\.agents/scripts/' "$MAESTRO_SH" &&
	grep -q 'has_code_drift' "$MAESTRO_SH"; then
	print_result "maestro.sh filters for framework code paths" 0
else
	print_result "maestro.sh filters for framework code paths" 1 \
...
SANDBOX_REPO="${TEST_ROOT}/maestro"
SANDBOX_MAESTRO="${SANDBOX_HOME}/.maestro"
SETUP_CALLS_LOG="${TEST_ROOT}/setup_calls.log"

mkdir -p "$SANDBOX_HOME" "$SANDBOX_MAESTRO/agents"
...
# Add an maestro.sh-only drift commit (per Gemini feedback on PR #20342:
# maestro.sh is the deployed CLI entry point, drift must trigger redeploy).
printf '#!/usr/bin/env bash\necho maestro v2\n' >"$SANDBOX_REPO/maestro.sh"
git -C "$SANDBOX_REPO" add -A
git -C "$SANDBOX_REPO" commit -qm "update maestro.sh" 2>/dev/null
HEAD_WITH_MAESTRO_DRIFT_SHA=$(git -C "$SANDBOX_REPO" rev-parse HEAD)

if [[ -z "$DEPLOYED_SHA" || -z "$HEAD_SHA" || -z "$HEAD_WITH_DOCS_SHA" ||
	-z "$HEAD_WITH_SETUP_DRIFT_SHA" || -z "$HEAD_WITH_MAESTRO_DRIFT_SHA" ]]; then
...
printf 'v1\n' >"$SANDBOX_MAESTRO/agents/VERSION"

# ---------------------------------------------------------------------------
# run_cmd_update_stamp_branch: extract just the stamp-check block from
# maestro.sh cmd_update and execute it against the sandbox state.
#
...
run_cmd_update_stamp_branch() {
	...
	if [[ -n "$stamp_content" ]]; then
		printf '%s\n' "$stamp_content" >"$SANDBOX_MAESTRO/.deployed-sha"
	else
		rm -f "$SANDBOX_MAESTRO/.deployed-sha"
	fi

	# Inline stamp check mirroring the t2706 addition in maestro.sh cmd_update.
...
# Test 11: stamp lags HEAD with maestro.sh drift → setup.sh fires
# (Gemini PR #20342 feedback: maestro.sh is deployed, drift must trigger redeploy.)
rm -f "$SETUP_CALLS_LOG"
run_cmd_update_stamp_branch "$HEAD_WITH_SETUP_DRIFT_SHA" "$HEAD_WITH_MAESTRO_DRIFT_SHA"
if [[ "$(count_setup_calls)" -eq 1 ]]; then
	print_result "setup.sh fires when maestro.sh drifts" 0
else
	print_result "setup.sh fires when maestro.sh drifts" 1 \
		"(expected 1 call, got $(count_setup_calls))"
fi
