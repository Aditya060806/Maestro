...
[[ -f "$HOME/.maestro/.opencode-bin-resolved" ]] && resolved6=$(cat "$HOME/.maestro/.opencode-bin-resolved") || resolved6="MISSING"
...
rm -f "$HOME/.maestro/.opencode-bin-resolved" "$SANDBOX/bin/opencode"
...
rm -f "$HOME/.maestro/.opencode-bin-resolved" "$SANDBOX/bin/opencode" "$SANDBOX/bin/bun"
cat >"$SANDBOX/bin/npm" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
