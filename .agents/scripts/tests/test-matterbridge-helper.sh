...
setup() {
	...
	mkdir -p "$TEST_DIR/.maestro/.agent-workspace/matterbridge"
	return 0
}
...
test_simplex_bridge_init() {
	...
	local compose_path="$TEST_DIR/.maestro/.agent-workspace/matterbridge/simplex-bridge/docker-compose.yml"
	...
	local config_path="$TEST_DIR/.maestro/.agent-workspace/matterbridge/simplex-bridge/matterbridge.toml"
	...
	local data_dir="$TEST_DIR/.maestro/.agent-workspace/matterbridge/simplex-bridge/data/simplex"
	if [[ -d "$data_dir" ]]; then
