...
test_large_file_premise_falsified() {
	...
	printf '<!-- maestro:generator=large-file-simplification-gate cited_file=.agents/scripts/big-file.sh threshold=100 -->\n## What\nSimplify big-file.sh\n' >"$body_file"
	...
	local rc=0
	"$HELPER_SCRIPT" validate "100" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_large_file_premise_holds() {
	...
	local rc=0
	"$HELPER_SCRIPT" validate "101" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_large_file_deleted() {
	...
	local body_file="${TEST_ROOT}/issue_body.txt"
	printf '<!-- maestro:generator=large-file-simplification-gate cited_file=.agents/scripts/deleted-file.sh threshold=100 -->\n## What\nSimplify deleted-file.sh\n' >"$body_file"
	create_gh_stub_with_body_file "$body_file"
	...
	local rc=0
	"$HELPER_SCRIPT" validate "102" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_agent_doc_premise_falsified() {
	...
	local body_file="${TEST_ROOT}/issue_body.txt"
	printf '<!-- maestro:generator=agent-doc-simplification-gate cited_file=.agents/reference/guide.md threshold=500 -->\n## Agent doc simplification\n' >"$body_file"
	create_gh_stub_with_body_file "$body_file"
	...
	local rc=0
	"$HELPER_SCRIPT" validate "106" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_agent_doc_premise_holds() {
	...
	local rc=0
	"$HELPER_SCRIPT" validate "107" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_function_complexity_falsified() {
	...
	local body_file="${TEST_ROOT}/issue_body.txt"
	printf '<!-- maestro:generator=function-complexity-gate cited_file=.agents/scripts/simple.sh threshold=50 -->\n## Complexity finding\n' >"$body_file"
	create_gh_stub_with_body_file "$body_file"
...
'
	...
	local rc=0
	"$HELPER_SCRIPT" validate "103" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_function_complexity_holds() {
	...
	local body_file="${TEST_ROOT}/issue_body.txt"
	printf '<!-- maestro:generator=function-complexity-gate cited_file=.agents/scripts/complex.sh threshold=5 -->\n## Complexity finding\n' >"$body_file"
	create_gh_stub_with_body_file "$body_file"
...
'
	...
	local rc=0
	"$HELPER_SCRIPT" validate "104" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_function_marker_markdown_uses_file_lines() {
	...
	local body_file="${TEST_ROOT}/issue_body.txt"
	printf '<!-- maestro:generator=function-complexity-gate cited_file=.agents/maestro/knowledge-plane.md threshold=500 -->\n## Agent doc simplification\n**Current size:** 1029 lines\n' >"$body_file"
	create_gh_stub_with_body_file "$body_file"
	...
	create_git_stub_with_file "success" ".agents/maestro/knowledge-plane.md" "$(printf '%b' "$file_content")"

	local comment_capture="${TEST_ROOT}/comment.txt"
	export GH_STUB_COMMENT_CAPTURE="$comment_capture"
	local rc=0
	"$HELPER_SCRIPT" validate "108" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
...
test_missing_attributes() {
	...
	local body_file="${TEST_ROOT}/issue_body.txt"
	printf '<!-- maestro:generator=large-file-simplification-gate -->\n## What\nSimplify something\n' >"$body_file"
	create_gh_stub_with_body_file "$body_file"

	local rc=0
	"$HELPER_SCRIPT" validate "105" "Aditya060806/Maestro" >/dev/null 2>&1 || rc=$?
