...
#   (3) honours MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE=1 as a no-op
...
PULSE_LOG="${HOME}/.maestro/logs/pulse.log"
export PULSE_LOG
{
	echo "[2026-04-30 00:00:00] [dispatch_with_dedup] (t2117) Dispatch deferred for #21840 in Aditya060806/Maestro: FOOTPRINT_OVERLAP"
...
OUT=$("$META_FILER" file \
	--issue 21840 --repo Aditya060806/Maestro \
...
# =============================================================================
# Test 1b: Public app repos without framework sources route metas to maestro
# =============================================================================
reset_stubs
...
if printf '%s' "$OUT" | tail -1 | grep -q 'github.com/Aditya060806/Maestro/issues/99999'; then
	print_result "cross-repo: files meta in framework repo" 0
else
	print_result "cross-repo: files meta in framework repo" 1 "stdout: $OUT"
fi

if grep -qE 'issue create --repo Aditya060806/Maestro' "$STUB_LOG"; then
...
if grep -qE 'issue edit 4003 --repo exampleorg/examplerepo --add-label blocked-by:Aditya060806/Maestro#99999' "$STUB_LOG"; then
...
if grep -q 'circuit-breaker-meta-filed:Aditya060806/Maestro#99999' "$STUB_LOG"; then
...
printf 'false' >"$FIXTURE_REPO_PRIVATE"
cat >"$FIXTURE_COMMENTS_JSON" <<'EOF'
[{"body":"<!-- circuit-breaker-meta-filed:Aditya060806/Maestro#51515 -->\n## prior cross-repo trip"}]
EOF
...
if printf '%s' "$OUT" | tail -1 | grep -q 'github.com/Aditya060806/Maestro/issues/51515'; then
...
MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE=1 "$META_FILER" file \
	--issue 21840 --repo Aditya060806/Maestro \
...
"$META_FILER" file --repo Aditya060806/Maestro --breaker cost \
...
if grep -qE 'issue edit 4003 --repo exampleorg/examplerepo --remove-label blocked-by:Aditya060806/Maestro#99999' "$STUB_LOG"; then
...
"$META_FILER" unblock-on-merge \
	--meta 99999 --repo Aditya060806/Maestro >/dev/null 2>&1
RC=$?
