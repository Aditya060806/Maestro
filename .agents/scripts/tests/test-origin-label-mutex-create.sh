...
# two.
#
# Failure motivating this test: Aditya060806/Maestro#21862 (PR #21825
...
: >"$GH_ARGV_RECORD_FILE"
MAESTRO_PR_CREATE_READY=1 SESSION_ORIGIN_OVERRIDE="origin:interactive" \
	gh_create_pr --repo o/r --title "t1: x" --body "Resolves #1" >/dev/null 2>&1
if grep -qx '<--draft>' "$GH_ARGV_RECORD_FILE"; then
	print_result "B8: MAESTRO_PR_CREATE_READY leaves interactive PR non-draft" 1 \
		"unexpected --draft in argv: $(tr '\n' ' ' <"$GH_ARGV_RECORD_FILE")"
else
	print_result "B8: MAESTRO_PR_CREATE_READY leaves interactive PR non-draft" 0
fi
unset MAESTRO_PR_CREATE_READY GH_ARGV_RECORD_FILE
