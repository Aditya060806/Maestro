...
# FULL_LOOP_HEADLESS / MAESTRO_HEADLESS / OPENCODE_HEADLESS / GITHUB_ACTIONS
...
unset FULL_LOOP_HEADLESS MAESTRO_HEADLESS OPENCODE_HEADLESS GITHUB_ACTIONS
...
# helper at ~/.maestro/ is not found. The _auto_assign_issue lookup prefers
# ${HOME}/.maestro/agents/scripts/ over ${SCRIPT_DIR}/; overriding HOME
# to FAKE_HOME ensures the stub in FAKE_HOME is the one found.
ISC_HELPER="${FAKE_HOME}/.maestro/agents/scripts/interactive-session-helper.sh"
...
if [[ "$cmd" == "write-stamp" && -n "$issue" && -n "$slug" ]]; then
	...
	flat_slug="${slug//\//-}"
	flat_slug="${flat_slug//./-}"
	stamp_dir="${CLAIM_STAMP_DIR:-${HOME}/.maestro/.agent-workspace/interactive-claims}"
