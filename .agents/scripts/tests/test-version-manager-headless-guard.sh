...
reset_guard_env() {
	unset MAESTRO_HEADLESS FULL_LOOP_HEADLESS OPENCODE_HEADLESS HEADLESS
	unset WORKER_TASK_NUMBER WORKER_ISSUE_NUMBER WORKER_SESSION_KEY MAESTRO_SESSION_KEY
	unset MAESTRO_RELEASE_CONTEXT_APPROVED VERSION_MANAGER_RELEASE_CONTEXT_APPROVED MAESTRO_TASK_SCOPE
	unset MAESTRO_SESSION_TITLE WORKER_SESSION_TITLE
	return 0
}
...
reset_guard_env
export MAESTRO_HEADLESS=false WORKER_ISSUE_NUMBER=24089
rc=0
_version_manager_is_headless_task_worker >/dev/null 2>&1 || rc=$?
if [[ "$rc" -ne 0 ]]; then
	print_result 'headless guard rejects false marker strings' 0
else
	print_result 'headless guard rejects false marker strings' 1 'expected non-zero rc for MAESTRO_HEADLESS=false'
fi

reset_guard_env
export MAESTRO_HEADLESS=1 WORKER_TASK_NUMBER=123
rc=0
_version_manager_is_headless_task_worker >/dev/null 2>&1 || rc=$?
if [[ "$rc" -eq 0 ]]; then
...
reset_guard_env
export MAESTRO_HEADLESS=false FULL_LOOP_HEADLESS=0 OPENCODE_HEADLESS=True WORKER_ISSUE_NUMBER=24089
rc=0
_version_manager_is_headless_task_worker >/dev/null 2>&1 || rc=$?
if [[ "$rc" -eq 0 ]]; then
...
reset_guard_env
export MAESTRO_HEADLESS=1 WORKER_TASK_NUMBER=123 WORKER_SESSION_KEY='task-123'
...
REPO_ROOT="${TMPDIR:-/tmp}/maestro-version-manager-missing-repo-root-$$"
...
reset_guard_env
original_repo_root="$REPO_ROOT"
REPO_ROOT="${TMPDIR:-/tmp}/maestro-version-manager-missing-repo-root-arg-$$"
rc=0
...
reset_guard_env
export MAESTRO_SESSION_KEY='RELEASE-20260525'
rc=0
...
reset_guard_env
export MAESTRO_HEADLESS=1 WORKER_ISSUE_NUMBER=24089 WORKER_SESSION_KEY='issue-24089' MAESTRO_SESSION_TITLE='ordinary issue mentioning release cleanup'
rc=0
