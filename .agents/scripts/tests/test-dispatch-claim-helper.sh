...
export MAESTRO_TEST_MODE=1
export MAESTRO_REPO_STATE_GUARD_TEST_BYPASS=1
...
EOF
	...
	if grep -q '/tmp/maestro-claim-post-error' "$CLAIM_HELPER"; then
	...
	if [[ "$chmod_mode" == "600" && "$chmod_path" == "${test_home}/.maestro-claim-post-error."* ]]; then
	...
	if [[ "$chmod_path" == *"//.maestro-claim-post-error."* ]]; then
...
test_claim_rejects_stale_same_runner_claim() {
	...
	output=$(PATH="${mock_path}:$PATH" \
		...
		MOCK_OLD_CLAIM_RUNNER="Aditya060806" \
		DISPATCH_CLAIM_WINDOW=0 \
		DISPATCH_CLAIM_SELF_RECLAIM_AGE=30 \
		"$CLAIM_HELPER" claim 42 owner/repo Aditya060806 2>&1)
	exit_code=$?
	set -e
...
test_claim_marks_stale_worker_takeover() {
	...
	if printf '%s' "$post_body" | grep -q 'version=[0-9][0-9.]*'; then
		print_result "claim includes maestro version" 0
	else
		print_result "claim includes maestro version" 1 "body: ${post_body:-none}"
	fi
