...
export MAESTRO_IDENTITY_ALIASES_CONF="${TMP}/identity-aliases.conf"
mkdir -p "${HOME}/.maestro/logs"
: >"$LOGFILE"

printf '%s\n' 'canonical-operator=local-user,github-user' >"$MAESTRO_IDENTITY_ALIASES_CONF"
...
gh() {
	...
	case "${HEALTH_FIXTURE:-}:$call" in
		...
		conflicting_operator:*"issue list --repo owner/repo --label source:health-dashboard"*)
			printf '%s' '[{"number":4643,"title":"[Supervisor:Aditya060806] stale dashboard","labels":[{"name":"source:health-dashboard"},{"name":"operator:alex-solovyev"},{"name":"alex-solovyev"},{"name":"supervisor"}]}]'
			return 0
			;;
		conflicting_operator:*"issue list --repo owner/repo --search in:title [Supervisor:Aditya060806]"*)
			printf '%s' '[{"number":4643,"title":"[Supervisor:Aditya060806] stale dashboard","labels":[{"name":"source:health-dashboard"},{"name":"operator:alex-solovyev"},{"name":"alex-solovyev"},{"name":"supervisor"}]}]'
			return 0
			;;
...
result=$(_find_health_issue "owner/repo" "github-user" "supervisor" "[Supervisor:canonical-operator]" \
	"supervisor" "Supervisor" "${HOME}/.maestro/logs/health-issue-canonical-owner-repo" \
	"$canonical" "$aliases")
...
result=$(_find_health_issue \
	"owner/repo" "Aditya060806" "supervisor" "[Supervisor:Aditya060806]" \
	"supervisor" "Supervisor" "${HOME}/.maestro/logs/health-issue-Aditya060806-owner-repo" \
	"Aditya060806" "Aditya060806")
...
result=$(_find_health_issue \
	...
	"supervisor" "Supervisor" "${HOME}/.maestro/logs/health-issue-legacy-owner-repo" \
	"canonical-operator" "$aliases")
unset HEALTH_FIXTURE
...
cache_file="${HOME}/.maestro/logs/health-issue-cache-conflict-owner-repo"
printf '%s\n' '4643' >"$cache_file"
: >"$GH_CALLS"
...
cache_file="${HOME}/.maestro/logs/health-issue-cache-open-owner-repo"
printf '%s\n' '4644' >"$cache_file"
: >"$GH_CALLS"
...
cache_file="${HOME}/.maestro/logs/health-issue-cache-closed-owner-repo"
printf '%s\n' '4645' >"$cache_file"
: >"$GH_CALLS"
...
missing_cache_file="${HOME}/.maestro/logs/health-issue-missing-owner-repo"
