...
setup_sandbox() {
	...
	mkdir -p \
		"${HOME}/.maestro/logs" \
		"${HOME}/.maestro/.agent-workspace/supervisor" \
		"${HOME}/.maestro/.agent-workspace/tmp/triage-cache" \
		"${HOME}/.config/maestro"
...
test_credentials_sourcing_warnings() {
	...
	mkdir -p \
		"${local_home}/.maestro/logs" \
		"${local_home}/.maestro/.agent-workspace/supervisor" \
		"${local_home}/.maestro/.agent-workspace/tmp/triage-cache" \
		"${local_home}/.config/maestro"

	local credentials_file="${local_home}/.config/maestro/credentials.sh"
...
test_ff_key_format() {
	local actual
	actual=$(_ff_key "123" "Aditya060806/Maestro")
	assert_equals "_ff_key simple slug" "Aditya060806/Maestro/123" "$actual"
