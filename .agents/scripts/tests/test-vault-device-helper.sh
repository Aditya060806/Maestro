...
TEST_ROOT="$(mktemp -d 2>/dev/null || mktemp -d -t maestro-vault-device-test)"
PASS=0
FAIL=0
...
export MAESTRO_VAULT_DEVICE_DIR="$TEST_ROOT/devices"
export MAESTRO_VAULT_DEVICE_STALE_SECONDS="1"
...
if [[ -s "$MAESTRO_VAULT_DEVICE_DIR/revocation-tasks.jsonl" ]]; then
	pass "revocation queues rotation task"
else
	fail "revocation queues rotation task"
fi

rm -f "$MAESTRO_VAULT_DEVICE_DIR/registry.json"
