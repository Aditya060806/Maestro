...
#   - MAESTRO_SKIP_CANONICAL_GUARD=1 escape valve
...
setup_test_repo() {
	...
	git -C "$TEST_ROOT" config user.name "Maestro Test"
...
# =============================================================================
# Test 10: MAESTRO_SKIP_CANONICAL_GUARD=1 bypasses off-default-branch deny;
...
test_canonical_guard_skip_env_var() {
	...
	# With MAESTRO_SKIP_CANONICAL_GUARD=1: allowed (explicit escape valve)
	local output_skip=""
	output_skip=$(run_hook "$TEST_ROOT" "$json" "MAESTRO_SKIP_CANONICAL_GUARD=1") || true
	...
	if [[ -n "$output_skip" ]]; then
		print_result "canonical guard: MAESTRO_SKIP_CANONICAL_GUARD=1 bypasses deny (allow)" 1 "output=${output_skip}"
		passed=1
	fi
