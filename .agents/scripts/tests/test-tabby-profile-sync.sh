...
assert 'maestro opencode; exec zsh' in repaired, repaired
...
_info "Test 3: generated profiles use maestro OpenCode launcher"
run_python_test "generated profile uses maestro launcher" "${load_module_code}
scheme = {'name': 'Test', 'foreground': '#fff', 'background': '#000', 'cursor': '#fff', 'colors': ['#000', '#fff']}
profile = mod.build_profile_yaml('maestro', '/tmp/maestro', '#123456', scheme, 'group-1')
...
assert \"- 'maestro opencode; exec zsh'\" in profile, profile
...
_info "Test 7: split direct profiles are upgraded to maestro launcher"
...
if [[ "${sync_output}" == *"Repaired 1 existing Tabby profile(s)."* ]] && grep -q -- "maestro opencode; exec zsh" "${tabby_config}" && ! grep -q -- "TABBY_AUTORUN: opencode" "${tabby_config}"; then
