...
test_counts_plain_and_dot_prefixed_opencode_workers() {
	...
	set_ps_fixture "123 S 00:10 opencode run --dir /tmp/maestro --title Issue #4342 \"/full-loop Implement issue #4342\"
124 S 00:11 /Users/test/.opencode/bin/opencode run --dir /tmp/maestro --title Issue #4343 \"/full-loop Implement issue #4343\"
125 S 00:20 opencode run --dir /tmp/maestro --title Supervisor Pulse \"/pulse\"
126 S 00:05 opencode run --dir /tmp/maestro --session-key issue-4344 --title Issue #4344 \"/full-loop Implement issue #4344 -- fix /pulse-related bug\""
...
test_deduplicates_process_chain_to_one_logical_worker() {
	...
	set_ps_fixture "200 S 00:30 bash /home/user/.maestro/agents/scripts/sandbox-exec-helper.sh run --timeout 3600 --allow-secret-io -- /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5072\" --dir /tmp/maestro --title Issue #5072
201 S 00:30 node /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5072\" --dir /tmp/maestro --title Issue #5072
202 S 00:30 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #5072\" --dir /tmp/maestro --title Issue #5072"
...
test_deduplicates_multiple_workers_with_process_chains() {
	...
	set_ps_fixture "300 S 01:00 bash /home/user/.maestro/agents/scripts/sandbox-exec-helper.sh run --timeout 3600 --allow-secret-io -- /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5001\" --dir /tmp/maestro --title Issue #5001
301 S 01:00 node /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5001\" --dir /tmp/maestro --title Issue #5001
302 S 01:00 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #5001\" --dir /tmp/maestro --title Issue #5001
310 S 00:20 bash /home/user/.maestro/agents/scripts/sandbox-exec-helper.sh run --timeout 3600 --allow-secret-io -- /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5002\" --dir /tmp/maestro --title Issue #5002
311 S 00:20 node /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5002\" --dir /tmp/maestro --title Issue #5002
312 S 00:20 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #5002\" --dir /tmp/maestro --title Issue #5002"
...
test_repo_issue_detection_uses_filtered_worker_list() {
	set_ps_fixture "211 S 00:31 opencode run --dir /tmp/maestro --session-key issue-4342 --title Issue #4342: fix \"/full-loop Implement issue #4342\"
212 S 00:31 opencode run --dir /tmp/other --session-key issue-4342 --title Issue #4342: other \"/full-loop Implement issue #4342\"
213 S 00:05 opencode run --dir /tmp/maestro --title Supervisor Pulse \"/pulse\"
214 S 00:12 opencode run --dir /tmp/maestro-tools --session-key issue-4342 --title Issue #4342: tools \"/full-loop Implement issue #4342\""

	if ! has_worker_for_repo_issue "4342" "Aditya060806/Maestro"; then
		print_result "has_worker_for_repo_issue matches scoped worker process" 1 "Expected worker match for repo issue"
		return 0
	fi

	if has_worker_for_repo_issue "9999" "Aditya060806/Maestro"; then
		print_result "has_worker_for_repo_issue rejects unrelated issues" 1 "Expected no worker match for issue 9999"
		return 0
	fi

	# Line 214 uses /tmp/maestro-tools — a prefix of /tmp/maestro — must NOT match
	# Add a second repo entry for maestro-tools to verify exact path matching
	cat >"${REPOS_JSON}" <<'JSON'
{
  "initialized_repos": [
    {
    ...
    {
      "slug": "Aditya060806/Maestro-tools",
      "path": "/tmp/maestro-tools"
    }
  ]
}
JSON
	# Worker 214 is for maestro-tools, not maestro — should not count for maestro
	local count_maestro
	count_maestro=$(list_active_worker_processes | awk -v path="/tmp/maestro" '
		BEGIN { esc = path; gsub(/[][(){}.^$*+?|\\]/, "\\\\&", esc) }
		$0 ~ ("--dir[[:space:]]+" esc "([[:space:]]|$)") { count++ }
		END { print count + 0 }
	')
	if [[ "$count_maestro" != "1" ]]; then
		print_result "has_worker_for_repo_issue does not match prefix-sibling repo path" 1 "Expected 1 match for /tmp/maestro, got ${count_maestro}"
		return 0
	fi
...
test_excludes_zombie_and_stopped_processes() {
	...
	set_ps_fixture "400 S 00:10 opencode run --dir /tmp/maestro --title Issue #4400 \"/full-loop Implement issue #4400\"
401 Z 00:30 opencode run --dir /tmp/maestro --title Issue #4401 \"/full-loop Implement issue #4401\"
402 SN 00:45 opencode run --dir /tmp/maestro --title Issue #4402 \"/full-loop Implement issue #4402\"
403 T 01:00 opencode run --dir /tmp/maestro --title Issue #4403 \"/full-loop Implement issue #4403\"
404 Ss 00:05 opencode run --dir /tmp/maestro --title Issue #4404 \"/full-loop Implement issue #4404\"
405 Zs 00:15 opencode run --dir /tmp/maestro --title Issue #4405 \"/full-loop Implement issue #4405\"
406 TN 00:20 opencode run --dir /tmp/maestro --title Issue #4406 \"/full-loop Implement issue #4406\""
...
JSON
	...
	set_ps_fixture "500 S 00:15 bash /home/user/.maestro/agents/scripts/sandbox-exec-helper.sh run --timeout 3600 --allow-secret-io -- opencode run \"/full-loop Implement issue #6426\" --dir /tmp/maestro --session-key issue-6426 --title Issue #6426"

	# Should detect the worker via session-key fallback even though slug is not in repos.json
	if ! has_worker_for_repo_issue "6426" "Aditya060806/Maestro"; then
...
test_counts_standalone_opencode_binary_workers() {
	...
	set_ps_fixture "600 S 00:15 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #12361\" --dir /tmp/maestro --session-key issue-12361 --title Issue #12361
601 S 00:20 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #12362\" --dir /tmp/maestro --session-key issue-12362 --title Issue #12362
602 S 00:25 opencode run --dir /tmp/maestro --title Issue #12363 \"/full-loop Implement issue #12363\""
...
test_counts_headless_runtime_helper_wrapper_without_child_process() {
	...
	set_ps_fixture "650 S 00:18 bash /Users/test/.maestro/agents/scripts/headless-runtime-helper.sh run --role worker --session-key issue-14944 --dir /tmp/maestro --title Issue #14944 --prompt-file /tmp/pulse-14944.prompt"
...
test_deduplicates_headless_runtime_helper_wrapper_with_child_processes() {
	...
	set_ps_fixture "660 S 00:20 bash /Users/test/.maestro/agents/scripts/headless-runtime-helper.sh run --role worker --session-key issue-14944 --dir /tmp/maestro --title Issue #14944 --prompt-file /tmp/pulse-14944.prompt
661 S 00:19 bash /Users/test/.maestro/agents/scripts/sandbox-exec-helper.sh run --timeout 3600 --allow-secret-io -- /opt/homebrew/bin/opencode run \"/full-loop Implement issue #14944\" --dir /tmp/maestro --session-key issue-14944 --title Issue #14944
662 S 00:19 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #14944\" --dir /tmp/maestro --session-key issue-14944 --title Issue #14944"
...
test_deduplicates_chain_but_keeps_standalone_opencode_binary() {
	...
	set_ps_fixture "700 S 01:00 bash /home/user/.maestro/agents/scripts/sandbox-exec-helper.sh run --timeout 3600 --allow-secret-io -- /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5001\" --dir /tmp/maestro --title Issue #5001
701 S 01:00 node /opt/homebrew/bin/opencode run \"/full-loop Implement issue #5001\" --dir /tmp/maestro --title Issue #5001
702 S 01:00 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #5001\" --dir /tmp/maestro --title Issue #5001
710 S 00:15 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #12361\" --dir /tmp/maestro --session-key issue-12361 --title Issue #12361"
...
test_counts_review_issue_pr_workers() {
	...
	set_ps_fixture "800 S 00:10 opencode run --dir /tmp/maestro --title Issue #9001 \"/review-issue-pr Review issue #9001\"
801 S 00:20 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run \"/full-loop Implement issue #9002\" --dir /tmp/maestro --session-key issue-9002 --title Issue #9002
802 S 00:15 opencode run --dir /tmp/maestro --title Issue #9003 \"/review-issue-pr Review issue #9003\""
...
test_review_issue_pr_session_key_fallback_dedup() {
	...
	set_ps_fixture "900 S 00:10 opencode run --dir /tmp/maestro --session-key issue-9010 --title review-9010 \"/review-issue-pr 9010\"
901 S 00:10 /opt/homebrew/lib/node_modules/opencode-ai/bin/.opencode run --dir /tmp/maestro --session-key issue-9010 --title review-9010 \"/review-issue-pr 9010\"
902 S 00:15 opencode run --dir /tmp/maestro --session-key issue-9011 --title review-9011 \"/review-issue-pr 9011\""
...
test_check_dispatch_dedup_treats_merged_pr_as_duplicate() {
	...
	if check_dispatch_dedup "4527" "Aditya060806/Maestro" "Issue #4527: prevent redispatch" "t4527: prevent redispatch"; then
...
test_dispatch_with_dedup_blocks_when_duplicate() {
	...
	local dispatch_rc=0
	dispatch_with_dedup "9999" "Aditya060806/Maestro" "Issue #9999: test dedup" "t9999: test dedup" \
		"testuser" "/tmp/maestro" "/full-loop test" || dispatch_rc=$?
...
test_dispatch_with_dedup_fails_closed_when_issue_metadata_missing() {
	...
	local dispatch_rc=0
	dispatch_with_dedup "7777" "Aditya060806/Maestro" "Issue #7777: fail-closed" "t7777: fail-closed" \
		"testuser" "/tmp/maestro" "/full-loop test" || dispatch_rc=$?
...
STUB
	...
	local dispatch_rc=0
	dispatch_with_dedup "8888" "Aditya060806/Maestro" "Issue #8888: test pass" "t8888: test pass" \
		"testuser" "/tmp/maestro" "/full-loop test" || dispatch_rc=$?
...
test_dispatch_with_dedup_detaches_worker_stdio() {
	...
	local issue_log="/tmp/pulse-Aditya060806-maestro-8890.log"
...
EOF
	...
	local dispatch_rc=0
	dispatch_with_dedup "8890" "Aditya060806/Maestro" "Issue #8890: stdio detach" "t8890: stdio detach" \
		"testuser" "/tmp/maestro" "/full-loop test" <<<"candidate-stream-must-not-leak" || dispatch_rc=$?
...
EOF
	...
	local dispatch_rc=0
	dispatch_with_dedup "8891" "Aditya060806/Maestro" "Issue #8891: model override" "t8891: model override" \
		"testuser" "/tmp/maestro" "/full-loop test" "issue-8891" "anthropic/claude-haiku-4-5" || dispatch_rc=$?
...
JSON
	...
	gh_issue_list() {
		if [[ "${1:-}" == "--repo" ]]; then
			if [[ "${2:-}" == "Aditya060806/Maestro" ]]; then
				printf '%s\n' '[
				  {"number":7001,"title":"tooling simplification","url":"https://github.com/Aditya060806/Maestro/issues/7001","updatedAt":"2026-03-31T00:00:00Z","assignees":[],"labels":[{"name":"file-size-debt"}]},
				  {"number":7002,"title":"tooling bug","url":"https://github.com/Aditya060806/Maestro/issues/7002","updatedAt":"2026-03-31T00:01:00Z","assignees":[],"labels":[{"name":"bug"}]}
				]'
				return 0
			fi
			if [[ "${2:-}" == "webapp" ]]; then
...
JSON
	...
	gh_issue_list() {
		if [[ "${1:-}" == "--repo" && "${2:-}" == "Aditya060806/Maestro" ]]; then
			printf '%s\n' '[
...
JSON
	...
	gh_issue_list() {
		if [[ "${1:-}" == "--repo" && "${2:-}" == "Aditya060806/Maestro" ]]; then
			printf '%s\n' '[
			  {"number":9301,"title":"pull request target","url":"https://github.com/Aditya060806/Maestro/pull/9301","updatedAt":"2026-03-31T00:01:00Z","assignees":[],"labels":[{"name":"bug"}],"pull_request":{"url":"https://api.github.com/repos/Aditya060806/Maestro/pulls/9301"}},
			  {"number":9302,"title":"issue target","url":"https://github.com/Aditya060806/Maestro/issues/9302","updatedAt":"2026-03-31T00:02:00Z","assignees":[],"labels":[{"name":"bug"}]}
			]'
			return 0
		fi
		return 1
...
test_dispatch_max_dispatches_up_to_capacity() {
	...
	build_ranked_dispatch_candidates_json() {
		printf '%s\n' '[
		  {"number":9101,"repo_slug":"Aditya060806/Maestro","repo_path":"/tmp/maestro","url":"https://github.com/Aditya060806/Maestro/issues/9101","title":"candidate one","labels":["bug"],"updatedAt":"2026-03-31T00:00:00Z","score":8000},
		  {"number":9102,"repo_slug":"Aditya060806/Maestro","repo_path":"/tmp/maestro","url":"https://github.com/Aditya060806/Maestro/issues/9102","title":"candidate two","labels":["file-size-debt","tier:simple"],"updatedAt":"2026-03-31T00:01:00Z","score":4000},
		  {"number":9103,"repo_slug":"Aditya060806/Maestro","repo_path":"/tmp/maestro","url":"https://github.com/Aditya060806/Maestro/issues/9103","title":"candidate three","labels":["function-complexity-debt"],"updatedAt":"2026-03-31T00:02:00Z","score":4000}
		]'
	}
...
JSON
	...
	check_repo_pulse_schedule() {
		[[ "$1" == "Aditya060806/Maestro" ]]
	}
	gh_issue_list() {
		if [[ "${1:-}" == "--repo" ]]; then
			if [[ "${2:-}" == "Aditya060806/Maestro" ]]; then
				printf '%s\n' '[{"number":9201,"title":"allowed","url":"https://github.com/Aditya060806/Maestro/issues/9201","updatedAt":"2026-03-31T00:00:00Z","assignees":[],"labels":[{"name":"bug"}]}]'
				return 0
			fi
			if [[ "${2:-}" == "webapp" ]]; then
...
JSON
	...
	gh_issue_list() {
		if [[ "${1:-}" == "--repo" && "${2:-}" == "Aditya060806/Maestro" ]]; then
			printf '%s\n' '[{"number":9251,"title":"array pulse hours","url":"https://github.com/Aditya060806/Maestro/issues/9251","updatedAt":"2026-03-31T00:00:00Z","assignees":[],"labels":[{"name":"bug"}]}]'
			return 0
		fi
		return 1
	}
	...
	if [[ "$candidate_numbers" == "9251" && "$schedule_args" == "Aditya060806/Maestro|9|17|" ]]; then
...
test_dispatch_max_honors_stop_flag() {
	...
	build_ranked_dispatch_candidates_json() {
		printf '%s\n' '[{"number":9301,"repo_slug":"Aditya060806/Maestro","repo_path":"/tmp/maestro","url":"https://github.com/Aditya060806/Maestro/issues/9301","title":"candidate one","labels":["bug"],"updatedAt":"2026-03-31T00:00:00Z","score":8000}]'
	}
	dispatch_with_dedup() {
		printf '%s\n' "$1" >>"$dispatch_log"
		return 0
...
test_dispatch_max_ignores_noisy_count_output() {
	...
	build_ranked_dispatch_candidates_json() {
		printf '%s\n' '[
		  {"number":9401,"repo_slug":"Aditya060806/Maestro","repo_path":"/tmp/maestro","url":"https://github.com/Aditya060806/Maestro/issues/9401","title":"candidate one","labels":["bug"],"updatedAt":"2026-03-31T00:00:00Z","score":8000}
		]'
	}
	get_max_workers_target() { echo 2; }
	count_active_workers() { echo 0; }
...
test_worker_detection_uses_unlimited_width_ps_flag() {
	...
	set_ps_fixture "100 S 00:10 bash /Users/test/.maestro/agents/scripts/headless-runtime-helper.sh run --role worker --session-key issue-2190 --dir /tmp/maestro --title Issue #2190 --prompt-file /tmp/pulse-2190.prompt"
