...
run_config_source() {
	...
	(
		...
		if [[ -n "$headless_models" ]]; then
			export MAESTRO_HEADLESS_MODELS="$headless_models"
		else
			unset MAESTRO_HEADLESS_MODELS || true
		fi
...
test_inherited_headless_models_without_credentials_export_is_quiet() {
	...
	mkdir -p "${test_home}/.config/maestro"
	printf '# export MAESTRO_HEADLESS_MODELS=old\n' >"${test_home}/.config/maestro/credentials.sh"

	run_config_source "$test_home" "$stderr_file" "openai/example" ""

	if [[ -s "$stderr_file" ]]; then
		print_result "inherited MAESTRO_HEADLESS_MODELS without active credentials export is quiet" 1 "stderr present"
	else
		print_result "inherited MAESTRO_HEADLESS_MODELS without active credentials export is quiet" 0
	fi
...
test_active_headless_models_credentials_export_warns_with_file() {
	...
	if grep -q "MAESTRO_HEADLESS_MODELS" "$stderr_file" && grep -q "${test_home}/.config/maestro/credentials.sh" "$stderr_file"; then
		print_result "active MAESTRO_HEADLESS_MODELS credentials export warns with file" 0
	else
		print_result "active MAESTRO_HEADLESS_MODELS credentials export warns with file" 1 "expected variable and file in warning"
	fi
...
test_active_pulse_model_assignment_warns_with_file() {
	...
	if grep -q "PULSE_MODEL" "$stderr_file" && grep -q "${test_home}/.config/maestro/credentials.sh" "$stderr_file"; then
