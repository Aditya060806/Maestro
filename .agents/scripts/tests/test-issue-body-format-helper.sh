...
#   5. MAESTRO_BODY_FORMAT_STRICT=1 makes non-dispatchable a hard error.
...
rc=0
stderr=$(MAESTRO_BODY_FORMAT_STRICT=1 "$HELPER" check "$BODY_NO_FILES_NO_HOW" 2>&1) || rc=$?
if [[ $rc -eq 1 ]]; then
...
rc=0
stderr=$(MAESTRO_BODY_FORMAT_STRICT=1 "$HELPER" check "$BODY_NO_ACCEPTANCE" 2>&1) || rc=$?
if [[ $rc -eq 1 ]]; then
...
rc=0
stderr=$(MAESTRO_BODY_FORMAT_STRICT=1 "$HELPER" check "$BODY_EDIT_PATTERN" 2>&1) || rc=$?
...
rc=0
stderr=$(MAESTRO_BODY_FORMAT_STRICT=1 "$HELPER" check "$BODY_CHECKLIST_ONLY" 2>&1) || rc=$?
