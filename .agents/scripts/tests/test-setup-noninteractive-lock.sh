...
make_temp_dir() {
	local tmp_dir=""
	tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-setup-lock-test.XXXXXX")
	printf '%s' "$tmp_dir"
	return 0
}
...
test_blocks_concurrent_live_owner() {
	...
	# MAESTRO_SETUP_WAIT_TIMEOUT_S=5 so the subshell exits within ~15 s.
	local tmp_dir=""
	local output=""
	local owner_pid=""
	local now_epoch=0
	...
	output=$(
		MAESTRO_SETUP_LOCK_DIR="$tmp_dir/lock.d"
		MAESTRO_SETUP_WAIT_TIMEOUT_S=5
		MAESTRO_SETUP_STALE_TIMEOUT_S=3600
		load_lock_functions
...
test_preserves_live_setup_owner_with_old_lock_timestamp() {
	...
	output=$(
		MAESTRO_SETUP_LOCK_DIR="$tmp_dir/lock.d"
		MAESTRO_SETUP_WAIT_TIMEOUT_S=5
		MAESTRO_SETUP_STALE_TIMEOUT_S=1800
		load_lock_functions
		_setup_acquire_noninteractive_setup_lock --non-interactive
		printf 'held=%s\n' "${SETUP_NONINTERACTIVE_LOCK_HELD:-false}"
		return 0
...
test_contention_message_includes_elapsed_and_stage() {
	...
	printf '%s\t%s\t%s\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "deploy_maestro_agents" "0.00" "RUNNING" >"$fake_stl"
	...
	output=$(
		...
		MAESTRO_SETUP_STALE_TIMEOUT_S=3600
		HOME="$tmp_dir"
		mkdir -p "$tmp_dir/.maestro/logs"
		cp "$fake_stl" "$tmp_dir/.maestro/logs/setup-stage-timings.log"
		load_lock_functions
		_setup_acquire_noninteractive_setup_lock --non-interactive
		printf 'exit=%s\n' "$?"
		return 0
	...
	[[ "$output" == *"stage: deploy_maestro_agents"* ]] || passed=3
...
test_default_wait_ceiling_is_longer_than_observed_slow_owner() {
	...
	if [[ "$output" == *'MAESTRO_SETUP_WAIT_TIMEOUT_S:-900'* ]]; then
...
test_periodic_wait_message_preserves_stage_and_command() {
	...
	tmp_dir=$(make_temp_dir)
	owner_pid=$(start_fake_setup_owner)
	mkdir -p "$tmp_dir/lock.d" "$tmp_dir/.maestro/logs"
	printf '%s\n' "$owner_pid" >"$tmp_dir/lock.d/owner.pid"
	printf '%s\n' './setup.sh --non-interactive --token TOKEN_VALUE_PLACEHOLDER' >"$tmp_dir/lock.d/command"
	past_epoch=$(( $(date +%s 2>/dev/null || echo "0") - 5 ))
	printf '%s\n' "$past_epoch" >"$tmp_dir/lock.d/started_at_epoch"
	printf '%s\t%s\t%s\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "deploy_maestro_agents" "0.00" "RUNNING" >"$tmp_dir/.maestro/logs/setup-stage-timings.log"

	output=$(
		MAESTRO_SETUP_LOCK_DIR="$tmp_dir/lock.d"
		MAESTRO_SETUP_WAIT_TIMEOUT_S=15
		MAESTRO_SETUP_STALE_TIMEOUT_S=3600
		MAESTRO_SETUP_LOCK_DIAG_INTERVAL_S=10
		HOME="$tmp_dir"
		load_lock_functions
		_setup_acquire_noninteractive_setup_lock --non-interactive
		printf 'exit=%s\n' "$?"
	...
	if [[ "$output" == *"Still waiting for setup lock"* && "$output" == *"stage: deploy_maestro_agents"* && "$output" == *"command: ./setup.sh --non-interactive --token [redacted]"* && "$output" == *"setup-stage-timings.log"* && "$output" != *"TOKEN_VALUE_PLACEHOLDER"* ]]; then
...
test_signal_cleanup_terminates_registered_children() {
	...
	output=$(
		MAESTRO_SETUP_CHILD_TERM_GRACE_S=1
		load_lock_functions
...
test_stale_live_owner_past_ceiling_is_reclaimed() {
	...
	output=$(
		MAESTRO_SETUP_LOCK_DIR="$tmp_dir/lock.d"
		MAESTRO_SETUP_STALE_TIMEOUT_S=1
		MAESTRO_SETUP_WAIT_TIMEOUT_S=300
		load_lock_functions
		_setup_acquire_noninteractive_setup_lock --non-interactive
		printf 'held=%s\n' "${SETUP_NONINTERACTIVE_LOCK_HELD:-false}"
		_setup_release_noninteractive_setup_lock
