...
#   1. ~/.maestro/.deployed-sha mtime  (Option B, level 1)
...
run_check_in_env() {
	...
	HOME="$fake_home" MAESTRO_AGENTS_DIR="$fake_agents_dir" bash -c "
set +e
...
test_fresh_deployed_sha_passes() {
	...
	mkdir -p "${fake_home}/.maestro"
	printf 'abc123\n' >"${fake_home}/.maestro/.deployed-sha"
...
test_stale_deployed_sha_fails() {
	...
	mkdir -p "${fake_home}/.maestro"
	printf 'abc123\n' >"${fake_home}/.maestro/.deployed-sha"
	set_mtime_days_ago "${fake_home}/.maestro/.deployed-sha" 31
...
test_no_stamp_fresh_yaml_passes() {
	...
	make_agents_dir "$agents_dir"
	# No .deployed-sha, no Git/maestro — yaml file has today's mtime
...
test_no_stamp_stale_yaml_fails() {
	...
	# No .deployed-sha, no Git/maestro
