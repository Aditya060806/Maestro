...
install_test_stubs() {
	gh_create_issue() {
		printf '%s\n' "$*" >>"$CREATE_LOG"
		printf 'https://github.com/Aditya060806/Maestro/issues/99999\n'
		return 0
	}
...
test_simplification_md_labels() {
	...
	_COMPLEXITY_SCAN_SKIP_REVIEW_GATE=true _complexity_scan_process_single_md_file \
		".agents/reference/example.md" "501" "Aditya060806/Maestro" "$PWD" "" "Aditya060806" >/dev/null
...
test_simplification_shell_labels() {
	...
	printf '{"initialized_repos":[{"slug":"Aditya060806/Maestro","path":"%s"}]}\n' "$PWD" >"$repos_json"
	reset_create_log
	_COMPLEXITY_SCAN_SKIP_REVIEW_GATE=true _complexity_scan_sh_create_issue \
		".agents/scripts/example.sh" "1" "$repos_json" "Aditya060806/Maestro" >/dev/null
...
test_failure_miner_labels() {
	...
	cluster_json='{"repo":"Aditya060806/Maestro","check_name":"ShellCheck","check_names":["ShellCheck"],"signature":"failure:shellcheck","count":2,"sources":["pr:#1"],"examples":[{"source_ref":"pr:#1"}]}'
	legacy_infra_cluster_json='{"repo":"Aditya060806/Maestro","check_name":"ShellCheck","signature":"failure:shellcheck","count":2,"sources":["pr:#1"],"examples":[{"source_ref":"pr:#1"}]}'
