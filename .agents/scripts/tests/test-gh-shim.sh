...
#   7. `MAESTRO_GH_SHIM_DISABLE=1` bypasses the shim entirely
#   8. Recursion guard: `_MAESTRO_GH_SHIM_ACTIVE=1` triggers pass-through
#
...
unset MAESTRO_HEADLESS
unset OPENCODE_HEADLESS
unset GITHUB_ACTIONS
unset MAESTRO_SESSION_ORIGIN
unset MAESTRO_USER_INSTIGATED_EXTERNAL_GH_WRITE
unset MAESTRO_EXTERNAL_GH_WRITE_ALLOWLIST
...
printf '\n\n<!-- maestro:sig -->\n---\n[maestro.sh](https://maestro.sh) v9.9.9 stub footer\n'
EOF
...
# instead of the real one installed in ~/.maestro/agents/scripts/.
...
marker_count=$(grep -c "<!-- maestro:sig -->" "$STUB_GH_LOG" 2>/dev/null || true)
...
if [[ -n "$resolved_body_file" && -f "$resolved_body_file" ]] && grep -q "<!-- maestro:sig -->" "$resolved_body_file"; then
	_pass "sig marker appended to temporary --body-file"
else
	_fail "--body-file sig injection" "argv: $argv"
fi
if grep -q "unsigned body content" "$body_file" && ! grep -q "<!-- maestro:sig -->" "$body_file"; then
...
_reset_log
if "$SHIM_RUN" pr create --repo Aditya060806/Maestro --title "fix: missing issue" --body "PR body" 2>"$TMP/pr-linked-issue.err"; then
...
_reset_log
"$SHIM_RUN" pr create --repo Aditya060806/Maestro --title "fix: linked issue" --body "Resolves #25901" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == *"Resolves #25901"* ]] && [[ "$argv" == *"<!-- maestro:sig -->"* ]]; then
...
printf '## Summary\n\nFor #25901\n' >"$linked_pr_body"
_reset_log
"$SHIM_RUN" pr create --repo Aditya060806/Maestro --title "fix: linked body-file" --body-file "$linked_pr_body" 2>/dev/null
argv=$(_read_argv)
resolved_pr_body_file=$(printf '%s\n' "$argv" | awk 'prev { print; exit } $0 == "--body-file" { prev=1 }')
if [[ -n "$resolved_pr_body_file" && -f "$resolved_pr_body_file" ]] && grep -q "<!-- maestro:sig -->" "$resolved_pr_body_file"; then
...
# =============================================================================
# Test 7: MAESTRO_GH_SHIM_DISABLE=1 bypasses the shim
# =============================================================================
echo ""
echo "Test 7: MAESTRO_GH_SHIM_DISABLE=1 bypass"
_reset_log
MAESTRO_GH_SHIM_DISABLE=1 "$SHIM_RUN" issue comment 999 --repo owner/repo --body "unsigned" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" != *"<!-- maestro:sig -->"* ]]; then
	_pass "MAESTRO_GH_SHIM_DISABLE=1 skips sig injection"
else
...
_reset_log
_MAESTRO_GH_SHIM_ACTIVE=1 "$SHIM_RUN" issue comment 111 --repo owner/repo --body "recursive" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" != *"<!-- maestro:sig -->"* ]]; then
...
rm -f "$MAESTRO_GH_API_LOG"
"$SHIM_RUN" issue list --repo owner/repo 2>/dev/null
"$SHIM_RUN" pr view 123 --repo owner/repo 2>/dev/null
if grep -q $'\tgh_issue_list\tgraphql' "$MAESTRO_GH_API_LOG" && grep -q $'\tgh_pr_view\tgraphql' "$MAESTRO_GH_API_LOG"; then
	_pass "read/list calls use operation-specific labels"
else
	_fail "operation-specific instrumentation labels" "log: $(cat "$MAESTRO_GH_API_LOG" 2>/dev/null || true)"
fi
...
_reset_log
export MAESTRO_GH_API_LOG="$TMP/gh-api-calls-json.log"
rm -f "$MAESTRO_GH_API_LOG"
_GH_SHOULD_FALLBACK_OVERRIDE=1 "$SHIM_RUN" pr view 123 --repo owner/repo --json number,statusCheckRollup 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == $'pr\nview\n123\n--repo\nowner/repo\n--json\nnumber,statusCheckRollup' ]] && grep -q $'\tgh_pr_view\tgraphql' "$MAESTRO_GH_API_LOG"; then
	_pass "--json read stays on GraphQL with operation label"
else
	_fail "--json read GraphQL preservation" "argv: $argv log: $(cat "$MAESTRO_GH_API_LOG" 2>/dev/null || true)"
fi
...
_reset_log
export MAESTRO_GH_API_LOG="$TMP/gh-api-calls-json-pr-list.log"
rm -f "$MAESTRO_GH_API_LOG"
...
if [[ "$output" == "22337" ]] &&
	...
	grep -q $'\tgh_pr_list\trest' "$MAESTRO_GH_API_LOG"; then
	_pass "gh pr list --json uses REST fallback with qualified --head"
else
	_fail "gh pr list --json REST fallback" "output: $output argv: $argv log: $(cat "$MAESTRO_GH_API_LOG" 2>/dev/null || true)"
fi
...
_reset_log
export MAESTRO_GH_API_LOG="$TMP/gh-api-calls-json-issue-list.log"
rm -f "$MAESTRO_GH_API_LOG"
...
if [[ "$output" == '"Reduce GraphQL list-call pressure"' ]] &&
	...
	grep -q $'\tgh_issue_list\trest' "$MAESTRO_GH_API_LOG"; then
	_pass "gh issue list --json uses REST fallback with compact issue fields"
else
	_fail "gh issue list --json REST fallback" "output: $output argv: $argv log: $(cat "$MAESTRO_GH_API_LOG" 2>/dev/null || true)"
fi
...
_reset_log
export MAESTRO_GH_API_LOG="$TMP/gh-api-calls-rest-first.log"
rm -f "$MAESTRO_GH_API_LOG"
output=$(MAESTRO_GH_REST_FIRST_READS=1 STUB_RATE_LIMIT_REMAINING=5000 "$SHIM_RUN" issue list --repo owner/repo \
	--state open --json number,title --jq '.[0].number' 2>/dev/null || true)
argv=$(_read_argv)
if [[ "$output" == "22430" ]] &&
	[[ "$argv" == *"/repos/owner/repo/issues?state=open&per_page=30"* ]] &&
	grep -q $'\tgh_issue_list\trest' "$MAESTRO_GH_API_LOG" &&
	! grep -q $'\tgh_issue_list\tgraphql' "$MAESTRO_GH_API_LOG"; then
	_pass "REST-first rewrites equivalent issue list without GraphQL"
else
	_fail "REST-first equivalent issue list rewrite" "output: $output argv: $argv log: $(cat "$MAESTRO_GH_API_LOG" 2>/dev/null || true)"
fi

_reset_log
export MAESTRO_GH_API_LOG="$TMP/gh-api-calls-rest-first-unsafe.log"
rm -f "$MAESTRO_GH_API_LOG"
MAESTRO_GH_REST_FIRST_READS=1 "$SHIM_RUN" pr list --repo owner/repo \
	--state open --json number,reviewDecision,headRefOid 2>/dev/null || true
argv=$(_read_argv)
if [[ "$argv" == $'pr\nlist\n--repo\nowner/repo\n--state\nopen\n--json\nnumber,reviewDecision,headRefOid' ]] &&
	grep -q $'\tgh_pr_list\tgraphql' "$MAESTRO_GH_API_LOG"; then
	_pass "REST-first leaves GraphQL-only pr list fields on GraphQL"
else
	_fail "REST-first GraphQL-only pr list preservation" "argv: $argv log: $(cat "$MAESTRO_GH_API_LOG" 2>/dev/null || true)"
fi

# =============================================================================
# Test 16: raw interactive maestro tracking issue creation is normalized
# =============================================================================
echo ""
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_USER_INSTIGATED_EXTERNAL_GH_WRITE=owner/repo "$SHIM_RUN" issue create --repo owner/repo --title "t3565: Headless labels" --body "tracking body" 2>/dev/null
argv=$(_read_argv)
...
_reset_log
if MAESTRO_HEADLESS=1 "$SHIM_RUN" issue comment 123 --repo external/repo --body "uninstigated" 2>"$TMP/guard-issue.err"; then
...
_reset_log
if MAESTRO_SESSION_ORIGIN=pulse "$SHIM_RUN" pr comment 456 --repo external/repo --body "uninstigated" 2>"$TMP/guard-pr.err"; then
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_USER_INSTIGATED_EXTERNAL_GH_WRITE=external/repo "$SHIM_RUN" pr comment 456 --repo external/repo --body "explicit" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == *"<!-- maestro:sig -->"* ]]; then
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json" "$SHIM_RUN" issue comment 789 --repo managed/repo --body "managed" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == *"<!-- maestro:sig -->"* ]]; then
...
- **Worker PID**: 123
<!-- ops:end -->'
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json" "$SHIM_RUN" api /repos/managed/repo/issues/789/comments -X POST -f body="$ops_body" 2>/dev/null
sig_argv=$(_read_sig_argv)
argv=$(_read_argv)
if [[ "$argv" == *"<!-- maestro:sig -->"* ]] && [[ "$sig_argv" == *$'--no-session'* ]]; then
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json_missing_role" "$SHIM_RUN" api /repos/managed/repo/issues/789/comments -X POST -f body="managed" 2>"$TMP/guard-api-missing-role.err"
argv=$(_read_argv)
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json_org_maintainer" "$SHIM_RUN" api /repos/org/repo/issues/789/comments -X POST -f body="managed" 2>"$TMP/guard-api-org-maintainer.err"
argv=$(_read_argv)
...
_reset_log
if MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json_org_nonmaintainer" "$SHIM_RUN" api /repos/org/repo/issues/789/comments -X POST -f body="managed" 2>"$TMP/guard-api-org-nonmaintainer.err"; then
...
_reset_log
if MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json_contributor" "$SHIM_RUN" api /repos/managed/repo/issues/789/comments -X POST -f body="managed" 2>"$TMP/guard-api-explicit-contributor.err"; then
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json" "$SHIM_RUN" issue comment 789 --repo ssh://git@github.com/managed/repo.git --body "managed" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == *"<!-- maestro:sig -->"* ]]; then
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json" "$SHIM_RUN" issue comment 789 --repo https://token@github.com/managed/repo.git --body "managed" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == *"<!-- maestro:sig -->"* ]]; then
...
_reset_log
MAESTRO_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json" "$SHIM_RUN" issue comment 789 --repo git://github.com/managed/repo.git --body "managed" 2>/dev/null
argv=$(_read_argv)
if [[ "$argv" == *"<!-- maestro:sig -->"* ]]; then
...
for short_opt in -q -p -t; do
	_reset_log
	if FULL_LOOP_HEADLESS=1 MAESTRO_REPOS_JSON="$repos_json" "$SHIM_RUN" api "$short_opt" /repos/managed/repo/issues/1/comments /repos/external/repo/issues/123/comments -X POST -f body="uninstigated" 2>"$TMP/guard-api-$short_opt-injection.err"; then
