...
setup_sandbox() {
	...
	mkdir -p \
		"${HOME}/.maestro/logs" \
		"${HOME}/.maestro/logs/quality-sweep-state" \
		"${HOME}/.config/maestro"
...
test_persist_role_cache() {
	...
	# _persist_role_cache(runner_user, repo_slug, role) writes to $HOME/.maestro/logs/
	_persist_role_cache "$runner" "$slug" "$role" 2>/dev/null || true

	local cache_file="${HOME}/.maestro/logs/runner-role-testuser-test-owner-test-repo"
