...
mkdir -p "${HOME}/.maestro/logs" "${TEST_ROOT}/bin"
...
rc=0
output=$("${TEST_SCRIPTS_DIR}/github-release-helper.sh" create 1.2.4 --repo Aditya060806/Maestro --notes 'notes' 2>&1) || rc=$?
...
if grep -q 'release view v1.2.4 --repo Aditya060806/Maestro' "$FAKE_GH_LOG" 2>/dev/null && grep -q 'release create v1.2.4' "$FAKE_GH_LOG" 2>/dev/null; then
