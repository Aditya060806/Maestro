...
source_pulse_cleanup_with_stubs() {
	...
	export LOGFILE
	MAESTRO_CLEANUP_LOG="${TEST_ROOT}/cleanup.log"
	export MAESTRO_CLEANUP_LOG
...
test_closed_issue_local_commit_no_pr_removes_before_age_threshold() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-closed-issue-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	grep -q 'worktree-removed.*local-commits-branch-preserved.*mode=branch-preserved' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved-closed-issue' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	print_result "closed issue local commits/no PR archives before age threshold" "$rc" \
		"cleanup_rc=$cleanup_rc branch_exists=$branch_exists log=$(cat "$MAESTRO_CLEANUP_LOG" 2>/dev/null)"
	return 0
}
...
test_closed_pr_reference_local_commit_no_pr_removes_before_age_threshold() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-closed-pr-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ "$branch_exists" -eq 0 ]] || rc=1
	grep -q 'worktree-removed.*local-commits-branch-preserved.*mode=branch-preserved' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'pr_state=pr-CLOSED' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved-closed-pr-23078' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_merged_branch_pr_removes_before_age_threshold() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-merged-branch-pr-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ "$branch_exists" -eq 0 ]] || rc=1
	grep -q 'worktree-removed.*local-commits-branch-preserved.*mode=branch-preserved' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'pr_state=pr-MERGED' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved-terminal-pr' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_closed_issue_dirty_worktree_stashes_and_preserves_branch() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-closed-issue-dirty-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	grep -q 'dirty=1' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved-closed-issue' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	print_result "closed issue dirty worktree stashes and preserves branch" "$rc" \
		"cleanup_rc=$cleanup_rc branch_exists=$branch_exists stash_count=$stash_count log=$(cat "$MAESTRO_CLEANUP_LOG" 2>/dev/null)"
	return 0
}
...
test_fix_numeric_closed_issue_worktree_archives() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-fix-numeric-closed-issue-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ ! -d "$wt_path" ]] || rc=1
	[[ "$branch_exists" -eq 0 ]] || rc=1
	grep -q 'issue=23082' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved-closed-issue' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_recent_metric_blocks_local_commit_no_pr_removal() {
	...
	MAESTRO_HEADLESS_METRICS_FILE="$metrics_file"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ -d "$wt_path" ]] || rc=1
	grep -q 'worktree-skipped.*active-worker-metric.*mode=skipped' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recent_session_guard=active' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'commits=1' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	print_result "recent metric blocks local commits/no PR permanent cleanup" "$rc" \
		"cleanup_rc=$cleanup_rc log=$(cat "$MAESTRO_CLEANUP_LOG" 2>/dev/null)"
	return 0
}
...
test_local_commit_no_pr_skips_without_recent_metric() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ "$cleanup_rc" -eq 1 ]] || rc=1
	[[ -d "$wt_path" ]] || rc=1
	grep -q 'worktree-skipped.*local-commits-no-pr.*mode=skipped' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'owner_guard=clear' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'process_guard=clear' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'pr_state=none' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved-after-' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_young_local_commit_logs_not_age_eligible() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-young-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ "$cleanup_rc" -eq 1 ]] || rc=1
	[[ -d "$wt_path" ]] || rc=1
	grep -q 'worktree-skipped.*not-age-eligible.*mode=skipped' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'pr_state=not-eligible' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'commits=1' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_local_only_repo_worktree_logs_explicit_skip() {
	...
	local branch_name="chore/maestro-init"
	setup_repo_with_worker_worktree "$repo_dir" "$wt_path" "$branch_name" || return 1
	source_pulse_cleanup_with_stubs || return 1
	printf 'local init artifact\n' >"$wt_path/.maestro.json"
	mkdir -p "${HOME}/.config/maestro"
	cat >"${HOME}/.config/maestro/repos.json" <<JSON
...
JSON
	...
	[[ "$cleanup_rc" -eq 0 ]] || rc=1
	[[ -d "$wt_path" ]] || rc=1
	grep -q 'worktree-skipped.*local-only-repo.*mode=skipped' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
...
test_stale_local_commit_no_pr_removes_worktree_preserves_branch() {
	...
	local now_epoch
	now_epoch=$(date +%s)
	MAESTRO_HEADLESS_METRICS_FILE="${TEST_ROOT}/missing-stale-metrics.jsonl"
	export MAESTRO_HEADLESS_METRICS_FILE
	...
	[[ "$branch_exists" -eq 0 ]] || rc=1
	grep -q 'worktree-removed.*local-commits-branch-preserved.*mode=branch-preserved' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
	grep -q 'recovery_path=branch-preserved' "$MAESTRO_CLEANUP_LOG" 2>/dev/null || rc=1
