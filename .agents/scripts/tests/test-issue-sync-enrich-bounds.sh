...
test_workflow_push_sets_bounded_enrich() {
	if grep -q 'MAESTRO_ENRICH_MAX_ISSUES' "$WORKFLOW_FILE" \
		&& grep -q 'MAESTRO_ENRICH_MAX_SECONDS' "$WORKFLOW_FILE" \
...
test_max_issues_bounds_enrich_loop() {
	...
	: >"$TMP/processed.log"
	local output
	output=$(MAESTRO_ENRICH_MAX_ISSUES=2 MAESTRO_ENRICH_MAX_SECONDS=0 cmd_enrich 2>&1)
...
test_target_task_ignores_routine_bounds() {
	...
	: >"$TMP/processed.log"
	local output
	output=$(MAESTRO_ENRICH_MAX_ISSUES=0 MAESTRO_ENRICH_MAX_SECONDS=0 cmd_enrich t9002 2>&1)
	local processed
	processed=$(tr -d '\n' <"$TMP/processed.log")
...
test_invalid_bounds_are_ignored() {
	...
	: >"$TMP/processed.log"
	local output
	output=$(MAESTRO_ENRICH_MAX_ISSUES=bad MAESTRO_ENRICH_MAX_SECONDS=bad cmd_enrich 2>&1)
	local processed_count
	processed_count=$(wc -l <"$TMP/processed.log" | tr -d ' ')
