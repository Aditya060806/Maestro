...
define_helpers_under_test() {
	...
	' "$NMR_SCRIPT")
	current_src=$(awk '
		/^_nmr_current_maestro_version\(\) \{/,/^}$/ { print }
	' "$NMR_SCRIPT")
	retry_src=$(awk '
...
test_signature_rejects_stale_recovery_marker() {
	...
	if _nmr_application_has_automation_signature 18623 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_signature_rejects_cost_circuit_breaker_marker() {
	...
	set_issue_meta '{"labels":[]}'
	if _nmr_application_has_automation_signature 18640 Aditya060806/Maestro "2026-04-13T05:00:02Z"; then
...
test_signature_rejects_circuit_breaker_escalated_marker() {
	...
	set_issue_meta '{"labels":[]}'
	if _nmr_application_has_automation_signature 18641 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_signature_detects_source_review_scanner_comment_marker() {
	...
	set_issue_meta '{"labels":[]}'
	if _nmr_application_has_automation_signature 18622 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_signature_ignores_marker_outside_window() {
	...
	set_issue_meta '{"labels":[]}'
	if _nmr_application_has_automation_signature 18623 Aditya060806/Maestro "2026-04-13T05:05:00Z"; then
...
test_signature_detects_review_followup_label_fallback() {
	...
	if _nmr_application_has_automation_signature 18539 Aditya060806/Maestro "2026-04-13T04:29:13Z"; then
...
test_signature_detects_source_review_scanner_label_fallback() {
	...
	if _nmr_application_has_automation_signature 18621 Aditya060806/Maestro "2026-04-13T06:39:22Z"; then
...
test_signature_ignores_unrelated_comment_in_window() {
	...
	if _nmr_application_has_automation_signature 42 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_signature_detects_lower_bound_comment_before_label() {
	...
	set_comments '[{"created_at":"2026-04-13T05:00:00Z","body":"<!-- source:review-scanner -->"}]'
	set_issue_meta '{"labels":[]}'
	if _nmr_application_has_automation_signature 42 Aditya060806/Maestro "2026-04-13T05:00:03Z"; then
...
test_breaker_trip_detects_stale_recovery_escalation_marker() {
	...
	set_comments '[{"created_at":"2026-04-13T05:00:02Z","body":"<!-- stale-recovery-tick:escalated (threshold=2) -->\n**Stale recovery threshold reached** (t2008)"}]'
	if _nmr_application_is_circuit_breaker_trip 18623 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_breaker_trip_detects_cost_circuit_breaker_marker() {
	set_comments '[{"created_at":"2026-04-13T05:00:00Z","body":"<!-- cost-circuit-breaker:fired tier=standard spent=120000 budget=100000 -->\n🛑 Cost circuit breaker fired"}]'
	if _nmr_application_is_circuit_breaker_trip 18640 Aditya060806/Maestro "2026-04-13T05:00:02Z"; then
...
test_breaker_trip_detects_legacy_escalated_marker() {
	set_comments '[{"created_at":"2026-04-13T05:00:00Z","body":"<!-- circuit-breaker-escalated -->"}]'
	if _nmr_application_is_circuit_breaker_trip 18641 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_breaker_trip_rejects_source_review_scanner_marker() {
	...
	set_comments '[{"created_at":"2026-04-13T05:00:00Z","body":"<!-- source:review-scanner -->"}]'
	if _nmr_application_is_circuit_breaker_trip 18621 Aditya060806/Maestro "2026-04-13T05:00:00Z"; then
...
test_breaker_trip_ignores_marker_outside_window() {
	...
	if _nmr_application_is_circuit_breaker_trip 42 Aditya060806/Maestro "2026-04-13T05:05:00Z"; then
...
test_security_sensitive_detects_security_label() {
	...
	if _nmr_application_is_security_sensitive 24841 Aditya060806/Maestro; then
...
test_security_sensitive_detects_security_review_label() {
	...
	if _nmr_application_is_security_sensitive 24842 Aditya060806/Maestro; then
...
test_security_sensitive_uses_precomputed_issue_meta() {
	...
	if _nmr_application_is_security_sensitive 24842 Aditya060806/Maestro \
...
test_security_sensitive_ignores_non_security_labels() {
	...
	if _nmr_application_is_security_sensitive 24843 Aditya060806/Maestro; then
...
test_19756_loop_prevention_breaker_trip_preserves_nmr() {
	...
	set_timeline '[{"event":"labeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"Aditya060806"},"created_at":"2026-04-19T05:00:00Z"}]'
	...
	if _nmr_applied_by_maintainer 19756 Aditya060806/Maestro Aditya060806; then
...
test_scanner_default_still_auto_approves() {
	...
	if _nmr_applied_by_maintainer 18539 Aditya060806/Maestro Aditya060806; then
...
test_security_review_feedback_preserves_nmr() {
	...
	set_timeline '[{"event":"labeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"Aditya060806"},"created_at":"2026-06-15T18:00:00Z"}]'
	set_comments '[]'
	...
	if _nmr_applied_by_maintainer 24841 Aditya060806/Maestro Aditya060806; then
...
test_manual_hold_still_preserves_nmr() {
	...
	if _nmr_applied_by_maintainer 42 Aditya060806/Maestro Aditya060806; then
...
test_non_maintainer_label_actor_is_not_manual_maintainer_hold() {
	...
	set_comments '[]'
	set_issue_meta '{"labels":[{"name":"needs-maintainer-review"}]}'
	if _nmr_applied_by_maintainer 99 Aditya060806/Maestro Aditya060806; then
...
test_paginated_timeline_latest_nmr_event_preserves_breaker_trip() {
	...
	set_timeline '[[{"event":"labeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"Aditya060806"},"created_at":"2026-05-06T10:00:00Z"}],[{"event":"unlabeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"Aditya060806"},"created_at":"2026-05-06T10:05:00Z"},{"event":"labeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"peer-runner"},"created_at":"2026-05-06T16:54:57Z"}]]'
	set_comments '[{"created_at":"2026-05-06T16:54:59Z","body":"<!-- dispatch-infrastructure-failure -->\n## Dispatch infrastructure failure detected"}]'
	set_issue_meta '{"labels":[{"name":"needs-maintainer-review"},{"name":"origin:worker"},{"name":"auto-dispatch"}]}'
...
test_release_upgrade_allows_rate_limit_breaker_retry() {
	# Outlier recovery: a rate-limit/capacity breaker from an older maestro
	# release should be allowed one fresh retry after a newer release lands.
	set_timeline '[{"event":"labeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"Aditya060806"},"created_at":"2026-05-07T21:26:05Z"}]'
	set_comments '[{"created_at":"2026-05-07T21:26:06Z","body":"<!-- dispatch-backoff:rate_limit_nmr -->\n## Rate-Limit Backoff Circuit Breaker (t2781)\n---\n[maestro.sh](https://maestro.sh) v3.14.91 automated scan."}]'
	set_issue_meta '{"labels":[{"name":"needs-maintainer-review"},{"name":"origin:worker"},{"name":"auto-dispatch"}]}'
	MAESTRO_CURRENT_VERSION_OVERRIDE=3.14.94
	export MAESTRO_CURRENT_VERSION_OVERRIDE
	if _nmr_applied_by_maintainer 4508 exampleorg/examplerepo Aditya060806; then
		unset MAESTRO_CURRENT_VERSION_OVERRIDE
		print_result "release upgrade allows automated rate-limit breaker retry" 1 \
			"Expected exit 1 — newer maestro release should permit auto-approval retry"
		return 0
	fi
	unset MAESTRO_CURRENT_VERSION_OVERRIDE
...
test_same_release_preserves_rate_limit_breaker_nmr() {
	...
	# no newer maestro release is available and cooldown has not expired.
	set_timeline '[{"event":"labeled","label":{"name":"needs-maintainer-review"},"actor":{"login":"Aditya060806"},"created_at":"2026-05-07T21:26:05Z"}]'
	set_comments '[{"created_at":"2026-05-07T21:26:06Z","body":"<!-- dispatch-backoff:rate_limit_nmr -->\n## Rate-Limit Backoff Circuit Breaker (t2781)\n---\n[maestro.sh](https://maestro.sh) v3.14.94 automated scan."}]'
	set_issue_meta '{"labels":[{"name":"needs-maintainer-review"},{"name":"origin:worker"},{"name":"auto-dispatch"}]}'
	MAESTRO_CURRENT_VERSION_OVERRIDE=3.14.94
	MAESTRO_RATE_LIMIT_NMR_AUTO_RETRY_SECONDS=999999999
	export MAESTRO_CURRENT_VERSION_OVERRIDE
	export MAESTRO_RATE_LIMIT_NMR_AUTO_RETRY_SECONDS
	if _nmr_applied_by_maintainer 4508 exampleorg/examplerepo Aditya060806; then
		unset MAESTRO_CURRENT_VERSION_OVERRIDE
		unset MAESTRO_RATE_LIMIT_NMR_AUTO_RETRY_SECONDS
...
test_rate_limit_cooldown_allows_retry() {
	...
	set_issue_meta '{"labels":[{"name":"needs-maintainer-review"},{"name":"origin:worker"},{"name":"auto-dispatch"}]}'
	MAESTRO_CURRENT_VERSION_OVERRIDE=3.14.94
	MAESTRO_RATE_LIMIT_NMR_AUTO_RETRY_SECONDS=1
	export MAESTRO_CURRENT_VERSION_OVERRIDE
	export MAESTRO_RATE_LIMIT_NMR_AUTO_RETRY_SECONDS
	if _nmr_applied_by_maintainer 4508 exampleorg/examplerepo Aditya060806; then
		unset MAESTRO_CURRENT_VERSION_OVERRIDE
