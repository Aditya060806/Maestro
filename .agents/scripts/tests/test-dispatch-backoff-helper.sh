...
#   8. Emergency bypass via MAESTRO_SKIP_DISPATCH_BACKOFF=1
...
reset_test_state() {
	...
	: >"$STATS_COUNTER_FILE"
	: >"${TMP}/gh-calls.log"
	unset MAESTRO_SKIP_DISPATCH_BACKOFF 2>/dev/null || true
...
test_bypass() {
	...
	export MAESTRO_SKIP_DISPATCH_BACKOFF=1
	local rc=0
	check_dispatch_backoff "22222" "owner/repo" 2>/dev/null || rc=$?
	unset MAESTRO_SKIP_DISPATCH_BACKOFF
	if [[ "$rc" -eq 0 ]]; then
		pass "MAESTRO_SKIP_DISPATCH_BACKOFF=1 → bypass (exit 0)"
	else
		fail "MAESTRO_SKIP_DISPATCH_BACKOFF=1 → bypass (exit 0)" "got exit ${rc}"
	fi
	return 0
}
