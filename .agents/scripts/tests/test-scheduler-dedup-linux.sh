...
test_duplicate_auto_update_cron_removed() {
	setup_fixture
	printf '%s\n' \
		'*/10 * * * * /home/example/.maestro/agents/scripts/auto-update-helper.sh check # maestro-auto-update' \
		'15 * * * * /usr/bin/true # keep-me' >"$CRON_FILE"
	printf '%s\n' 'maestro-auto-update.timer' >"$SYSTEMD_ENABLED_FILE"
	load_scheduler_functions
	...
	if [[ "$after_first" != *'maestro-auto-update'* && "$after_first" == *'keep-me'* && "$after_first" == "$after_second" && "$output" == *'Removed duplicate cron entry for auto-update'* && "$list_count_after_first" == "1" ]]; then
...
test_duplicate_pulse_merge_cron_removed() {
	setup_fixture
	printf '%s\n' \
		'* * * * * /home/example/.maestro/agents/scripts/pulse-merge-routine.sh run # maestro: pulse-merge-routine' \
		'0 6 * * * /usr/bin/true # legitimate-cron-only' >"$CRON_FILE"
	printf '%s\n' 'maestro-pulse-merge.timer' >"$SYSTEMD_ENABLED_FILE"
	load_scheduler_functions
...
test_multiple_duplicate_cron_entries_removed_with_single_crontab_write() {
	...
	printf '%s\n' 'maestro-auto-update.timer' 'maestro-pulse-merge.timer' >"$SYSTEMD_ENABLED_FILE"
	load_scheduler_functions
	...
	if [[ "$cron_after" != *'maestro-auto-update'* && "$cron_after" != *'pulse-merge-routine'* && "$cron_after" == *'keep-me'* && "$output" == *'Removed duplicate cron entry for auto-update'* && "$output" == *'Removed duplicate cron entry for pulse merge'* && "$list_count" == "1" && "$write_count" == "1" ]]; then
...
test_remove_cron_tag_uses_provided_crontab_without_extra_io() {
	...
	_scheduler_remove_cron_tag "# maestro-auto-update" "auto-update" "$provided_cron" true
	local list_count=""
	list_count="$(cat "$CRON_LIST_COUNT_FILE")"
	local write_count=""
	write_count="$(cat "$CRON_WRITE_COUNT_FILE")"

	if [[ "$_SCHEDULER_RECONCILED_CRON_RESULT" != *'maestro-auto-update'* && "$_SCHEDULER_RECONCILED_CRON_RESULT" == *'keep-me'* && "$list_count" == "0" && "$write_count" == "0" ]]; then
...
test_systemd_only_stats_wrapper_preserved() {
	...
	printf '%s\n' 'maestro-stats-wrapper.timer' >"$SYSTEMD_ENABLED_FILE"
	load_scheduler_functions
...
test_cron_only_scheduler_preserved() {
	setup_fixture
	printf '%s\n' '* * * * * /home/example/pulse-wrapper.sh # maestro: supervisor-pulse' >"$CRON_FILE"
	load_scheduler_functions
	...
	if [[ "$cron_after" == *'maestro: supervisor-pulse'* && -z "$output" ]]; then
