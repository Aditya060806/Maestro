...
# sibling-path false positives (e.g. /tmp/maestro matching /tmp/maestro-tools).
test_has_worker_exact_dir_match_no_sibling_false_positive() {
	local repos_json_path="${HOME}/.config/maestro/repos.json"
	mkdir -p "$(dirname "$repos_json_path")"
	printf '{"initialized_repos":[{"slug":"owner/repo","path":"/tmp/maestro","pulse":true}]}\n' \
		>"$repos_json_path"
	REPOS_JSON="$repos_json_path"

	# Sibling repo path — must NOT match
	PS_MOCK_OUTPUT="/usr/local/bin/.opencode run --dir /tmp/maestro-tools --title \"Issue #42\" \"/full-loop Implement issue #42\""

	if has_worker_for_repo_issue "42" "owner/repo"; then
		print_result "has_worker_for_repo_issue rejects sibling-path match" 1 \
			"Sibling path /tmp/maestro-tools incorrectly matched repo /tmp/maestro"
		return 0
	fi
...
test_has_worker_exact_dir_match_accepts_correct_path() {
	...
	PS_MOCK_OUTPUT="/usr/local/bin/.opencode run --dir /tmp/maestro --title \"Issue #42\" \"/full-loop Implement issue #42\""
	...
	print_result "has_worker_for_repo_issue accepts exact path match" 1 \
		"Exact path /tmp/maestro was not matched"
	return 0
}
...
test_queue_governor_enters_merge_heavy_at_critical_backlog() {
	...
	: >"$STATE_FILE"
	printf '12\n' >"${HOME}/.maestro/logs/pulse-max-workers"
