...
test_emit_optin() {
	...
	# Off-state: no output.
	local out_off
	out_off=$( (MAESTRO_VERBOSE_LIFECYCLE=0 _emit_verbose_checkpoint test_event "k=v") 2>&1 )
	if [[ -z "$out_off" ]]; then
		echo "${TEST_GREEN}PASS${TEST_NC}: _emit_verbose_checkpoint silent when MAESTRO_VERBOSE_LIFECYCLE=0"
	else
		TESTS_FAILED=$((TESTS_FAILED + 1))
		echo "${TEST_RED}FAIL${TEST_NC}: _emit_verbose_checkpoint silent when MAESTRO_VERBOSE_LIFECYCLE=0"
	...
	local out_on
	out_on=$( (MAESTRO_VERBOSE_LIFECYCLE=1 _emit_verbose_checkpoint test_event "k=v") 2>&1 )
...
test_preflight_sentinel() {
	...
	local rc_off
	(MAESTRO_WORKER_PREFLIGHT_SENTINEL=0 HOME="$_tmp_home" _t3077_write_preflight_sentinel) && rc_off=0 || rc_off=$?
	assert_rc "preflight sentinel no-op exits 0 when env=0" "0" "$rc_off"
	if [[ -z "$(find "$_tmp_home/.maestro/cache/worker-preflight" -type f 2>/dev/null)" ]]; then
	...
	local rc_on
	(MAESTRO_WORKER_PREFLIGHT_SENTINEL=1 HOME="$_tmp_home" _t3077_write_preflight_sentinel) && rc_on=0 || rc_on=$?
	assert_rc "preflight sentinel exits 0 when env=1" "0" "$rc_on"
	if [[ -n "$(find "$_tmp_home/.maestro/cache/worker-preflight" -type f 2>/dev/null)" ]]; then
