...
#   7. MAESTRO_BASH_REEXECED=1 prevents re-exec (loop guard)
#   8. No `$'\0'` in parameter expansion (GH#18830 regression) — sanity
#      check that the earlier class fix is still in place
#   9. (t2201) MAESTRO_BASH_REEXECED is UNSET after source on bash 4+,
...
#   - deployed:      ~/.maestro/agents/scripts/tests/ → ~/.maestro/agents/scripts/
...
if [[ ! -f "$SHARED" ]]; then
	...
	first_guard_line=$(grep -n "MAESTRO_BASH_REEXECED" "$SHARED" | head -1 | cut -d: -f1)
...
# -----------------------------------------------------------------------
# Test 7: MAESTRO_BASH_REEXECED=1 prevents re-exec loop (tested via a
# synthetic script that sources shared-constants and reports its version).
# -----------------------------------------------------------------------
TMP_SCRIPT="$(mktemp -t "maestro-test-bash-reexec.XXXXXX")" || {
	_fail "mktemp failed"
...
echo "reexeced=\${MAESTRO_BASH_REEXECED:-0}"
EOF
chmod +x "$TMP_SCRIPT"

# Run under /bin/bash 3.2 with MAESTRO_BASH_REEXECED already set — the
# guard should skip and we should see running_bash=3 (or whatever /bin/bash
# is on this system).
guard_output="$(MAESTRO_BASH_REEXECED=1 /bin/bash "$TMP_SCRIPT" 2>&1 || true)"
current_major=$(/bin/bash -c 'echo ${BASH_VERSINFO[0]}' 2>/dev/null || echo 0)
if [[ "$guard_output" == *"running_bash=${current_major}"* ]]; then
	_pass "MAESTRO_BASH_REEXECED=1 prevents re-exec (ran under bash ${current_major})"
else
	_fail "MAESTRO_BASH_REEXECED=1 did not prevent re-exec" \
		"output: $guard_output"
fi
...
if [[ -n "$modern_path" ]] && [[ "$current_major" -lt 4 ]]; then
	# Run WITHOUT MAESTRO_BASH_REEXECED set; the guard should fire and
...
# -----------------------------------------------------------------------
# Test 12 (GH#18965 / t2094): MAESTRO_AUTO_UPGRADE_BASH=0 short-circuits
# ensure without calling brew. We verify by running it with the env var
# set and confirming rc=0 + no brew-related error output.
# -----------------------------------------------------------------------
optout_output="$(MAESTRO_AUTO_UPGRADE_BASH=0 "$HELPER" ensure 2>&1 || true)"
optout_rc=0
MAESTRO_AUTO_UPGRADE_BASH=0 "$HELPER" ensure >/dev/null 2>&1 || optout_rc=$?
if [[ "$optout_rc" -eq 0 ]] && [[ "$optout_output" == *"MAESTRO_AUTO_UPGRADE_BASH=0"* ]]; then
	_pass "MAESTRO_AUTO_UPGRADE_BASH=0 short-circuits ensure (rc=0, skip message seen)"
elif [[ "$optout_rc" -eq 0 ]]; then
	# --quiet suppressed the message but rc=0 is still valid
	_pass "MAESTRO_AUTO_UPGRADE_BASH=0 short-circuits ensure (rc=0)"
else
	_fail "MAESTRO_AUTO_UPGRADE_BASH=0 did not short-circuit cleanly" \
...
if [[ -n "$modern_path" ]] && [[ "$current_major" -lt 4 ]]; then
	...
	TMP_INTERMEDIATE="$(mktemp -t "maestro-test-intermediate.XXXXXX")" || {
		_fail "mktemp for intermediate failed"
	}
	TMP_TOP="$(mktemp -t "maestro-test-toplevel.XXXXXX")" || {
...
TOP_EOF
	...
	# Run under /bin/bash (3.2) WITHOUT MAESTRO_BASH_REEXECED. The guard
	# should walk BASH_SOURCE to find the outermost caller (TMP_TOP) and
	# re-exec it — NOT the intermediate script.
	indirect_output="$(env -u MAESTRO_BASH_REEXECED /bin/bash "$TMP_TOP" 2>&1 || true)"
...
# -----------------------------------------------------------------------
TMP_NOBASH="$(mktemp -t "maestro-test-nobash.XXXXXX")" || {
...
# -----------------------------------------------------------------------
# Test 15 (t2201): MAESTRO_BASH_REEXECED env-var leak from parent to
...
# MAESTRO_BASH_REEXECED=1 from the pulse process's re-exec, saw the
...
if [[ -n "$modern_path" ]]; then
	...
	direct_output="$(MAESTRO_BASH_REEXECED=1 "$modern_path" -c "source '$SHARED'; echo \"after_source=\${MAESTRO_BASH_REEXECED:-UNSET}\"" 2>&1 || true)"
	if [[ "$direct_output" == *"after_source=UNSET"* ]]; then
		_pass "env-var leak: bash 4+ unsets MAESTRO_BASH_REEXECED after source (t2201)"
	else
		_fail "env-var leak: bash 4+ did not unset MAESTRO_BASH_REEXECED" \
	...
	if [[ "$current_major" -lt 4 ]]; then
		TMP_GRANDCHILD="$(mktemp -t "maestro-test-grandchild.XXXXXX")" || {
...
echo "grandchild_reexeced=\${MAESTRO_BASH_REEXECED:-UNSET}"
GRANDCHILD_EOF
			...
			leak_output="$(MAESTRO_BASH_REEXECED=1 "$modern_path" -c "source '$SHARED'; /bin/bash '$TMP_GRANDCHILD'" 2>&1 || true)"
