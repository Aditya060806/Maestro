...
#
# Regression tests for maestro status headless runtime configuration warnings.

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" || exit
STATUS_LIB="${SCRIPT_DIR}/../maestro-cli/maestro-status-lib.sh"
...
run_status_check() {
	...
	MAESTRO_CREDENTIALS_FILE="$credentials_file" _status_headless_runtime_config
	return 0
}
...
cleanup_tmp_dir() {
	...
	if [[ "$tmp_base" != maestro-status-env.* ]]; then
...
# shellcheck source=../maestro-cli/maestro-status-lib.sh
source "$STATUS_LIB"

TMP_DIR=$(mktemp -d -t maestro-status-env.XXXXXX) || exit 1
...
printf '%s\n' 'export MAESTRO_HEADLESS_PROVIDER_ALLOWLIST="anthropic"' >"$configured_credentials" || exit 1
printf '%s\n' 'export MAESTRO_HEADLESS_PROVIDER_ALLOWLIST="openai"' >"${config_dir}/credentials.sh" || exit 1

MAESTRO_HEADLESS_PROVIDER_ALLOWLIST="anthropic" output=$(run_status_check "$missing_credentials" 2>&1)
...
MAESTRO_HEADLESS_PROVIDER_ALLOWLIST="anthropic" output=$(run_status_check "$configured_credentials" 2>&1)
...
unset MAESTRO_CREDENTIALS_FILE
