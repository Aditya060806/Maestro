...
export MAESTRO_HEADLESS_METRICS_FILE="${HOME}/.maestro/logs/headless-runtime-metrics.jsonl"
: >"$LOGFILE"
: >"$MAESTRO_HEADLESS_METRICS_FILE"
...
reset_guardrail_env() {
	...
	unset MAESTRO_SKIP_PULSE_CURRENT_STATE_GUARDRAILS 2>/dev/null || true
...
test_disabled_guardrail_still_updates_available_slots_gauge() {
	reset_guardrail_env
	export MAESTRO_SKIP_PULSE_CURRENT_STATE_GUARDRAILS=1
	local slots
...
test_benign_block_ledger_is_cycle_local_and_cleaned() {
	...
	_dispatch_mark_benign_blocked_candidate 23541 Aditya060806/Maestro dedup_active_claim
	_dispatch_cleanup_benign_blocks_cycle
	_dispatch_begin_benign_blocks_cycle >/dev/null
	second_ledger="$_DISPATCH_BENIGN_BLOCKS_FILE"
	lingering_reason=$(_dispatch_benign_blocked_candidate_reason 23541 Aditya060806/Maestro 2>/dev/null || true)
	_dispatch_cleanup_benign_blocks_cycle
...
test_external_benign_block_ledger_is_preserved_and_refreshed() {
	...
	local ledger_path=""
	printf '%s\t%s\t%s\n' 101 Aditya060806/Maestro caller_managed >"$external_ledger"
	export MAESTRO_PULSE_BENIGN_BLOCKS_FILE="$external_ledger"
	_dispatch_begin_benign_blocks_cycle >/dev/null
	ledger_path="$_DISPATCH_BENIGN_BLOCKS_FILE"
	_dispatch_mark_benign_blocked_candidate 23575 Aditya060806/Maestro dedup_active_claim
	_dispatch_cleanup_benign_blocks_cycle
	unset MAESTRO_PULSE_BENIGN_BLOCKS_FILE
	if [[ "$ledger_path" == "$external_ledger" ]] && [[ -f "$external_ledger" ]] && ! grep -q $'^101\tAditya060806/Maestro\tcaller_managed$' "$external_ledger" && grep -q $'^23575\tAditya060806/Maestro\tdedup_active_claim$' "$external_ledger"; then
...
test_apply_dispatch_max_preserves_benign_ledger_across_refill() {
	reset_guardrail_env
	export MAESTRO_MIN_WORKER_CONCURRENCY=2
	...
	dispatch_max() {
		...
		if [[ "$current_calls" -eq 1 ]]; then
			...
			_dispatch_benign_blocked_candidate_reason 23541 Aditya060806/Maestro >"$refill_reason_file" 2>/dev/null || true
			printf '0\n'
		fi
		return 0
	}
	...
	unset MAESTRO_MIN_WORKER_CONCURRENCY
