...
title_output=$(run_case "title fallback" '
	...
	_fetch_target_title issue 123 Aditya060806/Maestro
' 0)
...
success_output=$(run_case "verified success" '
	...
	gh() {
		if [[ "${1:-}" == "api" && "${2:-}" == "user" ]]; then printf "Aditya060806"; return 0; fi
		if [[ "${1:-}" == "issue" && "${2:-}" == "lock" ]]; then return 1; fi
		if [[ "${1:-}" == "api" && "${2:-}" == "-X" && "${3:-}" == "PUT" ]]; then return 0; fi
		if [[ "${1:-}" == "api" && "${2:-}" == "/repos/Aditya060806/Maestro/issues/123" ]]; then
