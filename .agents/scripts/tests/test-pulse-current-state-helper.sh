...
open(os.path.join(root, 'dispatch-stages.tsv'), 'w').write(
    f'{iso}\t#1\tAditya060806/Maestro\tworker_launch_total\t123\n'
    f'{iso}\t#1\tAditya060806/Maestro\tceremony_total\t456\n'
)
...
export MAESTRO_PULSE_RATE_LIMIT_CACHE="$TMP_DIR/rate-limit-cache.json"
export MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR="$TMP_DIR/review-thread-state"
python3 - "$MAESTRO_PULSE_RATE_LIMIT_CACHE" <<'PY'
...
assert 'MAESTRO_ACTIVE_WORKER_PROCESSES' in helper_source
assert 'MAESTRO_ACTIVE_WORKER_PROCESSES' in implementation_source
