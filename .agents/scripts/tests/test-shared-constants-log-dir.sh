...
# back to ~/.maestro/logs when no config is present.
# =============================================================================
set -euo pipefail
...
_test "Default fallback when no _jsonc_get" "$HOME/.maestro/logs" "$result"
...
_jsonc_get() {
	...
	if [[ "$dotpath" == "paths.log_dir" ]]; then
		printf '%s' "/tmp/custom-maestro-logs"
	else
		printf '%s' "$default"
	fi
	return 0
...
result=$(_resolve_log_dir)
_test "Custom paths.log_dir via _jsonc_get" "/tmp/custom-maestro-logs" "$result"
...
result=$(_resolve_log_dir)
_test "Tilde expansion in paths.log_dir" "$HOME/.maestro/custom-logs" "$result"
...
result=$(_resolve_log_dir)
_test "_jsonc_get returns default value" "$HOME/.maestro/logs" "$result"
