...
setup_fixture() {
	...
	git -C "$REPO_DIR" config user.email "test@maestro.sh"
...
assert_eq "title renamed to feature branch with suffix" "feature/cool-thing · Maestro 9.8.7" "$(get_title)"
...
assert_eq "empty title filled in with suffix" "feature/empty-title · Maestro 9.8.7" "$(get_title)"
...
assert_eq "main title healed to feature with suffix" "feature/heal-me · Maestro 9.8.7" "$(get_title)"
...
assert_eq "master title healed with suffix" "feature/heal-master · Maestro 9.8.7" "$(get_title)"
...
assert_eq "explicit rename to main allowed with suffix" "main · Maestro 9.8.7" "$(get_title)"

# Test 9: explicit rename is idempotent and replaces older suffix versions
echo ""
echo "Test 9: explicit rename replaces existing Maestro suffix"
seed_session "New Session"
OPENCODE_DB="$DB_PATH" "$HELPER" rename "$SESSION_ID" "Issue #123: concise title · Maestro 1.2.3" >/dev/null 2>&1
rc=$?
assert_exit "exit 0 explicit suffix replacement" "0" "$rc"
assert_eq "old suffix replaced" "Issue #123: concise title · Maestro 9.8.7" "$(get_title)"
...
assert_eq "issue prefix preserved before suffix" "Issue #456: preserve prefix · Maestro 9.8.7" "$(get_title)"

# Test 11: version fallback reads the repository root VERSION
echo ""
echo "Test 11: repo-root VERSION fallback works"
seed_session "New Session"
env -u MAESTRO_VERSION OPENCODE_DB="$DB_PATH" "$HELPER" rename "$SESSION_ID" "Issue #789: root version" >/dev/null 2>&1
rc=$?
assert_exit "exit 0 root version fallback" "0" "$rc"
assert_eq "root VERSION suffix used" "Issue #789: root version · Maestro ${ROOT_VERSION}" "$(get_title)"
