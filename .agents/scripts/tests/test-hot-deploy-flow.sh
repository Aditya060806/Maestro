...
if [[ ! -f "$VM_SCRIPT" ]]; then
	VM_SCRIPT="$HOME/.maestro/agents/scripts/version-manager.sh"
fi

# Locate maestro-update-check.sh
UC_SCRIPT="${SCRIPT_DIR}/../maestro-update-check.sh"
if [[ ! -f "$UC_SCRIPT" ]]; then
	UC_SCRIPT="$HOME/.maestro/agents/scripts/maestro-update-check.sh"
fi
...
test_hotfix_conf_template() {
	...
	if [[ ! -f "$conf_path" ]]; then
		# Try deployed path
		conf_path="$HOME/.maestro/agents/configs/auto-hotfix.conf"
	fi
...
test_hotfix_check_function_exists() {
	...
	if [[ ! -f "$UC_SCRIPT" ]]; then
		skip "$test_name" "maestro-update-check.sh not found"
		return 0
	fi
...
# =============================================================================
# Test 9: MAESTRO_FORCE_HOTFIX_BANNER env var is respected
# =============================================================================
test_force_hotfix_banner_env() {
	local test_name="MAESTRO_FORCE_HOTFIX_BANNER env var is respected"
	...
	if grep -q 'MAESTRO_FORCE_HOTFIX_BANNER' "$UC_SCRIPT"; then
		pass "$test_name"
	else
		fail "$test_name" "MAESTRO_FORCE_HOTFIX_BANNER not found in update-check.sh"
	fi
	return 0
}
...
test_reference_doc_exists() {
	...
	if [[ ! -f "$doc_path" ]]; then
		doc_path="$HOME/.maestro/agents/reference/hot-deploy.md"
	fi
