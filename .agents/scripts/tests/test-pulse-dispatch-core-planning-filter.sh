...
test_planning_pr_squash_commit_does_not_false_positive() {
	...
	local result=1
	if _task_id_in_recent_commits "t2366: r914 repo-maestro-health" "$TEST_REPO_PATH" "2020-01-01T00:00:00Z"; then
		result=1
	else
		result=0
	fi
...
test_real_impl_pr_squash_commit_detected() {
	...
	commit_files_to_main "$TEST_REPO_PATH" \
		...
		"TODO.md" ".agents/scripts/repo-maestro-health-helper.sh"
