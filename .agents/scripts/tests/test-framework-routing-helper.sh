...
if [[ "${1:-}" == "issue" && "${2:-}" == "create" ]]; then
	printf '%s\n' "https://github.com/Aditya060806/Maestro/issues/9104"
	exit 0
fi
...
EOF
	...
	mkdir -p "${tmp_home}/.config/maestro"
	cat >"${tmp_home}/.config/maestro/repos.json" <<'EOF'
{"initialized_repos":[{"slug":"Aditya060806/Maestro","path":"/tmp/maestro"}]}
EOF
...
assert_result "~/.maestro path + agent prompt" "framework" \
	"update ~/.maestro/ agent prompt for cross-repo orchestration"
...
if run_log_framework_issue_case "[]" "$empty_array_output" "$empty_array_trace" "${TMP_DIR}/home-empty-array" "${TMP_DIR}/stub-empty-array"; then
	...
	assert_contains "$empty_array_text" "https://github.com/Aditya060806/Maestro/issues/9104" "empty array issue-list result creates issue"
...
if run_log_framework_issue_case "null" "$null_output" "$null_trace" "${TMP_DIR}/home-null" "${TMP_DIR}/stub-null"; then
	...
	assert_contains "$null_text" "https://github.com/Aditya060806/Maestro/issues/9104" "null issue-list result creates issue"
...
# --- get-maestro-path and get-maestro-slug ---
echo "Path/slug resolution:"

maestro_path=$("$HELPER" get-maestro-path 2>/dev/null) || maestro_path=""
if [[ -n "$maestro_path" && -d "$maestro_path" ]]; then
	PASS=$((PASS + 1))
	echo "  PASS: get-maestro-path returned valid path: $maestro_path"
else
	# This may fail in CI where the repo isn't at the expected path
	echo "  SKIP: get-maestro-path (repo not found at expected location)"
fi

maestro_slug=$("$HELPER" get-maestro-slug 2>/dev/null) || maestro_slug=""
if [[ -n "$maestro_slug" && "$maestro_slug" == *"maestro"* ]]; then
	PASS=$((PASS + 1))
	echo "  PASS: get-maestro-slug returned: $maestro_slug"
else
	echo "  SKIP: get-maestro-slug (slug not resolvable in this environment)"
fi
