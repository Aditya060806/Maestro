...
# `sudo maestro approve issue/pr <N>` and the pulse-merge cycle acting on
# the linked PR:
#
#   1. approval-helper.sh::_kick_pulse_after_approval writes a TSV record
#      to ~/.maestro/cache/pulse-merge-trigger.txt on every approval.
...
#   MAESTRO_SKIP_APPROVE_KICK_PULSE=1   — disable the helper entirely
#   MAESTRO_SKIP_TRIGGER_DRAIN=1        — disable the drain
#
...
mkdir -p "${test1_home}/.maestro/cache"
...
bash -c "
	...
	_kick_pulse_after_approval issue 21806 Aditya060806/Maestro
	_kick_pulse_after_approval pr 21807 Aditya060806/Maestro
	...
	_kick_pulse_after_approval bogustype 21809 Aditya060806/Maestro
" >/dev/null 2>&1

# Verify the marker landed at the canonical sandbox path.
expected_marker="${test1_home}/.maestro/cache/pulse-merge-trigger.txt"
...
assert_eq "first record slug field is Aditya060806/Maestro" "Aditya060806/Maestro" "$slug_field"
...
(
	...
	printf 'Aditya060806/Maestro\t21806\tpr\t2026-04-30T12:00:00Z\n' >"$TRIGGER_FILE"
...
if [[ -s "$PROCESS_LOG" ]]; then
	...
	assert_eq "process_pr called with slug + PR number" \
		"process_pr Aditya060806/Maestro 21806" "$logged_call"
else
...
(
	...
	# and skip the rest without aborting.
	{
		...
		printf 'Aditya060806/Maestro\tNOTANUMBER\tpr\t2026-04-30T12:00Z\n'   # bad num
		printf 'Aditya060806/Maestro\t21807\tbogus\t2026-04-30T12:00:00Z\n'  # bad type
		printf 'Aditya060806/Maestro\t21808\tpr\t2026-04-30T12:00:00Z\n'    # VALID
		printf 'Aditya060806/Maestro\t21809\tpr\t2026-04-30T12:00:00Z\n'    # VALID
	} >"$TRIGGER_FILE"
...
# -----------------------------------------------------------------------------
echo "Test 5: MAESTRO_SKIP_TRIGGER_DRAIN=1 short-circuits"
echo "===================================================="
...
(
	...
	export MAESTRO_SKIP_TRIGGER_DRAIN=1
	_drain_merge_trigger_file_if_present || exit 1
)
drain_rc=$?
...
(
	...
	{
		printf 'Aditya060806/Maestro\t12345\tissue\t2026-04-30T12:00:00Z\n'
		printf 'Aditya060806/Maestro\t99998\tissue\t2026-04-30T12:00:00Z\n'
	} >"$TRIGGER_FILE"
...
assert_eq "issue 12345 resolved to PR 99999 by stub" \
	"process_pr Aditya060806/Maestro 99999" "$logged_call"
...
(
	...
	_resolve_linked_pr_for_issue "Aditya060806/Maestro" "12345"
)
)
