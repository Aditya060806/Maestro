...
export HOME="$TMP"
mkdir -p "$TMP/.maestro/advisories"
...
export MAESTRO_AGENTS_DIR="$TMP/.maestro/agents"
mkdir -p "$MAESTRO_AGENTS_DIR"
...
reset_state() {
	...
	FINDINGS_JSON="[]"
	true >"$OUTPUT_LOG"
	rm -f "$TMP/.maestro/advisories/sync-pat-"*
	rm -f "$TMP/.maestro/advisories/.dismissed-sync-pat-"*
	# Reset stub defaults
...
ADVISORY_PATH="$TMP/.maestro/advisories/sync-pat-testowner-testrepo.advisory"
...
reset_state
touch "$TMP/.maestro/advisories/.dismissed-sync-pat-testowner-testrepo"
