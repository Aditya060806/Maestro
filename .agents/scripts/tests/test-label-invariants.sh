...
mkdir -p "${HOME}/.maestro/logs" "${HOME}/.maestro/.agent-workspace/supervisor" "${HOME}/.maestro/cache"
...
COUNTER_FILE="${HOME}/.maestro/cache/pulse-label-invariants.$(hostname -s 2>/dev/null || echo unknown).json"
