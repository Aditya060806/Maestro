...
test_dashboard_preserves_partial_cache() {
	...
	mkdir -p "${fake_home}/.maestro/agents/scripts" "${TMP_DIR}/cache" "${TMP_DIR}/bin"
	cat >"${fake_home}/.maestro/agents/scripts/contributor-activity-helper.sh" <<'FAKE'
#!/usr/bin/env bash
case "$1" in
person-stats)
...
exit 1
FAKE
	chmod +x "${fake_home}/.maestro/agents/scripts/contributor-activity-helper.sh"
...
test_dashboard_skips_marker_when_all_refreshes_fail() {
	...
	mkdir -p "${fake_home}/.maestro/agents/scripts" "${TMP_DIR}/cache-fail" "${TMP_DIR}/bin-fail"
	cat >"${fake_home}/.maestro/agents/scripts/contributor-activity-helper.sh" <<'FAKE'
#!/usr/bin/env bash
exit 124
FAKE
