...
test_missing_file() {
	...
	local output
	output=$(_build_plist_env_overrides_xml "com.maestro.maestro-supervisor-pulse" "$missing_file")
	if [[ -z "$output" ]]; then
...
EOF
	local output
	output=$(_build_plist_env_overrides_xml "com.maestro.maestro-supervisor-pulse" "$override_file")
...
test_malformed_json_logs_warn_and_emits_nothing() {
	...
	stderr_output=$(_build_plist_env_overrides_xml "com.maestro.maestro-supervisor-pulse" "$override_file" 2>&1 >/dev/null) || true
	output=$(_build_plist_env_overrides_xml "com.maestro.maestro-supervisor-pulse" "$override_file" 2>/dev/null) || true
...
EOF
	local xml_fragment
	xml_fragment=$(_build_plist_env_overrides_xml "com.maestro.maestro-supervisor-pulse" "$override_file")
