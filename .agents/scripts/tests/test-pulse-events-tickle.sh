...
setup_sandbox() {
	...
	mkdir -p "${HOME}/.maestro/cache/pulse-events-etag"
	mkdir -p "${HOME}/.maestro/cache"
	export MAESTRO_GH_SECONDARY_COOLDOWN_FILE="${HOME}/.maestro/cache/gh-secondary-cooldown.json"
	LOGFILE="${HOME}/.maestro/logs/pulse-wrapper.log"
	export LOGFILE
...
test_304_returns_fresh() {
	...
	printf '{"etag":"abc123","owner_type":"users","last_check":"2026-01-01T00:00:00Z"}\n' \
		>"${HOME}/.maestro/cache/pulse-events-etag/testowner.json"
...
test_200_first_call_caches_etag() {
	...
	assert_file_exists "test_200_first: cache file created" \
		"${HOME}/.maestro/cache/pulse-events-etag/newowner.json"
	assert_file_contains "test_200_first: ETag stored in cache" \
		"${HOME}/.maestro/cache/pulse-events-etag/newowner.json" "etag-first-call"
	assert_file_contains "test_200_first: owner_type stored as users" \
		"${HOME}/.maestro/cache/pulse-events-etag/newowner.json" "users"
...
test_200_updates_cache() {
	...
	printf '{"etag":"old-etag","owner_type":"users","last_check":"2026-01-01T00:00:00Z"}\n' \
		>"${HOME}/.maestro/cache/pulse-events-etag/existingowner.json"
	...
	assert_file_contains "test_200_update: cache updated with new ETag" \
		"${HOME}/.maestro/cache/pulse-events-etag/existingowner.json" "new-etag-value"
...
STUB_EOF
	...
	assert_file_contains "test_org_retry: owner_type set to orgs in cache" \
		"${HOME}/.maestro/cache/pulse-events-etag/myorg.json" "orgs"
	assert_file_contains "test_org_retry: org ETag stored in cache" \
		"${HOME}/.maestro/cache/pulse-events-etag/myorg.json" "org-etag"
...
test_rate_limit_returns_unknown() {
	...
	assert_file_exists "test_rate_limit: cooldown file created" \
		"${HOME}/.maestro/cache/gh-secondary-cooldown.json"
	assert_file_contains "test_rate_limit: cooldown reason recorded" \
		"${HOME}/.maestro/cache/gh-secondary-cooldown.json" "github-api-rate-limit-status-403"
...
STUB_EOF
	...
	printf '{"etag":"cached-etag","owner_type":"users","last_check":"2026-01-01T00:00:00Z"}\n' \
		>"${HOME}/.maestro/cache/pulse-events-etag/freshowner.json"
...
test_batch_prefetch_skips_fresh_owner() {
	...
	printf '{"etag":"seeded-etag","owner_type":"users","last_check":"2026-01-01T00:00:00Z"}\n' \
		>"${HOME}/.maestro/cache/pulse-events-etag/testpulseowner.json"
