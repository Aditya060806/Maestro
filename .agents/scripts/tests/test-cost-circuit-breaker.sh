...
write_fixture_comments() {
	...
	for amount in ${spends//,/ }; do
		...
		comments+="{\"body\":\"Worker comment.\\n\\n<!-- maestro:sig -->\\n---\\n[maestro.sh](https://maestro.sh) v3.7.0 plugin for [OpenCode](https://opencode.ai) v1.4.3 with claude-opus-4-6 spent 4m and ${amount} tokens on this as a headless worker.\"}"
	done
...
if grep -q 'sudo maestro approve issue 18002 owner/repo' "$STUB_LOG" 2>/dev/null; then
...
[
  ...
  {"created_at":"2026-05-03T19:25:00Z","body":"<!-- maestro-signed-approval -->\nMaintainer approval."},
