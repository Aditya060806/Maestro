...
#   3. Sets MAESTRO_WORKER_PREWARM_DIR env var for headless-runtime-helper.
...
run_prewarm_block() {
	...
	(
		...
		if command -v opencode >/dev/null 2>&1; then
			worker_prewarm_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-worker-auth.XXXXXX") || worker_prewarm_dir=""
...
# ---------------------------------------------------------------------------
# Test: headless-runtime-helper.sh honours MAESTRO_WORKER_PREWARM_DIR
# ---------------------------------------------------------------------------
test_headless_runtime_honours_prewarm_dir() {
	...
	if ! grep -q "MAESTRO_WORKER_PREWARM_DIR" "$helper_path"; then
		print_result "prewarm_env_var_in_helper" 1 "MAESTRO_WORKER_PREWARM_DIR not found in headless-runtime-helper.sh"
		rc=1
	else
...
test_launch_script_has_prewarm() {
	...
	if ! grep -q "MAESTRO_WORKER_PREWARM_DIR" "$launch_path"; then
		print_result "prewarm_env_var_in_launch_script" 1 "MAESTRO_WORKER_PREWARM_DIR not found in pulse-dispatch-worker-launch.sh"
		rc=1
	else
