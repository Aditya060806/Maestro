...
write_repos_config() {
	local repo_path="$1"
	local config_dir="${TEST_DIR}/.config/maestro"
	mkdir -p "$config_dir"
...
run_helper_in_isolation() {
	...
	if [[ -n "$extra_env_name" ]]; then
		env HOME="$TEST_DIR" \
			...
			MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT="${MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT:-1}" \
			MAESTRO_REMOTE_BRANCH_CLEANUP_APPLY="${MAESTRO_REMOTE_BRANCH_CLEANUP_APPLY:-0}" \
...
test_cold_start_dry_run() {
	...
	MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT=1 run_helper_in_isolation
...
test_apply_requires_explicit_flag() {
	...
	local marker_file="${TEST_DIR}/mock-ran"
	local last_run_file="${TEST_DIR}/.maestro/logs/cleanup_remote_branches.last-run"
	make_repo "$repo_path"
	write_repos_config "$repo_path"
	rm -f "$marker_file" "$last_run_file"

	MAESTRO_REMOTE_BRANCH_CLEANUP_APPLY=1 MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT=1 \
		run_helper_in_isolation
...
test_cadence_gate() {
	...
	local logs_dir="${TEST_DIR}/.maestro/logs"
	local marker_file="${TEST_DIR}/mock-ran"
	make_repo "$repo_path"
	write_repos_config "$repo_path"
	mkdir -p "$logs_dir"
	...
	MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT=1 CLEANUP_REMOTE_BRANCHES_ASYNC_CADENCE_MIN=10 \
		run_helper_in_isolation
...
STUB
	...
	env HOME="$TEST_DIR" PATH="$stub_dir:$PATH" \
		MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT=0 \
		MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING=100 \
		bash "${stub_dir}/cleanup-remote-branches-async-helper.sh" 2>/dev/null || true
