...
CACHE
	...
	if [[ ! -f "${TEST_ROOT}/home/.cache/gh/graphql-401.cache" ]] && \
		find "${TEST_ROOT}/home/.cache/gh" -path '*/maestro-quarantine-*/graphql-401.cache' -type f | grep -q .; then
		stale_quarantined=1
	fi
	...
	if [[ ! -f "${TEST_ROOT}/home/.cache/gh/api/shared.cache" ]] && \
		...
		find "${TEST_ROOT}/home/.cache/gh" -path '*/maestro-quarantine-*/api/shared.cache' -type f | grep -q . && \
		find "${TEST_ROOT}/home/.cache/gh" -path '*/maestro-quarantine-*/graphql/shared.cache' -type f | grep -q .; then
