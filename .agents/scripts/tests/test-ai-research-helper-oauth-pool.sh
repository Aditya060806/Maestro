...
setup_sandbox() {
	...
	mkdir -p "${HOME}/.maestro/cache" "${TEST_ROOT}/bin"
...
#!/usr/bin/env bash
if [[ "${MAESTRO_HEADLESS:-}" != "1" ]]; then
	printf 'missing MAESTRO_HEADLESS=1\n' >&2
	exit 43
fi
for arg in "$@"; do
	if [[ "$arg" == "--pure" ]]; then
...
test_detector_rc2_records_cooldown() {
	...
	(
		...
		export MAESTRO_FIX_THE_FIXER_DETECTOR_AUTH_COOLDOWN_SECONDS=3600
		"${REPO_ROOT}/.agents/scripts/pulse-fix-the-fixer-detector.sh" run \
			--repo example/repo --limit 1 >"${TEST_ROOT}/detector.log" 2>&1
	)

	local cooldown_file="${HOME}/.maestro/cache/fix-the-fixer-detector-auth.cooldown"
