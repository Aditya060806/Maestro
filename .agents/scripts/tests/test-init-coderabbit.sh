...
load_coderabbit_function() {
	local func_body
	func_body=$(sed -n '/^_init_configure_coderabbit_abort_on_close() {/,/^}/p' "$MAESTRO_INIT_LIB")
	eval "$func_body"
	return 0
}
...
test_reviews_with_inline_comment_is_reused() {
	...
	MAESTRO_CODERABBIT_ABORT_ON_CLOSE=false _init_configure_coderabbit_abort_on_close "$project_root" "true"
