...
test_fallback_home_workspace() {
	local root=""
	root=$(maestro_pulse_tmp_root)
	[[ "$root" == "${HOME}/.maestro/.agent-workspace/tmp/pulse" ]] || return 1
	[[ -d "$root" ]] || return 1
	return 0
}
...
test_xdg_runtime_preferred() {
	...
	root=$(maestro_pulse_tmp_root)
	[[ "$root" == "${runtime}/maestro/pulse" ]] || return 1
	return 0
}
...
test_override_wins() {
	...
	local root=""
	export MAESTRO_PULSE_TMP_DIR="$override"
	root=$(maestro_pulse_tmp_root)
...
test_worker_log_setup_uses_per_user_dir() {
	...
	fallback_log=$(maestro_pulse_worker_log_fallback_path "12345")
	[[ "$worker_log" == "${HOME}/.maestro/.agent-workspace/tmp/pulse/pulse-owner-repo-12345.log" ]] || return 1
