...
#   5. MAESTRO_SKIP_PULSE_IDLE_BACKOFF=1 disables gate (always proceed).
...
export MAESTRO_PULSE_IDLE_STATE_FILE="${TEST_STATE_DIR}/state.json"

# Predictable schedule (matches helper defaults).
export MAESTRO_PULSE_IDLE_BASE_INTERVAL_S=90
export MAESTRO_PULSE_IDLE_BACKOFF_THRESHOLD_5=5
export MAESTRO_PULSE_IDLE_BACKOFF_STEP_5_S=180
export MAESTRO_PULSE_IDLE_BACKOFF_THRESHOLD_10=10
export MAESTRO_PULSE_IDLE_BACKOFF_STEP_10_S=300
export MAESTRO_PULSE_IDLE_BACKOFF_THRESHOLD_20=20
export MAESTRO_PULSE_IDLE_BACKOFF_STEP_20_S=600
export MAESTRO_PULSE_IDLE_BACKOFF_THRESHOLD_30=30
export MAESTRO_PULSE_IDLE_BACKOFF_STEP_30_S=1800
unset MAESTRO_SKIP_PULSE_IDLE_BACKOFF
...
# -----------------------------------------------------------------------------
# Test 6: MAESTRO_SKIP_PULSE_IDLE_BACKOFF disables gate
# -----------------------------------------------------------------------------
echo ""
echo "Test 6: MAESTRO_SKIP_PULSE_IDLE_BACKOFF=1 disables gate"
reset_state
...
rc=0
MAESTRO_SKIP_PULSE_IDLE_BACKOFF=1 "$HELPER" should-skip "$((now - 100))" >/dev/null 2>&1 || rc=$?
...
rc=0
MAESTRO_PULSE_IDLE_AVAILABLE_WORK=1 "$HELPER" should-skip "$((now - 100))" >/dev/null 2>&1 || rc=$?
...
"$HELPER" record-cycle idle >/dev/null 2>&1
if jq -e . "$MAESTRO_PULSE_IDLE_STATE_FILE" >/dev/null 2>&1; then
	PASS=$((PASS + 1))
...
"$HELPER" record-cycle idle >/dev/null 2>&1
[[ -f "$MAESTRO_PULSE_IDLE_STATE_FILE" ]] && pre_exists="yes" || pre_exists="no"
assert_eq "state file exists pre-reset" "yes" "$pre_exists"

"$HELPER" reset >/dev/null 2>&1
[[ -f "$MAESTRO_PULSE_IDLE_STATE_FILE" ]] && post_exists="yes" || post_exists="no"
