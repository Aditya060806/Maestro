...
#
# See also:
#   - test-pulse-wrapper-headless-export.sh  (MAESTRO_HEADLESS scoping)
...
# defaults file to be present at $HOME/.maestro/agents/configs/. The
...
DEFAULTS_SOURCE="${SCRIPT_DIR}/../../configs/maestro.defaults.jsonc"
...
# t3016: Seed a sandbox HOME with the canonical maestro defaults config so
...
seed_sandbox_config() {
	...
	mkdir -p "${sandbox_home}/.maestro/agents/configs"
	cp "$DEFAULTS_SOURCE" \
		"${sandbox_home}/.maestro/agents/configs/maestro.defaults.jsonc"
	return 0
}
...
#
# Sandbox rationale: acquire_instance_lock writes to $HOME/.maestro/run/.
...
test_pr_cache_ttls_are_per_cycle() {
	local missing=""
	if ! grep -q 'MAESTRO_GH_PR_VIEW_CACHE_TTL=.*MAESTRO_PULSE_PR_VIEW_CACHE_TTL:-3600' "$WRAPPER_SCRIPT"; then
		missing="${missing} view-ttl"
	fi
	if grep -Eq "MAESTRO_GH_PR_VIEW_CACHE_DIR=.*\\$\\$|MAESTRO_GH_PR_VIEW_CACHE_DIR=.*\\$\\{\\$\\}" "$WRAPPER_SCRIPT"; then
		missing="${missing} pid-scoped-view-cache-dir"
	fi
	if ! grep -q 'MAESTRO_GH_PR_VIEW_CACHE_DIR=.*\.maestro/cache/pulse-pr-view-cache' "$WRAPPER_SCRIPT"; then
		missing="${missing} stable-view-cache-dir"
	fi
	if ! grep -q 'MAESTRO_GH_PR_LIST_CACHE_TTL=.*MAESTRO_PULSE_PR_LIST_CACHE_TTL:-3600' "$WRAPPER_SCRIPT"; then
		missing="${missing} list-ttl"
	fi
	if ! grep -q 'MAESTRO_GH_PR_LIST_CACHE_DIR=.*maestro-pulse-pr-list-cache' "$WRAPPER_SCRIPT"; then
