...
# _MAESTRO_CLAIM_TEST_IS_TTY and _MAESTRO_CLAIM_TEST_ANSWER override the
...
_TEST_SLUG="Aditya060806/Maestro"
...
test_case_a_no_hits() {
	...
	# Clear test hooks
	unset _MAESTRO_CLAIM_TEST_IS_TTY _MAESTRO_CLAIM_TEST_ANSWER 2>/dev/null || true
...
test_case_b_interactive_decline() {
	...
	export _MAESTRO_CLAIM_TEST_IS_TTY=1
	export _MAESTRO_CLAIM_TEST_ANSWER="n"

	local rc=0
	_pre_claim_discovery_pass "$_TEST_TITLE" "$_TEST_SLUG" 2>/dev/null || rc=$?

	unset _MAESTRO_CLAIM_TEST_IS_TTY _MAESTRO_CLAIM_TEST_ANSWER
	PATH="$saved_path"
...
test_case_c_interactive_confirm() {
	...
	export _MAESTRO_CLAIM_TEST_IS_TTY=1
	export _MAESTRO_CLAIM_TEST_ANSWER="y"
...
test_case_d_noninteractive_warns() {
	...
	unset _MAESTRO_CLAIM_TEST_IS_TTY _MAESTRO_CLAIM_TEST_ANSWER 2>/dev/null || true
	export _MAESTRO_CLAIM_TEST_IS_TTY=0

	local stderr_output rc=0
	stderr_output=$(_pre_claim_discovery_pass "$_TEST_TITLE" "$_TEST_SLUG" 2>&1) \
		|| rc=$?

	unset _MAESTRO_CLAIM_TEST_IS_TTY
	PATH="$saved_path"
