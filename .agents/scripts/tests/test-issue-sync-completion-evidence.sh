...
gh() {
	if [[ "$*" == *"issue view 123"* ]]; then
		printf '%s\n' '{"state":"CLOSED","stateReason":"COMPLETED","closedAt":"2026-06-30T01:53:39Z","body":"Completed interactively.\n<!-- maestro:sig -->","comments":[]}'
		return 0
	fi
	if [[ "$*" == *"issue view 124"* ]]; then
		printf '%s\n' '{"state":"CLOSED","stateReason":"COMPLETED","closedAt":"2026-06-30T01:53:39Z","body":"Closed without maestro evidence.","comments":[]}'
		return 0
	fi
	return 1
}
...
if completed_date=$(_closed_issue_maestro_complete_date "owner/repo" "123"); then
	if [[ "$completed_date" == "2026-06-30" ]]; then
		pass "closed maestro-signed issue is completion evidence for TODO refresh"
	else
		fail "closed maestro-signed issue returned wrong completion date"
	fi
else
	fail "closed maestro-signed issue should be completion evidence"
fi

if _closed_issue_maestro_complete_date "owner/repo" "124" >/dev/null; then
	fail "closed issue without maestro evidence is not completion evidence"
else
	pass "closed issue without maestro evidence is not completion evidence"
fi
