...
#     17:38:36Z  Aditya060806: labeled origin:worker       (backfilled by t2687)
#
...
#      MAESTRO_SESSION_ORIGIN=worker before calling gh_create_issue
...
#      MAESTRO_SESSION_ORIGIN=worker override)
#   2. _create_health_issue restores MAESTRO_SESSION_ORIGIN after the call
#   3. _create_health_issue restores unset MAESTRO_SESSION_ORIGIN correctly
#   4. When MAESTRO_HEADLESS is set, origin:worker is also auto-applied
#   5. When neither MAESTRO_HEADLESS nor MAESTRO_SESSION_ORIGIN is set,
#      the explicit MAESTRO_SESSION_ORIGIN=worker guard still applies
...
unset MAESTRO_SESSION_ORIGIN 2>/dev/null || true
unset MAESTRO_HEADLESS 2>/dev/null || true
...
# The function uses "${HOME}/.maestro/agents/scripts/gh-signature-helper.sh"
# We create a stub at the expected path override.
export HOME="${TMP}/fakehome"
mkdir -p "${HOME}/.maestro/agents/scripts"
cat >"${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" <<'SIGSTUB'
#!/usr/bin/env bash
# Stub gh-signature-helper.sh for test isolation
printf ''
SIGSTUB
chmod +x "${HOME}/.maestro/agents/scripts/gh-signature-helper.sh"
...
# =============================================================================
# Test 1: _create_health_issue passes origin:worker via MAESTRO_SESSION_ORIGIN
#         override even when neither MAESTRO_HEADLESS nor SESSION_ORIGIN is set.
...
if grep -qE '\-\-label origin:worker' "$GH_CALLS" 2>/dev/null; then
	pass "_create_health_issue passes origin:worker even without MAESTRO_HEADLESS"
else
	fail "_create_health_issue passes origin:worker even without MAESTRO_HEADLESS" \
...
# =============================================================================
# Test 2: _create_health_issue restores MAESTRO_SESSION_ORIGIN after the call
...
if [[ -z "${MAESTRO_SESSION_ORIGIN:-}" ]]; then
	pass "_create_health_issue restores unset MAESTRO_SESSION_ORIGIN after call"
else
	fail "_create_health_issue restores unset MAESTRO_SESSION_ORIGIN after call" \
		"MAESTRO_SESSION_ORIGIN is now '${MAESTRO_SESSION_ORIGIN:-}' (should be unset)"
fi

# =============================================================================
# Test 3: _create_health_issue restores a pre-existing MAESTRO_SESSION_ORIGIN
# =============================================================================
...
if [[ "${MAESTRO_SESSION_ORIGIN:-}" == "interactive" ]]; then
	pass "_create_health_issue restores pre-existing MAESTRO_SESSION_ORIGIN=interactive"
else
	fail "_create_health_issue restores pre-existing MAESTRO_SESSION_ORIGIN=interactive" \
		"MAESTRO_SESSION_ORIGIN is now '${MAESTRO_SESSION_ORIGIN:-}' (expected 'interactive')"
fi
unset MAESTRO_SESSION_ORIGIN

# =============================================================================
# Test 4: origin:worker is still passed when MAESTRO_HEADLESS=true
...
if grep -qE '\-\-label origin:worker' "$GH_CALLS" 2>/dev/null; then
	pass "_create_health_issue passes origin:worker with MAESTRO_HEADLESS=true"
else
	fail "_create_health_issue passes origin:worker with MAESTRO_HEADLESS=true" \
