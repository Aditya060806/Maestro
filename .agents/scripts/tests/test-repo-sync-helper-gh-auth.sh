...
run_case() {
	...
	tmp_dir=$(mktemp -d)
	TEMP_DIRS+=("$tmp_dir")
	mkdir -p "${tmp_dir}/home/.config/maestro" "${tmp_dir}/parent/repo/.git"
	make_fake_tools "$tmp_dir"
	printf '{"git_parent_dirs":["%s"]}\n' "${tmp_dir}/parent" >"${tmp_dir}/home/.config/maestro/repos.json"
...
assert_file_contains "${LAST_CASE_DIR}/home/.maestro/logs/repo-sync.log" "retrying git fetch with gh credential helper" "GitHub auth fallback is logged without credentials"
assert_file_not_contains "${LAST_CASE_DIR}/home/.maestro/logs/repo-sync.log" "SECRET_TOKEN_github_fallback" "GitHub auth fallback log does not contain token"
...
assert_file_contains "${LAST_CASE_DIR}/home/.maestro/logs/repo-sync.log" "git pull --ff-only failed (diverged?)" "diverged pull keeps existing failure message"
...
assert_file_not_contains "${LAST_CASE_DIR}/home/.maestro/logs/repo-sync.log" "SECRET_TOKEN_github_fallback_failure_redacts" "failed GitHub fallback log redacts token"
assert_file_contains "${LAST_CASE_DIR}/home/.maestro/logs/repo-sync.log" "[redacted-credential]" "failed GitHub fallback log includes redacted credential marker"
