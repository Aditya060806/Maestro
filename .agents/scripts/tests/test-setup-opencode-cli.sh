...
#   3. Verify setup_opencode_cli writes ~/.maestro/.opencode-bin-resolved
...
(
	source_lib
	export MAESTRO_OPENCODE_VERSION_TIMEOUT=1
	SECONDS=0
	rc=0
...
(
	...
	echo "rc=$rc"
	cat "$HOME/.maestro/.opencode-bin-resolved" 2>/dev/null || echo "MISSING"
) >"$SANDBOX/out5" 2>&1
...
rm -f "$HOME/.maestro/.opencode-bin-resolved" "$HOME/.local/bin/opencode"
mkdir -p "$HOME/.maestro/.agent-workspace/headless-runtime"
printf '0\n' >"$HOME/.maestro/.agent-workspace/headless-runtime/canary-last-fail"
printf 'config_error\n' >"$HOME/.maestro/.agent-workspace/headless-runtime/canary-last-fail.reason"
(
	source_lib
	export OPENCODE_BIN="$SANDBOX/bin/opencode-real"
	rc=0
...
if [[ ! -f "$HOME/.maestro/.agent-workspace/headless-runtime/canary-last-fail" ]] && \
	[[ ! -f "$HOME/.maestro/.agent-workspace/headless-runtime/canary-last-fail.reason" ]]; then
...
rm -f "$HOME/.maestro/.opencode-bin-resolved"
...
rm -f "$SANDBOX/bin/bun" "$HOME/.maestro/.opencode-bin-resolved"
(
	source_lib
	...
	export OPENCODE_BIN=""
	export MAESTRO_OPENCODE_VERSION_TIMEOUT=1
	export MAESTRO_OPENCODE_INSTALL_TIMEOUT=1
	SECONDS=0
	rc=0
	setup_opencode_cli || rc=$?
	printf 'rc=%s elapsed=%s\n' "$rc" "$SECONDS"
