...
# 2. The contract is NOT injected when MAESTRO_HEADLESS_APPEND_CONTRACT=0.
...
PY
	...
	MAESTRO_HEADLESS_APPEND_CONTRACT="$append_enabled" \
...
test_contract_not_injected_when_disabled() {
	...
	if [[ "$result" == *"HEADLESS_CONTINUATION_CONTRACT_V"* ]]; then
		print_result "contract not injected when MAESTRO_HEADLESS_APPEND_CONTRACT=0" 1 \
			"Expected no contract injection when disabled"
		return 0
	fi

	print_result "contract not injected when MAESTRO_HEADLESS_APPEND_CONTRACT=0" 0
	return 0
}
