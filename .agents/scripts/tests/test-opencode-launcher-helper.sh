...
printf 'MAESTRO_OPENCODE_ISOLATED_DB=%s\n' "${MAESTRO_OPENCODE_ISOLATED_DB:-}"
printf 'PWD=%s\n' "$PWD"
printf 'ARGS=%s\n' "$*"
SH
...
output=$(PATH="${fake_bin}:$PATH" HOME="${home_dir}" MAESTRO_WORK_DIR="${work_dir}" FAKE_OPENCODE_LOG="${fake_log}" \
...
if [[ "${output}" == *"MAESTRO_OPENCODE_ISOLATED_DB=1"* ]] \
...
output=$(PATH="${fake_bin}:$PATH" HOME="${home_dir}" MAESTRO_WORK_DIR="${work_dir}" XDG_DATA_HOME="${work_dir}/opencode-interactive/test-session" \
...
output=$(PATH="${fake_bin}:$PATH" HOME="${home_dir}" MAESTRO_WORK_DIR="${work_dir}" \
    "${HELPER}" --shared-db --dir "${launch_dir}" -- --version 2>&1)
if [[ "${output}" == *"MAESTRO_OPENCODE_ISOLATED_DB="* ]] \
...
desktop_app="${desktop_app_dir}/OpenCode Maestro.app"
desktop_wrapper="${desktop_app}/Contents/MacOS/opencode-maestro"
desktop_plist="${desktop_app}/Contents/Info.plist"
if [[ -x "${desktop_wrapper}" ]] \
    && [[ -f "${desktop_plist}" ]] \
    && grep -q "sh.maestro.opencode.desktop" "${desktop_plist}" \
    && grep -q "desktop launch --from-app" "${desktop_wrapper}" \
    && [[ "${output}" == *"Installed OpenCode Maestro.app"* ]]; then
...
if [[ "${output}" == *"XDG_DATA_HOME=${work_dir}/opencode-interactive/test-session"* ]] \
    && [[ "${output}" == *"MAESTRO_OPENCODE_ISOLATED_DB=1"* ]]; then
...
if [[ "${output}" == *"XDG_DATA_HOME=${work_dir}/opencode-desktop/desktop-project-repo-"* ]] \
    && [[ "${output}" == *"MAESTRO_OPENCODE_ISOLATED_DB=1"* ]] \
