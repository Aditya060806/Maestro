...
#
# Tests for the MAESTRO_HEADLESS=true export at the top of stats-wrapper.sh
# main() (GH#19913 / t2390).
#
...
# scheduler (15-min maestro-stats-wrapper.timer / launchd plist). PR
...
test_export_line_present_at_top_of_main() {
	...
	print_result "export MAESTRO_HEADLESS=true present at top of main()" 1 \
		"Expected 'export MAESTRO_HEADLESS=true' line between 'main() {' and the '--self-check' dispatch. Got snippet:${snippet}"
	return 0
}
...
# testing must not have MAESTRO_HEADLESS set on their behalf.
