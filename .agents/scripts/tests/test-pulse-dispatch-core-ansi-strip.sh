...
test_fixed_extractor_returns_clean_path() {
	local wt_out
	wt_out=$(build_simulated_wt_output "feature/auto-test" "/tmp/maestro-test-wt/Git/maestro-feature-auto-test")
	local extracted
	extracted=$(extract_path_fixed "$wt_out")
	if [[ "$extracted" == "/tmp/maestro-test-wt/Git/maestro-feature-auto-test" ]]; then
...
test_fixed_extractor_passes_dir_check_with_tmpdir() {
	...
	tmp_path="${tmp_base}/Git/maestro-feature-auto-realcheck"
	mkdir -p "$tmp_path"
...
test_extractor_handles_path_with_multiple_ansi_runs() {
	...
	local path='/tmp/Git/maestro-multi-ansi'
	local wt_out
