...
#   MAESTRO_VM_SKIP_BUMP_VERIFY — tag bypass skips bump-commit verify (version-manager-release.sh)
#   MAESTRO_HEADLESS_SANDBOX_DISABLED — sandbox bypass falls back to bare exec (headless-runtime-helper.sh)
...
#   MAESTRO_SKIP_TIER_VALIDATOR — test-tier-simple-body-shape.sh
#   MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY — test-pre-dispatch-eligibility.sh
#   MAESTRO_SKIP_PREDISPATCH_VALIDATOR   — test-pre-dispatch-validator.sh
#   MAESTRO_SKIP_AUTO_CLAIM    — test-worktree-auto-claim.sh (Test 7)
#   MAESTRO_BASH_REEXECED      — test-bash-reexec-guard.sh
#   MAESTRO_SCAN_STALE_AUTO_RELEASE — test-scan-stale-auto-release.sh
#
# Strategy:
...
# ============================================================
# Section 4: MAESTRO_VM_SKIP_BUMP_VERIFY bypass log (version-manager-release.sh)
# ============================================================
section "4. MAESTRO_VM_SKIP_BUMP_VERIFY bypass log (version-manager-release.sh)"
...
if [[ ! -f "$VERSION_MGMT_RELEASE" ]]; then
	...
	if grep -qF "MAESTRO_VM_SKIP_BUMP_VERIFY=1 — bypassing bump-commit verification" "$VERSION_MGMT_RELEASE"; then
		pass "MAESTRO_VM_SKIP_BUMP_VERIFY bypass site has info_log (structural assertion)"
	else
		fail "MAESTRO_VM_SKIP_BUMP_VERIFY bypass site should have info_log" \
	...
	if grep -qF "git tag -a" "$VERSION_MGMT_RELEASE"; then
		pass "MAESTRO_VM_SKIP_BUMP_VERIFY floor: git tag command still present after bypass site"
	else
		fail "MAESTRO_VM_SKIP_BUMP_VERIFY floor: git tag command missing from version-manager-release.sh"
	fi
fi

# ============================================================
# Section 5: MAESTRO_HEADLESS_SANDBOX_DISABLED bypass log (headless-runtime-helper.sh)
# ============================================================
section "5. MAESTRO_HEADLESS_SANDBOX_DISABLED bypass log (headless-runtime-helper.sh)"
...
if [[ ! -f "$HEADLESS_HELPER" ]]; then
	...
	count=$(grep -c "MAESTRO_HEADLESS_SANDBOX_DISABLED=1 —" "$HEADLESS_HELPER" 2>/dev/null || true)
	if [[ "$count" -ge 2 ]]; then
		pass "MAESTRO_HEADLESS_SANDBOX_DISABLED has info_log at both bypass sites (count=$count)"
	elif [[ "$count" -eq 1 ]]; then
		fail "MAESTRO_HEADLESS_SANDBOX_DISABLED: only 1 of 2 bypass sites has info_log (expected 2)"
	else
		fail "MAESTRO_HEADLESS_SANDBOX_DISABLED: no bypass sites have info_log" \
			"grep for 'MAESTRO_HEADLESS_SANDBOX_DISABLED=1 —' in $HEADLESS_HELPER returned 0"
	fi
	...
	if grep -q "timeout.*HEADLESS_SANDBOX_TIMEOUT" "$HEADLESS_HELPER"; then
		pass "MAESTRO_HEADLESS_SANDBOX_DISABLED floor: timeout limit still enforced after bypass"
	else
		fail "MAESTRO_HEADLESS_SANDBOX_DISABLED floor: timeout enforcement missing from bypass path"
	fi
fi
...
_check_audit_log_present "MAESTRO_SKIP_AUTO_CLAIM" "MAESTRO_SKIP_AUTO_CLAIM set" "${SCRIPTS_DIR}/worktree-helper.sh"
