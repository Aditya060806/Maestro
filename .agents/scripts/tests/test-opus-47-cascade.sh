...
#      or provider-disallowed by MAESTRO_HEADLESS_PROVIDER_ALLOWLIST.
...
if [[ "${1:-}" == "check" ]]; then
 if [[ "${2:-}" == "${MAESTRO_OPUS_ESCALATION_MODEL:-openai/gpt-5.5}" && "${OPUS_ESCALATION_AVAILABLE:-true}" != "false" ]]; then
  exit 0
 fi
 exit 1
fi
...
unset MAESTRO_HEADLESS_PROVIDER_ALLOWLIST MODEL_RESOLVE_OPUS
