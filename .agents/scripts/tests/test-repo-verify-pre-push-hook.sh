...
#
# Verifies:
#   1. MAESTRO_PREPUSH_REPO_VERIFY=0 short-circuits to exit 0
...
#   5. .maestro.json `.verify.enabled=false` opts out (exit 0)
#   6. .maestro.json with a passing format command: exit 0
#   7. .maestro.json with a failing format command, autofix off, mentor exit 1
#   8. .maestro.json with failing format + AUTOFIX=1 + working fix command: exit 0
...
# the hook as a subprocess. No network, no live GitHub API, no maestro state.
...
_mk_repo() {
	local _dir
	_dir=$(mktemp -d -t maestro-prepush-test.XXXXXX)
	(
		cd "$_dir" || exit 1
		git init -q
...
_run_hook() {
	...
	shift
	local _err
	_err=$(mktemp -t maestro-prepush-test-stderr.XXXXXX)
	local _ec=0
	(
		cd "$_repo" || exit 1
		# shellcheck disable=SC2068
		# Intentional unquoted expansion: each arg is a 'KEY=value' env assignment
		env -u FULL_LOOP_HEADLESS -u MAESTRO_HEADLESS -u OPENCODE_HEADLESS \
			-u MAESTRO_PREPUSH_AUTOFIX -u MAESTRO_PREPUSH_REPO_VERIFY \
			-u MAESTRO_PREPUSH_REPO_VERIFY_DEBUG -u GITHUB_ACTIONS \
			$@ bash "$HOOK" </dev/null
	) 2>"$_err"
	_ec=$?
	printf '%s\n' "$_ec"
...
# Test 1: MAESTRO_PREPUSH_REPO_VERIFY=0 — fast bypass
{
	repo=$(_mk_repo)
	out=$(_run_hook "$repo" MAESTRO_PREPUSH_REPO_VERIFY=0)
	ec=$(printf '%s' "$out" | head -n 1)
	assert_eq '1. MAESTRO_PREPUSH_REPO_VERIFY=0 → exit 0' '0' "$ec"
	rm -rf "$repo"
}
...
{
	non_repo=$(mktemp -d -t maestro-nonrepo.XXXXXX)
...
{
	repo=$(_mk_repo)
	out=$(_run_hook "$repo" MAESTRO_PREPUSH_REPO_VERIFY_DEBUG=1)
	ec=$(printf '%s' "$out" | head -n 1)
...
# Test 5: .maestro.json explicit opt-out
{
	repo=$(_mk_repo)
	cat >"$repo/.maestro.json" <<'JSON'
{ "verify": { "enabled": false, "format": "false" } }
JSON
	(cd "$repo" && git add .maestro.json && git commit -q -m 'add config')
	out=$(_run_hook "$repo")
	ec=$(printf '%s' "$out" | head -n 1)
	stderr=$(printf '%s' "$out" | tail -n +2)
...
# Test 6: .maestro.json with a trivially passing format command
{
	repo=$(_mk_repo)
	cat >"$repo/.maestro.json" <<'JSON'
{ "verify": { "format": "true" } }
...
# Test 7: .maestro.json failing format, no autofix command, AUTOFIX=0 → mentor + block
{
	repo=$(_mk_repo)
	cat >"$repo/.maestro.json" <<'JSON'
{ "verify": { "format": "false" } }
JSON
	(cd "$repo" && git add .maestro.json && git commit -q -m 'add config')
	out=$(_run_hook "$repo" MAESTRO_PREPUSH_AUTOFIX=0)
	ec=$(printf '%s' "$out" | head -n 1)
	stderr=$(printf '%s' "$out" | tail -n +2)
	assert_eq '7. failing format + autofix off → exit 1' '1' "$ec"
	assert_contains '7b. failure mentor: resolution section' 'Resolution:' "$stderr"
	assert_contains '7c. failure mentor: bypass hint' 'MAESTRO_PREPUSH_REPO_VERIFY=0' "$stderr"
	rm -rf "$repo"
}
...
JSON
	...
	(cd "$repo" && git add .maestro.json .needs-fmt && git commit -q -m 'add config + sentinel')
	out=$(_run_hook "$repo" MAESTRO_PREPUSH_AUTOFIX=1)
	ec=$(printf '%s' "$out" | head -n 1)
	stderr=$(printf '%s' "$out" | tail -n +2)
...
JSON
	...
	out=$(_run_hook "$repo" MAESTRO_PREPUSH_REPO_VERIFY_DEBUG=1 MAESTRO_PREPUSH_AUTOFIX=0)
	stderr=$(printf '%s' "$out" | tail -n +2)
...
JSON
	...
	out=$(_run_hook "$repo" MAESTRO_PREPUSH_AUTOFIX=1 MAESTRO_PREPUSH_REPO_VERIFY_DEBUG=1)
	stderr=$(printf '%s' "$out" | tail -n +2)
