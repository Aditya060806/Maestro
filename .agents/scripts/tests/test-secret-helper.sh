...
case "$cmd" in
	...
	insert)
		...
		path="${1:-}"
		printf '%s' "$path" >"${MAESTRO_TEST_DIR}/stored_path"
		cat >"${MAESTRO_TEST_DIR}/stored_value"
		exit 0
		;;
	*)
		exit 1
...
EOF
	...
	export MAESTRO_TEST_DIR="$TEST_DIR"
...
test_set_uses_provided_stdin_value() {
	...
	if [[ "$stored_value" == "actual-secret-value" && "$stored_path" == "maestro/TEST_KEY" ]]; then
...
test_set_rejects_command_literal_input() {
	...
	local output_file="$TEST_DIR/output.log"
	local exit_code=0
	printf '%s\n' 'maestro secret set TEST_KEY' | bash "$HELPER" set TEST_KEY >"$output_file" 2>&1 || exit_code=$?
