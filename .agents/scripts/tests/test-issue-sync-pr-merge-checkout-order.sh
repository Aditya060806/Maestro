...
# job uses two checkouts — an early framework checkout into `__maestro/`
...
# default-deletes the workspace contents — including the `__maestro/`
...
# fails with `bash: __maestro/.agents/scripts/issue-sync-git-push-helper.sh:
# No such file or directory` on every merged PR.
#
# This test asserts that the `sync-on-pr-merge` job has either:
#   (a) a `Re-checkout maestro framework scripts` step AFTER the
...
# ============================================================
# Test 3: job preserves __maestro/ across the second checkout.
...
if [[ -z "${CALLER_CHECKOUT_REL_LINE}" ]]; then
	...
	# Option B: a 'Re-checkout maestro framework scripts' step AFTER the
	...
	if printf '%s\n' "${STEPS_AFTER}" | grep -qE 'name:[[:space:]]+(Re-checkout|Restore|Re-pull) (maestro )?framework scripts'; then
		OPTION_B_OK=1
	fi

	if [[ "${OPTION_A_OK}" == "1" || "${OPTION_B_OK}" == "1" ]]; then
		check 1 "__maestro/ preserved after caller checkout (Option A clean:false OR Option B re-checkout)" ""
	else
		check 0 "__maestro/ preserved after caller checkout (Option A clean:false OR Option B re-checkout)" \
			"neither 'clean: false' nor a 'Re-checkout maestro framework scripts' step found after 'Checkout repo for TODO.md update' — workspace wipe will delete __maestro/ and the push-todo step will fail"
	fi
fi

# ============================================================
# Test 4: Update TODO.md proof-log step still references __maestro/
#         (sanity check — if this changes, the test premise needs updating)
# ============================================================
if printf '%s\n' "${JOB_BODY}" | grep -qE 'bash[[:space:]]+__maestro/\.agents/scripts/issue-sync-git-push-helper\.sh'; then
	check 1 "Update TODO.md proof-log invokes __maestro/.agents/scripts/issue-sync-git-push-helper.sh" ""
else
	check 0 "Update TODO.md proof-log invokes __maestro/.agents/scripts/issue-sync-git-push-helper.sh" \
