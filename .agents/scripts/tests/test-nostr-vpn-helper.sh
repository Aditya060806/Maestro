...
test_secret_guidance_mentions_maestro_secret() {
	local output=""
	output="$(bash "$HELPER_SCRIPT" secrets-help 2>&1)"

	if [[ "$output" == *"maestro secret set FIPS_NSEC"* && "$output" == *"maestro secret set OPENCODE_SERVER_TOKEN"* ]]; then
		print_result "secret guidance uses maestro secret" 0
		return 0
	fi

	print_result "secret guidance uses maestro secret" 1 "$output"
	return 0
}
...
test_opencode_guide_mentions_maestro_services() {
	local output=""
	output="$(bash "$HELPER_SCRIPT" opencode-guide 2>&1)"

	if [[ "$output" == *"MCP services"* && "$output" == *"Git operations"* && "$output" == *"loopback/FIPS"* ]]; then
		print_result "opencode guide mentions maestro services" 0
		return 0
	fi

	print_result "opencode guide mentions maestro services" 1 "$output"
	return 0
}
