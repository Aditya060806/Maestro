...
run_claude_registration_with_stub() {
	...
	PATH="${temp_dir}:$PATH" MAESTRO_MCP_CLAUDE_TIMEOUT_SECONDS=1 bash -c '
		set -euo pipefail
		source "$1"
...
run_two_claude_registrations_with_stub() {
	...
	PATH="${temp_dir}:$PATH" MAESTRO_MCP_CLAUDE_TIMEOUT_SECONDS=1 CLAUDE_STUB_LIST_COUNT_FILE="$list_count_file" bash -c '
		set -euo pipefail
		source "$1"
