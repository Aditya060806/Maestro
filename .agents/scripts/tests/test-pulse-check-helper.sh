...
APPLY_OUT=$(env "${COMMON_ENV[@]}" "$HELPER" apply --repo owner/maestro 2>&1)
assert_contains "apply reports issue filing" "pulse-check: filed" "$APPLY_OUT"
BODY=$(cat "${TEST_ROOT}/capture.txt.body")
assert_contains "apply body carries marker" "maestro:generator=pulse-check finding=" "$BODY"
