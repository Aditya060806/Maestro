...
# Clean up maestro runtime files and execute the git worktree remove.
...
_remove_cleanup_and_execute() {
	...
	# Clean up maestro runtime files before removal (prevents "contains untracked files" error)
...
OWNERSHIP SAFETY (t189)
  ...
  Registry: ~/.maestro/.agent-workspace/worktree-registry.db
