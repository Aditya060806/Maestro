...
_update_homebrew_formula() {
	...
	if [[ -n "$new_version" ]]; then
		print_info "Leaving homebrew/maestro.rb unchanged until release tarball exists" >&2
	fi
	unset new_version
	return 0
}
...
update_version_in_files() {
	...
	update_script_version_reference "$REPO_ROOT/maestro.sh" "$new_version" "maestro.sh" || errors=$((errors + 1))
