...
# Constants
readonly THUMBNAIL_CACHE_DIR="$HOME/.cache/maestro/thumbnails"
...
load_youtube_credentials() {
	...
	if command -v gopass >/dev/null 2>&1 && gopass show "maestro/youtube-api-key" >/dev/null 2>&1; then
		YOUTUBE_API_KEY="$(gopass show -o "maestro/youtube-api-key" 2>/dev/null)" || true
	fi
	if [[ -z "${YOUTUBE_API_KEY:-}" ]] && [[ -f "${HOME}/.config/maestro/credentials.sh" ]]; then
...
cmd_generate() {
	...
	if [[ -z "${HIGGSFIELD_API_KEY:-}" ]] || [[ -z "${HIGGSFIELD_SECRET:-}" ]]; then
		print_error "Higgsfield credentials not configured. Run: maestro secret set higgsfield-api-key"
		return 1
	fi
