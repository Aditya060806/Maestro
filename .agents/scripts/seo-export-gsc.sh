...
get_access_token() {
    ...
    print_error "Set GOOGLE_APPLICATION_CREDENTIALS or GSC_ACCESS_TOKEN in ~/.config/maestro/credentials.sh"
    return 1
}
...
Output:
    ~/.maestro/.agent-workspace/work/seo-data/{domain}/gsc-{start}-{end}.toon

Requirements:
    - GOOGLE_APPLICATION_CREDENTIALS pointing to service account JSON
    - Or GSC_ACCESS_TOKEN set in ~/.config/maestro/credentials.sh
