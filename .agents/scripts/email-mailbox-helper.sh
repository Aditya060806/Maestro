...
#
# Part of maestro email system (t1493, t1525)
...
readonly MAILBOX_WORKSPACE="${HOME}/.maestro/.agent-workspace/email-mailbox"
...
get_imap_password() {
	...
	if command -v gopass &>/dev/null; then
		local password
		password=$(gopass show -o "maestro/${secret_name}" 2>/dev/null) || password=""
		if [[ -n "$password" ]]; then
			echo "$password"
			return 0
		fi
	...
	print_info "Store via: maestro secret set ${secret_name}"
...
get_imap_user() {
	...
	if command -v gopass &>/dev/null; then
		local user
		user=$(gopass show "maestro/email-imap-${account}" user 2>/dev/null) || user=""
		if [[ -n "$user" ]]; then
			echo "$user"
			return 0
		fi
	...
	print_info "Store via: gopass edit maestro/email-imap-${account} (add 'user: you@example.com')"
	return 1
}
...
get_jmap_token() {
	...
	if command -v gopass &>/dev/null; then
		local token
		token=$(gopass show -o "maestro/${secret_name}" 2>/dev/null) || token=""
		if [[ -n "$token" ]]; then
			echo "$token"
			return 0
		fi
...
Credentials:
  IMAP: maestro secret set email-imap-<account>
  JMAP: maestro secret set email-jmap-<account> (or falls back to IMAP password)
  Username: gopass edit maestro/email-imap-<account> (add 'user: you@example.com')
