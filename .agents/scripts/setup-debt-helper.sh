...
#
# DESCRIPTION:
#   Reads ~/.maestro/advisories/sync-pat-*.advisory and
#   ~/.maestro/advisories/cross-account-inherit-*.advisory files (written by
#   security-posture-helper.sh during `maestro security check`, t2374/t2880)
#   and exposes the aggregated count + slug list to two consumers:
#
#     1. maestro-update-check.sh — emits a single [WARN] toast line so the
...
See also:
  ...
  maestro security check          Generates the SYNC_PAT + cross-account advisories
