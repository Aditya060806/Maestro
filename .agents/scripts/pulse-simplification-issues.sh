...
_complexity_scan_collect_md_violations() {
	...
	local md_files
	md_files=$(git -C "$maestro_path" ls-files '*.md' | grep -E '^\.agents/' | grep -Ev "$excluded_dirs" | grep -Ev "$excluded_files" | grep -Ev "$protected_pattern" || true)
	if [[ -z "$md_files" ]]; then
...
# Arguments: $1 - maestro_path, $2 - file_path (repo-relative)
# Outputs: topic label via stdout
_complexity_scan_extract_md_topic_label() {
	local maestro_path="$1"
	local file_path="$2"
	local full_path="${maestro_path}/${file_path}"
...
_complexity_scan_build_md_issue_body() {
	...
	cat <<ISSUE_BODY_EOF
<!-- maestro:generator=agent-doc-simplification-gate cited_file=${file_path} threshold=${COMPLEXITY_MD_MIN_LINES:-500} -->
...
# Arguments: $1 - maestro_slug, $2 - file_path, $3 - state_file, $4 - maestro_path, $5 - line_count
...
_complexity_scan_md_file_status() {
	...
	local file_path="$2"
	local state_file="$3"
	local maestro_path="$4"
	local line_count="${5:-0}"

	local file_status="new"
	if [[ -n "$state_file" && -n "$maestro_path" ]]; then
		file_status=$(_simplification_state_check "$maestro_path" "$file_path" "$state_file")
	...
	if _complexity_scan_has_existing_issue "$maestro_slug" "$file_path"; then
		printf '%s' "existing"
		return 0
	fi
...
_complexity_scan_process_single_md_file() {
	...
	local file_status
	file_status=$(_complexity_scan_md_file_status "$maestro_slug" "$file_path" "$state_file" "$maestro_path" "$line_count")
	case "$file_status" in
	unchanged)
	...
	local topic_label=""
	if [[ -n "$maestro_path" ]]; then
		topic_label=$(_complexity_scan_extract_md_topic_label "$maestro_path" "$file_path" 2>/dev/null || true)
	fi
	...
	if [[ "$create_ok" == true ]]; then
		_complexity_scan_close_duplicate_issues_by_title "$maestro_slug" "$issue_title"
		local log_suffix=""
...
# Arguments: $1 - scan_results (pipe-delimited: file_path|line_count), $2 - repos_json, $3 - maestro_slug
_complexity_scan_create_md_issues() {
	...
	_complexity_scan_check_open_cap "$maestro_slug" 500 "Complexity scan (.md)" || return 0
	...
	if [[ -z "$maintainer" ]]; then
		maintainer=$(echo "$maestro_slug" | cut -d/ -f1)
	fi
	...
	local state_file=""
	if [[ -n "$maestro_path" ]]; then
		state_file="${maestro_path}/.agents/configs/simplification-state.json"
	fi
	...
	while IFS='|' read -r file_path line_count; do
		...
		result=$(_complexity_scan_process_single_md_file "$file_path" "$line_count" \
			"$maestro_slug" "$maestro_path" "$state_file" "$maintainer")
...
_complexity_scan_sh_build_issue_body_with_sig() {
	...
	local issue_body
	issue_body="<!-- maestro:generator=function-complexity-gate cited_file=${file_path} threshold=${COMPLEXITY_FUNC_LINE_THRESHOLD} -->
...
# Arguments: $1 - file_path, $2 - violation_count, $3 - repos_json, $4 - maestro_slug
# Returns: 0 if issue created, 1 if failed
_complexity_scan_sh_create_issue() {
	local file_path="$1"
	local violation_count="$2"
	local repos_json="$3"
	local maestro_slug="$4"
	...
	maestro_path=$(jq -r --arg slug "$maestro_slug" \
		...
		"$repos_json" 2>/dev/null | head -n 1)
	local details=""
	if [[ -n "$maestro_path" && -f "${maestro_path}/${file_path}" ]]; then
		...
		' "${maestro_path}/${file_path}" | head -10)
	fi
...
# Arguments: $1 - scan_results (pipe-delimited: file_path|count), $2 - repos_json, $3 - maestro_slug
# Returns: 0 always
_complexity_scan_create_issues() {
	...
	# Total-open cap: stop creating when backlog is already large
	_complexity_scan_check_open_cap "$maestro_slug" 500 "Complexity scan" || return 0
	...
	while IFS='|' read -r file_path violation_count; do
		...
		if _complexity_scan_sh_create_issue "$file_path" "$violation_count" "$repos_json" "$maestro_slug"; then
