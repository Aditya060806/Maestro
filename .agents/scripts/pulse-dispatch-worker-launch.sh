...
: "${MAESTRO_UNKNOWN_VERSION:=unknown}"
...
_dlw_display_version_or_unknown() {
	local raw_version="$1"
	if declare -F maestro_display_version >/dev/null 2>&1; then
		maestro_display_version "$raw_version"
	else
		if [[ -n "$raw_version" && "$raw_version" != "$MAESTRO_UNKNOWN_VERSION" ]]; then
...
_dlw_setup_worker_log() {
	...
	maestro_pulse_tmp_cleanup "${MAESTRO_PULSE_TMP_MAX_AGE_MINUTES:-2880}" || true
	worker_log=$(maestro_pulse_worker_log_path "$repo_slug" "$issue_number") || return 1
	worker_log_fallback=$(maestro_pulse_worker_log_fallback_path "$issue_number") || return 1
...
_dlw_node_modules_restore_lock_dir() {
	local workspace_dir="${MAESTRO_WORKSPACE_DIR:-${HOME}/.maestro/.agent-workspace}"
...
# via MAESTRO_WORKER_PREWARM_DIR so it is reused instead of a fresh mktemp.
...
_dlw_prewarm_opencode_db() {
	...
	_DLW_PREWARM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/maestro-worker-auth.XXXXXX") || { _DLW_PREWARM_DIR=""; return 0; }
...
# intentional lifecycle owner outside maestro-supervisor-pulse.service.
_dlw_systemd_user_service_available() {
	[[ "${MAESTRO_SKIP_SYSTEMD_WORKER_SERVICE:-0}" == "1" ]] && return 1
...
_dlw_exec_systemd_user_service() {
	...
	local pid_file=""
	pid_file=$(mktemp "${TMPDIR:-/tmp}/maestro-systemd-worker.XXXXXX") || return 1
	...
	if ! systemd-run --user --unit="$unit_name" --collect --quiet \
		--description="maestro worker ${issue_number:-unknown}" \
...
_dlw_exec_detached() {
	...
	local -a worker_command=(
		env
		MAESTRO_GH_PR_LIST_CACHE_DISABLE=1
		PULSE_PR_LIST_PROVIDER_CACHE_DISABLE=1
		"$@"
	)
	# GH#26241: do not blanket-disable MAESTRO_GH_PR_VIEW_CACHE for workers.
	# Pulse now uses a stable TTL-scoped PR view cache, and mutation-sensitive
	# merge/update paths set MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1 at the individual
	...
	if _dlw_systemd_user_service_available; then
		if worker_pid=$(_dlw_exec_systemd_user_service "maestro-worker" "$worker_log" "$issue_number" "${worker_command[@]}"); then
	...
	if [[ -z "${worker_pid:-}" ]] && command -v setsid >/dev/null 2>&1; then
		...
		echo "[dispatch_worker_launch] ERROR: setsid missing — worker isolation broken; worker shares pulse PGID and will be killed on next pulse restart. Run: maestro update (GH#21102)" >>"$LOGFILE"
...
_dlw_spawn_early_exit_monitor() {
	...
	if _dlw_systemd_user_service_available; then
		_dlw_exec_systemd_user_service "maestro-worker-monitor" "/dev/null" "$issue_number" \
...
_dlw_spawn_lifecycle_observer() {
	...
	if _dlw_systemd_user_service_available; then
		_dlw_exec_systemd_user_service "maestro-worker-observer" "/dev/null" "$issue_number" \
...
_dlw_nohup_launch() {
	...
	local -a worker_cmd=(
		...
		MAESTRO_ALLOW_WORKER_WORKTREE_OWNER_TRANSFER=1
	)
	if _dlw_min_worker_floor_active; then
		worker_cmd+=(
			MAESTRO_MIN_WORKER_FLOOR_BYPASS_ACTIVE=1
		)
	fi
	...
	if [[ -n "$worker_prewarm_dir" ]]; then
		worker_cmd+=(MAESTRO_WORKER_PREWARM_DIR="$worker_prewarm_dir")
	fi
	worker_cmd+=(
...
# GH#17503 — 6 dispatches from Aditya060806 + 1 from alex-solovyev,
...
_dlw_post_launch_hooks() {
	...
	local maestro_version="$MAESTRO_UNKNOWN_VERSION" opencode_version="$MAESTRO_UNKNOWN_VERSION"
	if declare -F maestro_find_version >/dev/null 2>&1; then
		maestro_version=$(maestro_find_version 2>/dev/null || printf '%s' "$MAESTRO_UNKNOWN_VERSION")
	fi
	if declare -F _detect_opencode_version >/dev/null 2>&1; then
		opencode_version=$(_detect_opencode_version 2>/dev/null || printf '%s' "")
		opencode_version="${opencode_version:-$MAESTRO_UNKNOWN_VERSION}"
	fi
...
- **Tier**: ${dispatch_tier}
- **Runner**: ${self_login}
- **maestro**: $(_dlw_display_version_or_unknown "$maestro_version")
...
_dlw_min_worker_floor_active() {
	...
	min_worker_floor="${MAESTRO_MIN_WORKER_CONCURRENCY:-}"
...
_dlw_headless_state_dir() {
	printf '%s' "${MAESTRO_HEADLESS_RUNTIME_DIR:-${HOME}/.maestro/.agent-workspace/headless-runtime}"
	return 0
}
...
_dlw_recent_worker_evidence() {
	...
	[[ "$ttl" =~ ^[0-9]+$ ]] || ttl=900
	ledger_file="${MAESTRO_DISPATCH_LEDGER_DIR:-${HOME}/.maestro/.agent-workspace/tmp}/dispatch-ledger.jsonl"
...
_dlw_canary_preflight() {
	...
	local -a _canary_env=()
	if _dlw_min_worker_floor_active; then
		_canary_env+=(MAESTRO_MIN_WORKER_FLOOR_BYPASS_ACTIVE=1)
...
_dlw_opencode_storage_preflight() {
	...
	[[ "${MAESTRO_OPENCODE_STORAGE_PREFLIGHT:-1}" != "0" ]] || return 0
	...
	probe_dir=$(mktemp -d "${tmp_parent%/}/maestro-opencode-snapshot-preflight.XXXXXX" 2>/dev/null) || {
...
_dlw_load_preflight() {
	local issue_number="$1"
	local repo_slug="$2"
	local max_load_per_cpu="${MAESTRO_DISPATCH_MAX_LOAD_PER_CPU:-3.0}"
	local current_ratio=""

	[[ "${MAESTRO_DISPATCH_LOAD_PREFLIGHT:-1}" != "0" ]] || return 0
