...
[SECURITY ADVISORY] TanStack / Mini Shai-Hulud npm compromise — run \`maestro security supply-chain scan\`

Dismiss after scanning and mitigating: maestro security dismiss ${ADVISORY_ID}
EOF
	fi
	return 0
}
...
cmd_startup_check() {
	...
	ensure_advisory
	echo "[SECURITY ADVISORY] TanStack/Mini Shai-Hulud npm compromise: run \`maestro security supply-chain scan\`; dismiss with \`maestro security dismiss ${ADVISORY_ID}\` after mitigation."
	return 0
}
...
cmd_scan() {
	...
	echo "Dismiss after any separate CI/log review: maestro security dismiss ${ADVISORY_ID}"
	return 0
}
