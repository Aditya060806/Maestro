...
#
# Reads the observability SQLite DB written by the opencode-maestro plugin
# (plugins/opencode-maestro/observability.mjs). Surfaces anti-patterns the
...
#   MAESTRO_INTROSPECT_DB — DB path (takes precedence over --db and default)
...
readonly DEFAULT_OBS_DB="${HOME}/.maestro/.agent-workspace/observability/llm-requests.db"
...
_resolve_db_path() {
	local override="${1:-}"
	if [[ -n "${MAESTRO_INTROSPECT_DB:-}" ]]; then
		echo "${MAESTRO_INTROSPECT_DB}"
		return 0
	fi
	if [[ -n "$override" ]]; then
		echo "$override"
...
_require_db() {
	...
	if [[ ! -f "$db" ]]; then
		...
		print_info "is the opencode-maestro plugin active? start opencode and run one tool call to seed it"
		return 1
	fi
