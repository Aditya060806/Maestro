...
set_aws_credentials() {
	...
	if command -v gopass &>/dev/null; then
		...
		key_id=$(gopass show -o "maestro/AWS_ACCESS_KEY_ID" 2>/dev/null || echo "")
		secret_key=$(gopass show -o "maestro/AWS_SECRET_ACCESS_KEY" 2>/dev/null || echo "")
	...
	if [[ -z "${AWS_ACCESS_KEY_ID:-}" || -z "${AWS_SECRET_ACCESS_KEY:-}" ]]; then
		print_error "AWS credentials not found. Set via: maestro secret set AWS_ACCESS_KEY_ID && maestro secret set AWS_SECRET_ACCESS_KEY"
		return 1
	fi
	return 0
}
