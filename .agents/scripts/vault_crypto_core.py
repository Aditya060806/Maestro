...
"""Core primitives for the local maestro Vault helper."""
...
SETUP_TEST_ENTRY_NAME = "__maestro_setup_test__"
SETUP_TEST_VALUE = "maestro vault setup restart verification test"
...
def default_vault_dir() -> Path:
    ...
    if config_home:
        return Path(config_home) / "maestro" / "vault"
    return Path.home() / ".config" / "maestro" / "vault"


def runtime_dir(vault_dir: Path) -> Path:
    configured = os.environ.get("MAESTRO_VAULT_RUNTIME_DIR")
    if configured:
        return Path(configured).expanduser()
    ...
    if runtime:
        ...
        return Path(runtime) / "maestro-vault" / digest
...
def unwrap_root_key(meta: dict[str, Any], passphrase: str) -> bytes:
    ...
    try:
        payload = decrypt_json(kek, dict(meta["wrapped_root_key"]), b"maestro-vault-root-key")
...
def build_metadata(root_key: bytes, passphrase: str) -> dict[str, Any]:
    ...
    wrapped = encrypt_json(kek, {"root_key": b64e(root_key)}, b"maestro-vault-root-key")
    return {
