...
#
# When `maestro update` deploys new code, the pulse restarts and its
...
# (maestro update, setup.sh, manual restart, t2914 ensure-running)
...
# Skip:             MAESTRO_SKIP_CACHE_PRIME=1 pulse-cache-prime.sh
#
# Exit codes:
#   0 - Success or skipped via env opt-out
#   1 - Refresh failed (caller logs but should not abort restart)
#
# Logs to ~/.maestro/logs/pulse-cache-prime.log.
# Sentinel file: ~/.maestro/cache/pulse-cache-prime-last-run.
...
main() {
	if [[ "${MAESTRO_SKIP_CACHE_PRIME:-0}" == "1" ]]; then
		_log "MAESTRO_SKIP_CACHE_PRIME=1 — skipping cache prime"
		return 0
	fi
