...
# Env override: MAESTRO_GH_REST_FALLBACK_THRESHOLD
#
# Tuning rationale:
...
#                         the Aditya060806 runtime: 47 lifetime circuit-breaker
...
_GH_REST_FALLBACK_THRESHOLD="${MAESTRO_GH_REST_FALLBACK_THRESHOLD:-3000}"
...
# callers MUST bypass with MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1.
# Returns: 0 when enabled, 1 otherwise.
#######################################
_rest_pr_view_cache_enabled() {
	[[ "${MAESTRO_GH_PR_VIEW_CACHE:-0}" == "1" ]] || return 1
	[[ "${MAESTRO_GH_PR_VIEW_CACHE_DISABLE:-0}" == "1" ]] && return 1
	local ttl="${MAESTRO_GH_PR_VIEW_CACHE_TTL:-15}"
...
_rest_pr_view_cache_dir() {
	if [[ -n "${MAESTRO_GH_PR_VIEW_CACHE_DIR:-}" ]]; then
		mkdir -p "$MAESTRO_GH_PR_VIEW_CACHE_DIR" 2>/dev/null || return 1
		printf '%s' "$MAESTRO_GH_PR_VIEW_CACHE_DIR"
		return 0
	fi
	if [[ -z "$_GH_REST_PR_VIEW_CACHE_DIR" ]]; then
		_GH_REST_PR_VIEW_CACHE_DIR="${TMPDIR:-/tmp}/maestro-gh-pr-view-cache-${$}"
	fi
...
#######################################
# Append the maestro signature footer to a body file if not already present.
# Used by REST fallback write translators to ensure every GitHub write carries
# the canonical <!-- maestro:sig --> audit marker (t2707).
#
...
_rest_append_sig() {
	...
	grep -q "<!-- maestro:sig -->" "$body_file" 2>/dev/null && return 0
	local helper="" _cand
	for _cand in \
		"${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" \
...
_rest_should_fallback() {
	...
	[[ "${MAESTRO_GH_FORCE_REST_READS:-0}" == "1" ]] && return 0
	local remaining="${1:-}"
	if [[ -z "$remaining" ]]; then
		local _now=0
		local _ttl="${MAESTRO_GH_REST_FALLBACK_CACHE_TTL:-20}"
		if [[ "${MAESTRO_GH_REST_FALLBACK_DISABLE_CACHE:-0}" != "1" && "$_ttl" =~ ^[0-9]+$ && "$_ttl" -gt 0 ]]; then
...
_rest_read_first_enabled() {
	[[ "${MAESTRO_GH_REST_FIRST_READS:-0}" == "1" ]] && return 0
	return 1
}
...
_rest_issue_create() {
	...
	if [[ $has_body -eq 1 ]]; then
		if [[ -n "$body_file" ]]; then
			...
			tmp_body=$(mktemp -t maestro-gh-rest-body.XXXXXX) || return 1
			tmp_body_owned=1
...
_rest_pr_view() {
	...
	local cache_ttl="${MAESTRO_GH_PR_VIEW_CACHE_TTL:-15}"
	...
	elif [[ "${MAESTRO_GH_PR_VIEW_CACHE_DISABLE:-0}" == "1" ]]; then
		gh_record_call other "$cache_name" unknown other bypass-disabled 2>/dev/null || true
	elif [[ "${MAESTRO_GH_PR_VIEW_CACHE:-0}" == "1" ]]; then
