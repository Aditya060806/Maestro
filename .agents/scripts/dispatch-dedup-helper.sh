...
_check_db_entry() {
	...
	local supervisor_dir="${SUPERVISOR_DIR:-${HOME}/.maestro/.agent-workspace/supervisor}"
...
is_duplicate() {
	...
	local supervisor_db="${SUPERVISOR_DIR:-${HOME}/.maestro/.agent-workspace/supervisor}/supervisor.db"
...
check_worker_branch_orphan_loop() {
	...
	local pr_hint="$_DDH_ORPHAN_PR_HINT_NONE"
	pr_hint=$(_ddh_orphan_branch_pr_hint "$repo_slug" "$branch_name")
	local next_action="Open recovery PR: \`gh pr create --repo ${repo_slug} --head ${branch_name} --base ${pr_base_branch}\`" # maestro-allow: raw-gh-wrapper
...
_is_assigned_filter_override_assignees() {
	local assignees="$1"
	local self_login="$2"
	local override_conf="${HOME}/.config/maestro/dispatch-override.conf"
