...
check_review_bot_gate() {
	...
	if [[ -f "$gate_workflow" ]]; then
		...
		print_info "  Suggestion: Copy from maestro repo or add review-bot-gate as required check"
	fi
...
1. Create a fine-grained PAT (pre-filled URL):
   https://github.com/settings/personal-access-tokens/new?name=maestro-sync-pat&description=SYNC_PAT+for+maestro+TODO+auto-completion&expires_in=none&target_name=${slug}&permissions=contents:write,metadata:read
...
Dismiss once fixed:
   maestro security dismiss sync-pat-${slug_sanitised}
...
# per-repo advisories via ~/.maestro/advisories/sync-pat-<slug>.advisory.
check_sync_pat() {
	local repo_path="$1"
	local ruleset_details="${2:-}"
...
# pattern — i.e., it calls the Aditya060806/Maestro reusable workflow AND relies
...
#   1 — not broken (explicit mapping, OR not a Aditya060806/Maestro caller)
...
_detect_cross_account_inherit() {
	...
	# Must be a Aditya060806/Maestro reusable-workflow caller
	if ! printf '%s' "$decoded" | grep -q 'uses:.*Aditya060806/Maestro/'; then
		return 1
	fi
...
This repo's .github/workflows/issue-sync.yml calls the Aditya060806/Maestro
...
Dismiss once fixed:
  maestro security dismiss cross-account-inherit-${slug_sanitised}
...
# For each repo using the Aditya060806/Maestro reusable workflow with
...
check_cross_account_inherit() {
	...
	0)
		...
		print_warn "Cross-account secrets:inherit detected for $slug — run: maestro sync-workflows --apply --repo $slug"
...
# Store security posture in .maestro.json
store_posture() {
	local repo_path="$1"
	local config_file="$repo_path/.maestro.json"

	if [[ ! -f "$config_file" ]]; then
		print_info "No .maestro.json found — skipping posture storage"
		return 0
	fi

	if ! command -v jq &>/dev/null; then
		print_warn "jq not installed — cannot store posture in .maestro.json"
		return 0
	fi
	...
	print_info "Security posture stored in .maestro.json (status: $posture_status)"
	return 0
}
...
print_summary() {
	...
	if [[ -f "$config_file" ]] && command -v jq &>/dev/null; then
		...
		if [[ -n "$stored_status" ]]; then
			...
			critical)
				echo "Security: $stored_critical critical issue(s), $stored_warnings warning(s) — run \`maestro security audit\` (last: $stored_ts)"
				;;
			warning)
				echo "Security: $stored_warnings warning(s) — run \`maestro security audit\` for details (last: $stored_ts)"
				;;
			partial)
			...
			good)
				...
				;;
			*)
				echo "Security: run \`maestro security audit\` for baseline assessment"
				;;
			esac
			return 0
		fi
	...
	echo "Security: not yet audited — run \`maestro security audit\`"
	return 0
}
