...
#######################################
# Look up the local maestro repo slug for issue filing
# Outputs: owner/repo slug string
# Returns: 0 always
#######################################
_get_maestro_slug() {
	local slug=""
	if [[ -f "${HOME}/.config/maestro/repos.json" ]]; then
		slug=$(jq -r 'first(.initialized_repos[] | select(.is_framework_dir == true) | .slug) // ""' \
			"${HOME}/.config/maestro/repos.json" 2>/dev/null) || slug=""
	fi
	[[ -z "$slug" ]] && slug="Aditya060806/Maestro"
	printf '%s' "$slug"
	return 0
}
...
_upstream_watch_issue_creation_authorized() {
	...
	if [[ -z "$login" ]]; then
		_log_warn "Unable to resolve gh login -- upstream-watch public issue creation disabled for ${maestro_slug}"
		return 1
	fi

	local permission=""
	# #maestro:trust-boundary — public upstream-watch issue creation requires
	# confirmed write+ access; API lookup failures skip without claiming none.
	if ! _gh_collaborator_permission_lookup "$maestro_slug" "$login" permission; then
		_log_warn "Skipping public upstream-watch issue creation in ${maestro_slug}: permission check failed for gh user ${login} (HTTP ${MAESTRO_GH_COLLAB_PERMISSION_HTTP:-unknown})"
		return 1
	fi
	...
	case "$permission" in
		admin | maintain | write)
			...
			;;
		*)
			_log_warn "Skipping public upstream-watch issue creation in ${maestro_slug}: gh user ${login} has permission '${permission:-none}', not maintainer/collaborator write access"
			return 1
			;;
	esac
}
...
_write_upstream_watch_local_report() {
	local title="$1"
	local body="$2"
	local report_dir="${HOME}/.maestro/reports/upstream-watch"
	local stamp
...
_compose_upstream_batch_issue_body() {
	...
	printf '<!-- maestro:generator=upstream-watch batch=true -->\n'
	return 0
}
...
_file_upstream_batch_update_issue() {
	...
	local maestro_slug
	maestro_slug=$(_get_maestro_slug)
	if ! _upstream_watch_issue_creation_authorized "$maestro_slug"; then
		local report_file
	...
	existing_number=$(gh issue list --repo "$maestro_slug" --state open \
...
${sig_footer}"
	...
	if [[ -n "$existing_number" ]]; then
		...
		gh_issue_edit_safe "$existing_number" --repo "$maestro_slug" \
		...
		local issue_url=""
		issue_url=$(gh_create_issue --repo "$maestro_slug" \
			--title "$title" \
			--label "$UPSTREAM_WATCH_LABEL" \
			--label "auto-dispatch" \
			--label "tier:standard" \
...
<!-- maestro:generator=upstream-watch upstream_slug=${slug_or_name} -->
...
_close_upstream_update_issue() {
	...
	# Use a high limit because `gh issue list` does not support `--paginate`.
	local issue_number=""
	issue_number=$(gh issue list --repo "$maestro_slug" --state open \
		--label "$UPSTREAM_WATCH_LABEL" \
		--search "in:title \"upstream: ${slug_or_name}\"" \
		--limit 1000 \
	...
	local sig_footer=""
	if [[ -x "${SCRIPT_DIR}/gh-signature-helper.sh" ]]; then
		sig_footer=$("${SCRIPT_DIR}/gh-signature-helper.sh" footer --issue "${maestro_slug}#${issue_number}" --solved 2>/dev/null || true)
	fi
...
${sig_footer}"
	...
	gh_issue_comment "$issue_number" --repo "$maestro_slug" --body "$comment_body" >/dev/null 2>&1 || {
		_log_warn "Failed to post ack comment on issue #${issue_number}"
	}
	gh issue close "$issue_number" --repo "$maestro_slug" --reason "completed" >/dev/null 2>&1 || {
