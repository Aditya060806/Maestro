...
: "${LOGFILE:=${HOME}/.maestro/logs/pulse.log}"
...
if [[ -f "$_PULSE_MERGE_STUCK_CONF" ]]; then
	...
	[[ -z "${MAESTRO_MERGE_STUCK_AGE_MINUTES:-}" ]] && \
		MAESTRO_MERGE_STUCK_AGE_MINUTES=$(grep -E '^MAESTRO_MERGE_STUCK_AGE_MINUTES=' "$_PULSE_MERGE_STUCK_CONF" 2>/dev/null | tail -1 | cut -d= -f2)
	[[ -z "${MAESTRO_MERGE_ZERO_PROGRESS_CYCLES:-}" ]] && \
		MAESTRO_MERGE_ZERO_PROGRESS_CYCLES=$(grep -E '^MAESTRO_MERGE_ZERO_PROGRESS_CYCLES=' "$_PULSE_MERGE_STUCK_CONF" 2>/dev/null | tail -1 | cut -d= -f2)
	[[ -z "${MAESTRO_MERGE_PATTERN_MIN_PRS:-}" ]] && \
		MAESTRO_MERGE_PATTERN_MIN_PRS=$(grep -E '^MAESTRO_MERGE_PATTERN_MIN_PRS=' "$_PULSE_MERGE_STUCK_CONF" 2>/dev/null | tail -1 | cut -d= -f2)
	[[ -z "${MAESTRO_MERGE_STUCK_ENABLED:-}" ]] && \
		MAESTRO_MERGE_STUCK_ENABLED=$(grep -E '^MAESTRO_MERGE_STUCK_ENABLED=' "$_PULSE_MERGE_STUCK_CONF" 2>/dev/null | tail -1 | cut -d= -f2)
fi
...
: "${MAESTRO_MERGE_STUCK_AGE_MINUTES:=240}"
: "${MAESTRO_MERGE_ZERO_PROGRESS_CYCLES:=5}"
: "${MAESTRO_MERGE_PATTERN_MIN_PRS:=3}"
: "${MAESTRO_MERGE_STUCK_ENABLED:=1}"
: "${MAESTRO_MERGE_ZERO_PROGRESS_RECOVERY_CHECK_SECONDS:=3600}"
...
The pulse merge pass has classified PR #${pr_number} as \`${classification}\` and it has been sitting unmerged longer than \`MAESTRO_MERGE_STUCK_AGE_MINUTES\` (currently ${MAESTRO_MERGE_STUCK_AGE_MINUTES}m). The deterministic merge gates are evaluated every cycle (~120s) and this PR has consistently failed them.
...
Every pulse cycle (~120s) the deterministic merge pass re-evaluates open PRs. PRs that pass APPROVED + MERGEABLE but fail required checks have historically been re-evaluated silently every cycle until a human noticed. The stuck-merge detector (t3193) surfaces them after \`MAESTRO_MERGE_STUCK_AGE_MINUTES\` minutes idle. This comment is posted exactly once per linked issue — repeated stuck cycles will NOT spam the thread. If the PR merges and the issue is reopened later with a fresh stuck PR, the marker will allow a second comment.

<sub>Posted automatically by \`pulse-merge-stuck.sh\` (t3193 / GH#21895). Threshold env: \`MAESTRO_MERGE_STUCK_AGE_MINUTES=${MAESTRO_MERGE_STUCK_AGE_MINUTES}\`.</sub>
EOF
...
#######################################
# Group all stuck PRs in the repo by failure fingerprint. If ≥ MAESTRO_MERGE_PATTERN_MIN_PRS
...
_detect_pattern_outage() {
	...
	while IFS=$'\t' read -r count fingerprint prs; do
		[[ "$count" =~ ^[0-9]+$ ]] || continue
		[[ "$count" -ge "$MAESTRO_MERGE_PATTERN_MIN_PRS" ]] || continue
...
When ≥${MAESTRO_MERGE_PATTERN_MIN_PRS} PRs share an identical sorted set of FAILURE check names, the cause is overwhelmingly an upstream CI / base-branch break, not a coincidental cluster of PR-specific bugs. Workers re-dispatched against each PR will burn tokens fixing symptoms; this issue routes the diagnosis to the base.
...
<sub>Source: pulse-merge-stuck-detector. Fingerprint hash: ${fp_hash}. Threshold env: \`MAESTRO_MERGE_PATTERN_MIN_PRS=${MAESTRO_MERGE_PATTERN_MIN_PRS}\`.</sub>"
...
Detection thresholds: queued > \`${MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN:-50}\` AND ratio > \`${MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN:-10}\`. Both conditions held in this cycle.
...
- \`gh api "repos/${repo_slug}/actions/runs?status=queued&per_page=1" --jq '.total_count'\` returns ≤ \`${MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN:-50}\`.
...
<sub>Source: pulse-merge-stuck-detector. Threshold env: \`MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN=${MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN:-50}\`, \`MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN=${MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN:-10}\`.</sub>
EOF
	printf '%s' "$body"
	return 0
}
...
_pms_file_zero_progress_meta_issue() {
	...
	# the framework repo (Aditya060806/Maestro), not in the affected project.
	local meta_repo="Aditya060806/Maestro"
...
The pulse deterministic merge pass has had **${zero_cycles} consecutive cycles** with eligible-unmerged PRs but zero merges. The threshold is \`MAESTRO_MERGE_ZERO_PROGRESS_CYCLES=${MAESTRO_MERGE_ZERO_PROGRESS_CYCLES}\`.
...
1. Read \`~/.maestro/logs/pulse.log\` since the meta-issue was filed (\`grep 'Merge pass: skipping' \`) — the per-PR skip reason is logged for every cycle.
...
<sub>Source: pulse-merge-stuck-detector. Threshold env: \`MAESTRO_MERGE_ZERO_PROGRESS_CYCLES=${MAESTRO_MERGE_ZERO_PROGRESS_CYCLES}\`.</sub>"
...
_pms_close_zero_progress_meta_issue_if_recovered() {
	...
	# #maestro:trust-boundary — public issue comments can succeed for non-collaborators.
...
#
# Distinct from pulse_merge_stuck_run_pass which adds the MAESTRO_MERGE_STUCK_AGE_MINUTES
...
pulse_merge_stuck_run_pass() {
	...
	[[ -n "$repo_slug" ]] || return 0
	[[ "$MAESTRO_MERGE_STUCK_ENABLED" == "1" ]] || return 0
	...
	local age_threshold_secs=$((MAESTRO_MERGE_STUCK_AGE_MINUTES * 60))
	...
	if [[ "$eligible_stuck_count" -ge "$MAESTRO_MERGE_PATTERN_MIN_PRS" ]]; then
		_detect_pattern_outage "$repo_slug" "$(printf '%b' "$stuck_pr_numbers")" || true
	fi

	echo "[pulse-merge-stuck] pulse_merge_stuck_run_pass: ${repo_slug} — eligible_stuck=${eligible_stuck_count}, threshold=${MAESTRO_MERGE_STUCK_AGE_MINUTES}m, saturated=${is_saturated} (queued=${sat_queued}, in_progress=${sat_in_progress}, ratio=${sat_ratio}, blocked_by_saturation=${saturation_blocked_count})" >>"$LOGFILE"
	return 0
}
...
_pms_close_zero_progress_meta_issue_if_recovered_due() {
	...
	local interval="${MAESTRO_MERGE_ZERO_PROGRESS_RECOVERY_CHECK_SECONDS:-3600}"
...
#
# When the counter crosses MAESTRO_MERGE_ZERO_PROGRESS_CYCLES, files a
...
pulse_merge_zero_progress_record() {
	...
	local threshold="${MAESTRO_MERGE_ZERO_PROGRESS_CYCLES:-}"
