...
readonly LOCAL_SUBAGENT="${HOME}/.maestro/agents/content/humanise.md"
SCRIPT_DIR="$(dirname "$0")"
source "${SCRIPT_DIR}/shared-constants.sh"

readonly SCRIPT_DIR
readonly SOURCE_SUBAGENT="${SCRIPT_DIR}/../content/humanise.md"
readonly CACHE_DIR="${HOME}/.maestro/.agent-workspace/cache"
...
cmd_diff() {
	...
	echo "=========================================="
	echo ""
	echo "Note: Local subagent has maestro-specific adaptations."
	echo "Focus on content changes in the upstream patterns."
	echo ""

	# Strip YAML frontmatter (between --- markers) before diffing
	# Local subagent has maestro-specific frontmatter that differs from upstream
...
cmd_help() {
	...
	echo "  - maestro frontmatter format"
