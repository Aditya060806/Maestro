...
check_quality_standards() {
	...
	if command -v curl &>/dev/null && command -v jq &>/dev/null; then
		local response
		if response=$(curl -s --max-time 10 "https://sonarcloud.io/api/issues/search?componentKeys=Aditya060806_maestro&impactSoftwareQualities=MAINTAINABILITY&resolved=false&ps=1" 2>/dev/null); then
			local total_issues
...
_init_root_file_allowlist() {
	ROOT_FILE_ALLOWLIST=(
		...
		".maestro.json" ".bandit" ".gitattributes" ".gitignore"
		...
		# .agents/scripts/maestro-cli/ or .agents/scripts/setup/modules/.
		# Tool configs
...
check_workflow_files() {
	...
	if [[ ! -f "$helper_path" ]]; then
		helper_path="$HOME/.maestro/agents/scripts/lint-workflows-helper.sh"
	fi
...
_coderabbit_cli_review_timeout() {
	local review_timeout="${MAESTRO_CODERABBIT_CLI_REVIEW_TIMEOUT:-45}"
	if ! [[ "$review_timeout" =~ ^[0-9]+$ ]] || ((review_timeout < 1)); then
		print_warning "Invalid MAESTRO_CODERABBIT_CLI_REVIEW_TIMEOUT='${review_timeout}'; using 45s"
		review_timeout=45
	fi
	printf '%s\n' "$review_timeout"
	return 0
...
run_optional_coderabbit_cli_review() {
	if [[ "${MAESTRO_SKIP_CODERABBIT_CLI_REVIEW:-0}" == "1" ]]; then
		print_info "CodeRabbit CLI review skipped (MAESTRO_SKIP_CODERABBIT_CLI_REVIEW=1)"
		return 0
	fi
