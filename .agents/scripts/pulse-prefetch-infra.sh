...
# the maestro repo deploy. Per-repo overrides override default values;
...
_load_pulse_sweep_budget_config() {
	...
	if [[ -f "${HOME}/.maestro/agents/configs/pulse-sweep-budget.json" ]]; then
		config_file="${HOME}/.maestro/agents/configs/pulse-sweep-budget.json"
	elif [[ -n "${MAESTRO_REPO_ROOT:-}" && -f "${MAESTRO_REPO_ROOT}/.agents/configs/pulse-sweep-budget.json" ]]; then
		config_file="${MAESTRO_REPO_ROOT}/.agents/configs/pulse-sweep-budget.json"
	fi
...
prefetch_hygiene_anomalies() {
	...
	local counters_file="${HOME}/.maestro/cache/pulse-label-invariants.${hostname_short}.json"
