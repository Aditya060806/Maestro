...
# Matches the `~/.maestro/.agent-workspace/` convention from AGENTS.md.
CLAIM_STAMP_DIR="${HOME}/.maestro/.agent-workspace/interactive-claims"
...
# Check whether the authenticated user should run maestro maintainer-only
...
# internal maestro workflow state to an upstream project we cannot verify.
_isc_can_manage_issue_state() {
	...
	local permission=""
	# #maestro:trust-boundary — managed-repo claim/state writes require a
...
USAGE:
  ...
  interactive-session-helper.sh scan-stale [--auto-release | --no-auto-release]
      ...
      Phase 1  — Identify stamps with dead PID AND missing worktree path.
                 ...
                 MAESTRO_SCAN_STALE_AUTO_RELEASE=0|1 env var.
...
CONTRACT:
  ...
  Slug format: owner/repo (e.g., Aditya060806/Maestro).
