...
#   1. Stop flag  (~/.maestro/logs/pulse-session.stop)  — highest priority, pauses pulse
#   2. Session flag (~/.maestro/logs/pulse-session.flag) — explicit start, doesn't survive reboots
...
# Configuration
readonly SESSION_FLAG="${HOME}/.maestro/logs/pulse-session.flag"
readonly STOP_FLAG="${HOME}/.maestro/logs/pulse-session.stop"
readonly LOGFILE="${HOME}/.maestro/logs/pulse.log"
readonly WRAPPER_LOGFILE="${HOME}/.maestro/logs/pulse-wrapper.log"
readonly PIDFILE="${HOME}/.maestro/logs/pulse.pid"
readonly MAX_WORKERS_FILE="${HOME}/.maestro/logs/pulse-max-workers"
readonly REPOS_JSON="${HOME}/.config/maestro/repos.json"
...
# which handles: env var override (MAESTRO_SUPERVISOR_PULSE) >
...
is_scheduler_installed() {
	...
	Darwin)
		...
		if launchctl list 2>/dev/null | grep -qF "com.maestro.supervisor-pulse"; then
			return 0
		fi
		return 1
		;;
	*)
		...
		if command -v systemctl >/dev/null 2>&1 &&
			systemctl --user is-enabled maestro-supervisor-pulse.timer >/dev/null 2>&1; then
			return 0
		fi
		# Fall back to cron entry check
		if crontab -l 2>/dev/null | grep -qF "maestro-supervisor-pulse"; then
			return 0
		fi
		return 1
		;;
...
cmd_start() {
	...
	if is_session_active; then
		...
		echo ""
		echo "  To restart: maestro pulse stop && maestro pulse start"
		return 0
	fi
...
EOF
	...
	echo "  Stop with: maestro pulse stop"
	return 0
}
...
_stop_wait_for_workers() {
	...
	if [[ "$worker_count" -gt 0 ]]; then
		...
		echo "  No new work will be dispatched."
		echo "  Force stop: maestro pulse stop --force"
	else
		print_success "All workers finished — clean shutdown"
	fi
	return 0
...
_status_resolve_consent() {
	...
	if [[ "$has_stop_flag" == "true" ]]; then
		...
		effective_reason="stop flag (maestro pulse stop)"
	elif [[ "$has_session_flag" == "true" ]]; then
		effective_state="enabled"
		effective_reason="session flag (maestro pulse start)"
...
_status_print_hints() {
	...
	if [[ "$effective_state" == "enabled" ]]; then
		echo "  Stop:  maestro pulse stop"
		echo "  Force: maestro pulse stop --force"
	elif [[ "$effective_state" == "stopped" ]]; then
		echo "  Resume: maestro pulse start"
	else
		echo "  Start:  maestro pulse start"
...
ENVIRONMENT:
    ...
    MAESTRO_SUPERVISOR_PULSE  Override config consent (true/false)
...
HOW IT WORKS:
    ...
    1. Stop flag (~/.maestro/logs/pulse-session.stop)
       Highest priority. If present, pulse is paused regardless of other
       layers. Created by 'maestro pulse stop', cleared by 'start'.

    2. Session flag (~/.maestro/logs/pulse-session.flag)
       Explicit user action. Created by 'maestro pulse start'.
       Does NOT survive reboots — use for bounded work sessions.

    3. Config consent (orchestration.supervisor_pulse=true)
       Persistent setting in ~/.config/maestro/config.jsonc.
       Survives reboots — the pulse runs unattended after reboot.
       Set during 'maestro init' or manually in config.

    If none of the above are set, the pulse skips (no-op).

    For unattended operation: set config consent and don't use start/stop.
    For bounded sessions: use 'maestro pulse start' and 'stop'.
    To pause temporarily: 'maestro pulse stop' (overrides config consent).

EXAMPLES:
    maestro pulse start           # Enable pulse for this session
    maestro pulse status          # Show consent layers and workers
    maestro pulse stop            # Graceful stop (wait for workers)
    maestro pulse stop --force    # Stop immediately
...
main() {
	...
	*)
		...
		echo "Run 'maestro pulse help' for usage."
		return 1
		;;
	esac
}
