...
#
# Bridges Matrix chat rooms to maestro runners via OpenCode server.
...
#
# Configuration:
#   ~/.config/maestro/matrix-bot.json
#
# Security:
...
# Configuration
readonly CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/maestro"
readonly CONFIG_FILE="$CONFIG_DIR/matrix-bot.json"
readonly DATA_DIR="$HOME/.maestro/.agent-workspace/matrix-bot"
...
readonly ENTITY_HELPER="$HOME/.maestro/agents/scripts/entity-helper.sh"
readonly MEMORY_DB="${MAESTRO_MEMORY_DIR:-$HOME/.maestro/.agent-workspace/memory}/memory.db"
readonly RUNNER_HELPER="$HOME/.maestro/agents/scripts/runner-helper.sh"
...
AUTO-SETUP (Cloudron + Synapse):
    ...
    Prerequisites:
      ...
      - Synapse admin token stored via: maestro secret set SYNAPSE_ADMIN_TOKEN_<server>
...
CONFIGURATION:
    Config: ~/.config/maestro/matrix-bot.json
    Data:   ~/.maestro/.agent-workspace/matrix-bot/
    Logs:   ~/.maestro/.agent-workspace/matrix-bot/logs/
