...
# terminal-title-helper.sh - Terminal tab/window title integration for maestro
...
#
# Examples:
#   terminal-title-helper.sh sync                    # Set title to "maestro/feature/xyz"
...
TERMINAL_TITLE_ENABLED="${TERMINAL_TITLE_ENABLED:-${MAESTRO_TABBY_ENABLED:-true}}"
...
# emitting "main" or "maestro/main" into the OSC title clobbers the
...
cmd_sync() {
	...
	# terminal titles with "main"/"master"/"maestro/main" when the canonical
...
ENVIRONMENT VARIABLES:
    TERMINAL_TITLE_FORMAT
        ...
        - "repo/branch" (default): "maestro/feature/xyz"
        - "branch": "feature/xyz"
        - "repo": "maestro"
        - "branch/repo": "feature/xyz (maestro)"
...
SHELL INTEGRATION:
    ...
    # Bash
    PROMPT_COMMAND='~/.maestro/agents/scripts/terminal-title-helper.sh sync 2>/dev/null'

    # Zsh
    precmd() { ~/.maestro/agents/scripts/terminal-title-helper.sh sync 2>/dev/null }

    # Fish
    function fish_prompt
        ~/.maestro/agents/scripts/terminal-title-helper.sh sync 2>/dev/null
        # ... rest of prompt
    end
