...
#
# EOA API credentials:
#   maestro secret set EOA_API_KEY
#   maestro secret set EOA_API_PASSWORD
#   Or set in ~/.config/maestro/credentials.sh:
...
# Load EOA API credentials from maestro secret store or credentials.sh
load_eoa_credentials() {
	...
	if command -v gopass &>/dev/null; then
		api_key=$(gopass show -o "maestro/EOA_API_KEY" 2>/dev/null || echo "")
		api_password=$(gopass show -o "maestro/EOA_API_PASSWORD" 2>/dev/null || echo "")
	fi
...
eoa_auth() {
	...
	if [[ "$sandbox" == "true" ]]; then
		...
		if ! load_eoa_credentials; then
			...
			print_info "  maestro secret set EOA_API_KEY"
			print_info "  maestro secret set EOA_API_PASSWORD"
...
show_help() {
	...
	echo "  maestro secret set EOA_API_KEY"
	echo "  maestro secret set EOA_API_PASSWORD"
