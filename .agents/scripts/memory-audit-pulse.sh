...
#   - Cron: 0 4 * * * ~/.maestro/agents/scripts/memory-audit-pulse.sh run --quiet
...
readonly AUDIT_LOG_DIR="${MAESTRO_MEMORY_AUDIT_LOG_DIR:-$HOME/.maestro/.agent-workspace/work/memory-audit}"
...
_opportunities_action_details() {
	...
	local task_cmd="${MAESTRO_MEMORY_AUDIT_TASK_CMD:-claim-task-id.sh}"
...
INTEGRATION:
    ...
    # Cron (daily at 4 AM)
    0 4 * * * ~/.maestro/agents/scripts/memory-audit-pulse.sh run --quiet
