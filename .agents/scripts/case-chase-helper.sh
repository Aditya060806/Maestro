...
_chase_read_mailbox_json() {
	...
	[[ ! -f "$config_file" ]] && config_file="${HOME}/.config/maestro/mailboxes.json"

	if [[ ! -f "$config_file" ]]; then
		print_error "mailboxes.json not found. Create at _config/mailboxes.json or ~/.config/maestro/mailboxes.json"
		return 1
	fi
...
cmd_send() {
	...
	template_path=$(_chase_find_template "$repo_path" "$template") \
		|| { print_error "Template not found: ${template}. Run: maestro case chase-template list"; return 1; }
