...
#   - State file: ~/.maestro/cache/foss-contribution-state.json
#
# Usage:
...
#
# State file: ~/.maestro/cache/foss-contribution-state.json
# Config: ~/.config/maestro/config.jsonc (foss section)
# Repos: ~/.config/maestro/repos.json (foss fields per repo)
...
CONFIG_JSONC="${HOME}/.config/maestro/config.jsonc"
STATE_FILE="${HOME}/.maestro/cache/foss-contribution-state.json"
LOGFILE="${HOME}/.maestro/logs/foss-contribution.log"
...
cmd_scan() {
	...
	if ! _is_foss_enabled; then
		echo -e "${YELLOW}FOSS contributions are disabled globally. Enable with: maestro config set foss.enabled true${NC}"
		return 0
	fi
