...
# repo-maestro-health-helper.sh — r914 daily repo-maestro health keeper.
#
# Three drift checks across repos in ~/.config/maestro/repos.json:
#
#   1. Stale-version bump (autonomous, safe). For each initialized_repos[] entry
#      whose path exists and contains .maestro.json with an older version than
#      the currently-installed framework: rewrite .maestro.json on a dedicated
...
#      update a needs-maintainer-review issue on Aditya060806/Maestro.
...
#      .maestro-skip marker or .maestro.json: file or update an issue.
...
#
# Usage:
#   repo-maestro-health-helper.sh enable   Install daily scheduler (launchd/systemd/cron)
#   repo-maestro-health-helper.sh disable  Remove scheduler
#   repo-maestro-health-helper.sh status   Show current state and last run results
#   repo-maestro-health-helper.sh check    One-shot: run all three drift checks now
#   repo-maestro-health-helper.sh run      Alias for 'check' (matches r914 routine run: spec)
#   repo-maestro-health-helper.sh logs [--tail N]  View logs
#   repo-maestro-health-helper.sh help     Show this help
#
# Configuration:
#   ~/.config/maestro/repos.json            initialized_repos[] + git_parent_dirs[]
#   MAESTRO_REPO_HEALTH=false               Disable even if scheduler is installed
#   MAESTRO_REPO_HEALTH_INTERVAL=1440       Minutes between runs (default 1440 = daily)
#   REPOS_DRIFT_FLAG_INTERVAL_DAYS=7         Min days between re-flagging the same drift
#   MAESTRO_REPO_HEALTH_DRY_RUN=1           Log detections without writes/pushes/issue creation
#
# Logs: ~/.maestro/logs/repo-maestro-health.log
# State: ~/.maestro/cache/repo-maestro-health-state.json (includes last_flagged timestamps)
...
# Configuration
readonly CONFIG_FILE="$HOME/.config/maestro/repos.json"
readonly LOCK_DIR="$HOME/.maestro/locks"
readonly LOCK_FILE="$LOCK_DIR/repo-maestro-health.lock"
readonly LOG_FILE="$HOME/.maestro/logs/repo-maestro-health.log"
readonly STATE_FILE="$HOME/.maestro/cache/repo-maestro-health-state.json"
readonly CRON_MARKER="# maestro-repo-maestro-health"
readonly DEFAULT_INTERVAL=1440
readonly REPO_MAESTRO_HEALTH_SCHEDULE='daily(@03:30)'
readonly LAUNCHD_LABEL="sh.maestro.repo-maestro-health"
...
readonly SYSTEMD_UNIT_NAME="maestro-repo-maestro-health"
readonly INSTALL_DIR="$HOME/Git/maestro"
...
#######################################
ensure_dirs() {
	mkdir -p "$LOCK_DIR" "$HOME/.maestro/logs" "$HOME/.maestro/cache" 2>/dev/null || true
	return 0
}
...
_get_scheduler_backend() {
	# Source platform-detect.sh if MAESTRO_SCHEDULER is not already set
	if [[ -z "${MAESTRO_SCHEDULER:-}" ]]; then
		local _pd_path
	...
	if [[ -n "${MAESTRO_SCHEDULER:-}" ]]; then
		echo "$MAESTRO_SCHEDULER"
...
#######################################
# Check if the repo-maestro-health LaunchAgent is loaded
...
#######################################
# Generate repo-maestro-health LaunchAgent plist content
# Arguments:
#   $1 - script_path
...
_generate_plist() {
	...
	env_path=$(maestro_launchd_sanitized_path "$env_path")
...
_version_bump_branch_name() {
	...
	safe_slug="${safe_slug##-}"
	safe_slug="${safe_slug%%-}"
	printf 'chore/maestro-version-v%s-%s\n' "$target_version" "$safe_slug"
	return 0
}
...
- Bump \`.maestro.json\` from v${entry_version} to v${target_version}.
- Keep r914 repo-maestro health updates compatible with protected default branches.

## Testing

- Generated by \`repo-maestro-health-helper.sh\` after validating a clean default-branch worktree.
EOF
	return 0
}
...
_open_version_bump_pr() {
	...
	pr_title="chore: bump .maestro.json to v${target_version}"
...
#######################################
# Read the configured maestro version for one managed repo.
# Args:
#   $1 — .maestro.json path
# Outputs: version string, or empty when unreadable/missing.
#######################################
_repo_maestro_entry_version() {
	local adj_file="$1"
	jq -r '.maestro_version // empty' "$adj_file" 2>/dev/null || true
	return 0
}
...
#######################################
# Publish a preserved default-branch-only .maestro.json bump through a PR.
# Args:
#   $1 — slug
#   $2 — repo_path
#   $3 — local_only flag
...
_publish_preserved_default_branch_bump() {
	...
	if [[ "$ahead_count" == "0" || "$ahead_files" != ".maestro.json" ]]; then
		echo skipped
		return 0
	fi
...
#######################################
# Atomically rewrite .maestro.json with the target framework version.
# Args:
#   $1 — slug
#   $2 — adj_file
#   $3 — target_version
...
#######################################
_rewrite_repo_maestro_version() {
	local slug="$1"
	local adj_file="$2"
	local target_version="$3"
	local tmp_adj
	tmp_adj=$(mktemp) || return 1
	if ! jq --arg v "$target_version" '.maestro_version = $v' "$adj_file" >"$tmp_adj" 2>/dev/null; then
...
#######################################
# Commit the rewritten .maestro.json in a managed repo.
# Args:
#   $1 — slug
#   $2 — repo_path
#   $3 — target_version
...
#######################################
_commit_repo_maestro_version() {
	local slug="$1"
	local repo_path="$2"
	local target_version="$3"
	local default_branch="$4"
	if ! git -C "$repo_path" add .maestro.json >/dev/null 2>&1 ||
		! git -C "$repo_path" commit -m "chore: bump .maestro.json to v${target_version} (r914)" >/dev/null 2>&1; then
...
#######################################
# Bump one repo's .maestro.json to the current framework version.
# Globals: CONFIG_FILE (implicit via callers), MAESTRO_* env vars
# Args:
#   $1 — slug (for logs)
...
_bump_single_repo() {
	...
	local adj_file="$repo_path/.maestro.json"
	local entry_version
	entry_version=$(_repo_maestro_entry_version "$adj_file")
	...
	if ! _rewrite_repo_maestro_version "$slug" "$adj_file" "$target_version"; then
		if [[ "$local_only" != "true" ]]; then
			git -C "$repo_path" checkout -f -q "$default_branch" >/dev/null 2>&1 || true
		fi
		echo failed
	...
	if ! _commit_repo_maestro_version "$slug" "$repo_path" "$target_version" "$default_branch"; then
		echo failed
		return 0
	fi
...
_check_version_bumps() {
	...
	local slug repo_path local_only outcome
	{
		while IFS=$'\t' read -r slug repo_path local_only; do
			...
			[[ -f "$repo_path/.maestro.json" ]] || continue
	...
	for parent_dir in "${parent_dirs[@]}"; do
		...
		raw=$(find "$parent_dir" -maxdepth 1 -mindepth 1 -type d 2>/dev/null)
		{
			while IFS= read -r candidate; do
				...
				[[ -f "$candidate/.maestro-skip" ]] && continue
				[[ -f "$candidate/.maestro.json" ]] && continue
				log_warn "no-init: $candidate — git repo with no .maestro.json and no .maestro-skip marker. Will file issue in follow-up implementation."
...
_current_framework_version() {
	local version_file="$HOME/.maestro/agents/VERSION"
	[[ -f "$version_file" ]] || {
		echo ""
		return 0
	}
...
# Honours: MAESTRO_REPO_HEALTH (false disables), MAESTRO_REPO_HEALTH_DRY_RUN=1.
# Returns: 0 on clean run, 1 if any bump failed.
#######################################
cmd_check() {
	ensure_dirs
	if [[ "${MAESTRO_REPO_HEALTH:-}" == "false" ]]; then
		log_info "r914 disabled via MAESTRO_REPO_HEALTH=false"
		return 0
	fi
	...
	local dry_run=0
	if [[ "${MAESTRO_REPO_HEALTH_DRY_RUN:-0}" == "1" ]]; then
		dry_run=1
...
#######################################
# Enable repo-maestro-health via launchd (macOS)
# Arguments:
#   $1 - script_path
#   $2 - interval (minutes)
#######################################
_enable_launchd() {
	...
	# Migrate from old label if present (com.maestro -> sh.maestro)
	local old_label="com.maestro.maestro-repo-maestro-health"
	...
	# Create named symlink so macOS System Settings shows "maestro-repo-maestro-health"
	local bin_dir="$HOME/.maestro/bin"
	mkdir -p "$bin_dir"
	local display_link="$bin_dir/maestro-repo-maestro-health"
	...
	if launchctl load -w "$LAUNCHD_PLIST" 2>/dev/null; then
		...
		echo "  Logs:      $LOG_FILE"
		echo ""
		echo "  Disable with: maestro repo-maestro-health disable"
		echo "  Sync now:     maestro repo-maestro-health check"
	else
...
#######################################
# Enable repo-maestro-health via systemd user timer (Linux with systemd)
# Arguments:
#   $1 - script_path
#   $2 - interval (minutes)
...
_enable_systemd() {
	...
	local on_calendar=""
	on_calendar=$("${SCRIPT_DIR}/routine-schedule-helper.sh" systemd-calendar "$REPO_MAESTRO_HEALTH_SCHEDULE") || return 1

	mkdir -p "${SYSTEMD_SERVICE_DIR}"

	printf '%s' "[Unit]
Description=maestro repo-maestro-health
...
" >"$service_file"
	...
	printf '%s' "[Unit]
Description=maestro repo-maestro-health Timer
...
" >"$timer_file"
	...
	print_success "Repo sync enabled (${REPO_MAESTRO_HEALTH_SCHEDULE})"
	echo ""
...
#######################################
# Disable repo-maestro-health systemd user timer
...
#######################################
# Enable repo-maestro-health via cron (Linux)
# Arguments:
#   $1 - script_path
#   $2 - interval (minutes)
#######################################
...
#######################################
# Enable repo-maestro-health scheduler (platform-aware)
...
cmd_enable() {
	...
	local interval="${MAESTRO_REPO_HEALTH_INTERVAL:-$DEFAULT_INTERVAL}"
	local script_path="$HOME/.maestro/agents/scripts/repo-maestro-health-helper.sh"

	# Verify the script exists at the deployed location
	if [[ ! -x "$script_path" ]]; then
		# Fall back to repo location
		script_path="$INSTALL_DIR/.agents/scripts/repo-maestro-health-helper.sh"
		if [[ ! -x "$script_path" ]]; then
			print_error "repo-maestro-health-helper.sh not found"
			return 1
		fi
	fi
...
#######################################
# Disable repo-maestro-health scheduler (platform-aware)
#######################################
cmd_disable() {
	local backend
	backend="$(_get_scheduler_backend)"
	...
	if [[ "$backend" == "launchd" ]]; then
		...
		# Also clean up old label if present (com.maestro -> sh.maestro migration)
		local old_label="com.maestro.maestro-repo-maestro-health"
		local old_plist="${LAUNCHD_DIR}/${old_label}.plist"
		# Capture output first to avoid SIGPIPE (141) under set -o pipefail (t3270)
...
cmd_status() {
	...
	if [[ "${MAESTRO_REPO_HEALTH:-}" == "false" ]]; then
		echo ""
		echo -e "  ${YELLOW}Note: MAESTRO_REPO_HEALTH=false is set (overrides scheduler)${NC}"
	fi
...
_dirs_list() {
	...
	if [[ -z "$dirs" ]]; then
		...
		echo "Add one with: maestro repo-maestro-health dirs add ~/Git"
		return 0
	fi
...
_dirs_add() {
	local new_dir="${1:-}"
	if [[ -z "$new_dir" ]]; then
		print_error "Usage: maestro repo-maestro-health dirs add <path>"
		return 1
	fi
...
_dirs_remove() {
	local rm_dir="${1:-}"
	if [[ -z "$rm_dir" ]]; then
		print_error "Usage: maestro repo-maestro-health dirs remove <path>"
		return 1
	fi
...
cmd_dirs() {
	...
	*)
		...
		echo "Usage: maestro repo-maestro-health dirs [list|add|remove|rm]"
		return 1
		;;
	esac
	return 0
...
cmd_config() {
	...
	echo "  maestro repo-maestro-health dirs list          # Show configured directories"
	echo "  maestro repo-maestro-health dirs add ~/Projects  # Add a directory"
	echo "  maestro repo-maestro-health dirs remove ~/Old    # Remove a directory"
	echo ""
	return 0
}
...
cmd_logs() {
	...
	if [[ -f "$LOG_FILE" ]]; then
		...
		print_info "No log file yet (repo-maestro-health hasn't run)"
	fi
	return 0
}
...
cmd_help() {
	cat <<'EOF'
repo-maestro-health-helper.sh - Daily git pull of repos in configured parent directories

USAGE:
    repo-maestro-health-helper.sh <command> [options]
    maestro repo-maestro-health <command> [options]
...
ENVIRONMENT:
    MAESTRO_REPO_HEALTH=false             Disable even when scheduler is installed
    MAESTRO_REPO_HEALTH_INTERVAL=1440     Minutes between syncs (default: 1440 = daily)

CONFIGURATION:
    Manage with: maestro repo-maestro-health dirs [add|remove|list]
    Or manually add "git_parent_dirs" array to ~/.config/maestro/repos.json:
...
SCHEDULER BACKENDS:
    macOS:  launchd LaunchAgent (~/Library/LaunchAgents/sh.maestro.repo-maestro-health.plist)
            - Runs daily (every 1440 minutes by default)
    Linux:  cron (daily at 3am, crontab entry with # maestro-repo-maestro-health marker)

HOW IT WORKS:
    1. Scheduler runs 'repo-maestro-health-helper.sh check' daily
    2. Reads git_parent_dirs from ~/.config/maestro/repos.json
    ...
    5. Logs results (pulled/skipped/failed) to ~/.maestro/logs/repo-maestro-health.log

LOGS:
    ~/.maestro/logs/repo-maestro-health.log
...
if [[ "${MAESTRO_REPO_HEALTH_HELPER_SOURCE_ONLY:-0}" != "1" ]]; then
	main "$@"
fi
