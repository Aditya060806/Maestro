...
# auto-update-helper.sh - Automatic update polling daemon for maestro
#
# Lightweight cron job that checks for new maestro releases every 10 minutes
...
#
# Configuration:
#   All values can be set via JSONC config (maestro config set <key> <value>)
#   or overridden per-session via environment variables (higher priority).
#
#   JSONC key                          Env override                    Default
#   updates.auto_update                MAESTRO_AUTO_UPDATE            true
#   updates.update_interval_minutes    MAESTRO_UPDATE_INTERVAL        10
#   updates.skill_auto_update          MAESTRO_SKILL_AUTO_UPDATE      true
#   updates.skill_freshness_hours      MAESTRO_SKILL_FRESHNESS_HOURS  24
#   updates.openclaw_auto_update       MAESTRO_OPENCLAW_AUTO_UPDATE   true
#   updates.openclaw_freshness_hours   MAESTRO_OPENCLAW_FRESHNESS_HOURS 24
#   updates.tool_auto_update           MAESTRO_TOOL_AUTO_UPDATE       true
#   updates.tool_freshness_hours       MAESTRO_TOOL_FRESHNESS_HOURS   6
#   updates.tool_idle_hours            MAESTRO_TOOL_IDLE_HOURS        6
#   updates.upstream_watch             MAESTRO_UPSTREAM_WATCH         true
#   updates.upstream_watch_hours       MAESTRO_UPSTREAM_WATCH_HOURS   24
#   updates.venv_health_check          MAESTRO_VENV_HEALTH_CHECK      true
#   updates.venv_health_hours          MAESTRO_VENV_HEALTH_HOURS      24
#
# Logs: ~/.maestro/logs/auto-update.log

set -euo pipefail

# Resolve symlinks to find real script location (t1262)
# When invoked via symlink (e.g. ~/.maestro/bin/maestro-auto-update),
...
readonly LOG_FILE="$HOME/.maestro/logs/auto-update.log"
readonly STATE_FILE="$HOME/.maestro/cache/auto-update-state.json"
readonly CRON_MARKER="# maestro-auto-update"
...
readonly LAUNCHD_LABEL="com.maestro.maestro-auto-update"
...
readonly SYSTEMD_UNIT_NAME="maestro-auto-update"
...
_lock_holder_is_wedged() {
	...
	local _log="${LOG_FILE:-$HOME/.maestro/logs/auto-update.log}"
...
get_remote_version() {
	...
	# See: https://github.com/Aditya060806/Maestro/issues/4142
	if command -v gh &>/dev/null && gh auth status &>/dev/null; then
		version=$(gh api repos/Aditya060806/Maestro/contents/VERSION \
	...
	if command -v jq &>/dev/null; then
		version=$(curl --proto '=https' -fsSL --max-time 10 \
			"https://api.github.com/repos/Aditya060806/Maestro/contents/VERSION" 2>/dev/null |
	...
	curl --proto '=https' -fsSL --max-time 10 \
		"https://raw.githubusercontent.com/Aditya060806/Maestro/main/VERSION" 2>/dev/null |
		tr -d '\n' || echo "unknown"
	return 0
}

#######################################
# Check if setup.sh or maestro update is already running
#######################################
is_update_running() {
	...
	# Check for running maestro update
	if pgrep -f "maestro update" >/dev/null 2>&1; then
		return 0
	fi
	return 1
}
...
_cmd_check_stale_agent_redeploy() {
	...
	# See: https://github.com/Aditya060806/Maestro/issues/3980
	...
	if [[ "$current" != "$deployed_version" ]]; then
		...
		if bash "$INSTALL_DIR/setup.sh" --non-interactive >>"$LOG_FILE" 2>&1; then
			...
			redeployed_version=$(cat "$HOME/.maestro/agents/VERSION" 2>/dev/null || echo "none")
	...
	# Replacement: compare the canonical HEAD SHA against ~/.maestro/.deployed-sha
	...
	# PR #4704 was not deployed to ~/.maestro/ before the next pulse cycle.
...
_cmd_check_git_update() {
	...
	# See: https://github.com/Aditya060806/Maestro/issues/2286
	...
	# Mirrors recovery logic from maestro.sh cmd_update()
	# See: https://github.com/Aditya060806/Maestro/issues/4142
	...
	if ! git -C "$INSTALL_DIR" pull --ff-only origin main --quiet 2>>"$LOG_FILE"; then
		...
		# by maestro and should always track origin/main exactly.
		# See: https://github.com/Aditya060806/Maestro/issues/2288
...
_cmd_check_perform_update() {
	...
	# "tail -50 ~/.maestro/logs/auto-update.log | grep Slowest" is
	# sufficient to diagnose which stage hung, without bash -x re-runs.
	local _stl="$HOME/.maestro/logs/setup-stage-timings.log"
	if [[ -f "$_stl" ]]; then
...
_cmd_enable_launchd() {
	...
	local old_label="com.maestro.auto-update"
	...
	# Create named symlink so macOS System Settings shows "maestro-auto-update"
	# instead of the raw script name (t1260)
	local bin_dir="$HOME/.maestro/bin"
	mkdir -p "$bin_dir"
	local display_link="$bin_dir/maestro-auto-update"
	...
	if launchctl load -w "$LAUNCHD_PLIST" 2>/dev/null; then
		...
		echo "  Logs:      $LOG_FILE"
		echo ""
		echo "  Disable with: maestro auto-update disable"
		echo "  Check now:    maestro auto-update check"
	else
...
_cmd_enable_systemd() {
	...
	printf '%s' "[Unit]
Description=maestro auto-update
...
" >"$service_file"
	...
	printf '%s' "[Unit]
Description=maestro auto-update Timer
...
cmd_enable() {
	...
	local script_path="$HOME/.maestro/agents/scripts/auto-update-helper.sh"
...
_cmd_status_scheduler() {
	...
	if [[ "$backend" == "launchd" ]]; then
		...
		if crontab -l 2>/dev/null | grep -q "$CRON_MARKER"; then
			echo -e "  ${YELLOW}Note: legacy cron entry found — run 'maestro auto-update disable && enable' to migrate${NC}"
		fi
...
cmd_health_check() {
	...
	if ! _daemon_is_loaded; then
		...
		_hc_say "fix: ~/.maestro/agents/scripts/auto-update-helper.sh enable"
		return 2
	fi
	...
	if last_run_epoch=$(date -u -d "$last_ts" '+%s' 2>/dev/null); then
		...
		_hc_say "fix: ~/.maestro/agents/scripts/auto-update-helper.sh check"
		return 1
	fi
...
cmd_help() {
	cat <<'EOF'
auto-update-helper.sh - Automatic update polling for maestro

USAGE:
    auto-update-helper.sh <command> [options]
    maestro auto-update <command> [options]
...
CONFIGURATION:
    Persistent settings: maestro config set <key> <value>
...
SCHEDULER BACKENDS:
    macOS:  launchd LaunchAgent (~/Library/LaunchAgents/com.maestro.maestro-auto-update.plist)
            - Native macOS scheduler, survives reboots without cron
            - Auto-migrates existing cron entries on first 'enable'
    Linux:  systemd user timer preferred (~/.config/systemd/user/maestro-auto-update.timer)
            ...
            - See 'maestro auto-update status' for current linger state
    Linux:  cron fallback (crontab entry with # maestro-auto-update marker)
...
HOW IT WORKS:
    ...
    7. Runs daily OpenClaw update check (24h gate, if openclaw CLI is installed):
       ...
       d. Opt-out: MAESTRO_OPENCLAW_AUTO_UPDATE=false
    8. Runs 6-hourly tool freshness check (idle-gated):
       ...
       e. Opt-out: MAESTRO_TOOL_AUTO_UPDATE=false
