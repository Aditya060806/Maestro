...
# Default: keep canonical repos under ~/Git and create maestro linked worktrees
# in one flat, backup-excludable directory:
#   ~/Git/_worktrees/<repo>-<branch-slug>

[[ -n "${_MAESTRO_WORKTREE_PATHS_LOADED:-}" ]] && return 0
_MAESTRO_WORKTREE_PATHS_LOADED=1

maestro_expand_home_path() {
	local path="$1"
	case "$path" in
	\~) printf '%s\n' "${HOME:-}" ;;
	\~/*) printf '%s/%s\n' "${HOME:-}" "${path#\~/}" ;;
...
maestro_worktree_base_dir_configured() {
	local repos_json="${MAESTRO_REPOS_JSON:-${HOME:-}/.config/maestro/repos.json}"
	local configured=""
	if [[ -n "${MAESTRO_WORKTREE_BASE_DIR:-}" ]]; then
		configured="$MAESTRO_WORKTREE_BASE_DIR"
	elif [[ -n "${MAESTRO_WORKTREES_DIR:-}" ]]; then
		configured="$MAESTRO_WORKTREES_DIR"
	...
	maestro_expand_home_path "$configured"
	return 0
}

maestro_ensure_worktree_base_dir() {
	local base_dir="$1"
	[[ -n "$base_dir" ]] || return 1
...
maestro_worktree_base_dir() {
	local base_dir
	base_dir=$(maestro_worktree_base_dir_configured)
	maestro_ensure_worktree_base_dir "$base_dir" || return 1
	printf '%s\n' "$base_dir"
	return 0
}
...
maestro_canonical_worktree_path() {
	local repo_path="${1:-.}"
	local main_wt=""
...
maestro_repo_worktree_name() {
	local repo_path="${1:-.}"
	local canonical=""
	canonical=$(maestro_canonical_worktree_path "$repo_path")
	basename "$canonical"
	return 0
}

maestro_generate_worktree_path() {
	local repo_path="$1"
	local branch="$2"
	local repo_name slug base_dir
	repo_name=$(maestro_repo_worktree_name "$repo_path")
	slug=$(maestro_branch_slug "$branch")
	base_dir=$(maestro_worktree_base_dir) || return 1
	printf '%s/%s-%s\n' "$base_dir" "$repo_name" "$slug"
	return 0
}

maestro_migrate_repos_json_worktree_base_dir() {
	local repos_json="${1:-${MAESTRO_REPOS_JSON:-${HOME:-}/.config/maestro/repos.json}}"
