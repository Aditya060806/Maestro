...
_pulse_legacy_model_config_files() {
	local primary_file="${HOME}/.config/maestro/credentials.sh"
...
_pulse_file_has_active_legacy_model_var() {
	...
	case "$var_name" in
		PULSE_MODEL | MAESTRO_HEADLESS_MODELS) ;;
		*) return 1 ;;
	esac
...
# GH#17769: PULSE_MODEL and MAESTRO_HEADLESS_MODELS are deprecated.
...
_pulse_warn_if_legacy_model_var_from_credentials "MAESTRO_HEADLESS_MODELS" "${MAESTRO_HEADLESS_MODELS:-}"
...
PULSE_STAGE_TIMINGS_LOG="${PULSE_STAGE_TIMINGS_LOG:-${HOME}/.maestro/logs/pulse-stage-timings.log}"       # Structured TSV stage timing log (GH#20025)
PULSE_LOCK_MAX_AGE_S="${MAESTRO_PULSE_LOCK_MAX_AGE_S:-1800}"                                              # Force-reclaim mkdir lock after this age in seconds (GH#20025)
PULSE_MIN_INTERVAL_S="${MAESTRO_PULSE_MIN_INTERVAL_S:-90}"                                                # Entry-point rate-limit: skip cycle if last run was less than this many seconds ago (GH#20578)
...
PULSE_PREFETCH_CACHE_FILE="${PULSE_PREFETCH_CACHE_FILE:-${HOME}/.maestro/logs/pulse-prefetch-cache.json}" # Delta prefetch state cache (GH#15286)
PULSE_RATE_LIMIT_FLAG="${PULSE_RATE_LIMIT_FLAG:-${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag}"   # GH#18979: set by prefetch on detected GraphQL rate-limit exhaustion; checked by _preflight_prefetch_and_scope to abort cycle cleanly
...
if [[ -z "${MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD+x}" ]] && [[ -f "$_PULSE_RL_CONF" ]]; then
	# shellcheck disable=SC1090
	source "$_PULSE_RL_CONF"
fi
MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD="${MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD:-0.30}"              # t2690/t2768: canonical default in .agents/configs/pulse-rate-limit.conf. Set to 0 to disable.
export MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD
...
NO_WORK_WINDOW_SECS="${MAESTRO_NO_WORK_WINDOW_SECS:-${NO_WORK_WINDOW_SECS:-600}}"    # Rolling window in seconds (default 10 min)
NO_WORK_WINDOW_MAX="${MAESTRO_NO_WORK_WINDOW_MAX:-${NO_WORK_WINDOW_MAX:-10}}"         # Max no_work events in window before trip
...
PULSE_BATCH_PREFETCH_CACHE_DIR="${PULSE_BATCH_PREFETCH_CACHE_DIR:-${HOME}/.maestro/logs/batch-prefetch}"  # GH#19963: Directory for batch prefetch per-slug cache files
...
EVER_NMR_CACHE_FILE="${EVER_NMR_CACHE_FILE:-${HOME}/.maestro/.agent-workspace/supervisor/ever-nmr-cache.json}"
...
PIDFILE="${HOME}/.maestro/logs/pulse.pid"
LOCKDIR="${HOME}/.maestro/logs/pulse-wrapper.lockdir"
...
WRAPPER_LOGFILE="${HOME}/.maestro/logs/pulse-wrapper.log"
SESSION_FLAG="${HOME}/.maestro/logs/pulse-session.flag"
STOP_FLAG="${HOME}/.maestro/logs/pulse-session.stop"
...
# Previously defaulted to ~/Git/maestro, which caused 155+ orphaned sessions
...
PULSE_DIR="${PULSE_DIR:-${HOME}/.maestro/.agent-workspace}"
...
MODEL_AVAILABILITY_HELPER="${MODEL_AVAILABILITY_HELPER:-${SCRIPT_DIR}/model-availability-helper.sh}"
REPOS_JSON="${REPOS_JSON:-${HOME}/.config/maestro/repos.json}"
STATE_FILE="${HOME}/.maestro/logs/pulse-state.txt"
QUEUE_METRICS_FILE="${HOME}/.maestro/logs/pulse-queue-metrics"
SCOPE_FILE="${HOME}/.maestro/logs/pulse-scope-repos"
COMPLEXITY_SCAN_LAST_RUN="${HOME}/.maestro/logs/complexity-scan-last-run"
COMPLEXITY_SCAN_INTERVAL="${COMPLEXITY_SCAN_INTERVAL:-900}"                       # 15 min — runs each pulse cycle, per-run cap governs throughput
COMPLEXITY_SCAN_TREE_HASH_FILE="${HOME}/.maestro/logs/complexity-scan-tree-hash" # cached git tree hash for skip-if-unchanged
COMPLEXITY_LLM_SWEEP_LAST_RUN="${HOME}/.maestro/logs/complexity-llm-sweep-last-run"
COMPLEXITY_LLM_SWEEP_INTERVAL="${COMPLEXITY_LLM_SWEEP_INTERVAL:-21600}"   # 6h — daily LLM sweep when debt is stalled
COMPLEXITY_DEBT_COUNT_FILE="${HOME}/.maestro/logs/complexity-debt-count" # tracks open function-complexity-debt count for stall detection
DEDUP_CLEANUP_LAST_RUN="${HOME}/.maestro/logs/dedup-cleanup-last-run"
...
CODERABBIT_REVIEW_LAST_RUN="${HOME}/.maestro/logs/coderabbit-review-last-run"
...
POST_MERGE_SCANNER_LAST_RUN="${HOME}/.maestro/logs/post-merge-scanner-last-run"
...
AUTO_DECOMPOSER_PARENT_STATE="${HOME}/.maestro/logs/auto-decomposer-parent-state.json" # per-parent last-filed epochs (t2573)
...
PULSE_HEALTH_FILE="${HOME}/.maestro/logs/pulse-health.json"
FAST_FAIL_STATE_FILE="${HOME}/.maestro/.agent-workspace/supervisor/fast-fail-counter.json"
DEP_GRAPH_CACHE_FILE="${DEP_GRAPH_CACHE_FILE:-${HOME}/.maestro/.agent-workspace/supervisor/dep-graph-cache.json}" # Dependency graph cache (t1935)
...
PULSE_LOG_ARCHIVE_DIR="${PULSE_LOG_ARCHIVE_DIR:-${HOME}/.maestro/logs/pulse-archive}"
PULSE_CYCLE_INDEX_FILE="${PULSE_CYCLE_INDEX_FILE:-${HOME}/.maestro/logs/pulse-cycle-index.jsonl}"
