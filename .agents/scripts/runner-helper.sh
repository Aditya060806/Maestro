...
#
# Directory: ~/.maestro/.agent-workspace/runners/<name>/
...
#   - Override: MAESTRO_DISPATCH_BACKEND=claude|opencode
...
# Configuration
readonly RUNNERS_DIR="${MAESTRO_RUNNERS_DIR:-$HOME/.maestro/.agent-workspace/runners}"
readonly MEMORY_HELPER="$HOME/.maestro/agents/scripts/memory-helper.sh"
readonly MAIL_HELPER="$HOME/.maestro/agents/scripts/mail-helper.sh"
...
mailbox_before_run() {
	...
	MAESTRO_AGENT_ID="$name" "$MAIL_HELPER" register \
		--agent "$name" --role worker 2>/dev/null || true

	# Check for unread messages
	local unread_messages
	unread_messages=$(MAESTRO_AGENT_ID="$name" "$MAIL_HELPER" check --unread-only 2>/dev/null)
...
mailbox_after_run() {
	...
	# Send status report
	MAESTRO_AGENT_ID="$name" "$MAIL_HELPER" send \
		--to coordinator \
		--type status_report \
	...
	MAESTRO_AGENT_ID="$name" "$MAIL_HELPER" deregister --agent "$name" 2>/dev/null || true
...
# Sets MAESTRO_DISPATCH_BACKEND to a runtime ID from the registry.
#
# Priority (aligned with supervisor/dispatch.sh resolve_ai_cli):
#   1. MAESTRO_DISPATCH_BACKEND env var (explicit override)
...
detect_dispatch_backend() {
	if [[ -n "${MAESTRO_DISPATCH_BACKEND:-}" ]]; then
	...
	if [[ -n "${SUPERVISOR_CLI:-}" ]]; then
		...
		if command -v "$cli_binary" &>/dev/null; then
			MAESTRO_DISPATCH_BACKEND="$cli_binary"
			log_info "Using dispatch backend: $MAESTRO_DISPATCH_BACKEND (from SUPERVISOR_CLI)"
			return 0
		fi
	...
	if type rt_list_headless &>/dev/null; then
		...
		while IFS= read -r rt_id; do
			...
			if [[ -n "$bin" ]] && command -v "$bin" &>/dev/null; then
				MAESTRO_DISPATCH_BACKEND="$bin"
				log_info "Using dispatch backend: $MAESTRO_DISPATCH_BACKEND (runtime: $rt_id)"
				return 0
			fi
	...
	if command -v opencode &>/dev/null; then
		MAESTRO_DISPATCH_BACKEND="opencode"
	elif command -v claude &>/dev/null; then
		MAESTRO_DISPATCH_BACKEND="claude"
	else
	...
	log_info "Using dispatch backend: $MAESTRO_DISPATCH_BACKEND"
	return 0
}
...
_build_dispatch_cmd() {
	...
	if [[ "$MAESTRO_DISPATCH_BACKEND" == "opencode" ]]; then
	...
	elif [[ "$MAESTRO_DISPATCH_BACKEND" == "claude" ]]; then
		local claude_model
...
cmd_destroy() {
	...
	local ns_dir="${MAESTRO_MEMORY_DIR:-$HOME/.maestro/.agent-workspace/memory}/namespaces/$name"
	if [[ -d "$ns_dir" ]]; then
		rm -rf "$ns_dir"
...
DIRECTORY:
    Runners: ~/.maestro/.agent-workspace/runners/
...
BACKEND DETECTION:
    ...
    Override with: MAESTRO_DISPATCH_BACKEND=claude runner-helper.sh run ...
