...
# Arguments: $1 - maestro_path
# Output: threshold integer to stdout (defaults to 260 if config unreadable)
# Returns: 0 always
_check_ci_nesting_read_threshold() {
	local maestro_path="$1"
	local threshold=260
	local conf_file="${maestro_path}/.agents/configs/complexity-thresholds.conf"
	if [[ -f "$conf_file" ]]; then
		local val
...
_check_ci_nesting_count_violations() {
	...
	local violations=0
	local lint_files
	lint_files=$(git -C "$maestro_path" ls-files '*.sh' 2>/dev/null |
		grep -v 'node_modules\|vendor\|\.git' |
		sed "s|^|${maestro_path}/|" || true)
...
\`\`\`"
	...
	sig_footer=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer \
...
#
# Arguments:
#   $1 - maestro_path (repo root)
#   $2 - maestro_slug (owner/repo)
...
_check_ci_nesting_threshold_proximity() {
	...
	local _unused_maestro_path="${1:-}"
	local _unused_maestro_slug="${2:-}"
...
_complexity_scan_state_refresh() {
	...
	local pruned_count
	pruned_count=$(_simplification_state_prune "$maestro_path" "$state_file")
	...
	refreshed_count=$(_simplification_state_refresh "$maestro_path" "$state_file")
	...
	backfilled_count=$(_simplification_state_backfill_closed "$maestro_path" "$state_file" "$maestro_slug")
	...
	spurious_closed=$(_simplification_close_spurious_requeue_issues "$maestro_path" "$maestro_slug")
	...
	if [[ "$state_updated" == true ]]; then
		_simplification_state_push "$maestro_path"
	fi
	return 0
}
...
_complexity_scan_lang_shell() {
	local scan_helper="$1"
	local maestro_path="$2"
	local state_file="$3"
	local repos_json="$4"
	local maestro_slug="$5"
	...
	sh_scan_output=$("$scan_helper" scan "$maestro_path" --type sh --state-file "$state_file" 2>>"$LOGFILE") || true
	if [[ -n "$sh_scan_output" ]]; then
		...
		if [[ -n "$sh_results" ]]; then
			...
			_complexity_scan_create_issues "$sh_results" "$repos_json" "$maestro_slug"
		fi
	fi
	return 0
}
...
_complexity_scan_lang_md() {
	...
	md_scan_output=$("$scan_helper" scan "$maestro_path" --type md --state-file "$state_file" 2>>"$LOGFILE") || true
	if [[ -n "$md_scan_output" ]]; then
		...
		if [[ -n "$md_results" ]]; then
			...
			_complexity_scan_create_md_issues "$md_results" "$repos_json" "$maestro_slug"
		fi
	fi
	return 0
}
...
_complexity_scan_sweep_check() {
	...
	local sweep_result
	sweep_result=$("$scan_helper" sweep-check "$maestro_slug" 2>>"$LOGFILE") || sweep_result=""
	if [[ "$sweep_result" == needed* ]]; then
		...
		sweep_issue_exists=$(gh_issue_list --repo "$maestro_slug" \
			--label "function-complexity-debt" --state open \
		...
		if [[ "${sweep_issue_exists:-0}" -eq 0 ]]; then
			...
			gh_create_issue --repo "$maestro_slug" \
...
_complexity_scan_ratchet_check() {
	...
	local _unused_maestro_path="${2:-}"
	local _unused_maestro_slug="${3:-}"
	if [[ -n "${LOGFILE:-}" ]]; then
...
run_weekly_complexity_scan() {
	local repos_json="$REPOS_JSON"
	local maestro_slug="Aditya060806/Maestro"

	# t2145: complexity scan creates function-complexity-debt issues — maintainer-only.
	local _cs_role
	_cs_role=$(get_repo_role_by_slug "$maestro_slug")
	if [[ "$_cs_role" != "maintainer" ]]; then
		echo "[pulse-wrapper] Complexity scan skipped: role=$_cs_role for $maestro_slug (t2145)" >>"$LOGFILE"
		return 0
	fi
	...
	_complexity_scan_permission_gate "$maestro_slug" || return 0

	local maestro_path
	maestro_path=$(_complexity_scan_find_repo "$repos_json" "$maestro_slug" "$now_epoch") || return 0

	_complexity_scan_pull_latest "$maestro_path" || return 0

	# GH#15285: O(1) tree hash check — skip file iteration if no tracked files changed.
	local tree_changed=true
	if ! _complexity_scan_tree_changed "$maestro_path"; then
		tree_changed=false
	fi

	local maintainer
	maintainer=$(jq -r --arg slug "$maestro_slug" \
		'.initialized_repos[] | select(.slug == $slug) | .maintainer // empty' \
		"$repos_json" 2>/dev/null)
	[[ -z "$maintainer" ]] && maintainer=$(printf '%s' "$maestro_slug" | cut -d/ -f1)

	# LLM sweep + CI proximity guard run independently of tree change (GH#15285, GH#17808).
	if _complexity_llm_sweep_due "$now_epoch" "$maestro_slug"; then
		_complexity_run_llm_sweep "$maestro_slug" "$now_epoch" "$maintainer"
	fi
	_check_ci_nesting_threshold_proximity "$maestro_path" "$maestro_slug" "$maintainer" || true
	...
	gh label create "recheck-simplicity" --repo "$maestro_slug" --color "D4C5F9" \
		--description "File changed since last simplification and needs recheck" --force 2>/dev/null || true

	local state_file="${maestro_path}/.agents/configs/simplification-state.json"
	_complexity_scan_state_refresh "$maestro_path" "$state_file" "$maestro_slug"

	local scan_helper="${SCRIPT_DIR}/complexity-scan-helper.sh"
	if [[ -x "$scan_helper" ]]; then
		_complexity_scan_lang_shell "$scan_helper" "$maestro_path" "$state_file" "$repos_json" "$maestro_slug"
		_complexity_scan_lang_md "$scan_helper" "$maestro_path" "$state_file" "$repos_json" "$maestro_slug"
		_complexity_scan_sweep_check "$scan_helper" "$maestro_slug"
		_complexity_scan_ratchet_check "$scan_helper" "$maestro_path" "$maestro_slug"
	else
		...
		local sh_results
		sh_results=$(_complexity_scan_collect_violations "$maestro_path" "$now_epoch") || true
		if [[ -n "$sh_results" ]]; then
			sh_results=$(printf '%s' "$sh_results" | sort -t'|' -k2 -rn)
			_complexity_scan_create_issues "$sh_results" "$repos_json" "$maestro_slug"
		fi
		local md_results
		md_results=$(_complexity_scan_collect_md_violations "$maestro_path") || true
		if [[ -n "$md_results" ]]; then
			_complexity_scan_create_md_issues "$md_results" "$repos_json" "$maestro_slug"
		fi
	fi
