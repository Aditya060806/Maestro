...
#   - .maestro/agents/ symlink (read-only access to agent prompts)
...
readonly SANDBOX_BASE="${WORKER_SANDBOX_BASE:-/tmp/maestro-worker}"
...
setup_sandbox_git_config() {
	...
	git_name=$(git config --global user.name 2>/dev/null || echo "maestro-worker")
	git_email=$(git config --global user.email 2>/dev/null || echo "worker@maestro.sh")
...
setup_sandbox_gh_config() {
	...
	local agents_source="${REAL_HOME}/.maestro"
	if [[ -d "$agents_source" ]]; then
		ln -sf "$agents_source" "$sandbox_dir/.maestro"
	fi
...
setup_sandbox_dirs_and_sentinel() {
	...
	cat >"$sandbox_dir/.maestro-sandbox" <<-SENTINEL || return 1
		task_id=${task_id}
...
generate_sandbox_env() {
	...
	echo "export MAESTRO_SANDBOXED=true"
	printf "export MAESTRO_SANDBOX_DIR=%q\n" "${sandbox_dir}"
...
cleanup_worker_sandbox() {
	...
	if [[ ! -f "$sandbox_dir/.maestro-sandbox" ]]; then
	...
	done <"$sandbox_dir/.maestro-sandbox")
...
#######################################
is_sandboxed() {
	if [[ "${MAESTRO_SANDBOXED:-}" == "true" ]]; then
		if [[ -f "${HOME}/.maestro-sandbox" ]]; then
			grep '^task_id=' "${HOME}/.maestro-sandbox" 2>/dev/null | cut -d= -f2
			return 0
		fi
	fi
	return 1
...
main() {
	...
	help | --help | -h)
		...
		echo "  WORKER_SANDBOX_BASE      Base path for sandboxes (default: /tmp/maestro-worker)"
