...
#   NO-BLOCK    — README exists but no maestro:badges:start/end markers
...
#   Aditya060806, essentials-com, wpallstars
#   Override by creating ~/.config/maestro/badge-orgs.conf with one org per line.
...
_resolve_badges_helper() {
	local _deployed="$HOME/.maestro/agents/scripts/readme-badges-helper.sh"
...
# Default owned orgs — expanded via ~/.config/maestro/badge-orgs.conf if present.
...
_classify_badges_repo() {
	...
	if ! grep -qF '<!-- maestro:badges:start -->' "$_readme" 2>/dev/null; then
		if [[ "$_is_external" -eq 1 ]]; then
			printf 'EXTERNAL\tno badge block\n'
		else
			printf 'NO-BLOCK\trun: maestro badges sync --repo %s --apply\n' "$_slug"
		fi
		return 0
	fi
	...
	if [[ "$_check_rc" -eq 0 ]]; then
		...
		if _metrics_note=$(_repo_metrics_missing_note "$_path"); then
			printf 'METRICS-MISSING\trun: maestro badges sync --repo %s --apply (%s)\n' "$_slug" "$_metrics_note"
		else
			printf 'CURRENT\t\n'
		fi
	elif [[ "$_check_rc" -eq 3 ]]; then
		printf 'DRIFTED\trun: maestro badges sync --repo %s --apply\n' "$_slug"
	else
...
_process_repos() {
	...
	if ! _badges_helper=$(_resolve_badges_helper); then
		_die "readme-badges-helper.sh not found — run: maestro update"
	fi
