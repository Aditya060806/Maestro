...
cmd_pre_merge_gate() {
	...
	if [[ ! -f "$rbg_helper" ]]; then
		...
		rbg_helper="${HOME}/.maestro/agents/scripts/review-bot-gate-helper.sh"
	fi
...
_validators_should_run() {
	...
	if [[ "${MAESTRO_SKIP_PROJECT_VALIDATORS:-0}" == "1" ]]; then
		print_info "[validators] MAESTRO_SKIP_PROJECT_VALIDATORS=1, skipping"
		return 1
	fi
	local last_msg
...
_detect_node_project() {
	...
	if ! command -v "$pm" >/dev/null 2>&1; then
		print_warning "[validators] $pm not available on PATH — skipping (set MAESTRO_SKIP_PROJECT_VALIDATORS=1 to silence)"
		return 1
	fi
	return 0
}
...
_run_node_typecheck() {
	...
	print_error "               (or MAESTRO_SKIP_PROJECT_VALIDATORS=1 env)"
	return 1
}
...
_run_project_validators() {
	...
	if ! _detect_node_project; then
		# Silent skip when no project detected (non-node project = most maestro paths).
		return 0
	fi
	print_info "[validators] running node project validators ($pm)..."
	local validator_timeout
	validator_timeout="${MAESTRO_VALIDATOR_TIMEOUT:-300}"
...
#
# Behaviour:
#   MAESTRO_SHALLOW_UNSHALLOW=0  → warn and return 1 (operator must fix)
#   MAESTRO_SHALLOW_UNSHALLOW=1  → attempt git fetch --unshallow and return 0
...
_check_and_handle_shallow_clone() {
	...
	local opt="${MAESTRO_SHALLOW_UNSHALLOW:-1}"
	if [[ "$opt" == "0" ]]; then
	...
	print_warning "Shallow clone detected — running git fetch --unshallow origin (set MAESTRO_SHALLOW_UNSHALLOW=0 to disable)..."
...
_create_pr() {
	...
	pr_error=$(mktemp -t maestro-pr-create-error.XXXXXX) || return 1
