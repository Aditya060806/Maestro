...
readonly DEFAULT_LOGFILE="${HOME}/.maestro/logs/pulse.log"
readonly DEFAULT_LOGDIR="${HOME}/.maestro/logs"
readonly DEFAULT_METRICS_FILE="${HOME}/.maestro/logs/headless-runtime-metrics.jsonl"
readonly DEFAULT_STATS_FILE="${HOME}/.maestro/logs/pulse-stats.json"
readonly DEFAULT_GH_API_LOG="${HOME}/.maestro/logs/gh-api-calls.log"
readonly DEFAULT_SYSTEMD_TIMER_FILE="${HOME}/.config/systemd/user/maestro-supervisor-pulse.timer"
...
pm-wb-disabled|pulse-merge.sh|1514|worker-briefed auto-merge: disabled by|Worker-briefed auto-merge disabled via MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0
...
pmc-handover|pulse-merge-conflict.sh|308|handover: PR #.*handed over to worker pipeline|Interactive PR handed over to worker pipeline (idle >MAESTRO_IDLE_INTERACTIVE_HANDOVER_SECONDS, default 4h)
...
_api_budget_cache_dir_state() {
	...
	if [[ -n "${MAESTRO_GH_PR_VIEW_CACHE_DIR:-}" ]]; then
		shared="yes"
		reason="env_configured"
		if [[ -d "${MAESTRO_GH_PR_VIEW_CACHE_DIR}" ]]; then
			present="yes"
		else
			present="no"
		fi
	elif [[ -n "${HOME:-}" && -d "${HOME:-}/.maestro/cache/gh-pr-view-snapshots" ]]; then
		present="yes"
		reason="using_default_exact_cache_dir"
	fi
	printf 'shared=%s present=%s reason=%s recommendation=%s' \
		"$shared" "$present" "$reason" "run_during_pulse_or_export_MAESTRO_GH_PR_VIEW_CACHE_DIR_for_shared_repo_pr_cache_evidence"
	return 0
}

_api_budget_cooldown_file() {
	printf '%s' "${PULSE_DIAGNOSE_GH_SECONDARY_COOLDOWN_FILE:-${MAESTRO_GH_SECONDARY_COOLDOWN_FILE:-${HOME}/.maestro/cache/gh-secondary-cooldown.json}}"
	return 0
}

_api_budget_cooldown_events_file() {
	printf '%s' "${PULSE_DIAGNOSE_GH_SECONDARY_COOLDOWN_EVENTS_FILE:-${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE:-${HOME}/.maestro/cache/gh-cooldown-events.jsonl}}"
	return 0
}
...
_api_budget_cache_key_counts_csv() {
	local exact_dir="${MAESTRO_GH_PR_VIEW_CACHE_DIR:-${HOME}/.maestro/cache/gh-pr-view-snapshots}"
	local shared="no"
	[[ -n "${MAESTRO_GH_PR_VIEW_CACHE_DIR:-}" ]] && shared="yes"
	local exact_keys=0
	local rest_keys=0
	local cache_file=""
	if [[ -d "$exact_dir" ]]; then
