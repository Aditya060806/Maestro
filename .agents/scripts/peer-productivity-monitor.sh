...
#
# Architecture:
#   - Runs every 30 min via launchd (sh.maestro.peer-productivity-monitor.plist)
...
#
# Env:
#   MAESTRO_PEER_MONITOR_DISABLE=1   — short-circuit, do nothing
#   MAESTRO_PEER_MONITOR_DRY_RUN=1   — observe + log, don't write config
#   MAESTRO_PEER_MONITOR_WINDOW_H=24 — rolling window in hours (default 24)
#   MAESTRO_PEER_MONITOR_HYSTERESIS=3 — vote count for flip (default 3)
...
# Config paths
OVERRIDE_CONF="${HOME}/.config/maestro/dispatch-override.conf"
STATE_DIR="${HOME}/.maestro/state"
STATE_FILE="${STATE_DIR}/peer-productivity-state.json"
LOG_FILE="${HOME}/.maestro/logs/peer-productivity.log"
...
# Defaults
WINDOW_HOURS="${MAESTRO_PEER_MONITOR_WINDOW_H:-24}"
HYSTERESIS="${MAESTRO_PEER_MONITOR_HYSTERESIS:-3}"
DRY_RUN="${MAESTRO_PEER_MONITOR_DRY_RUN:-0}"
...
cmd_observe() {
	if [[ "${MAESTRO_PEER_MONITOR_DISABLE:-0}" == "1" ]]; then
		log_msg INFO "MAESTRO_PEER_MONITOR_DISABLE=1 — skipping cycle"
		return 0
	fi
...
cmd_dry_run() {
	MAESTRO_PEER_MONITOR_DRY_RUN=1 DRY_RUN=1 cmd_observe
	return 0
}
