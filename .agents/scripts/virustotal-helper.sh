...
[[ -f "${HOME}/.config/maestro/credentials.sh" ]] && source "${HOME}/.config/maestro/credentials.sh"
...
resolve_api_key() {
	...
	if command -v gopass &>/dev/null; then
		...
		key=$(gopass show -o "maestro/VIRUSTOTAL_ADITYA060806" 2>/dev/null || true)
		if [[ -z "$key" ]]; then
			key=$(gopass show -o "maestro/VIRUSTOTAL_API_KEY" 2>/dev/null || true)
		fi
		if [[ -n "$key" ]]; then
			echo "$key"
			return 0
	...
	# Try maestro secret helper
	if [[ -x "${_vt_script_dir}/secret-helper.sh" ]]; then
		local key=""
		key=$("${_vt_script_dir}/secret-helper.sh" get VIRUSTOTAL_ADITYA060806 2>/dev/null || true)
		if [[ -z "$key" ]]; then
...
vt_request() {
	...
	api_key=$(resolve_api_key) || {
		...
		log_info "Set with: maestro secret set VIRUSTOTAL_ADITYA060806"
...
cmd_status() {
	...
	if [[ -n "$api_key" ]]; then
		...
		echo ""
		echo "  Set with:"
		echo "    maestro secret set VIRUSTOTAL_ADITYA060806"
		echo "  Or:"
		echo "    export VIRUSTOTAL_API_KEY=<key> in ~/.config/maestro/credentials.sh"
	fi
