...
#
# Bypass:
#   MAESTRO_SKIP_TIER_VALIDATOR=1 — exit 0 immediately (with log)
...
_check_estimate() {
	...
	# Strip the signature footer block (anything after <!-- maestro:sig -->)
...
cmd_check() {
	...
	if [[ "${MAESTRO_SKIP_TIER_VALIDATOR:-0}" == "1" ]]; then
		_log "INFO" "MAESTRO_SKIP_TIER_VALIDATOR=1 — bypassing check for #${issue_num}"
		return 0
	fi
...
Bypass:
  MAESTRO_SKIP_TIER_VALIDATOR=1 to exit 0 immediately without checking.
