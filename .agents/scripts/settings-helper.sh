...
# settings-helper.sh - Canonical settings file manager for maestro
#
# Manages ~/.config/maestro/settings.json as the single source of truth
# for all maestro configuration. All settings configurable via /onboarding
...
#   settings-helper.sh export-env        Export settings as MAESTRO_* env vars
#   settings-helper.sh help              Show this help
#
# Settings are read with this precedence (highest wins):
#   1. Environment variable (MAESTRO_*)
...
readonly SETTINGS_DIR="$HOME/.config/maestro"
...
_generate_defaults() {
	cat <<'DEFAULTS'
{
  "$schema": "https://maestro.sh/schemas/settings.json",
  "$comment": "maestro settings — edit directly or via /onboarding. Docs: ~/.maestro/agents/reference/settings.md",
  "auto_update": {
    "enabled": true,
    "interval_minutes": 10,
...
# =============================================================================
# Maps dot-notation settings keys to their corresponding MAESTRO_* env vars.
# This is the bridge between the old env-var-based config and the new file-based config.

_env_var_for_key() {
	local key="$1"
	case "$key" in
	auto_update.enabled) echo "MAESTRO_AUTO_UPDATE" ;;
	auto_update.interval_minutes) echo "MAESTRO_UPDATE_INTERVAL" ;;
	auto_update.skill_auto_update) echo "MAESTRO_SKILL_AUTO_UPDATE" ;;
	auto_update.skill_freshness_hours) echo "MAESTRO_SKILL_FRESHNESS_HOURS" ;;
	auto_update.tool_auto_update) echo "MAESTRO_TOOL_AUTO_UPDATE" ;;
	auto_update.tool_freshness_hours) echo "MAESTRO_TOOL_FRESHNESS_HOURS" ;;
	auto_update.tool_idle_hours) echo "MAESTRO_TOOL_IDLE_HOURS" ;;
	auto_update.openclaw_auto_update) echo "MAESTRO_OPENCLAW_AUTO_UPDATE" ;;
	auto_update.openclaw_freshness_hours) echo "MAESTRO_OPENCLAW_FRESHNESS_HOURS" ;;
	supervisor.pulse_enabled) echo "MAESTRO_SUPERVISOR_PULSE" ;;
	supervisor.peak_hours_enabled) echo "MAESTRO_PEAK_HOURS_ENABLED" ;;
	supervisor.peak_hours_start) echo "MAESTRO_PEAK_HOURS_START" ;;
	supervisor.peak_hours_end) echo "MAESTRO_PEAK_HOURS_END" ;;
	supervisor.peak_hours_tz) echo "MAESTRO_PEAK_HOURS_TZ" ;;
	supervisor.peak_hours_worker_fraction) echo "MAESTRO_PEAK_HOURS_WORKER_FRACTION" ;;
	repo_sync.enabled) echo "MAESTRO_REPO_SYNC" ;;
	*) echo "" ;;
	esac
	return 0
}
...
cmd_list() {
	...
	echo ""
	echo -e "${BLUE}maestro Settings${NC}"
	echo "=================="
	echo ""
	echo -e "${DIM:-}File: $SETTINGS_FILE${NC}"
	echo -e "${DIM:-}Docs: ~/.maestro/agents/reference/settings.md${NC}"
	echo ""
...
cmd_validate() {
	...
	if [[ "$has_supervisor_interval" == "yes" && "$has_orchestration_interval" == "no" ]]; then
		...
		print_info "  Run: maestro update  (auto-migrates on next update)"
...
cmd_export_env() {
	# Export settings as MAESTRO_* environment variables
...
PRECEDENCE:
    ...
    Environment variables (MAESTRO_*) always override file settings.
...
DOCS:
    ~/.maestro/agents/reference/settings.md
