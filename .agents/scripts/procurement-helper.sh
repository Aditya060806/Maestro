...
get_api_key() {
	...
	stripe)
		...
		if command -v gopass &>/dev/null; then
			gopass show -o "maestro/STRIPE_ISSUING_SECRET_KEY" 2>/dev/null && return 0
		fi
		if [[ -n "${STRIPE_ISSUING_SECRET_KEY:-}" ]]; then
			echo "${STRIPE_ISSUING_SECRET_KEY}"
			return 0
		fi
		print_error "STRIPE_ISSUING_SECRET_KEY not found. Run: maestro secret set STRIPE_ISSUING_SECRET_KEY"
		return 1
		;;
	revolut)
		if command -v gopass &>/dev/null; then
			gopass show -o "maestro/REVOLUT_API_TOKEN" 2>/dev/null && return 0
		fi
		if [[ -n "${REVOLUT_API_TOKEN:-}" ]]; then
			echo "${REVOLUT_API_TOKEN}"
			return 0
		fi
		print_error "REVOLUT_API_TOKEN not found. Run: maestro secret set REVOLUT_API_TOKEN"
		return 1
		;;
	*)
...
get_mission_dir() {
	...
	elif [[ -d "${HOME}/.maestro/missions/${mission_id}" ]]; then
		echo "${HOME}/.maestro/missions/${mission_id}"
	else
