...
# in _inbox/_drop/ and _inbox/_needs-review/ older than MAESTRO_INBOX_DIGEST_AGE_DAYS
# (default: 7), and writes/removes advisories in ~/.maestro/advisories/.
#
# Advisory filename: inbox-stale-{slug-sanitized}.advisory (stable; overwritten)
# Advisory ID used by: maestro security dismiss inbox-stale-{slug-sanitized}
...
#
# Environment:
#   MAESTRO_INBOX_DIGEST_AGE_DAYS         Age threshold in days (default: 7)
#   MAESTRO_INBOX_DIGEST_INTERVAL_HOURS   Re-run guard interval (default: 168)
# =============================================================================
...
readonly DIGEST_STAMP_FILE="${TMPDIR:-/tmp}/maestro-inbox-digest.last"

INBOX_DIGEST_AGE_DAYS="${MAESTRO_INBOX_DIGEST_AGE_DAYS:-7}"
INBOX_DIGEST_INTERVAL_HOURS="${MAESTRO_INBOX_DIGEST_INTERVAL_HOURS:-168}"
...
_process_workspace_inbox() {
	...
	# Pass workspace parent as --repo: ~/.maestro/.agent-workspace has no _inbox/
