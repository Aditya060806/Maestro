...
#   - LOGFILE, REPOS_JSON
# Globals written:
#   - none (caches written to disk under ~/.maestro/logs/)
...
#   3. Persistent disk cache with 24h TTL (~/.maestro/logs/)
...
_get_runner_role() {
	...
	local disk_cache_dir="${HOME}/.maestro/logs"
	...
	# #maestro:trust-boundary — runner role classification uses confirmed
...
_dashboard_identity_alias_config_path() {
	local configured_path="${MAESTRO_IDENTITY_ALIASES_CONF:-}"
	...
	local deployed_config="${HOME}/.maestro/agents/configs/identity-aliases.conf"
