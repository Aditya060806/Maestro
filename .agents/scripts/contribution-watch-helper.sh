...
# exclusion to suppress maestro automation noise.
#
...
#                                                         Drafts stored in ~/.maestro/.agent-workspace/draft-responses/
...
# Launchd label: sh.maestro.contribution-watch
...
STATE_FILE="${HOME}/.maestro/cache/contribution-watch.json"
REPOS_JSON="${REPOS_JSON:-${HOME}/.config/maestro/repos.json}"
LOGFILE="${HOME}/.maestro/logs/contribution-watch.log"
PID_FILE="${HOME}/.maestro/.pid/contribution-watch.pid"
PLIST_LABEL="sh.maestro.contribution-watch"
...
_scan_process_notifications() {
	...
	while IFS= read -r row; do
		...
		# detect self activity by maestro signature footer, not only by username.
		if [[ "$reason" == "comment" || "$reason" == "mention" ]]; then
			local number latest_meta latest_author latest_time latest_is_maestro
			number="${item_key##*#}"
			latest_author=""
			latest_time=""
			latest_is_maestro="false"
			...
			if [[ "$number" =~ ^[0-9]+$ ]]; then
				...
				if [[ -n "$latest_meta" && "$latest_meta" != "null" ]]; then
					...
					latest_is_maestro=$(echo "$latest_meta" | jq -r '.is_maestro // false')
				fi
			fi
			...
			if [[ "$latest_author" == "$username" || "$latest_is_maestro" == "true" ]]; then
				_SCAN_STATE=$(echo "$_SCAN_STATE" | jq \
					--arg key "$item_key" \
					--arg type "$item_type" \
					--arg title "$title" \
...
_scan_backfill_fetch_latest_comment() {
	...
	jq -s 'add | sort_by(.created) | reverse | .[0] | {
		author: (.author // ""),
		created: (.created // ""),
		is_maestro: ((.body // "") | test("maestro\\.sh"))
	}' \
...
_scan_backfill_process_item() {
	...
	local latest_comment_author latest_comment_time latest_comment_is_maestro
	latest_comment_author=$(echo "$comments_meta" | jq -r '.author // ""')
	latest_comment_time=$(echo "$comments_meta" | jq -r '.created // ""')
	latest_comment_is_maestro=$(echo "$comments_meta" | jq -r '.is_maestro // false')
	...
	if [[ "$latest_comment_author" == "$username" || "$latest_comment_is_maestro" == "true" ]]; then
...
_scan_print_results() {
	...
	if [[ "$needs_attention" -gt 0 ]]; then
		...
		# 	osascript -e "display notification \"${needs_attention} contribution(s) need reply\" with title \"maestro\"" 2>/dev/null || true
		# fi
	else
...
cmd_scan() {
	...
	if [[ -z "$last_scan" ]]; then
		echo -e "${YELLOW}[contribution-watch] skipped — no previous scan found; run maestro contribution-watch seed to enable${NC}"
...
cmd_help() {
	...
	echo "                                                             Drafts stored in ~/.maestro/.agent-workspace/draft-responses/"
