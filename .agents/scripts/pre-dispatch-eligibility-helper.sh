...
#   MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY=1 — bypass entirely (emergency escape hatch)
#   MAESTRO_PREDISPATCH_RECENT_MERGE_WINDOW=<seconds> — recent-merge detection window (default 300)
#   MAESTRO_PREDISPATCH_RECENT_COMMIT_WINDOW=<seconds> — recent-commit detection window (default 600)
#
# Counter output:
#   Pre-dispatch aborts are tracked in ~/.maestro/logs/pulse-stats.json
#   via pulse-stats-helper.sh (sourced below). The 24h count is surfaced
#   by `maestro status` to make churn visible to operators.
#   Counters:
...
_check_recent_commit_closes_issue() {
	...
	local commit_window="${MAESTRO_PREDISPATCH_RECENT_COMMIT_WINDOW:-600}"
	local now_epoch
...
is_issue_eligible_for_dispatch() {
	...
	if [[ "${MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY:-0}" == "1" ]]; then
		echo "[dispatch-precheck] MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY=1 — bypassing eligibility check for #${issue_number}" >>"$LOGFILE"
		return 0
	fi
	...
	local merge_window="${MAESTRO_PREDISPATCH_RECENT_MERGE_WINDOW:-300}"
	local now_epoch
	now_epoch=$(date +%s 2>/dev/null) || now_epoch=0
...
_main() {
	...
	case "$cmd" in
		...
		help | --help | -h)
			...
			echo ""
			echo "Environment:"
			echo "  MAESTRO_SKIP_PREDISPATCH_ELIGIBILITY=1    bypass (emergency)"
			echo "  MAESTRO_PREDISPATCH_RECENT_MERGE_WINDOW   merge window seconds (default 300)"
			echo "  MAESTRO_PREDISPATCH_RECENT_COMMIT_WINDOW  commit window seconds (default 600)"
