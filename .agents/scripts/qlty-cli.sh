...
load_api_config() {
	local org="${1:-Aditya060806}" # Default to Aditya060806 organization
	...
	if [[ -n "$account_api_key" ]]; then
		...
		print_info "Add to ~/.config/maestro/credentials.sh:"
...
show_help() {
	...
	echo "  $0 install"
	echo "  $0 init"
	echo "  $0 check 5           # Check sample of 5 issues (default: Aditya060806 org)"
	echo "  $0 check 5 myorg     # Check sample of 5 issues for 'myorg' organization"
	echo "  $0 fmt --all         # Format entire codebase (default: Aditya060806 org)"
	...
	echo "  Add to ~/.config/maestro/credentials.sh:"
