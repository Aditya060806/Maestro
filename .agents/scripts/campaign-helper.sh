...
_require_campaigns_plane() {
	...
	if [[ ! -d "$campaigns_dir" ]]; then
		...
		print_error "Run 'maestro campaign init' first (requires P1 to be deployed)."
		return 1
	fi
	return 0
}
...
_require_launched_campaign() {
	...
	if [[ ! -d "$launched_dir" ]]; then
		...
		print_error "Run: maestro campaign launch ${campaign_id}"
		return 1
	fi
	echo "$launched_dir"
	return 0
...
_Next: fill in sections above, then run: \`maestro campaign status ${campaign_id}\`_
_To launch: \`maestro campaign launch ${campaign_id}\`_
BRIEF
	return 0
}
...
cmd_new() {
	...
	echo "  2. Status:       maestro campaign status ${dir_name}"
	echo "  3. Launch:       maestro campaign launch ${dir_name}"
	return 0
}
...
_Promote with: \`maestro campaign promote ${campaign_id} --results\`_
RESULTS
	return 0
}
...
_Promote with: \`maestro campaign promote ${campaign_id} --learnings\`_
LEARNINGS
	return 0
}
...
cmd_launch() {
	...
	echo "  2. Run: maestro campaign promote ${campaign_id} --results"
	echo "  3. Fill in ${CAMPAIGNS_LEARNINGS_FILE} with retrospective insights"
	echo "  4. Run: maestro campaign promote ${campaign_id} --learnings"
	return 0
}
...
cmd_feedback() {
	...
	if [[ -z "$campaign_id" ]]; then
		...
		echo ""
		echo "Import into a campaign: maestro campaign feedback <id>"
		return 0
	fi
...
_Draft generated by maestro campaign draft on ${draft_date}._
...
_print_draft_summary() {
	...
	echo "  3. Variant: maestro campaign draft ${campaign_id} --channel ${channel} --variant $((variant + 1))"
...
Prerequisites:
  _campaigns/ plane (P1 — maestro t2962 #21250)
  ANTHROPIC_API_KEY for draft command (gopass or env var)
HELP
	return 0
}
