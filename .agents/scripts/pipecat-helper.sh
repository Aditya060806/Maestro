...
readonly PIPECAT_DIR="${HOME}/.maestro/.agent-workspace/work/pipecat-voice-agent"
readonly PIPECAT_VENV="${PIPECAT_DIR}/.venv"
readonly PIPECAT_BOT="${PIPECAT_DIR}/bot.py"
readonly PIPECAT_PID_FILE="${HOME}/.maestro/.agent-workspace/tmp/.pipecat-agent.pid"
readonly PIPECAT_LOG_FILE="${HOME}/.maestro/logs/pipecat-agent.log"
readonly CLIENT_DIR="${PIPECAT_DIR}/client"
readonly CLIENT_PID_FILE="${HOME}/.maestro/.agent-workspace/tmp/.pipecat-client.pid"
...
load_api_key() {
	...
	if command -v gopass &>/dev/null; then
		local key
		key=$(gopass show -o "maestro/${key_name}" 2>/dev/null) || true
		if [[ -n "${key:-}" ]]; then
			export "${key_name}=${key}"
			return 0
		fi
	...
	local cred_file="${HOME}/.config/maestro/credentials.sh"
	if [[ -f "${cred_file}" ]]; then
		local key
	...
	local tenant_file="${HOME}/.config/maestro/tenants/default/credentials.sh"
...
Store keys via: maestro secret set <KEY_NAME>
...
SYSTEM_PROMPT = (
    "You are a Maestro assistant in a real-time voice conversation. "
...
def get_llm_service(provider: str):
    ...
    if provider == "anthropic":
        ...
        if not api_key:
            print("ERROR: ANTHROPIC_API_KEY not set. Run: maestro secret set ANTHROPIC_API_KEY")
            sys.exit(1)
        ...
        if not api_key:
            print("ERROR: OPENAI_API_KEY not set. Run: maestro secret set OPENAI_API_KEY")
            sys.exit(1)
...
async def run_bot(
    ...
    if not soniox_key:
        print("ERROR: SONIOX_API_KEY not set. Run: maestro secret set SONIOX_API_KEY")
        return
    ...
    if not cartesia_key:
        print("ERROR: CARTESIA_API_KEY not set. Run: maestro secret set CARTESIA_API_KEY")
        return
...
# Store actual keys via: maestro secret set <KEY_NAME>
...
cmd_setup() {
	...
	echo ""
	echo "Next steps:"
	echo "  1. Store API keys:  maestro secret set SONIOX_API_KEY"
	echo "                      maestro secret set CARTESIA_API_KEY"
	echo "                      maestro secret set ANTHROPIC_API_KEY"
...
export const metadata: Metadata = {
  ...
  description: "Maestro voice assistant powered by Pipecat",
};
...
_start_check_api_keys() {
	...
	if [[ ${_key_check} -ne 0 ]]; then
		...
		print_info "Set it: maestro secret set SONIOX_API_KEY"
	...
	if [[ ${_key_check} -ne 0 ]]; then
		...
		print_info "Set it: maestro secret set CARTESIA_API_KEY"
		eval "${keys_ok_ref}=false"
	fi
	...
	anthropic)
		...
		if [[ ${_key_check} -ne 0 ]]; then
			...
			print_info "Set it: maestro secret set ANTHROPIC_API_KEY"
			eval "${keys_ok_ref}=false"
		fi
		;;
	openai)
		...
		if [[ ${_key_check} -ne 0 ]]; then
			...
			print_info "Set it: maestro secret set OPENAI_API_KEY"
			eval "${keys_ok_ref}=false"
		fi
		;;
	local)
...
Setup:
  ...
  2. maestro secret set SONIOX_API_KEY   # Store API keys
     maestro secret set CARTESIA_API_KEY
     maestro secret set ANTHROPIC_API_KEY
...
Directories:
  Agent:  ~/.maestro/.agent-workspace/work/pipecat-voice-agent/
  Client: ~/.maestro/.agent-workspace/work/pipecat-voice-agent/client/
  Logs:   ~/.maestro/logs/pipecat-agent.log
