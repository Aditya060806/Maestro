...
#
# Thread storage: SQLite at ~/.maestro/.agent-workspace/mail/mission-email.db
# Templates: ~/.maestro/agents/templates/email/
#
# Integrates with:
...
# Configuration
readonly EMAIL_DB_DIR="${MAESTRO_MAIL_DIR:-$HOME/.maestro/.agent-workspace/mail}"
...
if [[ -d "${SCRIPT_DIR}/../templates/email" ]]; then
	...
	readonly TEMPLATE_DIR="$HOME/.maestro/agents/templates/email"
fi
...
build_raw_email() {
	...
	local message_id
	message_id="<$(generate_id "mid")@maestro.sh>"
	local date_header
	...
	raw+="X-Mailer: maestro-mission-email/1.0"$'\n'
...
Templates:
  ...
  Store templates in: ~/.maestro/agents/templates/email/
