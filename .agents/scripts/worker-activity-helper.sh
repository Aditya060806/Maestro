...
WAH_METRICS_FILE="${WAH_METRICS_FILE:-${HOME}/.maestro/logs/headless-runtime-metrics.jsonl}"
WAH_PULSE_STATS_FILE="${WAH_PULSE_STATS_FILE:-${HOME}/.maestro/logs/pulse-stats.json}"
WAH_PR_CACHE_FILE="${WAH_PR_CACHE_FILE:-${HOME}/.maestro/cache/worker-activity-prs.json}"
WAH_OAUTH_POOL_FILE="${WAH_OAUTH_POOL_FILE:-${HOME}/.maestro/oauth-pool.json}"
...
Sources read (in canonical-precedence order):
  1. ~/.maestro/logs/headless-runtime-metrics.jsonl  (worker outcomes)
  2. ~/.maestro/logs/pulse-stats.json                (dispatch counters)
...
Examples:
  ...
  worker-activity-helper.sh summary --since 7d --repo Aditya060806/Maestro --no-pr-check
...
NOT a substitute for:
  ...
  - recursive grep over ~/.maestro/logs or OpenCode storage → unbounded noise.
