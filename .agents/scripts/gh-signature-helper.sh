...
# by maestro agents. Format:
#
#   ---
#   [OpenCode CLI](https://opencode.ai) v1.3.3, [maestro.sh](https://maestro.sh) v3.5.6, anthropic/opus-4-6, 1,234 tokens
#
# Usage:
...
#   MAESTRO_SIG_CLI          CLI name (e.g., "OpenCode CLI")
#   MAESTRO_SIG_CLI_VERSION  CLI version (e.g., "1.3.3")
#   MAESTRO_SIG_MODEL        Model ID (e.g., "anthropic/opus-4-6")
#   MAESTRO_SIG_TOKENS       Token count (e.g., "1234")
#
# Runtime-aware: OpenCode DB queries gated behind _is_opencode_runtime() (GH#17689).
# Dependencies: lib/version.sh (maestro version), maestro-update-check.sh (CLI detection)
#
...
_parse_generate_args() {
	local model="${MAESTRO_SIG_MODEL:-}"
	local tokens="${MAESTRO_SIG_TOKENS:-}"
	local cli_name="${MAESTRO_SIG_CLI:-}"
	local cli_version="${MAESTRO_SIG_CLI_VERSION:-}"
...
_build_signature() {
	...
	maestro_version=$(maestro_find_version)
	...
	# Target: [maestro.sh](...) v3.5.10 in [CLI](...) v1.3.3 with claude-opus-4-6 used N tokens for Xm, Zm since this issue was created.
	local sig="[maestro.sh](https://maestro.sh) v${maestro_version}"
...
cmd_footer() {
	...
	# Dedup: if the body already contains an maestro signature, skip
	if [[ -n "$body_to_check" ]] && [[ "$body_to_check" == *"maestro.sh"* ]]; then
		return 0
	fi
...
Environment variables (override auto-detection):
  MAESTRO_SIG_CLI          CLI name
  MAESTRO_SIG_CLI_VERSION  CLI version
  MAESTRO_SIG_MODEL        Model ID
  MAESTRO_SIG_TOKENS       Token count
