...
get_api_key() {
    ...
    if [[ -z "${BING_WEBMASTER_API_KEY:-}" ]]; then
        ...
        print_error "Set it in ~/.config/maestro/credentials.sh"
        return 1
    fi
...
Output:
    ~/.maestro/.agent-workspace/work/seo-data/{domain}/bing-{start}-{end}.toon

Requirements:
    - BING_WEBMASTER_API_KEY set in ~/.config/maestro/credentials.sh
