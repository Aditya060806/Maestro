...
# startup. Deploying updated scripts to ~/.maestro/agents/scripts/ does NOT
...
#                           Used by setup.sh and maestro update.
#
# Env:
#   MAESTRO_SKIP_PULSE_RESTART=1     Skip restart operations (for debug).
#   MAESTRO_PULSE_RESTART_WAIT=3     Seconds between stop and start (default 3).
#   MAESTRO_PULSE_SIGTERM_WAIT=2     Seconds before escalating to SIGKILL.
#   MAESTRO_PULSE_EXPECTED_MAX_INSTANCES=3
...
#      — count exceeds MAESTRO_PULSE_EXPECTED_MAX_INSTANCES (default 3)
#
...
# Paths
_PULSE_AGENTS_DIR="${MAESTRO_AGENTS_DIR:-${HOME}/.maestro/agents}"
_PULSE_SCRIPT="${_PULSE_AGENTS_DIR}/scripts/pulse-wrapper.sh"
_PULSE_LOG="${HOME}/.maestro/logs/pulse-wrapper.log"
...
_PULSE_PATTERN="${MAESTRO_PULSE_PROCESS_PATTERN:-(^|/)pulse-wrapper\\.sh( |\$)}"

# Timing
_PULSE_RESTART_WAIT="${MAESTRO_PULSE_RESTART_WAIT:-3}"
_PULSE_SIGTERM_WAIT="${MAESTRO_PULSE_SIGTERM_WAIT:-2}"
...
#
# Override: MAESTRO_PULSE_EXPECTED_MAX_INSTANCES=<integer>. Setting to 1
...
_PULSE_EXPECTED_MAX_INSTANCES="${MAESTRO_PULSE_EXPECTED_MAX_INSTANCES:-3}"
...
_start() {
	...
	# launchd respawn, maestro update, setup.sh ensure-running).

	# GH#20580: set MAESTRO_PULSE_SOURCE so pulse-wrapper.sh records this
	# invocation as "lifecycle-helper" in its invocation_sources counter.
	MAESTRO_PULSE_SOURCE=lifecycle-helper nohup "$_PULSE_SCRIPT" >>"$_PULSE_LOG" 2>&1 &
...
# _restart: force stop + start. Honours MAESTRO_SKIP_PULSE_RESTART env opt-out.
_restart() {
	if [[ "${MAESTRO_SKIP_PULSE_RESTART:-0}" == "1" ]]; then
		_pl_info "MAESTRO_SKIP_PULSE_RESTART=1 — skipping pulse restart"
		return 0
	fi
...
_restart_if_running() {
	if [[ "${MAESTRO_SKIP_PULSE_RESTART:-0}" == "1" ]]; then
		_pl_info "MAESTRO_SKIP_PULSE_RESTART=1 — skipping pulse restart-if-running"
		return 0
	fi
...
# when the alive-PID count exceeds MAESTRO_PULSE_EXPECTED_MAX_INSTANCES
...
_status() {
	...
	# MAESTRO_PULSE_LOCK_DIR overrides the default path — useful for tests
	# that must isolate from the real user lockdir (GH#21581).
	local _lockdir="${MAESTRO_PULSE_LOCK_DIR:-${HOME}/.maestro/logs/pulse-wrapper.lockdir}"
	local _lock_pid=""
...
Env:
  MAESTRO_SKIP_PULSE_RESTART=1            Skip restart operations.
  MAESTRO_PULSE_RESTART_WAIT=3            Seconds between stop and start.
  MAESTRO_PULSE_SIGTERM_WAIT=2            Seconds before escalating to SIGKILL.
  MAESTRO_PULSE_EXPECTED_MAX_INSTANCES=3  Status warn threshold (MAIN only;
                                           sidecars excluded — GH#21903).
  MAESTRO_AGENTS_DIR=<path>               Override ~/.maestro/agents.
