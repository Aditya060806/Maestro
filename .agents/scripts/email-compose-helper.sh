...
# Email Composition Helper for Maestro Framework
...
# Credentials: maestro secret set EMAIL_FROM / AWS_ACCESS_KEY_ID
#
# Part of maestro email system (t1495)
...
readonly WORKSPACE_DIR="${EMAIL_COMPOSE_WORKSPACE:-${HOME}/.maestro/.agent-workspace/email-compose}"
...
Workflow:
  ...
  2. Draft saved to ~/.maestro/.agent-workspace/email-compose/drafts/
  3. Editor opens for human review (unless --no-review)
  4. Confirm send prompt
  5. Sent archive: ~/.maestro/.agent-workspace/email-compose/sent/
EOF
	return 0
}
