...
if [[ ! -f "$WEBHOOK_CONF" ]]; then
	...
	WEBHOOK_CONF="${HOME}/.maestro/agents/configs/webhook-receiver.conf"
fi
if [[ ! -f "$WEBHOOK_CONF" ]]; then
...
WEBHOOK_LOG_FILE="${WEBHOOK_LOG_FILE:-${HOME}/.maestro/logs/pulse-merge-webhook.log}"
...
_resolve_secret() {
	...
	if [[ -z "$secret_val" ]]; then
		...
		if command -v gopass >/dev/null 2>&1; then
			secret_val=$(gopass show -o "maestro/${env_var_name}" 2>/dev/null || true)
		fi
	fi
	printf '%s' "$secret_val"
	return 0
...
cmd_check() {
	...
	if [[ -n "$secret" ]]; then
		...
		printf '  secret:  MISSING — set with: maestro secret set %s\n' "${WEBHOOK_SECRET_ENV}" >&2
		errors=$((errors + 1))
	fi
...
Secret:
  ...
  Set with: maestro secret set ${WEBHOOK_SECRET_ENV:-GITHUB_WEBHOOK_SECRET}
  Or in ~/.config/maestro/credentials.sh (mode 600).
