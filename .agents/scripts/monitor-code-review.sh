...
check_sonarcloud() {
	...
	local api_url="https://sonarcloud.io/api/measures/component?component=Aditya060806_maestro&metricKeys=bugs,vulnerabilities,code_smells,coverage,duplicated_lines_density"
	local response
...
run_qlty_analysis() {
	...
	if bash "$REPO_ROOT/.agents/scripts/qlty-cli.sh" check 5 >"$REPO_ROOT/.agents/tmp/qlty-results.txt" 2>&1; then
		...
		# See: https://github.com/Aditya060806/Maestro/issues/333
