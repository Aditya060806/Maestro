...
_detect_opencode_version() {
	...
	if [[ -n "${MAESTRO_SIG_CLI_VERSION:-}" ]]; then
		echo "$MAESTRO_SIG_CLI_VERSION"
		return 0
	fi
	...
	local greeting_file="${HOME}/.maestro/cache/session-greeting.txt"
...
# =============================================================================
# CLI detection (reuses maestro-update-check.sh logic)
# =============================================================================

_detect_cli() {
	# Inline detection (mirrors maestro-update-check.sh detect_app)
