...
# =============================================================================
# Runs every 60s via the sh.maestro.pulse-watchdog launchd job. Independent
# of the pulse plist itself — survives `maestro update` plist regeneration.
#
# Layered defense:
...
#
# Env:
#   MAESTRO_PULSE_WATCHDOG_GRACE     Seconds beyond StartInterval to wait
#                                     before reviving (default: 120)
#   MAESTRO_PULSE_WATCHDOG_DISABLE=1 Disable the watchdog (no-op exit 0)
...
if [[ "${MAESTRO_PULSE_WATCHDOG_DISABLE:-0}" == "1" ]]; then
	exit 0
fi

_AGENTS_DIR="${MAESTRO_AGENTS_DIR:-${HOME}/.maestro/agents}"
_LIFECYCLE_HELPER="${_AGENTS_DIR}/scripts/pulse-lifecycle-helper.sh"
_LOG_DIR="${HOME}/.maestro/logs"
_WATCHDOG_LOG="${_LOG_DIR}/pulse-watchdog.log"
_LAST_RUN_FILE="${_LOG_DIR}/pulse-wrapper-last-run.ts"
_SETTINGS_FILE="${HOME}/.config/maestro/settings.json"
...
_GRACE="${MAESTRO_PULSE_WATCHDOG_GRACE:-120}"
