...
_enrich_resolve_bounds() {
	local target_task="$1"
	ENRICH_MAX_ISSUES_RESOLVED="${MAESTRO_ENRICH_MAX_ISSUES:-${ENRICH_MAX_ISSUES:-0}}"
	ENRICH_MAX_SECONDS_RESOLVED="${MAESTRO_ENRICH_MAX_SECONDS:-${ENRICH_MAX_SECONDS:-0}}"
...
_enrich_check_active_claim() {
	...
	if [[ -x "$_dedup_helper" ]]; then
		...
		local _user="${MAESTRO_SESSION_USER:-}"
