...
compare_rtk_output() {
	...
	local tmp_rtk=""
	local tmp_filtered=""
	tmp_raw=$(mktemp "${TMPDIR:-/tmp}/maestro-rtk-raw.XXXXXX")
	push_cleanup "rm -f '${tmp_raw}'"
	tmp_rtk=$(mktemp "${TMPDIR:-/tmp}/maestro-rtk-proxied.XXXXXX")
	push_cleanup "rm -f '${tmp_rtk}'"
	tmp_filtered=$(mktemp "${TMPDIR:-/tmp}/maestro-rtk-filtered.XXXXXX")
...
main() {
	...
	local tmp_output=""
	tmp_output=$(mktemp "${TMPDIR:-/tmp}/maestro-rtk.XXXXXX")
