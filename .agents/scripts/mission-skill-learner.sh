...
readonly MEMORY_DIR="${MAESTRO_MEMORY_DIR:-$HOME/.maestro/.agent-workspace/memory}"
readonly MEMORY_DB="${MEMORY_DIR}/memory.db"
readonly DRAFT_DIR="$HOME/.maestro/agents/draft"
readonly CUSTOM_DIR="$HOME/.maestro/agents/custom"
...
#######################################
# Score adherence to maestro standard format (+15 max)
# $1: artifact content (string)
# $2: artifact type (agent|script)
# Outputs: integer points earned
#######################################
...
#   - Standard format (follows maestro conventions): +15
...
cmd_scan_all() {
	...
	local homeless_base="$HOME/.maestro/missions"
	...
	if [[ ${#mission_dirs[@]} -eq 0 ]]; then
		...
		log_info "  - ~/.maestro/missions/*/mission.md (homeless)"
		return 0
	fi
...
cmd_help() {
	...
	echo "  Standard format (maestro conventions): +15"
