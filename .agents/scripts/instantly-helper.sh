...
print_usage() {
	echo "Instantly Helper - Maestro Framework"
	echo ""
	...
	echo ""
	echo "Auth:"
	echo "  Export INSTANTLY_API_KEY in your terminal (recommended via: maestro secret run INSTANTLY_API_KEY -- instantly-helper.sh ...)."
	return 0
}
...
require_api_key() {
	if [[ -z "${INSTANTLY_API_KEY:-}" ]]; then
		...
		print_info "Set it in your terminal via: maestro secret set INSTANTLY_API_KEY"
		print_info "Then run using: maestro secret run INSTANTLY_API_KEY -- instantly-helper.sh status"
		return 1
	fi
	return 0
}
