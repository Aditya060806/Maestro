...
def vault_dir_from(value: str | None) -> Path:
    ...
    configured = os.environ.get("MAESTRO_VAULT_AUDIT_DIR") or os.environ.get("MAESTRO_VAULT_DIR")
    if configured:
        return Path(configured).expanduser()
    return Path.home() / ".config" / "maestro" / "vault" / "audit"
...
def cmd_append(args: argparse.Namespace) -> int:
    ...
    ciphertext = AESGCM(b64d(str(key[FIELD_AUDIT_PAYLOAD_KEY]))).encrypt(nonce, canonical(event_payload), b"maestro-vault-audit-event")
