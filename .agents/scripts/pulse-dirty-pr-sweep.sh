...
DIRTY_PR_SWEEP_LAST_RUN="${DIRTY_PR_SWEEP_LAST_RUN:-${HOME}/.maestro/logs/dirty-pr-sweep-last-run}"
DIRTY_PR_SWEEP_STATE_FILE="${DIRTY_PR_SWEEP_STATE_FILE:-${HOME}/.maestro/.agent-workspace/supervisor/dirty-pr-sweep-state.json}"
...
_dps_linked_issue_is_worker_backed() {
	...
	[[ "${MAESTRO_DIRTY_PR_WORKER_BACKED_CHECK:-1}" == "1" ]] || return 1
...
# claim stamps at ~/.maestro/.agent-workspace/interactive-claims/ are not
...
Examples:
  ...
  pulse-dirty-pr-sweep.sh --dry-run --repo Aditya060806/Maestro
  pulse-dirty-pr-sweep.sh --dry-run --repo Aditya060806/Maestro --pr 19696
