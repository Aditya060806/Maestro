...
# =============================================================================
# Guards maestro issue-state writes (dispatch claims, claim releases, labels,
...
# valid PR targets, but they must not receive maestro internal coordination
# spam.
...
#######################################
maestro_repo_state_current_user() {
	local login=""
...
#######################################
# Check whether maestro may mutate issue workflow state for a repo.
#
# Args:
...
#######################################
maestro_can_manage_repo_issue_state() {
	local slug="${1:-}"
	local user="${2:-}"

	if [[ "${MAESTRO_TEST_MODE:-}" == "1" && \
		"${MAESTRO_REPO_STATE_GUARD_TEST_BYPASS:-}" == "1" ]]; then
		return 0
	fi
	...
	if [[ -z "$user" ]]; then
		user=$(maestro_repo_state_current_user)
	fi
	if [[ -z "$user" ]]; then
		return 1
	fi
	...
	local permission=""
	# #maestro:trust-boundary — issue-state writes require confirmed write+ access;
...
#######################################
# Check whether maestro may run public repo routines for a repo.
#
...
#######################################
maestro_can_run_repo_routines() {
	local slug="${1:-}"
	local user="${2:-}"

	if [[ "${MAESTRO_TEST_MODE:-}" == "1" && \
		"${MAESTRO_REPO_ROUTINE_GUARD_TEST_BYPASS:-}" == "1" ]]; then
		return 0
	fi
