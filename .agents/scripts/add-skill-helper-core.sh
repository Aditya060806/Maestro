...
show_help() {
	cat <<'EOF'
Add External Skill Helper - Import skills from GitHub, ClawdHub, or URLs to maestro
...
SUPPORTED FORMATS:
    ...
    - AGENTS.md (maestro/Windsurf format)
    - .cursorrules (Cursor format)
    - Raw markdown files

The skill will be converted to maestro format and placed in .agents/
...
# Convert SKILL.md to maestro format
convert_skill_md() {
	local source_file="$1"
	local target_file="$2"
	local skill_name="$3"
	...
	# Create maestro-style header with properly quoted description
	cat >"$target_file" <<EOF
---
