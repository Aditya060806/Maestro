...
# Paths
_DSI_LOG_DIR="${HOME}/.maestro/logs"
...
_dsi_guard_no_maintainer_review_required() {
	...
	#maestro:trust-boundary -- manual dispatch must not bypass signed/maintainer issue approval.
	if [[ "$labels_with_commas" == *",needs-maintainer-review,"* ]]; then
		...
		_dsi_info "  Required action: run 'sudo maestro approve issue ${issue_number} ${repo_slug}' or record an equivalent maintainer decision before dispatch."
		return 1
	fi
...
_dsi_resolve_model() {
	...
	if [[ "$needle" == *",model:opus-4-7,"* || "$needle" == *",tier:thinking,"* ]]; then
		...
		if [[ "$needle" == *",model:opus-4-7,"* ]]; then
			_DSI_SELECTED_MODEL="${MAESTRO_OPUS_ESCALATION_MODEL:-openai/gpt-5.5}"
		fi
		return 0
	fi
...
_dsi_create_worktree() {
	...
	if [[ -z "$repo_path" || ! -d "$repo_path" ]]; then
		_dsi_err "Cannot resolve local repo path for ${repo_slug}; check ~/.config/maestro/repos.json"
		return 1
	fi
	local ts
	...
	for attempt in 1 2 3; do
		...
		if (cd "$repo_path" && MAESTRO_SKIP_AUTO_CLAIM=1 "$_DSI_WORKTREE_HELPER" add "$branch" \
...
_dsi_dispatch_base_branch() {
	local repo_slug="$1"
	local repo_path="$2"
	local base_branch="${MAESTRO_DISPATCH_BASE_BRANCH:-${DISPATCH_REPO_PR_BASE_BRANCH:-${MAESTRO_PR_BASE_BRANCH:-}}}"

	if [[ -z "$base_branch" && -n "$repo_path" && -f "${repo_path}/.maestro.json" ]] && command -v jq >/dev/null 2>&1; then
		base_branch=$(jq -r '.dispatch_base_branch // .dispatch_base // .pr_base_branch // .pr_target_branch // .base_branch // empty' \
			"${repo_path}/.maestro.json" 2>/dev/null | sed -n '1p') || base_branch=""
	fi

	local repos_json="${MAESTRO_REPOS_FILE:-${HOME}/.config/maestro/repos.json}"
...
_dsi_dispatch_base_ref() {
	local repo_slug="$1"
	local repo_path="$2"
	local base_ref="${MAESTRO_DISPATCH_BASE_REF:-${MAESTRO_WORKTREE_BASE:-}}"
...
_dsi_repo_path_for_slug() {
	local repo_slug="$1"
	local repos_file="${MAESTRO_REPOS_FILE:-$HOME/.config/maestro/repos.json}"
...
_dsi_wait_for_worker_readiness() {
	...
	local timeout_s="${MAESTRO_DSI_READY_TIMEOUT_SECONDS:-20}"
	local attempts=0
...
_dsi_parse_dispatch_args() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--base)
			...
			MAESTRO_DISPATCH_BASE_REF="${2}"
			export MAESTRO_DISPATCH_BASE_REF
			shift 2
			;;
		--dry-run)
			_DSI_ARG_DRYRUN=1
...
Examples:
  dispatch-single-issue-helper.sh dispatch 20882 Aditya060806/Maestro
  dispatch-single-issue-helper.sh dispatch 20882 Aditya060806/Maestro --model anthropic/claude-opus-4-7
  dispatch-single-issue-helper.sh dispatch 20882 Aditya060806/Maestro --agent Build+
  dispatch-single-issue-helper.sh dispatch 20882 Aditya060806/Maestro --base origin/develop
  dispatch-single-issue-helper.sh dispatch 20882 Aditya060806/Maestro --dry-run
  dispatch-single-issue-helper.sh dispatch 20882 Aditya060806/Maestro --no-ceremony
EOF
	return 0
}
...
Examples:
  ...
  # Check status
  dispatch-single-issue-helper.sh status 20882 Aditya060806/Maestro
