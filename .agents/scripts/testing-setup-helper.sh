...
readonly TESTING_CONFIG_FILE=".maestro-testing.json"
...
# Detect Bash test scripts (maestro pattern) and append to runners JSON array (stdout)
...
# Detect agent test helper (maestro-specific) and append to runners JSON array (stdout)
...
cmd_status() {
	...
	# Check for .maestro-testing.json
