...
# Onboarding Helper - Interactive setup and service status for maestro
#
...
readonly SETTINGS_FILE="$HOME/.config/maestro/settings.json"
readonly SETTINGS_HELPER="${SCRIPT_DIR}/settings-helper.sh"

# Credential file locations
readonly CREDENTIALS_FILE="$HOME/.config/maestro/credentials.sh"
...
check_wordpress() {
	...
	if [[ -f "$HOME/.config/maestro/mainwp-config.json" ]]; then
...
check_repo_sync() {
	...
	local config_file="$HOME/.config/maestro/repos.json"

	# Check if scheduler is installed
	local scheduler_active=false
	if launchctl list 2>/dev/null | grep -q "com.maestro.maestro-repo-sync"; then
	...
	if [[ "$scheduler_active" == "true" ]] && [[ "$dir_count" -gt 0 ]]; then
		...
		print_service "Repo Sync" "optional" "not enabled (maestro repo-sync enable)"
	else
		print_service "Repo Sync" "optional" "not installed (maestro update)"
	fi
...
configure_dirs() {
	...
	if [[ "$added" -gt 0 ]]; then
		...
		echo "Run 'maestro repo-sync check' to sync now."
...
check_orchestration() {
	...
	if launchctl list 2>/dev/null | grep -qF "com.maestro.maestro-supervisor-pulse"; then
		pulse_active=true
	elif launchctl list 2>/dev/null | grep -qF "com.maestro.supervisor-pulse"; then
		pulse_active=true
	elif crontab -l 2>/dev/null | grep -qF "maestro-supervisor-pulse"; then
		pulse_active=true
	fi

	if [[ "$pulse_active" == "true" ]]; then
		print_service "Supervisor Pulse" "ready" "dispatches workers, merges PRs every 2 min"
		echo "    Runs 24/7 by default. Use 'maestro pulse stop' for manual start/stop mode." >&2
	else
	...
	if [[ -f "$repos_config" ]] && command -v jq &>/dev/null; then
		...
		print_service "Cross-Repo Visibility" "optional" "run maestro init in your projects"
	fi
	...
	# Budget tracking
	local budget_script="$HOME/.maestro/agents/scripts/budget-tracker-helper.sh"
...
show_status() {
	echo ""
	echo -e "${BLUE}maestro Service Status${NC}"
	echo "========================"
	echo ""
	...
	echo -e "${DIM}---${NC}"
	echo -e "Run ${BLUE}~/.maestro/agents/scripts/list-keys-helper.sh${NC} for detailed key status"
...
show_recommendations() {
	...
	*)
		...
		echo "  • Orchestration: maestro pulse start (autonomous workers)"
...
_guide_openai() {
	...
	echo "2. Store key:"
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set OPENAI_API_KEY \"sk-...\""
	echo "3. Restart terminal or: source ~/.config/maestro/credentials.sh"
	return 0
}
...
_guide_anthropic() {
	...
	echo "2. Store key:"
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set ANTHROPIC_API_KEY \"sk-ant-...\""
	echo "3. Restart terminal or: source ~/.config/maestro/credentials.sh"
	return 0
}
...
_guide_hetzner() {
	...
	echo "4. Store token:"
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set HCLOUD_TOKEN_MAIN \"your-token\""
	return 0
}
...
_guide_cloudflare() {
	...
	echo "4. Store token:"
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set CLOUDFLARE_API_TOKEN \"your-token\""
	return 0
}
...
_guide_dataforseo() {
	...
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set DATAFORSEO_USERNAME \"your-email\""
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set DATAFORSEO_PASSWORD \"your-password\""
	return 0
}
...
_guide_sonarcloud() {
	...
	echo "4. Store token:"
	echo "   ~/.maestro/agents/scripts/setup-local-api-keys.sh set SONAR_TOKEN \"your-token\""
	return 0
}
...
_guide_orchestration() {
	...
	echo ""
	echo "Docs: ~/.maestro/agents/reference/orchestration.md"
	return 0
}
...
# Show help
show_help() {
	echo "Onboarding Helper - Interactive setup and service status for maestro"
	echo ""
...
_json_orchestration() {
	local pulse
	pulse=false
	if launchctl list 2>/dev/null | grep -qF "com.maestro.maestro-supervisor-pulse" ||
		launchctl list 2>/dev/null | grep -qF "com.maestro.supervisor-pulse" ||
		crontab -l 2>/dev/null | grep -qF "maestro-supervisor-pulse"; then
		pulse=true
	fi
