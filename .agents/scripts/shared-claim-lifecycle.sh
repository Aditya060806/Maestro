...
#     ~/.maestro/.agent-workspace/interactive-claims)
#   - AGENTS_DIR env var (optional; defaults to ~/.maestro/agents)
#
...
if ! declare -F maestro_pulse_worker_log_candidates >/dev/null 2>&1 && [[ -r "${_scl_script_dir}/shared-constants.sh" ]]; then
...
release_interactive_claim_on_merge() {
	...
	local _stamp_base="${CLAIM_STAMP_DIR:-${HOME}/.maestro/.agent-workspace/interactive-claims}"
	...
	local _isc_helper="${AGENTS_DIR:-${HOME}/.maestro/agents}/scripts/interactive-session-helper.sh"
...
_resolve_orphan_recovery_base_branch() {
	local repo_slug="$1"
	local work_dir="${2:-}"
	local base_branch="${WORKER_PR_BASE_BRANCH:-${DISPATCH_REPO_PR_BASE_BRANCH:-${MAESTRO_PR_BASE_BRANCH:-}}}"

	if [[ -z "$base_branch" && -n "$work_dir" && -f "${work_dir}/.maestro.json" ]] && command -v jq >/dev/null 2>&1; then
		base_branch=$(jq -r '.pr_base_branch // .pr_target_branch // .base_branch // .default_branch // empty' "${work_dir}/.maestro.json" 2>/dev/null || true)
	fi

	local repos_json="${MAESTRO_REPOS_JSON:-${REPOS_JSON:-${HOME}/.config/maestro/repos.json}}"
...
_build_orphan_recovery_pr_body() {
	...
	local pr_marker="<!-- maestro:orphan-recovery worker_branch_orphan session=${session_key} -->"
	if [[ "${published_local_branch:-}" == "1" ]]; then
		...
		pr_marker="<!-- maestro:orphan-recovery worker_local_branch_unpushed session=${session_key} -->"
	fi
...
_attempt_orphan_recovery_pr() {
	...
	gh pr create "${create_args[@]}" >/dev/null 2>&1 # maestro-allow: raw-gh-wrapper
	return $?
}
