...
#   provision-workspace       Create workspace-level inbox at ~/.maestro/.agent-workspace/inbox/
...
readonly WORKSPACE_INBOX_DIR="${HOME}/.maestro/.agent-workspace/inbox"
...
_add_url() {
	...
	if command -v curl >/dev/null 2>&1; then
		if curl -fsSL --max-time 30 -A "maestro-inbox/1.0" "$url" -o "$html_file" 2>/dev/null; then
			curl_ok=1
		fi
	fi
...
cmd_find() {
	...
	if [[ ${#log_paths[@]} -eq 0 ]]; then
		print_warning "No triage.log found. Run: maestro inbox provision"
		return 0
	fi
...
cmd_correct() {
	...
	repo_root="$(pwd)"
	inbox_dir="${repo_root}/${INBOX_DIR_NAME}"
	[[ ! -d "$inbox_dir" ]] && print_warning "_inbox/ not found. Run: maestro inbox provision" && return 1
...
cmd_status() {
	...
	if [[ ! -d "$inbox_dir" ]]; then
		print_warning "_inbox/ not found at ${inbox_dir}. Run: maestro inbox provision"
		return 0
	fi
...
_triage_run_classification() {
	...
	prompt_file="$(mktemp "${TMPDIR:-/tmp}/maestro-inbox-prompt.XXXXXX")"
	{
		printf 'Classify this _inbox item into the correct maestro plane. Return JSON with target_plane, sub_folder, confidence, and reasoning.\n\n'
...
cmd_triage() {
	...
	[[ ! -d "$inbox_dir" ]] && print_warning "_inbox/ not found. Run: maestro inbox provision" && return 0
	local log_path="${inbox_dir}/${TRIAGE_LOG}"
...
#   --include-workspace   Also scan ~/.maestro/.agent-workspace/inbox/
...
Commands:
  ...
  digest [options]          Show stale items in _drop/ and _needs-review/ (default: >= 7d old)
    ...
    --include-workspace       Also scan workspace inbox (~/.maestro/.agent-workspace/inbox/)
    --json                    Machine-readable JSON array output
