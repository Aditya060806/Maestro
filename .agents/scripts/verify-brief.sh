...
#
# Runtime method reads testing.json from repo root or ~/.maestro/testing.json for defaults.
...
# Read a field from testing.json (repo-local or ~/.maestro/testing.json).
...
_read_testing_json_field() {
	...
	# Search order: repo-local testing.json, then ~/.maestro/testing.json
	local config_file=""
	if [[ -f "${repo_path}/testing.json" ]]; then
		config_file="${repo_path}/testing.json"
	elif [[ -f "${HOME}/.maestro/testing.json" ]]; then
		config_file="${HOME}/.maestro/testing.json"
	fi
