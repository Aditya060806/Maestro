...
#
# Cache: ~/.maestro/cache/gh-status-{status,incidents}.json (60s freshness).
# Statuspage rate limit ~100 req/min/IP; cache prevents hammering during
# tight diagnostic loops.
#
# Mock interface (for tests):
#   MAESTRO_GH_STATUS_MOCK_DIR=/tmp/mocks gh-status-helper.sh check
...
readonly CACHE_DIR="${MAESTRO_GH_STATUS_CACHE_DIR:-$HOME/.maestro/cache}"
readonly CACHE_STATUS="${CACHE_DIR}/gh-status-status.json"
readonly CACHE_INCIDENTS="${CACHE_DIR}/gh-status-incidents.json"
readonly CACHE_MAX_AGE_SECONDS="${MAESTRO_GH_STATUS_CACHE_TTL:-60}"
...
# Honours MAESTRO_GH_STATUS_MOCK_DIR for tests; otherwise curls the URL.
_fetch() {
	local url="$1"
	local cache_file="$2"

	if [[ -n "${MAESTRO_GH_STATUS_MOCK_DIR:-}" ]]; then
		local mock_name
		mock_name=$(basename "$url")
		local mock_path="${MAESTRO_GH_STATUS_MOCK_DIR}/${mock_name}"
		if [[ -f "$mock_path" ]]; then
...
cmd_correlate() {
	...
	printf '<!-- maestro:gh-status-correlation -->\n'
...
Cache: ~/.maestro/cache/gh-status-*.json (60s TTL, override via
MAESTRO_GH_STATUS_CACHE_TTL).

Tests can stub the API by setting MAESTRO_GH_STATUS_MOCK_DIR to a
