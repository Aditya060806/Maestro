...
#
# Storage: ~/.config/maestro/tenants/{tenant}/credentials.sh
# Active:  ~/.config/maestro/active-tenant
# Project: .maestro-tenant (per-project override)
#
# For encrypted storage with gopass, use: secret-helper.sh (maestro secret)
#
...
readonly PROJECT_TENANT_FILE=".maestro-tenant"
...
MAESTRO_ACTIVE_TENANT="$active_tenant"
export MAESTRO_ACTIVE_TENANT
...
cmd_use() {
	...
	if [[ -f ".gitignore" ]] && ! grep -q "^\.maestro-tenant$" ".gitignore" 2>/dev/null; then
		# Ensure trailing newline before appending (prevents malformed entries)
		[[ -s ".gitignore" && $(tail -c1 ".gitignore" | wc -l) -eq 0 ]] && printf '\n' >>".gitignore"
		echo ".maestro-tenant" >>".gitignore"
		print_info "Added .maestro-tenant to .gitignore"
	fi
...
cmd_export() {
	...
	echo "export MAESTRO_ACTIVE_TENANT=\"$tenant\""
	return 0
}
...
cmd_help() {
	echo ""
	print_info "Maestro - Multi-Tenant Credential Storage"
	echo ""
