...
_preflight_cleanup_and_ledger() {
	...
	# Progress and last-run timestamp: ~/.maestro/logs/cleanup_worktrees.*
	...
	# default 10 min). Progress: ~/.maestro/logs/cleanup_stashes.*
	...
	if [[ -x "$_cleanup_stashes_async_helper" ]]; then
		nohup "$_cleanup_stashes_async_helper" \
			>>"${HOME}/.maestro/logs/cleanup_stashes.log" 2>&1 &
		disown $! 2>/dev/null || true
	else
	...
	# Progress: ~/.maestro/logs/cleanup_remote_branches.*
	...
	if [[ -x "$_cleanup_remote_branches_async_helper" ]]; then
		nohup "$_cleanup_remote_branches_async_helper" \
			>>"${HOME}/.maestro/logs/cleanup_remote_branches.log" 2>&1 &
		disown $! 2>/dev/null || true
	fi
