...
load_credentials() {
	if [[ -z "${RUNWAYML_API_SECRET:-}" ]]; then
		...
		if command -v gopass &>/dev/null; then
			RUNWAYML_API_SECRET="$(gopass show -o "maestro/RUNWAYML_API_SECRET" 2>/dev/null || true)"
		fi
		# Fallback to credentials file
		if [[ -z "${RUNWAYML_API_SECRET:-}" ]] && [[ -f "$HOME/.config/maestro/credentials.sh" ]]; then
			# shellcheck source=/dev/null
			source "$HOME/.config/maestro/credentials.sh"
		fi
		if [[ -z "${RUNWAYML_API_SECRET:-}" ]]; then
			echo "ERROR: RUNWAYML_API_SECRET not set." >&2
			echo "Run: maestro secret set RUNWAYML_API_SECRET" >&2
			echo "Or export RUNWAYML_API_SECRET in ~/.config/maestro/credentials.sh" >&2
			return 1
		fi
	fi
	return 0
