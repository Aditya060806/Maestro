...
# doctor-helper.sh — Detect and consolidate duplicate maestro/opencode installs
#
# Finds all install locations for both `maestro` and `opencode` binaries,
...
# Diagnose a single binary (maestro or opencode)
diagnose_binary() {
	local binary_name="$1"
	local locations_raw=""
...
main() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--help | -h)
			...
			echo ""
			echo "Detect and consolidate duplicate maestro/opencode installs."
			echo ""
			echo "Options:"
	...
	if [[ "$FIX_MODE" == "true" ]]; then
		print_header "Maestro Doctor — Fix Mode"
		...
		echo ""
		diagnose_binary "maestro"
		diagnose_binary "opencode"
		return 0
	fi

	if [[ "$QUIET_MODE" != "true" ]]; then
		print_header "Maestro Doctor"
	...
	if [[ "$QUIET_MODE" != "true" ]]; then
		...
		if [[ $CONFLICTS_FOUND -eq 0 ]]; then
			...
			print_warning "Conflicts detected. Run 'maestro doctor --fix' to resolve interactively."
		fi
	fi
