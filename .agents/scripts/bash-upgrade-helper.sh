...
_ADVISORY_DIR="${HOME}/.maestro/advisories"
_ADVISORY_FILE="${_ADVISORY_DIR}/bash-3.2-upgrade.advisory"
_STATE_DIR="${HOME}/.maestro/state"
...
_bu_cmd_install() {
	...
	if [[ -z "$yes" ]] && [[ -t 0 ]]; then
		...
		*)
			...
			_bu_write_advisory "bash-3.2-upgrade | macOS bash 3.2 detected. Run: brew install bash (or: maestro security posture)"
			return 1
			;;
		esac
	fi
...
# and maestro-update-check.sh both call. Behaviour:
...
#   - Current → no-op
#   - Non-macOS → no-op
#   - MAESTRO_AUTO_UPGRADE_BASH=0 → honoured as opt-out (no install,
...
_bu_cmd_ensure() {
	...
	# Opt-out: MAESTRO_AUTO_UPGRADE_BASH=0 disables automated install/upgrade.
	# Default (unset or any other value) = enabled.
	if [[ "${MAESTRO_AUTO_UPGRADE_BASH:-1}" == "0" ]]; then
		_bu_info "MAESTRO_AUTO_UPGRADE_BASH=0 — skipping ensure"
		return 0
	fi
...
Subcommands:
  ...
  path            Print absolute path to modern bash (empty if none).
                  ...
                  Silent on upgrade (no prompt). Respects MAESTRO_AUTO_UPGRADE_BASH=0 opt-out.
...
Environment:
  MAESTRO_AUTO_UPGRADE_BASH  Set to "0" to disable automated install/upgrade in \`ensure\`.
                              Default (unset or any other value) = enabled.
...
Examples:
  ...
  bash-upgrade-helper.sh ensure --yes --quiet    # Used by maestro update
