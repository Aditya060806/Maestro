...
text='Mention of <webapp> as a generic placeholder, plus Aditya060806/Maestro'
hits=$(privacy_scan_text "$text" "$SLUGS_FILE")
rc=$?
if [[ "$rc" -eq 0 && -z "$hits" ]]; then
...
mkdir -p "$PRIV_HOME/.config/maestro" "$PRIV_HOME/.maestro/cache"
cat >"$PRIV_HOME/.config/maestro/repos.json" <<'EOF'
{
  "initialized_repos": [
    {
...
cat >"$PRIV_HOME/.maestro/cache/repo-privacy.json" <<EOF
{
  "Aditya060806/Maestro": { "private": false, "checked_at": $(date +%s) },
...
run_shim() {
	...
	shift
	HOME="$PRIV_HOME" \
		PRIVACY_REPOS_CONFIG="$PRIV_HOME/.config/maestro/repos.json" \
		PRIVACY_CACHE_FILE="$PRIV_HOME/.maestro/cache/repo-privacy.json" \
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body "Mentions acme/longprivate today" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]] && printf '%s' "$out" | grep -q 'BLOCK.*Aditya060806/Maestro'; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body "Hit a bug in longprivate today" 2>&1)
rc=$?
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body "Clean content with <webapp> placeholder" 2>&1)
rc=$?
...
rm -f "$RECORD_FILE"
out=$(MAESTRO_GH_PRIVACY_BYPASS=1 run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body "Mentions acme/longprivate today" 2>&1)
rc=$?
if [[ "$rc" -eq 0 ]] && [[ -f "$RECORD_FILE" ]] && printf '%s' "$out" | grep -q 'BYPASSED'; then
	pass "MAESTRO_GH_PRIVACY_BYPASS=1 → ALLOWED with audit notice"
else
...
out=$(run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body-file "$BF" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "Bug in longprivate observed" --body "Clean body" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
...
# -- Test 2.8: MAESTRO_GH_SHIM_DISABLE bypasses entire shim → ALLOWED
rm -f "$RECORD_FILE"
out=$(MAESTRO_GH_SHIM_DISABLE=1 run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body "Mentions acme/longprivate" 2>&1)
rc=$?
if [[ "$rc" -eq 0 ]] && [[ -f "$RECORD_FILE" ]]; then
	pass "MAESTRO_GH_SHIM_DISABLE=1 bypasses privacy-scan (and entire shim)"
else
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue list --repo Aditya060806/Maestro 2>&1)
rc=$?
if [[ "$rc" -eq 0 ]] && [[ -f "$RECORD_FILE" ]]; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" pr edit 123 --repo Aditya060806/Maestro --body "Updated to mention acme/longprivate" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue edit 456 --repo Aditya060806/Maestro --body "Edit referencing acme/longprivate" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" issue create --repo Aditya060806/Maestro --title "test" --body "See /Users/example/Git/private-project for repro" 2>&1)
rc=$?
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" pr review 123 --repo Aditya060806/Maestro --comment --body "Review mentions acme/longprivate" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" api -X POST repos/Aditya060806/Maestro/issues -f title=test -f body="API mentions acme/longprivate" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
...
rm -f "$RECORD_FILE"
out=$(run_shim "$PATH_DIR" api --method POST /repos/Aditya060806/Maestro/pulls/123/reviews -f body="Review path /home/example/private" 2>&1)
rc=$?
if [[ "$rc" -eq 1 ]] && [[ ! -f "$RECORD_FILE" ]]; then
