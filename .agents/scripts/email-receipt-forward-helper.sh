...
#
# Config: ~/.config/maestro/email-receipt-forward.json
# Credentials: maestro secret set EMAIL_RECEIPT_FORWARD_ACCOUNTS_EMAIL
#
# Dependencies: dig, jq, aws (SES), python3
# Part of maestro email system (t1507)
...
readonly WORKSPACE_DIR="${HOME}/.maestro/.agent-workspace/email-receipt-forward"
...
get_accounts_email() {
	...
	print_info "Set via: maestro secret set EMAIL_RECEIPT_FORWARD_ACCOUNTS_EMAIL"
...
forward_to_accounts() {
	...
	if command -v aws &>/dev/null; then
		local sender_email
		sender_email=$(get_config_value_from_file ".sender_email" "${CONFIG_FILE}" "noreply@maestro.sh")
...
body_text = f"""Receipt/invoice forwarded by Maestro email-receipt-forward-helper.
...
CREDENTIALS
  maestro secret set EMAIL_RECEIPT_FORWARD_ACCOUNTS_EMAIL
