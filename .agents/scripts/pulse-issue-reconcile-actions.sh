...
#   _should_lia   — stage 5 predicate (labelless maestro-shaped)
#
...
# Stage 5 predicate: issue is an maestro-shaped labelless candidate.
# Title must match tNNN: or GH#NNN: AND no origin:/tier:/status: labels.
# Args: $1 = issue_title, $2 = labels_csv
_should_lia() {
	local issue_title="$1"
	local labels_csv="$2"
	# Title must match maestro task shape
