...
#
# When a contributor runs maestro with a repo they don't own in repos.json
...
#      ~/.maestro/agents/, client/project names, credential patterns
...
_load_private_slugs() {
	...
	local extras="${HOME}/.maestro/configs/privacy-guard-extra-slugs.txt"
	if [[ -f "$extras" ]]; then
...
_compose_instruction_issue() {
	...
	body+="<!-- maestro:generator=contributor-insight-helper -->"$'\n\n'
