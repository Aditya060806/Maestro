...
#   ~/.maestro/logs/efficiency-report-YYYY-MM-DD.json
#
...
readonly LOGS_DIR="${HOME}/.maestro/logs"
readonly AGENT_WORKSPACE="${HOME}/.maestro/.agent-workspace"
...
collect_concurrency() {
	...
	local backoff_db
	backoff_db=$(find "${HOME}/.maestro/.agent-workspace" -name "*.db" -path "*/headless-runtime/*" 2>/dev/null | head -1 || echo "")
	if [[ -z "$backoff_db" ]]; then
		# Try the headless-runtime-helper's default DB path
		backoff_db="${HOME}/.maestro/.agent-workspace/headless-runtime/headless-runtime.db"
	fi
