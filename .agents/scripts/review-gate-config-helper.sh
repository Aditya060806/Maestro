...
# in ~/.config/maestro/repos.json without hand-editing JSON.
#
# Usage:
#   maestro review-gate                              Show config for all repos
#   maestro review-gate list                         Same as above (explicit)
#   maestro review-gate <slug>                       Show config for one repo
#   maestro review-gate <slug> pass|wait|unset       Set per-repo default
#   maestro review-gate <slug> --tool <bot> pass|wait|unset  Per-tool override
#   maestro review-gate <slug> --completion fast|strict|unset  Completion policy
#   maestro review-gate <slug> --tool <bot> --completion fast|strict|unset
#   maestro review-gate --help                       Show this help
#
# Schema written to ~/.config/maestro/repos.json under the matching
...
_require_repos_json() {
	if [[ ! -f "$REPOS_JSON" ]]; then
		...
		_print_info "Run 'maestro init' to create it."
		return 1
	fi
	return 0
}
...
cmd_list() {
	...
	if [[ -n "$filter_slug" ]]; then
		...
		if [[ -z "$resolved" ]]; then
			...
			_print_info "Run 'maestro repos' to list registered repos."
			return 1
		fi
		slugs="$resolved"
	else
	...
	if [[ -z "$slugs" ]]; then
		_print_info "No repos registered. Run 'maestro init' or 'maestro repos'."
		return 0
	fi
...
cmd_help() {
	...
	echo ""
	echo "Examples:"
	echo "  maestro review-gate                              # list all repos"
	echo "  maestro review-gate Aditya060806/myrepo          # show one repo"
	echo "  maestro review-gate Aditya060806/myrepo wait     # block merges on rate-limit"
	echo "  maestro review-gate Aditya060806/myrepo pass     # restore default (pass)"
	echo "  maestro review-gate Aditya060806/myrepo unset    # remove per-repo override"
	echo "  maestro review-gate Aditya060806/myrepo --tool coderabbitai wait"
	echo "  maestro review-gate Aditya060806/myrepo --tool coderabbitai unset"
	echo "  maestro review-gate Aditya060806/myrepo --completion strict"
	echo "  maestro review-gate Aditya060806/myrepo --tool coderabbitai --completion strict"
	echo ""
