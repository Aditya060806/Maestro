...
load_credentials() {
	...
	case "$provider" in
	sora | openai)
		if command -v gopass >/dev/null 2>&1 && gopass show "maestro/openai-api-key" >/dev/null 2>&1; then
			OPENAI_API_KEY="$(gopass show -o "maestro/openai-api-key" 2>/dev/null)" || true
		fi
		if [[ -z "${OPENAI_API_KEY:-}" ]] && [[ -f "${HOME}/.config/maestro/credentials.sh" ]]; then
			source "${HOME}/.config/maestro/credentials.sh"
		fi
		if [[ -z "${OPENAI_API_KEY:-}" ]]; then
			print_error "OPENAI_API_KEY not set. Run: maestro secret set openai-api-key"
			return 1
		fi
		;;
	veo | google)
		# Veo uses gcloud ADC or GOOGLE_API_KEY
		if command -v gopass >/dev/null 2>&1 && gopass show "maestro/google-api-key" >/dev/null 2>&1; then
			GOOGLE_API_KEY="$(gopass show -o "maestro/google-api-key" 2>/dev/null)" || true
		fi
		if [[ -z "${GOOGLE_API_KEY:-}" ]] && [[ -f "${HOME}/.config/maestro/credentials.sh" ]]; then
			source "${HOME}/.config/maestro/credentials.sh"
		fi
	...
	nanobanana | higgsfield)
		if command -v gopass >/dev/null 2>&1 && gopass show "maestro/higgsfield-api-key" >/dev/null 2>&1; then
			HIGGSFIELD_API_KEY="$(gopass show -o "maestro/higgsfield-api-key" 2>/dev/null)" || true
			HIGGSFIELD_SECRET="$(gopass show -o "maestro/higgsfield-secret" 2>/dev/null)" || true
		fi
		if [[ -z "${HIGGSFIELD_API_KEY:-}" ]] && [[ -f "${HOME}/.config/maestro/credentials.sh" ]]; then
			source "${HOME}/.config/maestro/credentials.sh"
		fi
		if [[ -z "${HIGGSFIELD_API_KEY:-}" ]] || [[ -z "${HIGGSFIELD_SECRET:-}" ]]; then
			print_error "HIGGSFIELD_API_KEY/HIGGSFIELD_SECRET not set. Run: maestro secret set higgsfield-api-key"
			return 1
		fi
		;;
	*)
...
Credentials:
  Store via: maestro secret set <key-name>
