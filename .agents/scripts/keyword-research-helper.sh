...
readonly CONFIG_FILE="$HOME/.config/maestro/keyword-research.json"
readonly CONFIG_DIR="$HOME/.config/maestro"
readonly DOWNLOADS_DIR="$HOME/Downloads"
readonly CACHE_DIR="$HOME/.cache/maestro/keyword-research"
...
check_credentials() {
	...
	source "$HOME/.config/maestro/credentials.sh" 2>/dev/null || true

	case "$provider" in
	"dataforseo" | "both")
		if [[ -z "${DATAFORSEO_USERNAME:-}" ]] || [[ -z "${DATAFORSEO_PASSWORD:-}" ]]; then
			print_error "DataForSEO credentials not found."
			echo "  Set via: maestro secret set DATAFORSEO_USERNAME <username>"
			echo "  Set via: maestro secret set DATAFORSEO_PASSWORD <password>"
			echo "  Or add to ~/.config/maestro/credentials.sh"
			return 1
		fi
		;;
	"serper")
		if [[ -z "${SERPER_API_KEY:-}" ]]; then
			...
			echo "  Set via: maestro secret set SERPER_API_KEY <key>"
			echo "  Or add to ~/.config/maestro/credentials.sh"
			return 1
		fi
		;;
	"ahrefs")
		if [[ -z "${AHREFS_API_KEY:-}" ]]; then
			...
			echo "  Set via: maestro secret set AHREFS_API_KEY <key>"
			echo "  Or add to ~/.config/maestro/credentials.sh"
			return 1
		fi
		;;
