...
#   status  — print human-readable status to stdout (for `maestro status`)
#   help    — usage information
#
# Environment overrides:
#   MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD — fraction of total budget below
...
#   MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER=1 — emergency bypass (dispatch proceeds
...
#
# Counter:
#   `pulse_dispatch_circuit_broken` in ~/.maestro/logs/pulse-stats.json
#   (via pulse-stats-helper.sh). Surfaced by `maestro status`.
#
...
if [[ -z "${MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD+x}" ]] && [[ -f "$_CB_RL_CONF" ]]; then
	# shellcheck disable=SC1090
	source "$_CB_RL_CONF"
fi

# LOGFILE for sourced-mode usage (caller sets it; standalone mode defines a default).
LOGFILE="${LOGFILE:-${HOME}/.maestro/logs/pulse.log}"

# State file for tracking when the breaker last tripped (for status reporting).
_CIRCUIT_BREAKER_STATE_FILE="${HOME}/.maestro/logs/pulse-graphql-circuit-breaker.state"
...
_CB_RL_CACHE_FILE="${MAESTRO_PULSE_RATE_LIMIT_CACHE:-${HOME}/.maestro/cache/pulse-graphql-rate-limit.json}"
_CB_RL_CACHE_TTL="${MAESTRO_PULSE_RATE_LIMIT_CACHE_TTL:-20}"
...
_cb_gh_read() {
	local rc=0
	local secs="${MAESTRO_GH_READ_TIMEOUT:-15}"
	[[ "$secs" =~ ^[0-9]+$ ]] || secs=15
...
_cb_allow_dispatch_with_rest_fallback() {
	...
	if [[ "${MAESTRO_PULSE_DISPATCH_REST_FALLBACK:-1}" != "1" ]]; then
		return 1
	fi
	...
	min_core="${MAESTRO_PULSE_REST_DISPATCH_MIN_CORE_REMAINING:-250}"
	...
	export MAESTRO_PULSE_DISPATCH_REST_FALLBACK_ACTIVE=1
	export MAESTRO_PULSE_DISPATCH_REST_FALLBACK_CORE_REMAINING="$core_remaining"
...
is_graphql_budget_sufficient() {
	# Emergency bypass.
	if [[ "${MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER:-0}" == "1" ]]; then
		echo "${_CB_RL_LOG_PREFIX} MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER=1 — bypassing rate-limit check" >>"$LOGFILE"
		return 0
	fi

	local threshold="${MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD:-0.05}"
...
# Used by `maestro status` to surface breaker state.
#
...
_circuit_breaker_status() {
	...
	if [[ "${MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER:-0}" == "1" ]]; then
		printf 'BYPASSED: MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER=1\n'
		return 0
	fi
...
#   - queued > MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN (default 50)
#   - queued / max(in_progress, 1) > MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN (default 10)
#
...
#
# Bypass:
#   MAESTRO_SKIP_ACTIONS_QUEUE_SATURATION=1 — return saturated=0 unconditionally
#   QUEUED_MIN=0                              — disable check via threshold
#######################################
_check_actions_queue_saturation() {
	local repo_slug="$1"
	local queued_min="${MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN:-50}"
	local ratio_min="${MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN:-10}"
	...
	# Emergency bypass.
	if [[ "${MAESTRO_SKIP_ACTIONS_QUEUE_SATURATION:-0}" == "1" ]]; then
		echo "${_CB_RL_LOG_PREFIX} MAESTRO_SKIP_ACTIONS_QUEUE_SATURATION=1 — bypassing actions queue check for ${repo_slug}" >>"$LOGFILE"
		printf 'queued=0\nin_progress=0\nratio=0\nsaturated=0\n'
		return 0
	fi
...
_main() {
	...
	case "$cmd" in
		...
		help | --help | -h)
			...
			echo "  MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD  fraction threshold (default 0.05 = 5%)"
			echo "  MAESTRO_SKIP_PULSE_CIRCUIT_BREAKER=1     emergency bypass"
			echo "  MAESTRO_PULSE_RATE_LIMIT_CACHE_TTL       rate_limit cache TTL seconds (default 20)"
			echo ""
			echo "Environment (Actions queue, t3211):"
			echo "  MAESTRO_ACTIONS_QUEUE_SATURATION_QUEUED_MIN  min queued runs (default 50; 0 disables)"
			echo "  MAESTRO_ACTIONS_QUEUE_SATURATION_RATIO_MIN   min queued/in_progress ratio (default 10)"
			echo "  MAESTRO_SKIP_ACTIONS_QUEUE_SATURATION=1      emergency bypass"
			return 0
			;;
		*)
