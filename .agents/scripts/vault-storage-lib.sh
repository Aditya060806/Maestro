...
[[ -n "${_MAESTRO_VAULT_STORAGE_LIB_LOADED:-}" ]] && return 0
_MAESTRO_VAULT_STORAGE_LIB_LOADED=1

vault_storage_helper_path() {
	local helper="${MAESTRO_VAULT_HELPER:-}"
	if [[ -n "$helper" ]]; then
		printf '%s\n' "$helper"
		return 0
	fi
...
vault_storage_dir() {
	local configured="${MAESTRO_VAULT_DIR:-}"
	if [[ -n "$configured" ]]; then
		printf '%s\n' "$configured"
		return 0
	fi
	printf '%s\n' "${XDG_CONFIG_HOME:-${HOME:-}/.config}/maestro/vault"
	return 0
}
...
vault_storage_active() {
	...
	if [[ "${MAESTRO_VAULT_REQUIRE:-0}" == "1" ]]; then
		return 0
	fi
...
vault_storage_require_unlocked() {
	...
	if [[ "$status_rc" -ne 0 || "$status_output" != "unlocked" ]]; then
		printf '%s\n' "VAULT_LOCKED: ${collection} requires an unlocked maestro Vault" >&2
		return 6
	fi
	return 0
}
