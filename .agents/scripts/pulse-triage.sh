...
_issue_needs_consolidation() {
	...
	substantive_count=$(printf '%s' "$comments_json" | jq -s --argjson min "$min_chars" '
		...
		(
			...
			+ "|^<!-- (nmr-hold|maestro-signed|ops:start|provenance:start)"
			+ "|^<!-- WORKER_SUPERSEDED"
			+ "|^<!-- stale-recovery-tick"
...
_dispatch_issue_consolidation() {
	...
	mkdir -p "${HOME}/.maestro/cache" 2>/dev/null || true
	touch "${HOME}/.maestro/cache/pulse-cycle-$$-consolidation-fired" 2>/dev/null || true
