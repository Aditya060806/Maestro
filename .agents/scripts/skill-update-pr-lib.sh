...
# Get the repo root (must be run from within the maestro repo)
get_repo_root() {
...
_push_and_create_pr() {
	...
	if ! command -v gh &>/dev/null; then
		...
		log_info "Create PR manually: gh pr create --head $branch_name" # maestro-allow: raw-gh-wrapper
		return 0
	fi
	...
	local sig_footer=""
	sig_footer=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$pr_body" 2>/dev/null || true)
	...
	pr_create_output=$(gh_create_pr \
		...
		log_info "Branch is pushed — create PR manually: gh pr create --head $branch_name" # maestro-allow: raw-gh-wrapper
		return 1
	}
