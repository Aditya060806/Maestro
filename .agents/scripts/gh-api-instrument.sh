...
# a JSON report at ~/.maestro/logs/gh-api-calls-by-stage.json so heavy
...
#
# Override env vars:
#   MAESTRO_GH_API_LOG          — path to the log file (default
#                                   ~/.maestro/logs/gh-api-calls.log)
#   MAESTRO_GH_API_REPORT       — path to the JSON report (default
#                                   ~/.maestro/logs/gh-api-calls-by-stage.json)
#   MAESTRO_GH_API_LOG_MAX_LINES — when set and exceeded, trim() retains
#                                   the most-recent half (default 50000)
#   MAESTRO_GH_API_INSTRUMENT_DISABLE=1 — make all calls no-ops
#
...
if [[ -z "$_GH_API_HOME" ]]; then
	...
	_GH_API_HOME="${_GH_API_HOME:-${TMPDIR:-/tmp}/maestro-${USER:-${_GH_API_UID:-shared}}}"
fi
...
# Otherwise a pre-created /tmp/maestro-* tree can contain symlinks that make the
# later append follow attacker-controlled paths.
case "$_GH_API_HOME" in
"${TMPDIR:-/tmp}"/* | /tmp/*)
	if [[ -L "$_GH_API_HOME" ]]; then
		MAESTRO_GH_API_INSTRUMENT_DISABLE=1
	elif [[ ! -e "$_GH_API_HOME" ]]; then
		mkdir -p "$_GH_API_HOME" 2>/dev/null || MAESTRO_GH_API_INSTRUMENT_DISABLE=1
	fi
	if [[ -e "$_GH_API_HOME" && "${MAESTRO_GH_API_INSTRUMENT_DISABLE:-0}" != "1" ]]; then
...
GH_API_LOG="${MAESTRO_GH_API_LOG:-${_GH_API_HOME}/.maestro/logs/gh-api-calls.log}"
GH_API_REPORT="${MAESTRO_GH_API_REPORT:-${_GH_API_HOME}/.maestro/logs/gh-api-calls-by-stage.json}"
GH_API_LOG_MAX_LINES="${MAESTRO_GH_API_LOG_MAX_LINES:-50000}"
...
# Returns: 0 always.
gh_record_call() {
	[[ "${MAESTRO_GH_API_INSTRUMENT_DISABLE:-0}" == "1" ]] && return 0
	local path="${1:-other}"
	local caller="${2:-}"
	local auth_mode="${3:-${MAESTRO_GH_AUTH_MODE:-}}"
	local api_pool="${4:-${MAESTRO_GH_API_POOL:-}}"
	local route_decision="${5:-${MAESTRO_GH_ROUTE_DECISION:-}}"
	local budget_remaining="${6:-${MAESTRO_GH_BUDGET_REMAINING:-${_GH_LAST_GRAPHQL_REMAINING:-}}}"
	if [[ -z "$caller" ]]; then
	...
	# MAESTRO_GH_API_LOG="gh.log" would expand ${GH_API_LOG%/*} to "gh.log").
...
Subcommands:
  ...
  trim                     Rotate log if larger than \$MAESTRO_GH_API_LOG_MAX_LINES
