...
ORIG_HOME="$HOME"
export HOME="$TMP"
mkdir -p "$HOME/.maestro/configs"
cp "$EXTRA_FILE" "$HOME/.maestro/configs/privacy-guard-extra-slugs.txt"
...
# entry for a slug we KNOW is public (our own maestro repo), set TTL=1, and
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
	_seed_cache "Aditya060806/Maestro" true 9999
	PRIVACY_CACHE_TTL=1 privacy_is_target_public "git@github.com:Aditya060806/Maestro.git"
	rc=$?
	# Live maestro repo is public → fresh probe should return 0, overriding the stale private entry
	if [[ "$rc" -eq 0 ]]; then
