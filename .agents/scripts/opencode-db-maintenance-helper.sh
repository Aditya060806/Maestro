...
readonly STATE_DIR="${HOME}/.maestro/.agent-workspace/work/opencode-maintenance"
...
readonly LAUNCHD_LABEL="sh.maestro.opencode-db-maintenance"
readonly LAUNCHD_DIR="${HOME}/Library/LaunchAgents"
readonly LAUNCHD_PLIST="${LAUNCHD_DIR}/${LAUNCHD_LABEL}.plist"
readonly SCHEDULER_LOG_DIR="${HOME}/.maestro/.agent-workspace/logs"
...
# -----------------------------------------------------------------------------
# Explicitly disruptive run for off-hours windows: stop maestro-managed pulse
...
# Prefer the deployed copy at ~/.maestro/agents/scripts/ so the LaunchAgent
# survives repo moves. Fall back to the running script's dir when the deployed
# copy is missing (first-install from a checkout).
_resolve_self_path() {
	local deployed="${HOME}/.maestro/agents/scripts/opencode-db-maintenance-helper.sh"
	if [[ -f "$deployed" ]]; then
		printf '%s' "$deployed"
	else
...
$([[ "$scheduler_subcommand" == "$MAINTENANCE_WINDOW_MODE" ]] && printf '\t\t<string>--force-opencode</string>\n')
	...
	<dict>
		<key>PATH</key>
		<string>$(maestro_launchd_sanitized_path "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin")</string>
		<key>HOME</key>
		<string>${HOME}</string>
	</dict>
	<key>RunAtLoad</key>
...
cmd_status() {
	...
	if command -v systemctl >/dev/null 2>&1 &&
		systemctl --user is-enabled maestro-opencode-db-maintenance.timer >/dev/null 2>&1; then
		print_success "Linux systemd timer enabled: maestro-opencode-db-maintenance.timer"
		installed=true
	fi

	if command -v crontab >/dev/null 2>&1 &&
		crontab -l 2>/dev/null | grep -qF "maestro: opencode-db-maintenance"; then
...
cmd_notice() {
	...
	if [[ "$due" == true ]]; then
		printf '[OPENCODE MAINTENANCE] Recommended: %s. Run maestro opencode-db maintenance-window off-hours; pulse/headless workers pause during the window.\n' "$reason"
	elif [[ "$compact_notice" == true ]]; then
		printf '[OPENCODE MAINTENANCE] Compact but large: DB %sMB >= %sMB, WAL is low, and free pages are below threshold after recent successful maintenance. Re-running maintenance-window is unlikely to reclaim space; run maestro opencode-db report for retained-data details.\n' \
