...
gh_create_issue() {
	...
	if ! _gh_wrapper_args_have_assignee "$@"; then
		if _gh_wrapper_args_have_label "auto-dispatch" "$@"; then
			...
			if [[ -n "$auto_assignee" ]]; then
				issue_output=$("${_issue_cmd[@]}" --assignee "$auto_assignee") # maestro-allow: raw-gh-wrapper
				rc=$?
	...
	issue_output=$("${_issue_cmd[@]}") # maestro-allow: raw-gh-wrapper
	rc=$?
	if [[ $rc -ne 0 ]] && _rest_should_fallback; then
		print_info "[INFO] gh-wrapper: GraphQL exhausted, falling back to REST for issue create"
...
_gh_create_pr_should_default_draft() {
	...
	if [[ "${MAESTRO_PR_CREATE_READY:-0}" == "1" ]]; then
		return 1
	fi
...
gh_create_pr() {
	...
	local pr_output rc
	pr_output=$("${pr_cmd[@]}") # maestro-allow: raw-gh-wrapper
	rc=$?
	if [[ $rc -ne 0 ]] && _rest_should_fallback; then
...
# <!-- maestro:sig --> marker, so callers that build their own footer
