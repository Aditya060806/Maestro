...
_gather_worker_zero_diagnostics() {
	...
	local metrics_file="${HOME}/.maestro/logs/headless-runtime-metrics.jsonl"
	local dispatch_stages_file="${HOME}/.maestro/logs/dispatch-stages.tsv"
	local pulse_log="${LOGFILE:-${HOME}/.maestro/logs/pulse.log}"
...
_gather_health_stats() {
	...
	local max_workers="?"
	local max_workers_file="${HOME}/.maestro/logs/pulse-max-workers"
...
_gather_activity_stats_for_repo() {
	local repo_path="$1"
	local activity_helper="${HOME}/.maestro/agents/scripts/contributor-activity-helper.sh"
