...
Set MAESTRO_VAULT_MANAGED_SESSION_HISTORY=1 to require the Vault boundary.
...
vault_history_enabled() {
	[[ "${MAESTRO_VAULT_MANAGED_SESSION_HISTORY:-}" == "1" || "${MAESTRO_VAULT_MANAGED_SESSION_HISTORY:-}" == "true" ]]
	return $?
}

vault_status() {
	local helper="${MAESTRO_VAULT_HELPER:-${SCRIPT_DIR}/vault-helper.sh}"
	if [[ -n "${MAESTRO_VAULT_STATUS_OVERRIDE:-}" ]]; then
		printf '%s\n' "$MAESTRO_VAULT_STATUS_OVERRIDE"
		return 0
	fi
	if [[ ! -x "$helper" ]]; then
		printf '%s\n' "missing"
