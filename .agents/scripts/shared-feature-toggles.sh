...
#   1. Defaults file (shipped with maestro, overwritten on update)
#      ~/.maestro/agents/configs/maestro.defaults.jsonc
#   2. User overrides (~/.config/maestro/config.jsonc)
...
FEATURE_TOGGLES_HOME="${HOME:-/tmp/maestro-uid-${UID:-$(id -u || echo 'unknown')}}"
FEATURE_TOGGLES_DEFAULTS="${FEATURE_TOGGLES_HOME}/.maestro/agents/configs/feature-toggles.conf.defaults"
FEATURE_TOGGLES_USER="${FEATURE_TOGGLES_HOME}/.config/maestro/feature-toggles.conf"
...
_ft_env_map() {
	local key="$1"
	case "$key" in
	auto_update) echo "MAESTRO_AUTO_UPDATE" ;;
	update_interval) echo "MAESTRO_UPDATE_INTERVAL" ;;
	skill_auto_update) echo "MAESTRO_SKILL_AUTO_UPDATE" ;;
	skill_freshness_hours) echo "MAESTRO_SKILL_FRESHNESS_HOURS" ;;
	tool_auto_update) echo "MAESTRO_TOOL_AUTO_UPDATE" ;;
	tool_freshness_hours) echo "MAESTRO_TOOL_FRESHNESS_HOURS" ;;
	tool_idle_hours) echo "MAESTRO_TOOL_IDLE_HOURS" ;;
	supervisor_pulse) echo "MAESTRO_SUPERVISOR_PULSE" ;;
	repo_sync) echo "MAESTRO_REPO_SYNC" ;;
	repo_maestro_health) echo "MAESTRO_REPO_HEALTH" ;;
	openclaw_auto_update) echo "MAESTRO_OPENCLAW_AUTO_UPDATE" ;;
	openclaw_freshness_hours) echo "MAESTRO_OPENCLAW_FRESHNESS_HOURS" ;;
	upstream_watch) echo "MAESTRO_UPSTREAM_WATCH" ;;
	upstream_watch_hours) echo "MAESTRO_UPSTREAM_WATCH_HOURS" ;;
	max_interactive_sessions) echo "MAESTRO_MAX_SESSIONS" ;;
	*) echo "" ;;
	esac
	return 0
}
...
_load_config() {
	...
	local jsonc_defaults="${JSONC_DEFAULTS:-${HOME}/.maestro/agents/configs/maestro.defaults.jsonc}"
	if command -v jq &>/dev/null && [[ -r "$jsonc_defaults" ]] &&
		type config_get &>/dev/null && type config_enabled &>/dev/null; then
		_MAESTRO_CONFIG_MODE="$CONFIG_MODE_JSONC"
		...
		local jsonc_user="${JSONC_USER:-${HOME}/.config/maestro/config.jsonc}"
		if [[ -f "$FEATURE_TOGGLES_USER" && ! -f "$jsonc_user" ]]; then
			if type _migrate_conf_to_jsonc &>/dev/null; then
				if ! _migrate_conf_to_jsonc; then
					echo "[WARN] Auto-migration from legacy config failed. Run 'maestro config migrate' manually." >&2
				fi
			fi
		fi
	else
		_MAESTRO_CONFIG_MODE="$CONFIG_MODE_LEGACY"
		_load_feature_toggles_legacy
	fi
...
get_feature_toggle() {
	...
	if [[ "$_MAESTRO_CONFIG_MODE" == "$CONFIG_MODE_JSONC" ]]; then
		local dotpath
...
# shared-constants.sh). Ensures _MAESTRO_CONFIG_MODE and _FT_* variables
