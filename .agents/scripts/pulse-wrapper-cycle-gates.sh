...
_pulse_available_auto_dispatch_work_exists() {
	[[ "${MAESTRO_SKIP_PULSE_IDLE_AVAILABLE_WORK_CHECK:-0}" == "1" ]] && return 1
	local _slug=""
	while IFS= read -r _slug; do
		[[ -n "$_slug" ]] || continue
		local _count=""
...
_pulse_check_idle_backoff_gate() {
	...
	local _ib_ts_file="${HOME}/.maestro/logs/pulse-wrapper-last-run.ts"
	local _ib_last=0
	...
	if MAESTRO_PULSE_IDLE_AVAILABLE_WORK="$_ib_available_work" "$_ib_helper" should-skip "$_ib_last" >/dev/null 2>&1; then
...
#
# Sentinel: ~/.maestro/cache/pulse-fix-the-fixer-last-run (mtime).
# Default cadence: 3600s (1 hour). Override via env:
#   MAESTRO_PULSE_FIX_THE_FIXER_MAX_AGE  — staleness threshold in seconds
#   MAESTRO_SKIP_FIX_THE_FIXER_DETECTOR  — set to 1 to short-circuit
#
...
_pulse_run_fix_the_fixer_detector_if_stale() {
	[[ "${MAESTRO_SKIP_FIX_THE_FIXER_DETECTOR:-0}" == "1" ]] && return 0

	local _ftf_helper=""
	local _ftf_sentinel=""
	local _ftf_max_age=""
	_ftf_helper="${SCRIPT_DIR}/pulse-fix-the-fixer-detector.sh"
	_ftf_sentinel="${HOME}/.maestro/cache/pulse-fix-the-fixer-last-run"
	_ftf_max_age="${MAESTRO_PULSE_FIX_THE_FIXER_MAX_AGE:-3600}"
