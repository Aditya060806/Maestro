...
#
# State file: ~/.maestro/cache/foss-wp-handler.json
...
readonly WP_HANDLER_STATE="${HOME}/.maestro/cache/foss-wp-handler.json"
...
readonly WP_ENV_CONFIG_FILE=".wp-env.maestro.json"
...
# Generate .wp-env.maestro.json with multisite config — writes into the plugin directory.
...
'
	...
	if (cd "$plugin_dir" && wp-env run cli bash -c \
		"mkdir -p /var/www/html/wp-content/mu-plugins && cat > /var/www/html/wp-content/mu-plugins/maestro-proxy-headers.php << 'MUPLUGIN_EOF'
${muplugin_php}
...
cmd_help() {
	printf '%s\n' \
		...
		'      Fork, clone, generate .wp-env.maestro.json (multisite), start wp-env,' \
		...
		'' \
		'STATE FILE' \
		'  ~/.maestro/cache/foss-wp-handler.json' \
		'' \
		'SMOKE TEST TEMPLATE' \
