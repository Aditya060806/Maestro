...
- \`sudo maestro approve issue <number> ${repo_slug}\` -- cryptographically signs approval for automated dispatch
- Comment \`declined: <reason>\` -- closes this issue (include your reason after the colon)"
	...
	# (MAESTRO_SIG_MODEL, MAESTRO_SIG_CLI, etc. set by calling LLM session).
	...
	if [[ -n "${MAESTRO_SIG_MODEL:-}" ]]; then
		sig_args+=(--model "$MAESTRO_SIG_MODEL")
	fi
	qf_sig=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" "${sig_args[@]}" 2>/dev/null || true)
...
\`\`\`
sudo maestro approve issue ${issue_num} ${repo_slug}
\`\`\`
...
\`\`\`
sudo maestro approve setup
\`\`\`
This creates a root-protected ED25519 signing key. Check status anytime with \`maestro approve status\`.
</details>
...
<!-- provenance:end -->"
	...
	if [[ -n "${MAESTRO_SIG_MODEL:-}" ]]; then
		append_sig_args+=(--model "$MAESTRO_SIG_MODEL")
	fi
	append_sig=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" "${append_sig_args[@]}" 2>/dev/null || true)
...
#
# Environment:
#   REPOS_JSON — path to repos.json (defaults to ~/.config/maestro/repos.json)
_is_maintainer_equivalent_author() {
	...
	local repos_json="${REPOS_JSON:-${HOME}/.config/maestro/repos.json}"
	local maintainer=""
	...
	# #maestro:trust-boundary — maintainer-equivalent classification requires
	...
	_gh_collaborator_permission_lookup "$repo_slug" "$pr_author" collab_permission || {
		echo "[quality-feedback] permission check failed for author ${pr_author} on ${repo_slug} (HTTP ${MAESTRO_GH_COLLAB_PERMISSION_HTTP:-unknown}) — not treating as maintainer-equivalent" >&2
		return 1
	}
