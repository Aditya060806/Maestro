...
get_api_key() {
	...
	if command -v gopass &>/dev/null; then
		local key
		key=$(gopass show -o "maestro/smartlead-api-key" 2>/dev/null) || true
		if [[ -n "$key" ]]; then
			printf '%s' "$key"
			return 0
		fi
	...
	log_error "Smartlead API key not found. Set SMARTLEAD_API_KEY or run: maestro secret set smartlead-api-key"
	return 1
}
...
AUTHENTICATION:
  Set SMARTLEAD_API_KEY environment variable, or store via:
    maestro secret set smartlead-api-key
