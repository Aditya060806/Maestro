...
# Format: [MAESTRO-MAIL] v1|id|from|to|type|priority|convoy|payload
...
transport_simplex_send() {
	...
	log_error "SimpleX transport: no group or contact configured (set MAESTRO_SIMPLEX_MAIL_GROUP or MAESTRO_SIMPLEX_MAIL_CONTACT)"
	return 1
}
...
transport_matrix_send() {
	...
	if [[ -z "$MATRIX_MAIL_ROOM" ]]; then
		log_error "Matrix transport: no room configured (set MAESTRO_MATRIX_MAIL_ROOM)"
		return 1
	fi
