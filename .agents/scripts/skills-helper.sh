...
_is_search_stopword() {
	...
	a | an | and | are | as | at | be | by | can | do | does | for | from | have | how | i | in | into | is | it | my | of | on | or | our | the | their | this | to | use | using | what | with | you | your)
		return 0
		;;
	agent | agents | maestro | capability | capabilities | external | framework | model | models | review | reviews | tool | tools)
		return 0
		;;
	esac
	return 1
...
_describe_print_header() {
	...
	if [[ "$filename" == *-skill ]]; then
		...
		echo -e "  ${CYAN}Type:${NC}        native (maestro built-in)"
	fi
	return 0
}
...
cmd_search_registry() {
	...
	if [[ "$pkg_count" -gt 0 ]]; then
		...
		echo -e "       Or:         maestro skills install <owner/repo@skill>"
	fi
...
cmd_install() {
	...
	if [[ $exit_code -eq 0 ]]; then
		...
		echo ""
		echo -e "  ${CYAN}Tip:${NC} Run 'maestro update' to deploy to ~/.maestro/agents/"
	else
