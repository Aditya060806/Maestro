...
LOGFILE="${ROUTINE_COMMENT_LOGFILE:-${HOME}/.maestro/.agent-workspace/cron/routine-comments/responder.log}"
STATE_DIR="${ROUTINE_COMMENT_STATE_DIR:-${HOME}/.maestro/.agent-workspace/cron/routine-comments}"
mkdir -p "$STATE_DIR"
...
_routine_ops_comment_regex() {
	printf '%s\n' '^(DISPATCH_CLAIM|DISPATCH_RELEASED|CLAIM_RELEASED|<!-- ops:|<!-- routine-description|<!-- maestro:sig|## (Cascade Tier Escalation|BLOCKED|Closing|Routine|Worker|Dispatch|Audit|MERGE_SUMMARY)|### (Worker|Audit)|\[maestro\])'
	return 0
}
...
1. If it's a QUESTION about the routine: answer using the issue body description,
   routine logs at ~/.maestro/.agent-workspace/cron/<routine-id>/, and
...
2. If it's a CHANGE REQUEST (e.g. change schedule, disable, enable):
   ...
   - If the change requires modifying framework code, explain that and suggest filing
     an issue on the main maestro repo instead

3. If it's a BUG REPORT about the routine itself:
   - Post a comment acknowledging the report
   - Create an issue on the main maestro repo with the bug details
