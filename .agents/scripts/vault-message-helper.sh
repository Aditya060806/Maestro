...
GIT_TRANSPORT_HELPER="${MAESTRO_VAULT_GIT_TRANSPORT_HELPER:-${SCRIPT_DIR}/vault-git-transport-helper.sh}"
...
Commands:
  ...
  receive --repo DIR [--vault-dir DIR] [--revoked-devices FILE]
      Collect encrypted messages. Decrypt only when MAESTRO_VAULT_MESSAGE_UNLOCKED=1.
...
AAD = b"maestro-vault-device-message-v1"
FIELD_BODY = "body"
FIELD_CLASS = "class"
...
def ensure_transport_available(transport: str) -> None:
    ...
    if transport == "simplex":
        adapter = os.environ.get("MAESTRO_VAULT_SIMPLEX_ADAPTER")
...
def cmd_send(args: argparse.Namespace) -> int:
    ...
    if args.transport == "git":
        ...
        subprocess.run([os.environ["MAESTRO_VAULT_SIMPLEX_ADAPTER"], "send", str(message_file)], check=True)
...
def local_unlocked() -> bool:
    return os.environ.get("MAESTRO_VAULT_MESSAGE_UNLOCKED") == "1"
