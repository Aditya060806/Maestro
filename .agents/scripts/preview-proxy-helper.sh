...
#
# State: ~/.maestro/state/worktree-ports.json
# Config: ~/.config/maestro/preview-proxy.json (optional)
#
# Design: GH#21560
# =============================================================================
...
# Constants
# -----------------------------------------------------------------------------
readonly PP_STATE_DIR="${HOME}/.maestro/state"
readonly PP_STATE_FILE="${PP_STATE_DIR}/worktree-ports.json"
readonly PP_CONFIG_FILE="${HOME}/.config/maestro/preview-proxy.json"
...
# Build the start hint from .maestro.json in the worktree (if present).
_pp_build_start_hint() {
    local repo_slug="$1"
    local port="$2"

    # Try to find the project's .maestro.json
    local repo_root
    repo_root="$(git rev-parse --show-toplevel 2>/dev/null)" || repo_root=""

    if [[ -n "$repo_root" ]] && [[ -f "${repo_root}/.maestro.json" ]] && command -v jq >/dev/null 2>&1; then
        local cmd port_env
        cmd="$(jq -r '.preview.command // empty' "${repo_root}/.maestro.json" 2>/dev/null)" || cmd=""
        port_env="$(jq -r '.preview.port_env // empty' "${repo_root}/.maestro.json" 2>/dev/null)" || port_env=""

        if [[ -n "$cmd" ]]; then
            if [[ -n "$port_env" ]]; then
                echo "${port_env}=${port} ${cmd}"
            else
                echo "MAESTRO_PREVIEW_PORT=${port} ${cmd}"
            fi
            return 0
        fi
    fi
    ...
    # Generic fallback
    echo "MAESTRO_PREVIEW_PORT=${port} <dev-command>"
    return 0
}
