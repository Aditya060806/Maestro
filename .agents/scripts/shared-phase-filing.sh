...
#
# Feature flag:
#   MAESTRO_SEQUENTIAL_PHASE_AUTOFILE=0|1 (default 1, ON since t2787)
...
# Override: MAESTRO_SEQUENTIAL_PHASE_AUTOFILE=0 to disable.
MAESTRO_SEQUENTIAL_PHASE_AUTOFILE="${MAESTRO_SEQUENTIAL_PHASE_AUTOFILE:-1}"
...
block while preserving the parent as the tracker."
	...
	cat <<MD
<!-- maestro:generator=phase-autofile parent=${parent_issue} phase=${phase_num} -->
...
_phase_acquire_filing_lock() {
	...
	local parent_issue="$2"
	local phase_num="$3"
	local lock_root="${MAESTRO_PHASE_FILING_LOCK_ROOT:-${HOME}/.maestro/locks/phase-filing}"
	local lock_key
...
_create_phase_child_issue() {
	...
	local _phase_sig=""
	local _sig_helper="${AGENTS_DIR:-${HOME}/.maestro/agents}/scripts/gh-signature-helper.sh"
...
auto_file_next_unfiled_parent_phase() {
	...
	[[ "${MAESTRO_SEQUENTIAL_PHASE_AUTOFILE:-0}" == "1" ]] || return 1
...
#
# Guards:
#   1. Feature flag MAESTRO_SEQUENTIAL_PHASE_AUTOFILE must be 1
...
auto_file_next_phase() {
	...
	if [[ "${MAESTRO_SEQUENTIAL_PHASE_AUTOFILE:-0}" != "1" ]]; then
		return 0
	fi
