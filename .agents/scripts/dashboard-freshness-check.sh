...
LOGFILE="${HOME}/.maestro/logs/dashboard-freshness.log"
STATE_DIR="${HOME}/.maestro/cache/dashboard-freshness"
LAST_RUN_FILE="${STATE_DIR}/last-scan"
HEALTH_ISSUE_CACHE_DIR="${HOME}/.maestro/logs"
...
# ~/.maestro/logs/health-issue-* cache files written by stats-health-dashboard.sh,
...
1. Inspect the stats scheduler status.
   ...
   \`\`\`bash
   ${macos_launchctl} list | grep -i maestro-stats-wrapper
   tail -40 ~/.maestro/logs/stats.log
   ls -la ~/Library/LaunchAgents/com.maestro.maestro-stats-wrapper.plist
   \`\`\`

   Linux / systemd user timers:

   \`\`\`bash
   ${linux_systemctl} --user status maestro-stats-wrapper.timer --no-pager
   ${linux_systemctl} --user status maestro-stats-wrapper.service --no-pager
   ${linux_systemctl} --user list-timers --all --no-pager 'maestro*'
   tail -40 ~/.maestro/logs/stats.log
   ls -la ~/.config/systemd/user/maestro-stats-wrapper.*
   \`\`\`

2. Run the refresh manually and capture the error:

   \`\`\`bash
   bash ~/.maestro/agents/scripts/stats-wrapper.sh 2>&1 | tail -40
   \`\`\`
...
- **macOS missing plist:** re-run \`setup.sh --non-interactive\` (or \`maestro update\`) with PULSE_ENABLED=true so \`setup_stats_wrapper\` reinstalls \`com.maestro.maestro-stats-wrapper.plist\`.
- **Linux missing or inactive systemd timer:** re-run \`setup.sh --non-interactive\` (or \`maestro update\`) with PULSE_ENABLED=true so the Linux scheduler setup reinstalls and enables \`maestro-stats-wrapper.timer\` and \`maestro-stats-wrapper.service\` under \`~/.config/systemd/user\`.
...
_file_stale_alert() {
	...
	local marker="<!-- maestro:dashboard-freshness:${slug}:${dash_issue} -->"
	local generator_marker="<!-- maestro:generator=dashboard-freshness-check -->"
...
State:
  ~/.maestro/cache/dashboard-freshness/last-scan
  ~/.maestro/logs/dashboard-freshness.log
USAGE
}
