...
: "${MAESTRO_GH_SECONDARY_COOLDOWN_SECS:=300}"
: "${MAESTRO_GH_SECONDARY_COOLDOWN_HOME:=${HOME:-/tmp/.maestro-${USER:-uid-${UID:-unknown}}}}"
: "${MAESTRO_GH_SECONDARY_COOLDOWN_FILE:=${MAESTRO_GH_SECONDARY_COOLDOWN_HOME}/.maestro/cache/gh-secondary-cooldown.json}"
: "${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE:=${MAESTRO_GH_SECONDARY_COOLDOWN_HOME}/.maestro/cache/gh-cooldown-events.jsonl}"
: "${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_LINES:=100}"
: "${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_BYTES:=262144}"
: "${MAESTRO_GH_READ_RAMP_ENABLED:=1}"
: "${MAESTRO_GH_READ_RAMP_BOOT_SECS:=180}"
: "${MAESTRO_GH_READ_RAMP_RECOVERY_SECS:=300}"
: "${MAESTRO_GH_READ_RAMP_BUDGET:=60}"
: "${MAESTRO_GH_READ_RAMP_STATE_FILE:=${MAESTRO_GH_SECONDARY_COOLDOWN_HOME}/.maestro/cache/gh-read-ramp-state.tsv}"
...
_gh_secondary_cooldown_file() {
	printf '%s' "${MAESTRO_GH_SECONDARY_COOLDOWN_FILE}"
	return 0
}

_gh_secondary_cooldown_event_file() {
	printf '%s' "${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_FILE}"
	return 0
}
...
_gh_secondary_cooldown_sanitized_endpoint() {
	local endpoint="${1:-${MAESTRO_GH_COOLDOWN_ENDPOINT:-$_GH_SECONDARY_COOLDOWN_UNKNOWN}}"
	local rest=""
	local suffix=""
...
_gh_secondary_cooldown_sanitized_query_shape() {
	local query="${1:-}"
	local endpoint="${2:-${MAESTRO_GH_COOLDOWN_ENDPOINT:-}}"
	local rest=""
	local part=""
	local key=""
	local shape=""
...
_gh_secondary_cooldown_method() {
	local method="${1:-${MAESTRO_GH_COOLDOWN_METHOD:-unknown}}"
...
_gh_secondary_cooldown_auth_mode() {
	local auth_mode="${MAESTRO_GH_AUTH_MODE:-}"
...
_gh_secondary_cooldown_auth_principal() {
	local principal="${1:-${MAESTRO_GH_AUTH_PRINCIPAL:-}}"
	local auth_mode="${2:-${MAESTRO_GH_AUTH_MODE:-}}"
	...
	elif [[ -z "$principal" && -n "${MAESTRO_SESSION_USER:-}" ]]; then
		principal="user:${MAESTRO_SESSION_USER}"
	fi
...
_gh_secondary_cooldown_endpoint_family() {
	local context_endpoint="${1:-${MAESTRO_GH_COOLDOWN_ENDPOINT:-}}"
	local endpoint="${MAESTRO_GH_COOLDOWN_ENDPOINT_FAMILY:-${MAESTRO_GH_API_POOL:-${MAESTRO_GH_ROUTE_DECISION:-unknown}}}"
...
_gh_secondary_cooldown_trim_events() {
	local file="$1"
	local max_lines="${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_LINES:-100}"
	local max_bytes="${MAESTRO_GH_SECONDARY_COOLDOWN_EVENTS_MAX_BYTES:-262144}"
	local line_count="0"
	local byte_count="0"
	local keep_lines="0"
	[[ -f "$file" ]] || return 0
...
_gh_secondary_cooldown_record_event() {
	...
	operation_source="${operation_arg:-${MAESTRO_GH_COOLDOWN_OPERATION:-$_GH_SECONDARY_COOLDOWN_UNKNOWN}}"
	wrapper_source="${wrapper_arg:-${MAESTRO_GH_COOLDOWN_WRAPPER:-$(_gh_secondary_cooldown_caller_family)}}"
	pulse_stage_source="${pulse_stage_arg:-${MAESTRO_GH_COOLDOWN_STAGE:-$_GH_SECONDARY_COOLDOWN_UNKNOWN}}"
...
_gh_secondary_cooldown_write_until() {
	...
	if [[ "$expires_at" =~ ^[0-9]+$ && "$expires_at" -gt "$now" ]]; then
		...
		expires=$((now + MAESTRO_GH_SECONDARY_COOLDOWN_SECS))
	fi
	...
	query_shape="$(_gh_secondary_cooldown_sanitized_query_shape "$query_shape_arg" "$endpoint_arg")"
	operation_source="${operation_arg:-${MAESTRO_GH_COOLDOWN_OPERATION:-$_GH_SECONDARY_COOLDOWN_UNKNOWN}}"
	wrapper_source="${wrapper_arg:-${MAESTRO_GH_COOLDOWN_WRAPPER:-$caller_family}}"
	pulse_stage_source="${pulse_stage_arg:-${MAESTRO_GH_COOLDOWN_STAGE:-$_GH_SECONDARY_COOLDOWN_UNKNOWN}}"
	operation="$(_gh_secondary_cooldown_safe_family "$operation_source" 120)"
	wrapper="$(_gh_secondary_cooldown_safe_family "$wrapper_source" 120)"
	pulse_stage="$(_gh_secondary_cooldown_safe_family "$pulse_stage_source" 120)"
...
_gh_secondary_cooldown_header_expires_at() {
	...
	printf '%s' $((now + MAESTRO_GH_SECONDARY_COOLDOWN_SECS))
	return 0
}
...
_gh_secondary_read_ramp_phase() {
	...
	if [[ "$boot_ts" =~ ^[0-9]+$ && "$MAESTRO_GH_READ_RAMP_BOOT_SECS" =~ ^[0-9]+$ ]]; then
		if [[ "$now" -ge "$boot_ts" && $((now - boot_ts)) -lt "$MAESTRO_GH_READ_RAMP_BOOT_SECS" ]]; then
			printf 'boot'
			return 0
		fi
	fi
	expires="$(_gh_secondary_cooldown_expires_at 2>/dev/null || true)"
	if [[ "$expires" =~ ^[0-9]+$ && "$MAESTRO_GH_READ_RAMP_RECOVERY_SECS" =~ ^[0-9]+$ ]]; then
		if [[ "$now" -ge "$expires" && $((now - expires)) -lt "$MAESTRO_GH_READ_RAMP_RECOVERY_SECS" ]]; then
...
_gh_secondary_read_ramp_state_file() {
	printf '%s' "$MAESTRO_GH_READ_RAMP_STATE_FILE"
	return 0
}
...
_gh_secondary_read_ramp_take_token() {
	...
	local old_count=""
	local count=0
	[[ "$MAESTRO_GH_READ_RAMP_BUDGET" =~ ^[0-9]+$ && "$MAESTRO_GH_READ_RAMP_BUDGET" -gt 0 ]] || return 0
	now="$(_gh_secondary_cooldown_now)"
	minute=$((now / 60))
	...
	if [[ "$count" -ge "$MAESTRO_GH_READ_RAMP_BUDGET" ]]; then
...
_gh_secondary_read_ramp_log_deferred() {
	local phase="$1"
	local budget="$MAESTRO_GH_READ_RAMP_BUDGET"
...
_gh_secondary_read_ramp_preflight() {
	...
	[[ "${MAESTRO_GH_READ_RAMP_ENABLED:-1}" == "1" ]] || return 0
	[[ "$op_class" == "$_GH_SECONDARY_READ_OP" ]] || return 0
	[[ "${MAESTRO_GH_READ_RAMP_OVERRIDE:-0}" == "1" ]] && return 0
...
_gh_secondary_cooldown_preflight() {
	...
	if [[ "${MAESTRO_GH_SECONDARY_COOLDOWN_OVERRIDE:-0}" == "1" ]]; then
