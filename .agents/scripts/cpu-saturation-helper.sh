...
#
# Environment:
#   MAESTRO_CPU_SATURATION_STATE_DIR — override state directory (default
#       ~/.maestro/.agent-workspace/cpu-saturation).
#   MAESTRO_CPU_SATURATION_FAKE_PCT — for tests: bypass real sampling and
#       use this value as the current utilisation percent. Whitespace-
#       separated multiple values cycle through samples (one per call).
#   MAESTRO_CPU_SATURATION_FAKE_NOW — for tests: override `date +%s`.
...
CPU_SATURATION_STATE_DIR="${MAESTRO_CPU_SATURATION_STATE_DIR:-${HOME}/.maestro/.agent-workspace/cpu-saturation}"
...
_csh_now_epoch() {
	if [[ -n "${MAESTRO_CPU_SATURATION_FAKE_NOW:-}" ]]; then
		printf '%s\n' "${MAESTRO_CPU_SATURATION_FAKE_NOW}"
		return 0
	fi
	date +%s
	return 0
...
# Return current utilisation. Honours MAESTRO_CPU_SATURATION_FAKE_PCT for tests.
_csh_current_utilisation() {
	if [[ -n "${MAESTRO_CPU_SATURATION_FAKE_PCT:-}" ]]; then
		...
		if [[ ! -f "$queue_file" ]]; then
			printf '%s\n' "${MAESTRO_CPU_SATURATION_FAKE_PCT}" >"$queue_file"
		fi
		queue_state=$(cat "$queue_file" 2>/dev/null || printf '%s' "${MAESTRO_CPU_SATURATION_FAKE_PCT}")
		...
		if [[ "${#arr[@]}" -gt 1 ]]; then
			...
			remaining="${MAESTRO_CPU_SATURATION_FAKE_PCT}"
		fi
