...
# knowledge-helper.sh — Knowledge plane provisioning for maestro-managed repos
#
...
# knowledge mode from ~/.config/maestro/repos.json (field: "knowledge").
#
# Usage:
...
#   personal  Tree at ~/.maestro/.agent-workspace/knowledge/
#
# Directory contract:
...
#
# Blob threshold: files >=30MB go to ~/.maestro/.agent-workspace/knowledge-blobs/
...
REPOS_FILE="${REPOS_FILE:-${HOME}/.config/maestro/repos.json}"
PERSONAL_PLANE_BASE="${PERSONAL_PLANE_BASE:-${HOME}/.maestro/.agent-workspace/knowledge}"
...
_prompt_knowledge_mode() {
	...
	echo "  2) personal — knowledge lives at ~/.maestro/.agent-workspace/knowledge/"
...
cmd_status() {
	...
	repo)
		if _is_provisioned "$repo_path"; then
			...
			echo "  State: not provisioned (run: maestro knowledge init repo)"
		fi
		;;
	personal)
		if _is_provisioned "$PERSONAL_PLANE_BASE"; then
			...
			echo "  State: not provisioned (run: maestro knowledge init personal)"
		fi
		;;
	esac
	echo ""
...
_cmd_add_store_file() {
	...
	if [[ "$size_bytes" -ge "$BLOB_THRESHOLD_BYTES" ]]; then
		...
		local blob_dir="${HOME}/.maestro/.agent-workspace/knowledge-blobs/${repo_name}/${source_id}"
		mkdir -p "$blob_dir"
		local blob_path
