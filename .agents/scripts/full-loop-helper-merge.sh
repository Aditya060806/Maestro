...
_signal_admin_merge_fallback() {
	...
	local _sig_footer=""
	_sig_footer=$(gh-signature-helper.sh footer --model "${MAESTRO_MODEL:-unknown}" 2>/dev/null) || _sig_footer=""
...
_merge_is_headless_session() {
	case "${FULL_LOOP_HEADLESS:-}${MAESTRO_HEADLESS:-}${Claude_HEADLESS:-}${GITHUB_ACTIONS:-}" in
	*true* | *1*) return 0 ;;
	*) return 1 ;;
	esac
}
...
_merge_try_interactive_admin_auto_fallback() {
	...
	#maestro:trust-boundary -- interactive admin fallback still enforces linked NMR crypto gate before bypassing review-count protection.
...
_merge_guard_admin_merge_maintainer_review() {
	...
	#maestro:trust-boundary -- admin merge must not bypass signed/maintainer issue approval.
	if [[ -n "$blocked_issues" ]]; then
		print_error "Admin merge blocked: linked issue(s)${blocked_issues} still require maintainer review"
		print_info "Required action: run 'sudo maestro approve issue <issue_number> ${repo}' or record an equivalent maintainer decision before merging."
		return 1
	fi
