...
# Trusted maintainer key for maestro framework supply chain verification.
# This fingerprint is checked during `maestro update` to verify that pulled
...
readonly TRUSTED_KEY="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIMnXVft9/hT5P2dIICJMMmXeg6HUnKGCvR4VzkKpyJza marcus@Aditya060806.com"
readonly TRUSTED_EMAIL="6428977+Aditya060806@users.noreply.github.com"
...
cmd_check() {
	...
	if [[ "$gpg_format" == "ssh" && "$signing_key" != "not set" && "$commit_sign" == "true" ]]; then
		...
		echo ""
		echo "Run: maestro signing setup"
	fi
	...
	if [[ -f "$SSH_KEY_HEADLESS" ]]; then
		...
		if ssh-add -l 2>/dev/null | grep -qF "$SSH_KEY_HEADLESS"; then
			...
			_print_warn "Key not loaded in ssh-agent — run: maestro signing agent-start"
		fi
	else
		_print_warn "No headless signing key — run: maestro signing headless-setup"
	fi
	return 0
}
...
cmd_setup() {
	...
	# Also add the maestro trusted key so users can verify framework updates
	if ! grep -q "$TRUSTED_EMAIL" "$ALLOWED_SIGNERS_FILE" 2>/dev/null; then
		echo "$TRUSTED_EMAIL $TRUSTED_KEY" >>"$ALLOWED_SIGNERS_FILE"
		_print_ok "Added maestro maintainer key to allowed_signers"
	fi
...
cmd_headless_setup() {
	...
	if [[ -f "$SSH_KEY_HEADLESS" ]]; then
		...
		ssh-keygen -t ed25519 -C "maestro-headless-signing" -f "$SSH_KEY_HEADLESS" -N "" -q
	...
	echo "     maestro signing agent-start"
...
cmd_agent_start() {
	...
	if [[ ! -f "$SSH_KEY_HEADLESS" ]]; then
		...
		echo "Run: maestro signing headless-setup"
		return 1
	fi
...
cmd_verify_tag() {
	local tag="${1:-}"
	if [[ -z "$tag" ]]; then
		echo "Usage: maestro signing verify-tag <tag>"
		return 1
	fi
...
# Used by `maestro update` to verify supply chain integrity.
#
...
cmd_verify_update() {
	local repo_path="${1:-$HOME/Git/maestro}"
...
cmd_help() {
	...
	echo "  1. maestro signing headless-setup   # one-time setup (run in terminal)"
	echo "  2. maestro signing agent-start      # start agent (run before workers)"
