...
# t1984: uses MAESTRO_SESSION_USER when set (workflow env -> github.actor).
_push_auto_assign_interactive() {
	...
	local current_user="${MAESTRO_SESSION_USER:-}"
...
_push_create_issue() {
	...
	{
		combined=$(gh issue create "${args[@]}" 2>&1) # maestro-allow: raw-gh-wrapper
		gh_exit=$?
	} || true
	...
	if [[ -n "$num" ]]; then
		...
		local _lock_user="${MAESTRO_SESSION_USER:-}"
