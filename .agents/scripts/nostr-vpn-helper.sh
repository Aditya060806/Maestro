...
Commands:
  ...
  secrets-help     Show maestro secret setup guidance
...
FIPS/Nostr VPN privacy guidance:
  ...
  - Do not paste nsec/private keys into chat; store recovery material only with maestro secret storage.
...
Use maestro secret storage for sensitive values:
...
Guidance:
  ...
  - Pass secrets via environment or maestro secret execution context, not CLI arguments.
...
Secure OpenCode remote compute over Nostr VPN/FIPS:
  ...
  3. Store auth with: maestro secret set OPENCODE_SERVER_TOKEN
...
Useful maestro service candidates after SSH is proven:
