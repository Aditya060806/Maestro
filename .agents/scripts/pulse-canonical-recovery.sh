...
# (`~/.maestro/advisories/canonical-recovery-<basename>.advisory`) with
# the exact remediation commands the user can run. The local file is
# surfaced in the session-start greeting via maestro-update-check.sh —
...
# the local channel (`~/.maestro/advisories/`) and never touch GitHub.
...
PULSE_CANONICAL_RECOVERY_STATE="${PULSE_CANONICAL_RECOVERY_STATE:-${HOME}/.maestro/.agent-workspace/supervisor/canonical-recovery-state.json}"

# Local advisory directory — surfaced in session greeting via
# maestro-update-check.sh::_check_advisories.  This is the canonical channel
...
PULSE_CANONICAL_RECOVERY_ADVISORY_DIR="${PULSE_CANONICAL_RECOVERY_ADVISORY_DIR:-${HOME}/.maestro/advisories}"
...
# (`~/.maestro/advisories/*.advisory`) which `maestro-update-check.sh::
...
#
# Pre-t2871 behaviour filed GitHub issues against `Aditya060806/Maestro` with
...
[CANONICAL RECOVERY] ${repo_basename} stash conflict — manual intervention required
  ...
  Dismiss after fixing:  maestro security dismiss canonical-recovery-${repo_basename}
EOF
	return 0
}
...
# session greeting by `maestro-update-check.sh::_check_advisories` (line 1
# becomes the toast summary). Dismissable via
# `maestro security dismiss canonical-recovery-<basename>`.
_pcr_file_advisory() {
	local repo_path="$1"
	local failure_mode="$2"
	local repo_basename
...
Environment:
  ...
  PULSE_CANONICAL_RECOVERY_STATE               State file path
                                                (default: ~/.maestro/.agent-workspace/supervisor/canonical-recovery-state.json).
