...
# =============================================================================
# maestro Headless Runtime Library -- Stable Utility Functions (t2013)
# =============================================================================
...
# Logs to ~/.maestro/logs/worker-empty-results.log for pattern analysis.
...
_log_empty_result_gaps() {
	...
	local diag_log="${HOME}/.maestro/logs/worker-empty-results.log"
	...
	_py_script=$(mktemp "${TMPDIR:-/tmp}/maestro-empty-gaps-XXXXXX") || return 0
...
build_sandbox_passthrough_csv() {
	...
	while IFS='=' read -r name _; do
		...
		# all maestro.* plugin span enrichment is silently dropped (t2186).
		MAESTRO_* | PULSE_* | GH_* | GITHUB_* | OPENCODE_* | XDG_* | OTEL_* | REAL_HOME | TMPDIR | TMP | TEMP | RTK_* | VERIFY_*)
			if [[ "$seen_names" == *" ${name} "* ]]; then
				continue
			fi
			seen_names+="${name} "
...
# Env:
#   MAESTRO_HEADLESS_APPEND_CONTRACT=0 disables prompt augmentation.
...
- Do NOT call maestro-update-check.sh -- it exits immediately for headless workers.
...
- Normal project repos: full-loop workflow is deployed at ~/.maestro/agents/scripts/commands/full-loop.md
- Normal project repos: maestro framework scripts live under ~/.maestro/agents/scripts/ (not project-local .agents/scripts/)
- Maestro source repo only: the same files are edited at .agents/scripts/commands/full-loop.md and under .agents/scripts/
...
Exception: if your changes modify full-loop-helper.sh or its sourced helper libraries, commit first and then merge with the committed worktree helper path:
  "$PWD/.agents/scripts/full-loop-helper.sh" merge "$PR_NUMBER" "${GITHUB_REPOSITORY:-Aditya060806/Maestro}"
...
append_worker_headless_contract() {
	local prompt_text="$1"
	local append_enabled="${MAESTRO_HEADLESS_APPEND_CONTRACT:-1}"
...
# (`maestro update`) or maintainer intervention. Hammering an issue with
...
_headless_runtime_is_linux() {
	local platform="${MAESTRO_TEST_UNAME_S:-}"
	if [[ -z "$platform" ]]; then
...
_run_canary_test() {
	...
	# will fail identically. Bypass: MAESTRO_SKIP_CANARY_NEG_CACHE=1.
	#
	...
	if [[ "${MAESTRO_SKIP_CANARY_NEG_CACHE:-0}" != "1" ]] && [[ -f "$fail_cache_file" ]]; then
	...
	local canary_output
	canary_output=$(mktemp "${TMPDIR:-/tmp}/maestro-canary.XXXXXX")
	...
	# use `--pure`; load the maestro plugin and suppress interactive hooks via
	# MAESTRO_HEADLESS=1 below.
	...
	_canary_data_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-canary-db.XXXXXX")
	...
	#
	# GH#23598: include the opencode-maestro plugin when present so OAuth
	...
	_canary_config_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-canary-config.XXXXXX")
	mkdir -p "${_canary_config_dir}/opencode"
	local _canary_plugin_path
	_canary_plugin_path="${MAESTRO_PLUGIN_INDEX:-${HOME}/.maestro/agents/plugins/opencode-maestro/index.mjs}"
	...
	#
	# GH#23598/GH#23950: MAESTRO_HEADLESS=1 prevents plugin greeting/TTSR
