...
_ddh_apply_recovery_loop_hold() {
	...
	if [[ "$existing_block" -eq 0 ]]; then
		local version=""
		version=$(maestro_find_version 2>/dev/null || true)
		if [[ -z "$version" && -r "${AGENTS_DIR:-$HOME/.maestro/agents}/VERSION" ]]; then
			version=$(tr -d '[:space:]' <"${AGENTS_DIR:-$HOME/.maestro/agents}/VERSION" 2>/dev/null || true)
		fi
