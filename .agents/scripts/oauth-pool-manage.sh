...
cmd_status() {
	...
	*)
	...
	if [[ "$found_any" == "false" ]]; then
		...
		echo ""
		echo "Add an account:"
		echo "  maestro model-accounts-pool add anthropic    # Claude Pro/Max"
		echo "  maestro model-accounts-pool add openai       # ChatGPT Plus/Pro (device flow default)"
		echo "  maestro model-accounts-pool add cursor       # Cursor Pro"
		echo "  maestro model-accounts-pool add google       # Google AI Pro/Ultra/Workspace"
		echo "  maestro model-accounts-pool import claude-cli"
	fi
...
cmd_assign_pending() {
	...
	*)
	...
	if [[ "$pending_info" == "NONE" ]]; then
		...
		print_info "  maestro model-accounts-pool add ${provider}"
		return 0
	fi
	...
	if [[ -z "$email" ]]; then
		...
		echo ""
		echo "Usage: maestro model-accounts-pool assign-pending ${provider} <email>"
		return 0
	fi
	...
	ERROR:not_found)
		print_error "Account ${email} not found in ${provider} pool"
		print_info "Run 'maestro model-accounts-pool list' to see existing accounts"
		return 1
		;;
	esac
