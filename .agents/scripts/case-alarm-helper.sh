...
_load_config() {
	...
	if [[ ! -f "$config_file" ]]; then
		...
		if [[ -f "$template_file" ]]; then
			...
			printf '{"stages_days":[30,7],"channels":["gh-issue","ntfy"],"ntfy_topic":"maestro-case-alarms","per_case_overrides":{}}\n' > "$config_file"
...
_config_ntfy_topic() {
	local config="$1"
	echo "$config" | jq -r '.ntfy_topic // "maestro-case-alarms"'
	return 0
}
...
<!-- maestro:case-alarm case-id="${case_id}" deadline-label="${label}" -->
EOF
)"
...
_alarm_ntfy() {
	local case_id="$1" label="$2" date="$3" days="$4" topic="${5:-maestro-case-alarms}"
	local days_str
	if [[ "$days" -ge 0 ]]; then
		days_str="${days} days"
	else
	...
	local ntfy_server="${MAESTRO_NTFY_SERVER:-https://ntfy.sh}"
...
Channels:
  ...
  ntfy       POST to ntfy topic (curl). Server via MAESTRO_NTFY_SERVER
...
Env vars:
  ...
  MAESTRO_NTFY_SERVER    ntfy server URL (default: https://ntfy.sh)
