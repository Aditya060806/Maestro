...
load_api_key() {
    ...
    if [[ -z "${WAVESPEED_API_KEY:-}" ]] && command -v gopass &>/dev/null; then
        WAVESPEED_API_KEY="$(gopass show -o "maestro/WAVESPEED_API_KEY" 2>/dev/null)" || true
    fi

    if [[ -z "${WAVESPEED_API_KEY:-}" ]]; then
        print_error "WAVESPEED_API_KEY not set"
        print_info "Run: maestro secret set WAVESPEED_API_KEY"
        print_info "Or add to ~/.config/maestro/credentials.sh"
        return 1
    fi
...
Environment:
  WAVESPEED_API_KEY  API key (get from https://wavespeed.ai/accesskey)
                     Store via: maestro secret set WAVESPEED_API_KEY
