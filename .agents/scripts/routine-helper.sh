...
cmd_plan() {
	...
	printf 'Cron entry:\n'
	printf "%s %s >> \$HOME/.maestro/logs/routine-%s.log 2>&1 # maestro: routine-%s\n\n" \
		"$ROUTINE_SCHEDULE" "$command" "$ROUTINE_NAME" "$ROUTINE_NAME"

	printf 'launchd label: sh.maestro.routine-%s\n' "$ROUTINE_NAME"
	printf 'launchd plist: ~/Library/LaunchAgents/sh.maestro.routine-%s.plist\n' "$ROUTINE_NAME"
...
cmd_install_cron() {
	...
	local marker="# maestro: routine-${ROUTINE_NAME}"
	local entry
	entry="$ROUTINE_SCHEDULE $command >> $HOME/.maestro/logs/routine-${ROUTINE_NAME}.log 2>&1 $marker"
...
cmd_install_launchd() {
	...
	local label_escaped
	label_escaped=$(xml_escape "sh.maestro.routine-${ROUTINE_NAME}")
	local command_escaped
	command_escaped=$(xml_escape "$command")
	local home_escaped
	home_escaped=$(xml_escape "$HOME")
	local env_path_escaped
	env_path_escaped=$(xml_escape "$(maestro_launchd_sanitized_path "/bin:/usr/bin:/usr/local/bin:/opt/homebrew/bin:${PATH}")")
	local log_path_escaped
	log_path_escaped=$(xml_escape "${HOME}/.maestro/logs/routine-${ROUTINE_NAME}.log")

	local plist_dir="$HOME/Library/LaunchAgents"
	local plist_name="sh.maestro.routine-${ROUTINE_NAME}.plist"
	local plist_path="$plist_dir/$plist_name"

	mkdir -p "$plist_dir" "$HOME/.maestro/logs"
