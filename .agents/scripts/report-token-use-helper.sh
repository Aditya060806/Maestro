...
OUTPUT:
  ...
  ~/.maestro/_reports/token-use/<UTC-run-id>/

ENVIRONMENT OVERRIDES:
  MAESTRO_REPORT_TOKEN_USE_OPENCODE_DB  Override OpenCode SQLite DB path
  MAESTRO_REPORT_TOKEN_USE_OBS_DB       Override maestro observability SQLite DB path
  MAESTRO_REPORT_TOKEN_USE_ROOT         Override report output root
EOF
	return 0
}
