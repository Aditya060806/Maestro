...
_copy_cmd_kimi_skill() {
	...
	if grep -q '^description:' "$tmp2"; then
		...
		awk -v n="$name" '
			NR == 1 && /^---$/ { print; print "description: maestro primary agent routing command: " n; next }
			{ print }
		' "$tmp2" >"$dest"
	fi
...
# Differentiates maestro commands from native client slash commands and
# groups them alphabetically in the client's command picker.
_MAESTRO_CMD_PREFIX="maestro-"
...
_deploy_one_command() {
	...
	kilo | windsurf | aider)
		...
		# repo-local dir created by `maestro init`; Aider has no native
...
#   1. ~/.maestro/agents/commands/        -- main-agent symlinks (already
#                                            prefixed with `maestro-`)
#   2. ~/.maestro/agents/scripts/commands/ -- skills/workflows/utilities
...
_generate_commands_for_runtime() {
	...
	# via MAESTRO_FEATURE_COMMANDS_<SUFFIX>=no.
	...
	local main_src_dir="$HOME/.maestro/agents/commands"
	if [[ -d "$main_src_dir" ]]; then
		...
		for cmd_file in "$main_src_dir"/*.md; do
			...
			# Deploy with name as-is (already carries `maestro-` prefix).
	...
	# These files are NOT prefixed at the source -- prepend `maestro-` at deploy.
	local skills_src_dir="$HOME/.maestro/agents/scripts/commands"
	if [[ -d "$skills_src_dir" ]]; then
		...
		for cmd_file in "$skills_src_dir"/*.md; do
			...
			deployed_name="${_MAESTRO_CMD_PREFIX}${cmd_name}"
...
# Generate lifecycle hardcoded commands (release, onboarding, setup-maestro).
# Arguments: $1 - runtime_id, $2 - cmd_dir
# Returns: count of generated commands via exit code
...
Arguments: $ARGUMENTS'; then
	...
	# --- Setup ---
	# shellcheck disable=SC2016
	if _maybe_write_hardcoded_command "$runtime_id" "$cmd_dir" "setup-maestro" \
