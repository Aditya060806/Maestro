...
# `maestro update`. If the update leaves the deployed agents aligned with
...
#   help                               — show usage.
#
# State file: ~/.maestro/cache/runner-health.json (v1 schema in brief).
# Advisory:   ~/.maestro/advisories/runner-health-degraded.advisory
# Stamp:      ~/.maestro/cache/runner-health-advisory.stamp (dedup).
#
...
# Paths.
RUNNER_HEALTH_CACHE_DIR="${RUNNER_HEALTH_CACHE_DIR:-${HOME}/.maestro/cache}"
RUNNER_HEALTH_STATE_FILE="${RUNNER_HEALTH_CACHE_DIR}/runner-health.json"
RUNNER_HEALTH_ADVISORY_DIR="${RUNNER_HEALTH_ADVISORY_DIR:-${HOME}/.maestro/advisories}"
...
RUNNER_HEALTH_PULSE_LOG="${RUNNER_HEALTH_PULSE_LOG:-${HOME}/.maestro/logs/pulse-wrapper.log}"
RUNNER_HEALTH_DEPLOYED_VERSION_FILE="${RUNNER_HEALTH_DEPLOYED_VERSION_FILE:-${HOME}/.maestro/agents/VERSION}"
RUNNER_HEALTH_DEPLOYED_SHA_FILE="${RUNNER_HEALTH_DEPLOYED_SHA_FILE:-${HOME}/.maestro/.deployed-sha}"
...
# breaker trips and `maestro update` runs synchronously.
#######################################
cmd_record_outcome() {
	local signal="${1:-}"
...
# synchronously run `maestro update`. If the update changed VERSION
...
_rh_trip_breaker() {
	...
	local update_helper="${HOME}/.maestro/agents/scripts/auto-update-helper.sh"
	...
	if [[ -x "$update_helper" ]]; then
		...
		local update_log="${HOME}/.maestro/logs/runner-health-update.log"
...
_rh_classify_diagnose() {
	...
	if [[ "$state" == "tripped" ]]; then
		...
		# fired but the synchronous `maestro update` failed, so resume hook
