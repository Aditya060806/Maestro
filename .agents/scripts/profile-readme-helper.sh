...
METRICS_FILE="${HOME}/.maestro/.agent-workspace/observability/metrics.jsonl"
OBS_DB_FILE="${HOME}/.maestro/.agent-workspace/observability/llm-requests.db"
...
# --- Inject maestro markers into an existing README that lacks them ---
...
_recover_diverged_profile() {
	...
	if git clone "git@github.com:${repo_slug}.git" "$repo_dir" 2>/dev/null ||
		...
		if [[ -n "$(git -C "$repo_dir" status --porcelain README.md 2>/dev/null)" ]]; then
			...
			git -C "$repo_dir" commit -m "feat: initialize profile README with maestro stat markers" --no-verify 2>/dev/null || true
...
_generate_rich_readme() {
	...
	# Compose the README
	{
		...
		echo ">"
		echo "> Stats auto-updated by [maestro](https://maestro.sh)."
		echo ""
		echo "<!-- STATS-START -->"
...
_update_inject_timestamp() {
	...
	awk -v ts="$updated_at" '
		...
		/<!-- UPDATED-END -->/ {
			skip = 0
			printf "_Stats auto-updated %s by [maestro](https://maestro.sh) pulse._\n", ts
			print "<!-- UPDATED-END -->"
			next
		}
		!skip { print }
...
_update_inject_markers_if_needed() {
	...
	_inject_markers_into_readme "$readme_path"
	git -C "$profile_repo" add README.md
	git -C "$profile_repo" commit -m "feat: initialize profile README with maestro stat markers" --no-verify 2>/dev/null || true
	return 0
}
