...
_resolve_pulse_consent() {
	...
	if type _jsonc_get_raw &>/dev/null && [[ -f "${JSONC_USER:-$HOME/.config/maestro/config.jsonc}" ]]; then
		_pulse_user_config=$(_jsonc_get_raw "${JSONC_USER:-$HOME/.config/maestro/config.jsonc}" "orchestration.supervisor_pulse")
	fi

	# Also check legacy .conf user override
	if [[ -z "$_pulse_user_config" && -f "${FEATURE_TOGGLES_USER:-$HOME/.config/maestro/feature-toggles.conf}" ]]; then
		local _legacy_val
		...
		_legacy_val=$(awk -F= '/^supervisor_pulse=/{val=$2} END{print val}' "${FEATURE_TOGGLES_USER:-$HOME/.config/maestro/feature-toggles.conf}")
	...
	if [[ -n "${MAESTRO_SUPERVISOR_PULSE:-}" ]]; then
		_pulse_user_config="$MAESTRO_SUPERVISOR_PULSE"
	fi
	...
	if [[ "$_pulse_lower" == "false" ]]; then
		...
		if [[ "$NON_INTERACTIVE" == "true" ]]; then
			...
			if [[ "$enable_pulse" =~ ^[Yy]$ ]]; then
				...
				print_info "Skipped. Enable later: maestro config set orchestration.supervisor_pulse true && ./setup.sh" >&2
			fi
		fi
	fi
...
_pulse_clear_canary_negative_cache() {
	local _state_dir="${MAESTRO_HEADLESS_RUNTIME_DIR:-${HOME}/.maestro/.agent-workspace/headless-runtime}"
...
_pulse_daemon_path_for_opencode() {
	...
	_path="${HOME}/.local/bin:${HOME}/.maestro/agents/scripts${_opencode_dir:+:$_opencode_dir}:/usr/local/bin:/usr/bin:/bin"
	printf '%s' "$_path"
	return 0
}
...
_resolve_pulse_runtime_binary() {
	...
	# invocations so maestro-auto-update.timer (systemd minimal PATH)
	...
	local _persisted_file="$HOME/.config/maestro/scheduler-runtime-bin"
	local opencode_bin=""
...
_build_pulse_linux_env() {
	...
	local _pulse_env="PULSE_DIR=${HOME}/.maestro/.agent-workspace
PULSE_STALE_THRESHOLD=${PULSE_STALE_THRESHOLD_SECONDS}
MAESTRO_PULSE_ASYNC_POST_DISPATCH_HOUSEKEEPING=0"
	...
	# minimal PATH (e.g. when maestro-auto-update.timer regenerates the
...
_install_supervisor_pulse() {
	...
	_install_scheduler_linux \
		"maestro-supervisor-pulse" \
		"maestro: supervisor-pulse" \
		"${_pulse_cron_schedule}" \
		"\"${wrapper_script}\"" \
		"${_pulse_interval_sec}" \
		"$HOME/.maestro/logs/pulse-wrapper.log" \
		"$_pulse_env" \
...
_schedulers_record_template_hash() {
	local state_dir="$HOME/.maestro/.agent-workspace/tmp"
...
setup_supervisor_pulse() {
	...
	local wrapper_script="$HOME/.maestro/agents/scripts/pulse-wrapper.sh"
	local pulse_label="com.maestro.maestro-supervisor-pulse"
...
_cleanup_old_pulse_plists() {
	...
	local old_plist="$HOME/Library/LaunchAgents/com.maestro.supervisor-pulse.plist"
	if [[ -f "$old_plist" ]]; then
...
#
# Args: $1=plist_label (e.g. "com.maestro.maestro-supervisor-pulse")
...
_build_plist_env_overrides_xml() {
	local _label="$1"
	local _override_file="${2:-$HOME/.maestro/agents/configs/plist-env-overrides.json}"
	local _indent="${3:-		}"
...
_generate_pulse_plist_content() {
	...
	_xml_pulse_dir=$(_xml_escape "${HOME}/.maestro/.agent-workspace")
	_xml_path=$(_xml_escape "$(maestro_launchd_sanitized_path "$PATH")")
	...
	# Reads ~/.maestro/agents/configs/plist-env-overrides.json when present.
...
<dict>
	...
	<string>${_xml_home}/.maestro/logs/pulse-wrapper.log</string>
...
_install_pulse_launchd() {
	...
	# Users on stale installs may have com.maestro.supervisor-pulse (legacy) and
	# com.maestro.maestro-supervisor-pulse (current) both loaded, causing 2x
	# dispatch.  Only targets the hardcoded legacy path; idempotent — no-op when
	# the legacy file is absent.
	local _legacy_plist="$HOME/Library/LaunchAgents/com.maestro.supervisor-pulse.plist"
...
<dict>
	...
	<integer>60</integer>
	<key>StandardOutPath</key>
	<string>${_xml_home}/.maestro/logs/pulse-watchdog-launchd.log</string>
	<key>StandardErrorPath</key>
	<string>${_xml_home}/.maestro/logs/pulse-watchdog-launchd.log</string>
	<key>EnvironmentVariables</key>
	<dict>
...
_install_pulse_watchdog_launchd() {
	local watchdog_label="sh.maestro.pulse-watchdog"
	local tick_script="$HOME/.maestro/agents/scripts/pulse-watchdog-tick.sh"
...
_install_pulse_watchdog_systemd() {
	local tick_script="$HOME/.maestro/agents/scripts/pulse-watchdog-tick.sh"
	local watchdog_systemd="maestro-pulse-watchdog"
	local watchdog_log="$HOME/.maestro/logs/pulse-watchdog-launchd.log"
