...
# Agent deployment functions: deploy_maestro_agents, deploy_ai_templates, inject_agents_reference
...
_restart_pulse_if_running() {
	...
	# background launch never depends on a path inside ~/.maestro/agents/scripts/
	...
	if [[ ! -x "$pulse_script" ]]; then
		pulse_script="${HOME}/.maestro/agents/scripts/pulse-wrapper.sh"
	fi
	if [[ -x "$pulse_script" ]]; then
		nohup "$pulse_script" >>"${HOME}/.maestro/logs/pulse-wrapper.log" 2>&1 &
...
readonly _MAESTRO_REFERENCE_LINE='Add ~/.maestro/agents/AGENTS.md to context for Maestro capabilities.'
...
_deploy_agents_copy() {
	...
	if command -v rsync &>/dev/null; then
		...
		local rsync_timeout="${MAESTRO_RSYNC_TIMEOUT:-120}"
...
# Returns 0 when a plugin namespace collides with a core maestro agents
...
_inject_plan_reminder() {
	local target_dir="$1"
	local plan_reminder="$HOME/.maestro/cache/opencode-prompts/plan-reminder.txt"
...
# are created by create_backup_with_rotation as ~/.maestro/agents-backups/<ts>/agents.
_restore_latest_agents_backup() {
	local target_dir="$1"
	local backup_base="$HOME/.maestro/agents-backups"
	local latest_backup=""
	local parent_dir=""
...
_verify_deployed_agents_tree() {
	local target_dir="$1"
	local min_files="${MAESTRO_AGENT_DEPLOY_MIN_FILES:-100}"
	local file_count="0"
...
_verify_deployed_core_plugin_freshness() {
	...
	local -a core_plugin_files=(
		"plugins/opencode-maestro/model-limits.mjs"
	)
...
# Installs node_modules for the opencode-maestro plugin.
...
_install_opencode_plugin_deps() {
	...
	local plugin_dir="$target_dir/plugins/opencode-maestro"
...
_atomic_stage_and_deploy_agents() {
	...
	# setup left ~/.maestro/agents missing while reporting [SETUP_COMPLETE].
	if [[ -d "$target_dir" ]]; then
		if ! mv "$target_dir" "$old_dir"; then
...
# Copies *.advisory files to ~/.maestro/advisories/ (shown in session greeting).
_deploy_security_advisories_files() {
	local source_dir="$1"
	local advisories_source="$source_dir/advisories"
	local advisories_target="$HOME/.maestro/advisories"
...
_migrate_mailbox_if_needed() {
	local target_dir="$1"
	local maestro_workspace_dir="${MAESTRO_WORKSPACE_DIR:-$HOME/.maestro/.agent-workspace}"
	local mail_dir="${MAESTRO_MAIL_DIR:-${maestro_workspace_dir}/mail}"
...
# This catches the case where someone edited ~/.maestro/agents/scripts/ directly
...
_validate_agent_source_dir() {
	...
	if [[ ! -d "$source_dir" ]]; then
		...
		print_info "  cd ~/Git/maestro && ./setup.sh"
		return 1
	fi
...
_prepare_agents_deploy_target() {
	...
	if [[ -d "$target_dir" ]]; then
		...
		dep_sha=$(cat "${HOME}/.maestro/.deployed-sha" 2>/dev/null || echo "")
...
_write_deployed_agents_sha() {
	...
	if [[ -n "$deployed_sha" ]]; then
		local maestro_dir="${HOME}/.maestro"
		mkdir -p "$maestro_dir"
		printf '%s\n' "$deployed_sha" >"${maestro_dir}/.deployed-sha"
	fi
...
deploy_maestro_agents() {
	print_info "Deploying maestro agents to ~/.maestro/agents/..."
	...
	local plugins_file="$HOME/.config/maestro/plugins.json"
...
inject_agents_reference() {
	print_info "Adding maestro reference to AI assistant configurations..."
...
_inject_agents_reference_legacy() {
	local reference_line="$_MAESTRO_REFERENCE_LINE"
	...
	for entry in "${ai_agent_dirs[@]}"; do
		...
		if [[ -d "$config_dir" ]]; then
			...
			if [[ -f "$agents_file" ]]; then
				...
				if [[ "$first_line" != *"maestro/agents/AGENTS.md"* ]]; then
					local temp_file
					temp_file=$(mktemp)
				...
				print_success "Created $agents_file with maestro reference"
				((++updated_count))
			fi
		fi
	done
...
_deploy_codex_instructions() {
	...
	local reference_content="$_MAESTRO_REFERENCE_LINE"

	if [[ -f "$instructions_file" ]]; then
		# shellcheck disable=SC2088  # Tilde is a literal grep pattern, not a path
		if grep -q '~/.maestro/agents/AGENTS.md' "$instructions_file" 2>/dev/null; then
			print_info "Codex instructions.md already has maestro reference"
			return 0
		fi
		...
		print_success "Added maestro reference to $instructions_file"
	else
		echo "$reference_content" >"$instructions_file"
		print_success "Created $instructions_file with maestro reference"
	fi
	return 0
}
...
_deploy_cursor_agents_reference() {
	...
	local agents_file="$rules_dir/maestro.md"
	...
	if [[ -f "$agents_file" ]]; then
		# shellcheck disable=SC2088  # Tilde is a literal grep pattern, not a path
		if grep -q '~/.maestro/agents/AGENTS.md' "$agents_file" 2>/dev/null; then
			print_info "Cursor rules/maestro.md already has maestro reference"
			return 0
		fi
	fi

	echo "$reference_content" >"$agents_file"
	print_success "Deployed maestro reference to $agents_file"
	return 0
}
...
_deploy_droid_agents_reference() {
	...
	local agents_file="$skills_dir/maestro.md"
	...
	if [[ -f "$agents_file" ]]; then
		# shellcheck disable=SC2088  # Tilde is a literal grep pattern, not a path
		if grep -q '~/.maestro/agents/AGENTS.md' "$agents_file" 2>/dev/null; then
			print_info "Droid skills/maestro.md already has maestro reference"
			return 0
		fi
	fi
