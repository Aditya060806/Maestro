...
# Install maestro inside an OrbStack Linux VM (macOS only, user chose option 2)
_bootstrap_orbstack_install() {
	...
	print_info "Installing maestro inside the VM..."
	echo ""
	orb run -m "$vm_name" bash -c 'bash <(curl -fsSL https://maestro.sh/install)'

	echo ""
	print_success "maestro installed in OrbStack VM: $vm_name"
	echo ""
	echo "To use maestro in the VM:"
...
bootstrap_repo() {
	...
	if [[ -z "$script_path" || "$script_path" == "/dev/stdin" || "$script_path" == "bash" || "$script_path" == /dev/fd/* ]]; then
		...
		[[ "${MAESTRO_NON_INTERACTIVE:-false}" == "true" ]] && _bootstrap_non_interactive=true
		[[ ! -t 0 ]] && _bootstrap_non_interactive=true

		if [[ "$(uname)" == "Darwin" && "$_bootstrap_non_interactive" == "false" ]]; then
			echo ""
			echo "Where would you like to install maestro?"
			echo ""
		...
		if [[ -d "$INSTALL_DIR/.git" ]]; then
			...
			print_info "Cloning maestro to $INSTALL_DIR..."
...
# Many tools in the maestro ecosystem (Beads, Worktrunk, bv) are distributed
	...
	if verified_install "Homebrew" "https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh"; then
		...
		while IFS= read -r rc_file; do
			...
			if ! grep -q 'linuxbrew' "$rc_file" 2>/dev/null; then
				{
					echo ""
					echo "# Homebrew (Linuxbrew) - added by maestro setup"
					echo "$brew_line"
				} >>"$rc_file"
			fi
		...
		if [[ "$NON_INTERACTIVE" != "true" ]]; then
			...
			while IFS= read -r rc_file; do
				...
				if ! grep -q '/opt/homebrew/bin/brew' "$rc_file" 2>/dev/null; then
					{
						echo ""
						echo "# Homebrew (added by maestro setup)"
						echo "$brew_line"
					} >>"$rc_file"
			...
			if [[ "$NON_INTERACTIVE" != "true" ]]; then
				...
				while IFS= read -r intel_rc; do
					...
					if ! grep -q '/usr/local/bin/brew' "$intel_rc" 2>/dev/null; then
						{
							echo ""
							echo "# Homebrew Intel Mac (added by maestro setup)"
...
verify_location() {
	local current_dir
	current_dir="$(pwd)"
	local expected_location="$HOME/Git/maestro"
...
set_permissions() {
	...
	local deployed_dir="$HOME/.maestro/agents"
	...
	chmod 600 "$HOME/.config/maestro/"*.json 2>/dev/null || true
