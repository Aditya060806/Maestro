...
# Part of maestro setup.sh modularization (GH#5793)
#
...
if ! declare -F maestro_launchd_sanitized_path >/dev/null 2>&1; then
...
setup_stats_wrapper() {
	...
	local stats_script="$HOME/.maestro/agents/scripts/stats-wrapper.sh"
	local stats_label="com.maestro.maestro-stats-wrapper"
	local stats_systemd="maestro-stats-wrapper"
	local stats_log="$HOME/.maestro/logs/stats.log"
	if [[ -x "$stats_script" ]] && [[ "$_pulse_effective" == "true" ]]; then
		...
		if [[ "$(uname -s)" == "Darwin" ]]; then
			...
			_xml_stats_path=$(_xml_escape "$(maestro_launchd_sanitized_path "$PATH")")
...
<dict>
	...
	<string>${_xml_stats_home}/.maestro/logs/stats.log</string>
...
setup_failure_miner() {
	local _pulse_lower="$1"
	local _pulse_effective="${PULSE_ENABLED:-$_pulse_lower}"
	local miner_script="$HOME/.maestro/agents/scripts/gh-failure-miner-helper.sh"
	local miner_label="sh.maestro.routine-gh-failure-miner"
	local miner_systemd="maestro-gh-failure-miner"
	local miner_log="$HOME/.maestro/logs/routine-gh-failure-miner.log"
	...
	if [[ "$(uname -s)" == "Darwin" ]]; then
		...
		_xml_miner_path=$(_xml_escape "$(maestro_launchd_sanitized_path "/bin:/usr/bin:/usr/local/bin:/opt/homebrew/bin:${PATH}")")
...
setup_process_guard() {
	local guard_script="$HOME/.maestro/agents/scripts/process-guard-helper.sh"
	local guard_label="sh.maestro.process-guard"
	local guard_systemd="maestro-process-guard"
	local guard_log="$HOME/.maestro/logs/process-guard.log"
	...
	if [[ "$(uname -s)" == "Darwin" ]]; then
		...
		_xml_guard_path=$(_xml_escape "$(maestro_launchd_sanitized_path "$PATH")")
...
<dict>
	...
	<integer>30</integer>
	<key>StandardOutPath</key>
	<string>${_xml_guard_home}/.maestro/logs/process-guard.log</string>
	<key>StandardErrorPath</key>
	<string>${_xml_guard_home}/.maestro/logs/process-guard.log</string>
	<key>EnvironmentVariables</key>
	<dict>
...
setup_memory_pressure_monitor() {
	local monitor_script="$HOME/.maestro/agents/scripts/memory-pressure-monitor.sh"
	local monitor_label="sh.maestro.memory-pressure-monitor"
	local monitor_systemd="maestro-memory-pressure-monitor"
	local monitor_log="$HOME/.maestro/logs/memory-pressure-launchd.log"
...
<dict>
	...
	<integer>60</integer>
	<key>StandardOutPath</key>
	<string>${_xml_monitor_home}/.maestro/logs/memory-pressure-launchd.log</string>
	<key>StandardErrorPath</key>
	<string>${_xml_monitor_home}/.maestro/logs/memory-pressure-launchd.log</string>
	<key>EnvironmentVariables</key>
	<dict>
...
setup_screen_time_snapshot() {
	local st_script="$HOME/.maestro/agents/scripts/screen-time-helper.sh"
	local st_label="sh.maestro.screen-time-snapshot"
	local st_systemd="maestro-screen-time-snapshot"
	local st_log="$HOME/.maestro/.agent-workspace/logs/screen-time-snapshot.log"
	if [[ ! -x "$st_script" ]]; then
		return 0
	fi
...
<dict>
	...
	<key>StandardOutPath</key>
	<string>${_xml_st_home}/.maestro/.agent-workspace/logs/screen-time-snapshot.log</string>
	<key>StandardErrorPath</key>
	<string>${_xml_st_home}/.maestro/.agent-workspace/logs/screen-time-snapshot.log</string>
	<key>EnvironmentVariables</key>
	<dict>
