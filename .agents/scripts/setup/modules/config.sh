...
# Configuration functions: setup_configs, set_permissions, ssh, maestro-cli, opencode-config, claude-config, validate, extract-prompts, drift-check
...
install_maestro_cli() {
	print_info "Installing maestro CLI command..."
	...
	local cli_source="${INSTALL_DIR:?INSTALL_DIR not set}/maestro.sh"
	local cli_target="/usr/local/bin/maestro"

	if [[ ! -f "$cli_source" ]]; then
		print_warning "maestro.sh not found at $cli_source - skipping CLI installation"
		return 0
	fi
	...
	if [[ -w "/usr/local/bin" ]]; then
		...
		print_success "Installed maestro command to $cli_target"
	elif [[ -w "$HOME/.local/bin" ]] || mkdir -p "$HOME/.local/bin" 2>/dev/null; then
		# Use ~/.local/bin instead
		cli_target="$HOME/.local/bin/maestro"
		ln -sf "$cli_source" "$cli_target"
		print_success "Installed maestro command to $cli_target"
		...
		# Need sudo
		print_info "Installing maestro command requires sudo..."
		if sudo ln -sf "$cli_source" "$cli_target"; then
			print_success "Installed maestro command to $cli_target"
		else
			print_warning "Could not install maestro command globally"
...
update_codex_config() {
	...
	# Deploy maestro MCP servers to Codex config.toml
	_deploy_codex_mcps
...
# Deploy standard maestro MCP servers to ~/.codex/config.toml
...
update_cursor_config() {
	...
	# Deploy maestro MCP servers to Cursor mcp.json
	_deploy_cursor_mcps
...
# Deploy standard maestro MCP servers to ~/.cursor/mcp.json
...
# per-runtime via MAESTRO_FEATURE_COMMANDS_<SUFFIX>=no. Clients with no
