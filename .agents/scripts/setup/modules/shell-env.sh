...
_shell_compat_write_profile_header() {
...

	{
		...
		echo "# Created by maestro setup to preserve customizations across shell switches"
...
_shell_compat_add_sourcing() {
	...
	if ! grep -q 'shell_common' "$zsh_rc"; then
		{
			echo ""
			echo "# Cross-shell compatibility (added by maestro setup)"
...
_local_bin_report_results() {
	...
	if [[ -n "$added_to" ]]; then
		...
		print_info "Restart your terminal to use 'maestro' command"
	fi
...
# Add ~/.local/bin to PATH in all shell rc files for the maestro CLI
...
#   0. PATH shim — ~/.maestro/bin/shellcheck symlink + PATH prepend (GH#2993)
...
# "shellcheck". By placing ~/.maestro/bin first on PATH with a symlink to
...
#
# CRITICAL: ~/.maestro/bin MUST be at the START of PATH, not the end.
...
# Places a symlink to the wrapper in ~/.maestro/bin so bash-language-server
# finds the wrapper via PATH lookup before the real shellcheck binary.
_shellcheck_wrapper_setup_shim() {
	local wrapper_path="$1"
	local shim_dir="$HOME/.maestro/bin"
...
_shellcheck_wrapper_setup_zshenv() {
	...
	if [[ -f "$zshenv" ]] || command -v zsh >/dev/null 2>&1; then
		...
		if grep -q 'SHELLCHECK_PATH' "$zshenv"; then
			...
			{
				echo ""
				echo "# Added by maestro setup (GH#2915: prevent ShellCheck memory explosion)"
				echo "$env_line"
			} >>"$zshenv"
			added_to_out="$zshenv"
		fi

		# PATH prepend for ~/.maestro/bin (GH#2993: shim must be on PATH)
		...
		if grep -q 'case ":$PATH:" in.*\.maestro/bin' "$zshenv"; then
			# Remove the old case-guard line (sed is appropriate here — targeted single-line removal)
			# shellcheck disable=SC2016
			sed -i.bak '/case ":$PATH:" in.*\.maestro\/bin/d' "$zshenv"
			rm -f "${zshenv}.bak"
		fi
		# Use exact-line match for the new sanitize-and-prepend form
		if ! grep -Fq '_maestro_shim' "$zshenv"; then
			{
				echo ""
				echo "# Added by maestro setup (GH#2993: shellcheck shim on PATH)"
				echo "$path_line"
			} >>"$zshenv"
...
# Remove stale old-form PATH entries for the maestro shim from an rc file.
...
_shellcheck_wrapper_remove_stale_path() {
	...
	# not position — left the shim at the end of PATH on upgrades)
	# shellcheck disable=SC2016 # Matching literal $PATH text in rc files, not expanding
	if grep -q 'case ":$PATH:" in.*\.maestro/bin' "$rc_file"; then
		# shellcheck disable=SC2016
		sed -i.bak '/case ":$PATH:" in.*\.maestro\/bin/d' "$rc_file"
		rm -f "${rc_file}.bak"
	fi
	# For fish: also remove old 'contains' form that only checked presence
	if [[ "$is_fish_rc" == "true" ]] && grep -q 'contains.*\.maestro/bin' "$rc_file"; then
		sed -i.bak '/contains.*\.maestro\/bin/d' "$rc_file"
		rm -f "${rc_file}.bak"
	fi
...
_shellcheck_wrapper_setup_one_rc() {
	...
	# PATH prepend for ~/.maestro/bin (GH#2993)
	_shellcheck_wrapper_remove_stale_path "$rc_file" "$is_fish_rc"
	# Check for the new sanitize-and-prepend form (uses _maestro_shim variable)
	if ! grep -Fq '_maestro_shim' "$rc_file"; then
		{
			echo ""
			echo "# Added by maestro setup (GH#2993: shellcheck shim on PATH)"
			echo "$rc_path_line"
...
setup_shellcheck_wrapper() {
	local wrapper_path="$HOME/.maestro/agents/scripts/shellcheck-wrapper.sh"
	...
	env_line='export SHELLCHECK_PATH="$HOME/.maestro/agents/scripts/shellcheck-wrapper.sh"'
	...
	local path_line='_maestro_shim="$HOME/.maestro/bin"; PATH="$(printf '\''%s'\'' "$PATH" | tr '\'':'\'' '\''\n'\'' | grep -Fxv -- "$_maestro_shim" | paste -sd: -)"; export PATH="$_maestro_shim${PATH:+:$PATH}"; unset _maestro_shim'
	...
	local env_line_fish='set -gx SHELLCHECK_PATH "$HOME/.maestro/agents/scripts/shellcheck-wrapper.sh"'
	# shellcheck disable=SC2016 # fish path line: strip existing, then prepend
	local path_line_fish='set -l _maestro_shim "$HOME/.maestro/bin"; set -l _maestro_rest (string match -v -- "$_maestro_shim" $PATH); set -gx PATH $_maestro_shim $_maestro_rest'
...
# Replace the maestro alias block in a single rc file.
...
_aliases_replace_in_file() {
	...
	local tmp_file
	tmp_file=$(mktemp "${TMPDIR:-/tmp}/maestro-aliases-XXXXXX") || return 1
...
# Always replaces an existing maestro alias block so updates propagate.
...
_terminal_title_is_configured() {
	...
	while IFS= read -r rc_file; do
		[[ -z "$rc_file" ]] && continue
		if [[ -f "$rc_file" ]] && grep -q "maestro terminal-title" "$rc_file"; then
			return 0
		fi
	done < <(get_all_shell_rcs)
	return 1
...
_terminal_title_show_status() {
	...
	print_info "Example: Tab shows 'maestro/feature/xyz' when in that branch"
	echo ""
	echo "Current status:"
...
_terminal_title_prompt_and_install() {
	...
	if [[ "$install_title" =~ ^[Yy]?$ ]]; then
		...
		print_info "You can install later with: ~/.maestro/agents/scripts/terminal-title-setup.sh install"
	fi
