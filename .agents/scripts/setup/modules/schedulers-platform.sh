...
#   - Repo maestro health (r914): daily drift keeper
...
<dict>
	...
	<dict>
		<key>PATH</key>
		<string>$(maestro_launchd_sanitized_path)</string>
		<key>HOME</key>
...
_install_cw_linux() {
	local cw_script="$1"
	local _cw_log_dir="$2"
	local cw_systemd="maestro-contribution-watch"
	_install_scheduler_linux \
		"$cw_systemd" \
		"maestro: contribution-watch" \
		"$CRON_HOURLY" \
		"\"${cw_script}\" scan" \
		"3600" \
...
# Respects config: maestro config set orchestration.contribution_watch false
setup_contribution_watch() {
	local cw_script="$HOME/.maestro/agents/scripts/contribution-watch-helper.sh"
	local cw_label="sh.maestro.contribution-watch"
	local cw_state="$HOME/.maestro/cache/contribution-watch.json"
	...
	mkdir -p "$HOME/.maestro/cache" "$_cw_log_dir"
...
_install_complexity_scan_linux() {
	local cs_script="$1"
	local _cs_log_dir="$2"
	local cs_systemd="maestro-complexity-scan"
	_install_scheduler_linux \
		"$cs_systemd" \
		"maestro: complexity-scan" \
		"$CRON_HOURLY" \
		"\"${cs_script}\" run" \
		"3600" \
...
setup_complexity_scan() {
	local cs_script="$HOME/.maestro/agents/scripts/complexity-scan-runner.sh"
	local cs_label="sh.maestro.complexity-scan"
	if ! [[ -x "$cs_script" ]]; then
		return 0
	fi
...
# Supersedes the t2862 sh.maestro.pulse-merge-routine label while keeping the
# timeout-protected helper implementation:
#   - Label: com.maestro.maestro-supervisor-merge
...
_install_pulse_merge_routine_launchd() {
	...
	# One-time migration: unload and remove the legacy sh.maestro.pulse-merge-routine
	# plist (installed by t2862) to avoid running two merge passes simultaneously.
	local _legacy_pmr_label="sh.maestro.pulse-merge-routine"
...
_install_pulse_merge_routine_linux() {
	...
	local pmr_systemd="maestro-pulse-merge"
	...
	# One-time migration: remove legacy systemd/cron entry for maestro-pulse-merge-routine.
	if _systemd_user_available 2>/dev/null; then
		systemctl --user disable --now "maestro-pulse-merge-routine.timer" 2>/dev/null || true
		rm -f "$HOME/.config/systemd/user/maestro-pulse-merge-routine.service" 2>/dev/null || true
		rm -f "$HOME/.config/systemd/user/maestro-pulse-merge-routine.timer" 2>/dev/null || true
		systemctl --user daemon-reload 2>/dev/null || true
	fi
	if crontab -l 2>/dev/null | grep -qF "pulse-merge-routine"; then
		crontab -l 2>/dev/null | grep -v 'maestro: pulse-merge-routine' | crontab - 2>/dev/null || true
	fi

	_install_scheduler_linux \
		"$pmr_systemd" \
		"maestro: pulse-merge-routine" \
		"* * * * *" \
		"\"${pmr_script}\" run" \
		"60" \
...
setup_pulse_merge_routine() {
	local pmr_wrapper="$HOME/.maestro/agents/scripts/pulse-merge-routine.sh"
	local pmr_label="com.maestro.maestro-supervisor-merge"
...
# Respects config: maestro config set orchestration.draft_responses false
setup_draft_responses() {
	local dr_script="$HOME/.maestro/agents/scripts/draft-response-helper.sh"
	if [[ -x "$dr_script" ]] && is_feature_enabled orchestration.draft_responses 2>/dev/null && is_feature_enabled orchestration.contribution_watch 2>/dev/null && command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
		mkdir -p "$HOME/.maestro/.agent-workspace/draft-responses"
...
_core_routine_logged_command() {
	...
	local log_helper="\$HOME/.maestro/agents/scripts/routine-log-helper.sh"
...
<dict>
	...
	<integer>3600</integer>
	<key>StandardOutPath</key>
	<string>${_xml_pr_home}/.maestro/.agent-workspace/logs/profile-readme-update.log</string>
	<key>StandardErrorPath</key>
	<string>${_xml_pr_home}/.maestro/.agent-workspace/logs/profile-readme-update.log</string>
	<key>EnvironmentVariables</key>
	<dict>
...
setup_profile_readme() {
	local pr_script="$HOME/.maestro/agents/scripts/profile-readme-helper.sh"
	local pr_label="sh.maestro.profile-readme-update"
	...
	local pr_systemd="maestro-profile-readme-update"
	local pr_log="$HOME/.maestro/.agent-workspace/logs/profile-readme-update.log"
	mkdir -p "$HOME/.maestro/.agent-workspace/logs"
...
_install_token_refresh_schtasks() {
	local tr_script="$1"
	local log_dir="$2"
	local task_name="maestro-token-refresh"
	...
	if schtasks /Create \
		...
		print_warning "Failed to create token refresh scheduled task. Run manually: schtasks /Create /TN maestro-token-refresh /TR \"bash '${tr_script_win}' refresh anthropic\" /SC MINUTE /MO 30"
	fi
	return 0
}
...
_oauth_token_refresh_ready() {
	...
	if ! [[ -f "$HOME/.maestro/oauth-pool.json" ]]; then
		return 1
	fi
	return 0
}
...
<dict>
	...
	<key>StandardOutPath</key>
	<string>${_xml_tr_home}/.maestro/.agent-workspace/logs/token-refresh.log</string>
	<key>StandardErrorPath</key>
	<string>${_xml_tr_home}/.maestro/.agent-workspace/logs/token-refresh.log</string>
	<key>EnvironmentVariables</key>
	<dict>
...
setup_oauth_token_refresh() {
	local tr_script="$HOME/.maestro/agents/scripts/oauth-pool-helper.sh"
	local tr_label="sh.maestro.token-refresh"
	if ! _oauth_token_refresh_ready "$tr_script"; then
		return 0
	fi

	local tr_log_dir="$HOME/.maestro/.agent-workspace/logs"
	mkdir -p "$tr_log_dir"
...
setup_opencode_db_maintenance() {
	local ocdbm_script="$HOME/.maestro/agents/scripts/opencode-db-maintenance-helper.sh"
	if ! [[ -x "$ocdbm_script" ]]; then
		return 0
	fi

	local ocdbm_log_dir="$HOME/.maestro/.agent-workspace/logs"
	...
	if [[ "$(uname -s)" == "Darwin" ]]; then
		...
		_install_scheduler_linux \
			"maestro-opencode-db-maintenance" \
			"maestro: opencode-db-maintenance" \
...
setup_opencode_db_archive() {
	...
	local archive_script="$archive_home/.maestro/agents/scripts/opencode-db-archive-async-helper.sh"
	if ! [[ -x "$archive_script" ]]; then
		return 0
	fi

	local archive_log_dir="$archive_home/.maestro/logs"
	...
	if [[ "$(uname -s)" == "Darwin" ]]; then
		local archive_label="sh.maestro.opencode-db-archive"
		...
		_xml_archive_path=$(_xml_escape "$(maestro_launchd_sanitized_path "/bin:/usr/bin:/usr/local/bin:/opt/homebrew/bin${PATH:+:${PATH}}")")
...
# Respects config: maestro config set orchestration.repo_sync false
setup_repo_sync() {
	local repo_sync_script="$HOME/.maestro/agents/scripts/repo-sync-helper.sh"
	if ! [[ -x "$repo_sync_script" ]] || ! is_feature_enabled repo_sync 2>/dev/null; then
		return 0
	fi

	local _repo_sync_installed=false
	if _launchd_has_agent "com.maestro.maestro-repo-sync"; then
		_repo_sync_installed=true
	elif _launchd_has_agent "sh.maestro.repo-sync"; then
		_repo_sync_installed=true
	elif crontab -l 2>/dev/null | grep -qF "maestro-repo-sync"; then
		_repo_sync_installed=true
	elif command -v systemctl >/dev/null 2>&1 &&
		systemctl --user is-enabled "maestro-repo-sync.timer" >/dev/null 2>&1; then
		_repo_sync_installed=true
	fi
	if [[ "$_repo_sync_installed" == "false" ]]; then
		if [[ "$NON_INTERACTIVE" == "true" ]]; then
			bash "$repo_sync_script" enable >/dev/null 2>&1 || true
			print_info "Repo sync enabled (daily). Disable: maestro repo-sync disable"
		else
			echo ""
			...
			if [[ "$enable_repo_sync" =~ ^[Yy]?$ || -z "$enable_repo_sync" ]]; then
				...
				print_info "Skipped. Enable later: maestro repo-sync enable"
			fi
		fi
	fi
	return 0
...
# Setup r914 repo-maestro-health scheduler if not already installed.
# Daily drift keeper for repos.json: bumps stale .maestro.json versions
# and surfaces missing-folder / no-init drift for human triage.
# Respects config: maestro config set orchestration.repo_maestro_health false
setup_repo_maestro_health() {
	local repo_health_script="$HOME/.maestro/agents/scripts/repo-maestro-health-helper.sh"
	if ! [[ -x "$repo_health_script" ]] || ! is_feature_enabled repo_maestro_health 2>/dev/null; then
		return 0
	fi

	local _repo_health_installed=false
	if _launchd_has_agent "sh.maestro.repo-maestro-health"; then
		_repo_health_installed=true
	elif crontab -l 2>/dev/null | grep -qF "maestro-repo-maestro-health"; then
		_repo_health_installed=true
	elif command -v systemctl >/dev/null 2>&1 &&
		systemctl --user is-enabled "maestro-repo-maestro-health.timer" >/dev/null 2>&1; then
		_repo_health_installed=true
	fi
	if [[ "$_repo_health_installed" == "false" ]]; then
		if [[ "$NON_INTERACTIVE" == "true" ]]; then
			bash "$repo_health_script" enable >/dev/null 2>&1 || true
			print_info "r914 repo-maestro-health enabled (daily @03:30). Disable: maestro repo-maestro-health disable"
		else
			echo ""
			echo "r914 keeps \`.maestro.json\` versions current across all registered"
			echo "repos and surfaces registry drift (missing folders, unregistered git"
			echo "repos) for human triage. Runs daily at 03:30."
			echo ""
			setup_prompt enable_repo_health "Enable daily r914 repo-maestro-health? [Y/n]: " "Y"
			if [[ "$enable_repo_health" =~ ^[Yy]?$ || -z "$enable_repo_health" ]]; then
				bash "$repo_health_script" enable
			else
				print_info "Skipped. Enable later: maestro repo-maestro-health enable"
			fi
		fi
	fi
	return 0
...
# every 30 min and updates ~/.config/maestro/dispatch-override.conf to
...
_install_peer_productivity_monitor_linux() {
	...
	local ppm_systemd="maestro-peer-productivity-monitor"
	_install_scheduler_linux \
		"$ppm_systemd" \
		"maestro: peer-productivity-monitor" \
		"*/30 * * * *" \
		"\"${ppm_script}\" observe" \
		"1800" \
...
# every 30 min and updates ~/.config/maestro/dispatch-override.conf so the
# local pulse competes with broken peers and collaborates with healthy ones.
# Manual entries in dispatch-override.conf above the auto-managed marker
# always take precedence.
setup_peer_productivity_monitor() {
	local ppm_script="$HOME/.maestro/agents/scripts/peer-productivity-monitor.sh"
	local ppm_label="sh.maestro.peer-productivity-monitor"
