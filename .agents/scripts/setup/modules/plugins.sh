...
check_python_for_skill_scanner() {
	...
	print_info "After installing, re-run: maestro update"
	return 1
}
...
deploy_plugins() {
	...
	while IFS=$'\t' read -r pname prepo pns pbranch; do
		...
		if [[ -d "$clone_dir" ]]; then
			...
			# Users can force update via: maestro plugin update [name]
			skipped=$((skipped + 1))
			continue
		fi
	...
	if [[ "$skipped" -gt 0 ]]; then
		print_info "$skipped plugin(s) already deployed (use 'maestro plugin update' to refresh)"
	fi
	if [[ "$failed" -gt 0 ]]; then
...
generate_agent_skills() {
	...
	local skills_script="$HOME/.maestro/agents/scripts/generate-skills.sh"
	local timeout_seconds="${MAESTRO_SKILLS_GENERATE_TIMEOUT:-90}"
...
remove_duplicate_opencode_skill_symlink() {
	...
	# directory. Keeping the maestro-managed copy only in ~/.claude/skills avoids
...
create_skill_symlinks() {
	...
	local skill_sources="$home_dir/.maestro/agents/configs/skill-sources.json"
	local agents_dir="$home_dir/.maestro/agents"
	...
	# imported maestro skills use that shared location instead of duplicating the
...
check_skill_updates() {
	...
	local skill_sources="$HOME/.maestro/agents/configs/skill-sources.json"
	...
	if [[ $updates_available -gt 0 ]]; then
		...
		print_info "Run: ~/.maestro/agents/scripts/add-skill-helper.sh check-updates"
		print_info "To update a skill: ~/.maestro/agents/scripts/add-skill-helper.sh add <url> --force"
	else
...
_install_skill_scanner() {
	...
	if [[ "$installed" == "false" ]]; then
		...
		if [[ -n "$py_interp" ]]; then
			local venv_dir="$HOME/.maestro/.agent-workspace/work/cisco-scanner-env"
...
scan_imported_skills() {
	...
	local security_helper="$HOME/.maestro/agents/scripts/security-helper.sh"
	...
	if PATH="$scanner_dir:$PATH" bash "$security_helper" skill-scan all 2>/dev/null; then
		...
		print_warning "Some imported skills have security findings - review with: maestro skill scan"
	fi
...
setup_multi_tenant_credentials() {
	# Check prerequisites before announcing setup (GH#5240)
	local credential_helper="$HOME/.maestro/agents/scripts/credential-helper.sh"
	...
	if [[ -d "$HOME/.config/maestro/tenants" ]]; then
		local tenant_count
		tenant_count=$(find "$HOME/.config/maestro/tenants" -maxdepth 1 -type d | wc -l)
	...
	if [[ -f "$HOME/.config/maestro/credentials.sh" ]]; then
		local key_count
		key_count=$(grep -c "^export " "$HOME/.config/maestro/credentials.sh" 2>/dev/null) || true
...
check_tool_updates() {
	# Check prerequisites before announcing setup (GH#5240)
	local tool_check_script="$HOME/.maestro/agents/scripts/tool-version-check.sh"
	...
	if [[ $check_exit_code -ne 0 ]]; then
		...
		print_info "Run 'maestro update-tools' manually to check for updates"
		return 0
	fi
	...
	if [[ "$do_update" =~ ^[Yy]?$ || "$do_update" == "Y" ]]; then
		...
		print_info "Run 'maestro update-tools' anytime to update tools"
	fi
