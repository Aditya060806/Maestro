...
# Part of maestro setup.sh modularization (t316.3)
...
cleanup_deprecated_paths() {
	local agents_dir="$HOME/.maestro/agents"
	local cleaned=0
	...
	# See: ~/.config/maestro/settings.json { "preserve_oh_my_opencode": true }
	...
	# Remove opencode-antigravity-auth — third-party Google OAuth plugin removed from maestro.
	# When present but unresolvable it breaks the OpenCode plugin chain, preventing the
	# maestro pool from injecting tokens and causing "API key missing" errors for all providers.
...
# Remove osgrep completely — one-time cleanup for all maestro users
...
# Remove opencode-antigravity-auth plugin — third-party Google OAuth plugin removed from maestro.
# When present but unresolvable it breaks the OpenCode plugin chain, preventing the maestro
...
cleanup_worktree_entries_in_repos_json() {
	# t2250: `find ~/Git -name .maestro.json` during auto-discovery picks up
	...
	local flag_file="${HOME}/.maestro/logs/.migrated-worktree-repos-json-t2250"
	[[ -f "$flag_file" ]] && return 0

	local repos_json="${HOME}/.config/maestro/repos.json"
...
cleanup_stale_health_issue_caches() {
	local flag_file="${HOME}/.maestro/logs/.migrated-health-issue-caches-t1929"
	[[ -f "$flag_file" ]] && return 0

	local cache_dir="${HOME}/.maestro/logs"
	...
	if [[ "${NON_INTERACTIVE:-false}" == "true" ]]; then
		if [[ -f "$gitignore" ]]; then
			...
			if [[ "$needs_gitignore_update" == "true" ]]; then
				...
				print_info "  Run 'maestro init' in $(basename "$repo_path") or 'setup.sh -i' to apply"
			fi
		fi
		return 0
	fi
...
_migrate_ai_config_agent_refs() {
	...
	local greeting_cache="$HOME/.maestro/cache/session-greeting.txt"
...
# 5. References in ~/.maestro/ config files
#
...
migrate_agent_to_agents_folder() {
	local _sentinel="${HOME}/.maestro/.migrations/agent-to-agents-done"
	if [[ -f "$_sentinel" ]]; then
		return 0
	fi
	...
	if jq -e '.mcp.outscraper.command | join(" ") | test("bash.*outscraper")' "$tmp_config" >/dev/null 2>&1; then
		...
		if [[ -n "$outscraper_path" ]]; then
			...
			if [[ -f "$HOME/.config/maestro/credentials.sh" ]]; then
				...
				outscraper_key=$(source "$HOME/.config/maestro/credentials.sh" && echo "${OUTSCRAPER_API_KEY:-}")
			fi
...
cleanup_deprecated_mcps() {
	...
	local _sentinel="${HOME}/.maestro/.migrations/cleanup-deprecated-mcps-v2"
	if [[ ! -f "$_sentinel" ]]; then
		local cleaned=0
		local tmp_config
		tmp_config=$(mktemp)
...
# Uses MAESTRO_OPENCODE_VERSION_TIMEOUT (default 5s) so a hung Node.js process
...
_validate_opencode_config_schema() {
	...
	local _oc_timeout
	local _oc_rc
	_oc_timeout="${MAESTRO_OPENCODE_VERSION_TIMEOUT:-5}"
	_oc_rc=0
...
validate_opencode_config() {
	...
	if [[ "$needs_repair" == "true" ]]; then
		...
		local generator_script="$HOME/.maestro/agents/scripts/generate-opencode-agents.sh"
...
migrate_mcp_env_to_credentials() {
	...
	for rc_file in "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile"; do
		if [[ -f "$rc_file" ]] && grep -q 'source.*mcp-env\.sh' "$rc_file" 2>/dev/null; then
			...
			sed -i '' 's|source.*\.config/maestro/mcp-env\.sh|source "$HOME/.config/maestro/credentials.sh"|g' "$rc_file" 2>/dev/null ||
				sed -i 's|source.*\.config/maestro/mcp-env\.sh|source "$HOME/.config/maestro/credentials.sh"|g' "$rc_file" 2>/dev/null || true
			((++migrated))
...
migrate_old_backups() {
	local old_backup_dir="$HOME/.maestro/config-backups"
	...
	mkdir -p "$HOME/.maestro/agents-backups"
	mkdir -p "$HOME/.maestro/opencode-backups"
	...
	for backup in $(find "$old_backup_dir" -maxdepth 1 -type d -name "20*" 2>/dev/null | sort -r | head -n "$BACKUP_KEEP_COUNT"); do
		...
		if [[ -d "$backup/agents" ]]; then
			mv "$backup" "$HOME/.maestro/agents-backups/$backup_name"
			((++migrated))
		# Check if it contains opencode.json
		elif [[ -f "$backup/opencode.json" ]]; then
			mv "$backup" "$HOME/.maestro/opencode-backups/$backup_name"
			((++migrated))
		fi
	done
...
migrate_loop_state_directories() {
	local _sentinel="${HOME}/.maestro/.migrations/loop-state-dirs-migrated"
	if [[ -f "$_sentinel" ]]; then
		return 0
	fi
...
migrate_pulse_repos_to_repos_json() {
	local pulse_file="$HOME/.config/maestro/pulse-repos.json"
	local repos_file="$HOME/.config/maestro/repos.json"
...
#   - ~/.maestro/agents/scripts/supervisor-helper.sh (old entry point)
#   - ~/.maestro/agents/scripts/supervisor/ (old module directory)
#   - ~/.maestro/agents/scripts/archived/ (removed from repo)
#   - ~/.maestro/agents/scripts/supervisor-archived/ (removed from repo)
...
migrate_orphaned_supervisor() {
	...
	#    Old label: com.maestro.supervisor-pulse (from cron.sh/launchd.sh)
	#    New label: com.maestro.maestro-supervisor-pulse (from setup.sh)
	#    setup.sh already handles the new label cleanup at line ~1000, but
	#    the old label from cron.sh may also be present
	if [[ "$(uname -s)" == "Darwin" ]]; then
		local old_label="com.maestro.supervisor-pulse"
...
backfill_issue_relationships() {
	local marker_file="$HOME/.maestro/.migrations/t1889-relationships-backfill"
	local marker_dir
	...
	local sync_script="$HOME/.maestro/agents/scripts/issue-sync-helper.sh"
...
# Migrate maestro cron entries to systemd user timers (GH#17695 Finding D).
# On Linux systems with systemd, scans cron for maestro markers and removes
...
migrate_cron_to_systemd() {
	...
	local marker_dir="$HOME/.maestro/cache/migrations"
	...
	local cron_markers="maestro: stats-wrapper
maestro: gh-failure-miner
maestro: process-guard
maestro: memory-pressure-monitor
maestro: screen-time-snapshot
...
maestro-repo-sync"
	...
	local systemd_timers="maestro-stats-wrapper
maestro-gh-failure-miner
maestro-process-guard
maestro-memory-pressure-monitor
maestro-screen-time-snapshot
...
# entries even after maestro-routines disables r912, creating perpetual
# restart churn for an unmanaged service.
cleanup_legacy_dashboard_launchagent() {
	local label="com.maestro.dashboard"
	local systemd_unit="sh.maestro.dashboard"
	local legacy_systemd_unit="maestro-dashboard"
	local routines_todo="$HOME/Git/maestro-routines/TODO.md"
