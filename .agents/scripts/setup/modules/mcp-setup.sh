...
setup_localwp_mcp() {
	...
	if [[ "$install_mcp" =~ ^[Yy]?$ ]]; then
		if run_with_spinner "Installing LocalWP MCP server" npm_global_install "@verygoodplugins/mcp-local-wp"; then
			print_info "Start with: ~/.maestro/agents/scripts/localhost-helper.sh start-mcp"
...
_setup_browser_tools_install_bun() {
	...
	while IFS= read -r bun_rc; do
		...
		if ! grep -q '\.bun' "$bun_rc" 2>/dev/null; then
			{
				echo ""
				echo "# Bun (added by maestro setup)"
				echo "$bun_path_line"
...
_setup_browser_tools_dev_browser() {
	...
	local dev_browser_dir="$HOME/.maestro/dev-browser"
	...
	if dev_browser_output=$(bash "$HOME/.maestro/agents/scripts/dev-browser-helper.sh" setup 2>&1); then
		print_success "dev-browser installed"
		print_info "Start server with: bash ~/.maestro/agents/scripts/dev-browser-helper.sh start"
	else
		...
		print_info "  bash ~/.maestro/agents/scripts/dev-browser-helper.sh setup"
	fi
	return 0
}
...
_setup_opencode_plugins_register_file_url() {
	# Mechanism 1: register maestro plugin via file:// URL in opencode.json.
	# Also removes the broken opencode-cursor-oauth plugin if present.
	# Prints "true" or "false" to indicate registration status.
	local opencode_config="$1"
	local maestro_plugin_entrypoint="$2"
	local plugin_url="file://${maestro_plugin_entrypoint}"
	...
	if [[ "$already_registered" -eq 0 ]]; then
		...
		if jq --arg url "$plugin_url" \
			...
			mv "$tmp_config" "$opencode_config"
			print_success "maestro plugin registered in opencode.json"
		else
			rm -f "$tmp_config"
			print_warning "Failed to update opencode.json plugin array (file: $opencode_config)"
		fi
	else
		print_success "maestro plugin already registered in opencode.json"
	fi
...
_setup_opencode_plugins_register_symlink() {
	...
	local plugins_dir="$1"
	local maestro_plugin_src="$2"
	local maestro_plugin_dst="$3"

	mkdir -p "$plugins_dir"
	if [[ -L "$maestro_plugin_dst" ]]; then
		if [[ ! -e "$maestro_plugin_dst" ]]; then
			print_warning "Broken maestro plugin symlink detected; recreating"
			ln -sfn "$maestro_plugin_src" "$maestro_plugin_dst"
		fi
	elif [[ ! -d "$maestro_plugin_dst" ]]; then
		ln -sfn "$maestro_plugin_src" "$maestro_plugin_dst"
	fi
	return 0
}
...
_setup_opencode_plugins_print_pool_guidance() {
	...
	if [[ "$pool_plugin_registered" == "true" ]]; then
		print_info "Use the maestro OAuth pool (provided by the maestro plugin above):"
		print_info "  1. Run: opencode auth login"
		print_info "  2. Select: 'Anthropic Pool' (added by maestro plugin)"
		...
		print_warning "maestro OpenCode plugin was not registered; 'Anthropic Pool' may be unavailable"
		print_info "Re-run maestro setup to register the plugin, then run: opencode auth login"
	fi
	return 0
}
...
_setup_opencode_plugins_auth_guidance() {
	...
	if [[ "$builtin_auth_removed" == "true" ]]; then
		...
		print_info "  - Or use the maestro OAuth pool: Select 'Anthropic Pool' for multi-account rotation"
	fi
	return 0
}
...
setup_opencode_plugins() {
	...
	# Register maestro plugin using two complementary mechanisms:
	...
	local maestro_plugin_src="$HOME/.maestro/agents/plugins/opencode-maestro"
	local maestro_plugin_dst="$plugins_dir/opencode-maestro"
	local maestro_plugin_entrypoint="$maestro_plugin_src/index.mjs"

	if [[ ! -f "$maestro_plugin_entrypoint" ]]; then
		print_skip "OpenCode plugins" "maestro plugin entry point not found: $maestro_plugin_entrypoint"
		setup_track_deferred "OpenCode plugins" "Install/restore maestro plugin at $maestro_plugin_entrypoint"
		return 0
	fi
	...
	if opencode_config=$(find_opencode_config); then
		pool_plugin_registered=$(_setup_opencode_plugins_register_file_url "$opencode_config" "$maestro_plugin_entrypoint")
	else
	...
	_setup_opencode_plugins_register_symlink "$plugins_dir" "$maestro_plugin_src" "$maestro_plugin_dst"
...
setup_seo_mcps() {
	...
	if [[ -f "$HOME/.config/maestro/credentials.sh" ]]; then
		...
		source "$HOME/.config/maestro/credentials.sh"
		...
		print_info "Configure SEO API credentials in ~/.config/maestro/credentials.sh"
	fi

	# GSC uses MCP (OAuth2 complexity warrants it)
	local gsc_creds="$HOME/.config/maestro/gsc-credentials.json"
	if [[ -f "$gsc_creds" ]]; then
		print_success "Google Search Console credentials configured"
	else
		print_info "GSC: Create service account JSON at $gsc_creds"
		print_info "  See: ~/.maestro/agents/seo/google-search-console.md"
	fi

	print_info "SEO documentation: ~/.maestro/agents/seo/"
	return 0
}
...
setup_google_analytics_mcp() {
	...
	print_info "Documentation: ~/.maestro/agents/services/analytics/google-analytics.md"
...
_setup_quickfile_mcp_clone_and_build() {
	...
	if [[ ! "$install_qf" =~ ^[Yy]?$ ]]; then
		...
		print_info "Install later: mkdir -p ~/Git/mcp && git clone https://github.com/Aditya060806/quickfile-mcp.git ~/Git/mcp/quickfile-mcp"
		return 1
	fi

	if [[ ! -d "$quickfile_dir" ]]; then
		mkdir -p "$(dirname "$quickfile_dir")"
		if ! run_with_spinner "Cloning quickfile-mcp" git clone https://github.com/Aditya060806/quickfile-mcp.git "$quickfile_dir"; then
...
setup_quickfile_mcp() {
	...
	print_info "Documentation: ~/.maestro/agents/services/accounting/quickfile.md"
	return 0
}
