...
# Args:
#   $1 = service_name    (e.g. "maestro-stats-wrapper")
...
_install_scheduler_systemd() {
	...
	if [[ "$service_name" == "maestro-supervisor-pulse" ]]; then
	...
	printf '%s' "[Unit]
Description=maestro ${service_name}
...
" >"$service_file"
	...
	printf '%s' "[Unit]
Description=maestro ${service_name} Timer
...
# Reconcile known Linux maestro routines that may have migrated from cron to systemd.
...
# Args:
#   $1 = service_name   (systemd service name, e.g. "maestro-stats-wrapper")
#   $2 = cron_tag       (comment tag for cron line, e.g. "maestro: stats-wrapper")
...
#   $2 = launchd_label (e.g. "sh.maestro.stats-wrapper")
#   $3 = systemd_name  (e.g. "maestro-stats-wrapper")
#   $4 = cron_tag      (grep pattern for cron line, e.g. "maestro: stats-wrapper")
...
_uninstall_pulse() {
	...
	if [[ "$_os" == "Darwin" ]]; then
		...
		local service_name="maestro-supervisor-pulse"
		...
		if crontab -l 2>/dev/null | grep -qF "pulse-wrapper"; then
			crontab -l 2>/dev/null | grep -v 'maestro: supervisor-pulse' | crontab - || true
