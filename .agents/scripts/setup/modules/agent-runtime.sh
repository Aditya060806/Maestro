...
# Strips maestro-only frontmatter (mode, subagents); keeps standard fields
...
# _convert_agent_frontmatter: strips maestro-only fields from agent markdown.
...
_agent_source_dirs() {
	...
	for dir in "$agents_source"/*/; do
		...
		scripts | reference | prompts | templates | configs | hooks | \
			plugins | bundles | loop-state | advisories | maestro | \
...
# deploys maestro agents to each runtime's native agent directory.
...
deploy_agents_to_runtimes() {
	...
	local agents_source="${HOME}/.maestro/agents"
	...
	while IFS= read -r runtime_id; do
		...
		# runtime via MAESTRO_FEATURE_AGENTS_<SUFFIX>=no (see runtime-registry).
...
#
# Configurable via MAESTRO_DEPLOY_RUNTIMES_TIMEOUT (default 120s).
