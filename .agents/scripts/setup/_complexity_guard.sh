...
#
# Opt out by exporting MAESTRO_COMPLEXITY_GUARD=false before running setup.
...
#
# Opt out: MAESTRO_COMPLEXITY_GUARD=false
#######################################
setup_complexity_guard() {
	if [[ "${MAESTRO_COMPLEXITY_GUARD:-true}" == "false" ]]; then
		print_info "Complexity guard install disabled via MAESTRO_COMPLEXITY_GUARD=false"
		setup_track_skipped "Complexity guard" "opted out via MAESTRO_COMPLEXITY_GUARD=false"
		return 0
	fi
