...
#
# Opt out by exporting MAESTRO_TASK_ID_GUARD=false before running setup.
...
#
# Opt out: MAESTRO_TASK_ID_GUARD=false
#######################################
setup_task_id_guard() {
	if [[ "${MAESTRO_TASK_ID_GUARD:-true}" == "false" ]]; then
		print_info "Task-ID guard install disabled via MAESTRO_TASK_ID_GUARD=false"
		setup_track_skipped "Task-ID guard" "opted out via MAESTRO_TASK_ID_GUARD=false"
		return 0
	fi
