...
_should_setup_noninteractive_stats_wrapper() {
	if _should_setup_noninteractive_scheduler \
		"Stats wrapper" \
		"com.maestro.maestro-stats-wrapper" \
		"maestro: stats-wrapper" \
		"maestro-stats-wrapper"; then
		return 0
	fi
...
# Linux/systemd installs without maestro-complexity-scan.timer (GH#24841).
_should_setup_noninteractive_complexity_scan() {
	if _should_setup_noninteractive_scheduler \
		"Complexity scan" \
		"sh.maestro.complexity-scan" \
		"maestro: complexity-scan" \
		"maestro-complexity-scan"; then
		return 0
	fi
...
_should_setup_noninteractive_pulse_merge_routine() {
	if _should_setup_noninteractive_scheduler \
		"Pulse merge routine" \
		"sh.maestro.pulse-merge-routine" \
		"maestro: pulse-merge-routine" \
		"maestro-pulse-merge-routine"; then
		return 0
	fi
...
verified_install() {
	...
	tmp_script=$(mktemp "${TMPDIR:-/tmp}/maestro-install-XXXXXX") || {
...
# Creates: ~/.maestro/agents-backups/20251221_123456/
create_backup_with_rotation() {
	local source_path="$1"
	local backup_name="$2"
	local backup_base="$HOME/.maestro/${backup_name}-backups"
	local backup_dir
