...
#
# Opt out: export MAESTRO_CANONICAL_GUARD_INSTALL=false before setup.
...
setup_canonical_guard() {
	if [[ "${MAESTRO_CANONICAL_GUARD_INSTALL:-true}" == "false" ]]; then
		print_info "Canonical guard install disabled via MAESTRO_CANONICAL_GUARD_INSTALL=false"
		setup_track_skipped "Canonical guard" "opted out via MAESTRO_CANONICAL_GUARD_INSTALL=false"
		return 0
	fi
