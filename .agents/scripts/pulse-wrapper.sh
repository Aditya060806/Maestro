...
if [[ -z "${BASH_VERSION:-}" ]]; then
	printf '[pulse-wrapper] must be run by bash; use: bash -lc '\''source ~/.maestro/agents/scripts/pulse-wrapper.sh; <command>'\''\n' >&2
...
_pulse_wrapper_deployed_scripts_dir() {
	local agents_dir="${MAESTRO_AGENTS_DIR:-}"
	local home_dir="${HOME:-}"
	...
	if [[ -n "$home_dir" && -d "${home_dir}/.maestro/agents/scripts" ]]; then
		(cd "${home_dir}/.maestro/agents/scripts" && pwd -P) || return 1
		return 0
	fi
...
if [[ "$_pulse_skip_jitter" -eq 0 ]]; then
	_pw_ffjit_lockdir="${HOME}/.maestro/logs/pulse-wrapper.lockdir"
		...
		if [[ "$_pw_ffjit_pid" =~ ^[0-9]+$ ]] && kill -0 "$_pw_ffjit_pid" 2>/dev/null; then
			...
			if [[ "$_pw_ffjit_age" -le "${PULSE_LOCK_MAX_AGE_S:-1800}" ]]; then
				printf '[pulse-wrapper] another instance running (PID %s, age %ss), skipping\n' \
					...
					>>"${HOME}/.maestro/logs/pulse-wrapper.log" 2>/dev/null || true
				exit 0
			fi
...
# maestro gh shim. Re-prioritise the framework scripts directory once it is
...
#######################################
if [[ -f "${HOME}/.config/maestro/credentials.sh" ]]; then
	# shellcheck source=/dev/null
	if ! . "${HOME}/.config/maestro/credentials.sh" 2>/dev/null; then
...
_pulse_gh_rate_limit_json() {
	local _rate_json=""
	local _rc=0
	local _secs="${MAESTRO_GH_READ_TIMEOUT:-15}"
	[[ "$_secs" =~ ^[0-9]+$ ]] || _secs=15
...
_pulse_graphql_budget_priority_decision() {
	local _threshold="${MAESTRO_PULSE_OPTIONAL_BUDGET_THRESHOLD:-${MAESTRO_PULSE_PREFETCH_BUDGET_THRESHOLD:-1250}}"
...
_pulse_set_graphql_budget_priority() {
	...
	export MAESTRO_PULSE_GRAPHQL_BUDGET_CLASS="$_class"
	export MAESTRO_PULSE_GRAPHQL_BUDGET_REMAINING="$_remaining"
	export MAESTRO_PULSE_GRAPHQL_BUDGET_LIMIT="$_limit"
	export MAESTRO_PULSE_GRAPHQL_BUDGET_THRESHOLD="$_threshold"
...
_pulse_should_defer_budget_priority_stage() {
	local _stage="$1"
	[[ "${MAESTRO_PULSE_GRAPHQL_BUDGET_CLASS:-normal}" == "reserve" ]] || return 1
	case "$_stage" in
...
_pulse_defer_budget_priority_stage() {
	local _stage="$1"
	echo "[pulse-wrapper] budget-priority: deferred optional stage '${_stage}' (class=${MAESTRO_PULSE_GRAPHQL_BUDGET_CLASS:-unknown}, remaining=${MAESTRO_PULSE_GRAPHQL_BUDGET_REMAINING:-?}/${MAESTRO_PULSE_GRAPHQL_BUDGET_LIMIT:-?}, threshold=${MAESTRO_PULSE_GRAPHQL_BUDGET_THRESHOLD:-?})" >>"$LOGFILE"
	if declare -F pulse_stats_increment >/dev/null 2>&1; then
...
#   1. Session stop flag — `maestro pulse stop` creates this to pause
#      the pulse without uninstalling it. Checked first so stop always wins.
#   2. Session start flag — `maestro pulse start` creates this. If present,
...
check_session_gate() {
	...
	if [[ -f "$STOP_FLAG" ]]; then
		echo "[pulse-wrapper] Pulse paused (stop flag present) — resume with: maestro pulse start" >>"$LOGFILE"
		return 1
	fi
	...
	# (MAESTRO_SUPERVISOR_PULSE) > user JSONC config > defaults.
	...
	echo "[pulse-wrapper] Pulse not enabled — set orchestration.supervisor_pulse=true in config or run: maestro pulse start" >>"$LOGFILE"
	return 1
}
...
# new comment.
#######################################
TRIAGE_CACHE_DIR="${TRIAGE_CACHE_DIR:-${HOME}/.maestro/.agent-workspace/tmp/triage-cache}"
...
#
# State file: ~/.maestro/.agent-workspace/routine-state.json
# Schedule parser: routine-schedule-helper.sh
#######################################
ROUTINE_STATE_FILE="${HOME}/.maestro/.agent-workspace/routine-state.json"
...
#
# State file: ~/.maestro/logs/pulse-no-work-breaker.state
...
#   MAESTRO_NO_WORK_WINDOW_SECS  — rolling window duration (default 600)
#   MAESTRO_NO_WORK_WINDOW_MAX   — max events in window (default 10; 0=disable)
#   MAESTRO_SKIP_NO_WORK_BREAKER=1 — emergency bypass
#######################################
is_no_work_rate_acceptable() {
	# Emergency bypass.
	if [[ "${MAESTRO_SKIP_NO_WORK_BREAKER:-0}" == "1" ]]; then
		echo "[pulse-wrapper] MAESTRO_SKIP_NO_WORK_BREAKER=1 — bypassing no_work rate check (t2770)" >>"$LOGFILE"
		return 0
	fi

	local window_secs="${NO_WORK_WINDOW_SECS:-600}"
	local max_events="${NO_WORK_WINDOW_MAX:-10}"
	window_secs="${MAESTRO_NO_WORK_WINDOW_SECS:-$window_secs}"
	max_events="${MAESTRO_NO_WORK_WINDOW_MAX:-$max_events}"
	...
	local state_file="${HOME}/.maestro/logs/pulse-no-work-breaker.state"
	local logfile="${LOGFILE:-${HOME}/.maestro/logs/pulse.log}"
	local now
	now=$(date +%s)
...
#
# Bypass: MAESTRO_SKIP_PULSE_IDLE_BACKOFF=1
#
...
_pulse_run_deterministic_pipeline() {
	...
	local _pulse_merge_routine_last_run="${HOME}/.maestro/logs/pulse-merge-routine-last-run"
	local _pulse_merge_routine_lock_dir="${HOME}/.maestro/.agent-workspace/locks/pulse-merge-routine.lock"
	local _pmr_skip=0
	...
	if [[ -f "$STOP_FLAG" ]]; then
		...
		# breaker auto-trips an `maestro update`; if VERSION changed, the
...
main() {
	...
	# so every child shell stage sees MAESTRO_HEADLESS and
	...
	# If the source file changes during a cycle (e.g. maestro update / setup.sh),
	...
	# bug class as the maestro.sh getent regression (GH#18784) and the
	...
	# 60s plist (com.maestro.maestro-supervisor-merge) fires this path every
	...
	if [[ "${PULSE_DRY_RUN:-0}" != "1" && "${PULSE_CANARY_MODE:-0}" != "1" ]]; then
		...
		local _rl_ts_file="${HOME}/.maestro/logs/pulse-wrapper-last-run.ts"
		local _rl_now
		local _rl_last
		local _rl_elapsed
		_rl_now=$(date +%s)
	...
	if [[ "${MAESTRO_PULSE_REST_FIRST_READS:-1}" == "1" ]]; then
	...
	if [[ "${MAESTRO_PULSE_PR_VIEW_CACHE:-1}" == "1" ]]; then
		export MAESTRO_GH_PR_VIEW_CACHE=1
		export MAESTRO_GH_PR_VIEW_CACHE_DIR="${MAESTRO_PULSE_PR_VIEW_CACHE_DIR:-${HOME}/.maestro/cache/pulse-pr-view-cache}"
		export MAESTRO_GH_PR_VIEW_CACHE_TTL="${MAESTRO_PULSE_PR_VIEW_CACHE_TTL:-3600}"
		_save_cleanup_scope
		...
		if ! mkdir -p "$MAESTRO_GH_PR_VIEW_CACHE_DIR"; then
			printf '[pulse-wrapper] Failed to create REST PR view cache directory: %s\n' "$MAESTRO_GH_PR_VIEW_CACHE_DIR" >&2
		fi
		echo "[pulse-wrapper] Persistent PR view cache enabled for duplicate repo#PR reads dir=${MAESTRO_GH_PR_VIEW_CACHE_DIR} ttl=${MAESTRO_GH_PR_VIEW_CACHE_TTL}s (GH#23433/GH#23604/GH#26241)" >>"$LOGFILE"
	fi

	if [[ "${MAESTRO_PULSE_PR_LIST_CACHE:-1}" == "1" ]]; then
		export MAESTRO_GH_PR_LIST_CACHE_DIR="${TMPDIR:-/tmp}/maestro-pulse-pr-list-cache-${$}"
		export MAESTRO_GH_PR_LIST_CACHE_TTL="${MAESTRO_PULSE_PR_LIST_CACHE_TTL:-3600}"
		export PULSE_PR_LIST_PROVIDER_CACHE_DIR="${TMPDIR:-/tmp}/maestro-pulse-pr-list-provider-cache-${$}"
		export PULSE_PR_LIST_PROVIDER_CACHE_TTL="${MAESTRO_PULSE_PR_LIST_CACHE_TTL:-3600}"
		...
		push_cleanup "${_pulse_rm_rf} \"${MAESTRO_GH_PR_LIST_CACHE_DIR}\""
		push_cleanup "${_pulse_rm_rf} \"${PULSE_PR_LIST_PROVIDER_CACHE_DIR}\""
		if ! rm -rf "$MAESTRO_GH_PR_LIST_CACHE_DIR"; then
			printf '[pulse-wrapper] Failed to reset PR list cache directory: %s\n' "$MAESTRO_GH_PR_LIST_CACHE_DIR" >&2
		fi
		if ! mkdir -p "$MAESTRO_GH_PR_LIST_CACHE_DIR"; then
		...
		echo "[pulse-wrapper] Per-cycle PR list cache enabled for duplicate repo open-PR reads ttl=${MAESTRO_GH_PR_LIST_CACHE_TTL}s; provider exact-output cache enabled for GraphQL-only PR lists (GH#23604)" >>"$LOGFILE"
	else
	...
	if [[ "${MAESTRO_PULSE_FORCE_REST_READS_ON_LOW_GRAPHQL:-1}" == "1" ]] && declare -F _cb_rate_limit_json >/dev/null 2>&1; then
		local _gh22478_rate_json="" _gh22478_remaining="" _gh22478_threshold="${MAESTRO_GH_REST_FALLBACK_THRESHOLD:-3000}"
	...
	rm -f "${HOME}/.maestro/cache/pulse-cycle-"*"-consolidation-fired" 2>/dev/null || true
	...
	#
	# If maestro update / setup.sh deployed a new pulse-wrapper.sh while this
	...
	#
	# Opt-out: MAESTRO_SKIP_SOURCE_REEXEC=1 (debugging / testing).
	if [[ "${MAESTRO_SKIP_SOURCE_REEXEC:-0}" != "1" ]]; then
		local _pw_mtime_now
...
# `maestro update` would have bricked the pulse loop. Fix: do the check
