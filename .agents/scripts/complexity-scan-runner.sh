...
# under its own launchd schedule (sh.maestro.complexity-scan, hourly).
#
...
#
# Lock:    ~/.maestro/.agent-workspace/locks/complexity-scan.lock
# Runner log: ~/.maestro/logs/complexity-scan-runner.log
# Scan log: ~/.maestro/logs/pulse.log (the underlying function writes here)
# Last-run: ~/.maestro/logs/complexity-scan-last-run (touched by the scan itself)
#
...
LOCK_DIR="${HOME}/.maestro/.agent-workspace/locks/complexity-scan.lock"
...
Scheduled via launchd: sh.maestro.complexity-scan (hourly, RunAtLoad=true).
Install via setup.sh / setup_complexity_scan in .agents/scripts/setup/modules/schedulers.sh.

Paths:
  Lock dir:    ${LOCK_DIR}
  Runner log:  ${RUNNER_LOG_FILE}
  Scan log:    ${LOGFILE:-~/.maestro/logs/pulse.log}
  Last-run:    ${COMPLEXITY_SCAN_LAST_RUN:-~/.maestro/logs/complexity-scan-last-run}
