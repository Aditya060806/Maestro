...
cmd_setup() {
	...
	"$PLATFORM_GITHUB")
		...
		echo ""
		echo "See: ~/.maestro/agents/tools/git/opencode-github.md"
		;;
	"$PLATFORM_GITLAB")
		...
		echo ""
		echo "See: ~/.maestro/agents/tools/git/opencode-gitlab.md"
		;;
	*)
...
cmd_create_secure_workflow() {
	...
	# Check if maestro has the template
	local maestro_template="$HOME/Git/maestro/.github/workflows/opencode-agent.yml"

	mkdir -p .github/workflows

	if [[ -f "$maestro_template" ]]; then
		cp "$maestro_template" .github/workflows/opencode-agent.yml
		print_success "Copied secure workflow from maestro template"
	else
	...
	echo ""
	echo "Documentation: ~/.maestro/agents/tools/git/opencode-github-security.md"
	return 0
}
