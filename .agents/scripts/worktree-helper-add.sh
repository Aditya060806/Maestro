...
# Excludes maestro runtime directories that are safe to discard.
...
worktree_has_changes() {
	...
	if [[ -d "$worktree_path" ]]; then
		...
		local changes
		# Exclude maestro runtime files: .agents/loop-state/, .agents/tmp/, .DS_Store
...
# repository in ~/.config/maestro/repos.json. This is the last-line
...
generate_worktree_path() {
	...
	if declare -F maestro_generate_worktree_path >/dev/null 2>&1; then
		maestro_generate_worktree_path "$repo_root" "$branch"
		return $?
	fi
...
_interactive_session_auto_claim() {
	...
	if [[ -n "${FULL_LOOP_HEADLESS:-}" ]] || [[ -n "${MAESTRO_HEADLESS:-}" ]] ||
		[[ -n "${Claude_HEADLESS:-}" ]] || [[ -n "${OPENCODE_HEADLESS:-}" ]] ||
		[[ -n "${GITHUB_ACTIONS:-}" ]]; then
		return 0
	fi
	if [[ "${MAESTRO_SESSION_ORIGIN:-}" == "worker" ]]; then
		return 0
	fi
	# Opt-out for scripted bulk worktree operations
	if [[ -n "${MAESTRO_SKIP_AUTO_CLAIM:-}" ]]; then
		print_info "MAESTRO_SKIP_AUTO_CLAIM set — skipping worktree auto-claim (GH#20146 audit)"
		return 0
	fi
	...
	if [[ -x "${HOME}/.maestro/agents/scripts/interactive-session-helper.sh" ]]; then
		helper="${HOME}/.maestro/agents/scripts/interactive-session-helper.sh"
...
_apply_worktree_exclusions() {
	...
	local helper=""
	if [[ -x "${HOME}/.maestro/agents/scripts/worktree-exclusions-helper.sh" ]]; then
		helper="${HOME}/.maestro/agents/scripts/worktree-exclusions-helper.sh"
...
#
# Env override: MAESTRO_WORKTREE_ALLOW_NESTED=1 bypasses the check for rare
# legitimate cases (test fixtures, documented intent).
_cmd_add_assert_path_outside_repo() {
	local path="$1"
	local branch="$2"

	if [[ "${MAESTRO_WORKTREE_ALLOW_NESTED:-0}" = "1" ]]; then
		return 0
	fi
...

	{
		...
		echo ""
		echo "(Override: MAESTRO_WORKTREE_ALLOW_NESTED=1 — use only with a documented"
...
# Precedence:
#   1. Explicit --base <ref> (or MAESTRO_WORKTREE_BASE env var) — caller intent wins.
...
_resolve_worktree_base_ref() {
	...
	local env_base="${MAESTRO_WORKTREE_BASE:-}"
...
_cmd_add_create_worktree() {
	...
	echo -e "${YELLOW}  re-create the worktree with '--base <ref>' or set MAESTRO_WORKTREE_BASE.${NC}" >&2
