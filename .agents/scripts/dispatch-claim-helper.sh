...
#
# Config file (optional, user-level): ~/.config/maestro/dispatch-override.conf
...
# ~/.maestro/agents/VERSION. Legacy claims (pre-t2401) have no version field
...
# incidents.
DISPATCH_OVERRIDE_CONF="${DISPATCH_OVERRIDE_CONF:-${HOME}/.config/maestro/dispatch-override.conf}"
...
MAESTRO_VERSION_FILE="${MAESTRO_VERSION_FILE:-${HOME}/.maestro/agents/VERSION}"
...
#######################################
# t2401: Resolve the framework version from MAESTRO_VERSION_FILE.
...
_resolve_version() {
	if declare -F maestro_find_version >/dev/null 2>&1; then
		local detected
		detected=$(maestro_find_version 2>/dev/null || printf '%s' "$MAESTRO_UNKNOWN_VERSION")
		if [[ -n "$detected" && "$detected" != "$MAESTRO_UNKNOWN_VERSION" ]]; then
			printf '%s' "$detected"
			return 0
		fi
	fi
	if [[ -r "$MAESTRO_VERSION_FILE" ]]; then
		local ver
		ver=$(head -n1 "$MAESTRO_VERSION_FILE" 2>/dev/null | tr -d '[:space:]')
		if [[ -n "$ver" ]]; then
			printf '%s' "$ver"
			return 0
		fi
...
_claim_post_error_fallback_path() {
	if [[ -n "${HOME:-}" && -d "$HOME" && -w "$HOME" ]]; then
		local home_dir="${HOME%/}"
		local path="${home_dir}/.maestro-claim-post-error.$$.$RANDOM"
...
_post_claim() {
	...
	if declare -F maestro_can_manage_repo_issue_state >/dev/null 2>&1; then
		if ! maestro_can_manage_repo_issue_state "$repo_slug" "$runner"; then
	...
	local opencode_version="$MAESTRO_UNKNOWN_VERSION"
...
_guard_no_active_assignment_before_claim() {
	...
	if [[ "${MAESTRO_TEST_MODE:-}" == "1" && -z "${MAESTRO_FORCE_CLAIM_ASSIGNMENT_GUARD:-}" ]]; then
		return 0
	fi
