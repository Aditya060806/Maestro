...
#
# Environment:
#   MAESTRO_BODY_FORMAT_STRICT=1  — treat non-dispatchable as hard error (default: 0)
...
# Exit 1 if non-dispatchable (controlled by MAESTRO_BODY_FORMAT_STRICT).
# ---------------------------------------------------------------------------
cmd_check() {
	local body="$1"
	local strict="${MAESTRO_BODY_FORMAT_STRICT:-0}"
	...
	if [[ $non_dispatchable -eq 1 && "$strict" == "1" ]]; then
		log_error "issue-body-format: body is non-dispatchable (MAESTRO_BODY_FORMAT_STRICT=1)"
		return 1
	fi
