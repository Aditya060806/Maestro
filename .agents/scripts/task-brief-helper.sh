...
readonly SUPERVISOR_DB="${HOME}/.maestro/.agent-workspace/supervisor/supervisor.db"
...
_derive_attribution() {
	...
	elif [[ "$commit_author" != "Aditya060806" ]]; then
	...
	elif [[ "$commit_author" != "Aditya060806" && "$commit_author" != "GitHub Actions" ]]; then
...
#
# Environment:
#   MAESTRO_DISPATCH_PATH_FILES_CONF — override conf file path
...
_append_dispatch_path_notice() {
	...
	if [[ "${MAESTRO_SKIP_DISPATCH_PATH_CHECK:-}" == "1" ]]; then
		return 0
	fi
	...
	local _conf_file="${MAESTRO_DISPATCH_PATH_FILES_CONF:-${_script_dir}/../configs/self-hosting-files.conf}"
