...
All data stays local. Output goes to ~/.maestro/.agent-workspace/work/session-miner/
...
OUTPUT_DIR = Path.home() / ".maestro/.agent-workspace/work/session-miner"
...
def resolve_managed_history_db(db_path: Path) -> Path:
    ...
    if not VAULT_HISTORY_HELPER.exists():
        return db_path
    if "MAESTRO_VAULT_MANAGED_SESSION_HISTORY" not in __import__("os").environ:
        return db_path
