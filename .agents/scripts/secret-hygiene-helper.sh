...
readonly SCRIPT_DIR
readonly ADVISORIES_DIR="$HOME/.maestro/advisories"
...
# Scan maestro credentials.sh (check 1)
_scan_maestro_creds() {
	local creds="$HOME/.config/maestro/credentials.sh"
	[[ -f "$creds" ]] || return 0
	local perms
...
_scan_ssh_and_env() {
	...
	if [[ "${env_count:-0}" -gt 0 ]]; then
		report_finding "medium" "$HOME/Git/**/.env" \
			...
			"Ensure .env is in .gitignore; migrate secrets to gopass: maestro secret set NAME"
	fi
...
cmd_startup_check() {
	...
	for py in /opt/homebrew/opt/python@*/bin/python3.* /usr/local/bin/python3.*; do
		...
		if [[ -f "$sp/litellm_init.pth" ]]; then
			echo "CRITICAL: LiteLLM supply chain malware detected! Run in your terminal: maestro security"
			return 1
		fi
		for pth in "$sp"/*.pth; do
			[[ -f "$pth" ]] || continue
	...
	if [[ "$findings" -gt 0 ]]; then
		echo "WARNING: $findings suspicious .pth file(s) found. Run in your terminal: maestro security"
		return 1
	fi

	if [[ "$plaintext_count" -gt 0 ]]; then
		echo "Secret hygiene: $plaintext_count high-risk plaintext credential location(s). Run in your terminal: maestro security"
		return 1
	fi
...
cmd_scan() {
	...
	for advisory in "$ADVISORIES_DIR"/*.advisory; do
		...
		cat "$advisory"
		echo ""
		echo -e "  Dismiss after taking action: maestro security dismiss $advisory_id"
		echo ""
	done
	...
	if [[ "$FINDINGS_TOTAL" -eq 0 ]]; then
		...
		echo "  maestro security dismiss <advisory-id>"
	fi
...
print_usage() {
	cat <<'EOF'
Usage: maestro security [command]
...
Commands:
  maestro security              Full scan (posture + secrets + supply chain)
  maestro security scan         Secret hygiene & supply chain scan
  maestro security scan-pth     Python .pth file audit only (supply chain IoC)
  maestro security scan-secrets Plaintext secret locations only
  maestro security scan-deps    Unpinned dependency check only
  maestro security scan-remotes Git remote URL credential audit (t2458)
  maestro security posture      Interactive security posture setup
  maestro security dismiss <id> Dismiss a security advisory after taking action

Advisory System:
  Advisories are delivered via maestro updates as files in:
    ~/.maestro/advisories/<id>.advisory

  They appear in the session greeting until dismissed.
  Dismiss after taking action: maestro security dismiss <id>

Examples:
  maestro security                        # Full scan (recommended)
  maestro security scan-pth               # Check for .pth malware
  maestro security dismiss litellm-2026-03  # Dismiss after rotating creds
EOF
	return 0
}
