...
_foss_recent_runtime_evidence() {
	local session_key="$1"
	local mode="$2"
	local metrics_file="${FOSS_RUNTIME_METRICS_FILE:-${HOME}/.maestro/logs/headless-runtime-metrics.jsonl}"
...
_post_triage_escalation_comment() {
	...
	infra-markers-after-extraction)
		...
		;;
	oversized-output)
		reason_human="Worker produced a suspiciously long output (>20KB of extracted text). Likely a malfunctioning worker — format drift, runaway tool exploration, or a prompt that failed to constrain the response. See \`~/.maestro/logs/triage-review-debug.log\` for a redacted sample (t2019)."
		;;
	esac

	# Compose signature footer via the canonical helper.
	local footer=""
	if [[ -x "$HOME/.maestro/agents/scripts/gh-signature-helper.sh" ]]; then
		footer=$("$HOME/.maestro/agents/scripts/gh-signature-helper.sh" footer \
			--model "pulse-triage" \
...
2. **Force a retry** (optional) — delete the cache entry to let the next pulse cycle re-attempt:
    ...
    \`\`\`bash
    rm -f ~/.maestro/.agent-workspace/tmp/triage-cache/$(echo "$repo_slug" | tr '/' '_')-${issue_num}.hash
...
# to ~/.maestro/logs/triage-review-debug.log so future failures can
...
_log_suppressed_triage_output() {
	...
	local debug_log="${HOME}/.maestro/logs/triage-review-debug.log"
...
dispatch_triage_reviews() {
	local available="$1"
	local repos_json="${2:-${REPOS_JSON:-~/.config/maestro/repos.json}}"
...
#######################################
relabel_needs_info_replies() {
	local repos_json="${1:-${REPOS_JSON:-~/.config/maestro/repos.json}}"
...
dispatch_foss_workers() {
	...
	while IFS=$'\t' read -r foss_slug foss_path disclosure labels_filter_json; do
		...
		"$HEADLESS_RUNTIME_HELPER" run \
			...
			</dev/null >>"${HOME}/.maestro/logs/pulse-foss-${foss_issue_num}.log" 2>&1 &
		sleep 2
