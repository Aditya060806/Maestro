...
# /tmp/pulse-*.log names collide across multiple maestro users on one server;
# this helper prefers XDG_RUNTIME_DIR when available and falls back to the
# maestro user workspace on macOS, cron, SSH, and headless sessions.

[[ -n "${_PULSE_TEMP_HELPER_LOADED:-}" ]] && return 0
_PULSE_TEMP_HELPER_LOADED=1

maestro_pulse_tmp_root() {
	local root=""
	if [[ -n "${MAESTRO_PULSE_TMP_DIR:-}" ]]; then
		root="$MAESTRO_PULSE_TMP_DIR"
	elif [[ -n "${XDG_RUNTIME_DIR:-}" && -d "${XDG_RUNTIME_DIR:-}" && -w "${XDG_RUNTIME_DIR:-}" ]]; then
		root="${XDG_RUNTIME_DIR}/maestro/pulse"
	else
		root="${HOME}/.maestro/.agent-workspace/tmp/pulse"
	fi
...
maestro_pulse_worker_log_candidates() {
	local repo_slug="$1"
	local issue_number="$2"
	local primary="" fallback=""
	primary=$(maestro_pulse_worker_log_path "$repo_slug" "$issue_number") || return 1
	fallback=$(maestro_pulse_worker_log_fallback_path "$issue_number") || return 1
	printf '%s\n%s\n' "$primary" "$fallback"
	return 0
}

maestro_pulse_tmp_cleanup() {
	local max_age_minutes="${1:-2880}"
	local root=""
	[[ "$max_age_minutes" =~ ^[0-9]+$ ]] || max_age_minutes=2880
	root=$(maestro_pulse_tmp_root) || return 0
