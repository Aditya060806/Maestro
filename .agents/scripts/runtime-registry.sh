...
# repo-local commands — handled separately by `maestro init`).
_RT_COMMAND_DIR=(
...
# Values:
#   managed     maestro can route this runtime's session/history store behind
...
#               Vault protection.
#   external    runtime stores history outside local maestro control.
...
# Vault-managed path relative to ${MAESTRO_VAULT_MANAGED_HISTORY_ROOT}.
...
# =============================================================================
# Per-runtime toggles for which maestro features are enabled during setup.
# Values: "yes" / "no".
#
# Defaults:
...
#   MAESTRO_FEATURE_AGENTS_<SUFFIX>=no
#   MAESTRO_FEATURE_COMMANDS_<SUFFIX>=no
#   MAESTRO_FEATURE_MEMORY_<SUFFIX>=yes
...
_RT_FEATURE_COMMANDS=(
	...
	"yes" # droid
	"yes" # gemini-cli
	"yes" # windsurf (via .windsurf/workflows/ symlink from `maestro init`)
	"yes" # continue
	"yes" # kilo
	"yes" # kiro
	"yes" # aider
...
# --- Memory mining flag ---
# Whether maestro attempts to mine this runtime's session history for
...
rt_vault_session_history_path() {
	...
	root="${MAESTRO_VAULT_MANAGED_HISTORY_ROOT:-$HOME/.maestro/.agent-workspace/vault/managed-session-history}"
	echo "${root%/}/$rel"
	return 0
}
...
#   MAESTRO_FEATURE_<FEATURE>_<RUNTIME_SUFFIX>   (value: "yes" | "no")
...
rt_feature_agents() {
	...
	varname="MAESTRO_FEATURE_AGENTS_${suffix}"
	override="${!varname:-}"
...
rt_feature_commands() {
	...
	local suffix varname override
	suffix=$(_rt_env_suffix "$id")
	varname="MAESTRO_FEATURE_COMMANDS_${suffix}"
	override="${!varname:-}"
	if [[ "$override" == "yes" || "$override" == "no" ]]; then
		echo "$override"
		return 0
...
rt_feature_memory() {
	...
	local suffix varname override
	suffix=$(_rt_env_suffix "$id")
	varname="MAESTRO_FEATURE_MEMORY_${suffix}"
	override="${!varname:-}"
	if [[ "$override" == "yes" || "$override" == "no" ]]; then
		echo "$override"
		return 0
