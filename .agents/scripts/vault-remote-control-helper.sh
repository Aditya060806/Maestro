...
MESSAGE_HELPER="${MAESTRO_VAULT_MESSAGE_HELPER:-${SCRIPT_DIR}/vault-message-helper.sh}"
VAULT_HELPER="${MAESTRO_VAULT_HELPER:-${SCRIPT_DIR}/vault-helper.sh}"
AUDIT_HELPER="${MAESTRO_VAULT_AUDIT_HELPER:-${SCRIPT_DIR}/vault-audit-helper.sh}"
...
def vault_dir_from(value: str | None) -> Path:
    ...
    configured = os.environ.get("MAESTRO_VAULT_DIR")
    if configured:
        return Path(configured).expanduser()
    return Path.home() / ".config" / "maestro" / "vault"
...
def require_no_secret_env() -> None:
    for name in os.environ:
        if name.startswith("MAESTRO_VAULT_REMOTE_TEST_"):
            continue
        if name.startswith("MAESTRO_") and any(marker in name.upper() for marker in SENSITIVE_ENV_MARKERS):
...
def require_controller_sudo_and_tty() -> None:
    ...
    assume_root = os.environ.get("MAESTRO_VAULT_REMOTE_TEST_ASSUME_ROOT") == "1"
...
def apply_command(vault_dir: Path, vault_helper: str, audit_helper: str, body: dict[str, Any], sender: str) -> str:
    ...
    if action == ACTION_UNLOCK_GRANT:
        if hasattr(os, "geteuid") and os.geteuid() != 0 and os.environ.get("MAESTRO_VAULT_REMOTE_TEST_ASSUME_ROOT") != "1":
...
def cmd_receive(args: argparse.Namespace, message_helper: str, vault_helper: str, audit_helper: str) -> int:
    ...
    receive_env["MAESTRO_VAULT_MESSAGE_UNLOCKED"] = "1"
    subprocess.run(
