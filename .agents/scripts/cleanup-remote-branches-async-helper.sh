...
# MAESTRO_REMOTE_BRANCH_CLEANUP_APPLY=1 opt-in and still delegates safety
...
MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING="${MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING:-1000}"
MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING="${MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING//[!0-9]/}"
[[ -n "$MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING" ]] || MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING=1000
...
_gh_budget_ok() {
	if [[ "${MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_RATE_LIMIT:-0}" == "1" ]]; then
		return 0
	fi
	if [[ "${MAESTRO_REMOTE_BRANCH_CLEANUP_SKIP_GH:-0}" == "1" ]]; then
		return 0
	fi
	...
	min_remaining="$MAESTRO_REMOTE_BRANCH_CLEANUP_MIN_GH_REMAINING"
...
_run_cleanup_for_repo() {
	...
	if [[ "${MAESTRO_REMOTE_BRANCH_CLEANUP_APPLY:-0}" == "1" ]]; then
		apply_args+=(--apply)
	fi
	if [[ "${MAESTRO_REMOTE_BRANCH_CLEANUP_INCLUDE_CLOSED_PR:-0}" == "1" ]]; then
		apply_args+=(--include-closed-pr)
	fi

	printf '[cleanup-remote-branches-async] Auditing repo=%s mode=%s\n' \
		"$repo_path" "$([[ "${MAESTRO_REMOTE_BRANCH_CLEANUP_APPLY:-0}" == "1" ]] && printf apply || printf dry-run)" >>"$LOGFILE"
