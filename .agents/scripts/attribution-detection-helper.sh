...
# attribution-detection-helper.sh — Monitor GitHub for maestro framework copies (t1883)
#
# Uses GitHub Code Search to detect when distinctive maestro patterns appear in
...
# ~/.maestro/cache/ — not committed to the public repo — to avoid tipping off
...
#
# State files:
#   ~/.maestro/cache/attribution-canaries.json  — canary patterns (auto-created)
#   ~/.maestro/cache/attribution-detections.json — scan results (auto-created)
#
...
CACHE_DIR="${HOME}/.maestro/cache"
LOG_DIR="${HOME}/.maestro/logs"
...
PLIST_LABEL="sh.maestro.attribution-detection"
...
# Default canary patterns — distinctive maestro strings unlikely to appear elsewhere
# These are embedded in the public framework and can be searched on GitHub
DEFAULT_CANARIES='[
  {
    "name": "spdx-header",
    "pattern": "SPDX-FileCopyrightText: 2025-2026 Aditya Pandey and Harvest",
    "description": "Standard SPDX copyright header used in all maestro files",
    "attributed_repos": ["Aditya060806/Maestro"]
  },
  {
    "name": "maestro-sh-domain",
    "pattern": "maestro.sh",
...
cmd_canary_add() {
	...
	new_entry=$(jq -n \
		...
		'{name: $name, pattern: $pattern, description: $description, attributed_repos: ["Aditya060806/Maestro"]}')
...
cmd_setup_private_repo() {
	...
	printf '  gh repo create <your-org>/maestro-provenance --private\n\n'
	printf "${YELLOW}Step 2: Clone it locally${NC}\n"
	printf '  git clone git@github.com:<your-org>/maestro-provenance.git\n'
	printf '  cd maestro-provenance\n\n'
	printf "${YELLOW}Step 3: Initialize structure${NC}\n"
	printf '  mkdir -p scripts config reports\n'
	printf '  cp ~/.maestro/cache/attribution-canaries.json config/search-strings.json\n'
...
cmd_install() {
	...
	if [[ "${MAESTRO_SCHEDULER:-}" == "launchd" ]] || [[ "$(uname -s)" == "Darwin" ]]; then
...
_install_cron() {
	local script_path="$1"
	local cron_entry="0 3 * * 1 ${script_path} scan >> ${LOGFILE} 2>&1 # maestro: attribution-detection"
...
cmd_help() {
	cat <<'EOF'
attribution-detection-helper.sh — Monitor GitHub for maestro framework copies
...
State files (local, not committed to public repo):
  ~/.maestro/cache/attribution-canaries.json   — canary patterns
  ~/.maestro/cache/attribution-detections.json — scan results
  ~/.maestro/logs/attribution-detection.log    — operation log
