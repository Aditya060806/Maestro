...
# =============================================================================
# maestro Headless Runtime Provider — Auth & Backoff Functions (GH#19699)
# =============================================================================
...
provider_signature_override_var() {
	local provider="$1"
	case "$provider" in
	anthropic) printf '%s' "MAESTRO_HEADLESS_AUTH_SIGNATURE_ANTHROPIC" ;;
	openai) printf '%s' "MAESTRO_HEADLESS_AUTH_SIGNATURE_OPENAI" ;;
	*) printf '%s' "" ;;
	esac
	return 0
}
...
clamp_rate_limit_retry_seconds() {
	...
	local max_seconds="${MAESTRO_OAUTH_POOL_MAX_RATE_LIMIT_COOLDOWN_SECONDS:-21600}"
...
record_startup_no_model_feedback() {
	...
	local threshold="${MAESTRO_STARTUP_NO_MODEL_FEEDBACK_THRESHOLD:-2}"
	local window_seconds="${MAESTRO_STARTUP_NO_MODEL_FEEDBACK_WINDOW_SECONDS:-900}"
	local cooldown_seconds="${MAESTRO_STARTUP_NO_MODEL_FEEDBACK_COOLDOWN_SECONDS:-300}"
