...
# =============================================================================
# maestro Issue Sync — Relationships & Backfill (GH#19502)
# =============================================================================
...
_init_node_id_cache() {
	if [[ -z "$_NODE_ID_CACHE_FILE" ]]; then
		_NODE_ID_CACHE_FILE=$(mktemp "${TMPDIR:-/tmp}/maestro-node-cache.XXXXXX")
		_NODE_ID_RATE_LIMITED_FILE=$(mktemp "${TMPDIR:-/tmp}/maestro-ratelimited.XXXXXX")
...
_detect_parent_from_gh_state() {
	...
	# GitHub number ("#NNN"), or the maestro GH#NNN notation.
	local parent_ref
