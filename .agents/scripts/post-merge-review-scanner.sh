...
#        SCANNER_CURSOR_DIR (default ~/.maestro/logs/post-merge-review-scanner)
...
#
# t1386: https://github.com/Aditya060806/Maestro/issues/2785
...
if [[ -z "${SCANNER_CURSOR_DIR:-}" ]]; then
	if [[ -n "${HOME:-}" ]]; then
		SCANNER_CURSOR_DIR="${HOME}/.maestro/logs/post-merge-review-scanner"
	else
		SCANNER_TMP_USER="${USER:-${LOGNAME:-$(id -un || printf '%s' shared)}}"
		SCANNER_CURSOR_DIR="/tmp/.maestro-${SCANNER_TMP_USER}/logs/post-merge-review-scanner"
	fi
fi
...
create_issue() {
	...
	# against pulse-wrapper.sh forgetting to export MAESTRO_HEADLESS=true
...
_refresh_one_issue() {
	...
	# helper emits a block starting with "<!-- maestro:sig -->".
	local old_body_stripped="${old_body%%<!-- maestro:sig -->*}"
	local new_body_stripped="${new_body_with_sig%%<!-- maestro:sig -->*}"
