...
# Pulse-driven IMAP polling for the maestro knowledge plane.
#
...
#   ~/.config/maestro/mailboxes.json      Personal / global config
#
...
_LOCK_DIR="${HOME}/.maestro/.agent-workspace/locks"
...
cmd_tick() {
	...
	if ! config_path=$(_find_config 2>&1); then
		log_info "skipped — no mailboxes configured; run maestro email mailbox add to enable"
		return 0
	fi
...
cmd_help() {
	cat <<'EOF'
email-poll-helper.sh — IMAP polling for the maestro knowledge plane (t2855)
...
Config locations (first-found wins):
  ...
  ~/.config/maestro/mailboxes.json            Personal / global

State:  _knowledge/.imap-state.json
Inbox:  _knowledge/inbox/

Credentials:
  Store passwords in gopass: maestro secret set email/<mailbox-id>/password
  Reference in config as:    "password_ref": "gopass:maestro/email/<id>/password"

Setup:
  maestro email mailbox add                   Interactive guided setup
  maestro email mailbox list                  Show all mailboxes
  maestro email mailbox test <id>             Test connection
  maestro email mailbox remove <id>           Remove a mailbox
EOF
	return 0
}
