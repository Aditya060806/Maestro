...
# Auto Version Bump Script for Maestro Framework
...
main() {
	...
	if [[ -x "$VERSION_MANAGER" ]]; then
		...
		if [[ $? -eq 0 ]]; then
			...
			git add VERSION README.md sonar-project.properties setup.sh maestro.sh package.json .claude-plugin/marketplace.json 2>/dev/null
...
if [[ $# -eq 0 && ! -d .git ]]; then
	echo "Auto Version Bump for Maestro Framework"
	echo ""
