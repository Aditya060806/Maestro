...
# =============================================================================
# maestro Update Check - Clean version check for session start
# =============================================================================
...
get_remote_version() {
	...
	if command -v jq &>/dev/null; then
		...
		version=$(curl --proto '=https' -fsSL "https://api.github.com/repos/Aditya060806/Maestro/contents/VERSION" 2>/dev/null | jq -r '.content // empty' 2>/dev/null | base64 -d 2>/dev/null | tr -d '\n')
		if [[ -n "$version" ]]; then
			echo "$version"
			return 0
		fi
	...
	# Use --proto =https to enforce HTTPS and prevent protocol downgrade
	curl --proto '=https' -fsSL "https://raw.githubusercontent.com/Aditya060806/Maestro/main/VERSION" 2>/dev/null || echo "unknown"
}
...
_build_version_str() {
	...
	if [[ "$current" == "unknown" ]]; then
		...
		echo "maestro v$current (unable to check for updates)"
		return 0
...
_is_auto_update_active() {
	local launchd_label="com.maestro.maestro-auto-update"
	if launchctl list "$launchd_label" &>/dev/null; then
		return 0
	fi
	if crontab -l 2>/dev/null | grep -q 'maestro-auto-update'; then
		return 0
	fi
	return 1
}
...
_check_gh_slurp_prerequisite() {
	if ! declare -F maestro_gh_slurp_supported >/dev/null 2>&1; then
		echo ""
		return 0
	fi
	if maestro_gh_slurp_supported; then
		echo ""
		return 0
	fi
	if declare -F maestro_gh_slurp_warning_line >/dev/null 2>&1; then
		maestro_gh_slurp_warning_line
	else
		printf '[WARN] GitHub CLI (gh) does not satisfy the maestro gh api --paginate --slurp prerequisite. Run maestro status.'
	fi
	return 0
}
...
_check_advisories() {
	...
	for advisory_file in "$advisories_dir"/*.advisory; do
		...
		if [[ -n "$first_line" ]]; then
			local entry
			entry=$(printf "%s Run in your terminal: \`maestro security\` | Dismiss: \`maestro security dismiss %s\`" "$first_line" "$adv_id")
...
_check_contribution_watch() {
	...
	if [[ "${cw_count:-0}" -gt 0 ]]; then
		contribution_watch="${cw_count} external contribution(s) need your reply (run \`maestro contributions\` to see them)."
	fi
...
# Reads auto_hotfix_accept from ~/.maestro/configs/auto-hotfix.conf.
...
_hotfix_auto_apply() {
	...
	local framework_repo="${MAESTRO_FRAMEWORK_REPO:-$HOME/Git/maestro}"
	...
	# by setting MAESTRO_SKIP_PULSE_RESTART when false.
	local _skip_env=""
	if [[ "$auto_restart_pulse" != "true" ]]; then
		_skip_env="MAESTRO_SKIP_PULSE_RESTART=1"
	fi
...
_check_hotfix_available() {
	...
	local agents_dir="${MAESTRO_DIR:-$HOME/.maestro}"
	...
	if [[ "${MAESTRO_FORCE_HOTFIX_BANNER:-}" == "1" ]]; then
	...
	echo "Hotfix available: v${latest_hotfix_version} (current: v${current_version}). Run 'maestro update' to apply, or set auto_hotfix_accept=true in ${hotfix_conf}"
	return 0
}
...
_check_script_drift() {
	...
	local setup_script="$framework_repo/setup.sh"
	if [[ ! -x "$setup_script" ]]; then
		echo "Script drift detected (${deployed_sha:0:7}→${current_sha:0:7}) but setup.sh not executable — run: cd ~/Git/maestro && ./setup.sh --non-interactive"
		return 0
	fi
...
_check_origin() {
	local agents_dir="${MAESTRO_DIR:-$HOME/.maestro}"
	local framework_repo="$HOME/Git/maestro"
	local origin_notice=""
	...
	origin_notice="Fork detected: origin is ${slug}. Check https://github.com/Aditya060806/Maestro for upstream updates."
	...
	local canary_endpoint="https://provenance-ping.Aditya060806.workers.dev/ping"
...
# -----------------------------------------------------------------------------
_check_signing() {
	local dismissed_file="$HOME/.maestro/cache/signing-nudge-dismissed"
	...
	if [[ "$signing_format" != "ssh" || "$signing_enabled" != "true" ]]; then
		echo "Commit signing not configured. Run: maestro signing setup"
		return 0
	fi
...
# -----------------------------------------------------------------------------
_check_pulse_health() {
	local health_file="$HOME/.maestro/logs/pulse-health.json"
	...
	if [[ "$age_seconds" -gt 1800 ]]; then
		...
		if [[ "$(uname)" == "Darwin" ]]; then
			local plist_label="com.maestro.maestro-supervisor-pulse"
			local launchd_state=""
...
_refresh_oauth_tokens() {
	local agents_dir="${MAESTRO_DIR:-$HOME/.maestro}/agents"
	local oauth_helper="$agents_dir/scripts/oauth-pool-helper.sh"
	if [[ -f "$oauth_helper" && -f "$HOME/.maestro/oauth-pool.json" ]]; then
		(
...
_run_session_advisories() {
	...
	# Belt-and-braces alongside the `maestro update` cron path — covers
...
main() {
	...
	if is_headless "$@"; then
		local current
		current=$(get_version)
		echo "maestro v$current (headless - skipped update check)"
		return 0
	fi
	...
	local cache_dir="$HOME/.maestro/cache"
	...
	# Opt-out: MAESTRO_AUTO_UPGRADE_BASH=0 disables.
