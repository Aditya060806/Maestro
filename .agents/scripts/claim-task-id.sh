...
#                              or value from .maestro.json "remote" key)
#   --counter-branch BRANCH    Branch holding .task-counter (default: main,
#                              or value from .maestro.json "counter_branch" key)
...
#
# Project-level config (.maestro.json in repo root):
#   {
#     "remote": "upstream",
...
#   CLI flags --remote and --counter-branch override .maestro.json values.
#
# Exit codes:
...
# Remote and branch — defaults; overridden by .maestro.json and/or CLI flags
REMOTE_NAME="origin"
COUNTER_BRANCH="main"
DEFAULT_BRANCH="main"
...
# Load project-level config from .maestro.json in the repo root.
...
load_project_config() {
	local repo_path="$1"
	local config_file="${repo_path}/.maestro.json"
	...
	if ! command -v jq &>/dev/null; then
		log_warn ".maestro.json found but jq is not installed — project config ignored"
		return 0
	fi

	log_info "Loading project config from .maestro.json"
	...
	if [[ -n "$remote_val" ]] && [[ "$_REMOTE_NAME_SET" == "false" ]]; then
		...
		log_info "remote set from .maestro.json: $REMOTE_NAME"
	fi

	if [[ -n "$counter_branch_val" ]] && [[ "$_COUNTER_BRANCH_SET" == "false" ]]; then
		COUNTER_BRANCH="$counter_branch_val"
		log_info "counter_branch set from .maestro.json: $COUNTER_BRANCH"
	fi

	if [[ -n "$default_branch_val" ]]; then
		DEFAULT_BRANCH="$default_branch_val"
		log_info "default_branch set from .maestro.json: $DEFAULT_BRANCH"
	fi
...
_validate_and_normalize_args() {
	...
	# Non-blocking by default; set MAESTRO_BODY_FORMAT_STRICT=1 to hard-fail.
...
# Advisory by default; MAESTRO_BODY_FORMAT_STRICT=1 hard-fails on non-dispatchable.
...
_validate_description_format() {
	...
	if [[ $rc -eq 1 && "${MAESTRO_BODY_FORMAT_STRICT:-0}" != "1" ]]; then
...
# Warns when claim-task-id.sh is called from a non-maestro repo with a title
...
check_framework_routing() {
	...
	# Check if we're already in the maestro repo — no routing needed
	local remote_url
	...
	if [[ "$detection_result" == "framework" ]]; then
		...
		log_warn "  Framework issues should be filed on Aditya060806/Maestro, not this repo."
...
#      temp file ($MAESTRO_LABEL_CACHE_FILE or auto-created).
...
_validate_labels_exist() {
	...
	local cache_file="${MAESTRO_LABEL_CACHE_FILE:-}"
	if [[ -z "$cache_file" ]]; then
		cache_file=$(mktemp /tmp/maestro-label-cache-XXXXXX 2>/dev/null) || return 0
		export MAESTRO_LABEL_CACHE_FILE="$cache_file"
...
#
# Env vars:
#   MAESTRO_CLAIM_DEDUP_DAYS   recency window for merged PRs (default 14)
#
# Test hooks (for test-claim-task-id-discovery.sh only):
#   _MAESTRO_CLAIM_TEST_IS_TTY  "1" → force interactive path in non-TTY tests
#   _MAESTRO_CLAIM_TEST_ANSWER  "y"/"n" → override user prompt answer
# ---------------------------------------------------------------------------
...
_pc_prompt_or_warn() {
	...
	[[ ${#hits[@]} -eq 0 ]] && return 0
	local is_tty=false
	if [[ "${_MAESTRO_CLAIM_TEST_IS_TTY:-0}" == "1" ]]; then
		is_tty=true
	elif [[ -t 0 && -t 1 ]]; then
		is_tty=true
	fi
	...
	if [[ "$is_tty" == "true" ]]; then
		...
		local answer="${_MAESTRO_CLAIM_TEST_ANSWER:-}"
...
_pre_claim_discovery_pass() {
	local title="$1"
	local repo_slug="$2"
	local dedup_days="${MAESTRO_CLAIM_DEDUP_DAYS:-14}"
...
#
# Environment:
#   MAESTRO_SKIP_DISPATCH_PATH_CHECK=1 — disable entirely
# ---------------------------------------------------------------------------
_warn_dispatch_path_auto_dispatch() {
	[[ "${MAESTRO_SKIP_DISPATCH_PATH_CHECK:-}" == "1" ]] && return 0
	...
	local _conf_file="${MAESTRO_DISPATCH_PATH_FILES_CONF:-${SCRIPT_DIR}/../configs/self-hosting-files.conf}"
	local _patterns=()
...
main() {
	...
	# with #dispatch-path-ok or MAESTRO_SKIP_DISPATCH_PATH_CHECK=1.
	_warn_dispatch_path_auto_dispatch

	# Framework routing guard: warn if title looks like a framework issue
	# but we're not in the maestro repo (GH#5149)
