...
#   - Cache: SQLite at ~/.maestro/.agent-workspace/model-availability.db
#
# Exit codes:
...
readonly AVAILABILITY_DIR="${HOME}/.maestro/.agent-workspace"
...
get_tier_models() {
	...
	# User-local override checked first — survives maestro update.
...
resolve_api_key() {
	...
	if [[ -n "$key_vars" ]]; then
		...
		if command -v gopass &>/dev/null; then
			for var_name in "${var_names[@]}"; do
				local gopass_path="maestro/${var_name}"
