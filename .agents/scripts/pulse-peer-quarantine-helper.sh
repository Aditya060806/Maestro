...
#   help                                   — show usage.
#
# State file: ~/.maestro/cache/peer-quarantine.json (v1 schema).
# Advisory:   ~/.maestro/advisories/peer-quarantine-<peer>.advisory
# Stamp:      ~/.maestro/cache/peer-quarantine-advisory-<peer>.stamp (24h dedup).
# Override:   ~/.config/maestro/dispatch-override.conf (shared with t2422).
#
...
# Paths.
PEER_QUARANTINE_CACHE_DIR="${PEER_QUARANTINE_CACHE_DIR:-${HOME}/.maestro/cache}"
PEER_QUARANTINE_STATE_FILE="${PEER_QUARANTINE_STATE_FILE:-${PEER_QUARANTINE_CACHE_DIR}/peer-quarantine.json}"
PEER_QUARANTINE_ADVISORY_DIR="${PEER_QUARANTINE_ADVISORY_DIR:-${HOME}/.maestro/advisories}"
PEER_QUARANTINE_OVERRIDE_CONF="${PEER_QUARANTINE_OVERRIDE_CONF:-${HOME}/.config/maestro/dispatch-override.conf}"
...
_pq_ensure_dirs() {
	...
	# Override conf parent (~/.config/maestro) — best-effort, may already exist.
...
#######################################
# Atomically rewrite ~/.config/maestro/dispatch-override.conf so that the
