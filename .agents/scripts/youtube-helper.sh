...
readonly YT_CACHE_DIR="$HOME/.cache/maestro/youtube"
...
resolve_sa_key() {
	...
	if [[ -z "$key_file" ]] && [[ -f "$HOME/.config/maestro/credentials.sh" ]]; then
		key_file=$(grep -oP 'GCP_SA_KEY_FILE="\K[^"]+' "$HOME/.config/maestro/credentials.sh" 2>/dev/null | head -1 || true)
	...
	# Default location
	if [[ -z "$key_file" ]]; then
		key_file="$HOME/.config/maestro/keys/evergreen-je-sa.json"
	fi

	if [[ ! -f "$key_file" ]]; then
		print_error "Service account key not found at: $key_file"
		print_info "Store your key: cp <key.json> ~/.config/maestro/keys/evergreen-je-sa.json && chmod 600 ~/.config/maestro/keys/evergreen-je-sa.json"
		print_info "Then add to credentials.sh: export GCP_SA_KEY_FILE=\"\$HOME/.config/maestro/keys/evergreen-je-sa.json\""
		return 1
	fi
...
Authentication:
  ...
  Key file: ~/.config/maestro/keys/evergreen-je-sa.json
