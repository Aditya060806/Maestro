...
# Stagehand Python Setup Script for Maestro Framework
...
readonly STAGEHAND_PYTHON_CONFIG_DIR="${HOME}/.maestro/stagehand-python"
...
async def search_products(query: str, max_results: int = 5) -> ProductResults:
    ...
    try:
        ...
        # Save results
        results_dir = f"{os.path.expanduser('~')}/.maestro/stagehand-python/results"
...
main() {
	...
	"setup")
		...
		print_info "2. Configure API keys in ~/.maestro/stagehand-python/.env"
		print_info "3. Activate venv: source ~/.maestro/stagehand-python/.venv/bin/activate"
		print_info "4. Try examples: cd ~/.maestro/stagehand-python && python examples/basic_example.py" || exit
		;;
	"examples")
		create_python_examples
		;;
