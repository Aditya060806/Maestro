...
# feature-toggle-helper.sh - Backward-compatible wrapper for maestro config
#
# DEPRECATED: This script delegates to config-helper.sh (JSONC config system).
# Kept for backward compatibility with existing `maestro config` invocations.
...
echo "  Run 'maestro update' to restore the JSONC config system." >&2
exit 1
