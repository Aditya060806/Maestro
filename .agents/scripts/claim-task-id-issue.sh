...
_auto_assign_issue() {
	...
	if [[ -z "${FULL_LOOP_HEADLESS:-}${MAESTRO_HEADLESS:-}${OPENCODE_HEADLESS:-}${GITHUB_ACTIONS:-}" ]]; then
		local _isc_helper=""
...
# prompt-injection. The approval marker (<!-- maestro-signed-approval -->)
...
_compose_issue_body() {
	...
	if [[ -n "$description" ]]; then
		...
		log_error "  OR: gh issue create --title \"${title}\" --body \"<description>\"" # maestro-allow: raw-gh-wrapper
		echo ""
		return 1
	fi
...
_ensure_todo_entry_written() {
	...
	if [[ -n "$labels" ]]; then
		...
		for label in $labels; do
			...
			[[ -z "$label" ]] && continue
			case "$label" in
			status:* | tier:* | origin:* | dispatched:* | implemented:* | maestro:*)
				continue ;;
