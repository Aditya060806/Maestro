...
_create_health_issue() {
	...
	local sig_footer=""
	sig_footer=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$health_body" 2>/dev/null || true)
	health_body="${health_body}${sig_footer}"

	# t2691/GH#20311: Health dashboard issues are always created by the
	# headless pulse context. Force MAESTRO_SESSION_ORIGIN=worker so
	# gh_create_issue applies origin:worker regardless of whether the
	# caller has MAESTRO_HEADLESS set (defense-in-depth; stats-wrapper.sh
	# already exports MAESTRO_HEADLESS=true, but this guards test contexts
	# and any future invocation path that bypasses that wrapper).
	local health_issue_number
	health_issue_number=$(MAESTRO_SESSION_ORIGIN=worker gh_create_issue --repo "$repo_slug" \
