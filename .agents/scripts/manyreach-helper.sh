...
#
# Part of maestro outreach tooling (t1513)
...
resolve_api_key() {
	...
	if command -v gopass &>/dev/null; then
		local key=""
		key=$(gopass show -o "maestro/MANYREACH_API_KEY" 2>/dev/null || true)
		if [[ -n "$key" ]]; then
			echo "$key"
			return 0
		fi
...
manyreach_request() {
	...
	api_key=$(resolve_api_key) || {
		...
		log_info "Set with: maestro secret set MANYREACH_API_KEY"
		log_info "Or add MANYREACH_API_KEY to ~/.config/maestro/credentials.sh"
		return 1
	}
