...
check_sonarcloud_status() {
	...
	local gate_response
	if gate_response=$(curl -s "https://sonarcloud.io/api/qualitygates/project_status?projectKey=Aditya060806_maestro"); then
		local gate_status
	...
	local response
	if response=$(curl -s "https://sonarcloud.io/api/issues/search?componentKeys=Aditya060806_maestro&impactSoftwareQualities=MAINTAINABILITY&resolved=false&ps=1&facets=rules"); then
		local total_issues
...
check_pulse_canary() {
	...
	defaults_source=".agents/configs/maestro.defaults.jsonc"
	defaults_target="${sandbox_home}/.maestro/agents/configs/maestro.defaults.jsonc"
