...
#
# Usage:
#   ~/.maestro/agents/scripts/pre-edit-check.sh
#   ~/.maestro/agents/scripts/pre-edit-check.sh --loop-mode --file "path/to/file"
#   ~/.maestro/agents/scripts/pre-edit-check.sh --loop-mode --task "description"
#   ~/.maestro/agents/scripts/pre-edit-check.sh --check-command "git push --force origin main"
#   ~/.maestro/agents/scripts/pre-edit-check.sh --verify-op "git push --force origin main"
#
...
# Works in both deployed (~/.maestro/agents/scripts/) and dev (.agents/scripts/) contexts.
_resolve_triggers_config() {
	...
	# Deployed layout: ~/.maestro/agents/scripts/ -> ~/.maestro/agents/configs/
...
_show_protected_branch_warning() {
	...
	if command -v wt &>/dev/null; then
		...
		echo "    ~/.maestro/agents/scripts/worktree-helper.sh add {type}/{description}"
...
_check_task_assignee() {
	...
	# Must match get_maestro_identity() in pulse-session-helper.sh
	local my_identity
	my_identity="${MAESTRO_IDENTITY:-$(whoami 2>/dev/null || echo unknown)@$(hostname -s 2>/dev/null || echo local)}"
