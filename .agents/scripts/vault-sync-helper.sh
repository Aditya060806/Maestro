...
def cmd_export(args: argparse.Namespace) -> int:
    ...
    ciphertext = AESGCM(b64d(str(key[FIELD_TRANSPORT_KEY]))).encrypt(nonce, canonical(plaintext), b"maestro-vault-sync-record")
...
def cmd_import(args: argparse.Namespace) -> int:
    ...
    try:
        plaintext = AESGCM(b64d(str(key[FIELD_TRANSPORT_KEY]))).decrypt(
            b64d(str(ciphertext["nonce"])), b64d(str(ciphertext["payload"])), b"maestro-vault-sync-record"
        )
