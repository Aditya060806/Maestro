...
Constants are read from:
  1. ~/.maestro/cch-constants.json (if --cache or file exists)
...
CACHE_FILE = Path.home() / ".maestro" / "cch-constants.json"
...
def load_constants(use_cache: bool = True) -> dict:
    ...
    try:
        result = subprocess.run(
            [str(Path.home() / ".maestro" / "agents" / "scripts" / "cch-extract.sh")],
            capture_output=True,
            text=True,
            timeout=10,
        )
