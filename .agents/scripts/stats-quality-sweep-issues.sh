...
_ensure_quality_issue() {
	...
	local cache_file="${HOME}/.maestro/logs/quality-issue-${slug_safe}"
	...
	if [[ -z "$issue_number" ]]; then
		...
		local qa_sig=""
		qa_sig=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$qa_body" 2>/dev/null || true)
...
${sonar_section}"
	...
	local gate_sig=""
	gate_sig=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$issue_body" 2>/dev/null || true)
...
_compute_qlty_grade_from_count() {
	...
	# (~/.maestro/agents/) and development (~/Git/maestro/.agents/) trees.
...
"
	...
	cat <<BODY
<!-- maestro:generator=function-complexity-sweep cited_file=${file_path} smell_count=${smell_count} -->
...
_gather_quality_issue_stats() {
	...
	local slug_safe="${repo_slug//\//-}"
	local scan_state_file="${HOME}/.maestro/logs/review-scan-state-${slug_safe}.json"
	...
	repo_path=$(jq -r --arg slug "$repo_slug" \
		...
		"${HOME}/.config/maestro/repos.json" 2>/dev/null) || repo_path=""
	local state_file=""
	if [[ -n "$repo_path" ]]; then
