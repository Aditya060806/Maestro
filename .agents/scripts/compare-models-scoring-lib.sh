...
check_provider_key() {
	...
	if command -v gopass &>/dev/null && gopass ls "maestro/${key_name}" &>/dev/null 2>&1; then
		FOUND_SOURCE="gopass"
		return 0
	fi
...
probe_provider() {
	...
	if [[ -n "${!key_name:-}" ]]; then
		key_value="${!key_name}"
	elif command -v gopass &>/dev/null && gopass ls "maestro/${key_name}" &>/dev/null 2>&1; then
		key_value=$(gopass show "maestro/${key_name}" 2>/dev/null) || return 1
	else
		return 1
	fi
...
RESULTS_DB="${MAESTRO_WORKSPACE_DIR:-$HOME/.maestro/.agent-workspace}/memory/model-comparisons.db"
