...
# ~/.config/maestro/repos.json (.initialized_repos[].path), 1 otherwise.
...
#
# Env:
#   MAESTRO_REPOS_JSON — override the default repos.json path (for tests)
#   MAESTRO_CANONICAL_EXTRA_PATHS — optional newline-separated extra paths
...
is_registered_canonical() {
	...
	if [[ -n "${MAESTRO_CANONICAL_EXTRA_PATHS:-}" ]]; then
		registered_paths="${registered_paths}
${MAESTRO_CANONICAL_EXTRA_PATHS}"
	fi
