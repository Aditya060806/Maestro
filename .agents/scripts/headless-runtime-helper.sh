...
readonly STATE_DIR="${MAESTRO_HEADLESS_RUNTIME_DIR:-${HOME}/.maestro/.agent-workspace/headless-runtime}"
...
readonly HEADLESS_SANDBOX_TIMEOUT_DEFAULT="${MAESTRO_HEADLESS_SANDBOX_TIMEOUT:-3600}"
readonly OPENCODE_AUTH_FILE="${HOME}/.local/share/opencode/auth.json"
readonly LOCK_DIR="${STATE_DIR}/locks"
readonly METRICS_DIR="${HOME}/.maestro/logs"
...
_cleanup_headless_runtime_temp_paths() {
	...
	while IFS= read -r path; do
		[[ -n "$path" ]] || continue
		case "$path" in
		"$tmp_root"/maestro-* | /tmp/maestro-* | /var/folders/*/T/*/maestro-*)
...
_prepare_runtime_prompt_transport() {
	...
	local prompt_dir=""
	prompt_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-headless-prompt.XXXXXX") || return 0
...
_invoke_opencode() {
	...
	if [[ "${MAESTRO_HEADLESS_AUTH_ISOLATION:-1}" == "1" ]]; then
		...
		if [[ -n "${MAESTRO_WORKER_PREWARM_DIR:-}" && -d "${MAESTRO_WORKER_PREWARM_DIR:-}" ]]; then
			isolated_data_dir="$MAESTRO_WORKER_PREWARM_DIR"
			print_info "[lifecycle] opencode_warm_done dir=$isolated_data_dir (reusing pre-warmed dir) pid=$$"
		else
			isolated_data_dir=$(mktemp -d "${TMPDIR:-/tmp}/maestro-worker-auth.XXXXXX")
		fi
	...
	(
		...
		if [[ -x "$SANDBOX_EXEC_HELPER" && "${MAESTRO_HEADLESS_SANDBOX_DISABLED:-}" != "1" ]]; then
			...
			if [[ "${MAESTRO_HEADLESS_SANDBOX_DISABLED:-}" == "1" ]]; then
				print_info "MAESTRO_HEADLESS_SANDBOX_DISABLED=1 — using bare timeout (no privilege isolation) (GH#20146 audit)"
			fi
...
#   - MAESTRO_VERBOSE_LIFECYCLE=1     — extra checkpoint emits in worker log
#   - MAESTRO_WORKER_PREFLIGHT_SENTINEL=1 — fail-fast preflight check
...
_t3077_setup_fix_the_fixer_observability() {
	...
	export MAESTRO_VERBOSE_LIFECYCLE=1
	export MAESTRO_WORKER_PREFLIGHT_SENTINEL=1
...
#
# When MAESTRO_WORKER_PREFLIGHT_SENTINEL=1, write a sentinel file before
...
#
# Sentinel path: ~/.maestro/cache/worker-preflight/<pid>.txt
# On write failure, returns 1 — caller aborts dispatch with exit code 11.
# When MAESTRO_WORKER_PREFLIGHT_SENTINEL is unset/empty, returns 0 (no-op).
#
# Returns: 0 success or no-op, 1 on write failure (caller aborts)
#######################################
_t3077_write_preflight_sentinel() {
	if [[ "${MAESTRO_WORKER_PREFLIGHT_SENTINEL:-}" != "1" ]]; then
		return 0
	fi

	local sentinel_dir="${HOME}/.maestro/cache/worker-preflight"
...
_metric_failure_excerpt_path() {
	...
	local excerpt_dir="${HOME}/.maestro/logs/worker-failure-excerpts"
...
_execute_run_attempt() {
	...
	# worker:
	#   - MAESTRO_VERBOSE_LIFECYCLE=1   — extra checkpoints in worker log
	#   - HEADLESS_ACTIVITY_TIMEOUT_SECONDS=180  — tighter watchdog (vs 600s)
	#   - MAESTRO_WORKER_PREFLIGHT_SENTINEL=1  — fail-fast preflight check
	#
	...
	# When MAESTRO_WORKER_PREFLIGHT_SENTINEL=1 (set by the helper above
	...
	if [[ -x "$RESOURCE_METRICS_HELPER" ]]; then
		"$RESOURCE_METRICS_HELPER" sample \
			...
			--interval "${MAESTRO_RESOURCE_SAMPLE_INTERVAL_SECONDS:-30}" >/dev/null 2>&1 &
		resource_sampler_pid="$!"
	fi

	# t3077 — spawn the verbose lifecycle watcher (background subshell).
	# Fail-open: returns silently if MAESTRO_VERBOSE_LIFECYCLE != 1.
	...
	*) _invoke_opencode "$output_file" "$exit_code_file" "${cmd[@]}" ;;
	...
	if [[ "$runtime" != "claude" ]] && _opencode_project_table_migration_replay_detected "$exit_code" "$output_file"; then
		...
		unset MAESTRO_WORKER_PREWARM_DIR
...
cmd_run() {
	...
	if [[ "$role" == "worker" ]]; then
		...
		# The sandbox allowlist already forwards MAESTRO_*; legacy generic
		...
		_worker_session_origin="${MAESTRO_SESSION_ORIGIN:-worker}"
		local MAESTRO_SESSION_ORIGIN
		MAESTRO_SESSION_ORIGIN="$_worker_session_origin"
		export MAESTRO_SESSION_ORIGIN
		local _worker_headless_marker
		_worker_headless_marker="${MAESTRO_HEADLESS:-true}"
		local MAESTRO_HEADLESS
		MAESTRO_HEADLESS="$_worker_headless_marker"
		export MAESTRO_HEADLESS
	fi
...
Defaults:
  ...
  MAESTRO_HEADLESS_MODELS is deprecated — respected as override for one release cycle.
  MAESTRO_HEADLESS_PROVIDER_ALLOWLIST can restrict selection to providers like: openai
  MAESTRO_HEADLESS_VARIANT_SONNET / MAESTRO_HEADLESS_VARIANT_OPUS can set tier defaults.
  GPT-5.5 sonnet-tier worker dispatch omits env-derived variants so OpenCode sends no explicit thinking override.
  MAESTRO_HEADLESS_VARIANT sets an OpenCode model variant (for example: high, xhigh).
  MAESTRO_HEADLESS_PULSE_VARIANT / MAESTRO_HEADLESS_WORKER_VARIANT override by role.
  MAESTRO_HEADLESS_APPEND_CONTRACT=0 disables worker /full-loop contract injection
