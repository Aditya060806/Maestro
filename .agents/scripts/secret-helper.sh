...
is_tenant_loader() {
	[[ -f "$CREDENTIALS_FILE" ]] && grep -q 'MAESTRO_ACTIVE_TENANT=' "$CREDENTIALS_FILE" 2>/dev/null
}
...
read_secret_input() {
	...
	if [[ "$value" =~ ^[[:space:]]*maestro[[:space:]]+secret[[:space:]]+set([[:space:]]|$) ]]; then
		print_error "Input for $name looks like a command, not a secret value" >&2
		print_info "Paste the secret value itself, not 'maestro secret set ...'" >&2
		return 1
	fi
...
# Initialize gopass store for maestro
cmd_init() {
	if ! command -v gopass &>/dev/null; then
	...
	# Create maestro subfolder if it doesn't exist
	...
	print_success "gopass initialized for maestro"
	print_info "Store secrets: maestro secret set SECRET_NAME"
	print_info "Import existing: maestro secret import-credentials"
	return 0
}
...
cmd_set() {
	...
	if [[ -z "$name" ]]; then
		print_error "Usage: maestro secret set SECRET_NAME"
		return 1
	fi
	...
	if has_gopass; then
		...
		print_info "Recommend: maestro secret init (to enable encrypted storage)"
	fi
	print_info "Verify key name only: maestro secret list"
...
cmd_get() {
	...
	if [[ -z "$name" ]]; then
		print_error "Usage: maestro secret get SECRET_NAME"
		return 1
	fi
...
cmd_list() {
	...
	if [[ "$has_secrets" == "false" ]]; then
		...
		print_info "Add secrets: maestro secret set SECRET_NAME"
	fi
...
cmd_run() {
	...
	if [[ ${#cmd_args[@]} -eq 0 ]]; then
		print_error "Usage: maestro secret run COMMAND [ARGS...]"
		return 1
	fi
...
cmd_run_specific() {
	...
	if [[ ${#cmd_args[@]} -eq 0 ]]; then
		print_error "Usage: maestro secret NAME [NAME...] -- COMMAND [ARGS...]"
		return 1
	fi
...
_import_credential_file() {
	...
	while IFS= read -r line; do
		if [[ "$line" =~ ^export[[:space:]]+([A-Z_][A-Z0-9_]*)=(.*) ]]; then
			...
			# Build gopass path: maestro/NAME or maestro/tenants/TENANT/NAME
...
cmd_import_credentials() {
	if ! has_gopass; then
		print_error "gopass not initialized. Run: maestro secret init"
		return 1
	fi
	...
	if [[ $imported -gt 0 ]]; then
		print_info "Verify with: maestro secret list"
...
cmd_status() {
	...
	if command -v gopass &>/dev/null; then
		...
		if gopass ls &>/dev/null; then
			...
			echo -e "                Run: maestro secret init"
		fi
	else
...
cmd_help() {
	echo ""
	print_info "Maestro - Secret Management"
	echo ""
	...
	echo "  maestro secret set STRIPE_KEY"
	echo ""
	...
	echo "  maestro secret list"
	echo ""
	echo "  # Run a command with secrets injected (output redacted)"
	echo "  maestro secret run curl -H 'Authorization: Bearer \$STRIPE_KEY' https://api.stripe.com/v1/charges"
	echo ""
	echo "  # Inject specific secrets only"
	echo "  maestro secret GITHUB_TOKEN -- gh api /user"
	echo ""
	echo "  # Import existing credentials to gopass"
	echo "  maestro secret import-credentials"
	echo ""
	print_info "Security:"
	echo ""
