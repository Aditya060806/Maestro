...
#                                       (default: ~/.maestro/logs/headless-runtime-metrics.jsonl)
...
#   MAESTRO_SKIP_DISPATCH_BACKOFF=1  — emergency bypass (dispatch proceeds unconditionally)
#
# Integration:
...
#
# Counter:
#   dispatch_backoff_skipped in ~/.maestro/logs/pulse-stats.json
#   (via pulse-stats-helper.sh / pulse_stats_increment). Surfaced by `maestro status`.
#
...
DISPATCH_BACKOFF_METRICS_FILE="${DISPATCH_BACKOFF_METRICS_FILE:-${HOME}/.maestro/logs/headless-runtime-metrics.jsonl}"
...
: "${MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_OVERRIDE:=0}"
...
_db_check_rate_limit_release_circuit_comment() {
	...
	if [[ "$MAESTRO_RATE_LIMIT_RELEASE_CIRCUIT_OVERRIDE" == "1" ]]; then
		return 0
	fi
...
check_dispatch_backoff() {
	...
	# Emergency bypass.
	if [[ "${MAESTRO_SKIP_DISPATCH_BACKOFF:-0}" == "1" ]]; then
		echo "${_DB_LOG_PREFIX} MAESTRO_SKIP_DISPATCH_BACKOFF=1 — bypassing rate-limit backoff check" >>"$LOGFILE"
		return 0
	fi
...
**Recommended actions:**
1. Check \`maestro status\` for current GraphQL budget and account pool state
...
_main() {
	...
	case "$cmd" in
		...
		help | --help | -h)
			...
			printf '  MAESTRO_SKIP_DISPATCH_BACKOFF=1   emergency bypass\n'
			return 0
			;;
		*)
