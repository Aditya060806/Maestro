...
HOOK_MARKER="# maestro-canonical-guard"
...
DEPLOYED_HOOK="$HOME/.maestro/agents/hooks/canonical-on-main-guard.sh"
...
cmd_install() {
	...
	if [[ -f "$hook_path" ]]; then
		if grep -q "$HOOK_MARKER" "$hook_path" 2>/dev/null; then
			...
			print_error "existing post-checkout hook at $hook_path is NOT managed by maestro"
...
# Dispatcher that calls the maestro canonical-on-main guard.
# Bypass with: MAESTRO_CANONICAL_GUARD=bypass git checkout ...
# Strict mode: MAESTRO_CANONICAL_GUARD=strict git checkout ...
...
HOOKEOF
	...
	print_info "bypass: MAESTRO_CANONICAL_GUARD=bypass git checkout ..."
	print_info "strict: MAESTRO_CANONICAL_GUARD=strict git checkout ..."
	return 0
}
...
cmd_uninstall() {
	...
	print_warning "post-checkout hook at $hook_path is NOT managed by maestro — leaving it alone"
	return 1
}
...
cmd_status() {
	...
	if grep -q "$HOOK_MARKER" "$hook_path" 2>/dev/null; then
		printf 'post-checkout hook: installed (maestro canonical-on-main guard)\n'
		printf '  path: %s\n' "$hook_path"
	else
