...
2. **Config file** — `~/.config/maestro/simplex-bot.json`
...
| `/status` | Y | Y | Show maestro system status |
...
| Command | DM | Group | Description |
|---------|:--:|:-----:|-------------|
| `/run <command>` | Y | - | Execute maestro CLI command (requires approval) |
...
Config (src/config.ts) — env > file > defaults
    |
Runner (src/runner.ts) — maestro CLI bridge
```
