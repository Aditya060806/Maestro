...
describe("ApprovalManager", () => {
  ...
  describe("classify", () => {
    test("allowlisted commands return 'allowed'", () => {
      expect(manager.classify("maestro status")).toBe("allowed");
      expect(manager.classify("maestro repos")).toBe("allowed");
      expect(manager.classify("maestro features")).toBe("allowed");
    });
    ...
    test("allowlist matches prefix (allows args after allowlisted command)", () => {
      expect(manager.classify("maestro status --verbose")).toBe("allowed");
    });
  ...
  describe("custom config", () => {
    test("custom allowlist overrides defaults", () => {
      ...
      expect(custom.classify("maestro status")).toBe("approval-required");
    });
