 ...
 *
 * Bridges bot commands to the maestro runner system.
 ...
 *
 * The allowlist entries are full command strings (e.g. "maestro status"), so
...
/**
 * Execute an maestro CLI command via Bun.spawn.
 *
...
export async function executeCommand(
  ...
  try {
    ...
    const proc = Bun.spawn(parts, {
      ...
      env: {
        ...process.env,
        // Ensure maestro scripts can find their config
        HOME: homedir(),
      },
    });
