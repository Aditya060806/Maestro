...
#
# Monitors maestro process health: individual RSS, process runtime, process count.
# This is the RIGHT signal — the March 3 kernel panic was caused by maestro
...
#   Total maestro:   > 8 GB aggregate RSS → warning
#
...
#   AGGREGATE_RSS_WARN_MB     Total maestro RSS warning (default: 8192)
...
#
# If no modern bash is installed (user set MAESTRO_AUTO_UPGRADE_BASH=0),
...
# Paths
readonly LOG_DIR="${MEMORY_LOG_DIR:-${HOME}/.maestro/logs}"
readonly LOG_FILE="${LOG_DIR}/memory-pressure.log"
readonly STATE_DIR="${HOME}/.maestro/.agent-workspace/tmp"

readonly LAUNCHD_LABEL="sh.maestro.memory-pressure-monitor"
readonly PLIST_PATH="${HOME}/Library/LaunchAgents/${LAUNCHD_LABEL}.plist"

# Process name patterns to monitor (maestro ecosystem)
...
_check_aggregate_and_os() {
	...
	if [[ "$total_rss_mb" -ge "$AGGREGATE_RSS_WARN_MB" ]]; then
		_check_findings+=("WARNING|aggregate|0|Total maestro RSS: ${total_rss_mb} MB across ${process_count} processes (limit: ${AGGREGATE_RSS_WARN_MB} MB)")
		_check_has_warning=true
	fi
...
_status_print_os_and_config() {
	...
	local lock_dir="${TMPDIR:-/tmp}/maestro-memory-pressure.lockdir"
...
cmd_install() {
	...
	local installed_path="${HOME}/.maestro/agents/scripts/${SCRIPT_NAME}.sh"
...
<dict>
	...
	<string>${home_escaped}/.maestro/logs/memory-pressure-launchd.log</string>
