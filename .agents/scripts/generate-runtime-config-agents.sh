...
# =============================================================================
# Agent auto-discovery from ~/.maestro/agents/*.md is handled by the Python
...
_generate_greeting_agents_md() {
	...
	opencode)
		...
		cache_path="~/.maestro/cache/session-greeting.txt"
		plugin_name="opencode-maestro plugin"
		# shellcheck disable=SC2088
		global_config_path="~/.config/opencode/opencode.json"
		;;
	claude-code)
		# shellcheck disable=SC2088
		cache_path="~/.maestro/cache/session-greeting-claude-code.txt"
		plugin_name="Claude-maestro plugin"
		# shellcheck disable=SC2088
		global_config_path="~/.config/Claude/Claude.json"
		;;
	*)
		# shellcheck disable=SC2088
		cache_path="~/.maestro/cache/session-greeting-${runtime_id}.txt"
		plugin_name="${runtime_id}-maestro plugin"
		# shellcheck disable=SC2088
...
1. Read line 1 of \`${cache_path}\`. Format: \`maestro v{X} running in ${display_name} v{Y} | ...\`. Extract \`{X}\` and \`{Y}\`.
2. Before any tool call or task work, make the first visible text in your first assistant response exactly this template — no extra prose, no status dump:
       ...
       We're running maestro v{X} in ${display_name} v{Y}.
...
If the user later asks about maestro updates, direct them to run \`maestro update\` in a terminal session (or type \`!maestro update\` below). Do not announce updates unprompted — the toast already did.
...
_generate_agents_opencode() {
	...
	# The opencode-maestro plugin already renders full framework status as a
	# session-start toast; the model must NOT re-run maestro-update-check.sh
...
#
# Multiple source files with the same basename (e.g. `maestro/architecture.md`
...
_generate_agents_claude() {
	...
	# reads the correct maestro + runtime version from cache (t2766 Phase B).
