...
#
# Strategy:
#   - Iterate ~/.config/maestro/repos.json `initialized_repos[]` entries
...
MARKER_FILE="${HOME}/.maestro/logs/.migrated-health-issue-duplicates-t2687"
...
parse_args() {
	...
	# per maestro Quality Rules ("never use $1 directly in function bodies").
	while (($# > 0)); do
		local arg="$1"
		case "$arg" in
