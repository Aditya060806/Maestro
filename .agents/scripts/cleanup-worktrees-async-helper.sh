...
#
# Lifecycle:
#   1. Acquire a mkdir-based single-runner lock (~/.maestro/logs/cleanup_worktrees.lock).
#   2. Check cadence gate: skip if last successful run < N minutes ago.
#   3. Source pulse-cleanup.sh deps and call cleanup_worktrees.
#   4. Update ~/.maestro/logs/cleanup_worktrees.last-run on success.
...
#     >>"${HOME}/.maestro/logs/cleanup_worktrees.log" 2>&1 &
...
#   ~/.maestro/logs/cleanup_worktrees.log      — progress log
#   ~/.maestro/logs/cleanup_worktrees.last-run — epoch of last successful run
#   ~/.maestro/logs/cleanup_worktrees.lock/    — lock dir (present = running)
#   ~/.maestro/logs/cleanup_worktrees.lock/pid — PID of holder
