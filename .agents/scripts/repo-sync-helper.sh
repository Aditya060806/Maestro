...
#
# Configuration:
#   ~/.config/maestro/repos.json        Add "git_parent_dirs" array
#   MAESTRO_REPO_SYNC=false             Disable even if scheduler is installed
#   MAESTRO_REPO_SYNC_INTERVAL=1440     Minutes between syncs (default: 1440 = daily)
#
# Logs: ~/.maestro/logs/repo-sync.log
...
readonly LOG_FILE="$HOME/.maestro/logs/repo-sync.log"
readonly STATE_FILE="$HOME/.maestro/cache/repo-sync-state.json"
readonly CRON_MARKER="# maestro-repo-sync"
readonly DEFAULT_INTERVAL=1440
readonly LAUNCHD_LABEL="sh.maestro.repo-sync"
...
readonly SYSTEMD_UNIT_NAME="maestro-repo-sync"
...
cmd_check() {
	...
	if [[ "${MAESTRO_REPO_SYNC:-}" == "false" ]]; then
		log_info "Repo sync disabled via MAESTRO_REPO_SYNC=false"
		return 0
	fi
...
_enable_launchd() {
	...
	local old_label="com.maestro.maestro-repo-sync"
	...
	# Create named symlink so macOS System Settings shows "maestro-repo-sync"
	local bin_dir="$HOME/.maestro/bin"
	mkdir -p "$bin_dir"
	local display_link="$bin_dir/maestro-repo-sync"
	...
	if launchctl load -w "$LAUNCHD_PLIST" 2>/dev/null; then
		...
		echo "  Logs:      $LOG_FILE"
		echo ""
		echo "  Disable with: maestro repo-sync disable"
		echo "  Sync now:     maestro repo-sync check"
	else
...
" >"$service_file"
	...
	printf '%s' "[Unit]
Description=maestro repo-sync Timer
...
cmd_enable() {
	...
	local interval="${MAESTRO_REPO_SYNC_INTERVAL:-$DEFAULT_INTERVAL}"
	local script_path="$HOME/.maestro/agents/scripts/repo-sync-helper.sh"
...
cmd_status() {
	...
	if [[ "${MAESTRO_REPO_SYNC:-}" == "false" ]]; then
		echo ""
		echo -e "  ${YELLOW}Note: MAESTRO_REPO_SYNC=false is set (overrides scheduler)${NC}"
	fi
...
_dirs_list() {
	...
	if [[ -z "$dirs" ]]; then
		...
		echo "Add one with: maestro repo-sync dirs add ~/Git"
	else
...
_dirs_add() {
	local new_dir="${1:-}"
	if [[ -z "$new_dir" ]]; then
		print_error "Usage: maestro repo-sync dirs add <path>"
		return 1
	fi
...
_dirs_remove() {
	local rm_dir="${1:-}"
	if [[ -z "$rm_dir" ]]; then
		print_error "Usage: maestro repo-sync dirs remove <path>"
		return 1
	fi
...
cmd_dirs() {
	...
	*)
		...
		echo "Usage: maestro repo-sync dirs [list|add|remove|rm]"
		return 1
		;;
	esac
	return 0
...
cmd_config() {
	...
	echo "  maestro repo-sync dirs list          # Show configured directories"
	echo "  maestro repo-sync dirs add ~/Projects  # Add a directory"
	echo "  maestro repo-sync dirs remove ~/Old    # Remove a directory"
	echo ""
	return 0
}
...
USAGE:
    ...
    maestro repo-sync <command> [options]
...
CONFIGURATION:
    Manage with: maestro repo-sync dirs [add|remove|list]
...
SCHEDULER BACKENDS:
    macOS:  launchd LaunchAgent (~/Library/LaunchAgents/sh.maestro.repo-sync.plist)
            - Runs daily (every 1440 minutes by default)
    Linux:  cron (daily at 3am, crontab entry with # maestro-repo-sync marker)
...
HOW IT WORKS:
    ...
    5. Logs results (pulled/skipped/failed) to ~/.maestro/logs/repo-sync.log

LOGS:
    ~/.maestro/logs/repo-sync.log
