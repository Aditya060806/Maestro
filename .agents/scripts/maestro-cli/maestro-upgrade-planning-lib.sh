...
# =============================================================================
# maestro Upgrade Planning Library — planning template upgrade command
# =============================================================================
# Helper functions for `maestro upgrade-planning`, extracted from maestro.sh
# to keep the CLI orchestrator below the large-file gate while preserving behaviour.
#
# Usage: source "${INSTALL_DIR}/maestro-upgrade-planning-lib.sh"
...
# Include guard
[[ -n "${_MAESTRO_UPGRADE_PLANNING_LIB_LOADED:-}" ]] && return 0
_MAESTRO_UPGRADE_PLANNING_LIB_LOADED=1
...
_MAESTRO_UPGRADE_TRUE=true
_MAESTRO_UPGRADE_TOON_META="TOON:meta"
...
_upgrade_validate() {
	...
	[[ ! -f "$project_root/.maestro.json" ]] && {
		print_error "maestro not initialized in this project"
		print_info "Run 'maestro init' first"
		return 1
	}
	if command -v jq &>/dev/null; then
		jq -e '.features.planning == true' "$project_root/.maestro.json" &>/dev/null || {
			print_error "Planning feature not enabled"
			print_info "Run 'maestro init planning' to enable"
			return 1
		}
	else
		local pe
		pe=$(grep -o '"planning": *true' "$project_root/.maestro.json" 2>/dev/null || echo "")
		[[ -z "$pe" ]] && {
			print_error "Planning feature not enabled"
			print_info "Run 'maestro init planning' to enable"
			return 1
...
_upgrade_check_version() {
	...
	if check_planning_file_version "$file" "$template"; then
		if [[ -f "$file" ]]; then
			if ! grep -q "$_MAESTRO_UPGRADE_TOON_META" "$file" 2>/dev/null; then
				print_warning "$label uses minimal template (missing TOON markers)"
			else
				local cv tv
				cv=$(grep -A1 "$_MAESTRO_UPGRADE_TOON_META" "$file" 2>/dev/null | tail -1 | cut -d',' -f1)
				tv=$(grep -A1 "$_MAESTRO_UPGRADE_TOON_META" "$template" 2>/dev/null | tail -1 | cut -d',' -f1)
...
cmd_upgrade_planning() {
	...
	[[ "$dry_run" != "$_MAESTRO_UPGRADE_TRUE" ]] && { check_protected_branch "chore" "upgrade-planning" || return 1; }
	local project_root
	...
	echo ""
	if [[ "$dry_run" == "$_MAESTRO_UPGRADE_TRUE" ]]; then
		print_info "Dry run - no changes will be made"
		echo ""
		[[ "$todo_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && echo "  Would upgrade: TODO.md"
		[[ "$plans_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && echo "  Would upgrade: todo/PLANS.md"
		return 0
	fi
	if [[ "$force" == "false" ]]; then
		echo "Files to upgrade:"
		[[ "$todo_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && echo "  - TODO.md"
		[[ "$plans_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && echo "  - todo/PLANS.md"
		echo ""
		echo "This will:"
	...
	echo ""
	[[ "$todo_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && _upgrade_todo "$todo_file" "$todo_template" "$backup"
	[[ "$plans_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && _upgrade_plans "$plans_file" "$plans_template" "$backup" "$project_root"
	_upgrade_config_version "$project_root/.maestro.json"
	echo ""
	...
	if [[ "$backup" == "$_MAESTRO_UPGRADE_TRUE" ]]; then
		echo "  3. Remove .bak files when satisfied"
		echo ""
		echo "If issues occurred, restore from backups:"
		[[ "$todo_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && echo "  mv TODO.md.bak TODO.md"
		[[ "$plans_needs" == "$_MAESTRO_UPGRADE_TRUE" ]] && echo "  mv todo/PLANS.md.bak todo/PLANS.md"
	fi
	return 0
}
