...
# =============================================================================
# maestro Skills and Plugin Library
# =============================================================================
# Skill and plugin management functions extracted from maestro.sh to keep
...
#
# Usage: source "${SCRIPT_DIR}/maestro-skills-plugin-lib.sh"
#
# Dependencies:
...
# Include guard
[[ -n "${_MAESTRO_SKILLS_PLUGIN_LIB_LOADED:-}" ]] && return 0
_MAESTRO_SKILLS_PLUGIN_LIB_LOADED=1
...
_skill_help() {
	...
	echo "Skills are converted to maestro format with upstream tracking."
	echo "Telemetry is disabled - no data sent to third parties."
	echo ""
	echo "Usage: maestro skill <command> [options]"
	echo ""
	echo "Commands:"
	...
	echo ""
	echo "Examples:"
	echo "  maestro skill add vercel-labs/agent-skills"
	echo "  maestro skill add anthropics/skills/pdf"
	echo "  maestro skill add expo/skills --name expo-dev"
	echo "  maestro skill check"
	echo "  maestro skill update"
	echo "  maestro skill scan"
	echo "  maestro skill scan cloudflare-platform"
	echo "  maestro skill generate --dry-run"
	echo ""
	echo "Imported skills are saved with a -skill suffix to distinguish"
	echo "from native maestro subagents (e.g., playwright-skill.md vs playwright.md)."
	echo ""
...
_skill_add_usage() {
	...
	echo ""
	echo "Usage: maestro skill add <source> [options]"
	echo ""
	echo "Examples:"
	echo "  maestro skill add vercel-labs/agent-skills"
	echo "  maestro skill add anthropics/skills/pdf"
	echo "  maestro skill add https://github.com/owner/repo"
	echo ""
	echo "Options:"
...
cmd_skill() {
	...
	add | a)
		...
		[[ ! -f "$add_skill_script" ]] && {
			...
			print_info "Run 'maestro update' to get the latest scripts"
			return 1
		}
	...
	remove | rm)
		[[ $# -lt 1 ]] && {
			...
			echo "Usage: maestro skill remove <name>"
			return 1
		}
		[[ ! -f "$add_skill_script" ]] && {
			print_error "add-skill-helper.sh not found"
	...
	*)
		...
		echo "Run 'maestro skill help' for usage information."
		return 1
		;;
	esac
}
...
_PLUGIN_RESERVED="custom draft scripts tools services workflows templates memory plugins seo wordpress maestro"
...
_plugin_regenerate_discovery_index() {
	...
	print_warning "Subagent index helper not found; run 'maestro update' to regenerate discovery"
	return 1
}
...
_plugin_add() {
	...
	if [[ $# -lt 1 ]]; then
		...
		echo ""
		echo "Usage: maestro plugin add <repo-url> [options]"
		echo ""
		echo "Options:"
		...
		echo ""
		echo "Examples:"
		echo "  maestro plugin add https://github.com/Aditya060806/Maestro-pro.git --namespace pro"
		echo "  maestro plugin add https://github.com/Aditya060806/Maestro-anon.git --namespace anon"
		return 1
	fi
	local repo_url="$1"
	shift
	...
	[[ -z "$namespace" ]] && {
		namespace=$(basename "$repo_url" .git | sed 's/^maestro-//')
	...
	[[ -n "$existing" ]] && {
		print_error "Plugin '$plugin_name' already exists. Use 'maestro plugin update $plugin_name' to update."
		return 1
	}
	...
	echo ""
	echo "  Agents available at: ~/.maestro/agents/$namespace/"
	echo "  Update: maestro plugin update $plugin_name"
	echo "  Remove: maestro plugin remove $plugin_name"
	return 0
}
...
_plugin_list() {
	...
	if [[ "$count" == "0" ]]; then
		...
		echo ""
		echo "Add a plugin: maestro plugin add <repo-url> --namespace <name>"
		return 0
	fi
...
_plugin_scaffold() {
	...
	[[ ! -d "$tpl" ]] && {
		...
		print_info "Run 'maestro update' to get the latest templates."
		return 1
	}
	local pnu
	pnu=$(echo "$pn" | tr '[:lower:]' '[:upper:]' | tr '-' '_')
	sed -e "s|{{PLUGIN_NAME}}|$pn|g" -e "s|{{PLUGIN_NAME_UPPER}}|$pnu|g" -e "s|{{NAMESPACE}}|$ns|g" -e "s|{{REPO_URL}}|https://github.com/user/maestro-$ns.git|g" "$tpl/AGENTS.md" >"$td/AGENTS.md"
	sed -e "s|{{PLUGIN_NAME}}|$pn|g" -e "s|{{PLUGIN_DESCRIPTION}}|$pn plugin for maestro|g" -e "s|{{NAMESPACE}}|$ns|g" "$tpl/main-agent.md" >"$td/$ns.md"
	mkdir -p "$td/$ns"
	...
	[[ -f "$tpl/plugin.json" ]] && sed -e "s|{{PLUGIN_NAME}}|$pn|g" -e "s|{{PLUGIN_DESCRIPTION}}|$pn plugin for maestro|g" -e "s|{{NAMESPACE}}|$ns|g" "$tpl/plugin.json" >"$td/plugin.json"
	...
	echo "  5. Install: maestro plugin add <repo-url> --namespace $ns"
	return 0
}
...
_plugin_help() {
	...
	echo ""
	echo "Manage third-party agent plugins that extend maestro."
	echo "Plugins deploy to ~/.maestro/agents/<namespace>/ (isolated from core)."
	echo ""
	echo "Usage: maestro plugin <command> [options]"
	echo ""
	echo "Commands:"
	...
	echo "  --namespace <name>   Directory name under ~/.maestro/agents/"
	echo "  --branch <branch>    Branch to track (default: main)"
	...
	echo "  maestro plugin add https://github.com/Aditya060806/Maestro-pro.git --namespace pro"
	echo "  maestro plugin add https://github.com/Aditya060806/Maestro-anon.git --namespace anon"
	echo "  maestro plugin list"
	echo "  maestro plugin update"
	echo "  maestro plugin update pro"
	echo "  maestro plugin disable pro"
	echo "  maestro plugin enable pro"
	echo "  maestro plugin remove pro"
	echo "  maestro plugin init ./my-plugin my-plugin my-plugin"
	echo ""
	echo "Plugin docs: ~/.maestro/agents/maestro/plugins.md"
	return 0
}
...
cmd_plugin() {
	...
	enable)
		[[ $# -lt 1 ]] && {
			...
			echo "Usage: maestro plugin enable <name>"
			return 1
		}
	...
	disable)
		[[ $# -lt 1 ]] && {
			print_error "Plugin name required"
			echo "Usage: maestro plugin disable <name>"
			return 1
		}
	...
	remove | rm)
		[[ $# -lt 1 ]] && {
			print_error "Plugin name required"
			echo "Usage: maestro plugin remove <name>"
			return 1
		}
	...
	*)
		...
		echo "Run 'maestro plugin help' for usage information."
		return 1
		;;
	esac
	return 0
...
cmd_skills() {
	...
	help | --help | -h)
		...
		echo "For importing/managing skills, use: maestro skill <cmd>"
		echo ""
		echo "Usage: maestro skills <command> [options]"
		echo ""
		echo "Commands:"
		...
		echo ""
		echo "Examples:"
		echo "  maestro skills search \"browser automation\""
		echo "  maestro skills search --registry \"seo\""
		echo "  maestro skills browse tools"
		echo "  maestro skills browse tools/browser"
		echo "  maestro skills describe playwright"
		echo "  maestro skills info seo-audit-skill"
		echo "  maestro skills list --imported"
		echo "  maestro skills categories"
		echo "  maestro skills recommend \"deploy a Next.js app\""
		echo "  maestro skills install vercel-labs/agent-browser@agent-browser"
		echo ""
		echo "See also: maestro skill help  (import/manage skills)"
		;;
	*)
