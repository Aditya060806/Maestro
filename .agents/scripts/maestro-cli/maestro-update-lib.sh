...
# =============================================================================
# maestro Update Library — update command helper functions
# =============================================================================
# Helper functions for `maestro update`, extracted from maestro.sh to keep
# the CLI orchestrator below the large-file gate while preserving behaviour.
#
# Usage: source "${INSTALL_DIR}/maestro-update-lib.sh"
...
# Include guard
[[ -n "${_MAESTRO_UPDATE_LIB_LOADED:-}" ]] && return 0
_MAESTRO_UPDATE_LIB_LOADED=1
...
_update_fresh_install() {
	...
	tmp_setup=$(mktemp "${TMPDIR:-/tmp}/maestro-setup-XXXXXX") || {
		print_error "Failed to create temp file for setup script"
		return 1
	}
	trap 'rm -f "${tmp_setup:-}"' RETURN
	if curl -fsSL "https://raw.githubusercontent.com/Aditya060806/Maestro/main/setup.sh" -o "$tmp_setup" 2>/dev/null && [[ -s "$tmp_setup" ]]; then
		chmod +x "$tmp_setup"
		bash "$tmp_setup"
		local setup_exit=$?
		rm -f "$tmp_setup"
		...
		print_info "Try: git clone https://github.com/Aditya060806/Maestro.git $INSTALL_DIR && bash $INSTALL_DIR/setup.sh"
		return 1
	fi
	return 0
}
...
_update_sync_projects() {
	...
	if [[ "$skip" == "$_MAESTRO_UPDATE_TRUE" ]]; then
	...
	for repo in "${repos_needing_upgrade[@]}"; do
		...
		if command -v jq &>/dev/null; then
			local temp_file="${repo}/.maestro.json.tmp"
			if jq --arg version "$current_ver" '.version = $version' "$repo/.maestro.json" >"$temp_file" 2>/dev/null && [[ -s "$temp_file" ]]; then
				mv "$temp_file" "$repo/.maestro.json"
				local features
		...
		if [[ "$did_sync" != "$_MAESTRO_UPDATE_TRUE" ]]; then
			sed -i '' "s/\"version\": *\"[^\"]*\"/\"version\": \"$current_ver\"/" "$repo/.maestro.json" 2>/dev/null && did_sync=true
		fi
		[[ "$did_sync" == "$_MAESTRO_UPDATE_TRUE" ]] && synced=$((synced + 1)) || failed=$((failed + 1))
	done
	[[ $synced -gt 0 ]] && print_success "Synced $synced project(s) to v$current_ver"
	[[ $skipped -gt 0 ]] && print_info "Skipped $skipped uninitialized project(s) (run 'maestro init' in each to enable)"
...
_update_sync_agent_source_repos() {
	...
	while IFS= read -r repo; do
		...
		if seed_agent_source_repo_templates "$repo"; then
			synced=$((synced + 1))
			if [[ -f "$repo/.maestro.json" ]] && command -v jq &>/dev/null; then
				local temp_file="${repo}/.maestro.json.tmp"
				jq --arg version "$current_ver" '.version = $version | .agent_source = true' "$repo/.maestro.json" >"$temp_file" 2>/dev/null && mv "$temp_file" "$repo/.maestro.json" || rm -f "$temp_file"
			fi
		else
			failed=$((failed + 1))
		fi
...
_update_check_planning() {
	...
	while IFS= read -r repo_path; do
		...
		if [[ -f "$repo_path/.maestro.json" ]]; then
			local has_planning
			has_planning=$(grep -o '"planning": *true' "$repo_path/.maestro.json" 2>/dev/null || true)
	...
	else print_info "Run 'maestro upgrade-planning' in each project to upgrade manually"; fi
	return 0
}
...
_update_check_tools() {
	...
	if declare -F maestro_gh_slurp_supported >/dev/null 2>&1 && ! maestro_gh_slurp_supported; then
		local gh_slurp_message=""
		if declare -F maestro_gh_slurp_status_message >/dev/null 2>&1; then
			gh_slurp_message=$(maestro_gh_slurp_status_message)
		else
			gh_slurp_message="GitHub CLI (gh) is below the maestro minimum for gh api --paginate --slurp"
		fi
		print_warning "$gh_slurp_message"
		if declare -F maestro_gh_slurp_remediation_hint >/dev/null 2>&1; then
			print_info "$(maestro_gh_slurp_remediation_hint)"
		else
			print_info "Run maestro setup or upgrade gh manually, then rerun maestro status."
		fi
	fi
	local idx=0
	...
	if [[ "$stale_count" -eq 0 ]]; then
		...
		[[ "$response" =~ ^[Yy]$ ]] && bash "$tool_check_script" --update || print_info "Run 'maestro update-tools --update' to update later"
	fi
	return 0
}
...
_update_sweep_opencode_symlinks() {
	local sym_helper="${HOME}/.maestro/agents/scripts/agent-sources-helper.sh"
...
_update_check_homebrew() {
	...
	brew list maestro &>/dev/null 2>&1 || return 0
	local brew_version=""
	brew_version=$(brew info maestro --json=v2 2>/dev/null | jq -r '.formulae[0].installed[0].version // empty' 2>/dev/null || true)
	...
	if [[ "$brew_version" != "$current_version" ]]; then
		...
		echo ""
		read -r -p "Run 'brew upgrade maestro' now? [y/N] " response
		if [[ "$response" =~ ^[Yy]$ ]]; then
			brew upgrade maestro 2>&1 || print_warning "brew upgrade failed — run manually: brew upgrade maestro"
		else
			print_info "Run 'brew upgrade maestro' to sync the Homebrew copy"
		fi
	fi
	return 0
}

# t2926 / GH#21102: Re-check setsid on every 'maestro update' run.
...
# both already exist (`maestro check-workflows`, `maestro sync-workflows
...
_update_check_workflow_drift() {
	...
	print_info "  Detect drift: maestro check-workflows"
	print_info "  Apply fix:    maestro sync-workflows --apply [--repo OWNER/REPO]"
	...
	# 'maestro security dismiss <id>' silences a specific day's advisory.
...
_update_emit_workflow_drift_advisory() {
	...
	local advisories_dir="${HOME}/.maestro/advisories"
	local adv_id
...

	{
		...
		printf '\n'
		printf 'Detect drift: maestro check-workflows\n'
		printf 'Apply fix:    maestro sync-workflows --apply [--repo OWNER/REPO]\n'
...
_update_verify_signature() {
	...
	UNTRUSTED)
		...
		print_info "Run 'maestro signing setup' to configure signature verification"
		;;
	BAD_SIGNATURE)
...
# Logs to ~/.maestro/logs/settings-migration.log.
_migrate_settings_supervisor_to_orchestration() {
	local _settings_file="${HOME}/.config/maestro/settings.json"
	local _log_file="${HOME}/.maestro/logs/settings-migration.log"
...
_update_check_daemon_health() {
	local helper="$HOME/.maestro/agents/scripts/auto-update-helper.sh"
	[[ -x "$helper" ]] || return 0
	local advisory_dir="$HOME/.maestro/advisories"
	...
	local fix_cmd="maestro auto-update enable"
	[[ "$hc_rc" -eq 1 ]] && fix_cmd="maestro auto-update check"
...
Diagnose: maestro auto-update health-check
Fix:      ${fix_cmd}
EOF
