...
# =============================================================================
# maestro Status Library — status command helper functions
# =============================================================================
# Helper functions for `maestro status`, extracted from maestro.sh to keep
# the CLI orchestrator below the large-file gate while preserving behaviour.
#
# Usage: source "${INSTALL_DIR}/maestro-status-lib.sh"
...
# Include guard
[[ -n "${_MAESTRO_STATUS_LIB_LOADED:-}" ]] && return 0
_MAESTRO_STATUS_LIB_LOADED=1
...
_status_headless_runtime_config() {
	...
	local credentials_file="${MAESTRO_CREDENTIALS_FILE:-${CONFIG_DIR:-$HOME/.config/maestro}/credentials.sh}"
	local credentials_has_allowlist=false

	if [[ -n "${MAESTRO_HEADLESS_PROVIDER_ALLOWLIST:-}" ]]; then
		allowlist_set=true
	fi
	if [[ -f "$credentials_file" ]] && grep -Eq '^[[:space:]]*(export[[:space:]]+)?MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=' "$credentials_file" 2>/dev/null; then
	...
	if [[ "$credentials_has_allowlist" == "true" ]]; then
		...
		print_info "Add the export to ~/.config/maestro/credentials.sh for daemon-visible headless routing"
	else
...
# Status command
cmd_status() {
	print_header "Maestro Framework Status"
	echo "=========================="
	echo ""
	...
	if [[ "$current_version" != "$remote_version" && "$remote_version" != "unknown" ]]; then
		print_warning "Update available! Run: maestro update"
	...
	if ! check_cmd gh; then
		...
		print_success "$(maestro_gh_slurp_status_message)"
	elif declare -F maestro_gh_slurp_status_message >/dev/null 2>&1; then
		print_warning "$(maestro_gh_slurp_status_message)"
	else
	...
	if [[ "$signing_format" == "ssh" && -n "$signing_key" && "$signing_enabled" == "true" ]]; then
		...
		if check_file "$HOME/.ssh/allowed_signers"; then
			...
			print_warning "No allowed_signers file — run: maestro signing setup"
		fi
	else
		print_warning "Commit signing not configured — run: maestro signing setup"
	fi
	echo ""
