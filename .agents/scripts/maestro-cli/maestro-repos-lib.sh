...
# =============================================================================
# maestro Repo Management Library
# =============================================================================
# Repository registration, discovery, and validation functions extracted from
# maestro.sh to keep the orchestrator under the 2000-line file-size threshold.
#
# Covers:
...
#
# Usage: source "${SCRIPT_DIR}/maestro-repos-lib.sh"
#
# Dependencies:
#   - INSTALL_DIR, AGENTS_DIR, CONFIG_DIR, REPOS_FILE (set by maestro.sh)
...
# Include guard
[[ -n "${_MAESTRO_REPOS_LIB_LOADED:-}" ]] && return 0
_MAESTRO_REPOS_LIB_LOADED=1

_maestro_repos_source_worktree_paths() {
	local helper=""
	...
	elif [[ -f "${HOME}/.maestro/agents/scripts/worktree-paths.sh" ]]; then
		helper="${HOME}/.maestro/agents/scripts/worktree-paths.sh"
	fi
	[[ -n "$helper" ]] || return 0
...
init_repos_file() {
	if [[ ! -f "$REPOS_FILE" ]]; then
		...
		if declare -F maestro_migrate_repos_json_worktree_base_dir >/dev/null 2>&1; then
			maestro_migrate_repos_json_worktree_base_dir "$REPOS_FILE" "~"'/Git/_worktrees' || true
...
# Priority: .maestro.json > repos.json entry > context inference.
...
_infer_init_scope() {
	...
	# 1. Check .maestro.json
	if [[ -f "$project_root/.maestro.json" ]]; then
		local json_scope
		json_scope=$(jq -r '.init_scope // empty' "$project_root/.maestro.json" 2>/dev/null || echo "")
	...
	if command -v jq &>/dev/null && [[ -f "${REPOS_FILE:-$HOME/.config/maestro/repos.json}" ]]; then
		local repos_file="${REPOS_FILE:-$HOME/.config/maestro/repos.json}"
...
is_agent_source_repo() {
	...
	if command -v jq &>/dev/null && [[ -f "$project_root/.maestro.json" ]]; then
		local project_flag
		project_flag=$(jq -r 'if .agent_source == true or .role == "agent-source" then "true" else "false" end' "$project_root/.maestro.json" 2>/dev/null || echo "false")
...
#
# Why this exists: `find ~/Git -name .maestro.json` in auto-discovery and
# similar scans pick up .maestro.json files that exist in linked worktrees
...
_repo_config_has_interface_value() {
	local repo_path="$1"
	local has_interface=""
	if command -v jq &>/dev/null && [[ -f "$repo_path/.maestro.json" ]]; then
		has_interface=$(jq -r 'if has("has_interface") then .has_interface elif has("interface") then .interface else empty end' "$repo_path/.maestro.json" 2>/dev/null || echo "")
...
_init_scaffold_design_md() {
	...
	helper=$(_init_design_helper_path) || {
		print_warning "DESIGN.md helper not found; run maestro update and retry"
		return 0
	}
...
# Detect if current directory has maestro but isn't registered
...
check_protected_branch() {
	...
	local branch_suffix="${2:-maestro-setup}"
