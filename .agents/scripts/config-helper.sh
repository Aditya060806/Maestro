...
# config-helper.sh - JSONC configuration reader/writer for maestro
#
# Provides get/set/list/reset/migrate operations on the maestro JSONC config.
# Called by `maestro config <command>` CLI and sourced by shared-constants.sh.
#
# Usage (CLI):
...
#
# Files:
#   Defaults:    ~/.maestro/agents/configs/maestro.defaults.jsonc
#   User config: ~/.config/maestro/config.jsonc
#   Old config:  ~/.config/maestro/feature-toggles.conf (migrated on first use)
...
# GH#3981: https://github.com/Aditya060806/Maestro/issues/3981
...
# ---------------------------------------------------------------------------
_CONFIG_HOME="${HOME:-/tmp/maestro-${USER:-uid-${UID:-shared}}}"
...
JSONC_DEFAULTS="${JSONC_DEFAULTS:-${_CONFIG_HOME}/.maestro/agents/configs/maestro.defaults.jsonc}"
JSONC_USER="${JSONC_USER:-${_CONFIG_HOME}/.config/maestro/config.jsonc}"
JSONC_SCHEMA="${JSONC_SCHEMA:-${_CONFIG_HOME}/.maestro/agents/configs/maestro-config.schema.json}"
OLD_CONF_USER="${OLD_CONF_USER:-${_CONFIG_HOME}/.config/maestro/feature-toggles.conf}"
OLD_CONF_DEFAULTS="${OLD_CONF_DEFAULTS:-${_CONFIG_HOME}/.maestro/agents/configs/feature-toggles.conf.defaults}"
MIGRATE_FAILED_FLAG="${MIGRATE_FAILED_FLAG:-${_CONFIG_HOME}/.maestro/migrate_failed}"
...
_merge_configs() {
	...
	if [[ -f "$JSONC_USER" ]]; then
		user_json=$(_strip_jsonc "$JSONC_USER") || {
			...
			echo "  Run 'maestro config validate' to diagnose, or 'maestro config reset' to fix." >&2
			return 1
		}
	else
		user_json="{}"
	...
	if command -v jq &>/dev/null; then
		...
		merge_stderr=$(mktemp 2>/dev/null || echo "/tmp/maestro-merge-err.$$")
...
_config_env_map() {
	local dotpath="$1"
	case "$dotpath" in
	updates.auto_update) echo "MAESTRO_AUTO_UPDATE" ;;
	updates.update_interval_minutes) echo "MAESTRO_UPDATE_INTERVAL" ;;
	updates.skill_auto_update) echo "MAESTRO_SKILL_AUTO_UPDATE" ;;
	updates.skill_freshness_hours) echo "MAESTRO_SKILL_FRESHNESS_HOURS" ;;
	updates.tool_auto_update) echo "MAESTRO_TOOL_AUTO_UPDATE" ;;
	updates.tool_freshness_hours) echo "MAESTRO_TOOL_FRESHNESS_HOURS" ;;
	updates.tool_idle_hours) echo "MAESTRO_TOOL_IDLE_HOURS" ;;
	updates.openclaw_auto_update) echo "MAESTRO_OPENCLAW_AUTO_UPDATE" ;;
	updates.openclaw_freshness_hours) echo "MAESTRO_OPENCLAW_FRESHNESS_HOURS" ;;
	updates.upstream_watch) echo "MAESTRO_UPSTREAM_WATCH" ;;
	updates.upstream_watch_hours) echo "MAESTRO_UPSTREAM_WATCH_HOURS" ;;
	orchestration.supervisor_pulse) echo "MAESTRO_SUPERVISOR_PULSE" ;;
	orchestration.interactive_pr_auto_merge) echo "MAESTRO_INTERACTIVE_PR_AUTO_MERGE" ;;
	orchestration.repo_sync) echo "MAESTRO_REPO_SYNC" ;;
	orchestration.repo_maestro_health) echo "MAESTRO_REPO_HEALTH" ;;
	orchestration.max_workers_cap) echo "MAESTRO_MAX_WORKERS_CAP" ;;
	orchestration.min_worker_concurrency) echo "MAESTRO_MIN_WORKER_CONCURRENCY" ;;
	orchestration.quality_debt_cap_pct) echo "MAESTRO_QUALITY_DEBT_CAP_PCT" ;;
...
_legacy_key_to_dotpath() {
	...
	repo_maestro_health) echo "orchestration.repo_maestro_health" ;;
...
_migrate_conf_to_jsonc() {
	...
	{
		echo "// maestro Configuration — User Overrides"
		echo "// ========================================="
		echo "// Migrated from feature-toggles.conf on $(date -u +%Y-%m-%dT%H:%M:%SZ)"
		echo "// This file overrides defaults in:"
		echo "//   ~/.maestro/agents/configs/maestro.defaults.jsonc"
		echo "//"
		echo "// Run 'maestro config list' to see all available options."
		echo "// Run 'maestro config reset' to remove all overrides."
		echo ""
...
cmd_list() {
	...
	echo ""
	echo "  Set a value:   maestro config set <dotpath> <value>"
	echo "  Reset a value: maestro config reset <dotpath>"
	echo "  Reset all:     maestro config reset"
	echo ""
	return 0
}
...
cmd_get() {
	...
	if [[ -z "$dotpath" ]]; then
		echo "[ERROR] Usage: maestro config get <dotpath>" >&2
		echo "  Example: maestro config get updates.auto_update" >&2
		return 1
	fi
	...
	if [[ -z "$value" ]]; then
		...
		echo "  Run 'maestro config list' to see available options." >&2
		return 1
	fi
...
_cmd_set_validate_args() {
	...
	if [[ -z "$dotpath" || -z "$value" ]]; then
		echo "[ERROR] Usage: maestro config set <dotpath> <value>" >&2
		echo "  Example: maestro config set updates.auto_update false" >&2
		return 1
	fi
...
// maestro Configuration — User Overrides
// =========================================
// This file overrides defaults in:
//   ~/.maestro/agents/configs/maestro.defaults.jsonc
//
// Run 'maestro config list' to see all available options.
// Run 'maestro config reset' to remove all overrides.
...
cmd_set() {
	...
	if [[ "$dotpath" == "orchestration.supervisor_pulse" ]]; then
		...
		if [[ "$lower_val" == "true" ]]; then
			...
			echo "[INFO] Run 'maestro pulse stop' to switch to manual start/stop mode (persistent across reboots)." >&2
		fi
	fi
...
cmd_reset() {
	...
	if [[ -n "$dotpath" ]]; then
		...
		local user_json
		user_json=$(_strip_jsonc "$JSONC_USER") || {
			echo "[ERROR] Malformed user config: $JSONC_USER — consider 'maestro config reset' to remove it" >&2
			return 1
		}
...
cmd_path() {
	...
	if [[ -f "$OLD_CONF_USER" ]]; then
		echo "Legacy config: $OLD_CONF_USER (run 'maestro config migrate' to convert)"
	fi
	return 0
}
...
cmd_help() {
	cat <<'EOF'
config-helper.sh - Manage maestro configuration (JSONC)

USAGE:
    maestro config <command> [args]
...
EXAMPLES:
    ...
    maestro config set integrations.manage_opencode_config false
    maestro config get orchestration.supervisor_pulse
    maestro config reset updates.auto_update
...
PRIORITY ORDER:
    1. Environment variables (e.g. MAESTRO_AUTO_UPDATE=false)
    2. User config (~/.config/maestro/config.jsonc)
    3. Defaults (~/.maestro/agents/configs/maestro.defaults.jsonc)

FILES:
    ~/.config/maestro/config.jsonc                    User overrides (JSONC)
    ~/.maestro/agents/configs/maestro.defaults.jsonc  Shipped defaults (JSONC)
    ~/.maestro/agents/configs/maestro-config.schema.json  JSON Schema
...
main() {
	...
	if [[ -f "$OLD_CONF_USER" && ! -f "$JSONC_USER" ]]; then
		...
		if [[ "$migrate_rc" -ne 0 ]]; then
			echo "[WARN] Auto-migration from legacy config failed (exit ${migrate_rc}). Run 'maestro config migrate' manually." >&2
