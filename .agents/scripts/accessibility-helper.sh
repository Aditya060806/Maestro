...
# Configuration
readonly A11Y_REPORTS_DIR="$HOME/.maestro/reports/accessibility"
...
load_wave_api_key() {
	...
	if command -v gopass &>/dev/null; then
		WAVE_API_KEY=$(gopass show -o "maestro/WAVE_API_KEY" 2>/dev/null || echo "")
		if [[ -z "$WAVE_API_KEY" ]]; then
			WAVE_API_KEY=$(gopass show -o "maestro/wave-api-key" 2>/dev/null || echo "")
		fi
		if [[ -n "$WAVE_API_KEY" ]]; then
			export WAVE_API_KEY
			return 0
	...
	print_info "Set via: maestro secret set wave-api-key"
...
run_full_audit() {
	...
	if load_wave_api_key 2>/dev/null; then
		...
		print_warning "Skipping WAVE API (no API key). Set via: maestro secret set wave-api-key"
	fi
...
_main_print_help() {
	...
	echo "  WAVE_API_KEY       WAVE API key (or use: maestro secret set wave-api-key)"
	echo ""
	echo "Examples:"
