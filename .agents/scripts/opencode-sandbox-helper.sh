...
SANDBOX_ROOT="$HOME/.maestro/.agent-workspace/opencode-sandbox"
...
Examples:
  maestro opencode-sandbox install 1.3.0    # Install 1.3.0 to sandbox
  maestro opencode-sandbox run 1.3.0        # Launch it (isolated config/data)
  maestro opencode-sandbox check 1.3.0      # Check auth.json in sandbox
  maestro opencode-sandbox clean 1.3.0      # Remove when done
  maestro opencode-sandbox clean all        # Remove all sandboxes

What's isolated:
  - Binary:  ~/.maestro/.agent-workspace/opencode-sandbox/<version>/
...
Testing OAuth in a sandbox:
  1. maestro opencode-sandbox install 1.3.0
  2. maestro opencode-sandbox run 1.3.0
  3. Inside the sandbox session, check stderr for [maestro] plugin lines
  4. Use Ctrl+A to test provider auth flow
  5. maestro opencode-sandbox check 1.3.0   # inspect auth.json
  6. maestro opencode-sandbox clean 1.3.0   # clean up
EOF
	return 0
}
...
cmd_install() {
	...
	if npm install --prefix "$version_dir" "opencode-ai@${version}" 2>&1; then
		...
		if [[ -n "$bin_path" ]]; then
			...
			print_info "Run with: maestro opencode-sandbox run ${version}"
		else
...
cmd_run() {
	...
	if [[ -z "$version" ]]; then
		...
		if [[ -z "$version" ]]; then
			...
			print_info "Install one: maestro opencode-sandbox install <version>"
			return 1
		fi
	...
	bin_path=$(_sandbox_bin "$version") || {
		...
		print_info "Install it: maestro opencode-sandbox install ${version}"
		return 1
	}
...
cmd_check() {
	...
	if [[ ! -f "$auth_file" ]]; then
		...
		print_info "Run the sandbox first: maestro opencode-sandbox run ${version}"
		return 0
	fi
