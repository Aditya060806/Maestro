...
AGENTS_DIR="${AGENTS_DIR:-${MAESTRO_AGENTS_DIR:-$HOME/.maestro/agents}}"
...
# Convert fetched URL content to maestro format.
...
# Fetch SKILL.md from ClawdHub and convert to maestro format.
...
_fetch_and_convert_clawdhub() {
	...
	# Create target directory and convert to maestro format (target_file is absolute via AGENTS_DIR)
	local target_dir
	target_dir="$(dirname "$target_file")"
	mkdir -p "$target_dir"
...
cmd_add_url() {
	...
	local fetch_dir="${TMPDIR:-/tmp}/maestro-url-fetch"
	rm -rf "$fetch_dir"
	mkdir -p "$fetch_dir"
	...
	# Convert URL content to maestro format
	local target_file="${AGENTS_DIR}/${target_path}.md"
...
cmd_check_updates() {
	...
	while IFS='|' read -r name url commit; do
		...
		if [[ "$latest_commit" != "$commit" ]]; then
			...
			echo "  Run: maestro skill update $name"
			echo ""
		else
