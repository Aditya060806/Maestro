...
main() {
	local window="15m"
	local repo_path="${MAESTRO_REPO_PATH:-$HOME/Git/maestro}"
	local log_dir="${MAESTRO_LOG_DIR:-$HOME/.maestro/logs}"
	local review_thread_state_dir="${MAESTRO_PR_REVIEW_THREAD_RESPONSE_STATE_DIR:-$HOME/.maestro/.agent-workspace/pr-review-thread-response}"
	...
	MAESTRO_ACTIVE_WORKER_PROCESSES="$active_worker_processes" \
		MAESTRO_WORKER_WORKTREE_COUNT="$worker_worktree_count" \
		MAESTRO_GRAPHQL_BUDGET_STATUS="$graphql_budget_status" \
