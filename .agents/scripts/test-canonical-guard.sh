...
cat >"$HOME/.config/maestro/repos.json" <<EOF
{
  "initialized_repos": [
    {
...
# -----------------------------------------------------------------------------
_run_hook 1 "MAESTRO_CANONICAL_GUARD=bypass"
rc=$?
...
mv "$HOME/.config/maestro/repos.json" "$HOME/.config/maestro/repos.json.bak"
cat >"$HOME/.config/maestro/repos.json" <<'EOF'
...
# Restore repos.json
mv "$HOME/.config/maestro/repos.json.bak" "$HOME/.config/maestro/repos.json"
...
git checkout feature-branch --quiet
popd >/dev/null || exit 1
_run_hook 1 "MAESTRO_CANONICAL_GUARD=strict"
rc=$?
...
git checkout feature-branch --quiet
popd >/dev/null || exit 1
_run_hook 1 "MAESTRO_CANONICAL_GUARD=warn"
rc=$?
