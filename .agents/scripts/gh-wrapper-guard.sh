...
#   Any line ending with "# maestro-allow: raw-gh-wrapper" is skipped.
#
...
readonly ALLOWLIST_MARKER='# maestro-allow: raw-gh-wrapper'
...
_report_violations() {
	if [[ "$_SCAN_VIOLATIONS" -gt 0 ]]; then
		...
		printf 'Suppress: append "# maestro-allow: raw-gh-wrapper" to the line.\n\n'
...
show_help() {
	...
	printf '  Append "# maestro-allow: raw-gh-wrapper" to any line to skip it.\n'
	return 0
}
