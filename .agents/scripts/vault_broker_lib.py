...
"""In-memory Unix-socket broker for the local maestro Vault."""
...
def read_store(vault_dir: Path, root_key: bytes) -> dict[str, Any]:
    ...
    if not path.exists():
        return {"entries": {}}
    payload = decrypt_json(root_key, load_json(path), b"maestro-vault-store")
...
def write_store(vault_dir: Path, root_key: bytes, payload: dict[str, Any]) -> None:
    write_private_json(store_path(vault_dir), encrypt_json(root_key, payload, b"maestro-vault-store"))
