...
# Version Manager for Maestro Framework
...
_version_manager_is_headless_task_worker() {
	local worker_marker="${WORKER_TASK_NUMBER:-}${WORKER_ISSUE_NUMBER:-}${WORKER_SESSION_KEY:-}${MAESTRO_SESSION_KEY:-}"
	local worker_marker_lower=""

	_version_manager_marker_is_truthy "${MAESTRO_HEADLESS:-}" "${FULL_LOOP_HEADLESS:-}" "${OPENCODE_HEADLESS:-}" "${HEADLESS:-}" || return 1
...
_version_manager_has_approved_release_context() {
	local branch_name="${1:-}"
	local session_key="${WORKER_SESSION_KEY:-${MAESTRO_SESSION_KEY:-}}"
	local session_key_lower=""
	local session_title="${MAESTRO_SESSION_TITLE:-${WORKER_SESSION_TITLE:-}}"
	...
	[[ "${MAESTRO_RELEASE_CONTEXT_APPROVED:-}" == "1" ]] && return 0
	[[ "${VERSION_MANAGER_RELEASE_CONTEXT_APPROVED:-}" == "1" ]] && return 0
	[[ "${MAESTRO_TASK_SCOPE:-}" == "$_VERSION_MANAGER_ACTION_RELEASE" ]] && return 0
...
_version_manager_guard_headless_release_scope() {
	...
	print_info "Task worker: ${WORKER_TASK_NUMBER:-unknown}; issue: ${WORKER_ISSUE_NUMBER:-unknown}; repo: ${REPO_ROOT}; session: ${WORKER_SESSION_KEY:-${MAESTRO_SESSION_KEY:-unknown}}; branch: ${branch_name}"
	print_info "Approved release contexts: MAESTRO_RELEASE_CONTEXT_APPROVED=1, VERSION_MANAGER_RELEASE_CONTEXT_APPROVED=1, MAESTRO_TASK_SCOPE=release, or a release/*/hotfix/* branch/session."
...
_main_usage() {
	echo "Maestro Framework Version Manager"
	echo ""
