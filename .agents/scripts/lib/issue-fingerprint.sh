...
# =============================================================================
# maestro Issue Fingerprint Library
# =============================================================================
...
# Strips maestro sig footer, source-context trailers, trailing blank lines,
...
_normalize_body_for_fingerprint() {
	local body="$1"
	# Strip maestro sig footer (everything from <!-- maestro:sig --> to end)
	body=$(printf '%s' "$body" | awk '/<!-- maestro:sig -->/{exit} {print}')
