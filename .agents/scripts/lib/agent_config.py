...
# Note: Build+ is now the single unified coding agent (Plan+ and Maestro consolidated)
AGENT_ORDER = ["Build+", "Automate"]

# Files to skip (not primary agents - includes demoted agents)
# plan-plus.md and maestro.md are now subagents, not primary agents
# browser-extension-dev.md and mobile-app-dev.md are specialist subagents under Build+
SKIP_PRIMARY_AGENTS = {"plan-plus.md", "maestro.md", "browser-extension-dev.md", "mobile-app-dev.md"}
...
#   - openapi-search_*: ~500 tokens - enabled for Build+, Maestro, Research only
AGENT_TOOLS = {
    "Build+": {
...
DEFAULT_PROMPT = "~/.maestro/agents/prompts/build.txt"
...
def get_agent_config(display_name, filename, subagents=None, model_tier=None):
    ...
    config = {
        "description": f"Read ~/.maestro/agents/{filename}",
        "mode": "primary",
        "temperature": temp,
        "permission": {},
        "tools": tools
