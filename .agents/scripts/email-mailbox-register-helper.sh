...
#   ~/.config/maestro/mailboxes.json      Global
#
# Part of maestro email system (t2855).
...
_GLOBAL_CONFIG="${HOME}/.config/maestro/mailboxes.json"
...
_load_or_init_config() {
	...
	if [[ ! -f "$config_path" ]]; then
		if [[ -f "$MAILBOXES_TEMPLATE" ]]; then
			...
			MAESTRO_CFG="$config_path" python3 -c "
import json, os
cfg = os.environ['MAESTRO_CFG']
...
_provider_defaults() {
	...
	MAESTRO_TEMPLATE="$PROVIDERS_TEMPLATE" MAESTRO_PROVIDER="$provider" \
	python3 -c "
import json, os
p = json.load(open(os.environ['MAESTRO_TEMPLATE'])).get('providers', {}).get(os.environ['MAESTRO_PROVIDER'], {})
imap = p.get('imap', {})
prov = os.environ['MAESTRO_PROVIDER']
...
_list_known_providers() {
	...
	MAESTRO_TEMPLATE="$PROVIDERS_TEMPLATE" python3 -c "
import json, os
print(' '.join(json.load(open(os.environ['MAESTRO_TEMPLATE'])).get('providers', {}).keys()))
...
_write_mailbox_entry() {
	...
	MAESTRO_CFG="$config_path" MAESTRO_MBID="$mb_id" \
	MAESTRO_PROV="$provider" MAESTRO_HOST="$host" MAESTRO_PORT="$port" \
	MAESTRO_USER="$user" MAESTRO_PWREF="$password_ref" \
	MAESTRO_FOLDERS="$folders_json" MAESTRO_SINCE="$since" \
	python3 -c "
import json, os; e = os.environ
with open(e['MAESTRO_CFG']) as f: d = json.load(f)
entry = {'id': e['MAESTRO_MBID'], 'provider': e['MAESTRO_PROV'],
    'host': e['MAESTRO_HOST'], 'port': int(e['MAESTRO_PORT']),
    'user': e['MAESTRO_USER'], 'password_ref': e['MAESTRO_PWREF'],
    'folders': json.loads(e['MAESTRO_FOLDERS']), 'since': e['MAESTRO_SINCE']}
d['mailboxes'] = [m for m in d.get('mailboxes', []) if m.get('id') != entry['id']]
d['mailboxes'].append(entry)
with open(e['MAESTRO_CFG'], 'w') as f: json.dump(d, f, indent=2); f.write('\n')
"
	return 0
}
...
cmd_add() {
	...
	read -r -p "gopass path for password (e.g. maestro/email/${mb_id}/password): " gopass_path
	...
	local folders_json
	folders_json=$(MAESTRO_FOLDERS_INPUT="$folders_input" python3 -c "
import json, os
folders = [f.strip() for f in os.environ['MAESTRO_FOLDERS_INPUT'].split(',') if f.strip()]
...
cmd_remove() {
	...
	MAESTRO_CFG="$config_path" MAESTRO_MBID="$mailbox_id" python3 -c "
import json, sys, os
cfg, mb_id = os.environ['MAESTRO_CFG'], os.environ['MAESTRO_MBID']
with open(cfg) as f: d = json.load(f)
...
Invoked via: maestro email mailbox add|remove
EOF
	return 0
}
