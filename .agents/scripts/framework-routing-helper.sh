...
# framework-routing-helper.sh - Detect and route framework-level tasks to the maestro repo
#
# Solves: GH#5149 — workers create framework tasks in project repos instead of
# routing to maestro. Prose-only guidance (GH#2849) is insufficient for
...
#   2. get-maestro-path: Resolve the local path to the maestro repo
#   3. get-maestro-slug: Resolve the GitHub slug for the maestro repo
#   4. log-framework-issue: Create an issue on the maestro repo for a
...
#   framework-routing-helper.sh get-maestro-path
#   framework-routing-helper.sh get-maestro-slug
...
#   get-maestro-*: 0 = found, 1 = not found
...
#
# Pattern categories:
#   1. Framework file paths (~/.maestro/, .agents/, prompts/build.txt)
...
# shellcheck disable=SC2088 # Tilde is intentional — matching literal "~/.maestro" in text
...
# =============================================================================
# get_maestro_path — resolve the local filesystem path to the maestro repo
# =============================================================================
...
get_maestro_path() {
	...
	if [[ -f "$repos_json" ]] && command -v jq &>/dev/null; then
		local path
		path=$(jq -r '.initialized_repos[] | select(.slug | test("maestro$")) | .path' "$repos_json" 2>/dev/null | head -1)
	...
	log_warn "Could not find maestro repo path"
	return 1
}

# =============================================================================
# get_maestro_slug — resolve the GitHub slug for the maestro repo
# =============================================================================
# Returns:
...
get_maestro_slug() {
	...
	if [[ -f "$repos_json" ]] && command -v jq &>/dev/null; then
		local slug
		slug=$(jq -r '.initialized_repos[] | select(.slug | test("maestro$")) | .slug' "$repos_json" 2>/dev/null | head -1)
		if [[ -n "$slug" ]]; then
			echo "$slug"
			return 0
		fi
	...
	local maestro_path
	if maestro_path=$(get_maestro_path); then
		local remote_url
		remote_url=$(git -C "$maestro_path" remote get-url origin 2>/dev/null || echo "")
	...
	log_warn "Could not resolve maestro slug"
	return 1
}

# =============================================================================
# log_framework_issue — create a GitHub issue on the maestro repo
# =============================================================================
...
log_framework_issue() {
	...
	# Resolve maestro slug
	local slug
	if ! slug=$(get_maestro_slug); then
		log_error "Cannot resolve maestro repo slug — cannot create issue"
		return 1
	fi
...
Commands:
  ...
  get-maestro-path       Resolve local path to maestro repo
  get-maestro-slug       Resolve GitHub slug for maestro repo
  log-framework-issue     Create an issue on the maestro repo
    --title "..."         Issue title (required)
