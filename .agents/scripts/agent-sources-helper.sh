...
# layout. The full tree is synced into ~/.maestro/agents/custom/<source-name>/.
...
show_help() {
	...
	echo "The full .agents tree is synced into ~/.maestro/agents/custom/<source-name>/"
	return 0
}
		...
		cat >"${CONFIG_FILE}" <<'DEFAULTJSON'
{
  ...
  "$comment": "Private agent repositories synced into ~/.maestro/agents/custom/",
  "version": "1.0.0",
  "sources": []
}
DEFAULTJSON
...
# Read sources from config using node (available everywhere maestro runs)
...
cmd_add_remote() {
	local remote_url="$1"
	local clone_dir="${MAESTRO_GIT_DIR:-${HOME}/Git}"
...
# subcommand (invoked by `maestro update` and session-start update-check as
