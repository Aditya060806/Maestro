...
# Globals written:
#   - none (state written to disk under ~/.maestro/logs/)
...
run_daily_quality_sweep() {
	...
	routine_runner_user=$(maestro_repo_state_current_user)
	...
	while IFS='|' read -r slug path; do
		[[ -z "$slug" ]] && continue
		[[ ! -d "$path" ]] && continue
		if ! maestro_can_run_repo_routines "$slug" "$routine_runner_user"; then
...
_simplification_issue_maintainer() {
	...
	maintainer=$(jq -r --arg slug "$repo_slug" \
		...
		"${HOME}/.config/maestro/repos.json" 2>/dev/null) || maintainer=""
...
_simplification_issue_label_csv() {
	...
	# #maestro:trust-boundary — NMR is an external-origin approval gate. When
	...
	if declare -F _gh_current_user_allows_repo_write >/dev/null 2>&1 \
		...
		echo "[stats] Function-complexity-debt issues: trusted current user ${MAESTRO_GH_WRITE_PERMISSION_USER:-unknown} (${MAESTRO_GH_WRITE_PERMISSION_LEVEL:-unknown}) — skipping needs-maintainer-review" >>"$LOGFILE"
	else
		simplification_labels="${simplification_labels},needs-maintainer-review"
		echo "[stats] Function-complexity-debt issues: current user not verified as repo writer (${MAESTRO_GH_WRITE_PERMISSION_REASON:-helper-unavailable}) — keeping needs-maintainer-review" >>"$LOGFILE"
	fi
...
_create_single_simplification_issue() {
	...
	qlty_sig=$("${HOME}/.maestro/agents/scripts/gh-signature-helper.sh" footer --body "$issue_body" 2>/dev/null || true)
