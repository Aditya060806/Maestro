...
setup_ampcode_config() {
	...
	if ! load_api_config; then
		...
		print_info "Then run: Add AMPCODE_API_KEY to ~/.config/maestro/credentials.sh"
		return 1
	fi
...
show_help() {
	...
	echo "  3. Run: Add AMPCODE_API_KEY to ~/.config/maestro/credentials.sh"
	echo "  4. Run: $0 setup"
	echo ""
	echo "This script integrates AmpCode's professional AI coding assistant"
	echo "with the Maestro Framework for enhanced code quality and productivity."
	echo ""
	return 0
}
