...
Preferred CLI (same commands, no path needed):
  maestro model-accounts-pool <command>
...
Quickstart (if you see "Key Missing", "invalid request data", or auth errors):
  maestro model-accounts-pool diagnose          # 0. Full pipeline check (start here)
  maestro model-accounts-pool status            # 1. See pool health at a glance
  maestro model-accounts-pool check             # 2. Test token validity live
  maestro model-accounts-pool rotate anthropic  # 3. Switch to next account if rate-limited
  maestro model-accounts-pool reset-cooldowns   # 4. Clear cooldowns if all accounts stuck
  maestro model-accounts-pool add anthropic     # 5. Re-add account if pool empty
...
Notes:
  - Pool file: ~/.maestro/oauth-pool.json (600 permissions)
...
_diag_check_pool() {
	if [[ ! -f "$POOL_FILE" ]]; then
		_diag_print "Pool file" "FAIL" "Missing: $POOL_FILE — run 'maestro model-accounts-pool add anthropic'"
		return 1
	fi
	local perms
	...
	if [[ "$count" == "0" ]]; then
		_diag_print "Anthropic accounts" "FAIL" "No accounts in pool — run 'maestro model-accounts-pool add anthropic'"
		return 1
	fi
...
_diag_check_plugin() {
	local plugin_dir="$HOME/.maestro/agents/plugins/opencode-maestro"
	if [[ ! -d "$plugin_dir" ]]; then
		_diag_print "Plugin directory" "FAIL" "Missing: $plugin_dir — run 'maestro update'"
		return 1
	fi
	local missing=""
...
_diag_check_plugin_registered() {
	...
	if grep -q "opencode-maestro" "$opencode_config" 2>/dev/null; then
...
_diag_check_cch() {
	local cch_file="$HOME/.maestro/cch-constants.json"
	if [[ ! -f "$cch_file" ]]; then
	...
	if command -v claude &>/dev/null; then
		...
		if [[ -n "$live_ver" ]]; then
			if [[ "$cached_ver" == "$live_ver" ]]; then
				...
				_diag_print "CCH version match" "WARN" "cache=$cached_ver, cli=$live_ver — run 'maestro client-format extract'"
			fi
		fi
	else
...
_diag_check_opencode_version() {
	...
	local tracked_ver=""
	local pkg_file="$HOME/.maestro/agents/plugins/opencode-maestro/package.json"
	if [[ -f "$pkg_file" ]]; then
...
_diag_check_recent_errors() {
	local db_path="$HOME/.maestro/.agent-workspace/observability/llm-requests.db"
	if [[ ! -f "$db_path" ]]; then
...
cmd_diagnose() {
	...
	if [[ "$has_failures" == "true" ]]; then
		...
		echo "  grep '\\[maestro\\]' ~/opencode-debug.log"
	fi
	echo ""
	return 0
}
