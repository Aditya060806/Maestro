...
_file_circuit_breaker_meta_no_work() {
	...
	local filer="${SCRIPT_DIR:-${HOME}/.maestro/agents/scripts}/circuit-breaker-meta-filer.sh"
	[[ -x "$filer" ]] || return 0
...
_Automated by \`escalate_issue_tier()\` cascade dispatch in worker-lifecycle-common.sh_"
	...
	local ledger_helper="${HOME}/.maestro/agents/scripts/dispatch-ledger-helper.sh"
...
#
# When MAESTRO_VERBOSE_LIFECYCLE=1 is set in the worker's environment
...
# per-event sentinel file under ~/.maestro/cache/lifecycle-watch-<pid>/.
...
_verbose_lifecycle_sentinel_dir() {
	local pid="${1:-$$}"
	local dir="${HOME}/.maestro/cache/lifecycle-watch-${pid}"
...
#
# Gates on MAESTRO_VERBOSE_LIFECYCLE=1. Idempotent: each (pid, event)
...
_emit_verbose_checkpoint() {
	...
	[[ "${MAESTRO_VERBOSE_LIFECYCLE:-0}" != "1" ]] && return 0
	[[ -z "$event" ]] && return 0
	...
	printf '[lifecycle] %s ts=%s pid=%s session=%s%s\n' \
		"$safe_event" "$ts" "$$" "${WORKER_SESSION_KEY:-${MAESTRO_SESSION_KEY:-unknown}}" "$extra" >&2
...
# timeout (default 30 min, env MAESTRO_VERBOSE_LIFECYCLE_WATCH_TIMEOUT).
...
_start_verbose_lifecycle_watcher() {
	...
	local timeout="${MAESTRO_VERBOSE_LIFECYCLE_WATCH_TIMEOUT:-1800}"
...
_cleanup_verbose_lifecycle_watcher() {
	...
	local sentinel_dir="${HOME}/.maestro/cache/lifecycle-watch-${worker_pid}"
