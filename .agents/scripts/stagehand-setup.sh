...
# Stagehand Setup Script for Maestro Framework
...
readonly STAGEHAND_CONFIG_DIR="${HOME}/.maestro/stagehand"
...
async function searchProducts(query, maxResults = 5) {
    ...
    try {
        ...
        const filepath = `${process.env.HOME}/.maestro/stagehand/results/${filename}`;
        
        // Ensure results directory exists
        fs.mkdirSync(`${process.env.HOME}/.maestro/stagehand/results`, { recursive: true });
...
setup_mcp_integration() {
	...
	local mcp_config="${HOME}/.maestro/mcp/stagehand-config.json"
...
main() {
	...
	"setup")
		...
		print_info "2. Configure API keys in ~/.maestro/stagehand/.env"
		print_info "3. Try examples: cd ~/.maestro/stagehand && npm run search-products" || exit
		;;
	"examples")
		create_advanced_examples
		;;
