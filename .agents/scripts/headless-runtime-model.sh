...
# =============================================================================
# maestro Headless Runtime Model — Model Choice & Cmd Builders (GH#19699)
# =============================================================================
...
# usable auth at dispatch time. This eliminates MAESTRO_HEADLESS_MODELS as a
# user-configurable env var while allowing temporary provider pinning via
# MAESTRO_HEADLESS_PROVIDER_ALLOWLIST.
get_configured_models() {
	local tier_name="${1:-sonnet}"
	local allowlist_raw="${MAESTRO_HEADLESS_PROVIDER_ALLOWLIST:-}"
	local -a allowlist=()
	local -a models=()
	...
	if [[ -n "${MAESTRO_HEADLESS_MODELS:-}" ]]; then
		print_warning "MAESTRO_HEADLESS_MODELS is deprecated (v3.7+). Model routing is now automatic via pool + routing table. Remove this export from credentials.sh. Respecting override for this release cycle."
		local -a raw_models=()
		IFS=',' read -r -a raw_models <<<"$MAESTRO_HEADLESS_MODELS"
...
_choose_model_tier_downgrade() {
	...
	local downgrade_task_type="${MAESTRO_TIER_DOWNGRADE_TASK_TYPE:-}"
	...
	*pro*) current_tier="pro" ;;
	...
	if [[ ! -x "$pattern_helper" ]]; then
		pattern_helper="${HOME}/.maestro/agents/scripts/archived/pattern-tracker-helper.sh"
	fi
	...
	lower_tier=$("$pattern_helper" tier-downgrade-check \
		...
		--min-samples "${MAESTRO_TIER_DOWNGRADE_MIN_SAMPLES:-3}" \
		2>/dev/null || true)
...
resolve_headless_variant() {
	...
	local tier="${2:-}"
	local selected_model="${3:-}"
	local variant="${MAESTRO_HEADLESS_VARIANT:-}"
	local tier_upper=""
	...
	if [[ -n "$tier" ]]; then
		...
		HAIKU | FLASH | SONNET | PRO | OPUS | HEALTH | EVAL | CODING)
			local tier_env_var="MAESTRO_HEADLESS_VARIANT_${tier_upper}"
	...
	case "$role" in
	pulse)
		if [[ -n "${MAESTRO_HEADLESS_PULSE_VARIANT:-}" ]]; then
			variant="${MAESTRO_HEADLESS_PULSE_VARIANT}"
		fi
		;;
	worker)
		if [[ -n "${MAESTRO_HEADLESS_WORKER_VARIANT:-}" ]]; then
			variant="${MAESTRO_HEADLESS_WORKER_VARIANT}"
		fi
		;;
	esac
...
_build_claude_cmd() {
	...
	if [[ -n "$agent_name" ]]; then
		...
		# maestro agent behaviour as interactive sessions.
