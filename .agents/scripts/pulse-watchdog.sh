...
PULSE_WATCHDOG_LOG="${PULSE_WATCHDOG_LOG:-${HOME}/.maestro/logs/pulse-watchdog.log}"
...
LIFECYCLE_LOG="${LIFECYCLE_LOG:-${HOME}/.maestro/logs/pulse-dispatch.log}"
...
_check_watchdog_conditions() {
	...
	# Check 0: Stop flag — user ran `maestro pulse stop` during this cycle (t2943)
	# t3060: Enum prefix — see _watchdog_check_progress for rationale.
	if [[ -f "$STOP_FLAG" ]]; then
