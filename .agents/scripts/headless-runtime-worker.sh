...
_maybe_rotate_isolated_auth() {
	...
	local provider="$2"
	local pool_file="${MAESTRO_OAUTH_POOL_FILE:-${HOME}/.maestro/oauth-pool.json}"
	local oauth_helper="${OAUTH_POOL_HELPER:-${HOME}/.maestro/agents/scripts/oauth-pool-helper.sh}"
	...
	(
		...
		if [[ -x "$SANDBOX_EXEC_HELPER" && "${MAESTRO_HEADLESS_SANDBOX_DISABLED:-}" != "1" ]]; then
			...
			if [[ "${MAESTRO_HEADLESS_SANDBOX_DISABLED:-}" == "1" ]]; then
				print_info "MAESTRO_HEADLESS_SANDBOX_DISABLED=1 — using bare exec (no privilege isolation) (GH#20146 audit)"
			fi
			if [[ -n "${_HEADLESS_CLAUDE_STDIN_FILE:-}" && -f "${_HEADLESS_CLAUDE_STDIN_FILE:-}" ]]; then
...
# to ~/.maestro/logs/worker-no-activity/<session>-<ts>.log. Size-cap
...
_preserve_no_activity_output() {
	...
	local diag_dir="${HOME}/.maestro/logs/worker-no-activity"
...
_increment_orphan_count_stat() {
	...
	local _stats_file="${PULSE_STATS_FILE:-${HOME}/.maestro/logs/pulse-stats.json}"
	local _stats_dir
...
_hrw_reclaim_stale_worker_worktree_owner() {
	...
	if _hrw_is_dispatch_precreate_owner "$owner_session" || [[ "${MAESTRO_ALLOW_WORKER_WORKTREE_OWNER_TRANSFER:-0}" == "1" ]]; then
	...
	local reclaim_age_s="${MAESTRO_WORKER_WORKTREE_OWNER_RECLAIM_AGE_SECONDS:-900}"
