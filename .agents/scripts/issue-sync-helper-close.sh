...
_closed_issue_maestro_complete_date() {
	local repo="$1" issue_number="$2"
	...
	if printf '%s' "$evidence" | grep -qE 'maestro:sig|CLAIM_RELEASED reason=worker_complete|Task t[0-9]+(\.[0-9]+)* done in TODO\.md|Completed via (\[)?PR #[0-9]+'; then
		printf '%s\n' "${closed_at%%T*}"
		return 0
	fi
	return 1
...
_reopen_mark_if_completed() {
	...
	completed_date=$(_closed_issue_maestro_complete_date "$repo" "$ref_num" || true)
	if [[ -n "$completed_date" ]]; then
		_mark_reopen_completed_task "$tid" "$todo_file" "$ref_num" "$completed_date" "maestro close evidence"
		return 0
	fi
	return 1
}
