...
_append_audit_log() {
	...
	actor="${MAESTRO_ACTOR:-$(whoami 2>/dev/null || echo "agent")}"
...
_build_nmr_body_file() {
	...
	if [[ -f "$tmpl" ]]; then
		...
		cat > "$body_file" <<BODY
<!-- maestro:knowledge-review source_id:${source_id} -->
...
- Approve: \`sudo maestro approve issue <this-issue-number> ${repo_slug}\`
...
_file_nmr_issue() {
	...
	if command -v gh-signature-helper.sh >/dev/null 2>&1; then
		gh-signature-helper.sh footer --model "${MAESTRO_MODEL:-unknown}" \
...
cmd_promote() {
	...
	local actor
	actor="${MAESTRO_ACTOR:-$(whoami 2>/dev/null || echo "maintainer")}"
...
Crypto-approval flow (untrusted sources):
  sudo maestro approve issue <N> <owner/repo>  →  promotes source from staging/ to sources/
