...
# `~/.maestro/advisories/canonical-recovery-<basename>.advisory` when it
...
# Behaviour
# ---------
# 1. Scan `~/.maestro/advisories/canonical-recovery-*.advisory`
...
# Env
# ---
#   MAESTRO_ADVISORY_DIR         — override advisory dir (default ~/.maestro/advisories)
#   MAESTRO_SKIP_CANONICAL_RECOVERY_ROUTINE=1 — disable (always exit 0)
#
...
ADVISORY_DIR="${MAESTRO_ADVISORY_DIR:-${HOME}/.maestro/advisories}"
...
# ---------------------------------------------------------------------------
_crr_cmd_tick() {
	if [[ "${MAESTRO_SKIP_CANONICAL_RECOVERY_ROUTINE:-0}" == "1" ]]; then
		_crr_log "MAESTRO_SKIP_CANONICAL_RECOVERY_ROUTINE=1 — skipping"
		return 0
	fi
...
# `maestro security` integration and ad-hoc diagnosis.
# ---------------------------------------------------------------------------
_crr_cmd_list() {
	if [[ ! -d "$ADVISORY_DIR" ]]; then
...
Env:
  MAESTRO_ADVISORY_DIR                       Override advisory dir
  MAESTRO_SKIP_CANONICAL_RECOVERY_ROUTINE=1  Disable (always exit 0)
