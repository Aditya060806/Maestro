...
#
# Enumerates private repo slugs from ~/.config/maestro/repos.json and scans
...
#
# Cache: ~/.maestro/cache/repo-privacy.json (TTL 10m)
# Repos: ~/.config/maestro/repos.json
...
PRIVACY_REPOS_CONFIG="${PRIVACY_REPOS_CONFIG:-$HOME/.config/maestro/repos.json}"
PRIVACY_CACHE_FILE="${PRIVACY_CACHE_FILE:-$HOME/.maestro/cache/repo-privacy.json}"
PRIVACY_CACHE_TTL="${PRIVACY_CACHE_TTL:-600}" # 10 minutes
# Paths whose diffs we scan. Default to the full repository because maestro is
...
# Caches positive + negative results to ~/.maestro/cache/repo-privacy.json
...
# ~/.maestro/configs/privacy-guard-extra-slugs.txt (one per line).
#
# Arguments:
...
privacy_enumerate_private_slugs() {
	...
	local extra_file="$HOME/.maestro/configs/privacy-guard-extra-slugs.txt"
...
# configured ERE patterns live in ~/.maestro/configs/privacy-guard-private-path-patterns.txt.
# Arguments:
#   $1 - text content to scan
# Returns:
#   0 if no hits, 1 if at least one hit, 2 on setup error.
...
privacy_scan_local_paths() {
	...
	local patterns_file="$HOME/.maestro/configs/privacy-guard-private-path-patterns.txt"
...
#######################################
# List maestro script basenames that look credential-like and may be redacted.
# Output:
#   newline-delimited basenames
#######################################
_privacy_maestro_script_reference_basenames() {
...
#######################################
# Redact maestro script file references before credential-prefix scanning.
#
# Some maestro helper basenames legitimately contain substrings such as
...
#######################################
_privacy_redact_maestro_script_references() {
	local text="$1"
	...
	if [[ -z "$basenames_provided" ]]; then
		basenames_text=$(_privacy_maestro_script_reference_basenames)
	fi
	...
	while IFS= read -r basename || [[ -n "$basename" ]]; do
		[[ -z "$basename" ]] && continue
		redacted="${redacted//.agents\/scripts\/$basename/[maestro-script-reference]}"
		redacted="${redacted//.\/\.agents\/scripts\/$basename/[maestro-script-reference]}"
		redacted="${redacted//\`$basename\`/[maestro-script-reference]}"
		redacted="${redacted//\"$basename\"/[maestro-script-reference]}"
		redacted="${redacted// $basename/ [maestro-script-reference]}"
		redacted="${redacted//$'\n'$basename/$'\n'[maestro-script-reference]}"
	done <<<"$basenames_text"
...
privacy_scan_secret_material_text() {
	...
	scan_text=$(_privacy_redact_maestro_script_references "$text")
	if [[ "$scan_text" != "$text" ]]; then
		printf '%s\n' '[privacy-scan][ALLOW] maestro script file reference' >&2
	fi
...
privacy_scan_secret_material_diff() {
	...
	local maestro_script_basenames
	maestro_script_basenames=$(_privacy_maestro_script_reference_basenames)
	while IFS= read -r line; do
		...
		"+"*)
			...
			local added="${line:1}"
			local scan_added
			scan_added=$(_privacy_redact_maestro_script_references "$added" "$maestro_script_basenames")
			if [[ "$scan_added" != "$added" ]]; then
				printf '%s:%s: [privacy-scan][ALLOW] maestro script file reference\n' "$current_file" "$line_num" >&2
			fi
