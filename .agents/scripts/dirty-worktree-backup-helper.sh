...
BACKUP_ROOT="${MAESTRO_DIRTY_BACKUP_ROOT:-${HOME}/.maestro/.agent-workspace/tmp/dirty-main-backups}"
DEFAULT_RETENTION_DAYS="${MAESTRO_DIRTY_BACKUP_RETENTION_DAYS:-30}"
...
Environment:
  MAESTRO_DIRTY_BACKUP_ROOT            Override backup directory.
  MAESTRO_DIRTY_BACKUP_RETENTION_DAYS Override stale-backup retention (default: 30).
USAGE
	return 0
}
...
cmd_backup() {
	...
	local pr_number=""
	local issue_number=""
	local session_key="${MAESTRO_SESSION_KEY:-${WORKER_SESSION_KEY:-}}"
	local task_id="${MAESTRO_TASK_ID:-${WORKER_TASK_NUMBER:-}}"
	local arg=""
