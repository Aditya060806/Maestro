...
get_mcp_command() {
	...
	"grep-vercel") echo "" ;;
	"claude-code-mcp") echo "npx -y github:Aditya060806/claude-code-mcp" ;;
	"stagehand") echo "node ${HOME}/.maestro/stagehand/examples/basic-example.js" ;;
	"stagehand-python") echo "${HOME}/.maestro/stagehand-python/.venv/bin/python ${HOME}/.maestro/stagehand-python/examples/basic_example.py" ;;
...
print_mcp_security_warning() {
	...
	print_info "  See: ~/.maestro/.agents/tools/mcp-toolkit/mcporter.md 'Security Considerations'"
	echo ""
	return 0
}
...
_install_ahrefs() {
	...
	echo ""
	print_info "Store in ~/.config/maestro/credentials.sh:"
...
_install_grep_vercel() {
	print_info "Grep by Vercel MCP (grep.app) is no longer installed by maestro"
...
_install_claude_code_mcp() {
	...
	print_info "Source: https://github.com/Aditya060806/claude-code-mcp"
...
_install_stagehand() {
	...
	if command -v claude &>/dev/null; then
		claude mcp add stagehand "node" --args "${HOME}/.maestro/stagehand/examples/basic-example.js"
	fi
...
_install_stagehand_python() {
	...
	# Add to Claude MCP if available
	if command -v claude &>/dev/null; then
		local python_path="${HOME}/.maestro/stagehand-python/.venv/bin/python"
		claude mcp add stagehand-python "$python_path" --args "${HOME}/.maestro/stagehand-python/examples/basic_example.py"
	fi
...
_install_dataforseo() {
	...
	print_info "  bash ~/.maestro/agents/scripts/setup-local-api-keys.sh set DATAFORSEO_USERNAME your_username"
	print_info "  bash ~/.maestro/agents/scripts/setup-local-api-keys.sh set DATAFORSEO_PASSWORD your_password"
	echo ""
	print_info "For OpenCode, use bash wrapper pattern in opencode.json:"
	print_info '  "dataforseo": {'
	print_info '    "type": "local",'
	print_info '    "command": ["/bin/bash", "-c", "source ~/.config/maestro/credentials.sh && DATAFORSEO_USERNAME=\$DATAFORSEO_USERNAME DATAFORSEO_PASSWORD=\$DATAFORSEO_PASSWORD npx dataforseo-mcp-server"],'
	print_info '    "enabled": true'
	print_info '  }'
	echo ""
...
_install_unstract() {
	...
	if [[ -f "$helper_script" ]]; then
		...
		print_info "  git clone https://github.com/Zipstack/unstract.git ~/.maestro/unstract"
		print_info "  cd ~/.maestro/unstract && ./run-platform.sh"
		return 1
	fi
...
_install_context7() {
	...
	print_info "Docs: ~/.maestro/agents/tools/context/context7.md"
	return 0
}
...
_install_shopify_dev_mcp() {
	...
	echo ""
	print_info "Docs: ~/.maestro/agents/services/ecommerce/shopify.md"
	print_info "Config template: ~/.maestro/agents/configs/mcp-templates/shopify-dev-mcp-config.json.txt"
	return 0
}
  ...
  "mcpServers": {
    "stagehand-python": {
      ...
      "env": {
        ...
        "STAGEHAND_VERBOSE": "1",
        "STAGEHAND_HEADLESS": "false",
        "PYTHONPATH": "${HOME}/.maestro/stagehand-python/.venv/lib/python3.11/site-packages"
      }
    }
  }
}
