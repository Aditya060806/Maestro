...
PERSONAL_PLANE_BASE="${HOME}/.maestro/.agent-workspace/knowledge"
...
_resolve_knowledge_root() {
	...
	print_error "Cannot resolve knowledge root. Run: maestro knowledge init repo"
	return 1
}
...
cmd_tick() {
	...
	if ! knowledge_root=$(_resolve_knowledge_root "$knowledge_root_override"); then
		print_info "tick: skipped — knowledge plane not initialized; run maestro knowledge init repo to enable"
		return 0
	fi
