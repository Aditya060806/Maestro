...
# #maestro:trust-boundary — contributor/read-only repos are external
# observation targets. Pulse must never comment, label, close, approve, or
# impose maestro maintainer-gate policy on repos it does not manage.
...
_trusted_dependabot_updates_conf() {
	...
	printf '%s' "${MAESTRO_TRUSTED_DEPENDABOT_UPDATES_CONF:-$default_conf}"
	return 0
}
...
#
# #maestro:trust-boundary — Dependabot bypass is not author-name trust. It
...
_is_trusted_dependabot_update_pr() {
	...
	[[ "${MAESTRO_TRUSTED_DEPENDABOT_UPDATES:-1}" != "0" ]] || return 1
...
2. Cryptographic approval on that linked issue (\`sudo maestro approve issue NNN ${repo_slug}\`)
...
- \`declined: <reason>\` — closes this PR (include your reason after the colon)" &&
		...
		gh_pr_comment "$pr_number" --repo "$repo_slug" \
				--body "**Missing linked issue.** This PR has no linked issue. External contributor PRs require a linked issue before they can be considered for merge. Add \`Resolves #NNN\` to the PR body (or use \`GH#NNN:\` in the title), then ensure that issue has been cryptographically approved by a maintainer (\`sudo maestro approve issue NNN ${repo_slug}\`)." || true
...
#
# A cryptographic approval (sudo maestro approve) is at least as strong a
...
#
# #maestro:trust-boundary — this function gates security-relevant approval
...
_has_maintainer_crypto_approval() {
	...
	local _approval_marker="maestro-signed-approval"
...
# #maestro:trust-boundary — approval must never be posted by an unconfirmed
...
approve_collaborator_pr() {
	...
	#
	# #maestro:trust-boundary — t3063 crypto-approval bypass:
