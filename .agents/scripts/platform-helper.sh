...
#   Operations are logged to ~/.maestro/logs/platform-local-ops.log and exit 0.
...
PLATFORM_LOCAL_LOG="${HOME}/.maestro/logs/platform-local-ops.log"
_PLAT_LOCAL="local"
...
platform_create_issue() {
	...
	github)
		...
		gh issue create --repo "$slug" --title "$title" --body-file "$body_file" "${label_args[@]}" # maestro-allow: raw-gh-wrapper
		;;
	local)
...
platform_create_pr() {
	...
	github)
		_platform_require_gh || return 1
		gh pr create --repo "$slug" --title "$title" --body-file "$body_file" --base "$base" --head "$head" # maestro-allow: raw-gh-wrapper
		;;
	local)
