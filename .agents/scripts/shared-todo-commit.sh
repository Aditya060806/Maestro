...
#   - TODO_LOCK_DIR              — ${HOME}/.maestro/locks, or a user-scoped
...
#   - MAESTRO_PLANNING_FORCE_PR_FALLBACK=1
#                                 — force the protected-default planning PR
#                                   path (test/operator override).
#   - MAESTRO_PLANNING_PR_REPO_SLUG=owner/repo
...
#   - $MAESTRO_LOG_FILE (optional — log target for lock + retry diagnostics;
...
if [[ -z "${TODO_LOCK_DIR:-}" ]]; then
	readonly TODO_LOCK_DIR="${HOME:-/tmp/maestro-${USER:-uid-${UID:-shared}}}/.maestro/locks"
...
_todo_origin_slug() {
	local repo_path="$1"
	local override_slug="${MAESTRO_PLANNING_PR_REPO_SLUG:-}"
...
_todo_branch_requires_planning_pr() {
	...
	if [[ "${MAESTRO_PLANNING_FORCE_PR_FALLBACK:-0}" == "1" ]]; then
		return 0
	fi
...
_todo_open_planning_pr() {
	...
	pr_url=$(MAESTRO_PR_CREATE_READY=1 gh_create_pr \
		--repo "$slug" \
		--base "$default_branch" \
		--head "$branch_name" \
		--title "$pr_title" \
