...
# normal non-cache callers noisy. Pulse enables MAESTRO_GH_PR_VIEW_CACHE, so
# these records expose disabled/invalid TTL states during cache triage.
# Returns: 0 always.
#######################################
_gh_pr_view_snapshot_record_disabled() {
	if [[ "${MAESTRO_GH_PR_VIEW_CACHE_DISABLE:-0}" == "1" ]]; then
...
_gh_pr_list_snapshot_get() {
	local _ttl="${MAESTRO_GH_PR_LIST_CACHE_TTL:-15}"
	[[ "${MAESTRO_GH_PR_LIST_CACHE_DISABLE:-0}" == "1" ]] && return 1
	...
	_path="${MAESTRO_GH_PR_LIST_CACHE_DIR:-${HOME}/.maestro/cache/gh-pr-list-snapshots}/${_key}.json"
...
_gh_pr_list_snapshot_put() {
	local _body="$1"; shift
	local _ttl="${MAESTRO_GH_PR_LIST_CACHE_TTL:-15}"
	[[ "${MAESTRO_GH_PR_LIST_CACHE_DISABLE:-0}" == "1" ]] && return 0
	[[ "$_ttl" =~ ^[0-9]+$ && "$_ttl" -gt 0 ]] || return 0
	_gh_pr_list_snapshot_cacheable "$@" || return 0
	local _dir _key _path _tmp
	_dir="${MAESTRO_GH_PR_LIST_CACHE_DIR:-${HOME}/.maestro/cache/gh-pr-list-snapshots}"
...
# to pulse cycles (or explicit callers) through MAESTRO_GH_PR_VIEW_CACHE and is
...
# Mutation-sensitive merge gates must set MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1
...
_gh_pr_view_snapshot_enabled() {
	...
	local _ttl="${MAESTRO_GH_PR_VIEW_CACHE_TTL:-15}"
	[[ "$_ttl" =~ ^[0-9]+$ && "$_ttl" -gt 0 ]] || return 1
	return 0
}
...
_gh_pr_view_snapshot_path() {
	local _dir="${MAESTRO_GH_PR_VIEW_CACHE_DIR:-${HOME}/.maestro/cache/gh-pr-view-snapshots}"
