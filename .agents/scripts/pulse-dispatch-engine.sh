...
: "${PULSE_RATE_LIMIT_FLAG:=${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag}"
: "${PULSE_RUNNABLE_ISSUE_LIMIT:=1000}"
: "${MAESTRO_PULSE_ASYNC_POST_DISPATCH_HOUSEKEEPING:=1}"
...
check_worker_launch() {
	...
	done < <(maestro_pulse_worker_log_candidates "$repo_slug" "$issue_number" 2>/dev/null || true)
...
build_ranked_dispatch_candidates_json() {
	...
	tmp_candidates=$(mktemp 2>/dev/null || echo "/tmp/maestro-pulse-candidates.$$")
	: >"$tmp_candidates"
...
dispatch_max() {
	...
	_DISPATCH_THROTTLE_FILE="${HOME}/.maestro/logs/dispatch-throttle"
	_DISPATCH_CANARY_CACHE="${MAESTRO_HEADLESS_RUNTIME_DIR:-${HOME}/.maestro/.agent-workspace/headless-runtime}/canary-last-pass"
	...
	_dispatch_candidate_file=$(mktemp 2>/dev/null || echo "/tmp/maestro-dff-candidates.$$")
	...
	if ((_dispatch_max_parallel <= 1)); then
		...
		_dispatch_outcomes_file=$(mktemp 2>/dev/null || echo "/tmp/maestro-dispatch-outcomes.$$")
...
_dispatch_should_use_floor_path() {
	...
	[[ -f "${_DISPATCH_THROTTLE_FILE:-${HOME}/.maestro/logs/dispatch-throttle}" ]] && return 0
	return 1
}
...
_pulse_post_dispatch_housekeeping_lockdir() {
	printf '%s\n' "${HOME}/.maestro/logs/pulse-post-dispatch-housekeeping.lock"
	return 0
}
...
_pulse_start_post_dispatch_housekeeping() {
	...
	if [[ "${MAESTRO_PULSE_ASYNC_POST_DISPATCH_HOUSEKEEPING:-1}" != "1" ]]; then
...
apply_dispatch_max() {
	...
	local _p2_sentinel="${HOME}/.maestro/cache/pulse-cycle-$$-consolidation-fired"
...
_dispatch_min_worker_floor_refill() {
	local min_worker_floor="${MAESTRO_MIN_WORKER_CONCURRENCY:-}"
...
run_underfill_worker_recycler() {
	...
	local recycle_throttle_file="${HOME}/.maestro/logs/underfill-recycle-last-run"
...
_run_preflight_stages() {
	...
	# MAESTRO_PULSE_ASYNC_POST_DISPATCH_HOUSEKEEPING=0 to restore the legacy
	...
	# critically low (< MAESTRO_PULSE_PREFETCH_BUDGET_THRESHOLD points,
	...
	# Bypass: MAESTRO_SKIP_PULSE_PREFETCH_BUDGET_GATE=1.
	# Counter incremented: _PULSE_HEALTH_PREFETCH_THROTTLED.
	local _budget_gate_skip=0
	if [[ "${MAESTRO_SKIP_PULSE_PREFETCH_BUDGET_GATE:-0}" != "1" ]]; then
		local _bg_threshold="${MAESTRO_PULSE_PREFETCH_BUDGET_THRESHOLD:-1250}"
		local _bg_remaining
