...
#
# install-pre-push-guards.sh — Install maestro git pre-push/pre-commit hooks (t2198, t2446, t2458, t2745, t3020, t3224).
#
...
#
# Existing hooks NOT managed by maestro are refused and left untouched.
#
...
#   MAESTRO_PREPUSH_REPO_VERIFY=0   skip repo-verify check for this push (also: MAESTRO_PREPUSH_AUTOFIX=0|1)
...
HOOK_MARKER_MANAGED="# maestro-pre-push-guards"
HOOK_MARKER_PRECOMMIT_MANAGED="# maestro-pre-commit-guards"
...
DEPLOYED_PRIVACY_HOOK="$HOME/.maestro/agents/hooks/privacy-guard-pre-push.sh"
DEPLOYED_COMPLEXITY_HOOK="$HOME/.maestro/agents/hooks/complexity-regression-pre-push.sh"
DEPLOYED_SCOPE_HOOK="$HOME/.maestro/agents/hooks/scope-guard-pre-push.sh"
DEPLOYED_CREDENTIAL_HOOK="$HOME/.maestro/agents/hooks/credential-emission-pre-push.sh"
DEPLOYED_DUP_TODO_HOOK="$HOME/.maestro/agents/hooks/pre-push-dup-todo-guard.sh"
DEPLOYED_REPO_VERIFY_HOOK="$HOME/.maestro/agents/hooks/repo-verify-pre-push.sh"
DEPLOYED_BRIEF_FILENAME_HOOK="$HOME/.maestro/agents/hooks/brief-filename-guard.sh"
...
# Creates .git/hooks/pre-commit managed by maestro.
...
_install_precommit_brief_filename() {
	...
	if [[ -f "$_chook_path" ]]; then
		if ! grep -q "$HOOK_MARKER_PRECOMMIT_MANAGED" "$_chook_path" 2>/dev/null; then
			print_error "existing pre-commit hook at $_chook_path is NOT managed by maestro"
...
#!/usr/bin/env bash
# maestro-pre-commit-guards
# Managed by .agents/scripts/install-pre-push-guards.sh — do not edit.
# Chains installed maestro pre-commit guards in order.
...
#######################################
# Uninstall the maestro-managed pre-commit hook.
...
_uninstall_precommit_brief_filename() {
	...
	if ! grep -q "$HOOK_MARKER_PRECOMMIT_MANAGED" "$_chook_path" 2>/dev/null; then
		print_warning "pre-commit hook at $_chook_path is NOT managed by maestro — leaving it alone"
		return 0
	fi
	rm -f "$_chook_path"
...
_hook_is_managed() {
	...
	grep -q "# maestro-privacy-guard" "$_p" 2>/dev/null && return 0
	return 1
}
...
#!/usr/bin/env bash
# maestro-pre-push-guards
# Managed by .agents/scripts/install-pre-push-guards.sh — do not edit.
# Chains installed maestro pre-push guards in order.
...
#              DUP_TODO_GUARD_DISABLE=1, or MAESTRO_PREPUSH_REPO_VERIFY=0
...
# Args: _hook_path
# Returns 1 (and prints guidance) if hook exists and is not maestro-managed.
#######################################
_install_reject_unmanaged_hook() {
	local _hook_path="$1"
	[[ ! -f "$_hook_path" ]] && return 0
	_hook_is_managed "$_hook_path" && return 0
	print_error "existing pre-push hook at $_hook_path is NOT managed by maestro"
...
cmd_install() {
	...
	*)
	...
	print_info "bypass individual: PRIVACY_GUARD_DISABLE=1, COMPLEXITY_GUARD_DISABLE=1, SCOPE_GUARD_DISABLE=1, CREDENTIAL_GUARD_DISABLE=1, DUP_TODO_GUARD_DISABLE=1, MAESTRO_PREPUSH_REPO_VERIFY=0, or BRIEF_FILENAME_GUARD_DISABLE=1"
	return 0
}
...
cmd_uninstall() {
	...
	if ! _hook_is_managed "$_hook_path"; then
		print_warning "hook at $_hook_path is NOT managed by maestro — leaving it alone"
		return 1
	fi
...
cmd_status() {
	...
	if ! _hook_is_managed "$_hook_path"; then
		printf 'pre-push hook: installed (unknown manager — not managed by maestro)\n'
		printf '  path: %s\n' "$_hook_path"
		return 0
	fi

	printf 'pre-push hook: installed (maestro managed)\n'
	printf '  path: %s\n' "$_hook_path"
	...
	printf 'pre-commit hook: %s\n' "$(
		if [[ -f "$_chook_path" ]] && grep -q "$HOOK_MARKER_PRECOMMIT_MANAGED" "$_chook_path" 2>/dev/null; then
			printf 'installed (maestro managed)\n  path: %s' "$_chook_path"
		elif [[ -f "$_chook_path" ]]; then
			printf 'installed (unknown manager — not managed by maestro)\n  path: %s' "$_chook_path"
		else
