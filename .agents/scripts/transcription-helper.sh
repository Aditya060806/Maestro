...
readonly CONFIG_DIR="$HOME/.config/maestro/transcription"
readonly CONFIG_FILE="$CONFIG_DIR/config.json"
readonly CACHE_DIR="$HOME/.cache/maestro/transcription"
readonly VENV_DIR="$HOME/.maestro/.agent-workspace/work/speech-to-speech/.venv"
...
transcribe_groq() {
	...
	if [[ -z "$api_key" ]]; then
		...
		print_info "Set via: maestro secret set GROQ_API_KEY"
		return 1
	fi
...
transcribe_openai() {
	...
	if [[ -z "$api_key" ]]; then
		...
		print_info "Set via: maestro secret set OPENAI_API_KEY"
		return 1
	fi
...
select_backend() {
	...
	if [[ -n "$preferred" ]]; then
		...
		groq)
			...
			print_error "GROQ_API_KEY not set. Run: maestro secret set GROQ_API_KEY"
			return 1
			;;
		openai)
			...
			print_error "OPENAI_API_KEY not set. Run: maestro secret set OPENAI_API_KEY"
			return 1
			;;
		buzz)
			if has_buzz; then
	...
	print_info "  maestro secret set GROQ_API_KEY  (cloud, free tier)"
	return 1
}
...
cmd_status() {
	...
	if [[ -n "${GROQ_API_KEY:-}" ]]; then
		...
		print_info "Groq API: no key (maestro secret set GROQ_API_KEY)"
	fi

	if [[ -n "${OPENAI_API_KEY:-}" ]]; then
		print_success "OpenAI API: key configured"
	else
		print_info "OpenAI API: no key (maestro secret set OPENAI_API_KEY)"
	fi
