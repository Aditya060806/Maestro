...
run_daily_codebase_review() {
	...
	local _cr_role
	_cr_role=$(get_repo_role_by_slug "$maestro_slug")
	...
	local perm_level
	# #maestro:trust-boundary — review-trigger comments require confirmed
	...
	if ! _gh_collaborator_permission_lookup "$maestro_slug" "$current_user" perm_level; then
		echo "[pulse-wrapper] CodeRabbit review: skipped — permission check failed for user '$current_user' on $maestro_slug (HTTP ${MAESTRO_GH_COLLAB_PERMISSION_HTTP:-unknown})" >>"$LOGFILE"
		return 0
	fi
	case "$perm_level" in
	admin | maintain | write) ;; # allowed
	*)
		echo "[pulse-wrapper] CodeRabbit review: skipped — user '$current_user' has '$perm_level' permission on $maestro_slug (need write+)" >>"$LOGFILE"
		return 0
		;;
	esac
...
_run_post_merge_review_scanner() {
	...
	local stage_budget="${MAESTRO_POST_MERGE_SCANNER_STAGE_BUDGET_SECONDS:-${PREFLIGHT_GROUP_TIMEOUT:-${PRE_RUN_STAGE_TIMEOUT:-600}}}"
	[[ "$stage_budget" =~ ^[0-9]+$ ]] || stage_budget=600
	local stage_reserve="${MAESTRO_POST_MERGE_SCANNER_STAGE_RESERVE_SECONDS:-30}"
	...
	local scanner_cursor_dir="${MAESTRO_POST_MERGE_SCANNER_CURSOR_DIR:-${HOME:-}/.maestro/logs/post-merge-review-scanner}"
...
_run_pr_review_thread_response_scanner() {
	if [[ "${MAESTRO_PR_REVIEW_THREAD_RESPONSE_ENABLED:-1}" != "1" ]]; then
		echo "[pulse-wrapper] PR review-thread response: disabled by MAESTRO_PR_REVIEW_THREAD_RESPONSE_ENABLED" >>"$LOGFILE"
		return 0
	fi
