...
# =============================================================================
# Core plugin structure and agent loader for the maestro framework.
...
# See .agents/maestro/plugins.md for full documentation.
#
...
PLUGINS_FILE="${MAESTRO_CONFIG_DIR:-$HOME/.config/maestro}/plugins.json"
PLUGIN_CACHE_DIR="${HOME}/.maestro/.agent-workspace/tmp/plugin-cache"
...
# Check that the current maestro version satisfies min_maestro_version.
# Arguments: manifest_path
# Returns: 0 always (warnings only, no hard errors)
_validate_manifest_version_compat() {
	...
	local min_version
	min_version=$(jq -r '.min_maestro_version // empty' "$manifest" 2>/dev/null)
	...
	if [[ "$cur_major" -lt "$min_major" ]] || { [[ "$cur_major" -eq "$min_major" ]] && [[ "$cur_minor" -lt "$min_minor" ]]; }; then
		log_warning "Plugin requires maestro >= $min_version (current: $current_version)"
	fi
...
run_hook() {
	...
	*)
	...
	local exit_code=0
	MAESTRO_PLUGIN_NAMESPACE="$namespace" \
		MAESTRO_PLUGIN_DIR="$plugin_dir" \
		MAESTRO_AGENTS_DIR="$AGENTS_DIR" \
		MAESTRO_HOOK="$hook_name" \
...
LIFECYCLE HOOKS:
    ...
    Hooks receive environment variables:
      ...
      MAESTRO_HOOK              Current hook name
