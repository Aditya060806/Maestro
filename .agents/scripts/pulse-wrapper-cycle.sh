...
# MAESTRO_PULSE_PRIME_MAX_AGE). Steady-state launchd respawns (every 120s)
...
# abort the cycle. Honours MAESTRO_SKIP_CACHE_PRIME=1 for debug.
#
...
_pulse_prime_caches_if_stale() {
	[[ "${MAESTRO_SKIP_CACHE_PRIME:-0}" == "1" ]] && return 0

	local _prime_helper=""
	local _prime_sentinel=""
	local _prime_max_age=""
	_prime_helper="${SCRIPT_DIR}/pulse-cache-prime.sh"
	_prime_sentinel="${HOME}/.maestro/cache/pulse-cache-prime-last-run"
	_prime_max_age="${MAESTRO_PULSE_PRIME_MAX_AGE:-1800}"
...
_pulse_check_runaway_log() {
	[[ "${MAESTRO_SKIP_RUNAWAY_LOG_CHECK:-0}" == "1" ]] && return 0
	...
	_detector_sentinel="${HOME}/.maestro/cache/pulse-runaway-log-check-last-run"
