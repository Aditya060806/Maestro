...
#
# Launch OpenCode with an maestro-managed per-session data directory.
...
usage() {
    cat <<'EOF'
Usage: maestro opencode [options] [--] [opencode args...]
       maestro opencode-desktop [launch options]
       maestro opencode-desktop install-shortcut [options]
       maestro opencode-desktop status [options]
...
Examples:
  maestro opencode
  maestro opencode --dir ~/Git/maestro
  maestro opencode -- --version
  maestro opencode-desktop
  maestro opencode-desktop install-shortcut
EOF
    return 0
}
...
desktop_usage() {
    cat <<EOF
Usage: maestro opencode-desktop [launch options]
       maestro opencode-desktop install-shortcut [options]
       maestro opencode-desktop status [options]

Launch OpenCode Desktop with an maestro-managed XDG_DATA_HOME so the Desktop
...
Shortcut options:
  --name NAME            App display name (default: OpenCode Maestro)
...
Environment overrides:
  MAESTRO_OPENCODE_DESKTOP_APP_DIR       Parent directory for generated .app
  MAESTRO_OPENCODE_DESKTOP_BINARY        OpenCode Desktop executable path
  MAESTRO_OPENCODE_DESKTOP_DATA_DIR      Explicit Desktop XDG_DATA_HOME
EOF
    return 0
}
...
build_session_data_dir() {
    ...
    printf '%s/opencode-interactive/%s' "${MAESTRO_WORK_DIR:-${HOME}/.maestro/.agent-workspace/work}" "${safe_id}"
    return 0
}
...
build_desktop_data_dir() {
    ...
    safe_id=$(sql_escape_label "${session_id}")
    printf '%s/opencode-desktop/%s' "${MAESTRO_WORK_DIR:-${HOME}/.maestro/.agent-workspace/work}" "${safe_id}"
    return 0
}

launcher_state_dir() {
    printf '%s/opencode-launcher' "${MAESTRO_WORK_DIR:-${HOME}/.maestro/.agent-workspace/work}"
    return 0
}
...
detect_desktop_binary() {
    ...
    if [[ -n "${MAESTRO_OPENCODE_DESKTOP_BINARY:-}" ]]; then
        [[ -x "${MAESTRO_OPENCODE_DESKTOP_BINARY}" ]] || return 1
        printf '%s' "${MAESTRO_OPENCODE_DESKTOP_BINARY}"
        return 0
    fi
...
desktop_app_path() {
    ...
    safe_name=${app_name//\//-}
    safe_name=${safe_name:-OpenCode Maestro}
...
helper="${MAESTRO_OPENCODE_LAUNCHER_HELPER:-${HOME}/.maestro/agents/scripts/opencode-launcher-helper.sh}"
if [[ ! -x "${helper}" ]]; then
    osascript -e 'display alert "OpenCode Maestro launcher is not installed" message "Run maestro update, then open this app again."' >/dev/null 2>&1 || true
    exit 1
fi
...
cmd_desktop_install_shortcut() {
    local app_name="OpenCode Maestro"
    local app_dir="${MAESTRO_OPENCODE_DESKTOP_APP_DIR:-${HOME}/Applications}"
    local source_binary=""
    local dry_run=0
    ...
    local macos_dir=""
    local plist_path=""
    local executable_name="opencode-maestro"
    local executable_path=""
    local bundle_id="sh.maestro.opencode.desktop"
...
cmd_desktop_launch() {
    ...
    local from_app=0
    local session_id=""
    local data_dir="${MAESTRO_OPENCODE_DESKTOP_DATA_DIR:-}"
    local source_binary=""
    ...
    if ((dry_run == 1)); then
        printf 'cd %q && XDG_DATA_HOME=%q MAESTRO_OPENCODE_ISOLATED_DB=1 %q' "${launch_dir}" "${data_dir}" "${desktop_binary}"
    ...
    export MAESTRO_OPENCODE_ISOLATED_DB=1
...
main() {
    ...
    if ((dry_run == 1)); then
        printf 'cd %q && XDG_DATA_HOME=%q MAESTRO_OPENCODE_ISOLATED_DB=1 opencode' "${launch_dir}" "${data_dir}"
        printf ' %q' "${opencode_args[@]}"
        printf '\n'
        return 0
    fi
