...
# Manage API keys in ~/.config/maestro/credentials.sh (sourced by shell configs)
#
...
readonly API_KEY_DIR="$HOME/.config/maestro"
...
# Location: ~/.config/maestro/credentials.sh
#
...
setup_shell_integration() {
	local source_line='[[ -f ~/.config/maestro/credentials.sh ]] && source ~/.config/maestro/credentials.sh'
	local updated=0

	for config in "${SHELL_CONFIGS[@]}"; do
		if [[ -f "$config" ]] && ! grep -q "credentials.sh" "$config" 2>/dev/null; then
			echo "" >>"$config"
			echo "# Maestro API Keys (single source of truth)" >>"$config"
...
# Show help
show_help() {
	print_info "Maestro - Secure Local API Key Management"
	echo ""
