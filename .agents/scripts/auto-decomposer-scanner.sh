...
#   Aditya060806/Maestro sat in this state by the time t2442 was filed.
#
...
#   marker in body (`<!-- maestro:generator=auto-decompose parent=NNNN -->`)
...
#   AUTO_DECOMPOSER_PARENT_STATE  (default ~/.maestro/logs/auto-decomposer-parent-state.json) — per-parent state file
#
# t2442: https://github.com/Aditya060806/Maestro/issues/20139
# t2573: https://github.com/Aditya060806/Maestro/issues/20242
...
AUTO_DECOMPOSER_PARENT_STATE="${AUTO_DECOMPOSER_PARENT_STATE:-${HOME}/.maestro/logs/auto-decomposer-parent-state.json}"
...
#   - `<!-- maestro:generator=`            (any auto-decompose / scanner output)
# Comments carrying ONLY the `<!-- maestro:sig -->` signature footer
...
_has_recent_maintainer_comment() {
	...
	count=$(gh api --method GET --paginate "$api_path" -f since="$cutoff_iso" \
		--jq "[.[] | select(.created_at > \"${cutoff_iso}\")
			...
			| select(.body | contains(\"<!-- maestro:generator=\") | not)
		] | length" \
		2>/dev/null || echo "0")
	[[ "$count" =~ ^[0-9]+$ ]] || count=0
	if [[ "$count" -gt 0 ]]; then
...
_build_decompose_body() {
	...
	cat <<MD
<!-- maestro:generator=auto-decompose parent=${parent_num} -->
...
_create_decompose_issue() {
	...
	# against pulse-wrapper.sh missing export of MAESTRO_HEADLESS=true
