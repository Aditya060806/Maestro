...
# memory-helper.sh - Lightweight memory system for maestro
...
main() {
	...
	insights)
		...
		MAESTRO_MEMORY_DIR="$MEMORY_DIR" "$audit_script" "${insights_args[@]}"
		;;
	export) cmd_export "$@" ;;
	graduate)
		...
		MAESTRO_MEMORY_DIR="$MEMORY_DIR" "$graduate_script" "$@"
		;;
	namespaces)
