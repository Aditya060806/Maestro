...
readonly BRACKET_DIR="${HOME}/.maestro/.agent-workspace/work/seed-brackets"
...
load_credentials() {
	...
	if [[ -z "${HIGGSFIELD_API_KEY:-}" || -z "${HIGGSFIELD_SECRET:-}" ]]; then
		...
		print_info "  ~/.config/maestro/credentials.sh"
		print_info "Or run: maestro secret set HIGGSFIELD_API_KEY"
		return 1
	fi
	return 0
}
