...
_find_explicit_session_id() {
	local db_path="$1"
	local explicit_session_id="${MAESTRO_SIG_SESSION_ID:-${OPENCODE_SESSION_ID:-}}"
...
_allow_pid_session_match() {
	if [[ "${MAESTRO_SIG_ALLOW_PID_SESSION_MATCH:-}" == "1" ]]; then
		return 0
	fi
...
# 1. Explicit session env (MAESTRO_SIG_SESSION_ID / OPENCODE_SESSION_ID)
...
_detect_explicit_session_type() {
	...
	if _gh_sig_env_truthy "${FULL_LOOP_HEADLESS:-}" ||
		_gh_sig_env_truthy "${MAESTRO_HEADLESS:-}" ||
...
# File path: ~/.maestro/.agent-workspace/tmp/{parent_session_id}.children.tsv
#
# Runtime adapters:
...
_child_token_ledger_dir() {
	printf '%s' "${HOME}/.maestro/.agent-workspace/tmp"
	return 0
}
