...
#   gopass path: maestro/leadsforge-api-key
#
...
readonly LEADSFORGE_REPORTS_DIR="${HOME}/.maestro/reports/leadsforge"
...
resolve_api_key() {
	...
	if command -v gopass &>/dev/null; then
		local key
		key=$(gopass show -o "maestro/leadsforge-api-key" 2>/dev/null) || true
		if [[ -n "$key" ]]; then
			echo "$key"
			return 0
		fi
...
api_call() {
	...
	api_key=$(resolve_api_key) || {
		...
		print_info "Set via: maestro secret set LEADSFORGE_API_KEY"
...
cmd_setup() {
	...
	echo "  gopass insert maestro/leadsforge-api-key"
	echo ""
...
Environment:
  LEADSFORGE_API_KEY   API key (or use gopass: maestro/leadsforge-api-key)
