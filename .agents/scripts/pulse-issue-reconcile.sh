...
#   - reconcile_labelless_maestro_issues (t2112 — >100 lines, identity key preserved)
...
: "${REPOS_JSON:=${HOME}/.config/maestro/repos.json}"
...
# Env:    PULSE_PREFETCH_CACHE_FILE (default ~/.maestro/logs/pulse-prefetch-cache.json)
...
_read_cache_issues_for_slug() {
	local slug="$1"
	local cache_file="${PULSE_PREFETCH_CACHE_FILE:-${HOME}/.maestro/logs/pulse-prefetch-cache.json}"
...
#   - no matching stamp file in $HOME/.maestro/.agent-workspace/interactive-claims/
...
_normalize_unassign_stampless_interactive() {
	...
	local stamp_dir="${HOME}/.maestro/.agent-workspace/interactive-claims"
...
normalize_active_issue_assignments() {
	...
	local dedup_helper="${HOME}/.maestro/agents/scripts/dispatch-dedup-helper.sh"
	local now_epoch
	now_epoch=$(date +%s)
...
#######################################
# t2112: backfill labelless maestro-shaped issues.
#
# Scans each pulse:true repo for open issues whose titles match the maestro
# shape (`^tNNN(\.NNN)*: ` or `^GH#NNN: `) but that have ZERO labels in any
# maestro namespace (origin:*, tier:*, status:*). Such issues were created
...
#      sentinel marker `<!-- maestro:labelless-backfill -->`. The comment
...
#######################################
reconcile_labelless_maestro_issues() {
	local repos_json="$REPOS_JSON"
	[[ -f "$repos_json" ]] || return 0

	local issue_sync_helper="${HOME}/.maestro/agents/scripts/issue-sync-helper.sh"
	...
	local sentinel='<!-- maestro:labelless-backfill -->'
...
This issue was created via a bare \`${_raw_cmd}\` call that bypassed the \`${_wrap_cmd}\` wrapper in \`shared-constants.sh\`. The framework's reconcile pass (\`reconcile_labelless_maestro_issues\` in \`pulse-issue-reconcile.sh\`, t2112) has backfilled \`origin:worker\` + \`tier:standard\` as conservative defaults and extracted hashtag labels from the body.
...
This comment is idempotent; the HTML sentinel prevents duplicates on subsequent pulse cycles."
	...
	local external_sentinel='<!-- maestro:labelless-backfill-external -->'
	local external_comment_template
	external_comment_template="${external_sentinel}
Thanks for filing this issue. Because it was created by a contributor outside the maintainer team, the framework's reconcile pass (\`reconcile_labelless_maestro_issues\` in \`pulse-issue-reconcile.sh\`, t2450) has applied \`needs-maintainer-review\` and extracted hashtag labels from the body — but intentionally withheld the \`origin:*\` and \`tier:*\` labels that would otherwise make this issue dispatchable to an automated worker.

**What happens next:** a maintainer will triage this issue and either

- approve it cryptographically with \`sudo maestro approve issue <N> <owner/repo>\` (which clears \`needs-maintainer-review\`), after which the pulse may dispatch a worker, OR
...
This comment is idempotent; the HTML sentinel prevents duplicates on subsequent pulse cycles."
	...
	while IFS= read -r slug; do
		...
		# in the maestro namespaces. Cap at 10 candidates per repo per cycle.
		local candidates
...
#######################################
# Stage 5 action: backfill labels on a labelless maestro-shaped issue.
# (Per-issue body of reconcile_labelless_maestro_issues — no slug loop.)
#
...
- approve it cryptographically with \`sudo maestro approve issue ${issue_num} ${slug}\` (which clears \`needs-maintainer-review\`), after which the pulse may dispatch a worker, OR
- claim a fresh internal task ID via \`claim-task-id.sh\` and file a maintainer-authored follow-up that credits you as reporter.
...
#   5. reconcile_labelless_maestro_issues   (_should_lia + _action_lia_single)
#
...
reconcile_issues_single_pass() {
	...
	local dedup_helper="${HOME}/.maestro/agents/scripts/dispatch-dedup-helper.sh"
	local verify_helper="${HOME}/.maestro/agents/scripts/verify-issue-close-helper.sh"
	local issue_sync_helper="${HOME}/.maestro/agents/scripts/issue-sync-helper.sh"
	[[ -x "$issue_sync_helper" ]] || issue_sync_helper=""
	...
	local _pbf_state_file="${HOME}/.maestro/state/parent-backfill-last-run.epoch"
	local _pbf_interval="${MAESTRO_PARENT_BACKFILL_INTERVAL_SECS:-3600}"
	...
	local _cbb_state_file="${HOME}/.maestro/state/cross-phase-blocked-by-last-run.epoch"
	local _cbb_interval="${MAESTRO_CROSS_PHASE_BLOCKED_BY_INTERVAL_SECS:-3600}"
	...
	while IFS= read -r slug; do
		...
		while IFS='|' read -r issue_num issue_title_b64 labels_csv issue_body_b64; do
			...
			# Stage 5: backfill labelless maestro-shaped issues (per-repo cap)
