...
#
# Environment:
#   MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE  if "1", the file subcommand
#                                               is a no-op (emergency bypass).
#   MAESTRO_CIRCUIT_BREAKER_META_LABELS        override labels applied to
#                                               the meta-issue (comma-separated).
#                                               Default:
#                                                 auto-dispatch,tier:thinking,
#                                                 model:opus-4-7,bug,pulse,
#                                                 framework,circuit-breaker-meta
#   MAESTRO_CIRCUIT_BREAKER_META_TARGET_REPO   force the repo where the
...
#   MAESTRO_CIRCUIT_BREAKER_META_FRAMEWORK_REPO
#                                               framework source repo fallback.
#                                               Default: Aditya060806/Maestro.
#   MAESTRO_CIRCUIT_BREAKER_META_FORENSIC_LINES
#                                               max log lines included in the
#                                               body. Default 50.
#   PULSE_LOG                                   pulse log path (default
#                                               ~/.maestro/logs/pulse.log).
#   DISPATCH_STAGES_TSV                         dispatch stages file (default
#                                               ~/.maestro/logs/dispatch-stages.tsv).
#
# Exit codes:
...
readonly _CB_DEFAULT_FRAMEWORK_REPO='Aditya060806/Maestro'
...
# into application repos that do not contain maestro framework scripts, so the
...
_cb_meta_target_repo() {
	...
	if [[ -n "${MAESTRO_CIRCUIT_BREAKER_META_TARGET_REPO:-}" ]]; then
		printf '%s' "$MAESTRO_CIRCUIT_BREAKER_META_TARGET_REPO"
		return 0
	fi
	...
	printf '%s' "${MAESTRO_CIRCUIT_BREAKER_META_FRAMEWORK_REPO:-$_CB_DEFAULT_FRAMEWORK_REPO}"
	return 0
}
...
_cb_meta_log_slice() {
	...
	local pulse_log="${PULSE_LOG:-${HOME}/.maestro/logs/pulse.log}"
...
_cb_meta_stages_slice() {
	local issue_number="$1"
	local stages_tsv="${DISPATCH_STAGES_TSV:-${HOME}/.maestro/logs/dispatch-stages.tsv}"
...
_cb_meta_body_intro() {
	...
	cat <<EOF
<!-- maestro:generator=circuit-breaker-meta-filer -->
...
- **Plugin init crash** — opencode plugin (`opencode-maestro`) failing during boot due to FD exhaustion, env pollution, or a stale cache. Check the worker's stderr in `~/.maestro/logs/worker-*-stderr.log`.
...
`circuit-breaker-meta-filer.sh` (t3076). To suppress: set `MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE=1` in the pulse environment.
EOF
	return 0
}
...
_cb_meta_body() {
	...
	local max_lines="${MAESTRO_CIRCUIT_BREAKER_META_FORENSIC_LINES:-50}"
...
_Auto-filed by \`circuit-breaker-meta-filer.sh\` (t3076). Set \`MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE=1\` to suppress._"
...
cmd_file() {
	...
	if [[ "${MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE:-0}" == "1" ]]; then
		log_info "[circuit-breaker-meta-filer] disabled via MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE — skip" >&2
		return 0
	fi
	...
	local labels="${MAESTRO_CIRCUIT_BREAKER_META_LABELS:-${_CB_META_DEFAULT_LABELS}}"
...
ENVIRONMENT:
  MAESTRO_CIRCUIT_BREAKER_META_FILE_DISABLE   set to 1 to disable
  MAESTRO_CIRCUIT_BREAKER_META_LABELS         override labels (CSV)
  MAESTRO_CIRCUIT_BREAKER_META_TARGET_REPO    force meta-issue target repo
  MAESTRO_CIRCUIT_BREAKER_META_FRAMEWORK_REPO framework source repo fallback
  MAESTRO_CIRCUIT_BREAKER_META_FORENSIC_LINES max log lines (default 50)
