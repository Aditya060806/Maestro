...
# (Aditya060806, essentials-com, wpallstars, or ~/.config/maestro/badge-orgs.conf).
...
#   - maestro repo itself is never touched by install (loc-badge runs natively there).
...
_resolve_helper() {
	local _name="$1"
	local _deployed="$HOME/.maestro/agents/scripts/${_name}"
...
readonly _DEFAULT_OWNED_ORGS=("Aditya060806" "essentials-com" "wpallstars")

_load_owned_orgs() {
	local _conf="$HOME/.config/maestro/badge-orgs.conf"
	if [[ -f "$_conf" ]]; then
...
_resolve_loc_badge_template() {
	local _candidates=(
		"$HOME/.maestro/agents/templates/workflows/loc-badge-caller.yml"
...
_sync_readme() {
	...
	if ! git -C "$_path" diff --cached --quiet; then
		local _commit_msg="chore: refresh maestro README badges and repo metrics"
...
_install_workflow() {
	...
	if ! git -C "$_path" diff --cached --quiet; then
		local _commit_msg="chore: install repo metrics refresh workflow (via maestro badges sync)"
		if ! git -C "$_path" commit -q -m "$_commit_msg"; then
			printf '%s\t%s\tgit commit failed\n' "$_slug" "$_STATUS_FAILED"
			git -C "$_path" checkout -q "$_default_branch" || true
			return 1
...
_open_pr() {
	...
	local _pr_title="chore: maestro badge sync — ${_description}"
	local _pr_body
	_pr_body=$(printf '%s\n' \
		"## Summary" \
		"" \
		...
		"## Why" \
		"" \
		"The maestro framework provides a canonical badge block for README files" \
		"and dependency-light local repository metrics. This PR brings this repo in line with" \
		"the canonical template." \
		"" \
		"## How to verify" \
		"" \
		"After merge: check README.md contains \`<!-- maestro:badges:start -->\` block." \
		"Check \`docs/metrics/repo-metrics.json\` and \`docs/metrics/badges/*.svg\` exist." \
		"" \
		"Generated by: \`maestro badges sync --apply\` (canonical badge sync).")
...
_process_rows() {
	...
	while IFS=$'\t' read -r _slug _path _status; do
		[[ -z "$_slug" ]] && continue
		# Never touch maestro itself
		[[ "$_slug" == "Aditya060806/Maestro" ]] && continue
...
main() {
	...
	[[ -f "$REPOS_JSON" ]] || _die "repos.json not found at $REPOS_JSON — maestro may not be initialised"
	command -v jq >/dev/null 2>&1 || _die "jq required — install via Homebrew/apt"
	[[ -f "$_CHECK_HELPER" ]] || _die "badges-check-helper.sh not found at $_CHECK_HELPER — run: maestro update"
	[[ -f "$_BADGES_HELPER" ]] || _die "readme-badges-helper.sh not found at $_BADGES_HELPER — run: maestro update"
