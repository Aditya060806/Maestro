...
# Log file: ~/.maestro/logs/task-claim.log (created on first append).
#
...
_append_claim_audit_log() {
	...
	pid="${BASHPID:-$$}"
	sid="${MAESTRO_SESSION_ID:-${Claude_SESSION_ID:-${OPENCODE_SESSION_ID:-unknown}}}"
...
_cas_log_protected_branch_rejection() {
	...
	log_error "Recovery: configure .maestro.json counter_branch to an unprotected counter branch,"
...
_task_counter_status() {
	...
	local detail="${2:-}"
	if [[ -n "$detail" ]]; then
		log_info "MAESTRO_TASK_COUNTER_STATUS=${status} detail=${detail}"
	else
		log_info "MAESTRO_TASK_COUNTER_STATUS=${status}"
	fi
	return 0
}
...
#
# Lock path: ~/.maestro/locks/claim-task-id.<remote>.<branch>.lock/
...
CAS_LOCAL_LOCK_DIR="${CAS_LOCAL_LOCK_DIR:-${HOME}/.maestro/locks}"
...
allocate_offline() {
	...
	(
		...
		GIT_AUTHOR_NAME="maestro" GIT_AUTHOR_EMAIL="maestro@local" \
		GIT_COMMITTER_NAME="maestro" GIT_COMMITTER_EMAIL="maestro@local" \
