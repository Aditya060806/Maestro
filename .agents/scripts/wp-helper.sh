...
# Supports per-tenant configs: ~/.config/maestro/tenants/{tenant}/wordpress-sites.json
# Tenant resolution: project (.maestro-tenant) > active-tenant > "default" > global
...
# Priority: 1) Project override (.maestro-tenant), 2) Global active-tenant, 3) "default"
...
#   1) ~/.config/maestro/tenants/{tenant}/wordpress-sites.json
#   2) ~/.config/maestro/wordpress-sites.json (global fallback)
resolve_config_file() {
	local tenant
...
show_config() {
	...
	if [[ -f "$WP_PROJECT_TENANT_FILE" ]]; then
		echo "  [active] .maestro-tenant = $(tr -d '[:space:]' <"$WP_PROJECT_TENANT_FILE")"
	else
		echo "  [skip]   .maestro-tenant (not found)"
	fi
	if [[ -f "$WP_ACTIVE_TENANT_FILE" ]]; then
...
Configuration (tenant-aware, resolved in priority order):
  1. ~/.config/maestro/tenants/{tenant}/wordpress-sites.json  (per-tenant)
  2. ~/.config/maestro/wordpress-sites.json                   (global fallback)

  Active tenant is resolved from (same chain as credential-helper.sh):
    1. .maestro-tenant  (project-level override)
    2. ~/.config/maestro/active-tenant  (global active)
    3. "default"

  Template: ~/.maestro/agents/configs/wordpress-sites.json.txt

Setup (global):
  mkdir -p ~/.config/maestro
  cp ~/.maestro/agents/configs/wordpress-sites.json.txt ~/.config/maestro/wordpress-sites.json

Setup (per-tenant, e.g. "acme"):
  mkdir -p ~/.config/maestro/tenants/acme
  cp ~/.maestro/agents/configs/wordpress-sites.json.txt \
     ~/.config/maestro/tenants/acme/wordpress-sites.json
