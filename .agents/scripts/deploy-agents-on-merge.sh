...
#
# Options:
#   --repo <path>       Path to the maestro repo (default: ~/Git/maestro)
...
TARGET_DIR="${HOME}/.maestro/agents"
PLUGINS_FILE="${HOME}/.config/maestro/plugins.json"
SCRIPTS_ONLY=false
FULL_DEPLOY=false
DRY_RUN=false
DIFF_COMMIT=""
...
main() {
	...
	if [[ "$FULL_DEPLOY" == "true" ]]; then
		...
		if [[ "$DRY_RUN" == "true" ]]; then
			log_info "[dry-run] Would run: MAESTRO_NON_INTERACTIVE=true $REPO_DIR/setup.sh --non-interactive"
			return 0
		fi
		MAESTRO_NON_INTERACTIVE=true bash "$REPO_DIR/setup.sh" --non-interactive
		local _full_rc=$?
