...
define_prd_commands() {
	create_command "create-prd" \
		...
		"$AGENT_BUILD" "" <<'BODY'
Read ${MAESTRO_DIR:-$HOME/.maestro}/agents/workflows/plans.md and follow its PRD generation instructions.

Feature to document: $ARGUMENTS

**Workflow:**
1. Ask 3-5 clarifying questions with numbered options (1A, 2B format)
2. Generate PRD using template from ${MAESTRO_DIR:-$HOME/.maestro}/agents/templates/prd-template.md
...
BODY
	...
	create_command "generate-tasks" \
		...
		"$AGENT_BUILD" "" <<'BODY'
Read ${MAESTRO_DIR:-$HOME/.maestro}/agents/workflows/plans.md and follow its task generation instructions.
...
- [ ] 0.0 Create safe linked worktree for the feature
  - [ ] 0.1 Create safe linked worktree: `${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add feature/{slug}`
