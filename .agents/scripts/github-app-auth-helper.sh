...
# MAESTRO_GITHUB_APP_ALLOW_TOKEN_STDOUT=1 escape hatch is set for automation.
# =============================================================================
...
: "${MAESTRO_GITHUB_APP_HOME:=${HOME:-$(_github_app_default_home)}}"
: "${MAESTRO_GITHUB_APP_CONFIG:=${MAESTRO_GITHUB_APP_HOME}/.config/maestro/github-app-auth.json}"
: "${MAESTRO_GITHUB_APP_CACHE_DIR:=${MAESTRO_GITHUB_APP_HOME}/.maestro/cache/github-app}"
: "${MAESTRO_GITHUB_APP_RATE_LIMIT_CACHE_TTL:=20}"
: "${MAESTRO_GITHUB_APP_REST_FIRST:=1}"
...
_github_app_config_value() {
	...
	if [[ -f "$MAESTRO_GITHUB_APP_CONFIG" ]] && command -v jq >/dev/null 2>&1; then
		value=$(jq -r --arg key "$key" '.[$key] // empty' "$MAESTRO_GITHUB_APP_CONFIG" 2>/dev/null || true)
...
github_app_app_id() {
	_github_app_config_value "app_id" "MAESTRO_GITHUB_APP_ID" "GITHUB_APP_ID"
	return 0
}

github_app_installation_id() {
	_github_app_config_value "installation_id" "MAESTRO_GITHUB_APP_INSTALLATION_ID" "GITHUB_APP_INSTALLATION_ID"
	return 0
}

github_app_private_key_path() {
	local value=""
	value=$(_github_app_config_value "private_key_path" "MAESTRO_GITHUB_APP_PRIVATE_KEY_PATH" "GITHUB_APP_PRIVATE_KEY_PATH")
	if [[ -z "$value" ]]; then
		local secret_name=""
		secret_name=$(_github_app_config_value "private_key_path_secret" "MAESTRO_GITHUB_APP_PRIVATE_KEY_PATH_SECRET" "GITHUB_APP_PRIVATE_KEY_PATH_SECRET")
		if [[ -n "$secret_name" ]] && command -v gopass >/dev/null 2>&1; then
			value=$(gopass show -o "maestro/${secret_name}" 2>/dev/null || true)
		fi
	fi
	[[ -n "$value" ]] && printf '%s\n' "$value"
	return 0
...
github_app_enabled() {
	local enabled=""
	enabled=$(_github_app_config_value "enabled" "MAESTRO_GITHUB_APP_ENABLED" "GITHUB_APP_ENABLED")
	if [[ -n "$enabled" ]]; then
...
_github_app_cache_dir() {
	local cache_dir="${MAESTRO_GITHUB_APP_CACHE_DIR:-}"
	local app_home="${MAESTRO_GITHUB_APP_HOME:-}"
	local old_umask=""
	if [[ -z "$cache_dir" ]]; then
		return 1
	fi
...
github_app_api_call() {
	...
	context_stage="${MAESTRO_GH_COOLDOWN_STAGE:-github-app-auth}"
	[[ "$context_wrapper" == "_rest_api_call" ]] && context_stage="${MAESTRO_GH_COOLDOWN_STAGE:-rest-fallback}"
	old_token="${GH_TOKEN:-}"; token_was_set="${GH_TOKEN+x}"
	old_auth="${MAESTRO_GH_AUTH_MODE:-}"; auth_was_set="${MAESTRO_GH_AUTH_MODE+x}"
	old_principal="${MAESTRO_GH_AUTH_PRINCIPAL:-}"; principal_was_set="${MAESTRO_GH_AUTH_PRINCIPAL+x}"
	old_pool="${MAESTRO_GH_API_POOL:-}"; pool_was_set="${MAESTRO_GH_API_POOL+x}"
	old_route="${MAESTRO_GH_ROUTE_DECISION:-}"; route_was_set="${MAESTRO_GH_ROUTE_DECISION+x}"
	old_context_operation="${MAESTRO_GH_COOLDOWN_OPERATION:-}"; context_operation_was_set="${MAESTRO_GH_COOLDOWN_OPERATION+x}"
	old_context_wrapper="${MAESTRO_GH_COOLDOWN_WRAPPER:-}"; context_wrapper_was_set="${MAESTRO_GH_COOLDOWN_WRAPPER+x}"
	old_context_stage="${MAESTRO_GH_COOLDOWN_STAGE:-}"; context_stage_was_set="${MAESTRO_GH_COOLDOWN_STAGE+x}"
	MAESTRO_GH_COOLDOWN_OPERATION="$context_operation"
	MAESTRO_GH_COOLDOWN_WRAPPER="$context_wrapper"
	MAESTRO_GH_COOLDOWN_STAGE="$context_stage"
	MAESTRO_GH_API_POOL="$api_pool"
	if [[ -n "$token" ]]; then
		installation_id=$(github_app_installation_id 2>/dev/null || true)
		GH_TOKEN="$token"
		MAESTRO_GH_AUTH_MODE="github-app"
		MAESTRO_GH_AUTH_PRINCIPAL="app-installation:${installation_id:-unknown}"
		MAESTRO_GH_ROUTE_DECISION="${api_pool}-github-app"
	else
		MAESTRO_GH_AUTH_MODE="gh-pat"
		MAESTRO_GH_AUTH_PRINCIPAL="${MAESTRO_GH_AUTH_PRINCIPAL:-unknown}"
		MAESTRO_GH_ROUTE_DECISION="${api_pool}-gh-pat"
	fi
	export GH_TOKEN MAESTRO_GH_AUTH_MODE MAESTRO_GH_AUTH_PRINCIPAL MAESTRO_GH_API_POOL MAESTRO_GH_ROUTE_DECISION MAESTRO_GH_COOLDOWN_OPERATION MAESTRO_GH_COOLDOWN_WRAPPER MAESTRO_GH_COOLDOWN_STAGE
	...
	if [[ -n "$auth_was_set" ]]; then
		MAESTRO_GH_AUTH_MODE="$old_auth"
	else
		unset MAESTRO_GH_AUTH_MODE
	fi
	if [[ -n "$principal_was_set" ]]; then
		MAESTRO_GH_AUTH_PRINCIPAL="$old_principal"
	else
		unset MAESTRO_GH_AUTH_PRINCIPAL
	fi
	if [[ -n "$pool_was_set" ]]; then
		MAESTRO_GH_API_POOL="$old_pool"
	else
		unset MAESTRO_GH_API_POOL
	fi
	if [[ -n "$route_was_set" ]]; then
		MAESTRO_GH_ROUTE_DECISION="$old_route"
	else
		unset MAESTRO_GH_ROUTE_DECISION
	fi
	if [[ -n "$context_operation_was_set" ]]; then
		MAESTRO_GH_COOLDOWN_OPERATION="$old_context_operation"
	else
		unset MAESTRO_GH_COOLDOWN_OPERATION
	fi
	if [[ -n "$context_wrapper_was_set" ]]; then
		MAESTRO_GH_COOLDOWN_WRAPPER="$old_context_wrapper"
	else
		unset MAESTRO_GH_COOLDOWN_WRAPPER
	fi
	if [[ -n "$context_stage_was_set" ]]; then
		MAESTRO_GH_COOLDOWN_STAGE="$old_context_stage"
	else
		unset MAESTRO_GH_COOLDOWN_STAGE
	fi
	return "$rc"
}
...
github_app_rate_limit_json() {
	...
	if [[ -f "$cache_file" ]]; then
		cached_ts=$(jq -r '._maestro_cached_at // 0' "$cache_file" 2>/dev/null || printf '0')
		if [[ "$cached_ts" =~ ^[0-9]+$ && $((now - cached_ts)) -le "$MAESTRO_GITHUB_APP_RATE_LIMIT_CACHE_TTL" ]]; then
			cat "$cache_file"
			return 0
		fi
	fi
	...
	printf '%s\n' "$response" | jq --arg auth_mode "$auth_mode" --arg principal "$principal" --argjson cached_at "$now" \
		'. + {_maestro_auth_mode:$auth_mode, _maestro_principal:$principal, _maestro_cached_at:$cached_at}' >"$tmp" || return 1
	chmod 600 "$tmp" 2>/dev/null || true
	mv "$tmp" "$cache_file"
	cat "$cache_file"
	return 0
...
github_app_should_route_rest() {
	...
	local remaining=""
	[[ "${MAESTRO_GITHUB_APP_DISABLE_ROUTING:-0}" == "1" ]] && return 1
	case "$api_pool" in
	rest-core | rest-search) ;;
	*) return 1 ;;
	esac
	if github_app_is_configured; then
		if _github_app_bool_true "${MAESTRO_GITHUB_APP_REST_FIRST:-1}"; then
			return 0
		fi
		remaining=$(gh api rate_limit --jq '.resources.graphql.remaining' 2>/dev/null || true)
		if [[ "$remaining" =~ ^[0-9]+$ && "$remaining" -le "${MAESTRO_GH_REST_FALLBACK_THRESHOLD:-3000}" ]]; then
			return 0
		fi
	fi
...
github_app_status_json() {
	...
	auth_mode=$(printf '%s\n' "$rate_json" | jq -r '._maestro_auth_mode // "gh-pat"' 2>/dev/null || printf 'gh-pat')
...
Subcommands:
  ...
  token [--repo owner/repo]               Print token only when MAESTRO_GITHUB_APP_ALLOW_TOKEN_STDOUT=1
  help                                    Show this help
...
~/.config/maestro/github-app-auth.json or set MAESTRO_GITHUB_APP_* env vars.
EOF
	return 0
}
...
_github_app_cli() {
	...
	rate-limit)
		...
		;;
	clear-cache)
		rm -f "${MAESTRO_GITHUB_APP_CACHE_DIR}"/token-*.json "${MAESTRO_GITHUB_APP_CACHE_DIR}"/rate-limit-*.json 2>/dev/null || true
		printf 'GitHub App auth cache cleared\n'
		return 0
		;;
	token)
		if [[ "${MAESTRO_GITHUB_APP_ALLOW_TOKEN_STDOUT:-0}" != "1" ]]; then
			print_error "Refusing to print installation token without MAESTRO_GITHUB_APP_ALLOW_TOKEN_STDOUT=1"
			return 2
		fi
