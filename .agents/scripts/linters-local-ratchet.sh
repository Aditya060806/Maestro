...
# _ratchet_count_hardcoded_path: count literal ~/.maestro or /Users/ in scripts
# Returns: count via stdout
_ratchet_count_hardcoded_path() {
	local scripts_dir="$1"
	local count=0
	# Tilde is intentional: we search for the literal string ~/.maestro in scripts
	# shellcheck disable=SC2088
	count=$(rg '~/.maestro|/Users/' --type sh "$scripts_dir" 2>/dev/null |
		grep -v '^\s*#' |
		grep -cv '# ') || count=0
	[[ "$count" =~ ^[0-9]+$ ]] || count=0
	echo "$count"
...
_ratchet_count_all() {
	...
	count_hardcoded=$(_ratchet_count_with_progress "hardcoded_maestro_path" "_ratchet_count_hardcoded_path" "$scripts_dir") || return 1
	...
	new_json=$(jq -n \
		...
		'{
			...
			ratchets: {
				...
				hardcoded_maestro_path: {
					count: $hardcoded,
					description: "Literal ~/.maestro or /Users/ instead of \${HOME}/.maestro or variable",
					pattern: "~/.maestro|/Users/"
				},
				broad_catch_or_true: {
					count: $broad,
...
_ratchet_load_baselines() {
	...
	baseline_hardcoded=$(jq -r '.ratchets.hardcoded_maestro_path.count // 0' "$baseline_file" 2>/dev/null) || baseline_hardcoded=0
...
_ratchet_load_all_exceptions() {
	...
	exc_hardcoded=$(_ratchet_load_exceptions "${exceptions_dir}/hardcoded_maestro_path.txt")
...
_ratchet_run_checks() {
	...
	_ratchet_check_pattern "hardcoded_maestro_path" "$count_hardcoded" "$baseline_hardcoded" "$exc_hardcoded" "$strict_mode" || ratchet_failures=$((ratchet_failures + 1))
