...
# Configuration
readonly MEMORY_BASE_DIR="${MAESTRO_MEMORY_DIR:-$HOME/.maestro/.agent-workspace/memory}"
...
#######################################
# Get OpenAI API key from maestro secret store
...
get_openai_key() {
	...
	# Check maestro secret store (gopass)
	if command -v gopass &>/dev/null; then
		local key
		key=$(gopass show -o "maestro/openai-api-key" 2>/dev/null || echo "")
		if [[ -n "$key" ]]; then
			echo "$key"
			return 0
		fi
