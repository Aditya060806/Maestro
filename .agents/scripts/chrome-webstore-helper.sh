...
load_credentials() {
	...
	# Try to load from maestro secret storage
	if command -v maestro &>/dev/null; then
		for key in "${REQUIRED_CREDENTIALS[@]}"; do
			if [[ -z "${!key:-}" ]]; then
				local value
				value=$(maestro secret get "$key" 2>/dev/null || echo "")
				if [[ -n "$value" ]]; then
					export "$key=$value"
				fi
			fi
...
cmd_setup() {
	...
	if command -v maestro &>/dev/null; then
		print_info "Using maestro secret storage (encrypted)"
		echo "$CWS_CLIENT_ID" | maestro secret set CWS_CLIENT_ID
		echo "$CWS_CLIENT_SECRET" | maestro secret set CWS_CLIENT_SECRET
		echo "$CWS_REFRESH_TOKEN" | maestro secret set CWS_REFRESH_TOKEN
		echo "$CWS_EXTENSION_ID" | maestro secret set CWS_EXTENSION_ID
		echo "$CWS_PUBLISHER_ID" | maestro secret set CWS_PUBLISHER_ID
	else
		print_warning "maestro not found, saving to .env file (plaintext)"
...
Credentials can be stored via:
  - maestro secret storage (encrypted, recommended)
