...
#
# Reads ~/.maestro/logs/gh-audit.log (or the path in GH_AUDIT_LOG_FILE),
# extracts entries with non-empty suspicious[] arrays, and opens a GitHub
# issue on Aditya060806/Maestro when anomalies are found.
#
# State is persisted in ~/.maestro/logs/gh-audit-scanner.state (JSON):
...
#   GH_AUDIT_SCANNER_REPO     Repo slug for filed issues (default: Aditya060806/Maestro)
...
readonly GH_ANOMALY_STATE_FILE_DEFAULT="${HOME}/.maestro/logs/gh-audit-scanner.state"
readonly GH_ANOMALY_LOG_DIR_DEFAULT="${HOME}/.maestro/logs"
readonly GH_ANOMALY_LOG_FILENAME="gh-audit.log"
readonly GH_ANOMALY_DEFAULT_REPO="Aditya060806/Maestro"
...
1. Review each entry in \`~/.maestro/logs/gh-audit.log\`
...
Options:
  ...
  --repo SLUG  Override the repo to file issues in (default: Aditya060806/Maestro)
...
State: ~/.maestro/logs/gh-audit-scanner.state
Log:   ~/.maestro/logs/gh-audit.log
Docs:  reference/gh-audit-log.md
HELP
	return 0
}
