...
: "${PULSE_MERGE_CHECKPOINT_FILE:=${HOME}/.maestro/logs/pulse-merge-checkpoint}"
...
_pmp_refresh_unknown_mergeable_state_into() {
	...
	if [[ "$_pmp_refreshed_mergeable" == "UNKNOWN" || -z "$_pmp_refreshed_mergeable" ]]; then
		_pmp_refreshed_mergeable=$(MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1 gh_pr_view "$_pmp_pr_number" --repo "$_pmp_repo_slug" \
...
_pmp_refresh_unknown_review_decision_into() {
	...
	if [[ "$_pmp_refreshed_review" == "UNKNOWN" ]]; then
		_pmp_refreshed_review=$(MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1 gh_pr_view "$_pmp_pr_number" --repo "$_pmp_repo_slug" \
...
merge_ready_prs_all_repos() {
	...
	local _mr_stop_flag="${STOP_FLAG:-${HOME}/.maestro/logs/pulse-session.stop}"
	local _mr_repos_json="${REPOS_JSON:-${HOME}/.config/maestro/repos.json}"
	local _mr_logfile="${LOGFILE:-${HOME}/.maestro/logs/pulse.log}"
	local _mr_checkpoint_file="${PULSE_MERGE_CHECKPOINT_FILE:-${HOME}/.maestro/logs/pulse-merge-checkpoint}"
...
_merge_ready_prs_for_repo() {
	...
	local MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR=""
	local MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR=""
	MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/maestro-pulse-required-contexts.XXXXXX" 2>/dev/null) || MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR=""
	MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/maestro-pulse-author-perms.XXXXXX" 2>/dev/null) || MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR=""
	if [[ -z "$MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR" || -z "$MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR" ]]; then
	...
	[[ -n "$MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR" ]] && rm -rf -- "$MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR"
	[[ -n "$MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR" ]] && rm -rf -- "$MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR"
...
_resolve_pr_mergeable_status() {
	...
	if [[ "$pr_mergeable" == "UNKNOWN" || -z "$pr_mergeable" ]]; then
		...
		_retry_output=$(MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1 gh_pr_view "$pr_number" --repo "$repo_slug" \
			--json mergeable --jq '.mergeable // ""')
		_retry_exit=$?
...
_attempt_existing_auto_merge_behind_update_branch() {
	...
	local _pr_state=""
	_pr_state=$(MAESTRO_GH_PR_VIEW_CACHE_DISABLE=1 gh_pr_view "$pr_number" --repo "$repo_slug" \
...
# cryptographic approval (sudo maestro approve issue N).
#
# Config: MAESTRO_TRUSTED_AUTHORS_CONF env var (override) or
...
_is_trusted_issue_author() {
	local login="$1"
	[[ -z "$login" ]] && return 1
	local _trusted_conf="${MAESTRO_TRUSTED_AUTHORS_CONF:-${_PULSE_MERGE_DIR:+${_PULSE_MERGE_DIR}/../configs/trusted-issue-authors.conf}}"
...
# per-issue cryptographic approvals for every maintainer-run maestro worker.
#
...
_issue_author_has_maintainer_authority() {
	...
	if [[ -z "$permission" ]]; then
		#maestro:trust-boundary GH#24958: ambiguous GitHub issue
...
_issue_has_verified_crypto_approval() {
	...
	local approval_helper="${AGENTS_DIR:-$HOME/.maestro/agents}/scripts/approval-helper.sh"
...
#
# Feature flag: MAESTRO_WORKER_BRIEFED_AUTO_MERGE (default: 1=on, 0=off)
...
_attempt_worker_briefed_auto_merge() {
	...
	if [[ "${MAESTRO_WORKER_BRIEFED_AUTO_MERGE:-1}" == "0" ]]; then
		echo "[pulse-merge] worker-briefed auto-merge: disabled by MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0 for PR #${pr_number} in ${repo_slug} (t2449)" >>"$LOGFILE"
		return 1
	fi
	...
	#maestro:trust-boundary — reuse PR-author permission only for the same non-empty issue-author login.
	...
	# via cryptographic approval (sudo maestro approve issue N), NOT via
...
_required_contexts_for_default_branch() {
	local repo_slug="$1"
	local cache_dir="${MAESTRO_PULSE_REQUIRED_CONTEXTS_CACHE_DIR:-}"
...
_repo_allows_auto_merge() {
	local repo_slug="$1"
	local cache_dir="${TMPDIR:-/tmp}/maestro-pulse-allow-auto-merge-$$"
	local cache_key
...
# MAESTRO_PULSE_AUTO_MERGE_STUCK_SECONDS.
#
...
_auto_merge_stuck_seconds() {
	...
	local repo_slug="$2"
	local pr_state="$3"
	local threshold="${MAESTRO_PULSE_AUTO_MERGE_STUCK_SECONDS:-300}"
...
_set_native_auto_merge_or_skip() {
	...
	if [[ -n "$_existing_auto" ]]; then
		...
		if _stuck_seconds=$(_auto_merge_stuck_seconds "$pr_number" "$repo_slug" "$_pr_state"); then
			local _threshold="${MAESTRO_PULSE_AUTO_MERGE_STUCK_SECONDS:-300}"
