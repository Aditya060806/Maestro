...
readonly COLLECTOR_DATA_DIR="${HOME}/.maestro/.agent-workspace/work/code-audit"
...
get_sonar_token() {
	...
	if command -v gopass >/dev/null 2>&1; then
		token=$(gopass show -o maestro/SONAR_TOKEN 2>/dev/null || echo "")
	fi

	# Fallback to credentials.sh
	if [[ -z "$token" ]] && [[ -f "${HOME}/.config/maestro/credentials.sh" ]]; then
	...
	if [[ -z "$token" ]]; then
		log_error "SONAR_TOKEN not found. Set via: maestro secret set SONAR_TOKEN"
		return 1
	fi
...
Environment:
  SONAR_TOKEN         SonarCloud API token (required)
                      Set via: maestro secret set SONAR_TOKEN
