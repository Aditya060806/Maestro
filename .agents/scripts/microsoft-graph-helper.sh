...
# Credentials: maestro secret set MSGRAPH_CLIENT_ID / MSGRAPH_CLIENT_SECRET / MSGRAPH_TENANT_ID
#
# Part of maestro email system (t1526)
...
readonly TOKEN_CACHE_DIR="${HOME}/.maestro/.agent-workspace/microsoft-graph"
...
load_credentials() {
	...
	if command -v gopass &>/dev/null; then
		...
		client_id=$(gopass show -o "maestro/MSGRAPH_CLIENT_ID" 2>/dev/null || echo "")
		tenant_id=$(gopass show -o "maestro/MSGRAPH_TENANT_ID" 2>/dev/null || echo "")
		client_secret=$(gopass show -o "maestro/MSGRAPH_CLIENT_SECRET" 2>/dev/null || echo "")
	...
	if [[ -z "${MSGRAPH_CLIENT_ID:-}" || -z "${MSGRAPH_TENANT_ID:-}" ]]; then
		...
		print_info "Set via: maestro secret set MSGRAPH_CLIENT_ID && maestro secret set MSGRAPH_TENANT_ID"
		print_info "For app-only flow also set: maestro secret set MSGRAPH_CLIENT_SECRET"
		return 1
	fi
	return 0
}
...
auth_client_credentials() {
	...
	if [[ -z "${MSGRAPH_CLIENT_SECRET:-}" ]]; then
		...
		print_info "Set via: maestro secret set MSGRAPH_CLIENT_SECRET"
		return 1
	fi
...
cmd_status() {
	...
	if command -v gopass &>/dev/null; then
		...
		client_id_set=$(gopass show -o "maestro/MSGRAPH_CLIENT_ID" 2>/dev/null && echo "set" || echo "not set")
		tenant_id_set=$(gopass show -o "maestro/MSGRAPH_TENANT_ID" 2>/dev/null && echo "set" || echo "not set")
		client_secret_set=$(gopass show -o "maestro/MSGRAPH_CLIENT_SECRET" 2>/dev/null && echo "set" || echo "not set")
...
SETUP:
  ...
  4. Set credentials:
       maestro secret set MSGRAPH_CLIENT_ID
       maestro secret set MSGRAPH_TENANT_ID
       maestro secret set MSGRAPH_CLIENT_SECRET  # app-only flow only
