...
# Writes ~/.maestro/cache/session-greeting-{runtime}.txt with a normalised
# line 1 in the format:
#   maestro vX running in {display_name} vY | {repo_slug}
#
# Also writes to the shared ~/.maestro/cache/session-greeting.txt for
...
#
# Environment:
#   MAESTRO_GREETING_REPO_SLUG — override repo slug (default: from git remote)
...
readonly CACHE_DIR="${HOME}/.maestro/cache"
...
_detect_repo_slug() {
	if [[ -n "${MAESTRO_GREETING_REPO_SLUG:-}" ]]; then
		echo "${MAESTRO_GREETING_REPO_SLUG}"
		return 0
	fi
	local slug
...
write_greeting_cache() {
	...
	maestro_version=$(_read_maestro_version)

	local repo_slug
	repo_slug=$(_detect_repo_slug)

	# Normalised line 1 format (as specified in issue #20599 / t2737 Phase A)
	local greeting_line_1="maestro v${maestro_version} running in ${display_name} v${runtime_version} | ${repo_slug}"
...
Commands:
  write <runtime-id>   Write greeting cache for the given runtime ID.
                       Creates ~/.maestro/cache/session-greeting-<id>.txt
                       and updates ~/.maestro/cache/session-greeting.txt.
...
Environment:
  MAESTRO_GREETING_REPO_SLUG  Override detected repo slug in line 1.
EOF
	return 0
}
