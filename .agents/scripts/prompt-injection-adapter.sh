...
# adapter that knows how to inject the maestro system prompt (AGENTS.md,
...
readonly _PIA_AGENTS_MD="${HOME}/.maestro/agents/AGENTS.md"
readonly _PIA_REFERENCE_LINE='Add ~/.maestro/agents/AGENTS.md to context for Maestro capabilities.'
# Grep pattern to detect existing maestro reference in files (literal string, not a path)
readonly _PIA_GREP_PATTERN='maestro/agents/AGENTS.md'
...
#   $3 - grep pattern to detect existing reference (optional, defaults to 'maestro')
_pia_ensure_reference_in_file() {
	local target_file="$1"
	local ref_line="$2"
	local grep_pattern="${3:-maestro}"
	local target_dir
	...
	if [[ -f "$target_file" ]]; then
		...
		_pia_log "success" "Created $target_file with maestro reference"
	fi
	return 0
}
...
_deploy_prompt_json_instructions() {
	...
	if command -v jq &>/dev/null; then
		...
		if echo "$current" | grep -q "maestro"; then
	...
	for template_source in \
		...
		"${HOME}/.maestro/templates/opencode-config-agents.md"; do
...
_deploy_prompt_codex_instructions() {
	...
	local ref_line='Read ~/.maestro/agents/AGENTS.md for Maestro framework capabilities and rules.'
	...
	_pia_ensure_reference_in_file "$instructions_file" "$ref_line" "maestro"
	return 0
}
...
_deploy_prompt_aider() {
	...
	if grep -q "maestro" "$aider_config" 2>/dev/null; then
		_pia_log "info" "Aider config already references maestro"
		return 0
	fi
...
show_prompt_deployment_status() {
	...
	for i in "${!_PIA_RUNTIME_IDS[@]}"; do
		...
		json-instructions)
			if [[ -f "${HOME}/.config/opencode/opencode.json" ]]; then
				if command -v jq &>/dev/null; then
					...
					if echo "$instr" | grep -q "maestro"; then
						deployed="yes"
					fi
				fi
			fi
		...
		agents-md-autodiscovery)
			case "$rid" in
			claude-code)
				if grep -q "maestro" "${HOME}/.claude/AGENTS.md" 2>/dev/null ||
					grep -q "maestro" "${HOME}/.config/Claude/AGENTS.md" 2>/dev/null; then
					deployed="yes"
				fi
				;;
			*)
				if grep -q "maestro" "${HOME}/.${rid}/AGENTS.md" 2>/dev/null; then
					deployed="yes"
				fi
				;;
			esac
			;;
		codex-instructions-md)
			if grep -q "maestro" "${HOME}/.codex/instructions.md" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		cursorrules-plus-agents)
			if grep -q "maestro" "${HOME}/.cursor/rules/AGENTS.md" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		factory-skills)
			if grep -q "maestro" "${HOME}/.factory/skills/AGENTS.md" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		gemini-agents-md)
			if grep -q "maestro" "${HOME}/.gemini/AGENTS.md" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		windsurfrules)
			if grep -q "maestro" "${HOME}/.codeium/windsurf/AGENTS.md" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		continue-rules)
			if grep -q "maestro" "${HOME}/.continue/AGENTS.md" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		aider-read)
			if grep -q "maestro" "${HOME}/.aider.conf.yml" 2>/dev/null; then
				deployed="yes"
			fi
			;;
		esac
...
_pia_usage() {
	...
	echo ""
	echo "Deploy maestro system prompts via each runtime's native mechanism."
	echo ""
	echo "Commands:"
