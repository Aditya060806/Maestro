...
#       Reads ~/.config/maestro/repos.json for repo metadata (foss flag,
...
#       by the markers <!-- maestro:badges:start --> / <!-- ...:end -->.
...
#       if drift detected. Used by Phase 2's `maestro badges check`.
#
# The template lives at:
#   .agents/templates/readme/badges.md.tmpl   (in the maestro repo)
#   ~/.maestro/agents/templates/readme/badges.md.tmpl   (deployed)
#
...
MARKER_START='<!-- maestro:badges:start -->'
MARKER_END='<!-- maestro:badges:end -->'
MARKER_NOTICE='<!-- managed by maestro badges; edit the template, not this block -->'

# Resolve template location: prefer in-repo (during framework dev), fall
# back to the deployed copy under ~/.maestro/agents/.
default_template_path() {
	local _script_dir
	_script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
	local _candidates=(
		"$_script_dir/../templates/readme/badges.md.tmpl"
		"$HOME/.maestro/agents/templates/readme/badges.md.tmpl"
	)
	local _candidate
