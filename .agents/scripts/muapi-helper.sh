...
load_api_key() {
	...
	if [[ -z "${MUAPI_API_KEY:-}" ]] && command -v gopass &>/dev/null; then
		MUAPI_API_KEY="$(gopass show -o "maestro/MUAPI_API_KEY" 2>/dev/null)" || true
	fi

	if [[ -z "${MUAPI_API_KEY:-}" ]]; then
		print_error "MUAPI_API_KEY not set"
		print_info "Run: maestro secret set MUAPI_API_KEY"
...
Environment:
  MUAPI_API_KEY  API key (get from https://muapi.ai/access-keys)
                 Store via: maestro secret set MUAPI_API_KEY
EOF
	return 0
}
