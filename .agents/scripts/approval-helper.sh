...
#   sudo maestro approve setup          # One-time: generate approval key pair
#   sudo maestro approve issue <number> [owner/repo] # Approve an issue for development
#   sudo maestro approve pr <number>    # Approve a PR for merge
#   maestro approve verify <number>     # Verify approval on an issue (no sudo)
#   maestro approve status              # Show approval key setup status
#
# Security model:
#   - Private signing key stored root-only (~/.maestro/approval-keys/private/)
...
# missed in the sourcing sweep (t2408 / GH#19997). Under `sudo maestro
...
readonly APPROVAL_DIR="$_APPROVAL_HOME/.maestro/approval-keys"
...
readonly APPROVAL_NAMESPACE="maestro-approve"
readonly APPROVAL_MARKER="<!-- maestro-signed-approval -->"
...
_detect_slug() {
	...
	if [[ -z "$slug" || "$slug" != *"/"* ]]; then
		local repos_json="$_APPROVAL_HOME/.config/maestro/repos.json"
...
_require_gh_auth() {
	...
	_print_info "Check 'gh auth status' as your user, then retry sudo maestro approve. If sudo strips auth, pass GH_TOKEN via sudo --preserve-env=GH_TOKEN."
	return 1
}
...
_approval_private_key_path() {
	...
	printf '%s' "$real_home/.maestro/approval-keys/private/approval.key"
	return 0
}
...
_require_approval_key() {
	...
	if [[ ! -f "$actual_key" ]]; then
		_print_error "No approval key found. Run: sudo maestro approve setup"
		return 1
	fi
...
_validate_approval_target_kind() {
	...
	issue_json=$(_approval_fetch_issue_json "$target_number" "$slug") || {
		...
		if [[ "$target_type" == "pr" ]]; then
			_print_info "If this is an issue, use: sudo maestro approve issue ${target_number} ${slug}"
		else
			_print_info "If this is a PR, use: sudo maestro approve pr ${target_number} ${slug}"
		fi
		return 1
	}
	...
	if [[ "$target_type" == "pr" ]]; then
		if ! printf '%s' "$issue_json" | jq -e 'has("pull_request")' >/dev/null 2>&1; then
			...
			_print_info "Use: sudo maestro approve issue ${target_number} ${slug}"
			return 1
		fi
		return 0
	fi
	...
	if printf '%s' "$issue_json" | jq -e 'has("pull_request")' >/dev/null 2>&1; then
		...
		_print_info "Use: sudo maestro approve pr ${target_number} ${slug}"
		return 1
	fi
...
_approval_apply_issue_lifecycle_updates() {
	...
	if [[ "$_ah_labels_json" == *"status:in-review"* ]]; then
		local _ah_helper=""
		if [[ -x "${HOME}/.maestro/agents/scripts/interactive-session-helper.sh" ]]; then
			_ah_helper="${HOME}/.maestro/agents/scripts/interactive-session-helper.sh"
		else
...
#
#   1. Marker file (~/.maestro/cache/pulse-merge-trigger.txt) — append a
...
#   $3 - slug (owner/repo)
#
# Bypass: MAESTRO_SKIP_APPROVE_KICK_PULSE=1 disables both layers (used by
...
_kick_pulse_after_approval() {
	...
	if [[ "${MAESTRO_SKIP_APPROVE_KICK_PULSE:-0}" == "1" ]]; then
		return 0
	fi
	...
	local trigger_file="${_APPROVAL_HOME}/.maestro/cache/pulse-merge-trigger.txt"
	...
	local pulse_wrapper="${_APPROVAL_HOME}/.maestro/agents/scripts/pulse-wrapper.sh"
	if [[ ! -x "$pulse_wrapper" ]]; then
		return 0
	fi

	local kick_log="${_APPROVAL_HOME}/.maestro/logs/pulse-approve-kick.log"
...
_approve_target() {
	...
	local target_number="${2:-}"
	local slug="${3:-}"
	local usage="Usage: sudo maestro approve ${target_type} <number> [owner/repo]"
	local slug_error="Could not detect repo slug. Provide it: sudo maestro approve ${target_type} ${target_number} owner/repo"
...
cmd_setup() {
	...
	if [[ "$(id -u)" -ne 0 ]]; then
		_print_error "This command must be run with sudo"
		echo "Usage: sudo maestro approve setup"
		return 1
	fi
	...
	local real_home
	real_home=$(_resolve_real_home)
	local actual_approval_dir="$real_home/.maestro/approval-keys"
	...
	if [[ -f "$actual_key" ]]; then
		...
		ssh-keygen -t ed25519 -C "maestro-approval-signing" \
			-f "$actual_key" -N "" -q
	...
	echo "  sudo maestro approve issue <number> <owner/repo>"
	echo "  sudo maestro approve pr <number> <owner/repo>"
	return 0
}
...
cmd_verify() {
	...
	_require_number_arg "$issue_number" "Issue" "Usage: maestro approve verify <number> [owner/repo]" || return 1
	...
	local pub_key="${MAESTRO_APPROVAL_PUB:-$APPROVAL_PUB}"
	if [[ ! -f "$pub_key" ]]; then
		echo "NO_KEY"
		return 1
	fi
...
cmd_status() {
	...
	if [[ -f "$APPROVAL_PUB" ]]; then
		...
		echo "  Run: sudo maestro approve setup"
	fi
...
cmd_help() {
	...
	echo ""
	echo "Examples:"
	echo "  sudo maestro approve setup"
	echo "  sudo maestro approve issue 17438 <owner/repo>"
	echo "  maestro approve verify 17438"
	echo ""
