...
<!-- maestro:generator=function-complexity-gate cited_file=${file_path} threshold=${COMPLEXITY_FUNC_LINE_THRESHOLD:-100} -->
...
REQUEUE_BODY_EOF
	...
	created_number=$(gh_create_issue --repo "$maestro_slug" \
		--title "$issue_title" \
...
#
# Arguments: $1 - repo_path, $2 - state_file, $3 - maestro_slug
...
#   $2 - file_path       (repo-relative)
#   $3 - maestro_slug   (owner/repo for issue creation)
...
_simplification_backfill_verify_remaining_smells() {
	...
	requeue_result=$(_create_requeue_issue "$maestro_slug" "$file_path" "$remaining_smells" "$new_passes" "$issue_num") || true
...
#   $3 - maestro_slug   (owner/repo for gh issue queries)
# Outputs:
...
_simplification_state_backfill_closed() {
	...
	local closed_issues
	closed_issues=$(gh_issue_list --repo "$maestro_slug" \
	...
	while IFS= read -r issue; do
		...
		_simplification_backfill_verify_remaining_smells \
			"$repo_path" "$file_path" "$maestro_slug" "$new_passes" "$issue_num"
...
# Arguments:
#   $1 - repo_path      (absolute worktree path)
#   $2 - maestro_slug  (owner/repo for issue queries/closures)
# Outputs:
...
_simplification_close_spurious_requeue_issues() {
	...
	local issues_json
	issues_json=$(gh_issue_list --repo "$maestro_slug" \
...
\`${file_path}\` currently has zero Qlty smells. This re-queue issue was generated by the pre-PR-#18848 \`grep -c\` bug in \`_simplification_backfill_verify_remaining_smells\` (\`pulse-simplification-state.sh\`) which produced a literal \`0\\n0\` string for files Qlty reported clean and then failed the \`-eq 0\` test. The bug is fixed in commit 625c6da5e (PR #18848); this sweep cleans up stragglers and self-heals any future regression of the same class."
		...
		if gh issue close "$num" --repo "$maestro_slug" \
			--reason "not planned" \
