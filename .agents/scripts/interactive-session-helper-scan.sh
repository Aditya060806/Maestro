...
_isc_scan_stampless_phase() {
	...
	local repos_json_1a="${HOME}/.config/maestro/repos.json"
...
_isc_scan_dead_stamps_phase() {
		...
		for stamp in "$CLAIM_STAMP_DIR"/*.json; do
			...
			if [[ $pid_alive -eq 0 ]]; then
				if [[ "$auto_release_flag" == "1" ]]; then
					...
					printf '    release:  maestro issue release %s\n' "$issue"
					printf '\n'
	...
	if [[ "$auto_release_flag" == "1" ]]; then
		...
		if [[ $stale_count -eq 0 ]]; then
			...
			printf 'Total: %d stale claim(s). Release via `maestro issue release <N>` or reclaim by `cd`-ing into the worktree.\n' "$stale_count"
		fi
	fi
	return 0
}
...
#
# Env: MAESTRO_SCAN_STALE_AUTO_RELEASE=0|1 — overrides runtime detection.
#
# Auto-release detection priority (t3205):
#   1. Explicit --auto-release / --no-auto-release flag
#   2. MAESTRO_SCAN_STALE_AUTO_RELEASE env var (0 or 1)
#   3. TTY (stdin AND stdout are terminals) → ON
#   4. Headless markers (FULL_LOOP_HEADLESS / MAESTRO_HEADLESS /
...
_isc_cmd_scan_stale() {
	...
	if [[ -z "$auto_release_flag" && -n "${MAESTRO_SCAN_STALE_AUTO_RELEASE:-}" ]]; then
		auto_release_flag="${MAESTRO_SCAN_STALE_AUTO_RELEASE}"
	fi
	if [[ -z "$auto_release_flag" ]]; then
		if [[ -t 0 && -t 1 ]]; then
			# Human-driven TTY session.
			auto_release_flag=1
		elif [[ -n "${FULL_LOOP_HEADLESS:-}${MAESTRO_HEADLESS:-}${OPENCODE_HEADLESS:-}${GITHUB_ACTIONS:-}" ]]; then
