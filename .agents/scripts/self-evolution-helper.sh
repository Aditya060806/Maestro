...
# self-evolution-helper.sh - Self-evolution loop for maestro
...
readonly EVOL_MEMORY_BASE_DIR="${MAESTRO_MEMORY_DIR:-$HOME/.maestro/.agent-workspace/memory}"
...
readonly EVOL_STATE_DIR="${HOME}/.maestro/logs"
...
CREATE-TODO OPTIONS:
    --repo-path <path>  Repository path for TODO creation (default: ~/Git/maestro)
...
SELF-EVOLUTION LOOP:
    ...
    5. The TODO enters the normal maestro task lifecycle (dispatch, PR, merge)
...
EXAMPLES:
    ...
    self-evolution-helper.sh create-todo gap_xxx --repo-path ~/Git/maestro
