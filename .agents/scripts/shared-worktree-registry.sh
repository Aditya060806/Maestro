...
if [[ -z "$_WORKTREE_REGISTRY_HOME" ]]; then
	...
	_WORKTREE_REGISTRY_HOME="${_WORKTREE_REGISTRY_TMPDIR}/maestro-${_WORKTREE_REGISTRY_UID}"
	if ! _worktree_registry_ensure_dir "$_WORKTREE_REGISTRY_HOME"; then
		_WORKTREE_REGISTRY_HOME="${_WORKTREE_REGISTRY_TMPDIR}/maestro-${_WORKTREE_REGISTRY_UID}-$$"
		if ! _worktree_registry_ensure_dir "$_WORKTREE_REGISTRY_HOME"; then
			_WORKTREE_REGISTRY_HOME="$(mktemp -d "${_WORKTREE_REGISTRY_TMPDIR}/maestro-${_WORKTREE_REGISTRY_UID}.XXXXXXXXXX")" || _WORKTREE_REGISTRY_HOME=""
		fi
	fi
...
WORKTREE_REGISTRY_DIR="${WORKTREE_REGISTRY_DIR:-${_WORKTREE_REGISTRY_HOME}/.maestro/.agent-workspace}"
