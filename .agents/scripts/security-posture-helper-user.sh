...
check_prompt_guard_patterns() {
	...
	local deployed_sha_file="${HOME}/.maestro/.deployed-sha"
...
check_secret_storage() {
	...
	if command -v gopass &>/dev/null; then
		...
		CHECK_FIX="Run: maestro secret init"
		return 1
	fi
	...
	CHECK_FIX="Run: maestro secret init (gopass) or create ~/.config/maestro/credentials.sh"
	return 1
}
...
cmd_startup_check() {
	...
	echo "Security: ${actions_needed} action${plural} needed — run \`maestro security setup\` for details"
	return 1
}
...
cmd_status() {
	...
	echo -e "${YELLOW}${actions_needed} action${plural} needed.${NC} Run: maestro security setup"
	return 1
}
...
_setup_prompt_guard() {
	...
	echo ""
	local response
	read -r -p "    Run maestro update now? [Y/n] " response
	response="${response:-y}"
	if [[ "$response" =~ ^[Yy]$ ]]; then
		echo ""
		if command -v maestro &>/dev/null; then
			...
			bash "$HOME/Git/maestro/setup.sh" --non-interactive
		fi
...
_setup_secret_storage() {
	...
	if command -v gopass &>/dev/null; then
		...
		echo "    Options:"
		echo "      1. Install gopass (recommended): brew install gopass && maestro secret init"
