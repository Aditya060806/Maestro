...
readonly SEO_DATA_DIR="$HOME/.maestro/.agent-workspace/work/seo-data"
...
get_auth_header() {
    ...
    if [[ -z "${DATAFORSEO_USERNAME:-}" ]] || [[ -z "${DATAFORSEO_PASSWORD:-}" ]]; then
        ...
        print_error "Set DATAFORSEO_USERNAME and DATAFORSEO_PASSWORD in ~/.config/maestro/credentials.sh"
        return 1
    fi
...
Output:
    ~/.maestro/.agent-workspace/work/seo-data/{domain}/dataforseo-{start}-{end}.toon
...
Requirements:
    - DATAFORSEO_USERNAME and DATAFORSEO_PASSWORD in ~/.config/maestro/credentials.sh
