...
# Configuration
readonly MAIL_DIR="${MAESTRO_MAIL_DIR:-$HOME/.maestro/.agent-workspace/mail}"
...
readonly MAIL_TRANSPORT="${MAESTRO_MAIL_TRANSPORT:-local}"
readonly SIMPLEX_HELPER="${SCRIPT_DIR}/simplex-helper.sh"
readonly SIMPLEX_MAIL_CONTACT="${MAESTRO_SIMPLEX_MAIL_CONTACT:-}"
readonly SIMPLEX_MAIL_GROUP="${MAESTRO_SIMPLEX_MAIL_GROUP:-#maestro-mail}"
readonly MATRIX_MAIL_ROOM="${MAESTRO_MATRIX_MAIL_ROOM:-}"
readonly MATRIX_BOT_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/maestro/matrix-bot.json"
# Envelope prefix for structured messages over chat transports
readonly MAIL_ENVELOPE_PREFIX="[MAESTRO-MAIL]"
...
#######################################
get_agent_id() {
	if [[ -n "${MAESTRO_AGENT_ID:-}" ]]; then
		echo "$MAESTRO_AGENT_ID"
		return 0
	fi
	local branch
	branch=$(git branch --show-current 2>/dev/null || echo "unknown")
	local worktree_name
	worktree_name=$(basename "$(pwd)" | sed 's/^maestro[.-]//')
	if [[ "$worktree_name" != "maestro" && "$worktree_name" != "." ]]; then
...
Environment:
  MAESTRO_AGENT_ID              Override auto-detected agent identity
  MAESTRO_MAIL_DIR              Override mail directory location
  MAESTRO_MAIL_TRANSPORT        Default transport (local|simplex|matrix|all)
  MAESTRO_SIMPLEX_MAIL_GROUP    SimpleX group for mail relay (default: #maestro-mail)
  MAESTRO_SIMPLEX_MAIL_CONTACT  SimpleX contact for mail relay (fallback)
  MAESTRO_MATRIX_MAIL_ROOM      Matrix room ID for mail relay
