...
REPOS_JSON="${PULSE_CHECK_REPOS_JSON:-${HOME}/.config/maestro/repos.json}"
...
_existing_open_issue_for_finding() {
	local slug="$1"
	local finding_id="$2"
	local marker="maestro:generator=pulse-check finding=${finding_id}"
...
_finding_issue_body() {
	...
	cat <<EOF
<!-- maestro:generator=pulse-check finding=${finding_id} -->
