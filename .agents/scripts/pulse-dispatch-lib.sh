...
_dispatch_begin_benign_blocks_cycle() {
	...
	if [[ -n "${MAESTRO_PULSE_BENIGN_BLOCKS_FILE:-}" ]]; then
		ledger_file="$MAESTRO_PULSE_BENIGN_BLOCKS_FILE"
	else
		ledger_managed_by_dispatch="1"
		if ! mkdir -p "${HOME}/.maestro/logs"; then
			printf 'Failed to create benign block ledger directory: %s\n' "${HOME}/.maestro/logs" >&2
		fi
		ledger_file=$(mktemp "${HOME}/.maestro/logs/pulse-dispatch-benign-blocks.XXXXXX" || printf '%s\n' "${HOME}/.maestro/logs/pulse-dispatch-benign-blocks.$$.$RANDOM")
	fi
...
_dispatch_provider_pressure_points() {
	...
	if [[ "$provider_backoff_active" == "1" || "$recent_rate_limits" -gt 0 || -f "${PULSE_RATE_LIMIT_FLAG:-${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag}" ]]; then
		printf '6\n'
		return 0
	fi
	printf '0\n'
...
_dispatch_ramp_now() {
	if [[ "${MAESTRO_PULSE_DISPATCH_RAMP_NOW:-}" =~ ^[0-9]+$ ]]; then
		printf '%s' "$MAESTRO_PULSE_DISPATCH_RAMP_NOW"
		return 0
	fi
	date +%s
	return 0
...
_dispatch_ramp_cooldown_expires_at() {
	local expires="${1:-}"
	local file="${MAESTRO_GH_SECONDARY_COOLDOWN_FILE:-${HOME}/.maestro/cache/gh-secondary-cooldown.json}"
	if [[ "$expires" =~ ^[0-9]+$ ]]; then
		printf '%s' "$expires"
		return 0
	fi
...
_dispatch_ramp_phase_start() {
	...
	local boot_ts="${1:-}"
	local expires="${2:-}"
	local boot_secs="${MAESTRO_PULSE_DISPATCH_RAMP_BOOT_SECS:-${MAESTRO_GH_READ_RAMP_BOOT_SECS:-180}}"
	local recovery_secs="${MAESTRO_PULSE_DISPATCH_RAMP_RECOVERY_SECS:-${MAESTRO_GH_READ_RAMP_RECOVERY_SECS:-300}}"
	if [[ "${MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH:-}" =~ ^[0-9]+$ ]]; then
		printf '%s %s\n' "${MAESTRO_PULSE_DISPATCH_RAMP_PHASE:-manual}" "$MAESTRO_PULSE_DISPATCH_RAMP_START_EPOCH"
		return 0
	fi
...
_dispatch_apply_startup_capacity_ramp() {
	...
	local slot_secs="${MAESTRO_PULSE_DISPATCH_RAMP_SLOT_SECS:-120}"
	local boot_ts="${3:-}"
	local expires="${4:-}"
	local now=""
	local phase_line=""
	...
	local elapsed=0
	local ramp_cap=1
	[[ "${MAESTRO_PULSE_DISPATCH_RAMP_ENABLED:-1}" == "1" ]] || {
		printf '%s\n' "$max_workers"
		return 0
	}
...
_dispatch_with_timeout() {
	...
	raw_rc_file=$(mktemp 2>/dev/null || printf '/tmp/maestro-dispatch-raw-rc.%s.%s' "$$" "$issue_number")
...
#   1. MAESTRO_OPUS_CONCURRENCY_CAP env var
...
_dispatch_check_model_concurrency_cap() {
	...
	*) return 0 ;;
	...
	local opus_cap="${MAESTRO_OPUS_CONCURRENCY_CAP:-${OPUS_CONCURRENCY_CAP}}"
