...
LLM_COSTS_PATH="${LLM_COSTS_PATH:-${HOME}/.maestro/.agent-workspace/llm-costs.json}"
...
_CONFIG_SEARCH_PATHS=(
	...
	"${HOME}/.maestro/_config/llm-routing.json"
...
_resolve_costs_path() {
	...
	if [[ -n "$costs_path" ]]; then
		...
		costs_path="${HOME}/.maestro/.agent-workspace/llm-costs.json"
	fi
	printf '%s' "$costs_path"
	return 0
}
