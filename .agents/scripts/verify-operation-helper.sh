...
readonly VERIFY_DIR="${HOME}/.maestro/.agent-workspace/observability"
readonly VERIFY_LOG="${VERIFY_DIR}/verifications.jsonl"
readonly VERIFY_CONFIG_DIR="${HOME}/.config/maestro"
...
_check_provider_available() {
	...
	case "$provider" in
	anthropic) _has_api_key "ANTHROPIC_API_KEY" "maestro/anthropic-api-key" ;;
	google) _has_api_key "GOOGLE_API_KEY" "maestro/google-api-key" ;;
	openai) _has_api_key "OPENAI_API_KEY" "maestro/openai-api-key" ;;
	*) return 1 ;;
	esac
}
...
_cmd_verify_check_skip() {
	...
	# Check global skip
	if [[ "${MAESTRO_SKIP_VERIFY:-}" == "1" ]]; then
		log_warn "Verification skipped (MAESTRO_SKIP_VERIFY=1)"
		_log_verification "${op_type:-unknown}" "${risk_tier:-unknown}" "unknown" "none" "none" \
			"skipped" "0" "" "true" "env:MAESTRO_SKIP_VERIFY" "$session_id" "$repo" "$branch"
		echo "SKIPPED"
		return 1
	fi
...
Environment:
  MAESTRO_SKIP_VERIFY=1    Skip all verification (not recommended)
