...
emit_deploy_phase() {
	...
	! is_maestro_repo && {
		print_info "Not maestro repo, skipping deploy"
		return 0
	}
...
_linked_issue_structural_blocker_reasons() {
	...
	local dedup_out
	dedup_out=$("$dedup_helper" enumerate-blockers "$issue_num" "$repo" "${MAESTRO_SESSION_USER:-${USER:-}}" 2>/dev/null || true)
	local _blocker_line
...
cmd_complete() {
	...
	is_maestro_repo && printf " | Deploy: done"
	printf "\n\n"
	rm -f "$STATE_FILE"
