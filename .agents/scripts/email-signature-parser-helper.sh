...
# Email Signature Parser Helper for Maestro Framework
...
${sig_block}"
	...
	if [[ -z "$result" ]]; then
		...
		if command -v gopass &>/dev/null; then
			api_key=$(gopass show -o "maestro/anthropic-api-key" 2>/dev/null) || true
		fi
		# Try credentials file
		if [[ -z "$api_key" && -f "${HOME}/.config/maestro/credentials.sh" ]]; then
			api_key=$(grep -E '^ANTHROPIC_API_KEY=' "${HOME}/.config/maestro/credentials.sh" 2>/dev/null | cut -d= -f2- | tr -d '"'"'" || true)
		fi
		# Try environment
		if [[ -z "$api_key" ]]; then
...
show_help() {
	cat <<'HELP'
Email Signature Parser Helper - Maestro Framework
