...
- **Local LLM only** — `maestro campaign` commands that process intel
...
This directory is managed by the maestro campaigns plane.
See `.agents/maestro/campaigns-plane.md` for the full contract.
...
```bash
maestro campaign init        # Provision this plane (idempotent)
maestro campaign provision   # Re-provision / repair
maestro campaign status      # Show provisioning state + counts
maestro campaign ls          # List active + launched campaigns
maestro campaign ls --active # List active campaigns only

# Asset binary management (P4):
maestro campaign asset add <file> [--target lib-brand|lib-swipe|campaign] [--campaign <id>]
maestro campaign asset list [--type image|video|audio|pdf|all]
maestro campaign asset preview <file> [--size 640]
maestro campaign asset manifest <asset-id>
```

Managed by: maestro campaigns-provision-helper.sh (t2962), campaign-asset-helper.sh (t2965)
CONTRACT
	return 0
}
...
cmd_init() {
	...
	echo "  4. View status:      maestro campaign status"
	return 0
}
...
cmd_status() {
	...
	if ! _is_provisioned "$repo_path"; then
		print_warning "Not provisioned. Run: maestro campaign init"
		return 0
	fi
...
cmd_ls() {
	...
	if ! _is_provisioned "$repo_path"; then
		print_warning "_campaigns/ not provisioned. Run: maestro campaign init"
		return 0
	fi
...
See: .agents/maestro/campaigns-plane.md for full contract.
