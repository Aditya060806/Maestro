...
# Maestro Framework - Version Consistency Validator
...
# Check version comment in a shell script (setup.sh, maestro.sh)
...
# Check homebrew/maestro.rb against the referenced released tag, not VERSION.
...
_check_homebrew_formula() {
	local repo_root="$1"
	if [[ ! -f "$repo_root/homebrew/maestro.rb" ]]; then
		return 0
	fi
	local formula_version
	local formula_tag
	local formula_sha
	formula_version=$(grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+\.tar\.gz' "$repo_root/homebrew/maestro.rb" | head -1 | sed 's/^v//; s/\.tar\.gz$//' || echo "")
	formula_sha=$(grep -o 'sha256 "[^"]\+"' "$repo_root/homebrew/maestro.rb" | head -1 | cut -d'"' -f2 || echo "")

	if [[ -z "$formula_version" ]]; then
		print_error "homebrew/maestro.rb version URL not found"
		_vc_errors=$((_vc_errors + 1))
		return 0
	fi
	if [[ -z "$formula_sha" ]]; then
		print_error "homebrew/maestro.rb sha256 not found"
		_vc_errors=$((_vc_errors + 1))
		return 0
	fi
	...
	if git rev-parse -q --verify "refs/tags/${formula_tag}" >/dev/null 2>&1 || git ls-remote -q --exit-code --tags origin "refs/tags/${formula_tag}" >/dev/null 2>&1; then
		local release_sha
		release_sha=$(fetch_with_retry "https://github.com/Aditya060806/Maestro/archive/refs/tags/${formula_tag}.tar.gz" | compute_sha256 | cut -d' ' -f1)
		if [[ -z "$release_sha" ]]; then
			print_error "homebrew/maestro.rb checksum lookup failed for ${formula_tag}"
			_vc_errors=$((_vc_errors + 1))
		elif [[ "$formula_sha" == "$release_sha" ]]; then
			print_success "homebrew/maestro.rb: ${formula_tag} checksum verified"
		else
			print_error "homebrew/maestro.rb checksum mismatch for ${formula_tag}"
			_vc_errors=$((_vc_errors + 1))
		fi
	else
		print_error "homebrew/maestro.rb references unreleased tag ${formula_tag}; leave formula pinned to the latest published release"
		_vc_errors=$((_vc_errors + 1))
	fi
	return 0
}
...
# Check config/build file version references (sonar, setup.sh, maestro.sh, package.json, homebrew, marketplace)
# Arguments: expected_version, repo_root
# Outputs: increments _vc_errors or _vc_warnings on mismatch
_check_config_files() {
	local expected_version="$1"
	...
	_check_shell_script_version "$expected_version" "$repo_root/maestro.sh" "maestro.sh"
