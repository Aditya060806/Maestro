...
cmd_add() {
	...
	if [[ "$provider" == "openai" ]]; then
		local openai_add_mode="${MAESTRO_OPENAI_ADD_MODE:-device}"
		...
		device)
			...
			;;
		callback)
			print_info "Using callback URL flow for OpenAI (MAESTRO_OPENAI_ADD_MODE=callback)"
			;;
		*)
			print_error "Invalid MAESTRO_OPENAI_ADD_MODE: ${openai_add_mode} (valid: device, callback)"
			return 1
			;;
		esac
	fi
...
print(json.dumps({
	...
	_oauth_exchange_code "$GOOGLE_TOKEN_ENDPOINT" "application/json" "maestro/1.0" "$token_body"
	return 0
}
