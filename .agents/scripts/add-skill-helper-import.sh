...
_handle_conflicts() {
	...
	if echo "$blocking_conflicts" | grep -q "^NATIVE:"; then
		log_warning "Conflicts with native maestro subagent(s):"
...
_fetch_url_content() {
	...
	http_code=$(curl -sS -L --connect-timeout 15 --max-time 60 \
		...
		-H "User-Agent: maestro-skill-importer/1.0" \
		"$url" 2>/dev/null) || true
...
# Convert source files to maestro format and copy to target location.
