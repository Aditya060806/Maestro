...
#   Global:  ~/.maestro/.agent-workspace/datasets/   (cross-project)
...
readonly GLOBAL_DATASETS_DIR="${HOME}/.maestro/.agent-workspace/datasets"
readonly OBS_METRICS="${HOME}/.maestro/.agent-workspace/observability/metrics.jsonl"
	...
	cat <<'SCHEMA'
{
  ...
  "title": "Maestro Evaluation Dataset Entry",
...
Directory convention:
  Global:  ~/.maestro/.agent-workspace/datasets/
  Project: <repo>/datasets/

Examples:
  dataset-helper.sh create golden-prompts
  dataset-helper.sh add ~/.maestro/.agent-workspace/datasets/golden-prompts.jsonl \
    --input "What is the capital of France?" --expected "Paris" --tags "geography,factual"
  dataset-helper.sh validate ~/.maestro/.agent-workspace/datasets/golden-prompts.jsonl
  dataset-helper.sh stats ~/.maestro/.agent-workspace/datasets/golden-prompts.jsonl
