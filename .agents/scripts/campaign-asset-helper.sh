...
#   pdf     PDF
#
# Blob threshold: files >=30MB stored at ~/.maestro/.agent-workspace/knowledge-blobs/
...
_require_campaigns_plane() {
	...
	if [[ ! -d "$campaigns_dir" ]]; then
		...
		print_error "Run 'maestro campaign init' first."
		return 1
	fi
	return 0
}
...
_store_asset_blob() {
	...
	local asset_id="$2"
	local repo_name="$3"
	local blob_dir="${HOME}/.maestro/.agent-workspace/knowledge-blobs/${repo_name}/campaigns/${asset_id}"
	mkdir -p "$blob_dir"
	local blob_path
...
BLOB THRESHOLD
  Files >= 30MB are stored in ~/.maestro/.agent-workspace/knowledge-blobs/
