...
DEFAULT_OPEN_DESIGN_DIR="${HOME}/.maestro/peripherals/open-design"
...
Commands:
  ...
  skills                         Print maestro ingestion recommendation summary
...
Notes:
  Open Design is optional. maestro remains self-contained and keeps .agents/
...
cmd_status() {
	...
	if command_exists localdev-helper.sh; then
		...
		warn "localdev-helper.sh not found on PATH; use ~/.maestro/agents/scripts/localdev-helper.sh if deployed"
	fi
...
cmd_start() {
	...
	if [[ -n "$https_name" ]]; then
		if command_exists localdev-helper.sh; then
			...
			(cd "$resolved_dir" && "${HOME}/.maestro/agents/scripts/localdev-helper.sh" run --name "$https_name" corepack pnpm tools-dev run web)
		fi
		return $?
	fi
...
Adapt with maestro verification/tooling:
...
cmd_route() {
	...
	*deck*|*slides*|*PPT*|*presentation*|*keynote*)
		printf 'Route: Open Design deck skill candidate, then maestro export/fidelity verification.\n'
		;;
	*email*|*newsletter*)
		printf 'Route: Open Design email-marketing candidate, then email-design-test-helper.sh verification.\n'
		;;
	*mobile*|*app*|*onboarding*)
		printf 'Route: Open Design mobile prototype candidate, then maestro UI verification.\n'
		;;
	...
	*production*|*implement*|*code*)
		printf 'Route: maestro native implementation; use Open Design only for preview exploration.\n'
		;;
	*)
		printf 'Route: start with maestro DESIGN.md/design-agent workflow; use Open Design if live artifact preview/export is valuable.\n'
		;;
	esac
	return 0
}
