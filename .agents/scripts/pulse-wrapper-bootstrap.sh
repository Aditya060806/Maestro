...
# (com.maestro.maestro-supervisor-merge, 60s interval) to invoke the full
...
#
# Execution:
#   1. Acquire ~/.maestro/locks/pulse-merge-instance.lock (mkdir-atomic).
...
#
# Lock:  ~/.maestro/locks/pulse-merge-instance.lock
# Log:   ~/.maestro/logs/pulse-merge.log
# Stamp: ~/.maestro/logs/pulse-merge-routine-last-run
#
# Exit code: always 0 (non-fatal; merge failures are logged and skipped)
# ---------------------------------------------------------------------------
_pulse_run_merge_only() {
	local _mo_lockdir="${HOME}/.maestro/locks/pulse-merge-instance.lock"
	local _mo_log="${HOME}/.maestro/logs/pulse-merge.log"
	local _mo_last_run="${HOME}/.maestro/logs/pulse-merge-routine-last-run"

	mkdir -p "${HOME}/.maestro/locks" "$(dirname "$_mo_log")" 2>/dev/null || true
	...
	local _mo_saved_logfile="${LOGFILE:-${HOME}/.maestro/logs/pulse.log}"
	LOGFILE="$_mo_log"
...
#
# Drains ~/.maestro/cache/pulse-merge-trigger.txt — the marker file written
...
#
# Bypass: MAESTRO_SKIP_TRIGGER_DRAIN=1 (used by tests for negative cases).
#
# Exit code: always 0 (drain is best-effort).
# ---------------------------------------------------------------------------
_drain_merge_trigger_file_if_present() {
	if [[ "${MAESTRO_SKIP_TRIGGER_DRAIN:-0}" == "1" ]]; then
		return 0
	fi

	local trigger_file="${PULSE_MERGE_TRIGGER_FILE:-${HOME}/.maestro/cache/pulse-merge-trigger.txt}"
	...
	if ! declare -F process_pr >/dev/null 2>&1; then
		printf '[%s] _drain_merge_trigger_file_if_present: process_pr not defined (pulse-merge.sh not sourced?) — restoring marker\n' \
			...
			>>"${LOGFILE:-${HOME}/.maestro/logs/pulse.log}" 2>/dev/null || true
...
#
# Sources:
#   lifecycle-helper  MAESTRO_PULSE_SOURCE=lifecycle-helper (set by _start())
...
_detect_invocation_source() {
	...
	if [[ "${MAESTRO_PULSE_SOURCE:-}" == "lifecycle-helper" ]]; then
...
_record_invocation_source() {
	...
	local log_dest="${LOGFILE:-${HOME}/.maestro/logs/pulse.log}"
