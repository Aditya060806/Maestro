...
# Arguments: $1 - now_epoch, $2 - maestro_slug, $3 - window_seconds
...
_complexity_recent_debt_closures() {
	...
	recent_closures=$(gh api graphql \
		-f query="query { search(query:\"repo:${maestro_slug} label:function-complexity-debt is:issue is:closed closed:>${since_date}\", type:ISSUE) { issueCount } }" \
...
# Arguments: $1 - now_epoch, $2 - maestro_slug
...
_complexity_llm_sweep_due() {
	...
	current_count=$(gh api graphql \
		-f query="query { repository(owner:\"${maestro_slug%%/*}\", name:\"${maestro_slug##*/}\") { issues(labels:[\"function-complexity-debt\"], states:OPEN) { totalCount } } }" \
	...
	local recent_closures
	recent_closures=$(_complexity_recent_debt_closures "$now_epoch" "$maestro_slug" "$sweep_interval")
	[[ "$recent_closures" =~ ^[0-9]+$ ]] || recent_closures="0"
	...
	dispatched_count=$(gh_issue_list --repo "$maestro_slug" \
	...
	actionable_count=$(gh_issue_list --repo "$maestro_slug" \
		--label "function-complexity-debt" --state open \
...
# Arguments: $1 - maestro_slug, $2 - now_epoch, $3 - maintainer
# Returns: 0 always (best-effort)
_complexity_run_llm_sweep() {
	local maestro_slug="$1"
	local now_epoch="$2"
	local maintainer="$3"
	...
	local sweep_exists
	sweep_exists=$(gh_issue_list --repo "$maestro_slug" \
		--label "function-complexity-debt" --state open \
...
**To dismiss**, comment \`dismissed: <reason>\` on this issue."
	...
	if gh_create_issue --repo "$maestro_slug" \
...
# Resolve the maestro repo path and validate lint-file-discovery.sh exists.
# Arguments: $1 - repos_json path, $2 - maestro_slug, $3 - now_epoch
# Outputs: maestro_path via stdout (empty on failure)
...
_complexity_scan_find_repo() {
	...
	if [[ -f "$repos_json" ]] && command -v jq &>/dev/null; then
		maestro_path=$(jq -r --arg slug "$maestro_slug" \
			'.initialized_repos[] | select(.slug == $slug) | .path' \
			"$repos_json" 2>/dev/null | head -n 1)
	fi
	if [[ -z "$maestro_path" || "$maestro_path" == "null" || ! -d "$maestro_path" ]]; then
		echo "[pulse-wrapper] Complexity scan: maestro repo path not found — skipping" >>"$LOGFILE"
		echo "$now_epoch" >"$COMPLEXITY_SCAN_LAST_RUN"
		return 1
	fi
	local lint_discovery="${maestro_path}/.agents/scripts/lint-file-discovery.sh"
...
# Arguments: $1 - maestro_path, $2 - now_epoch
...
_complexity_scan_collect_violations() {
	...
	local now_epoch="$2"
	local shell_files
	shell_files=$(git -C "$maestro_path" ls-files '*.sh' | grep -Ev '_archive/' || true)
	...
	while IFS= read -r file; do
		[[ -n "$file" ]] || continue
		local full_path="${maestro_path}/${file}"
...
# Arguments: $1 - maestro_slug, $2 - cap (default 100), $3 - log_prefix
...
_complexity_scan_check_open_cap() {
	...
	local total_open
	total_open=$(gh api graphql -f query="query { repository(owner:\"${maestro_slug%%/*}\", name:\"${maestro_slug##*/}\") { issues(labels:[\"function-complexity-debt\"], states:OPEN) { totalCount } } }" \
...
_complexity_scan_permission_gate() {
	...
	if [[ -n "$current_user" ]]; then
		local perm_level
		# #maestro:trust-boundary — simplification issue creation requires
		# confirmed admin access; permission lookup failures skip explicitly.
		if ! _gh_collaborator_permission_lookup "$maestro_slug" "$current_user" perm_level; then
			echo "[pulse-wrapper] Complexity scan: skipped — permission check failed for user '$current_user' on $maestro_slug (HTTP ${MAESTRO_GH_COLLAB_PERMISSION_HTTP:-unknown})" >>"$LOGFILE"
			return 1
		fi
		case "$perm_level" in
		admin) ;; # allowed — repo owner/admin only
		*)
			echo "[pulse-wrapper] Complexity scan: skipped — user '$current_user' has '$perm_level' permission on $maestro_slug (need admin)" >>"$LOGFILE"
			return 1
			;;
		esac
	fi
	...
	maintainer_from_config=$(jq -r --arg slug "$maestro_slug" \
		'.initialized_repos[] | select(.slug == $slug) | .maintainer // empty' \
		"$REPOS_JSON" 2>/dev/null)
	[[ -z "$maintainer_from_config" ]] && maintainer_from_config=$(printf '%s' "$maestro_slug" | cut -d/ -f1)
...
# _complexity_scan_pull_latest — pull maestro repo before scanning (GH#17848, t2001)
...
_complexity_scan_pull_latest() {
	...
	if git -C "$maestro_path" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
		if ! GIT_TERMINAL_PROMPT=0 timeout_sec 30 \
			git -C "$maestro_path" pull --ff-only --no-rebase >>"$LOGFILE" 2>&1; then
			echo "[pulse-wrapper] Complexity scan: git pull failed for ${maestro_path} — skipping this cycle to avoid stale-state warnings" >>"$LOGFILE"
			return 1
		fi
	fi
	return 0
