...
#
# Reads $PID_FILE (default ~/.maestro/logs/pulse.pid). If it shows
...
#
# Inputs (env):
#   PID_FILE       default ~/.maestro/logs/pulse.pid
...
runtime_audit_check() {
	local pid_file="${PID_FILE:-${HOME}/.maestro/logs/pulse.pid}"
...
2. Inspect what was in flight when SETUP started: \`grep -A 20 "SETUP" ~/.maestro/logs/pulse-wrapper.log | tail -30\`.
3. Clear the marker: \`echo > ~/.maestro/logs/pulse.pid\` (or remove the file).
...
\`\`\`
cat ~/.maestro/logs/pulse.pid
# Should show a single integer (the running pulse PID), no "SETUP:" prefix.
~/.maestro/agents/scripts/pulse-lifecycle-helper.sh status
# Should report "running"
\`\`\`

<!-- maestro:generator=runtime-audit detector=idle-state-stuck -->
MARKDOWN
)
