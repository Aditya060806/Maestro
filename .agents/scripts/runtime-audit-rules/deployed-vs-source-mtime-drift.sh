...
# ~/.maestro/agents/scripts/<f> to the source at ~/Git/maestro/.agents/
...
#
# Inputs (env):
#   MAESTRO_DEPLOYED_DIR  default ~/.maestro/agents/scripts
#   MAESTRO_SOURCE_DIR    default ~/Git/maestro/.agents/scripts
...
runtime_audit_check() {
	local deployed_dir="${MAESTRO_DEPLOYED_DIR:-${HOME}/.maestro/agents/scripts}"
	local source_dir="${MAESTRO_SOURCE_DIR:-${HOME}/Git/maestro/.agents/scripts}"
...
This is the canonical t2036 footgun — debugging a runtime symptom by reading source instead of the deployed copy. When source is committed but the deploy never ran (or the user skipped \`maestro update\`), the running pulse keeps the old behaviour while the operator believes the new fix is live.

## How

1. Compare the diff: \`diff <(cat ${deployed_dir}/<file>) <(cat ${source_dir}/<file>)\` for each drifted file.
2. If the source-side change is desired live: run \`maestro update\` (or \`setup.sh --non-interactive\` from the source repo).
...
\`\`\`
~/.maestro/agents/scripts/runtime-health-audit-helper.sh --dry-run --only deployed-vs-source-mtime-drift
\`\`\`

<!-- maestro:generator=runtime-audit detector=deployed-vs-source-mtime-drift -->
MARKDOWN
)
