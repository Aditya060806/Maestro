...
2. Identify the launchd/systemd path: \`launchctl list | grep maestro\` (macOS) or \`systemctl list-units --user | grep maestro\` (Linux).
3. Read \`pulse-lifecycle-helper.sh\` for the reap path. Look for orphaned PIDs in \`~/.maestro/logs/pulse.pid\` and \`~/.maestro/cache/pulse-state/\`.
...
<!-- maestro:generator=runtime-audit detector=process-count-anomaly -->
MARKDOWN
)
