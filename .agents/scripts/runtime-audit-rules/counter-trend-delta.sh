...
#
# Reads $STATS_FILE (default ~/.maestro/logs/pulse-stats.json) and computes
...
#   STATS_FILE        path to pulse-stats.json (default ~/.maestro/logs/pulse-stats.json)
...
runtime_audit_check() {
	local stats_file="${STATS_FILE:-${HOME}/.maestro/logs/pulse-stats.json}"
...
2. Cross-reference with \`~/.maestro/logs/pulse-wrapper.log\` for the same window using \`grep\` on the counter name or related script names.
...
\`\`\`
~/.maestro/agents/scripts/runtime-health-audit-helper.sh --dry-run
\`\`\`
The regression block for the cited counters should no longer appear.

<!-- maestro:generator=runtime-audit detector=counter-trend-delta -->
MARKDOWN
	local body
	body=$(cat "$_body_tmp")
	rm -f "$_body_tmp"
