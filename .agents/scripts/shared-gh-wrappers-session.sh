...
#   MAESTRO_SESSION_ORIGIN=worker — canonical worker-origin override
#   MAESTRO_HEADLESS=true    — headless-runtime-helper.sh
...
detect_session_origin() {
	# t1984: Explicit override via MAESTRO_SESSION_ORIGIN takes precedence
	...
	case "${MAESTRO_SESSION_ORIGIN:-}" in
	interactive)
		echo "interactive"
		return 0
		;;
...
_gh_wrapper_auto_sig() {
	...
	if [[ $body_file_idx -ge 0 && -n "$body_file_val" && -f "$body_file_val" ]]; then
		...
		if grep -q '<!-- maestro:sig -->' "$abs_body_file_val" 2>/dev/null; then
		...
		signed_body_file=$(mktemp "${TMPDIR:-/tmp}/maestro-gh-body.XXXXXX") || return 0
