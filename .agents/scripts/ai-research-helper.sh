...
#
# Environment:
#   MAESTRO_AI_RESEARCH_PROVIDER — auto (default), anthropic, or opencode
#   MAESTRO_AI_RESEARCH_OPENCODE_MODEL — OpenCode model for default runtime path
...
provider_gopass_secret() {
	local provider="$1"
	case "$provider" in
	anthropic) printf 'maestro/anthropic-api-key\n' ;;
	*) return 1 ;;
	esac
	return 0
}
...
call_opencode() {
	...
	local run_dir="${MAESTRO_AI_RESEARCH_OPENCODE_DIR:-/tmp/opencode}"
	model_id="${MAESTRO_AI_RESEARCH_OPENCODE_MODEL:-}"
	if [[ -z "$model_id" ]]; then
	...
	if [[ -n "$timeout_cmd" ]]; then
		raw=$(cd "$run_dir" && MAESTRO_HEADLESS=1 $timeout_cmd opencode run --format json -m "$model_id" "$wrapped_prompt" 2>&1) || {
			log_error "OpenCode AI research call failed"
			return 1
		}
	else
		raw=$(cd "$run_dir" && MAESTRO_HEADLESS=1 opencode run --format json -m "$model_id" "$wrapped_prompt" 2>&1) || {
			log_error "OpenCode AI research call failed"
			return 1
		}
	fi
...
main() {
	...
	local provider="${MAESTRO_AI_RESEARCH_PROVIDER:-auto}"
