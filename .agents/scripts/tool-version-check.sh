...
# See: https://github.com/Aditya060806/Maestro/issues/6763
...
_classify_tool_status() {
	...
	if [[ "$cmd" == "gh" && "$installed" != "not installed" ]] && ! maestro_gh_slurp_supported; then
		class_status="minimum_required"
		class_icon="!"
		class_color="$RED"
		effective_latest=">=${MAESTRO_GH_MIN_SLURP_VERSION}"
		((++OUTDATED_COUNT))
...
_print_tool_status() {
	...
	unknown)
		...
		;;
	minimum_required)
		echo -e "${color}${icon}  $name: $installed (requires >= ${MAESTRO_GH_MIN_SLURP_VERSION} for gh api --paginate --slurp)${NC}"
		;;
	up_to_date)
...
_output_summary_and_updates() {
	...
	if [[ $OUTDATED_COUNT -gt 0 ]]; then
		...
		for update_cmd in "${OUTDATED_PACKAGES[@]}"; do
			...
			# interactive password prompt during unattended `maestro update`.
