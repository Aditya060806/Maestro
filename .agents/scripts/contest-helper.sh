...
SUPERVISOR_DIR="${MAESTRO_SUPERVISOR_DIR:-$HOME/.maestro/.agent-workspace/supervisor}"
...
SCORING_DB="${HOME}/.maestro/.agent-workspace/response-scoring.db"
...
ensure_contest_tables() {
	if [[ ! -f "$SUPERVISOR_DB" ]]; then
		...
		log_error "Supervisor DB not found at $SUPERVISOR_DB — run 'maestro pulse start' to initialize"
		return 1
	fi
