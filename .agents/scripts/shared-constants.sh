...
# Shared Constants for Maestro Framework Provider Scripts
...
#   - MAESTRO_BASH_REEXECED=1 is set before exec to prevent infinite
...
if [[ "${BASH_VERSINFO[0]:-0}" -lt 4 ]] &&
	...
	# (GH#19632 / t2176)
	_maestro_top_caller=""
	for _maestro_src in "${BASH_SOURCE[@]}"; do
		_maestro_top_caller="$_maestro_src"
	done
	unset _maestro_src
	# Safety: skip if outermost caller is this file itself (direct execution
	# of shared-constants.sh, which has no useful main).
	if [[ "$_maestro_top_caller" != "${BASH_SOURCE[0]}" ]]; then
		_maestro_macos_platform="$(printf '\104\141\162\167\151\156')"
		_maestro_bash_candidates="/opt/homebrew/bin/bash /usr/local/bin/bash"
		if [[ "$(uname -s 2>/dev/null || true)" != "$_maestro_macos_platform" ]]; then
			_maestro_bash_candidates="${_maestro_bash_candidates} /home/linuxbrew/.linuxbrew/bin/bash"
		fi
		_maestro_path_bash="$(command -v bash 2>/dev/null || true)"
		[[ -n "$_maestro_path_bash" ]] && _maestro_bash_candidates="${_maestro_bash_candidates} ${_maestro_path_bash}"
		for _maestro_bash_candidate in $_maestro_bash_candidates; do
			if [[ -n "$_maestro_bash_candidate" && -f "$_maestro_bash_candidate" && -x "$_maestro_bash_candidate" && "$_maestro_bash_candidate" != "/bin/bash" ]]; then
				export MAESTRO_BASH_REEXECED=1
				exec "$_maestro_bash_candidate" "$_maestro_top_caller" "$@"
			fi
		done
		unset _maestro_bash_candidate _maestro_bash_candidates _maestro_path_bash _maestro_macos_platform
	fi
	unset _maestro_top_caller
	# Fall through: no modern bash found. The calling script will run
	# on bash 3.2 and may hit compat bugs. The maestro update check
	# will surface an advisory on the next cycle (bash-upgrade-helper.sh
	# update-check, rate-limited to 24h).
fi

# t2201: Clear MAESTRO_BASH_REEXECED once we are stably on bash 4+. The
...
# child's guard sees MAESTRO_BASH_REEXECED=1 and short-circuits the
...
maestro_ensure_pulse_start_epoch() {
...
readonly MAESTRO_GH_MIN_SLURP_VERSION="2.51.0"

maestro_parse_semver() {
	local input="$1"
	local parsed=""
...
maestro_gh_installed_version() {
	local output=""
	if ! command -v gh >/dev/null 2>&1; then
		return 1
	fi
	output=$(gh --version 2>/dev/null || true)
	maestro_parse_semver "$output"
	return $?
}

maestro_gh_slurp_supported() {
	local installed=""
	installed=$(maestro_gh_installed_version) || return 1
	maestro_version_at_least "$installed" "$MAESTRO_GH_MIN_SLURP_VERSION"
	return $?
}

maestro_gh_slurp_status_message() {
	local installed=""
	if ! command -v gh >/dev/null 2>&1; then
		printf 'GitHub CLI (gh) is not installed; gh api --paginate --slurp requires gh >= %s. %s' \
			"$MAESTRO_GH_MIN_SLURP_VERSION" "$(maestro_gh_slurp_remediation_guidance)"
		return 0
	fi
	installed=$(maestro_gh_installed_version) || installed="unknown"
	if [[ "$installed" == "unknown" ]]; then
		printf 'GitHub CLI (gh) version could not be parsed; gh api --paginate --slurp requires gh >= %s; gh --version output is malformed or empty. %s' \
			"$MAESTRO_GH_MIN_SLURP_VERSION" "$(maestro_gh_slurp_remediation_guidance "$installed")"
		return 0
	fi
	if maestro_version_at_least "$installed" "$MAESTRO_GH_MIN_SLURP_VERSION"; then
		printf 'GitHub CLI (gh) detected version %s supports gh api --paginate --slurp (minimum required %s)' \
			"$installed" "$MAESTRO_GH_MIN_SLURP_VERSION"
		return 0
	fi
	printf 'GitHub CLI (gh) detected version %s is too old; gh api --paginate --slurp requires gh >= %s. %s' \
		"$installed" "$MAESTRO_GH_MIN_SLURP_VERSION" "$(maestro_gh_slurp_remediation_guidance "$installed")"
	return 0
}

maestro_gh_slurp_remediation_guidance() {
	local installed="${1:-}"
	if [[ "$installed" == "unknown" ]]; then
		printf 'On Ubuntu/Debian, fix or upgrade the existing gh installation from the official GitHub CLI package repository rather than the Ubuntu universe package, then rerun maestro status.'
	elif [[ -n "$installed" ]]; then
		printf 'On Ubuntu/Debian, avoid apt-pinned Ubuntu universe gh packages such as %s; install or upgrade from the official GitHub CLI package repository, then rerun maestro status.' \
			"$installed"
	else
		printf 'On Ubuntu/Debian, use the official GitHub CLI package repository rather than the Ubuntu universe package, then rerun maestro status.'
	fi
	return 0
}

maestro_gh_slurp_remediation_hint() {
	local os_name=""
	os_name=$(uname -s 2>/dev/null || printf 'unknown')
	case "$os_name" in
	Linux)
		printf 'Run maestro setup to upgrade gh, or install GitHub CLI from the official package source for your distribution.'
		;;
	Darwin)
		printf 'Run brew update && brew upgrade gh, then rerun maestro status.'
		;;
	*)
		printf 'Upgrade gh to >= %s, then rerun maestro status.' "$MAESTRO_GH_MIN_SLURP_VERSION"
		;;
	esac
	return 0
}

maestro_gh_slurp_warning_line() {
	local status_message=""
	status_message=$(maestro_gh_slurp_status_message)
...
readonly USER_AGENT="User-Agent: Maestro-Framework/1.6.0"
...
_maestro_append_launchd_path_dir() {
	local dir="$1"
	[[ -n "$dir" ]] || return 0
	[[ -d "$dir" ]] || return 0
	case ":${_maestro_launchd_path_seen:-}:" in
	*":${dir}:"*) return 0 ;;
	esac
	_maestro_launchd_path_seen="${_maestro_launchd_path_seen:+${_maestro_launchd_path_seen}:}${dir}"
	_maestro_launchd_path_result="${_maestro_launchd_path_result:+${_maestro_launchd_path_result}:}${dir}"
	return 0
}

maestro_launchd_sanitized_path() {
	local input_path="${1:-${PATH:-}}"
	local default_path="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
	local _maestro_launchd_path_result=""
	local _maestro_launchd_path_seen=""
	local dir=""
	local old_ifs="$IFS"

	IFS=':'
	for dir in $default_path; do
		_maestro_append_launchd_path_dir "$dir"
	done
	...
	printf '%s' "$_maestro_launchd_path_result"
	return 0
}
...
_source_shared_module_with_retry() {
	local module_path="$1"
	local attempts="${MAESTRO_SHARED_SOURCE_ATTEMPTS:-5}"
	local interval="${MAESTRO_SHARED_SOURCE_INTERVAL:-1}"
	local attempt=1
...
#   init_log_file                           # Set up MAESTRO_LOG_FILE for script
#
# Guidelines:
...
# Resolve the canonical maestro log directory at runtime.
# Reads paths.log_dir from ~/.maestro/config/paths.jsonc when config-helper.sh
# is sourced (provides _jsonc_get), falls back to ~/.maestro/logs otherwise.
...
_resolve_log_dir() {
	...
	if type _jsonc_get >/dev/null 2>&1; then
		resolved=$(_jsonc_get "paths.log_dir" "~/.maestro/logs")
	else
		resolved="~/.maestro/logs"
	fi
	# Tilde expansion
...
# Sets MAESTRO_LOG_FILE to <log_dir>/<script-name>.log
...
init_log_file() {
	...
	MAESTRO_LOG_FILE="${log_dir}/${script_name}.log"
	export MAESTRO_LOG_FILE
	return 0
}
...
log_stderr() {
	local context="$1"
	shift
	local log_target="${MAESTRO_LOG_FILE:-/dev/null}"
	local timestamp
...
#
# Example:
#   clear_active_status_on_release 20026 Aditya060806/Maestro "$(whoami)"
#######################################
...
[[ -z "${FRAMEWORK_PROCESS_PATTERN+x}" ]] && FRAMEWORK_PROCESS_PATTERN='opencode|claude|Claude|pulse|maestro|headless-runtime'
...
#            _MAESTRO_CONFIG_MODE (populated on load).
