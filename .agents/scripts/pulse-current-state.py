...
def worker_worktree_placeholders():
    worker_worktree_count = os.environ.get('MAESTRO_WORKER_WORKTREE_COUNT', '0')
...
def active_worker_process_count():
    active_worker_output = os.environ.get('MAESTRO_ACTIVE_WORKER_PROCESSES', '')
...
def graphql_budget_status_from_env():
    return os.environ.get('MAESTRO_GRAPHQL_BUDGET_STATUS') or 'UNKNOWN: no cached status'
