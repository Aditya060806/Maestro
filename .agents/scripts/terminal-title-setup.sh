...
MARKER_START="# >>> maestro terminal-title integration >>>"
MARKER_END="# <<< maestro terminal-title integration <<<"
...
tabby_enable_dynamic_titles() {
	...
	# Create backup
	cp "$TABBY_CONFIG_FILE" "${TABBY_CONFIG_FILE}.maestro-backup"
	...
	log_info "Backup saved to: ${TABBY_CONFIG_FILE}.maestro-backup"
...
if [[ -n "${ZSH_VERSION:-}" ]] && [[ "${DISABLE_AUTO_TITLE:-}" != "true" ]]; then
    ...
    add-zsh-hook precmd _maestro_terminal_title
fi
EOF
}
...
if [[ -z "${PROMPT_COMMAND:-}" ]]; then
    PROMPT_COMMAND="_maestro_terminal_title"
else
    PROMPT_COMMAND="_maestro_terminal_title; ${PROMPT_COMMAND}"
fi
EOF
}
...
# clobber meaningful session titles when the canonical repo sits on main (t2252).
function _maestro_terminal_title --on-event fish_prompt
...
remove_integration() {
	...
	# Create backup
	cp "$rc_file" "${rc_file}.maestro-backup"
	...
	log_success "Removed integration from $rc_file (backup: ${rc_file}.maestro-backup)"
}
...
cmd_install() {
	...
	echo ""
	echo "Your terminal tab will now show: repo/branch (e.g., maestro/feature/xyz)"
	return 0
}
...
WHAT IT DOES:
    ...
    Format: repo/branch (e.g., maestro/feature/xyz)
