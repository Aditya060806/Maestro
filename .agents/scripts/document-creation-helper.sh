...
VENV_DIR="${HOME}/.maestro/.agent-workspace/python-env/document-creation"
TEMPLATE_DIR="${HOME}/.maestro/.agent-workspace/templates"
...
run_ocr() {
	...
	tesseract)
		...
		if [[ "$image_file" == /tmp/* || "$image_file" == /private/tmp/* || "$image_file" == /var/folders/* ]]; then
			local work_dir="${HOME}/.maestro/.agent-workspace/tmp"
			mkdir -p "$work_dir"
...
ocr_scanned_pdf() {
	...
	local tmp_dir="${HOME}/.maestro/.agent-workspace/tmp/ocr-$$"
	mkdir -p "${tmp_dir}"
