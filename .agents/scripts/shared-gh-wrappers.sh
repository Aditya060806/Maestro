...
# Match the pattern at pulse-wrapper-config.sh:99-105 for MAESTRO_PULSE_CIRCUIT_BREAKER_THRESHOLD:
...
if [[ -n "$_SHARED_GH_WRAPPERS_DIR" && -f "$_SHARED_GH_WRAPPERS_DIR/../configs/pulse-rate-limit.conf" ]]; then
	if [[ -z "${MAESTRO_GH_READ_TIMEOUT+x}" ]] && [[ -z "${MAESTRO_GH_WRITE_TIMEOUT+x}" ]]; then
		# shellcheck disable=SC1091
		source "$_SHARED_GH_WRAPPERS_DIR/../configs/pulse-rate-limit.conf"
	fi
fi
: "${MAESTRO_GH_READ_TIMEOUT:=15}"
: "${MAESTRO_GH_WRITE_TIMEOUT:=45}"
...
_gh_cooldown_context_from_gh_api_args() {
	...
	_GH_COOLDOWN_CONTEXT_OPERATION="${MAESTRO_GH_COOLDOWN_OPERATION:-gh_api}"
	_GH_COOLDOWN_CONTEXT_WRAPPER="${MAESTRO_GH_COOLDOWN_WRAPPER:-$_GH_COOLDOWN_CONTEXT_DEFAULT_WRAPPER}"
	_GH_COOLDOWN_CONTEXT_STAGE="${MAESTRO_GH_COOLDOWN_STAGE:-wrapper-${op_class}}"
	return 0
}
...
_gh_with_timeout() {
	...
	local secs
	case "$op_class" in
	read) secs="${MAESTRO_GH_READ_TIMEOUT:-15}" ;;
	write) secs="${MAESTRO_GH_WRITE_TIMEOUT:-45}" ;;
	*) secs=30 ;;
	esac
	local cmd="${1:-}"
	local err_file=""
	...
	if [[ -n "$cmd" ]] && declare -f "$cmd" >/dev/null; then
		err_file=$(mktemp -t maestro-gh-secondary.XXXXXX) || {
			"$@"
			return $?
		}
		out_file=$(mktemp -t maestro-gh-response.XXXXXX) || {
			"$@" 2>"$err_file"
			rc=$?
...
$(cat "$err_file" 2>/dev/null || true)"
	...
	err_file=$(mktemp -t maestro-gh-secondary.XXXXXX) || err_file=""
	out_file=""
	if [[ -n "$err_file" ]]; then
		out_file=$(mktemp -t maestro-gh-response.XXXXXX) || {
			rm -f "$err_file"
...
_gh_body_has_substantive_content() {
	local body_value="$1"
	local unsigned_body
	unsigned_body="${body_value%%<!-- maestro:sig -->*}"
...
set_origin_label() {
	...
	local _err_file
	_err_file=$(mktemp -t maestro-origin-label.XXXXXX) || {
