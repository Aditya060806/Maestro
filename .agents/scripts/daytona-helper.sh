...
Environment:
  DAYTONA_API_KEY   API key (or set via: maestro secret set DAYTONA_API_KEY)
...
get_api_key() {
	...
	if [ -z "$key" ] && command -v gopass >/dev/null 2>&1; then
		key="$(gopass show -o maestro/daytona/api-key 2>/dev/null || true)"
	fi

	# 3. credentials.sh
	if [ -z "$key" ] && [ -f "$HOME/.config/maestro/credentials.sh" ]; then
	...
	if [ -z "$key" ]; then
		log_error "DAYTONA_API_KEY not set. Run: maestro secret set DAYTONA_API_KEY"
		return 1
	fi
