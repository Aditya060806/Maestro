...
#######################################
# Detect maestro OpenCode wrapper-scoped SQLite session DBs.
# Output: zero or more database paths, one per line
#######################################
_session_time_detect_wrapper_db_paths() {
	local work_dir="${MAESTRO_WORK_DIR:-${HOME}/.maestro/.agent-workspace/work}"
	local candidate
...
# followed by maestro wrapper-scoped OpenCode DBs.
#######################################
...
_session_time_obs_db_path() {
	local obs_db="${MAESTRO_OBS_DB_FILE:-${OBS_DB_FILE:-${HOME}/.maestro/.agent-workspace/observability/llm-requests.db}}"
