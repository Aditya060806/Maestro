...
#   CURRENT/SELF-CALLER — maestro itself: uses `./.github/workflows/...` instead of remote ref
...
#
# Why this exists:
#   Before the reusable-workflow architecture (t2770), every maestro-enabled repo
...
#   maestro reusable workflow. This helper detects which repos still use the
...
#
#   Phase 2 (`maestro sync-workflows --apply`) will migrate or refresh callers
...
# reusable_file  — the reusable workflow filename in Aditya060806/Maestro
...
_resolve_canonical_template() {
	...
	local _deployed="$HOME/.maestro/agents/templates/workflows/${_template_filename}"
...
# Emits the maestro source reusable workflow path for content-update checks.
...
_classify_workflow() {
	...
	# Detect self-caller (maestro itself) — uses: ./.github/workflows/<reusable-name>
	# This is the intended pattern for the maestro repo and must not be flagged
...
_iterate_repos() {
	if [[ ! -f "$REPOS_JSON" ]]; then
		_die "repos.json not found at $REPOS_JSON — maestro may not be initialised"
	fi
...
_classify_reusable_workflow() {
	...
	if diff -q "$_canonical_reusable" "$_wf" >/dev/null 2>&1; then
		printf 'CURRENT/REUSABLE\treusable workflow content matches maestro baseline\n'
	else
		printf 'DRIFTED/REUSABLE\torg-owned reusable workflow differs; review/update from maestro baseline\n'
	fi
	return 0
}
...
_classify_row() {
	...
	case "$_class" in
	NEEDS-MIGRATION)
		_note="legacy full-copy; run: maestro sync-workflows --apply"
		;;
	esac
	printf '%s\t%s\n' "$_class" "$_note"
	return 0
