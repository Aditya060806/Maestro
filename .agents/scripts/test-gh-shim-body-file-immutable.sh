...
# ---------------------------------------------------------------------------
_fixture=$(mktemp -t maestro-test-brief.XXXXXX)
printf 'Test body\n\nSecond paragraph.\n' >"$_fixture"
...
_resolved=$(SHIM_TEST_MODE=1 MAESTRO_GH_SHIM_DISABLE=0 \
...
# ---------------------------------------------------------------------------
_fixture2=$(mktemp -t maestro-test-brief.XXXXXX)
printf 'Attached form test\n' >"$_fixture2"
_hash_before2=$(shasum -a 256 "$_fixture2" | awk '{print $1}')

SHIM_TEST_MODE=1 MAESTRO_GH_SHIM_DISABLE=0 \
