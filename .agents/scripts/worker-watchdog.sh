...
_resolve_script_dir_with_retry() {
	local source_path="$1"
	local lib_dir
	local attempts="${MAESTRO_SCRIPT_DIR_ATTEMPTS:-5}"
	local interval="${MAESTRO_SCRIPT_DIR_INTERVAL:-1}"
	local attempt=1
	...
	if [[ -d "${HOME}/.maestro/agents/scripts" ]]; then
		cd "${HOME}/.maestro/agents/scripts" && pwd
		return $?
	fi
...
HEADLESS_RUNTIME_DB="${HEADLESS_RUNTIME_DB:-${HOME}/.maestro/.agent-workspace/headless-runtime/state.db}"
...
readonly LAUNCHD_LABEL="sh.maestro.worker-watchdog"
readonly PLIST_PATH="${HOME}/Library/LaunchAgents/${LAUNCHD_LABEL}.plist"
readonly CRON_MARKER="# maestro: worker-watchdog"
readonly SYSTEMD_SERVICE_NAME="maestro-worker-watchdog"
