...
# =============================================================================
# maestro Reach Core JSON Library
# =============================================================================
...
reach_workspace_dir() {
	printf '%s' "${MAESTRO_REACH_WORKSPACE:-${HOME}/.maestro/.agent-workspace/reach}"
	return 0
}
...
reach_session_ref() {
	local session_source="${MAESTRO_SESSION_ID:-${OPENCODE_SESSION_ID:-${CLAUDE_SESSION_ID:-}}}"
...
reach_performance_log_path() {
	if [[ -n "${MAESTRO_REACH_PERFORMANCE_LOG:-}" ]]; then
		printf '%s' "$MAESTRO_REACH_PERFORMANCE_LOG"
		return 0
	fi
	...
	local workspace_dir="${MAESTRO_WORKSPACE:-${HOME}/.maestro/.agent-workspace}"
