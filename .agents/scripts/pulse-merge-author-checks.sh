...
# #maestro:trust-boundary — collaborator/maintainer trust checks must not
...
_pulse_author_permission_lookup_uncached() {
	...
	if declare -F _gh_collaborator_permission_lookup >/dev/null 2>&1; then
		...
		local rc=$?
		if [[ "$rc" -ne 0 ]]; then
			_pulse_author_permission_state_set "failed" "${MAESTRO_GH_COLLAB_PERMISSION_HTTP:-$_PULSE_AUTHOR_PERMISSION_UNKNOWN}"
			return 2
		fi
		_pulse_author_permission_state_set "ok" "${MAESTRO_GH_COLLAB_PERMISSION_HTTP:-$_PULSE_AUTHOR_PERMISSION_UNKNOWN}" "$pulse_perm_value"
		if [[ -n "$out_var" ]]; then
...
_pulse_author_permission_lookup() {
	...
	local repo_slug="$2"
	local out_var="${3:-}"
	local cache_dir="${MAESTRO_PULSE_AUTHOR_PERMISSION_CACHE_DIR:-}"
...
_interactive_pr_auto_merge_allowed() {
	...
	if [[ -n "${MAESTRO_INTERACTIVE_PR_AUTO_MERGE+x}" && -n "${MAESTRO_INTERACTIVE_PR_AUTO_MERGE:-}" ]]; then
		local env_value="${MAESTRO_INTERACTIVE_PR_AUTO_MERGE:-}"
		if _interactive_pr_bool_enabled "$env_value"; then
			echo "[pulse-wrapper] Merge pass: PR #${pr_number} in ${repo_slug} — MAESTRO_INTERACTIVE_PR_AUTO_MERGE opts origin:interactive PRs into automated merge throughput (GH#23238)" >>"$LOGFILE"
			return 0
		fi
		echo "[pulse-wrapper] Merge pass: skipping PR #${pr_number} in ${repo_slug} — MAESTRO_INTERACTIVE_PR_AUTO_MERGE=${env_value} requires manual merge for origin:interactive PRs (GH#23238)" >>"$LOGFILE"
		return 1
	fi
