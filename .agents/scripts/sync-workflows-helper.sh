...
#
# Design invariants:
#   - Never touch the maestro repo itself (CURRENT/SELF-CALLER is not drift).
...
# Mandatory — this script ships as part of maestro so the wrappers are
...
readonly _DEFAULT_WORKFLOW_REUSABLE_REPO="Aditya060806/Maestro"
...
_resolve_canonical_template() {
	...
	local _candidates=(
		"$HOME/.maestro/agents/templates/workflows/${_template_filename}"
...
_render_template_with_target() {
	...
	# Template ships with `Aditya060806/Maestro@main` by default; rewrite the
	...
	sed -E \
		-e 's|(uses:[[:space:]]*)Aditya060806/Maestro(/\.github/workflows/[^@[:space:]]+)@[^[:space:]]+|\1'"$_repo_escaped"'\2@'"$_ref_escaped"'|' \
		-e 's|Aditya060806/Maestro(/\.github/workflows/[^[:space:]]+)|'"$_repo_escaped"'\1|g' \
		"$_template"
	return 0
}
...
_format_commit_body() {
	...
	printf 'Resync `%s` to the canonical maestro caller template.\n\n' "$_workflow_path"
	printf 'Classification before: %s\n' "$_status"
	printf 'Ref: %s\n\n' "$_ref"
	printf 'This migrates/refreshes the workflow to the reusable-workflow pattern.\n'
	printf 'The caller now delegates all logic to the maestro reusable workflow,\n'
	printf 'eliminating drift between this repo and the framework canonical version.\n\n'
	printf 'Generated by: `maestro sync-workflows --apply` (see Aditya060806/Maestro#20649)\n'
	return 0
}
...
_format_pr_body() {
	...
	printf '**Ref**: `%s`\n\n' "$_ref"
	printf '## Why\n\n'
	printf 'The maestro framework ships managed GitHub Actions workflows as reusable\n'
	printf 'workflows. Downstream repos carry ~45-line callers that delegate all logic\n'
	printf 'to the configured maestro reusable workflow repository/ref.\n\n'
...
_sync_open_pr() {
	...
	local _pr_title="chore: resync framework workflow to maestro canonical caller"
	local _pr_body
...
_process_rows() {
	...
	while IFS=$'\t' read -r _slug _path _status _workflow_name; do
		[[ -z "$_slug" ]] && continue
		# Never touch maestro itself (defence in depth; Phase 1 also emits
		# CURRENT/SELF-CALLER).
