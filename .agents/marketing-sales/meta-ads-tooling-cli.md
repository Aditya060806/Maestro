...
```bash
# Good
source <(maestro secret env meta-ads-clientA-prod)
...
For the maestro framework: store via `maestro secret set meta-ads-<account>-<env>` (gopass-backed),
retrieve via `maestro secret env meta-ads-<account>-<env>`. The naming convention is
...
`staging`. Plaintext fallback: `~/.config/maestro/credentials.sh` (chmod 600).
...
source <(maestro secret env meta-ads-brandB-dev)
meta ads campaigns list

# Run insights for client A staging without polluting the shell
env $(maestro secret env meta-ads-clientA-staging) \
...
maestro secret set meta-ads-clientA-prod
```
...
if ! meta ads campaign list --format json > /tmp/campaigns.json; then
  case $? in
    3) echo "auth"; maestro secret rotate meta-ads-clientA-prod ;;
...
4. **Stay in your ad account.** Always set `META_AD_ACCOUNT_ID` per session; never rely on a default. If you manage multiple accounts (agency model), use `source <(maestro secret env meta-ads-<account>-<env>)` at the top of each script to load the correct account credentials explicitly.
5. **Audit trail.** Pipe writes through a logger: `meta ads campaign create … --format json | tee -a ~/.maestro/logs/meta-ads-writes.jsonl`. Combine with `audit-log-helper.sh log meta-ads "<msg>"` for security ops (creative deletes, dataset disconnects).
