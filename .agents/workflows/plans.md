...
**`/ready` command**: `~/.maestro/agents/scripts/todo-ready.sh` — shows tasks with no open blockers and lists blocked tasks with their dependencies.
...
Configure per-repo in `.maestro.json`: `{ "time_tracking": "prompt", "features": ["planning", "time-tracking", "beads"] }`. Values: `true` = always prompt | `false` = never | `prompt` = ask once per session. Use `/log-time-spent` to manually log time.
...
**Identity**: Set `MAESTRO_IDENTITY` env var, or defaults to `$(whoami)@$(hostname -s)`. **Status labels**: `status:available` → `status:claimed` → `status:in-review` → `status:done`
