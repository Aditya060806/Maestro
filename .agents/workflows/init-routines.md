...
```bash
maestro init-routines                  # Personal: <username>/maestro-routines
maestro init-routines --org <name>     # Org: <org>/maestro-routines (private)
maestro init-routines --local          # Local-only, local_only: true in repos.json
maestro init-routines --dry-run        # Preview without changes

# Or via the helper directly:
~/.maestro/agents/scripts/init-routines-helper.sh [--org <name>] [--local] [--dry-run]
```
...
Registered in `~/.config/maestro/repos.json` with `pulse: true, priority: "tooling"`.
...
Create `routines/custom/<id>.md` for your own routines. Custom descriptions override core descriptions when both exist for the same ID. Use `~/.maestro/agents/templates/routine-description-template.md` as a starting point.

## After setup

Add your own routines to `~/Git/maestro-routines/TODO.md` under `## User Routines`. Use IDs r001-r899 (core routines use r901+). See `.agents/reference/routines.md` for field specification.
...
- `~/.config/maestro/repos.json` — repo registration
