...
```bash
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/pre-edit-check.sh  # Verifies branch and canonical-checkout safety
```
...
Main repo (`~/Git/{repo}/` or grouped `~/Git/{ecosystem}/{repo}/`) ALWAYS stays on `main`. Create linked worktrees under `${MAESTRO_WORKTREE_BASE_DIR:-~/Git/_worktrees}` using flat `<repo>-<branch-slug>` names; do not create durable implementation worktrees in runtime temp paths such as macOS `/var/folders/.../T/opencode/`.

```bash
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add feature/my-feature
...
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add {type}/{issue_number}-{slug-from-title}
# Then cd into the printed linked worktree path before editing.
```
...
Management: `install-hooks-helper.sh [status|install|test|uninstall]`. Files: `~/.maestro/hooks/git_safety_guard.py`, `~/.claude/settings.json`. Installed by `setup.sh`. Requires Python 3 + Claude Code restart.
...
**If changes include `.agents/` files**: Offer to run `./setup.sh` to deploy to `~/.maestro/agents/`.

## Branch Cleanup

After postflight, delete merged branches. Keep unmerged unless stale (>30 days) — ask user. Prefer the maestro cleanup route for remote branch sweeps because it checks merged/open-PR/worktree evidence before deletion.

```bash
git checkout main && git pull origin main
wt prune                                      # Local worktrees
maestro cleanup remote-branches             # Dry-run remote audit
maestro cleanup remote-branches --apply     # Delete safe remote candidates
...
When the authenticated account lacks maintainer-equivalent access (`admin`, `maintain`, or `write`) on the target repo, treat the repo as an upstream community project rather than an maestro-managed queue:
...
- Do NOT post worker-ready briefs, internal lifecycle markers, repeated diagnostics, or maestro audit/signature boilerplate in upstream issue threads.
...
- `/release-issue <N>` and `maestro issue release <N>` exist as fallbacks only; the agent should never punt to them. Detect intent and act.
...
**Git-readiness:** Non-git project with ongoing development? Flag: "No git tracking. Consider `git init` + `maestro init`."
...
**Reusable workflows:** downstream repos use thin caller YAMLs that reference maestro reusable workflows. Full architecture/pinning/drift tools: `reference/reusable-workflows.md`.

**Badge management (t2975):** README badge blocks and LOC badge workflows are managed by `maestro badges`; full docs: `.agents/maestro/badges.md`.
