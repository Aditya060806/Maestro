...
```bash
~/.maestro/agents/scripts/memory-helper.sh recall "success pattern" --type SUCCESS_PATTERN --limit 20
~/.maestro/agents/scripts/memory-helper.sh recall "failure pattern" --type FAILURE_PATTERN --limit 20
~/.maestro/agents/scripts/memory-helper.sh recall "working solution" --type WORKING_SOLUTION --limit 10
~/.maestro/agents/scripts/memory-helper.sh recall "failed approach" --type FAILED_APPROACH --limit 10
```
