...
| Signal | Suggestion |
|--------|-----------|
| Repo is maestro | "agent instruction optimization (reduce tokens, improve pass rate)" |
...
```text
1. This repo (.)                    [default]
2. Another managed repo (which?)    → validate in ~/.config/maestro/repos.json
...
|---------|---------|------|-----------|
| maestro + "agent" | `agent-test-helper.sh run {suite} --json \| jq '.composite_score'` | `composite_score` | higher |
| maestro + "token" | `agent-test-helper.sh run {suite} --json \| jq '.avg_response_chars'` | `avg_response_chars` | lower |
...
maestro init --path "$REPO_PATH" --non-interactive  # warn and continue if unavailable
```

**I6: Register in `~/.config/maestro/repos.json`:**
