...
```bash
~/.maestro/agents/scripts/dispatch-ledger-helper.sh tier-report
```
...
```bash
CORPUS="${HOME}/.maestro/.agent-workspace/work/tier-corpus"
mkdir -p "$CORPUS"
...
for slug in $(jq -r '.initialized_repos[] | select(.pulse == true) | .slug' ~/.config/maestro/repos.json); do
  ~/.maestro/agents/scripts/brief-tier-test-helper.sh extract \
...
```bash
~/.maestro/agents/scripts/brief-tier-test-helper.sh enrich \
...
```bash
CORPUS="${HOME}/.maestro/.agent-workspace/work/tier-corpus"
RESULTS="${HOME}/.maestro/.agent-workspace/work/tier-results.tsv"

~/.maestro/agents/scripts/brief-tier-test-helper.sh test \
  --corpus "$CORPUS" --model haiku --results "$RESULTS"

~/.maestro/agents/scripts/brief-tier-test-helper.sh report --results "$RESULTS"
```
...
LAST_EXPAND="${HOME}/.maestro/.agent-workspace/tmp/tier-corpus-last-expand"
...
- Append-only log: `~/.maestro/.agent-workspace/tmp/tier-telemetry.jsonl`
