...
~/.maestro/agents/scripts/pre-edit-check.sh --loop-mode --task "task description"
```
...
Keep `~/Git/{repo}/` on `main`. Create linked worktrees under `${MAESTRO_WORKTREE_BASE_DIR:-~/Git/_worktrees}`. This avoids blocked branch switches, parallel sessions inheriting the wrong branch, and `local changes would be overwritten` errors.
...
~/.maestro/agents/scripts/worktree-helper.sh add {type}/{name}
```
...
- Source: `~/Git/maestro/.agents/` — git-tracked, branch matters
- Deployed: `~/.maestro/agents/` — copied output, not a git repo
