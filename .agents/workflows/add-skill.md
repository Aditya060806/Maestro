---
description: Import external skills from GitHub, ClawdHub, or URLs into maestro
agent: Build+
mode: subagent
---
...
Import an external skill, convert to maestro format, and register for update tracking. URL/Repo: `$ARGUMENTS`
...
`-skill` suffix marks imported skills: `playwright-skill.md` (imported, upstream-tracked) vs `playwright.md` (native). No name clashes; `*-skill.md` glob finds all imports; `maestro skill check` knows which to update.

## Workflow

1. **Parse input** — GitHub shorthand, full URL, ClawdHub shorthand/URL, raw URL, or management command.
2. **Run helper:** `~/.maestro/agents/scripts/add-skill-helper.sh add "$ARGUMENTS"` (other commands: `list | check-updates | remove <name>`)
3. **Handle conflicts** (if file exists): Merge / Replace / Separate / Skip.
4. **Security scan:** [Cisco Skill Scanner](https://github.com/cisco-ai-defense/skill-scanner) if installed — CRITICAL/HIGH blocks import. `--skip-security` bypasses; `--force` only controls file overwrite. Scan also runs on `maestro skill update`.
...
| AGENTS.md | maestro/Windsurf | Direct copy with mode: subagent |
...
- `scripts/generate-skills.sh` — SKILL.md stub generation for maestro agents
