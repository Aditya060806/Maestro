...
- **Private state**: `~/.maestro/.agent-workspace/auto-browse/<workflow-id>/`.
- **Private promoted agents**: prefer `~/.maestro/agents/custom/auto-browse/` after user review; keep raw run traces in the private state folder.
...
6. Auth/session plan: existing browser session, maestro-managed profile, cookies/storage state, official API, or user will log in manually.
...
Use secure references for credentials (`maestro secret`, gopass, or documented credential refs). Never write credential values, cookies, bearer tokens, private client names, or private local paths into repo artifacts or chat.
...
2. For repeatable capture/data-mining work, run `maestro reach doctor` and
   `maestro reach route --objective "<sanitized objective>" --format json` as a
...
```text
~/.maestro/agents/custom/auto-browse/<workflow-slug>.md
```
