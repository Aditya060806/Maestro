...
MAX_WORKERS=$(test -r ~/.maestro/logs/pulse-max-workers && cat ~/.maestro/logs/pulse-max-workers || echo 4)
...
if [[ "$(uname)" == "Darwin" ]]; then
  launchctl list | grep -i 'maestro.*pulse' || echo "No launchd pulse found"
else
...
- **System Health** — pulse scheduler (launchd/macOS, cron/Linux), recent log: `tail -20 ~/.maestro/logs/pulse.log`
