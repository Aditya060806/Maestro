...
3. **Store** — after confirmation:
   ...
   ```bash
   ~/.maestro/agents/scripts/memory-helper.sh store --type "{type}" --content "{content}" --tags "{tags}" --project "{project}"
   ```
...
`~/.maestro/.agent-workspace/memory/memory.db` (SQLite FTS5)

Stats: `~/.maestro/agents/scripts/memory-helper.sh stats`
