...
For maestro-managed GitHub PRs, create and sign the body file before the
...
Interactive maestro PRs default to draft until the user explicitly asks to finalise/ready the PR or invokes `/pr-loop`. To permit pulse merge throughput after finalisation, also opt in with `allow-auto-merge`, `MAESTRO_INTERACTIVE_PR_AUTO_MERGE=1`, global `orchestration.interactive_pr_auto_merge=true`, or per-repo `repos.json` `interactive_pr_auto_merge=true`.
...
```bash
~/.maestro/agents/scripts/review-bot-gate-helper.sh check 123   # PASS/WAITING/SKIP
~/.maestro/agents/scripts/review-bot-gate-helper.sh wait 123    # Wait up to 10 min
~/.maestro/agents/scripts/review-bot-gate-helper.sh list 123    # List bot activity
```
...
Run `~/.maestro/agents/scripts/linters-local.sh` — ShellCheck, secretlint, pattern validation (return statements, positional parameters), markdown formatting.

### 2. Remote Auditing (`/code-audit-remote`)

Run `~/.maestro/agents/scripts/code-audit-helper.sh audit [repo]` — CodeRabbit (AI review), Codacy (quality), SonarCloud (security/maintainability). Monitored AI reviewers: `coderabbit*`, `gemini-code-assist[bot]`, `augment-code[bot]`/`augmentcode[bot]`, `copilot[bot]`.
...
2. Sync: `~/.maestro/agents/scripts/beads-sync-helper.sh push`
