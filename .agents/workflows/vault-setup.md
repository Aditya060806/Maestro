...
**TL;DR:** Vault setup is local-first. Use `maestro vault init`, `maestro
vault unlock`, `maestro vault lock`, `maestro vault status`, and `maestro
...
4. Classifies existing maestro data before migration.
...
1. Is the Vault passphrase saved in a trusted password manager with a separate
   backup path?
2. Has the user acknowledged that maestro cannot recover a lost passphrase?
...
```bash
maestro vault init
maestro vault setup-state
maestro vault status
maestro vault unlock
maestro vault setup-state
maestro vault lock
```
...
- Before setup: `maestro vault status` prints `uninitialized`.
...
- After `maestro vault lock`, broker exit, crash, or restart: status prints
...
events under `~/.config/maestro/vault/` unless `MAESTRO_VAULT_DIR` relocates
...
local acknowledgement `I UNDERSTAND` that maestro cannot recover a lost
...
```bash
maestro vault lost-passphrase
maestro vault lost-passphrase archive-and-start-fresh
```
