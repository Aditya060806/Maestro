...
```bash
~/.maestro/agents/scripts/tech-stack-helper.sh lookup "$ARGUMENTS"
```
...
cmd_array=(~/.maestro/agents/scripts/tech-stack-helper.sh reverse "$tech_name")
...
```bash
~/.maestro/agents/scripts/tech-stack-helper.sh cache-stats
~/.maestro/agents/scripts/tech-stack-helper.sh cache-clear --older-than 7
```
...
| BuiltWith API | $295/month; free tier: 100 lookups/month | `maestro secret set BUILTWITH_API_KEY` |
