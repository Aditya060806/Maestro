...
```bash
~/.maestro/agents/scripts/budget-analysis-helper.sh recommend --goal "{mission_desc}" --json
~/.maestro/agents/scripts/budget-analysis-helper.sh analyse --budget {cost_budget} --hours {time_budget} --json
```
...
if top_level=$(git rev-parse --show-toplevel 2>/dev/null); then
  ...
  MISSION_HOME="$HOME/.maestro/missions"
fi
...
**Optional repo creation** (homeless + greenfield): `1. Create + init (rec) — git init + maestro init  2. Use existing repo  3. Keep homeless`. If option 1:
...
while IFS= read -r feature_title; do
  output=$(~/.maestro/agents/scripts/claim-task-id.sh --title "$feature_title" --repo-path "$repo_path")
...
Mission dirs: `research/` (comparisons, API evals), `agents/` (temp agents, draft tier), `scripts/` (automation), `assets/` (screenshots, PDFs). Promote generally-useful agents/scripts to `~/.maestro/agents/draft/` and log in decisions log.
