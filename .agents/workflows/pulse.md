...
from zsh. Example: `bash -lc 'source ~/.maestro/agents/scripts/pulse-wrapper.sh; list_active_worker_processes'`.

```bash
export PATH="/bin:/usr/bin:/usr/local/bin:/opt/homebrew/bin:$PATH"
~/.maestro/agents/scripts/circuit-breaker-helper.sh check  # exit 1 = stop

MAX_WORKERS=$(cat ~/.maestro/logs/pulse-max-workers 2>/dev/null || echo 4)
[[ "$MAX_WORKERS" =~ ^[0-9]+$ ]] || MAX_WORKERS=4
source ~/.maestro/agents/scripts/pulse-wrapper.sh
...
State: `~/.maestro/.agent-workspace/routine-state.json`. Schedules: `daily(@HH:MM)`, `weekly(day@HH:MM)`, `monthly(N@HH:MM)`, `cron(5-field-expr)`. The LLM does NOT evaluate routines.

### 5. Record initial dispatch success

```bash
~/.maestro/agents/scripts/circuit-breaker-helper.sh record-success
```
...
1. **Create a todo batch** for this cycle (drift prevention):
   ...
   - [x] Dispatch worker for issue #3567 (Aditya060806/Maestro)
   - [x] Merge PR #4551 (Aditya060806/Maestro.sh)
...
```bash
~/.maestro/agents/scripts/circuit-breaker-helper.sh record-success
~/.maestro/agents/scripts/session-miner-pulse.sh 2>&1 || true
~/.maestro/agents/scripts/backfill-status-available.sh --apply 2>&1 || true
```
...
For repos where `repo_allows_pulse_write_actions SLUG` fails (role=`contributor`/read-only), observe only: never comment, label, approve, close, merge, dispatch, or apply maestro maintainer-gate policy.
...
RESOLVED_MODEL=$(~/.maestro/agents/scripts/model-availability-helper.sh resolve <tier>)
...
```bash
SIG_FOOTER=$(~/.maestro/agents/scripts/gh-signature-helper.sh footer \
