...
```bash
~/.maestro/agents/scripts/memory-graduate-helper.sh status
```

2. Show candidates for graduation:

```bash
~/.maestro/agents/scripts/memory-graduate-helper.sh candidates
```

3. If arguments include `--dry-run`, preview without writing:

```bash
~/.maestro/agents/scripts/memory-graduate-helper.sh graduate --dry-run
```

4. If the user confirms (or no `--dry-run`), graduate the memories:

```bash
~/.maestro/agents/scripts/memory-graduate-helper.sh graduate
```

5. After graduation, remind the user to commit the updated
   `.agents/maestro/graduated-learnings.md` file.
