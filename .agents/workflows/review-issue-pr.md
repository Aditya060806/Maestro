...
maestro has several safety gates. Every non-trivial change must be mapped against them:
...
Accepted. Tracked internally as #{internal_issue} and implemented in PR #{pr_number} (now merged). This will be included in the next `maestro update`.
