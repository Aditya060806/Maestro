...
```bash
~/.maestro/agents/scripts/venv-health-check-helper.sh scan $ARGUMENTS
```
...
| Invocation | Result |
|------------|--------|
| `/venv-health` | Scan all repos in `~/.config/maestro/repos.json` |
...
Runs daily via `auto-update-helper.sh` when idle. Results: `~/.maestro/logs/auto-update.log`.
...
```bash
maestro config set updates.venv_health_check false
maestro config set updates.venv_health_hours 12
```
