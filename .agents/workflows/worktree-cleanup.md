...
Use the maestro CLI route when the remote has accumulated old worker/worktree branches:

```bash
maestro cleanup remote-branches              # dry-run audit
maestro cleanup remote-branches --apply      # delete safe candidates
maestro cleanup branches --repo ~/Git/maestro --remote origin
```
