...
- Fallback: `${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add {type}/{name}`, then `cd` into the printed linked worktree path
- Keep `~/Git/{repo}/` or grouped `~/Git/{ecosystem}/{repo}/` on `main`; do task work in the linked worktree path under `${MAESTRO_WORKTREE_BASE_DIR:-~/Git/_worktrees}`
...
|-------|-----------------|-------|
| Create | `wt switch -c {type}/{desc}` or `${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add {type}/{desc}` then `cd` into the printed path | Safe linked worktree from `main` |
