...
mission-skill-learner.sh scan-all [--repo <path>]    # Scan all missions; paths: {repo}/todo/missions/*/mission.md + ~/.maestro/missions/*/mission.md
...
| Standard format | +15 | Follows maestro conventions (frontmatter, set -euo, local vars) |
...
| Draft | >= 40 | `~/.maestro/agents/draft/` | Experimental, survives updates. `promote <path> draft` |
| Custom | >= 70 | `~/.maestro/agents/custom/` | Proven useful across missions. `promote <path> custom` |
| Shared | >= 85 | Requires PR to maestro repo | Flagged as candidate; user/supervisor creates PR |
...
2. For each suggestion with score >= 40, promote based on tier:
   ...
   - **Score >= 85**: do not use CLI promote; create an maestro PR and follow shared-tier review
