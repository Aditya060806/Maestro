...
```bash
curl -s "https://sonarcloud.io/api/qualitygates/project_status?projectKey=Aditya060806_maestro"
```
...
Preflight in a worktree checks **worktree files**, not deployed `~/.maestro/agents/`. Issues resolve only after merge + `./setup.sh` redeployment.
