...
**Evolving draft agents**: When a loop iteration discovers reusable domain patterns, capture them as a draft agent in `~/.maestro/agents/draft/`. See `tools/build-agent/build-agent.md` "Agent Lifecycle Tiers".
...
```bash
~/.maestro/agents/scripts/pre-edit-check.sh --loop-mode --task "Build a REST API for todos"
```
...
For maestro repo: also run `readme-helper.sh check`. See `scripts/commands/full-loop.md` Step 3.
...
```bash
~/.maestro/agents/scripts/worktree-sessions.sh list
...
- Upstream sync: `~/.maestro/agents/scripts/ralph-upstream-check.sh`
