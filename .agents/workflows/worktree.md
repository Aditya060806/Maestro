...
- **Core principle**: Main repo (`~/Git/{repo}/` or grouped parent) ALWAYS stays on `main`. Create durable linked worktrees under `${MAESTRO_WORKTREE_BASE_DIR:-~/Git/_worktrees}`, not runtime temp dirs. Group WordPress repos under `~/Git/wordpress/`, EspoCRM under `~/Git/espocrm/`, and MCP under `~/Git/mcp/` (details: `reference/repo-organization.md`). **Never `git checkout -b` in the main repo** — the next session inherits wrong state.
- **Preferred tool**: [Worktrunk](https://worktrunk.dev) (`brew install max-sixty/worktrunk/wt`) — full docs: `tools/git/worktrunk.md`
- **Fallback**: `~/.maestro/agents/scripts/worktree-helper.sh`
...
```bash
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add feature/my-feature          # Auto-path: ~/Git/_worktrees/<repo>-feature-my-feature; cd into printed path
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add feature/my-feature ~/custom  # Custom path; cd into that path
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh list                            # List worktrees
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh status                          # Status overview
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh remove feature/auth             # Removes directory, NOT the branch
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh clean                           # Batch cleanup merged branches (interactive, runs git fetch --prune)
```
...
```bash
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-sessions.sh list   # List worktrees with matching sessions
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-sessions.sh open   # Interactive: select + open
```
...
Worktrees registered to creating session's PID in SQLite (`~/.maestro/.agent-workspace/worktree-registry.db`) — prevents cross-session removal.

```bash
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh registry list    # View ownership
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh registry prune   # Prune stale entries (dead PIDs, missing dirs)
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh remove feature/branch --force  # Override ownership (use with caution)
```
