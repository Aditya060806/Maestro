...
~/.maestro/agents/scripts/linters-local.sh
```
...
Only bump version after fix is confirmed working. Use `version-manager.sh release patch` for maestro releases.
...
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add hotfix/v{VERSION} --base v{LATEST_TAG}
