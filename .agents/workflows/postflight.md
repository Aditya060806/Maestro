...
${MAESTRO_DIR:-$HOME/.maestro}/agents/scripts/worktree-helper.sh add hotfix/v{VERSION}.1 --base v{VERSION}
...
```bash
curl -s "https://sonarcloud.io/api/hotspots/search?projectKey=Aditya060806_maestro&status=TO_REVIEW" | \
...
```bash
curl -s "https://sonarcloud.io/api/issues/search?componentKeys=Aditya060806_maestro&resolved=false&types=BUG,VULNERABILITY" | \
...
~/.maestro/agents/scripts/worktree-helper.sh clean  # Auto-detects squash merges
```
