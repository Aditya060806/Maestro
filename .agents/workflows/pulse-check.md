...
1. Run the deterministic helper first:
   ...
   ```bash
   ~/.maestro/agents/scripts/pulse-check-helper.sh report $ARGUMENTS
   ```

2. If the user asks for machine-readable evidence:

   ```bash
   ~/.maestro/agents/scripts/pulse-check-helper.sh json $ARGUMENTS
   ```
...
3. If the user explicitly asks to file self-improvement work, or this is the
   ...
   ```bash
   ~/.maestro/agents/scripts/pulse-check-helper.sh apply $ARGUMENTS
   ```
