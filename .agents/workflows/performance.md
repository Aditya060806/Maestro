...
Analyze web performance for `$ARGUMENTS` using Chrome DevTools MCP. Verify: `which npx && npx chrome-devtools-mcp@latest --version || echo "Install: npm i -g chrome-devtools-mcp"`. Read `~/.maestro/agents/tools/performance/performance.md` for CWV thresholds and fix patterns.
