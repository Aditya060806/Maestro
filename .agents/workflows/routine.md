...
Fields: `repeat:` (`daily(@HH:MM)`, `weekly(day@HH:MM)`, `monthly(N@HH:MM)`, `cron(expr)`), `run:` (script path relative to `~/.maestro/agents/`), `agent:` (LLM-backed via `headless-runtime-helper.sh`). Prefer `run:` over `agent:`. Default: `run:custom/scripts/{routine-name}.sh` if present, else `agent:Build+`.
...
```bash
~/.maestro/agents/scripts/routine-helper.sh plan \
...
```bash
# maestro: weekly client rankings
...
```bash
# Ad-hoc report
~/.maestro/agents/scripts/gh-failure-miner-helper.sh report --since-hours 24 --pulse-repos

# Auto-file deduplicated systemic-fix issues
~/.maestro/agents/scripts/gh-failure-miner-helper.sh create-issues \
...
~/.maestro/agents/scripts/gh-failure-miner-helper.sh install-launchd-routine
```
