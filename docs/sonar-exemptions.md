...
Tracks all SonarCloud rule suppressions for the Maestro Framework codebase.
Suppressions are either config-level (via `sonar-project.properties`) or per-site (`# NOSONAR` inline annotations).

See also: [parent task #20401](https://github.com/Aditya060806/Maestro/issues/20401) — SonarCloud rule tuning initiative.
