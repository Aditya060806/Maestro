...
# maestro GUI control plane

This document is the canonical product memory for the maestro GUI/control
...
managing, and scaling maestro across local machines, Cloudron, git platforms,
infrastructure providers, routines, and AI-agent workflows.

The first usable product remains local-first:

- It must run without signing, VPN access, or a hosted maestro service.
- It must not centralise secrets or make a remote control plane mandatory.
- It uses maestro helper scripts and git platforms as sources of truth instead
...
- Git platforms remain the primary source of truth for projects, planning,
  ...
  collaboration.
- The GUI is a control surface over existing maestro helpers and config, not a
  second source of truth.
- Secrets stay in maestro secret storage, gopass, Vaultwarden, OS keychains, or
...
- The app should guide choices with maestro preferences rather than hiding the
...
| Long-running local access | `maestrod` typed daemon | Browser UI never gets direct shell access |
...
packages/gui-adapters/     maestro helpers, OpenCode, git hosts, gopass, Cloudron
...
- maestro version and update status;
...
- local maestro/OpenCode instances and machines with available compute.
...
The GUI should index Git state and maestro metadata, but Git platforms remain
...
- local maestro/OpenCode instance;
...
- link providers to setup guides and relevant maestro helpers.
...
Expose setup, scheduling, status, and troubleshooting for maestro routines.
...
The source of truth remains TODO/routine definitions and maestro scheduler
...
The maestro agents are the growing knowledgebase of capabilities and
...
- Local API can call a small allowlist of maestro helpers.
...
- adapter tests with fake maestro helper outputs before calling real helpers;
...
| maestro settings | Existing config/settings helpers | Read, validate, call safe write helpers |
| Repo registry | `repos.json` | Validate, back up, edit through helper APIs |
| Secrets | maestro secrets/gopass/Vaultwarden/OS keychain | Status and setup guidance only |
| Routines | TODO/routine definitions and scheduler helpers | Dashboard, editor, run history |
| OpenCode config | maestro OpenCode plugin registry | Display generated state; edit upstream config only |
