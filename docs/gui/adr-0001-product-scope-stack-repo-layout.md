...
maestro needs a GUI/control plane that helps users set up, understand, manage,
...
- Integration should stay maestro-helper-first: existing helper scripts,
...
The maestro repository root is a public contract. New top-level paths require a
...
- SQLite-first local storage for cached status, projections, and user-owned
  control-plane data.
- maestro helper scripts, git providers, and repo files as authoritative
  integration sources.
...
No first-use path may require code signing, VPN access, a hosted maestro
...
stable. The first implementation should live with the maestro helpers it wraps.
...
- Phase 6 may begin coding the read-only local API and dashboard scaffold after
  ...
  place.
- First implementation work should call existing maestro helpers rather than
