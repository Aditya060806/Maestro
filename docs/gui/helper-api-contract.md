...
Accepted for planning. This contract defines how the maestro GUI local API wraps
existing maestro helpers and configuration surfaces. It does not implement new
helper commands.

## Goals

- Keep maestro helpers, config files, git platforms, and secret backends as the
  sources of truth.
...
|---------|-------------------|-----------|------------|
| Setup/status | `setup.sh`, `maestro status`, version files, deployed agent paths | Installed versions, path health, helper availability, update posture | Later: invoke setup helpers through a dry-run/validate/apply action |
| Settings/config | `settings-helper.sh`, `config-helper.sh`, `~/.config/maestro/settings.json`, config templates | Effective settings, precedence source, validation errors | Later: validated settings changes through `settings-helper.sh set` |
| Repo registry | `~/.config/maestro/repos.json`, repo helpers, git platforms | Registered repos, parent dirs, repo health, sync status | Later: validate, back up, atomically replace, then reconcile |
...
| OpenCode | generated OpenCode config, maestro plugin, session DB/log refs | Runtime version, generated config state, session summaries | Edit upstream maestro config only; never edit generated entries directly |
...
2. User config under `~/.config/maestro/`.
...
|-------|--------------|-------------------------|---------------|
| `GET /api/status` | `setup.status.read` | `maestro status` plus version/path checks | maestro version, runtime versions, deployed path health, update posture |
...
```json
{
  ...
  "risk": "high",
  "mode": "dry_run",
  "target": {"surface": "repos", "path_ref": "~/.config/maestro/repos.json"},
  "params": {},
  "confirmation": null
}
```
...
- create a backup in the maestro config backup location;
