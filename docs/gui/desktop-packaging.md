...
open maestro from the Applications folder while the web/API contract stabilises.
...
`maestro.sh` website favicon.

For test installs without writing to `/Applications`:

```bash
bash packages/gui-desktop/scripts/install-macos-app.sh --app-dir /tmp/maestro-app-test
```
