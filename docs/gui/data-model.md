...
first implementation contract for the maestro GUI/control plane. It is a
...
- Model local-first maestro status, infrastructure inventory, provider
  bookmarks, routines, calendar/contact integrations, and machine delegation.
- Keep git platforms, maestro helpers, scheduler definitions, and provider
...
- `secret_refs`: names or IDs in maestro secrets, gopass, Vaultwarden, OS
...
A `Machine` represents local or paired compute that can run maestro, OpenCode,
...
- `capabilities`: capability labels such as `opencode`, `maestro`,
...
A `Project` maps GUI inventory to git and maestro project state.
...
A `Routine` describes scheduled or manually triggered maestro operations.
...
A `Capability` is a discoverable maestro ability surfaced by agents, services,
...
- `agent_ref`: optional maestro/OpenCode agent reference.
...
| Local maestro and OpenCode | `maestro_install`, `opencode_runtime`, `session_log`, `helper_set`, `config_file` | A local machine owns runtime resources, helper capability records, routine state, and session references. |
...
- Backend type such as `maestro`, `gopass`, `vaultwarden`, `os_keychain`,
