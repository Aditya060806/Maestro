...
export MAESTRO_HEADLESS_RUNTIME_DIR="$TEST_TMP_DIR/runtime"
...
# GH#17769: MAESTRO_HEADLESS_MODELS is deprecated but respected as override
# for backward compat. Using agentic models only (codex models removed).
export MAESTRO_HEADLESS_MODELS="anthropic/claude-sonnet-4-6,openai/gpt-5.4"
unset MAESTRO_HEADLESS_PROVIDER_ALLOWLIST
# Disable sandbox for tests — the sandbox strips env vars (STUB_*) needed
# by the opencode stub, causing test failures.
export MAESTRO_HEADLESS_SANDBOX_DISABLED=1
# Disable pattern-driven downgrades so selection-order tests stay deterministic.
unset MAESTRO_TIER_DOWNGRADE_TASK_TYPE
...
section "Allowlist"
allowlisted_model=$(MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=openai bash "$HELPER" select --role worker 2>/dev/null || true)
...
no_auth_model=$(
	...
	MAESTRO_HEADLESS_AUTH_SIGNATURE_OPENAI="" \
...
export MAESTRO_HEADLESS_AUTH_SIGNATURE_OPENAI="sig-old"
bash "$HELPER" backoff set openai auth_error 3600 >/dev/null
export MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=openai
export MAESTRO_HEADLESS_AUTH_SIGNATURE_OPENAI="sig-new"
...
rm -f "$STUB_LOG_FILE"
MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=openai bash "$HELPER" run \
	--role worker \
...
if MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=openai bash "$HELPER" run \
	--role worker \
...
MAESTRO_HEADLESS_MODELS="anthropic/claude-sonnet-4-6,anthropic/claude-opus-4-6" \
...
gateway_model=$(
	HOME="$TEST_TMP_DIR/fake-opencode-home" \
		MAESTRO_HEADLESS_MODELS="opencode/minimax-m2.5-free" \
		bash "$HELPER" select --role worker 2>/dev/null || true
)
...
no_auth_gateway=$(
	HOME="$TEST_TMP_DIR/no-auth-home" \
		...
		MAESTRO_HEADLESS_MODELS="opencode/minimax-m2.5-free,anthropic/claude-sonnet-4-6" \
		bash "$HELPER" select --role worker 2>/dev/null || true
)
...
rm -f "$STUB_LOG_FILE"
MAESTRO_HEADLESS_PROVIDER_ALLOWLIST=anthropic bash "$HELPER" run \
	--role worker \
...
config_selected=$(
	MAESTRO_HEADLESS_MODELS="openai/gpt-5.4" \
		OPENAI_API_KEY="test-key-for-provider-auth-check" \
		bash "$HELPER" select --role worker 2>/dev/null || true
)
if [[ "$config_selected" == "openai/gpt-5.4" ]]; then
	pass "Deprecated MAESTRO_HEADLESS_MODELS env var override is respected"
else
	fail "Deprecated MAESTRO_HEADLESS_MODELS env var override is respected" "got: $config_selected"
fi

section "Metrics Review Signals"
METRICS_PATH="$HOME/.maestro/logs"
