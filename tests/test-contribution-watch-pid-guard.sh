...
echo '{"initialized_repos":[],"git_parent_dirs":[]}' >"$HOME/.config/maestro/repos.json"

# Minimal state file so scan doesn't fail on missing state
echo '{"last_scan":"2026-01-01T00:00:00Z","items":{}}' >"$HOME/.maestro/cache/contribution-watch.json"

PID_FILE_PATH="$HOME/.maestro/.pid/contribution-watch.pid"
...
test_scan_without_seed_skips() {
	rm -f "$PID_FILE_PATH"
	echo '{"last_scan":"","items":{}}' >"$HOME/.maestro/cache/contribution-watch.json"
