...
export MAESTRO_MEMORY_DIR="$TEST_MEM_DIR"
...
DEPLOYED_PATTERN_HELPER="${HOME}/.maestro/agents/scripts/archived/pattern-tracker-helper.sh"
if [[ ! -x "$DEPLOYED_PATTERN_HELPER" ]]; then
	...
	# Initialize DB
	"${HOME}/.maestro/agents/scripts/memory-helper.sh" store \
		--content "test init" --type "CONTEXT" >/dev/null 2>&1 || true

	# Record 3 successful downgrades
	for _i in 1 2 3; do
		MAESTRO_MEMORY_DIR="$SAMPLE_MEM_DIR" "$DEPLOYED_PATTERN_HELPER" record-tier-downgrade-ok \
			--from-tier opus --to-tier sonnet \
			--task-type feature --quality-score 2 >/dev/null 2>&1 || true
	done

	check_out=$(MAESTRO_MEMORY_DIR="$SAMPLE_MEM_DIR" "$DEPLOYED_PATTERN_HELPER" tier-downgrade-check \
		--requested-tier opus \
		--task-type feature \
		--min-samples 3 \
		2>/dev/null) || true
...
if [[ ! -x "$DEPLOYED_PATTERN_HELPER" ]]; then
	...
	# Record 3 successful downgrades
	for _i in 1 2 3; do
		MAESTRO_MEMORY_DIR="$SAMPLE_MEM_DIR2" "$DEPLOYED_PATTERN_HELPER" record-tier-downgrade-ok \
			--from-tier opus --to-tier sonnet \
			--task-type feature --quality-score 2 >/dev/null 2>&1 || true
	done

	# Record a failure at sonnet for the same task type
	MAESTRO_MEMORY_DIR="$SAMPLE_MEM_DIR2" "$DEPLOYED_PATTERN_HELPER" record \
		--outcome failure \
		--model sonnet \
		--task-type feature \
		--description "sonnet failed on feature task" >/dev/null 2>&1 || true

	check_out=$(MAESTRO_MEMORY_DIR="$SAMPLE_MEM_DIR2" "$DEPLOYED_PATTERN_HELPER" tier-downgrade-check \
		--requested-tier opus \
		--task-type feature \
		--min-samples 3 \
		2>/dev/null) || true
...
ORIG_MEM_DIR="$MAESTRO_MEMORY_DIR"
export MAESTRO_MEMORY_DIR="/tmp/nonexistent-maestro-test-$$"
...
export MAESTRO_MEMORY_DIR="$ORIG_MEM_DIR"
