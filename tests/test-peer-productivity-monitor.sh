...
mkdir -p "$HOME/.maestro/state" "$HOME/.maestro/logs" "$HOME/.config/maestro"
...
test_login_to_var() {
	...
	result=$(_login_to_var "Aditya060806")
	if [[ "$result" == "ADITYA060806" ]]; then
		pass "Aditya060806 → ADITYA060806"
	else
		fail "Aditya060806: expected ADITYA060806, got $result"
	fi
...
test_rewrite_preserves_manual_entries() {
	local conf="$HOME/.config/maestro/dispatch-override.conf"
...
test_cli_report_no_state() {
	...
	rm -f "$HOME/.maestro/state/peer-productivity-state.json"
