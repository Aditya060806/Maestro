...
DEPLOYED_DIR="$HOME/.maestro/agents/scripts"
VERBOSE="${1:-}"
...
# ============================================================
# t138: Bound maestro update Output (PR #426)
# ============================================================
section "t138: Update Output Bounding"

# Test: git pull uses --quiet flag
if grep -qE 'git pull.*--quiet|git pull.*-q' "$REPO_DIR/maestro.sh"; then
	pass "maestro update uses --quiet git pull"
else
	fail "maestro update may produce unbounded git pull output"
fi

# Test: Commit log is filtered to meaningful changes
if grep -q 'feat|fix|refactor|perf|docs' "$REPO_DIR/maestro.sh"; then
...
if grep -q 'head -20' "$REPO_DIR/maestro.sh"; then
...
if grep -qE 'and more|full list' "$REPO_DIR/maestro.sh"; then
...
# Test: maestro.sh passes bash syntax check
if bash -n "$REPO_DIR/maestro.sh" 2>/dev/null; then
	pass "maestro.sh passes bash -n syntax check"
else
	fail "maestro.sh has syntax errors"
fi
...
if [[ -f "$HOME/.maestro/.agent-workspace/supervisor/supervisor.db" ]]; then
	task_count=$(sqlite3 "$HOME/.maestro/.agent-workspace/supervisor/supervisor.db" "SELECT COUNT(*) FROM tasks;" 2>/dev/null || echo "0")
...
if [[ -d "$HOME/.maestro/.agent-workspace/supervisor/pids" ]]; then
	stale_pids=0
	live_pids=0
	for pid_file in "$HOME/.maestro/.agent-workspace/supervisor/pids"/*.pid; do
		[[ -f "$pid_file" ]] || continue
		pid=$(cat "$pid_file")
		task=$(basename "$pid_file" .pid)
		status=$(sqlite3 "$HOME/.maestro/.agent-workspace/supervisor/supervisor.db" \
