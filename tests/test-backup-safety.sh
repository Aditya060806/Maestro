...
export MAESTRO_SUPERVISOR_DIR="$TEMP_DIR/supervisor"
mkdir -p "$MAESTRO_SUPERVISOR_DIR"
sup_db="$MAESTRO_SUPERVISOR_DIR/supervisor.db"
...
sup_backup_count=$(find "$MAESTRO_SUPERVISOR_DIR" -maxdepth 1 -type f -name 'supervisor-backup-*.db' | wc -l | tr -d ' ')
