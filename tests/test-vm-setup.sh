...
#   bash tests/test-vm-setup.sh --update           # Also test maestro update
...
info "Cloning maestro repo (branch: $TEST_BRANCH)..."
vm_run "git clone --branch '$TEST_BRANCH' --single-branch https://github.com/Aditya060806/Maestro.git ~/Git/maestro 2>&1" >/dev/null

vm_test "Repo cloned" "test -f ~/Git/maestro/setup.sh && echo exists" "exists"

# Run setup.sh non-interactive
info "Running setup.sh --non-interactive..."
setup_output=""
setup_status=0
if setup_output=$(vm_run "cd ~/Git/maestro && bash setup.sh --non-interactive 2>&1"); then
	setup_status=0
else
	setup_status=$?
fi
...
# Agents deployed
vm_test "Agents deployed to ~/.maestro/agents/" \
	"test -d ~/.maestro/agents && ls ~/.maestro/agents/*.md 2>/dev/null | wc -l" \
	"[1-9]"

# AGENTS.md exists
vm_test "AGENTS.md deployed" \
	"test -f ~/.maestro/agents/AGENTS.md && echo exists" \
	"exists"

# Onboarding agent exists at correct path
vm_test "onboarding.md at correct path" \
	"test -f ~/.maestro/agents/maestro/onboarding.md && echo exists" \
	"exists"
...
if grep -q "yes" <<<"$(vm_run "test -f ~/.config/opencode/command/onboarding.md && echo yes")"; then
	vm_test "/onboarding references correct path" \
		"grep 'maestro/onboarding.md' ~/.config/opencode/command/onboarding.md" \
		"maestro/onboarding.md"
fi

# Scripts deployed
vm_test "Scripts deployed" \
	"test -x ~/.maestro/agents/scripts/pre-edit-check.sh && echo exists" \
	"exists"

# maestro CLI installed
vm_test "maestro CLI available" \
	"command -v maestro && echo found || echo 'not in PATH'" \
	"found|not in PATH"

# VERSION file
vm_test "VERSION file deployed" \
	"test -f ~/.maestro/agents/VERSION && cat ~/.maestro/agents/VERSION" \
	"[0-9]+\.[0-9]+\.[0-9]+"
...
if [[ "$TEST_UPDATE" == "true" ]]; then
	section "Testing maestro update"

	info "Running setup.sh --non-interactive again (simulates update)..."
	update_output=""
	update_status=0
	if update_output=$(vm_run "cd ~/Git/maestro && git pull origin '$TEST_BRANCH' 2>/dev/null && bash setup.sh --non-interactive 2>&1"); then
		update_status=0
	else
		update_status=$?
	fi
	...
	vm_test "Agents still deployed after update" \
		"test -d ~/.maestro/agents && echo exists" \
		"exists"
