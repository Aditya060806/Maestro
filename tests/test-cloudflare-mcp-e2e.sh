...
if grep -q "cloudflare-api" "$AGENTS_DIR/maestro/mcp-integrations.md" 2>/dev/null; then
...
if grep -q "mcp.cloudflare.com/mcp" "$AGENTS_DIR/maestro/mcp-integrations.md" 2>/dev/null; then
...
if command -v gopass &>/dev/null; then
	...
	if gopass show -o maestro/CLOUDFLARE_ADITYA060806_GLOBAL_ACCOUNT_ID &>/dev/null; then
		acct_len=$(gopass show -o maestro/CLOUDFLARE_ADITYA060806_GLOBAL_ACCOUNT_ID 2>/dev/null | wc -c | tr -d ' ')
	...
	if gopass show -o maestro/CLOUDFLARE_ADITYA060806_GLOBAL_KEY &>/dev/null; then
		key_len=$(gopass show -o maestro/CLOUDFLARE_ADITYA060806_GLOBAL_KEY 2>/dev/null | wc -c | tr -d ' ')
