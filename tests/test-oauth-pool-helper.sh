...
run_test_expired_cooldown_auto_clear() {
	...
	mkdir -p "$test_home/.maestro"
	local now_ms
	now_ms=$(python3 -c "import time; print(int(time.time() * 1000))")
	local expired_ms
	expired_ms=$((now_ms - 60000))

	python3 - "$test_home/.maestro/oauth-pool.json" "$expired_ms" <<'PY'
import json
import sys
...
PY
	...
	local status_after
	status_after=$(jq -r '.openai[0].status' "$test_home/.maestro/oauth-pool.json")
	local cooldown_after
	cooldown_after=$(jq -r '.openai[0].cooldownUntil' "$test_home/.maestro/oauth-pool.json")
...
run_test_set_priority() {
	...
	python3 - "$test_home/.maestro/oauth-pool.json" "$now_ms" <<'PY'
import json, sys
path = sys.argv[1]
...
PY
	...
	local priority_val
	priority_val=$(jq -r '.anthropic[] | select(.email=="work@example.com") | .priority' "$test_home/.maestro/oauth-pool.json")
	...
	priority_after_clear=$(jq -r '.anthropic[] | select(.email=="work@example.com") | .priority // "null"' "$test_home/.maestro/oauth-pool.json")
...
PY
	...
	local sort_result
	sort_result=$(
		python3 - "$test_home/.maestro/oauth-pool.json" <<'PY'
import json, sys
