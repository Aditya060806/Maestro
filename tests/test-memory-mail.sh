...
TEST_DIR=$(mktemp -d)
export MAESTRO_MEMORY_DIR="$TEST_DIR/memory"
export MAESTRO_MAIL_DIR="$TEST_DIR/mail"
...
mem_db() {
	sqlite3 -cmd ".timeout 5000" "$MAESTRO_MEMORY_DIR/memory.db" "$@"
}

# Helper: query mail DB
mail_db() {
	sqlite3 -cmd ".timeout 5000" "$MAESTRO_MAIL_DIR/mailbox.db" "$@"
}
...
if [[ -f "$MAESTRO_MEMORY_DIR/memory.db" ]]; then
...
if echo "$ns_output" | grep -qi "stored\|ok\|success"; then
	...
	if [[ -d "$MAESTRO_MEMORY_DIR/namespaces/test-runner" ]]; then
...
rm -f "$MAESTRO_MEMORY_DIR/.last_auto_prune"
...
if [[ -f "$MAESTRO_MEMORY_DIR/.last_auto_prune" ]]; then
...
if [[ -f "$MAESTRO_MAIL_DIR/mailbox.db" ]]; then
...
mkdir -p "$MAESTRO_MEMORY_DIR"
echo "provider=openai" >"$MAESTRO_MEMORY_DIR/.embeddings-config"
echo "configured_at=2025-01-01T00:00:00Z" >>"$MAESTRO_MEMORY_DIR/.embeddings-config"
...
echo "provider=local" >"$MAESTRO_MEMORY_DIR/.embeddings-config"
...
rm -f "$MAESTRO_MEMORY_DIR/embeddings.db"
...
rm -f "$MAESTRO_MEMORY_DIR/.embeddings-config"
...
config_provider=$(grep '^provider=' "$MAESTRO_MEMORY_DIR/.embeddings-config" | cut -d= -f2)
...
config_date=$(grep '^configured_at=' "$MAESTRO_MEMORY_DIR/.embeddings-config" | cut -d= -f2)
