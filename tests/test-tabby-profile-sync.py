...
``wpallstars.com-chore-maestro-init``).
"""
...
"""
  ...
  - name: foo
    options:
      cwd: >-
        /Users/Aditya060806/Git/wordpress/wp-plugin-starter-template-for-ai-coding
    color: '#DA5CD3'
"""
        )
        self.assertIn(
...
class TestIsLinkedWorktree(unittest.TestCase):
    ...
    def test_worktree_with_dot_in_repo_name_is_detected(self):
        ...
        ``wpallstars.com`` worktree named ``wpallstars.com-chore-maestro-init``
        was not caught by the old heuristic because splitting on the first
        dot yielded ``com-chore-maestro-init``, which does not start with
        ...
        self._git("init", "-q", "-b", "main", cwd=repo)
        self._git("commit", "--allow-empty", "-q", "-m", "init", cwd=repo)
        wt = self.tmp / "wpallstars.com-chore-maestro-init"
        self._git(
            "worktree", "add", "-q",
            str(wt), "-b", "chore/maestro-init",
            cwd=repo,
        )
        self.assertTrue(tabby_profile_sync.is_linked_worktree(str(wt)))
