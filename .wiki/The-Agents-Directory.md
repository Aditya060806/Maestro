...
ls ~/git/maestro/.agents/scripts/

# Get help for any script
bash ~/git/maestro/.agents/scripts/hostinger-helper.sh help

# Common pattern: service helper with command
bash ~/git/maestro/.agents/scripts/[service]-helper.sh [command] [account] [target]
```text
...
**Note:** Actual memory files are stored in `~/.maestro/.agent-workspace/memory/` (outside Git).
