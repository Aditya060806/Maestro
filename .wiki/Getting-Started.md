...
This guide helps you set up and start using the Maestro Framework.
...
- Installs the `maestro` CLI command globally
...
After installation, you have the `maestro` command available globally:

```bash
maestro status      # Check installation status
maestro update      # Update to latest version
maestro uninstall   # Remove from system
...
**Keys are stored securely in:** `~/.config/maestro/mcp-env.sh`

## Directory Structure

```text
~/Git/maestro/              # Repository location
├── AGENTS.md                # AI assistant instructions
├── maestro.sh              # CLI source script
...
~/.maestro/                 # User installation
├── agents/                  # Deployed agent files
│   └── AGENTS.md            # User guide
└── backups/                 # Configuration backups

/usr/local/bin/maestro      # CLI command (or ~/.local/bin/)
```
...
1. **[CLI Reference](CLI-Reference)** - Master the `maestro` command
...
```bash
ls ~/Git/maestro/AGENTS.md
```

### Scripts Not Executable

```bash
chmod +x ~/Git/maestro/.agents/scripts/*.sh
```
...
source ~/.config/maestro/mcp-env.sh
env | grep -i api
```
