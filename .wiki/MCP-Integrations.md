...
Complete guide to Model Context Protocol (MCP) integrations in the Maestro Framework.
...
| Location | Purpose |
|----------|---------|
| `~/.config/maestro/` | **Secrets only** - `mcp-env.sh` (600 perms) |
| `~/.maestro/` | **Working directories** - agno, stagehand, reports |
...
Keys are automatically available in all shells via `~/.config/maestro/mcp-env.sh`:
