...
[![GitHub Actions](https://github.com/Aditya060806/Maestro/actions/workflows/code-quality.yml/badge.svg?branch=main)](https://github.com/Aditya060806/Maestro/actions/workflows/code-quality.yml)
[![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=Aditya060806_maestro&metric=alert_status)](https://sonarcloud.io/summary/new_code?id=Aditya060806_maestro)
[![CodeFactor](https://www.codefactor.io/repository/github/Aditya060806/Maestro/badge)](https://www.codefactor.io/repository/github/Aditya060806/Maestro)
[![Maintainability](https://qlty.sh/gh/Aditya060806/projects/maestro/maintainability.svg)](https://qlty.sh/gh/Aditya060806/projects/maestro)
[![Codacy Badge](https://app.codacy.com/project/badge/Grade/2b1adbd66c454dae92234341e801b984)](https://app.codacy.com/gh/Aditya060806/Maestro/dashboard?utm_source=gh&utm_medium=referral&utm_content=&utm_campaign=Badge_grade)
...
[![Version](https://img.shields.io/badge/Version-3.31.53-blue.svg)](https://github.com/Aditya060806/Maestro/releases)
[![npm version](https://img.shields.io/npm/v/maestro)](https://www.npmjs.com/package/maestro)
...
[![AI Optimized](https://img.shields.io/badge/AI%20Optimized-Yes-brightgreen.svg)](https://github.com/Aditya060806/Maestro/blob/main/AGENTS.md)
...
maestro is MIT licensed. Reuse is welcome, including commercial use, provided
...
- **Install**: `npm install -g maestro && maestro update`
...
- `maestro init` - Initialize in any project
- `maestro update` - Update framework
- `maestro auto-update` - Automatic update polling (enable/disable/status)
- `maestro secret` - Manage secrets (gopass encrypted, AI-safe)
- `maestro security` - Full security assessment (posture, secrets, supply chain)
- `maestro metrics generate` - Generate local LOC, language, and dependency data for README badges or app about pages
...
### What You Can Ask maestro To Do
...
Since the last README feature refresh, maestro has added or expanded:
...
**Vault security model:** maestro defines protected data classes, provider
...
**Vault passphrase warning:** maestro cannot recover a lost Vault passphrase.
...
```bash
maestro security              # Run ALL checks (posture + hygiene + supply chain)
maestro security posture      # Interactive security posture setup (gopass, gh auth, SSH)
maestro security status       # Combined posture + hygiene summary
maestro security scan         # Secret hygiene & supply chain scan only
maestro security scan-pth     # Python .pth file audit (supply chain attack vector)
maestro security scan-secrets # Plaintext credential locations only
maestro security scan-deps    # Unpinned dependency check
maestro security check        # Per-repo security posture assessment
maestro security dismiss <id> # Dismiss a security advisory after taking action
```

Running `maestro security` with no arguments is the single command that covers everything — user security posture, plaintext secret detection, supply chain IoC scanning, and active advisories.

**Security advisories** are delivered via `maestro update` and shown in the session greeting until dismissed. The scanner never exposes secret values — only file locations and key names. All remediation commands should be run in a separate terminal, not inside AI chat sessions.
...
```bash
bun install -g maestro && maestro update
```
...
**Direct from source** (maestro.sh):

```bash
bash <(curl -fsSL https://maestro.sh/install)
```

**Manual** (git clone):

```bash
git clone https://github.com/Aditya060806/Maestro.git ~/Git/maestro
~/Git/maestro/setup.sh
```

**That's it!** The setup script will:
- Clone/update the repo to `~/Git/maestro`
- Deploy agents to `~/.maestro/agents/`
- Install the `maestro` CLI command
...
**New users: Start [OpenCode](https://opencode.ai/) and type `/onboarding`** to configure your services interactively. OpenCode is the recommended tool for maestro; pair it with OpenAI GPT-5.5 and GPT-5.4 mini for the best current results across agent tiers. The onboarding wizard will:
- Explain what **[maestro](https://maestro.sh)** can do
...
maestro doctor           # Detect duplicate installs and PATH conflicts
maestro update           # Update framework + check registered projects
maestro auto-update      # Manage automatic update polling (every 10 min)
maestro init             # Initialize maestro in any project
maestro features         # List available features
maestro repos            # List/add/remove registered projects
maestro design           # DESIGN.md detection + brand guideline HTML/PDF exports
maestro detect           # Scan for unregistered maestro projects
maestro upgrade-planning # Upgrade TODO.md/PLANS.md to latest templates
maestro update-tools     # Check and update installed tools
maestro uninstall        # Remove maestro
```

### Optional Design Artifact Studio

maestro now treats design as a self-contained stack with optional peripherals:
...
- **Init-aware design files**: `maestro init` records `has_interface` and seeds root `DESIGN.md` for standard repos plus minimal-scope repos with detected GUI/interface markers.
- **Brand guideline exports**: `maestro design guidelines . --pdf` generates `_reports/brand-guidelines/brand-guidelines.md`, HTML, and A4/US Letter/slides PDFs from `DESIGN.md`.
- **Repo rollout**: `maestro design survey --json` audits owned initialized GUI repos; `maestro design issues --apply` files worker-ready auto-dispatch issues for missing `DESIGN.md`/brand-guideline artifacts.
...
- **Artifact routing commands**: `/design-artifact` decides whether to use maestro-native implementation or a companion artifact studio; `/open-design` manages optional Open Design workflows.
...
Optional companion: [Open Design](https://github.com/nexu-io/open-design) by nexu-io (Apache-2.0) is supported as a **peripheral** for live sandboxed previews, design-skill pickers, `.od/` artifact workspaces, and HTML/PDF/PPTX/ZIP-style exports. maestro remains canonical for agents, skill ingestion, Google `DESIGN.md`, local hosting, and verification.
...
# Install alongside maestro only after opting in
open-design-helper.sh install --execute

# Start through maestro local HTTPS if Open Design only prints localhost
open-design-helper.sh start --https-local open-design
# → https://open-design.local when localdev is configured
```

Imported Open Design skills are not copied verbatim. They are evaluated through maestro build-agent methodology, deduplicated against existing agents, flattened into maestro `*-skill.md` structure, attributed to upstream, and given verification commands. See `.agents/tools/design/open-design-ingestion.md` for the full skill-value matrix.

**Project tracking:** When you run `maestro init`, the project is automatically registered in `~/.config/maestro/repos.json`. Running `maestro update` checks all registered projects for version updates.

### **Use maestro in Any Project**

Initialize **[maestro](https://maestro.sh)** features in any git repository:

```bash
cd ~/your-project
maestro init                         # Enable all features
maestro init planning                # Enable only planning
maestro init planning,time-tracking  # Enable specific features
```

This creates:
- `.maestro.json` - Configuration with enabled features
- `.agents` symlink → `~/.maestro/agents/`
...
After `maestro init` registers a new repo, run `/setup-git` in your AI assistant
...
Run `/setup-git` again whenever you register a new repo with `maestro repos add`
...
When maestro templates evolve, upgrade existing projects to the latest format:

```bash
maestro upgrade-planning           # Interactive upgrade with backup
maestro upgrade-planning --dry-run # Preview changes without modifying
maestro upgrade-planning --force   # Skip confirmation prompt
```

This preserves your existing tasks while adding TOON-enhanced parsing, dependency tracking, and better structure.

**Automatic detection:** `maestro update` now scans all registered projects for outdated planning templates (comparing TOON meta version numbers) and offers to upgrade them in-place with backups.
...
```bash
maestro init beads              # Enable beads (includes planning)
```
...
**Architecture:** **[maestro](https://maestro.sh)** markdown files (TODO.md, PLANS.md) are the source of truth. Beads syncs from them for visualization.
...
OpenCode with OpenAI is the current recommended maestro setup. Use GPT-5.5 for complex reasoning, architecture, security-sensitive review, and hard agent tiers; use GPT-5.4 mini for fast triage, routine implementation, retries, and lower-cost worker throughput.
...
```bash
maestro model-accounts-pool add anthropic
...
maestro model-accounts-pool add google
...
- **OpenAI GPT-5.5 / GPT-5.4 mini** - Recommended model pair for maestro today. Use GPT-5.5 for complex reasoning and high-impact agent tiers; use GPT-5.4 mini for triage, routine implementation, and cost-efficient parallel workers.
...
```bash
maestro model-accounts-pool status       # counts: available / rate-limited / auth-error
maestro model-accounts-pool check        # live token validity test per account
```
...
| Symptom | Command |
|---------|---------|
| OpenAI account shows `rate-limited` | `maestro model-accounts-pool rotate openai` |
| Anthropic account shows `rate-limited` | `maestro model-accounts-pool rotate anthropic` |
| All accounts in cooldown | `maestro model-accounts-pool reset-cooldowns` |
| OpenAI account shows `auth-error` | `maestro model-accounts-pool add openai` (re-auth) |
| Anthropic account shows `auth-error` | `maestro model-accounts-pool add anthropic` (re-auth) |
| Pool is empty (no accounts) | `maestro model-accounts-pool add openai` |
| Recently re-authenticated, still broken | `maestro model-accounts-pool assign-pending openai` |
| Google Gemini CLI rate-limited | `maestro model-accounts-pool rotate google` |
| Google token expired | `maestro model-accounts-pool add google` (re-auth) |

**Step 3 — If still broken, re-add the account**

```bash
maestro model-accounts-pool add openai        # ChatGPT Plus/Pro
maestro model-accounts-pool add anthropic     # Claude Pro/Max — opens browser OAuth
maestro model-accounts-pool add cursor        # Cursor Pro (reads from local IDE)
maestro model-accounts-pool add google        # Google AI Pro/Ultra/Workspace — browser OAuth
maestro model-accounts-pool import claude-cli # Import from existing Claude CLI auth
```
...
```bash
maestro model-accounts-pool status            # Pool health at a glance
maestro model-accounts-pool list              # Per-account detail + expiry
maestro model-accounts-pool check             # Live API validity test
maestro model-accounts-pool rotate [provider] # Switch to next available account NOW
maestro model-accounts-pool reset-cooldowns   # Clear all rate-limit cooldowns
maestro model-accounts-pool assign-pending <p># Assign stranded pending token
maestro model-accounts-pool remove <p> <email># Remove an account
```
...
**Collaborator compatibility:** Projects initialized with `maestro init` include pointer files (`.cursorrules`, `.windsurfrules`, etc.) that reference `AGENTS.md`, helping collaborators using other editors find project context. maestro does not install into or configure those tools.

**Repo courtesy files:** `maestro init` scaffolds standard repo files if they don't exist: `DESIGN.md` for GUI/interface repos, `README.md`, `LICENCE` (MIT), `CHANGELOG.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`. Author name and email are auto-detected from git config. Existing files are never overwritten.
...
- **GUI control plane planning** - Local-first product scope, stack and repo-layout ADRs, GUI trust boundaries, and threat model for future maestro UI surfaces
...
- **Review gate preferences**: choose whether true review-bot rate limits block merges (`maestro review-gate owner/repo wait`) or allow merge with follow-up quality coverage (`maestro review-gate owner/repo pass`, the default). Per-tool overrides are supported, for example `maestro review-gate owner/repo --tool coderabbitai wait`. Failed, skipped, or placeholder bot states are not treated as rate limits and continue to block until a real review/status appears or a human resolves them.
...
maestro includes curated skills imported from external sources. Skills support automatic update tracking:
...
```bash
maestro skill add <owner/repo>    # Import a skill from GitHub
maestro skill add clawdhub:<slug> # Import a skill from ClawdHub
maestro skill list                # List imported skills
maestro skill check               # Check for upstream updates
maestro skill update [name]       # Update specific or all skills
maestro skill scan [name]         # Security scan skills (Cisco Skill Scanner)
maestro skill remove <name>       # Remove an imported skill
```

Skills are registered in `~/.maestro/agents/configs/skill-sources.json` with upstream tracking for update detection.
...
When a [VirusTotal](https://www.virustotal.com/) API key is configured (`maestro secret set VIRUSTOTAL_ADITYA060806`), an advisory second layer scans file hashes against 70+ AV engines and checks domains/URLs referenced in skill content. VT scans are non-blocking -- the Cisco scanner remains the security gate.

| Scenario | Security scan runs? | CRITICAL/HIGH blocks? |
|----------|--------------------|-----------------------|
| `maestro skill add <source>` | Yes | Yes |
| `maestro skill update [name]` | Yes | Yes |
| `maestro skill add <source> --force` | Yes | Yes |
| `maestro skill add <source> --skip-security` | Yes (reports only) | No (warns) |
| `maestro skill scan [name]` | Yes (standalone) | Report only |
...
- [vercel-labs/add-skill](https://github.com/vercel-labs/add-skill) - The upstream CLI tool (maestro uses its own implementation)
...
Sync agents from private Git repositories into the framework. Private repos keep their own agents, helper scripts, and slash commands — `maestro sources sync` deploys them alongside the core agents.

```bash
maestro sources add ~/Git/my-private-agents     # Register a local repo
maestro sources add-remote git@github.com:u/r.git  # Clone and register a remote repo
maestro sources list                             # List configured sources
maestro sources sync                             # Sync all sources
maestro sources remove my-private-agents         # Remove a source
```

**How it works:** Private repos contain a `.agents/` directory with agent subdirectories. Agents with `mode: primary` in their frontmatter are symlinked to the agents root for auto-discovery as primary agent tabs. Markdown files with `agent:` frontmatter are deployed as `/slash` commands. All sources sync automatically during `maestro update`.

**Reference:** `.agents/maestro/agent-sources.md`

## **Agent Design Patterns**

maestro implements proven agent design patterns identified by [Lance Martin (LangChain)](https://x.com/RLanceMartin/status/2009683038272401719).

| Pattern | Description | maestro Implementation |
|---------|-------------|------------------------|
| **Give Agents a Computer** | Filesystem + shell for persistent context | `~/.maestro/.agent-workspace/`, 1,480+ helper scripts |
...
| **Token-Efficient Serialisation** | Minimise context overhead for structured data | [TOON format](https://github.com/Aditya060806/Maestro/blob/main/.agents/toon-format.md) — 20-60% token reduction vs JSON/YAML for agent indexes, registries, and data exchange |
...
**Key insight**: Context is a finite resource with diminishing returns. maestro treats every token as precious - loading only what's needed, when it's needed.

See `.agents/maestro/architecture.md` for detailed implementation notes and references.
...
**Compaction plugin** (`.agents/plugins/opencode-maestro/`): When OpenCode compacts context (at ~200K tokens), the plugin injects current session state - agent registry, pending mailbox messages, git context, and relevant memories - ensuring continuity across compaction boundaries.

**Custom system prompt** (`.agents/prompts/build.txt`): Based on upstream OpenCode with maestro-specific overrides for tool preferences, professional objectivity, and per-model reinforcements for weaker models.
...
The pulse is the heartbeat of maestro — an autonomous AI supervisor that runs every 2 minutes via launchd. There is no human at the terminal. It manages the entire development pipeline across all repos registered with `pulse: true`.
...
**Resolution priority:** Explicit `bundle` field in `repos.json` > `.maestro.json` project config > auto-detection from marker files.
...
0 9 * * * ~/.maestro/agents/scripts/runner-helper.sh run "seo-analyst" "Generate daily SEO report"
```
...
maestro itself is lightweight (shell scripts + markdown), but AI model workloads benefit from capable hardware:
...
**Note:** Most maestro features (infrastructure management, SEO, code quality, Git workflows) require no GPU. GPU is only needed for local AI model inference (voice pipeline, vision models, local LLMs).
...
- **Google `DESIGN.md` standard**: Canonical AI-readable design systems with YAML tokens, Markdown rationale, linting, Tailwind/DTCG export, and preview generation. maestro keeps `DESIGN.md` as the source of truth for UI agents.
...
- **[Open Design](https://github.com/nexu-io/open-design)** *Optional peripheral*: Local-first design artifact studio by nexu-io (Apache-2.0) for sandboxed previews, design-skill pickers, `.od/` workspaces, and exports. It installs alongside maestro only when requested; selected skills are ingested via maestro build-agent optimisation, not imported verbatim.
...
- **[gopass](https://github.com/gopasspw/gopass)**: GPG-encrypted secret management with AI-native wrapper (`maestro secret`) - subprocess injection + output redaction keeps secrets out of AI context
...
- **[TOON Format](https://github.com/Aditya060806/Maestro/blob/main/.agents/toon-format.md)**: Token-Oriented Object Notation - 20-60% token reduction for LLM prompts
...
MCP packages are installed globally via `bun install -g` for instant startup (no `npx` registry lookups). Run `setup.sh` or `maestro update-tools` to update to latest versions.
...
| [Amazon Order History](https://github.com/Aditya060806/amazon-order-history-csv-download-mcp) | Order data extraction | Per-agent | No |
...
| [QuickFile](https://github.com/Aditya060806/quickfile-mcp) | Accounting API | Per-agent | Yes |
...
- **Auto-browse workflow** - `/auto-browse` orchestrates the tools below to learn messy browser tasks, choose the cheapest reliable path, preserve private profile/session state under `~/.maestro/`, and graduate reusable private agents or sanitized `todo/` plans
...
- [QuickFile](https://github.com/Aditya060806/quickfile-mcp) - Accounting API integration (MCP)
- [Amazon Order History](https://github.com/Aditya060806/amazon-order-history-csv-download-mcp) - Order data extraction (MCP)
...
OpenCode includes [built-in LSP servers](https://opencode.ai/docs/lsp/) for 35+ languages. For maestro projects that use Markdown and TOON extensively, add these optional LSP servers to your `opencode.json` for real-time diagnostics during editing:
...
For repeatable browser operations and web data mining, use `/auto-browse`. It runs an intake and learning loop that starts with cheap fetch/API/crawler options, escalates to deterministic or high-agency browser tools only when needed, and then graduates the workflow into a private custom agent, helper, schema, or sanitized `todo/` plan. Account-specific workflows, cookies, profile state, downloads, and traces stay in private maestro user data; only generalized plans should be committed to the repo.
...
maestro implements the [Agent Skills](https://agentskills.io/) standard for cross-tool compatibility. Skills are auto-discovered by compatible AI assistants.

**Generated SKILL.md files** in `~/.maestro/agents/` provide skill metadata following the [Agent Skills standard](https://agentskills.io/specification). These are discoverable by any compatible tool.

### Claude Code Plugin Marketplace

maestro is registered as a **Claude Code plugin marketplace**. Install with two commands:

```bash
/plugin marketplace add Aditya060806/Maestro
...
Import skills from GitHub or ClawdHub using the `maestro skill` CLI:

```bash
# Import from GitHub (auto-detects format)
maestro skill add owner/repo

# Import from ClawdHub (skill registry with vector search)
maestro skill add clawdhub:caldav-calendar
maestro skill add https://clawdhub.com/owner/slug

# More examples
maestro skill add anthropics/skills/pdf           # Specific skill from multi-skill repo
maestro skill add vercel-labs/agent-skills         # All skills from a repo
maestro skill add expo/skills --name expo-dev      # Custom name
maestro skill add owner/repo --dry-run             # Preview without changes
```
...
- Upstream commit tracking for update detection (`maestro skill check`)
- Conversion to maestro subagent format with YAML frontmatter
...
| | `maestro skill add` | `npx add-skill` |
|---|---|---|
| **Target** | Converts to maestro format in `.agents/` | Copies SKILL.md to agent-specific dirs |
...
| **Updates** | `maestro skill check` (GitHub API) | `npx skills check` (Vercel API) |
...
| Maestro | `maestro.md` | Framework development — meta-agent for improving maestro itself | opus |
...
| **Claude Code / Codex / Cursor / Droid / Kiro / Continue / Kimi / Qwen / Amp / Windsurf / Gemini CLI** | Slash command, namespaced with the `maestro-` prefix: `/maestro-build-plus`, `/maestro-automate`, `/maestro-seo`, etc. |
| **Aider** | No native slash command support — use a shell alias: `alias aider-build='aider --message-file ~/.maestro/agents/build-plus.md'`. |

The `maestro-` prefix differentiates framework commands from each client's native slash commands and groups them alphabetically in the command picker. It applies to **every** maestro slash command — not just main agents — so `/maestro-preflight`, `/maestro-release`, `/maestro-commit` etc. all sort together in your `/` menu.
...
| Codex CLI | ✅ `~/.codex/prompts/` | — | ✅ default | Invoked as `/prompts:maestro-<name>` |
...
| Windsurf | repo-local `.windsurf/workflows/` | — | ❌ (protobuf) | Symlinked by `maestro init` |
...
```bash
MAESTRO_FEATURE_MEMORY_CLAUDE_CODE=no setup.sh
MAESTRO_FEATURE_COMMANDS_CURSOR=no setup.sh
```
...
```bash
# Install maestro agents for OpenCode
...
maestro is available in the Claude marketplace:

```bash
/plugin marketplace add Aditya060806/Maestro
/plugin install maestro-all@maestro
```

**Agent Skills (SKILL.md):** Auto-discovered from `~/.maestro/agents/` after running `setup.sh`. Compatible with any tool that supports the [Agent Skills standard](https://agentskills.io/specification).
...
5. **Optionally compose a PR** to contribute improvements back to maestro
...
The agent will create a safe linked worktree, apply changes, and submit a PR to `Aditya060806/Maestro` with a structured description. Your real-world usage helps improve the framework for everyone.
...
Configure time tracking per-repo via `.maestro.json`.
...
| `/log-issue-maestro` | Report issues with maestro (gathers diagnostics, checks duplicates, creates GitHub issue) |
...
```bash
~/.maestro/agents/scripts/worktree-helper.sh add feature/my-feature
# Creates: ~/Git/{repo}-feature-my-feature/ (cd there manually)
~/.maestro/agents/scripts/worktree-helper.sh list
~/.maestro/agents/scripts/worktree-helper.sh clean
```
...
# Or fallback:
# ~/.maestro/agents/scripts/worktree-helper.sh add feature/next-feature
...
```bash
# Store a memory
~/.maestro/agents/scripts/memory-helper.sh store --type "WORKING_SOLUTION" --content "Fixed CORS with nginx headers" --tags "cors,nginx"

# Store auto-captured memory (from AI agent)
~/.maestro/agents/scripts/memory-helper.sh store --auto --content "Fixed CORS with nginx headers" --type WORKING_SOLUTION

# Recall memories (keyword search - default)
~/.maestro/agents/scripts/memory-helper.sh recall "cors"

# Recall memories (semantic similarity - opt-in)
~/.maestro/agents/scripts/memory-helper.sh recall "cors" --semantic

# Show auto-capture log
~/.maestro/agents/scripts/memory-helper.sh log

# View statistics (includes auto-capture counts)
~/.maestro/agents/scripts/memory-helper.sh stats

# Maintenance
~/.maestro/agents/scripts/memory-helper.sh validate   # Check for stale entries
~/.maestro/agents/scripts/memory-helper.sh prune      # Remove stale memories

# Namespaces (per-runner memory isolation)
~/.maestro/agents/scripts/memory-helper.sh --namespace my-runner store --content "Runner learning"
~/.maestro/agents/scripts/memory-helper.sh --namespace my-runner recall "query" --shared
~/.maestro/agents/scripts/memory-helper.sh namespaces  # List all namespaces
```

**Storage:** `~/.maestro/.agent-workspace/memory/memory.db` (+ optional `embeddings.db` for semantic search, `namespaces/` for per-runner isolation)

#### Session mining (cross-client)

Beyond explicit `/remember` calls, maestro can harvest structured session data from each supported AI client's on-disk conversation history. This turns every prior session — regardless of which tool you were using — into searchable context for future sessions.
...
```bash
MAESTRO_FEATURE_MEMORY_GEMINI_CLI=yes setup.sh
MAESTRO_FEATURE_MEMORY_CONTINUE=yes setup.sh
```
...
User-created agents survive `maestro update`. Agents progress through tiers as they mature:

| Tier | Location | Purpose | Survives Update |
|------|----------|---------|-----------------|
| **Draft** | `~/.maestro/agents/draft/` | R&D, experimental, auto-created by orchestration tasks | Yes |
| **Custom** | `~/.maestro/agents/custom/` | User's permanent private agents | Yes |
...
- **[CLI Reference](.wiki/CLI-Reference.md)** - maestro command documentation
...
- **[API Integrations](.agents/maestro/api-integrations.md)** - Service APIs
...
- **[Security](.agents/maestro/security.md)** - Enterprise security standards
...
│   ├── AGENTS.md                  # User guide (deployed to ~/.maestro/agents/)
...
maestro is configured through two JSONC/JSON files that control framework behaviour, model routing, quality gates, and user preferences. Both are optional -- sensible defaults apply out of the box.

| File | Purpose | CLI |
|------|---------|-----|
| [`~/.config/maestro/config.jsonc`](.agents/reference/configuration.md#configjsonc--full-reference) | Framework config: updates, models, safety, quality, orchestration, paths | `maestro config` |
| [`~/.config/maestro/settings.json`](.agents/reference/configuration.md#settingsjson--full-reference) | User preferences: onboarding, UI, model routing defaults | `settings-helper.sh` |

**Quick examples:**

```bash
# Disable automatic updates
maestro config set updates.auto_update false

# Use opus-tier verification for destructive operations
maestro config set safety.verification_tier opus
...
**Precedence:** Environment variables (`MAESTRO_*`) > config file > built-in defaults. See the **[full configuration reference](.agents/reference/configuration.md)** for every option, type, default, and example.
...
echo "client-acme" > .maestro-tenant

# Export for scripts
source <(credential-helper.sh export)
```

**Resolution priority:** Project `.maestro-tenant` → Global active tenant → Default
...
**Created by Aditya Pandey and Harvest** - Copyright © 2025-2026
...
[![Star History Chart](https://api.star-history.com/svg?repos=Aditya060806/Maestro&type=Date)](https://star-history.com/#Aditya060806/Maestro&Date)
