...
3. **Plugin decomposition** (t1860, t1914) — decompose the opencode-maestro plugin directory. ~12.5h. Blocked by phase 2 for oauth-pool.mjs.
...
- [ ] t1914: Full opencode-maestro plugin directory (~1300 combined) ~7h (blocked-by:t1860)
...
**Logged:** 2026-04-03
**Trigger:** Analysis of [imbue-ai/mngr](https://github.com/imbue-ai/mngr/) (MIT-licensed agent process manager) identified 4 adoptable patterns for maestro. Three are quality/pipeline improvements; one is strategic research for the SaaS agent hosting roadmap.

#### Purpose

Adopt proven patterns from mngr's architecture to improve maestro in two dimensions:
...
2. **Strategic research** (t1879) -- evaluate mngr's tmux/provider/idle-detection architecture as a design reference for maestro SaaS agent hosting, where users need AI agents running in secure containers.
...
- mngr and maestro are complementary, not competing: mngr manages agent *processes*; maestro manages agent *intelligence and workflow*. mngr could be a provider backend for maestro headless dispatch.
...
**Logged:** 2026-03-31
**Trigger:** User wants an maestro-owned `chromium-debug-use` agent/skill for inspecting what is already open in a Chromium-based browser, teaching the user how to enable the required debugging path for that browser, and using the live session as a fast discovery step before formal automation planning.
...
| Date | Decision | Rationale |
|------|----------|-----------|
| 2026-03-31 | Name the maestro-owned capability `chromium-debug-use` | User explicitly wants that identity, and it distinguishes the tool from broader “browser-use” style automation claims. |
...
- The upstream `chrome-cdp-skill` model proves there is a small-footprint Node 22+ path with no npm install and a daemonized attach model, so the main maestro work is packaging, consent, routing, and verification rather than raw protocol feasibility.
...
**Logged:** 2026-03-12
**Trigger:** Issue [GH#4205](https://github.com/Aditya060806/Maestro/issues/4205) plus observed post-release drift where new SEO subagent and slash-command docs were present in repo but missing in `~/.maestro/agents/` until manual `rsync`.
...
Eliminate deployment drift between repo state (`.agents/`) and runtime state (`~/.maestro/agents/`) after merge/release workflows. Newly merged docs/agents/commands should become available without requiring operators to remember manual sync commands.
...
- [ ] **Phase 1 — Reproduce and instrument drift check** (~45m): Add a deterministic check command/script that compares `.agents/` against `~/.maestro/agents/` for tracked files relevant to runtime behavior (especially `seo/`, `scripts/commands/`, and top-level agent docs). Emit clear PASS/DRIFT output.

- [ ] **Phase 2 — Wire automatic sync into post-merge/release paths** (~1.5h): Integrate `deploy_maestro_agents` (or equivalent safe sync wrapper) into the existing merge/release lifecycle where drift can occur. Ensure sync runs from canonical repo path and logs success/failure with actionable diagnostics.
...
- Standard merge/release flow updates `~/.maestro/agents/` without manual `rsync`.
...
- [ ] **Phase 2 — Root cause and fix** (~1.5h): Trace why `gh` attempts to spawn `/bin/zsh` for mutation paths. Check `SHELL`, git editor/pager env vars, `gh` config, and any maestro wrappers that may inject shell-specific behavior. Implement the narrowest reliable fix in maestro or local runtime setup.
...
- `gh api` mutation commands no longer fail with `/bin/zsh` `posix_spawn` in normal maestro sessions.
...
**Interactive session security posture dashboard** — deferred to a future maestro interface project. The information (accessible sensitive paths, network egress posture, gopass entries) is security-sensitive and should not be published to public GitHub issues. Will be part of a local-only maestro UI. For now, `security-posture-helper.sh` (t1412.6) provides CLI-based posture checks.

#### Decision Log

| Date | Decision | Rationale |
|------|----------|-----------|
| 2026-03-09 | Defer interactive dashboard to future maestro UI | Security posture data (accessible keys, credentials, network state) should not be published to public GitHub issues. Needs a local-only interface. |
...
- [ ] **Phase 1 — Create agent file** (t1414.1, ~1h): Ingest upstream skill content into `.agents/services/communications/convos.md`. Add maestro frontmatter (mode, tools, description), `AI-CONTEXT` markers, Quick Reference section, and Related section linking to `xmtp.md`, `simplex.md`, `matterbridge.md`. Adapt bridge script section to reference maestro dispatch patterns instead of OpenClaw-specific calls.
...
| 2026-03-07 | Ingest upstream skill rather than link-only | The skill content is substantial and well-structured. Ingesting ensures availability even if the upstream URL changes, and allows maestro-specific adaptations. |
...
| Shell script, not TypeScript | Consistent with maestro infrastructure. Fractals is TypeScript but the pattern is language-agnostic. |
...
| maestro capability | Fractals gap |
|---|---|
...
| 2026-03-06 | Shell script implementation, not TypeScript | Consistent with maestro infrastructure; pattern is language-agnostic |
...
- maestro already has all the infrastructure Fractals is missing (merge flow, dependencies, persistence, quality gates) — we just need the decomposition step
...
Build a lightweight LLM evaluation toolkit into maestro — live model benchmarking, output quality scoring, reusable test datasets, and prompt version tracking. Inspired by [LangWatch](https://github.com/langwatch/langwatch) but implemented as CLI tools that fit our existing infrastructure (shell scripts, JSONL, pattern tracker) rather than requiring a 6-container Docker stack.
...
**Logged:** 2026-03-05
**Issue:** [GH#2854](https://github.com/Aditya060806/Maestro/issues/2854) CLOSED
**Replaces:** PR #2792 (declined)

#### Purpose

Root-cause fix for the March 3 kernel panic and ongoing memory pressure. Analysis identified three sources of excessive RAM consumption, all caused by maestro itself:
...
- **2026-03-05:** Root cause is maestro processes, not external memory pressure — the framework should fix its own resource consumption before adding a generic OS monitor.

---

### [2026-03-02] Prompt Injection Scanner — Tool-Agnostic Defense for maestro and Agentic Apps
...
Agent doc teaches:
  1. maestro agents: when to scan (webfetch, MCP, untrusted repos)
...
- [ ] (2026-03-02) Phase 1: Core implementation ~4h
  ...
  - [ ] t1375.2 Agent doc for maestro + agentic app developers
...
| 2026-03-01 | Agent doc only, not MCP integration | zvec is for app development, not for maestro internal use — no MCP server needed |
...
Add a comprehensive UI/UX design intelligence system to maestro that bridges the gap between design agents and content agents. Currently, design decisions (colours, typography, layout) and content decisions (copywriting voice, imagery style) are made independently — a project can have beautiful UI with copy that reads like a different brand.
...
Inspiration entries (per-project — context/inspiration/):
  ...
  Avoids leaking competitive intelligence into the public maestro repo.
...
Add scene text / screenshot OCR capability to maestro. Current stack (MinerU + Docling + ExtractThinker + LibPDF) handles document parsing and structured extraction well, but cannot read text from screenshots, photos, UI captures, signs, or other non-document images.
...
**Research:** [#2558](https://github.com/Aditya060806/Maestro/issues/2558) — Perplexity Computer + Microsoft Amplifier comparison
...
- **Source**: Issue #2558 — research review comparing maestro against Perplexity Computer and Microsoft Amplifier
...
Close the gap between "I have an idea" and "autonomous execution." Current maestro handles task-level work (`/full-loop`) and supervisor dispatch (`/pulse`), but nothing takes a high-level goal and drives it to completion over days. Missions extend beyond code into research, procurement, infrastructure setup, and 3rd-party communication — making maestro a true autonomous project agent.
...
Phase 4: COMPLETION
    ...
    ├── Offer improvements back to maestro
    └── Summary report
```

#### Mission Homes

- `~/.maestro/missions/{id}/` — homeless missions (POC drafting, no repo yet)
- `todo/missions/{id}/` — missions attached to a project repo
- Migration: homeless → repo-attached when `maestro init` + `git init` runs
...
**What maestro already has (strong overlap):**
...
When agents working on other repos (e.g. a webapp repo) discover maestro framework improvements, they currently edit the installed copy at `~/.maestro/agents/` which is overwritten on next `maestro update`. There's no guidance telling agents to make changes on the source repo or capture them as todos there, with PLANS.md entries recommended for clarity of objectives when the improvement is non-trivial.

#### Context

Observed in a webapp session: a `build-plus.md` improvement was made to `~/.maestro/agents/build-plus.md` (the installed copy). This edit will be lost on next `maestro update` because `setup.sh` copies from `~/Git/maestro/.agents/` to `~/.maestro/agents/`. The agent had no guidance that improvements must go to the source repo.

#### Execution (single phase)

- [x] Add section to root `AGENTS.md` (developer guide) explaining: framework improvements must be made in `~/Git/maestro/` or captured as todos/plans in that repo's TODO.md. Recommend PLANS.md entries for non-trivial improvements to ensure clarity of objectives
- [x] Add matching guidance to `.agents/AGENTS.md` (user guide) so agents working on other repos know to create todos (and PLANS.md entries for non-trivial changes) in the maestro repo rather than editing the installed copy
...
- Prevents future loss of improvements on `maestro update`
...
Add local AI model inference to maestro via llama.cpp + HuggingFace, completing the cost spectrum from free (local) through budget (haiku) to premium (opus). Users get guided hardware-aware setup, access to any HuggingFace GGUF model, usage tracking, and disk cleanup recommendations.
...
Analysis of doodledood/manifest-dev revealed two high-value ideas worth adapting to maestro:
...
Deep analysis of oh-my-pi revealed 10+ patterns applicable to maestro, ranging from edit tool improvements to real-time streaming policy enforcement (TTSR), observability, and multi-agent orchestration. This plan captures all actionable learnings as concrete implementation tasks.
...
| SQLite observability                          | High -- cost/performance tracking, budget enforcement     | No equivalent exists in maestro                              |
...
Implement SQLite-based LLM request tracking: model, provider, tokens, costs, duration, TTFT, stop reason. Incremental session parsing. CLI dashboard (`maestro stats`). Wire into budget enforcement (t1100).
...
**Subagent pattern**: Disabled globally, enabled on-demand per `add-new-mcp-to-maestro.md` checklist. Same pattern as FluentCRM, Unstract, iOS Simulator MCPs.
...
| Maestro | Yes     | Infrastructure and integration work                          |
...
- Enable `openapi-search_*: true` for Build+, Maestro, Research agents
...
- d003: Enable for Build+, Maestro, Research only — API discovery is a development/research task. Domain agents (SEO, WordPress, etc.) can invoke via subagent reference if needed. 2026-02-21
...
Adding MCPorter as an maestro agent provides:
...
| Capability     | Command                             | maestro Use Case                   |
| -------------- | ----------------------------------- | ----------------------------------- |
...
- Create `.agents/tools/mcp-toolkit/mcporter.md` following `add-new-mcp-to-maestro.md` template
...
- d003: Enable for Build+ and Maestro agents only — MCP toolkit operations are developer/infrastructure tasks, not domain-specific. Other agents can invoke via subagent reference. 2026-02-21

#### Risks

- **Pre-1.0 API instability**: v0.7.3 means breaking changes possible. Mitigation: document version pinning, link to changelog.
- **Node/pnpm dependency**: maestro prefers Bun. Mitigation: MCPorter has Bun support (`dist-bun/`), document both options.
...
ALWAYS LOADED (every session):
  ...
  repo AGENTS.md (~1,000 tokens)
    - Contributing guide for maestro framework development
  ...
  build-plus.md (~2,500 tokens)
    ...
    - Build workflow (maestro-specific steps only)
...
- Container image needs: claude CLI, git, node/bun, maestro agents (volume mount)
...
Make `maestro setup` and `maestro update` deploy equivalent configuration to Claude Code.
...
**Verification gate:** Fresh `maestro setup` on a machine with both CLIs produces working configs for both. Claude Code interactive session has slash commands and MCPs available.
...
- [ ] t1165.1 Container image design:
  ...
  - Volume mounts: repo checkout (read-write), `~/.maestro/agents/` (read-only)
...
- [ ] t1165.2 Container pool manager:
  ...
  - Pool config: `~/.config/maestro/container-pool.json` (image, count, tokens, hosts)
...
| Claude Code updates break our generated config               | Low        | Medium | `update_claude_config()` is idempotent, re-runs on every `maestro update`              |
...
- Claude Code rewrites "OpenCode" references to "Claude Code" when loading AGENTS.md files. This is a Claude Code behavior, not something in our codebase. The deployed file at `~/.maestro/agents/AGENTS.md` correctly says "OpenCode". Confirmed by comparing on-disk content vs system prompt content.
- The t1022 revert (PR #1329) left a residual "Claude Code" reference in `.agents/maestro/architecture.md:44` that should be corrected.
...
- Updates the deployed copy at `~/.maestro/agents/` for that user's local sessions
- Repo version wins on next `maestro update` (no conflict risk)
...
auto-setup <cloudron-server> [--runners "code-reviewer,seo-analyst,ops-monitor"]
    ...
    ├─ 6. Store access token via maestro secret set MATRIX_BOT_TOKEN
...
- **2026-02-14**: Store bot credentials via `maestro secret set` (gopass) rather than plaintext — follows framework security conventions
...
**Unified audit findings schema** (new table in `~/.maestro/.agent-workspace/work/code-audit/audit.db`):
...
- Log every drop to `~/.maestro/logs/stash-audit.log` with stash content summary
...
**Email lifecycle in maestro after this plan:**
...
- **2026-02-10**: EOA API credentials stored via `maestro secret set EOA_API_KEY` and `maestro secret set EOA_API_PASSWORD`. Never in config files or conversation.
...
**Accessibility testing lifecycle in maestro after this plan:**
...
- WAVE API: key via `maestro secret set WAVE_API_KEY`, HTTP GET
...
- **2026-02-10**: WAVE API credentials stored via `maestro secret set WAVE_API_KEY`. Never in config files or conversation. Credit tracking built into helper script to avoid unexpected costs.
...
- **Decision:** PDF knowledge stored in memory + agent files, not referenced by source name
  **Rationale:** The guides are session inspiration. The extracted frameworks, prompts, and tactics become native maestro knowledge attributed to the agent system, not to external products or creators.
  **Date:** 2026-02-09
...
d066,p026,PDF knowledge stored natively not by source name,Extracted knowledge becomes native maestro knowledge,2026-02-09,Scope
-->
...
Create a plugin architecture for maestro that allows private extension repos (`maestro-pro`, `maestro-anon`) to overlay additional agents and scripts onto the base framework. Plugins are git repos that extend maestro without modifying the core, enabling tiered access (public/pro/private) and fast evolution of specialized features.

#### Context from Discussion

**Repos to support:**

- `~/Git/maestro-pro` (github.com/Aditya060806/Maestro-pro) - Pro features
- `~/Git/maestro-anon` (gitea.Aditya060806.com/marcus/maestro-anon) - Anonymous/private features
...
2. **Plugin structure mirrors main** - Same `.agents/` pattern:
   ...
   ├── README.md
   ├── VERSION
   ├── .maestro.json      # Plugin config with base_repo reference
   └── .agents/
...
3. **Plugin AGENTS.md points to base** - Minimal, references main framework:
   ...
   For framework documentation: `~/.maestro/agents/AGENTS.md`
   For architecture: `~/.maestro/agents/maestro/architecture.md`

   ## Plugin Development

   This plugin deploys to `~/.maestro/agents/pro/` (namespaced).
   ```

4. **`.maestro.json` plugin config**:
   ...
   ```json
   {
     ...
     "plugin": {
       "name": "pro",
       "base_repo": "~/Git/maestro",
       "namespace": "pro"
     }
   }
   ```

5. **`maestro update` deploys main + plugins** - Single command updates everything
...
- Work in plugin repo directly (`~/Git/maestro-pro/`)
- Run `maestro update` to redeploy all (main + plugins)
- Plugin changes immediately visible in `~/.maestro/agents/pro/`
...
- **Decision:** `maestro init` detects plugin repos via `.maestro.json` plugin field
...
d059,p024,maestro init detects plugin repos,Consistent initialization and AI context,2026-02-07,UX
-->

#### Open Questions

1. **License** - Same MIT for plugins, or proprietary for pro/anon?
2. **Gitea Actions** - Is it enabled on gitea.Aditya060806.com, or local-only linting?
...
- [ ] (2026-02-07) Phase 1: Add plugin support to `.maestro.json` schema ~1h (t136.1)
  - Add `plugin` field with `name`, `base_repo`, `namespace`
  - Update `maestro init` to detect and configure plugin repos
  - Add `features: ["plugin"]` option
- [ ] (2026-02-07) Phase 2: Add `plugins.json` config and CLI commands ~2h (t136.2)
  - Create `~/.config/maestro/plugins.json` schema
  - Add `maestro plugin add/list/enable/disable/remove/update` commands
  - Support GitHub and Gitea URLs
- [ ] (2026-02-07) Phase 3: Extend `setup.sh` to deploy plugins ~2h (t136.3)
  - Add `deploy_plugins()` function after `deploy_maestro_agents()`
  - Respect namespace (deploy to `~/.maestro/agents/{namespace}/`)
  - Handle script prefix convention (`{namespace}-*.sh`)
- [ ] (2026-02-07) Phase 4: Create plugin template ~1h (t136.4)
  - `maestro plugin create <name>` scaffolds structure
  - Template AGENTS.md, README.md, .maestro.json, .github/workflows/ci.yml
  - Minimal GHA (ShellCheck + Secretlint + Markdownlint)
- [ ] (2026-02-07) Phase 5: Scaffold maestro-pro and maestro-anon repos ~2h (t136.5)
...
m109,p024,Phase 1: Add plugin support to .maestro.json schema,1h,,2026-02-07T00:00Z,,pending
...
m113,p024,Phase 5: Scaffold maestro-pro and maestro-anon repos,2h,,2026-02-07T00:00Z,,pending
-->
...
Enable cross-provider model routing so that any maestro session can dispatch tasks to the optimal model regardless of which provider the parent session runs on. A Claude session should be able to request a Gemini code review; a Gemini session should be able to escalate complex reasoning to Claude Opus. Models should fall back gracefully when unavailable, and the system should detect when provider/model names change upstream.
...
- [ ] (2026-02-06) Phase 2: Provider/model registry with periodic sync ~2h (t132.2)
  ...
  - Run on `maestro update` and optionally via cron
...
Replace plaintext `mcp-env.sh` credential storage with gopass (GPG-encrypted, git-versioned, team-shareable). Build an AI-native wrapper (`maestro secret`) that keeps secret values out of agent context windows via subprocess injection and output redaction. Rename `mcp-env.sh` to `credentials.sh` across the entire codebase for accuracy.
...
- [ ] (2026-02-06) Part B: gopass integration + maestro secret wrapper ~6h
...
m096,p021,Part B: gopass integration + maestro secret wrapper,6h,,2026-02-06T20:00Z,,pending
...
Discovered during Tabby tab dispatch experiments that maestro has all the worker components but no supervisor loop. The gap: nothing evaluates whether a dispatched task succeeded, retries on failure, or updates TODO.md on completion. This is the "brain stem" connecting the existing "limbs."
...
### [2025-12-21] maestro-opencode Plugin

**Status:** Completed
**Estimate:** ~2d (ai:1d test:0.5d read:0.5d)
**Architecture:** [.agents/build-mcp/maestro-plugin.md](../.agents/build-mcp/maestro-plugin.md)

<!--TOON:plan{id,title,status,phase,total_phases,owner,tags,est,est_ai,est_test,est_read,logged,started,completed}:
p001,maestro-opencode Plugin,completed,4,4,,opencode|plugin,2d,1d,0.5d,0.5d,2025-12-21T01:50Z,,2026-03-21T00:00Z
-->

#### Purpose

Create an optional OpenCode plugin that provides native integration for maestro. This enables lifecycle hooks (pre-commit quality checks), dynamic agent loading, and cleaner npm-based installation for OpenCode users who want tighter integration.
...
- maestro remains compatible with Claude, Cursor, Windsurf, etc.
- Plugin loads agents from `~/.maestro/agents/` at runtime
- Should detect and complement oh-my-opencode if both installed

**Architecture (from maestro-plugin.md):**

- Agent loader from `~/.maestro/agents/`
- MCP registration programmatically
- Pre-commit quality hooks (ShellCheck)
- maestro CLI exposed as tool
...
- **Decision:** Keep as optional plugin, not replace current approach
  **Rationale:** maestro must remain multi-tool compatible (Claude, Cursor, etc.)
  **Date:** 2025-12-21

<!--TOON:decisions[1]{id,plan_id,decision,rationale,date,impact}:
d001,p001,Keep as optional plugin,maestro must remain multi-tool compatible,2025-12-21,None - additive feature
-->
...
- Adapt for maestro: install to `~/.maestro/hooks/` not `.claude/hooks/`
...
- [x] (2026-02-08) Phase 1: Create git_safety_guard.py adapted for maestro ~1h
...
<!--TOON:milestones[4]{id,plan_id,desc,est,actual,scheduled,completed,status}:
m005,p002,Phase 1: Create git_safety_guard.py adapted for maestro,1h,15m,2025-12-21T12:00Z,2026-02-08,complete
...
- **Decision:** Install hooks to `~/.maestro/hooks/` by default
  **Rationale:** Consistent with maestro directory structure, global protection
  **Date:** 2025-12-21
...
d002,p002,Install hooks to ~/.maestro/hooks/,Consistent with maestro directory structure,2025-12-21,None
...
### [2025-12-21] Evaluate Merging build-agent and build-mcp into maestro

**Status:** Completed
**Estimate:** ~4h (ai:2h test:1h read:1h)

<!--TOON:plan{id,title,status,phase,total_phases,owner,tags,est,est_ai,est_test,est_read,logged,started,completed}:
p003,Evaluate Merging build-agent and build-mcp into maestro,completed,3,3,,architecture|agents,4h,2h,1h,1h,2025-12-21T14:00Z,,2026-03-21T00:00Z
-->

#### Purpose

Evaluate whether `build-agent.md` and `build-mcp.md` should be merged into `maestro.md`. When enhancing maestro, we often build agents and MCPs - these are tightly coupled activities that may benefit from consolidation.
...
- `maestro.md` - Framework operations, already references build-agent as "Related Main Agent"
- All three are `mode: subagent` - called from maestro context

**Options to evaluate:**

1. **Merge fully** - Combine into maestro.md with expanded subagent folders
2. **Keep separate but link better** - Improve cross-references, keep modularity
3. **Hybrid** - Move build-agent into maestro/, keep build-mcp separate (MCP is more specialized)
...
### [2025-12-21] Uncloud Integration for maestro
...
<!--TOON:plan{id,title,status,phase,total_phases,owner,tags,est,est_ai,est_test,est_read,logged,started,completed}:
p006,Uncloud Integration for maestro,completed,4,4,,deployment|docker|orchestration,1d,4h,4h,2h,2025-12-21T04:00Z,2026-02-08T00:00Z,2026-02-08T00:00Z
-->

#### Purpose

Add Uncloud as a deployment provider option in maestro. Uncloud is a lightweight container orchestration tool that enables multi-machine Docker deployments without complex Kubernetes infrastructure. It aligns with maestro philosophy of simplicity and developer experience.
...
### [2025-12-21] SEO Machine Integration for maestro
...
<!--TOON:plan{id,title,status,phase,total_phases,owner,tags,est,est_ai,est_test,est_read,logged,started,completed}:
p007,SEO Machine Integration for maestro,completed,5,5,,seo|content|agents,2d,1d,0.5d,0.5d,2025-12-21T15:00Z,,2026-03-21T00:00Z
-->

#### Purpose

Fork and adapt SEO Machine capabilities into maestro to add comprehensive SEO content creation workflows. SEO Machine is a Claude Code workspace with specialized agents and Python analysis modules that fill significant gaps in maestro content capabilities.
...
- maestro needs multi-tool compatibility (OpenCode, Cursor, Windsurf, etc.)
- Can leverage existing maestro SEO tools (DataForSEO, GSC, E-E-A-T, site-crawler)
- Opportunity to improve with maestro patterns (subagent architecture, TOON, scripts)

#### Context from Discussion

**Gap analysis - what maestro gains:**

| Capability                | SEO Machine              | maestro Current           | Action                  |
| ------------------------- | ------------------------ | -------------------------- | ----------------------- |
...
- Adapt agents to maestro subagent pattern under `seo/` and `content/`
- Port Python modules to `~/.maestro/agents/scripts/seo-*.py`
...
- [ ] (2025-12-21) Phase 4: Add context management system ~3h
  ...
  - Per-project context in `.maestro/context/`
...
- **Decision:** Fork and adapt rather than integrate directly
  **Rationale:** SEO Machine is Claude Code-specific; maestro needs multi-tool compatibility
  **Date:** 2025-12-21

- **Decision:** Port Python modules to scripts/ rather than keeping as separate package
  **Rationale:** Consistent with maestro pattern; scripts are self-contained and portable
  **Date:** 2025-12-21
...
<!--TOON:decisions[3]{id,plan_id,decision,rationale,date,impact}:
d008,p007,Fork and adapt rather than integrate directly,SEO Machine is Claude Code-specific; maestro needs multi-tool compatibility,2025-12-21,None
d009,p007,Port Python modules to scripts/,Consistent with maestro pattern; scripts are self-contained and portable,2025-12-21,None
...
### [2025-12-21] Beads Integration for maestro Tasks & Plans
...
<!--TOON:plan{id,title,status,phase,total_phases,owner,tags,est,est_ai,est_test,est_read,logged,started,completed}:
p009,Beads Integration for maestro Tasks & Plans,completed,3,3,,beads|tasks|sync|planning,2d,1d,0.5d,0.5d,2025-12-21T16:00Z,2025-12-21T16:00Z,2025-12-22T00:00Z
-->

#### Purpose

Integrate Beads task management concepts and bi-directional sync into maestro Tasks & Plans system. This provides:
...
**Key decision:** Keep maestro markdown as source of truth, sync bi-directionally to Beads for visualization and graph features. Include Beads as default with maestro (not optional).
...
- [x] (2025-12-21) Phase 3: Default installation ~4h actual:3h
  ...
  - [x] 3.2 Add `maestro init beads` feature
...
m045,p009,3.2 Add maestro init beads,1h,45m,2025-12-21T16:00Z,2025-12-22T00:00Z,done
...
- **Decision:** Keep maestro markdown as source of truth, Beads as sync target
...
- **Decision:** Include Beads as default with maestro (not optional)
...
<!--TOON:decisions[4]{id,plan_id,decision,rationale,date,impact}:
d013,p009,Keep maestro markdown as source of truth,Markdown is portable and human-readable; Beads provides graph features,2025-12-21,None
d014,p009,Command-led sync only (no automatic),Prevents race conditions and ensures data integrity,2025-12-21,None
d015,p009,Include Beads as default with maestro,Graph features valuable enough to justify default installation,2025-12-21,None
...
Implement remaining agent design pattern improvements identified from Lance Martin's analysis of successful agents (Claude Code, Manus, Cursor). While maestro already implements most patterns, these enhancements will further optimize context efficiency and enable automatic learning.
...
**Status:** Completed
**Estimate:** ~2d (ai:1d test:0.5d read:0.5d)
**Branch:** `feature/add-skill-command` (worktree at `~/Git/maestro.feature-add-skill-command/`)
...
Create a comprehensive skill import system that allows rapid adoption of external AI agent skills into maestro, with upstream tracking for updates and multi-assistant compatibility.

**Problem:** Many people are creating and sharing Claude Code skills, OpenCode skills, and other AI assistant configurations. maestro has its own superior `.agents/` folder structure. We need to rapidly import external skills, convert to maestro format, handle conflicts intelligently, and track upstream for updates.
...
- Source of truth: `.agents/` (maestro format)
...
- **Decision:** Complex skills become folders with subagents
  **Rationale:** Follows maestro nesting convention (parent.md + parent/)
  **Date:** 2026-01-21
...
d024,p012,Complex skills become folders with subagents,Follows maestro nesting convention,2026-01-21,None
...
Add automatic memory capture to maestro, inspired by claude-mem but tool-agnostic. Currently, memory requires manual `/remember` invocation. Auto-capture will:
...
- maestro needs tool-agnostic solution
...
- **Decision:** Implement ourselves rather than depend on claude-mem
  **Rationale:** claude-mem is Claude Code-only; maestro needs multi-tool support
  **Date:** 2026-01-11
...
Evolve maestro from single-session workflows to scalable multi-agent orchestration with:
...
- Solution: maestro plugin injects critical rules into every compaction
...
- [ ] (2026-01-23) Phase 2: Compaction Plugin ~4h
  - Create `opencode-maestro-plugin/` package (TypeScript)
...
- [ ] (2026-01-23) Phase 4: TOON Mailbox System ~4h
  ...
  - Create directory structure: `~/.maestro/.agent-workspace/mail/{inbox,outbox,archive}/`
...
| `opencode-maestro-plugin/index.ts`     | Compaction hook plugin                                      | 2     |
| `opencode-maestro-plugin/package.json` | Plugin package manifest                                     | 2     |
...
Create a comprehensive document extraction capability in maestro that:
...
- [ ] (2026-01-25) Phase 1: Research & Environment Setup ~4h
  - Create Python venv at `~/.maestro/.agent-workspace/python-env/document-extraction/`
...
Selectively adopt high-value concepts from Claude-Flow v3 while maintaining maestro' lightweight, shell-script-based philosophy. Claude-Flow is a heavy orchestration platform (~340MB, TypeScript) - we cherry-pick concepts, not implementation.
...
| Feature         | Claude-Flow      | maestro Current   | Adoption Priority |
| --------------- | ---------------- | ------------------ | ----------------- |
...
- **Swarm consensus**: Skip - maestro philosophy is simpler async coordination
...
- [ ] (2026-01-31) Phase 2: Semantic Memory with Embeddings ~8h
  ...
  - Add optional HNSW index to `~/.maestro/.agent-workspace/memory/`
...
- [ ] (2026-01-31) Phase 4: Documentation & Integration ~6h
  - Create `maestro/claude-flow-comparison.md` documenting differences
...
- **Decision:** Cherry-pick concepts, not implementation
  **Rationale:** Claude-Flow is 340MB TypeScript; maestro is lightweight shell scripts. Different philosophies.
  **Date:** 2026-01-31
...
- **Decision:** Skip swarm consensus and WASM transforms
  **Rationale:** maestro uses simpler async mailbox; Edit tool is already fast enough.
  **Date:** 2026-01-31
...
<!--TOON:decisions[4]{id,plan_id,decision,rationale,date,impact}:
d035,p015,Cherry-pick concepts not implementation,Claude-Flow is 340MB TypeScript; maestro is lightweight shell,2026-01-31,Architecture
d036,p015,Keep FTS5 as default embeddings as opt-in,FTS5 is fast zero dependencies works for most cases,2026-01-31,None
d037,p015,Skip swarm consensus and WASM transforms,maestro uses simpler async mailbox; Edit tool fast enough,2026-01-31,None
...
| `maestro/claude-flow-comparison.md`  | Feature comparison documentation       | 4     |
...
3. Project-specific patterns from `.maestro/privacy-patterns.txt`
...
Create a comprehensive voice integration for maestro supporting both local and cloud-based speech capabilities. This enables hands-free AI interaction via voice-to-text, text-to-speech, and full speech-to-speech conversation loops with OpenCode.
...
SimpleX Chat is the most privacy-respecting messaging platform available — no user identifiers (not even random ones), no phone numbers, no central metadata storage. This integration brings secure, zero-knowledge communications to maestro for:

1. **Secure remote AI agent control** — initiate sessions on remote devices running maestro without exposing management interfaces to the public internet
...
7. **Future: real-time voice/video calls** between users and AI agents (when maestro gains those capabilities)

No existing maestro integration covers secure messaging at this level. Matrix (existing agent) is federated but has user identifiers and server-side metadata. SimpleX fills the zero-knowledge gap.
...
SimpleX CLI (WebSocket server, port 5225)
    ...
    |
maestro SimpleX Bot (TypeScript/Bun)
    |--- maestro CLI commands (/run, /status, /deploy)
...
SimpleX bot commands use `/` in SimpleX chat context. maestro commands use `/` in terminal context. Separate environments, no collision. Bot command menu configured via `/set bot commands` with hierarchical structure (similar to Telegram inline keyboards).
...
**Opsec Agent Scope (existing maestro tools + confirmed additions):**
...
- [ ] Create `.agents/services/communications/simplex.md` — comprehensive knowledge base
  ...
  - Integration with other maestro capabilities
...
- [ ] Create TypeScript/Bun bot scaffold as **channel-agnostic gateway** (inspired by OpenClaw/IronClaw gateway pattern):
  ...
  - DM pairing flow — unknown contacts get pairing code, admin approves via `maestro simplex pairing approve`
...
- [ ] Create `.agents/tools/security/opsec.md`:
  ...
  - maestro-specific opsec (multi-tenant credentials, privacy-filter, audit trails)
...
| 2026-02-25 | TypeScript/Bun for bot, not Haskell                 | Aligns with maestro ecosystem, TypeScript SDK available                                                                                   |
...
| 2026-02-25 | Opsec scoped to existing maestro tools + confirmed | NetBird, Mullvad, IVPN, Brave, CamoFox + existing agents. Others assessed as needs arise                                                   |
...
| 2026-02-25 | Voice/video calls as future phase                   | Depends on maestro gaining real-time audio/video processing                                                                               |
...
| 2026-02-25 | Chat security as dedicated phase                    | IronClaw/OpenClaw both treat inbound DMs as untrusted — prompt injection defense, leak detection, exec approvals are real gaps in maestro |
...
- AGPL license means any bot framework we build and distribute must be open source — aligns with maestro's open approach
...
- 15 of 45 files in `.agents/plugins/opencode-maestro/` (~1,300 combined complexity)
...
See [Active Plans > Beads Integration](#2025-12-21-beads-integration-for-maestro-tasks--plans) for full details.
...
p009,Beads Integration for maestro Tasks & Plans,,beads|tasks|sync|planning,2d,1.5d,2025-12-21T16:00Z,2025-12-21T16:00Z,2025-12-22T00:00Z,1
-->
