...
status: active
mode: full
repo: "/Users/Aditya060806/Git/maestro"
created: "2026-05-08"
started: "2026-05-08"
completed: ""
...
preferences:
  ...
  deploy_target: "maestro local deployment plus example-repo GitHub CI"
  test_framework: "ShellCheck, maestro script tests, example-repo CI"
  ci_provider: "GitHub Actions"
  coding_style: "Follow maestro AGENTS.md and example-repo AGENTS.md per repo"
---
...
> Keep improving maestro and example-repo until open example-repo issues can be solved by workers as green PRs that merge, with duplicate/red PR churn systematically reduced.

## Origin

- **Created:** 2026-05-08
- **Created by:** Aditya Pandey and Harvest
- **Session:** OpenCode interactive session
- **Context:** After maestro v3.14.97 restored breaker-held issue retries, example-repo workers produced useful PRs but many issues still had multiple sibling PRs and red CI. The observed blockers are a mix of maestro orchestration gaps and example-repo CI/repo policy gaps.

## Scope

**Goal:** example-repo open issues should steadily converge to one healthy candidate PR per issue, required checks should distinguish code failures from infrastructure/advisory failures, maestro should avoid duplicate worker churn, and green approved PRs should merge and close issues with evidence.

**Success criteria:**

- Maestro classifies CI timeouts/kills and advisory E2E failures distinctly from PR-specific code failures.
- Maestro suppresses duplicate dispatch when an approved or healthiest PR already exists for the same issue.
- Maestro can consolidate/supersede sibling PRs against the verified newest/healthiest candidate.
...
- Use worktree + PR flow for maestro and example-repo code changes.
- Release/deploy maestro changes after merge before relying on them operationally.
...
| 1.2 | Add or improve maestro reporting for duplicate PR groups and CI blocker classes | pending | active | ~3h | interactive | |
| 1.3 | Publish mission status summaries with evidence and next recommended action | pending | pending | ~2h | interactive | |

### Milestone 2: Maestro orchestration fixes

**Status:** pending
**Estimate:** ~18h
**Validation:** maestro no longer treats known CI infra/advisory failures as worker-code failures, avoids duplicate redispatch where a healthy candidate exists, and can close superseded sibling PRs safely.
...
| 5.2 | Store lessons and update maestro worker discipline/configs from observed failure modes | pending | pending | ~4h | interactive/worker | |
...
|------|------|---------|--------|-------|
| GitHub CLI access | credential | Read/write PRs/issues/checks in maestro and example-repo | configured | Use gh wrappers/signature discipline for writes |
| maestro release pipeline | infrastructure | Ship systemic orchestration fixes | configured | version-manager release patch/hotfix after merge |
...
|------|----------|--------|-------------|-----------|
| 2026-05-08 | time | initial | Mission created and scoped from current example-repo/maestro evidence | 1 |

## Decision Log

| # | Date | Decision | Rationale | Alternatives Considered |
|---|------|----------|-----------|------------------------|
| 1 | 2026-05-08 | Treat this as a mixed maestro + example-repo mission | Evidence shows both orchestration duplicate/red-PR churn and example-repo CI timeout/advisory ambiguity | Only fix maestro; only fix example-repo |
...
| 3 | 2026-05-08 | First maestro change should stop CI repair redispatch for infra/advisory failures | `pulse-merge-feedback.sh` routed advisory/non-required and timed-out/cancelled checks as CI repair feedback, which closes PRs and requeues duplicate workers | Start with example-repo CI changes only |
...
|-----------|-------|---------|
| 2026-05-08T00:00:00Z | Mission created | Goal: keep improving maestro and example-repo until open issues converge to green merged PRs. |
