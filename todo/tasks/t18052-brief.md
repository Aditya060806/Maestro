...
- [x] Memory recall: `maestro reach efficiency headed headless offload routine audit` → 0 hits — no relevant prior lesson found.
...
- **Read first:** `.agents/maestro/reach-capture.md` and `reach-helper.sh` after t18047-t18051.
...
- `EDIT: .agents/maestro/reach-capture.md` — document budgets, headed/headless/offload, routines, and audit chain.
...
7. Update `/auto-browse` docs to run `maestro reach doctor` and `maestro reach route` before high-agency browser discovery for repeatable capture/data-mining workflows.
...
./maestro.sh reach route --objective "repeat public changelog capture" --scope public --format json
./maestro.sh reach watch --once --dry-run --format json
```
