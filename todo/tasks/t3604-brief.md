...
Fix `maestro check-workflows` so canonical caller templates are rendered consistently when `workflow_reusable_repo`/`workflow_reusable_ref` points at an organization-owned reusable workflow repository.
...
Template comments such as `# Upstream logic lives in: Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml` remain unrendered. Org-owned callers with equivalent comments can then fail byte comparison and classify as `DRIFTED/CALLER` even though the functional caller is current.
...
- `EDIT: .agents/scripts/check-workflows-helper.sh` — update `_render_template_for_target()` so it renders both:
  - `uses: Aditya060806/Maestro/.github/workflows/<reusable-file>@<ref>` → configured repo/ref.
  - Exact documentation/reference occurrences of `Aditya060806/Maestro/.github/workflows/<reusable-file>` → configured repo, without changing unrelated repo text.
...
- `.agents/templates/workflows/issue-sync-caller.yml:4` contains a comment reference to `Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml`; line 57 contains the functional `uses:` target.
...
1. Update `_render_template_for_target()` using existing escaping helpers:
   ...
   - Keep the replacement scoped to the reusable workflow path; do not globally replace `Aditya060806/Maestro` or ignore YAML comments.
