...
- **Parent task:** none
- **Conversation context:** After the t1440 fix landed, `version-manager.sh release patch` is still blocked by repo-wide preflight failures. The task needs to be tracked in maestro with a valid task ID, linked GitHub issue, and TODO entry on main so the remaining blocker debt can be cleared and patch releases can ship again.
