...
- `MAESTRO_SESSION_USER` env-var override (from t1984) is honoured — the sync-todo-to-issues workflow sets this to `github.actor` when the actor is human, so issues created from human-triggered pushes get the correct assignee.
...
_gh_wrapper_auto_assignee() {
	...
	# t1984 override: sync-todo-to-issues workflow sets MAESTRO_SESSION_USER
	...
	# a workflow run.
	if [[ -n "${MAESTRO_SESSION_USER:-}" ]]; then
		printf '%s' "$MAESTRO_SESSION_USER"
		return 0
	fi
	local current_user
...
_gh_wrapper_auto_assignee
# Expect: your gh login (e.g., "Aditya060806")
```
...
- `.agents/scripts/issue-sync-helper.sh:467-483` — the `MAESTRO_SESSION_USER` override pattern being mirrored.
