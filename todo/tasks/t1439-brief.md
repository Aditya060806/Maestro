...
- **Parent task:** none
- **Conversation context:** The user asked for a tracked maestro planning task covering a pulse fix where status should report the real last pulse time and worker dispatch should honor the configured max worker limit. The goal is to secure a valid task ID and linked issue so follow-up implementation can use a PR title starting with `t1439:`.
...
- Preserve the standard maestro planning workflow: atomic task ID first, linked GitHub issue, TODO tracking on main, and a brief for follow-up implementation.
