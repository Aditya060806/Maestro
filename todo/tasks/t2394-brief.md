...
- Date: 2026-04-19
- Context: Interactive diagnostic session reviewing why `Aditya060806/Maestro` pulse dispatched zero issues for multiple cycles while 28 issues sat open and worker capacity was idle (0/24).
...
**Root cause confirmed in production 2026-04-19.** Current behaviour traced on issue #19924 (`Aditya060806/Maestro`):
...
16:04:46 → 16:34:46 Aditya060806's healthy pulse sees ACTIVE_CLAIM and skips the issue
```

Over the diagnostic window, alex-solovyev's runner fast-failed 5+ times in 3 hours (likely running pre-t2392 `model-availability-helper.sh` that fails model probes). Each failure poisoned cross-runner dispatch for 30 minutes while the local state was already reset. `Aditya060806`'s pulse logs show `processed=23 dispatched=0` cycle after cycle — every candidate blocked by stale `ACTIVE_CLAIM: runner=alex-solovyev`.
