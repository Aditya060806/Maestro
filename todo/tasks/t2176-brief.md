...
The pulse supervisor process (PID varies, launched by `~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist` every 120s) must run under bash 4+ so that every framework script it invokes — including `memory-pressure-monitor.sh` which uses `${var,,}` — works without compatibility errors.
...
- **Direct failure.** `memory-pressure-monitor.sh` (routine `r905`, every 1 minute) exits with code 1 on line 488 (`${cmd_name,,}: bad substitution`) on every pulse cycle. The routine's job — detecting memory-pressure kill candidates — silently fails. Observed in `~/.maestro/logs/pulse.log`:
  ```
  /Users/Aditya060806/.maestro/agents/scripts/memory-pressure-monitor.sh: line 488: ${cmd_name,,}: bad substitution
...
/bin/bash -c 'source /Users/Aditya060806/.maestro/agents/scripts/shared-constants.sh; echo "BASH_VERSINFO[0]=${BASH_VERSINFO[0]}" ; echo "MAESTRO_BASH_REEXECED=${MAESTRO_BASH_REEXECED:-UNSET}"'
# If re-exec works: BASH_VERSINFO[0]=5, MAESTRO_BASH_REEXECED=1
# If re-exec fails: BASH_VERSINFO[0]=3, MAESTRO_BASH_REEXECED=UNSET
...
# - ~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist — hard-codes /bin/bash
...
- `EDIT: .agents/scripts/shared-constants.sh:44-58` — possibly: walk BASH_SOURCE[] to find the outermost caller, not just BASH_SOURCE[1]. Or: set MAESTRO_BASH_REEXECED=1 with a loop-guard timestamp to prevent runaway self-exec.
...
- `EDIT: setup.sh OR ~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist` template — stop hard-coding `/bin/bash` in the `ProgramArguments` array; either use modern bash directly if available, or invoke the script and let the shebang resolve via `/usr/bin/env bash`.
...
1. **Diagnose first.** Reproduce the failure in isolation:
   ...
   source /Users/Aditya060806/.maestro/agents/scripts/shared-constants.sh
...
2. **Pick the right fix based on diagnosis.** Three paths:
   - **Path A (most likely root cause): BASH_SOURCE[] stack depth.** If `pulse-wrapper.sh` sources `config-helper.sh` first, and `config-helper.sh` (directly or transitively) sources `shared-constants.sh`, then inside the guard `BASH_SOURCE[1]` is `config-helper.sh`, and the `exec` re-launches config-helper.sh as a standalone script — which doesn't do the pulse's work. Fix: walk the BASH_SOURCE stack to find the outermost caller:
     ```bash
     local _maestro_top_caller
     _maestro_top_caller="${BASH_SOURCE[${#BASH_SOURCE[@]}-1]:-${BASH_SOURCE[1]:-}}"
     # ... then exec with $_maestro_top_caller instead of ${BASH_SOURCE[1]}
     ```
   ...
   - **Path B: launchd plist hard-codes the interpreter.** `~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist` has:
     ```xml
     <array>
         <string>/bin/bash</string>
         <string>/Users/Aditya060806/.maestro/agents/scripts/pulse-wrapper.sh</string>
     </array>
     ```
...
3. **Audit other launchd plists.** Run:
   ```bash
   find ~/Library/LaunchAgents /Library/LaunchAgents /Library/LaunchDaemons -name "*maestro*.plist" -o -name "*sh.maestro*.plist" 2>/dev/null | \
       xargs -I{} grep -l "/bin/bash" {}
   ```
   Every match is a candidate for the same hard-code bug. Update the plists OR the generator templates in `setup.sh` / `~/.maestro/agents/scripts/maestro-*-helper.sh` that emit them.
...
4. **Regression test.** Create `.agents/scripts/tests/test-bash-reexec-guard.sh` modelled on `.agents/scripts/tests/test-compute-counter-seed-octal.sh`. Test cases:
   ...
   - Source with `MAESTRO_BASH_REEXECED=1` preset → no re-exec, runs under original bash (anti-loop guard intact).
...
5. **Deploy path.** Run `~/Git/maestro/setup.sh --non-interactive` after the fix so the deployed copies at `~/.maestro/agents/scripts/` match source. Restart the pulse:
   ```bash
   launchctl kickstart -k gui/$(id -u)/com.maestro.maestro-supervisor-pulse
   ```
...
sleep 180 && grep "bad substitution" ~/.maestro/logs/pulse.log | tail -5
...
- [ ] Zero `bad substitution` lines in `~/.maestro/logs/pulse.log` during a fresh pulse cycle after deploy.

  ```yaml
  verify:
    method: manual
    prompt: "After deploy and a full 3-minute pulse cycle, grep ~/.maestro/logs/pulse.log for 'bad substitution' from after the deploy timestamp. Expect zero matches."
  ```
...
- `~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist` — the plist that hard-codes `/bin/bash`.
