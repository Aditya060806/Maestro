...
- **Parent task:** t1741
- **Conversation context:** Discussed how Hyperspace AGI's cross-pollination (agents adopt peers' discoveries) maps to maestro's cross-session memory and the existing SQLite mailbox system. Each research session should start by recalling what worked before and checking peer discoveries, and end by storing what was learned. Concurrent agents share findings in real-time via mailbox `discovery` messages grouped by `convoy` ID.
...
The Hyperspace model shows that cross-pollination (sharing discoveries between agents) accelerates convergence. In maestro, two mechanisms serve this:
...
sqlite3 ~/.maestro/.agent-workspace/mail/mailbox.db \
...
- [ ] Memory store called after each experiment with structured finding
  ...
  verify:
    method: codebase
    pattern: "memory.*store|maestro_memory.*store"
    path: ".agents/tools/autoresearch/"
  ```
...
- `~/.maestro/.agent-workspace/memory/` — memory storage location
- `~/.maestro/.agent-workspace/mail/mailbox.db` — SQLite mailbox for inter-agent discoveries
