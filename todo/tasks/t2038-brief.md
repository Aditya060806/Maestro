...
EOF
curl -sS -i -X PATCH "https://api.github.com/repos/Aditya060806/Maestro/branches/main/protection/required_pull_request_reviews" \
...
```bash
gh api "repos/Aditya060806/Maestro/branches/main/protection" --jq '
  {
...
```bash
~/.maestro/agents/scripts/claim-task-id.sh \
  --repo-path /Users/Aditya060806/Git/maestro \
...
- **Repository access:** only `Aditya060806/Maestro` (no `all repositories` selector).
...
- **Owner identity:** `Aditya060806` (repo admin). When the PAT pushes, the commit author is the workflow identity but the authenticated principal is the admin user, which bypasses PR-review requirements via `enforce_admins: false`.
