...
- **Created:** 2026-04-21
- **Session:** opencode:interactive (Aditya060806)
- **Created by:** ai-interactive
- **Parent task:** —
- **Conversation context:** While cleaning 443 duplicate nudge comments from webapp after the v3.8.88 dedup fix, the agent posted a hallucinated signature footer (human-readable prose containing `maestro.sh` but none of the runtime/version/model/token metadata that `gh-signature-helper.sh` emits). Investigation showed the plugin hook accepted any command string containing `maestro.sh` as sufficient evidence of a valid footer, and raw `gh` on PATH bypasses the shell wrappers entirely. Both gaps allow hallucinated footers through.

## What

Close the two gaps that let hallucinated signature footers through review:

1. **PATH shim** at `.agents/scripts/gh` (deployed to `~/.maestro/agents/scripts/gh`, which is first on PATH via `shell-env.mjs`). Intercepts `gh issue comment|create` and `gh pr comment|create`, injects the helper-generated signature into `--body`/`--body-file` when the canonical marker `<!-- maestro:sig -->` is absent, then exec's the real `gh`. All other subcommands pass through with one case-match + one exec. Bypass: `MAESTRO_GH_SHIM_DISABLE=1`.
2. **Plugin hook tightening** in `.agents/plugins/opencode-maestro/quality-hooks.mjs::checkSignatureFooterGate`: replace the loose `cmd.includes("maestro.sh")` check with marker-based detection (`<!-- maestro:sig -->`), add transparent repair via `output.args.command` mutation, and throw a mentoring error when repair isn't safe.
...
- **Plugin hook** (`checkSignatureFooterGate`) — the literal prose `maestro.sh` matched `cmd.includes("maestro.sh")`; the hook logged WARN and allowed execution.
...
- **EDIT: `.agents/plugins/opencode-maestro/quality-hooks.mjs`** — split `checkSignatureFooterGate` into composable helpers (`isGhWriteCommand`, `isMachineProtocolCommand`, `hasTrustedSignatureSignal`, `tryRepairSignature`), mutate `output.args.command` for transparent repair, throw for unparseable commands with an error message that teaches the next attempt. Export functions for test harness.
...
- **NEW: `.agents/plugins/opencode-maestro/tests/test-signature-footer-gate.mjs`** — 32 test cases (5 describe blocks) covering each helper + end-to-end block vs repair paths, including the exact t2685 regression case (`--body "... maestro.sh ..."` with no marker).

### Files Scope

- `.agents/scripts/gh`
- `.agents/scripts/setup/_deployment.sh`
- `.agents/plugins/opencode-maestro/quality-hooks.mjs`
- `.agents/plugins/opencode-maestro/tests/test-signature-footer-gate.mjs`
...
node --test .agents/plugins/opencode-maestro/tests/test-signature-footer-gate.mjs
...
for f in .agents/plugins/opencode-maestro/tests/*.mjs; do node --test "$f"; done
```
...
2. **Plugin hook tests pass** — `node --test .agents/plugins/opencode-maestro/tests/test-signature-footer-gate.mjs` reports 32/32 pass.
...
4. **Hallucination regression case covered** — test file explicitly asserts that `hasTrustedSignatureSignal('... --body "... from maestro.sh session"')` returns `false` and that `checkSignatureFooterGate` on an unparseable body containing bare `maestro.sh` throws.
