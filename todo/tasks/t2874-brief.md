...
- [x] Memory recall: "structured content format markdown tags schema" — no prior maestro work on this; BaseHub uses Markdoc for similar problem (audited 2026-04-26 in t2840 design session)
...
- **Phase 6 — retrieval consumer** — wire P5/P6 retrieval (`maestro case draft`, `maestro knowledge search`) to filter and rank by tag attributes (e.g. `--sensitivity privileged --case acme-dispute`).
...
- **Why `source.md + meta.json` not single-file `.bshb`-style:** BaseHub uses single-file-per-item (binary + metadata + content together). That fights our binary/text-separation design (large blobs route to `~/.maestro/.agent-workspace/knowledge-blobs/`, binary diffs are git-hostile). Directory-per-source with `source.md` + `meta.json` + `extracted.json` + `tree.json` keeps each artefact diff-friendly and tool-friendly.
