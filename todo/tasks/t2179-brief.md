...
- **Conversation context:** While clearing 4 stuck PRs in `Aditya060806/Maestro`, PR #19630 (t2174 opencode-db-maintenance) sat BLOCKED for ~1 hour with all CI green. The blocker was a CodeRabbit `CHANGES_REQUESTED` review whose actionable items were pure cosmetics (function ordering, dead var, fence language tag). User asked: "should the pulse have a way to honour these as 'I've read them, ship it' without me dismissing each time?"
...
- Related but distinct: `ai-approved` label (collaborator gate, not cryptographic), `<!-- maestro-signed-approval -->` marker (crypto-signed approval for external contributors). This is neither — it's a "I read CodeRabbit's nits and they're fine" override.
