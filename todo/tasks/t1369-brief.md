...
- **Conversation context:** User asked whether PaddleOCR was in maestro. Comparison revealed our stack (MinerU + Docling + ExtractThinker) handles document parsing well but lacks scene text OCR — reading text from screenshots, photos, signs, UI captures. User anticipates screenshot reading needs and wants PaddleOCR added.

## What

Add PaddleOCR as a specialist OCR tool in maestro, focused on **scene text / screenshot OCR** — the capability gap our current stack doesn't cover. Deliverables:
...
- **Gap:** No tool in maestro can read text from screenshots, photos, UI captures, or scene images. MinerU/Docling are document parsers, not scene text OCR engines.
