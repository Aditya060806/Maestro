...
- [x] File refs verified: `.agents/scripts/stats-health-dashboard.sh` (1419 lines, deployed matches source), `.agents/scripts/shared-gh-wrappers-rest-fallback.sh` (present), `~/.maestro/logs/stats.log` (84 rate-limit failures, 19 creation failures today)
...
- **Created by:** ai-interactive (Aditya060806 directed investigation)
- **Parent task:** none
- **Conversation context:** User noticed duplicate `[Supervisor:*]` issues on `Aditya060806/Maestro` (and later `exampleorg/examplerepo`). Investigation traced the incident to a GraphQL rate-limit window on 2026-04-21 04:00-13:30 UTC during which `_find_health_issue` silently treated query failures as "not found" and created duplicates.
...
- **Signal integrity:** Duplicate supervisor issues corrupt pin slots, confuse human monitoring, and create noise in per-runner dashboards. Multiple open `[Supervisor:Aditya060806]` and `[Supervisor:alex-solovyev]` issues actively mislead.
...
- `NEW: .agents/scripts/migrate-health-issue-duplicates-t2687.sh` — one-shot migration that closes duplicates per (repo, runner, role). Marker file: `~/.maestro/logs/.migrated-health-issue-duplicates-t2687`.
...
4. **Periodic dedup on cache hits.** Track last-dedup-scan time per repo in `~/.maestro/logs/health-dedup-last-scan-<slug_safe>`. On each `_update_health_issue_for_repo` call, if > 1 hour since last scan, run the label-based dedup scan even if cache is valid. Close duplicates per the existing `:82-95` logic.
...
for slug in $(jq -r '.initialized_repos[] | select(.pulse == true) | .slug' ~/.config/maestro/repos.json); do
...
- `~/.maestro/logs/stats.log` — evidence trail
