...
# t2858: `maestro case chase` (template-only, opt-in auto-send)
...
7. CLI: `maestro case chase <case-id> --template <name>`; `maestro case chase-template add|list|test`
...
- EDIT: `.agents/cli/maestro` (add `case chase` and `case chase-template` subcommand groups)
...
- EDIT: `.agents/maestro/cases-plane.md` (chasing section + opt-in policy)

## Acceptance Criteria

- [ ] `maestro case chase <case-id> --template payment-reminder` substitutes fields, sends email, records timeline + sent.jsonl
...
- [ ] `maestro case chase-template test --case <id> --template <name>`: dry-run substitution shows the would-send email
- [ ] `maestro case chase-template list` shows all templates with description
...
- [ ] Documentation: chasing + opt-in policy in `.agents/maestro/cases-plane.md`
