...
- **Created by:** Aditya060806 (human, operator during live dispatch outage diagnosis)
- **Parent task:** none
...
jq '.anthropic[] | {email, status, cooldownUntil}' ~/.maestro/oauth-pool.json

# 3. Existing isolation site to hook into:
grep -n "maestro-worker-auth" .agents/scripts/headless-runtime-helper.sh
...
@@ -28,7 +28,11 @@
 POOL_FILE="${MAESTRO_OAUTH_POOL_FILE:-${HOME}/.maestro/oauth-pool.json}"
...
- **Alternatives considered and rejected:**
  1. _Split env vars (e.g., `MAESTRO_POOL_AUTH_FILE`)_ — cleaner in isolation but introduces a new coupling point; XDG_DATA_HOME is the existing standard.
