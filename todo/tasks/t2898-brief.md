...
# t2898: maestro update + setup.sh — verify auto-update daemon is enabled and warn if not
...
- [x] File refs verified: `maestro.sh`, `setup.sh`, `auto-update-helper.sh`, the launchd plist template (search `setup-modules/` for the install path) all present at HEAD.
...
3. **`maestro update` warns if the daemon is unhealthy after the update completes.** When invoked interactively (not from the daemon itself — detect via `MAESTRO_AUTO_UPDATE=1` or similar process-env signal), call `health-check` and print a yellow warning if non-zero, with the remediation command. Also write a `daemon-disabled` advisory to `~/.maestro/advisories/` so the session greeting picks it up if/when the user is interactive.

The combined effect: every release self-heals the daemon. Even a runner that was installed without the daemon (or had it removed) gets it re-installed automatically the next time `setup.sh` runs — which is every release when the daemon IS working. For a runner where the daemon was never enabled, the user just has to run `maestro update` once to bootstrap the loop.
...
The fix is to put the verification on paths that run in both modes: `setup.sh` (runs during every release deploy) and `maestro update` (runs every 10 minutes from the daemon, when it works). When the daemon is healthy, every release re-verifies it. When it's broken, the next manual `maestro update` re-enables it. The path that doesn't break naturally heals the path that did.
...
- [x] 2 or fewer files to modify? — borderline (4 files: maestro.sh, setup.sh, auto-update-helper.sh, tests). Treating as more than 2.
...
- [ ] Dispatch-path classification (t2821): `auto-update-helper.sh`, `setup.sh`, `maestro.sh` are NOT in `.agents/configs/self-hosting-files.conf`. Auto-dispatch is appropriate.
...
- `EDIT: maestro.sh` — find the `update` subcommand handler. After the update completes, if invoked interactively (heuristic: `[[ -t 0 ]] && [[ -z "${MAESTRO_AUTO_UPDATE:-}" ]]`), call `auto-update-helper.sh health-check` and print a yellow warning + remediation command on non-zero exit.
...
_cmd_health_check() {
    ...
    case "$platform" in
        Darwin)
            ...
            if launchctl list 2>/dev/null | grep -q "com.maestro.maestro-auto-update"; then
                unit_loaded=1
            fi
            ;;
        Linux)
            # Try systemd user unit first, then cron.
            if systemctl --user is-active --quiet maestro-auto-update.timer 2>/dev/null; then
                unit_loaded=1
    ...
    if [[ $unit_loaded -eq 0 ]]; then
        ...
        printf "fix: ~/.maestro/agents/scripts/auto-update-helper.sh enable\n" >&2
        return 2
    fi
    ...
    if (( age_sec > 2 * interval_sec )); then
        ...
        printf "fix: ~/.maestro/agents/scripts/auto-update-helper.sh check\n" >&2
        return 1
    fi
...
if "$HOME/.maestro/agents/scripts/auto-update-helper.sh" enable --idempotent; then
    if "$HOME/.maestro/agents/scripts/auto-update-helper.sh" health-check; then
...
**4. Hook into `maestro.sh update` subcommand.** After update completes, if interactive:

```bash
if [[ -t 0 ]] && [[ -z "${MAESTRO_AUTO_UPDATE:-}" ]]; then
    if ! "$HOME/.maestro/agents/scripts/auto-update-helper.sh" health-check; then
        printf "\n\033[33mwarning:\033[0m auto-update daemon is not running. Run: auto-update-helper.sh enable\n" >&2
        # Drop a session-greeting advisory.
        mkdir -p "$HOME/.maestro/advisories"
        cat >"$HOME/.maestro/advisories/daemon-disabled.advisory" <<EOF
...
upstream. Fix: ~/.maestro/agents/scripts/auto-update-helper.sh enable
EOF
    fi
fi
```
...
# Manual smoke test
~/.maestro/agents/scripts/auto-update-helper.sh health-check && echo "healthy"
~/.maestro/agents/scripts/auto-update-helper.sh enable --idempotent
~/.maestro/agents/scripts/auto-update-helper.sh enable --idempotent  # second call should no-op cleanly

# Verify maestro update warns when daemon disabled (manual):
launchctl unload ~/Library/LaunchAgents/com.maestro.maestro-auto-update.plist
maestro update                                # should print yellow warning + advisory file
ls ~/.maestro/advisories/daemon-disabled.advisory  # should exist
launchctl load ~/Library/LaunchAgents/com.maestro.maestro-auto-update.plist
maestro update                                # advisory should be cleared
...
- [ ] Running `maestro update` interactively when the daemon is unhealthy prints a yellow warning AND writes `~/.maestro/advisories/daemon-disabled.advisory`. The advisory is removed (or rewritten as cleared) on the next update where the daemon is healthy.
...
- **Advisory dedup.** Writing `~/.maestro/advisories/daemon-disabled.advisory` is idempotent (overwrite). The session greeting reads it once per session start. No need for explicit dedup.
...
- **Blocked by:** none.
- **Blocks:** none directly; t2897 benefits from this landing first because the breaker's `maestro update` call is more useful when the daemon-install path is self-healing. But neither blocks the other strictly.
- **External:** none.
...
| Phase | Time | Notes |
|-------|------|-------|
| Research/read | 20m | auto-update-helper.sh dispatch block, setup-modules/agent-deploy.sh end-of-run, maestro.sh update subcommand. |
