...
```yaml
          ...
          if [[ "$PUSH_SUCCEEDED" != "true" ]]; then
            ...
            **Run this locally to complete the audit trail:**
            ...
            \`\`\`bash
            ~/.maestro/agents/scripts/task-complete-helper.sh $TASK_ID --pr $PR_NUMBER --testing-level self-assessed
            \`\`\`
...
```bash
~/.maestro/agents/scripts/claim-task-id.sh --repo-path /Users/Aditya060806/Git/maestro --title "chore: migrate classic branch protection to Rulesets with github-actions bypass (follow-up to t2029)"
```
