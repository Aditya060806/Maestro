...
Reduce total file complexity of `.agents/plugins/opencode-maestro/oauth-pool.mjs` (3229 lines) from 440 to under 120 by decomposing the 12 high-complexity functions into smaller units. Each function should have complexity under 15 after refactoring. The file's exported API must remain unchanged.
...
1. Run `qlty smells .agents/plugins/opencode-maestro/oauth-pool.mjs --sarif` to get precise function names and line ranges
...
Key files:
- `.agents/plugins/opencode-maestro/oauth-pool.mjs` — target (3229 lines, 96 functions, 12 high-complexity)
- `.agents/plugins/opencode-maestro/provider-auth.mjs` — tightly coupled, imports from oauth-pool
- `.agents/plugins/opencode-maestro/index.mjs` — plugin entry point
...
- [ ] All exported functions maintain identical signatures
  ...
  verify:
    method: bash
    run: "cd /Users/Aditya060806/Git/maestro && grep -c 'export' .agents/plugins/opencode-maestro/oauth-pool.mjs"
  ```
...
- `.agents/plugins/opencode-maestro/oauth-pool.mjs:1` — target file (3229 lines)
- `.agents/plugins/opencode-maestro/provider-auth.mjs` — coupled; refactored in t1858
- `.agents/plugins/opencode-maestro/index.mjs` — plugin entry, wires pool
