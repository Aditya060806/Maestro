...
2. **Add regression test.** First locate the existing test file that covers `_is_protected_label` and `_reconcile_labels`:
   ...
   If no test file exists for this function specifically, create `.agents/scripts/tests/test-issue-sync-protected-labels.sh` with the standard maestro test harness pattern. Test cases:
