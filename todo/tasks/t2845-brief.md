...
- [x] Memory recall: `nmr review gate maintainer approval` → existing `pulse-nmr-approval.sh` and `sudo maestro approve issue` flow
...
5. Crypto-approval hook: extend `maestro approve issue <N>` to recognise knowledge-review issues and promote staging → sources atomically (move dir + update meta.json `state` field)
...
4. **Crypto-approval hook** — extend `sudo maestro approve issue <N>` (or its dispatch path in `pulse-nmr-approval.sh`) to detect knowledge-review issues (label `kind:knowledge-review`) and call `knowledge-review-helper.sh promote <source-id>` after approval
...
- EDIT: `.agents/maestro/knowledge-plane.md` (review gate section)
...
- [ ] `sudo maestro approve issue <N>` on a knowledge-review issue: removes NMR, promotes source to `sources/`, closes issue with summary comment
...
- [ ] Documentation: review gate diagram + trust ladder in `.agents/maestro/knowledge-plane.md`
