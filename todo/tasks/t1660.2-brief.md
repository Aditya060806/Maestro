...
# t1660.2: .maestro/testing.json Schema Definition
...
Define the JSON Schema for `.maestro/testing.json` — the per-repo testing configuration file that the maestro framework reads to understand how to start, test, and smoke-check a project's development environment.

**Deliverables:**
- `.agents/configs/testing.schema.json` — JSON Schema (draft/2020-12) for `.maestro/testing.json`
...
6. Schema lives in `.agents/configs/` (deployed to `~/.maestro/agents/configs/`) — not in the project root

**Schema location:** `.agents/configs/testing.schema.json`
**User file location:** `.maestro/testing.json` (per-repo, in project root, gitignored by default)
...
- [ ] Schema uses JSON Schema draft/2020-12 (consistent with `bundle.schema.json` and `maestro-config.schema.json`)
- [ ] `$id` is `https://maestro.sh/schemas/testing.json` (consistent with other schemas)
...
- Existing schema style reference: `.agents/bundles/schema.json`, `.agents/configs/maestro-config.schema.json`
