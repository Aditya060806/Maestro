...
- **Conversation context:** During this session, two pulse runners (`Aditya060806` and `alex-solovyev`) raced repeatedly on issues. The race patterns are documented in scattered places (PR bodies, AGENTS.md, GH#18352, t1986 brief) but no single doc explains the multi-runner mental model: who locks what, who can override what, when the runners coordinate vs when they don't, what failure modes to expect, and what the maintainer should configure on each machine.

## What

Create `.agents/reference/cross-runner-coordination.md` documenting how maestro handles multiple concurrent pulse runners. Audience: a maintainer who is bringing up a new runner machine, or debugging a multi-runner race.
...
7. **The setup of `~/.config/maestro/repos.json` `pulse: true`** and the implications of dispatching to issues outside `PULSE_SCOPE_REPOS`.
...
gh pr view 18352 --repo Aditya060806/Maestro  # GH#18352 cross-runner assignee guard
gh pr view 18374 --repo Aditya060806/Maestro  # t1970 interactive-claim race fix
gh pr view 18419 --repo Aditya060806/Maestro  # t1986 parent-task guard

# 3. Issue threads with the race incidents
gh issue view 18356 --repo Aditya060806/Maestro --comments  # the parent-task incident (t1962 Phase 3)
gh issue view 18367 --repo Aditya060806/Maestro --comments  # the t1967 interactive-claim race
gh issue view 18399 --repo Aditya060806/Maestro --comments  # the t1986 self-fulfilling race
