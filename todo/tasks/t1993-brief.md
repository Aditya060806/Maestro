...
1. **Scheduling**: add a routine that invokes the scanner regularly. Two options:
   ...
   - **Option B (fallback)**: add a TODO.md routine entry `- [x] rNNN` with `repeat: daily(@03:00)` and `run: scripts/post-merge-review-scanner.sh scan Aditya060806/Maestro`. This is the same cadence as the current milohiss sweep, lives outside the pulse code path, and is easier to disable if it misbehaves.
...
- No mention of `post-merge-review-scanner.sh` in `~/.maestro/logs/pulse.log`, `~/.maestro/logs/stats.log`, any `~/Library/LaunchAgents/` plist, or `crontab -l`.
- The helper is listed alphabetically in `~/.maestro/logs/routine-gh-failure-miner.log` but that's just a directory listing, not an invocation.
...
- `EDIT: TODO.md` — add a new `- [x] r0NN` routine under the `## Routines` section with `repeat: daily(@03:00)`, `run: scripts/post-merge-review-scanner.sh scan Aditya060806/Maestro`, and appropriate SCANNER_DAYS env override.
...
6. **Execute the one-shot backfill** (not part of the recurring routine — runs once during implementation verification):
   ...
   ```bash
   SCANNER_DAYS=30 bash .agents/scripts/post-merge-review-scanner.sh scan Aditya060806/Maestro
   ```
...
2. The routine's `run:` points to the scanner directly. The routine dispatcher will invoke `scripts/post-merge-review-scanner.sh scan` — verify the scanner accepts zero-arg invocation for the default repo, or extend the routine `run:` to pass `scan Aditya060806/Maestro` explicitly.
...
SCANNER_DAYS=30 bash .agents/scripts/post-merge-review-scanner.sh scan Aditya060806/Maestro
gh issue list --repo Aditya060806/Maestro --label review-followup --state open --limit 20
...
- [ ] One-shot 30-day backfill has been executed and at least one `review-followup` issue has been created referencing PR #18405 with the jq-in-loop finding.
  ...
  verify:
    method: manual
    prompt: "Run: gh issue list --repo Aditya060806/Maestro --label review-followup --search '#18405' --state all — confirm at least one issue exists and its body cites pulse-triage.sh and Gemini's jq consolidation finding."
  ```
...
- **Ruled out:**
  ...
  - *Scanning all repos in `repos.json` rather than just `Aditya060806/Maestro`* — Option A does this already; Option B would need the routine to loop. The scanner handles one repo per invocation so this works naturally.
