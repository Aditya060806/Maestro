...
Document — and decide how to close — the gap between what the maestro opencode plugin's OTEL enrichment expects (an active `Tool.execute` span to attach `maestro.intent`, `maestro.task_id`, `maestro.session_origin`, `maestro.runtime` to) and what opencode v1.4.11 actually emits in `run` mode.
...
Consequence: the framework's primary audit-trail join key — `maestro.task_id` → TODO.md — is invisible on every tool call in headless `run` mode, even with the t2186 OTEL passthrough fix in place. The plugin records the call in its own SQLite (`~/.maestro/plugin-data/opencode-maestro.db`, `tool_calls` table), but OTEL-side observability is effectively empty of per-tool context.
