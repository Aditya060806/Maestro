...
- [x] File refs verified: `.agents/plugins/opencode-maestro/quality-hooks-signature.mjs:299-377`, `.agents/scripts/gh:198,228`, `.agents/AGENTS.md` "Signature footer hallucination (t2685)" section all checked at HEAD `dcce82c66`
...
- `EDIT: .agents/plugins/opencode-maestro/quality-hooks-signature.mjs:226-238` — `_repairBodyFile`: return a structured failure object instead of null on error
- `EDIT: .agents/plugins/opencode-maestro/quality-hooks-signature.mjs:197-214` — `_generateSignature`: same structured-failure pattern
- `EDIT: .agents/plugins/opencode-maestro/quality-hooks-signature.mjs:299-326` — `tryRepairSignature`: thread the structured failure up
- `EDIT: .agents/plugins/opencode-maestro/quality-hooks-signature.mjs:335-377` — `checkSignatureFooterGate`: format the throw message using the specific failure cause
...
- `.agents/plugins/opencode-maestro/quality-hooks-signature.mjs:178-186` (`_hasUnparseableBody`) — current pattern for distinguishing failure modes via separate predicates. Extend with a "reason" field instead of bool.
- `.agents/plugins/opencode-maestro/validators.mjs` (any structured-failure pattern in there) — model on existing JS conventions in the plugin codebase.
   ...
   export function checkSignatureFooterGate(cmd, log, scriptsDir, output) {
     ...
     if (scriptsDir && output && output.args) {
       ...
       const sameCallHint = result.reason === FAIL_REASON.FILE_NOT_FOUND
         ...
         : "";
       throw new Error(
         `maestro: gh write command blocked at signature gate (t2685).\n\n` +
           ...
           `Emergency bypass (breaks audit trail): MAESTRO_GH_SHIM_DISABLE=1 gh ... — but the plugin hook still blocks.`,
       );
     }
   }
   ```
...
- **Action required:** none — buffer to 80-line warning threshold (`MAESTRO_COMPLEXITY_WARN_THRESHOLD`) is comfortable.
...
gh issue comment 99999 --repo Aditya060806/Maestro-test --body-file /tmp/sig-test/does-not-exist.md 2>&1 | tee /tmp/sig-test/error.txt
...
bash -c "source ~/.maestro/agents/scripts/shared-gh-wrappers.sh && gh_issue_comment 99999 --repo Aditya060806/Maestro-test --body-file /tmp/sig-test/wrapper-test.md --dry-run 2>&1" | grep -qv "Hook auto-repair could not parse" && echo "PASS: wrapper path clean"
...
gh issue comment 99999 --repo Aditya060806/Maestro-test --body-file /tmp/sig-test/precreated.md --dry-run 2>&1 | grep -qv "missing signature footer" && echo "PASS: pre-created file unaffected"

# 5. Verify quality-hooks log records the structured cause
tail -5 ~/.maestro/logs/quality-hooks.log | grep -E "Specific cause:|FAIL_REASON" && echo "PASS: structured logging"
...
- `.agents/plugins/opencode-maestro/quality-hooks-signature.mjs`
- `.agents/scripts/gh`
- `.agents/AGENTS.md`
...
- `.agents/plugins/opencode-maestro/quality-hooks-signature.mjs:299-377` — primary edit target
...
- `.agents/plugins/opencode-maestro/quality-hooks.mjs` — sibling hook file (sourced via `from "./quality-hooks-signature.mjs"`)
- `~/.maestro/logs/quality-hooks.log` — where structured failure logs land
