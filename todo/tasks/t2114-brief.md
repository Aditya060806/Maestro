...
Having a dedicated subcommand also makes it easy to invoke from `pulse-issue-reconcile.sh reconcile_labelless_maestro_issues` (t2112) as a targeted one-issue call, closing the loop: reconcile detects the labelless child, calls `backfill-sub-issues --issue N`, parent-child link appears.
