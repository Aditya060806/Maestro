...
- [x] Memory recall: `maestro reach proxy vpn fingerprint failover classification` → 0 hits — no relevant prior lesson found.
...
- `EDIT: .agents/maestro/reach-capture.md` — document failure classes and failover rules.
...
./maestro.sh reach network doctor --format json
./maestro.sh reach classify-failure --http-status 429 --format json
```
