...
- **Conversation context:** Review of GH#24520 proved that org-owned reusable workflow callers are misclassified because workflow helpers hardcode `Aditya060806/Maestro` as the only trusted reusable workflow target. The review approved the issue but required a worker-ready TODO/brief before dispatch.
...
must classify as `CURRENT/CALLER` when the rest of the caller shape matches the canonical template, and `sync-workflows` must render the same configured target. Unconfigured users must continue to get `Aditya060806/Maestro/.github/workflows/*-reusable.yml@main`.
...
- [x] **No cross-package or cross-module changes?** Yes — all changes remain within maestro workflow tooling/docs/tests.
...
- `.agents/scripts/check-workflows-helper.sh:157` currently normalizes refs only for `Aditya060806/Maestro/.github/workflows/${_reusable_escaped}`.
- `.agents/scripts/check-workflows-helper.sh:214` currently treats only `uses: Aditya060806/Maestro/.github/workflows/${_reusable_escaped}@` as a downstream caller.
- `.agents/scripts/sync-workflows-helper.sh:203` currently rewrites refs only for `uses: Aditya060806/Maestro/.github/workflows/...`.
- `.agents/scripts/sync-workflows-helper.sh:257` currently injects `runner:` only after a `uses:` line beginning with `Aditya060806/Maestro`.
- `.agents/templates/workflows/issue-sync-caller.yml:57` still ships default `uses: Aditya060806/Maestro/.github/workflows/issue-sync-reusable.yml@main`.
...
1. Add a shared target resolver in the workflow helpers.
   - Provide defaults: repo `Aditya060806/Maestro`, ref `main` or `@main` normalized internally to exactly one leading `@` at render time.
...
2. Use the resolver everywhere caller identity is built or matched.
   ...
   - Keep self-caller detection (`uses: ./.github/workflows/...`) unchanged for the maestro repo.
...
4. Separate caller-target drift from reusable-content/update drift.
   ...
   - Reusable content drift: an organization-owned repository may also carry copies of maestro `*-reusable.yml`. `check-workflows` must still be able to identify that those reusable workflow files are older than the maestro templates/baseline and report an update recommendation rather than hiding the update behind `CURRENT/CALLER`.
...
- [ ] Unconfigured behavior remains byte-compatible for downstream callers using `Aditya060806/Maestro/.github/workflows/*-reusable.yml@main` except intentional ref normalization.
