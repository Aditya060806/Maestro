...
GH#12141 root cause: both `Aditya060806` (v3.5.87 opus) and `alex-solovyev` (v3.5.86 sonnet) pulse sessions dispatched workers for the same issue within 2 minutes. **Neither pulse ran `check_dispatch_dedup` at all** — zero `DISPATCH_CLAIM` comments exist on the issue, and no "Dispatch skipped" comment was posted.
...
- 00:36 — `Aditya060806` opus pulse posts "Dispatching worker" (no prior claim)
