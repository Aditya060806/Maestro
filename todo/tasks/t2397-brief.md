...
**Root cause confirmed in production 2026-04-19.** Pulse log (`~/.maestro/logs/pulse.log`):
...
- Storage: fast-fail counter file likely lives at `~/.maestro/.agent-workspace/fast-fail/${slug-safe}-${issue}.json` — verify and preserve the existing schema when adding `last_reset_ts`.
...
tail -f ~/.maestro/logs/pulse-wrapper.log | grep "fast_fail_age_out"

# Confirm #19864 and #19740 dispatch on next cycle
gh api repos/Aditya060806/Maestro/issues/19864 --jq '.labels[].name'
...
- Not the same as manual `sudo maestro approve issue N` — age-out is autonomous, approval is human override.
