...
When all 5 children merge, the maestro repo should show:
...
- **Cluster-per-task over file-per-task:** PRs #18893, #18906, #18948 all shipped multi-file decompositions in single PRs. Matches the natural coupling (files in `higgsfield/` share state, files in `opencode-maestro/oauth-pool*` share the rotation layer). One-file-per-task would create 20 worker dispatches and 20 PRs for work that logically belongs in 5 units.
