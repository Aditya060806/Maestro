...
Today: `pulse-wrapper.sh` pre-fetches all open issues + PRs into a state blob, injects between `--- PRE-FETCHED STATE ---` markers, LLM reads linearly. ~3K tokens/cycle on maestro (12 issues). At 1 200 issues: ~60K tokens. At 12 000 issues: unusable. Cost grows linearly with backlog even when almost nothing changed.
...
Without this: maestro works, other pulse-enabled repos don't scale, and the framework can't be recommended for anything beyond small personal repos.
...
2. **`pulse-state-digest.sh`** — three commands:
   ...
   - Cache path: `~/.maestro/cache/pulse-state-digest.$(hostname -s).${slug//\//__}.json`
