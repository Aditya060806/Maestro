...
- **Conversation context:** Anthropic released Opus 4.7 on 2026-04-16. User asked to add it as an option in the maestro plugin, explicitly noting documented tradeoffs (Anthropic's own migration guide: 1.0-1.35x tokenizer expansion, more output tokens, stricter literal instruction-following) plus third-party data (MRCR v2 long-context collapse: 256K 91.9%->59.2%, 1M 78.3%->32.2%; English tokenizer +58.8% vs 4.6 on paragraph samples). Decision: register 4.7 so it's selectable but keep every default on 4.6 until we've validated it against the framework's agentic workloads.
...
- EDIT: `.agents/plugins/opencode-maestro/config-hook.mjs` — add `claude-opus-4-7` entry to `CLAUDE_MODEL_LIMITS` and both name maps
- EDIT: `.agents/plugins/opencode-maestro/claude-proxy.mjs` — add `claude-opus-4-7` entry to `getClaudeProxyModels()`
...
3. `grep -c "claude-opus-4-7" .agents/plugins/opencode-maestro/config-hook.mjs` — returns 3 (limits + 2 name maps)
