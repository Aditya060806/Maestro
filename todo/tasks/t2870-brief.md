...
- **Asset binary heavy:** logos, video, audio — heavy use of the `~/.maestro/.agent-workspace/knowledge-blobs/` 30MB-threshold path.
...
- Phase 2 — campaign CLI surface (`maestro campaign new|list|status|launch|archive`) + campaign-id provisioning
...
- Phase 4 — asset binary integration (large files routed to `~/.maestro/.agent-workspace/knowledge-blobs/` with path traversal validation and filename collision avoidance to prevent writes outside the managed workspace or accidental data loss; thumbnail/preview generation)
- Phase 5 — AI creative agent (`maestro campaign draft <id> --channel <name>`) — RAG-grounded in `lib/brand/`, human-gated, channel-aware
