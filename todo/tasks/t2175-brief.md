...
1. **Tighten the scan selector.** In `pulse-routines.sh`, the grep at line ~250 currently reads:
   ...
   Verify by running against `~/Git/maestro/TODO.md` — the t2160 line must NOT match; the dozen r-prefix entries in `~/Git/maestro-routines/TODO.md` must still match.
...
tail -f ~/.maestro/logs/pulse-wrapper.log | grep -E "unrecognised schedule expression"
...
grep "routine r912" ~/.maestro/logs/pulse.log | tail -5
...
- [ ] Zero `unrecognised schedule expression` lines in a fresh pulse cycle after deploy.
  ...
  verify:
    method: manual
    prompt: "After deploy, tail ~/.maestro/logs/pulse-wrapper.log for 5 minutes and confirm no 'unrecognised schedule expression' lines appear."
  ```
...
- **Why not remove `repeat:persistent` from `maestro-routines/TODO.md:28` instead?** The dashboard IS conceptually a persistent routine, managed by launchd. The TODO.md entry serves as documentation that it exists and is enabled. Making the scheduler aware of the keyword (rather than forcing users to omit/hide persistent jobs) is the cleaner long-term model — future persistent routines (other daemons) will want the same treatment.
...
- `~/Git/maestro-routines/TODO.md:28` — the `r912 persistent` entry that triggered error-path #2.
- `~/Git/maestro/TODO.md` — contains the `t2160` completed entry that triggered error-path #1 (description has `` `repeat:([^[:space:]]+)` ``).
