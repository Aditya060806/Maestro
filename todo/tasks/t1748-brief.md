...
- `MAESTRO_CLIPBOARD_COPY` — `pbcopy`, `xclip -selection clipboard`, `clip.exe`
- `MAESTRO_CLIPBOARD_PASTE` — `pbpaste`, `xclip -selection clipboard -o`, `powershell.exe -c Get-Clipboard`
- `MAESTRO_OPEN_CMD` — `open`, `xdg-open`, `wslview`
- `MAESTRO_FILE_SEARCH` — `mdfind`, `locate`/`fd`
...
- [ ] `setup.sh` completes without error on Ubuntu 22.04 (WSL2 or native)
  ...
  verify:
    method: bash
    run: "grep -q 'platform-detect\\|MAESTRO_PLATFORM' .agents/scripts/platform-detect.sh"
  ```
...
- [ ] No script uses `pbcopy`, `mdfind`, `osascript`, or `open` without platform guard
  ...
  verify:
    method: bash
    run: "! rg -l '\\b(pbcopy|pbpaste|mdfind|osascript)\\b' .agents/scripts/ --glob '!platform-detect.sh' | rg -v 'platform.*guard\\|uname\\|OSTYPE\\|MAESTRO_PLATFORM'"
  ```
