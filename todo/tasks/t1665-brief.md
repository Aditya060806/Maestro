...
| Runtime | Mechanism | How maestro uses it |
|---------|-----------|---------------------|
| OpenCode | `instructions` field in `opencode.json` (array of file paths, auto-loaded every session) | Points to `~/.maestro/agents/AGENTS.md` |
...
- `.agents/plugins/opencode-maestro/` — OpenCode plugin API
...
- [ ] `runtime-registry.sh` defines 11+ runtimes with all properties (binary, config path, format, MCP root key, command dir, prompt mechanism, session DB, process pattern)
  ...
  verify:
    method: bash
    run: "source ~/.maestro/agents/scripts/runtime-registry.sh && [[ ${#RUNTIME_IDS[@]} -ge 11 ]]"
  ```
- [ ] `detect_installed_runtimes()` returns only runtimes whose binary is in PATH
  ```yaml
  verify:
    method: bash
    run: "source ~/.maestro/agents/scripts/runtime-registry.sh && detect_installed_runtimes | head -1"
  ```
...
- [ ] No regression in `maestro update` — existing OpenCode and Claude Code configs still generated correctly
