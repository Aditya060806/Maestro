...
- `.agents/maestro/security.md` - Update security guidance
...
- [ ] 4.0 Update plugins and configs ~15m (ai:15m)
  - [ ] 4.1 Update `.agents/plugins/opencode-maestro/index.mjs` ~5m
...
- [ ] 5.0 Verify rename completeness ~30m (ai:15m test:15m)
  ...
  - [ ] 5.3 Test `source ~/.config/maestro/mcp-env.sh` still works via symlink ~5m
  - [ ] 5.4 Test `source ~/.config/maestro/credentials.sh` works directly ~5m
...
- [ ] 6.0 gopass setup and documentation ~2h (ai:1.5h read:30m)
  - [ ] 6.1 Create `.agents/tools/credentials/gopass.md` subagent ~45m
    ...
    - Store structure for maestro (`maestro/` prefix)
...
- [ ] 7.0 Build `maestro secret` wrapper ~3h (ai:2.5h test:30m)
  - [ ] 7.1 Create `secret-helper.sh` with core commands ~1.5h
    - `init` -- run `gopass setup` if not initialized
    - `set NAME` -- delegate to `gopass insert maestro/NAME`
    - `list` -- delegate to `gopass ls maestro/`
  ...
  - [ ] 7.4 Add `maestro secret` subcommand to `maestro.sh` CLI ~15m
...
- [ ] 10.0 Update agent instructions ~1h (ai:45m read:15m)
  ...
  - [ ] 10.4 Update `maestro/security.md` with gopass guidance ~15m
...
- [ ] 12.0 Testing & Quality ~1h (ai:30m test:30m)
  ...
  - [ ] 12.3 Test `maestro secret run` with output redaction ~10m
...
7.0,,Build maestro secret wrapper,3h,2.5h,30m,pending,,
...
- [ ] `maestro secret set/list/run` functional with gopass
