...
- **Worktree:** `~/Git/maestro-bugfix-t2019-triage-headerless-output`
...
- Parent session: t2016 (PR #18476, released v3.7.3) made triage failures
  ...
  hit.
- Evidence: `~/.maestro/logs/pulse-wrapper.log` contains three consecutive
...
source ~/.maestro/agents/scripts/shared-constants.sh
source ~/.maestro/agents/scripts/pulse-ancillary-dispatch.sh
LOGFILE=/tmp/t2019-debug.log \
  _build_triage_review_prompt <N> Aditya060806/Maestro ~/Git/maestro
...
rm -f ~/.maestro/.agent-workspace/tmp/triage-cache/Aditya060806_maestro-<N>.hash
/usr/bin/env bash ~/.maestro/agents/scripts/pulse-wrapper.sh --once
gh issue view <N> --repo Aditya060806/Maestro --json comments \
...
- [ ] PR body cites the `~/.maestro/logs/pulse-wrapper.log` evidence from
...
- `.agents/scripts/headless-runtime-helper.sh` — the runtime that launches
  the sandboxed agent
- `~/.maestro/logs/pulse-wrapper.log` — historical failure evidence for
  #18428
