...
- **Observable impact.** In the current backlog (verified 2026-04-20), 743+ `PARENT_TASK_BLOCKED` events in `~/.maestro/logs/pulse.log`. All stuck issues except #20128 have `parent-task` label and an unactioned nudge comment. The supervisor loops fruitlessly because there is no forward path.
...
- **NEW: `.agents/scripts/auto-decomposer-scanner.sh`** — standalone scanner script. Model on `post-merge-review-scanner.sh` structure (header comments, `SCANNER_DAYS`/`SCANNER_MAX_ISSUES` env vars, `log()` helper, `get_lookback_date` for nudge-age filtering, main scan loop).
  ...
  - For each, read comments: if `<!-- parent-needs-decomposition -->` marker is present AND its comment age ≥24h AND no `<!-- maestro:generator=auto-decompose parent=<N> -->` child issue is still open (search via `gh search issues`), file a new worker-ready issue.
...
```bash
# Inside the worktree
cd /Users/Aditya060806/Git/maestro-feature-t2442-auto-decomposer-parent-task-gate
...
# Dry-run the scanner against the live maestro repo
./.agents/scripts/auto-decomposer-scanner.sh dry-run Aditya060806/Maestro
```
...
- **Deployed-vs-source gotcha.** Fixes land in `~/Git/maestro/.agents/scripts/`. Pulse reads from `~/.maestro/agents/scripts/`. Deploy requires `setup.sh --non-interactive`. Merge + release hooks handle this automatically per framework convention (see AGENTS.md "Pulse restart after deploying pulse script fixes").
