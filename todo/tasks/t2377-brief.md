...
**Data loss in progress.** The bug is not theoretical — it is actively destroying content. Observed evidence (`~/.maestro/logs/pulse.log:85900-85904`):

```
[dispatch_with_dedup] t2063: issue #19778 has brief on disk but stub body — force-enriching
[INFO] Enriching 1 issue(s) in Aditya060806/Maestro
...
for n in 19778 19779 19780; do
  gh api "/repos/Aditya060806/Maestro/issues/$n" --jq '{title, body_len: (.body | length)}'
done
...
FORCE_ENRICH=true REPO_SLUG=Aditya060806/Maestro \
...
bash .agents/scripts/interactive-session-helper.sh release 19778 Aditya060806/Maestro
# (wait 2 minutes, then verify)
gh api /repos/Aditya060806/Maestro/issues/19778 --jq '{title, body_len: (.body | length)}'
...
- `~/.maestro/logs/pulse.log:85895-85917` — observed evidence of the destructive path (lines may shift over time; `grep "force-enriching.*19778"` to locate)
