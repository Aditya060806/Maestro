...
- **EDIT:** `maestro.sh` — adds `campaign | campaigns` dispatch case + help text
...
# Verify maestro.sh dispatch wired
grep -n "campaign" maestro.sh
```
...
- [x] `maestro campaign` dispatches to `campaign-helper.sh`
- [x] `maestro help` includes `campaign` in command list and detailed section
