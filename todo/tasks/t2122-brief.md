...
**Session origin:** interactive (follow-up filed during `/review-issue-pr 19174` with Aditya060806)
...
- NEW: `.agents/plugins/opencode-maestro/response-helpers.mjs` — shared
...
- EDIT: `.agents/plugins/opencode-maestro/cursor/proxy.js` — 5 `new Response(...)`
  call sites migrated (4 `jsonResponse`, 1 `textResponse` 404). Adds one
  import line.
- EDIT: `.agents/plugins/opencode-maestro/cursor/proxy-stream.js` — 1 SSE
  streaming response routed through `textResponse` for consistency.
- EDIT: `.agents/plugins/opencode-maestro/provider-auth-request.mjs` — 1
...
- `.agents/plugins/opencode-maestro/claude-proxy.mjs` and `google-proxy.mjs`
