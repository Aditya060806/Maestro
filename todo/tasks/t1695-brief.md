...
Create `foss-contribution-helper.sh` — the orchestration layer that enables maestro to autonomously contribute fixes to open-source projects. The orchestrator is app_type-agnostic: it handles the universal workflow (scan issues, triage, fork, worktree, submit PR) and delegates build/test/review to type-specific handlers.
