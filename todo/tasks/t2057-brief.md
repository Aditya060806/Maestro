...
3. **`approval-helper.sh`** — in `_post_issue_approval_updates`, after the existing `needs-maintainer-review` → `auto-dispatch` churn, idempotently clear `status:in-review` if present (no new user commands, no new prompts — pure additive behaviour on the existing `sudo maestro approve issue` path).
...
# sudo maestro approve issue <contributor-issue-number>
