...
- Origin: interactive (Aditya060806)
- Tier: standard
...
(`Aditya060806/Maestro-routines#17`), and has a state directory — but
`~/Library/LaunchAgents/sh.maestro.opencode-db-maintenance.plist` does not
exist and no systemd/cron equivalent is installed. It only runs when someone
invokes it manually.

**Deliverable**: after `maestro update` on a supported platform, the routine
...
`~/Library/LaunchAgents/sh.maestro.*.plist` vs the routine declaration. The
...
- `install` — generate `~/Library/LaunchAgents/sh.maestro.opencode-db-maintenance.plist`,
  ...
  Logs to `$HOME/.maestro/.agent-workspace/logs/opencode-db-maintenance.log`.
...
- **Readiness guard**: only proceed if
  `$HOME/.maestro/agents/scripts/opencode-db-maintenance-helper.sh`
...
2. `ls ~/Library/LaunchAgents/sh.maestro.opencode-db-maintenance.plist`
   — file exists.
3. `launchctl list | grep opencode-db-maintenance` — loaded.
4. `plutil -p ~/Library/LaunchAgents/sh.maestro.opencode-db-maintenance.plist`
   — shows `StartCalendarInterval => { Weekday => 0, Hour => 4, Minute => 0 }`.
5. `bash ~/.maestro/agents/scripts/opencode-db-maintenance-helper.sh uninstall`
