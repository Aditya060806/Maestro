...
- **Created by:** Aditya060806 (human + ai-interactive)
...
- `install` / `uninstall` — launchd plist (`sh.maestro.contribution-watch`)

State file: `~/.maestro/cache/contribution-watch.json`
```json
{
  "last_scan": "ISO8601",
  "items": {
...
**Terminal greeting** — `maestro-update-check.sh` already runs on every interactive session start. When `contribution-watch.json` has items needing attention, append a line to the greeting: `"2 external contributions need your reply (run /contributions to see them)."` This is when you're already in a position to act.

**macOS notification** — the launchd scan job fires `osascript -e 'display notification "2 contributions need reply" with title "maestro"'` when new items are detected. Catches time-sensitive replies between sessions (e.g., maintainer requesting changes on your PR).
...
- [ ] `contribution-watch-helper.sh install` creates launchd plist `sh.maestro.contribution-watch`
...
- **Username resolution:** The seed command resolves the username dynamically from `gh api user --jq '.login'` — never hardcoded. This ensures it works for any maestro user with `gh` authenticated.

## Relevant Files

- `~/.config/maestro/repos.json` — schema extension for `contributed` type
- `~/.maestro/agents/scripts/pulse-wrapper.sh` — integration point for scan step
- `~/.maestro/agents/scripts/pulse.md` — pulse documentation
- `~/.maestro/agents/tools/security/prompt-injection-defender.md` — injection defense patterns
- `~/.maestro/agents/scripts/prompt-guard-helper.sh` — content scanning
- `~/.maestro/cache/contribution-watch.json` — state file (new)
