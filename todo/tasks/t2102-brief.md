...
# t2102: Pull maestro worktree fresh before ratchet-check in pulse simplification
...
In `.agents/scripts/pulse-simplification.sh` `_complexity_scan_ratchet_check` (around line 1673), add a `git -C "$maestro_path" pull --ff-only origin main` step immediately before the `ratchet-check` invocation at line 1682. If the pull fails (offline, non-main HEAD, or conflict), skip the ratchet-check this cycle and log — don't crash the pulse.
