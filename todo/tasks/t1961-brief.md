...
```bash
cd ~/Git/maestro.chore-t1961-is-assigned-complexity-refactor
shellcheck .agents/scripts/dispatch-dedup-helper.sh
bash .agents/scripts/tests/test-dispatch-dedup-helper-is-assigned.sh
~/.maestro/agents/scripts/complexity-scan-helper.sh ratchet-check
```
...
- [ ] Complexity ratchet shows `func:40` (not 41)
  ...
  verify:
    method: bash
    run: "~/.maestro/agents/scripts/complexity-scan-helper.sh ratchet-check 2>&1 | grep -E 'Actual violations.*func:40 '"
  ```
