...
- [x] Discovery pass: no recent commits, merged PRs, or open PRs touched `.agents/maestro/projects.md` or `todo/tasks/t3536-brief.md`.
- [x] File refs verified: `.agents/maestro/knowledge-plane.md`, `.agents/maestro/campaigns-plane.md`, and parent brief `todo/tasks/t3476-brief.md` present at HEAD.
...
Create the initial `_projects/` plane contract in `.agents/maestro/projects.md`, defining directory layout, required files, optional files, project ID rules, manifest fields, versioning/sensitivity defaults, and the relationship to repo-local `TODO.md` / `todo/` planning files.
...
- **Status:** `not-created`
- **Freshness evidence:** `prework-discovery-helper.sh --keywords "_projects directory contract t3536" --files ".agents/maestro/projects.md,todo/tasks/t3536-brief.md" --repo "Aditya060806/Maestro"` found no collisions.
- **Verification run:** Pending implementation verification in this task.
- **Stale-assumption warning:** If another PR lands `.agents/maestro/projects.md` before merge, rebase and reconcile this contract rather than creating a competing plane definition.

## How (Approach)

### Files to Modify

- `NEW: .agents/maestro/projects.md` — initial `_projects/` plane directory contract, modelled on `.agents/maestro/knowledge-plane.md` and `.agents/maestro/campaigns-plane.md`.
...
1. Add `.agents/maestro/projects.md` with:
...
```bash
npx --yes markdownlint-cli2 .agents/maestro/projects.md todo/tasks/t3536-brief.md
test -f .agents/maestro/projects.md
test -f todo/tasks/t3536-brief.md
gh issue view 22371 --repo Aditya060806/Maestro --json body,state --jq 'select(.state == "OPEN") | .body' | grep -q 't3536 / #22537'
```

### Files Scope

- `.agents/maestro/projects.md`
...
- `.agents/maestro/projects.md` — new contract.
- `.agents/maestro/knowledge-plane.md` — source-plane contract pattern.
- `.agents/maestro/campaigns-plane.md` — peer plane contract pattern.
- `todo/tasks/t3476-brief.md` — parent decomposition context.

## Notes

- Existing `_projects/README.md` is a lightweight placeholder for the user-data plane root. This task deliberately defines the framework contract in `.agents/maestro/projects.md` without implementing provisioners or CLI commands.
