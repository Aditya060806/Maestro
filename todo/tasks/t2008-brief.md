...
3. **Local state file.** `~/.maestro/.agent-workspace/cache/stale-recovery-counts.json` keyed by repo+issue. **Pros:** clean, fast. **Cons:** not cross-runner; each runner has its own count, so escalation only fires when ONE runner has done 2 consecutive recoveries.
