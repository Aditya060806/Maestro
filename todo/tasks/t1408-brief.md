...
- **Fractals as inspiration, not a port.** Fractals is TypeScript; maestro is shell scripts + agent docs. We adopt the *pattern* (classify -> decompose -> lineage -> batch), not the code.
...
- **Depth limit of 3.** Fractals allows depth 5 but notes recursive depth as an open question. For maestro, 3 levels (parent -> child -> grandchild) is sufficient. Deeper decomposition suggests the original task was poorly scoped.
