...
- Manual smoke test: create an issue with `--body "Parent: #20402"` and confirm the child appears in `gh api repos/Aditya060806/Maestro/issues/20402/sub_issues`.
...
- `maestro parent-status <N>` CLI helper for inspecting decomposition state. Separate issue (Gap D).
