...
# t2265 — Add `init_scope` field to scope `maestro init` scaffolding

## Session Origin

Interactive session with Aditya060806 on 2026-04-19. While initialising four
...
that `maestro init planning` creates ~14 files per repo — and for a private
...
User's observation: "we'll always have the master maestro installed on this
...
`~/.config/maestro/repos.json` and to the per-project `.maestro.json` file.
...
- The framework lives at `~/.maestro/agents/` globally. Every init'd repo
...
| `.maestro.json` | ✅ | ✅ | ✅ |
| `.gitignore` (maestro-runtime entries) | ✅ | ✅ | ✅ |
...
- If `.maestro.json` already has a non-empty `init_scope`, use it.
...
- If the remote slug is under an owner with `public_default_init_scope: "public"`
  in a new `~/.config/maestro/init-scope-defaults.json` (optional file) → `"public"`.
- Otherwise → `"standard"` (current behaviour — backward compatible).

### 3. Files to modify

- **EDIT** `/usr/local/bin/maestro` (the CLI; in-repo source at
  `bin/maestro` or wherever the build writes from — trace via `maestro doctor`
  or `file $(which maestro)` to confirm):
  - `cmd_init` (currently at `/usr/local/bin/maestro:1675-2356`): read
    `init_scope` early, gate each optional scaffold call behind a
    `_scope_includes <feature>` check.
  - `register_repo` (currently at `/usr/local/bin/maestro:351-449`):
    preserve user-set `init_scope` on re-registration (same pattern as
    `pulse`, `priority`, `maintainer` preservation already in the jq
    update block).
  - `scaffold_repo_courtesy_files` (currently at `/usr/local/bin/maestro:1270-1336`):
  ...
  - Collaborator pointer generation (currently at
    `/usr/local/bin/maestro:2199-2215`): skip entire block for `minimal`.
  - `DESIGN.md` seed (currently at `/usr/local/bin/maestro:2217-2227`):
    skip for `minimal`.
  - `MODELS.md` generation (currently at `/usr/local/bin/maestro:2232-2243`):
    skip for `minimal`.
...
- **NEW** `.agents/scripts/tests/test-init-scope.sh`: unit test that
  creates three tmp repos (minimal, standard, public), runs `maestro init
...
- Existing `.maestro.json` files have no `init_scope` → falls through to
...
- `maestro upgrade-planning` is unaffected (it only touches TODO/PLANS
  templates).

## Acceptance criteria

1. `maestro init planning` in a `local_only: true` folder creates **no**
...
2. `maestro init planning` in a repo with `init_scope: "public"` in
   `repos.json` creates the full current file set.
3. Re-running `maestro init` on an existing repo preserves its
   `init_scope` (round-trip survival).
4. `.agents/scripts/tests/test-init-scope.sh` passes and covers all three
   scopes.
5. `maestro doctor` surfaces repos whose on-disk scaffolding does not
...
- **Scope migration tooling.** Changing a repo from `standard` → `minimal`
  requires deletion of now-excluded files; that's `maestro init --reconcile`
  territory (future).
...
- Session thread: 2026-04-19 interactive run; user picked option A when
  ...
  (option A).
- Companion task: repo maestro health keeper routine (filed separately in
  `maestro-routines`) will use `init_scope` once this ships.
- Related framework files already parse `.maestro.json`:
