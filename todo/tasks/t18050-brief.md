...
- [x] Memory recall: `maestro reach capture inbox knowledge workflow` → 0 hits — no relevant prior lesson found.
- [x] Discovery pass: no t18050 brief/open related PR found; `_inbox` and `_knowledge` helpers/docs are present.
- [x] File refs verified: `.agents/scripts/inbox-helper.sh`, `.agents/templates/inbox-readme.md`, `.agents/maestro/knowledge-plane.md`, and t18047-t18049 targets are present or declared blockers.
...
- **Read first:** `.agents/maestro/knowledge-plane.md:25-75` — knowledge directory and metadata contract.
...
- `EDIT: .agents/maestro/knowledge-plane.md` — document reach-captured sources entering knowledge via inbox/staging.
...
./maestro.sh reach capture --input .agents/templates/inbox-readme.md --dest inbox --method file --format json
```
...
- `.agents/maestro/knowledge-plane.md`
- `.agents/scripts/tests/test-reach-capture.sh`
