...
- Create temporary agents and scripts for the mission, and offer improvements back to maestro
...
- Reference maestro patterns for how to do things it already has working examples for
- Research its own examples when maestro doesn't have them
- Know user preferences and constraints

### Mission homes:
- `~/.maestro/missions/{id}/` — homeless missions (no repo yet, POC drafting)
...
Current maestro handles task-level work (`/full-loop`) and supervisor-level dispatch (`/pulse`), but nothing takes a high-level goal and drives it to completion over days. The gap between "I have an idea" and "tasks are in TODO.md ready for dispatch" requires manual decomposition. Missions close this gap and extend beyond code into research, procurement, and infrastructure setup — making maestro a true autonomous project agent.
...
- Missions start homeless in `~/.maestro/missions/`, migrate to `todo/missions/` when a repo exists
- Mission agents/scripts are temporary (draft tier), with promotion path to maestro shared
...
- [ ] `/mission "description"` starts an interactive scoping interview
  ...
  verify:
    method: bash
    run: "test -f ~/.maestro/agents/scripts/commands/mission.md"
  ```
