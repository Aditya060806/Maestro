...
Ship the `maestro knowledge add|list|search` CLI surface for adding sources to the inbox, listing staged/promoted sources, and searching the index. Route all platform-touching operations (PR creation, issue references, gh queries) through a new platform abstraction layer so future Gitea/GitLab/local-only adapters slot in cleanly.

**Concrete deliverables:**

1. `maestro knowledge add <path|url>` — copies/downloads to `_knowledge/inbox/<source-id>/`, creates `meta.json` skeleton (sensitivity defaults to detector output once P0.5a lands; placeholder until then)
2. `maestro knowledge list [--state inbox|staging|sources|all] [--kind <type>]` — lists sources with state, kind, sha, sensitivity
3. `maestro knowledge search <query> [--scope repo|personal|both]` — substring + meta-field search (full RAG arrives in P1c with PageIndex; this is the v1 grep-and-jq baseline)
...
3. **CLI wiring** — extend `bin/maestro` (or wherever t2842 added `knowledge init`) with the new subcommands.
...
5. **30MB threshold dispatch** — if file size ≥ 30MB, write the original to `~/.maestro/.agent-workspace/knowledge-blobs/<repo>/<source-id>/<filename>` and put a stub at `_knowledge/sources/<id>/blob_pointer.txt` with sha+path; otherwise put original directly in `_knowledge/sources/<id>/`.
...
- EDIT: `.agents/cli/maestro` (or wherever main CLI lives; verify with t2842 changes)
- NEW: `.agents/tests/test-knowledge-cli.sh`
- NEW: `.agents/tests/test-platform-helper.sh`
- EDIT: `.agents/maestro/knowledge-plane.md` (CLI usage section)

## Acceptance Criteria

- [ ] `maestro knowledge add /path/to/file.pdf` copies to inbox with correct sha256 + meta.json
- [ ] `maestro knowledge add https://example.com/doc.pdf` downloads, validates size, places in inbox or blob store per threshold
- [ ] `maestro knowledge list` shows all known sources across inbox/staging/sources with state column
- [ ] `maestro knowledge list --state staging --kind document` filters correctly
- [ ] `maestro knowledge search <query>` returns matching source IDs with brief context
...
- [ ] Documentation: `.agents/maestro/knowledge-plane.md` updated with CLI section + platform abstraction overview
...
- Existing CLI dispatcher (verify path): `.agents/cli/maestro` or `bin/maestro`
