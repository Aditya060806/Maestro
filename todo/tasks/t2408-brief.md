...
**Issue:** GH#19997
**Origin:** interactive (user hit the bug running `sudo maestro approve issue 19992`)
**Tier:** simple
**Estimate:** ~15m
...
The wrappers are defined in `shared-constants.sh:1167,1174` so they auto-append the t2393 signature footer. `approval-helper.sh` does NOT source `shared-constants.sh`, so under `sudo maestro approve issue <N>` the wrappers are unbound and bash emits:
...
This blocks cryptographic approval entirely — the whole point of the `sudo maestro approve` gate is to post the SSH-signed comment. Currently every approval attempt fails.
...
sudo maestro approve issue 19997
```
...
- [ ] `sudo maestro approve issue <N>` posts a signed approval comment end-to-end
...
- Downstream impact: every `sudo maestro approve issue <N>` / `sudo maestro approve pr <N>` has failed since PR #19953 merged (~3 hours before this task was filed). Issue GH#19992 is currently blocked on manual approval.
