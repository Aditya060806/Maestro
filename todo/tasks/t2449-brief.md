...
- **NEW**: `.agents/scripts/tests/test-pulse-merge-worker-briefed.sh` — regression test harness. Must cover:
  ...
  - (e) `origin:worker` + issue had NMR cleared via `sudo maestro approve issue N` → auto-merges
...
read -r has_crypto has_auto < <(gh api "repos/$REPO/issues/$ISSUE/comments" --jq '
  [
    (any(.[] | .body | strings; contains("maestro:approval-signature:"))),
...
`MAESTRO_WORKER_BRIEFED_AUTO_MERGE` env var, default `1`. Set to `0` to disable the new path and fall back to manual-merge-only for `origin:worker`. Validation window: 48h post-deploy; if any unexpected merge fires, flip to `0`, file follow-up, fix before re-enabling.

### Verification

- **Local dry-run**: `MAESTRO_DRY_RUN=1 pulse-merge.sh process-repo <slug>` with synthetic PRs in all 10 coverage cases. Dry-run should print decisions without acting.
- **Integration**: after merge, monitor first 5 `origin:worker` PRs on `Aditya060806/Maestro`. Confirm auto-merge matches expected gate behaviour for each case encountered.
- **Rollback**: if any unexpected merge occurs, flip `MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0` in `~/.maestro/.env` or the pulse-wrapper defaults; pulse picks up on next cycle start.
- **Audit log**: every auto-merge decision writes a structured line to `~/.maestro/logs/pulse-merge.log` with PR number, gate decisions, outcome.
...
- [ ] Feature flag `MAESTRO_WORKER_BRIEFED_AUTO_MERGE` is respected (default: 1, set to 0 to disable)
...
- **Feature flag defaulting ON**: the 48h validation window with `MAESTRO_WORKER_BRIEFED_AUTO_MERGE=0` as rollback was chosen over defaulting OFF because the entire point is reducing merge friction. Starting OFF would require a second PR to flip it, adding ceremony.
