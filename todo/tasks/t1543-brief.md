...
Add an OAuth multi-account pool module to the existing `opencode-maestro` plugin (`~/.maestro/agents/plugins/opencode-maestro/`) that:

1. Registers a custom provider (`anthropic-pool`) via the plugin `auth` hook
2. Stores multiple OAuth credentials in `~/.maestro/oauth-pool.json` keyed by account email
...
Module in `~/.maestro/agents/plugins/opencode-maestro/` containing:

- `ANTHROPIC_MODELS` — model definitions mirroring built-in Anthropic provider
- `loadPool()` / `savePool()` — read/write `~/.maestro/oauth-pool.json`
...
### Pool file format (`~/.maestro/oauth-pool.json`)
...
- Existing plugin: `~/.maestro/agents/plugins/opencode-maestro/index.mjs`
