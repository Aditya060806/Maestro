...
for pr in $(jq -r '.[].number' /tmp/recent-prs.json); do
  gh pr view "$pr" --repo Aditya060806/Maestro --comments | grep -oE 'tokens?[=:][0-9]+' | head -1
...
2. **How to aggregate spend.** Recommended: parse signature footers from `gh issue view --comments` JSON. No new state file. Cache the per-issue aggregate in `~/.maestro/.agent-workspace/cache/cost-aggregates/<issue>.json` with TTL=15m if performance matters.
