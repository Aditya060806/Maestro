...
- [x] File refs verified: `.agents/setup.sh`, `~/.config/maestro/repos.json` (schema in `reference/repos-json-fields.md`)
...
Define and provision the `_knowledge/` directory contract in any maestro-managed repo, gated by a new `knowledge: off | repo | personal` field in `repos.json`. Ship an `maestro knowledge init` CLI to flip a repo from `off` → `repo` (or `personal`). Update `setup.sh` to provision the skeleton on `init` and on first `update` after the field is set.
...
3. Personal plane skeleton at `~/.maestro/.agent-workspace/knowledge/` (same structure)
4. `.gitignore` rules for `inbox/`, `staging/` (gitignored — pre-review zone), `sources/` (versioned)
5. 30MB threshold logic: originals ≥30MB go to `~/.maestro/.agent-workspace/knowledge-blobs/<repo>/<source-id>/` with hash+pointer in `meta.json`; below threshold, in-repo
6. Source `meta.json` contract: `{id, kind, source_uri, sha256, ingested_at, ingested_by, sensitivity, trust, blob_path?, size_bytes}`
7. `maestro knowledge init` CLI: prompt for `repo` vs `personal`, write to `repos.json`, run provisioning
...
`personal` mode (cross-repo, in `~/.maestro/.agent-workspace/knowledge/`) covers the use case of "knowledge that doesn't belong to any one repo yet" or "knowledge that spans multiple repos" — important for early-stage work where a repo might not yet exist.
...
2. **Define directory contract** — write `.agents/maestro/knowledge-plane.md` documenting:
...
3. **Add `knowledge` subcommand to maestro CLI** — extend `bin/maestro` (or `cli/main.sh`) with `knowledge init|status` subcommands. `init` prompts for mode, writes to `repos.json`, calls provisioning helper.
...
- [ ] `maestro knowledge init` (interactive) writes `knowledge: "repo"` (or `"personal"`) to `~/.config/maestro/repos.json` for the current repo
...
- [ ] Personal mode provisions at `~/.maestro/.agent-workspace/knowledge/` instead of in-repo
...
- [ ] Documentation: `.agents/maestro/knowledge-plane.md` written with full directory contract spec
...
- Existing personal-data location convention: `~/.maestro/.agent-workspace/`
