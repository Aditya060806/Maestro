...
1. What are you researching?
   → Suggest based on repo context (e.g., "agent instruction optimization" in maestro)
...
4. What's the success metric?
   ...
   → Suggest based on repo type:
     - maestro: "agent-test-helper.sh run → pass rate + token count"
...
| Signal | Inference |
|---|---|
| Repo is maestro | Suggest agent optimization, list test suites |
