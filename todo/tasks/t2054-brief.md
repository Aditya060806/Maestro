...
- [ ] `post-merge-review-scanner.sh refresh-dry-run Aditya060806/Maestro` runs cleanly and reports planned edits/closes without errors.
...
- [ ] After merge: one-off `refresh` run against `Aditya060806/Maestro` updates/closes the stale open review-followup backlog.
...
bash -c '
  ...
  build_pr_followup_body Aditya060806/Maestro 18343 && echo UNEXPECTED || echo "OK (no unresolved findings)"
'

# Dry-run refresh against maestro
.agents/scripts/post-merge-review-scanner.sh refresh-dry-run Aditya060806/Maestro
```
