...
Design and implement the first local Vault broker and CLI surface that keeps maestro protected data locked by default, unlocks only through hidden passphrase prompts, and gates helpers/UI/agents through a single access boundary.
...
- EDIT: `maestro.sh` — add `maestro vault` command group.
...
- [ ] `maestro vault status` distinguishes uninitialized, locked, unlocked, and corrupted metadata states.
- [ ] `maestro vault unlock` fails safely on wrong passphrase and never prints secrets.
...
Model CLI grouping on `maestro secret` and `.agents/scripts/secret-helper.sh`, but do not reuse gopass as the Vault data encryption boundary.
