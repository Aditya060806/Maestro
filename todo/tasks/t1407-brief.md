...
- **Created by:** Aditya060806 (human, with AI assistance)
...
Add guidance and a pre-submission check so maestro agents format issues and PRs correctly when filing on external (non-maintained) repos. When `gh issue create --repo <external-slug>` or `gh pr create` targets a repo not in our repos.json, the agent should:
...
- `.agents/scripts/commands/log-issue-maestro.md` — existing issue creation workflow (internal only, but pattern to reference)
