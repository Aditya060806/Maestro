# t2692: scrub private repo references from maestro codebase
...
- **Conversation context:** During the t2685 signature-footer hardening session, the agent composed issue/PR/brief/commit content that referenced a managed private repo by name. The user flagged this as a privacy violation: the private repo name must not appear in the maestro codebase. Separately, the privacy-guard was structurally unable to catch it because the repo's entry in `~/.config/maestro/repos.json` had `local_only: null` and `mirror_upstream: null` — the guard treats only repos with one of those flags set as private.

## What

Two coordinated fixes:

1. **Code-tree scrub**: replace every occurrence of the private repo slug and URL with the generic placeholder `webapp` across 21 files (TODO.md, 7 brief files, 1 research doc, 6 scripts + top-level `maestro.sh`, 5 test files).
2. **Privacy-guard config fix** (already applied out-of-band): `~/.config/maestro/repos.json` → `mirror_upstream: true` on the private repo entry, and `~/.maestro/configs/privacy-guard-extra-slugs.txt` created with the slug so the guard catches any future regression regardless of repos.json state.

## Why

Cross-repo privacy rule (`prompts/build.txt` + `.agents/AGENTS.md`) is explicit: private repo names must never appear in the maestro codebase. The leak is most severe in:
