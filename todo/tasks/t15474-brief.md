...
After merging [PR #18096](https://github.com/Aditya060806/Maestro/pull/18096) and running `./setup.sh --non-interactive`, only **OpenCode** and **Claude Code** received the new `maestro-*` prefixed slash commands.
...
Once the generator was invoked directly, each client received its 93 `maestro-*` commands in the correct per-client format:
...
```bash
ls "$(rt_command_dir <id>)" | grep -c '^maestro-'
```

to return the same count as `ls ~/.claude/commands | grep -c '^maestro-'` (currently 93 commands). If the format is per-client (`.toml`, `.prompt`), match on the appropriate pattern.
