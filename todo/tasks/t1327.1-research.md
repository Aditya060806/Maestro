...
**Ref:** GH#2246
**Sources:** simplex-chat/simplex-chat GitHub repo (bots/, packages/, docs/), npm registry, existing maestro patterns
...
## 8. Pattern Analysis: Existing maestro Helpers

### 8.1 matrix-bot.md Patterns to Reuse

| Pattern | matrix-bot | simplex-bot |
|---------|-----------|-------------|
| Config file | `~/.config/maestro/matrix-bot.json` (600 perms) | `~/.config/maestro/simplex-bot.json` (600 perms) |
| Data dir | `~/.maestro/.agent-workspace/matrix-bot/` | `~/.maestro/.agent-workspace/simplex-bot/` |
...
├── config.ts         # Config loader (~/.config/maestro/simplex-bot.json)
...
**t1327.3 (Helper script):** Follow ip-reputation-helper.sh shell conventions exactly. Use `shared-constants.sh`. Subcommands: `install|init|bot-start|bot-stop|bot-status|send|connect|group|status|server`. Config at `~/.config/maestro/simplex-bot.json`.
