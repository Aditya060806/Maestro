...
Rollout is gated by a new env `MAESTRO_INTERACTIVE_PR_HANDOVER_MODE` with values `off` | `detect` | `enforce`, default `detect`. The mode determines whether the helpers log-only, comment-only, or fully enforce. A follow-up one-line PR flips the default to `enforce` once telemetry is clean.
...
# $CLAIM_STAMP_DIR = ~/.maestro/.agent-workspace/interactive-claims/
...
#   - PR's last activity (updatedAt) > $MAESTRO_INTERACTIVE_PR_HANDOVER_HOURS (default 24)
#   - Linked issue is open
#
# Env:
#   MAESTRO_INTERACTIVE_PR_HANDOVER_HOURS — default 24
#   MAESTRO_INTERACTIVE_PR_HANDOVER_MODE  — off | detect | enforce (default: detect)
...
_interactive_pr_is_stale() {
    ...
    local mode="${MAESTRO_INTERACTIVE_PR_HANDOVER_MODE:-detect}"
    [[ "$mode" == "off" ]] && return 1
    ...
    threshold_hours="${MAESTRO_INTERACTIVE_PR_HANDOVER_HOURS:-24}"
    ...
    slug_flat="${repo_slug//\//-}"
    stamp_path="${CLAIM_STAMP_DIR:-$HOME/.maestro/.agent-workspace/interactive-claims}/${slug_flat}-${linked_issue}.json"
...
# routing override).
#
# Honours MAESTRO_INTERACTIVE_PR_HANDOVER_MODE:
...
This \`origin:interactive\` PR has been idle past the handover threshold (\`MAESTRO_INTERACTIVE_PR_HANDOVER_HOURS=${MAESTRO_INTERACTIVE_PR_HANDOVER_HOURS:-24}\`h). The pulse is now routing it through the worker pipeline:
...
past `MAESTRO_INTERACTIVE_PR_HANDOVER_HOURS` (default 24h) with no linked-issue
...
`MAESTRO_INTERACTIVE_PR_HANDOVER_MODE=off|detect|enforce` (default `detect` for
...
~/Git/maestro/.agents/scripts/complexity-regression-helper.sh check function-complexity
```
...
- [ ] `_interactive_pr_is_stale()` function exists in pulse-merge.sh, under 100 lines
  ...
  verify:
    method: bash
    run: "awk '/^_interactive_pr_is_stale\\(\\)/,/^}/' ~/Git/maestro/.agents/scripts/pulse-merge.sh | wc -l | awk '{exit ($1 < 100) ? 0 : 1}'"
  ```
- [ ] `_interactive_pr_trigger_handover()` function exists in pulse-merge.sh, under 100 lines
  ```yaml
  verify:
    method: bash
    run: "awk '/^_interactive_pr_trigger_handover\\(\\)/,/^}/' ~/Git/maestro/.agents/scripts/pulse-merge.sh | wc -l | awk '{exit ($1 < 100) ? 0 : 1}'"
  ```
- [ ] All three routing gates (lines ~840, ~1121, ~1154) accept `origin:worker-takeover` as an alternative to `origin:worker`
  ...
  ```
- [ ] Mode flag `MAESTRO_INTERACTIVE_PR_HANDOVER_MODE` is respected: `off` disables, `detect` logs, `enforce` acts
  ```yaml
  verify:
    method: codebase
    pattern: "MAESTRO_INTERACTIVE_PR_HANDOVER_MODE"
    path: ".agents/scripts/pulse-merge.sh"
  ```
- [ ] Default mode is `detect` (conservative rollout)
  ...
  verify:
    method: codebase
    pattern: 'MAESTRO_INTERACTIVE_PR_HANDOVER_MODE:-detect'
    path: ".agents/scripts/pulse-merge.sh"
  ```
...
- [ ] Test harness exists with ≥8 cases covering staleness truth table + idempotence + mode behaviour
  ...
  verify:
    method: bash
    run: "bash ~/Git/maestro/.agents/scripts/tests/test-pulse-merge-interactive-handover.sh"
  ```
- [ ] shellcheck clean on pulse-merge.sh (no new violations)
  ```yaml
  verify:
    method: bash
    run: "shellcheck ~/Git/maestro/.agents/scripts/pulse-merge.sh"
  ```
- [ ] Complexity ratchet: new head ≤ current base (no new functions >100 lines introduced)
  ```yaml
  verify:
    method: bash
    run: "bash ~/Git/maestro/.agents/scripts/complexity-regression-helper.sh check function-complexity"
  ```
...
- **Not building `maestro pr reclaim <N>` in this task** — the two-line reclaim is documented inline in the handover comment and in AGENTS.md. A convenience wrapper can be filed as a follow-up if friction warrants it.
