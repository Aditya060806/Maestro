...
# t2070: Decompose `.agents/plugins/opencode-maestro/claude-proxy.mjs` (6 qlty smells)
...
~/.qlty/bin/qlty smells --all --sarif --no-snippets --quiet 2>/dev/null \
  | jq -r '.runs[0].results[] | select(.locations[0].physicalLocation.artifactLocation.uri == ".agents/plugins/opencode-maestro/claude-proxy.mjs") | "\(.ruleId)\t\(.message.text)\t\(.locations[0].physicalLocation.region.startLine)"'

# 2. Key targets
rg -n "processStreamEvent|tryStreamWithAccount" .agents/plugins/opencode-maestro/claude-proxy.mjs

# 3. Related test/integration harness
ls .agents/plugins/opencode-maestro/test* 2>/dev/null
rg -l "claude-proxy" .agents/plugins/opencode-maestro/
```

### Files to Modify

- `EDIT: .agents/plugins/opencode-maestro/claude-proxy.mjs` — primary target
- `NEW: .agents/plugins/opencode-maestro/claude-proxy-stream.mjs` — extracted stream processing (processStreamEvent + event-type handlers)
- `NEW: .agents/plugins/opencode-maestro/claude-proxy-retry.mjs` — extracted account retry logic (tryStreamWithAccount helpers)
...
# Lints clean
cd .agents/plugins/opencode-maestro && bun run lint 2>&1 || echo "no lint"

# Smoke test
cd .agents/plugins/opencode-maestro && bun test 2>&1 || echo "no tests"
```
...
- [ ] Existing opencode-maestro tests pass (`bun test` in the plugin dir)
...
- `.agents/plugins/opencode-maestro/claude-proxy.mjs` — primary target
- `.agents/plugins/opencode-maestro/provider-auth.mjs` — sibling; may share helpers for account state
- `.agents/plugins/opencode-maestro/oauth-pool.mjs` — likely the account pool abstraction
