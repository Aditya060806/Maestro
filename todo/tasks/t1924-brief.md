...
# t1924: maestro init-routines — scaffold private routines repo
...
New CLI command `maestro init-routines` and helper script that scaffolds a private git repo for routine definitions. Supports personal repos, per-org repos, and local-only (no remote).
...
- `EDIT: .agents/scripts/maestro-cli.sh` (or equivalent) — add `init-routines` subcommand routing
...
1. Create `init-routines-helper.sh` with:
   - `init_personal()` — `gh repo create maestro-routines --private`, clone, scaffold
   - `init_org(org_name)` — `gh repo create $org/maestro-routines --private`, clone, scaffold
   ...
   - `detect_and_create_all()` — for setup integration:
     ...
     - Check if `<username>/maestro-routines` exists, create if not
     - Detect admin orgs: `gh api user/orgs --jq '.[].login'`, filter by admin role
     - For each admin org, check/create `<org>/maestro-routines`
...
Note: `maestro update` does NOT auto-create repos. At most verifies local clone exists and warns if missing.

### Verification

```bash
# Dry run test
~/.maestro/agents/scripts/init-routines-helper.sh --help
# After running:
test -f ~/Git/maestro-routines/TODO.md
jq '.initialized_repos[] | select(.slug | contains("routines"))' ~/.config/maestro/repos.json
```

## Acceptance Criteria

- [ ] `maestro init-routines` creates a private repo with correct structure
  ```yaml
  verify:
    method: bash
    run: "grep -q 'init.routines\\|init-routines' .agents/scripts/init-routines-helper.sh"
  ```
- [ ] `maestro init-routines --org <name>` creates per-org variant
- [ ] `maestro init-routines --local` creates local-only (no remote)
...
- Per-org variant uses `<org>/maestro-routines` naming convention
...
- Custom scripts/agents for routines live in `~/.maestro/agents/custom/` (referenced by `run:` field)

## Relevant Files

- `.agents/scripts/init-helper.sh` — existing init pattern to model on
- `~/.config/maestro/repos.json` — registration target
