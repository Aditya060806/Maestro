...
Claude Pro/Max". Users who relied on this flow will lose OAuth access. The maestro OAuth pool
...
- The maestro OAuth pool (t1543) already provides the replacement — it just needs to be surfaced
...
After the maestro plugin is registered, add a version check:
...
- Use `opencode auth login` → "Anthropic Pool" (provided by maestro plugin)
...
- The maestro OAuth pool (`oauth-pool.mjs`) is the replacement
...
- OAuth pool implementation: `.agents/plugins/opencode-maestro/oauth-pool.mjs` (t1543, GH#5243)
...
- Pool file: `~/.maestro/oauth-pool.json` (separate from OpenCode's `auth.json`)
