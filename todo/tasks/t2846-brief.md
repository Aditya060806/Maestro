...
3. Detection rules (in priority order):
   ...
   - Maintainer override: explicit tier from a `--sensitivity` flag on `maestro knowledge add` writes through
...
6. Override CLI: `maestro knowledge sensitivity <source-id> <tier> [--reason "..."]` for human correction with audit trail
...
- EDIT: `.agents/maestro/knowledge-plane.md` (add sensitivity section)
...
- [ ] Override CLI: `maestro knowledge sensitivity <source-id> internal --reason "...corrected"` updates meta.json + audit log
...
- [ ] Documentation: `.agents/maestro/knowledge-plane.md` sensitivity section
