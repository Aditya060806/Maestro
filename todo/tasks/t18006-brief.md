...
4. Require `sudo maestro fleet unlock <device>` on controlling machine plus hidden target Vault passphrase prompt; never accept passphrase in args/env/chat.
...
- [ ] Docs clearly state cold boot/FileVault/OS login may still require local access before maestro can receive commands.
