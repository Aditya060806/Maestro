...
Document the future extension envelope for the merged `chromium-debug-use` guide: which classes of Electron apps may be able to expose a compatible debugging path, where macOS-level app/window automation could help maestro discover or focus the right local app/browser context, and what remains explicitly out of scope for v1. The deliverable should ground future follow-up work in documented constraints instead of implied promises.
