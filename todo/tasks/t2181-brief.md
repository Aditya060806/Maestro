...
`detectTaskId(cwd)` in `.agents/plugins/opencode-maestro/otel-enrichment.mjs`
extracts the maestro task ID from the current working directory so the
`maestro.task_id` OTEL attribute can be attached to every tool span. The
...
which match. Result: in the common case `maestro.task_id=""`.

## Why

`maestro.task_id` is the primary join key from OTEL traces back to
...
`~/Git/maestro.feature-t2177-otel-introspect`, ran `detectTaskId` against
...
| Path | Expected | Got |
|------|----------|-----|
| `~/Git/maestro.feature-t2177-otel-introspect` | `t2177` | `t2177` |
| `~/Git/maestro-feature-t2181-desc` | `t2181` | `t2181` |
...
- EDIT: `.agents/plugins/opencode-maestro/otel-enrichment.mjs:126,128` — swap regexes; update `@example` block in JSDoc to list all three shapes.
- NEW: `.agents/plugins/opencode-maestro/tests/test-otel-enrichment.mjs` — Node native `node:assert/strict` test; 8 assertions covering 3 worktree shapes (2 task types each), env-var precedence, false-positive negative case.

## Acceptance

1. All 8 test assertions pass under plain `node <file>`.
2. `detectTaskId("~/Git/maestro.feature-t2177-otel-introspect")` returns `"t2177"`.
...
```bash
node .agents/plugins/opencode-maestro/tests/test-otel-enrichment.mjs
