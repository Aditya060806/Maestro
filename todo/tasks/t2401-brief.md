...
- **EDIT**: `.agents/scripts/dispatch-claim-helper.sh`
  - `_post_claim` (~line 155): read framework version from `~/.maestro/agents/VERSION`; append `version=X.Y.Z` to the claim body template. Backward-compatible — consumers tolerate missing field.
...
- Version read: `cat ~/.maestro/agents/VERSION` or source from `shared-constants.sh` if it exposes a constant.
