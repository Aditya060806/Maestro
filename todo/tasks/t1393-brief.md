...
- **Conversation context:** While evaluating LangWatch for maestro integration, identified that our compare-models tooling only compares models by static specs (pricing, context window). LangWatch's ComparisonCharts/ComparisonTable run the same prompt through N models and compare actual outputs with cost/latency/quality metrics. This is the highest-value gap to fill.
...
Use `ai-research` MCP tool or direct provider API calls (via `curl` with keys from `$ANTHROPIC_API_KEY`/`$OPENAI_API_KEY` env vars). Store API keys in a secure secrets store (`maestro secret set <NAME>` via gopass) — never hardcode credentials in scripts or briefs. The `compare-models-helper.sh discover` command already detects available providers — reuse that for model availability.
...
Store at `~/.maestro/.agent-workspace/observability/bench-results.jsonl`.
...
- [ ] Results stored in JSONL at `~/.maestro/.agent-workspace/observability/bench-results.jsonl`
...
- CLI-first approach chosen over UI — fits maestro's terminal-native workflow
