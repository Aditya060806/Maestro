...
7. CLI: `maestro email thread <message-id>`, `maestro email filter add|list|test`
...
- EDIT: `.agents/cli/maestro` (add `email thread` and `email filter` subcommand groups)
...
- EDIT: `.agents/maestro/knowledge-plane.md` (threading + filtering section)

## Acceptance Criteria

- [ ] `email-thread-helper.sh build` reconstructs threads across email corpus and writes `_knowledge/index/email-threads/<id>.json`
- [ ] `maestro email thread <message-id>` returns thread ID + chronological source list
...
- [ ] Documentation: thread + filter sections in `.agents/maestro/knowledge-plane.md`
