...
Investigate and fix intermittent failures in mutating GitHub CLI commands (`gh pr merge`, `gh api` writes, issue comment/close paths) that currently fail with `/bin/zsh` `posix_spawn` errors in maestro sessions.
