...
- **Inspired by** Google's always-on-memory-agent consolidation loop, but adapted to maestro's existing architecture (SQLite + bash + existing `learning_relations` table)
...
- **Not a separate daemon**: runs as part of the existing audit pulse cycle (every 24h), not as a persistent background process. This matches maestro's batch-oriented architecture.
