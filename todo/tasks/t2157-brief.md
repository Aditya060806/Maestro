...
2. **Add a log line for the skipped case.** Right after the closing `fi` of the new conditional, add an `else` branch (or inline `print_info` before the conditional) that logs:
   ...
   ```
   This makes the deviation visible in `~/.maestro/logs/issue-sync.log` for future diagnosis.
...
sleep 30
gh issue list --repo Aditya060806/Maestro --search "t9998 Test sync skip" --json number,assignees
...
- [ ] Skipped case logs an `[INFO]` line to `~/.maestro/logs/issue-sync.log`
