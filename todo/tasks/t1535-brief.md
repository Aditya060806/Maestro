...
# t1535: Fix maestro pulse start silently succeeds on Linux without scheduler installed

## Origin
Pulse session 2026-03-16. Filed as GH#5085.

## What
Add scheduler verification to `pulse-session-helper.sh` so `maestro pulse start` warns when no scheduler (launchd/cron) is configured, and replace hardcoded "launchd" strings with OS-appropriate terminology.
