...
Design and implement maestro-managed runtime profiles that place AI session/history databases and transcript caches behind the Vault boundary, so supported AI apps can access them only while maestro is running and unlocked.
...
- [ ] Locked Vault blocks maestro session lookup/mining against managed encrypted histories.
