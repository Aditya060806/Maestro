...
- **Created:** 2026-04-21
- **Session:** claude-code:maestro-t2450-postmerge
...
3. **Layer 3 — Per-repo remote audit.** New `maestro security scan-remotes` subcommand that walks `repos.json` and reports remotes with credential-bearing URLs, emitting an advisory when findings exist.
...
4. Extend `install-pre-push-guards.sh`:
   - Add `DEPLOYED_CREDENTIAL_HOOK="$HOME/.maestro/agents/hooks/credential-emission-pre-push.sh"`.
...
5. Add `scan-remotes` to `secret-hygiene-helper.sh`:
   - Function `cmd_check_remotes()` that reads `~/.config/maestro/repos.json`, iterates `initialized_repos[].path`, runs `git -C <path> remote get-url origin 2>/dev/null`, matches against credential-URL pattern (`://[^@/]+@`).
   - On finding: emit `[DIRTY] <slug>: <remediation>` (NEVER emit the URL itself).
   - On summary: if any dirty, write advisory file to `~/.maestro/advisories/remote-credentials-<date>.advisory`.
...
cd ~/Git/maestro-bugfix-t2458-credential-url-sanitize
...
- `.agents/scripts/secret-hygiene-helper.sh` — existing `maestro security` subcommand dispatcher.
