...
Add a `maintainer` field to each repo entry in `~/.config/maestro/repos.json`. This field stores the GitHub username of the person responsible for reviewing code-simplifier issues and other maintainer-gated workflows for that repo.
...
2. Add a `get_repo_maintainer` helper function to `maestro.sh` that reads the field with slug-owner fallback
3. Update `register_repo` in `maestro.sh` to auto-detect maintainer from `gh api` when registering new repos
...
- [ ] `get_repo_maintainer` function exists in maestro.sh and returns the correct username
...
- repos.json: `~/.config/maestro/repos.json` (user config, not committed)
- maestro.sh: `register_repo` function (lines 147-204)
