...
# t1914: Decompose opencode-maestro plugin directory (architectural refactor)
...
- `.agents/plugins/opencode-maestro/index.mjs` — plugin entry point (1406 lines)
- `.agents/plugins/opencode-maestro/oauth-pool.mjs` — OAuth pool management (3229 lines)
- `.agents/plugins/opencode-maestro/provider-auth.mjs` — auth hook (1155 lines)
- `.agents/plugins/opencode-maestro/cursor/proxy.js` — Cursor proxy (1245 lines)
- `.agents/plugins/opencode-maestro/google-proxy.mjs` — Google proxy (535 lines)
- `.agents/plugins/opencode-maestro/agent-loader.mjs` — agent config loader
- `.agents/plugins/opencode-maestro/observability.mjs` — telemetry (750 lines)
- `.agents/plugins/opencode-maestro/ttsr.mjs` — TTSR handler

### Verification

```bash
# All plugin files under complexity 50
~/.qlty/bin/qlty smells --all 2>&1 | grep 'plugins/opencode-maestro' | grep -v 'High total complexity'

# Plugin loads successfully
# (manual: start opencode session, verify plugin initialises)

# No regression in external API
grep -r 'export' .agents/plugins/opencode-maestro/index.mjs | wc -l
```
...
- [ ] No individual file in the plugin directory exceeds Qlty complexity 50
  ...
  verify:
    method: bash
    run: "~/.qlty/bin/qlty smells --all 2>&1 | grep 'plugins/opencode-maestro' | grep -c 'High total complexity' | grep -q '^0$' && echo PASS || echo FAIL"
  ```
...
- [ ] Plugin loads and initialises in opencode without errors
  ...
  verify:
    method: manual
    prompt: "Start an opencode session and verify the maestro plugin loads (check for 'maestro plugin loaded' in logs)"
  ```
...
- `.agents/plugins/opencode-maestro/` — entire plugin directory (15 files)
- `.agents/plugins/opencode-maestro/index.mjs:1` — plugin entry (1406 lines, complexity 223)
- `.agents/plugins/opencode-maestro/oauth-pool.mjs:1` — pool manager (3229 lines, complexity 440)
- `.agents/plugins/opencode-maestro/provider-auth.mjs:1` — auth hook (1155 lines, complexity 334)
- `.agents/plugins/opencode-maestro/cursor/proxy.js:1` — Cursor proxy (1245 lines, complexity 171)
