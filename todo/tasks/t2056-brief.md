...
claim <issue> <slug>
    ...
    stamp to ~/.maestro/.agent-workspace/interactive-claims/<slug-flat>-<issue>.json.
...
```json
{
  ...
  "worktree_path": "/Users/Aditya060806/Git/maestro-feature-t2055-interactive-session-claim",
  "claimed_at": "2026-04-13T22:30:00Z",
  "pid": 12345,
  "hostname": "mbp-Aditya060806"
}
```
...
- The slash command `/release-issue <N>` and the CLI `maestro issue release
...
# 3. Manual smoke test
./.agents/scripts/interactive-session-helper.sh claim 18738 Aditya060806/Maestro
gh issue view 18738 --repo Aditya060806/Maestro --json labels,assignees
./.agents/scripts/interactive-session-helper.sh status 18738
./.agents/scripts/interactive-session-helper.sh release 18738 Aditya060806/Maestro
gh issue view 18738 --repo Aditya060806/Maestro --json labels,assignees
...
- **Why stamp directory at `~/.maestro/.agent-workspace/interactive-claims/`?** Matches the existing workspace convention in `prompts/build.txt` ("Working Directories" section). Persistent across sessions but per-user, per-machine.
