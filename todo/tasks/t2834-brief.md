...
- **Created by:** Aditya060806 (human) via interactive conversation
- **Conversation context:** User asked whether maestro can add a lines-of-code badge across all repos. After discussing shields.io / codetabs / self-hosted options, user chose self-hosted SVG via reusable workflow with language breakdowns. User then expanded scope to "maestro should have a badges template that we can apply to the readme of all repos we maestro init and manage with repos.json". This task ships Phase 1 (the framework artifacts); Phase 2 (CLI + init hook + check/sync) is filed separately.
...
4. **`badges.md.tmpl`** — README badges template with `{{SLUG}}` / `{{DEFAULT_BRANCH}}` / `{{HAS_RELEASES}}` placeholders. Logical badge groups: Build/Quality, License, GitHub Stats (LOC, languages, last commit, contributors, commit activity, code size), Release, Community, Framework-specific (optional). Bounded by `<!-- maestro:badges:start -->` / `<!-- maestro:badges:end -->` markers for idempotent updates.
...
6. **`badges.md`** — feature documentation under `.agents/maestro/`.
...
- The maestro README has a great badge block but it's hand-maintained; replicating that quality across every repo by hand doesn't scale.
...
- Establishes the artifacts that Phase 2 (`maestro badges check|sync|render` + `maestro init` hook) will operate on. Phase 1 must ship first so Phase 2 has something to apply.
...
- `.agents/maestro/badges.md`
- `.github/workflows/loc-badge-reusable.yml`
- `todo/tasks/t2834-brief.md`

### Implementation notes

- **`loc-badge-helper.sh`**: depend on `tokei` + `jq` (both Ubuntu-installable in a single apt-get). Output dir defaults to `.github/badges/`. Top-N defaults to 6. Inline SVG generation (no external SVG libraries) — language colors map from a small table mirroring GitHub's language colors for the most common ones, with a fallback grey. Exclude common dirs by default (`__maestro/`, `node_modules/`, `vendor/`, `.git/`, `dist/`, `build/`).
- **`readme-badges-helper.sh`**: read repo metadata from `~/.config/maestro/repos.json` for the given slug. Compute `HAS_LICENSE` (file exists), `HAS_RELEASES` (gh api call, fail-soft to false), `IS_FOSS` (repos.json `foss` key). Use `awk` for marker-block replacement to keep dependency surface small.
- **`loc-badge-reusable.yml`**: model on `issue-sync-reusable.yml` exactly — same pattern for `actions/checkout` of maestro scripts at runtime, same `SYNC_PAT||GITHUB_TOKEN` token logic, same bot-author guard for loop prevention. The reusable workflow checks out maestro at `${{ inputs.maestro_ref || 'main' }}`.
- **Caller template**: model on `issue-sync-caller.yml` — minimal, declares triggers, `uses: Aditya060806/Maestro/.github/workflows/loc-badge-reusable.yml@main`, `secrets: inherit`.
- **README block markers**: use HTML comments so they're invisible in rendered markdown. `<!-- maestro:badges:start -->` / `<!-- maestro:badges:end -->`. The injector writes a notice line inside: `<!-- managed by maestro badges; edit template not block -->`.
...
.agents/scripts/readme-badges-helper.sh render Aditya060806/Maestro > /tmp/badges.md
...
2. `readme-badges-helper.sh render Aditya060806/Maestro` outputs a markdown block bounded by the start/end markers, with all `{{SLUG}}` placeholders substituted.
...
6. `.agents/maestro/badges.md` documents installation, the marker convention, the SVG output paths, and forward-references the Phase 2 CLI work.
...
Add `maestro badges check|sync|render|install` subcommand mirroring `maestro check-workflows`/`sync-workflows`. Hook `maestro init` to offer badge-block injection on first repo setup. Cross-repo drift detection via the helper's `check` subcommand iterating `~/.config/maestro/repos.json`.
