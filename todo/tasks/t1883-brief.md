...
Interactive session (t1880 attribution protection work). The user correctly identified that putting detection search patterns in the public maestro repo would reveal what strings we're looking for, allowing a copycat to strip them first.

## What

Create a separate **private** GitHub repo (`Aditya060806/Maestro-provenance` or similar) containing:

1. **GitHub Code Search script** — periodically searches for distinctive strings from the maestro codebase across all public GitHub repos
...
3. **Attribution manifest** — the private inventory of all watermark artifacts, their locations, and verification procedures (currently at `~/.maestro/.agent-workspace/work/provenance-worker/MANIFEST.md`)
4. **Scheduled routine** — launchd plist or GitHub Actions cron to run detection periodically

## Why

The public maestro repo cannot contain detection logic without revealing what we're searching for. A copycat monitoring our repo would see the search strings and strip them preemptively. The detection infrastructure must be in a separate private repository.
...
- **Unique to maestro** — not found in other projects naturally
...
1. SPDX copyright text: `"SPDX-FileCopyrightText: 2025-2026 Aditya Pandey and Harvest"`
2. Canary endpoint: `"provenance-ping.Aditya060806.workers.dev"` or `"provenance.maestro.sh"`
...
1. Create private repo: `gh repo create Aditya060806/Maestro-provenance --private`
...
8. Create local launchd plist `sh.maestro.provenance-scan` for on-demand runs
...
1. Private repo exists at `Aditya060806/Maestro-provenance`
2. `search-github.sh` finds the maestro repo itself when run (baseline validation)
...
- Cloudflare Worker: `provenance-ping` at `provenance.maestro.sh`
- KV namespace ID: `3a58e39603844456937cfc44c89993d6`
- Cloudflare account ID: `18b144721614d93010eb869196ab0c1c`
- Worker source: `~/.maestro/.agent-workspace/work/provenance-worker/`
- Current MANIFEST: `~/.maestro/.agent-workspace/work/provenance-worker/MANIFEST.md`
...
- The detection repo should NOT be referenced from the public maestro repo in any way
