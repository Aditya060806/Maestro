...
7. `.agents/bundles/agent.json` — maestro agent/MCP development
...
- Use JSON Schema for validation (compatible with existing .maestro.json pattern)
...
- Bundles are JSON (not YAML or TOML) to match existing maestro config patterns (.maestro.json, repos.json)
- Composition uses union for quality_gates and most-restrictive for model_defaults
- The `agent` bundle is meta — it configures maestro to develop itself (shellcheck, markdown-lint, agent-review)
- Bundle names are lowercase kebab-case to match file naming conventions

## Relevant Files

- `~/.config/maestro/repos.json` — repo configuration to extend
