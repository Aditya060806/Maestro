...
if [[ $_body_file_idx -ge 0 && -n "$_body_file_val" && -f "$_body_file_val" ]]; then
    if ! grep -q "<!-- maestro:sig -->" "$_body_file_val" 2>/dev/null; then
...
if [[ $_body_file_idx -ge 0 && -n "$_body_file_val" && -f "$_body_file_val" ]]; then
    if ! grep -q "<!-- maestro:sig -->" "$_body_file_val" 2>/dev/null; then
        ...
        if [[ -n "$_sig_footer" ]]; then
            ...
            _tmp_body_file=$(mktemp -t maestro-gh-shim-body.XXXXXX) || _tmp_body_file=""
...
gh issue create --repo Aditya060806/Maestro-test --title "shim test" --body-file /tmp/brief-test.md --dry-run 2>&1 || true
...
ls /tmp/maestro-gh-shim-body.* 2>/dev/null
sleep 35
ls /tmp/maestro-gh-shim-body.* 2>/dev/null && echo "FAIL: temp files leak" || echo "PASS: temp cleanup"
```
