...
- **Source:** Review of [GH#17404](https://github.com/Aditya060806/Maestro/issues/17404) — closing comment identified documentation/onboarding gap

## What

When a user enables `orchestration.supervisor_pulse` (via `maestro config set` or during onboarding), print a one-liner informing them that the pulse runs 24/7 by default and that `maestro pulse stop` creates a persistent stop flag for manual start/stop mode.
...
1. **`scripts/config-helper.sh`** — In the `set` subcommand handler, after successfully writing `orchestration.supervisor_pulse` to `true`, echo guidance:
   ...
   [INFO] Run 'maestro pulse stop' to switch to manual start/stop mode (persistent across reboots).
   ```
...
- When `maestro config set orchestration.supervisor_pulse true` succeeds, guidance message is printed to stderr
- When `maestro config set orchestration.supervisor_pulse false` succeeds, no guidance is printed
...
- The stop flag (`~/.maestro/.pulse-stopped`) persists across reboots and is only cleared by `maestro pulse start`
