...
`version-manager.sh commit_version_changes` currently fails on the pre-commit quality hook because the hook flags pre-existing `$1` usage in `maestro.sh` one-liner print helpers (lines 64-66) every time the file is touched — even when the only change is a VERSION string bump. Pick one of three mitigation approaches and apply it.
...
```text
[ERROR] Direct positional parameter usage in maestro.sh
...
~/.maestro/agents/scripts/version-manager.sh release patch --dry-run
...
- `maestro.sh` lines 64-66 have been idiomatic shell since at least v3.8.67 (confirmed via `git log --oneline -- maestro.sh`).
