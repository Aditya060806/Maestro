...
A two-layer memory system that gives maestro multi-channel agents the ability to:
...
Entity interactions (Layer 0)
    ...
    → System upgrade (normal maestro task lifecycle)
...
- [ ] Layer 0: Raw interactions are stored immutably with entity_id, channel, direction, timestamp
  ...
  verify:
    method: bash
    run: "sqlite3 ~/.maestro/.agent-workspace/memory/memory.db '.tables' | grep -q interactions"
  ```
...
- [ ] Identity resolution: entities can be linked across channels with verified/unverified status
  ...
  verify:
    method: bash
    run: "sqlite3 ~/.maestro/.agent-workspace/memory/memory.db 'SELECT sql FROM sqlite_master WHERE name=\"entity_channels\"' | grep -q verified"
  ```
...
- [ ] Existing memory system (`/remember`, `/recall`) continues to work unchanged
  ...
  verify:
    method: bash
    run: "~/.maestro/agents/scripts/memory-helper.sh recall --query 'test' --limit 1 2>&1; echo $?"
  ```
...
- [ ] Tests pass, ShellCheck clean on all new scripts
  ...
  verify:
    method: bash
    run: "shellcheck ~/.maestro/agents/scripts/entity-helper.sh ~/.maestro/agents/scripts/conversation-helper.sh"
  ```
