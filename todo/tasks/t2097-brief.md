...
1. **Add the flag-file constant** in `pulse-wrapper.sh` config block (around line 294 where `PULSE_PREFETCH_CACHE_FILE` is defined):
    ...
    ```bash
    PULSE_RATE_LIMIT_FLAG="${PULSE_RATE_LIMIT_FLAG:-${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag}" # GH#<NEW>: set by prefetch on detected GraphQL rate-limit exhaustion; checked by _preflight_prefetch_and_scope to abort cycle cleanly
    ```
...
touch "${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag"
echo "2026-04-14T00:00:00Z test" > "${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag"
...
#           Return code 1
rm -f "${HOME}/.maestro/logs/pulse-graphql-rate-limited.flag"
```
