...
- **Parent task:** t1741
- **Conversation context:** User identified that standalone research repos (for topics without existing code) should get `maestro init` applied so they benefit from task management, git workflow, memory, and pulse visibility. Unmanaged research repos are invisible to cross-repo tools.

## What

Implement the `/autoresearch init "name"` subcommand that scaffolds a new research repository with full maestro integration.

The command creates:
1. `~/Git/autoresearch-{name}/` directory
2. `git init` inside it
3. `maestro init` for task management
4. Registration in `~/.config/maestro/repos.json`
5. Research-specific scaffold (program.md, baseline/, results tracking)
6. Optional: immediately begin the experiment loop

## Why

Some research topics don't have an existing codebase — ML model training, algorithm comparison, prompt engineering, data analysis experiments. These need a repo to live in, and that repo should be a first-class citizen in the maestro ecosystem (task tracking, memory, pulse dispatch, cross-session learning). Without `maestro init`, the research repo is orphaned and loses most of the value.
...
│   └── .gitkeep
├── TODO.md                 # from maestro init
├── todo/
│   └── research/
│       └── {name}.md       # research program file (from t1742 template)
└── .maestro.json          # from maestro init
```
...
```json
{
  ...
  "slug": "Aditya060806/autoresearch-{name}",
  "pulse": false,
  "local_only": true,
  "priority": "research",
  "app_type": "generic"
...
- `grep autoresearch ~/.config/maestro/repos.json` finds all registered ones
...
- [ ] `/autoresearch init "name"` creates `~/Git/autoresearch-{name}/` with git init
  ...
  ```
- [ ] `maestro init` is applied to the new repo
  ```yaml
  verify:
    method: codebase
    pattern: "maestro init"
    path: ".agents/scripts/commands/autoresearch.md"
  ```
...
- `setup.sh` — `maestro init` implementation reference
...
| Phase | Time | Notes |
|-------|------|-------|
| Research/read | 20m | Review maestro init, repos.json format |
