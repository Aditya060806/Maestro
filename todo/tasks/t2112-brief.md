...
# t2112: feat(pulse): reconcile pass for labelless maestro-shaped issues
...
4. Posts a one-time mentorship comment (idempotent, guarded by an HTML sentinel marker `<!-- maestro:labelless-backfill -->`) linking the operator to the `gh_create_issue` wrapper rule in `prompts/build.txt` "Origin labelling (MANDATORY)".
...
- EDIT: `.agents/scripts/pulse-issue-reconcile.sh` — add `reconcile_labelless_maestro_issues` function after `reconcile_stale_done_issues`, model on the existing function's structure (per-repo loop, 20-issue cap, log to `$LOGFILE`).
...
**Step 1 — add `reconcile_labelless_maestro_issues` function.** Follows the same scaffolding as `reconcile_stale_done_issues`:
...
- `gh issue list --repo "$slug" --state open --json number,title,body,labels --limit 100` then `jq` filter for title-matches-shape AND labels-array-is-empty-or-missing-maestro-namespaces.
- For each hit:
...
**Step 2 — wire into pulse cycle.** Find the block in `pulse-wrapper.sh` that calls `reconcile_stale_done_issues` and add a call to `reconcile_labelless_maestro_issues` immediately after it, guarded on the same conditions.

**Step 3 — test harness.** Stub `gh` on `PATH` with a shim that:

- Responds to `gh issue list` with canned JSON containing: one labelless maestro-shaped issue, one labelless non-maestro issue (ignored), one already-labelled maestro issue (ignored).
- Records `gh issue edit` / `gh issue comment` calls to a trace file.
- Assert: the labelless maestro issue triggered `--add-label origin:worker`, `--add-label tier:standard`, and exactly one comment with the sentinel marker.

### Verification

```bash
cd /Users/Aditya060806/Git/maestro-feature-t2112-pulse-labelless-reconcile-gh-wrapper-sub-issue-body
...
1. `reconcile_labelless_maestro_issues` is defined in `pulse-issue-reconcile.sh` and exported through the include-guard block comment.
2. `pulse-wrapper.sh` calls the new function once per cycle in the same block as `reconcile_stale_done_issues`.
3. Labelless issues with maestro-shaped titles receive `origin:worker` + `tier:standard` labels on the first pass.
