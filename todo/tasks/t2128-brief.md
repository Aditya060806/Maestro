...
# t2128: refactor(opencode-maestro) — decompose oauth-pool plugin cluster
...
- **Conversation context:** Child of the qlty A-grade campaign. The oauth-pool plugin was partially decomposed in PR #18906 (t2071), which extracted `cursor/proxy.js`, `ttsr.mjs`, `provider-auth.mjs`, `google-proxy.mjs` from the opencode-maestro cluster. However, the 3 `oauth-pool*.mjs` files remain at file-complexity levels that qlty flags.

## What

Clear the 3 remaining `qlty:file-complexity` smells in `.agents/plugins/opencode-maestro/`:
...
head -35 .agents/plugins/opencode-maestro/oauth-pool-auth.mjs
...
rg "from.*oauth-pool" .agents/plugins/opencode-maestro/ --no-heading
...
- `EDIT: .agents/plugins/opencode-maestro/oauth-pool.mjs` (1220 lines, complexity 117) — the core module. Extract into:
  - `NEW: .agents/plugins/opencode-maestro/oauth-pool-constants.mjs` — shared constants, user agents, URLs (resolves the circular dep)
  - `NEW: .agents/plugins/opencode-maestro/oauth-pool-storage.mjs` — pool file I/O: `loadPool`, `savePool`, `withPoolLock`, `getPoolFilePath`, `getAccounts`, `upsertAccount`, `removeAccount`, `patchAccount`
  - `NEW: .agents/plugins/opencode-maestro/oauth-pool-rotation.mjs` — the 429 rotation fetch wrapper, cooldown tracking, `resolveInjectFn`, injection functions
...
- `EDIT: .agents/plugins/opencode-maestro/oauth-pool-auth.mjs` (312 lines, complexity 121) — split provider-specific OAuth flows into:
  - `NEW: .agents/plugins/opencode-maestro/oauth-pool-auth-anthropic.mjs` — Anthropic OAuth flow
  - `NEW: .agents/plugins/opencode-maestro/oauth-pool-auth-openai.mjs` — OpenAI OAuth flow
...
- `EDIT: .agents/plugins/opencode-maestro/oauth-pool-tool.mjs` (~350 lines, complexity 72) — extract action handlers:
  - `NEW: .agents/plugins/opencode-maestro/oauth-pool-actions.mjs` — the per-action handlers (`list`, `rotate`, `remove`, `assign-pending`, `check`, `status`, `reset-cooldowns`, `set-priority`). Keep `createPoolTool` in `oauth-pool-tool.mjs` as the dispatcher.
...
for f in .agents/plugins/opencode-maestro/oauth-pool*.mjs; do node --check "$f" && echo "OK: $f" || echo "FAIL: $f"; done

# 2. Import resolution (the main module must still be importable):
node -e "import('./agents/plugins/opencode-maestro/oauth-pool.mjs').then(() => console.log('OK')).catch(e => { console.error(e); process.exit(1); })"
...
- [ ] `node --check` passes for every `oauth-pool*.mjs` file (original + new)
  ...
  verify:
    method: bash
    run: "for f in .agents/plugins/opencode-maestro/oauth-pool*.mjs; do node --check \"$f\" || exit 1; done"
  ```
...
- [ ] Existing `from "./oauth-pool.mjs"` imports throughout the plugin directory still resolve (barrel re-export)
  ...
  verify:
    method: bash
    run: "rg 'from.*oauth-pool\\.mjs' .agents/plugins/opencode-maestro/ -l | while read f; do node --check \"$f\" || exit 1; done"
  ```
...
- **Barrel re-export pattern:** The prior decomposition (t2071, PR #18906) used barrel re-exports in `index.mjs` for the opencode-maestro plugin. `oauth-pool.mjs` should follow the same pattern — keep it as the public entry that re-exports from the new modules.
...
- **Prior art:** PR #18893 (claude-proxy → 6 modules), PR #18906 (opencode-maestro cluster), PR #18860 (oauth-pool namespace fix).

## Relevant Files

- `.agents/plugins/opencode-maestro/oauth-pool.mjs:1` — core module (1220 lines)
- `.agents/plugins/opencode-maestro/oauth-pool-auth.mjs:1` — auth hooks (312 lines)
- `.agents/plugins/opencode-maestro/oauth-pool-tool.mjs:1` — MCP tool (350 lines)
