...
- `run:` — path to a script for pure-script routines (no LLM tokens). Relative to `~/.maestro/agents/`
...
- `run:` — path to script for pure-script execution (no LLM tokens). Relative to `~/.maestro/agents/`
...
- [ ] `repeat:` field documented in TODO.md Format section with all 4 syntax variants
  ...
  verify:
    method: bash
    run: "grep -q 'repeat:daily' ~/Git/maestro/TODO.md"
  ```
- [ ] `run:` and `agent:` fields documented with dispatch rules
  ```yaml
  verify:
    method: bash
    run: "grep -q 'run:' ~/Git/maestro/TODO.md && grep -q 'agent:' ~/Git/maestro/TODO.md"
  ```
- [ ] `r`-prefix ID convention documented
  ```yaml
  verify:
    method: bash
    run: "grep -q 'r001' ~/Git/maestro/TODO.md"
  ```
...
- Custom agents and scripts live in `~/.maestro/agents/custom/` (survives updates)
