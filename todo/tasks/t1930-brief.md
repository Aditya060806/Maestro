...
time ~/.maestro/agents/scripts/memory-pressure-monitor.sh --check
...
- [ ] `_check_processes` completes in <2s on a typical macOS system (~500 processes)
  ...
  verify:
    method: bash
    run: "timeout 5 bash -c 'time ~/.maestro/agents/scripts/memory-pressure-monitor.sh --check 2>&1' | grep -q real"
  ```
