...
Establishes `_inbox/` as a top-level peer to the planes — visible in any maestro-managed repo, treated as a transit zone rather than authoritative storage. Provisioning helper creates the structure on `maestro init` for new repos and on demand for existing repos. Files in `_inbox/` are by-default `unverified` for sensitivity until triage classifies them.

After completion:

- `maestro init` provisions `_inbox/` with sub-folders.
- `maestro inbox provision` adds it to existing repos idempotently.
- The directory has a `README.md` documenting the transit-zone semantics.
- Workspace-level (cross-repo) inbox at `~/.maestro/.agent-workspace/inbox/` provisioned on first install.
...
- `EDIT: .agents/scripts/maestro` — register `inbox` subcommand (delegate to `inbox-helper.sh`).
- `EDIT: setup.sh` — call `inbox-helper.sh provision-workspace` on install for the cross-repo inbox at `~/.maestro/.agent-workspace/inbox/`.
...
3. Implement `inbox-helper.sh provision-workspace`:
   - Same structure under `~/.maestro/.agent-workspace/inbox/`
...
5. Hook into `maestro` CLI: `maestro inbox <subcommand>` delegates to helper.
...
- [ ] `maestro inbox provision <repo>` creates the full directory contract idempotently.
- [ ] Workspace inbox provisioned at `~/.maestro/.agent-workspace/inbox/` on `setup.sh` run.
- [ ] `_inbox/.gitignore` excludes binaries (sub-folder contents) but keeps `README.md` + `triage.log`.
- [ ] `maestro inbox status` reports counts and oldest-item age.
- [ ] `shellcheck` clean.
- [ ] Smoke test `test-inbox-provision.sh` PASSes.

## Context & Decisions

- **Why top-level peer to planes:** Visibility — users must see the inbox to use it. Hiding it under `~/.maestro/.agent-workspace/` defeats the capture-friction goal.
