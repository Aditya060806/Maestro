...
- `~/.maestro/.agent-workspace/supervisor/supervisor.db` (if exists)
...
6. **Remove runtime SQLite DBs:** Document which files to remove from `~/.maestro/.agent-workspace/` and add a cleanup step to `setup.sh` or document manual removal.
