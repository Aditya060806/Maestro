...
Evidence (live DB `~/.maestro/.agent-workspace/observability/llm-requests.db`):
...
- [x] No cross-package changes — scoped to `.agents/plugins/opencode-maestro/`.
...
- **EDIT:** `.agents/plugins/opencode-maestro/provider-auth-request.mjs` (add `injectIntentParameter` function; wire it into `applyBodyTransforms` after `prefixToolNames`).
- **NEW:** `.agents/plugins/opencode-maestro/tests/test-intent-schema-injection.mjs` (mirror the pattern at `tests/test-observability-tool-calls.mjs`).
...
- `node --check .agents/plugins/opencode-maestro/provider-auth-request.mjs` — syntax passes.
- `node --test .agents/plugins/opencode-maestro/tests/test-intent-schema-injection.mjs` — all 12 assertions pass.
- `node --test .agents/plugins/opencode-maestro/tests/*.mjs` — 53 total assertions pass (12 new + 41 regression).
- Post-merge smoke test: spin up a fresh opencode session, make a few tool calls, run `sqlite3 ~/.maestro/.agent-workspace/observability/llm-requests.db "SELECT COUNT(*), SUM(intent IS NOT NULL) FROM tool_calls WHERE session_id = '<session>'"` — expect non-zero intent count for `toolu_*` call IDs.
...
3. Test suite passes: `node --test .agents/plugins/opencode-maestro/tests/*.mjs`.
