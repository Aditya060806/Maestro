...
- **Conversation context:** While debugging t2859 (unbound `ORPHAN_WORKTREE_GRACE_SECS` causing 49 worktree destructions in 1h), inspected the SQLite worktree registry at `~/.maestro/.agent-workspace/worktree-registry.db`. Found 815 April entries with stale PIDs — the cleanup pass deletes worktrees but never deregisters them from the ownership registry, so entries accumulate forever.
...
# Baseline: sqlite3 ~/.maestro/.agent-workspace/worktree-registry.db "SELECT COUNT(*) FROM worktree_owners;"
...
- `~/.maestro/.agent-workspace/worktree-registry.db` — current registry state (815 stale-looking April entries).
