...
1. **Detection latency.** By the time alex-solovyev's degradation was spotted 2026-04-19, Aditya060806's runner had sat at `dispatched=0` for most of a day.
...
- Watchdog kills: `~/.maestro/logs/pulse.log` grep `[worker-activity-watchdog]` over window.
...
Subcommands:
- `snapshot` — compute current success-rate for the local runner, write to `~/.maestro/logs/fleet-health-self.json` with timestamp.
...
- `auto-update` — if `FLEET_AUTO_OVERRIDE_ENABLED=true`, apply the suggested list into `~/.config/maestro/dispatch-override.conf` with an audit timestamp header.
...
- Audit: every change writes to `~/.maestro/logs/fleet-health-audit.log` (timestamped, human-readable).
...
- **Emergency opt-out**: `FLEET_AUTO_OVERRIDE_ENABLED=false` in `~/.config/maestro/dispatch-override.conf` forces back to manual mode.
...
- Dashboard write: find existing supervisor dashboard logic (`grep -rn "supervisor.*dashboard\|pinned.*issue" ~/.maestro/agents/scripts/`).
...
- #10944 — Aditya060806 supervisor dashboard (stale since 2026-04-08; refresh needs fixing as part of step (a)).
