...
3. Stats counter `pre_dispatch_aborts_recent_commit` in `pulse-stats-helper.sh` and surfaced in `maestro status` output.
...
5. Env override `MAESTRO_PREDISPATCH_RECENT_COMMIT_WINDOW=<seconds>` (default 600) mirroring the existing `MAESTRO_PREDISPATCH_RECENT_MERGE_WINDOW` pattern.
...
grep -c "no_work skip-escalation" ~/.maestro/logs/pulse-wrapper.log \
...
5. **Stats counter:** add `pre_dispatch_aborts_recent_commit` as a sibling to the existing `pre_dispatch_aborts_*` keys in `pulse-stats-helper.sh`. Surface in `maestro.sh::cmd_status` output.
...
# maestro status surfaces the new counter
maestro status | grep -q "pre_dispatch_aborts_recent_commit"
```

## Acceptance Criteria

- [ ] `is_issue_eligible_for_dispatch()` returns exit code 5 when a commit on the default branch within `MAESTRO_PREDISPATCH_RECENT_COMMIT_WINDOW` seconds contains a valid closing keyword referencing the candidate issue number.
  ```yaml
  verify:
    method: bash
...
- [ ] `Ref #NNN` and `For #NNN` planning-reference commits do NOT trigger the new check (regex precision matches `pulse-merge.sh::_extract_linked_issue`).
  ...
  ```
- [ ] `pre_dispatch_aborts_recent_commit` counter increments when the new check fires and is surfaced in `maestro status`.
  ```yaml
  verify:
    method: codebase
    pattern: "pre_dispatch_aborts_recent_commit"
    path: ".agents/scripts/pulse-stats-helper.sh .agents/maestro.sh"
  ```
