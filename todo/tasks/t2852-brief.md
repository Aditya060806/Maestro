...
1. `maestro case attach <case-id> <source-id> [--role evidence|reference|background]` — appends to `sources.toon`, timeline entry
2. `maestro case status <case-id> <new-status> [--reason "..."]` — open|hold|closed; timeline entry
3. `maestro case close <case-id> --outcome <outcome> [--summary "..."]` — sets status=closed + outcome field; final timeline entry
4. `maestro case archive <case-id>` — moves case to `_cases/archived/` (still readable, filtered out of default list)
5. `maestro case list [--status open|hold|closed|archived] [--kind <type>] [--party <name>]` — table output
6. `maestro case show <case-id>` — pretty-prints dossier + timeline + attached sources
7. `maestro case note <case-id> [--message "..."]` — appends to `notes/notes.md` and timeline
8. `maestro case deadline add|remove <case-id> [--date ISO --label "..."]`
9. `maestro case party add|remove <case-id> --name "..." --role "..."`
10. `maestro case comm log <case-id> --direction in|out --channel <c> --summary "..."` — communications log entry (full email content lands later via P5; this is for paper-trail entries)
...
- EDIT: `.agents/cli/maestro` (route subcommands)
- EDIT: `.agents/maestro/cases-plane.md` (CLI section)
- NEW: `.agents/tests/test-case-cli.sh`

## Acceptance Criteria

- [ ] `maestro case attach <case-id> <source-id> --role evidence` appends to `sources.toon` and timeline; refuses attach to non-promoted sources
- [ ] `maestro case status <case-id> hold --reason "awaiting client response"` updates dossier + timeline
- [ ] `maestro case close <case-id> --outcome settled --summary "..."` sets status=closed + outcome + summary; refuses close-without-outcome
- [ ] `maestro case archive <case-id>` moves dir to `_cases/archived/` and adds timeline entry
- [ ] `maestro case list` shows open cases by default; `--status all` shows closed+archived too
- [ ] `maestro case list --party "ACME Ltd"` filters
- [ ] `maestro case show <case-id>` produces a readable markdown dossier
- [ ] `maestro case note <case-id> --message "..."` appends to notes/notes.md AND timeline
- [ ] `maestro case deadline add <case-id> --date 2026-08-31 --label "filing deadline"` updates dossier and timeline
