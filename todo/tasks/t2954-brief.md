...
`maestro update` is the framework's auto-update path — when it breaks dispatch this badly on Linux, every Linux runner is affected silently. This fix makes the resolver Linux-native and product-aware so the same class of failure cannot recur.
