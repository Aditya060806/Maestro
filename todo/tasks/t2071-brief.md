...
After this change, `qlty smells --all --sarif | jq '[.runs[0].results[] | select(.locations[0].physicalLocation.artifactLocation.uri | test("opencode-maestro/(cursor|ttsr|provider-auth|google-proxy)"))] | length'` returns `0`.
...
for f in cursor/proxy.js ttsr.mjs provider-auth.mjs google-proxy.mjs; do
  ...
  ~/.qlty/bin/qlty smells --all --sarif --no-snippets --quiet 2>/dev/null \
    | jq -r --arg f ".agents/plugins/opencode-maestro/$f" \
...
cat .agents/plugins/opencode-maestro/claude-proxy-stream.mjs 2>/dev/null \
  || echo "t2070 not yet merged - design fresh pattern"
```

### Files to Modify

- `EDIT: .agents/plugins/opencode-maestro/cursor/proxy.js`
- `EDIT: .agents/plugins/opencode-maestro/ttsr.mjs`
- `EDIT: .agents/plugins/opencode-maestro/provider-auth.mjs`
- `EDIT: .agents/plugins/opencode-maestro/google-proxy.mjs`
...
~/.qlty/bin/qlty smells --all --sarif --no-snippets --quiet 2>/dev/null \
  | jq '[.runs[0].results[] | select(.locations[0].physicalLocation.artifactLocation.uri | test("opencode-maestro/(cursor|ttsr|provider-auth|google-proxy)"))] | length'
# Expected: 0

# Plugin still loads
cd .agents/plugins/opencode-maestro && node -e "import('./ttsr.mjs').then(() => console.log('ok'))"
cd .agents/plugins/opencode-maestro && node -e "import('./google-proxy.mjs').then(() => console.log('ok'))"
```
...
- [ ] Opencode plugin tests pass (`bun test` in `.agents/plugins/opencode-maestro/`)
...
- `.agents/plugins/opencode-maestro/claude-proxy.mjs` — reference pattern if t2070 lands first
- `.agents/plugins/opencode-maestro/README.md` (if exists) — plugin architecture docs
