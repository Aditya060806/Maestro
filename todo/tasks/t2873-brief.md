...
In `.agents/plugins/opencode-maestro/provider-auth-request.mjs`, the `sanitizeSystemPrompt` function applies two regex replacements to every system-prompt block:
...
- .agents/plugins/opencode-maestro/provider-auth-request.mjs
...
Persistent test scripts live at `~/.maestro/.agent-workspace/work/maestro/third-party-name-test/`:
...
All scripts load a token from `~/.maestro/oauth-pool.json` and post to `https://api.anthropic.com/v1/messages?beta=true` with the full real-CLI header set.
...
- `node .agents/plugins/opencode-maestro/tests/test-intent-schema-injection.mjs` passes.
...
- `.agents/plugins/opencode-maestro/provider-auth-request.mjs:114-121` — the function being changed
- `.agents/plugins/opencode-maestro/provider-auth-request.mjs:97-101` — `TAG_RENAMES` (kept)
- `.agents/plugins/opencode-maestro/provider-auth-request.mjs:128-173` — `redistributeSystemToMessages` (handles the size trigger)
...
- `~/.maestro/.agent-workspace/work/maestro/third-party-name-test/` — the persistent test harness
