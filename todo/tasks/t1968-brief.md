...
- **Conversation context:** After t1965 landed we manually loop-installed the guard into 27 existing repos with a one-off script. For the rollout to be durable, every future `maestro init` on a new repo and every `maestro update` on an existing repo should install (or refresh) the hook automatically. Otherwise the guarantee decays: a cloned-from-scratch repo has no protection until the user remembers to run `install-privacy-guard.sh install`.

## What

Extend `.agents/scripts/setup/_routines.sh` (or a sibling `_privacy_guard.sh` module) so that `setup.sh` calls `install-privacy-guard.sh install` once per initialized repo on every `maestro init` and `maestro update` cycle. The installer is already idempotent (it refuses to overwrite unmanaged hooks, re-installs managed hooks cleanly), so repeat invocation is safe.
...
- Tolerate conflict cases (user has a pre-existing non-maestro pre-push hook) without failing the whole setup run — log at info and move on
- Respect an opt-out env var `MAESTRO_PRIVACY_GUARD=false` for users who explicitly don't want the hook installed globally
...
Without this, the privacy guard is a point-in-time installation that erodes as new repos appear. A fresh `git clone` + `maestro init` today does not get the hook. A teammate of the user who follows the setup docs does not get the hook. And the installed repos gradually fall out of sync with the framework's deployed helper because nothing refreshes the dispatcher when the hook path changes.

Integrating into `setup.sh` makes the guarantee durable: every `maestro update` (which runs every ~10 minutes via launchd) will re-confirm the hook is present and pointing at the current deployed helper. New repos get protected immediately on init.
...
1. Create `.agents/scripts/setup/_privacy_guard.sh`:
    ...
    # Opt out by exporting MAESTRO_PRIVACY_GUARD=false before running setup.
    setup_privacy_guard() {
        if [[ "${MAESTRO_PRIVACY_GUARD:-true}" == "false" ]]; then
            print_info "Privacy guard install disabled via MAESTRO_PRIVACY_GUARD=false"
            setup_track_skipped "Privacy guard" "opted out via MAESTRO_PRIVACY_GUARD=false"
            return 0
        fi
        ...
        local repos_config="${HOME}/.config/maestro/repos.json"
...
4. Test: `bash setup.sh --non-interactive` from `~/Git/maestro` and verify stdout reports the privacy-guard summary line.
...
# Opt-out
MAESTRO_PRIVACY_GUARD=false bash setup.sh --non-interactive 2>&1 | grep "opted out"
```
...
- [ ] A repo with a pre-existing non-maestro pre-push hook is counted in `conflict` and does NOT fail the whole setup run.
- [ ] Setting `MAESTRO_PRIVACY_GUARD=false` before running setup skips the privacy-guard step entirely and logs the opt-out.
...
- **Why iterate via `setup.sh` rather than per-repo `maestro init`:** both. `setup.sh` runs during `maestro update` (every 10 min), which catches existing repos on every cycle. The per-repo `maestro init` call also eventually lands in the same setup flow via the `Syncing Initialized Projects` step.
