...
- [x] Memory recall: `maestro reach performance feedback miner issue generator` → 0 hits — no relevant prior lesson found.
- [x] Discovery pass: no t18051 brief/open related PR found; `_performance` and `_feedback` docs are present.
- [x] File refs verified: `.agents/maestro/performance.md`, `.agents/maestro/feedback.md`, and reach/capture target files are present or declared blockers.
...
- **Read first:** `.agents/maestro/performance.md:14-83` — result record semantics.
- **Read first:** `.agents/maestro/feedback.md:141-202` — mining loop and review gates.
...
- `EDIT: .agents/maestro/performance.md` — document reach metric fields/storage.
- `EDIT: .agents/maestro/feedback.md` — document reach feedback thresholds and promotion to tasks.
...
1. Append reach performance JSONL to repo `_performance/reach-capture.jsonl` when available, otherwise to `~/.maestro/.agent-workspace/performance/reach-capture.jsonl`.
...
./maestro.sh reach feedback mine --window 7d --format json
./maestro.sh reach feedback issue --dry-run --format markdown
```
