...
Add post-creation hooks to `worktree-helper.sh add` that exclude every new worktree from macOS Spotlight, Time Machine, and (where scriptable) Backblaze. Also provide a backfill mode for existing worktrees, and integrate detection + one-time setup prompts into `setup.sh` and `maestro update`.

## Why

1. **Universal failure mode.** Every macOS maestro user with backup/index tools running hits this identically — not specific to one machine. It's structural: parallel pulse workers × `node_modules` × OS file watchers.
...
- **NEW:** `.agents/scripts/worktree-exclusions-helper.sh` — model on `.agents/scripts/install-hooks-helper.sh` (similar shape: detect-installed → apply → backfill).
  - Subcommands: `apply <path>` (single worktree), `backfill` (all worktrees in `~/.config/maestro/repos.json`), `detect` (report which tools are present), `setup-backblaze` (print sudo command for manual run).
...
- **EDIT:** `.agents/scripts/maestro-update-check.sh` (or wherever `maestro update` orchestrates) — same detection + advisory pattern as setup.sh. One-time advisory file in `~/.maestro/advisories/` so it doesn't nag.
...
- Advisory mechanism: `~/.maestro/advisories/*.advisory` files documented in `prompts/build.txt` "Security" section.
...
TMPWT=$(mktemp -d)/wt
git -C ~/Git/maestro worktree add -b chore/t2885-test "$TMPWT" main
test -f "$TMPWT/.metadata_never_index" && echo "spotlight: ok"
tmutil isexcluded "$TMPWT" | grep -q '\[Excluded\]' && echo "tmutil: ok"
git -C ~/Git/maestro worktree remove --force "$TMPWT"
...
2. `worktree-exclusions-helper.sh backfill` applies exclusions to all worktrees registered across `~/.config/maestro/repos.json` repos. Idempotent.
...
4. `setup.sh` and `maestro update` detect installed backup tools and print the Backblaze sudo command once. Advisory dismissable per existing mechanism.
