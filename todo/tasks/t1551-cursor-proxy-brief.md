...
Our `opencode-maestro` plugin already runs at startup. Add a Cursor integration phase that:

1. **Read Cursor token from pool** — `~/.maestro/oauth-pool.json` cursor accounts (already populated by `oauth-pool-helper.sh add cursor`)
...
**Recommend vendoring** the relevant source files into `.agents/plugins/opencode-maestro/cursor/` rather than depending on the npm package. Reasons:
...
if (cursorAccounts.length > 0) {
  try {
    ...
    console.error(`[maestro] Cursor proxy started on port ${port}`);
  } catch (err) {
    console.error(`[maestro] Cursor proxy failed: ${err.message}`);
  }
}
```
...
3. Our maestro plugin continues to load normally (no crash from Cursor integration)
...
- Pool file: `~/.maestro/oauth-pool.json` — cursor accounts already work via `oauth-pool-helper.sh add cursor`
