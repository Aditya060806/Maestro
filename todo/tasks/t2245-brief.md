...
**Session origin:** interactive (maintainer, Aditya Pandey and Harvest)
**GitHub:** GH#19763
**Tier:** standard (NOT auto-dispatch — research phase first)

## What

Research + detection task. During PR #19758 post-merge cleanup, the canonical `~/Git/maestro` repo was found in a stuck 3-way merge state on `.agents/plugins/opencode-maestro/provider-auth.mjs` — 1105-line diff, unmerged stages 1/2/3 in the index, but no active `MERGE_HEAD` / `REBASE_HEAD` / interactive operation. HEAD was correct (`cac55bd61 t2228: ... (#19758)`, matching `origin/main`); only the index held the conflict.
...
2. `maestro-update-check.sh` session-start routine does NOT currently detect or warn about this state.
...
- Check reflog for `HEAD` + `main` + the conflicting file across all maestro worktrees for any partial merge/pull/rebase that bailed out without unstaging.
...
- **EDIT:** `.agents/scripts/maestro-update-check.sh` — add `_detect_stuck_index_conflict()` function.
...
- Advisory goes through `~/.maestro/advisories/` pipeline so user can dismiss it with `maestro security dismiss <id>`.
...
- Phase 2 primary: `.agents/scripts/maestro-update-check.sh`
...
- `shellcheck .agents/scripts/maestro-update-check.sh` clean.
...
- [ ] `maestro-update-check.sh` detects unmerged index state in canonical repos
