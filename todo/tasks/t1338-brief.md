...
- **Parent task:** none
- **Conversation context:** User researched local model runtimes (Ollama, LM Studio, llama.cpp, Jan.ai) and concluded llama.cpp is the best option for maestro integration based on open-source licensing (MIT), security posture (no daemon by default), raw speed (20-70% faster than Ollama), and full HuggingFace GGUF access. Discussion covered macOS Apple Silicon + Linux support, guided setup, model discovery, usage logging, and disk cleanup recommendations.

## What

Add local AI model inference support to maestro via llama.cpp as the primary runtime, with HuggingFace as the model source. This includes:
...
The user/system will experience: `maestro local install` detects hardware, downloads llama.cpp binary, recommends models → `maestro local pull <model>` downloads from HuggingFace → `maestro local serve` starts OpenAI-compatible API on localhost → model-routing can route cheap tasks to local models → periodic cleanup nudges save disk space.
...
- **Framework completeness**: maestro already routes across 5 cloud tiers. Adding local as tier 0 completes the cost spectrum.
...
- `install`: detect platform (macOS ARM/x64, Linux x64/ARM), download correct llama.cpp binary from GitHub releases, verify checksum, install to `~/.maestro/bin/`
...
- `pull <repo/model>`: download from HuggingFace with progress to `~/.maestro/models/`
...
- SQLite table in `~/.maestro/.agent-workspace/memory/local-models.db`:
  ```sql
