...
2. Created a profile for `wpallstars.com-chore-maestro-init` — a linked git worktree, not a canonical repo.
...
`register_repo()` in `maestro.sh` has no worktree detection on its general
path (only `cmd_init` does, and only when `WORKTREE_PATH` is set). The
auto-discovery scan (`find ~/Git -name .maestro.json`) picks up
`.maestro.json` files from inside worktrees (because worktrees inherit working
...
- Flag file `~/.maestro/logs/.migrated-worktree-repos-json-t2250` suppresses
  re-execution.
...
- `.agents/scripts/tests/test-resolve-canonical-repo-path.sh` × 4: shell-level
  tests for the maestro.sh function. Uses temporary git fixtures.
...
- Canonical worktree leak: `wpallstars.com-chore-maestro-init`.
