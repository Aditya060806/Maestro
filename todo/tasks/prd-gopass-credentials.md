...
prd-gopass-credentials,gopass Integration & Credentials Rename,maestro,draft,2d,1d,4h,4h,2026-02-06T20:00Z
-->
...
maestro stores credentials as plaintext `export` statements in `~/.config/maestro/mcp-env.sh` (permissions 600). While file permissions prevent other users from reading the file, the secrets are:
...
2. Build an AI-native wrapper (`maestro secret`) that keeps secret values out of agent context windows via subprocess injection and output redaction
...
As an maestro user, I want my API keys encrypted at rest and invisible to AI agents so that a stolen laptop or context window leak doesn't expose my credentials.
...
- As a user, I want to store secrets via `gopass insert` or `maestro secret set` at my terminal prompt so that the value is never typed into an AI conversation.
...
2. **`maestro secret` wrapper command**: CLI subcommand that provides:
   - `maestro secret set NAME` -- delegates to `gopass insert` (interactive hidden input)
   - `maestro secret list` -- delegates to `gopass ls` (names only, never values)
   - `maestro secret run CMD` -- inject all secrets into subprocess environment, redact output
   - `maestro secret NAME [NAME...] -- CMD` -- inject specific secrets, redact output
   - `maestro secret import-credentials` -- migrate from credentials.sh to gopass vault
3. **Output redaction**: All stdout/stderr from `maestro secret run` must have secret values replaced with `[REDACTED]` before reaching the agent's context.
...
6. **Agent instructions**: AGENTS.md updated with mandatory rule: "When a user needs to store a secret, ALWAYS instruct them to run `maestro secret set NAME` or `gopass insert NAME` at their shell. NEVER accept secret values in conversation context."
...
8. **gopass environment mapping**: Map maestro multi-tenant concept to gopass mounts (default store + per-tenant mounted stores).
9. **MCP server credential injection**: Document `maestro secret run -- npx some-mcp-server` pattern as replacement for `source credentials.sh && npx some-mcp-server`.
...
```text
Agent: "I need a Stripe API key. Please run: maestro secret set STRIPE_KEY"
User runs at terminal: maestro secret set STRIPE_KEY
  Enter value for STRIPE_KEY: ******* (hidden input)
Agent: "Now I can use it: maestro secret STRIPE_KEY -- curl ..."
```

### User Experience

1. `maestro update` installs gopass if not present (with user confirmation)
2. `maestro secret init` runs `gopass setup` (one-time)
3. `maestro secret import-credentials` migrates existing keys
4. Day-to-day: agent uses `maestro secret NAME -- command` transparently
...
```text
                    User Terminal (interactive)
                           |
                    maestro secret set NAME
                           |
                    gopass insert maestro/NAME
                           |
              ~/.local/share/gopass/stores/root/
              (GPG-encrypted files, git-versioned)
                           |
              maestro secret NAME -- command
                           |
              1. gopass show -o maestro/NAME
...
- **git**: Required for gopass versioned storage (already required by maestro)
...
| Backward compat | Zero breakage | `source ~/.config/maestro/mcp-env.sh` still works via symlink |
...
- [ ] Should `maestro secret` be a new script or extend `credential-helper.sh`?
- [ ] Should gopass store path use `maestro/` prefix or flat namespace?
- [ ] Should `maestro update` auto-install gopass or just recommend it?
...
| Date | Author | Changes |
|------|--------|---------|
| 2026-02-06 | maestro | Initial draft from research session |
