...
# t2700: redirect broken routine run: fields to maestro CLI via bin/ wrapper shims

## Pre-flight (auto-populated by briefing workflow)

- [x] Memory recall: `routine run field wrapper bin maestro CLI` → 0 hits — no relevant prior lessons
...
- **Created by:** Aditya060806 (human, via ai-interactive)
- **Parent task:** none
...
- `~/.maestro/agents/bin/maestro-auto-update` exists, invokes `maestro auto-update check` via the shell PATH, and exits 0 on current setups.
- `~/.maestro/agents/bin/maestro-repo-sync` exists, invokes `maestro repo-sync check`.
- `~/.maestro/agents/bin/maestro-skills-sync` exists, invokes `maestro skill generate`.
- Routine entries in `core-routines.sh` resolve to single-token paths so the `-x` check in `pulse-routines.sh` succeeds.
- On the next pulse cycle after deploy, r902/r906/r910 run to completion without "script not found or not executable" errors in `~/.maestro/logs/pulse-wrapper.log`.

## Why

GH#20315 observed r902/r906/r910 printing `[pulse-wrapper] routine rNNN: script not found or not executable: /Users/…/.maestro/agents/bin/maestro-<name> check` on every dispatch. Two compounding bugs:

1. **Wrapper path mismatch.** Entries point to `bin/maestro-<name>` (resolved against `~/.maestro/agents/`), but the wrappers that do exist live at `~/.maestro/bin/` (one directory up). Only `gh_create_issue` and `gh_create_pr` shipped under `.agents/bin/`.
...
- `NEW: .agents/bin/maestro-auto-update` — execs `maestro auto-update check`; model on `.agents/bin/gh_create_issue` (SPDX header + `set -euo pipefail` + minimal PATH discovery)
- `NEW: .agents/bin/maestro-repo-sync` — execs `maestro repo-sync check`; same pattern
- `NEW: .agents/bin/maestro-skills-sync` — execs `maestro skill generate`; same pattern
...
1. **Create the three wrapper scripts** under `.agents/bin/`. Each wrapper:
   ...
   - Prepends `/usr/local/bin` and `${HOME}/.local/bin` to `PATH` so cron/launchd (which start children with minimal PATH — typically `/usr/bin:/bin`) can still find `maestro` (installed by `setup-modules/config.sh::install_maestro_cli` as a symlink to either of those two locations).
   - Hard-fails with a descriptive stderr message if `command -v maestro` still returns nothing after the PATH prepend.
   - `exec maestro <subcmd> <arg> "$@"` so any routine-supplied args are forwarded.
...
3. **Edit `core-routines.sh` describe functions** (r902 at 188, r906 at 392, r910 at 552) so the user-facing Schedule tables match the new entries. Annotate with `(wraps \`maestro <subcmd> <arg>\`)` so operators reading `maestro routine describe r902` see exactly what the wrapper does.
...
# Expected output:
#   bin/maestro-auto-update
#   bin/maestro-repo-sync
#   bin/maestro-skills-sync
...
shellcheck .agents/bin/maestro-auto-update .agents/bin/maestro-repo-sync .agents/bin/maestro-skills-sync .agents/scripts/routines/core-routines.sh

# 5. Post-deploy smoke test (after setup.sh --non-interactive rsyncs .agents/ to ~/.maestro/agents/):
ls -l ~/.maestro/agents/bin/maestro-auto-update ~/.maestro/agents/bin/maestro-repo-sync ~/.maestro/agents/bin/maestro-skills-sync
# All three files present and executable.

# 6. Next pulse cycle post-deploy: no "script not found" errors for r902/r906/r910
tail -50 ~/.maestro/logs/pulse-wrapper.log | grep -E "routine r(902|906|910):"
# Should show "executing script" and "script completed successfully" — NOT "script not found".
```

### Files Scope

- `.agents/bin/maestro-auto-update`
- `.agents/bin/maestro-repo-sync`
- `.agents/bin/maestro-skills-sync`
...
- [ ] Three new files exist at `.agents/bin/maestro-auto-update`, `.agents/bin/maestro-repo-sync`, `.agents/bin/maestro-skills-sync`, all with executable bit set.
  ```yaml
  verify:
    method: bash
    run: "test -x .agents/bin/maestro-auto-update && test -x .agents/bin/maestro-repo-sync && test -x .agents/bin/maestro-skills-sync"
  ```
- [ ] All three wrappers pass `shellcheck` with zero warnings.
  ```yaml
  verify:
    method: bash
    run: "shellcheck .agents/bin/maestro-auto-update .agents/bin/maestro-repo-sync .agents/bin/maestro-skills-sync"
  ```
- [ ] `core-routines.sh` r902, r906, r910 entries have single-token `run:` paths (no trailing ` check`).
  ```yaml
  verify:
    method: bash
    run: "! grep -E 'bin/maestro-(auto-update|repo-sync) check' .agents/scripts/routines/core-routines.sh"
  ```
