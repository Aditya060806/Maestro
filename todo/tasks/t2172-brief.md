...
- **Parent task:** none
- **Conversation context:** User reported that new OpenCode sessions fail to start with "Failed to parse command /Users/Aditya060806/.config/opencode/command/jersey-planning-search.md" on the splash screen. Root cause is a broken symlink in `~/.config/opencode/command/` pointing to a target directory that no longer exists. Urgent patch release requested.
...
1. Cron path: `maestro-update-check.sh` (runs every ~10 min as part of the existing update check).
2. Install path: `setup.sh` (fresh installs + `maestro update --force`).
...
- `EDIT: maestro.sh` — add a fail-open call to `agent-sources-helper.sh cleanup-broken-symlinks` near the end of `cmd_update` (line ~1010). This reaches every user on the auto-update cron.
- `EDIT: .agents/scripts/maestro-update-check.sh` — add a fail-open call at the end of `main()` for session-start self-heal (belt-and-braces alongside the cron path).
...
5. **Wire into cron path** — `maestro-update-check.sh`. Add a single call near the end (fail-open):
...
- [ ] `agent-sources-helper.sh cleanup-broken-symlinks` removes dangling symlinks from `command/`, `agent/`, `skills/`, and `tool/` under `~/.config/opencode/`.
  ...
  ```
- [ ] The cleanup is called from `maestro-update-check.sh` (cron path) and fails open.
  ```yaml
  verify:
    method: codebase
    pattern: "cleanup-broken-symlinks"
    path: ".agents/scripts/maestro-update-check.sh"
  ```
- [ ] The cleanup is called from `cmd_update` in `maestro.sh` (cron/update path).
  ```yaml
  verify:
    method: codebase
    pattern: "cleanup-broken-symlinks"
...
- **Why cleanup instead of tolerance-in-opencode:** OpenCode is the runtime; maestro is the creator of these symlinks. The fix belongs on the creator side — maestro should not leave orphaned symlinks that confuse any parser (opencode today, something else tomorrow).
...
- `.agents/scripts/maestro-update-check.sh` — cron-invoked update check
...
- **External:** patch release tag must be pushed for users to receive the fix via `maestro update`
