...
SQLite metadata index at `~/.maestro/.agent-workspace/email-mailbox/index.db` — stores message-id, date, from, to, subject, flags, classification. Never caches full message bodies (privacy, disk space, staleness).
...
- Credentials via gopass: `maestro secret set email-imap-{account}`
