...
3. **Auto-install on setup** — `setup.sh` (or the `maestro init` flow) calls `install-hooks-helper.sh install` on every `initialized_repos[]` entry with a present `.git/` directory, same way privacy-guard is now installed per t1968. Opt-out via `MAESTRO_MAIN_BRANCH_GUARD=false` env var.
...
- [x] **Complete code blocks for every edit?** Yes — the hook body is already proven working in `~/Git/maestro/.git/hooks/post-checkout` (ready to copy verbatim into `.agents/hooks/`).
...
- `NEW: .agents/hooks/main-branch-guard-post-checkout.sh` — the hook body. Copy verbatim from the proven local install at `~/Git/maestro/.git/hooks/post-checkout` (available for reference during implementation — do not touch it during this task, it is the production local install).
...
- `NEW: .agents/scripts/install-main-branch-guard.sh` (optional, if the one-sibling-per-hook pattern is preferred) — dedicated installer mirroring `install-privacy-guard.sh` structure: `install`, `uninstall`, `status` subcommands, idempotent, opt-out via `MAESTRO_MAIN_BRANCH_GUARD=false`.
...
- [ ] `install-hooks-helper.sh install` installs the main-branch-guard into every `initialized_repos[]` entry with a present `.git/` directory, idempotently, with an opt-out via `MAESTRO_MAIN_BRANCH_GUARD=false`.

  ```yaml
  verify:
    method: codebase
    pattern: "MAESTRO_MAIN_BRANCH_GUARD"
    path: ".agents/scripts"
  ```
...
- **Prior art consulted:**
  - The hook body was proven during the t1988 session with 4 test cases against the real `~/Git/maestro` canonical repo. This task is translating that proven local install into framework plumbing.
...
- `~/Git/maestro/.git/hooks/post-checkout` — the proven local install from the t1988 session; reference implementation
...
- `setup.sh` — where the per-repo install loop runs during `maestro init`
