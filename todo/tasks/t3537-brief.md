...
- [x] Discovery pass: 0 commits / 0 merged PRs / 0 open PRs touch `.agents/maestro/performance.md` or this brief.
- [x] File refs verified: `.agents/maestro/knowledge-plane.md` exists; `.agents/maestro/performance.md` and this brief were absent before this task; parent #22372 is open and references #22538 under `## Children`.
...
Create `.agents/maestro/performance.md` as the initial `_performance/` plane contract for KPI/result records. The document defines metric identity, subject and dimensions, measurement units, timestamps, confidence/source provenance, and baseline comparison semantics.
...
- `NEW: .agents/maestro/performance.md` — initial `_performance/` KPI/result schema contract, modelled on `.agents/maestro/knowledge-plane.md` document style.
- `NEW: todo/tasks/t3537-brief.md` — child implementation brief with files scope and verification.

### Implementation Steps

1. Add `.agents/maestro/performance.md` with a Phase 1-only scope statement.
...
- `.agents/maestro/performance.md`
- `todo/tasks/t3537-brief.md`

### Verification

- `npx --yes markdownlint-cli2 .agents/maestro/performance.md todo/tasks/t3537-brief.md`
- `gh issue view 22372 --repo Aditya060806/Maestro --json state,body --jq '.state + " " + ((.body | contains("#22538")) | tostring)'`
...
- `.agents/maestro/knowledge-plane.md` — reference document style for plane contracts.
- `.agents/maestro/performance.md` — new KPI/result schema contract.
