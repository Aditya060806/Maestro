...
3. State file: `~/.maestro/.agent-workspace/routine-state.json`
   ```json
   {
...
~/.maestro/agents/scripts/routine-schedule-helper.sh is-due "daily(@09:00)" 0 && echo "due" || echo "not due"
~/.maestro/agents/scripts/routine-schedule-helper.sh next-run "weekly(mon@09:00)"
# Verify pulse integration
grep -q 'routine' ~/.maestro/agents/scripts/pulse-wrapper.sh
```
...
- Custom scripts live in `~/.maestro/agents/custom/scripts/` — the `run:` path is relative to `~/.maestro/agents/`
