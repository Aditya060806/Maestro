...
- [x] Memory recall: `"labelless backfill author association"`, `"reconcile_labelless_maestro_issues origin:worker"` → 0 + 1 hits — the one hit (t2112 working solution) confirms the function exists as described and has no prior author-association check.
- [x] Discovery pass: 0 merged PRs / 0 open PRs touch `pulse-issue-reconcile.sh`'s `reconcile_labelless_maestro_issues` in last 48h. Last change was the t2112 introduction (PR #19098, 2026-04-09). Workflow `maintainer-gate.yml` was last touched by t2395 (#19989, 2026-04-19) for the `source:*` assignee-exemption — no overlap with origin:worker protection block at lines 720-793.
...
- **Conversation context:** User reviewed #20180 and asked why the external contributor's issue wasn't NMR-gated. Sub-agent review confirmed `reconcile_labelless_maestro_issues` applies `origin:worker + tier:standard` based on title shape alone with no author-association check; the server-side `origin:worker` protection handler only strips the origin label, leaving `tier:standard` intact and no NMR applied — so the issue remained dispatchable.
...
1. **Primary: `reconcile_labelless_maestro_issues`** (`pulse-issue-reconcile.sh`) — the pulse's cycle-time label backfill for labelless maestro-shaped issues. Currently applies `origin:worker + tier:standard + body_tags` based on title shape alone. Must check `authorAssociation` and branch behaviour by class.
...
- `EDIT: .agents/scripts/pulse-issue-reconcile.sh:1508-1676` — function `reconcile_labelless_maestro_issues`. Extend jq filter to project `authorAssociation`; add `_is_external_association` helper; branch label application and mentorship comment template on association.
...
- `EDIT: .agents/scripts/tests/test-pulse-labelless-reconcile.sh` — add fixture issues #503 (external CONTRIBUTOR, maestro-shaped, labelless) and #504 (external NONE, maestro-shaped, labelless). Extend assertions: #503/#504 MUST get `--add-label needs-maintainer-review` + NO `--add-label origin:worker` + NO `--add-label tier:standard`, and MUST get the external-specific comment sentinel.
...
if [[ "$is_external" == "true" ]]; then
    ...
    comment_sentinel='<!-- maestro:labelless-backfill-external -->'
    ...
    comment_sentinel="$sentinel"  # existing <!-- maestro:labelless-backfill -->
...
```bash
local external_comment_template="<!-- maestro:labelless-backfill-external -->
...
This gate exists because the maestro pulse auto-dispatches workers on issues that carry maintainer-trust labels (\`origin:worker\`, \`tier:*\`). Issues from external contributors need a maintainer in the loop before dispatch to catch injection attempts, scope/trust mismatches, and speculative work the pulse shouldn't burn a worker on.
...
if [[ "$ACTION" == "labeled" ]]; then
    ...
    if [[ "$EXTERNAL_AUTHOR" == "true" ]]; then
        ...
        NOTICE="The \`origin:worker\` and \`tier:*\` labels were removed automatically and \`needs-maintainer-review\` applied. This issue was filed by an external contributor (\`author_association=${ISSUE_AUTHOR_ASSOC}\`) and requires maintainer review before a worker can be dispatched. Maintainers: review the proposal, then either claim a fresh internal task or apply \`sudo maestro approve issue ${ISSUE_NUMBER}\` after review."
    else
...
3. Add assertions:
   ...
   - `#503` comment MUST contain `maestro:labelless-backfill-external` marker
   - Same three assertions for `#504`
   - `#500` (existing MEMBER fixture, maestro-shaped, labelless) still gets origin:worker + tier:standard (regression on unchanged path).
...
gh api "repos/Aditya060806/Maestro/issues/20180" --jq '.author_association'
...
- [ ] `reconcile_labelless_maestro_issues` fetches `authorAssociation` via `gh issue list --json` and branches its label-application on it.
...
- [ ] Two mentorship comment sentinels exist and are idempotent: `<!-- maestro:labelless-backfill -->` (internal) and `<!-- maestro:labelless-backfill-external -->` (external).

  ```yaml
  verify:
    method: codebase
    pattern: "maestro:labelless-backfill-external"
    path: ".agents/scripts/pulse-issue-reconcile.sh"
  ```
