...
- **Created by:** Aditya060806 (via t2038 research decision)
...
2. **Token name:** `maestro-sync-on-pr-merge`.
3. **Expiration:** 366 days (maximum allowed for fine-grained PATs). Calendar reminder at day 330 to rotate.
4. **Resource owner:** `Aditya060806` (the repo owner).
5. **Repository access:** Only select repositories → `Aditya060806/Maestro`.
6. **Repository permissions:**
   - `Contents`: Read and write
   - (All others: No access, including `Workflows` — explicitly NOT `Workflows: Write`.)
7. Generate the token and copy it.
8. Navigate to <https://github.com/Aditya060806/Maestro/settings/secrets/actions/new>.
9. **Name:** `SYNC_PAT`
...
```bash
gh api "repos/Aditya060806/Maestro/actions/secrets" --jq '.secrets[] | select(.name == "SYNC_PAT") | .name'
# Expected: SYNC_PAT
```
