# t2390: fix(stats): export MAESTRO_HEADLESS in stats-wrapper.sh main() to unblock quality-debt dispatch

## Session origin

Interactive, Aditya060806, 2026-04-19. Picked up #19913 after maintainer crypto-approval.

## What

Add `export MAESTRO_HEADLESS=true` at the top of `stats-wrapper.sh main()`, before the `--self-check` dispatch. Add a sibling regression test at `.agents/scripts/tests/test-stats-wrapper-headless-export.sh` modelled on the existing `test-pulse-wrapper-headless-export.sh`.

## Why

Direct regression of GH#18670. PR #18676 added the headless export to `pulse-wrapper.sh main()` but did not extend it to `stats-wrapper.sh`, which is a second `gh_create_issue` entry point driven by a separate scheduler (15-min `maestro-stats-wrapper.timer` / launchd plist).
...
2. `detect_session_origin()` returns `"worker"` when `MAESTRO_HEADLESS=true`
...
1. `stats-wrapper.sh main()` exports `MAESTRO_HEADLESS=true` before any other statement.
...
- **Issue**: #19913 (crypto-approved by Aditya060806 2026-04-19T16:13:14Z)
