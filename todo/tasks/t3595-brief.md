...
- [x] File refs verified: 3 refs checked at HEAD: `.github/workflows/loc-badge-reusable.yml:111-118`, `.agents/maestro/badges.md:124-125`, `.agents/scripts/loc-badge-helper.sh:40-42`.
...
- `EDIT: .agents/maestro/badges.md:124-125` — update local-development dependency guidance so Ubuntu does not imply apt `tokei` is the supported CI path.
...
- `.agents/maestro/badges.md:124-125` currently documents `brew install tokei jq` and `# apt install tokei jq` for local development.
...
3. Update docs/helper dependency guidance.
   - `.agents/maestro/badges.md` should show macOS and Linux local-development commands without claiming the workflow-supported Linux path is `apt install tokei` unless that remains intentionally true only for local use.
...
```bash
rg 'apt(-get)? install .*tokei|apt: tokei|curl .*rustup|rustup' .github/workflows/loc-badge-reusable.yml .agents/maestro/badges.md .agents/scripts/loc-badge-helper.sh .agents/scripts/tests
...
- [ ] `.agents/maestro/badges.md` and `.agents/scripts/loc-badge-helper.sh` dependency guidance matches the supported install strategy.
...
- `.agents/maestro/badges.md:124-125` — current local dependency docs.
