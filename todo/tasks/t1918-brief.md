...
- **Conversation context:** User filed GH#17754 reporting that `sudo maestro approve issue` silently fails on Linux when gnome-keyring is inaccessible under sudo. Review confirmed two bugs: silent `gh` auth failure and unchecked exit code.

## What

Fix `approval-helper.sh` so that `sudo maestro approve` fails loudly when `gh` cannot authenticate, instead of printing a false `[OK]` message. Also document the `[owner/repo]` positional argument syntax in AGENTS.md.
...
- `EDIT: .agents/AGENTS.md:168` — document `[owner/repo]` positional argument for `sudo maestro approve`
...
_require_gh_auth() {
    if ! gh auth status >/dev/null 2>&1; then
        ...
        _print_info "Workaround: export GH_TOKEN before sudo, or run: sudo GH_TOKEN=\$(gh auth token) maestro approve ..."
        return 1
    fi
    return 0
}
...
```markdown
`sudo maestro approve issue <number> [owner/repo]`
```
