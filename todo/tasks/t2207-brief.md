...
2. **Update `install-hooks-helper.sh`:**
   - Add `install_pre_push_quality_hook()` function. It chains into the existing `.git/hooks/pre-push` (which already runs `gh-wrapper-guard` and `privacy-guard`). Use the same marker-based detection pattern (`# maestro-pre-push-quality-hook`).
...
4. **Timing verification** — commit a single file in the maestro repo with both hooks installed, time it:
...
# Timing check on the maestro repo itself:
cd ~/Git/maestro
...
- [ ] Timing: on the maestro repo, `git commit --allow-empty` completes in under 5s with both hooks installed.
