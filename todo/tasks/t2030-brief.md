...
- **Created by:** Aditya060806 (ai-interactive, gap-closing per build.txt policy clarification)
- **Parent task:** none
...
> **`origin:interactive` implies maintainer approval**: PRs tagged `origin:interactive` pass the maintainer gate automatically when the PR author is `OWNER` or `MEMBER` — the maintainer was present and directing the work. No separate `sudo maestro approve` is needed. Contributors (`COLLABORATOR`) with `origin:interactive` still go through the normal gate — the label alone is not sufficient.
...
4. User ran `sudo maestro approve issue 18429` (~30s of context switch + sudo password prompt).
