...
Establishes a standardised framework primitive — `_knowledge/`, `_cases/` and supporting infrastructure — applicable to any maestro-managed repo. The framework primitive serves multiple matter types (code projects, business cases, limited companies, asset portfolios, customer relationships, investigations, research projects) via the same engine, parallel to existing `.agents/`, `TODO.md`, `todo/`.
...
6. AI comms agent: `maestro case draft` (human-gated, RAG over case sources, provenance footer) + `maestro case chase` (template-only auto-send for routine follow-ups)
...
Without this primitive: every repo that needs document-shaped data invents its own ingestion + index + retrieval pattern. With it: standardised, audit-trailed, sensitivity-aware, retrievable across all maestro-managed repos via one CLI surface.
...
- **Phase P4** — cases plane (3 children: case dossier contract + `maestro case open`, case CLI surface, milestone + deadline alarming)
...
- **Phase P6** — AI comms agent (2 children: `maestro case draft` human-gated, `maestro case chase` template-only auto-send)
...
Originals ≥30MB go to `~/.maestro/.agent-workspace/knowledge-blobs/<repo>/<source-id>/` with hash+pointer in `meta.json`. Below 30MB, stored in `_knowledge/sources/<id>/`.

Personal/cross-repo plane: `~/.maestro/.agent-workspace/knowledge/` (out-of-repo for content that doesn't belong to any single repo or that's still being staged).
...
`setup.sh` provisions skeletons + `.gitignore` based on values. `maestro <plane> init` flips a repo from off to repo.
...
| `untrusted` | Anyone else | NMR-gated; `sudo maestro approve issue <N>` to promote |
...
| P4 | t2851 / GH#20904 | case dossier contract + `maestro case open` |
...
| P6 | t2857 / GH#20911 | `maestro case draft` agent (RAG, human-gated, provenance) |
| P6 | t2858 / GH#20912 | `maestro case chase` (template-only auto-send, opt-in per case) |
...
- [ ] MVP exit criteria (when all 16 children merged):
  - [ ] `maestro knowledge add <path>` works for a PDF, a URL, an `.eml`
  - [ ] `maestro case open <slug>` creates a case dossier
  - [ ] `maestro case draft <case>` produces a draft with provenance footer (human-gated)
  - [ ] `maestro case chase <case> --template <name>` sends a template-only chaser (opt-in)
  ...
  - [ ] Standard skeleton provisions in any new maestro-init repo
...
5. **example-repo t357 framework-first** — build the framework primitive in maestro, then example-repo t357 becomes a thin SaaS layer over it (post-MVP).
6. **Maintainer drops auto-promote** — NMR only for untrusted sources. Crypto-approved (`sudo maestro approve issue <N>`) for explicit gate.
...
11. **Strategic vs routine comms split** — `maestro case draft` always human-gated; `maestro case chase` opt-in per case for template-only auto-send (no LLM at send time, just template substitution with verified data fields).
12. **Cross-case privilege firewall** — `maestro case draft <case-id>` only sees that case's `sources.toon`. Cross-case search needs explicit `--include-case <other-id>` and is logged.
