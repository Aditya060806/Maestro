...
2. **Characterisation test: capture current agent-discovery output.** Before any changes:
   ```bash
   python3 .agents/scripts/agent-discovery.py ~/.maestro/agents claude > /tmp/agent-discovery-before.json
   python3 .agents/scripts/opencode-agent-discovery.py ~/.maestro/agents opencode > /tmp/opencode-agent-discovery-before.json
   ```
...
python3 .agents/scripts/agent-discovery.py ~/.maestro/agents claude > /tmp/agent-discovery-after.json 2>/dev/null
