...
1. **Scanner-body enhancement**
   ...
   - Current body template has `## What`, `## Why`, `## How` sections. Enhance `## How` with:
     ...
     **Generator marker** (for pre-dispatch validator, t2367):
     <!-- maestro:generator=large-file-simplification-gate cited_file={cited_file} threshold={threshold} -->
     ```
...
- Side benefit: the `maestro:generator=` marker enables t2367's validator path with zero extra work.
...
- [ ] Bodies emit the `<!-- maestro:generator=... -->` marker (completes t2367's upstream half)
