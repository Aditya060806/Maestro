...
1. Replace SQLite with an append-only cost log: `~/.maestro/logs/cost.log` (timestamp, model, tokens, cost).
...
1. Replace SQLite with structured log: `~/.maestro/logs/observability.log` (JSON lines).
...
- [ ] No SQLite databases remain in `.agent-workspace/` for budget, observability, or issue-sync
  ...
  verify:
    method: bash
    run: "fd -e db ~/.maestro/.agent-workspace/ 2>/dev/null | grep -iE 'budget|observ|issue.sync' | wc -l | grep -q '^0$'"
  ```
