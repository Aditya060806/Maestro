...
and `maestro campaign init/provision/status/ls` commands for directory introspection.
...
3. `.agents/maestro/campaigns-plane.md` — directory contract documentation (peer to `cases-plane.md`)
4. `.agents/scripts/campaigns-provision-helper.sh` — provisioning helper with `init`, `provision`, `status`, `ls` commands
5. `maestro.sh` — dispatch P1 commands (`init/provision/status/ls/list`) to new helper; P6 commands stay in `campaign-helper.sh`
...
- NEW: `.agents/maestro/campaigns-plane.md`
- NEW: `.agents/scripts/campaigns-provision-helper.sh`
- EDIT: `maestro.sh` (campaign dispatch + help text)

## Acceptance Criteria

- [x] `maestro campaign init` provisions `_campaigns/` with `lib/brand/`, `lib/swipe/`, `intel/`, `active/`, `launched/`, `_config/`
...
- [x] `maestro campaign provision` is idempotent (safe to re-run on provisioned plane)
- [x] `maestro campaign status` shows provisioning state + active/launched/asset counts
- [x] `maestro campaign ls` lists active + launched campaigns
- [x] `maestro campaign ls --active` shows only active campaigns
