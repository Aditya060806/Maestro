...
# t1504: Inbound email command interface — permitted senders trigger maestro tasks
...
- **Conversation context:** Email could be used by permitted senders to prompt the maestro system to create todos, plans, or answer questions. Critical security: only permitted senders, mandatory prompt injection scanning.
...
2. Parse email body as task/question for maestro
...
Email is a universal interface. Allowing permitted users to create tasks or ask questions via email extends maestro accessibility beyond the terminal. But it's also an attack vector — security must be the primary concern.
