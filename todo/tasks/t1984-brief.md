...
1. Verify that `$MAESTRO_SESSION_ORIGIN` or equivalent is set in the workflow step's env so the helper's origin detection produces `origin:interactive`. If not set, add:

    ```yaml
    - name: Sync TODO → Issues
      env:
        MAESTRO_SESSION_ORIGIN: interactive  # or derive from commit message / TODO tags
      run: |
...
sleep 30
gh issue list --repo Aditya060806/Maestro --search "t9999 in:title" --json number,assignees
