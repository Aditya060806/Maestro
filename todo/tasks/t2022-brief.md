...
- **Created by:** Aditya060806 (ai-interactive) — observed while running `setup.sh --non-interactive` during the GH#18439 post-merge deploy
...
/Users/Aditya060806/Git/maestro/.agents/scripts/setup/../init-routines-helper.sh: line 22: GREEN: readonly variable
```
...
cat > /tmp/repro.sh <<'SH'
#!/usr/bin/env bash
source ~/Git/maestro/.agents/scripts/shared-constants.sh
echo "BEFORE source"
source ~/Git/maestro/.agents/scripts/init-routines-helper.sh
...
```
BEFORE source
/Users/Aditya060806/Git/maestro/.agents/scripts/init-routines-helper.sh: line 22: GREEN: readonly variable
exit=1
```
...
- **Security control gap**: privacy-guard pre-push hook never gets refreshed by `maestro update`. On a fresh machine, it never gets installed at all. A user who edits `TODO.md` with a private repo slug will push it to a public repo because the client-side hook isn't there.
...
The collision surfaces only when the helper is SOURCED into a shell that already sourced `shared-constants.sh`. Direct execution (`./init-routines-helper.sh` or `maestro init-routines`) works because the child bash process starts fresh and `shared-constants.sh` is sourced inside the helper's own scope without preexisting readonly bindings. **This is why the bug went undetected: the helper's own tests all execute it as a child process, not as a source.**
...
Guard each color assignment with a "set it only if unset" pattern. Do not remove the definitions entirely — the helper can also be executed directly (as `maestro init-routines` does), in which case `shared-constants.sh` has not been sourced and the colors must be defined.
...
nohup bash ~/Git/maestro/setup.sh --non-interactive >/tmp/t2022-setup.log 2>&1 &
SETUP_PID=$!
wait "$SETUP_PID"
echo "setup.sh exit=$?"
...
- [ ] `shellcheck .agents/scripts/init-routines-helper.sh` exits 0.
  ...
  verify:
    method: bash
    run: "shellcheck ~/Git/maestro/.agents/scripts/init-routines-helper.sh"
  ```
- [ ] Sourcing `init-routines-helper.sh` after `shared-constants.sh` does not emit a `readonly variable` error and the sourcing shell continues executing subsequent commands.
  ...
  verify:
    method: bash
    run: "bash -c 'source ~/Git/maestro/.agents/scripts/shared-constants.sh; source ~/Git/maestro/.agents/scripts/init-routines-helper.sh; echo OK' | grep -q '^OK$'"
  ```
- [ ] A full `setup.sh --non-interactive` run completes without printing `GREEN: readonly variable` anywhere in its output.
  ```yaml
  verify:
    method: bash
    run: "bash ~/Git/maestro/setup.sh --non-interactive 2>&1 | tee /tmp/t2022-verify.log; ! grep -q 'GREEN: readonly variable' /tmp/t2022-verify.log"
  ```
...
- **Blocked by:** none
- **Blocks:** nothing directly, but unblocks `setup_privacy_guard` and `setup_canonical_guard` refresh paths across all maestro installs. Worth flagging in the PR body so users know to re-run `setup.sh` after the release lands.
- **External:** none
