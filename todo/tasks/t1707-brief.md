...
- **Parent task:** t1706
- **Conversation context:** The user wants the new `chromium-debug-use` capability to be maestro-owned, low-dependency, and focused on investigating the live browser state the user already has open.
...
PR #14956 established the guide and basic routing, but it did not add a loadable skill entry point or an maestro-owned helper surface for repeated use. Without those pieces, the concept remains partly documentary rather than a routable framework capability.
...
- The goal is an maestro-owned implementation surface, not a thin pointer to the upstream repo.
