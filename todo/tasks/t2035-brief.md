...
# 3. Manual smoke test (in any worktree of an maestro repo):
source .agents/scripts/shared-constants.sh
URL=$(gh_create_issue --repo Aditya060806/Maestro \
    --title "smoke-test t2035 (interactive auto-assign)" \
    --label "test" --body "ignore" 2>&1 | tail -1)
NUM=$(echo "$URL" | grep -oE '[0-9]+$')
gh issue view "$NUM" --repo Aditya060806/Maestro --json assignees --jq '.assignees[].login'
# Expected: Aditya060806 (or the current authenticated gh user)
gh issue close "$NUM" --repo Aditya060806/Maestro --reason "not planned"

# 4. Worker-context smoke test (verify workers do NOT self-assign):
MAESTRO_HEADLESS=1 bash -c '
    source .agents/scripts/shared-constants.sh
    URL=$(gh_create_issue --repo Aditya060806/Maestro \
        --title "smoke-test t2035 (worker no-assign)" \
        --label "test" --body "ignore" 2>&1 | tail -1)
    NUM=$(echo "$URL" | grep -oE "[0-9]+$")
    ASSIGNEES=$(gh issue view "$NUM" --repo Aditya060806/Maestro --json assignees --jq ".assignees | length")
...
- [ ] `gh_create_issue` calls `_gh_auto_assign_from_url` after a successful create when the session origin is `interactive`
  ...
  ```
- [ ] Worker-origin sessions (`MAESTRO_HEADLESS=1` or detected via `session_origin_label`) do NOT trigger the self-assign path
  ```yaml
  verify:
    method: codebase
