...
2. Deploy (setup.sh --non-interactive) and trigger a pulse cycle (`launchctl kickstart -k gui/$(id -u)/com.maestro.maestro-supervisor-pulse`).
3. Wait for the fill-floor stage, then grep `~/.maestro/logs/pulse.log` for `dwd-fence.*#<any-blocked-issue>` — the LAST fence line before the silent abort names the function containing the bug.
