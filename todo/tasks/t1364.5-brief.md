...
- **Resolution chain**: repos.json `bundle` → .maestro.json `bundle` → auto-detected → `cli-tool` fallback
...
- **Caching**: store detection result in .maestro.json for performance (re-detect on `--force`)
...
- `~/.config/maestro/repos.json` — explicit bundle configuration
- `.maestro.json` — per-repo configuration
