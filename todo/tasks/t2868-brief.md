...
Implements `maestro inbox triage` — the routine that processes captured items in `_inbox/`, classifies them, and routes to the appropriate plane. Runs via pulse (configurable interval, default hourly) or on-demand.
...
- `maestro inbox triage` processes all `pending` items in `_inbox/`.
...
maestro inbox add /tmp/privileged.eml
maestro inbox triage --dry-run --limit 1
...
maestro inbox add /tmp/ambiguous.txt
maestro inbox triage --limit 1
