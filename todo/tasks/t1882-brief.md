...
2. **Interactive users**: `maestro signing setup` should guide users through ssh-agent configuration so signing "just works" after initial setup.
...
- **`maestro-update-check.sh`**: If signing is enabled but `can-sign` fails, add to the startup check output: "Signing enabled but key not loaded in ssh-agent. Run: ssh-add"
...
- **launchd agent**: Optionally create `sh.maestro.ssh-agent.plist` that ensures ssh-agent is running and key is loaded on login.
...
- `.agents/scripts/maestro-update-check.sh` — startup warning

## Acceptance Criteria

1. `maestro signing can-sign` returns 0 when key is loaded in ssh-agent, 1 when not
2. Headless workers commit without signing when key is not available — no hangs, no errors
3. `maestro signing setup` on macOS offers Keychain integration
4. `maestro signing setup` on Linux offers ssh-agent + .bashrc integration
