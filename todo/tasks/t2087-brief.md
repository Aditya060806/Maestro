...
This task ships a four-part fix: make modern bash (4+) automatically available on macOS via Homebrew, have scripts self-heal at runtime by re-execing under modern bash when 3.2 is detected, and integrate the install/update into the existing `setup.sh` and `maestro-update-check.sh` rails.
...
- Homebrew is already installed on virtually every macOS dev machine that uses maestro. The framework already uses `brew install` as its package install mechanism for macOS (see `platform-detect.sh:134`). Adding bash to the install list is a natural extension, not a new dependency class.
...
- If `check` returns 1 on macOS:
  ...
  - Non-interactive mode (`--non-interactive`): if `MAESTRO_AUTO_UPGRADE_BASH=1` environment variable is set, auto-install. Otherwise emit advisory and continue.
...
- Advisory file: `~/.maestro/advisories/bash-3.2-upgrade.advisory` — written on every failure path so the reminder survives across sessions and appears in session greetings until dismissed.

### Part 3 — Hook into `maestro-update-check.sh`

- EDIT: `.agents/scripts/maestro-update-check.sh` — add a cheap bash check after the main update check (~end of the script).
...
- Rate-limit: only check once per 24h. State file: `~/.maestro/state/bash-upgrade-last-check` (epoch seconds).
- NEVER auto-upgrade during an update cycle — advisory-only. Upgrading bash mid-operation could disrupt running workers. Advisory + manual upgrade is the same pattern used for `maestro update` itself.
...
- Logic:
  ...
  if [[ ${BASH_VERSINFO[0]} -lt 4 && -z "${MAESTRO_BASH_REEXECED:-}" && -n "${BASH_SOURCE[1]:-}" ]]; then
      for _maestro_bash_candidate in /opt/homebrew/bin/bash /usr/local/bin/bash /home/linuxbrew/.linuxbrew/bin/bash; do
          if [[ -x "$_maestro_bash_candidate" ]]; then
              export MAESTRO_BASH_REEXECED=1
              exec "$_maestro_bash_candidate" "${BASH_SOURCE[1]}" "$@"
          fi
      done
      unset _maestro_bash_candidate
  fi
  ```

- Critical subtleties:
  - `MAESTRO_BASH_REEXECED=1` guard prevents infinite re-exec loops if a symlink points wrong
  ...
  - `setup.sh` MUST NOT source shared-constants.sh at the top (or must set `MAESTRO_BASH_REEXECED=1` first) because setup is the script that installs modern bash — a chicken-and-egg
  - `bash-upgrade-helper.sh` itself MUST NOT source shared-constants.sh — same chicken-and-egg
- Add a regression test: `.agents/scripts/tests/test-bash-reexec-guard.sh` that verifies (a) running a fake script under `/bin/bash` re-execs to modern bash when available, (b) `MAESTRO_BASH_REEXECED=1` prevents re-exec, (c) re-exec is skipped on bash ≥ 4.
...
- [ ] `maestro-update-check.sh` emits an advisory when modern bash is outdated, rate-limited to once per 24h
...
- [ ] Live verification: run `/bin/bash ~/.maestro/agents/scripts/pulse-wrapper.sh --help` (or any shared-constants-sourcing script) and observe that it re-execs to modern bash
...
- **Existing infrastructure**:
  - `platform-detect.sh:134` — exports `MAESTRO_PKG_INSTALL="brew install"` on macOS
  - `setup.sh` — platform detection + deploy entrypoint
  - `maestro-update-check.sh` — periodic update advisory emitter
  - `security-posture-helper.sh` — pattern for interactive prerequisite setup
  - `~/.maestro/advisories/*.advisory` — advisory delivery system
...
- **Session origin**: interactive, Aditya060806 approved the four-part plan in this conversation.
...
shellcheck .agents/scripts/bash-upgrade-helper.sh \
           ...
           .agents/scripts/maestro-update-check.sh \
           setup.sh \
...
# Live demonstration
/bin/bash ~/.maestro/agents/scripts/memory-helper.sh recall --query test --limit 1
