...
- **Parent task:** none
- **Conversation context:** After GEO/SRO SEO agent rollout and release, runtime deployment in `~/.maestro/agents/` did not include newly merged files until a manual `rsync` was run. User requested a durable framework fix.

## What

Implement an automatic post-merge/release sync mechanism so the runtime agents directory (`~/.maestro/agents/`) always reflects repo `.agents/` changes immediately after normal merge/release flows.
...
1. Add deterministic drift detection between repo `.agents/` and `~/.maestro/agents/` to make failures observable.
...
- [ ] Post-merge/release lifecycle syncs `~/.maestro/agents/` without manual intervention.
...
- **Blocked by:** none
- **Blocks:** none
- **External:** GitHub issue [GH#4205](https://github.com/Aditya060806/Maestro/issues/4205)
