...
Interactive follow-up to t2724/t2728/t2730/t2731. User asked: stop the model from running `maestro-update-check.sh` and dumping the output into chat at session start. The plugin already renders the full framework status as a TUI toast — the chat greeting should be a one-liner.
...
We're running maestro v{X} in OpenCode v{Y}.
...
Current live `~/.config/opencode/AGENTS.md:1-12` instructs the model to `Run bash ~/.maestro/agents/scripts/maestro-update-check.sh --interactive`, then implicitly dumps the 8+ line output (security advisories, contribution counts, update hints, etc.) into the chat. This is redundant with the toast the `opencode-maestro` plugin renders at `session-start` and wastes user attention plus tokens on every session start.
...
- The cache file `~/.maestro/cache/session-greeting.txt` line 1 is always `maestro v{X} running in OpenCode v{Y} | {repo}/{branch}` — populated by the opencode-maestro plugin at session start. Line 1 is the cheapest source for both versions in a single `Read` call.
...
- Fallback: `~/.maestro/agents/VERSION` holds just the maestro version if the cache is missing.
...
The opencode-maestro plugin renders session-start framework status (versions,
security posture, advisories, update hints) as a TUI toast before your first
turn. Raw output is cached at `~/.maestro/cache/session-greeting.txt`. The
user has already seen it — do NOT re-run `maestro-update-check.sh` and do
NOT repeat toast content in the chat.

**On interactive conversation start** (skip for headless sessions like `/pulse`, `/full-loop`):

1. Read line 1 of `~/.maestro/cache/session-greeting.txt`. Format: `maestro v{X} running in OpenCode v{Y} | ...`. Extract `{X}` and `{Y}`.
...
3. If the cache file is missing, read `~/.maestro/agents/VERSION` for `{X}` and greet: "Hi!\n\nWe're running maestro v{X}.\n\nWhat would you like to work on?"
4. Then respond to the user's actual message.

If the user asks about maestro updates, direct them to run `maestro update` in a terminal session (or `!maestro update` in chat). Do not announce updates unprompted — the toast already did.
...
2. Run `~/.maestro/agents/scripts/generate-runtime-config.sh agents --runtime opencode` to regenerate.
...
- [ ] Regenerated `~/.config/opencode/AGENTS.md` contains: (a) runtime-identity line, (b) explicit "do NOT re-run maestro-update-check.sh" directive, (c) concise greeting template with `{X}` and `{Y}` placeholders sourced from cache file line 1
- [ ] Regenerated file does NOT contain the string `Run bash ~/.maestro/agents/scripts/maestro-update-check.sh`
