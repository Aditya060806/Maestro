...
**Scope**: this affects every maestro user who runs opencode without a paid static Anthropic API key — the default recommended configuration. Reported by the repo maintainer in an interactive session; likely hitting other users silently.
...
- Evidence: `~/.maestro/logs/pulse.log` (2026-04-19 ~18:30Z) shows `No available model for tier: opus` followed by `dispatch_triage_reviews: model resolution failed (opus and sonnet unavailable)` despite 25 DFF candidates enumerated
