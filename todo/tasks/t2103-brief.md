...
Generator identification uses a hidden body marker `<!-- maestro:generator=ratchet-down -->` that the issue generator in `_complexity_scan_ratchet_check` must emit. Generator-marker extraction is safer than parsing titles or labels.
...
- Generator marker `<!-- maestro:generator=ratchet-down -->` emitted by `_complexity_scan_ratchet_check`
- Exit 10 triggers issue closure with rationale + signature footer
- Emergency bypass env var `MAESTRO_SKIP_PREDISPATCH_VALIDATOR=1`
