...
1. `_config/mailboxes.json` (per-repo) and `~/.maestro/configs/mailboxes.json` (personal plane) — registry of mailboxes to poll
...
4. Credentials via gopass: `gopass maestro/email/<mailbox-id>/password` (never inline in `mailboxes.json`)
5. Routine `r044` in `TODO.md`, repeat: `cron(*/10 * * * *)` (every 10 min), run: `scripts/email-poll-helper.sh tick`
6. CLI: `maestro email mailbox add|remove|list|test`
     ...
     "mailboxes": [
       {
         ...
         "password_ref": "maestro/email/personal-icloud/password",
...
4. **CLI surface** — `maestro email mailbox`:
...
- EDIT: `.agents/cli/maestro` (add `email mailbox` subcommand group)
- EDIT: `TODO.md` (add `r044` routine entry — done in this task's PR)
- NEW: `.agents/tests/test-email-poll.sh`
- EDIT: `.agents/maestro/knowledge-plane.md` (IMAP polling section)

## Acceptance Criteria

- [ ] `maestro email mailbox add` interactive flow registers a new mailbox with credentials from gopass
...
- [ ] `maestro email mailbox test <id>` performs a dry-run fetch (1 message), does not commit state
...
- [ ] `maestro email mailbox list` shows last-polled-at + last-error per mailbox
...
- [ ] Documentation: IMAP setup guide in `.agents/maestro/knowledge-plane.md`
