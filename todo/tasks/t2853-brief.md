...
4. Alarm channels:
   ...
   - `ntfy` — push notification (uses existing `maestro` ntfy infrastructure if present)
...
6. Manual: `maestro case alarm-test <case-id>` — re-fires alarms once for a case (testing/debug)
...
- EDIT: `.agents/cli/maestro` (add `case alarm-test` subcommand)
- EDIT: `TODO.md` (add `r043` routine entry — done in this task's PR)
- NEW: `.agents/tests/test-case-alarm.sh`
- EDIT: `.agents/maestro/cases-plane.md` (alarming section)
...
- [ ] `maestro case alarm-test <case-id>`: forces re-fire for testing without bumping stage memory
...
- ntfy integration pattern (if existing): check `~/.maestro/agents/scripts/` for any `ntfy-helper.sh`; otherwise use plain `curl -d 'msg' https://ntfy.sh/<topic>`

<!-- maestro:sig -->
---
[maestro.sh](https://maestro.sh) v3.11.2 plugin for [OpenCode](https://opencode.ai) v1.14.25 with claude-opus-4-7 spent 2h 18m and 138,839 tokens on this with the user in an interactive session.