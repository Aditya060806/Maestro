...
Add `cmd_install_systemd()` function and `install-systemd` case to `routine-helper.sh`, plus a platform-auto-detecting `install` subcommand. On Linux with `MAESTRO_SCHEDULER=systemd`, users get systemd timer installation with `Persistent=true` catch-up, journal logging, and concurrent-run prevention.

## Why

`routine-helper.sh` predates the t1748 Linux platform work. When GH#17447 centralised systemd in `setup-modules/schedulers.sh`, the user-facing `routine-helper.sh` was not updated. On Linux, users currently have no path to install scheduled routines via systemd. The framework already detects and exports `MAESTRO_SCHEDULER=systemd` via `platform-detect.sh`, but `routine-helper.sh` ignores it.
...
cmd_install_systemd() {
	...
	local service_name="sh.maestro.routine-${ROUTINE_NAME}"
	local service_dir="$HOME/.config/systemd/user"
	local service_file="${service_dir}/${service_name}.service"
	local timer_file="${service_dir}/${service_name}.timer"
	local log_file="$HOME/.maestro/logs/routine-${ROUTINE_NAME}.log"

	mkdir -p "$service_dir" "$HOME/.maestro/logs"

	cat >"$service_file" <<EOF
[Unit]
Description=maestro routine ${ROUTINE_NAME}
...
EOF
	...
	cat >"$timer_file" <<EOF
[Unit]
Description=maestro routine ${ROUTINE_NAME} Timer
...
```bash
	...
	install)
		...
		case "${MAESTRO_SCHEDULER:-cron}" in
...
- [ ] `routine-helper.sh install-systemd` writes `.service` and `.timer` files to `~/.config/systemd/user/`
  ...
  ```
- [ ] `routine-helper.sh install` auto-detects scheduler from `MAESTRO_SCHEDULER` and delegates
  ```yaml
  verify:
    method: codebase
    pattern: "MAESTRO_SCHEDULER"
    path: ".agents/scripts/routine-helper.sh"
  ```
...
- [ ] `print_usage()` documents all five subcommands (plan, install, install-launchd, install-cron, install-systemd)
  ...
  ```
- [ ] Naming convention `sh.maestro.routine-{name}` matches launchd convention
  ```yaml
  verify:
    method: codebase
    pattern: "sh\\.maestro\\.routine-"
    path: ".agents/scripts/routine-helper.sh"
  ```
...
- **Auto-detect `install` subcommand**: Sources `platform-detect.sh` to read `MAESTRO_SCHEDULER` and delegates to the right backend. This eliminates the need for users to know their scheduler.
...
- `.agents/scripts/platform-detect.sh:38-40` — where `MAESTRO_SCHEDULER=systemd` is exported
