...
**Issue:** GH#19686 (Aditya060806/Maestro) — issue body is the canonical spec.
...
See issue body at https://github.com/Aditya060806/Maestro/issues/19686 for:
...
- setup.sh symlink integration into `~/.maestro/bin/`
- Shim pattern: source shared-constants.sh, exec function with `"$@"`

## Acceptance criteria

Listed in the issue body. Key gates: `type gh_create_pr` returns PATH hit after `maestro update`; auto-applies `origin:interactive` or `origin:worker` based on `MAESTRO_HEADLESS`; works from any directory.
