...
Implements `maestro inbox digest [--age-days N]` — reports the oldest unsorted/needs-review items in `_inbox/` across all repos plus the workspace-level inbox. Runs as a pulse routine producing a weekly digest (configurable interval) that surfaces in the session greeting / stale-claim scan output.

Two surfaces:

1. **CLI:** `maestro inbox digest` — interactive query, returns top N stale items.
2. **Pulse routine:** runs weekly; if stale items > threshold, posts an advisory in `~/.maestro/advisories/inbox-stale-{repo-slug}.advisory` so the next interactive session greeting includes a nudge.
...
- User running `maestro inbox digest` sees a list with file path, age, sub-folder, and any prior triage attempt notes.
...
Advisory mechanism reuses the existing `~/.maestro/advisories/` flow already used for security and update advisories — consistent UX, no new surface.
...
- `EDIT: .agents/scripts/maestro` — wire `maestro inbox digest`.
- `EDIT: .agents/scripts/maestro-update-check.sh` — surface inbox advisories in session greeting alongside existing advisory types.
...
2. `inbox-digest-routine.sh`:
   ...
   - If count > 0: write `~/.maestro/advisories/inbox-stale-{repo-slug}-{ts}.advisory` with the digest
   - If count == 0: clean up any existing stale advisories for that repo (resolved)
   - Schedule: weekly default (configurable via env `MAESTRO_INBOX_DIGEST_INTERVAL_HOURS`, default 168)

3. Greeting integration:
   - `maestro-update-check.sh` already reads `~/.maestro/advisories/*.advisory` for session greeting
   - Inbox advisories follow the same format (just a different filename prefix)
   - Dismiss mechanism: `maestro security dismiss inbox-stale-{slug}` (reuses existing advisory dismissal)
...
- `.agents/scripts/inbox-digest-routine.sh`
- `.agents/scripts/maestro`
- `.agents/scripts/maestro-update-check.sh`

## Acceptance Criteria

- [ ] `maestro inbox digest` lists stale items sorted by age.
...
- [ ] Dismissal via existing `maestro security dismiss` mechanism works.
- [ ] `shellcheck` clean.
...
- **Why surface count, not contents in greeting:** privacy. Greeting may be visible to others; just shows count + slug. User runs `maestro inbox digest` to see actual filenames.

## Relevant Files

- `.agents/scripts/maestro-update-check.sh` — advisory reader
- `~/.maestro/advisories/` — advisory store
