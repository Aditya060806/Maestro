...
# t1440: Fix `maestro update` failures when Homebrew is unavailable or token refresh returns 500
...
- **Parent task:** none
- **Conversation context:** Users reported urgent `maestro update` failures with errors like `timeout: failed to run command 'brew': No such file or directory` and `Error: Token refresh failed: 500`. The task needs a tracked maestro planning entry and linked GitHub issue so the update path can be fixed quickly.

## What

Investigate and fix the `maestro update` flow so it does not abort when Homebrew is unavailable in the runtime environment and so upstream token refresh failures return clear, actionable guidance instead of raw provider or shell errors. Cover both the command path that runs the framework update and any helper/auth integration involved in refresh-driven update checks.

## Why

- **Restore trust in self-update:** `maestro update` is the recovery path users are told to run when tooling drifts or setup breaks
...
1. Trace the `maestro update` execution path in `maestro.sh` and any called setup/update helpers to identify where `brew` is invoked without a presence check and where token refresh or authenticated fetches can fail
...
- [ ] `maestro update` no longer emits `timeout: failed to run command 'brew': No such file or directory` when Homebrew is absent
  ```yaml
  verify:
    method: bash
    run: "git grep -n 'command -v brew\|brew not found\|Homebrew' -- maestro.sh setup.sh setup-modules .agents/scripts"
  ```
- [ ] Token refresh failures during update are caught and reported with actionable guidance, including 5xx refresh responses
  ```yaml
  verify:
    method: bash
    run: "git grep -n 'refresh.*500\|Token refresh\|re-auth\|retry later' -- maestro.sh .agents/scripts setup.sh"
  ```
- [ ] Regression coverage exists for both the missing-Homebrew and token-refresh failure scenarios
  ...
  ```
- [ ] `maestro update` still completes the normal update/setup path after the hardening changes
  ```yaml
  verify:
    method: bash
...
- If the refresh failure originates in a downstream CLI or helper, wrap it at the maestro boundary with better guidance rather than leaking internal transport detail

## Relevant Files

- `maestro.sh` — primary `cmd_update()` flow and user-facing update messaging
- `setup.sh` — non-interactive setup path run by `maestro update`
- `setup-modules/` — package-manager and setup helpers that may invoke Homebrew or auth-sensitive install steps
- `.agents/scripts/maestro-update-check.sh` — update-related messaging and possible shared upgrade logic
