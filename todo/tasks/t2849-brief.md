...
6. CLI: `maestro knowledge enrich <source-id>` invokes helper; `maestro knowledge enrich --kind <override>` allows manual kind override
...
4. **CLI integration** — extend `maestro knowledge enrich <id>` and add a routine `r041` that runs enrichment on freshly-promoted sources (`status: sources` and no `extracted.json` yet)
...
- EDIT: `.agents/maestro/knowledge-plane.md` (enrichment section + Markdoc draft-namespace section)

## Acceptance Criteria

- [ ] `maestro knowledge enrich <invoice-source-id>` produces `extracted.json` with at least 5 of 6 invoice fields populated
...
- [ ] Documentation: schema authoring guide in `.agents/maestro/knowledge-plane.md` + Markdoc draft-namespace cross-reference to t2874
