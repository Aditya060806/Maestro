...
4. Recommended bundle (existing `recommend` subcommand): document official `maestro/llm-bundle-default` set in `~/.maestro/configs/ollama-bundle.json` — auto-pull on first call to `chat` if missing
...
3. **Recommended bundle config** — `~/.maestro/configs/ollama-bundle.json`:
   ```json
   {
...
- EDIT: `.agents/maestro/knowledge-plane.md` (Ollama integration section + privacy guarantee)
...
- [ ] Documentation: Ollama setup + privacy section in `.agents/maestro/knowledge-plane.md`
