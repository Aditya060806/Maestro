...
- [x] Memory recall: `maestro reach capture auto-dispatch task briefs TODO issue-sync worker-ready` → 0 hits — no relevant prior lesson found.
...
- [x] File refs verified: `maestro.sh`, `/auto-browse`, browser profile/proxy/cookie docs, and inbox/knowledge/performance/feedback docs are present at HEAD.
...
- **Parent task:** none
- **Blocked by:** none
- **Conversation context:** We chose an maestro-native reach/capture layer over cloning Agent-Reach, using existing browser/profile/proxy/inbox/knowledge/performance/feedback primitives.

## What

Create the reach/capture foundation: `.agents/maestro/reach-capture.md`, `.agents/scripts/reach-helper.sh`, `maestro reach ...` dispatch, and focused tests. The first helper commands are `doctor`, `capabilities`, and `route`; capture/profile/proxy/logging behavior is deferred to follow-up tasks.
...
- **Read first:** `maestro.sh:1715-1725` — top-level command dispatch insertion point.
- **Stop when:** `reach-helper.sh doctor|capabilities|route` work locally and no capture/profile/proxy mutation is implemented.

### Files to Modify

- `NEW: .agents/maestro/reach-capture.md` — capability registry and route decision schema.
- `NEW: .agents/scripts/reach-helper.sh` — CLI entry point for doctor/capabilities/route.
- `EDIT: maestro.sh:1715-1725` — dispatch `maestro reach ...`.
- `NEW: .agents/scripts/tests/test-reach-helper.sh` — focused helper tests.

### Implementation Steps

1. Document the capability registry in `.agents/maestro/reach-capture.md`. Include existing capabilities only: fetch/static parse, Crawl4AI/WaterCrawl crawler, deterministic browser, persistent profile, cookie-session reuse, anti-detect profile, proxy/VPN, `_inbox` capture, `_knowledge` staging, `_performance` logging, and `_feedback` mining.
...
5. Wire `reach) _dispatch_helper "reach-helper.sh" "reach-helper.sh" "$@" ;;` into `maestro.sh`.
...
./maestro.sh reach doctor --format json
./maestro.sh reach route --objective "capture public documentation" --scope public --format json
```

### Files Scope

- `.agents/maestro/reach-capture.md`
...
- [ ] `maestro reach doctor --format json` reports capability health without contacting arbitrary targets.
- [ ] `maestro reach capabilities --format json` lists fetch/crawler/browser/profile/cookie/proxy/inbox/knowledge/performance/feedback capabilities.
- [ ] `maestro reach route ... --format json` emits backend, agency level, headed/headless recommendation, profile/cookie/proxy policy, offload recommendation, and blocked reason.
