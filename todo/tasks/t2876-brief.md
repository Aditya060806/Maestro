...
## Session Origin
Interactive session, Aditya060806 2026-04-26. Surfaced after redacting 70+ historical issues/PRs in `Aditya060806/Maestro` containing the private repo basename `<webapp>` (alias for the actual private slug). Eight of those were auto-generated leaks from `pulse-canonical-recovery.sh` (fixed by t2871/PR #20945); the remaining 60+ were prose typed by maintainer/workers describing real bugs. The existing `privacy-guard-pre-push.sh` covers `git push` only — every `gh issue/pr create|comment|edit` call to the public framework repo bypasses it.
...
6. Bypass: `MAESTRO_GH_PRIVACY_BYPASS=1` skips the scan with stderr audit notice.
...
## Acceptance
1. `gh issue create --repo Aditya060806/Maestro --body 'mentions <webapp>'` → exit 1, mentoring error citing the slug
2. `gh issue create --repo Aditya060806/Maestro --body 'mentions <webapp> alias'` → exit 0 (alias allowed)
3. `gh issue create --repo private/repo --body 'mentions <webapp>'` → exit 0 (private target — out of scope)
4. `MAESTRO_GH_PRIVACY_BYPASS=1 gh issue create … 'mentions <webapp>'` → exit 0 with stderr audit notice
...
## Notes
- The `<webapp>` alias is the user's chosen generic placeholder for the actual private repo basename. The basename appears in the user's `~/.maestro/configs/privacy-guard-extra-slugs.txt` (single-line entry) and in `repos.json` with `mirror_upstream: true`, so `privacy_enumerate_private_slugs` already returns it.
...
- The plugin-hook layer (`Claude-maestro/quality-hooks.mjs`) is not modified here — the PATH shim covers all gh calls regardless of agent runtime, including bash scripts and headless workers. A plugin-hook companion is a separate enhancement if richer in-IDE feedback is desired.
