...
Evaluate rtk (Rust Token Killer) — a CLI proxy that intercepts shell command outputs and compresses them before they reach the LLM context — for integration into maestro framework.

## Why

Token consumption is a direct cost driver for AI-assisted development. rtk claims 60-90% reduction on common CLI operations (git, ls, grep, test runners) with <10ms overhead. If validated, this could significantly reduce per-session costs across all maestro-managed projects.
...
### Measured Output Compression (on maestro repo)
...
**Typical 30-min maestro session (estimated):**
...
**Important context:** Most file operations in maestro use MCP tools (Read, Grep, Glob), not bash. The 81% reduction applies only to the bash-command subset of token usage. Overall session token reduction would be lower — estimated **15-25%** of total tokens, depending on the ratio of bash vs MCP tool usage.
...
- [x] Compatibility with maestro workflows assessed
