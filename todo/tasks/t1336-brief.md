...
6. **Remove runtime SQLite DBs:** Document which `.db` files in `~/.maestro/.agent-workspace/` are safe to delete.
...
- [ ] No orphaned SQLite databases remain in .agent-workspace that reference archived scripts
  ...
  verify:
    method: bash
    run: "fd -e db ~/.maestro/.agent-workspace/ 2>/dev/null | grep -iE 'quality|finding|sweep' | wc -l | grep -q '^0$'"
  ```
