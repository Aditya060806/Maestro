...
- **Parent task:** none
- **Conversation context:** User reviewed GH#17677 (external reporter asking for quieter generate-skills.sh output during `maestro update`). Review confirmed the root cause: three generation loops each call `log_success "Generated: $skill_file"` unconditionally, producing hundreds of lines. The fix is to gate those calls behind `DRY_RUN == true`, matching the existing dry-run pattern in the same loops.

## What

`generate-skills.sh` produces per-file `log_success` lines in normal (non-dry-run) mode. With hundreds of SKILL.md files, `maestro update` scrolls through hundreds of "Generated: ..." lines. After this task, normal mode prints only the summary count; `--dry-run` retains per-file output for debugging.

## Why

UX/DX: the verbose output obscures meaningful update output and is noise during routine `maestro update` runs. The summary line already exists (lines 361–363) and provides sufficient feedback. Reported externally in GH#17677; maintainer accepted it.
