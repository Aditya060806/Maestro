...
- [ ] Detected leaks are redacted with `[REDACTED]` in the outbound message
  ...
  verify:
    method: bash
    run: "cd ~/Git/maestro.feature-t1327.9-outbound-leak-detection/.agents/scripts/simplex-bot && bun test 2>&1 | grep -q 'REDACTED'"
  ```
...
- [ ] Tests pass (`bun test`)
  ...
  verify:
    method: bash
    run: "cd ~/Git/maestro.feature-t1327.9-outbound-leak-detection/.agents/scripts/simplex-bot && bun test"
  ```
- [ ] Typecheck passes (`bun x tsc --noEmit`)
  ```yaml
  verify:
    method: bash
    run: "cd ~/Git/maestro.feature-t1327.9-outbound-leak-detection/.agents/scripts/simplex-bot && bun x tsc --noEmit"
  ```
