...
A 1h backoff for config errors reduces this to **~2-3 attempts/hour per issue** until the runner is fixed (manually or via a future maestro update that ships a self-heal). The clear diagnostic message means when the runner-owner comes online and reads the worker log, they immediately know what to fix.
...
4. **Simulate alex's case and confirm structured failure + 1h cache**:
   ...
   ```bash
   OPENCODE_BIN=$(which claude) bash -c 'source .agents/scripts/headless-runtime-lib.sh; _run_canary_test || true; cat ~/.maestro/state/canary-last-fail.reason 2>/dev/null'
   ```
