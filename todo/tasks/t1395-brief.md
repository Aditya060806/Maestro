...
- **Conversation context:** LangWatch treats datasets as first-class objects — collections of test cases with inputs and optional expected outputs. You can build datasets from production traces (annotate failures, add to dataset, re-evaluate). maestro has no dataset concept. The pattern tracker stores outcomes but not reusable test cases. This convention enables repeatable evaluations across t1393 (bench) and t1394 (evaluators).
...
2. Directory convention (`~/.maestro/.agent-workspace/datasets/` for global, `datasets/` in project repos)
...
```text
~/.maestro/.agent-workspace/datasets/    # Global datasets (cross-project)
