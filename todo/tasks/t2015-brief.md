...
- **Created by:** Aditya060806 (ai-interactive, triage of external bug report)
- **Parent task:** none
...
rm -f ~/.maestro/.agent-workspace/supervisor/dep-graph-cache.json
bash .agents/scripts/pulse-dep-graph.sh build
jq '.repos | to_entries[] | {repo: .key, blocked_by_count: (.value.blocked_by | length)}' \
  ~/.maestro/.agent-workspace/supervisor/dep-graph-cache.json
...
- [ ] Rebuilt dep graph cache contains non-trivial `blocked_by` entries for repos that have markdown-formatted blocked-by references in issue bodies.
  ...
  verify:
    method: manual
    prompt: "Delete ~/.maestro/.agent-workspace/supervisor/dep-graph-cache.json, run pulse-dep-graph.sh build, and confirm jq shows non-zero blocked_by counts for repos known to have blocked issues."
  ```
