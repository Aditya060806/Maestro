...
- **Created by:** ai-interactive (Aditya060806 directing)
- **Parent task:** none
- **Conversation context:** While diagnosing t2153 (stale-recovery age-floor guard), discovered that the fix was merged to `main` at 3bbe31f36 but the production pulse kept stale-recovering fresh issues for 90+ minutes. Root cause: `maestro update` deploys scripts only when the local `VERSION` differs from the released `VERSION` on `main` — local commits merged AFTER the latest release (v3.8.63) stay invisible to the runtime until the next release/version bump. Workaround was a manual `cp` + pulse restart at 01:32 BST. This is a systemic gap that affects every script-only fix between releases.

## What

`maestro-update-check.sh` (or a new sibling) MUST detect when the canonical repo at `~/Git/maestro/.agents/scripts/` contains commits that have not been deployed to `~/.maestro/agents/scripts/`, and trigger a redeploy automatically — independent of whether `VERSION` has been bumped.

Detection signal: the git SHA of the canonical repo's `HEAD` (`git -C ~/Git/maestro rev-parse HEAD`) MUST match a recorded "deployed SHA" stamp file at `~/.maestro/.deployed-sha`. On drift (canonical SHA ahead of deployed SHA AND any of `.agents/scripts/**`, `.agents/agents/**`, `.agents/workflows/**`, `.agents/prompts/**`, `.agents/hooks/**` differs since the deployed SHA), trigger `setup.sh --non-interactive --silent` and update the stamp.

Drift check should run on the same ~10min cadence as `maestro-update-check.sh` (already wired via launchd). On drift detected, it MUST NOT prompt the user — silent redeploy + pulse restart, identical to how a normal `maestro update` flow handles a remote VERSION bump.
...
- [ ] **2 or fewer files to modify?** (likely 2: `maestro-update-check.sh` + `setup.sh` plus a regression test)
- [ ] **Every target file under 500 lines?** `maestro-update-check.sh` is ~200 lines, `setup.sh` is ~1500
...
- **EDIT:** `.agents/scripts/maestro-update-check.sh` — add `_check_script_drift()` called from the top-level update loop after the existing VERSION check. If drift detected and no remote VERSION bump pending, trigger silent redeploy.
- **EDIT:** `setup.sh` — at the end of the deploy section (after scripts/agents/workflows/prompts copied), write the canonical-repo HEAD SHA to `~/.maestro/.deployed-sha`. Pattern: append after the existing "Deployment complete" message (search the file for the canonical end-of-deploy marker).
- **NEW:** `.agents/scripts/tests/test-script-drift-detection.sh` — regression test using a sandbox repo + stub `~/.maestro/.deployed-sha` to verify drift triggers redeploy and in-sync skips it. Model on `tests/test-stale-recovery-age-floor.sh` for sandbox setup pattern.
- **REFERENCE:** `~/.maestro/agents/scripts/maestro-update-check.sh:110-117` for the curl-based VERSION fetch pattern (similar HTTP/git-based drift check).

### Implementation Steps

1. **Stamp file format.** Single line at `~/.maestro/.deployed-sha`: `<sha> <timestamp_iso8601>`. Read with `awk '{print $1}'`. Missing file = cold install (treat all canonical-repo files as needing deploy).

2. **Drift detection function.** In `maestro-update-check.sh`:

   ```bash
   _check_script_drift() {
     local canonical_repo="$HOME/Git/maestro"
     ...
     deployed_sha=$(awk '{print $1}' "$HOME/.maestro/.deployed-sha" 2>/dev/null || echo "")
     ...
     "$canonical_repo/setup.sh" --non-interactive --silent >> "$HOME/.maestro/logs/auto-update.log" 2>&1
   }
   ```
...
3. **Stamp write in setup.sh.** At the end of the deploy section, after the canonical-repo files are copied:
   ...
   if [[ -d "$AGENTS_REPO/.git" ]]; then
     ...
     printf '%s %s\n' "$_sha" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$HOME/.maestro/.deployed-sha"
   fi
   ```

4. **Wire drift check into update loop.** In `maestro-update-check.sh`, after the existing VERSION check returns "no update needed", call `_check_script_drift` (so released-VERSION updates take precedence).
...
rm -f ~/.maestro/.deployed-sha
~/.maestro/agents/scripts/maestro-update-check.sh --interactive
...
echo "$(git -C ~/Git/maestro rev-parse HEAD~5) 2026-01-01T00:00:00Z" > ~/.maestro/.deployed-sha
~/.maestro/agents/scripts/maestro-update-check.sh --interactive
# Expect: drift detected, setup.sh runs, stamp updated to HEAD

# 4. Docs-only drift (should skip)
echo "$(git -C ~/Git/maestro log -n2 --pretty=%H -- TODO.md | tail -1) 2026-01-01T00:00:00Z" > ~/.maestro/.deployed-sha
~/.maestro/agents/scripts/maestro-update-check.sh --interactive
# Expect: skip, no redeploy

# 5. ShellCheck clean
shellcheck .agents/scripts/maestro-update-check.sh
...
- [ ] `~/.maestro/.deployed-sha` is written by `setup.sh` after every successful deploy
- [ ] `maestro-update-check.sh` detects SHA drift between canonical-repo HEAD and the stamp
