...
_dispatch_issue_consolidation() {
    ...
    substantive_json=$(printf '%s' "$comments_json" | jq --argjson min "$min_chars" '
        [.[] | select(
            ...
            and (.body | test("^<!-- (nmr-hold|maestro-signed|ops:start|provenance:start)") | not)
            ...
            and (.body | test("sudo maestro approve") | not)
...
2. **Compose a single coherent issue body** in the maestro brief format (see \`templates/brief-template.md\`):
...
```bash
cd ~/Git/maestro-bugfix-consolidation-worker
