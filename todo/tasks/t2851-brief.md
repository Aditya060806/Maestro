...
# t2851: case dossier contract + `maestro case open`
...
4. `maestro case open <slug> [--kind <type>] [--party <name>] [--deadline <iso-date>]` — interactive prompts for missing required fields
...
6. Provisioning wired into `maestro case init` (analogous to `maestro knowledge init`) — adds `cases: "repo"` to `repos.json` and creates skeleton
...
1. **Directory contract** — write `.agents/maestro/cases-plane.md`:
...
4. **Provisioning integration** — same shape as P0a: `maestro case init` flips `repos.json: cases: "repo"`, runs case provisioning helper that creates `_cases/`, `_cases/.gitignore` (gitignore drafts/ by default), `_cases/.case-counter`
...
- [ ] `maestro case init` writes `cases: "repo"` to `repos.json` and creates `_cases/` skeleton
- [ ] `maestro case open dispute-2026-acme --kind dispute --party "ACME Ltd" --deadline 2026-08-31` creates `_cases/case-2026-0001-dispute-2026-acme/` with valid dossier
...
- [ ] Personal plane mode: `_cases/` at `~/.maestro/.agent-workspace/cases/` works equivalently
- [ ] ShellCheck zero violations
- [ ] Tests pass: `bash .agents/tests/test-case-helper.sh`
- [ ] Documentation: full directory contract in `.agents/maestro/cases-plane.md`
