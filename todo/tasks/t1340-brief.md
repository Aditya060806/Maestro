...
- [ ] `opus-review-helper.sh check` returns exit 0 when no review has run in 4+ hours
  ...
  verify:
    method: bash
    run: "rm -f ~/.maestro/.agent-workspace/supervisor/.opus-review-last && bash ~/.maestro/agents/scripts/opus-review-helper.sh check"
  ```
...
- [ ] `opus-review-helper.sh check` returns exit 1 when review ran recently
  ...
  verify:
    method: bash
    run: "mkdir -p ~/.maestro/.agent-workspace/supervisor && date +%s > ~/.maestro/.agent-workspace/supervisor/.opus-review-last && ! bash ~/.maestro/agents/scripts/opus-review-helper.sh check"
  ```
...
- [ ] ShellCheck clean on opus-review-helper.sh
  ...
  verify:
    method: bash
    run: "shellcheck ~/.maestro/agents/scripts/opus-review-helper.sh"
  ```
