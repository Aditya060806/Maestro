...
Encryption is destructive if the user loses the passphrase or the unlock path is broken. maestro must prove round-trip unlock before offering to encrypt real data.
...
2. Require 12+ character passphrase, confirmation, strength/advice copy, and acknowledgement that maestro cannot recover it.
...
Model user warning tone on password-manager vaults and existing `maestro secret set` hidden-input guidance in `.agents/tools/credentials/gopass.md`.
