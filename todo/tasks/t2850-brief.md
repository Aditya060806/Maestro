...
6. CLI integration: `maestro knowledge search <query>` (from t2843) upgrades from substring grep to tree-walk when tree exists
7. Personal plane: separate tree at `~/.maestro/.agent-workspace/knowledge/index/tree.json`
...
- EDIT: `.agents/maestro/knowledge-plane.md` (indexing section)
- EDIT: `.agents/tools/context/pageindex.md` (cross-reference corpus-wide usage)

## Acceptance Criteria

- [ ] `maestro knowledge index build` writes `_knowledge/index/tree.json` covering all promoted sources
- [ ] Each source has its own `tree.json` after build
- [ ] `maestro knowledge index query "all invoices over £5k"` returns ranked source IDs with anchors and excerpt
- [ ] `maestro knowledge search <query>` (CLI from t2843) auto-routes to tree-walk when tree exists, falls back to grep otherwise
...
- [ ] Personal plane tree at `~/.maestro/.agent-workspace/knowledge/index/tree.json` works equivalently
