...
2. Identifies whether the current working copy is the **canonical directory** for the repo (i.e. `$(git rev-parse --show-toplevel)` matches one of the paths listed in `~/.config/maestro/repos.json` `initialized_repos[].path`)
...
5. If all three conditions hold: **warn loudly** (stderr banner), and:
   ...
   - **Strict mode (`MAESTRO_CANONICAL_GUARD=strict`):** also return non-zero to fail the checkout (caller can then retry on main or create a worktree)
...
1. I was working in canonical `~/Git/maestro` on `main`
...
1. **Hook body** (`canonical-on-main-guard.sh`):
    ...
    repos_config="${HOME}/.config/maestro/repos.json"
    ...
    RED=$'\033[0;31m'; YELLOW=$'\033[1;33m'; NC=$'\033[0m'
    {
        ...
        printf '\n'
        printf 'maestro convention (t1990): canonical directories stay on main.\n'
        ...
        printf '  MAESTRO_CANONICAL_GUARD=bypass\n'
        printf '\n'
        printf '%s============================================================%s\n' "$RED" "$NC"
        printf '\n'
    } >&2
    ...
    if [[ "${MAESTRO_CANONICAL_GUARD:-warn}" == "strict" ]]; then
        exit 1
    fi
...
# Live install in the maestro repo and manually test
...
- [ ] `MAESTRO_CANONICAL_GUARD=strict` causes the hook to exit non-zero (block the checkout); default is warn-only (exit 0).
...
- **Install via `setup.sh` auto-install:** mirrors t1968's privacy-guard rollout. Every initialized repo gets the guard on the next `maestro update` cycle. No manual rollout needed.
