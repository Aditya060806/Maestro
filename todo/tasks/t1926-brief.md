...
2. Issue description template (rewritten by `update`):
   ...
   **Detailed logs**: `~/.maestro/.agent-workspace/cron/{routine-id}/`
   ```

3. Local log reads: the helper reads `~/.maestro/.agent-workspace/cron/<routine-id>/` to compute period aggregates. It does not push raw logs to git.

### Verification

```bash
~/.maestro/agents/scripts/routine-log-helper.sh --help
...
- [ ] Detailed logs remain in `~/.maestro/.agent-workspace/cron/` (never pushed to git)
...
- `~/.maestro/.agent-workspace/cron/` — local log directory (existing)
