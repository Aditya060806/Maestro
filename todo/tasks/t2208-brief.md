...
1. Codacy top-level grade = **A** for `Aditya060806/Maestro`.
...
```bash
# Requires CODACY_API_TOKEN in gopass (maestro/CODACY_API_TOKEN)
token=$(maestro secret get maestro/CODACY_API_TOKEN 2>/dev/null \
  || cat ~/.config/maestro/credentials.sh | awk -F'=' '/CODACY_API_TOKEN/ {print $2}' | tr -d '"')
curl -s -H "api-token: $token" \
  'https://app.codacy.com/api/v3/analysis/organizations/gh/Aditya060806/repositories/maestro' | \
...
- Codacy dashboard: https://app.codacy.com/gh/Aditya060806/Maestro/dashboard

## Dependencies

- **Blocked by:** Codacy re-index reaching A (external).
- **Blocks:** none.
- **External:** Codacy API token (gopass `maestro/CODACY_API_TOKEN`).
