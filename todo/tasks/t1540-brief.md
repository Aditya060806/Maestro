...
Add the `workflow` scope to the required GitHub OAuth scopes in the maestro setup flow, and add pre-push detection so workers fail early (with actionable guidance) rather than silently failing at push time.
