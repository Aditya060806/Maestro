...
# Gastown vs Maestro — Comparative Analysis

**Date**: 2026-03-16
**Analyst**: Maestro Framework
...
Gastown (12.2k stars, 1k forks) is a multi-agent orchestration system for Claude Code with git-backed persistence. While philosophically similar to maestro (both solve multi-agent coordination), gastown has a fundamentally different implementation approach that offers valuable lessons.

**Key Difference**: Gastown is a compiled Go binary with a structured domain model; maestro is shell-script-based with intelligence-over-determinism philosophy.
...
| Gastown | Maestro | Assessment |
|---------|-----------|------------|
| **Mayor** (AI coordinator) | `/pulse` supervisor | Gastown has a dedicated interactive coordinator agent; maestro uses ephemeral pulse cycles |
| **Town** (`~/gt/`) | `~/.maestro/` workspace | Similar workspace concept |
| **Rig** (project container) | Registered repos in `repos.json` | Gastown's rig wraps repos with agent management; maestro tracks repos in JSON |
| **Polecats** (worker agents) | Workers dispatched via `opencode run` | Both ephemeral workers; gastown uses tmux sessions, maestro uses `opencode run` |
| **Crew Members** | Interactive sessions | Both support personal workspaces |
| **Hooks** (git worktrees for persistence) | Standard git worktrees | Gastown elevates worktrees to first-class "hooks"; maestro uses them transparently |
| **Convoys** (work bundling) | Task decomposition (t1408) | Gastown bundles at orchestration level; maestro decomposes pre-dispatch |
| **Beads** (git-backed issues) | GitHub Issues + TODO.md | Gastown uses Dolt (Git for data); maestro uses GitHub as state DB |

### 1.2 Unique Gastown Features (Not in Maestro)

| Feature | Description | maestro Gap |
|---------|-------------|--------------|
...
### 1.3 Unique Maestro Features (Not in Gastown)
...
**Maestro**: Explicitly favors intelligence over determinism. "The harness gives you goals, tools, and boundaries — not scripts for every scenario." No state machines, no SQLite for orchestration state.

**Takeaway**: Gastown's structured approach to orchestration (formulas, health states) could inform maestro without violating the intelligence principle. The "Problems View" pattern is particularly valuable.

### 2.2 State Management

**Gastown**: Uses Dolt (Git for data) for structured state. Beads are atomic work units with IDs like `gt-abc12`.

**Maestro**: Uses GitHub Issues + TODO.md as the "only state DB." "Never duplicate this state into separate files, databases, or logs."

**Takeaway**: maestro' approach is simpler but may lack granularity for tracking sub-task state. Gastown's bead/convoy model could inspire better task bundling.
...
**Maestro**: Claude Code is primary; others supported as "courtesy." Uses `opencode run` exclusively for dispatch.

**Takeaway**: Gastown's agnostic approach to runtimes is more robust. maestro' Claude-first approach is pragmatic but may limit portability.

---

## 3. Specific Improvement Opportunities for Maestro
...
**maestro current**: `/dashboard` generates static HTML; `mission-dashboard-helper.sh` outputs formatted text.
...
**maestro current**: Pulse detects stuck workers (>3h with no PR) and kills them. Less structured.
...
**maestro current**: No formal workflow templates. Release process is documented in `release.md` but not executable.
...
**maestro current**: Task decomposition (t1408) splits tasks into subtasks but doesn't bundle existing issues.
...
**maestro current**: `/dashboard browser` generates static HTML, no auto-refresh.
...
**maestro current**: Cross-session memory (`/remember`, `/recall`) but no formal message passing.

**Recommendation**: Add mailbox system using existing file structure:
- `~/.maestro/.agent-workspace/mail/inbox/`
- `~/.maestro/.agent-workspace/mail/outbox/`
- `~/.maestro/.agent-workspace/mail/archive/`
...
**maestro current**: Implicit in `/full-loop` but not documented as principle.
...
**maestro current**: Claude Code primary; `opencode run` for dispatch.
...
**maestro current**: No tmux integration.
...
**maestro current**: JSONL files, SQLite for memory.
...
1. **Compiled binary**: Gastown's Go binary requires build/deployment; maestro shell scripts are more hackable
...
4. **Structured bead IDs**: `gt-abc12` format requires central allocation; maestro' `claim-task-id.sh` with CAS is simpler
...
Gastown and Maestro share similar goals but different philosophies. Gastown provides structure (formulas, health states, TUI); maestro provides intelligence (no state machines, GitHub as DB).

**The best improvements for maestro would incorporate gastown's clarity and user experience without compromising its core intelligence-over-determinism philosophy.**
