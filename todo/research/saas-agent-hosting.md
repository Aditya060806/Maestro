...
### Provider comparison for maestro SaaS
...
### Session persistence model for maestro
...
| **Stack fit** | Poor (Python dependency; maestro is shell-native) |
| **Multi-tenant** | Requires orchestration layer regardless |
| **Risk** | Upstream API changes; single-user assumptions in core |

**Verdict:** Viable if maestro adds a Python service layer. Not viable for pure shell-native implementation.
...
Implement mngr's patterns in shell, tailored to maestro's stack and multi-tenant requirements.
...
| **Stack fit** | Excellent (maestro is shell-native) |
| **Multi-tenant** | Designed in from the start |
| **Risk** | Reimplementation risk; must test edge cases mngr already handles |

**Verdict:** Recommended for MVP. The core patterns are well-understood from mngr's docs. Shell implementation avoids Python dependency and integrates naturally with maestro's existing helper scripts.
...
```
┌─────────────────────────────────────────────────────────────┐
│  maestro SaaS Orchestration Layer (shell + provider APIs)   │
│  - Tenant → host mapping                                     │
...
- The orchestration layer is shell-native, consistent with maestro's existing tooling
...
flowchart TB
    ...
    subgraph orch["maestro Orchestration (shell scripts + provider APIs)"]
        direction LR
...
- maestro headless dispatch: `.agents/scripts/headless-runtime-helper.sh`
- maestro pulse: `.agents/scripts/commands/pulse.md`
