...
launchctl unload ~/Library/LaunchAgents/sh.maestro.supervisor-pulse.plist

# 2. Wait for in-flight pulse to complete (check PIDFILE)
while [[ -f ~/.maestro/logs/pulse.pid ]]; do
  content=$(cat ~/.maestro/logs/pulse.pid)
  [[ "$content" == IDLE:* ]] && break
  sleep 5
done

# 3. Pull main, rerun setup to deploy
cd ~/Git/maestro && git pull --ff-only main
./setup.sh --non-interactive

# 4. Self-check and dry-run
~/.maestro/agents/scripts/pulse-wrapper.sh --self-check
PULSE_DRY_RUN=1 ~/.maestro/agents/scripts/pulse-wrapper.sh

# 5. One real cycle under manual control, watch the log
~/.maestro/agents/scripts/pulse-wrapper.sh 2>&1 | tee /tmp/pulse-smoke.log
# Look for ERROR, "function not found", "unbound variable"

# 6. Reload launchd
launchctl load ~/Library/LaunchAgents/sh.maestro.supervisor-pulse.plist

# 7. Watch the next two live cycles
tail -f ~/.maestro/logs/pulse.log
```
...
```bash
launchctl unload ~/Library/LaunchAgents/sh.maestro.supervisor-pulse.plist
git -C ~/Git/maestro revert --no-edit HEAD
git -C ~/Git/maestro push origin main
./setup.sh --non-interactive
launchctl load ~/Library/LaunchAgents/sh.maestro.supervisor-pulse.plist
```
...
```bash
launchctl unload sh.maestro.supervisor-pulse
./setup.sh --non-interactive
~/.maestro/agents/scripts/pulse-wrapper.sh --self-check && \
  PULSE_DRY_RUN=1 ~/.maestro/agents/scripts/pulse-wrapper.sh && \
  launchctl load sh.maestro.supervisor-pulse
tail -f ~/.maestro/logs/pulse.log  # watch two live cycles
```
...
3. **Pulse pause mechanism: `STOP_FLAG`.** Non-destructive; in-flight workers finish; next pulse cycle short-circuits. No launchd unload needed. `touch ~/.maestro/logs/pulse-session.stop` before merge; `rm` after smoke test.
