...
# maestro GUI Control Plane Brief Plan
...
worker-ready phase briefs for building the maestro GUI/control plane over time.
...
1. Use maestro issue/TODO wrappers, not raw `gh issue create`.
...
```text
Build maestro GUI control plane for setup, infrastructure, routines, and safe multi-machine AI-agent operations
```
...
Build the maestro GUI/control plane as the primary visual surface for setting
up, understanding, managing, and scaling maestro across local machines,
...
git runners, secrets, routines, projects, and OpenCode/maestro instances.

## Why

maestro has growing operational knowledge in agents, helpers, workflows, and
...
├── P4 Helper/API contract for existing maestro surfaces (open: GH#25233 / t3611)
...
2. Review existing maestro repo layout policy in `.agents/maestro/architecture.md`.
...
2. Read maestro secret-handling references before proposing any secret UX.
...
### P4: Helper/API contract for existing maestro surfaces
...
- Contract for how the GUI calls existing maestro helpers for setup, settings,
...
- Minimal local read-only GUI that can show maestro setup/status from typed
...
4. Add a basic dashboard shell in `apps/gui-web` with maestro branding.
...
- Dashboard cards for maestro, OpenCode, helper availability, config/settings,
...
- [ ] Each card links to the relevant maestro helper/doc reference.
...
- Browser for maestro capabilities: agents, tools, services, workflows,
...
- Design for pairing user-owned machines, issuing scoped task capsules, and
  coordinating maestro/OpenCode work across available compute.
...
- [ ] Does not bypass OpenCode permissions or maestro hooks.
...
|-------|--------------------|--------------------|
| P1 | `architecture`, product planning context | `docs/gui/control-plane.md`, `.agents/maestro/architecture.md` |
