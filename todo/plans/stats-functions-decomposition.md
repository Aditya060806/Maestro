...
- **Preserve `LOGFILE` semantics.** The sourcer assigns `LOGFILE="$STATS_LOGFILE"` before sourcing so all log output goes to `~/.maestro/logs/stats.log`. Every extracted module must continue to write to `$LOGFILE`, not hardcode a path.
...
LOGFILE                   = ${HOME}/.maestro/logs/stats.log     (overridden by sourcer to STATS_LOGFILE)
QUALITY_SWEEP_INTERVAL    = 86400 seconds (1 day)
PERSON_STATS_INTERVAL     = 3600 seconds (1 hour)
QUALITY_SWEEP_LAST_RUN    = ${HOME}/.maestro/logs/quality-sweep-last-run
PERSON_STATS_LAST_RUN     = ${HOME}/.maestro/logs/person-stats-last-run
PERSON_STATS_CACHE_DIR    = ${HOME}/.maestro/logs
QUALITY_SWEEP_STATE_DIR   = ${HOME}/.maestro/logs/quality-sweep-state
...
No mutable counters. No per-cycle state held in shell variables. Stats state lives entirely on disk under `~/.maestro/logs/`.
...
setup_sandbox() {
    ...
    mkdir -p "$HOME/.maestro/logs" "$HOME/.config/maestro"
    echo '{"initialized_repos": []}' > "$HOME/.config/maestro/repos.json"
    export LOGFILE="$HOME/.maestro/logs/stats.log"
}
...
2. Wait for any in-flight stats process (check `~/.maestro/logs/stats.pid`).
...
6. Run one real cycle manually: `bash .agents/scripts/stats-wrapper.sh`. Watch `~/.maestro/logs/stats.log` for ERROR, "function not found", "unbound variable".
...
# Globals written:
#   - none (stats modules write only to disk under ~/.maestro/logs/)
...
2. `bash setup.sh --non-interactive` to redeploy the modified script set to `~/.maestro/agents/scripts/`.
3. Stop the launchd `stats-wrapper.sh` job: `launchctl stop sh.maestro.stats-wrapper` (or whatever the actual label is — check `launchctl list | grep maestro` first).
4. Wait for any in-flight stats process to exit (check `~/.maestro/logs/stats.pid`).
5. Run one manual cycle: `bash ~/.maestro/agents/scripts/stats-wrapper.sh`. Watch `~/.maestro/logs/stats.log` for ERROR, function-not-found, or unbound-variable.
