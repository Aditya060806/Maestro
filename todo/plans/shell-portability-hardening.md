...
**Started:** 2026-04-14
**Origin:** `sudo maestro approve` failing on macOS with `getent: command not found`
...
| **GH#18784** | `maestro.sh:26` | Unguarded `getent passwd` crashed on macOS (no getent binary) | User ran `sudo maestro approve` and saw `getent: command not found` |
...
1. **The fix** — `maestro.sh:26` guard mirroring `approval-helper.sh:33`. One line.
...
3. **Regression test** — `test-maestro-sh-portability.sh` with three assertions: structural grep, runtime simulation (PATH stripped of getent + mocked `id -u`), sibling-guard intact.
...
3. At least one test suite actually RUNS `pulse-wrapper.sh` and `maestro.sh` under real `set -e` on both Linux AND macOS, and asserts they reach a known checkpoint. Measured by: the canary tests from t2080 + PR #18785 are in CI.
...
- Windows portability (no Windows runners, no Windows users of maestro today)
