...
```text
2026-04-12T17:05:23Z labeled status:available by Aditya060806
2026-04-12T17:05:23Z labeled auto-dispatch by Aditya060806
2026-04-12T17:05:23Z labeled test by Aditya060806
2026-04-12T17:05:24Z labeled origin:interactive by Aditya060806
2026-04-12T17:05:24Z labeled tier:standard by Aditya060806
2026-04-12T17:05:35Z assigned Aditya060806 by Aditya060806
2026-04-12T17:12:53Z unassigned Aditya060806 by Aditya060806   <-- churn starts
...
2026-04-12T17:25:05Z assigned Aditya060806 by Aditya060806    <-- manual recovery
```

**At the moment of dispatch (17:12:53), issue state was:**

- Assignee: `Aditya060806` (me, the repo owner)
...
2. Compute `assignees = [Aditya060806]`
...
4. For each assignee, check if it's owner/maintainer:
   - `Aditya060806` IS the repo owner (line 748)
   - `active_claim == true`, so the owner IS blocking (line 751-753 skipped)
5. `blocking_assignees = "Aditya060806"`
6. `is_assigned` returns `0` (blocked) — dispatch halted

**What actually happened:** the dispatch proceeded, stripped `Aditya060806`, assigned `alex-solovyev`. The only code path that produces this is `pulse-dispatch-core.sh:889-898` (GH#17777), which is AFTER the `check_dispatch_dedup` call at line 884. So `check_dispatch_dedup` must have returned success (not blocked), which means either:
...
Either way, **alex-solovyev is running an older maestro version** where the interactive-session guard doesn't exist yet.
...
Any maestro install at the latest version will NOT reproduce this churn on issues that carry `origin:interactive`.
...
2. Workflow runs `issue-sync-helper.sh push` with `MAESTRO_SESSION_ORIGIN=interactive` + `MAESTRO_SESSION_USER=<github.actor>`
...
### Primary fix — alex-solovyev updates maestro

Operational, not code:

1. Have alex-solovyev run `maestro update` on their machine
...
Not something we can fix. Document it in operator docs so future investigations don't waste time chasing "shared credentials" hypotheses when they see "by Aditya060806" on events that were actually performed by another operator's pulse.
...
The attribution pattern looked suspicious initially but is fully explained by GitHub's event attribution quirk. alex-solovyev has their own `gh` token with write access (confirmed via `gh api repos/Aditya060806/Maestro/collaborators/alex-solovyev/permission` earlier in the session). No rotation needed.

## Operational recommendations

1. **Ask alex-solovyev to run `maestro update`** and report their version.
...
The framework-level code is correct. This ticket documents a one-off cross-operator version skew, not a latent maestro bug.
