...
describe("dashboard shell", () => {
  test("renders setup/status placeholders", () => {
    ...
    expect(html).toContain("maestro app interface");
    ...
    expect(html).toContain("Encrypted by maestro Vault; contents visible only when unlocked through app or authorised vault commands.");
    expect(html).toContain("AI Provider Pools");
    expect(html).toContain("AI Apps");
    expect(html).toContain("Installed maestro targets");
  ...
  test("renders channel and DM conversation surfaces from the unified model", () => {
    ...
    expect(dmHtml).toContain("Maestro");
  ...
  test("rejects unsafe Tambo component payloads during conversation rendering", () => {
    const html = renderToStaticMarkup(createElement(CommsConversationSurface, {
      mode: "channels",
      threads: [{
        conversation: { id: "channel-unsafe", type: "channel", title: "unsafe", scope: { tenant_ref: "local", workspace_ref: "maestro", repo_ref: null }, source_ref: "test", status: "read_only", created_at: "2026-06-27T00:00:00Z", updated_at: "2026-06-27T00:00:00Z" },
        participants: [{ id: "participant-ai", conversation_id: "channel-unsafe", kind: "ai_assistant", display_name: "Maestro", identity_ref: null, agent_ref: "maestro", worker_ref: null, membership_state: "active", joined_at: "2026-06-27T00:00:00Z" }],
  ...
  test("renders conversation threads when optional collections are absent", () => {
    const html = renderToStaticMarkup(createElement(CommsConversationSurface, {
      mode: "channels",
      threads: [{
        conversation: { id: "channel-partial", type: "channel", title: "partial", scope: { tenant_ref: "local", workspace_ref: "maestro", repo_ref: null }, source_ref: "test", status: "read_only", created_at: "2026-06-27T00:00:00Z", updated_at: "2026-06-27T00:00:00Z" },
  ...
  test("normalizes legacy status payloads from older local API processes", async () => {
    ...
    expect(status.data.setup_targets[0].path_ref).toBe("~/.maestro/agents/VERSION");
  ...
  test("renders updated Apps copy, casing, filter order, and compact managed metadata", () => {
    ...
    expect(html).not.toContain("App and CLI inventory for tools installed or updated by maestro");
  ...
  test("wires desktop screenshot capture controls to native save notifications", () => {
    ...
    expect(screenshotSource).toContain("maestro:screenshot-captured");
    ...
    expect(desktopInstaller).toContain("maestro-app-screenshot-\\(screenshotTimestamp())-\\(sanitizeFilenameComponent(nameComponent)).png");
  ...
  test("ships critical loading styles before React hydrates", () => {
    ...
    expect(html).toContain("maestro-gui-theme");
  ...
  test("desktop wrapper defers to the web loading shell and refreshes app icon metadata", () => {
    ...
    expect(desktopInstaller).not.toContain('loadStatusHTML(title: "Starting maestro"');
    expect(desktopInstaller).toContain('viewBox="0 0 1024 1024" role="img" aria-labelledby="title desc"');
    expect(desktopInstaller).toContain('rm -f "${resources_dir}/maestro.icns" "${resources_dir}/maestro.svg"');
    expect(desktopInstaller).toContain('touch "${resources_dir}/maestro.icns" "${contents_dir}/Info.plist" "$app_path"');
...
const managedAppFixture: GuiManagedAppSummary = {
  actions: [{ command_preview: "maestro app update maestro", confirmation: "recommended", enabled: true, id: "update", label: "Update" }],
  maestro_install: true,
  maestro_update: true,
  category: "core",
  description: "Framework CLI, agents, workflows, scripts, and local GUI assets.",
  id: "maestro",
  install_path_ref: "/usr/local/bin/maestro",
