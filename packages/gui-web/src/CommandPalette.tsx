...
const commandPaletteRecentStorageKey = "maestro-gui-command-palette-recents";
...
const plannedChannelItems: CommandPaletteItem[] = [
  { id: "channel-devops", label: "#devops", description: "Chat channel for maestro operations", icon: "hash", searchText: "#devops devops channel chat", surface: "messagingAccounts", shellMode: "sessions", conversationMode: "people", tag: "Channel" },
...
function commandPaletteItems(status: GuiStatusData, activeSurface: SurfaceId): CommandPaletteItem[] {
  ...
  const currentLinkItems: CommandPaletteItem[] = [
    { action: "copy-current-link", id: "copy-current-link", label: `^ copy ${activeSurface} link`, description: "Copy a link that can reopen this page in the web or local app", icon: "link", searchText: `^ link copy current page url ${activeSurface} maestro://surface/${activeSurface}`, surface: activeSurface, tag: "Link" },
  ];
...
async function copyCurrentSurfaceLink(surface: SurfaceId): Promise<void> {
  ...
  const link = browserUrl.length > 0 ? `${browserUrl}#surface=${surface}` : `maestro://surface/${surface}`;
