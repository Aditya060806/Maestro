...
export function renderDashboardHtml(status: GuiResponseEnvelope<GuiStatusData>): string {
  ...
  const localRepos = status.data.local_repos.repos
    .map((repo) => `<li>${escapeHtml(repo.name)}: ${escapeHtml(repo.maestro_version)} ${escapeHtml(repo.default_branch)}</li>`)
    .join("");
  ...
  const aiApps = status.data.ai_apps
    .map((app) => `<li>${escapeHtml(app.name)}: ${escapeHtml(app.status)} ${escapeHtml(app.maestro_version)} → ${escapeHtml(app.latest_version)}</li>`)
    .join("");
  ...
  return [
    `<section aria-label="maestro status">`,
    `<h1>maestro app interface</h1>`,
    ...
    `<p>Vault surfaces show padlock indicators. Encrypted by maestro Vault; contents visible only when unlocked through app or authorised vault commands.</p>`,
    `<p>${escapeHtml(status.data.update.message)}</p>`,
    renderedNavSections,
    `<h2>Path health</h2><ul>${paths}</ul>`,
    `<h2>Installed maestro targets</h2><ul>${setupTargets}</ul>`,
