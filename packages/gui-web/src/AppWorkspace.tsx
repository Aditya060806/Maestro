...
const communityLinks = {
  github: "https://github.com/Aditya060806/Maestro",
...
function NotificationsMenu({ notifications, openSurface }: { notifications: GuiNotificationSummary[]; openSurface: (surface: SurfaceId) => void }): ReactElement {
  ...
  return (
    <div className="popover-menu notifications-menu" role="menu">
      ...
      {preview.length === 0 ? <p>No current maestro notifications.</p> : null}
      ...
      <div className="chat-thread-panel" data-tour="ai-session-transcript">
        ...
        <div className="chat-message-list" data-tour="message-scroller">
          ...
          <TamboConversationPart
            ...
            scope={{ tenant_ref: "local", workspace_ref: "maestro", repo_ref: selectedRepo?.path_ref ?? null }}
          />
        </div>
  ...
  return (
    <section className="settings-surface notifications-surface">
      <div className="planned-card notifications-hero">
        <h2>Notifications</h2>
        <p>OpenCode startup toasts and the GUI notification centre read the same maestro status cache and local readiness data, so resolved items disappear or downgrade everywhere after the underlying status changes.</p>
      ...
      <ul aria-label="maestro notifications" className="github-notification-list">
        {status.notifications.length === 0 ? <p className="empty-state">No maestro notifications are currently reported.</p> : null}
