/* jshint esversion: 11 */
import type { GuiMachineSummary, GuiStatusData } from "@maestro/gui-shared";
...
function MaestroPromptIcon() {
  return (
    <svg className="brand-icon" viewBox="0 0 1024 1024" role="img" aria-label="Maestro prompt icon">
...
export function MachineRail({ machine }: { machine?: GuiMachineSummary }) {
  ...
  return (
    <aside className="machine-rail" aria-label="maestro machines and clients">
...
function PeopleChannelList() {
  return (
    <section aria-label="People channels">
      ...
      <section className="sidebar-group session-history-list">
        <ul>
          {["maestro", "clients", "ops"].map((team) => <li key={team}><button className="surface-link" type="button"><span className="surface-icon" aria-hidden="true"><FiHash /></span><span className="surface-copy"><strong>{team}</strong><small>SimpleX team channel placeholder</small></span></button></li>)}
        </ul>
      </section>
      <section className="sidebar-group session-history-list">
        <ul>
          {["Marcus", "Maestro"].map((person) => <li key={person}><button className="surface-link" type="button"><span className="surface-icon" aria-hidden="true"><FiMessageSquare /></span><span className="surface-copy"><strong>{person}</strong><small>encrypted DM placeholder</small></span></button></li>)}
        </ul>
      </section>
    </section>
  );
