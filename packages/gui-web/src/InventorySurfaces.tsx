import type { GuiAppActionJobSummary, GuiManagedAppSummary, GuiResponseEnvelope, GuiStatusData } from "@maestro/gui-shared";
...
export function AppsSurface({ status }: { status: GuiStatusData }): ReactElement {
  const [appCollection, setAppCollection] = useState<AppCollectionId>("maestro");
  ...
  return (
    <section className="apps-surface" aria-label={text.apps} onBlur={() => setTooltip(null)} onFocus={showFocusedAppTooltip(setTooltip)} onPointerLeave={() => setTooltip(null)} onPointerMove={showPointedAppTooltip(setTooltip)}>
      ...
      {appCollection === "maestro" ? <>
        <TabNav label="maestro app groups" tabs={managedCategoryTabs} value={managedCategory} onChange={(value) => setManagedCategory(value)} />
...
type AppCollectionId = "maestro" | "recommended";
...
function applyManagedPolicyToggles(app: GuiManagedAppSummary, policy: Partial<Record<ManagedPolicyId, boolean>> | undefined): GuiManagedAppSummary {
  ...
  return {
    ...app,
    maestro_install: policy.setup ?? app.maestro_install,
    maestro_update: policy.update ?? app.maestro_update,
  };
}

function createPolicyJob(appId: string, policy: ManagedPolicyId, value: boolean): GuiAppActionJobSummary {
  const mode = value ? "on" : "off";
  const command = `maestro app policy set ${appId} --${policy === "setup" ? "setup-installs" : "update-maintains"} ${mode}`;
  ...
  return {
    ...
    output: [
      `$ ${command}`,
      `Saved local maestro.app policy override for ${appId}: ${policy}=${mode}.`,
      "Persistence: browser localStorage key maestro-gui-managed-app-policy.",
    ],
  };
}
...
function readManagedPolicyToggles(): ManagedPolicyToggleState {
  ...
  try {
    const raw = window.localStorage.getItem("maestro-gui-managed-app-policy");
...
function writeManagedPolicyToggles(policy: ManagedPolicyToggleState): void {
  ...
  window.localStorage.setItem("maestro-gui-managed-app-policy", JSON.stringify(policy));
}
