...
describe("status adapter", () => {
  test("uses an exact helper command pattern", () => {
    expect(STATUS_ADAPTER_COMMAND).toEqual(["maestro", "status"]);
  });
  ...
  test("returns typed read-only status data", () => {
    ...
    expect(response.data.repos.path_ref).toBe("~/.config/maestro/repos.json");
    ...
    expect(response.data.setup_targets.map((target) => target.path_ref)).toContain("~/.maestro/agents/VERSION");
    ...
    expect(response.source.path_refs).toContain("~/.maestro/logs/headless-runtime-metrics.jsonl");
  ...
  test("populates Pulse and Workers status from local telemetry files", () => {
    const root = mkdtempSync(join(tmpdir(), "maestro-gui-pulse-workers-"));
    ...
    writeFileSync(metricsPath, [
      JSON.stringify({ ts: 1782810000, result: "success", issue: 25914, pr: 25999, repo: "Aditya060806/Maestro", provider: "openai", model: "gpt-5.5", input_tokens: 1200, output_tokens: 300, cached_tokens: 100, estimated_cost_usd: 0.42, duration_ms: 60000, issue_origin: "origin_interactive", author_association: "MEMBER" }),
      JSON.stringify({ ts: 1782806400, result: "blocked", issue: 25910, repo: "Aditya060806/Maestro", provider: "anthropic", model: "sonnet", total_tokens: 900, issue_origin: "third_party", author_association: "CONTRIBUTOR" }),
      JSON.stringify({ ts: 1782806300, result: "failed", issue: 25911, repo: "Aditya060806/Maestro", provider: "anthropic", model: "sonnet", total_tokens: 700, estimated_cost_usd: 0.28, duration_ms: 1900000, issue_origin: "maestro_created", author_association: "OWNER", failure_category: "review_gate" }),
      JSON.stringify({ ts: 1782720000, result: "success", issue: 25800, repo: "Aditya060806/Maestro", provider: "openai", model: "gpt-5.5", total_tokens: 300, estimated_cost_usd: 0.10, issue_origin: "self_created", author_association: "MEMBER", verification: "bun test passed" }),
    ].join("\n"));
  ...
  test("returns safe Pulse and Workers warnings for missing or malformed telemetry", () => {
    const root = mkdtempSync(join(tmpdir(), "maestro-gui-pulse-workers-missing-"));
  ...
  test("reads Vault helper output through sh without requiring an executable bit", () => {
    const repoRoot = mkdtempSync(join(tmpdir(), "maestro-gui-vault-helper-"));
  ...
  test("does not search relative home tool paths when HOME is unset", () => {
    ...
    const repoRoot = mkdtempSync(join(tmpdir(), "maestro-gui-relative-path-"));
    const fakeBinDir = join(repoRoot, ".bun", "bin");
    const fakeBinary = join(fakeBinDir, "maestro-relative-path-sentinel");
    ...
    try {
      ...
      expect(resolveBinary("maestro-relative-path-sentinel")).toBeNull();
    } finally {
