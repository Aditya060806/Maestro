...
function localRepoSummaryFromPath(pathName: string, registryEntry?: Record<string, unknown>): GuiLocalRepoSetupSummary {
  const config = readJsonObject(join(pathName, ".maestro.json")).value;
  ...
  return {
    ...
    maestro_version: stringField(initConfig, "version") ?? stringField(registry, "version") ?? "not initialized",
