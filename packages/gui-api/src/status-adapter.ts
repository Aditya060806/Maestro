...
export const STATUS_ADAPTER_COMMAND = ["maestro", "status"] as const;
...
export function readStatus(
  ...
  const settingsPathRef = "~/.config/maestro/settings.json";
  const reposPathRef = "~/.config/maestro/repos.json";
  const oauthPoolPathRef = "~/.maestro/oauth-pool.json";
  const opencodeDbPathRef = "~/.local/share/opencode/opencode.db";
  const greetingCachePathRef = "~/.maestro/cache/session-greeting.txt";
  const agentsPathRef = "~/.maestro/agents";
  ...
  const maestroVersion = existsSync(versionPath)
    ? readFileSync(versionPath, "utf8").trim()
    : statusFixture.maestro_version;
  const installedVersion = readOptionalText(expandHome("~/.maestro/agents/VERSION")) ?? maestroVersion;
  const restartRequired = installedVersion !== maestroVersion;
  const setupTargets = readSetupTargets(maestroVersion || "unknown", installedVersion || "unknown");
  const aiApps = readAiApps(maestroVersion || "unknown", installedVersion || "unknown");
  const managedApps = readManagedApps(maestroVersion || "unknown", installedVersion || "unknown");
  ...
  const sourcePathRefs = [
    ...
    ...aiApps.flatMap((app) => [app.app_path_ref, app.binary_path_ref, app.config_path_ref, app.maestro_target_path_ref]),
  ...
  const data: GuiStatusData = {
    ...statusFixture,
    maestro_version: maestroVersion || "unknown",
    update: {
      running_version: maestroVersion || "unknown",
      installed_version: installedVersion || "unknown",
      restart_required: restartRequired,
      message: restartRequired
        ? "maestro has updated in the background. Restart the GUI app to use the latest installed version."
        : "The GUI app is using the latest installed maestro version.",
    },
    ...
    paths: [
      {
        ...
        health: existsSync(expandHome("~/.maestro/agents")) ? "present" : "missing",
      },
      {
        label: "settings",
...
export function readVaultStatus(
  ...
  const envelope = createEnvelope({
    ...
    source: {
      surface: "vault",
      authority: "maestro vault helper",
...
const SETUP_TARGET_DEFINITIONS: Array<Pick<GuiSetupTargetSummary, "label" | "path_ref" | "purpose">> = [
  {
    label: "Deployed agents",
    path_ref: "~/.maestro/agents/VERSION",
    purpose: "Canonical maestro agent, workflow, script, and reference bundle.",
  },
  {
    label: "OpenCode prompt",
    path_ref: "~/.config/opencode/AGENTS.md",
    purpose: "OpenCode system prompt pointer into the deployed maestro guide.",
  },
  {
  ...
  },
  {
    ...
    purpose: "Claude Code global instructions pointer into the deployed maestro guide.",
  },
  {
    label: "Claude commands",
    path_ref: "~/.claude/commands",
    purpose: "Claude Code slash commands generated by maestro.",
  },
  {
    label: "Codex instructions",
    path_ref: "~/.codex/instructions.md",
    purpose: "Codex global instructions generated by maestro.",
  },
  {
    label: "Cursor MCP config",
    path_ref: "~/.cursor/mcp.json",
    purpose: "Cursor MCP server configuration managed by maestro.",
  },
];
...
interface AiAppDefinition {
  ...
  maestro_target_path_ref: string;
  version_args: string[];
}
...
const AI_APP_DEFINITIONS: AiAppDefinition[] = [
  {
    name: "OpenCode",
    binary: "opencode",
    app_path_refs: ["~/Applications/OpenCode Maestro.app", "/Applications/OpenCode.app", "~/Applications/OpenCode.app"],
    config_path_ref: "~/.config/opencode/opencode.json",
    maestro_target_path_ref: "~/.config/opencode/AGENTS.md",
  ...
  },
  {
    ...
    maestro_target_path_ref: "~/.config/Claude/AGENTS.md",
    version_args: ["--version"],
  },
  {
    name: "Codex CLI",
    ...
    maestro_target_path_ref: "~/.codex/instructions.md",
    version_args: ["--version"],
  },
  {
    name: "Cursor",
    ...
    maestro_target_path_ref: "~/.cursor/rules",
    version_args: [],
  },
];
...
function aiAppSummary(definition: AiAppDefinition, latestVersion: string, installedVersion: string): GuiAiAppSummary {
  ...
  const hasMaestroTarget = existsSync(expandHome(definition.maestro_target_path_ref));
  const maestroTargetVersion = hasMaestroTarget ? installedVersion : "not installed";
  ...
  return {
    ...
    maestro_target_path_ref: definition.maestro_target_path_ref,
    app_version: binaryPath === null ? "unknown" : readBinaryVersion(binaryPath, definition.version_args),
    maestro_version: maestroTargetVersion,
    latest_version: latestVersion,
    needs_update: maestroTargetVersion !== latestVersion,
  };
}
