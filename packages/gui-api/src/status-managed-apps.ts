...
interface ManagedAppDefinition {
  ...
  maestro_install: boolean;
  maestro_update: boolean;
  latest_version_source: "maestro" | "binary" | "unknown";
  action_commands: ManagedAppActionCommands;
}

type BinaryAppInput = Omit<ManagedAppDefinition, "action_commands" | "maestro_install" | "maestro_update" | "latest_version_source"> & {
  binary: string;
  maestro_install?: boolean;
  maestro_update?: boolean;
};
...
const TOOL_ACTIONS: ManagedAppActionCommands = {
  ...
  update: "maestro update-tools --update",
};
...
const MANAGED_APP_DEFINITIONS: ManagedAppDefinition[] = [
  {
    ...
    install_path_refs: ["~/.maestro/agents", "/opt/homebrew/bin/maestro", "/usr/local/bin/maestro"],
    origin_website_url: "https://maestro.sh",
    origin_repo_url: "https://github.com/Aditya060806/Maestro.git",
    maestro_install: true,
    maestro_update: true,
    latest_version_source: "maestro",
    action_commands: {
      install: "./setup.sh --non-interactive",
  ...
  },
  {
    ...
    description: "Canonical maestro agents, commands, workflows, scripts, and reference files.",
    category: "core",
    binary: null,
    version_args: [],
    install_path_refs: ["~/.maestro/agents/VERSION"],
    origin_website_url: "https://maestro.sh",
    origin_repo_url: "https://github.com/Aditya060806/Maestro.git",
    maestro_install: true,
    maestro_update: true,
  ...
  },
  {
    ...
    binary: null,
    version_args: [],
    install_path_refs: ["~/Applications/maestro.app", "/Applications/maestro.app"],
    origin_website_url: "https://maestro.sh",
    origin_repo_url: "https://github.com/Aditya060806/Maestro.git",
    maestro_install: true,
    maestro_update: true,
  ...
  },
  {
    id: "opencode",
    name: "OpenCode Beta",
    description: "Native OpenCode beta desktop app configured by maestro.",
    category: "desktop",
    binary: null,
    version_args: [],
    install_path_refs: ["~/Applications/OpenCode Maestro.app", "~/Applications/OpenCode.app", "/Applications/OpenCode.app", "~/.config/opencode/opencode.json"],
  ...
  binaryApp({ id: "opencode-cli", name: "OpenCode CLI", description: "Command-line runtime installed with OpenCode and configured by maestro.", category: "cli", binary: "opencode", version_args: ["--version"], install_path_refs: ["/opt/homebrew/bin/opencode", "/usr/local/bin/opencode", "~/.bun/bin/opencode", "~/.config/opencode/opencode.json"], origin_website_url: "https://opencode.ai", origin_repo_url: "", maestro_install: true, maestro_update: true }),
  {
    ...
    binary: null,
    version_args: [],
    install_path_refs: ["~/.maestro/hooks", "~/.config/maestro"],
    origin_website_url: "https://maestro.sh",
    origin_repo_url: "https://github.com/Aditya060806/Maestro.git",
    maestro_install: true,
    maestro_update: true,
  ...
  },
  {
    ...
    description: "Launchd/cron supervisor for autonomous maestro maintenance and dispatch routines.",
    category: "automation",
    binary: null,
    version_args: [],
    install_path_refs: ["~/Library/LaunchAgents/sh.maestro.pulse.plist", "~/Library/LaunchAgents/com.maestro.maestro-supervisor-pulse.plist", "~/Library/LaunchAgents/sh.maestro.supervisor-pulse.plist", "~/.maestro/.agent-workspace/pulse"],
    origin_website_url: "https://maestro.sh",
    origin_repo_url: "https://github.com/Aditya060806/Maestro.git",
    maestro_install: true,
    maestro_update: true,
  ...
  binaryApp({ id: "tabby", name: "Tabby terminal profiles", description: "Terminal profile sync for maestro-managed shells.", category: "terminal", binary: "tabby", version_args: ["--version"], install_path_refs: ["~/Library/Application Support/tabby", "/Applications/Tabby.app"], origin_website_url: "https://github.com/Eugeny/tabby/releases/latest", origin_repo_url: "https://github.com/Eugeny/tabby/releases/latest" }),
  ...
  binaryApp({ id: "zed", name: "Zed", description: "Optional editor installed by setup when selected.", category: "editor", binary: "zed", version_args: ["--version"], install_path_refs: ["/Applications/Zed.app", "/opt/homebrew/bin/zed", "/usr/local/bin/zed"], origin_website_url: "https://zed.dev/download", origin_repo_url: "", maestro_install: false, maestro_update: false }),
  binaryApp({ id: "orbstack", name: "OrbStack", description: "Optional local container/VM runtime for macOS development.", category: "runtime", binary: "orb", version_args: ["version"], install_path_refs: ["/Applications/OrbStack.app", "/opt/homebrew/bin/orb"], origin_website_url: "https://orbstack.dev/", origin_repo_url: "", maestro_install: false, maestro_update: false }),
...
function binaryApp(input: BinaryAppInput): ManagedAppDefinition {
  const maestroInstall = input.maestro_install ?? true;
  const maestroUpdate = input.maestro_update ?? true;
  ...
  return {
    ...
    maestro_install: maestroInstall,
    maestro_update: maestroUpdate,
    latest_version_source: "binary",
    action_commands: maestroUpdate ? TOOL_ACTIONS : { install: "./setup.sh --non-interactive" },
  };
}

function setupScopeActions(scope: string): ManagedAppActionCommands {
  return {
    install: `maestro setup --scope ${scope}`,
    update: `maestro setup --scope ${scope}`,
    reinstall: `maestro setup --scope ${scope}`,
  };
}
...
function managedAppSummary(definition: ManagedAppDefinition, latestVersion: string, installedVersion: string): GuiManagedAppSummary {
  ...
  const latestVersionText = definition.latest_version_source === "maestro"
    ? latestVersion
  ...
  return {
    ...
    maestro_install: definition.maestro_install,
    maestro_update: definition.maestro_update,
...
function readManagedAppVersion(definition: ManagedAppDefinition, binaryPath: string | null, installedVersion: string): string {
  if (definition.latest_version_source === "maestro") {
