...
const appActionCommands: Record<string, Partial<Record<GuiAppActionId, string[]>>> = {
  maestro: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update"], reinstall: ["./setup.sh", "--non-interactive"] },
  agents: { install: ["maestro", "setup", "--scope", "agents"], update: ["maestro", "setup", "--scope", "agents"], reinstall: ["maestro", "setup", "--scope", "agents"] },
  "gui-desktop": { install: ["maestro", "setup", "--scope", "gui-desktop"], update: ["maestro", "setup", "--scope", "gui-desktop"], reinstall: ["maestro", "setup", "--scope", "gui-desktop"] },
  hooks: { install: ["maestro", "setup", "--scope", "hooks"], update: ["maestro", "setup", "--scope", "hooks"], reinstall: ["maestro", "setup", "--scope", "hooks"] },
  opencode: { install: ["maestro", "setup", "--scope", "opencode"], update: ["maestro", "setup", "--scope", "opencode"], reinstall: ["maestro", "setup", "--scope", "opencode"] },
  "opencode-cli": { install: ["maestro", "setup", "--scope", "opencode"], update: ["maestro", "setup", "--scope", "opencode"], reinstall: ["maestro", "setup", "--scope", "opencode"] },
  pulse: { install: ["maestro", "setup", "--scope", "pulse"], update: ["maestro", "setup", "--scope", "pulse"], reinstall: ["maestro", "setup", "--scope", "pulse"] },
  tabby: { install: ["maestro", "setup", "--scope", "tabby"], update: ["maestro", "setup", "--scope", "tabby"], reinstall: ["maestro", "setup", "--scope", "tabby"] },
  bun: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  cursor: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  fd: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  gh: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  glab: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  homebrew: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  node: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  ollama: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  orbstack: { install: ["./setup.sh", "--non-interactive"] },
  qlty: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  ripgrep: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  "ripgrep-all": { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  rtk: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  shellcheck: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
  shfmt: { install: ["./setup.sh", "--non-interactive"], update: ["maestro", "update-tools", "--update"] },
...
const pulseWorkerActionCommands: Record<GuiPulseWorkerActionId, { command: string[]; audit_ref: string; target_ref: string }> = {
  diagnose: { command: ["maestro", "pulse", "diagnose", "--gui", "--metadata-only"], audit_ref: "gui:pulse-workers:diagnose", target_ref: "selected event" },
  run_pulse: { command: ["maestro", "pulse", "run", "--scope", "gui"], audit_ref: "gui:pulse-workers:run-pulse", target_ref: "current filtered scope" },
  open_logs: { command: ["maestro", "pulse", "logs", "--metadata-only"], audit_ref: "gui:pulse-workers:open-logs", target_ref: "selected event evidence refs" },
  create_systemic_fix: { command: ["maestro", "task", "create", "--from-pulse", "--worker-ready"], audit_ref: "gui:pulse-workers:create-systemic-fix", target_ref: "selected event systemic-fix context" },
};
...
export function createGuiApiApp() {
  ...
  app.get("/api/health", (context) => {
    return context.json(createEnvelope({
      ...
      data: { status: "ok", service: "maestro-gui-api" },
    }));
  });
  ...
  app.get(TAMBO_PROXY_ROUTE_MANIFEST.route, (context) => {
    ...
    const workspaceRef = threadScopeRef(context.req.query("workspace_ref") ?? "maestro");
  ...
  app.post(APP_ACTION_ROUTE_MANIFEST.route, (context) => {
    ...
    return context.json(createEnvelope({
      operation_id: APP_ACTION_ROUTE_MANIFEST.operation_id,
      source: { surface: "apps", authority: "allowlisted local command runner", path_refs: ["setup.sh", "maestro.sh"] },
      data: job,
...
function startAppActionJob(appId: string, action: GuiAppActionId, command: string[]): GuiAppActionJobSummary {
  ...
  const child = spawn(command[0], command.slice(1), {
    cwd: process.cwd(),
    env: { ...process.env, MAESTRO_NON_INTERACTIVE: "true", CLICOLOR_FORCE: "1", FORCE_COLOR: "1", TERM: process.env.TERM ?? "xterm-256color" },
