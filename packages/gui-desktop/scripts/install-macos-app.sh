...
usage() {
  ...
  printf 'Environment: MAESTRO_GUI_DESKTOP_APP_DIR overrides the default app directory.\n'
  return 0
}
...
write_icon_assets() {
  ...
  local svg_path="${resources_dir}/maestro.svg"
  local png_path="${resources_dir}/maestro-source.png"
  local iconset_dir="${resources_dir}/maestro.iconset"

  cat > "$svg_path" <<'SVG'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" role="img" aria-labelledby="title desc">
  <title id="title">Maestro prompt icon</title>
...
SVG
  ...
  iconutil -c icns "$iconset_dir" -o "${resources_dir}/maestro.icns"
  rm -rf "$iconset_dir"
  rm -f "$png_path"
  return 0
}
...
write_service_helper() {
  ...
  local helper_path="${resources_dir}/maestro-gui-services.sh"
...
REPO_ROOT="${root}"
API_PORT="\${MAESTRO_GUI_API_PORT:-8787}"
WEB_PORT="\${MAESTRO_GUI_WEB_PORT:-5173}"
API_HEALTH_URL="http://127.0.0.1:\${API_PORT}/api/health"
WEB_HEALTH_URL="http://127.0.0.1:\${WEB_PORT}/"
LOG_DIR="\${HOME}/Library/Logs/maestro-gui"
...
notify() {
  local message="\$1"
  osascript -e "display notification \"\${message}\" with title \"maestro\"" >/dev/null || true
  return 0
}
...
require_bun() {
  ...
  if [[ -z "\${found}" ]]; then
    printf 'bun is required to launch maestro GUI; PATH=%s\n' "\${PATH}" >"\${LAUNCHER_LOG}"
    notify "maestro GUI cannot find bun. Install bun or add it to ~/.bun/bin."
    return 1
  fi
  BUN_BIN="\${found}"
  return 0
...
ensure_gui_dependencies() {
  ...
  if [[ ! -f "bun.lock" ]]; then
    ...
    notify "maestro GUI dependencies are missing and bun.lock was not found."
    return 1
  fi
  ...
  if ! "\${BUN_BIN}" install --frozen-lockfile >>"\${LAUNCHER_LOG}" 2>&1; then
    notify "maestro GUI dependency install failed. Check \${LAUNCHER_LOG}."
    return 1
  fi
  if ! gui_dependencies_present; then
    printf 'GUI dependencies are still missing after bun install in %s\n' "\${REPO_ROOT}" >>"\${LAUNCHER_LOG}"
    notify "maestro GUI dependencies are still missing after install."
    return 1
  fi
...
is_maestro_checkout() {
  ...
  [[ -f "\${candidate}/maestro.sh" && -f "\${candidate}/packages/gui-web/vite.config.ts" ]]
  return \$?
}
...
replace_stale_gui_on_port() {
  ...
  while IFS= read -r pid; do
    ...
    cwd="\$(pid_cwd "\${pid}" || true)"
    if [[ -n "\${cwd}" ]] && is_maestro_checkout "\${cwd}"; then
...
if [[ -f "\${HOME}/.maestro/agents/VERSION" && -f "\${REPO_ROOT}/VERSION" ]]; then
  INSTALLED_VERSION="\$(tr -d '\n' < "\${HOME}/.maestro/agents/VERSION")"
  RUNNING_VERSION="\$(tr -d '\n' < "\${REPO_ROOT}/VERSION")"
  if [[ "\${INSTALLED_VERSION}" != "\${RUNNING_VERSION}" ]]; then
    notify "Restart maestro GUI after update: installed \${INSTALLED_VERSION}, app \${RUNNING_VERSION}."
  fi
fi
...
if ! url_ready "\${API_HEALTH_URL}"; then
  nohup env MAESTRO_GUI_API_PORT="\${API_PORT}" "\${BUN_BIN}" run packages/gui-api/src/server.ts >"\${LOG_DIR}/api.log" 2>&1 &
fi
if ! url_ready "\${WEB_HEALTH_URL}"; then
...
if ! wait_for_url "\${API_HEALTH_URL}" 20 0.25; then
  printf 'maestro GUI API did not become ready. Check %s/api.log.\n' "\${LOG_DIR}" >"\${LAUNCHER_LOG}"
  notify "maestro GUI API did not become ready. Check \${LOG_DIR}/api.log."
  exit 1
fi
if ! wait_for_url "\${WEB_HEALTH_URL}" 30 0.25; then
  printf 'maestro GUI web did not become ready. Check %s/web.log.\n' "\${LOG_DIR}" >"\${LAUNCHER_LOG}"
  notify "maestro GUI web did not become ready. Check \${LOG_DIR}/web.log."
  exit 1
fi

printf 'maestro GUI services ready: API %s, web %s\n' "\${API_HEALTH_URL}" "\${WEB_HEALTH_URL}" >"\${LAUNCHER_LOG}"
exit 0
LAUNCHER
...
write_webview_source() {
  local resources_dir="$1"
  local swift_source="${resources_dir}/maestro-gui.swift"
...
final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, NSMenuItemValidation, NSWindowDelegate {
    ...
    private let apiPort = ProcessInfo.processInfo.environment["MAESTRO_GUI_API_PORT"] ?? "8787"
    private let webPort = ProcessInfo.processInfo.environment["MAESTRO_GUI_WEB_PORT"] ?? "5173"
    private let defaultAccentHue: CGFloat = 191
    private let mainWindowFrameAutosaveName = "maestro-main-window"
    private let titlebarHeight: CGFloat = 24
    private let serviceQueue = DispatchQueue(label: "sh.maestro.gui.services")
    ...
    private func configureMenu() {
        ...
        helpMenu.addItem(menuItem("maestro Help", action: #selector(showHelp(_:)), key: "?", modifiers: [.command, .shift], target: self))
    ...
    @objc private func showAbout(_ sender: Any?) {
        ...
        let title = aboutLabel("maestro", font: .systemFont(ofSize: 28, weight: .bold), color: .labelColor)
        let versionLabel = aboutLabel("Version \(version)", font: .systemFont(ofSize: 13, weight: .semibold), color: .secondaryLabelColor)
        let detail = aboutLabel("Native macOS WebKit wrapper for the local read-only maestro GUI. It provides AI-assisted development workflows, code quality, and deployment automation without exposing secrets or shell write routes.", font: .systemFont(ofSize: 13), color: .labelColor)
        let copyright = aboutLabel("Copyright © 2025-2026 Aditya Pandey and Harvest. Licensed under the MIT License.", font: .systemFont(ofSize: 12), color: .secondaryLabelColor)
    ...
    @objc private func openWebsite(_ sender: Any?) {
        openExternalURL("https://maestro.sh")
    }

    @objc private func openGitHub(_ sender: Any?) {
        openExternalURL("https://github.com/Aditya060806/Maestro")
    }
    ...
    @objc private func showHelp(_ sender: Any?) {
        window.makeKeyAndOrderFront(nil)
        NSApp.activate(ignoringOtherApps: true)
        loadStatusHTML(title: "maestro Help", detail: "Use the app menus for standard editing, view controls, reload/navigation, window management, and help. The desktop wrapper runs the local read-only maestro GUI in WebKit.")
    }
    ...
    private func makeTitlebarOverlay() -> NSView {
        ...
        cameraButton.toolTip = "Capture an maestro screenshot"
    ...
    @objc private func toggleMachineRail(_ sender: Any?) {
        webView.evaluateJavaScript("window.dispatchEvent(new CustomEvent('maestro:toggle-machine-rail')); true;", completionHandler: nil)
    }
    ...
    private func saveScreenshot(image: NSImage, nameComponent: String) {
        ...
        do {
            ...
            let fileURL = directory.appendingPathComponent("maestro-app-screenshot-\(screenshotTimestamp())-\(sanitizeFilenameComponent(nameComponent)).png")
    ...
    private func notifyScreenshotCaptured(fileURL: URL) {
        ...
        let script = "window.dispatchEvent(new CustomEvent('maestro:screenshot-captured', { detail: \(payload) })); true;"
        ...
        serviceQueue.async {
            ...
            DispatchQueue.main.async {
                if result.ok {
                    ...
                    self.loadStatusHTML(title: "maestro could not start", detail: result.message)
                }
            }
        }
    }
    ...
    private func runServiceHelper(arguments: [String]) -> (ok: Bool, message: String) {
        guard let helperPath = Bundle.main.path(forResource: "maestro-gui-services", ofType: "sh") else {
            return (false, "The bundled service helper is missing from maestro.app.")
        }
        ...
        environment["MAESTRO_GUI_WRAPPER"] = "webkit"
        ...
        do {
            ...
            if output.isEmpty {
                return (false, "The local GUI services exited with status \(process.terminationStatus). Check ~/Library/Logs/maestro-gui/.")
            }
            return (false, output)
        } catch {
    ...
    private func loadAppURL() {
        guard let url = URL(string: "http://127.0.0.1:\(webPort)/?desktop=macos") else {
            loadStatusHTML(title: "Invalid maestro URL", detail: "The configured web port is not valid: \(webPort)")
            return
        }
    ...
    private func loadStatusHTML(title: String, detail: String) {
        ...
        if title == "Starting maestro" {
            contentHTML = """
            <main class="shell" aria-busy="true" aria-label="Loading maestro interface">
              <section class="brand" aria-label="Starting maestro"><strong><svg viewBox="0 0 1024 1024" aria-hidden="true"><rect x="88" y="88" width="848" height="848" rx="188" fill="#000000"/><rect x="88" y="88" width="848" height="848" rx="188" fill="#66d9f2" opacity="0.22"/><rect x="116" y="116" width="792" height="792" rx="160" fill="none" stroke="#66d9f2" stroke-opacity="0.36" stroke-width="18"/><g transform="translate(512 522) scale(0.94) translate(-288 -256)"><path fill="#8ce8ff" d="M9.4 86.6C-3.1 74.1-3.1 53.9 9.4 41.4s32.8-12.5 45.3 0l192 192c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L178.7 256 9.4 86.6zM256 416H544c17.7 0 32 14.3 32 32s-14.3 32-32 32H256c-17.7 0-32-14.3-32-32s14.3-32 32-32z"/></g></svg><span>maestro</span><b>&gt;</b><em>_</em></strong><small>Preparing local GUI</small></section>
...
compile_webview_app() {
  ...
  env MACOSX_DEPLOYMENT_TARGET=13.0 swiftc -framework AppKit -framework WebKit "$swift_source" -o "${macos_dir}/maestro-gui"
  chmod 755 "${macos_dir}/maestro-gui"
  return 0
}
...
write_app_bundle() {
  ...
  rm -f "${resources_dir}/maestro.icns" "${resources_dir}/maestro.svg"
...
<dict>
  ...
  <key>CFBundleIdentifier</key><string>sh.maestro.gui</string>
  <key>CFBundleVersion</key><string>${version}</string>
  <key>CFBundleShortVersionString</key><string>${version}</string>
  <key>CFBundleExecutable</key><string>maestro-gui</string>
...
PLIST
  ...
  touch "${resources_dir}/maestro.icns" "${contents_dir}/Info.plist" "$app_path"
...
main() {
  local mode="install"
  local app_dir="${MAESTRO_GUI_DESKTOP_APP_DIR:-$DEFAULT_APP_DIR}"
  local root=""
