...
export const pulseWorkersFixture: GuiPulseWorkerSummary = {
  ...
  kpis: [
    ...
    { id: "systemic-fixes", label: "Systemic fixes created", value: "9", period_label: "Trailing 7d", scope_label: "maestro repo", comparison_label: "up 3 vs prior week", sample_size: 9, status: "completed", detail: "Blindspots become worker-ready follow-up tasks when patterns repeat." },
  ],
  attention: [
  ...
  insights: [
    { id: "insight-community-waiting", kind: "third_party_waiting", severity: "warning", title: "Third-party issues waiting", detail: "1 community-origin issue needs triage before dispatch.", likely_cause: "Community reports require collaborator trust-boundary handling and may need maintainer decisions.", evidence_refs: ["Aditya060806/Maestro", "#25876"], event_refs: ["event:community-25876"], confidence: "high", recommendation: "Create a systemic triage task with first-response thresholds and non-collaborator safety checks.", metric_label: "community waiting count", period_label: pulsePeriod, scope_label: pulseScope, comparison_label: "previous 24h baseline", sample_size: 1, primary_action: "create_systemic_fix" },
  ...
  filters: {
    repos: [{ id: "repo:Aditya060806/Maestro", label: "Aditya060806/Maestro", count: 184 }],
    ...
    issue_origins: [{ id: "origin_interactive", label: "Interactive origin", count: 8 }, { id: "maestro_created", label: "maestro-created", count: 9 }, { id: "third_party", label: "Third-party/community", count: 3 }],
  ...
  actions: [
    { id: "diagnose", label: "Diagnose", enabled: true, classification: "read", confirmation: "none", command_preview: "maestro pulse diagnose --gui --metadata-only", target_ref: "selected event", scope_copy: "Runs read-only diagnostics for the selected Pulse run, worker session, issue, or PR metadata.", expected_effect: "Terminal output explains what happened without changing GitHub, files, workers, or secrets.", audit_ref: "gui:pulse-workers:diagnose" },
    { id: "run_pulse", label: "Run Pulse now", enabled: true, classification: "write", confirmation: "required", command_preview: "maestro pulse run --scope gui", target_ref: "current filtered scope", scope_copy: "Starts a scoped Pulse run through the existing Pulse helper only.", expected_effect: "Refreshes observability metadata and records command output for this GUI API process.", audit_ref: "gui:pulse-workers:run-pulse" },
    { id: "open_logs", label: "Open logs/transcript", enabled: true, classification: "read", confirmation: "none", command_preview: "maestro pulse logs --metadata-only", target_ref: "selected event evidence refs", scope_copy: "Opens allowlisted metadata previews for logs or transcripts; arbitrary filesystem paths are not accepted.", expected_effect: "Shows redacted references and previews only.", audit_ref: "gui:pulse-workers:open-logs" },
    { id: "create_systemic_fix", label: "Create systemic fix task", enabled: true, classification: "write", confirmation: "required", command_preview: "maestro task create --from-pulse --worker-ready", target_ref: "selected event systemic-fix context", scope_copy: "Creates a worker-ready follow-up from the selected event using maestro task/issue wrappers.", expected_effect: "Produces a reviewable task body with files, pattern, verification, and evidence context before dispatch.", audit_ref: "gui:pulse-workers:create-systemic-fix" },
  ],
};

export const statusFixture: GuiStatusData = {
  maestro_version: "unknown",
  update: {
    running_version: "unknown",
    installed_version: "unknown",
    restart_required: false,
    message: "The GUI app is using the latest installed maestro version.",
  },
  runtime: {
    host: "local",
    api: "hono",
  ...
  paths: [
    ...
    {
      label: "settings",
      path_ref: "~/.config/maestro/settings.json",
      health: "unchecked",
    },
  ],
  helper_availability: [
  ...
  navigation: [
    ...
    ["config", "Config", "Read-only file explorer for maestro config."],
  ...
  repos: {
    path_ref: "~/.config/maestro/repos.json",
    health: "unchecked",
    total: 0,
    repos: [],
  },
  ...
  oauth_pool: {
    path_ref: "~/.maestro/oauth-pool.json",
    health: "unchecked",
  ...
  ai_apps: [
    {
      name: "OpenCode",
      status: "unchecked",
      app_path_ref: "~/Applications/OpenCode Maestro.app",
  ...
  vault: {
    ...
    path_ref: "~/.config/maestro/vault",
    value_policy: "metadata_only_no_secret_material",
    tooltip: "Encrypted by maestro Vault; contents visible only when unlocked through app or authorised vault commands.",
    unlock_hint: "Run `maestro vault unlock` in a local terminal and enter the passphrase only into the hidden prompt.",
    setup_hint: "Run `maestro vault init` locally, verify the restart test, then unlock before protected previews or write actions are available.",
    readiness: {
...
export const fileExplorerFixture: GuiFileExplorerData = {
  ...
  current_path_ref: "~/.maestro/agents",
  ...
  entries: [
    {
      name: "workflows",
      kind: "directory",
      path_ref: "~/.maestro/agents/workflows",
    ...
    },
    {
      name: "AGENTS.md",
      kind: "file",
      path_ref: "~/.maestro/agents/AGENTS.md",
