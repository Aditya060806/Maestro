  ...
  return greetingOutput
    ...
    .flatMap((line) => {
      ...
      return [notification({
        ...
        source_ref: "~/.maestro/cache/session-greeting.txt",
...
function buildGuiStateNotifications(input: BuildNotificationInput): GuiNotificationSummary[] {
  ...
  if (input.restartRequired) {
    notifications.push(notification({
      ...
      message: "maestro has updated in the background. Restart the GUI app to use the latest installed version.",
      severity: "warning",
      category: "release",
      source: "gui-status",
      source_ref: "VERSION and ~/.maestro/agents/VERSION",
      status: "active",
      actions: [surfaceAction("open-installation", "Open installation", "installation"), commandAction("restart-gui", "Restart GUI app", "Quit and reopen the maestro GUI app")],
    }));
  }
  ...
  if (setupNeedsUpdate.length > 0 || appNeedsUpdate.length > 0) {
    notifications.push(notification({
      ...
      message: `${setupNeedsUpdate.length} setup target(s) and ${appNeedsUpdate.length} AI app target(s) need the current maestro files.`,
      severity: "warning",
      category: "setup",
      source: "gui-status",
      source_ref: "setup_targets and ai_apps",
      status: "active",
      actions: [surfaceAction("open-installation", "Open installation", "installation"), commandAction("run-update", "Run update", "maestro update")],
    }));
  }
  ...
  for (const provider of authErrors) {
    notifications.push(notification({
      ...
      status: "active",
      actions: [surfaceAction("open-ai-providers", "Open AI providers", "aiProviders"), commandAction("check-oauth-pool", "Check provider pool", `maestro oauth-pool check ${provider.provider}`)],
    }));
  }
...
function titleForToastLine(line: string, severity: GuiNotificationSeverity): string {
  ...
  return rule?.title ?? (severity === "info" ? "Runtime status" : "maestro status");
}
...
function actionsForToastLine(line: string, category: GuiNotificationCategory, severity: GuiNotificationSeverity): GuiNotificationAction[] {
  ...
  if (severity === "warning") {
    return [surfaceAction("open-installation", "Open installation", "installation"), commandAction("run-update", "Run update", "maestro update")];
  }
