...
# The command -v getent guard MUST be present — omitting it crashes maestro.sh
# under `set -euo pipefail` on any BSD system and breaks sudo maestro approve.
...
if [[ -n "${SUDO_USER:-}" && "$(id -u)" -eq 0 ]]; then
	...
	if [[ -n "$_tmp_real_home" ]]; then
		_MAESTRO_REAL_HOME="$_tmp_real_home"
	fi
fi
INSTALL_DIR="$_MAESTRO_REAL_HOME/Git/maestro"
_MAESTRO_SOURCE_PATH="${BASH_SOURCE[0]}"
_MAESTRO_SOURCE_DIR="${_MAESTRO_SOURCE_PATH%/*}"
[[ "$_MAESTRO_SOURCE_DIR" == "$_MAESTRO_SOURCE_PATH" ]] && _MAESTRO_SOURCE_DIR="."
_MAESTRO_SOURCE_DIR="$(cd "$_MAESTRO_SOURCE_DIR" 2>/dev/null && pwd)" || _MAESTRO_SOURCE_DIR=""
if [[ -n "$_MAESTRO_SOURCE_DIR" && -L "$_MAESTRO_SOURCE_PATH" ]]; then
	_MAESTRO_LINK_TARGET="$(readlink "$_MAESTRO_SOURCE_PATH" 2>/dev/null || true)"
	if [[ -n "$_MAESTRO_LINK_TARGET" ]]; then
		[[ "$_MAESTRO_LINK_TARGET" != /* ]] && _MAESTRO_LINK_TARGET="$_MAESTRO_SOURCE_DIR/$_MAESTRO_LINK_TARGET"
		_MAESTRO_LINK_DIR="${_MAESTRO_LINK_TARGET%/*}"
		_MAESTRO_LINK_DIR="$(cd "$_MAESTRO_LINK_DIR" 2>/dev/null && pwd)" || _MAESTRO_LINK_DIR=""
		[[ -n "$_MAESTRO_LINK_DIR" ]] && _MAESTRO_SOURCE_DIR="$_MAESTRO_LINK_DIR"
	fi
fi
if [[ -n "$_MAESTRO_SOURCE_DIR" && -f "$_MAESTRO_SOURCE_DIR/.agents/scripts/maestro-cli/maestro-repos-lib.sh" ]]; then
	INSTALL_DIR="$_MAESTRO_SOURCE_DIR"
fi
unset _MAESTRO_SOURCE_PATH _MAESTRO_SOURCE_DIR _MAESTRO_LINK_TARGET _MAESTRO_LINK_DIR
AGENTS_DIR="$_MAESTRO_REAL_HOME/.maestro/agents"
CONFIG_DIR="$_MAESTRO_REAL_HOME/.config/maestro"
...
get_remote_version() {
	...
	if command -v jq &>/dev/null; then
		version=$(curl -fsSL "https://api.github.com/repos/Aditya060806/Maestro/contents/VERSION" 2>/dev/null | jq -r '.content // empty' 2>/dev/null | base64 -d 2>/dev/null | tr -d '\n')
		if [[ -n "$version" ]]; then
			echo "$version"
			return 0
		fi
	...
	curl -fsSL "https://raw.githubusercontent.com/Aditya060806/Maestro/main/VERSION" 2>/dev/null || echo "unknown"
}
...
# INSTALL_DIR is the canonical location of maestro.sh (set above). Using
# INSTALL_DIR rather than BASH_SOURCE[0] preserves installed symlink support:
# /usr/local/bin/maestro → $INSTALL_DIR/maestro.sh would otherwise resolve
# BASH_SOURCE[0] to /usr/local/bin instead of the checkout/deployed tree.
MAESTRO_CLI_MODULES_DIR="${INSTALL_DIR}/.agents/scripts/maestro-cli"
# shellcheck source=.agents/scripts/maestro-cli/maestro-repos-lib.sh
# shellcheck disable=SC1091  # module path resolved at runtime via $INSTALL_DIR
source "${MAESTRO_CLI_MODULES_DIR}/maestro-repos-lib.sh"
# shellcheck source=.agents/scripts/maestro-cli/maestro-init-lib.sh
# shellcheck disable=SC1091
source "${MAESTRO_CLI_MODULES_DIR}/maestro-init-lib.sh"
# shellcheck source=.agents/scripts/maestro-cli/maestro-skills-plugin-lib.sh
# shellcheck disable=SC1091
source "${MAESTRO_CLI_MODULES_DIR}/maestro-skills-plugin-lib.sh"
# shellcheck source=.agents/scripts/maestro-cli/maestro-status-lib.sh
# shellcheck disable=SC1091
source "${MAESTRO_CLI_MODULES_DIR}/maestro-status-lib.sh"
# shellcheck source=.agents/scripts/maestro-cli/maestro-update-lib.sh
# shellcheck disable=SC1091
source "${MAESTRO_CLI_MODULES_DIR}/maestro-update-lib.sh"
# shellcheck source=.agents/scripts/maestro-cli/maestro-upgrade-planning-lib.sh
# shellcheck disable=SC1091
source "${MAESTRO_CLI_MODULES_DIR}/maestro-upgrade-planning-lib.sh"
...
cmd_update() {
	...
	print_header "Updating Maestro Framework"
	echo ""
	...
	if check_dir "$INSTALL_DIR/.git"; then
		...
		if [[ "$local_hash" == "$remote_hash" ]]; then
			...
			deployed_version=$(cat "$HOME/.maestro/agents/VERSION" 2>/dev/null || echo "none")
			if [[ "$repo_version" != "$deployed_version" ]]; then
				...
				# maestro-update-check.sh already carrying the same check).
				# Docs-only drift is intentionally skipped — no runtime impact.
				local stamp_file="$HOME/.maestro/.deployed-sha"
				if [[ -f "$stamp_file" ]]; then
					...
					if [[ -n "$deployed_sha" && "$deployed_sha" != "$local_hash" ]]; then
						...
						# and maestro.sh itself, which are deployed/sourced by setup).
						if git -C "$INSTALL_DIR" diff --name-only "$deployed_sha" "$local_hash" -- \
							...
							setup.sh .agents/scripts/setup/modules/ maestro.sh 2>/dev/null | grep -q .; then
							has_code_drift=1
						fi
	...
	# auto-update daemon itself, which sets MAESTRO_AUTO_UPDATE=1 in its
	...
	if [[ -t 0 ]] && [[ -z "${MAESTRO_AUTO_UPDATE:-}" ]] && [[ "${NON_INTERACTIVE:-false}" != "true" ]]; then
	...
	# install the fix without a new release still get migrated on next 'maestro update'.
...
# Side effect: writes ~/.maestro/advisories/daemon-disabled.advisory when
...
_uninstall_cleanup_refs() {
	...
	for file in "${ai_agent_files[@]}"; do
		if check_file "$file"; then
			grep -q "Add ~/.maestro/agents/AGENTS.md" "$file" 2>/dev/null && {
				rm -f "$file"
...
# Uninstall command
cmd_uninstall() {
	print_header "Uninstall Maestro Framework"
	echo ""
	...
	check_dir "$HOME/.maestro" && {
		print_info "Removing $HOME/.maestro..."
		rm -rf "$HOME/.maestro"
		print_success "Removed maestro config directory"
	}
	_uninstall_cleanup_refs
	echo ""
	...
	echo "  npm install -g maestro && maestro update"
	echo "  OR: brew install Aditya060806/tap/maestro && maestro update"
}
...
cmd_features() {
	print_header "Maestro Features"
	echo ""

	echo "Available features for 'maestro init':"
	echo ""
	...
	echo "                 - Re-run anytime: maestro security audit"
	echo ""
	echo "Extensibility:"
	echo ""
	echo "  plugins        Third-party agent plugins (configured in .maestro.json)"
	echo "                 - Git repos deployed to ~/.maestro/agents/<namespace>/"
	echo "                 - Namespaced to avoid collisions with core agents"
	echo "                 - Enable/disable per-plugin without removal"
	echo "                 - See: .agents/maestro/plugins.md"
	echo ""
	echo "Usage:"
	echo "  maestro init                    # Enable all features (except sops)"
	echo "  maestro init planning           # Enable only planning"
	echo "  maestro init sops               # Enable SOPS encryption"
	echo "  maestro init security           # Enable security posture checks"
	echo "  maestro init beads              # Enable beads (includes planning)"
	echo "  maestro init database           # Enable only database"
	echo "  maestro init planning,security  # Enable multiple"
	echo ""
}
...
cmd_update_tools() {
	...
	if [[ ! -f "$tool_check_script" ]]; then
		...
		print_info "Run 'maestro update' first to get the latest scripts"
		return 1
	fi
...
_repos_list() {
	print_header "Registered Maestro Projects"
	echo ""
	init_repos_file
	...
	if [[ "$count" == "0" ]]; then
		...
		echo ""
		echo "Initialize a project with: maestro init"
		return 0
	fi
	local current_ver
...
_repos_add() {
	...
	[[ ! -f "$project_root/.maestro.json" ]] && {
		print_error "No .maestro.json found - run 'maestro init' first"
		return 1
	}
	local version features
	if command -v jq &>/dev/null; then
		version=$(jq -r '.version' "$project_root/.maestro.json" 2>/dev/null || echo "unknown")
		features=$(jq -r '[.features | to_entries[] | select(.value == true) | .key] | join(",")' "$project_root/.maestro.json" 2>/dev/null || echo "")
	else
		version="unknown"
		features=""
	fi
...
cmd_repos() {
	...
	clean) _repos_clean ;;
	*)
		echo "Usage: maestro repos <command>"
		echo ""
		echo "Commands:"
...
# Detect command - check for unregistered maestro repos
cmd_detect() {
	print_header "Detecting Maestro Projects"
	echo ""
	...
	if [[ -n "$unregistered" ]]; then
		print_info "Found unregistered maestro project:"
		echo "  $unregistered"
		echo ""
		...
		if [[ "$response" =~ ^[Yy]$ ]]; then
			local version features
			if command -v jq &>/dev/null; then
				version=$(jq -r '.version' "$unregistered/.maestro.json" 2>/dev/null || echo "unknown")
				features=$(jq -r '[.features | to_entries[] | select(.value == true) | .key] | join(",")' "$unregistered/.maestro.json" 2>/dev/null || echo "")
			else
				version="unknown"
				features=""
			fi
	...
	print_info "Scanning for maestro projects in ~/Git/..."

	local found=0
	local to_register=()

	if [[ -d "$HOME/Git" ]]; then
		while IFS= read -r -d '' maestro_json; do
			local repo_dir
			repo_dir=$(dirname "$maestro_json")
		...
		done < <(find "$HOME/Git" -maxdepth 3 -name ".maestro.json" -print0 2>/dev/null)
	fi

	if [[ $found -eq 0 ]]; then
		print_success "No unregistered maestro projects found"
		return 0
	fi
	...
	if [[ "$response" =~ ^[Yy]$ ]]; then
		for repo in "${to_register[@]}"; do
			local version features
			if command -v jq &>/dev/null; then
				version=$(jq -r '.version' "$repo/.maestro.json" 2>/dev/null || echo "unknown")
				features=$(jq -r '[.features | to_entries[] | select(.value == true) | .key] | join(",")' "$repo/.maestro.json" 2>/dev/null || echo "")
			else
				version="unknown"
				features=""
			fi
...
_help_commands() {
	echo "Commands:"
	echo "  init [features]    Initialize maestro in current project"
	...
	echo "  update             Update maestro to the latest version (alias: upgrade)"
	...
	echo "  opencode [args]    Launch OpenCode with maestro per-session DB isolation"
	echo "  opencode-desktop   Launch/install OpenCode Desktop with maestro DB isolation"
	...
	echo "  detect             Find and register maestro projects"
	echo "  uninstall          Remove maestro from your system"
...
_help_detailed_sections() {
	echo "Security:"
	echo "  maestro security            # Run ALL checks (posture + hygiene + supply chain)"
	echo "  maestro security posture    # Interactive security posture setup (gopass, gh, SSH)"
	echo "  maestro security status     # Combined posture + hygiene summary"
	echo "  maestro security scan       # Secret hygiene & supply chain scan"
	echo "  maestro security scan-pth   # Python .pth file audit (supply chain IoC)"
	echo "  maestro security scan-secrets # Plaintext credential locations"
	echo "  maestro security scan-deps  # Unpinned dependency check"
	echo "  maestro security supply-chain scan [path] # npm supply-chain IOC scan"
	echo "  maestro security check      # Per-repo security posture assessment"
	echo "  maestro security dismiss <id> # Dismiss a security advisory"
	echo ""
	echo "IP Reputation:"
	echo "  maestro ip-check check <ip> # Check IP reputation across providers"
	echo "  maestro ip-check batch <f>  # Batch check IPs from file"
	echo "  maestro ip-check report <ip># Generate markdown report"
	echo "  maestro ip-check providers  # List available providers"
	echo "  maestro ip-check cache-stats# Show cache statistics"
	echo ""
	echo "Model Accounts Pool (OAuth):"
	echo "  maestro model-accounts-pool status            # Pool health at a glance"
	echo "  maestro model-accounts-pool list              # Per-account detail"
	echo "  maestro model-accounts-pool check             # Live token validity test"
	echo "  maestro model-accounts-pool diagnose          # Full pipeline diagnostics (pool, plugin, CCH, runtime)"
	echo "  maestro model-accounts-pool rotate [provider] # Switch to next available account NOW (use when rate-limited)"
	echo "  maestro model-accounts-pool reset-cooldowns   # Clear rate-limit cooldowns"
	echo "  maestro model-accounts-pool add anthropic     # Add Claude Pro/Max account"
	echo "  maestro model-accounts-pool add openai        # Add ChatGPT Plus/Pro account"
	echo "  maestro model-accounts-pool add cursor        # Add Cursor Pro account"
	echo "  maestro model-accounts-pool add google        # Add Google AI Pro/Ultra/Workspace account"
	echo "  maestro model-accounts-pool import claude-cli # Import from Claude CLI auth"
	echo "  maestro model-accounts-pool assign-pending <provider># Assign stranded token"
	echo "  maestro model-accounts-pool remove <provider> <email># Remove an account"
	echo ""
	echo "  Auth troubleshooting (run diagnose first, then recovery if needed):"
	echo "    maestro model-accounts-pool diagnose        # 0. Full pipeline check (start here)"
	echo "    maestro model-accounts-pool status          # 1. Check pool health"
	echo "    maestro model-accounts-pool check           # 2. Test token validity"
	echo "    maestro model-accounts-pool rotate anthropic# 3. Switch account if rate-limited"
	echo "    maestro model-accounts-pool reset-cooldowns # 4. Clear cooldowns if all stuck"
	echo "    maestro model-accounts-pool add anthropic   # 5. Re-add if pool empty"
	echo ""
	echo "Client Format:"
	echo "  maestro client-format               # Show status + cached constants"
	echo "  maestro client-format extract        # Re-extract constants from installed CLI"
	echo "  maestro client-format check          # Verify cache matches installed CLI version"
	echo "  maestro client-format canary         # Run drift check against real CLI (uses tokens)"
	echo "  maestro client-format monitor [cmd]  # Traffic capture + diff (requires mitmproxy)"
	echo "  maestro client-format install-canary # Install daily drift check (launchd)"
	echo ""
	echo "  If requests stop working after a CLI update:"
	echo "    maestro client-format extract      # 1. Re-extract constants"
	echo "    maestro client-format canary       # 2. Verify against real CLI"
	echo ""
	echo "Secrets:"
	echo "  maestro secret set NAME     # Store a secret (hidden input)"
	echo "  maestro secret list         # List secret names (never values)"
	echo "  maestro secret run CMD      # Run with secrets injected + redacted"
	echo "  maestro secret init         # Initialize gopass encrypted store"
	echo "  maestro secret import       # Import from credentials.sh to gopass"
	echo "  maestro secret status       # Show backend status"
	echo ""
	echo "Vault:"
	echo "  maestro vault init          # Create local encrypted Vault metadata"
	echo "  maestro vault status        # Show uninitialized/locked/unlocked/corrupted"
	echo "  maestro vault unlock        # Unlock into an in-memory local broker"
	echo "  maestro vault lock          # Stop broker and forget in-memory keys"
	echo "  maestro vault read NAME     # Read protected data through broker"
	echo "  maestro vault update NAME   # Encrypt stdin value through broker"
	echo ""
	echo "GitHub App Auth:"
	echo "  maestro github-app-auth status --json       # Show active auth mode and budgets"
	echo "  maestro github-app-auth route issue-list    # Explain route decision"
	echo "  maestro github-app-auth rate-limit --json   # Show cached per-pool budgets"
	echo ""
	echo "Feature Toggles:"
	echo "  maestro config list         # List all toggles with current values"
	echo "  maestro config get <key>    # Get a toggle value"
	echo "  maestro config set <k> <v>  # Set a toggle (true/false)"
	echo "  maestro config reset [key]  # Reset toggle(s) to defaults"
	echo "  maestro config path         # Show config file path"
	echo ""
	echo "Knowledge Plane:"
	echo "  maestro knowledge init repo           # Provision _knowledge/ in current repo"
	echo "  maestro knowledge init personal       # Provision at ~/.maestro/.agent-workspace/knowledge/"
	echo "  maestro knowledge init off            # Disable knowledge plane"
	echo "  maestro knowledge status              # Show provisioning state"
	echo "  maestro knowledge provision [path]    # Re-provision (idempotent)"
	echo "  maestro knowledge add <file|url>      # Ingest file or URL into sources/"
	echo "  maestro knowledge list [--state s] [--kind k]  # List all known sources"
	echo "  maestro knowledge search <query>      # Search sources (grep fallback)"
	echo ""
	echo "Design Systems:"
	echo "  maestro design detect [path]          # Detect whether a repo has a GUI/interface"
	echo "  maestro design scaffold [path]        # Create DESIGN.md skeleton when missing"
	echo "  maestro design guidelines [path] --pdf # Generate brand guideline HTML/PDF exports"
	echo "  maestro design survey [--json]        # Audit owned initialized GUI repos"
	echo "  maestro design issues --apply         # File auto-dispatch issues for missing design artifacts"
	echo ""
	echo "Campaign Plane:"
	echo "  maestro campaign init [<path>]          # Provision _campaigns/ directory contract (P1)"
	echo "  maestro campaign provision [<path>]     # Re-provision / repair (idempotent, P1)"
	echo "  maestro campaign ls [--active|--launched|--all] [<path>]  # Directory listing (P1)"
	echo "  maestro campaign new <name> [--channel <ch>]  # Scaffold active/<id>/ (P2)"
	echo "  maestro campaign list                         # Show all campaigns (P2)"
	echo "  maestro campaign status <id>                  # Detailed dossier for a campaign (P2)"
	echo "  maestro campaign archive <id>                 # Move launched/<id> → archive/ (P2)"
	echo "  maestro campaign asset add <file> [--target lib-brand|lib-swipe|campaign]  # Ingest asset (P4)"
	echo "  maestro campaign asset list [--type image|video|audio|pdf|all]             # List assets (P4)"
	echo "  maestro campaign asset preview <file> [--size 640]                         # Generate preview PNG (P4)"
	echo "  maestro campaign asset manifest <asset-id>                                 # Show asset manifest entry (P4)"
	echo "  maestro campaign draft <id> --channel <ch>    # AI-generated content draft (P5)"
	echo "  maestro campaign launch <id>                  # Move active/<id> → launched/, create templates (P6)"
	echo "  maestro campaign promote <id> [--results|--learnings] # Cross-plane promotion (P6)"
	echo "  maestro campaign feedback [<id>]              # Surface _feedback/ insights for research (P6)"
	echo ""
	echo "LLM Stats:"
	echo "  maestro stats               # Show usage summary (last 30 days)"
	echo "  maestro stats summary       # Overall usage summary"
	echo "  maestro stats models        # Per-model breakdown"
	echo "  maestro stats projects      # Per-project breakdown"
	echo "  maestro stats costs         # Cost analysis with category breakdown"
	echo "  maestro stats trend         # Usage trends over time"
	echo "  maestro stats ingest        # Parse new Claude JSONL log entries"
	echo "  maestro stats sync-budget   # Sync to budget tracker (t1100)"
	echo ""
	echo "Supervisor Circuit Breaker (t1331):"
	echo "  maestro circuit-breaker             # Show breaker state (alias: cb)"
	echo "  maestro circuit-breaker status      # Show breaker state"
	echo "  maestro circuit-breaker reset       # Manually reset (resumes worker dispatch)"
	echo "  maestro circuit-breaker check       # Exit 0 if dispatch allowed, 1 if paused"
	echo "  maestro circuit-breaker trip        # Manually trip (testing)"
	echo "  maestro cb reset                    # Short alias for reset"
	echo ""
	echo "  When the breaker trips (auto-filed GH issue), copy/paste:"
	echo "    maestro circuit-breaker reset"
	echo ""
...
_help_management_sections() {
	echo "Auto-Update:"
	echo "  maestro auto-update enable  # Poll for updates every 10 min"
	echo "  maestro auto-update disable # Stop auto-updating"
	echo "  maestro auto-update status  # Show auto-update state"
	echo "  maestro auto-update check   # One-shot check and update now"
	echo ""
	echo "Repo Sync:"
	echo "  maestro repo-sync enable    # Enable daily git pull for repos"
	echo "  maestro repo-sync disable   # Disable daily sync"
	echo "  maestro repo-sync status    # Show sync state and last results"
	echo "  maestro repo-sync check     # One-shot sync all repos now"
	echo "  maestro repo-sync dirs list # List configured parent directories"
	echo "  maestro repo-sync dirs add  # Add a parent directory"
	echo "  maestro repo-sync dirs rm   # Remove a parent directory"
	echo "  maestro repo-sync config    # Show/edit configuration"
	echo "  maestro repo-sync logs      # View sync logs"
	echo ""
	echo "Agent Sources (private repos):"
	echo "  maestro sources add <path>  # Add a local repo as agent source"
	echo "  maestro sources add-remote <url> # Clone and add remote repo"
	echo "  maestro sources remove <n>  # Remove a source (keeps agents)"
	echo "  maestro sources list        # List configured sources"
	echo "  maestro sources status      # Show sync status"
	echo "  maestro sources sync        # Sync all sources to custom/"
	echo ""
	echo "Plugins:"
	echo "  maestro plugin add <url>    # Install a plugin from git repo"
	echo "  maestro plugin list         # List installed plugins"
	echo "  maestro plugin update       # Update all plugins"
	echo "  maestro plugin remove <n>   # Remove a plugin"
	echo ""
	echo "Skill Management:"
	echo "  maestro skill add <source>  # Import a skill from GitHub"
	echo "  maestro skill list          # List imported skills"
	echo "  maestro skill check         # Check for upstream updates"
	echo "  maestro skill update [name] # Update skills to latest"
	echo "  maestro skill remove <name> # Remove an imported skill"
	echo ""
	echo "Skill Discovery:"
	echo "  maestro skills search <q>   # Search skills by keyword"
	echo "  maestro skills browse       # Browse skills by category"
	echo "  maestro skills describe <n> # Show skill description"
	echo "  maestro skills recommend <t># Suggest skills for a task"
	echo "  maestro skills categories   # List all categories"
	echo ""
	echo "Installation:"
	echo "  brew install Aditya060806/tap/maestro && maestro update  # macOS/Homebrew"
	echo "  npm install -g maestro && maestro update                # Linux/cross-platform"
	echo ""
	echo "Documentation: https://github.com/Aditya060806/Maestro"
	return 0
}
...
cmd_help() {
	local version
	version=$(get_version)
	echo "Maestro Framework CLI v$version"
	echo ""
	echo "Usage: maestro <command> [options]"
	echo ""
	_help_commands
	echo ""
	echo "Examples:"
	echo "  maestro init                # Initialize with all features"
	echo "  maestro init planning       # Initialize with planning only"
	echo "  maestro upgrade-planning    # Upgrade planning files to latest"
	echo "  maestro features            # List available features"
	echo "  maestro status              # Check what's installed"
	echo "  maestro doctor              # Find duplicate/conflicting installs"
	echo "  maestro doctor --fix        # Interactively remove duplicates"
	echo "  maestro update              # Update framework + check projects"
	echo "  maestro repos               # List registered projects"
	echo "  maestro launch-worker 22259 Aditya060806/Maestro --dry-run"
	echo "  maestro repos add           # Register current project"
	echo "  maestro detect              # Find unregistered projects"
	echo "  maestro update-tools        # Check for outdated tools"
	echo "  maestro update-tools -u     # Update all outdated tools"
	echo "  maestro uninstall           # Remove maestro"
	echo ""
	_help_detailed_sections
	return 0
}
...
cmd_version() {
	...
	echo "maestro $current_version"

	if [[ "$remote_version" != "unknown" && "$current_version" != "$remote_version" ]]; then
		echo "Latest: $remote_version (run 'maestro update' to upgrade)"
	fi
}
...
_dispatch_helper() {
	...
	if [[ -f "$source_hp" && -f "$INSTALL_DIR/.agents/scripts/maestro-cli/maestro-repos-lib.sh" ]]; then
		hp="$source_hp"
	fi
	[[ ! -f "$hp" ]] && hp="$source_hp"
	if [[ -f "$hp" ]]; then
		...
		print_error "$error_name not found. Run: maestro update"
		exit 1
	fi
	return 0
}
...
_dispatch_config() {
	...
	if [[ -f "$ch" ]]; then
		...
		print_error "config-helper.sh not found. Run: maestro update"
		exit 1
	fi
	return 0
}
...
_launch_worker_usage() {
	cat <<'EOF'
Usage: maestro launch-worker <issue|issue,issue> [owner/repo] [options]
       maestro launch-worker --batch <issue,issue> [owner/repo] [options]
       maestro launch-worker status <issue> [owner/repo]
...
_launch_worker_helper_path() {
	...
	if [[ ! -f "$helper_path" ]]; then
		print_error "dispatch-single-issue-helper.sh not found. Run: maestro update"
		return 1
	fi
	printf '%s\n' "$helper_path"
	return 0
...
cmd_launch_worker() {
	...
	for issue in "${issues[@]}"; do
		...
		echo "Status: maestro launch-worker status $issue $repo_slug"
	done
	return "$rc"
}

# Emit tip if the current directory has maestro but isn't registered
_main_check_unregistered() {
	...
	if [[ -n "$unregistered" && "$command" != "detect" && "$command" != "repos" ]]; then
		echo -e "${YELLOW}[TIP]${NC} This project uses maestro but isn't registered. Run: maestro repos add"
		echo ""
	fi
	return 0
}
...
_main_check_version() {
	...
	if [[ "$agents_version" == "not installed" ]]; then
		echo -e "${YELLOW}[WARN]${NC} Agents not installed. Run: maestro update"
		echo ""
	elif [[ "$cli_version" != "$agents_version" ]]; then
		echo -e "${YELLOW}[WARN]${NC} Version mismatch - CLI: $cli_version, Agents: $agents_version"
		echo -e "       Run: maestro update"
		echo ""
	fi
	return 0
}

# Route 'maestro security [subcommand]' to appropriate helpers
_cmd_security() {
	case "${1:-}" in
	"")
...
# Route 'maestro email [subcommand]' to email helpers
_cmd_email() {
	local sub="${1:-help}"
	...
	mailbox)
		...
		*)
			echo "Usage: maestro email mailbox <add|list|test|remove>"
			echo ""
	...
	*)
		echo "Usage: maestro email <mailbox|poll|thread|build|filter> [subcommand]"
		echo ""
...
# Route 'maestro client-format [subcommand]' to appropriate helpers
_cmd_client_format() {
	case "${1:-status}" in
	extract | refresh)
	...
	status | "")
		...
		echo ""
		if [[ -f "$HOME/.maestro/cch-constants.json" ]]; then
			echo "Cached constants:"
			cat "$HOME/.maestro/cch-constants.json"
		else
			echo "No cached constants. Run: maestro client-format extract"
		fi
		;;
	*)
		print_error "Unknown subcommand: $1"
		echo "Usage: maestro client-format [extract|check|canary|monitor|install-canary|status]"
		exit 1
		;;
	esac
	return 0
...
main() {
	...
	badges)
		...
		# Bare 'maestro badges' with no subcommand shows a usage summary.
		# Subcommands:
		...
		render)
			...
			_render_helper=$(bash -c '
				d="$HOME/.maestro/agents/scripts/readme-badges-helper.sh"
			...
			if [[ -f "$_render_helper" ]]; then
				...
				print_error "readme-badges-helper.sh not found. Run: maestro update"
				exit 1
			fi
			;;
		check)
		...
		help | --help | -h | "")
			echo ""
			echo "maestro badges — README badge block and repo metrics workflow management (t2975)"
			echo ""
			echo "Subcommands:"
			...
			echo ""
			echo "Examples:"
			echo "  maestro badges check                       # scan all repos for badge drift"
			echo "  maestro badges check --json | jq '.[]'    # machine-readable output"
			echo "  maestro badges render owner/repo           # print badge block"
			echo "  maestro badges sync                        # dry-run sync across all repos"
			echo "  maestro badges sync --repo owner/r --apply # apply to a single repo"
			echo "  maestro metrics generate [PATH]            # generate local docs/metrics artifacts"
			echo ""
			;;
		*)
	...
	cleanup)
		...
		branches | remote-branches)
			...
			;;
		help | --help | -h | "")
			echo "Usage: maestro cleanup <branches|remote-branches> [options]"
			echo ""
	...
	contributions | contrib)
		# Bare `maestro contributions` defaults to status (most common use).
		...
		;;
	inbox)
		# Bare `maestro inbox` defaults to status (most common use).
		[[ $# -eq 0 ]] && set -- status
		_dispatch_helper "inbox-helper.sh" "inbox-helper.sh" "$@"
		;;
	case | cases)
		# Bare `maestro case` defaults to list (most common use).
		[[ $# -eq 0 ]] && set -- list
