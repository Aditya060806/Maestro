...
Sources, inspirations, and third-party resources incorporated into or referenced by the Maestro framework.
...
- **imbue-ai/mngr** (MIT License) -- Unix-style tool for managing coding agents at scale. Agents as tmux sessions on SSH-accessible hosts with pluggable providers (Docker, Modal, SSH, local), idle detection, and convention-based state discovery.
  ...
  - Relationship: mngr manages agent *processes*; maestro manages agent *intelligence and workflow*. Complementary, not competing.
...
When ingesting external resources, tools, or design systems into maestro, add a credit entry here with:
